...
static void cmd_process(char cmd[], unsigned long  para[], int para_num)
{
	if((para_num == 2)&&(strcmp(cmd, "IOR")==0)) {
		CCCI_UTIL_DBG_MSG("Command:%s, Base addr:0x%LX\n", cmd, (unsigned long long)para[0]);
		// Do IO-REMAP here
		if(aat_runtime_map_base_vir == NULL) {
			...
			if(aat_runtime_map_base_vir <=0 ) {
				snprintf(aat_err_str, 64, "Map phy addr:0x%016lX fail\n", para[0]);
				aat_err_no = 0x00002100;
			} else {
				snprintf(aat_err_str, 64, "Map phy addr:0x%016lX OK\n", para[0]);
				aat_err_no = 0x00001000;
				aat_runtime_map_base_phy = (phys_addr_t)para[0];
			}
		} else {
			snprintf(aat_err_str, 64, "Please Un-map phy addr:0x%016lX first\n", aat_runtime_map_base_phy);
			aat_err_no = 0x00002100;
		}
		aat_show_case = 0; //Error info
	} else if((para_num == 4)&&(strcmp(cmd, "WR")==0)) {
		CCCI_UTIL_DBG_MSG("Command:%s, Offset:0x%X, value:0x%X, type:0x%X\n", cmd, (unsigned int)para[0], (unsigned int)para[1], (unsigned int)para[2]);
		...
		if(aat_runtime_map_base_vir != NULL) {
			if(para[0] >= 4096L) {
				...
				snprintf(aat_err_str, 64, "Write phy addr:0x%016lX OK\n", ((unsigned long)aat_runtime_map_base_phy+para[0]));
				aat_err_no = 0x00001000;
			}
		} else {
		...
		if(aat_runtime_map_base_vir != NULL) {
			if(para[0] >= 4096L) {
				...
				snprintf(aat_err_str, 64, "Query offset:0x%016X size:0x%lX OK\n", (unsigned int)para[0], para[1]);
		...
		if(aat_runtime_map_base_vir != NULL) {
			...
			snprintf(aat_err_str, 64, "IO Un-map phy addr:0x%016lX OK\n", (unsigned long)aat_runtime_map_base_phy);
			aat_err_no = 0x00001000;
		} else {
