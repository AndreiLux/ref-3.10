...
static KAL_INT32 mdlog_read_cnt = 0;
...
static long eemcs_cdev_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        case CCCI_IOC_GET_MD_STATE:
        {
            ...
            DBGLOG(CHAR, INF, "CCCI_IOC_GET_MD_STATE(md_s=%d, eemcs_s=%d) by %s(%d)", md_state, eemcs_state, \
...
static ssize_t eemcs_cdev_read(struct file *fp, char *buf, size_t count, loff_t *ppos)
{
    ...
    else    
    {
        ...
        if (mdlog_read_cnt > 1)
            DBGLOG(CHAR,ERR, "eemcs_cdev_read reentered by mdlog.\n");
