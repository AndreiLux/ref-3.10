    ...
    if(NULL != skb){
    	...
    	if ((pccci_h->channel == CH_FS_TX) && (pccci_h->reserved > 0x4)) {
            ...
            DBGLOG(CCCI, ERR, "[TX]FS: 0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x, 0x%08x",\
            	pdata, (pdata+1), (pdata+2), (pdata+3), (pdata+4), (pdata+5), (pdata+6), (pdata+7), (pdata+8));
    	}
    } else {
...
int ccci_df_to_ccci_send_msg(CCCI_CHANNEL_T ch, unsigned int msg, unsigned int reserv){
    ...
    if (check_device_state() != EEMCS_BOOTING_DONE) {//modem not ready
        DBGLOG(CCCI, ERR, "CH%d send ccci msg(%d) fail when modem not ready", msg);
        return -ENODEV;
    }
