...
static struct platform_device uibc_dev = {
    .name =  UIBC_KBD_NAME,
    .id   = -1,
};

static int __init uibc_keyboard_init(void) {
    int ret;

    ret = platform_device_register (&uibc_dev);
    if (ret < 0 ){
        xlog_printk(ANDROID_LOG_INFO, UIBC_TAG,
                          "uibc_keyboard_init  platform_device_register failed\n");
        return ret;
    }

    ret = platform_driver_register(&uibc_keyboard_driver);
    if (ret) {
        xlog_printk(ANDROID_LOG_INFO, UIBC_TAG,
                          "uibc_keyboard_init  platform_driver_register failed\n");
        return ret;
    }
