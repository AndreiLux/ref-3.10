...
int mtk_xhci_eint_iddig_init(void)
{
	...
	retval =
	    request_irq(mtk_idpin_irqnum, xhci_eint_iddig_isr, IRQF_TRIGGER_LOW, "iddig_eint",
			NULL);
	if (retval != 0) {
		mtk_xhci_mtk_log("%s(%d): request_irq fail, ret %d, irqnum %d!!!\n", __func__,
...
void mtk_xhci_ck_timer_init(void)
{
	...
	addr = (__u32 __iomem *)SSUSB_SYS_CK_CTRL;
	temp = readl(addr);
	temp |= SSUSB_SYS_CK_DIV2_EN;
	writel(temp, addr);
	mtk_xhci_mtk_log("mu3d sys_clk, addr 0x%x, value %d\n", SSUSB_SYS_CK_CTRL, readl((__u32 __iomem *)SSUSB_SYS_CK_CTRL));
	...
	/* set U2 MAC SYS_CK */
	mtk_xhci_mtk_log("mu3d u2 mac sys_clk, addr 0x%x, value 0x%x\n",
						SSUSB_U2_SYS_BASE + USB20_TIMING_PARAMETER,
						readl((__u32 __iomem *)(SSUSB_U2_SYS_BASE + USB20_TIMING_PARAMETER)));
