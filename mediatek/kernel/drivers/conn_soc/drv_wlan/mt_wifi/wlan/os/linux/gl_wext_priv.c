    ...
    )
{
    ...
    nsize1 = kalSnprintf(text1, TMP_TEXT_LEN_S, "scancount=%d\nnextcount=%d\n", 
        (INT32)scancount, (INT32)scancount);
    if (nsize1 < nleft) {
        p += nsize1 = kalSnprintf(p, TMP_TEXT_LEN_S, "%s", text1);
        nleft -= nsize1;
    } else
        goto short_buf;
    ...
    for (j=0; j<CFG_BATCH_MAX_MSCAN; j++) {
        ...
        for (i=0; i<pBr->ucScanCount; i++)
        {
            ...
            if (prEntry->ucSSIDLen > ELEM_MAX_LEN_SSID)
            ...
            nsize3 = kalSnprintf(text3, TMP_TEXT_LEN_L, "freq=%d\nlevel=%d\ndist=%d\ndistSd=%d\n====\n", 
                (INT32)freq,
                (INT32)prEntry->cRssi,
                (INT32)prEntry->u4Dist,
                (INT32)prEntry->u4Distsd);
        ...
        p += kalSnprintf(p, TMP_TEXT_LEN_S, "%s", text1);

        pBr++;
    }

    nsize1 = kalSnprintf(text1, TMP_TEXT_LEN_S, "%s", "----\n");
    kalSnprintf(p, TMP_TEXT_LEN_S, "%s", text1);

    *pu4RetLen = u4MaxBufferLen - nleft;
    DBGLOG(SCN, TRACE, ("total len = %d (max len = %d)\n", (INT32)*pu4RetLen, (INT32)u4MaxBufferLen));
    
    return WLAN_STATUS_SUCCESS;

short_buf:
    DBGLOG(SCN, TRACE, ("Short buffer issue! %d > %d, %s\n", (INT32)(u4MaxBufferLen+(nsize-nleft)), (INT32)u4MaxBufferLen, (PCHAR)pvBuffer));
