	...
	)
{
	...
	{
		DBGLOG(TDLS, ERROR, ("%s kalIoctl fail:%lx\n", __FUNCTION__, rStatus));
		return;
	}
}
	...
	)
{
	...
	*/
	...
	DBGLOG(TDLS, INFO, ("<tdls_cfg> %s: ["MACSTR"] %d %d %d 0x%p %d\n",
		__FUNCTION__, MAC2STR(peer),
		action_code, dialog_token, status_code, buf, len));
	...
	*/
	...
	{
		DBGLOG(TDLS, ERROR,
			("%s enable or disable link fail:%lx\n", __FUNCTION__, rStatus));
		kalMemFree(prMgmtTxInfo, VIR_MEM_TYPE, sizeof(TDLS_MGMT_TX_INFO));
		return -EINVAL;
	}
