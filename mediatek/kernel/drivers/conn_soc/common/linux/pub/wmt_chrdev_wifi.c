...
int wifi_reset_start(void)
{
    ...
    /* No need to turn off p2p/ap mode here, since the operation is skipped due to fgIsResetting UP.
     * p2pNetUnregister and p2pRemove will be performed in wlanRemove.
     */
#if 0
    if (powered == 1) {
    ...
    if (0 == copy_from_user(local, buf, (count > sizeof(local)) ? sizeof(local) : count)) {
        ...
        if (local[0] == '0') {
            ...
            if (powered == 1) {
                ...
                /* workarount for wifi turn off fail. otherwise, 
                   will KE in register_netdevice with wrong dev->state */
                WIFI_ERR_FUNC("turn off wifi fail last time, can't turn on again!\n");
                goto done;
            }
            ...
            if (local[0] == 'S' || local[0] == 'P'){
                if (wlan_mode == WLAN_MODE_STA_P2P) {
                    WIFI_ERR_FUNC("WIFI write: already in WLAN_MODE_STA_P2P mode\n");
                    retval = count;
                    goto done;
                }
                p2pmode.u4Enable = 1;
                ...
                if (wlan_mode == WLAN_MODE_AP) {
                    WIFI_ERR_FUNC("WIFI write: already in WLAN_MODE_AP mode\n");
                    retval = count;
                    goto done;
                }
                p2pmode.u4Enable = 1;
