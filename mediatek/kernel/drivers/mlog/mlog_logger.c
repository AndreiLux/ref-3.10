...
static void mlog_procinfo(void)
{
    ...
    for_each_process(tsk) {
		...
		//printk("[mlog] p->pid: %d, p->signal->oom_score_adj:%d, oom_score_adj: %d\n", 
			//p->pid, p->signal->oom_score_adj, oom_score_adj);
#else
		oom_score_adj = p->signal->oom_adj;
		//printk("[mlog] p->pid: %d, p->signal->oom_adj: %d\n", 
	...
	/*
	ALPS01527750:
	cred is invalid, when put_cred. Possible reason:
	1. cred is changed between get_task_cred and put_cred.(stack corruption?)
	2. cred is already freed by others when mlog tried to put_cred.
	*/
	printk("cred: 0x%x\n", cred);
#endif
