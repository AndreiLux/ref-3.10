...
static void port_kernel_md_state_notice(struct ccci_port *port, MD_STATE state)
{
	...
	switch(state) {
	case RESET:
		spin_lock_irqsave(&port->modem->ctrl_lock, flags);
		if(port->modem->ee_info_flag & MD_EE_OK_MSG_GET)
			dump_again = 1;
		spin_unlock_irqrestore(&port->modem->ctrl_lock, flags);
		if(dump_again) // in case we reset MD before receving MD_EX_PASS or before its timer timeout
			mod_timer(&port->modem->ex_monitor2, jiffies);
