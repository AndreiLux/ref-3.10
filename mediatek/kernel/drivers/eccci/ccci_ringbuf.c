...
#define CCIF_RBF_READ(bufaddr, output_addr, read_size,read_pos,buflen)  \
    do {    \
        if(read_pos + read_size < buflen){ \
            memcpy((unsigned char *)output_addr , (unsigned char *)(bufaddr) + read_pos , read_size); \
        }else{\
            memcpy((unsigned char *)output_addr , (unsigned char *)(bufaddr) + read_pos , buflen - read_pos ); \
            output_addr = (unsigned char *)output_addr + buflen- read_pos; \
            memcpy((unsigned char *)output_addr , (unsigned char *)(bufaddr) , read_size - (buflen - read_pos)); \
        }\
    } while (0)
#define CCIF_RBF_WRITE(bufaddr, data_addr, data_size,write_pos,buflen)  \
            do {    \
                if(write_pos + data_size < buflen){ \
                    memcpy((unsigned char *)(bufaddr) + write_pos, (unsigned char *)data_addr, data_size); \
                }else{\
                    memcpy((unsigned char *)(bufaddr) + write_pos, (unsigned char *)data_addr ,  buflen - write_pos); \
                    data_addr = (unsigned char *)data_addr + buflen- write_pos; \
                    memcpy((unsigned char *)(bufaddr), (unsigned char *)data_addr ,  data_size - (buflen - write_pos)); \
                }\
            } while (0)
...
int ccci_ringbuf_write(int md_id, struct ccci_ringbuf *ringbuf, unsigned char *data, int data_len)
{
    ...
    //CCCI_INF_MSG(-1, TAG, "rbw:header 32 byte\n");
    //ccci_ringbuf_dump("write",tx_buffer,ringbuf->tx_control.write+44,length,16);
    // CCCI_INF_MSG(-1, TAG, "rbw:footer 16 byte\n");
    // ccci_ringbuf_dump(tx_buffer,ringbuf->tx_control.write+CCIF_HEADER_LEN+data_len,length,16);
    // Update read pointer    
...
int ccci_ringbuf_read(int md_id, struct ccci_ringbuf * ringbuf, unsigned char *buf, int read_size)
{
    ...
    //ccci_ringbuf_dump("read",ringbuf->buffer,read+44,length,16);
    return read_size;
}
