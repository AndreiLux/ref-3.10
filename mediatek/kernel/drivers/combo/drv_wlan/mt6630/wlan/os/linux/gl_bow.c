...
glRegisterAmpc (VOID)
{
    ...
    // 1. Get char device major number
#if defined (CONFIG_AMPC_CDEV_NUM)
    prBowInfo->u4DeviceNumber = MKDEV(CONFIG_AMPC_CDEV_NUM, 0);
#else
    prBowInfo->u4DeviceNumber = MKDEV(226, 0);
#endif

    if(register_chrdev_region(prBowInfo->u4DeviceNumber, 1, GLUE_BOW_DEVICE_NAME) != 0) {
        return FALSE;
    }
    ...
    // 3. initialize kfifo
    if((kfifo_alloc((struct kfifo *) &(prBowInfo->rKfifo), GLUE_BOW_KFIFO_DEPTH, GFP_KERNEL)))
        goto fail_kfifo_alloc;
             
    if(&(prBowInfo->rKfifo) == NULL)
        goto fail_kfifo_alloc;
        
    // 4. initialize cdev
    cdev_init(&(prBowInfo->cdev), &bow_ampc_fops);
    prBowInfo->cdev.ops = &bow_ampc_fops;

    // 5. add character device
    if(cdev_add(&(prBowInfo->cdev), prBowInfo->u4DeviceNumber, 1))
        goto fail_cdev_add;

    // 6. in queue initialization
    init_waitqueue_head(&(prBowInfo->outq));

    // 7. finish
    prBowInfo->fgIsRegistered = FALSE;

    return TRUE;

fail_cdev_add:
    kfifo_free(&(prBowInfo->rKfifo));

fail_kfifo_alloc:
    unregister_chrdev_region(prBowInfo->u4DeviceNumber, 1);
    ...
    IN OUT loff_t *ppos)
{
    ...
    if (prBowInfo->fgIsRegistered == FALSE) {
        return -EFAULT;
    }
    ...
    // size check
    if(kfifo_len(&(prBowInfo->rKfifo)) >= size)
        retval = size;
    else
        retval = kfifo_len(&(prBowInfo->rKfifo));

    if (!(kfifo_out(&(prBowInfo->rKfifo), aucBuffer, retval)))
        retval = -EIO;
    ...
    IN OUT loff_t *ppos)
{
    ...
    prGlueInfo = prBowInfo->prGlueInfo;
    ASSERT(prGlueInfo);
    ...
    DBGLOG(BOW, INFO, ("AMP driver CMD buffer size : %d.\n", size));

    for(i = 0; i < MAX_BUFFER_SIZE; i++)
    {
        DBGLOG(BOW, INFO, ("AMP write content : 0x%x.\n", aucBuffer[i]));
    }

    DBGLOG(BOW, INFO, ("BoW CMD write.\n"));

    prCmd = (P_AMPC_COMMAND) aucBuffer;

    DBGLOG(BOW, INFO, ("AMP write content payload length : %d.\n", prCmd->rHeader.u2PayloadLength));

    DBGLOG(BOW, INFO, ("AMP write content header length : %d.\n", sizeof(AMPC_COMMAND_HEADER_T)));

    // size check
    if(prCmd->rHeader.u2PayloadLength + sizeof(AMPC_COMMAND_HEADER_T) != size)
    {
        DBGLOG(BOW, INFO, ("Wrong CMD total length.\n"));
    ...
    )
{
    ...
    if (prGlueInfo->prBowInfo->fgIsRegistered == FALSE) {
        return;
    }
...
/*    u4AvailSize =
    ...
    u4AvailSize =
        GLUE_BOW_KFIFO_DEPTH - kfifo_len(&(prGlueInfo->prBowInfo->rKfifo));
    ...
    if(u4AvailSize < u4EventSize) {
        DBGLOG(BOW, INFO, ("[bow] no space for event: %d/%d\n",
                u4EventSize,
                u4AvailSize));
        return;
    }
...
//    kfifo_in(prGlueInfo->rBowInfo.prKfifo, (PUINT_8)prEvent, u4EventSize);
    kfifo_in(&(prGlueInfo->prBowInfo->rKfifo), (PUINT_8)prEvent, u4EventSize);
    wake_up_interruptible(&(prGlueInfo->prBowInfo->outq));
    ...
    )
{
    ...
    DBGLOG(BOW, INFO, ("kalGetBowState.\n"));

    for(i = 0 ; i < CFG_BOW_PHYSICAL_LINK_NUM ; i++)
    {
        if(EQUAL_MAC_ADDR(prGlueInfo->prBowInfo->arPeerAddr, aucPeerAddress) == 0)
        {
    DBGLOG(BOW, INFO, ("kalGetBowState, aucPeerAddress %x, %x:%x:%x:%x:%x:%x.\n", i,
        aucPeerAddress[0],
        aucPeerAddress[1],
        aucPeerAddress[2],
        aucPeerAddress[3],
    ...
    DBGLOG(BOW, INFO, ("kalGetBowState, prGlueInfo->prBowInfo->aeState %x, %x.\n", i, prGlueInfo->prBowInfo->aeState[i]));


            return prGlueInfo->prBowInfo->aeState[i];
        }
    }
    ...
    )
{
    ...
    DBGLOG(BOW, INFO, ("kalSetBowState.\n"));

    DBGLOG(BOW, INFO, ("kalSetBowState, prGlueInfo->prBowInfo->arPeerAddr, "MACSTR".\n",
        MAC2STR(prGlueInfo->prBowInfo->arPeerAddr[0])));

    DBGLOG(BOW, INFO, ("kalSetBowState, aucPeerAddress, %x:%x:%x:%x:%x:%x.\n",
        aucPeerAddress[0],
        aucPeerAddress[1],
        aucPeerAddress[2],
        aucPeerAddress[3],
    ...
    for(i = 0 ; i < CFG_BOW_PHYSICAL_LINK_NUM ; i++)
    {
        if(EQUAL_MAC_ADDR(prGlueInfo->prBowInfo->arPeerAddr, aucPeerAddress) == 0)
        {
            prGlueInfo->prBowInfo->aeState[i] = eBowState;

    DBGLOG(BOW, INFO, ("kalSetBowState, aucPeerAddress %x, %x:%x:%x:%x:%x:%x.\n", i,
        aucPeerAddress[0],
        aucPeerAddress[1],
        aucPeerAddress[2],
        aucPeerAddress[3],
    ...
    DBGLOG(BOW, INFO, ("kalSetBowState, prGlueInfo->prBowInfo->aeState %x, %x.\n", i, prGlueInfo->prBowInfo->aeState[i]));
...
//Henry, can reduce this logic to indentify state change
    ...
    for(i = 0 ; i < CFG_BOW_PHYSICAL_LINK_NUM ; i++) {
        if(prGlueInfo->prBowInfo->aeState[i] == BOW_DEVICE_STATE_CONNECTED) {
            return BOW_DEVICE_STATE_CONNECTED;
        }
    }

    for(i = 0 ; i < CFG_BOW_PHYSICAL_LINK_NUM ; i++) {
        if(prGlueInfo->prBowInfo->aeState[i] == BOW_DEVICE_STATE_STARTING) {
    ...
    )
{
    ...
    return prGlueInfo->prBowInfo->u4FreqInKHz;
}
    ...
    )
{
    ...
    for(i = 0 ; i < CFG_BOW_PHYSICAL_LINK_NUM ; i++) {
        if(EQUAL_MAC_ADDR(prGlueInfo->prBowInfo->arPeerAddr[i], rPeerAddr) == 0) {
            return prGlueInfo->prBowInfo->aucRole[i];
        }
    }
    ...
    )
{
    ...
    for(i = 0 ; i < CFG_BOW_PHYSICAL_LINK_NUM ; i++) {
        if(EQUAL_MAC_ADDR(prGlueInfo->prBowInfo->arPeerAddr[i], rPeerAddr) == 0) {
            prGlueInfo->prBowInfo->aucRole[i] = ucRole; //Henry, 0 : Responder, 1 : Initiator
        }
    }
}
    ...
    )
{
    ...
    for(i = 0 ; i < CFG_BOW_PHYSICAL_LINK_NUM ; i++) {
        if(prGlueInfo->prBowInfo->aeState[i] == BOW_DEVICE_STATE_DISCONNECTED) {
            ucLinkCount++;
        }
    }
    ...
    )
{
    ...
    ASSERT(prGlueInfo);
    ASSERT(prGlueInfo->prBowInfo->fgIsRegistered == TRUE);
    ...
    if(prGlueInfo->prBowInfo->fgIsNetRegistered == FALSE) {
        prGlueInfo->prBowInfo->prDevHandler = alloc_netdev_mq(sizeof(P_GLUE_INFO_T), prDevName, ether_setup, CFG_MAX_TXQ_NUM);

        if (!prGlueInfo->prBowInfo->prDevHandler) {
            return FALSE;
        }
        else {
            /* 1. setup netdev */
            ...
            prNetDevPriv = (P_NETDEV_PRIVATE_GLUE_INFO)netdev_priv(prGlueInfo->prBowInfo->prDevHandler);
            ...
            memcpy(prGlueInfo->prBowInfo->prDevHandler->dev_addr, rMacAddr, ETH_ALEN);
            memcpy(prGlueInfo->prBowInfo->prDevHandler->perm_addr, prGlueInfo->prBowInfo->prDevHandler->dev_addr, ETH_ALEN);

            /* 1.3 register callback functions */
            prGlueInfo->prBowInfo->prDevHandler->netdev_ops = &bow_netdev_ops;

#if (MTK_WCN_HIF_SDIO == 0)
            SET_NETDEV_DEV(prGlueInfo->prBowInfo->prDevHandler, &(prHif->func->dev));
#endif

            register_netdev(prGlueInfo->prBowInfo->prDevHandler);

            /* 2. net device initialize */
            netif_carrier_off(prGlueInfo->prBowInfo->prDevHandler);
            netif_tx_stop_all_queues(prGlueInfo->prBowInfo->prDevHandler);

            /* 2.1 bind NetDev pointer to NetDev index */
            wlanBindBssIdxToNetInterface(prGlueInfo, bowInit(prAdapter), (PVOID)prGlueInfo->prBowInfo->prDevHandler);
            ...
            /* 3. finish */
            prGlueInfo->prBowInfo->fgIsNetRegistered = TRUE;
        }
    }
    ...
    )
{
    ...
    if(prGlueInfo->prBowInfo->fgIsNetRegistered == TRUE) {

        prGlueInfo->prBowInfo->fgIsNetRegistered = FALSE;

        bowUninit(prAdapter);

        if(netif_carrier_ok(prGlueInfo->prBowInfo->prDevHandler)) {
            netif_carrier_off(prGlueInfo->prBowInfo->prDevHandler);
        }
        ...
        unregister_netdev(prGlueInfo->prBowInfo->prDevHandler);
        free_netdev(prGlueInfo->prBowInfo->prDevHandler);
        prGlueInfo->prBowInfo->prDevHandler = NULL;
