...
m4u_callback_ret_t mjc_m4u_fault_callback(int port, unsigned int mva, void* data)
{
	...
	printk("Sys Basic: 0x%x, Status: 0x%x, FS00: 0x%x, FS01: 0x%x, RDMA00: 0x%x, RDMA01: 0x%x, RDSTA00: 0x%x\n", 
		   MJC_Reg32(gulRegister+0x4),
		   MJC_Reg32(gulRegister+0x70), MJC_Reg32(gulRegister+0xc00), MJC_Reg32(gulRegister+0xc04),
		   MJC_Reg32(gulRegister+0x100), MJC_Reg32(gulRegister+0x104), MJC_Reg32(gulRegister+0x140));
...
static long mjc_ioctl(struct file *pfile, unsigned int u4cmd, unsigned long u4arg)
{
	...
	switch(u4cmd)
    {
		...
		case MJC_READ_REG:
		{
            ...
            {
                MJCMSG("[ERROR] mjc_ioctl() MJC_READ_REG, addr invalid, addr min=0x%lx, max=0x%lx, addr=0x%x \n",
                    gu1PaReg,
                    (gu1PaReg+gu1PaSize),
                    rReadReg.reg);
                return -1;
            ...
            i4Value = MJC_Reg32(ulAdd) & rReadReg.mask;

            MJCDBG("read 0x%x (0x%lx)= 0x%x (0x%x)\n", rReadReg.reg, ulAdd, i4Value, rReadReg.mask);
		...
		case MJC_WRITE_REG:
		{
            ...
            {
                MJCMSG("[ERROR] mjc_ioctl() MJC_WRITE_REG, addr invalid, addr min=0x%lx, max=0x%lx, addr=0x%x \n",
                    gu1PaReg,
                    (gu1PaReg+gu1PaSize),
                    rWriteReg.reg);
                return -1;
		...
		case MJC_REG_INFO:
		{
			...
			{
			   MJCMSG("[ERROR] mjc_ioctl() MJC_REG_INFO context size mismatch (user:%d, kernel:%d)\n", rRegInfo.u4StructSize, sizeof(MJC_IOCTL_REG_INFO_T));
			   return -1;
			}
...
static int mjc_mmap(struct file *pFile, struct vm_area_struct *pVma)
{
	...
	MJCMSG("start = 0x%lx, pgoff= 0x%x vm_end= 0x%lx, vm_page_prot= 0x%x\n",
	       pVma->vm_start,
	       pVma->vm_pgoff,
	       pVma->vm_end,
