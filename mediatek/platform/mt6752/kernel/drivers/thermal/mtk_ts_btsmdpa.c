...
static int __init mtkts_btsmdpa_init(void)
{
	...
	mtkts_btsmdpa_dir = proc_mkdir("mtktsbtsmdpa", NULL);
	if (!mtkts_btsmdpa_dir)
	{
		mtkts_btsmdpa_dprintk("[mtkts_btsmdpa_init]: mkdir /proc/mtktsbtsmdpa failed\n");
	}
	else
	{

		entry = proc_create("mtktsbtsmdpa", S_IRUGO | S_IWUSR | S_IWGRP, mtkts_btsmdpa_dir, &mtkts_btsmdpa_fops);
		if (entry) {
		...
		entry = proc_create("mtkts_btsmdpa_param", S_IRUGO | S_IWUSR | S_IWGRP, mtkts_btsmdpa_dir, &mtkts_btsmdpa_param_fops);
		if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
            proc_set_user(entry, 0, 1000);
#else // kernel ver
