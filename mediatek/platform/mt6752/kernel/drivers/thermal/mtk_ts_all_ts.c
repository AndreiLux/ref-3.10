...
static int __init tsallts_init(void)
{
    ...
    {
        entry = proc_create("mtkts1",  S_IRUGO | S_IWUSR | S_IWGRP, tsallts_dir, &tsallts_fops1);
        if (entry) {
        ...
        entry = proc_create("mtkts5",  S_IRUGO | S_IWUSR | S_IWGRP, tsallts_dir, &tsallts_fops5);
        if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
            proc_set_user(entry, 0, 1000);
#else
        ...
        entry = proc_create("mtkts3",  S_IRUGO | S_IWUSR | S_IWGRP, tsallts_dir, &tsallts_fops3);
        if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
            proc_set_user(entry, 0, 1000);
#else
        ...
        entry = proc_create("mtkts4",  S_IRUGO | S_IWUSR | S_IWGRP, tsallts_dir, &tsallts_fops4);
		if (entry) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
            proc_set_user(entry, 0, 1000);
#else
