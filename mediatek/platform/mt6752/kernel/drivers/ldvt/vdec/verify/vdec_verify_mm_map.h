...
#ifdef MPEG2_4K2K //set mpeg2 4k2k buffer size & reduce no-need buffer 
#define V_FIFO_SZ 0x4100000 //0x0800000
#define MVBUF_SZ	10*3072*2 + 4096*3072/4
#define PIC_Y_SZ          (4096*3072)*2 //(3840*2176) *2//(2048*1088) *2
#define PIC_C_SZ          (4096*3072/2) *2 //(3840*2176/2) *2//(2048*1088 / 2) *2
#define DEC_PP_SZ         ((10/16)*9*8)*2
#define DEC_PP_Y_SZ      (10*1088) *2
#define DEC_PP_C_SZ      (10*3072/2) *2
#define DCAC_SZ           ((((4096 / 16) * 4) * ((3072 / 16) * 4)) * 4)*2 // (MBx * 4) * (MBy * 4) = 25920
#define DCAC_SZ           ((((4096 / 16) * 8) * 128) / 8)*2 // (MBx * 8 * 128) bits = (MBx * 128) bytes = 15360
#define DATA_PARTITION_SZ   ((2 * (10 / 16) * (3072 / 16) * 128) / 8) // (2 * MBx * MBy * 128) bits = 261120 bytes
#define NOT_CODED_SZ        (((((10 / 16) * (3072 / 16) + 127) >> 7) << 7) / 8)*2 // ((((1 * MBx * MBy) + 127) >> 7) << 7) bits = 1024 bytes
#define MV_DIRECT_SZ        (((10 / 16) * (3072 / 16) * 128) / 8)*2 // (MBx * MBy) * 128 bit = 130560 bytes
#endif
