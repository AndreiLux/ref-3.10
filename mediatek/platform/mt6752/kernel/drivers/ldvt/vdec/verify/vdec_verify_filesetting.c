...
//add flush cache here qiguo 29/4
    ...
    else //check all bitstream golden is done
    {
        #if 1 // 8320
            #if VDEC_PP_ENABLE
                vConcateStr(_bFileStr1[u4InstID][3], _bFileStr1[u4InstID][0], "_y_dbk.out\0", _u4FileCnt[u4InstID]);
    ...
    //break bitstream test if golden compare fail or golden not found
...
else
{
    ...
    msleep(1000);
        ...
        {
            sprintf(strMessage,"%s Compare Error ==> Pic count to [%d] \n", _u4FileCnt[u4InstID] - 1);
            fgWrMsg2PC(strMessage,strlen(strMessage),8,&_tFileListRecInfo[u4InstID]);
...
#ifdef VDEC_SIM_DUMP
#define _FANTASIA_WRITE_OUT_DATA_ 1
#endif
...
int vH265CRCComparison(UINT32 u4InstID, UINT32* u4CRC){
    ...
    // Load Y CRC file
    if (_ptH265CurrFBufInfo[u4InstID]->bIsUFOEncoded){
        sprintf(file_name, "%scrc_ufo_single_Y.dat", bitstream_name, _u4PicCnt[u4InstID]);
    }else{
        sprintf(file_name, "%scrc_ufo_bypass_single_Y.dat", bitstream_name, _u4PicCnt[u4InstID]);
    }
    ...
    // Load C CRC file
    if (_ptH265CurrFBufInfo[u4InstID]->bIsUFOEncoded){
        sprintf(file_name, "%scrc_ufo_single_C.dat", bitstream_name, _u4PicCnt[u4InstID]);
    }else{
        sprintf(file_name, "%scrc_ufo_bypass_single_C.dat", bitstream_name, _u4PicCnt[u4InstID]);
    }
        ...
        for(u4Idx=0; u4Idx< _u4GoldenYSize[u4InstID]>>2; u4Idx++)
        {
            ...
            //if (1) // Cheng-Jung 20120904 Uncomment to dump error frame
            {
                ...
                //#endif        
					  ...
					  #else //kevin dump prevent sim register dump
					  printk("[H264] [k] dump\n");
                      fgWrData2PC((void*)pu4WorkBuf,_u4GoldenYSize[u4InstID],7,_output_file_name);
                      #endif
                      //CbCr
...
#if 1 //kevin test yuv dump
#define OUT_TO_FILE

void vH264DumpYUV( UINT32 u4InstID, UINT32  PP_OUT_Y_ADDR , UINT32  PP_OUT_C_ADDR, UINT32 PIC_SIZE_Y ){
    //transform 16*32 block mode to yuv video
    ...
    width = _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.prCurrFBufInfo->u4W;
    height = _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.prCurrFBufInfo->u4H;
	...
	sprintf( file_name, "d:\\CWKevin\\YUV\\%s_er%d_rec.yuv", bitstream_name, 0 );
    #else
	sprintf( file_name, "d:\\CWKevin\\YUV\\er%d_rec.y", 1 );
	sprintf( file_name1, "d:\\CWKevin\\YUV\\er%d_rec.uv", 1 );
	sprintf( file_nameu, "d:\\CWKevin\\YUV\\er%d_rec.u", 1 );
	sprintf( file_namev, "d:\\CWKevin\\YUV\\er%d_rec.v", 1 );
    #endif
    ...
    printk("\n[k] File Name:%s 0x%08X\n",file_name, EC_rec_filp);
	

    u4YLength = width*( (height+31)>>5 ) <<5;
	
	printk("[k]  u4YLength = %d, w = %d, h = %d \n",u4YLength,width,height);

    Y_out = (char*) vmalloc(sizeof(char)*u4YLength);
	#ifndef OUT_TO_FILE
    U_out = (char*) vmalloc(sizeof(char)*u4YLength/4);
    #else
	U_out = (char*) vmalloc(sizeof(char)*u4YLength/2);
	#endif
    V_out = (char*) vmalloc(sizeof(char)*u4YLength/4);
    ...
    #ifdef OUT_TO_FILE
	memcpy(U_out+sizeof(char)*u4YLength/4,U_out,sizeof(char)*u4YLength/4);
	fgWrData2PC((void*)Y_out,u4YLength,7,file_name);
	fgWrData2PC((void*)U_out,u4YLength/2,7,file_name1);
	fgWrData2PC((void*)U_out,u4YLength/4,7,file_nameu);
	fgWrData2PC((void*)V_out,u4YLength/4,7,file_namev);
    #endif
