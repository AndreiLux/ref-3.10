  ...
  while(fgVdecReadFileName(u4InstID, &_tFileListInfo[u4InstID], &_tFileListRecInfo[u4InstID], &_u4StartCompPicNum[u4InstID], &_u4EndCompPicNum[u4InstID], &_u4DumpRegPicNum[u4InstID]))
  {
    ...
    if (fgInit == TRUE)
    {
     ...
     _tInFileInfo[u4InstID].u4FileOffset = 0;
      ...
      #if VDEC_SIM_DUMP //kevin dump bitfream from memory
	  if (fgOpen)
	  {
	      char file_name[200]; 
		  sprintf( file_name, "d:\\CWKevin\\YUV\\er%d_rec.h264",1);
		  printk("[k] dump bitstream from memory\n");
		  printk("[k] %s\n",file_name);		  
		  printk("[k] sizd : %d\n",_tInFileInfo[u4InstID].u4RealGetBytes);
		  fgWrData2PC((void*)_tInFileInfo[u4InstID].pucTargetAddr,_tInFileInfo[u4InstID].u4RealGetBytes,7,file_name);
	  }
	  #endif
