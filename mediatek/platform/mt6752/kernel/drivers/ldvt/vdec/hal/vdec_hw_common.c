...
if (u4VDecType==VDEC_H265){
    ...
    vVDecWriteHEVCMISC(u4VDecID, 0x4, 0x10);    //K2 turn on CRC access
} else {
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
        ...
        if (DUMP_FOR_SIM)
        {
            ...
            #endif
            {
				...
				case AVC_VLD_REG_OFFSET0:
                    printk("        RISCWrite_AVC_VLD(%d, 32'h%x); /* base 0x%x , 0x%x */\n", u4Addr>>2, u4Val, u4BaseReg+VDEC_PHY_BASE , u4Addr);
                    break;
                default:
            ...
            else
            {
                ...
                case HEVC_VLD_REG_OFFSET0:
                     ...
                     break;
                case AVC_VLD_REG_OFFSET0:
                     _u4RegisterLogLen[u4VDecID] += sprintf(_pucRegisterLog[u4VDecID]+_u4RegisterLogLen[u4VDecID], "        RISCWrite_AVC_VLD(%d, 32'h%x); /* base 0x%x , 0x%x */\n", u4Addr>>2, u4Val, u4BaseReg+VDEC_PHY_BASE , u4Addr);
                    break;
                default:
        ...
        if (DUMP_FOR_SIM)
        {
            ...
            else
            {
                ...
                case HEVC_VLD_REG_OFFSET0:
                    ...
                    break;
                case AVC_VLD_REG_OFFSET0:
                    _u4RegisterLogLen[u4VDecID] += sprintf(_pucRegisterLog[u4VDecID]+_u4RegisterLogLen[u4VDecID], "        RISCRead_AVC_VLD(%d); /* base 0x%x, return 0x%x , 0x%x */\n", u4Addr>>2,u4BaseReg+VDEC_PHY_BASE ,u4Val,u4Addr);
                    break;
                default:
