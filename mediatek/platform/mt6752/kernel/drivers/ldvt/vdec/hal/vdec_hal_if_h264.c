...
INT32 i4VDEC_HAL_H264_DecStart(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T *prDecPrm)
{
	...
	vVDecWriteMISC(u4VDecID,0x04,0x03);

	// Fld & Frame height same?
	{
        vVDecWriteVLDTOP(u4VDecID, 0x70, ((prDecPrm->u4PicH << 16)| prDecPrm->u4PicW));
        printk("^^^^^^AVC_VLD_TOP_28 : 0x%08x^^^^^^\n\n", u4VDecReadVLDTOP(u4VDecID, 0x70));
	}
#if (!CONFIG_DRV_VERIFY_SUPPORT)
...
{
	UINT32 u4Value,u4Cnt;
#ifdef VDEC_SIM_DUMP //kevin 
	_fgRegLogConsole[u4VDecID] = TRUE; //to prevent out to file
#endif
	printk("[k] Dump DE Specify Register +\n");
#if 0
	for(u4Cnt = 33 ; u4Cnt <=41 ;u4Cnt++)
	{
		u4Value = u4VDecReadAVCVLD(u4VDecID,u4Cnt*4);
		printk("AVC_VLD(%d) 0x%x\n",u4Cnt,u4Value);
	}
    printk("\n");

	for(u4Cnt = 146 ; u4Cnt <=187 ;u4Cnt++)
	{
		u4Value = u4VDecReadAVCVLD(u4VDecID,u4Cnt*4);
		printk("AVC_VLD(%d) 0x%x\n",u4Cnt,u4Value);
	}
	...
	for(u4Cnt = 44 ; u4Cnt <=84 ;u4Cnt++)
	{
		u4Value = u4VDecReadVLD(u4VDecID,u4Cnt*4);
		printk("VLD(%d) 0x%x\n",u4Cnt,u4Value);
	}
	
	printk("\n");

	for(u4Cnt = 0 ; u4Cnt <=1023 ;u4Cnt++)
	{
		u4Value = u4VDecReadMC(u4VDecID,u4Cnt*4);
		printk("MC(%d) 0x%x\n",u4Cnt,u4Value);
	}
#endif
