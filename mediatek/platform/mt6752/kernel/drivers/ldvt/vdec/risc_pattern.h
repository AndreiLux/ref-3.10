...
//HEVC HAL
#define HEVC_DRV_WriteReg(IO_BASE, dAddr, dVal)  mt_reg_sync_writel((dVal),(IO_BASE)+(dAddr))
...
void Dump_Dram0x49B_0x4FB(){
    ...
    sprintf( file_name, "/mnt/sdcard/Dram_dump0x49B_0x4FB");
    fd_bitstream = openFile( file_name, O_CREAT|O_RDWR, 0 );

    for (i=0; i<0x100; i++){
        Dram_dump = ioremap_nocache( 0x49B00000 +  0x6000000*i/0x100, 0x6000000/0x100 );
        fd_bitstream->f_op->write( fd_bitstream, Dram_dump, 0x6000000/0x100, &fd_bitstream->f_pos );        
        iounmap(Dram_dump);
    }
