...
static phys_addr_t m4u_user_v2p(unsigned long va)
{
    ...
    { 
        ...
        pa=(pte_val(*pte) & (~((phys_addr_t)0xfff))) | pageOffset; 
        pte_unmap(pte);
        return pa; 
    }   
...
void m4u_get_pgd(m4u_client_t* client, M4U_PORT_ID port, void** pgd_va, unsigned long* pgd_pa, unsigned int* size)
{
    m4u_domain_t * pDomain;
    pDomain = m4u_get_domain_by_port(port);
    *pgd_va = pDomain->pgd;
    *pgd_pa = (unsigned long)pDomain->pgd_pa;
    *size = M4U_PGD_SIZE;
}
...
get_pages_done:
    ...
    if (kernel_va == 0 || (unsigned int)kernel_va & M4U_PAGE_MASK)
    {
