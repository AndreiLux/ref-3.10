...
void _debug_fps_meter(unsigned int mva, unsigned int va, unsigned int w, unsigned int h, unsigned int linepitch, unsigned int color, unsigned int layerid, unsigned int bufidx)
{
	int i = 0;
	unsigned int addr = 0;
	unsigned int layer_size = 0;
...
static void process_dbg_opt(const char *opt)
{
	...
	else if (0 == strncmp(opt, "fence_test", 10))
	{
		kernel_fence_usage_test();
		return;
	}
