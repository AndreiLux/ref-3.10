...
static int mtkfb_pan_display_impl(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	//disable all layers except aee layer
	for(i=0; i<MAX_INPUT_CONFIG; i++) {
		if(is_DAL_Enabled() && i==primary_display_get_option("ASSERT_LAYER"))
			continue;
		session_input.config[session_input.config_layer_num].layer_id = i;
		session_input.config[session_input.config_layer_num].next_buff_idx= i;
