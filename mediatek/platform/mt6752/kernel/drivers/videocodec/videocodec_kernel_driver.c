...
static int K2Driver_Open_Count;         //mutex : DriverOpenCountLock
...
unsigned long KVA_VDEC_MISC_BASE, KVA_VDEC_VLD_BASE, KVA_VDEC_BASE, KVA_VDEC_GCON_BASE;
unsigned int VENC_IRQ_ID, VDEC_IRQ_ID;


#ifdef VENC_PWR_FPGA
// Cheng-Jung 20120621 VENC power physical base address (FPGA only, should use API) [
unsigned long KVA_VENC_CLK_CFG_0_ADDR, KVA_VENC_CLK_CFG_4_ADDR, KVA_VENC_PWR_ADDR, KVA_VENCSYS_CG_SET_ADDR;
// ]
#endif

extern unsigned long pmem_user_v2p_video(unsigned long va);

#if defined(VDEC_USE_L2C) || defined(K2_VENC_USE_L2C)
        ...
        case VCODEC_LOCKHW:
            {
                    ...
                    rHWLock.eDriverType == VAL_DRIVER_TYPE_VP8_DEC)
                {
                    while (bLockedHW == VAL_FALSE)
                    {
                        ...
                        if (grVcodecDecHWLock.pvHandle == (VAL_VOID_T*)pmem_user_v2p_video((unsigned long)rHWLock.pvHandle))
                        {
                        ...
                        if (grVcodecDecHWLock.pvHandle == 0) // No one holds dec hw lock now
                        {
                            ...
                            grVcodecDecHWLock.pvHandle = (VAL_VOID_T*)pmem_user_v2p_video((unsigned long)rHWLock.pvHandle);
                     ...
                     rHWLock.eDriverType == VAL_DRIVER_TYPE_JPEG_ENC)
                {
                    ...
                    // Wait to acquire Enc HW lock
                        ...
                        // one process try to lock twice
                        if (grVcodecEncHWLock.pvHandle == (VAL_VOID_T*)pmem_user_v2p_video((unsigned long)rHWLock.pvHandle))
                        {
                        ...
                        if (VAL_RESULT_INVALID_ISR == eValRet)
                        {
                            ...
                            if (FirstUseEncHW != 1)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        MFV_LOGE("VCODEC_LOCKHW - ID %d  fail, someone locked HW time out more than 30 times %p, %lx, %p, type:%d\n", current->pid, grVcodecEncHWLock.pvHandle, pmem_user_v2p_video((unsigned long)rHWLock.pvHandle), rHWLock.pvHandle, rHWLock.eDriverType);
                                        gLockTimeOutCount = 0;
                                        mutex_unlock(&VencHWLock);
                                        return -EFAULT;
                                    }
                                    ...
                                    if (rHWLock.u4TimeoutMs == 0)
                                    {
                                        MFV_LOGE("VCODEC_LOCKHW - ID %d  fail, someone locked HW already %p, %lx, %p, type:%d\n", current->pid, grVcodecEncHWLock.pvHandle, pmem_user_v2p_video((unsigned long)rHWLock.pvHandle), rHWLock.pvHandle, rHWLock.eDriverType);
                                        gLockTimeOutCount = 0;
                                        mutex_unlock(&VencHWLock);
                                        return -EFAULT;
                                    }
                        ...
                        else    //someone use HW, and check timeout value
                        {
                            ...
                            MFV_LOGD("LockInstance = 0x%x, CurrentInstance = 0x%x, CurrentTID = %d, TimeInterval(ms) = %d, TimeOutValue(ms)) = %d\n",
                                grVcodecEncHWLock.pvHandle, pmem_user_v2p_video((unsigned long)rHWLock.pvHandle), current->pid, u4TimeInterval, rHWLock.u4TimeoutMs);

                            MFV_LOGD("LockInstance = 0x%x, CurrentInstance = 0x%x, CurrentTID = %d, rLockedTime(s, us) = %d, %d, rCurTime(s, us) = %d, %d\n",
                                grVcodecEncHWLock.pvHandle, pmem_user_v2p_video((unsigned long)rHWLock.pvHandle), current->pid,
                            ...
                            if (gLockTimeOutCount > 30)
                            {
                                MFV_LOGE("VCODEC_LOCKHW - ID %d  fail, someone locked HW over 30 times without timeout %p, %lx, %p, type:%d\n", current->pid, grVcodecEncHWLock.pvHandle, pmem_user_v2p_video((unsigned long)rHWLock.pvHandle), rHWLock.pvHandle, rHWLock.eDriverType);
                                gLockTimeOutCount = 0;
                                mutex_unlock(&VencHWLock);
                                return -EFAULT;
                            }
                        ...
                        {
                            MFV_LOGE("Lock ok grVcodecEncHWLock.pvHandle = 0x%x, va:%x, type:%d", grVcodecEncHWLock.pvHandle, (unsigned long)rHWLock.pvHandle, rHWLock.eDriverType);
                            gLockTimeOutCount = 0;
                        }
                        mutex_unlock(&VencHWLock);
                    }
        ...
        case VCODEC_UNLOCKHW:
            {
                ...
                {
                    mutex_lock(&VdecHWLock);
                    if (grVcodecDecHWLock.pvHandle == (VAL_VOID_T*)pmem_user_v2p_video((unsigned long)rHWLock.pvHandle)) // Current owner give up hw lock
                    {
                ...
                {
                    mutex_lock(&VencHWLock);
                    if (grVcodecEncHWLock.pvHandle == (VAL_VOID_T*)pmem_user_v2p_video((unsigned long)rHWLock.pvHandle)) // Current owner give up hw lock
                    {
                    ...
                    {
                        // [TODO] error handling
                        MFV_LOGE("[ERROR] Not owner trying to unlock enc hardware 0x%p, pa:%lx, va:%lx type:%d\n", grVcodecEncHWLock.pvHandle, pmem_user_v2p_video((unsigned long)rHWLock.pvHandle), (unsigned long)rHWLock.pvHandle, rHWLock.eDriverType);
                        mutex_unlock(&VencHWLock);
                        return -EFAULT;
                        }
                    mutex_unlock(&VencHWLock);
        ...
        case VCODEC_WAITISR:
            {
                ...
                {
                    mutex_lock(&VdecHWLock);
                    if (grVcodecDecHWLock.pvHandle == (VAL_VOID_T*)pmem_user_v2p_video((unsigned long)val_isr.pvHandle))
                    {
                        bLockedHW = VAL_TRUE;
                    }
                    else
                ...
                {
                    mutex_lock(&VencHWLock);
                    if (grVcodecEncHWLock.pvHandle == (VAL_VOID_T*)pmem_user_v2p_video((unsigned long)val_isr.pvHandle))
                    {
                        bLockedHW = VAL_TRUE;
                    }
                    else
                    ...
                    if (bLockedHW == VAL_FALSE)
                    {
                        MFV_LOGE("[ERROR] DO NOT have enc HWLock, so return fail pa:%lx, va:%lx\n", pmem_user_v2p_video((unsigned long)val_isr.pvHandle), val_isr.pvHandle);
                        break;
                    }
...
static int __init vcodec_driver_init(void)
{
    ...
    }
    {
        ...
        MFV_LOGD("[DeviceTree] KVA_VENC_BASE(0x%x), KVA_VDEC_BASE(0x%x), KVA_VDEC_GCON_BASE(0x%x)", KVA_VENC_BASE, KVA_VDEC_BASE, KVA_VDEC_GCON_BASE);
        MFV_LOGD("[DeviceTree] VDEC_IRQ_ID(%d), VENC_IRQ_ID(%d)", VDEC_IRQ_ID, VENC_IRQ_ID);
    }

//    KVA_VENC_IRQ_STATUS_ADDR =    (unsigned long)ioremap(VENC_IRQ_STATUS_addr, 4);
//    KVA_VENC_IRQ_ACK_ADDR  = (unsigned long)ioremap(VENC_IRQ_ACK_addr, 4);


#ifdef VENC_PWR_FPGA    // useless 2014_3_4
    KVA_VENC_CLK_CFG_0_ADDR = (unsigned long)ioremap(CLK_CFG_0_addr, 4);
    KVA_VENC_CLK_CFG_4_ADDR = (unsigned long)ioremap(CLK_CFG_4_addr, 4);
    KVA_VENC_PWR_ADDR = (unsigned long)ioremap(VENC_PWR_addr, 4);
    KVA_VENCSYS_CG_SET_ADDR = (unsigned long)ioremap(VENCSYS_CG_SET_addr, 4);
#endif
