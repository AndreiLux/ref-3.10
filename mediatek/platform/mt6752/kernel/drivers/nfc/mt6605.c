...
static int mt6605_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* MT6306 GPIO3 is GPIO_NFC_VENB, Thomas-CH for K2 common phone pin modification  */
	mt6306_set_gpio_dir(GPIO3, GPIO_DIR_OUT);
	mt6306_set_gpio_out(GPIO3, GPIO_OUT_ONE);
    usleep_range( 900, 1000); //debug
	
	/* MT6306 GPIO2 is GPIO_NFC_RSTB, Thomas-CH for K2 common phone pin modification */
