...
static unsigned int color_read_sw_reg(unsigned int reg_id)
{
    ...
    switch (reg_id)
    {
        ...
        {
            ret = dispsys_reg[DISP_REG_COLOR];
            break;
        }

        case SWREG_GAMMA_BASE_ADDRESS:
        {
            ret = dispsys_reg[DISP_REG_GAMMA];
            break;
        }

        case SWREG_AAL_BASE_ADDRESS:
        {
            ret = dispsys_reg[DISP_REG_AAL];
            break;
        }
...
static int _color_io(DISP_MODULE_ENUM module, int msg, unsigned long arg, void *cmdq)
{
    ...
    switch (msg) {
        ...
        case DISP_IOCTL_READ_REG:
        {
            ...
            if(0 == color_is_reg_addr_valid(rParams.reg))
            {
                COLOR_ERR("reg read, addr invalid, addr min=0x%x, max=0x%lx, addr=0x%lx \n",
            ...
            COLOR_DBG("read 0x%lx = 0x%x (0x%x)\n", rParams.reg, value, rParams.mask);
        ...
        case DISP_IOCTL_WRITE_REG:
        {
            ...
            ret = color_is_reg_addr_valid(wParams.reg);
            if(ret == 0)
            {
                COLOR_ERR("reg write, addr invalid, addr min=0x%x, max=0x%lx, addr=0x%lx \n",
                    DISPSYS_REG_ADDR_MIN,
                    DISPSYS_REG_ADDR_MAX,
                    wParams.reg);
                return -EFAULT;
            ...
            if (ret == 2)
            {
                if(cmdq == NULL)
                {
                    mt_reg_sync_writel((unsigned int)(INREG32(wParams.reg)&~(wParams.mask))|(wParams.val),(volatile unsigned long*)(wParams.reg) );\
                }
                else
                {
        ...
        case DISP_IOCTL_READ_SW_REG:
        {
            ...
            {
                COLOR_ERR("sw reg read, addr invalid, addr min=0x%x, max=0x%x, addr=0x%lx \n",
            ...
            COLOR_DBG("read sw reg 0x%lx = 0x%x\n", rParams.reg, rValue);
        ...
        case DISP_IOCTL_WRITE_SW_REG:
        {
            ...
            {
                COLOR_ERR("sw reg write, addr invalid, addr min=0x%x, max=0x%x, addr=0x%lx \n",
                    DISP_COLOR_SWREG_START,
                    DISP_COLOR_SWREG_END,
                    wParams.reg);
                return -EFAULT;
            ...
            COLOR_DBG("write sw reg  0x%lx = 0x%x \n", wParams.reg, wParams.val);
