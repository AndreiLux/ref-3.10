...
static int  mt_cirq_set_sens(unsigned int cirq_num, unsigned int sens)
{
    ...
    if(sens == MT_EDGE_SENSITIVE) {
        base = (cirq_num / 32) * 4 + (uintptr_t)CIRQ_SENS_CLR_BASE;
    } else if(sens == MT_LEVEL_SENSITIVE) {
        base = (cirq_num / 32) * 4 + (uintptr_t)CIRQ_SENS_SET_BASE;
    } else {
...
static int mt_cirq_set_pol(unsigned int cirq_num, unsigned int pol)
{
    ...
    if(pol == MT_CIRQ_POL_NEG) {
        base = (cirq_num / 32) * 4 + (uintptr_t)CIRQ_POL_CLR_BASE;
    } else if(pol == MT_CIRQ_POL_POS) {
        base = (cirq_num / 32) * 4 + (uintptr_t)CIRQ_POL_SET_BASE;
    } else {
...
int mt_cirq_test(void)
{
    ...
    if(mt_cirq_get_pol(cirq_num) != MT_CIRQ_POL_NEG)
        printk(KERN_NOTICE "mt_cirq_set_pol clear test failed!!\n");
    else
        printk(KERN_NOTICE "mt_cirq_set_pol clear test passed!!\n");
    mt_cirq_set_pol(cirq_num, MT_CIRQ_POL_POS);
    if(mt_cirq_get_pol(cirq_num) != MT_CIRQ_POL_POS)
        printk(KERN_NOTICE "mt_cirq_set_pol set test failed!!\n");
    else
        printk(KERN_NOTICE "mt_cirq_set_pol set test passed!!\n");

    /*test sensitivity*/
    mt_cirq_set_sens(cirq_num, MT_EDGE_SENSITIVE);
    if(mt_cirq_get_sens(cirq_num) != MT_EDGE_SENSITIVE)
        printk(KERN_NOTICE "mt_cirq_set_sens clear test failed!!\n");
    else
        printk(KERN_NOTICE "mt_cirq_set_sens clear test passed!!\n");
    mt_cirq_set_sens(cirq_num, MT_LEVEL_SENSITIVE);
    if(mt_cirq_get_sens(cirq_num) != MT_LEVEL_SENSITIVE)
        printk(KERN_NOTICE "mt_cirq_set_sens set test failed!!\n");
    else
        printk(KERN_NOTICE "mt_cirq_set_sens set test passed!!\n");
    ...
    if (mt_cirq_get_mask(cirq_num) != 1)
        ...
        printk(KERN_NOTICE "mt_cirq_mask test passed!!\n");
    mt_cirq_unmask(cirq_num);
    if (mt_cirq_get_mask(cirq_num) != 0)
        printk(KERN_NOTICE "mt_cirq_unmask test failed!!\n");
    else
        printk(KERN_NOTICE "mt_cirq_unmask test passed!!\n");
