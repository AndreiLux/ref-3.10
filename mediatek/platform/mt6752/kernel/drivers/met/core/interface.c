...
static ssize_t tracecmd_store(struct device *dev, struct device_attribute *attr, const char *buf,
			  size_t count);
static DEVICE_ATTR(tracecmd, 0664, tracecmd_show, tracecmd_store);
...
static unsigned int met_ksym_addr = 0;
			 ...
			 size_t count)
{
	...
	if (sscanf(buf, "%x", &met_ksym_addr) != 1) {
		mutex_unlock(&dev->mutex);
		return -EINVAL;
	}
...
int fs_reg(void)
{
	...
	if (ret != 0) {
		pr_err("can not create device file: ksym\n");
		return ret;
	}
