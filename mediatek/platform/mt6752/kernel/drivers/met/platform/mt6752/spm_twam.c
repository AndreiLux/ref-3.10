...
void spmtwam(struct twam_sig *ts)
{

	switch (used_count) {
	case 1:
		MET_PRINTK(SPM_TWAM_FMT1,
			   get_high_percent(ts->sig0));
		break;
	case 2:
		MET_PRINTK(SPM_TWAM_FMT2,
			   get_high_percent(ts->sig0),
			   get_high_percent(ts->sig1));
		break;
	case 3:
		MET_PRINTK(SPM_TWAM_FMT3,
			   get_high_percent(ts->sig0),
			   get_high_percent(ts->sig1),
			   get_high_percent(ts->sig2));
		break;
	case 4:
...
static int assign_slot(unsigned int event)
{
	...
	/* check event no */
	if (event >= ARRAY_SIZE(twam_sig_list))
		return -3;
...
static char help[] =
	...
	"                                        speed mode monitors 2 channels\n"
...
static int spmtwam_process_argument(const char *arg, int len)
{
	...
	} else if (strncmp(arg, "scpsys:", 7) == 0) {
		/* TODO: */
		event = 0;
		if (arg[7] == '0') {		/* infrasys */
