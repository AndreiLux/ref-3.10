...
#include "core/trace.h"
    ...
    extern unsigned int mt_cpufreq_get_cur_volt(enum mt_cpu_dvfs_id id);//CPU voltage
...
/*
 * FIXME : ptpod() using a workaround to generate rectangular waveform.
 *	It generates double information. Fix it after fronend start to support.
 */
...
{
	g_u4LittleCPUVolt = mt_cpufreq_get_cur_volt(MT_CPU_DVFS_LITTLE)/100;
...
static void ptpod_stop(void)
{
	...
	ptpod(MET_CPU_LITTLE, mt_cpufreq_get_cur_volt(MT_CPU_DVFS_LITTLE)/100);
	ptpod(MET_GPU, ((mt_gpufreq_get_cur_volt()+50)/100));
}

static char help[] =
	"  --ptpod                               Measure CPU/GPU voltage & freq\n";
 ...
 */
static char header[] =
	"met-info [000] 0.0: ms_ud_sys_header: ptpod,LittleCPUVolt,GPUVolt,d,d\n";
...
static char gpu_dvfs_help[] =
	"  --gpu_dvfs                            Measure GPU freq\n";
 ...
 * It will be called back when run "met-cmd --extract" and mode is 1
 */
static char gpu_dvfs_header[] =
	"met-info [000] 0.0: ms_ud_sys_header: GPUDVFS,freq(kHz),d\n";
