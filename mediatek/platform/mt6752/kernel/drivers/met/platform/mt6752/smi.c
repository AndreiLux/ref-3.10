...
{
	int	ret = 0;
	int	master = allport[toggle_idx].master;
	...
	if (master < SMI_LARB_NUMBER) {
		...
		/* restart counting */
		MET_SMI_Clear(master);
		MET_SMI_Enable(master);
	} else {
		...
		/* restart counting */
		MET_SMI_Comm_Clear(master);
		MET_SMI_Comm_Enable(master);
	}
...
static char help[] =
"  --smi=toggle                          monitor all SMI port banwidth\n"
"  --smi=toggle:master                   monitor one master's SMI port banwidth\n"
...
"# ms_smi: metadata,active,request,beat,byte,CP,DP,OSTD,CPMAX,OSTDMAX\n"
"met-info [000] 0.0: met_smi_header: metadata,"
"active,request,beat,byte,CP,DP,OSTD,CPMAX,OSTDMAX\n";

static char header_parallel_mode[] =
"# ms_smi: metadata,byte\n"
"met-info [000] 0.0: met_smi_header: metadata,byte\n";

static char header_toggle_mode[] =
"# ms_smit: metadata,active,request,beat,byte,CP,DP,OSTD,CPMAX,OSTDMAX\n"
"met-info [000] 0.0: met_smi_header: metadata,"
"active,request,beat,byte,CP,DP,OSTD,CPMAX,OSTDMAX\n";
...
static int get_num(const char *dc, unsigned int *pValue)
{
	unsigned int	value = 0, digit;
	int		i = 0;

	while (((*dc) >= '0') && ((*dc) <= '9')) {
		digit = (unsigned int)(*dc - '0');
...
static int check_port_vaild(int master, unsigned int port)
{
...
static int assign_port(int master, unsigned int port)
{
	int i;
...
static int check_rwtype_vaild(int master, unsigned int port, unsigned int rwtype)
{
...
static int assign_rwtype(int master, unsigned int rwtype)
{
	if ((master >= 0) && (master < SMI_LARB_NUMBER)) {
...
static int check_desttype_valid(int master, unsigned int port, unsigned int desttype)
{
...
static int assign_desttype(int master, unsigned int desttype)
{
	if ((master >= 0) && (master < SMI_LARB_NUMBER)) {
...
static int check_bustype_valid(int master, unsigned int port, unsigned int bustype)
{
...
static int assign_bustype(int master, unsigned int bustype)
{
	if ((master >= 0) && (master < SMI_LARB_NUMBER)) {
