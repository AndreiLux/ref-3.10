...
static int _build_path_direct_link(void)
{
	...
	if(pgc->dpmgr_handle)
	{
		EXT_DISP_LOG("dpmgr create path SUCCESS(0x%08x)\n", pgc->dpmgr_handle);
	}
	else
	{
...
static void _cmdq_build_trigger_loop(void)
{
	...
	EXT_DISP_LOG("ext_disp path trigger thread cmd handle=0x%08x\n", pgc->cmdq_handle_trigger);
...
int ext_disp_init(char *lcm_name, unsigned int session)
{
	...
	{
		EXT_DISP_LOG("cmdqRecCreate SUCCESS, g_cmdq_handle=0x%x \n", pgc->cmdq_handle_config);
	}
