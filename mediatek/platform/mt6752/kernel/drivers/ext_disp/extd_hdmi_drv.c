...
static unsigned int ovl_config_address[HDMI_OVERLAY_CNT];
...
static void hdmi_resolution_setting(u8 arg)
{
    HDMI_FUNC();
    switch (arg)
    {
...
#if defined(MTK_SMARTBOOK_SUPPORT) && defined(CONFIG_HAS_SBSUSPEND)
            if (hdmi_params->cabletype == MHL_SMB_CABLE)
            {
                sb_plug_out();
            }
...
{
    HDMI_LOG("video resolution configuration, res=%ld, %d, f %d video_on %d\n", res, hdmi_reschange, factory_mode, p->is_mhl_video_on);
    RETIF(!p->is_enabled, 0);
    ...
    hdmi_resolution_setting(res);  //<--Donglei  
    p->is_mhl_video_on = false;
...
int hdmi_screen_capture(void *info)
{
    ...
    if (copy_from_user(&capture_addr, info, sizeof(capture_addr)))
    {
        HDMI_LOG(": copy_to_user failed! line:%d \n", __LINE__);
        ret = -EFAULT;
    }
...
{
    HDMI_FUNC();
    HDMI_LOG("extd_hdmi_path_init mode:%d line:%d \n", mode, __LINE__);
