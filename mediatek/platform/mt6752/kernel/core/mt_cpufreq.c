...
// TODO: NEED to check power throttling settings (Brian)
...
static unsigned int _mt_cpufreq_get_cpu_level(void)
{
...
#ifdef CONFIG_OF
    {
        ...
        if (cpu_speed > 1700) {
            // use CPU lv0 only for specific product
            switch (func_code_1) {
                case 0:
                    if (func_code_0 == 6 || func_code_0 == 11)
                        lv = CPU_LEVEL_0;   // 2G
                    else
                        lv = CPU_LEVEL_1;   // 1.7G
                    break;
                case 8:
                    if (func_code_0 == 3 || func_code_0 == 6)
                        lv = CPU_LEVEL_0;   // 2G
                    else
                        lv = CPU_LEVEL_1;   // 1.7G
                    break;
...
#ifdef CONFIG_CPU_FREQ
    if (policy) {
        // update actul freq by new_opp_idx
...
void mt_cpufreq_enable_by_ptpod(enum mt_cpu_dvfs_id id)
{
     ...
     *  2. CPU segment is 1.7GHz or 2GHz
     *  3. PTPOD is enabled
     */    
...
unsigned int gEMI_DFS_enable = 0;
...
static void _mt_cpufreq_early_suspend(struct early_suspend *h)
{
    ...
    if (gEMI_DFS_enable && gMd32_alive)
    {
        //switch to 1 PLL
        do_DRAM_DFS(0);
...
{
    seq_printf(m, "EMI DFS: %s, %s\n", gEMI_DFS_enable?"enable":"disable", (mt_get_clk_mem_sel()==1)?"3PLL":"1PLL");
    return 0;
}
