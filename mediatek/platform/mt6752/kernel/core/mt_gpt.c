...
{
		//Timer DEBUG for memory corrupt
		if (!is_vmalloc_addr((void *)dev->base_addr)) {
			dev->base_addr = 0xf0008020;
			print_hex_dump(KERN_ALERT, "__gpt_get_cnt", DUMP_PREFIX_ADDRESS,
                        16, 1, ((unsigned int)gpt_devs - 1024), 2048, 1);
...
#ifndef CONFIG_ARM64
    //arm64 not support, move from "arch/arm/kernel/sched_clock.c" to "arch\arm64\kernel\time.c"
