...
/* MSDC_SD_NEED_POWER:
 * On some TABLET board, SDCARD detect pin connects to VMC,
 * which means if power off, card cannot be detected anymore,
 * but if we keep power always on, it would cause another problem,
 * after reset card by CMD0, do card initialization,
 * the card can NOT support 1.8V(OCR bit24),
 * and some special card failed to be intialized.
 */
