...
static int es705_slim_be_id[ES705_NUM_CODEC_SLIM_DAIS] = {
	...
	ES705_SLIM_3_PB, /* for ES705_SLIM_2_CAP rx to es705 */
#else
...
static int es705_codec_cfg_slim_tx(struct es705_priv *es705, int dai_id)
{
	...
	if (dai_id == ES705_SLIM_2_CAP)
		dai_id = ES705_SLIM_1_CAP;
	dev_dbg(&sbdev->dev, "%s(): dai_id = %d\n", __func__, dai_id);
...
#if defined(SUPPORT_DAI_3)
	if ((dai_id == ES705_SLIM_1_CAP) || (dai_id == ES705_SLIM_2_CAP)) {
	if ((dai_id == ES705_SLIM_2_CAP) && (es705_priv.rx2_route_enable==0))
			dai_id = ES705_SLIM_1_CAP;
		dev_info(&sbdev->dev, "%s(): assign the channel\n", __func__);
		}
	else
		return rc;
#else
...
#if defined(SLIMBUS_VER_2)
	es705->dai[DAI_INDEX(ES705_SLIM_3_CAP)].ch_num[0] = 149;
	es705->dai[DAI_INDEX(ES705_SLIM_3_CAP)].ch_num[1] = 150;
#else
...
static int es705_slim_probe_to_start(struct slim_device *sbdev)
{
	dev_info(&sbdev->dev, "%s: sbdev=%s\n", __func__, sbdev->name);
	/*
	 * Skip probe calling by device register functions,
	 * just activate es70x device
	 */
	if (sbdev->dev.of_node)
		es705_stimulate_start(&sbdev->dev);
	return 0;
}
...
struct slim_driver es70x_slim_driver = {
	...
	.probe = es705_slim_probe_to_start,
