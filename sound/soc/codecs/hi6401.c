...
/*note: this errno is only used for APR*/
enum APR_ERRNO {
	ANC_HS_CALIBRATION_ERR                  = 1,
	ANC_HS_QUEUE_NOTHING                    = 2,
};
...
#define  NO_CHARGE_DETECT_PERIOD_TIME          2*60*60*1000  // 2 hours
					...
					struct snd_ctl_elem_value *uc)
{
	...
	if(priv->p_anc_hs.anc_hs_mode == ANC_HS_CHARGE_ON ||
		...
		if(uc->value.integer.value[0] < 0) {
			pr_err("%s : read adc_h value err = %ld", __FUNCTION__, uc->value.integer.value[0]);
			uc->value.integer.value[0] = 0;
		}
	}else {
		pr_info("%s : anc_hs adc h = %d", __FUNCTION__,
					...
					struct snd_ctl_elem_value *uc)
{
	...
	if(priv->p_anc_hs.anc_hs_mode == ANC_HS_CHARGE_ON ||
		...
		if(uc->value.integer.value[0] < 0) {
			pr_err("%s : read adc_l value err = %ld", __FUNCTION__, uc->value.integer.value[0]);
			uc->value.integer.value[0] = 0;
		}
	}else {
		pr_info("%s : anc_hs adc l = %d", __FUNCTION__,
...
static int send_apr_log(int err_tag)
{
	char cmd[256];
	char time_buf[16];
	int ret = 0;
	...
	snprintf(cmd, 256, "%s%s%s%s%d%s",
		"archive -i /data/android_logs/kmsgcat-log -i /data/android_logs/kmsgcat-log.1",
		" -o ", time_buf, "_ANC_HS_", err_tag, " -z zip");
	ret = log_to_exception("audio", cmd);
	if(ret < 0 ){
		pr_err("%s: logexception err ret=%d\n", __FUNCTION__, ret);
	}
	return ret;
}
		...
		if(p_anc_hs->mic_used) {
			...
			if (!check_headset_pluged_in(priv->codec)) {
				...
				if(!ret) {
					...
					send_apr_log(ANC_HS_QUEUE_NOTHING);
				}
...
static bool need_charging(struct anc_hs_data *anc_hs)
{
	...
	delta = ear_pwr_h - ear_pwr_l - anc_hs->adc_calibration_base;
	if((delta > anc_hs->anc_hs_limit) &&
		(delta < ANC_HS_LIMIT_MAX))
		return true;
	else
		return false;
}
				   ...
				   size_t count, loff_t *ppos)
{
	...
	snprintf(buf, HS_INFO_SIZE, "%sanc_hs_limit: %d\n", buf, p_anc_hs->anc_hs_limit);
		...
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	if (copy_from_user(buf, user_buf, buf_size))
		return -EFAULT;
	ret = kstrtoint(buf, 10, &(p_anc_hs->anc_hs_limit));
	if(ret) {
...
static int anc_hs_init(struct anc_hs_data *p_anc_hs)
{
	...
	p_anc_hs->anc_hs_mode = ANC_HS_CHARGE_OFF;
	p_anc_hs->anc_hs_limit = ANC_HS_LIMIT_MIN;
	p_anc_hs->irq_flag = true;
	p_anc_hs->boost_flag = false;
	p_anc_hs->mic_used = false;
	p_anc_hs->sleep_time = ANC_CHIP_WARM_STARTUP_TIME;
	...
	printk(KERN_ERR "mic_irq = %d, mic_irq_gpio = %d", p_anc_hs->mic_irq, p_anc_hs->mic_irq_gpio);
	...
	if (!(p_anc_hs->anc_hs_btn_delay_wq)) {
		pr_err("%s : anc_hs_btn_delay_wq create failed", __FUNCTION__);
		ret = -ENOMEM;
	...
	if (!(p_anc_hs->anc_hs_charge_delay_wq)) {
		pr_err("%s : anc_hs_charge_delay_wq create failed", __FUNCTION__);
		ret = -ENOMEM;
	...
	enable_boost(p_anc_hs, false);
	if(loop == ADC_CALIBRATION_TIMES) {
		p_anc_hs->adc_calibration_base = 0;
		pr_err("[anc_hs] calibration whole failed\n");
	}else {
		p_anc_hs->adc_calibration_base /= (ADC_CALIBRATION_TIMES - loop);
		pr_info("anc_hs: adc_calibration_base = %d\n", p_anc_hs->adc_calibration_base);

		if(p_anc_hs->adc_calibration_base > 50 ||
			p_anc_hs->adc_calibration_base < -50) {
			send_apr_log(ANC_HS_CALIBRATION_ERR);
			p_anc_hs->adc_calibration_base = 0;
	...
	if (!debugfs_create_file("hs_info", S_IRUGO, debug_dir, p_anc_hs,
				&hs_info_fops)) {
		pr_err("anc_hs: Failed to create hs_info debugfs file\n");
	}
	if (!debugfs_create_file("detect_limit", S_IRUGO | S_IWUGO, debug_dir, p_anc_hs,
				&detect_limit_fops)) {
		pr_err("anc_hs: Failed to create detect_limit debugfs file\n");
	}
	if (!debugfs_create_file("detect_period", S_IRUGO | S_IWUGO, debug_dir, p_anc_hs,
				&detect_period_fops)) {
		pr_err("anc_hs: Failed to create detect_period debugfs file\n");
	}
	if (!debugfs_create_file("gpio_sw", S_IRUGO | S_IWUGO, debug_dir, p_anc_hs,
				&gpio_sw_fops)) {
		pr_err("anc_hs: Failed to create gpio_sw debugfs file\n");
	}
	if (!debugfs_create_file("sleep_time", S_IRUGO | S_IWUGO, debug_dir, p_anc_hs,
				&sleep_time_fops)) {
