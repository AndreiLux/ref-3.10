...
static const char *spk_state_text[] = { "Disable", "Enable" };

static const struct soc_enum spk_state_enum[] = { 
    SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(spk_state_text),  
                        spk_state_text), 
};
...
static const struct snd_kcontrol_new max98504_snd_controls[] = {
	...
	SOC_ENUM_EXT("SPK out", spk_state_enum[0], max98504_get_speaker, max98504_set_speaker), 
};
...
static const char *digital_mono_mux_text[] = { "CH0", "CH1", "CHMIX" };

static const struct soc_enum digital_mono_mux_enum =
	SOC_ENUM_SINGLE(MAX98504_REG_28_PCM_TO_SPEAKER_MONOMIX, M98504_PCM_TO_SPK_MONOMIX_CFG_SHIFT,
...
static const struct snd_soc_dapm_widget max98504_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MUX("DAC Mono Mux", SND_SOC_NOPM, 0, 0, &max98504_digital_mono_mux),

	SND_SOC_DAPM_MUX("SPK Source Mux", SND_SOC_NOPM, 0, 0, &max98504_spk_src_mux),
...
/* Configure Rx Mode */
	if(pdata->rx_mode == MODE_RX_PCM)	{
		...
		snd_soc_update_bits(codec, MAX98504_REG_25_PCM_DSP_CONFIG, M98504_PCM_DSP_CFG_RX_DITH_EN_MASK|M98504_PCM_DSP_CFG_RX_FLT_MODE_MASK, regval);
		snd_soc_write(codec,  MAX98504_REG_20_PCM_RX_ENABLES, (u8)cfg_data->rx_ch_en);
	}
	else if(pdata->rx_mode==MODE_RX_PDM0 || pdata->rx_mode==MODE_RX_PDM1)	{
		snd_soc_write(codec,  MAX98504_REG_33_PDM_RX_ENABLE, M98504_PDM_RX_EN_MASK);
	}
	snd_soc_write(codec,  MAX98504_REG_35_SPEAKER_SOURCE_SELECT, (u8) (M98504_SPK_SRC_SEL_MASK & pdata->rx_mode));
	...
	if(pdata->tx_mode == MODE_TX_PCM)	{
		...
		snd_soc_update_bits(codec, MAX98504_REG_25_PCM_DSP_CONFIG, M98504_PCM_DSP_CFG_TX_DITH_EN_MASK|M98504_PCM_DSP_CFG_MEAS_DCBLK_EN_MASK, regval);

		snd_soc_write(codec,  MAX98504_REG_21_PCM_TX_ENABLES, (u8)cfg_data->tx_ch_en);
		snd_soc_write(codec,  MAX98504_REG_22_PCM_TX_HIZ_CONTROL, (u8)cfg_data->tx_hiz_ch_en);
		snd_soc_write(codec,  MAX98504_REG_23_PCM_TX_CHANNEL_SOURCES, (u8)cfg_data->tx_ch_src);		
	}
	else {
		snd_soc_write(codec,  MAX98504_REG_30_PDM_TX_ENABLES, (u8)cfg_data->tx_ch_en);
		snd_soc_write(codec,  MAX98504_REG_31_PDM_TX_HIZ_CONTROL, (u8)cfg_data->tx_hiz_ch_en);
		snd_soc_write(codec,  MAX98504_REG_32_PDM_TX_CONTROL, (u8)cfg_data->tx_ch_src);
	}

#ifndef MAX98504_USE_DAPM
		snd_soc_write(codec,  MAX98504_REG_36_MEASUREMENT_ENABLES, M98504_MEAS_I_EN_MASK | M98504_MEAS_V_EN_MASK);
#endif
