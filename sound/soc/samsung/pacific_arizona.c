...
static void create_dsm_log_dump(struct snd_soc_codec *codec)
{
	...
	for (i = 0; i < dsm_log_info[BYTELOGINFO].size; i++) {
		val1  = (snd_soc_read(codec, dsm_log_info[BYTELOGINFO].start + i*2)) & 0xFFFF;
		val2  = (snd_soc_read(codec, dsm_log_info[BYTELOGINFO].start + i*2 +1)) & 0xFFFF;
		byte_buf[i] = ((val1 << 16) | val2);
	}

	for (i = 0; i < dsm_log_info[INTLOGINFO].size; i++) {
		val1  = (snd_soc_read(codec, dsm_log_info[INTLOGINFO].start + i*2)) & 0xFFFF;
		val2  = (snd_soc_read(codec, dsm_log_info[INTLOGINFO].start + i*2 +1)) & 0xFFFF;
		int_buf[i] = ((val1 << 16) | val2);
	}

	for (i = 0; i < dsm_log_info[AFTERBYTELOGINFO].size; i++) {
		val1  = (snd_soc_read(codec, dsm_log_info[AFTERBYTELOGINFO].start + i*2)) & 0xFFFF;
		val2  = (snd_soc_read(codec, dsm_log_info[AFTERBYTELOGINFO].start + i*2 +1)) & 0xFFFF;
		after_byte_buf[i] = ((val1 << 16) | val2);
	}

	for (i = 0; i < dsm_log_info[AFTERINTLOGINFO].size; i++) {
		val1  = (snd_soc_read(codec, dsm_log_info[AFTERINTLOGINFO].start + i*2)) & 0xFFFF;
		val2  = (snd_soc_read(codec, dsm_log_info[AFTERINTLOGINFO].start + i*2 +1)) & 0xFFFF;
...
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	unsigned int log_available = snd_soc_read(codec, ARIZONA_DSP4_CONTROL_1);
...
static void create_dsm_param_dump(struct snd_soc_codec *codec)
{
	...
	for (i = 0; i < dsm_param_info[0].size; i++) {
		val1  = (snd_soc_read(codec, dsm_param_info[0].start + i*2)) & 0xFFFF;
		val2  = (snd_soc_read(codec, dsm_param_info[0].start + i*2 +1)) & 0xFFFF;
...
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	unsigned int param_available = snd_soc_read(codec, ARIZONA_DSP4_CONTROL_1);
...
static int pacific_aif_startup(struct snd_pcm_substream *substream)
{
	...
	dev_info(card->dev, "%s-%d startup ++, playback=%d, capture=%d\n",
			rtd->dai_link->name, substream->stream, codec_dai->playback_active,
	...
	if (priv->aif[0] == codec_dai) {
		if ((substream->stream == SNDRV_PCM_STREAM_PLAYBACK) &&
			...
			snd_soc_dapm_force_enable_pin(&card->dapm, "VI SENSING");
...
static void pacific_aif_shutdown(struct snd_pcm_substream *substream)
{
	...
	dev_info(card->dev, "%s-%d shutdown++ codec_dai[%s]:playback=%d\n",
			rtd->dai_link->name, substream->stream, codec_dai->name, codec_dai->playback_active);
...
static void ez2ctrl_voicewakeup_cb(void)
{
	...
	if (priv->voice_uevent == 0) {
		...
		dev_err(the_codec->dev, "Invalid voice control mode =%d", priv->voice_uevent);
		return;
	}
	envp[0] = keyword_buf;
	envp[1] = NULL;
					...
					const char *buf, size_t count)
{
	...
	if (ret) {
		dev_err(codec->dev, "Failed to convert a string to an int: %d\n", ret);
	}
...
static int pacific_late_probe(struct snd_soc_card *card)
{
	...
	if (priv->seamless_voicewakeup) {
		...
		if (IS_ERR(svoice_class))
			pr_err("Failed to create class(svoice)!\n");

		keyword_dev = device_create(svoice_class, NULL, 0, NULL, "keyword");
		if (IS_ERR(keyword_dev))
			pr_err("Failed to create device(keyword)!\n");

		if (device_create_file(keyword_dev, &dev_attr_keyword_type) < 0)
...
static int pacific_check_clock_conditions(struct snd_soc_card *card)
{
	...
	return (!codec->active && priv->ear_mic && !mainmic_state);
}
