                                       ...
                                       struct snd_pcm_hw_params *hw_params)
{
    ...
    printk("mtk_mgrrx_awb_pcm_hw_params dma_bytes = %d dma_area = %p dma_addr = 0x%x\n",
                                  ...
                                  void __user *dst, snd_pcm_uframes_t count)
{
    ...
    if (DMA_Read_Ptr + read_size < Awb_Block->u4BufferSize)
    {
        ...
        {
            printk("AudDrv_MEMIF_Read 1, read_size:0x%x, DataRemained:0x%x, DMA_Read_Ptr:0x%x, DMAReadIdx:0x%x \r\n",
    ...
    else
    {
        ...
        if (DMA_Read_Ptr != Awb_Block->u4DMAReadIdx)
        {
            ...
            printk("AudDrv_MEMIF_Read 2, read_size1:0x%x, DataRemained:0x%x, DMA_Read_Ptr:0x%x, DMAReadIdx:0x%x \r\n",
...
static int mtk_afe_fm_i2s_awb_probe(struct snd_soc_platform *platform)
{
    ...
    AudDrv_Allocate_mem_Buffer(Soc_Aud_Digital_Block_MEM_AWB, FM_I2S_MAX_BUFFER_SIZE);
