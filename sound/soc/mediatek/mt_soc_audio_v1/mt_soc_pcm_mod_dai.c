                                     ...
                                     struct snd_pcm_hw_params *hw_params)
{
    ...
    if (mModDaiUseSram == true)
    {
        ...
        printk("mtk_mod_dai_pcm_hw_params mModDaiUseSram dma_bytes = %d \n", runtime->dma_bytes);
    ...
    printk("mtk_mod_dai_pcm_hw_params dma_bytes = %d dma_area = %p dma_addr = 0x%x\n",
...
static int mtk_mod_dai_pcm_open(struct snd_pcm_substream *substream)
{
    ...
    AfeControlSramLock();
    if ((GetSramState() ==  SRAM_STATE_FREE) || (GetSramState()&SRAM_STATE_SPH_SPK_MNTR_CAPTURE_DL))
    {
        mtk_mod_dai_hardware.buffer_bytes_max = 4096;
        printk("mtk_mod_dai_pcm_open use sram %d\n", mtk_mod_dai_hardware.buffer_bytes_max);
                                ...
                                void __user *dst, snd_pcm_uframes_t count)
{
    ...
    PRINTK_AUD_MODDAI("mtk_mod_dai_pcm_copy pos = %lucount = %lu \n ", pos, count);
    ...
    {
        PRINTK_AUD_MODDAI("mtk_mod_dai_pcm_copy u4DataRemained=%x > u4BufferSize=%x" , pModDai_Block->u4DataRemained, pModDai_Block->u4BufferSize);
        pModDai_Block->u4DataRemained = 0;
    ...
    PRINTK_AUD_MODDAI("mtk_mod_dai_pcm_copy finish0, read_count:%x, read_size:%x, u4DataRemained:%x, u4DMAReadIdx:0x%x, u4WriteIdx:%x \r\n",
    ...
    if (DMA_Read_Ptr + read_size < pModDai_Block->u4BufferSize)
    {
        ...
        {
            printk("mtk_mod_dai_pcm_copy 1, read_size:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x \r\n",
        ...
        if (copy_to_user((void __user *)Read_Data_Ptr, (pModDai_Block->pucVirtBufAddr + DMA_Read_Ptr), read_size))
        {
            ...
            printk("mtk_mod_dai_pcm_copy Fail 1 copy to user Read_Data_Ptr:%p, pucVirtBufAddr:%p, u4DMAReadIdx:0x%x, DMA_Read_Ptr:0x%x,read_size:%x", Read_Data_Ptr, pModDai_Block->pucVirtBufAddr, pModDai_Block->u4DMAReadIdx, DMA_Read_Ptr, read_size);
            return 0;
        }
    ...
    else
    {
        ...
        if (DMA_Read_Ptr != pModDai_Block->u4DMAReadIdx)
        {
            ...
            printk("mtk_mod_dai_pcm_copy 2, read_size1:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x \r\n",
        ...
        if (copy_to_user((void __user *)Read_Data_Ptr, (pModDai_Block->pucVirtBufAddr + DMA_Read_Ptr), size_1))
        {
            ...
            printk("mtk_mod_dai_pcm_copy Fail 2 copy to user Read_Data_Ptr:%p, pucVirtBufAddr:%p, u4DMAReadIdx:0x%x, DMA_Read_Ptr:0x%x,read_size:%x",
        ...
        if (DMA_Read_Ptr != pModDai_Block->u4DMAReadIdx)
        {
            ...
            printk("mtk_mod_dai_pcm_copy 3, read_size2:%x, DataRemained:%x, DMA_Read_Ptr:0x%x, DMAReadIdx:%x \r\n",
        ...
        if (copy_to_user((void __user *)(Read_Data_Ptr + size_1), (pModDai_Block->pucVirtBufAddr + DMA_Read_Ptr), size_2))
        {
            ...
            printk("mtk_mod_dai_pcm_copy Fail 3 copy to user Read_Data_Ptr:%p, pucVirtBufAddr:%p, u4DMAReadIdx:0x%x , DMA_Read_Ptr:0x%x, read_size:%x", Read_Data_Ptr, pModDai_Block->pucVirtBufAddr, pModDai_Block->u4DMAReadIdx, DMA_Read_Ptr, read_size);
            return read_count << 2;
        }
...
static u64 mod_dai_pcm_dmamask = DMA_BIT_MASK(32);
...
static int mtk_asoc_mod_dai_pcm_new(struct snd_soc_pcm_runtime *rtd)
{
    ...
    {
        card->dev->dma_mask = &mod_dai_pcm_dmamask;
    }
...
static int mtk_afe_mod_dai_probe(struct snd_soc_platform *platform)
{
    ...
    AudDrv_Allocate_mem_Buffer(Soc_Aud_Digital_Block_MEM_MOD_DAI, MOD_DAI_MAX_BUFFER_SIZE);
