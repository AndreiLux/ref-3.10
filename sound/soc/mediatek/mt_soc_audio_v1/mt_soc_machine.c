                                     ...
                                     size_t count, loff_t *pos)
{
    ...
    char buffer[size];
    int n = 0;
    printk("mt_soc_ana_debug_read count = %d\n", count);
    AudDrv_Clk_On();
    audckbufEnable(true);
                                  ...
                                  size_t count, loff_t *offset)
{
    ...
    {
        printk("copy_from_user mt_soc_debug_write count = %d temp = %s\n", count, InputString);
    }
    temp = kstrdup(InputString, GFP_KERNEL);
    printk("copy_from_user mt_soc_debug_write count = %d temp = %s pointer = %p\n", count, InputString, InputString);
