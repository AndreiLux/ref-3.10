...
//                                                                                         
static int wwan_ipv6_get_lladdr(struct net_device *dev, struct in6_addr *addr, unsigned char banned_flags);
//                                                                                         
...
//                                                                                          
static int wwan_ipv6_get_lladdr(struct net_device *dev, struct in6_addr *addr,
		    unsigned char banned_flags)
{
	struct inet6_dev *idev;
	struct inet6_ifaddr *ifp;
	...
	if (idev) {
		...
		list_for_each_entry(ifp, &idev->addr_list, if_list) {
			if (ifp->scope == IFA_LINK &&
			    !(ifp->flags & banned_flags)) {
			    memcpy(lladdr_iid, ifp->addr.s6_addr + 8, 8);
				if (lladdr_iid[0] == 0x79 && lladdr_iid[1] == 0x5E)
				{
				    printk(KERN_DEBUG "[LGE_DATA][%s()] A LinkLocal Addr for wwan is Found.\n", __func__);
					*addr = ifp->addr;
					err = 0;
					break;
				}
...
static int ipv6_generate_eui64(u8 *eui, struct net_device *dev)
{
	...
	case ARPHRD_ETHER:
		//                                                                                         
		if (dev->name != NULL && strncmp(dev->name, "wwan", 4) == 0) {
			if (wwan_ipv6_get_lladdr(dev, &lladdr, IFA_F_TENTATIVE)) {
				get_random_bytes(eui, 8);
				printk(KERN_DEBUG "[LGE_DATA][%s()] A random eui64 is generated for %s.\n", __func__, dev->name);
				eui[0] = 0x79;
				eui[1] = 0x5E;
			} else {
				memcpy(eui, lladdr.s6_addr + 8, 8);
				printk(KERN_DEBUG "[LGE_DATA][%s()] Use existing eui64 value for %s.\n", __func__, dev->name);
			}
			return 0;
		}
		//                                                                                          
...
//                                                                                                        
    ...
    const char InterfaceNameToApply[5]="wwan";
    char CurrentInterfaceName[5]={0};//initializing
    ...
    strncpy(CurrentInterfaceName,dev->name,4);
...
//                                                                                                        
    ...
    if(strcmp(InterfaceNameToApply, CurrentInterfaceName) == 0){//In case of wwan, this patch will be applied bacause We should not impact to the Wi-Fi and so on.
	    addrconf_mod_timer(ifp, AC_DAD, LGE_DATA_WAITING_TIME_FOR_DAD_OF_LGU);
	    printk(KERN_DEBUG "[LGE_DATA][%s()] The waiting time for link-local DAD is set as [%d] milli-seconds in case of only wwan interface !", __func__, LGE_DATA_WAITING_TIME_FOR_DAD_OF_LGU*10);
    }else{
