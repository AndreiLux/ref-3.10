...
#if defined(CONFIG_ODIN_DEBUG_LMK_MEMINFO)
...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
	...
	for_each_process(tsk) {
		...
		if (selected) {
			...
			if ( (!strcmp( selected->comm, "com.lge.art:service" )) ||	\
				(!strcmp( selected->comm, "com.lge.art:general_service" )) )
				continue;
#endif
		}
		selected = p;
...
#endif
#ifdef CONFIG_ODIN_DEBUG_LMK_MEMINFO
