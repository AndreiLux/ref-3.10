...
static int inv_check_akm_self_test(struct inv_mpu_state *st)
{
	...
	if (result) {
		result = inv_i2c_single_write(st, REG_INT_PIN_CFG,
				st->plat_data.int_config);
		printk("[sh.jin] %s: result = [%d] @ %d LINES\n", __func__, result, __LINE__);
		return result;
	}
	if (COMPASS_ID_AK09911 == st->plat_data.sec_slave_id)
		mode = REG_AK09911_CNTL2;
...
#if 1 // AK09911 - self test
		if (COMPASS_ID_AK09911 == st->plat_data.sec_slave_id) {
			result = inv_secondary_read(REG_AK09911_STATUS1, 1, data);
		} else {
	...
	if ((data[0] & DATA_AKM_DRDY) == 0) {
		result = -EINVAL;
		printk("[sh.jin] %s: result = [%d] , data[0] = %d, @ %d LINES\n", __func__, result, data[0], __LINE__);
		goto AKM_fail;
	}
	...
	if (COMPASS_ID_AK09911 == st->plat_data.sec_slave_id) {
		result = inv_secondary_read(REG_AK09911_CNTL2, 1, &cntl);
		if (result)
			goto AKM_fail;
		if (0 == (cntl & DATA_AKM8963_BIT)) {
			x <<= DEF_ST_COMPASS_8963_SHIFT;
	...
	result = -EINVAL;
	printk("[sh.jin] %s: result = [%d] @ %d LINES\n", __func__, result, __LINE__);
	printk("[sh.jin] x/y/z = %d/%d/%d\n", x, y ,z);
	printk("[sh.jin] st_upper[X] = %d, st_lower[X] =%d\n", st->slave_compass->st_upper[X], st->slave_compass->st_lower[X]);
	printk("[sh.jin] st_upper[Y] = %d, st_lower[Y] =%d\n", st->slave_compass->st_upper[Y], st->slave_compass->st_lower[Y]);
	printk("[sh.jin] st_upper[Z] = %d, st_lower[Z] =%d\n", st->slave_compass->st_upper[Z], st->slave_compass->st_lower[Z]);
