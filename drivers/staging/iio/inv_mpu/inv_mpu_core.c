...
/*              */
	gpio_request(st->plat_data.irq_gpio, "inv_mpu");
...
static int inv_mpu_resume(struct device *dev)
{
	...
	if(mutex_is_locked(&st->suspend_resume_lock)) {
		mutex_unlock(&st->suspend_resume_lock);
	} else {
		pr_debug("inv_mpu: No suspend_resume_lock\n");
	}
