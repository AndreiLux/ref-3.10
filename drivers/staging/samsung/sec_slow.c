...
static DEFINE_PER_CPU(struct observer_cpuinfo, cpuinfo);
...
#define DEFAULT_FULL_EXIT_TIME (16 * DEFAULT_TIMER_RATE)
...
static void observer_timer_resched(void)
{
	...
	for_each_cpu(cpu, cpu_possible_mask) {
		struct observer_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
...
static unsigned int check_slow_mode(int cpu, unsigned int cur_mode, u64 now)
{
	...
	for_each_cpu_mask(i, hmp_slow_cpu_mask) {
		struct observer_cpuinfo *picpu = &per_cpu(cpuinfo, i);
...
static void observer_timer(unsigned long data)
{
	...
	u64 now;
	struct slow_mode_tunables *tunables = &slow_tunables;
	struct observer_cpuinfo *pcpu_representive = &per_cpu(cpuinfo, CPU_REPRESENTIVE);
	...
	/* check slow mode */
	spin_lock_irqsave(&tunables->slow_mode_lock, flags);
	...
	spin_unlock_irqrestore(&tunables->slow_mode_lock, flags);
...
{
	struct observer_cpuinfo *pcpu = &per_cpu(cpuinfo, freq->cpu);

	/* for slow mode */
	if (freq->cpu == SLOW_MODE_CPU) {
		if (freq->new == pcpu->cpufreq_max && !loadinfo.observer_enabled)
			return TIMER_START;
		else if (freq->new < pcpu->cpufreq_max && loadinfo.observer_enabled)
			return TIMER_STOP;
	}
