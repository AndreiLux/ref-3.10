...
struct sched_log {
	...
	atomic_t timer_log_idx[NR_CPUS];
	struct timer_log {
		unsigned long long time;
		unsigned int type;
		void *fn;
...
int __init sec_debug_init(void)
{
	...
	for(i=0; i<NR_CPUS; ++i) {
		atomic_set(&(psec_debug_log->task_log_idx[i]), -1);
		atomic_set(&(psec_debug_log->irq_log_idx[i]), -1);
		atomic_set(&(psec_debug_log->work_log_idx[i]), -1);
#ifdef CONFIG_SEC_DEBUG_TIMER_LOG
		atomic_set(&(psec_debug_log->timer_log_idx[i]), -1);
#endif
	}
...
void __sec_debug_task_log(int cpu, struct task_struct *task, char *msg)
{
	...
	i = atomic_inc_return(&psec_debug_log->task_log_idx[cpu]) & (SCHED_LOG_MAX - 1);
...
void __sec_debug_irq_log(unsigned int irq, void *fn, int en)
{
	...
	i = atomic_inc_return(&psec_debug_log->irq_log_idx[cpu]) & (SCHED_LOG_MAX - 1);
			  ...
			  struct work_struct *work, work_func_t f, int en)
{
	...
	i = atomic_inc_return(&psec_debug_log->work_log_idx[cpu]) & (SCHED_LOG_MAX - 1);
...
void __sec_debug_timer_log(unsigned int type, void *fn)
{
	...
	i = atomic_inc_return(&psec_debug_log->timer_log_idx[cpu]) & (SCHED_LOG_MAX - 1);
...
static int __init sec_subsys_log_setup(char *str)
{
	...
	sec_debug_subsys_set_reserved_out_buf((unsigned int )buf_base,SZ_2M);
out:
	return 0;
}
