			     ...
			     unsigned int show_backtrace_type)
{
	...
	char tmp_string[KSYM_SYMBOL_LEN + 30];
	...
	for (i = 0; i < backtrace_count; i++) {
		ion_get_backtrace_info(tracking_info, tmp_string, sizeof(tmp_string), i,
				       show_backtrace_type);
		pr_info(ION_DEBUG_INFO "%s", tmp_string);
	}
	return 1;
}
...
void ion_debug_show_basic_info_record(struct ion_record_basic_info *tracking_info)
{
	if (tracking_info != NULL) {
		pr_info(ION_DEBUG_INFO "===recordID.pid : %d\n", tracking_info->recordID.pid);
		pr_info(ION_DEBUG_INFO "===recordID.group_pid : %d\n",
			tracking_info->recordID.group_pid);
		pr_info(ION_DEBUG_INFO "===recordID.client_address : 0x%x\n",
			(unsigned int)tracking_info->recordID.client_address);
		pr_info(ION_DEBUG_INFO "===recordID.client: 0x%x\n",
			(unsigned int)tracking_info->recordID.client);
		pr_info(ION_DEBUG_INFO "===recordID.buffer : 0x%x\n",
			(unsigned int)tracking_info->recordID.buffer);
		pr_info(ION_DEBUG_INFO "===record type  : %d\n", tracking_info->record_type);
		pr_info(ION_DEBUG_INFO "===from_kernel  : %d\n", tracking_info->from_kernel);
		pr_info(ION_DEBUG_INFO "===allocate_backtrace : 0x%x\n",
			(unsigned int)tracking_info->allocate_backtrace);
		pr_info(ION_DEBUG_INFO "===allocate_map : 0x%x\n",
			(unsigned int)tracking_info->allocate_map);
		ion_debug_show_backtrace(tracking_info, ALLOCATE_BACKTRACE_INFO);
		pr_info(ION_DEBUG_INFO "===release_backtrace : 0x%x\n",
			(unsigned int)tracking_info->release_backtrace);
		pr_info(ION_DEBUG_INFO "===release_map : 0x%x\n",
...
void ion_debug_show_buffer_usage_record(struct ion_buffer_usage_record *buffer_usage_record)
{
	if (buffer_usage_record != NULL) {
		pr_info(ION_DEBUG_INFO "===========================================\n");
		pr_info(ION_DEBUG_INFO "===buffer usage record : %x\n",
			(unsigned int)buffer_usage_record);
		pr_info(ION_DEBUG_INFO "===buffer_usage_record.next : 0x%x\n",
			(unsigned int)buffer_usage_record->next);
		ion_debug_show_basic_info_record(&(buffer_usage_record->tracking_info));
		pr_info(ION_DEBUG_INFO "===buffer_usage_record.handle : 0x%x\n",
...
void ion_debug_show_address_usage_record(struct ion_address_usage_record *address_usage_record)
{
	if (address_usage_record != NULL) {
		pr_info(ION_DEBUG_INFO "===========================================\n");
		pr_info(ION_DEBUG_INFO "===address usage record : %x\n",
			(unsigned int)address_usage_record);
		pr_info(ION_DEBUG_INFO "===address_usage_record.next : 0x%x\n",
			(unsigned int)address_usage_record->next);
		ion_debug_show_basic_info_record(&(address_usage_record->tracking_info));
		pr_info(ION_DEBUG_INFO "===address_usage_record.address_type : %x\n",
			(unsigned int)address_usage_record->address_type);
		pr_info(ION_DEBUG_INFO "===address_usage_record.mapping_address : 0x%x\n",
			(unsigned int)address_usage_record->mapping_address);
		pr_info(ION_DEBUG_INFO "===address_usage_record.size : %x\n",
			address_usage_record->size);
		pr_info(ION_DEBUG_INFO "===address_usage_record.fd : 0x%x\n",
...
void ion_debug_show_fd_usage_record(struct ion_fd_usage_record *fd_usage_record)
{
	if (fd_usage_record != NULL) {
		pr_info(ION_DEBUG_INFO "===========================================\n");
		pr_info(ION_DEBUG_INFO "===fd usage record : %x\n", (unsigned int)fd_usage_record);
		pr_info(ION_DEBUG_INFO "===fd_usage_record.next : 0x%x\n",
			(unsigned int)fd_usage_record->next);
		ion_debug_show_basic_info_record(&(fd_usage_record->tracking_info));
		pr_info(ION_DEBUG_INFO "===fd_usage_record.fd : 0x%d\n", fd_usage_record->fd);
		pr_info(ION_DEBUG_INFO "===========================================\n");
	}
}
...
void ion_debug_show_client_usage_record(struct ion_client_usage_record *client_usage_record)
{
	if (client_usage_record != NULL) {
		pr_info(ION_DEBUG_INFO "===========================================\n");
		pr_info(ION_DEBUG_INFO "===client usage record : %x\n",
			(unsigned int)client_usage_record);
		pr_info(ION_DEBUG_INFO "===client_sage_record.next : 0x%x\n",
			(unsigned int)client_usage_record->next);
		ion_debug_show_basic_info_record(&(client_usage_record->tracking_info));
		pr_info(ION_DEBUG_INFO "===client_usage_record.fd : 0x%d\n",
...
void ion_debug_show_process_record(struct ion_process_record *process_record)
{
	if (process_record != NULL) {
		pr_info(ION_DEBUG_INFO "============================================\n");
		pr_info(ION_DEBUG_INFO "===process : %x\n", (unsigned int)process_record);
		pr_info(ION_DEBUG_INFO "===process.next : %x\n",
			(unsigned int)process_record->next);
		pr_info(ION_DEBUG_INFO "===process.count : %d\n", process_record->count);
		pr_info(ION_DEBUG_INFO "===process.pid : %d\n", process_record->pid);
		pr_info(ION_DEBUG_INFO "===process.group_id : %d\n", process_record->group_id);
		pr_info(ION_DEBUG_INFO "===process.address_using_list : %x\n",
			(unsigned int)process_record->address_using_list);
		pr_info(ION_DEBUG_INFO "===process.address_freed_list : %x\n",
			(unsigned int)process_record->address_freed_list);
		pr_info(ION_DEBUG_INFO "===process.fd_using_list : %x\n",
			(unsigned int)process_record->fd_using_list);
		pr_info(ION_DEBUG_INFO "===process.fd_freed_list : %x\n",
...
void ion_debug_show_buffer_record(struct ion_buffer_record *buffer_record)
{
	if (buffer_record != NULL) {
		pr_info(ION_DEBUG_INFO "===========================================\n");
		pr_info(ION_DEBUG_INFO "===buffer_record : %x\n", (unsigned int)buffer_record);
		pr_info(ION_DEBUG_INFO "===buffer_record.next : 0x%x\n",
			(unsigned int)buffer_record->next);
		pr_info(ION_DEBUG_INFO "===buffer_record.buffer : 0x%x\n",
			(unsigned int)buffer_record->buffer_address);
		pr_info(ION_DEBUG_INFO "===buffer_record.heap_type : 0x%d\n",
			(unsigned int)buffer_record->heap_type);
		pr_info(ION_DEBUG_INFO "===buffer_record.size : 0x%d\n", buffer_record->size);
		pr_info(ION_DEBUG_INFO "===buffer_record.buffer_using_list : 0x%x\n",
			(unsigned int)buffer_record->buffer_using_list);
		pr_info(ION_DEBUG_INFO "===buffer_record.buffer_freed_list : 0x%x\n",
			(unsigned int)buffer_record->buffer_freed_list);
		pr_info(ION_DEBUG_INFO "===buffer_record.address_using_list : 0x%x\n",
			(unsigned int)buffer_record->address_using_list);
		pr_info(ION_DEBUG_INFO "===buffer_record.address_freed_list : 0x%x\n",
			     ...
			     unsigned int show_backtrace_type)
{
	...
	if (backtrace == NULL) {
		pr_info(ION_DEBUG_ERROR "[ion_get_backtrace_info]ERROR backtrace is NULL\n");
		return NULL;
	}
...
void *ion_get_list(unsigned int record_type, void *record, unsigned int list_type)
{
	...
	if (record_type == LIST_BUFFER) {
		...
		case BUFFER_ALLOCATION_LIST:
			{
				pr_info(ION_DEBUG_INFO
					"[ion_get_list return]buffer using list %x\n",
					(unsigned int)buffer_record->buffer_using_list);
		...
		case BUFFER_FREE_LIST:
			{
				pr_info(ION_DEBUG_INFO
					"[ion_get_list return]buffer freed list %x\n",
					(unsigned int)buffer_record->buffer_freed_list);
				ion_debug_show_buffer_usage_record(buffer_record->
								   buffer_freed_list);
...
struct ion_buffer_record *ion_get_inuse_buffer_record(void)
{
	...
	pr_info(ION_DEBUG_INFO "[ion_get_inuse_buffer_record]return buffer_record  %x\n",
		(unsigned int)tmp_buffer);
	if (tmp_buffer != NULL) {
		/* pr_info(ION_DEBUG_INFO "[ion_get_inuse_buffer_record]return buffer %x buffer_size %d\n",tmp_buffer->buffer,tmp_buffer->buffer->size); */
...
struct ion_buffer_record *ion_get_freed_buffer_record(void)
{
	...
	pr_info(ION_DEBUG_INFO "[ion_get_freed_buffer_record]return buffer_record  %x\n",
		(unsigned int)tmp_buffer);
	if (tmp_buffer != NULL) {
		/* pr_info(ION_DEBUG_INFO "[ion_get_freed_buffer_record]return buffer %x buffer_size %d\n",tmp_buffer->buffer,tmp_buffer->buffer->size); */
		ion_debug_show_buffer_record(tmp_buffer);
	}
...
struct ion_process_record *ion_get_inuse_process_usage_record2(void)
{
	...
	if (tmp_process != NULL) {
		...
		pr_info(ION_DEBUG_INFO "[ion_get_inuse_process_record]return process %x\n",
			(unsigned int)tmp_process);
		ion_debug_show_process_record(tmp_process);
	} else {
		pr_info(ION_DEBUG_WARN
...
struct ion_process_record *ion_get_freed_process_record(void)
{
	...
	pr_info(ION_DEBUG_INFO "[ion_get_freed_process_record]return process_record  %p\n",
		tmp_process);
	if (tmp_process)
		ion_debug_show_process_record(tmp_process);
...
struct ion_client_usage_record *ion_get_inuse_client_record(void)
{
	...
	if (tmp_client != NULL) {
		pr_info(ION_DEBUG_INFO "[ion_get_inuse_client_record]return client %x\n",
			(unsigned int)tmp_client);
...
unsigned int get_kernel_backtrace(unsigned long *backtrace)
{
	...
	pr_info(ION_DEBUG_INFO "	     [get_kernel_backtrace] backtrace num: [%d]\n",
		trace.nr_entries);
	if (trace.nr_entries > 0) {
		for (i = 0; i < trace.nr_entries; i++) {
			pr_info(ION_DEBUG_INFO "bactrace[%d] : %x  ", i,
	...
	while (current_node != NULL) {
		if ((search_type == SEARCH_PROCESS_PID) && (node_type == LIST_PROCESS)
			...
			pr_info(ION_DEBUG_INFO
				...
				(unsigned int)buffer_tmp->buffer_address, (unsigned int)data);
			...
			pr_info(ION_DEBUG_INFO
				...
				record_ID_tmp->recordID.pid,
				record_ID_tmp->recordID.client_address);
			...
			pr_info(ION_DEBUG_ERROR "            [find_node_in_list]Error!!!\n");
		}
...
void *allocate_record(unsigned int type, ion_sys_record_t *record_param)
{
	...
	default:
		{
			pr_info(ION_DEBUG_ERROR "[ERROR!!]allocate_record wrong type %d", type);
			break;
		}
	}
	return NULL;
...
char *get_string(char *string_name, unsigned int len, StringTable *table)
{
	...
	if (entry != NULL) {
		/* pr_info("     [get_string] find string in string hash table : addres 0x[%x]%s\n",entry->name,entry->name,entry->string_len); */
		return entry->name;
	} else {
		/* pr_info("     [get_string]can't get string in string hash table\n"); */
		...
		table->count++;
		pr_alert("string_struct_size [%d] string_size[%d]\n", string_struct_size,
			 string_size);
		/* pr_info("        [get_string]create new node in string hash table: address 0x[%x]%s size %d\n",entry->name,entry->name,len); */
		return entry->name;
	}
}
		...
		case HASH_NODE_BUFFER:
			{
				...
				if (entry != NULL) {
					/* pr_info("        [get_record][BUFFER]find the same entry\n "); */
					entry->reference++;
				} else {
					/* pr_info("        [get_record][BUFFER] can't find entry in hash table and create new entry\n"); */
					entry =
		...
		case HASH_NODE_USER_BACKTRACE:
			{
				...
				entry =
				    find_entry(&gUserBtTable, slot, (void *)param->backtrace,
					       param->backtrace_num);
				if (entry != NULL) {
					/* pr_info("        [get_record][USER_BACKTRACE]find the same entry entry 0x%x entry num %d\n",entry->object,entry->numEntries); */
					...
					gUserBtTable.count++;
					/* pr_info("        [get_record][USER_BACKTRACE]create new entry>object  0x%x entry num %d souce %x source2 %x\n",entry->object,entry->numEntries,&param->backtrace[0],&(param->backtrace[0])); */
		...
		case HASH_NODE_KERNEL_BACKTRACE:
			{
				...
				entry =
				    find_entry(&gKernelBtTable, slot, (void *)param->backtrace,
					       param->backtrace_num);
				if (entry != NULL) {
					/* pr_info("        [get_record][KERNEL_BACKTRACE]find the same entry entry 0x%x entry num %d\n",entry->object,entry->numEntries); */
					entry->reference++;
				} else {
					entry =
					    kmalloc(sizeof(ObjectEntry) +
					...
					/* pr_info("        [get_record][KERNEL_BACKTRACE]create new entry>object  0x%x entry num %d\n",entry->object,entry->numEntries); */
				}
				return entry;
			}
		case HASH_NODE_USER_MAPPING:
			{
				...
				entry =
				    find_mapping_entry(&gUserMappingTable, slot,
						       ...
						       param->backtrace_num);
				if (entry != NULL) {
					/* pr_info("        [get_record][USER_MAPPING_INFO]find the same entry entry 0x%x entry num %d\n",entry->object,entry->numEntries); */
					entry->reference++;
				} else {
					entry =
					    kmalloc(sizeof(ObjectEntry) +
					...
					/* pr_info("        [get_record][USER_MAPPING]create new entry>object  0x%x entry num %d source %x source2 %x\n",entry->object,entry->numEntries,&param->backtrace[0],&(param->backtrace[0])); */
				}
				return entry;
		...
		case HASH_NODE_KERNEL_SYMBOL:
			{
				...
				if (entry != NULL) {
					/* pr_info("        [get_record][KERNEL_SYMBOL]find the same entry entry 0x%x entry num %d\n",entry->object,entry->numEntries); */
					entry->reference++;
				} else {
				  ...
				  unsigned int node_type, unsigned int from_kernel)
{
	...
	if (new_node != NULL) {
		...
		case NODE_BUFFER:
			{
				...
				/* pr_info("[create_new_record_into_list]new_node %x buffer_using_list%x\n",new_node,buffer->buffer_using_list); */
				/* add new node into allocate list */
		...
		case NODE_MMAP:
			{
				...
				tmp->mapping_address = record_param->address;
				tmp->size = record_param->length;
				/* pr_info("[create_new_record_into_list]new_node %x address__using_list%x\n",new_node,buffer->address_using_list); */
				/* add new node into allocate list */
		...
		case NODE_CLIENT:
			{
				...
				/* pr_info("[ION_FUNCTION_OPEN]new_node %x client_using_list%x\n",new_node,client_using_list); */
				if (!from_kernel) {
					tmp->fd = record_param->fd;
				}
				/* add new node into allocate list */
		...
		pr_info("[create_new_record_into_list] can't get new node type\n");
	}
...
int record_ion_info(int from_kernel, ion_sys_record_t *record_param)
{
		...
		record_param->group_id, from_kernel, record_param->action);
	{
		...
		case ION_FUNCTION_CREATE_CLIENT:
			{
				...
				pr_info(ION_DEBUG_TRACE
					"    [%d][%d][ION_FUNCTION_OPEN(%d)] fd %d\n",
				...
				new_node =
				    ...
				    create_new_record_into_list(record_param, NULL, NODE_CLIENT,
								from_kernel);

				/* find process record in list */
				if (from_kernel == 0) {
					if ((process_created_list == NULL)
					    && (process_destroyed_list == NULL)) {
						mutex_init(&process_lifecycle_mutex);
						pr_info(ION_DEBUG_TRACE
							"    [%d][%d][ION_FUNCTION_OPEN(%d)]init process_lifecycle_mutex\n",
					...
					if (process_record == NULL) {
						...
						process_record =
						    (struct ion_process_record *)
						    allocate_record(LIST_PROCESS, record_param);
						if (process_record != NULL) {
							pr_info(ION_DEBUG_TRACE
								"    [%d][%d][ION_FUNCTION_OPEN(%d)]create new process record 0x%x in process [%d] process created list %x\n",
								record_param->pid,
								record_param->group_id, from_kernel,
								(unsigned int)process_record,
								(int)record_param->pid,
								(unsigned int)process_created_list);

						} else {
							pr_info(ION_DEBUG_ERROR
								"    [%d][%d][ION_FUNCTION_OPEN(%d)] ERROR !!!process %d can't get enough for LIST_PROCESS structure\n",
								record_param->pid,
								record_param->group_id, from_kernel,
								(int)record_param->pid);
						...
						process_record->count++;
						pr_info(ION_DEBUG_TRACE
							"    [%d][%d][ION_FUNCTION_OPEN(%d)]found process record 0x%x in process [%d] process->count %d\n",
							record_param->pid, record_param->group_id,
							from_kernel, (unsigned int)process_record,
							record_param->pid, process_record->count);
					...
					new_node2 =
					    ...
					    create_new_record_into_process(record_param,
									   process_record, NODE_FD,
									   from_kernel);
					pr_info(ION_DEBUG_TRACE
						"    [%d][%d][ION_FUNCTION_OPEN(%d)]done new fd node is %x insert node into fd using list %x \n",
						record_param->pid, record_param->group_id,
						from_kernel, (unsigned int)new_node2,
						(unsigned int)process_record->fd_using_list);
				}
				pr_info(ION_DEBUG_TRACE
					"    [%d][%d][ION_FUNCTION_OPEN(%d)] DONE  new_node %x client_using_list %x\n",
					record_param->pid, record_param->group_id, from_kernel,
					(unsigned int)new_node, (unsigned int)client_using_list);
				break;
			}
		...
		case ION_FUNCTION_SHARE:
			{
				...
				pr_info(ION_DEBUG_TRACE
					"    [%d][%d][ION_FUNCTION_SHARE(%d)]input buffer: 0x%x fd %d\n",
					record_param->pid, record_param->group_id, from_kernel,
				...
				if ((process_created_list == NULL)
					...
					mutex_init(&process_lifecycle_mutex);
					pr_info(ION_DEBUG_TRACE
						"    [%d][%d][ION_FUNCTION_SHARE(%d)]init process_lifecycle_mutex\n",
						record_param->pid, record_param->group_id,
						from_kernel);
				}
				mutex_lock(&process_lifecycle_mutex);
				...
				if (process_record == NULL) {
					pr_info(ION_DEBUG_TRACE
						"    [%d][%d][ION_FUNCTION_SHARE(%d)] can't found corresponding process %d in process created list %x",
						record_param->pid, record_param->group_id,
						from_kernel, record_param->pid,
						(unsigned int)process_created_list);
					...
					if (from_kernel == 1) {
						...
						if (process_record == NULL) {
							...
							process_record =
							    (struct ion_process_record *)
							    allocate_record(LIST_PROCESS,
									    record_param);
							if (process_record != NULL) {
								pr_info(ION_DEBUG_TRACE
									"    [%d][%d][ION_FUNCTION_SHARE(%d)]create new process record 0x%x in process [%d] count %d process created list %x\n",
									record_param->pid,
									record_param->group_id,
									from_kernel,
									(unsigned int)
									process_record,
									(int)record_param->group_id,
									process_record->count,
									(unsigned int)
									process_created_list);
								...
								pr_info(ION_DEBUG_ERROR
									"    [%d][%d][ION_FUNCTION_SHARE(%d)] ERROR !!!process [%d] can't get enough memory to create LIST_PROCESS strucutre\n",
									record_param->pid,
									record_param->group_id,
									from_kernel,
									(int)record_param->
						...
						pr_info(ION_DEBUG_ERROR
							"    [%d][%d][ION_FUNCTION_SHARE(%d)]Error !!!!Userspace should find correspond process to  buffer 0x%x in buffer created list %x",
							record_param->pid, record_param->group_id,
							from_kernel,
							(unsigned int)record_param->buffer,
							(unsigned int)buffer_created_list);
				...
				new_node =
				    (struct ion_fd_usage_record *)
				    create_new_record_into_process(record_param, process_record,
								   NODE_FD, from_kernel);
				pr_info(ION_DEBUG_TRACE
					"    [%d][%d][ION_FUNCTION_SHARE(%d)]done new  node is %x insert node into fd using list %x \n",
					record_param->pid, record_param->group_id, from_kernel,
					(unsigned int)new_node,
					(unsigned int)process_record->fd_using_list);
				break;
		...
		default:
			pr_info(ION_DEBUG_ERROR "[ERROR]record_ion_info error action\n");
		}
