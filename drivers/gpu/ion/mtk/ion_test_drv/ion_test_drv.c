...
/*
    ...
    pr_info("ion_test_drv: ioctl(%d)\n", arg);
    pTask = find_task_by_pid_ns(arg, &init_pid_ns);
    pr_info("ion_test_drv: Current task is 0x%08X pid=%d\n", (unsigned int) current, current->pid);
    if (pTask)
	pr_info("ion_test_drv: Target task is 0x%08X pid=%d\n", (unsigned int) pTask, pTask->pid);
    else
	pr_info("ion_test_drv: Target task is NULL\n");
*/
	return 0;
}
