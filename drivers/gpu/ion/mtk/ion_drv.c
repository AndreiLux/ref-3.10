			  ...
			  int from_kernel)
{
	...
	case ION_SYS_RECORD:
		{
			...
			if (!from_kernel && (Param.record_param.backtrace_num > 0)) {
				/* pr_info(ION_DEBUG_INFO "[ion_sys_ioctl]prepare
				 * to copy mapping info from userspace\n"); */
				for (i = 0; i < Param.record_param.backtrace_num; i++) {
					/* pr_info(ION_DEBUG_INFO "[ion_sys_ioctl]
					 * mapping[%d] name 0x%x address 0x%x size %d\n",
					 * i,Param.record_param.mapping_record[i].name,
					 * Param.record_param.mapping_record[i].address,
					 * Param.record_param.mapping_record[i].size); */
					unsigned int string_len = 0;
					int ret;
					if (Param.record_param.mapping_record[i].size <= 0)
						continue;
					string_len =
						   strlen_user((void __user *)Param.
							record_param.mapping_record[i].
							name);
					if (string_len <= 0) {
			...
			pr_info(ION_DEBUG_TRACE
				"[ION_FUNC%d][ion_sys_record] PID :%d client 0x%x from_kernel [%d] backtrace[%d]\n",
				Param.record_param.action, Param.record_param.pid,
				(unsigned int)Param.record_param.client, from_kernel,
				Param.record_param.backtrace_num);
			if (Param.record_param.handle != NULL) {
				...
				if (Param.record_param.buffer != NULL) {
					pr_info(ION_DEBUG_TRACE
						"[ION_FUNC%d][ion_sys_record]BUFFER :[%x] size :[%d]\n",
						Param.record_param.action,
						(unsigned int)Param.record_param.buffer,
						Param.record_param.buffer->size);
				} else {
					pr_info(ION_DEBUG_INFO "[ION_FUNC%d]buffer is NULL\n",
						Param.record_param.action);
				}
			}
			record_ion_info(from_kernel, &Param.record_param);
			pr_info(ION_DEBUG_TRACE "[ION_FUNC%d][ion_sys_ioctl]DONE\n",
				Param.record_param.action);
			break;
		}
	...
	default:
		pr_info(ION_DEBUG_ERROR "[ion_sys_ioctl]: Error. Invalid command.\n");
		ret = -EFAULT;
		break;
	}
	if (from_kernel)
