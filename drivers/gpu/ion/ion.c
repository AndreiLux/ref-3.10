...
static int ion_handle_add(struct ion_client *client, struct ion_handle *handle)
{
	...
	do {
		int id;
		rc = __idr_pre_get(&client->idr, GFP_KERNEL);
		if (!rc)
			return -ENOMEM;
		rc = __idr_get_new_above(&client->idr, handle, 1, &id);
		handle->id = id;
	} while (rc == -EAGAIN);
                                       ...
                                       enum dma_data_direction dir)
{
	...
	for (i = 0; i < pages; i++) {
		...
		//	__dma_page_cpu_to_dev(page, 0, PAGE_SIZE, dir);
            arm_dma_ops.sync_single_for_device(NULL, pfn_to_dma(NULL, page_to_pfn(page)), PAGE_SIZE, dir);
