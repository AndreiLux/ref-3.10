...
#define KBASE_VE_MEMORY_PER_PROCESS_LIMIT       (1024 * 1024 * 1024UL)	/* 1024MB */
#define KBASE_VE_MEMORY_OS_SHARED_MAX           (1280 * 1024 * 1024UL)	/* 1280MB */
...
static inline void kbase_platform_on(struct kbase_device *kbdev)
{
	...
	if (kbdev->clk) {
		if (unlikely(clk_prepare_enable(kbdev->clk)))
			KBASE_DEBUG_PRINT_ERROR(KBASE_PM, "Failed to enable clk");
	}
...
{
	if (kbdev->clk)
		clk_disable_unprepare(kbdev->clk);
...
kbase_platform_config k3v3_platform_config = {
