...
static int kbase_platform_device_probe(struct platform_device *pdev)
{
	...
	(CONFIG_CHROMEOS || CONFIG_MALI_PLATFORM_K3V3)
...
/** Shutdown callback from the OS.
 *
 * This is called by Linux when the system shutdown.
 *
 * @param dev  The device to shutdown
 *
 */
static void kbase_device_shutdown(struct platform_device *pdev)
{
	struct kbase_device *kbdev = to_kbase_device(&pdev->dev);
	...
	if (!pm_runtime_status_suspended(kbdev->osdev.dev))
		kbase_device_runtime_suspend(&pdev->dev);
#endif
...
#ifdef CONFIG_PM_RUNTIME
	if (!pm_runtime_status_suspended(kbdev->osdev.dev))
		kbase_device_runtime_suspend(dev);
#endif
...
#ifdef CONFIG_PM_RUNTIME
	if (!pm_runtime_status_suspended(kbdev->osdev.dev))
		kbase_device_runtime_resume(dev);
#endif
...
static struct platform_driver kbase_platform_driver = {
	...
	.shutdown = kbase_device_shutdown,
	.driver = {
		   .name = kbase_drv_name,
		   .owner = THIS_MODULE,
		   .pm = &kbase_pm_ops,
