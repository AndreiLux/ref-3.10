...
static int gpu_set_clock(struct exynos_context *platform, int clk)
{
	...
	if (g3d_rate != g3d_rate_prev) {
		...
		/*restore parent*/
		ret = clk_set_parent(mout_g3d, mout_g3d_pll);
		if (ret < 0) {
			GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "%s: failed to clk_set_parent [mout_g3d]\n", __func__);
			goto err;
		}
...
static int gpu_get_clock(kbase_device *kbdev)
{
	...
	fin_pll = clk_get(kbdev->osdev.dev, "fin_pll");
	...
	aclk_g3d = clk_get(kbdev->osdev.dev, "aclk_g3d");
	if (IS_ERR(aclk_g3d)) {
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "%s: failed to clk_get [aclk_g3d]\n", __func__);
		return -1;
	}

	dout_aclk_g3d = clk_get(kbdev->osdev.dev, "dout_aclk_g3d");
	if (IS_ERR(dout_aclk_g3d)) {
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "%s: failed to clk_get [dout_aclk_g3d]\n", __func__);
		return -1;
	}

	mout_g3d_pll = clk_get(kbdev->osdev.dev, "mout_g3d_pll");
	if (IS_ERR(mout_g3d_pll)) {
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "%s: failed to clk_get [mout_g3d_pll]\n", __func__);
		return -1;
	}

	mout_g3d = clk_get(kbdev->osdev.dev, "mout_g3d");
	if (IS_ERR(mout_g3d)) {
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "%s: failed to clk_get [mout_g3d]\n", __func__);
		return -1;
	}

	mout_bus_pll_g3d = clk_get(kbdev->osdev.dev, "mout_bus_pll_g3d");
	if (IS_ERR(mout_bus_pll_g3d)) {
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "%s: failed to clk_get [mout_bus_pll_g3d]\n", __func__);
		return -1;
	}
