/* drivers/gpu/t6xx/kbase/src/platform/gpu_control_exynos7420.c
 *
...
{
#if 0
	return ((__raw_readl(EXYNOS7420_G3D_STATUS) & EXYNOS_INT_LOCAL_PWR_EN) == EXYNOS_INT_LOCAL_PWR_EN) ? 1 : 0;
#else
	return 1;
#endif
}
...
int gpu_set_clock(struct exynos_context *platform, int freq)
{
	...
	if (g3d_rate != g3d_rate_prev) {
		...
		if (ret < 0) {
			GPU_LOG(DVFS_ERROR, "1. failed to clk_set_parent [mout_g3d_pll]\n");
			goto err;
		}
		/*change g3d pll*/
		ret = clk_set_rate(platform->fout_g3d_pll, g3d_rate);
		if (ret < 0) {
			GPU_LOG(DVFS_ERROR, "2. failed to clk_set_rate [fout_g3d_pll]\n");
			goto err;
		}
		/*restore parent*/
		ret = clk_set_parent(platform->mout_g3d_pll, platform->fout_g3d_pll);
		if (ret < 0) {
			GPU_LOG(DVFS_ERROR, "3. failed to clk_set_parent [mout_g3d_pll]\n");
			goto err;
		}
		/*restore parent*/
		ret = clk_set_parent(platform->mout_g3d, platform->mout_g3d_pll);
		if (ret < 0) {
			GPU_LOG(DVFS_ERROR, "4. failed to clk_set_parent [mout_g3d]\n");
			goto err;
		}
...
static int gpu_get_clock(kbase_device *kbdev)
{
	...
	platform->fout_g3d_pll = clk_get(kbdev->dev, "fout_g3d_pll");
	...
	platform->mout_g3d = clk_get(kbdev->dev, "mout_g3d");
	if (IS_ERR(platform->mout_g3d_pll)) {
		GPU_LOG(DVFS_ERROR, "failed to clk_get [mout_g3d]\n");
		return -1;
	}

	platform->mout_bus_pll_g3d = clk_get(kbdev->dev, "mout_bus_pll_g3d");
	if (IS_ERR(platform->mout_g3d_pll)) {
		GPU_LOG(DVFS_ERROR, "failed to clk_get [mout_bus_pll_g3d]\n");
		return -1;
	}
