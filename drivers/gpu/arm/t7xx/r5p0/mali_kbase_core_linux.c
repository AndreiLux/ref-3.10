...
static int kbase_platform_device_probe(struct platform_device *pdev)
{
	...
	kbdev->memory_profile_switch_val = 1;
	kbdev->memory_profile_switch_file = debugfs_create_bool("enable",
								S_IFREG | S_IRUGO | S_IWUSR,
								kbdev->memory_profile_directory,
								&kbdev->memory_profile_switch_val);
	if (NULL == kbdev->memory_profile_switch_file) {
		dev_err(kbdev->dev, "Couldn't create mali mem debugfs switch file\n");
		goto out_reg_unmap;
	}
#endif /* CONFIG_DEBUG_FS */
...
static int kbase_common_device_remove(struct kbase_device *kbdev)
{
	...
	kbase_device_term(kbdev);
#ifdef CONFIG_DEBUG_FS
  debugfs_remove(kbdev->memory_profile_switch_file);
	debugfs_remove(kbdev->memory_profile_directory);
	debugfs_remove(kbdev->mali_debugfs_directory);
#endif /* CONFIG_DEBUG_FS */
#ifdef CONFIG_MALI_NO_MALI
