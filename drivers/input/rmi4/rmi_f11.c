...
void suspendresumeE(void);
static void suspendresumeF(void);
static bool suspendresume = false;

void suspendresumeE(void)	{ suspendresume = true; };
static void suspendresumeF(void)	{ suspendresume = false; };
...
/* Setting ads param Max x,y position to Ctrl register */
	instance_data->dev_controls.ctrl0_9->sensor_max_x_pos = ODIN_LCD_MAX_X-1;
	instance_data->dev_controls.ctrl0_9->sensor_max_y_pos = ODIN_LCD_MAX_Y-1;
	//sensor->axis_align.flip_y = 1;		// flipping Y axis
	sensor->axis_align.flip_y = 0;		// flipping Y axis
	...
	//printk("%s max_x=%d   max_y=%d\n",__func__,device_x_max,device_y_max);	
...
static int rmi_f11_initialize(struct rmi_function_container *fc)
{
	...
	*/
	...
	for (i = 0; i < (f11->dev_query.nbr_of_sensors + 1); i++) {
		...
		//Test touch register setting _ writing Max x,y position 
		int buf_x, buf_y;
		buf_x = ODIN_LCD_MAX_X-1;
		buf_y = ODIN_LCD_MAX_Y-1;
	
		rc = rmi_write_block(rmi_dev,control_base_addr + F11_CTRL_SENSOR_MAX_X_POS_OFFSET,(u8 *)&buf_x,sizeof(buf_x));
		...
		rc = rmi_write_block(rmi_dev,control_base_addr + F11_CTRL_SENSOR_MAX_Y_POS_OFFSET,(u8 *)&buf_y,sizeof(buf_y));
		...
		//printk("%s max_x=%d  max_y=%d\n",__func__,max_x_pos,max_y_pos);
#if 0
// Setting Reporting mode 
    	f11->dev_controls.ctrl0_9->reporting_mode = 0x001;
...
int rmi_f11_attention(struct rmi_function_container *fc, u8 *irq_bits)
{
    ...
    int error;
    int i,rc;
    /* touch register setting _ writing Max x,y position */
