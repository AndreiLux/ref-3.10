...
#ifdef CONFIG_OF
static int s7020_parse_dt(struct device *dev,
						struct rmi_device_platform_data *pdata)
{
	...
	error = of_property_read_u32(np, "s7020,attn_gpio", &pdata->attn_gpio);
	error = of_property_read_u32(np, "s7020,odin_madk", &pdata->odin_madk);

	pr_info("%s: attn_gpio = %d, odin_madk =%d\n",__func__,
		pdata->attn_gpio, pdata->odin_madk);
				  ...
				  const struct i2c_device_id *id)
{
    ...
    of_id = of_match_device(touch_i2c_dt_ids,&client->dev);
	...
	if (&client->dev.of_node) {
		error = s7020_parse_dt(&client->dev, pdata);
		if (error != 0)
			return error;
	}
#endif
	...
	data->phys = rmi_phys;
	pr_info("[TouchIC] irq=%d, leveltrigger=%d,irq_flag = %x\n",
				data->irq,pdata->level_triggered,data->irq_flags);
	rmi_phys->data = data;
...
static struct i2c_driver rmi_i2c_driver = {
	.driver = {
		.owner	= THIS_MODULE,
		.name	= "rmi_i2c",
		.of_match_table = of_match_ptr(touch_i2c_dt_ids),    /* for DT */
	},
	.id_table	= rmi_id,
	.probe		= rmi_i2c_probe,
...
static int rmi_i2c_init(void)
{
	int odin_revision = odin_pmic_revision_get();
#ifdef CONFIG_ARABICA_CHECK_REV
	if (odin_revision < MADK_V1_2  || odin_revision == MADK_V1_3)
