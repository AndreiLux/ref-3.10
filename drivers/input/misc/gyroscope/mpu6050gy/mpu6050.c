...
/* static struct i2c_board_info i2c_mpu6050 __initdata = {
	I2C_BOARD_INFO(MPU6050_DEV_NAME, (MPU6050GY_I2C_SLAVE_ADDR >> 1)) }; */
...
#define GYRO_LOG(fmt, args...)    pr_info(GYRO_TAG fmt, ##args)
/*----------------------------------------------------------------------------*/
/*
...
static ssize_t show_chipinfo_value(struct device_driver *ddri, char *buf)
{
	...
	/* struct mpu6050_i2c_data *obj = i2c_get_clientdata(client); */
	char strbuf[MPU6050GY_BUFSIZE];
	if (NULL == client) {
		GYRO_ERR("i2c client is null!!\n");
		return 0;
	}

	MPU6050_ReadChipInfo(client, strbuf, MPU6050GY_BUFSIZE);
...
static ssize_t show_sensordata_value(struct device_driver *ddri, char *buf)
{
	...
	MPU6050_ReadGyroData(client, strbuf, MPU6050GY_BUFSIZE);
	return snprintf(buf, PAGE_SIZE, "%s\n", strbuf);
}
		    ...
		    void *buff_out, int size_out, int *actualout)
{
	...
	case SENSOR_GET_DATA:
		if ((buff_out == NULL) || (size_out < sizeof(hwm_sensor_data))) {
			...
			err = MPU6050_ReadGyroData(priv->client, buff, MPU6050GY_BUFSIZE);
			if (!err) {
...
static long mpu6050_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	char strbuf[MPU6050GY_BUFSIZE] = { 0 };
	s16 *SMTdata;
	void __user *data;
	long err = 0;
	int copy_cnt = 0;
	...
	case GYROSCOPE_IOCTL_READ_SENSORDATA_RAW:
		...
		MPU6050_ReadGyroDataRaw(client, strbuf, MPU6050GY_BUFSIZE);
		if (copy_to_user(data, strbuf, sizeof(strbuf))) {
			err = -EFAULT;
			break;
		}
	...
	case GYROSCOPE_IOCTL_READ_TEMPERATURE:
		...
		MPU6050_ReadTemperature(client, strbuf, MPU6050GY_BUFSIZE);
		if (copy_to_user(data, strbuf, sizeof(strbuf))) {
			err = -EFAULT;
			break;
		}
	...
	case GYROSCOPE_IOCTL_GET_POWER_STATUS:
		...
		MPU6050_ReadPowerStatus(client, strbuf, MPU6050GY_BUFSIZE);
		if (copy_to_user(data, strbuf, sizeof(strbuf))) {
			err = -EFAULT;
			break;
		}
...
#ifdef MPU6050_ACCESS_BY_GSE_I2C
	obj->hw->addr = MPU6050GY_I2C_SLAVE_ADDR;	/* mtk i2c not allow to probe two same address */
#endif
