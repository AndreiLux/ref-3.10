...
/* TODO : Currently allow setting hmp just only touch device
 * If you want to apply to other devices, remove below code.
 */
	...
	if (enable == dvfs->hmp_boosted) {
		DVFS_DEV_DBG(DBG_MISC, data->dev, "%s : HMP already %s\n",
	...
	if (retval < 0) {
		dev_err(data->dev, "%s : Fail to %s HMP[%d]\n",
			__func__, enable ? "Enabled" : "Disabled", retval);
		return;
	}

	dvfs->hmp_boosted = enable;
	DVFS_DEV_DBG(DBG_DVFS, data->dev, "%s : HMP %s\n",
		__func__, enable ? "Enabled" : "Disabled");
}
#endif
	...
	case BOOSTER_MODE_ON:
		...
		if (!dvfs->lock_status && !dvfs->short_press) {
			schedule_delayed_work(&dvfs->dvfs_chg_work,
				msecs_to_jiffies(dvfs->times[dvfs->level].chg_time));
	...
	case BOOSTER_MODE_OFF:
		if (dvfs->short_press) {
			...
			schedule_delayed_work(&dvfs->dvfs_off_work,
								msecs_to_jiffies(dvfs->times[dvfs->level].off_time));
			goto out;
		}
		if (dvfs->lock_status)
			schedule_delayed_work(&dvfs->dvfs_off_work,
...
DECLARE_DVFS_DELAYED_WORK_FUNC(CHG, TOUCH)
{
	...
	if (!dvfs->times[dvfs->level].pri_time)
		dvfs->pri_excuted = false;
	...
	case BOOSTER_LEVEL5:
		if (dvfs->pri_excuted) {
			...
			schedule_delayed_work(&dvfs->dvfs_chg_work,
				msecs_to_jiffies(dvfs->times[BOOSTER_LEVEL5].chg_time - dvfs->times[BOOSTER_LEVEL5].pri_time));
			dvfs->pri_excuted = true;
			DVFS_DEV_DBG(DBG_DVFS, data->dev, "%s : DVFS CHANGED [level %d, start time[%d]]\n",
				__func__, dvfs->level, dvfs->times[BOOSTER_LEVEL5].chg_time - dvfs->times[BOOSTER_LEVEL5].pri_time);
		}
	break;
	case BOOSTER_LEVEL9:
		SET_HMP(data, BOOSTER_DEVICE_TOUCH, true);
...
DECLARE_DVFS_WORK_FUNC(SET, TOUCH)
{
	...
	case BOOSTER_MODE_ON:
		...
		if (dvfs->times[dvfs->level].pri_time) {
			schedule_delayed_work(&dvfs->dvfs_chg_work,
								msecs_to_jiffies(dvfs->times[dvfs->level].pri_time));
			if (dvfs->pri_excuted)
				dvfs->pri_excuted = false;
		} else {
			schedule_delayed_work(&dvfs->dvfs_chg_work,
		...
		DVFS_DEV_DBG(DBG_DVFS, data->dev, "%s : DVFS ON [level %d][start time [%d]]\n",
			__func__, dvfs->level, dvfs->times[dvfs->level].pri_time ?: dvfs->times[dvfs->level].chg_time);
...
DECLARE_DVFS_WORK_FUNC(SET, PEN)
{
	...
	case BOOSTER_MODE_ON:
		...
		if (dvfs->level == BOOSTER_LEVEL9)
			schedule_delayed_work(&dvfs->dvfs_chg_work,
							msecs_to_jiffies(BOOSTER_DEBUG_CHG_TIME));
		else
			schedule_delayed_work(&dvfs->dvfs_chg_work,
							msecs_to_jiffies(dvfs->times[dvfs->level].chg_time));
		dvfs->lock_status = true;
...
void change_booster_level_for_tsp(unsigned char level)
{
	...
	if (!dvfs->initialized) {
		DVFS_DEV_DBG(DBG_DVFS, data->dev, "%s: Dvfs is not initialized\n",
			__func__);
		return;
	}
...
void change_booster_level_for_tsk(unsigned char level)
{
...
void input_booster_send_event(unsigned int code, int value)
{
	...
	if (!test_bit(code, g_data->keybit))
		return;
...
static void	input_booster_lookup_freqs(struct booster_dvfs *dvfs)
{
	...
	for (i = 0; i < BOOSTER_LEVEL_MAX; i++) {
		...
		dev_info(data->dev, "%s: %s's LEVEL%d cpu %d kfc %d mif %d int %d\n",
			__func__, dvfs->name, i, dvfs->freqs[i].cpu_freq,
			dvfs->freqs[i].kfc_freq, dvfs->freqs[i].mif_freq, dvfs->freqs[i].int_freq);
	}
	dev_info(data->dev, "#############################################################\n");
}
...
static void	input_booster_lookup_times(struct booster_dvfs *dvfs)
{
	...
	dev_info(data->dev, "#############################################################\n");
	for (i = 0; i < BOOSTER_LEVEL_MAX; i++) {
		if (!dvfs->times[i].chg_time
			&& !dvfs->times[i].off_time
			&& !dvfs->times[i].pri_time)
			continue;

		dev_info(data->dev, "%s: %s's LEVEL%d chg %d off %d pri %d\n",
			__func__, dvfs->name, i, dvfs->times[i].chg_time,
			dvfs->times[i].off_time, dvfs->times[i].pri_time);
	}
	dev_info(data->dev, "#############################################################\n");
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	struct input_booster *data =
		container_of(dvfs, struct input_booster, dvfses[dvfs->device_type]);
	int level;
	unsigned long cpu_freq, kfc_freq, mif_freq, int_freq;

	if (sscanf(buf, "%d%lu%lu%lu%lu",
		&level, &cpu_freq, &kfc_freq, &mif_freq, &int_freq) != 6) {
		dev_err(data->dev, "### Keep this format : [level cpu_freq kfc_freq mif_freq int_freq] (Ex: 1 1600000 0 1500000 667000 333000 ###\n");
		goto out;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	struct input_booster *data =
		container_of(dvfs, struct input_booster, dvfses[dvfs->device_type]);
	int level;
	unsigned long chg_time, off_time, pri_time;

	if (sscanf(buf, "%d%lu%lu%lu",
		&level, &chg_time, &off_time, &pri_time) != 4) {
		dev_err(data->dev, "### Keep this format : [level chg_time off_time pri_time] (Ex: 1 130 500 50 ###\n");
		goto out;
	}
	...
	dvfs->times[level].chg_time = chg_time;
	dvfs->times[level].off_time = off_time;
	dvfs->times[level].pri_time = pri_time;
...
static void input_booster_init_dvfs(struct input_booster *data, struct booster_key *key)
{
	...
	if (data->dvfses[device_type].initialized) {
		dev_info(data->dev, "%s: %s is already initialized [key = %s]\n",
	...
	memcpy(dvfs->times, key->time_table, sizeof(struct dvfs_time) * BOOSTER_LEVEL_MAX);
...
#ifdef CONFIG_OF
static struct dvfs_freq key_freq_table[BOOSTER_LEVEL_MAX];
static struct dvfs_freq touchkey_freq_table[BOOSTER_LEVEL_MAX];
static struct dvfs_freq touch_freq_table[BOOSTER_LEVEL_MAX];

static struct dvfs_time key_time_table[BOOSTER_LEVEL_MAX];
static struct dvfs_time touchkey_time_table[BOOSTER_LEVEL_MAX];
static struct dvfs_time touch_time_table[BOOSTER_LEVEL_MAX];
...
static int input_booster_parse_dt(struct device *dev)
{
	...
	struct booster_key *booster_key;
	...
	int key_count = 0, num_level = 0, level = 0;
	int retval = 0, i;

	np = dev->of_node;
	pdata->nkeys = of_get_child_count(np);
	if (!pdata->nkeys) {
		dev_err(dev, "There are no booster keys\n");
		return -ENODEV;
	}
	pdata->keys = devm_kzalloc(dev,
			sizeof(struct booster_key) * pdata->nkeys, GFP_KERNEL);
	if (!pdata->keys) {
		dev_err(dev, "Failed to allocate memory\n");
		return -ENOMEM;
	}

	pdata->get_device_type = get_booster_device;
	freq_tables[BOOSTER_DEVICE_KEY] = key_freq_table;
	freq_tables[BOOSTER_DEVICE_TOUCHKEY] = touchkey_freq_table;
	freq_tables[BOOSTER_DEVICE_TOUCH] = touch_freq_table;
	freq_tables[BOOSTER_DEVICE_PEN] = touch_freq_table;

	time_tables[BOOSTER_DEVICE_KEY] = key_time_table;
	time_tables[BOOSTER_DEVICE_TOUCHKEY] = touchkey_time_table;
	time_tables[BOOSTER_DEVICE_TOUCH] = touch_time_table;
	time_tables[BOOSTER_DEVICE_PEN] = touch_time_table;

	for_each_child_of_node(np, cnp) {
		booster_key = &pdata->keys[key_count];
		booster_key->desc = of_get_property(cnp, "input_booster,label", NULL);
		if (of_property_read_u32(cnp, "input_booster,code", &booster_key->code)) {
			dev_err(dev, "Failed to get code property\n");
			retval = -EINVAL;
			goto err_out;
		}
		...
		if (plevels && num_level) {
			...
			device_type = pdata->get_device_type(booster_key->code);
		} else {
		...
		for (i = 0; i < num_level; i++) {
			...
			retval = of_property_read_u32_index(cnp, "input_booster,chg_times", i,
				 &time_tables[device_type][level].chg_time);
			retval |= of_property_read_u32_index(cnp, "input_booster,off_times", i,
				 &time_tables[device_type][level].off_time);
			retval |= of_property_read_u32_index(cnp, "input_booster,pri_times", i,
				 &time_tables[device_type][level].pri_time);
			if (retval) {
		...
		booster_key->freq_table = freq_tables[device_type];
		booster_key->time_table = time_tables[device_type];

#ifdef BOOSTER_DEBUG
		dev_info(dev, "device_type:%d, label :%s, code: 0x%02x, num_level[%d]\n",
			device_type, booster_key->desc, booster_key->code, num_level);
		for (i = 0; i < BOOSTER_LEVEL_MAX; i++) {
			dev_info(dev, "Level %d : frequency[%d,%d,%d,%d]\n", i,
				booster_key->freq_table[i].cpu_freq,
				booster_key->freq_table[i].kfc_freq,
				booster_key->freq_table[i].mif_freq,
				booster_key->freq_table[i].int_freq);
			dev_info(dev, "Level %d : times[%d,%d,%d]\n", i,
				booster_key->time_table[i].chg_time,
				booster_key->time_table[i].off_time,
				booster_key->time_table[i].pri_time);
		}
#endif
		key_count++;
	}
...
static int input_booster_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->nkeys; i++) {
		dev_info(&pdev->dev, "%s: data->keys[%d] : desc=%s, code=0x%02X\n",
			__func__, i, data->keys[i].desc, data->keys[i].code);
	}
#endif
	...
	for (i = 0; i < data->nkeys; i++) {
		if (data->keys[i].code >= KEY_MAX) {
			dev_err(data->dev, "%s: data->keys[%d].code = 0x%02X\n", __func__, i, data->keys[i].code);
			continue;
		}
