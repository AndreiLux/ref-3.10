...
/* static struct i2c_board_info i2c_mc3210 __initdata = { I2C_BOARD_INFO("MC3210", 0x4C) }; */
...
/*----------------------------------------------------------------------------*/
static struct i2c_client *mc3210_i2c_client;
static struct platform_driver mc3210_gsensor_driver;
static struct mc3210_i2c_data *obj_i2c_data;
...
static int MC3210_SetDataResolution(struct mc3210_i2c_data *obj)
{
	...
	err = hwmsen_read_byte(obj->client, MC3210_REG_DATA_FORMAT, &dat);
	if (err) {
...
static int MC3210_ReadData(struct i2c_client *client, s16 data[MC3210_AXES_NUM])
{
	...
	if (err) {
		...
		/*
		   ...
		   {                                                 //pr_info("data 0 step %x\n",data[i]);
		   data[i] -= 0x1;                   //pr_info("data 1 step %x\n",data[i]);
		   data[i] = ~data[i];               //pr_info("data 2 step %x\n",data[i]);
		   data[i] &= 0x01ff;                //pr_info("data 3 step %x\n\n",data[i]);
		   data[i] = -data[i];
		   }
		   }
		 */
		...
		if (atomic_read(&priv->trace) & ADX_TRC_RAWDATA) {
			GSE_LOG("[%08X %08X %08X] => [%5d %5d %5d]\n", data[MC3210_AXIS_X],
				data[MC3210_AXIS_Y], data[MC3210_AXIS_Z], data[MC3210_AXIS_X],
				data[MC3210_AXIS_Y], data[MC3210_AXIS_Z]);
		}
		...
		if (atomic_read(&priv->filter)) {
			if (atomic_read(&priv->fir_en) && !atomic_read(&priv->suspend)) {
				...
				if (priv->fir.num < firlen) {
					priv->fir.raw[priv->fir.num][MC3210_AXIS_X] =
					    data[MC3210_AXIS_X];
					priv->fir.raw[priv->fir.num][MC3210_AXIS_Y] =
					    data[MC3210_AXIS_Y];
					priv->fir.raw[priv->fir.num][MC3210_AXIS_Z] =
					    data[MC3210_AXIS_Z];
					...
					if (atomic_read(&priv->trace) & ADX_TRC_FILTER) {
						GSE_LOG
						    ("add [%2d] [%5d %5d %5d] => [%5d %5d %5d]\n",
						     ...
						     priv->fir.raw[priv->fir.num][MC3210_AXIS_Y],
						     priv->fir.raw[priv->fir.num][MC3210_AXIS_Z],
						     priv->fir.sum[MC3210_AXIS_X],
						     priv->fir.sum[MC3210_AXIS_Y],
						     priv->fir.sum[MC3210_AXIS_Z]);
					}
					priv->fir.num++;
					priv->fir.idx++;
				} else {
					...
					priv->fir.sum[MC3210_AXIS_X] -=
					    priv->fir.raw[idx][MC3210_AXIS_X];
					priv->fir.sum[MC3210_AXIS_Y] -=
					    priv->fir.raw[idx][MC3210_AXIS_Y];
					priv->fir.sum[MC3210_AXIS_Z] -=
					    priv->fir.raw[idx][MC3210_AXIS_Z];
					...
					if (atomic_read(&priv->trace) & ADX_TRC_FILTER) {
						GSE_LOG
						    ("add [%2d] [%5d %5d %5d] => [%5d %5d %5d] : [%5d %5d %5d]\n",
						     idx, priv->fir.raw[idx][MC3210_AXIS_X],
						     priv->fir.raw[idx][MC3210_AXIS_Y],
						     priv->fir.raw[idx][MC3210_AXIS_Z],
						     priv->fir.sum[MC3210_AXIS_X],
						     priv->fir.sum[MC3210_AXIS_Y],
						     priv->fir.sum[MC3210_AXIS_Z],
						     data[MC3210_AXIS_X], data[MC3210_AXIS_Y],
						     data[MC3210_AXIS_Z]);
					}
				}
			}
...
#else
	err = hwmsen_read_block(client, MC3210_REG_OFSX, ofs, MC3210_AXES_NUM);
	if (err)
		GSE_ERR("error: %d\n", err);
#endif
...
#else
	err = hwmsen_write_block(client, MC3210_REG_OFSX, ofs, 4);
	if (err)
		GSE_ERR("error: %d\n", err);
#endif
...
static int MC3210_ReadCalibration(struct i2c_client *client, int dat[MC3210_AXES_NUM])
{
...
#else
	err = MC3210_ReadOffset(client, obj->offset);
	if (err) {
	...
	dat[obj->cvt.map[MC3210_AXIS_X]] =
	    obj->cvt.sign[MC3210_AXIS_X] * (obj->offset[MC3210_AXIS_X] * mul +
					    obj->cali_sw[MC3210_AXIS_X]);
	dat[obj->cvt.map[MC3210_AXIS_Y]] =
	    obj->cvt.sign[MC3210_AXIS_Y] * (obj->offset[MC3210_AXIS_Y] * mul +
					    obj->cali_sw[MC3210_AXIS_Y]);
	dat[obj->cvt.map[MC3210_AXIS_Z]] =
	    obj->cvt.sign[MC3210_AXIS_Z] * (obj->offset[MC3210_AXIS_Z] * mul +
					    obj->cali_sw[MC3210_AXIS_Z]);

	return 0;
}

/*----------------------------------------------------------------------------*/
static int MC3210_ReadCalibrationEx(struct i2c_client *client, int act[MC3210_AXES_NUM],
				    int raw[MC3210_AXES_NUM])
{
...
static int MC3210_WriteCalibration(struct i2c_client *client, int dat[MC3210_AXES_NUM])
{
	...
	err = MC3210_ReadCalibrationEx(client, cali, raw);
	...
	GSE_LOG("OLDOFF: (%+3d %+3d %+3d): (%+3d %+3d %+3d) / (%+3d %+3d %+3d)\n",
		...
		obj->cali_sw[MC3210_AXIS_X], obj->cali_sw[MC3210_AXIS_Y],
		obj->cali_sw[MC3210_AXIS_Z]);
...
#ifdef SW_CALIBRATION
	obj->cali_sw[MC3210_AXIS_X] =
	    obj->cvt.sign[MC3210_AXIS_X] * (cali[obj->cvt.map[MC3210_AXIS_X]]);
	obj->cali_sw[MC3210_AXIS_Y] =
	    obj->cvt.sign[MC3210_AXIS_Y] * (cali[obj->cvt.map[MC3210_AXIS_Y]]);
	obj->cali_sw[MC3210_AXIS_Z] =
	    obj->cvt.sign[MC3210_AXIS_Z] * (cali[obj->cvt.map[MC3210_AXIS_Z]]);
	...
	GSE_LOG("NEWOFF: (%+3d %+3d %+3d): (%+3d %+3d %+3d) / (%+3d %+3d %+3d)\n",
		...
		obj->cali_sw[MC3210_AXIS_Z]);
#else
	obj->offset[MC3210_AXIS_X] =
	    (s8) (obj->cvt.sign[MC3210_AXIS_X] * (cali[obj->cvt.map[MC3210_AXIS_X]]) / (divisor));
	obj->offset[MC3210_AXIS_Y] =
	    (s8) (obj->cvt.sign[MC3210_AXIS_Y] * (cali[obj->cvt.map[MC3210_AXIS_Y]]) / (divisor));
	obj->offset[MC3210_AXIS_Z] =
	    (s8) (obj->cvt.sign[MC3210_AXIS_Z] * (cali[obj->cvt.map[MC3210_AXIS_Z]]) / (divisor));

	/*convert software calibration using standard calibration */
	obj->cali_sw[MC3210_AXIS_X] =
	    obj->cvt.sign[MC3210_AXIS_X] * (cali[obj->cvt.map[MC3210_AXIS_X]]) % (divisor);
	obj->cali_sw[MC3210_AXIS_Y] =
	    obj->cvt.sign[MC3210_AXIS_Y] * (cali[obj->cvt.map[MC3210_AXIS_Y]]) % (divisor);
	obj->cali_sw[MC3210_AXIS_Z] =
	    obj->cvt.sign[MC3210_AXIS_Z] * (cali[obj->cvt.map[MC3210_AXIS_Z]]) % (divisor);
	...
	err = hwmsen_write_block(obj->client, MC3210_REG_OFSX, obj->offset, MC3210_AXES_NUM);
	if (err) {
...
static int MC3210_SetDataFormat(struct i2c_client *client, u8 dataformat)
{
	...
	if (hwmsen_read_block(client, MC3210_REG_DATA_FORMAT, databuf, 0x01)) {
		pr_info("mc3210 read Dataformat failt\n");
		return MC3210_ERR_I2C;
	}
	...
	/* pr_info("MC3210_SetDataFormat OK!\n"); */
...
static int MC3210_SetBWRate(struct i2c_client *client, u8 bwrate)
{
	...
	if (hwmsen_read_block(client, MC3210_REG_BW_RATE, databuf, 0x01)) {
		pr_info("mc3210 read rate failt\n");
		return MC3210_ERR_I2C;
	}
...
static int MC3210_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	res = MC3210_ReadData(client, obj->data);
	if (res) {
		...
		/*remap coordinate */
		acc[obj->cvt.map[MC3210_AXIS_X]] =
		    obj->cvt.sign[MC3210_AXIS_X] * obj->data[MC3210_AXIS_X];
		acc[obj->cvt.map[MC3210_AXIS_Y]] =
		    obj->cvt.sign[MC3210_AXIS_Y] * obj->data[MC3210_AXIS_Y];
		acc[obj->cvt.map[MC3210_AXIS_Z]] =
		    obj->cvt.sign[MC3210_AXIS_Z] * obj->data[MC3210_AXIS_Z];
		...
		/* Out put the mg */
		acc[MC3210_AXIS_X] =
		    acc[MC3210_AXIS_X] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
		acc[MC3210_AXIS_Y] =
		    acc[MC3210_AXIS_Y] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
		acc[MC3210_AXIS_Z] =
		    acc[MC3210_AXIS_Z] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;


		sprintf(buf, "%04x %04x %04x", acc[MC3210_AXIS_X], acc[MC3210_AXIS_Y],
			acc[MC3210_AXIS_Z]);
...
static int MC3210_JudgeTestResult(struct i2c_client *client)
{
	...
	res = MC3210_ReadData(client, acc);
	if (res) {
		GSE_ERR("I2C error: ret value=%d", res);
		return -EIO;
	} else {
		pr_info("0 step: %d %d %d\n", acc[0], acc[1], acc[2]);
		...
		pr_info("2 step: result = %d", self_result);
...
static ssize_t show_cali_value(struct device_driver *ddri, char *buf)
{
	...
	if (err)
		return -EINVAL;
	err = MC3210_ReadCalibration(client, tmp);
	if (err) {
		return -EINVAL;
	} else {
		mul = obj->reso->sensitivity / mc3210_offset_resolution.sensitivity;
		len +=
		    snprintf(buf + len, PAGE_SIZE - len,
			     ...
			     obj->offset[MC3210_AXIS_X], obj->offset[MC3210_AXIS_Y],
			     obj->offset[MC3210_AXIS_Z], obj->offset[MC3210_AXIS_X],
			     obj->offset[MC3210_AXIS_Y], obj->offset[MC3210_AXIS_Z]);
		len +=
...
static ssize_t store_cali_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "rst", 3)) {
		...
		dat[MC3210_AXIS_Y] = y;
		dat[MC3210_AXIS_Z] = z;
		err = MC3210_WriteCalibration(client, dat);
		if (err) {
...
static ssize_t show_firlen_value(struct device_driver *ddri, char *buf)
{
	...
	if (atomic_read(&obj->firlen)) {
		...
		for (idx = 0; idx < len; idx++) {
			GSE_LOG("[%5d %5d %5d]\n", obj->fir.raw[idx][MC3210_AXIS_X],
				obj->fir.raw[idx][MC3210_AXIS_Y], obj->fir.raw[idx][MC3210_AXIS_Z]);
		}

		GSE_LOG("sum = [%5d %5d %5d]\n", obj->fir.sum[MC3210_AXIS_X],
			obj->fir.sum[MC3210_AXIS_Y], obj->fir.sum[MC3210_AXIS_Z]);
		GSE_LOG("avg = [%5d %5d %5d]\n", obj->fir.sum[MC3210_AXIS_X] / len,
			obj->fir.sum[MC3210_AXIS_Y] / len, obj->fir.sum[MC3210_AXIS_Z] / len);
	}
...
static int mc3210_create_attr(struct device_driver *driver)
{
	...
	for (idx = 0; idx < num; idx++) {
		err = driver_create_file(driver, mc3210_attr_list[idx]);
		if (err) {
			GSE_ERR("driver_create_file (%s) = %d\n", mc3210_attr_list[idx]->attr.name,
				err);
			break;
		}
	}
...
/* unsigned long arg) */
static long mc3210_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case GSENSOR_IOCTL_GET_CALI:
		...
		err = MC3210_ReadCalibration(client, cali);
		if (err)
			break;
...
/*----------------------------------------------------------------------------*/
static const struct file_operations mc3210_fops = {
/* .owner = THIS_MODULE, */
	.open = mc3210_open,
...
static int mc3210_suspend(struct i2c_client *client, pm_message_t msg)
{
	...
	if (msg.event == PM_EVENT_SUSPEND) {
		...
		err = MC3210_SetPowerMode(obj->client, false);
		if (err) {
...
static int mc3210_resume(struct i2c_client *client)
{
	...
	err = mc3210_init_client(client, 0);
	if (err) {
...
static void mc3210_late_resume(struct early_suspend *h)
{
	...
	MC3210_power(obj->hw, 1);
	err = mc3210_init_client(obj->client, 0);
	if (err) {
		GSE_ERR("initialize client fail!!\n");
		return;
	}
...
static int mc3210_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = mc3210_init_client(new_client, 1);
	if (err)
		goto exit_init_failed;

	err = misc_register(&mc3210_device);
	if (err) {
		GSE_ERR("mc3210_device register failed\n");
		goto exit_misc_device_register_failed;
	}

	err = mc3210_create_attr(&mc3210_gsensor_driver.driver);
	if (err) {
	...
	obj->early_drv.level = EARLY_SUSPEND_LEVEL_DISABLE_FB - 1,
	    ...
	    obj->early_drv.resume = mc3210_late_resume, register_early_suspend(&obj->early_drv);
#endif
...
static int mc3210_i2c_remove(struct i2c_client *client)
{
	...
	err = mc3210_delete_attr(&mc3210_gsensor_driver.driver);
	if (err)
		GSE_ERR("mc3210_delete_attr fail: %d\n", err);

	err = misc_deregister(&mc3210_device);
	if (err)
