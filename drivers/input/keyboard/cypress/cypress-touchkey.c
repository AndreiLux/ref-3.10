...
/*	const struct firmware *firm_data = NULL;*/
	...
	while (retry--) {
		...
		if (ret != 0) {
			if (ret == 1) {
				dev_err(&tkey_i2c->client->dev, "not connected IC\n");
				goto err_fw_update;
			}
			msleep(50);
...
static void cypress_config_gpio_i2c(struct touchkey_i2c *tkey_i2c, int onoff)
{
	struct device *i2c_dev = tkey_i2c->client->dev.parent->parent;
...
/*	if (pdata->gpio_id_value) {
		dev_err(&client->dev, "touchkey not connected\n");
		goto err_i2c_check;
	}
*/
#if defined(CONFIG_GLOVE_TOUCH)
