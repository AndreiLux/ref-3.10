...
static u8 ProgramProtectionSettings(void)
{
	...
	dev_info(g_sd->dev, "%s: rowProtectionData0~2:0x%02x 0x%02x 0x%02x \n", __func__,
...
static u8 VerifyChecksum(void)
{
	...
	dev_info(g_sd->dev, "%s: checksum of entire flash=0x%08x\n", __func__, checksum_All);
	...
	size_t  fw_img_size)
{
	...
	sd = &_swd_data;
	sd->power = power;
	sd->swdio_gpio = swdio_gpio;
	sd->swdck_gpio = swdck_gpio;
	sd->fw_img = fw_img;
	sd->fw_img_size = fw_img_size;
	sd->dev = dev;
	sd->hex_or_bin = fw_img ? false : true;
	g_sd = sd;
	
	Swd_configPhysical(swdio_gpio, swdck_gpio);
	if (ProgramDevice(sd) == SUCCESS)
		rc = 0;
	else
		rc = -1;
	
	dev_info(dev, "%s: end, rc=%d\n", __func__, rc);
	return rc;
