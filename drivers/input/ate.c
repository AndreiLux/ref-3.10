...
#include "../huawei/touchscreen/huawei_touchscreen.h"
...
static int inputevent_data_save(void)
{
    ...
    set_fs(KERNEL_DS);
    pos = 0;
    snprintf(buf, sizeof(buf), "%lu", ate_dt->ie_times);
    vfs_write(fp, buf, sizeof(buf), &pos);
    memset(buf, 0, IE_SIZE);
    snprintf(buf, sizeof(buf), "%lu", ate_dt->playback_continuance_ms);
    vfs_write(fp, buf, sizeof(buf), &pos);

    strncpy(buf_name,RECORD_PHONE,sizeof(buf_name));
...
static ssize_t store_playback_delay_ms(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
    if(strict_strtoul(buf, 0, &ate_dt->playback_delay_ms))
        return -EINVAL;
    return count;
}
...
static ssize_t show_file_save_user_ops(struct device *dev,
	struct device_attribute *attr, char *buf)
{
    return snprintf(buf, MAX_FILE_DIR_NAME, "%s\n", ate_dt->iedt_file_dir);
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    ret = snprintf(ate_dt->iedt_file_dir, sizeof(ate_dt->iedt_file_dir), "%s", buf);
...
#endif
#ifdef HISI_K3_PLATFORM
    strncpy(ate_dt->idf.dev_name[0], TS_DEV_NAME, sizeof(ate_dt->idf.dev_name[0]));
    ate_dt->idf.input_event_type[0] = TOUCH_EVENT;
    strncpy(ate_dt->idf.dev_name[1], "so340010", sizeof(ate_dt->idf.dev_name[1]));
    ate_dt->idf.input_event_type[1] = TOUCH_BUTTON_EVENT;
    strncpy(ate_dt->idf.dev_name[2], "hi6421_on", sizeof(ate_dt->idf.dev_name[2]));
    ate_dt->idf.input_event_type[2] = POWER_KEY_EVENT;
    strncpy(ate_dt->idf.dev_name[3], "hisi_gpio_key.13", sizeof(ate_dt->idf.dev_name[3]));
    ate_dt->idf.input_event_type[3] = SOUND_KEY_EVENT;
    ate_dt->idf.filer_total = SOUND_KEY_EVENT+1;
#endif
}
...
static int __init ate_init(void)
{
    ...
    strncpy(ate_dt->iedt_file_dir, INPUT_EVENTS_DATA_FILE_DEFAULT, sizeof(ate_dt->iedt_file_dir));
    INIT_DEFERRABLE_WORK(&ate_dt->playback_work,
