				  ...
				  struct cyttsp4_hex_image *row_image)
{
	...
	if (row_image->row_size > ARRAY_SIZE(row_image->row_data)) {
		dev_err(&ttsp->dev, "%s: row data buffer overflow\n", __func__);
		retval = -EOVERFLOW;
			     ...
			     int fw_size)
{
	...
	if (retval < 0) {
		dev_err(dev, "%s: Fail reset device r=%d\n", __func__, retval);
		goto _cyttsp4_load_app_exit;
	}
...
static int cyttsp4_check_firmware_version_platform(struct cyttsp4_device *ttsp, struct cyttsp4_touch_firmware
						   *fw)
{
	struct device *dev = &ttsp->dev;
	struct cyttsp4_loader_data *data = dev_get_drvdata(dev);
...
static int cyttsp4_check_ttconfig_version_platform(struct cyttsp4_device *ttsp, struct cyttsp4_touch_config
						   *ttconfig)
{
	struct device *dev = &ttsp->dev;
	struct cyttsp4_loader_data *data = dev_get_drvdata(dev);
...
static DEVICE_ATTR(config_loading, S_IRUGO | S_IWUSR | S_IWGRP,
...
static DEVICE_ATTR(forced_upgrade, S_IWUSR | S_IWGRP,
...
static DEVICE_ATTR(manual_upgrade, S_IWUSR | S_IWGRP,
...
typedef char *charp;
static charp core_ids[CY_MAX_NUM_CORE_DEVS] = {
	CY_DEFAULT_CORE_ID,
	NULL,
	NULL,
	NULL,
