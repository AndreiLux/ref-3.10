...
#if 0
#define DEBUG(fmt, arg...) pr_info("<--GT-DEBUG-->"fmt, ##arg)
#else
#define DEBUG(fmt, arg...)
#endif

#if 1
#define NOTICE(fmt, arg...) pr_info("<--GT-NOTICE-->"fmt, ##arg)
#else
#define NOTICE(fmt, arg...)
#endif

#if 1
#define WARNING(fmt, arg...) pr_info("<--GT-WARNING-->"fmt, ##arg)
#else
...
static int i2c_read_bytes(struct i2c_client *client, u16 addr, u8 *rxbuf, int len)
{
	...
	while (left > 0) {
		if (left > MAX_I2C_MAX_TRANSFER_SIZE) {
			...
			retry = 0;
			while (i2c_master_recv(client, &rxbuf[offset], MAX_I2C_MAX_TRANSFER_SIZE) <
			       0) {
				retry++;
...
static s32 tool_i2c_write_no_extra(u8 *buf, u16 len)
{
	...
	if (len <= ADDR_MAX_LENGTH) {
		...
		if (i2c_write_bytes
		    (gt_client, buf[0] << 8 | buf[1], &buf[2], len - ADDR_MAX_LENGTH)) {
			ret = -1;
		}
	}
...
/* memcpy(&buf[ADDR_MAX_LENGTH - cmd_head.addr_len], &cmd_head.flag_addr, cmd_head.addr_len); */
	...
	for (i = 0; i < cmd_head.times; i++) {
		...
		if (true ==
		    relation(buf[ADDR_MAX_LENGTH], cmd_head.flag_val, cmd_head.flag_relation)) {
			     ...
			     void *data)
{
	...
	if (1 == cmd_head.wr) {
		...
		ret =
		    copy_from_user(&cmd_head.data[ADDR_MAX_LENGTH], &buff[CMD_HEAD_LENGTH],
				   cmd_head.data_len);
		...
		memcpy(&cmd_head.data[ADDR_MAX_LENGTH - cmd_head.addr_len], cmd_head.addr,
		       cmd_head.addr_len);
		...
		DEBUG_ARRAY(&cmd_head.data[ADDR_MAX_LENGTH - cmd_head.addr_len],
