...
struct gt8xx_platform_data *p_gt8xx_platform_data;
...
/* move to board-xxx.c */
/* static struct i2c_board_info i2c_tpd __initdata = { I2C_BOARD_INFO("mtk-tpd", (0xBA >> 1)) }; */
...
static struct workqueue_struct *tpd_esd_check_workqueue;
...
/* static struct proc_dir_entry *gt813_config_proc; */
...
static void tpd_esd_check_func(struct work_struct *work)
{
	...
	if (i >= 3) {
		...
		i2c_read_bytes(i2c_client, 0x0394,
			       &esd_check_data[esd_checked_time * ESD_CHECK_DATA_LEN],
			       ESD_CHECK_DATA_LEN);

		if ((++esd_checked_time) >= 3) {
			if (!strncmp
			    (&esd_check_data[0], &esd_check_data[ESD_CHECK_DATA_LEN],
			     ESD_CHECK_DATA_LEN)
			    && !strncmp(&esd_check_data[0], &esd_check_data[2 * ESD_CHECK_DATA_LEN],
					ESD_CHECK_DATA_LEN)) {
...
int i2c_read_bytes(struct i2c_client *client, u16 addr, u8 *rxbuf, int len)
{
	...
	while (left > 0) {
		...
		while (mt_i2c_transfer(client->adapter, &msg[0], 2) != 2) {
			retry++;
...
int i2c_write_bytes(struct i2c_client *client, u16 addr, u8 *txbuf, int len)
{
	...
	while (left > 0) {
		...
		if (left > MAX_I2C_TRANSFER_SIZE) {
			memcpy(&buffer[I2C_DEVICE_ADDRESS_LEN], &txbuf[offset],
		...
		while (mt_i2c_transfer(client->adapter, &msg, 1) != 1) {
			retry++;
...
static int i2c_write_dummy(struct i2c_client *client, u16 addr)
{
	...
	mt_i2c_transfer(client->adapter, &msg, 1);
...
/* #endif */
	...
	p_gt8xx_platform_data = client->dev.platform_data;

	/* Power on */
	hwPowerOn(TPD_POWER_LDO, VOL_3300, "TP");
	pr_info("MediaTek touch panel sets hwPowerOn!!!\n");
	/* tpd_open_gpio(); */
	/* msleep(10); */
	...
	for (i = 0; i < 14; i++)
		TPD_DEBUG("0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x,\n",
			  buffer[8 * i], buffer[8 * i + 1], buffer[8 * i + 2], buffer[8 * i + 3],
			  buffer[8 * i + 4], buffer[8 * i + 5], buffer[8 * i + 6],
			  buffer[8 * i + 7]);
	i2c_write_dummy(client, TPD_HANDSHAKING_END_REG);
#endif
	...
	err =
	    request_irq(EINT_IRQ(CUST_EINT_TOUCH_PANEL_NUM), tpd_eint_interrupt_handler,
			EINTF_TRIGGER_RISING, "GT813TB", NULL);

	if (err) {
		TPD_DMESG("Could not register GT813TB, err=%d\n", err);
	}
...
static int touch_event_handler(void *unused)
{
	...
	do {
		...
		if (key == 0x0f) {
			...
			pr_info("reload config.\n");
			err =
			    i2c_write_bytes(i2c_client, TPD_CONFIG_REG_BASE, cfg_data, CONFIG_LEN);
			i2c_write_dummy(i2c_client, TPD_HANDSHAKING_END_REG);

			if (err) {
				pr_info("Reload config failed!\n");
			}
		}
...
/* tpd_down( TPD_WARP_X(x), TPD_WARP_Y(y), size, id); */
				/* pr_info("[touch]x:%d, y:%d, size:%d, id:%d.\n", x, y, size, id); */
...
/* modified by renyufeng for entering SAFE MODE 201203062157 */
			for (idx = 0; idx < 3; idx++) {
				input_report_key(tpd->dev, tpd_keys_local[idx],
						 !!(key & (0x01 << idx)));
...
/* memcpy(tpd_def_calmat, tpd_def_calmat_local, 8*4); */
	if (FACTORY_BOOT == get_boot_mode()) {
		...
		memcpy(tpd_calmat_driver, tpd_def_calmat_local_rotation_0,
