...
#define GPIO_DIRECTION_INPUT(port)	do {\
					if (port == INT_PORT)\
						mt_set_gpio_mode(port, GPIO_CTP_EINT_PIN_M_GPIO);\
					else\
						mt_set_gpio_mode(port, GPIO_CTP_RST_PIN_M_GPIO);\
					mt_set_gpio_dir(port, GPIO_DIR_IN);\
					} while (0)
#define GPIO_DIRECTION_OUTPUT(port, val)	do {\
					if (port == INT_PORT)\
						mt_set_gpio_mode(port, GPIO_CTP_EINT_PIN_M_GPIO);\
					else\
						mt_set_gpio_mode(port, GPIO_CTP_RST_PIN_M_GPIO);\
...
#define GPIO_SET_VALUE(port, val) (val?mt_set_gpio_out(port, GPIO_OUT_ONE) : \
				mt_set_gpio_out(port, GPIO_OUT_ZERO))
#define GPIO_PULL_UPDOWN(port, val) (val?mt_set_gpio_pull_enable(port, GPIO_PULL_DISABLE) : \
				mt_set_gpio_pull_enable(port, GPIO_PULL_DISABLE))
#define GPIO_CFG_PIN(port, cfg)             enable_irq(EINT_IRQ(CUST_EINT_TOUCH_PANEL_NUM))
...
#if 0
#define DEBUG_UPDATE(fmt, arg...) pr_info("<--GT update-->"fmt, ##arg)
#else
...
typedef struct {
	...
	u16 lenth;		/* FW// */
	u8 chk_sum[3];
	u8 force_update[6];	/* ǿ־,Ϊ"GOODIX"ǿ// */
} st_fw_head;
#pragma pack()
...
static u8 get_ic_fw_msg(struct i2c_client *client)
{
	...
	for (i = 0; i < 2; i++) {
		if (fail ==
		    get_ic_msg(client, READ_FW_MSG_ADDR_H << 8 | READ_FW_MSG_ADDR_L, buf, 4)) {
...
u8 load_update_file(struct i2c_client *client, st_fw_head *fw_head, u8 *data, u8 *path)
{
	...
	ret =
	    update_msg.file->f_op->read(update_msg.file, (char *)&buf, 6, &update_msg.file->f_pos);
	if (ret < 0) {
		...
		if (is_equal
				...
				/* || update_msg.force_update == 0xAA) */
			{
				...
				if (is_equal(fw_head->force_update, "GOODIX", 6)) {
					update_msg.gt_loc =
					    update_msg.file->f_pos - FW_HEAD_LENGTH +
					    sizeof(st_fw_head) - sizeof(fw_head->force_update);
				}

				ret =
				    update_msg.file->f_op->read(update_msg.file, (char *)data,
								fw_head->lenth,
								&update_msg.file->f_pos);
				if (ret <= 0) {
...
static int guitar_update_proc(void *v_cli)
{
	...
	while (retry++ < 5) {
		...
		if (update_msg.gt_loc > 0) {
			...
			ret =
			    update_msg.file->f_op->write(update_msg.file, buf, 6,
							 &update_msg.gt_loc);
			if (ret < 0) {
