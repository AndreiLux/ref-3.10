		...
		u16 irq, size_t xfer_buf_size)
{
	...
	rc = cyttsp5_loader_probe(dev); // before other probe for corrupted f/w
	if (rc < 0) {
		pr_info( "%s: Error, fail loader probe\n", __func__);
		goto error_startup_loader;
	}

	if (!cd->sysinfo.ready) // previous startup() didn't get si because f/w corrupted
	{
		pr_info( "%s: call startup again\n", __func__);
		rc = cyttsp5_startup(cd);
		if (rc < 0) {
			pr_info( "%s: Fail 2nd startup r=%d\n",
				__func__, rc);
