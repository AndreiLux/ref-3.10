...
int cyttsp5_devtree_create_and_get_pdata(struct device *adap_dev)
{
	...
	pr_info("[wisdyd] %s: %d child node(s) found\n", __func__, count);
	pr_info("[wisdyd] %s: - \n", __func__);
	return rc;
}
