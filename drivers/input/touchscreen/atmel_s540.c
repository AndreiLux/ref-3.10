...
//#include <mach/board_lge.h> //yejini.lee block build_error
...
//#include <linux/mfd/pm8xxx/cradle.h> //sora.jin build error
...
#ifdef MXT_LPWG
#define MXT_LATEST_CONFIG_CRC	0x5809BB
u8 latest_firmware[] = {
	#include "mXT540S_V1.0.EE_.h"
};
#else
...
struct timeval atmel_t_ex_debug[TIME_EX_PROFILE_MAX];
#endif
...
int atmel_quick_cover_status = 0;
bool mxt_mfts = false;
/* check TA status */
...
#endif
struct workqueue_struct*	atmel_touch_wq;
...
//knock on
static bool touch_enable_sms = 1;
static void touch_enable_irq_sms(unsigned int irq_sms){
        printk("[lge_touch] enable touch irq sms(%d)\n", touch_enable_sms);
...
static void touch_disable_irq_sms(unsigned int irq_sms){
        printk("[lge_touch] disable touch irq sms(%d)\n", touch_enable_sms);
			       ...
			       u16 reg, u16 len, void *val)
{
	...
	do {
		...
		dev_dbg(&client->dev, "%s: i2c retry %d ret(%d)\n", __func__, i+1,ret);
...
retry_read:
	...
	if (ret != ARRAY_SIZE(xfer)) {
		if (!retry) {
			dev_err(&client->dev, "%s: i2c retry ret(%d)\n", __func__,ret);
			msleep(MXT_WAKEUP_TIME);
			retry = true;
			goto retry_read;
		} else {
			   ...
			   const void *val)
{
	...
	do {
		...
		dev_info(&client->dev, "%s: i2c retry %d ret(%d)\n", __func__, i+1,ret);
		msleep(MXT_WAKEUP_TIME);
	} while (++i < 3);
		dev_err(&client->dev, "%s: i2c transfer failed\n", __func__);
		kfree(buf);
...
static void mxt_proc_t100_message(struct mxt_data *data, u8 *message)
{
	...
	//if(status & (1 << 2))
		dev_dbg(dev, "[%u] %c%c%c %s%s%s (%02X) x:%u y:%u amp:%u area:%02X vec:%02X\n",
			id,
			((status & MXT_T100_STATUS_MASK) == MXT_T100_PRESS) ? 'P' : '.',
			((status & MXT_T100_STATUS_MASK) == MXT_T100_RELEASE) ? 'R' : '.',
			((status & MXT_T100_STATUS_MASK) == MXT_T100_MOVE) ? 'M' : '.',
...
void atmel_send_uevent(char* string[2])
{
...
static u8 t92_UDF_off_config[] = {	MXT_T92_NEW, 1,
	0, 0,
};
...
static u8 t92_UDF_on_config[] = {	MXT_T92_NEW, 1,
	0, 1,
};
...
static void mxt_proc_t24_messages(struct mxt_data *data, u8 *message)
{
	...
	if(msg == 0x04) {
		if(atmel_quick_cover_status == 1){
#ifdef MXT_LPWG
			dev_info(dev, "QuickCoverSize X %d-%d  Y %d-%d \n",
		...
		atmel_send_uevent(knockon_event);
	}
}
#endif
static int mxt_proc_message(struct mxt_data *data, u8 *message)
{
	...
	do_gettimeofday(&atmel_t_ex_debug[TIME_CURR_TIME]);
	...
	switch(object){
		...
		case MXT_PROCI_ONETOUCH_T24:
#ifdef MXT_LPWG
			if(!data->mxt_password_enable && data->mxt_knock_on_enable)	//hyunjee_temp
#else
		...
		if (data->ts_data.curr_data[i].status == FINGER_PRESSED) {
			...
			dev_info(dev, "%d %s Pressed <%d> : x[%4d] y[%4d], z[%3d]\n",
					...
					data->ts_data.curr_data[i].pressure);
						...
						&& data->ts_data.prev_data[i].status == FINGER_PRESSED) {
...
static irqreturn_t mxt_interrupt(int irq, void *dev_id)
{
	...
	if (data->T44_address) {
		...
		queue_delayed_work(atmel_touch_wq, &data->check_suspended_work, 0);
#else
		queue_work(atmel_touch_wq, &data->work);
#endif
...
//knock on
static irqreturn_t mxt_interrupt_sms(int irq_sms, void *dev_id)
{
        struct mxt_data *data = dev_id;
	if (data->in_bootloader) {
                /* bootloader state transition completion */
...
//sora.jin build error block code must check
/*
    if (i2c_suspended && data->suspended){
		dev_err(&data->client->dev, "lge_touch touch suspended. try i2c operation after 10ms.\n");
		queue_delayed_work(atmel_touch_wq, &data->check_suspended_work, msecs_to_jiffies(10));
		return;
	}else{
*/
...
static int mxt_set_t7_power_cfg(struct mxt_data *data, u8 sleep)
{
	...
	dev_info(dev, "Set T7 Addr(%d) ACTV:%d IDLE:%d\n",
		 data->T7_address,new_config->active, new_config->idle);
...
static void mxt_probe_regulators(struct mxt_data *data)
{
	...
	data->vdd_ana = regulator_get(dev, "+3.0V_TOUCH_AVCC");
	...
	data->vcc_i2c = regulator_get(dev, "+1.8V_TOUCH_IOVCC");
...
//	input_dev->close = mxt_input_close;
	...
	set_bit(INPUT_PROP_DIRECT, input_dev->propbit); //yejini.lee
...
static void mxt_gesture_wakeup_enable(struct mxt_data *data)
{
	...
	printk("[lge_touch]%s, power_status:%d\n",__func__,data->power_status);
...
#ifdef MXT_LPWG
static void mxt_lpwg_enable(struct mxt_data *data, u32 value);	//hyunjee_temp
static void mxt_lpwg_disable(struct mxt_data *data, u32 value);	//hyunjee_temp
#endif
...
#ifdef MXT_LPWG
		mxt_lpwg_enable(data, 0);	//hyunjee_temp
#else
		mxt_gesture_wakeup_enable(data);		//hyunjee_test
#endif
	}else{
#ifdef MXT_LPWG
		mxt_lpwg_disable(data, 0);	//hyunjee_temp
#else
		mxt_gesture_wakeup_disable(data);	//hyunjee_test
#endif
	}
...
static ssize_t store_quick_cover_status(struct mxt_data *data, const char *buf, size_t size)
{
	...
	if( (value == 1) && (atmel_quick_cover_status == 0) )
		atmel_quick_cover_status = 1;
	else if( (value == 0) && (atmel_quick_cover_status == 1) )
		atmel_quick_cover_status = 0;
	else
		return size;

	dev_info(&data->client->dev, "quick cover status = %s\n", (atmel_quick_cover_status == 1) ? "QUICK_COVER_ON" : "QUICK_COVER_OFF");
#ifdef TSP_PATCH
	if(atmel_quick_cover_status == 0)
...
static void change_ime_drumming_func(struct work_struct *work_ime_drumming)
{
	...
	dev_info(&data->client->dev, "change_ime_drumming_func. Palm detect %d\n", enable);
}
...
static ssize_t store_ime_drumming_status(struct mxt_data *data, const char *buf, size_t count)
{
	...
	if( (value == 1) && (ime_drumming_status == 0) ) {
		...
		queue_delayed_work(atmel_touch_wq, &data->work_ime_drumming, msecs_to_jiffies(10));
	}
...
static void mxt_lpwg_enable(struct mxt_data *data, u32 value)
{
	...
	touch_enable_irq(data->irq);
	touch_enable_irq_sms(data->irq_sms);
...
static void mxt_lpwg_disable(struct mxt_data *data, u32 value)
{
	...
	if(data->power_status == MXT_POWER_ON || data->power_status == MXT_POWER_CFG_RUN || data->power_status == MXT_POWER_CFG_KNOCKON){
		touch_disable_irq(data->irq);
		touch_disable_irq_sms(data->irq_sms);
		cancel_work_sync(&data->work);
#ifdef I2C_SUSPEND_WORKAROUND
		cancel_delayed_work_sync(&data->check_suspended_work);
#endif
...
static void lpwg_early_suspend(struct mxt_data *data)
{
	...
	switch (data->lpwg_mode) {
			...
			default:
				data->mxt_password_enable = 0;
				data->is_lpwg_report_enable = 0;
				gesture_control(data, 1);	//hyunjee_temp
				break;
			}
...
err_t atmel_ts_lpwg(struct i2c_client* client, u32 code, u32 value, struct point *tci_point)
{
	struct mxt_data *data = i2c_get_clientdata(client);
	int i;
	TOUCH_INFO_MSG("atmel_ts_lpwg Code: %d Value: %d\n", code, value);
	...
	case LPWG_REPLY:
		...
		if(value == 0 && data->mxt_password_enable){	/* password fail */
			data->is_lpwg_report_enable = 1;
			write_partial_configs(data, UDF_on_configs_);
			TOUCH_INFO_MSG("T35 ENABLE\n");
		}
...
static void mxt_reset_slots(struct mxt_data *data)
{
	...
	if(!input_dev){
		dev_err(&data->client->dev, "%s Error : input_dev NULL.\n",__func__);
		return;
	}
...
#endif
#ifdef MXT_LPWG
	dev_info(&data->client->dev,"Gesture_control config Change: OK (%s)- KnockOn: %d, PASSWD : %d\n",
				on ? "SUSPEND":"RESUME", data->mxt_knock_on_enable, data->mxt_password_enable);
#else
	dev_info(&data->client->dev,"Gesture_control config Change: OK (%s)- KnockOn: %d\n",
...
{
	dev_info(&data->client->dev, " %s: logo(%d) is_probing(%d) suspend(%d) bootloader(%d)\n", __func__, chargerlogo,is_probing,data->suspended,data->in_bootloader);
	...
	if(chargerlogo){
	 ...
	 if (!is_probing && (!data->suspended || data->in_bootloader))
		return;
	...
	touch_disable_irq_sms(data->irq_sms);
	//knock on
	do_gettimeofday(&atmel_t_ex_debug[TIME_START_TIME]);
}
...
static int mxt_parse_dt(struct device *dev, struct mxt_platform_data *pdata)
{
	...
	if (node == NULL)
		return -ENODEV;
	//yejini.lee
	rc = of_property_read_u32(node, "atmel,reset-gpio", &pdata->gpio_reset);
	if (rc && (rc != -EINVAL)) {
		dev_err(dev, "Unable to read reset-gpio\n");
		return -ENODEV;
	}

	rc = of_property_read_u32(node, "atmel,irq-gpio", &pdata->gpio_int);
	if (rc && (rc != -EINVAL)) {
		dev_err(dev, "Unable to read irq-gpio\n");
	    return -ENODEV;
	}
	//knock on
	rc = of_property_read_u32(node, "atmel,irq-gpio-sms", &pdata->gpio_int_sms);
	if (rc && (rc != -EINVAL)) {
		dev_err(dev, "Unable to read irq-gpio-sms\n");
	}
/*
...
static int mxt_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev, "before request_int\n");
	...
	//knock on
	/* request SMS interrupt pin */
	if(data->pdata->gpio_int_sms > 0){
		error = gpio_request(data->pdata->gpio_int_sms, "touch_int_sms");
		if (error < 0) {
			dev_err(&client->dev, "FAIL: touch_int_sms gpio_request\n");
			goto err_interrupt_failed;
		}
		gpio_direction_input(data->pdata->gpio_int_sms);
	}
	//knock on
	...
	//knock on
	data->irq_sms = gpio_to_irq(data->pdata->gpio_int_sms);

	error = odin_gpio_sms_request_threaded_irq(data->irq_sms, NULL, mxt_interrupt_sms,
					data->pdata->irqflags | IRQF_ONESHOT,
					"TOUCH_SMS", data);
	if (error) {
		dev_err(&client->dev, "Failed to register SMS interrupt\n");
	goto err_free_pdata;
	}
	//knock on
	...
	touch_disable_irq(data->irq);
	touch_disable_irq_sms(data->irq_sms);	//knock on;
...
#endif
//sora.jin build error block - must check
/*
...
static int mxt_resume(struct device *dev)
{
	...
	dev_info(&client->dev, "%s input_dev->users(%d)\n", __func__,input_dev->users);
...
static int mxt_init_kthread_fn(void)
#else
...
#endif
{
	...
	if(lge_get_board_revno() == HW_REV_A || lge_get_board_revno()== HW_REV_B){
		TOUCH_ERR_MSG("Return error cuz No use of atmel touch in rev_A.\n");
		return -ENOMEM;
	}

	atmel_touch_wq = create_singlethread_workqueue("atmel_touch_wq");
	if (!atmel_touch_wq) {
...
err_i2c_add_driver:
	destroy_workqueue(atmel_touch_wq);
err_work_queue:
	return ret;
}
...
static int __init mxt_init(void)
{
	...
	int cpu = 5; // FIXME: Need to find smarter way to get idle big cpu

	task = kthread_create_on_node(mxt_init_kthread_fn, NULL,
				cpu_to_node(cpu), "kthread_mxt_init/%d", cpu);
