	...
	if (mode == CY_HST_CAT) {
		...
		if (dad->own_exclusive) {
			...
			dev_vdbg(dev, "%s: %s mode=%02X(Operate)\n", __func__,
				 "do not own exclusive; cannot switch", mode);
	} else
...
/*	The original version from Cypress will ignore the offset value
 *	set by user, and use the default 0.
 *
 *	int read_element_offset = CY_CMD_IN_DATA_OFFSET_VALUE;
 */
	int read_element_offset = dad->heatmap.input_offset;
	...
	dad->ic_buf[CY_CMD_RET_PNL_OUT_ELMNT_SZ_OFFS_H + cmd_param_ofs] =
	    HI_BYTE(dad->heatmap.num_element);
	dad->ic_buf[CY_CMD_RET_PNL_OUT_ELMNT_SZ_OFFS_L + cmd_param_ofs] =
	    LO_BYTE(dad->heatmap.num_element);
					...
					const char *buf, size_t size)
{
	...
	dad->heatmap.input_offset = dad->ic_buf[2] + (dad->ic_buf[1] * 256);
					...
					int ic_grpoffset, u8 * buf, int length)
{
	...
	/* Call relevant store handler. */
	rc = cyttsp4_grpdata_store_functions[dad->ic_grpnum] (dev, buf, length);
	if (rc < 0)
		dev_err(dev, "%s: Failed to store for grpmun=%d.\n",
			__func__, dad->ic_grpnum);
