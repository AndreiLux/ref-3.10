...
unsigned char buf_debug[PAGE_SIZE];
...
/*write data by i2c*/
int ft5x0x_dma_i2c_Write(struct i2c_client *client, char *writebuf, int writelen)
{
	int ret;
...
int fts_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		/*********Step 1:Reset  CTPM *****/
		/*write 0xaa to register 0xfc */
		pr_info("[FT5606] [Step 1:Reset  CTPM]\n");
		...
		delay_qt_ms(30);
		/*********Step 2:Enter upgrade mode *****/
		pr_info("[FT5606] [Step 2:Enter upgrade mode]\n");
		...
		/*********Step 3:check READ-ID***********************/
		pr_info("[FT5606] [Step 3:check READ-ID]\n");
		auc_i2c_write_buf[0] = 0x90;
		auc_i2c_write_buf[1] = auc_i2c_write_buf[2] = auc_i2c_write_buf[3] = 0x00;
		ft5x0x_i2c_Read(client, auc_i2c_write_buf, 4, reg_val, 2);
	...
	pr_info("[FT5606] [Step 4:erase app and panel paramenter area]\n");
	...
	pr_info("[FT5606] [Step 5:write firmware(FW) to ctpm flash]\n");
	bt_ecc = 0;
	...
	for (j = 0; j < packet_number; j++) {
		...
		ft5x0x_dma_i2c_Write(client, (char *)gpDMABuf_pa, FTS_PACKET_LENGTH + 6);
		delay_qt_ms(30);
	...
	if ((dw_lenth) % FTS_PACKET_LENGTH > 0) {
		...
		ft5x0x_dma_i2c_Write(client, (char *)gpDMABuf_pa, temp + 6);
		delay_qt_ms(30);
	}
	...
	for (i = 0; i < 6; i++) {
		...
		ft5x0x_dma_i2c_Write(client, (char *)gpDMABuf_pa, 7);
		delay_qt_ms(30);
	}
	...
	pr_info("[FT5606] [Step 6: read out checksum]\n");
	...
	if (reg_val[0] != bt_ecc) {
		...
		if (gpDMABuf_va) {
			...
			gpDMABuf_va = NULL;
			gpDMABuf_pa = (u32) NULL;
		}
		return -EIO;
	}

	/*********Step 7: reset the new FW***********************/
	pr_info("[FT5606] [Step 7: reset the new FW]\n");
...
int fts_ctpm_fw_upgrade_PRAM(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	u8 reg_val[2] = { 0 };
	u32 i = 0;
	u32 packet_number;
...
static ssize_t ft5x0x_tprwreg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	/*place holder for future use */
	return -EPERM;
}
				    ...
				    struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (0 != retval) {
		dev_err(&client->dev, "%s() - ERROR: Could not convert the "
			"given input to a number." "The given input was: \"%s\"\n", __func__, buf);
		goto error_return;
	}
...
static ssize_t ft5x0x_fwupdate_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	/* place holder for future use */
	return -EPERM;
}
...
static int ft5x0x_debug_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	case PROC_UPGRADE:
		...
		if (ret < 0)
			num_read_chars = sprintf(buf_debug, "%s", "get fw version failed.\n");
		else
			num_read_chars =
			    sprintf(buf_debug, "current fw version:0x%02x\n", regvalue);
		break;
	case PROC_READ_REGISTER:
		readlen = 1;
		ret = ft5x0x_i2c_Read(client, NULL, 0, buf_debug, readlen);
		if (ret < 0) {
			dev_err(&client->dev, "%s:read iic error\n", __func__);
			return ret;
		} else
			DBG("%s:value=0x%02x\n", __func__, buf_debug[0]);
		num_read_chars = 1;
		break;
	case PROC_RAWDATA:
		break;
	...
	memcpy(page, buf_debug, num_read_chars);
