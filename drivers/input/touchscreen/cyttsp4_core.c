...
static int power_delay = 100;
module_param(power_delay, int, S_IRUGO | S_IWUSR);
...
{
	if (cd->pdata->xres) {
		dev_dbg(cd->dev, "power_delay=%dms\n", power_delay);
		cd->pdata->xres(cd->pdata, cd->dev, power_delay);
...
static ssize_t cyttsp4_rst_count_show(struct device *dev,
				      struct device_attribute *attr,
				      char *buf)
{
	return snprintf(buf, CY_MAX_PRBUF_SIZE, "reset_count=%d\n", rst_count);
}
...
static struct device_attribute attributes[] = {
	...
	__ATTR(rst_count, S_IRUGO, cyttsp4_rst_count_show, NULL),
};
