...
{
	bool ret = 0;
	unsigned char flag = info->lowpower_flag & 0x0F;
	...
	if (flag & FTS_LOWP_FLAG_QUICK_CAM)
		tsp_debug_dbg(true, &info->client->dev, "%s: quick cam *- on\n", __func__);
	if (flag & FTS_LOWP_FLAG_2ND_SCREEN)
		tsp_debug_dbg(true, &info->client->dev, "%s: 2nd screen on\n", __func__);
	if (flag & FTS_LOWP_FLAG_BLACK_UI)
		tsp_debug_dbg(true, &info->client->dev, "%s: swipe finger on\n", __func__);
	if (flag & FTS_LOWP_FLAG_TEMP_CMD)
		tsp_debug_dbg(true, &info->client->dev, "%s: known cmd on\n", __func__);
