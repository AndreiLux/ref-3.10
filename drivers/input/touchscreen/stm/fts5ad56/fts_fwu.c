...
#define FW_IMAGE_NAME_NOBLE			"tsp_stm/noble.fw"
#define FW_IMAGE_NAME_ZEN			"tsp_stm/zen.fw"
...
int fts_fw_update_on_probe(struct fts_ts_info *info)
{
	...
	if (info->board->firmware_name){
		...
		if(((lcdtype & LCD_ID2_MODEL_MASK) == MODEL_NOBLE) && ((lcdtype & LCD_ID3_DIC_MASK) == S6E3HA3_NOBLE) ){
			tsp_debug_err(true, info->dev,"%s:FTS5AD56 - NOBLE\n", __func__);
			info->firmware_name = FW_IMAGE_NAME_NOBLE;
		}
		else if(((lcdtype & LCD_ID2_MODEL_MASK) == MODEL_ZEN) && ((lcdtype & LCD_ID3_DIC_MASK) == S6E3HA3_ZEN) ){
			tsp_debug_err(true, info->dev,"%s:FTS5AD56 - ZEN\n", __func__);
			info->firmware_name = FW_IMAGE_NAME_ZEN;
		}
			...
			/* ZERO's module is reused at NOBLE/ZEN  */
			if((strncmp(info->board->project_name, "NOBLE", 5) == 0) || (strncmp(info->board->project_name, "ZEN", 3) == 0)){
				if((lcdtype == S6E3HA2_WQXGA_ZERO)||(lcdtype == S6E3HA2_WQXGA_ZERO1)||(lcdtype == S6E3HA2_WQXGA_ZERO2)){
					tsp_debug_err(true, info->dev,"%s:Reused ZEROF module\n", __func__);
					info->firmware_name = FW_IMAGE_NAME_D2_Z1;	// f/w name is zerof
				}
				else if((lcdtype & 0xf0ff00) == 0x400000){
					tsp_debug_err(true, info->dev,"%s:Reused ZEROF other module\n", __func__);
					info->firmware_name = FW_IMAGE_NAME_D2_Z1;	// f/w name is zerof
				}
				else{
					tsp_debug_err(true, info->dev,"%s:There is not avaiable firmware or model for fts5\n", __func__);
					goto exit_fwload;
				}
			}
			else{
