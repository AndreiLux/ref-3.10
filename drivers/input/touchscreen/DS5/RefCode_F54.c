		...
		switch (cFunc){
			...
			case 0x54:
				if (!bHaveF54){
					...
					RxChannelCount = 31;
					TxChannelCount = 18;
					Write8BitRegisters(F54QueryBase, &RxChannelCount, 1);
					Write8BitRegisters((F54QueryBase+1), &TxChannelCount, 1);
...
bool TestPreparation(void)
{
	...
	//if (bHaveCtrl149)
	{
		...
		data = data & 0xFE;
		Write8BitRegisters(addr, &data, 1);
		TOUCH_INFO_MSG("bHaveCtrl149 addr(0x%x) val(0x%x)\n",addr,data);
	}
...
//		return -1;
	...
	if (!switchPage(0x01))
	{
		TOUCH_INFO_MSG("switchPage(0x01) return FALSE\n");
		return false;
	}
	data = 0x00;
