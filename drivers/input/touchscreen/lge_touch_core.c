...
//                                                                                 
#include <linux/platform_data/gpio-odin.h> //knock on
...
/* usb callback is not used on liger (synaptic S3508) */
//#define TOUCH_CONFIG_USB_ODIN_DRD

#ifdef TOUCH_CONFIG_USB_ODIN_DRD
#include <linux/usb/odin_usb3.h>
#endif

#ifdef LIGER_SUSPEND_KNOCK_ON
...
#if 1 /* Factory AAT - Touch draw test */
bool touch_self_test_flag = 0;
#endif
bool touch_irq_mask_sms = 1;
bool touch_irq_mask2 = 0;
bool touch_irq_mask2_sms = 0;


/* for thermal setting */
#define TOUCH_THERMAL_CHECK
int touch_thermal_status = 0;
int current_thermal_mode = 0;
#if 1 /* below temperature represents top pcb temperature of Liger */
/* 08-21 : warnning : this has to be modified after thermail tunning */
#define TOUCH_HIGH_TEMPERATURE	63000
#define TOUCH_LOW_TEMPERATURE	61000
#else
#define TOUCH_HIGH_TEMPERATURE	55000
#define TOUCH_LOW_TEMPERATURE	52000
#endif

/* Clear insensible touch finger */
#define INSENSIBLE_TOUCH_FINGER
...
void touch_enable_irq_sms(unsigned int irq)
{
#if 1 /* Factory AAT - Touch draw test */
	if(touch_self_test_flag)
		return;
#endif
	if(!touch_irq_mask_sms){
		touch_irq_mask_sms = 1;
		enable_irq(irq);
		TOUCH_INFO_MSG("enable_irq(%d)\n",irq);
	}

	TOUCH_DEBUG(DEBUG_IRQ_HANDLE,"irq(%d),touch_enable_irq_sms(%d)\n", irq,touch_irq_mask_sms);
}

void touch_disable_irq_sms(unsigned int irq)
{
#if 1 /* Factory AAT - Touch draw test */
	if(touch_self_test_flag)
		return;
#endif
	if(touch_irq_mask_sms){
		touch_irq_mask_sms = 0;
		disable_irq_nosync(irq);
		TOUCH_INFO_MSG("disable_irq_nosync(%d)\n",irq);
	}
	TOUCH_DEBUG(DEBUG_IRQ_HANDLE,"irq(%d),touch_disable_irq_sms(%d)\n", irq,touch_irq_mask_sms);
}
...
static void release_all_touch_event(struct lge_touch_data *ts)
{
	...
	if (atomic_read(&ts->state.rebase_state) == REBASE_DONE)
...
static int power_control(struct lge_touch_data *ts, int on_off)
{
	...
	TOUCH_DEBUG(DEBUG_BASE_INFO, "power_state[%d], mfts_enable(%d)\n", on_off,mfts_enable);
	return 0;
error:
	return -1;
}
...
{
	TOUCH_DEBUG(DEBUG_BASE_INFO, "interrupt[%d]_state[%d->%d]\n",
                  ts->client->irq,atomic_read(&ts->state.interrupt_state), on_off);
...
static int interrupt_control_sms(struct lge_touch_data *ts, int on_off)
{
	TOUCH_DEBUG(DEBUG_BASE_INFO, "interrupt[%d]_state_SMS[%d->%d]\n"
		,ts->pdata->irq_sms, atomic_read(&ts->state.interrupt_state), on_off);

       if (atomic_read(&ts->state.interrupt_state_sms) != on_off) {
               atomic_set(&ts->state.interrupt_state_sms, on_off);
               if (ts->pdata->role->wake_up_by_touch)
                       on_off ? enable_irq_wake(ts->pdata->irq_sms)
                               : disable_irq_wake(ts->pdata->irq_sms);
               else
                       on_off ? enable_irq(ts->pdata->irq_sms)
                               : disable_irq(ts->pdata->irq_sms);
       }
	   if(on_off)
		   touch_irq_mask2_sms = 1;
	   else
		   touch_irq_mask2_sms = 0;
...
static int touch_ic_init(struct lge_touch_data *ts, int is_error)
{
	...
	TOUCH_INFO_MSG("%s : current_thermal_mode = THERMAL_%s\n",__func__,  current_thermal_mode ? "HIGH" : "LOW");

	if (current_thermal_mode)
		queue_delayed_work(touch_wq, &ts_data->work_thermal, msecs_to_jiffies(0));

#ifdef INSENSIBLE_TOUCH_FINGER
	ts->pdata->touch_count_num = 0;
#endif
	memset(&ts->ts_curr_data, 0, sizeof(struct touch_data));
...
int ghost_detect_solution(struct lge_touch_data *ts)
{
	...
	/* first input in 200ms check after resume */
...
#ifdef TOUCH_CONFIG_USB_ODIN_DRD
static void usb_event_func(struct work_struct *work_usb_status)
{
	struct lge_touch_data *ts = container_of(to_delayed_work(work_usb_status), struct lge_touch_data, work_usb_status);
	...
	TOUCH_DEBUG(DEBUG_BASE_INFO, "usb_event_func**********\n");
	...
	if (ts->odin_otg) {
		cabletype = ts->odin_otg->cabletype;
	}else{
		TOUCH_DEBUG(DEBUG_BASE_INFO, "Tusb_event_notifier_callback s->odin_otg = NULL\n");
		return;
	}

	TOUCH_DEBUG(DEBUG_BASE_INFO, "TA Type from USB : %d, cabletype = %d\n", event, cabletype);
	/*below has to be checked  ************/
	/* below is only for two status (TA connected and TA disconnected) */
	/* but, cabletype can have 5 status like as below
	enum bc_cabletype_odin_otg {
           BC_CABLETYPE_UNDEFINED = 0, //Disconnect
           BC_CABLETYPE_SDP, //USB
           BC_CABLETYPE_DCP, //TA
           BC_CABLETYPE_CDP, //USB+TA
           BC_CABLETYPE_SLIMPORT, //Slimport
	};
	TA Connected = 1, TA Disconnected = 0 */
	cabletype ? (cabletype = 1):(cabletype = 0);

	if ((cabletype && touch_ta_status) || (!boot_mode))
		return;

	if(!is_probe || atomic_read(&ts->state.pm_state) > PM_RESUME)
		return;

	switch (event) {
		case ODIN_USB_CABLE_EVENT:
			touch_ta_status = cabletype;
			TOUCH_DEBUG(DEBUG_BASE_INFO, "TA Type(touch_ta_status): %d\n", touch_ta_status);
	...
	if (atomic_read(&ts->state.power_state) != POWER_OFF)
		touch_device_func->notify(ts->client, (u8)NOTIFY_TA_CONNECTION, cabletype);

	TOUCH_DEBUG(DEBUG_BASE_INFO, "code[%d] cabletype[%d] power_state[%d]\n",
						NOTIFY_TA_CONNECTION, cabletype, ts->state.power_state);
}

static void usb_event_notifier_callback(struct notifier_block *nb,
													unsigned long event, void *data)
{
	struct lge_touch_data *ts = container_of(nb, struct lge_touch_data, odin_nb);

	ts->usb_event = event;
	TOUCH_DEBUG(DEBUG_BASE_INFO, "usb_event_notifier_callback event() : %d\n", event);
	queue_delayed_work(touch_wq, &ts->work_usb_status, 0);
}

static void usb_event_callback_register(struct lge_touch_data *ts)
{
	...
	ts->odin_otg = usb3_otg;
	ts->odin_nb.notifier_call = usb_event_notifier_callback;

	ret = usb_register_notifier(phy, &ts->odin_nb);
	if (ret)
		TOUCH_ERR_MSG("usb_event_callback_register fail\n");
	...
	TOUCH_DEBUG(DEBUG_BASE_INFO, "code[%d] cabletype[%d] power_state[%d]\n",
						NOTIFY_TA_CONNECTION, usb3_otg->cabletype, ts->state.power_state);
}
#endif /* TOUCH_CONFIG_USB_ODIN_DRD */
...
#ifdef LIGER_SUSPEND_KNOCK_ON
unsigned int prev_proximity_status = -1;
bool direct_proximity_check = false;
static void get_proximity_status(struct work_struct *proximity_check)
{
	struct lge_touch_data *ts = container_of(to_delayed_work(proximity_check),
		struct lge_touch_data, proximity_check);

	int value[4] = {0,};

	TOUCH_TRACE();

	if(prev_proximity_status == ps_real_detection) {
		return;
	}

	direct_proximity_check = true;
	prev_proximity_status = ps_real_detection;

	/* ps_real_detection: near=1 / far=0 */
	value[0] = ps_real_detection ? 0:1; /* mode: disable=0 / double tap=1 / knock code=2 */
	value[1] = 0; /* screen: off=0 / on=1 */
	value[2] = ps_real_detection ? 0:1; /* sensor: near(not available)=0 / far(available)=1 */
	value[3] = quick_cover_status; /* qcover: opened=0 / closed=1 */

	TOUCH_INFO_MSG("value[0]=%d, value[1]=%d, value[2]=%d, value[3]=%d\n" , value[0], value[1], value[2], value[3]);
	if (touch_device_func->lpwg) {
		mutex_lock(&ts->suspend_resume_lock);
		touch_device_func->lpwg(ts->client,
				LPWG_ENABLE, value[0], NULL);
		touch_device_func->lpwg(ts->client,
				LPWG_SENSOR_STATUS, value[2], NULL);
		mutex_unlock(&ts->suspend_resume_lock);
	} else {
		TOUCH_ERR_MSG("%s: touch_device_func->lpwg is not available\n", __FUNCTION__);
	}
	return;
}
#endif
...
static irqreturn_t touch_thread_irq_handler(int irq, void *dev_id)
{
	...
	TOUCH_TRACE();
#if 1 /* wake_lock_added_on_odin */
	if(wake_lock_active(&ts->int_wake_lock))
		wake_unlock(&ts->int_wake_lock);

	wake_lock(&ts->int_wake_lock);
#endif
	mutex_lock(&ts->suspend_resume_lock); //mutex_added_on_odin
	mutex_lock(&ts->thread_lock);
...
#ifndef TOUCH_THERMAL_CHECK
	if ((current_thermal_mode != touch_thermal_status) && (!ts->ts_report_data.total_num)) {
		current_thermal_mode = touch_thermal_status;
		TOUCH_INFO_MSG("current_thermal_mode is changed: THERMAL_%s, top_temp = %ld\n",
							current_thermal_mode ? "LOW->HIGH" : "HIGH->LOW", top_temp);
		queue_delayed_work(touch_wq, &ts_data->work_thermal, msecs_to_jiffies(0));
	}
#endif
do_not_report:
...
ignore:
	mutex_unlock(&ts->thread_lock);
	mutex_unlock(&ts->suspend_resume_lock); //mutex_added_on_odin
#if 1 /* wake_lock_added_on_odin */
	wake_unlock(&ts->int_wake_lock);
#endif
	return IRQ_HANDLED;
...
static irqreturn_t touch_thread_irq_handler_sms(int irq, void *dev_id)
{
       struct lge_touch_data *ts = (struct lge_touch_data *)dev_id;
       enum error_type ret = NO_ERROR;
       enum ghost_error_type ghost_alg_ret = NO_ACTION;
...
error:
       ...
       safety_reset(ts);
       touch_ic_init(ts, 0);
       TOUCH_ERR_MSG("SMS Interrupt Handling fail\n");
#if 1 /* wake_lock_added_on_odin */
	wake_unlock(&ts->int_wake_lock);
#endif
       return IRQ_NONE;
...
static irqreturn_t touch_irq_handler_sms(int irq, void *dev_id)
{
       struct lge_touch_data *ts = (struct lge_touch_data *)dev_id;
       ...
       if (atomic_read(&ts->state.pm_state) == PM_SUSPEND) {
               atomic_set(&ts->state.pm_state, PM_SUSPEND_IRQ);
               TOUCH_DEBUG(DEBUG_BASE_INFO, "SMS interrupt in suspend");
...
#ifdef TOUCH_THERMAL_CHECK
	unsigned long top_temp = 0;
#endif
	TOUCH_DEBUG(DEBUG_TRACE, "%s() start\n", __func__);

#ifdef TOUCH_THERMAL_CHECK
	get_pcb_therm_top_temp(&top_temp);
	TOUCH_DEBUG(DEBUG_TRACE, "top_temp = %ld\n", top_temp);

	if (top_temp >= TOUCH_HIGH_TEMPERATURE)
			touch_thermal_status = 1;
	else if (top_temp <= TOUCH_LOW_TEMPERATURE)
			touch_thermal_status = 0;
#endif

//                                                  
// block code no "pseudo_batt_info.mode", it must be checked existing substituable variable in LGAP.
/*
...
#ifdef TOUCH_THERMAL_CHECK
		queue_delayed_work(touch_wq, &ts_data->work_thermal, msecs_to_jiffies(1000));
	} else {
		queue_delayed_work(touch_wq, &ts_data->work_thermal, msecs_to_jiffies(1000));
	}
#endif
...
void set_quick_cover_reg(struct lge_touch_data *ts, int mode)
{
	u8 udata[2] = {0};
	u8 pdata[2] = {0};

	if (mode == QUICKCOVER_CLOSE) {
		touch_i2c_read(ts->client, 0x17, 2, pdata);
		udata[0] = 0x75; /* Finger Amplitude Threshold */
		udata[1] = 0x75; /* Small Finger Amplitude Threshold */
		pdata[1] = 0x55; /* Minimum Peak Amplitude */
		touch_i2c_write(ts->client, 0x1a, 2, udata);
		touch_i2c_write(ts->client, 0x17, 2, pdata);
	} else if (mode == QUICKCOVER_OPEN) {
		touch_i2c_read(ts->client, 0x17, 2, pdata);
		udata[0] = 0x64; /* Finger Amplitude Threshold */
		udata[1] = 0x64; /* Small Finger Amplitude Threshold */
		pdata[1] = 0x3C; /* Minimum Peak Amplitude */
		touch_i2c_write(ts->client, 0x1a, 2, udata);
		touch_i2c_write(ts->client, 0x17, 2, pdata);
	} else {
		TOUCH_ERR_MSG("%s: Cannot notify quick cover status: %d\n", __func__, mode);
	}
}
...
static ssize_t show_upgrade(struct i2c_client *client, char *buf)
{
	...
	if(lge_get_boot_mode()==LGE_BOOT_MODE_FACTORY || lge_get_boot_mode()== LGE_BOOT_MODE_CHARGERLOGO) {
		mfts_mode = 1;
		TOUCH_DEBUG(DEBUG_BASE_INFO, "FW upgrade in Factory Mode\n");
	}

	len = strlen(ts->pdata->inbuilt_fw_name_s3508);
	strncpy(ts->fw_info.fw_path_s3508, ts->pdata->inbuilt_fw_name_s3508, len);
...
#ifdef INSENSIBLE_TOUCH_FINGER
	if(type == 6 && value[0] == 1) { // if LCD ON
		release_all_touch_event(ts);
	}
#endif

#ifdef LIGER_SUSPEND_KNOCK_ON
	if(direct_proximity_check && (type == 1 || type == 7 || type == 9)) {
		if(type == 9) {
			direct_proximity_check = false;
		}
		return count;
	}
#endif
...
static LGE_TOUCH_ATTR(fw_upgrade, 0640, show_upgrade, store_upgrade);
...
#if 0 /* not used */
static void irq_set_pending(unsigned int irq)
{
...
static struct touch_platform_data* get_dts_data(struct device *dev)
{
	...
	//firmware
	p_data->inbuilt_fw_name_s3508 = NULL;
...
/*                                                             */
#if 1
	p_data->inbuilt_fw_name_s3508= "synaptics/PLG395-V0.31-PR1721505-DS5.1.5.0.1038_30060131.img";
	p_data->inbuilt_fw_name = "synaptics/PLG313-V1.06-PR1650170-DS5.5.1.0.1066_30055186_S3528A0.img";
	p_data->inbuilt_fw_name_s3621 =  "synaptics/PLG298-F004.img";
	p_data->inbuilt_fw_name_s3528_a1 = "synaptics/PLG313-V1.22-PR1653201-DS5.5.1.0.1066_30055196_S3528A1.img";
	p_data->inbuilt_fw_name_s3528_a1_suntel = "synaptics/PLG391-V1.03-PR1653201-DS5.5.1.0.1066_10055183_S3528A1.img";
	p_data->inbuilt_fw_name_s3528_a1_factory = "synaptics/PLG313-V1.22-PR1653201-DS5.5.1.0.1066_30055196_S3528A1.img";
#else
...
/*                                                             */
#if 1
	p_data->panel_spec = "synaptics/liger_limit.txt";
#else
...
/*                                                           */
        ...
        if(prop) {
            ...
            if (temp_val <= 4) {
                ...
                for (j = 0; j < temp_val; j++) {
                    ...
                    TOUCH_DEBUG(DEBUG_TRACE,"[%s]DT : ref_chk_option[%d] = %d \n",__FUNCTION__, j, temp_array[j]);
                }
            }
        }
    }
	...
	#else
	GET_PROPERTY_U32(np, "reset-gpio", p_data->reset_pin);
	GET_PROPERTY_U32(np, "irq-gpio", p_data->int_pin);
	GET_PROPERTY_U32(np, "irq-gpio-sms", p_data->int_pin_sms);
	#endif
	/*                                                           */
	...
	struct attribute **attribute_list)
{
	...
	TOUCH_DEBUG(DEBUG_TRACE, "n1[%d] n2[%d]\n", n1, n2);
...
static int touch_suspend(struct device *dev)
{
	...
	touch_enable_irq_sms(ts->pdata->irq_sms);
...
static int touch_resume(struct device *dev)
{
	...
	touch_disable_irq_sms(ts->pdata->irq_sms);
	...
	if(mfts_enable){
		TOUCH_DEBUG(DEBUG_BASE_INFO, "%s : 1 mfts_enable\n",__func__);
		power_control(ts, POWER_ON);
		touch_i2c_write_byte(ts->client, 0x25, 1);
		TOUCH_DEBUG(DEBUG_BASE_INFO, "%s : 2 after SW RESET\n",__func__);
	...
	const struct i2c_device_id *id)
{
	...
	if(ts->pdata->int_pin_sms > 0) {
		DO_SAFE(ret = gpio_request(ts->pdata->int_pin_sms,
			"touch_int_sms"), error);
		gpio_direction_input(ts->pdata->int_pin_sms);
	}
	...
	wake_lock_init(&ts->int_wake_lock, WAKE_LOCK_SUSPEND, "touch_wakelock");	//wake_lock_added_on_odin

	mutex_init(&ts->thread_lock); // it should be initialized before both init_func and enable_irq
	mutex_init(&ts->suspend_resume_lock); //mutex_added_on_odin
...
#ifdef TOUCH_CONFIG_USB_ODIN_DRD
	INIT_DELAYED_WORK(&ts->work_usb_status, usb_event_func);
#endif

#ifdef LIGER_SUSPEND_KNOCK_ON
	INIT_DELAYED_WORK(&ts->proximity_check, get_proximity_status);
#endif
	...
	if (ts->pdata->role->protocol_type == MT_PROTOCOL_A)
		...
		DO_SAFE(input_mt_init_slots(ts->input_dev,
			ts->pdata->caps->max_id,0), err_mt_init_slots); //sora.jin
	...
	ts->client->irq = gpio_to_irq(ts->pdata->int_pin);
	DO_SAFE(ret = request_threaded_irq(ts->client->irq,
		touch_irq_handler, touch_thread_irq_handler,
		/*ts->pdata->role->irqflags*/ IRQF_TRIGGER_LOW | IRQF_ONESHOT, ts->client->name, ts),
		err_request_threaded_irq);
	interrupt_control(ts, INTERRUPT_ENABLE);

	ts->pdata->irq_sms = gpio_to_irq(ts->pdata->int_pin_sms);
	DO_SAFE(ret = odin_gpio_sms_request_threaded_irq(ts->pdata->irq_sms,
		touch_irq_handler_sms, touch_thread_irq_handler_sms,
		ts->pdata->role->irqflags | IRQF_ONESHOT, "s3528_sms", ts),
		err_request_threaded_irq);
	interrupt_control_sms(ts, INTERRUPT_DISABLE);

	// if init was failed, firmware will be re-downloaded.
	if (touch_ic_init(ts, 0) < 0)
		ts->fw_info.fw_force_upgrade = 1;

	len_3508 =  strlen(ts->pdata->inbuilt_fw_name_s3508);
	...
	strncpy(ts->fw_info.fw_path_s3508, ts->pdata->inbuilt_fw_name_s3508, len_3508);
...
#ifdef TOUCH_CONFIG_USB_ODIN_DRD
	usb_event_callback_register(ts);
	ts->odin_nb.notifier_call = usb_event_notifier_callback;
#endif
...
static int touch_remove(struct i2c_client *client)
{
	...
	wake_lock_destroy(&ts->int_wake_lock);	//wake_lock_added_on_odin
...
#ifdef LIGER_SUSPEND_KNOCK_ON
	TOUCH_INFO_MSG("%s: prev= %u, now= %u\n", __FUNCTION__, prev_proximity_status, ps_real_detection);
	prev_proximity_status = ps_real_detection;
#endif
...
#if 0 /* not used */
/* //sora.jin temporally block code until find another method to handle irq while i2c initial delay.
...
#ifdef LIGER_SUSPEND_KNOCK_ON
	TOUCH_INFO_MSG("%s: prev= %u, now= %u\n", __FUNCTION__, prev_proximity_status, ps_real_detection);
	queue_delayed_work(touch_wq, &ts->proximity_check, msecs_to_jiffies(50));
#endif
	...
	struct of_device_id *match_table)
{
	...
	TOUCH_DEBUG(DEBUG_BASE_INFO, "touch_driver_register\n");
