...
#define TOUCH_PROX_MSG(fmt, args...) \
	pr_info("[Touch-PRX] %s: " fmt, __func__, ##args)
...
static int prox_i2c_read(struct synaptics_ts_data *ts,
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	if (retval < 0) {
		TOUCH_PROX_MSG(
				"Failed to read from the register(addr=0x%04x)\n",
				addr);
		return retval;
	}
...
static int prox_i2c_write(struct synaptics_ts_data *ts,
		unsigned short addr, unsigned char *data, unsigned short length)
{
	int retval;
	u8 page_old = 0;
...
static int prox_reg_init(void)
{
	...
	retval = prox_i2c_read(ts, (prox->query_base_addr + 5),
	...
	ctrl_23_offset =
		query_5.ctrl_00_is_present +
		query_5.ctrl_01_is_present +
		query_5.ctrl_02_is_present +
		query_5.ctrl_03_is_present +
		query_5.ctrl_04_is_present +
		query_5.ctrl_05_is_present +
		query_5.ctrl_06_is_present +
		query_5.ctrl_07_is_present +
		query_5.ctrl_08_is_present +
		query_5.ctrl_09_is_present +
		query_5.ctrl_10_is_present +
		query_5.ctrl_11_is_present +
		query_5.ctrl_12_is_present +
		query_5.ctrl_13_is_present +
		query_5.ctrl_14_is_present +
		query_5.ctrl_15_is_present +
		query_5.ctrl_16_is_present +
		query_5.ctrl_17_is_present +
		query_5.ctrl_18_is_present +
		query_5.ctrl_19_is_present +
		query_5.ctrl_20_is_present +
		query_5.ctrl_21_is_present +
		query_5.ctrl_22_is_present;

	prox->hover_finger_en_addr = prox->control_base_addr + ctrl_23_offset;

	TOUCH_PROX_MSG(
			"prox->hover_finger_en_addr=0x%04X\n",
			prox->hover_finger_en_addr);

	/* hover_finger_data_addr setting */
	retval = prox_i2c_read(ts, (prox->query_base_addr + 8),
	...
	TOUCH_PROX_MSG(
			"prox->hover_finger_data_addr=0x%04X\n",
...
static int prox_scan_pdt(void)
{
	...
	for (page = 0; page < PAGES_TO_SERVICE; page++) {
		for (addr = PDT_START; addr > PDT_END; addr -= PDT_ENTRY_SIZE) {
			...
			retval = prox_i2c_read(ts,
					addr, fd.data, sizeof(fd.data));
...
f12_found:
	...
	retval = prox_i2c_read(ts, prox->interrupt_enable_0_addr,
			&interrupt_enable_0, sizeof(interrupt_enable_0));

	if (retval < 0) {
		TOUCH_PROX_MSG(
				"Failed to read from interrupt_enable_0 register\n");
		return retval;
	}

	interrupt_enable_0 |= prox->intr_mask;

	retval = prox_i2c_write(ts, prox->interrupt_enable_0_addr,
			&interrupt_enable_0, sizeof(interrupt_enable_0));

	if (retval < 0) {
		TOUCH_PROX_MSG(
				"Failed to write to interrupt_enable_0 register\n");
		return retval;
	}
...
static int prox_set_hover_finger_en(bool enable)
{
	...
	retval = prox_i2c_read(ts, prox->hover_finger_en_addr,
			&object_report_enable, sizeof(object_report_enable));

	if (retval < 0) {
		TOUCH_PROX_MSG(
				"Failed to read from object_report_enable register\n");
		return retval;
	}

	if (enable == prox->hover_finger_en) {
		TOUCH_PROX_MSG(
				"enable(%d) and prox->hover_finger_en(%d) values are equal.\n",
	...
	retval = prox_i2c_write(ts, prox->hover_finger_en_addr,
			&object_report_enable, sizeof(object_report_enable));

	if (retval < 0) {
		TOUCH_PROX_MSG(
				"Failed to write to object_report_enable register\n");
		return retval;
	}
...
static void prox_set_params(void)
{
	...
	input_set_abs_params(prox->prox_dev, ABS_X,
			0, (int)prox->ts_data->pdata->caps->max_x, 0, 0);
	input_set_abs_params(prox->prox_dev, ABS_Y,
			0, (int)prox->ts_data->pdata->caps->max_y, 0, 0);
	input_set_abs_params(prox->prox_dev, ABS_DISTANCE,
			0, (int)HOVER_Z_MAX, 0, 0);
...
static void prox_hover_finger_report(void)
{
	...
	retval = prox_i2c_read(ts, prox->hover_finger_data_addr,
			data->proximity_data, sizeof(data->proximity_data));
