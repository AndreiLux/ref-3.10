...
#define __mxt_patch_debug(_data, ...)	do{} while(0);
#else
...
#define __mxt_patch_ddebug(_data, ...)	do{} while(0);
#else
				...
				bool do_action)
{
	if(do_action){
		__mxt_patch_debug(data, "TA:%d FC:%d AR:%d AP:%d"
			" SM:%d TC:%d AT:%d KC:%d KV:%d EV:%d SC:%d LC:%d\n",
			...
			data->patch.src_item[11],data->patch.src_item[12]);	//20131227
	}
}
...
static int mxt_patch_run_stage(struct mxt_data *data)
{
	...
	if (data->patch.tline_addr) {
		...
		data->patch.tline_addr = NULL;
	}
	if (data->patch.check_cnt) {
		kfree(data->patch.check_cnt);
		data->patch.check_cnt = NULL;
	}
