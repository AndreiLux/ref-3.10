...
static ssize_t show_normal_calibration(struct i2c_client *client, char *buf)
{
	...
	if (power_state == POWER_ON || power_state == POWER_WAKE) {
		...
		calibration_status = (buffer & 0x01);
		crc_status = (buffer & 0x02) >> 1;
		...
		if (calibration_status == 0) {
			...
			if (!crc_status) {
				...
				TOUCH_ERR_MSG("Checksum of calibration values is bad, Need to LPWG Calibration \n");
				ret += snprintf(buf + ret, PAGE_SIZE - ret,
						"CRC_status = %d\n", crc_status);
				ret += snprintf(buf + ret, PAGE_SIZE - ret,
						"Checksum of calibration values is bad, Need to LPWG Calibration \n");
			}
...
static ssize_t store_normal_calibration(struct i2c_client *client, const char *buf, size_t count)
{
	...
	if (power_state == POWER_ON || power_state == POWER_WAKE) {
		...
		TOUCH_INFO_MSG("Start Normal Calibration\n");
...
static ssize_t show_get_calibration(struct i2c_client *client, char *buf)
{
	...
	err = synaptics_ts_page_data_read(client,
					...
					1, &buffer);
	if (err < 0) {
		TOUCH_ERR_MSG("Failed to read F54 Data Base\n");
		goto error;
	}
	...
	err = synaptics_ts_page_data_write_byte(client,
				...
				start_get_cal_data);
	if (err < 0) {
		TOUCH_ERR_MSG("Failed to write F54 Data Base\n");
		goto error;
	}
	...
	err = synaptics_ts_page_data_write_byte(client,
				...
				fifo_init);
	if (err < 0) {
		TOUCH_ERR_MSG("Failed to write F54_FIFO_INDEX_LSB\n");
		goto error;
	}
	...
	err = synaptics_ts_page_data_write_byte(client,
				...
				fifo_init);
	if (err < 0) {
		TOUCH_ERR_MSG("Failed to write F54_FIFO_INDEX_MSB\n");
		goto error;
	}
	...
	for(freq = 0; freq < 3; freq++){
		...
		err = synaptics_ts_page_data_write_byte(client,
					...
					buffer);
		if (err < 0) {
			TOUCH_ERR_MSG("Failed to write F54 CMD Base\n");
			goto error;
		}
		...
		if (err < 0) {
			TOUCH_ERR_MSG("Failed to read F54_REPORT_DATA\n");
			goto error;
		}
		...
		err = synaptics_ts_page_data_read(client,
						ANALOG_PAGE, F54_REPORT_DATA,
						MAX_ND_CAL_DATA_SIZE, &nd_cal_data[0]);
		if (err < 0) {
			TOUCH_ERR_MSG("Failed to read calibration_status_reg\n");
			goto error;
		}
	...
	mutex_unlock(&ts->pdata->thread_lock);
	TOUCH_INFO_MSG("Cal Data Extract Complete.");
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
...
error:
	TOUCH_ERR_MSG("Fail to get Calibration Data");
	ret += snprintf(buf + ret, PAGE_SIZE - ret,
