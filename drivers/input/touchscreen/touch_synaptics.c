...
extern bool touch_irq_mask_sms;
extern bool touch_irq_mask2;
extern bool touch_irq_mask2_sms;
...
extern bool touch_self_test_flag;
#endif
...
void touch_enable_irq(unsigned int irq)
{
	...
	TOUCH_DEBUG(DEBUG_IRQ_HANDLE,"irq(%d),touch_irq_mask(%d)\n", irq,touch_irq_mask);
}
...
void write_firmware_version_log(struct synaptics_ts_data *ts)
{
    ...
    if (!(strncmp(ts->fw_info.fw_product_id, "PLG395", 6)) || !(strncmp(ts->fw_info.fw_product_id, "PLG313", 6)) || !(strncmp(ts->fw_info.fw_product_id, "PLG352", 6)) || !(strncmp(ts->fw_info.fw_product_id, "PLG391", 6))) {
...
static int get_tci_data(struct synaptics_ts_data *ts, int count)
{
	...
	for (i=0; i<count; i++) {
		...
		TOUCH_DEBUG(DEBUG_BASE_INFO || DEBUG_LPWG, "LPWG data(%d)(%d,%d)\n",i,ts->pw_data.data[i].x,ts->pw_data.data[i].y);
	}
...
/* print info */
	for (i = 0; i < 32; i++) {
		if (f12_info.ctrl_reg_is_present[i])
			TOUCH_DEBUG(DEBUG_TRACE,"f12_info.ctrl_reg_addr[%d]=0x%02X\n", i, f12_info.ctrl_reg_addr[i]);
	}

	for (i = 0; i < 16; i++) {
		if (f12_info.data_reg_is_present[i])
			TOUCH_DEBUG(DEBUG_TRACE,"f12_info.data_reg_addr[%d]=0x%02X\n", i, f12_info.data_reg_addr[i]);
	}
...
static ssize_t show_firmware(struct i2c_client *client, char *buf)
{
	...
	if (!(strncmp(ts->fw_info.fw_product_id, "PLG395", 6))) {
		ret += sprintf(buf+ret, "Touch IC : s3508\n\n");
...
static ssize_t show_sd(struct i2c_client *client, char *buf)
{
	...
	if (power_state == POWER_ON || power_state == POWER_WAKE) {
		...
		touch_disable_irq(ts->client->irq);
#if 1 /* Factory AAT - Touch draw test */
		touch_self_test_flag = true;
		TOUCH_INFO_MSG("%s: set touch_self_test_flag TRUE\n", __FUNCTION__);
#endif
		SCAN_PDT();
...
#if 1 /* Factory AAT - Touch draw test */
		touch_self_test_flag = false;
		TOUCH_INFO_MSG("%s: set touch_self_test_flag FALSE\n", __FUNCTION__);
#endif
...
static ssize_t show_touch_enable_irq(struct i2c_client *client, char *buf)
{
	int ret = 0;

	ret += sprintf(buf+ret, "show_touch_enable_irq\n");
	ret += sprintf(buf+ret, "[irq]%s, [irq_sms]%s\n", touch_irq_mask ? "enabled":"disabled",touch_irq_mask_sms ? "enabled":"disabled");
	ret += sprintf(buf+ret, "[irq2]%s, [irq_sms2]%s\n", touch_irq_mask2 ? "enabled":"disabled",touch_irq_mask2_sms ? "enabled":"disabled");

	return ret;
}

static ssize_t store_touch_enable_irq(struct i2c_client *client, const char *buf, size_t count)
{
	struct lge_touch_data *ts = i2c_get_clientdata(client);

	unsigned char string[30] = {0};
	unsigned char c_irq[30] = {0};

	sscanf(buf, "%s %s", string, c_irq);
	TOUCH_INFO_MSG("%s %s", string, c_irq);

	if (!strcmp(string, "enable")) {
		if (!strcmp(c_irq, "irq"))
			touch_enable_irq(ts->client->irq);
		else if (!strcmp(c_irq, "irq_sms"))
			touch_enable_irq_sms(ts->pdata->irq_sms);
		else
			TOUCH_INFO_MSG("No parameter\n");
	} else if (!strcmp(string, "disable")) {
		if (!strcmp(c_irq, "irq"))
			touch_disable_irq(ts->client->irq);
		else if (!strcmp(c_irq, "irq_sms"))
...
static LGE_TOUCH_ATTR(enable_irq, S_IRUGO | S_IWUSR, show_touch_enable_irq, store_touch_enable_irq);
...
static struct attribute *lge_specific_touch_attribute_list[] = {
	...
	&lge_touch_attr_enable_irq.attr,
	NULL,
};
...
static int get_ic_info(struct synaptics_ts_data *ts)
{
	...
	if (!(strncmp(ts->fw_info.fw_product_id, "PLG395", 6))) {
			rc = request_firmware(&fw_entry, ts->pdata->inbuilt_fw_name_s3508, &ts->client->dev);
			ts->fw_flag = S3508;
			if (rc != 0) {
				TOUCH_ERR_MSG("request_firmware() failed %d\n", rc);
				goto error;
			}
	} else if (!(strncmp(ts->fw_info.fw_product_id, "PLG313", 6))
...
#if 1 /* memory leak fix */
	if (fw_entry)
		release_firmware(fw_entry);
#endif
	TOUCH_DEBUG(DEBUG_BASE_INFO, "release_firmware success\n");
	return 0;
error:
#if 0 /* memory leak fix */
...
enum error_type synaptics_ts_init(struct i2c_client *client)
{
	...
	DO_SAFE(touch_i2c_read(client, DEVICE_CONTROL_REG, 1, &curr), error);
	next = curr |(DEVICE_CONTROL_NORMAL_OP| DEVICE_CONTROL_CONFIGURED);
	TOUCH_DEBUG(DEBUG_BASE_INFO, "%s DEVICE_CONTROL_REG ( 0x%x -> 0x%x)\n", __func__, curr, next);
	DO_SAFE(touch_i2c_write_byte(client, DEVICE_CONTROL_REG, next), error);
	...
	if (!(strncmp(ts->fw_info.fw_product_id, "PLG395", 6)) || !(strncmp(ts->fw_info.fw_product_id, "PLG313", 6)) || !(strncmp(ts->fw_info.fw_product_id, "PLG352", 6)) || !(strncmp(ts->fw_info.fw_product_id, "PLG391", 6))) {
		if(ts->pdata->role->use_lpwg_all) {
			if(atomic_read(&ts->state->safety_reset) == SAFETY_RESET_FINISH)
				ts->lpwg_ctrl.screen = 1;
...
static int synaptics_ts_noise_log(struct i2c_client *client, struct touch_data* curr_data, const struct touch_data* prev_data)
{
	...
	DO_SAFE((synaptics_ts_page_data_read(client, ANALOG_PAGE, 0x04, 1, &buf1)), error);
	DO_SAFE((synaptics_ts_page_data_read(client, ANALOG_PAGE, 0x05, 1, &buf2)), error);
	im = (buf2<<8)|buf1;
	im_sum += im;

	DO_SAFE((synaptics_ts_page_data_read(client, ANALOG_PAGE, 0x08, 1, &cns)), error);
	cns_sum += cns;

	DO_SAFE((synaptics_ts_page_data_read(client, ANALOG_PAGE, 0x09, 2, buffer)), error);
	cid_im = (buffer[1]<<8)|buffer[0];
	cid_im_sum += cid_im;

	DO_SAFE((synaptics_ts_page_data_read(client, ANALOG_PAGE, 0x0A, 2, buffer)), error);
...
enum error_type synaptics_ts_get_data(struct i2c_client *client, struct touch_data* curr_data, const struct touch_data* prev_data)
{
...
#if 1 /* Factory AAT - Touch draw test */
	if(touch_self_test_flag) {
		TOUCH_INFO_MSG("%s: touch_self_testing......\n", __FUNCTION__);
		return IGNORE_EVENT;
	}
#endif
	if (!ts->is_init)
	...
	DO_SAFE(touch_i2c_read(client, INTERRUPT_STATUS_REG,
		sizeof(temp_reg), &temp_reg), error);
...
enum error_type synaptics_ts_power(struct i2c_client *client, int power_ctrl)
{
	...
	case POWER_OFF:
			...
			{
				TOUCH_DEBUG(DEBUG_BASE_INFO, "LGE_BOOT_MODE_CHARGERLOGO\n");
				touch_disable_irq(ts->client->irq);
				touch_disable_irq_sms(ts->pdata->irq_sms);
			}
			#endif
	...
	case POWER_WAKE:
			...
			TOUCH_INFO_MSG("LIGER : IC SW RESET\n");
			#endif
			break;
	default:
			break;
	...
	if (info->fw_force_upgrade_cat) {
		if (mfts_mode) {
			...
			goto compare;
			 ...
			 if (ts->fw_flag == S3508) {
				memcpy(path, info->fw_path_s3508, sizeof(path));
...
enum error_type synaptics_ts_notify(struct i2c_client *client, u8 code, u32 value)
{
	...
	TOUCH_DEBUG(DEBUG_BASE_INFO, "synaptics_ts_notify code: %d value : %d\n", code, value);
	...
	case NOTIFY_TA_CONNECTION:
		{
			...
			newbuf = buf & ~CHARGER_CONNECTED;
			newbuf |= value ? CHARGER_CONNECTED : 0;
			if (newbuf != buf) {
				if (unlikely(touch_i2c_write_byte(client, DEVICE_CONTROL_REG, newbuf) < 0))
				{
					TOUCH_ERR_MSG("IC Reset command write fail\n");
					return -EIO;
				}
				TOUCH_DEBUG(DEBUG_BASE_INFO, "synaptics_ts_notify write success newbuf : 0x%x\n", newbuf);
			}
		}
		break;
...
enum error_type synaptics_ts_resume(struct i2c_client *client)
{
	...
	synaptics_ts_power(client, POWER_OFF);
...
enum error_type synaptics_ts_lpwg(struct i2c_client* client, u32 code, u32 value, struct point *data)
{
	...
	case LPWG_LCD_Y:
		...
		break;
	case LPWG_ACTIVE_AREA_X1: //Liger LPWG Active Area spec (3mm from edge)  [0/1080/0/1920] => [45/1035/45/1875]
		if((value == 0) && (ts->fw_flag == S3508)) {
			value = 45; }
		...
		for(i=0; i<2; i++){
			synaptics_ts_page_data_read(client, COMMON_PAGE, doubleTap_area_reg_addr, i+1, buffer);
			if(i==0)	temp_buf= buffer[i];
			else	temp_buf += buffer[i] << 8 ;
		}
		TOUCH_DEBUG(DEBUG_LPWG, "%s : LPWG_ACTIVE_AREA_X1(%d)(0x%x)\n", __func__, value,temp_buf);
		break;
	case LPWG_ACTIVE_AREA_X2:
		if((value == ts->pdata->caps->max_x) && (ts->fw_flag == S3508)) {
			value = 1035; }
		for(i=4; i<6; i++){
			synaptics_ts_page_data_read(client, COMMON_PAGE, doubleTap_area_reg_addr, i+1, buffer);
		...
		for(i=4; i<6; i++){
			synaptics_ts_page_data_read(client, COMMON_PAGE, doubleTap_area_reg_addr, i+1, buffer);
			if(i==4)	temp_buf= buffer[i];
			else	temp_buf += buffer[i] << 8 ;
		}
		TOUCH_DEBUG(DEBUG_LPWG, "%s : LPWG_ACTIVE_AREA_X2(%d)(0x%x)\n", __func__, value,temp_buf);
		break;
	case LPWG_ACTIVE_AREA_Y1:
		...
		for(i=2; i<4; i++){
			synaptics_ts_page_data_read(client, COMMON_PAGE, doubleTap_area_reg_addr, i+1, buffer);
			if(i==2)	temp_buf = buffer[i];
			else	temp_buf += buffer[i] << 8 ;
		}
		TOUCH_DEBUG(DEBUG_LPWG, "%s : LPWG_ACTIVE_AREA_Y1(%d)(0x%x)\n", __func__, value,temp_buf);
		break;
	case LPWG_ACTIVE_AREA_Y2:
		if((value ==  ts->pdata->caps->max_y) && (ts->fw_flag == S3508)) {
			value = 1875; }
		// Quick Cover Area
		for(i=6; i<8; i++){
			synaptics_ts_page_data_read(client, COMMON_PAGE, doubleTap_area_reg_addr, i+1, buffer);
		...
		for(i=6; i<8; i++){
			synaptics_ts_page_data_read(client, COMMON_PAGE, doubleTap_area_reg_addr, i+1, buffer);
			if(i==6)	temp_buf = buffer[i];
			else	temp_buf += buffer[i] << 8 ;
		}
		TOUCH_DEBUG(DEBUG_LPWG, "%s : LPWG_ACTIVE_AREA_Y2(%d)(0x%x)\n", __func__, value,temp_buf );
		sleep_control(ts, 1, 0);
		break;
	case LPWG_TAP_COUNT:
...
static int set_doze_param(struct synaptics_ts_data *ts)
{
	...
	/* max active duration */
	if (ts->pw_data.tap_count < 3)	//knock on
		buf[3] = 10;
	else				//knock code
		if (ts->pw_data.tap_count < 7)	// tap count: 3~6
			buf[3] = 16;
		else				// tap count: 7~8
			buf[3] = 10 + ts->pw_data.tap_count;
	...
	TOUCH_DEBUG(DEBUG_TRACE, "Set doze parameters\n");
...
//static int __devinit touch_init(void) //sora.jin
