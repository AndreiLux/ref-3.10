...
static void get_battery_id_voltage(struct hisi_hi6421v300_coul_device *di)
{
    int retry_times = 3;
    int cnt = 0;
    int Id_volt;
    ...
    if(udp_charge)
    {
        di->batt_id_vol = DEFAULT_BAT_ID_VOL;
        return;
    }
    while(cnt++ < retry_times)
    {
...
/**
 * hisi_hi6421v300_coul_config_power_off_reg - close coul when probe failed.
 */
static void hisi_hi6421v300_disable_coul(struct hisi_hi6421v300_coul_device *di)
{
    /* Mode */
    HISI_HI6421V300_REG_WRITE(HISI_HI6421V300_COUL_CTRL_REG, (~COUL_CTRL_ENABLE) );
}
...
coul_failed_0:
    hisi_hi6421v300_disable_coul(di);
    kfree(di);
coul_failed:
