...
static void mt_irq_set_polarity(unsigned int irq, unsigned int polarity)
{
    ...
    if (polarity == 0) {
        /* active low */
        value = readl_relaxed(IOMEM(int_pol_ctl_addr + (reg_index * 4)));
        value |= (1 << offset);
        writel_relaxed(value, (int_pol_ctl_addr + (reg_index * 4)));
    } else {
        /* active high */
        value = readl_relaxed(IOMEM(int_pol_ctl_addr + (reg_index * 4)));
        value &= ~(0x1 << offset);
