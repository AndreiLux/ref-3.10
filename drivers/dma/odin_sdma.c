...
static struct dma_pl330_platdata odin_sdma_pdata = {
	.nr_valid_peri = ARRAY_SIZE(sdma_peri),
	.peri_id = sdma_peri,
};

static int __init odin_sdma_init(void)
{
	dma_cap_set(DMA_MEMCPY, odin_sdma_pdata.cap_mask);

	return 0;
}
arch_initcall(odin_sdma_init);
...
static inline void _init_desc(struct dma_pl330_desc *desc)
{
	...
	desc->rqcfg.scctl = SCCTRL7;
	desc->rqcfg.dcctl = DCCTRL7;
...
static const struct of_device_id odin_sdma_ids[] __initconst = {
	{ .compatible = "arm,dma-pl330", .data = &odin_sdma_pdata, },
	{ /* Sentinel */ }
};
...
static int pl330_probe(struct amba_device *adev, const struct amba_id *id)
{
	...
	of_id = of_match_device(odin_sdma_ids, &adev->dev);
