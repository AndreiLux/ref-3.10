...
static int tdmb_parse_dt(struct device *dev);
static int broadcast_driver_init(void);
...
static struct odin_dmb_platform_data dmb_pdata;
...
#endif
        gpio_set_value(dmb_pdata.en_gpio, 0);
        mdelay(150);


        gpio_set_value(dmb_pdata.en_gpio, 1);
        mdelay(5);
#ifdef FEATURE_DMB_USE_XO
        if(fc8080_ctrl_info.clk != NULL) {
...
static struct spi_driver broadcast_tdmb_driver = {
    ...
    .driver = {
        .name = "tdmb",
        .of_match_table = of_match_ptr(tdmb_spi_table),
        .bus    = &spi_bus_type,
        .owner = THIS_MODULE,
    },
};
...
static int tdmb_configure_gpios(void)
{
    ...
    rc = gpio_request(dmb_pdata.en_gpio, "DMB_EN" );
    if (rc < 0) {
        err_count++;
        printk("%s:Failed GPIO DMB_EN request!!!\n",__func__);
    }

    rc = gpio_request(dmb_pdata.irq_gpio, "DMB_INT_N");
    if (rc < 0) {
        err_count++;
        printk("%s:Failed GPIO DMB_INT_N request!!!\n",__func__);
    }

    gpio_direction_output(dmb_pdata.en_gpio, 0);
    gpio_direction_input(dmb_pdata.irq_gpio);

    //request GPIO DMB_INT_N to use interrupt
    rc = gpio_to_irq(dmb_pdata.irq_gpio);
    if (rc < 0) {
        err_count++;
        printk("%s:Failed get GPIO DMB_INT_N number %d\n",__func__, dmb_pdata.irq_gpio);
    }
...
static int broadcast_tdmb_fc8080_probe(struct spi_device *spi)
{
    ...
    fc8080_ctrl_info.spi_ptr->max_speed_hz     = (12000*1000);
    ...
    if(fc8080_ctrl_info.pdev->dev.of_node)
    {
        rc = tdmb_parse_dt(&fc8080_ctrl_info.pdev->dev);

        if(rc)
        {
            printk("%s, Can't parsing DT\n");
            return rc;
        }

        printk("%s,irq-gpio = %d, en-gpio = %d \n", __func__, dmb_pdata.irq_gpio, dmb_pdata.en_gpio);
    }

    spi->irq = tdmb_configure_gpios();
    ...
    printk("broadcast_tdmb_fc8080_probe request_irq=%d, spi->irq num=%d\n", rc, spi->irq);
...
static int tdmb_parse_dt(struct device *dev)
{
    ...
    rc = of_property_read_u32(node, "en-gpio", &dmb_pdata.en_gpio);
    if(rc){
        printk("Unable to read en-gpio\n");
        return rc;
    }

    rc = of_property_read_u32(node, "irq-gpio", &dmb_pdata.irq_gpio);
    if(rc){
...
int __init broadcast_tdmb_fc8080_drv_init(void)
{
    async_schedule(broadcast_driver_init, NULL);
