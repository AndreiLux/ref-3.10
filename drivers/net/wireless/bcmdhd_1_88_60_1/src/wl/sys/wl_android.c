...
/*                                                          */
#ifdef CUSTOMER_LGE_ODIN
static struct regulator *wifi_io = NULL;
#endif
...
/*                                                          */
#if defined(CUSTOMER_HW2) || defined(CUSTOMER_HW4) || defined(CUSTOMER_LGE_ODIN)
...
/*                                                          */
#if defined (CONFIG_OF) && defined (CUSTOMER_LGE_ODIN)
...
int odin_wifi_set_carddetect(int status)
{
    if ((woden_wifi_status != WODEN_WIFI_ON) || !status) {
        printk(KERN_WARNING "[%s]: Cat't detect card\n", __func__);
        return -1;
    }
...
{
	if ((PTR_ERR(wifi_io) != -ENODEV) && on) {
		if (regulator_enable(wifi_io))
			DHD_ERROR(("%s:cannot enable regulator, maybe it does not have it\n", __FUNCTION__));
		else
	...
	if ((PTR_ERR(wifi_io) != -ENODEV) && !on) {
		if (regulator_disable(wifi_io))
			DHD_ERROR(("%s:cannot disable regulator, maybe it does not have it\n", __FUNCTION__));
	}

    return 0;
}

extern void *odin_brcm_wlan_get_mem(int section, unsigned long size);
...
static struct of_device_id woden_wifi_match[] = {
	{ .compatible = "odin,broadcom,bcm4335-wlan", .data = &odin_wifi_control},
	{ .compatible = "odin,broadcom,bcm4339-wlan", .data = &odin_wifi_control},
	{},
};
#endif
...
#if defined (CONFIG_OF) && defined (CUSTOMER_LGE_ODIN)
	int val = 0;
	struct wifi_platform_data *wifi_ctrl = NULL;
	const struct of_device_id *of_id;
	of_id = of_match_device(woden_wifi_match, &pdev->dev);
	pdev->dev.platform_data = of_id->data;
	wifi_ctrl = (struct wifi_platform_data *)(pdev->dev.platform_data);

	DHD_ERROR(("%s: Enter\n", __FUNCTION__));

	if (!of_property_read_u32(pdev->dev.of_node, "power_gpio", &val))
		wifi_power_gpio = val;

	if (!of_property_read_u32(pdev->dev.of_node, "wakeup_gpio", &val))
		wifi_wakeup_gpio = val;

	/* Taking the regulator for Wi-Fi @ Odin platform */
	wifi_io = regulator_get(&pdev->dev,
				"+1.8V_VDDIO_EXT");
	if (IS_ERR(wifi_io)) {
		err = PTR_ERR(wifi_io);
		DHD_ERROR(("%s:unable to get regulator, maybe it does not have it: %d, %p\n", __FUNCTION__, err, wifi_io));
		return PTR_ERR(wifi_io);
	}
#else
...
/*                                                          */
#if defined (CUSTOMER_LGE_ODIN)
	if (PTR_ERR(wifi_io) != -ENODEV)
		regulator_put(wifi_io);
#else
	if (wifi_regulator) {
...
#if defined (CONFIG_OF) && defined (CUSTOMER_LGE_ODIN)
		.owner			= THIS_MODULE,
		.of_match_table = of_match_ptr(woden_wifi_match),
#else
