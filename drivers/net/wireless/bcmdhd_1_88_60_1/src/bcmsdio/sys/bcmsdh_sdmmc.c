...
#if CUSTOMER_LGE_ODIN	/*                                                                           */
	/*                                                         */
	if ((!gInstance->func[1]) || (!gInstance->func[1]->card)) {
		sd_err(("%s: Won't be able to claim host, gInstance->func[1] or ->card FAILED\n", __FUNCTION__));
		sd_err(("%s: Fails\n", __FUNCTION__));
		return FALSE;
	}
#endif
...
#if CUSTOMER_LGE_ODIN	/*                                                                           */
	/*                                                         */
	if ((!gInstance->func[2]) || (!gInstance->func[2]->card)) {
		sd_err(("%s: Won't be able to claim host, gInstance->func[2] or ->card FAILED\n", __FUNCTION__));
		sd_err(("%s: Fails\n", __FUNCTION__));
...
#if CUSTOMER_LGE_ODIN	/*                                                                           */
#if 1 /*                                                         */
		if (gInstance == NULL || gInstance->func[func] == NULL || gInstance->func[func]->card == NULL) {
#else
                     ...
                     uint addr, void *pkt)
{
	...
	if ((!gInstance->func[func]) || (!gInstance->func[func]->card)) {
		sd_err(("%s: Won't be able to claim host, gInstance->func[func] or ->card FAILED\n", __FUNCTION__));
		sd_err(("%s: Fails\n", __FUNCTION__));
		return SDIOH_API_RC_FAIL;
	}
#endif
...
#if CUSTOMER_LGE_ODIN	/*                                                                           */
	/*                                                         */
	if ((!gInstance->func[0]) || (!gInstance->func[0]->card)) {
		sd_err(("%s: Won't be able to claim host, gInstance->func[0] or ->card FAILED\n", __FUNCTION__));
		sd_err(("%s: Fails\n", __FUNCTION__));
		return; 
	}
#endif
