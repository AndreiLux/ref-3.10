...
/*                                                                                   */
#if !defined(CUSTOMER_LGE_ODIN) && defined(CONFIG_HAS_WAKELOCK)
...
/*                                                                                      */
#if defined(CUSTOMER_LGE_ODIN) && defined(CONFIG_ODIN_OPTIMIZE_BOOTING_TIME)
#include <linux/kthread.h>
#endif

/*                                                                                     */
#if defined(CUSTOMER_LGE_ODIN) && defined(CONFIG_SIC_WLAN_CHIP_CRASH_RECOVERY)
...
/*                                                                                   */
#if !defined(CUSTOMER_LGE_ODIN) && defined(CONFIG_HAS_WAKELOCK) && (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27))
...
/* Print the CPUID to check Threaded init */
#if defined(CUSTOMER_LGE_ODIN) && defined(LGE_ODIN_CPUID_PRINT)
	unsigned long irq_flags = 0;
	uint8_t cpu = 0;
	local_irq_save(irq_flags);
	cpu = smp_processor_id();
	local_irq_restore(irq_flags);
	printk(KERN_ERR "%s: Thread start(cpu : %d)\n", __func__, cpu);
#endif
	...
	while (down_interruptible(&tsk->sema) == 0) {
		...
		cpu = smp_processor_id();
		local_irq_restore(irq_flags);
		printk(KERN_ERR "%s: Thread running(cpu : %d)\n", __func__, cpu);
#endif
#ifdef MCAST_LIST_ACCUMULATION
	...
	cpu = smp_processor_id();
	local_irq_restore(irq_flags);
	printk(KERN_ERR "%s: Thread exit(cpu : %d)\n", __func__, cpu);
#endif
...
/*                                                                                      */
#if defined(CUSTOMER_LGE_ODIN) && defined(CONFIG_ODIN_OPTIMIZE_BOOTING_TIME)
static int dhd_module_init_kthread_fn(void)
#else
static int __init
...
dhd_module_init(void)
{
	...
	int cpu = 6;	/* CA15 big core */

	task = kthread_create_on_node(dhd_module_init_kthread_fn, NULL,
...
/*                                                                                     */
#if defined(CUSTOMER_LGE_ODIN) && defined(CONFIG_SIC_WLAN_CHIP_CRASH_RECOVERY)
		wl_cfg80211_hang(dev, LGE_ODIN_WLAN_REASON_CHIP_HANG);
#else
...
/*                                                                                     */
#if defined(CUSTOMER_LGE_ODIN) && defined(CONFIG_SIC_WLAN_CHIP_CRASH_RECOVERY)
			ret = wl_cfg80211_hang(dev, LGE_ODIN_WLAN_REASON_CHIP_HANG);
#else
