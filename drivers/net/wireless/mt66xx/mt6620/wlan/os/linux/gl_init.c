...
static void wlanSetMulticastListWorkQueue(struct work_struct *work)
{
	...
	if (prDev->flags & IFF_BROADCAST) {
		...
		pr_info("u4PacketFilter |= PARAM_PACKET_FILTER_BROADCAST\n");
	}
	...
	if (prDev->flags & IFF_MULTICAST) {
...
#endif
		{
			...
			pr_info("u4PacketFilter |= PARAM_PACKET_FILTER_ALL_MULTICAST\n");
		}
		else {
			u4PacketFilter |= PARAM_PACKET_FILTER_MULTICAST;
			pr_info("u4PacketFilter |= PARAM_PACKET_FILTER_MULTICAST\n");
		}
	}
	...
	do {
		...
		for (i = 0; i < DBG_MODULE_NUM; i++) {
			aucDebugModule[i] = DBG_CLASS_ERROR |
			    DBG_CLASS_WARN |
			    DBG_CLASS_STATE | DBG_CLASS_TRACE | DBG_CLASS_EVENT | DBG_CLASS_INFO;
