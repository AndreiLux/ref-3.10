	      ...
	      IN UINT_8 ucResult, IN PUINT_8 pucSSID, IN UINT_8 ucSSIDLen, IN UINT_8 ucRole)
{
	...
	/* if (pucSSID) */
	/* pr_info("P2P GO SSID DIRECT-%c%c\n", pucSSID[7], pucSSID[8]); */
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 8, 0)
		cfg80211_chandef_create(&chandef, prIEEE80211ChnlStruct, eChnlType);
...
kalP2PIndicateChannelExpired(IN P_GLUE_INFO_T prGlueInfo, IN P_P2P_CHNL_REQ_INFO_T prChnlReqInfo)
{
	...
	do {
		...
		cfg80211_remain_on_channel_expired(prGlueP2pInfo->prDevHandler,	/* struct net_device * dev, */
						   prChnlReqInfo->u8Cookie, &chandef, GFP_KERNEL);
#else
		cfg80211_remain_on_channel_expired(prGlueP2pInfo->prDevHandler,	/* struct net_device * dev, */
