...
static int mtk_wifi_reset(struct sk_buff *skb, struct genl_info *info);
	...
	case WMTMSG_TYPE_RESET:
		if (u4MsgLength == sizeof(ENUM_WMTRSTMSG_TYPE_T)) {
			...
			case WMTRSTMSG_RESET_START:
				fgIsResetting = TRUE;
				glResetSendMessage(MTK_WIFI_RESET_START_NAME,
						   MTK_WIFI_COMMAND_RESET);
				break;
 ...
 */
/*----------------------------------------------------------------------------*/
int mtk_wifi_reset(struct sk_buff *skb, struct genl_info *info)
{
