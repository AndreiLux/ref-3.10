...
int firmware_download(char *filename, MTK_WCN_HIF_SDIO_CLTCTX cltCtx)
{
	...
	if (i == RETRY_ITERATION) {
		pr_warn("firmware own NOT acquired\n");
		goto fail_firmware;
	}
	...
	if (data == NULL) {
		pr_info("[MT662x] memory allocation error! (%s:%d)\n", __FILE__, __LINE__);
	}
	...
	if (prFwHead->u4Signature == MTK_WIFI_SIGNATURE &&
		...
		ValidHead = 1;
		pr_info("[MT6620] == DIVIDED DOWNLOAD ==\n");
	} else {
		ValidHead = 0;
		pr_info("[loopback][MT6620] == SINGLE DOWNLOAD ==\n");
	}
	...
	if (ucFreeBufferCount == 0) {
		...
		if (ucFreeBufferCount > ucMaxNumOfBuffer) {
			pr_warn("[START] TX resource = %d > %d..\n", ucFreeBufferCount,
				ucMaxNumOfBuffer);
			goto exit;
		} else if (ucFreeBufferCount == 0) {
			pr_warn("[START] no TX resource ..\n");
			goto exit;
		}
	}
	/* send it out */
	...
	for (i = 0; i < RETRY_ITERATION; i++) {
		...
		if ((err == 0) && ((ret & (0x1 << 21)) == (0x1 << 21))) {
			pr_info("[FWDL] Ready bit asserted (0x%08X)\n", ret);
			retValue = 0;
			break;
		}
		mdelay(10);
...
int firmware_power_off(MTK_WCN_HIF_SDIO_CLTCTX cltCtx)
{
	...
	for (i = 0; i < RETRY_ITERATION; i++) {
		...
		if ((err == 0) && ((ret & (0x1 << 21)) == 0)) {
			pr_info("[FWDL] Ready bit de-asserted (0x%08X)\n", ret);
			break;
		}
		mdelay(10);
	}
