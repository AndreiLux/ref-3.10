...
{
	pr_info("[loopback]: mt662x_remove++\n");
	down(&mt6620_init_sem);
	...
	up(&mt6620_init_sem);
	pr_info("[loopback]: mt662x_remvoe--\n");
	return 0;
}
...
static int __init mt6620_init(void)
{
	...
	if (ret != 0) {
		pr_info("==> alloc_chrdev_region() failure!\n");
		return ret;
	}
	...
	if (ret == HIF_SDIO_ERR_SUCCESS)
		...
		pr_info("%s(): mtk_wcn_hif_sdio_client_reg returned as %d\n", __func__, ret);
