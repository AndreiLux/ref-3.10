...
static BOOLEAN rsnCheckWpaRsnInfo(P_BSS_INFO_T prBss, P_RSN_INFO_T prWpaRsnInfo)
{
	...
	if (i == prWpaRsnInfo->u4AuthKeyMgtSuiteCount) {
		DBGLOG(RSN, WARN, ("SAKMS in bss info:0x%lx does not match that in AKMS in RsnInfo\n",prBss->u4RsnSelectedAKMSuite ));
		for (i = 0; i < prWpaRsnInfo->u4AuthKeyMgtSuiteCount; i++)
                     DBGLOG(RSN, WARN, ("i:%lx - <0x%lx>\n", i, prWpaRsnInfo->au4AuthKeyMgtSuite[i]));
		return TRUE;
	}
	...
	if (i == prWpaRsnInfo->u4PairwiseKeyCipherSuiteCount) {
		DBGLOG(RSN, WARN, ("SPC in bss:0x%lx does not match that in AKMS in RsnInfo\n",prBss->u4RsnSelectedAKMSuite));
		for (i = 0; i < prWpaRsnInfo->u4PairwiseKeyCipherSuiteCount; i++)
		    DBGLOG(RSN, WARN, ("i:%lx - <0x%lx>\n", i, prWpaRsnInfo->au4PairwiseKeyCipherSuite[i]));
		return TRUE;
	}
...
BOOLEAN rsnCheckSecurityModeChanged(P_ADAPTER_T prAdapter, P_BSS_INFO_T prBssInfo, P_BSS_DESC_T prBssDesc)
{
	...
	if ((prBssDesc->u2CapInfo & CAP_INFO_PRIVACY) != (prBssInfo->u2CapInfo & CAP_INFO_PRIVACY)) {
		DBGLOG(RSN, WARN, ("capInfo in BssDesc:0x%04x, cap in BssInfo:0x%04x\n", prBssDesc->u2CapInfo, prBssInfo->u2CapInfo));
		return TRUE;
	}
