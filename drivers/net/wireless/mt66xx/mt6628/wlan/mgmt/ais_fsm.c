...
static void aisFsmRoamout(IN P_ADAPTER_T prAdapter, P_BSS_DESC_T prBssDesc,
					ENUM_AIS_STATE_T * peNextState, BOOLEAN * pfgIsTransition);
					...
					ENUM_AIS_STATE_T *peNextState, BOOLEAN *pfgIsTransition)
{
	...
	P_BSS_INFO_T prAisBssInfo;
	P_CONNECTION_SETTINGS_T prConnSettings;
	ENUM_AIS_STATE_T eNextState = AIS_STATE_IDLE;
	...
	*peNextState = eNextState;
	*pfgIsTransition = fgIsTransition;
...
VOID aisFsmSteps(IN P_ADAPTER_T prAdapter, ENUM_AIS_STATE_T eNextState)
{
	...
	do {
		...
		case AIS_STATE_SEARCH:
			...
			/* 4 <3> We are under Roaming Condition. */
				...
				/* 4 <3.b> Try to roam out for JOIN this BSS_DESC_T. */
					...
					/* remove DEEP_INDENTATION and LONG_LINE to match kernel coding style*/
					aisFsmRoamout(prAdapter, prBssDesc,	&eNextState, &fgIsTransition);
				}
			}
		...
		case AIS_STATE_NORMAL_TR:
			if (prAisFsmInfo->fgIsInfraChannelFinished == FALSE) {
				...
				/* 3. Process for pending roaming scan */
					...
					AIS_REQUEST_REMAIN_ON_CHANNEL, TRUE) == TRUE) {
					eNextState = AIS_STATE_REQ_REMAIN_ON_CHANNEL;
					fgIsTransition = TRUE;
				}
#endif
		...
		case AIS_STATE_REQ_REMAIN_ON_CHANNEL:
			/* send message to CNM for acquiring channel */
			prMsgChReq = (P_MSG_CH_REQ_T)cnmMemAlloc(prAdapter,
				RAM_TYPE_MSG, sizeof(MSG_CH_REQ_T));
			if (!prMsgChReq) {
				ASSERT(0); /* Can't indicate CNM for channel acquiring*/
				return;
			}
		...
		if (prJoinCompMsg->ucSeqNum == prAisFsmInfo->ucSeqNumOfReqMsg) {
			...
			if (prJoinCompMsg->rJoinStatus == WLAN_STATUS_SUCCESS) {
				...
				if (prAisBssInfo->eConnectionState == PARAM_MEDIA_STATE_CONNECTED) {
					...
					/* add for ctia mode */
					if (EQUAL_SSID(aucP2pSsid, CTIA_MAGIC_SSID_LEN,
...
*/
/*----------------------------------------------------------------------------*/
VOID aisFsmRunEventScanDoneTimeOut(IN P_ADAPTER_T prAdapter, UINT_32 u4Param)
{
	P_AIS_FSM_INFO_T prAisFsmInfo;
	ENUM_AIS_STATE_T eNextState;
	P_CONNECTION_SETTINGS_T prConnSettings;
...
{
	struct MSG_REMAIN_ON_CHANNEL_T *prRemainOnChannel;
	P_AIS_FSM_INFO_T prAisFsmInfo;
	P_CONNECTION_SETTINGS_T prConnSettings;
	...
	prRemainOnChannel = (struct MSG_REMAIN_ON_CHANNEL_T *)prMsgHdr;
	...
	)
{
	P_AIS_FSM_INFO_T prAisFsmInfo;
	P_BSS_INFO_T prAisBssInfo;
	struct MSG_CANCEL_REMAIN_ON_CHANNEL_T *prCancelRemainOnChannel;

	ASSERT(prAdapter);
	prAisFsmInfo = &(prAdapter->rWifiVar.rAisFsmInfo);
	prAisBssInfo = &(prAdapter->rWifiVar.arBssInfo[NETWORK_TYPE_AIS_INDEX]);

	prCancelRemainOnChannel = (struct MSG_CANCEL_REMAIN_ON_CHANNEL_T *)prMsgHdr;
...
{
	P_AIS_FSM_INFO_T prAisFsmInfo;
	struct MSG_MGMT_TX_REQUEST_T *prMgmtTxMsg = (struct MSG_MGMT_TX_REQUEST_T *)NULL;
	...
	do {
		...
		prMgmtTxMsg = (struct MSG_MGMT_TX_REQUEST_T *)prMsgHdr;
...
{
	P_AIS_FSM_INFO_T prAisFsmInfo;
	struct AIS_MGMT_TX_REQ_INFO_T *prMgmtTxReqInfo = (struct AIS_MGMT_TX_REQ_INFO_T *)NULL;
	...
	do {
		...
		if (rTxDoneStatus != TX_RESULT_SUCCESS) {
			pr_info("Mgmt Frame TX Fail, Status:%d.\n", rTxDoneStatus);
		} else {
			fgIsSuccess = TRUE;
...
aisFuncTxMgmtFrame(
	IN P_ADAPTER_T prAdapter,
	IN struct AIS_MGMT_TX_REQ_INFO_T *prMgmtTxReqInfo,
	...
	)
{
	...
	do {
		...
		/*if (prStaRec != NULL)
			pr_info("Mgmt with station record: "MACSTR" .\n", MAC2STR(prStaRec->aucMacAddr));*/
