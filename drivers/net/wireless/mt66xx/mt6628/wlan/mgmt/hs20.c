	...
	)
{
	...
	prHS20IndicationIe = (struct IE_HS20_INDICATION_T *)pucIE;

	prHS20IndicationIe->ucId = ELEM_ID_VENDOR;
	prHS20IndicationIe->ucLength = sizeof(struct IE_HS20_INDICATION_T) - ELEM_HDR_LEN;
	...
	)
{
	...
	u4IeLength = sizeof(struct IE_HS20_INDICATION_T) + /* sizeof(IE_INTERWORKING_T) */
...
/*	UINT_8  aucMACAllZero[MAC_ADDR_LEN] = {0,0,0,0,0,0};*/
	...
	kalPrint("SENDER["MACSTR"] [%d:%d:%d:%d]\n", MAC2STR(pucSenderMac),
		*pucSenderIP, *(pucSenderIP+1), *(pucSenderIP+2), *(pucSenderIP+3));
	kalPrint("TARGET["MACSTR"] [%d:%d:%d:%d]\n", MAC2STR(pucTargetMac),
		*pucTargetIP, *(pucTargetIP+1), *(pucTargetIP+2), *(pucTargetIP+3));
#endif
	...
	)
{
	...
	if (*pucIpv6Protocol == IPV6_PROTOCOL_ICMPV6) {
		PUINT_8 pucICMPv6Type = ((PUINT_8)prCurrSwRfb->pvHeader
		...
		if (*pucICMPv6Type == ICMPV6_TYPE_NEIGHBOR_ADVERTISEMENT) {
			PUINT_8 pucICMPv6Flag = ((PUINT_8)prCurrSwRfb->pvHeader
				+ ETHER_HEADER_LEN + IPV6_HDR_LEN + ICMPV6_FLAG_OFFSET);
	...
	if ((prConnSettings->eEncStatus != ENUM_ENCRYPTION_DISABLED) && IS_BMCAST_MAC_ADDR(pucEthDestAddr)) {
		...
		if (*pu2PktIpVer == htons(ETH_P_IPV4)) {
			...
			for (ucIdx = 0; ucIdx < prBssInfo->prIpV4NetAddrList->ucAddrCount; ucIdx++) {
				...
				prPacketDA = (PUINT_32)((PUINT_8)prCurrSwRfb->pvHeader
					+ ETHER_HEADER_LEN + IPV4_HDR_IP_DST_ADDR_OFFSET);
			...
			PUINT_8 pucIdx = prCurrSwRfb->pvHeader + ETHER_HEADER_LEN
...
{
	struct PARAM_HS20_SET_BSSID_POOL *prParamBssidPool = (struct PARAM_HS20_SET_BSSID_POOL *)pvBuffer;
	struct HS20_INFO_T *prHS20Info;
	UINT_8 ucIdx;

	prHS20Info = &(prAdapter->rWifiVar.rHS20Info);

	pr_info("[%s]Set Bssid Pool! enable[%d] num[%d]\n", __func__,
		prParamBssidPool->fgIsEnable, prParamBssidPool->ucNumBssidPool);
