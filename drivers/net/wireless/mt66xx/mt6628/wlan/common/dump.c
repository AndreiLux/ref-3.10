...
VOID dumpMemory8(IN UINT_32 log_level, IN PUINT_8 pucStartAddr, IN UINT_32 u4Length)
{
	...
	if (log_level == ANDROID_LOG_ERROR) {
		pr_err(XLOG_TAG "DUMP8 ADDRESS: %08lx, Length: %ld\n",
			    (UINT_32) pucStartAddr, u4Length);
	} else if (log_level == ANDROID_LOG_WARN) {
		pr_warn(XLOG_TAG "DUMP8 ADDRESS: %08lx, Length: %ld\n",
			    (UINT_32) pucStartAddr, u4Length);
	} else if (log_level == ANDROID_LOG_INFO) {
		pr_info(XLOG_TAG "DUMP8 ADDRESS: %08lx, Length: %ld\n",
			    (UINT_32) pucStartAddr, u4Length);
	} else if (log_level == ANDROID_LOG_DEBUG) {
		pr_debug(XLOG_TAG "DUMP8 ADDRESS: %08lx, Length: %ld\n",
			    (UINT_32) pucStartAddr, u4Length);
