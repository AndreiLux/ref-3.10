				...
				IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = sizeof(struct PARAM_DRIVER_FW_RX_FILTER);
	...
	pr_info("RxFilter cmd=%d id=%u IsWhiteList=%d IsEqual=%d Offset=%lu Length=%lu\r\n",
				...
				IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = sizeof(struct PARAM_DRIVER_FW_TX_GENERATOR);
	...
	pr_info("TxHeartbeat cmd=%d id=%u Length=%u u4Period=%lu\r\n",
				...
				IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = sizeof(struct PARAM_DRIVER_FW_TCP_GENERATOR);
	...
	pr_info("TcpHeartbeat cmd=%d id=%u Period=%lu Timeout=%lu ipId=%lu Port=%lu %lu Seq=%lu",
			 ...
			 IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = 0;
	...
	prBssInfo->prIpV4NetAddrList =
		(struct IPV4_NETWORK_ADDRESS_LIST *)kalMemAlloc(u4IpV4AddrListSize, VIR_MEM_TYPE);
		...
		if (prSltInfo->prPseudoStaRec == NULL) {
			...
			if ((prTRSetting->u4FixedRate & FIXED_BW_DL40) != 0) {
				...
				case FIXED_EXT_CHNL_U20:
					...
					if (prTRSetting->rNetworkType == PARAM_NETWORK_TYPE_OFDM5) {
						prBssInfo->ucPrimaryChannel += 2;
					} else if (prBssInfo->ucPrimaryChannel < 5)
				...
				default:	/* 40M */
					...
					if (prTRSetting->rNetworkType == PARAM_NETWORK_TYPE_OFDM5) {
						prBssInfo->ucPrimaryChannel -= 2;
					} else if (prBssInfo->ucPrimaryChannel > 10)
	     ...
	     IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = sizeof(PTA_IPC_T);
	...
	pr_info(DRV_NAME "BCM BWCS CMD: BWCS CMD = %02x%02x%02x%02x\n",
			    ...
			    IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = sizeof(UINT_32);
	...
	if (prPtaInfo->fgSingleAntenna) {
		/* pr_warn(DRV_NAME"Q Single Ant = 1\r\n"); */
		*pu4SingleAntenna = 1;
	} else {
		/* pr_warn(DRV_NAME"Q Single Ant = 0\r\n"); */
		*pu4SingleAntenna = 0;
	}
			  ...
			  IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = sizeof(UINT_32);
	...
	if (u4SingleAntenna == 0) {
		/* pr_warn(DRV_NAME"Set Single Ant = 0\r\n"); */
		prPtaInfo->fgSingleAntenna = FALSE;
	} else {
		/* pr_warn(DRV_NAME"Set Single Ant = 1\r\n"); */
		...
		OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = sizeof(UINT_32);
	...
	if (prPtaInfo->fgEnabled) {
		/* pr_warn(DRV_NAME"PTA = 1\r\n"); */
		*pu4Pta = 1;
	} else {
		/* pr_warn(DRV_NAME"PTA = 0\r\n"); */
		*pu4Pta = 0;
	}
	      ...
	      IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = sizeof(UINT_32);
	...
	if (u4PtaCtrl == 0) {
		/* pr_warn(DRV_NAME"Set Pta= 0\r\n"); */
		nicPtaSetFunc(prAdapter, FALSE);
	} else {
		/* pr_warn(DRV_NAME"Set Pta= 1\r\n"); */
...
#if 0
	pr_info("c2GLegacyStaPwrOffset=%d\n", pTxPwr->c2GLegacyStaPwrOffset);
	pr_info("c2GHotspotPwrOffset=%d\n", pTxPwr->c2GHotspotPwrOffset);
	pr_info("c2GP2pPwrOffset=%d\n", pTxPwr->c2GP2pPwrOffset);
	pr_info("c2GBowPwrOffset=%d\n", pTxPwr->c2GBowPwrOffset);
	pr_info("c5GLegacyStaPwrOffset=%d\n", pTxPwr->c5GLegacyStaPwrOffset);
	pr_info("c5GHotspotPwrOffset=%d\n", pTxPwr->c5GHotspotPwrOffset);
	pr_info("c5GP2pPwrOffset=%d\n", pTxPwr->c5GP2pPwrOffset);
	pr_info("c5GBowPwrOffset=%d\n", pTxPwr->c5GBowPwrOffset);
	pr_info("ucConcurrencePolicy=%d\n", pTxPwr->ucConcurrencePolicy);

	for (i = 0; i < 14; i++)
		pr_info("acTxPwrLimit2G[%d]=%d\n", i, pTxPwr->acTxPwrLimit2G[i]);

	for (i = 0; i < 4; i++)
		pr_info("acTxPwrLimit5G[%d]=%d\n", i, pTxPwr->acTxPwrLimit5G[i]);
#endif
		  ...
		  IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	ASSERT(prAdapter);
	ASSERT(pu4SetInfoLen);
	pr_info("wlanoidSetP2pMode init status=0x%lx\n", status);
...
#endif
	pr_info("wlanoidSetP2pMode return status=0x%lx\n", status);
	return status;
...
{
	struct IE_HS20_INDICATION_T *prHS20IndicationIe;
	...
	*pu4SetInfoLen = u4SetBufferLen;
	...
	prHS20IndicationIe = (struct IE_HS20_INDICATION_T *)pvSetBuffer;
...
{
#if 0
	struct HS20_INFO_T *prHS20Info = NULL;
