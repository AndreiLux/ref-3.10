		     ...
		     IN OUT union iwreq_data *wrqu, IN OUT char *extra)
{
	...
	pr_info("set parameter, u4SubCmd=%d idx=%d value=%lu\n", (INT_16) u4SubCmd, (INT_16) index,
		value);
			...
			IN OUT union iwreq_data *wrqu, IN OUT char *extra)
{
	...
	case PRIV_CMD_OID:
		...
		if (!kalMemCmp(&(prGlueInfo->prP2PInfo->aucOidBuf[0]), extra, wrqu->data.length)) {
			pr_info(DRV_NAME "extra buffer is valid\n");
		} else {
			pr_info(DRV_NAME "extra 0x%p\n", extra);
		}
			...
			IN OUT union iwreq_data *wrqu, IN OUT char *extra)
{
	...
	if (!prDev || !wrqu) {
		pr_info(DRV_NAME "%s(): invalid param(0x%p, 0x%p)\n", __func__, prDev, wrqu);
		return -EINVAL;
	}
	...
	case PRIV_CMD_OID:
		if (copy_from_user(&(prGlueInfo->prP2PInfo->aucOidBuf[0]),
			...
			pr_notice("%s() copy_from_user oidBuf fail\n", __func__);
			return -EFAULT;
		}
