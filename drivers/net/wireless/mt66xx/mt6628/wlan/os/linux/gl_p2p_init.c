...
struct work_struct g_wlan_p2p_early_suspend_work;		/* early suspend work */
struct work_struct g_wlan_p2p_late_resume_work;		/* early suspend work */
KAL_WAKE_LOCK_T g_p2p_es_wakelock;
...
{
	flush_work(&g_wlan_p2p_late_resume_work);
	KAL_WAKE_LOCK(NULL, &g_p2p_es_wakelock);
	schedule_work(&g_wlan_p2p_early_suspend_work);
}

static void p2p_late_resume(struct early_suspend *h)
{
	flush_work(&g_wlan_p2p_early_suspend_work);
	KAL_WAKE_LOCK(NULL, &g_p2p_es_wakelock);
	schedule_work(&g_wlan_p2p_late_resume_work);
}

static void p2p_early_suspend_work(struct work_struct *work)
{
	pr_info("*********wlanP2P_early_suspend work************\n");
	wlanP2PEarlySuspend();
	KAL_WAKE_UNLOCK(NULL, &g_p2p_es_wakelock);
}
static void p2p_late_resume_work(struct work_struct *work)
{
	pr_info("*********wlanP2P_late_resume work************\n");
	wlanP2PLateResume();
	KAL_WAKE_UNLOCK(NULL, &g_p2p_es_wakelock);
}

static int p2p_early_suspend_work_init(void)
{
	/* init rx work */
	INIT_WORK(&g_wlan_p2p_early_suspend_work, p2p_early_suspend_work);
	INIT_WORK(&g_wlan_p2p_late_resume_work, p2p_late_resume_work);
	KAL_WAKE_LOCK_INIT(NULL, &g_p2p_es_wakelock, "wlan_p2p_early_suspend");
	pr_info("early_suspend work init done\n");
	return 0;
}

static int p2p_early_suspend_work_deinit(void)
{
	pr_info("early_suspend work deinit start\n");
	flush_work(&g_wlan_p2p_early_suspend_work);
	pr_info("early_suspend cancel work done\n");
	flush_work(&g_wlan_p2p_late_resume_work);
	pr_info("late_resume cancel work done\n");
	KAL_WAKE_LOCK_DESTROY(NULL, &g_p2p_es_wakelock);
	return 0;
