	      ...
	      IN struct iw_request_info *prIwReqInfo, IN struct iw_freq *prIwFreq, IN char *pcExtra)
{
	...
	/*
	   pr_info("set m:%d, e:%d, i:%d, flags:%d\n",
	       ...
	       IN struct iw_request_info *prIwrInfo, IN struct iw_point *prEssid, IN char *pcExtra)
{
	...
	if (prGlueInfo->rWpaInfo.u4WpaVersion == IW_AUTH_WPA_VERSION_DISABLED) {
		...
		/* pr_info("IW_AUTH_WPA_VERSION_DISABLED->Param_AuthMode%s\n", */
		...
		case IW_AUTH_KEY_MGMT_802_1X:
			...
			/* pr_info("IW_AUTH_KEY_MGMT_802_1X->AUTH_MODE_WPA%s\n", */
		...
		case IW_AUTH_KEY_MGMT_PSK:
			...
			/* pr_info("IW_AUTH_KEY_MGMT_PSK->AUTH_MODE_WPA%sPSK\n", */
...
/* //pr_info("IW_AUTH_KEY_MGMT_WPA_NONE->AUTH_MODE_WPA_NONE\n"); */
/* break; */
/* #endif */
#if CFG_SUPPORT_802_11W
...
#endif
		default:
			/* pr_info(DRV_NAME"strange IW_AUTH_KEY_MGMT : %ld set auto switch\n", */
	...
	if (cipher & IW_AUTH_CIPHER_CCMP) {
		/* pr_info("IW_AUTH_CIPHER_CCMP->ENUM_ENCRYPTION3_ENABLED\n"); */
		eEncStatus = ENUM_ENCRYPTION3_ENABLED;
	} else if (cipher & IW_AUTH_CIPHER_TKIP) {
		/* pr_info("IW_AUTH_CIPHER_TKIP->ENUM_ENCRYPTION2_ENABLED\n"); */
		eEncStatus = ENUM_ENCRYPTION2_ENABLED;
	} else if (cipher & (IW_AUTH_CIPHER_WEP104 | IW_AUTH_CIPHER_WEP40)) {
		/* pr_info("IW_AUTH_CIPHER_WEPx->ENUM_ENCRYPTION1_ENABLED\n"); */
		eEncStatus = ENUM_ENCRYPTION1_ENABLED;
	} else if (cipher & IW_AUTH_CIPHER_NONE) {
		/* pr_info("IW_AUTH_CIPHER_NONE->ENUM_ENCRYPTION_DISABLED\n"); */
		if (prGlueInfo->rWpaInfo.fgPrivacyInvoke)
			eEncStatus = ENUM_ENCRYPTION1_ENABLED;
		else
			eEncStatus = ENUM_ENCRYPTION_DISABLED;
	} else {
		/* pr_info("unknown IW_AUTH_CIPHER->Param_EncryptionDisabled\n"); */
		eEncStatus = ENUM_ENCRYPTION_DISABLED;
	}
	      ...
	      IN struct iw_request_info *prIwReqInfo, IN struct iw_param *prRate, IN char *pcExtra)
{
	...
	/*
	   pr_info("value = %d, fixed = %d, disable = %d, flags = %d\n",
	...
	if (prRate->value < 0) {
		if (prRate->fixed == 0) {
			...
			/* pr_info("wlanoidQuerySupportedRates():u4BufLen = %ld\n", u4BufLen); */
		...
		IN struct iw_request_info *prIwrInfo, IN struct iw_point *prEnc, IN char *pcExtra)
{
	...
	if (prEnc->length == 5 || prEnc->length == 13 || prEnc->length == 16) {
		...
		if (rStatus != WLAN_STATUS_SUCCESS) {
			/* pr_info(DRV_NAME"wlanoidSetAuthMode fail 0x%lx\n", rStatus); */
			return -EFAULT;
		}
		...
		if (rStatus != WLAN_STATUS_SUCCESS) {
			/* pr_info(DRV_NAME"wlanoidSetEncryptionStatus fail 0x%lx\n", rStatus); */
			return -EFAULT;
		}
	       ...
	       IN struct iw_request_info *prIwrInfo, OUT struct iw_param *prPower, IN char *pcExtra)
{
	...
	/* pr_info("wext_get_power value(%d) disabled(%d) flag(0x%x)\n", */
	/* prPower->value, prPower->disabled, prPower->flags); */
	      ...
	      IN struct iw_request_info *prIwrInfo, IN struct iw_param *prAuth, IN char *pcExtra)
{
...
#if CFG_SUPPORT_802_11W
	case IW_AUTH_MFP:
		/* pr_info("wext_set_auth IW_AUTH_MFP=%d\n", prAuth->value); */
	...
	default:
		/*
		   pr_info("[wifi] unsupported IW_AUTH_INDEX :%d\n", prAuth->flags);
		 */
		break;
	}
	return 0;
		    ...
		    IN struct iw_point *prEnc, IN char *pcExtra)
{
	...
	if (prIWEncExt->alg == IW_ENCODE_ALG_SMS4) {
		if (prEnc->flags & IW_ENCODE_DISABLED) {
			/* pr_info("[wapi] IW_ENCODE_DISABLED\n"); */
			return 0;
		}
		/* KeyID */
...
#endif
	{
		...
		if ((prEnc->flags & IW_ENCODE_MODE) == IW_ENCODE_DISABLED) {
			...
			/*
			   pr_info("IW_ENCODE_DISABLED: ID:%d, Addr:[" MACSTR "]\n",
...
int wext_support_ioctl(IN struct net_device *prDev, IN struct ifreq *prIfReq, IN int i4Cmd)
{
	...
	{
	/*remove DEEP_INDENTATION warning - 1*/
		P_GLUE_INFO_T prGlueInfo;
		if (!iwr->u.data.pointer)
			break;

		prGlueInfo = *((P_GLUE_INFO_T *) netdev_priv(prDev));
		/*remove DEEP_INDENTATION warning - 2*/
		...
		if (u4ExtraSize) {
			...
			if (copy_from_user(prExtraBuf,
				...
				if (rStatus != WLAN_STATUS_SUCCESS) {
					/* pr_info("[wapi] set wapi assoc info error:%lx\n", rStatus); */
#endif
#if CFG_SUPPORT_WPS2
					...
					if (wextSrchDesiredWPSIE(prExtraBuf,
						...
						rStatus =
						    kalIoctl(prGlueInfo,
							     ...
							     FALSE, &u4BufLen);
						/*if (rStatus != WLAN_STATUS_SUCCESS) {*/
							/* pr_info("[WSC] set WSC assoc info error:%lx\n", rStatus); */
						/*}*/
					}
#endif
#if CFG_SUPPORT_WAPI
	...
	case SIOCSIWPMKSA:	/* 0x8B36, pmksa cache operation */
		...
		if (iwr->u.data.pointer) {
			...
			if (copy_from_user(prExtraBuf,
				...
				case IW_PMKSA_ADD:
					/*
					   pr_info("IW_PMKSA_ADD [" MACSTR "]\n",
					   MAC2STR(((struct iw_pmksa *)pExtraBuf)->bssid.sa_data));
					 */
				{
				/*remove DEEP_INDENTATION warning*/
					P_GLUE_INFO_T prGlueInfo =
				...
				case IW_PMKSA_REMOVE:
					/*
					   pr_info("IW_PMKSA_REMOVE [" MACSTR "]\n",
					   MAC2STR(((struct iw_pmksa *)buf)->bssid.sa_data));
					 */
					break;
				case IW_PMKSA_FLUSH:
					/*
					   pr_info("IW_PMKSA_FLUSH\n");
					 */
				{
					P_GLUE_INFO_T prGlueInfo =
					    *((P_GLUE_INFO_T *) netdev_priv(prDev));
	...
	default:
		/* pr_notice("unsupported IOCTL: 0x%x\n", i4Cmd); */
		ret = -EOPNOTSUPP;
		break;
	}

	/* pr_info("%ld CMD:0x%x ret:%d\n", jiffies_to_msecs(jiffies), i4Cmd, ret); */
			 ...
			 IN unsigned char *pucData, IN unsigned int u4dataLen)
{
	...
	case IWEVASSOCREQIE:
		...
		/* pr_debug("assoc info buffer size needed:%d\n", infoElemLen * 2 + 17); */
...
#else
			/* pr_info("IWEVPMKIDCAND event skipped, WE < 18\n"); */
			goto skip_indicate_event;
#endif
		} else {
			/* pr_info("IWEVPMKIDCAND event skipped, NOT WPA2\n"); */
			goto skip_indicate_event;
		}
		break;
	...
	default:
		/* pr_info("Unsupported wext event:%x\n", cmd); */
		goto skip_indicate_event;
	}
