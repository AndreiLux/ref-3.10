	     ...
	     IN union iwreq_data *prIwReqData, IN char *pcExtra)
{
	...
	case PRIV_CMD_TEST_CMD:
		/* pr_info("CMD=0x%08lx, data=0x%08lx\n", pu4IntBuf[1], pu4IntBuf[2]); */
		prNdisReq = (P_NDIS_TRANSPORT_STRUCT) &aucOidBuf[0];
	...
	case PRIV_CMD_ACCESS_MCR:
		/* pr_info("addr=0x%08lx, data=0x%08lx\n", pu4IntBuf[1], pu4IntBuf[2]); */
		prNdisReq = (P_NDIS_TRANSPORT_STRUCT) &aucOidBuf[0];
	...
	case PRIV_CMD_TEST_CMD:
		...
		if (status == 0) {
			...
			/*
			   ...
			   pr_notice("priv_get_int() copy_to_user oidBuf fail(3)\n");
			   return -EFAULT;
			   }
			 */
		}
...
#if 0
			pr_info("Tx power num = %d\n", prIwReqData->data.length);

			pr_info("Tx power setting = %d %d %d %d\n",
...
#if 0
	pr_info(DRV_NAME "priv_set_struct(): prIwReqInfo->cmd(0x%X), u4SubCmd(%ld)\n",
...
#if 0
	pr_info(DRV_NAME "priv_get_struct(): prIwReqInfo->cmd(0x%X), u4SubCmd(%ld)\n",
		prIwReqInfo->cmd, u4SubCmd);
#endif
...
#if 0
	pr_info(DRV_NAME "priv_set_ndis(): prNdisReq->ndisOidCmd(0x%lX)\n", prNdisReq->ndisOidCmd);
#endif
...
#if 0
	pr_info(DRV_NAME "priv_set_ndis(): %s\n", prWlanReqEntry->pucOidName);
#endif
...
#if 0
	pr_info(DRV_NAME "priv_get_ndis(): prNdisReq->ndisOidCmd(0x%lX)\n", prNdisReq->ndisOidCmd);
#endif
...
#if 0
	pr_info(DRV_NAME "priv_get_ndis(): %s\n", prWlanReqEntry->pucOidName);
#endif
