	 ...
	 IN BOOL fgWaitResp, IN BOOL fgCmd, IN BOOL fgIsP2pOid, OUT PUINT_32 pu4QryInfoLen)
{
	...
	*if whole chip reset is ongoing, then ignore all kalIoctl
	*invocations, except the invocation of set p2p mode handler
	*/
	if (g_u4HaltFlag || (kalIsResetting() && (*pu4QryInfoLen != 0x12345678))) {
		up(&g_halt_sem);
		return WLAN_STATUS_ADAPTER_NOT_READY;
	}
