...
struct work_struct g_wlan_early_suspend_work;		/* early suspend work */
struct work_struct g_wlan_late_resume_work;		/* early suspend work */
KAL_WAKE_LOCK_T g_wlan_es_wakelock;
/*******************************************************************************
*                                 M A C R O S
********************************************************************************
*/
...
static VOID wlanNetUnregister(struct wireless_dev *prWdev)
{
	...
	KAL_WAKE_UNLOCK(NULL, &prGlueInfo->rWakeLock);
...
static void wlanEarlySuspend(void)
{
	...
	DBGLOG(SW1, STATE, ("*********wlanEarlySuspend************\n"));

	/* <1> Sanity check and acquire the net_device */
	ASSERT(u4WlanDevNum <= CFG_MAX_WLAN_DEVICES);
	if (u4WlanDevNum == 0) {
		DBGLOG(SW1, STATE, ("wlanEarlySuspend u4WlanDevNum==0 invalid!!\n"));
		return;
	}
	prDev = arWlanDevInfo[u4WlanDevNum - 1].prDev;
	ASSERT(prDev);
	...
	if (!prDev || !(prDev->ip_ptr) ||
		...
		DBGLOG(SW1, STATE, ("ip is not avaliable.\n"));
		return;
	}
	...
	DBGLOG(SW1, STATE, ("ip is %d.%d.%d.%d\n", ip[0], ip[1], ip[2], ip[3]));
	...
	if (!prDev || !(prDev->ip6_ptr) ||
		...
		DBGLOG(SW1, STATE, ("ipv6 is not avaliable.\n"));
		return;
	}
...
/* UINT_8 aucBuf[32] = {0}; */
		...
		if (rStatus != WLAN_STATUS_SUCCESS) {
			DBGLOG(SW1, ERROR, ("set ARP filter fail:0x%lx\n", rStatus));
		} else
			DBGLOG(SW1, INFO, ("set ARP filter succeed\n"));
	}
}
...
static void wlanLateResume(void)
{
	...
	ASSERT(u4WlanDevNum <= CFG_MAX_WLAN_DEVICES);
	if (u4WlanDevNum == 0) {
		DBGLOG(SW1, ERROR, ("wlanLateResume u4WlanDevNum==0 invalid!!\n"));
		return;
	}
	prDev = arWlanDevInfo[u4WlanDevNum - 1].prDev;
	ASSERT(prDev);
...
/* UINT_8 aucBuf[32] = {0}; */
		...
		if (rStatus != WLAN_STATUS_SUCCESS) {
			DBGLOG(SW1, ERROR, ("clear ARP filter fail 0x%lx\n", rStatus));
		} else
			DBGLOG(SW1, INFO, ("clear ARP filter succeed\n"));
	}
}
...
{
	flush_work(&g_wlan_late_resume_work);
	KAL_WAKE_LOCK(NULL, &g_wlan_es_wakelock);
	schedule_work(&g_wlan_early_suspend_work);
}

static void wlan_late_resume(struct early_suspend *h)
{
	flush_work(&g_wlan_early_suspend_work);
	KAL_WAKE_LOCK(NULL, &g_wlan_es_wakelock);
	schedule_work(&g_wlan_late_resume_work);
}
static void wlan_early_suspend_work(struct work_struct *work)
{
	DBGLOG(SW1, TRACE, ("*********wlan_early_suspend work************\n"));
	wlanEarlySuspend();
	KAL_WAKE_UNLOCK(NULL, &g_wlan_es_wakelock);
}
static void wlan_late_resume_work(struct work_struct *work)
{
	DBGLOG(SW1, TRACE, ("*********wlan_late_resume work************\n"));
	wlanLateResume();
	KAL_WAKE_UNLOCK(NULL, &g_wlan_es_wakelock);
}

static int wlan_early_suspend_work_init(void)
{
	/* init rx work */
	INIT_WORK(&g_wlan_early_suspend_work, wlan_early_suspend_work);
	INIT_WORK(&g_wlan_late_resume_work, wlan_late_resume_work);
	KAL_WAKE_LOCK_INIT(NULL, &g_wlan_es_wakelock, "wlan_early_suspend");
	DBGLOG(SW1, INFO, ("early_suspend work init for wlan donel\n"));
	return 0;
}

static int wlan_early_suspend_work_deinit(void)
{
	DBGLOG(SW1, INFO, ("early_suspend work deinit start\n"));
	flush_work(&g_wlan_early_suspend_work);
	DBGLOG(SW1, INFO, ("early_suspend cancel work done\n"));
	flush_work(&g_wlan_late_resume_work);
	DBGLOG(SW1, INFO, ("late_resume cancel work done\n"));
	KAL_WAKE_LOCK_DESTROY(NULL, &g_wlan_es_wakelock);
	return 0;
}
...
static int set_p2p_mode_handler(struct net_device *netdev, PARAM_CUSTOM_P2P_SET_STRUC_T p2pmode)
{
	...
	/*   this is a workaround for whole chip reset failed case.
	*	if this fuction is called when whole chip is resetting,
	*	assign u4BufLen to 0x12345678 to indicate this
	*	invocation to kalIoctl is made by whole chip reset.
	*/
	UINT_32 u4BufLen = 0x12345678;
...
static INT_32 wlanProbe(PVOID pvData)
{
	...
	do {
		...
		KAL_WAKE_LOCK_INIT(NULL, &prGlueInfo->rWakeLock, "wlan_driver_wakelock");
		DBGLOG(INIT, INFO, ("wlanProbe: wakelock inited.\n"));
...
static VOID wlanRemove(VOID)
{
	...
	KAL_WAKE_LOCK_DESTROY(NULL, &prGlueInfo->rWakeLock);
	...
	wlan_early_suspend_work_deinit();
#endif
...
static int __init initWlan(void)
{
	...
	aucDebugModule[DBG_RSN_IDX] |= DBG_CLASS_INFO;
	aucDebugModule[DBG_SW1_IDX] |= DBG_CLASS_INFO | DBG_CLASS_STATE;
	aucDebugModule[DBG_SW2_IDX] |= DBG_CLASS_INFO | DBG_CLASS_STATE;
	aucDebugModule[DBG_MEM_IDX] |= DBG_CLASS_LOUD;
#endif				/* DBG */
