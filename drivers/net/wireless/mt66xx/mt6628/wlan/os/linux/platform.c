...
*/
#if CONFIG_PM
#define DEV_NAME "wlan_mt6628"

static atomic_t fgSuspendFlag = ATOMIC_INIT(0);
static atomic_t fgIndicateWoW = ATOMIC_INIT(0);
static int wlan_mt6628_probe(struct platform_device *pdev);
static int wlan_mt6628_remove(struct platform_device *pdev);
static int wlan_mt6628_suspend(struct platform_device *pdev, pm_message_t state);
static int wlan_mt6628_resume(struct platform_device *pdev);
static void wlan_mt6628_release(struct device *dev);
static void wlan_mt6628_shutdown(struct platform_device *pdev);

static struct platform_device mtk_wlan_mt6628_dev = {
	.name			= DEV_NAME,
	.id			= -1,
	.dev = {
		.release = wlan_mt6628_release,
	}
};

static struct platform_driver mtk_wlan_mt6628_drv = {
	.probe = wlan_mt6628_probe,
	.remove = wlan_mt6628_remove,
#ifdef CONFIG_PM
	.shutdown = wlan_mt6628_shutdown,
	.suspend = wlan_mt6628_suspend,
	.resume = wlan_mt6628_resume,
#endif
	.driver = {
		.name = DEV_NAME,
		.owner = THIS_MODULE,
...
/* #ifdef  CONFIG_IPV6 */
	...
	if (prDev == NULL) {
		DBGLOG(SW2, INFO, ("netdev_event: device is empty.\n"));
		return NOTIFY_DONE;
	}

	if ((strncmp(prDev->name, "p2p", 3) != 0) && (strncmp(prDev->name, "wlan", 4) != 0)) {
		DBGLOG(SW2, INFO, ("netdev_event: xxx\n"));
		return NOTIFY_DONE;
	}
	...
	if (prGlueInfo == NULL) {
		DBGLOG(SW2, INFO, ("netdev_event: prGlueInfo is empty.\n"));
		return NOTIFY_DONE;
	}
	ASSERT(prGlueInfo);
	...
	if (fgIsUnderEarlierSuspend == false) {
		DBGLOG(SW2, INFO,
		       ("netdev_event: MEDIA_STATE change.status (%d)\n",
...
/* #ifdef  CONFIG_IPV6 */
		...
		if (rStatus != WLAN_STATUS_SUCCESS) {
			DBGLOG(SW2, ERROR, ("set set ARP filter fail 0x%lx\n", rStatus));
		} else
			DBGLOG(SW2, INFO, ("set set ARP filter succeed\n"));
	}
...
static void wlan_mt6628_shutdown(struct platform_device *pdev)
{
	...
	if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_off(WMTDRV_TYPE_WIFI)) {
		...
		DBGLOG(INIT, INFO, ("WMT turn off WIFI OK!\n"));
	}
}
...
int glRegisterPlatformDev(void)
{
	...
	retval = platform_device_register(&mtk_wlan_mt6628_dev);
	if (retval) {
		DBGLOG(INIT, ERROR,
	...
	retval = platform_driver_register(&mtk_wlan_mt6628_drv);
	if (retval) {
		DBGLOG(INIT, ERROR,
...
{
	platform_device_unregister(&mtk_wlan_mt6628_dev);
	platform_driver_unregister(&mtk_wlan_mt6628_drv);
	return 0;
}
