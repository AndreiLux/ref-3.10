		...
		if (IS_NET_ACTIVE(prAdapter, prCurrentMsduInfo->ucNetworkType)) {
			...
			default:
				...
				do {
					...
					/*remove DEEP_INDENTATION warning for switch in prStaRec->fgIsQoS*/
					if (!(prStaRec->fgIsQoS)) {
		...
		do {
			...
			if (u4Share) {
				for (i = 0; i < TC_NUM; i++) {
					...
					/*remove DEEP_INDENTATION warning for ai4PerTcResourceDemand[i]*/
		...
		if (!HIF_RX_HDR_GET_80211_FLAG(prHifRxHdr)) {
			...
			if (prAdapter->rRxCtrl.rFreeSwRfbList.u4NumElem
				...
				if (OP_MODE_ACCESS_POINT == prBssInfo->eCurrentOPMode) {
					...
					} else if (UNEQUAL_MAC_ADDR(prBssInfo->aucOwnMacAddr,
						 pucEthDestAddr)) {
						prCurrSwRfb->eDst =
