...
VOID nicpmSetFWOwn(IN P_ADAPTER_T prAdapter, IN BOOLEAN fgEnableGlobalInt)
{
	...
	if (prAdapter->fgIsFwOwn == TRUE) {
		KAL_WAKE_UNLOCK(prAdapter, &prAdapter->prGlueInfo->rWakeLock);
		return;
	} else {
		if (nicProcessIST(prAdapter) != WLAN_STATUS_NOT_INDICATING) {
			/* pending interrupts */
			DBGLOG(INIT, INFO, ("set firmware own fail, pending interrupts\n"));
			return;
		}
	}
	...
	if (fgEnableGlobalInt) {
		...
		if (u4RegValue & WHLPCR_FW_OWN_REQ_SET) {
			...
			DBGLOG(INIT, ERROR, ("set firmware own fail, will try again\n"));
			return;
		}
...
#define LP_OWN_BACK_CLR_OWN_ITERATION   256	/* exponential of 2 */
	...
	KAL_WAKE_LOCK(prAdapter, &prAdapter->prGlueInfo->rWakeLock);
