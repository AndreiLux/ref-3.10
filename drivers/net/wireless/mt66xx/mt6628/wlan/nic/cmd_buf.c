...
P_CMD_INFO_T cmdBufAllocateCmdInfo(IN P_ADAPTER_T prAdapter, IN UINT_32 u4Length)
{
	...
	if (prCmdInfo) {
		...
		if (prCmdInfo->pucInfoBuffer == NULL) {
			...
			DBGLOG(MEM, ERROR, ("cmd, cnmMemAlloc fail\n"));
			/*end*/
			prCmdInfo = NULL;
		} else {
	...
	/*mtk80707 debug ALPS01428223*/
		...
		P_QUE_T prPendingCmd = &(prAdapter->rPendingCmdQueue);
		P_QUE_T prCmdQue = &(prAdapter->prGlueInfo->rCmdQueue);
		P_QUE_ENTRY_T prQueueEntry = QUEUE_GET_HEAD(prPendingCmd);

		DBGLOG(MEM, LOUD, ("++dump pendCmd total=%lu\n",
			prPendingCmd->u4NumElem));

		while (prQueueEntry) {
			prCmdInfo = (P_CMD_INFO_T)prQueueEntry;
			DBGLOG(MEM, LOUD, ("cmd, ucid=%u, TIME=%lu seq=%u, sta=%d\n",
				prCmdInfo->ucCID,
				prCmdInfo->u4InqueTime,
				prCmdInfo->ucCmdSeqNum,
				prCmdInfo->ucStaRecIndex));
			prQueueEntry = QUEUE_GET_NEXT_ENTRY(prQueueEntry);
		}
		DBGLOG(MEM, LOUD, ("--end pendingCmd\n"));

		prQueueEntry = QUEUE_GET_HEAD(prCmdQue);
		DBGLOG(MEM, LOUD, ("++dump CmdQueue total=%lu\n",
			prCmdQue->u4NumElem));

		while (prQueueEntry) {
			prCmdInfo = (P_CMD_INFO_T)prQueueEntry;
			DBGLOG(MEM, LOUD, ("cmd, ucid=%u, TIME=%lu seq=%u, sta=%u\n",
				prCmdInfo->ucCID,
				prCmdInfo->u4InqueTime,
				prCmdInfo->ucCmdSeqNum,
				prCmdInfo->ucStaRecIndex));
