...

{
	...
	if (event == WLC_E_ACTION_FRAME_RX) {
		...
		if (wl_cfgp2p_is_pub_action(&mgmt_frame[DOT11_MGMT_HDR_LEN],
			...
			if (discover_cfgdev(cfgdev, wl)){
				cfgdev = cfgdev_to_prmry_cfgdev(cfgdev, wl);
				WL_ERR(("use primary device instead of p2p's\n"));
			}
#endif /* WL_ENABLE_P2P_IF */
...
wl_cfg80211_event(struct net_device *ndev, const wl_event_msg_t * e, void *data)
{
	...
	if (!wl->p2p) {
		WL_ERR(("wl->p2p is null for debug, ignore event %d\n", event_type));
	}
	if (wl_get_p2p_status(wl, IF_CHANGING) || wl_get_p2p_status(wl, IF_ADD) ) {
