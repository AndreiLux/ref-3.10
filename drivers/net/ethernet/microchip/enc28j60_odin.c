/*
 *   drivers/net/ethernet/nicrochip/enc28j60_odin.c
 */
...
#else
/* for Woden/Odin SPI, kimhj
  CS signal of Woden/Odin SPI controller toggles at every SPI data
  because SSI_SCPH0_SSTOGGLE = 1 in SoC level.
  The 1 xfer should be used for read operation
  when CS of SPI controller is used instead of GPIO
*/
static u8 spi_read_op(struct enc28j60_net *priv, u8 op,
			   u8 addr)
{
...
static void enc28j60_irq_work_handler(struct work_struct *work)
{
	...
	enable_irq(priv->spi->irq);	/* for level trigger interrupt */
}
...
static irqreturn_t enc28j60_irq(int irq, void *dev_id)
{
	...
	disable_irq_nosync(priv->spi->irq);	/* for level trigger interrupt */
...
#define SPI_ETHERNET_GPIO_IRQ (34+17)	/*Woden HDK SYS_EINT19(AD29) */

static int gpio_irq_setup(int gpio_num)
{
	int ret;

	ret = gpio_request(gpio_num, "SPI Ethernet Interrupt");
	if (ret < 0){
		pr_err("%s: Failed to get Ethernet IRQ gpio %d. Code: %d.",
	...
	ret = gpio_to_irq(gpio_num);
	if (ret < 0){
        pr_err("%s: Failed to get gpio irq number %d. Code: %d.",
												__func__, gpio_num, ret);
        goto fail_gpio;
    }
...
void enc28j60_spi_cscontrol(u32 command)
{
	if (command)
...
static int enc28j60_probe(struct spi_device *spi)
{
...
#if CONFIG_OF
	if (of_property_read_u32(spi->dev.of_node, "spi_cs_gpio",&spi_cs_gpio))
		goto error_irq;
#endif
	if (gpio_request(spi_cs_gpio, "SPI Ethernet SPI cs control") < 0){
		pr_err("%s: Failed to get Ethernet SPI gpio %d\n",
										__func__, spi_cs_gpio);
		goto error_irq;
	}
	enc28j60_spi_cscontrol(0);	/* starting cs value is HIGH */

	cd = spi->controller_state;
	cd->cs_control = enc28j60_spi_cscontrol;
	...
	/* SPI-Ethernet GPIO IRQ register, kimhj */
#if CONFIG_OF
	if (of_property_read_u32(spi->dev.of_node, "irq_gpio",&irq_gpio))
	goto error_irq;
#endif
	spi->irq = gpio_irq_setup(irq_gpio);
	if (spi->irq < 0)
		goto error_irq;

	/* There's a problem of not receiving interrupt
	 *	when edge trigger is used in Woden/Odin.
	 *  Level trigger should be used and there're some code modification for it.
	 */
	ret = request_irq(spi->irq, enc28j60_irq, IRQF_TRIGGER_LOW, DRV_NAME, priv);
	if (ret < 0) {
		if (netif_msg_probe(priv))
	...
	dev_info(&dev->dev, DRV_NAME " driver registered,"
		"cs_gpio:%d, irq_gpio:%d\n",spi_cs_gpio, irq_gpio);
...
#ifdef CONFIG_OF
static const struct of_device_id odin_enc28j60_dt_match[] __initdata = {
...
static struct spi_driver enc28j60_driver = {
	.driver = {
		   .name = DRV_NAME,
		   .owner = THIS_MODULE,
		   .of_match_table = of_match_ptr(odin_enc28j60_dt_match),
	 },
