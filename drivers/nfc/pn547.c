...
#if defined(CONFIG_SOC_EXYNOS5430)
unsigned int pn547_prepare_clk(const char *conid, bool enable)
{
	struct clk *target;
...
static int pn547_clk_prepare(struct pn547_dev *pn547_dev, bool enable)
{
	...
	if (pn547_dev->clk_use_check == CLK_USE_CAM1) {
		if (enable) {
			int ret;
			ret = pn547_prepare_clk("sclk_isp_sensor1", true);
			if (ret)
				pr_err("%s, prepare sclk_isp_sensor1 Error !!:%d\n", __func__, ret);
		} else {
			pn547_prepare_clk("sclk_isp_sensor1", false);
		}
	}
#endif
