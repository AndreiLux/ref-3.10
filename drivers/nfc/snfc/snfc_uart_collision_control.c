...
static int snfc_uart_control_open(struct inode *inode, struct file *fp)
{
    ...
    if(gpio_init ==0)
    {
        rc = gpio_request_one(snfc_gpios.gpio_hvdd,GPIOF_OUT_INIT_LOW, "snfc_hvdd");
        if(rc){
...
static long snfc_uart_control_read(struct file *pf, char *pbuf, size_t size, loff_t *pos)
{
    int current_status;
    int rc;
...
* Output :
*/
static long snfc_uart_control_write(struct file *pf, const char *pbuf, size_t size, loff_t *pos)
{
    int new_status;
    int rc;
...
static struct file_operations snfc_uart_control_fops =
{
    ...
    .compat_ioctl = snfc_uart_control_ioctl,
...
int snfc_uart_control_probe(struct device_node *np)
{
    ...
    rc = gpio_request_one(snfc_gpios.gpio_hsel, GPIOF_OUT_INIT_LOW, "snfc_hsel");
    if (rc)
    {
        SNFC_DEBUG_MSG("[snfc_driver] gpio_request snfc_hsel fail\n");
    }
    rc = gpio_request_one(snfc_gpios.gpio_pon, GPIOF_OUT_INIT_LOW, "snfc_pon");
    if (rc)
    {
    ...
    rc = gpio_request_one(snfc_gpios.gpio_uicc_con, GPIOF_OUT_INIT_LOW, "snfc_uicc_con");
    if(rc){
