...
static void pn547_enable_irq(struct pn547_dev *pn547_dev)
{
    ...
    if (!pn547_dev->irq_enabled) {
        ...
        enable_irq(pn547_get_irq_pin(pn547_dev));
...
static irqreturn_t pn547_dev_irq_handler(int irq, void *dev_id)
{
    ...
    if (sPowerState == NFC_POWER_ON) {
        ...
        if (sIsWakeLocked == false) {
            ...
            //dprintk("%s already wake locked!\n", __func__); // for debug
        }
        spin_unlock_irqrestore(&pn547_dev->irq_enabled_lock, flags);
        //dprintk("%s: wake_lock (%d)\n", __func__, gpio_get_value(pn547_dev->irq_gpio)); // for debug
    }
    else {
         printk("%s, NFC IRQ Triggered during NFC OFF\n", __func__);
    }
//    dprintk(PN547_DRV_NAME ":pn547_dev_irq_handler : %d, GPIO = %d\n", irq, irq_gpio_val);
...
wait:
    if (isFinalPacket == true) {
        ...
        if (ret == 0) {
            dprintk("%s: pass wait_event_interruptible by %dms timeout. restart waiting!\n", __func__, NFC_TIMEOUT_MS); // for debug
            spin_lock_irqsave(&pn547_dev->irq_enabled_lock, flags);
    ...
    if (ret == -ERESTARTSYS) {
        //dprintk("%s: pass wait_event_interruptible by signal. Skip!\n", __func__); // for debug
        return -0xFF;
    }
    else {
        //dprintk("%s: pass wait_event_interruptible by condition (%d)\n", __func__, gpio_get_value(pn547_dev->irq_gpio)); // for debug
    }
    ...
    if (count == 0) {
        printk("%s: reading 0 bytes! skip! (%d)\n", __func__, ret);
        return ret;
    }
    ...
    //dprink("%s: i2c_master_recv success (%d)\n", __func__, ret); // for debug
    return ret;
}
        ...
        size_t count, loff_t *offset)
{
    ...
    if (copy_from_user(tmp, buf, count)) {
        printk(PN547_DRV_NAME ":%s : failed to copy from user space\n", __func__);
        return -EFAULT;
    }
    ...
    //dprintk(PN547_DRV_NAME ":write: pn547_write tmp[0]=:%x tmp[1]=:%x tmp[2]=:%x tmp[3]=:%x\n", tmp[0],tmp[1],tmp[2],tmp[3]);
...
//                    enable_irq_wake(pn547_get_irq_pin(pn547_dev));
                    ...
                    printk("%s IRQ is already enabled!\n", __func__);
                }
#endif
...
//                    disable_irq_wake(pn547_get_irq_pin(pn547_dev));
                    ...
                    printk("%s IRQ is already disabled!\n", __func__);
                }
#endif
                if (sIsWakeLocked == true) {
                    printk("%s: Release Wake_Lock\n", __func__);
                    sIsWakeLocked = false;
                    wake_unlock(&nfc_wake_lock);
                }
                //dprink("%s NFC_POWER_OFF\n", __func__); // for debug
                sPowerState = NFC_POWER_OFF;
                spin_unlock_irqrestore(&pn547_dev->irq_enabled_lock, flags);
            }
            else {
                printk("%s NFC is alread Off!\n", __func__);
            }
        } else {
                printk("%s bad arg %ld\n", __func__, arg);
            return -EINVAL;
        ...
        const struct i2c_device_id *id)
{
   ...
   /* request irq.  the irq is set whenever the chip has data available
     ...
     */
    pr_info("%s : requesting IRQ %d\n", __func__, pn547_dev->irq_gpio);
    pn547_dev->irq_enabled = true;
#ifdef CONFIG_LGE_NFC_HW_ODIN
    ret = odin_gpio_sms_request_irq(pn547_gpio_to_irq(pn547_dev), pn547_dev_irq_handler, IRQF_TRIGGER_RISING|IRQF_NO_SUSPEND,
            "pn547_IRQ", pn547_dev);
#else
    ret = request_irq(pn547_gpio_to_irq(pn547_dev), pn547_dev_irq_handler,
              IRQF_TRIGGER_RISING|IRQF_NO_SUSPEND, pdev->name, pn547_dev);
#endif
...
err_exit:
    printk(PN547_DRV_NAME ": pn547_dev is null\n");
    printk(PN547_DRV_NAME ": pn547_probe() end with error!\n");
...
{
    int ret = 0;
    dprintk(PN547_DRV_NAME ": Start async init\n");

    ret = i2c_add_driver(&pn547_driver);
    if (ret < 0) {
        printk("[NFC]failed to i2c_add_driver\n");
    }
    printk(PN547_DRV_NAME ": Loading PN547 driver Success! \n");
    return;
}

static int __init pn547_dev_init(void)
{
    printk(PN547_DRV_NAME ": Loading PN547 driver\n");
