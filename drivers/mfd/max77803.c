			      ...
			      const struct i2c_device_id *id)
{
	...
	if (pdata) {
		max77803->irq_base = irq_alloc_descs(-1, 0, MAX77803_IRQ_NR, -1);
		if (max77803->irq_base < 0) {
			pr_err("%s: irq_alloc_descs Fail ret(%d)\n",
					__func__, max77803->irq_base);
			ret = -EINVAL;
		} else {
			pdata->irq_base = max77803->irq_base;
		}
...
//	max77803_update_reg(i2c, MAX77803_CHG_REG_SAFEOUT_CTRL, 0x00, 0x30);
	...
	ret = mfd_add_devices(max77803->dev, -1, max77803_devs,
			ARRAY_SIZE(max77803_devs), NULL, 0, NULL);
	if (ret < 0)
		goto err_mfd;
