			      ...
			      const struct i2c_device_id *id)
{
	...
	if (pdata) {
		max77804k->pdata = pdata;
		max77804k->irq_base = irq_alloc_descs(-1, 0, MAX77804K_IRQ_NR, -1);
		if (max77804k->irq_base < 0) {
			pr_err("%s: irq_alloc_descs Fail ret(%d)\n",
					__func__, max77804k->irq_base);
			ret = -EINVAL;
		} else {
			pdata->irq_base = max77804k->irq_base;
		}
	...
	max77804k_update_reg(max77804k->muic, MAX77804K_MUIC_REG_CTRL4,
			MAX77804K_ADC_ONESHOT<<MAX77804K_CTRL4_ADCMODE_SHIFT,
			MAX77804K_CTRL4_ADCMODE_MASK);
#else
	/* Set continuous mode */
	max77804k_update_reg(max77804k->muic, MAX77804K_MUIC_REG_CTRL4,
			MAX77804K_ADC_ALWAYS<<MAX77804K_CTRL4_ADCMODE_SHIFT,
			MAX77804K_CTRL4_ADCMODE_MASK);
#endif
	return ret;
