...
static bool max77843_charger_unlock(struct max77843_charger_data *charger)
{
	...
	do {
		max77843_read_reg(i2c, MAX77843_CHG_REG_CNFG_06, &reg_data);
		chgprot = ((reg_data & 0x0C) >> 2);
		if (chgprot != 0x03) {
			pr_err("%s: unlock err, chgprot(0x%x), retry(%d)\n",
					__func__, chgprot, retry_cnt);
			max77843_write_reg(i2c, MAX77843_CHG_REG_CNFG_06,
					   (0x03 << 2));
			need_init = true;
			msleep(20);
		} else {
...
static void max77843_chg_cable_work(struct work_struct *work)
{
	struct max77843_charger_data *charger =
	container_of(work, struct max77843_charger_data,chg_cable_work.work);
	int quotient, remainder;
	u8 reg_data;
	...
	int enable)
{
	...
	max77843_write_reg(charger->i2c,MAX77843_CHG_REG_CNFG_00, reg_data);
}
...
static int max77843_charger_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&charger->chg_cable_work, max77843_chg_cable_work);
