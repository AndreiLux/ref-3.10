			 ...
			 int temperature)
{
	...
	max77843_bulk_read(fuelgauge->i2c, TEMPERATURE_REG,
			   2, data);
...
int max77843_fg_adjust_capacity(struct max77823_fuelgauge_data *fuelgauge)
{
	u8 data[2];
	...
	pr_info("%s: After adjust - RepSOC(%d)\n", __func__,
		max77843_fg_read_soc(fuelgauge));
...
void max77843_fg_fullcharged_compensation(struct max77823_fuelgauge_data *fuelgauge,
...
void max77843_fg_check_vf_fullcap_range(struct max77823_fuelgauge_data *fuelgauge)
{
...
void max77843_fg_set_full_charged(struct max77823_fuelgauge_data *fuelgauge)
{
	pr_info("[FG_Set_Full] (B) FullCAP(%d), RemCAP(%d)\n",
		(max77843_read_word(fuelgauge->i2c, FULLCAPREP_REG)/2),
...
static bool max77843_is_booted_in_low_battery(struct max77823_fuelgauge_data *fuelgauge)
{
	int fg_vcell = max77843_get_fuelgauge_value(fuelgauge, FG_VOLTAGE);
	int fg_current = max77843_get_fuelgauge_value(fuelgauge, FG_CURRENT);
	int threshold = 0;
...
static bool max77843_fuelgauge_recovery_handler(struct max77823_fuelgauge_data *fuelgauge)
{
	...
	if (fuelgauge->info.soc >= LOW_BATTERY_SOC_REDUCE_UNIT) {
		pr_err("%s: Reduce the Reported SOC by 1%%\n",
			__func__);
		current_soc =
			max77843_get_fuelgauge_value(fuelgauge, FG_LEVEL) / 10;
		...
		if (current_soc) {
			...
			temperature =
				max77843_get_fuelgauge_value(fuelgauge, FG_TEMPERATURE);
			avsoc =
				max77843_get_fuelgauge_value(fuelgauge, FG_AV_SOC);
...
static int max77843_get_fuelgauge_soc(struct max77823_fuelgauge_data *fuelgauge)
{
	...
	if (fullcap_check_interval >
		...
		max77843_fg_check_vf_fullcap_range(fuelgauge);
	...
	if (fuelgauge->info.low_batt_boot_flag) {
		...
		if (fuelgauge->pdata->check_cable_callback &&
			...
			!max77843_is_booted_in_low_battery(fuelgauge)) {
			max77843_fg_adjust_capacity(fuelgauge);
	...
	if (value.intval != POWER_SUPPLY_STATUS_DISCHARGING &&
		...
		if (fuelgauge->info.full_check_flag == 2) {
			...
			max77843_fg_set_full_charged(fuelgauge);
			fg_soc = max77843_get_fuelgauge_value(fuelgauge, FG_LEVEL);
...
static void max77843_full_comp_work_handler(struct work_struct *work)
{
	...
	if (avg_current >= 25) {
		...
		max77843_fg_fullcharged_compensation(fuelgauge,
			(int)(value.intval ==
...
bool max77843_fg_full_charged(struct max77823_fuelgauge_data *fuelgauge)
{
	union power_supply_propval value;
