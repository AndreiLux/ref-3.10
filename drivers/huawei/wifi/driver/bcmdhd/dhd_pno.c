	...
	char *buf, int nbufsize)
{
	...
	if (!params_batch->get_batch.batch_started) {
		bp += nreadsize = snprintf(bp,nbufsize, "scancount=%d\n",	params_batch->get_batch.expired_tot_scan_cnt);
		nleftsize -= nreadsize;
...
#endif
				/* BSSID info */
				bp += nreadsize = snprintf(bp,nbufsize, "bssid=%s\n",
				bcm_ether_ntoa((const struct ether_addr *)&iter->BSSID, eabuf));
				nleftsize -= nreadsize;
				/* SSID */
				bp += nreadsize = snprintf(bp,nbufsize, "ssid=%s\n", iter->SSID);
				nleftsize -= nreadsize;
				/* channel */
				bp += nreadsize = snprintf(bp,nbufsize, "freq=%d\n",
				wf_channel2mhz(iter->channel,
				iter->channel <= CH_MAX_2G_CHANNEL?
				WF_CHAN_FACTOR_2_4_G : WF_CHAN_FACTOR_5_G));
				nleftsize -= nreadsize;
				/* RSSI */
				bp += nreadsize = snprintf(bp,nbufsize, "level=%d\n", iter->RSSI);
				nleftsize -= nreadsize;
				/* add the time consumed in Driver to the timestamp of firmware */
				iter->timestamp += t_delta;
				bp += nreadsize = snprintf(bp,nbufsize, "age=%d\n", iter->timestamp);
				nleftsize -= nreadsize;
				/* RTT0 */
				bp += nreadsize = snprintf(bp,nbufsize, "dist=%d\n",
				(iter->rtt0 == 0)? -1 : iter->rtt0);
				nleftsize -= nreadsize;
				/* RTT1 */
				bp += nreadsize = snprintf(bp,nbufsize, "distSd=%d\n",
				(iter->rtt0 == 0)? -1 : iter->rtt1);
				nleftsize -= nreadsize;
				bp += nreadsize = snprintf(bp,nbufsize, "%s", AP_END_MARKER);
				nleftsize -= nreadsize;
				list_del(&iter->list);
...
dhd_pno_event_handler(dhd_pub_t *dhd, wl_event_msg_t *event, void *event_data)
{
	...
	DHD_PNO(("%s enter : event_type :%d,flags:%d, status:%d,datalen:%d;\n", __FUNCTION__, event_type,flags,status,datalen));
	switch (event_type) {
...
int dhd_pno_init(dhd_pub_t *dhd)
{
	...
	if(dhd->pno_state == NULL)
	{
		DHD_ERROR(("%s:dhd->pno_state malloc failed.\n",__FUNCTION__));
		return BCME_ERROR;
	}
