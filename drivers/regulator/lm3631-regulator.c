...
static int lm3631_regulator_list_voltage(struct regulator_dev *rdev,
					 unsigned selector)
{
...
static int lm3631_regulator_set_voltage_sel(struct regulator_dev *rdev,
					    unsigned selector)
{
...
static int lm3631_regulator_get_voltage_sel(struct regulator_dev *rdev)
{
	struct lm3631_regulator *lm3631_regulator = rdev_get_drvdata(rdev);
	enum lm3631_regulator_id id = rdev_get_id(rdev);
	u8 addr, mask, val;
...
static struct regulator_ops lm3631_boost_voltage_table_ops = {
	.list_voltage     = lm3631_regulator_list_voltage,
	.set_voltage_sel  = lm3631_regulator_set_voltage_sel,
	.get_voltage_sel  = lm3631_regulator_get_voltage_sel,
};
...
struct of_regulator_match lm3631_regulator_matches[] = {
...
*/
	config.dev         = dev;
	config.init_data   = lm3631_regulator->init_data;
	config.of_node     = dev->of_node;
	config.driver_data = lm3631_regulator;
	config.regmap	   = NULL;


	rdev = regulator_register(&lm3631_regulator_desc[id], &config);
#if 0
