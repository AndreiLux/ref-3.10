...
static struct voltage_map_desc buck_voltage_map_desc = {
	.min = 400000,	.max = 1500000,	.step = 12500,	.n_bits = 8,
};
...
static int s2mpb01_ldo_is_enabled(struct regulator_dev *rdev)
{
	...
	pr_debug("%s: id=%d, val=0x%x, enval=0x%x\n",
		__func__, rdev_get_id(rdev), val, ((val >> shift) == mask));
...
/* TEMP: use prev sub pmic(max77826) regulator name */
	...
	struct s2mpb01_platform_data *pdata)
{
	...
	/* TEMP: reduce 2 num of regulators because do not init BUCK */
	s2mpb01->num_regulators = pdata->num_regulators - 2;
...
#ifdef CONFIG_OF
static int s2mpb01_pmic_dt_parse_pdata(struct i2c_client *i2c,
					struct s2mpb01_platform_data *pdata)
{
	return 0;
}
			    ...
			    const struct i2c_device_id *id)
{
	...
	if (i2c->dev.of_node) {
		...
		ret = s2mpb01_pmic_dt_parse_pdata(i2c, pdata);
		if (ret)
			return ret;
	} else {
		pr_info("%s: pdata\n", __func__);
