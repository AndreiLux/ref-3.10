...
crypto_create_session(struct fcrypt *fcr, struct session_op *sop)
{
	...
	if (unlikely(!sop->cipher && !sop->mac && !sop->rng)) {
		ddebug(1, "All of 'cipher', 'mac', and 'rng' unset.");
		return -EINVAL;
	}
	if (unlikely(sop->rng && (sop->cipher || sop->mac))) {
		ddebug(1, "Requested randomness at the same time as a cipher or hash operation.");
		return -EINVAL;
	}
	...
	if (rng_name) {
		if (unlikely(sop->keylen > CRYPTO_RNG_MAX_KEY_LEN)) {
			ddebug(1, "Setting seed failed for %s-%zu.",
				rng_name, (size_t)sop->keylen*8);
			ret = -EINVAL;
			goto error_rng;
		}

		if (sop->key && unlikely(copy_from_user(keys.ckey, sop->key,
					    sop->keylen))) {
			ret = -EFAULT;
			goto error_rng;
		}

		ret = cryptodev_rng_init(&ses_new->rdata, rng_name,
							keys.ckey, sop->keylen);
		if (ret != 0) {
			ddebug(1, "Failed to load random number generator for %s", rng_name);
			ret = -EINVAL;
			goto error_rng;
		}
	}
...
crypto_destroy_session(struct csession *ses_ptr)
{
	...
	cryptodev_rng_deinit(&ses_ptr->rdata);
...
static int fill_kcop_from_cop(struct kernel_crypt_op *kcop, struct fcrypt *fcr)
{
	...
	kcop->rnglen     = 0; /* will be updated during operation */
...
static int fill_cop_from_kcop(struct kernel_crypt_op *kcop, struct fcrypt *fcr)
{
	...
	if (kcop->rnglen) {
		ret = copy_to_user(kcop->cop.dst,
				kcop->rng_output, kcop->rnglen);
		kfree(kcop->rng_output);
		kcop->rng_output = NULL;
		kcop->rnglen = 0;
		if (unlikely(ret))
			return -EFAULT;
	}
...
static int get_session_info(struct fcrypt *fcr, struct session_info_op *siop)
{
	...
	if (ses_ptr->rdata.init) {
		tfm = crypto_rng_tfm(ses_ptr->rdata.s);
		tfm_info_to_alg_info(&siop->rng_info, tfm);
#ifdef CRYPTO_ALG_KERN_DRIVER_ONLY
		if (tfm->__crt_alg->cra_flags & CRYPTO_ALG_KERN_DRIVER_ONLY)
			siop->flags |= SIOP_FLAG_KERNEL_DRIVER_ONLY;
#else
