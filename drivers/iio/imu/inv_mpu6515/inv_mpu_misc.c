...
#define DEF_ACCEL_OFFSET_MAX            500
...
#define DEF_SELFTEST_ACCEL_SENS          (32768 / 2000)
...
static int inv_check_6500_gyro_self_test(struct inv_mpu_state *st,
	...
	if (result)
		return result;
	printk("%s self_test gyro shift_code - %02x %02x %02x\n",
	...
	printk("%s self_test gyro st_shift_prod - %+d %+d %+d\n",
	...
	printk("%s self_test gyro st_shift_cust - %+d %+d %+d\n",
...
#define ACCEL_ST_AL_MAX ((DEF_ACCEL_ST_AL_MAX * DEF_ST_SCALE \
	...
	if (result)
		return result;
	printk("%s self_test accel shift_code - %02x %02x %02x\n",
		 st->hw->name, regs[0], regs[1], regs[2]);
	...
	printk("%s self_test accel st_shift_prod - %+d %+d %+d\n",
		 st->hw->name, st_shift_prod[0], st_shift_prod[1],
		 st_shift_prod[2]);
	...
	if (!otp_value_zero) {
		...
		for (i = 0; i < 3; i++) {
			...
			printk("ratio=%d, threshold=%d\n", st_shift_ratio[i],
							DEF_6500_ACCEL_ST_SHIFT_DELTA);
			if (st_shift_ratio[i] > DEF_6500_ACCEL_ST_SHIFT_DELTA) {
				printk("Accel axis:%d > Delta Shift\n", i);
				ret_val = 1;
			}
		}
	} else {
	...
	printk("%s self_test accel st_shift_cust - %+d %+d %+d\n",
		 st->hw->name, st_shift_cust[0], st_shift_cust[1],
		 st_shift_cust[2]);
	...
	if (ret_val == 0) {
		/* Self Test Pass/Fail Criteria C */
		for (i = 0; i < 1; i++)
			if (abs(reg_avg[i]) > DEF_ACCEL_OFFSET_MAX *
						DEF_SELFTEST_ACCEL_SENS *
						DEF_ST_PRECISION) {
				printk("ACCEL:CRITERIA C: Accel axis:%d\n", i);
				ret_val = 1;
			}
		//this is for testing the offset for the Z axis. This is only for
		//if Axis Z is against or with gravity
		i = 2;
		reg_temp = abs(reg_avg[i]) - (DEF_SELFTEST_ACCEL_SENS *
			DEF_ST_PRECISION * 1000);
		if (abs(reg_temp) > DEF_ACCEL_OFFSET_MAX *
						DEF_SELFTEST_ACCEL_SENS *
						DEF_ST_PRECISION) {
			printk("ACCEL:CRITERIA C: Accel axis:3\n");
			ret_val = 1;
		}
	}
...
int inv_hw_self_test(struct inv_mpu_state *st)
{
	...
	printk("Starting MPU6500 Sensitivity Self Test\n");
	...
	if (result)
	{
		printk("Failed to run Self Test: inv_reset_offset_reg fail\n");
		return result;
	}
	compass_result = 0;
	accel_result = 0;
...
static int inv_accel_dmp_cal(struct inv_mpu_state *st)
{
	...
	if ((inv_accel_orient & 3) > 2)
		return 0;
	if (((inv_accel_orient >> 3) & 3) > 2)
		return 0;
	if (((inv_accel_orient >> 6) & 3) > 2)
		return 0;
...
int inv_set_accel_bias_dmp(struct inv_mpu_state *st)
{
	...
	for (i = 0; i < 3; i++) {
		index = (inv_accel_orient >> (i * 3)) & 3;
		if (index > 2)
			return 0;
		accel_bias_body[i] =
			st->input_accel_dmp_bias[index] * tmp[i];
	}
	for (i = 0; i < 3; i++)
	...
	char *buf, loff_t pos, size_t size)
{
	...
	pr_err("%s firmware loaded size %d actual %d\n",
	       __func__, DMP_IMAGE_SIZE, size);
