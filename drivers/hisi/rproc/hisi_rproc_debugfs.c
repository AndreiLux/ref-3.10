...
#define MODULE_NAME "RPROC_DEBUGFS"
	...
	for (i = 0; i < times; i++) {
		if (rproc_work->is_sync_call) {
			pr_info("[RPROC DEBUGFS] SYNC-CALL start ---- No.%d\n",
				i + 1);
			ret = RPROC_SYNC_SEND(rproc_work->name,
						rproc_work->msg,
						rproc_work->len,
						ASYNC_MSG,
						ack_buffer,
			...
			pr_info("[RPROC DEBUGFS] ASYNC-CALL start ---- No.%d\n",
				i + 1);
			ret = RPROC_ASYNC_SEND(rproc_work->name,
						rproc_work->msg,
						rproc_work->len,
						ASYNC_MSG,
...
static void
rproc_debugfs_schedule_work(const char *name, rproc_msg_t *msg,
		rproc_msg_len_t len, int times, int is_sync_call)
{
	...
	for (i = 0; i < RPROC_TX_WORK_NUM; i++) {
		...
		if (!work_pending(&rproc_work->work)) {
			memset(rproc_work->name, 0, sizeof(rproc_work->name));
			strncpy(rproc_work->name, name, strlen(name));
			*(rproc_work->name + strlen(name)) = '\0';
...
static void
rproc_debugfs_sync_call_test(const char *name, rproc_msg_t *msg,
				rproc_msg_len_t len, int times)
{
	rproc_debugfs_schedule_work(name, msg, len, times, SYNC_CALL);
}

static void
rproc_debugfs_async_call_test(const char *name, rproc_msg_t *msg,
				rproc_msg_len_t len, int times)
{
	rproc_debugfs_schedule_work(name, msg, len, times, ASYNC_CALL);
}
...
rproc_debugfs_notifier(struct notifier_block *nb, unsigned long len, void *msg)
{
	...
	pr_info("[RPROC DEBUGFS] %s rx notifier\n", someone->name);
	for (i = 0; i < len; i++)
...
static void rproc_debugfs_rx_register(const char *rproc_name)
{
	...
	if (!new_listener)
		return;
	strncpy(new_listener->name, rproc_name, strlen(rproc_name));
	*(new_listener->name + strlen(rproc_name)) = '\0';

	new_listener->nb.next = NULL;
	new_listener->nb.notifier_call = rproc_debugfs_notifier;

	if (RPROC_MONITOR_REGISTER(rproc_name, &new_listener->nb)) {
		pr_info("[RPROC DEBUGFS] fail to register rx listener, rproc %s\n",
			rproc_name);
		kfree(new_listener);
		return;
	}
...
static void rproc_debugfs_rx_unregister(int listeners)
{
	...
	(void)RPROC_MONITOR_UNREGISTER(de_listener->name, &de_listener->nb);
					...
					size_t cnt, loff_t *ppos)
{
	...
	if (!strncmp("register ", _cmd, strlen("register "))) {
		...
		*_cmd = '\0';
		...
		strncpy(rproc_name, cmd, strlen(cmd));
		*(rproc_name + strlen(cmd)) = '\0';

		cmd = _cmd + 1;
		len = stoa(cmd, msg);

		rproc_debugfs_sync_call_test(rproc_name, msg, len, times);
		...
		*(rproc_name + strlen(cmd)) = '\0';
		...
		rproc_debugfs_async_call_test(rproc_name, msg, len, times);
	} else {
		pr_err("[RPROC DEBUGFS] cmd error\n");
		cnt = -EINVAL;
	}
