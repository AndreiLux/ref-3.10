...
int comp_dcdc_type = NO_DEV;
EXPORT_SYMBOL(comp_dcdc_type);
...
void ncp6335b_set_voltage(int version)
{
	...
	if ( ncp_dev == NULL ) {
		pr_err("[syscamera][%s::%d][ERR] ncp_dev is NULL \n", __func__, __LINE__);
		return;
	} else {
		if ( ncp_dev->client == NULL) {
			pr_err("[syscamera][%s::%d][ERR] client is NULL \n", __func__, __LINE__);
			return;
		}
	}
...
static int ncp6335b_dcdc_remove(struct i2c_client *client)
{
	kfree(ncp_dev);
	ncp_dev = NULL;
	return 0;
...
struct i2c_driver ncp6335b_driver = {
	.probe = ncp6335b_dcdc_probe,
	.remove = ncp6335b_dcdc_remove,
	.driver = {
		.name = DRV_NAME,
