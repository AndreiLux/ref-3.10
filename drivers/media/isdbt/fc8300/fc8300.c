...
#define TS0_1SEG_LENGTH	(188 * 5)
#define TS0_TMM_13SEG_LENGTH (188 * 32)

#define USE_GPIO_EXPANDER_BT_EN
...
/*		
	gpio_tlmm_config(GPIO_CFG(isdbt_pdata->gpio_i2c_sda, GPIOMUX_FUNC_GPIO,
					 GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
					 GPIO_CFG_ENABLE);
	
	gpio_tlmm_config(GPIO_CFG(isdbt_pdata->gpio_i2c_scl, GPIOMUX_FUNC_GPIO,
					 GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
					 GPIO_CFG_ENABLE);
*/
...
static struct isdbt_platform_data *isdbt_populate_dt_pdata(struct device *dev)
{
 ...
 int rc = 0;
	...
	rc = of_property_read_u32(dev->of_node, "qcom,isdb-gpio-pwr-en", &pdata->gpio_en );
	if (rc < 0) {
	...
	rc = of_property_read_u32(dev->of_node, "qcom,isdb-gpio-rst", &pdata->gpio_rst);
	if (rc < 0) {
