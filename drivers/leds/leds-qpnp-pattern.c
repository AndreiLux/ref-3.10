 ...
 */
#define BRIGHTNESS_MAX_FOR_RGB          255
#define BRIGHTNESS_MAX_FOR_LUT          511
#define BRIGHTNESS_MAX_SCALE             97

#define TUNING_FACTOR_BASE              100
#define TUNING_FACTOR_RED               100
#define TUNING_FACTOR_GREEN             100
#define TUNING_FACTOR_BLUE               40
 ...
 */
static int                    qpnp_brightness_scale = BRIGHTNESS_MAX_SCALE;
...
static int qpnp_pattern_play(int parameter_pattern [])
{
	...
	for( i=0; i<PATTERN_SIZE_LUT; i++ )
		parameter_scaled[i] = parameter_pattern[i] * qpnp_brightness_scale / BRIGHTNESS_MAX_FOR_RGB;
	...
	for ( i=0; i<PATTERN_SIZE_LUT; i++ ) {
		...
		if (pattern_red_start<=i && i<pattern_red_start+pattern_red_length)
			lut_tuning = TUNING_FACTOR_RED;
		else if (pattern_green_start<=i && i<pattern_green_start+pattern_green_length)
			lut_tuning = TUNING_FACTOR_GREEN;
		else if (pattern_blue_start<=i && i<pattern_blue_start+pattern_blue_length)
			lut_tuning = TUNING_FACTOR_BLUE;
		else
			lut_tuning = 0;

		parameter_scaled[i] = parameter_scaled[i] * lut_tuning / TUNING_FACTOR_BASE;
	}
...
static int qpnp_pattern_solid(struct qpnp_led_data *led)
{
	...
	parameter_solid[0] = parameter_solid[1] = (led->cdev.brightness & 0xFF)
			* qpnp_brightness_scale / BRIGHTNESS_MAX_FOR_RGB * lut_tuning / TUNING_FACTOR_BASE;
