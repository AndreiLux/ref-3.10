        ...
        if (onoff) { 	/* enable */
            if (mode) { 	/* flash */
                ...
                if (ret)
                    ...
                    pr_info("[LED] Turn on LED for Flash\n");
...
static void max77828_flash_set(struct max77828_led_data *led_data)
{
    ...
    if(brightness == LED_OFF) {
        ...
        pr_info("[LED](%s) turn on as brightness %d\n",__func__, brightness);

        ret = max77828_write_reg(led_data->i2c, MAX77828_LED_REG_DCDC_CNTL1, 0xC0);
        if(IS_ERR_VALUE(ret))
...
static int max77828_led_hw_setup(struct max77828_led_data *led_data)
{
    ...
    /*  pin control setting. value is 0xCA */
...
static int max77828_led_setup(struct max77828_led_data *led_data)
{
    ...
    case MAX77828_FLASH:
        ...
        ret = max77828_write_reg(led_data->i2c, MAX77828_LED_REG_I_FLASH1, MAX77828_FLASH_FLED1_EN);
        if (IS_ERR_VALUE(ret))
            pr_err("(%s) :  MAX77828_LED_REG_I_FLASH1 write failed\n",__func__);
        break;
    case MAX77828_TORCH:
        ...
        ret = max77828_write_reg(led_data->i2c, MAX77828_LED_REG_I_TORCH1, MAX77828_TORCH_FLED1_EN);
        if(IS_ERR_VALUE(ret))
            pr_err("(%s) :  MAX77828_LED_REG_I_TORCH1 write failed\n",__func__);
        break;
    default :
        pr_err("Error : No such LED present\n");
        break;
    }
    return ret;
}

static ssize_t max77828_flash_show(struct device *dev,
                                   struct device_attribute *attr, char *buf)
{
    char temp_print[] = "rear flash : max77828\n";
                              ...
                              struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if (count == size) {
        ...
        DEBUG_MAX77828("[LED] %s : brightness level %d\n",
                       __func__, (int)state);
...
static int max77828_led_probe(struct platform_device *pdev)
{
    ...
    fdata->flash_ramp_up = flash_ramp[6];
    fdata->flash_ramp_down = flash_ramp[6];
    fdata->torch_ramp_up = torch_ramp[1];
    fdata->torch_ramp_down = torch_ramp[1];
    ...
    for (i = 0; i != pdata->num_leds; ++i) {
        ...
        if (unlikely(ret)) {
            pr_err("unable to register LED, line(%d)\n", __LINE__);
            kfree(led_data);
            ret = -EFAULT;
            continue;
        }
...
static int max77828_led_remove(struct platform_device *pdev)
{
...
static int __init max77828_led_init(void)
{
    ...
    rc = platform_driver_register(&max77828_led_driver);
    if (rc)
        pr_err("%s : failed\n", __FUNCTION__);
    else
        pr_info("%s : X\n", __FUNCTION__);
