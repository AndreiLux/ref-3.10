...
static ssize_t max77804k_flash_show(struct device *dev,
                                   struct device_attribute *attr, char *buf)
{
    char temp_print[] = "rear flash : max77804k\n";
...
static DEVICE_ATTR(rear_flash, S_IWUSR|S_IWGRP|S_IROTH,
	max77804k_flash_show, max77804k_flash);
...
static int __init max77804k_led_init(void)
{
    ...
    rc = platform_driver_register(&max77804k_led_driver);
    if (rc)
        pr_err("%s : failed\n", __FUNCTION__);
    else
        pr_info("%s : X\n", __FUNCTION__);
