...
void max77804_led_get_status(struct led_classdev *led_cdev, bool status, bool onoff)
{
	int ret = 0;
	u8 value[6] = {0, };
	struct max77804_led_data *led_data
	...
	ret = max77804_read_reg(led_data->i2c, 0x02, &value[0]); //Torch Current setting
	ret |= max77804_read_reg(led_data->i2c, 0x03, &value[1]); //Torch Timer
	ret |= max77804_read_reg(led_data->i2c, 0x05, &value[2]); //Torch Enable
	ret |= max77804_read_reg(led_data->i2c, 0x06, &value[3]); //Max Flash setting
	ret |= max77804_read_reg(led_data->i2c, 0x07, &value[4]); //Status
	ret |= max77804_read_reg(led_data->i2c, 0x08, &value[5]); //Status
	if (unlikely(ret < 0)) {
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (count == size) {
		ret = count;
		max77804_led_get_status(led_cdev, false, state);
		...
		max77804_led_get_status(led_cdev, true, state);
	}
