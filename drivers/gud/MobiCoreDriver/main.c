...
static int mc_fd_mmap(struct file *file, struct vm_area_struct *vmarea)
{
	...
	MCDRV_DBG_VERBOSE(mcd, "enter (vma start=0x%p, size=%ld, "
		"offset=%ld, mci=0x%llX)",
...
#if defined(TBASE_CORE_SWITCHER)
static void fake_func(void *info)
{
	return;
}
			...
			size_t buffer_len, loff_t *x)
{
	...
	/* If it's a digit then switch cores */
		...
		cpu_new = buf[0] - '0';
		if (cpu_new <= 8) {
			if (cpu_new == mc_cpu_active()) {
				MCDRV_DBG_VERBOSE(mcd, "No need for same cpu");
				return buffer_len;
			}
			MCDRV_DBG_VERBOSE(mcd, "Set Active Cpu: %d\n", cpu_new);
			odin_idle_disable(cpu_new, true);
			smp_call_function_single(cpu_new, fake_func, NULL, 1);
			mc_switch_core(cpu_new);
			odin_idle_disable(cpu_new, false);
		}
	} else {
		return -EINVAL;
	}
