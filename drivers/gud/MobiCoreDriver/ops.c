...
#ifdef CONFIG_EVENT_TRACING
	trace_secure_enter((unsigned int)MC_SMC_N_YIELD);
	mc_fastcall(&yield);
	trace_secure_exit((unsigned int)MC_SMC_N_YIELD);
#else
	mc_fastcall(&yield);
#endif /* CONFIG_EVENT_TRACING */
...
#ifdef CONFIG_EVENT_TRACING
	trace_secure_enter((unsigned int)MC_SMC_N_SIQ);
	mc_fastcall(&nsiq);
	trace_secure_exit((unsigned int)MC_SMC_N_SIQ);
#else
	mc_fastcall(&nsiq);
#endif /* CONFIG_EVENT_TRACING */
