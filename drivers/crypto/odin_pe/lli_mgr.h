...
/* LLI Manager object handle (per device context) */
typedef void *llimgr_h;
...
#define MLLI_TABLES_LIST_INIT(mlli_tables_list_ptr) do {		\
	memset((void *)(mlli_tables_list_ptr), 0,			\
 ...
 * @sg_orig:		Original S/G received from (kernel) client
...
struct client_dma_buffer {
	...
	/* DMA mapping info.
	(either created for user space pages or retrieved from kernel client) */
...
struct mlli_tables_list {
	uint32_t link_to_first_table[2 * SEP_LLI_ENTRY_WORD_SIZE];
 ...
 *
 */
void llimgr_dump_mlli_tables_list(
