...
typedef u64 host_crypto_ctx_id_t;
#define CTX_ID_INVALID ((host_crypto_ctx_id_t)0)
...
#define HOST_CTX_COMMON \
	...
	enum host_ctx_state state; /* To hold CTX_VALID_SIG when initialized */\
	enum crypto_alg_class alg_class;/*determine the context specification*/\
	/* Cast the whole struct to matching user_ctx_* struct */              \
	bool is_encrypted        /* Algorithm properties are encrypted */      \
...
#define SEP_CTX_SEGMENT                                                        \
	uint8_t reserved_before[L1_CACHE_BYTES];			       \
	struct sep_ctx_cache_entry sep_ctx;                                    \
	uint8_t reserved_after[L1_CACHE_BYTES]
#else
...
struct hash_block_remainder {
	uint16_t size; /* Octets available in data_blk_tail */
	uint8_t data[HASH_BLK_SIZE_MAX] __attribute__((aligned(8)));
...
struct host_crypto_ctx_mac {
	HOST_CTX_COMMON;
	uint64_t client_data_size; /* Sum. up the data processed so far */
...
/**
 * ctxmgr_is_valid_data_unit_size() - Validate given data unit for given alg./mode
 * @ctx_info:
 * @data_unit_size:
 * @is_finalize:
 *
...
int ctxmgr_set_symcipher_iv_user(
	uint32_t __user *user_ctx_ptr, uint8_t *iv_ptr);
 ...
 * Returns int
 */
int ctxmgr_get_symcipher_iv_user(
	uint32_t __user *user_ctx_ptr, uint8_t *iv_ptr);
