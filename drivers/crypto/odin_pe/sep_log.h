...
#define SEP_LOG_MASK_IPSEC_QS    (1<<10)
...
#define SEP_LOG_MASK_ALL        (SEP_LOG_MASK_MAIN | SEP_LOG_MASK_SEP_INIT |\
	...
	SEP_LOG_MASK_SEP_APP | SEP_LOG_MASK_IPSEC_QS | SEP_LOG_MASK_SEP_PRINTF)
...
#define MODULE_PRINTK(level, format, ...) \
...
#define SEP_LOG_TRACE(format, ...) \
	if (sep_log_level >= SEP_LOG_LEVEL_TRACE) \
		MODULE_PRINTK(KERN_DEBUG, "<trace> " format, ##__VA_ARGS__)
#else
