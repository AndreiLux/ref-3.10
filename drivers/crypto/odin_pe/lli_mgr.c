...
#define SEP_MLLI_SET_NEXT_VA(cur_mlli_p, next_mlli_p) do {             \
	SEP_LLI_SET(cur_mlli_p , ADDR, (uint32_t)next_mlli_p);         \
	} while (0)
...
#define SEP_MLLI_GET_NEXT_VA(cur_mlli_start) \
	((uint32_t *)SEP_LLI_GET((cur_mlli_start), ADDR))
...
static void cleanup_mlli_tables_list(
	struct llimgr_obj *llimgr_p,
 ...
 */
llimgr_h llimgr_create(struct device *dev, unsigned long mlli_table_size)
{
	...
	new_llimgr_p->mlli_cache = dma_pool_create(
				"dx_sep_mlli_tables", dev,
				mlli_table_size,
				L1_CACHE_BYTES, 0);
	...
	new_llimgr_p->lli_tbl_size = mlli_table_size;

	/* Create pool for holding buffer "tails" which share cache lines with
	   other data buffers */
	new_llimgr_p->edge_bufs_pool = dma_pool_create(
		"dx_sep_edge_bufs", dev,
		EDGE_BUFS_POOL_ITEM_SIZE,	EDGE_BUFS_POOL_ITEM_SIZE, 0);
	...
	new_llimgr_p->dlli_bufs_pool = dma_pool_create("dx_sep_dlli_bufs", dev,
		DLLI_AUX_BUF_LIMIT, DLLI_AUX_BUF_LIMIT, 0);
 ...
 *
 */
void llimgr_destroy(llimgr_h llimgr)
{
 ...
 * Returns void
 */
static void calc_aux_bufs_size(
	const unsigned long buf_start,
	...
	unsigned long *end_aux_buf_size_p)
{
	...
	*end_aux_buf_size_p = buf_size >= DIN_LAST_LLI_GATHER_SIZE ?
	...
	if ((*end_aux_buf_size_p + *start_aux_buf_size_p) >
	    buf_size) {
	...
	if (((*end_aux_buf_size_p + *start_aux_buf_size_p) > buf_size) ||
		...
		SEP_LOG_ERR("Invalid aux. buffer sizes: buf_size=%lu B, "
			    "start_aux=%lu B, end_aux=%lu B\n",
...
#ifdef DEBUG
static void dump_client_buf_pages(
	const struct client_dma_buffer *client_dma_buf_p)
{
	int i;
	struct scatterlist *sgentry;

	if (client_dma_buf_p->user_buf_ptr != NULL) {
		SEP_LOG_DEBUG("Client DMA buffer %p maps %lu B over %d pages "
			      "at user_ptr=0x%p (dma_dir=%d):\n",
		...
		SEP_LOG_DEBUG("Client DMA buffer %p maps %lu B "
			      "(dma_dir=%d):\n",
			client_dma_buf_p, client_dma_buf_p->buf_size,
			client_dma_buf_p->dma_direction);
	}
	...
	if (client_dma_buf_p->user_pages != NULL) {
		...
		for (i = 0; i < client_dma_buf_p->num_of_pages; i++) {
			SEP_LOG_DEBUG("%d. phys_addr=0x%08lX\n", i,
				page_to_pfn(client_dma_buf_p->user_pages[i]) <<
				PAGE_SHIFT);
		}
	}
	...
	for_each_valid_sg(client_dma_buf_p->sg_head, sgentry) {
		SEP_LOG_DEBUG("%d. phys_addr=0x%08lX len=0x%08X\n", i,
			(unsigned long)sg_phys(sgentry), sgentry->length);
		i++;
	}
#endif
	...
	for_each_valid_sg(client_dma_buf_p->sg_main, sgentry) {
		/* "main" segment is DMA-mapped, so we use the DMA info fields*/
		SEP_LOG_DEBUG("%d. dma_addr=0x%08lX len=0x%08X\n", i,
			(unsigned long)sg_dma_address(sgentry),
			sg_dma_len(sgentry));
		i++;
	}
	...
	struct page ***next_page_p, unsigned long *next_page_offset_p)
{
	...
	*new_sg_list_p = kmalloc(
		sizeof(struct scatterlist) * num_of_sg_ents, GFP_KERNEL);
	...
	*next_page_offset_p = 0;
	...
	for (i = 1; i < (num_of_sg_ents - 1); i++) {
		...
		if (unlikely(cur_sge == NULL)) {
			SEP_LOG_ERR("Reached end of sgl before (%d) "
				    "num_of_sg_ents (%lu)\n",
				i, num_of_sg_ents);
			kfree(*new_sg_list_p);
			*new_sg_list_p = NULL;
	...
	if (num_of_sg_ents > 1) { /* only if was not handled already as first */
		cur_sge = sg_next(cur_sge);
#ifdef DEBUG
		if (unlikely(cur_sge == NULL)) {
			SEP_LOG_ERR("Cannot put last page in given "
				    "num_of_sg_ents (%lu)\n", num_of_sg_ents);
			kfree(*new_sg_list_p);
			*new_sg_list_p = NULL;
			return -EBUG;
...
/**
 * clone_sg_list() - Clone a chunk of given s/g list at given offset/size
 *
 * @sgl_source:		The base s/g list
 * @clone_offset:	The byte offset in sgl_source to start the clone
 * @clone_len:		The byte size of the clone
 * @sgl_clone_p:	The returned s/g list clone
 */
static int clone_sg_list(
	struct scatterlist *sgl_source,
	unsigned long clone_offset,
	unsigned long clone_len,
	struct scatterlist **sgl_clone_p)
{
	struct scatterlist *cur_sge_source, *cur_sge_clone;
	/* s/g entry where clone start/end */
	struct scatterlist *first_sge, *last_sge;
	/* Offsets from first/last sge offset */
	unsigned long first_sge_offset, last_sge_len;
	unsigned long cur_offset;
	unsigned int sge_num;

	SEP_LOG_DEBUG("src=%p off.=%lu len=%lu\n",
		sgl_source, clone_offset, clone_len);
	if (clone_len == 0) { /* Empty S/G list */
		*sgl_clone_p = NULL;
		return 0;
	}
	/* First find first/last entry in source */
	cur_offset = 0;
	cur_sge_source = sgl_source;
	while ((cur_sge_source != NULL) &&
	       ((cur_offset + cur_sge_source->length) <= clone_offset)) {
		cur_offset += cur_sge_source->length;
		cur_sge_source = sg_next(cur_sge_source);
	}
	if (cur_sge_source == NULL) {
		SEP_LOG_ERR("Reached end of source sgl before offset %lu\n",
			clone_offset);
		return -ENOENT;
	}
	first_sge = cur_sge_source;
	/* cur_offset is the global offset at start of current sge.
	   Calculate offset within that sge based on remainder to clone_offset*/
	first_sge_offset = clone_offset - cur_offset;
	SEP_LOG_DEBUG("first_sge=%p first_sge_offset=%lu\n",
		first_sge, first_sge_offset);
	sge_num = 1; /*count num. of entries for clone (first already counted)*/
	/* Find last entry */
	while ((cur_sge_source != NULL) &&
	       (cur_offset + cur_sge_source->length) <
	...
	last_sge = cur_sge_source;
	/* Remainder in last_sge */
	last_sge_len = clone_offset + clone_len - cur_offset;
	SEP_LOG_DEBUG("last_sge=%p last_sge_len=%lu (%u ent.)\n",
		last_sge, last_sge_len, sge_num);

	/* Allocate array for cloned sgl */
	*sgl_clone_p = kmalloc(
		sizeof(struct scatterlist) * sge_num, GFP_KERNEL);
	if (unlikely(*sgl_clone_p == NULL)) {
		SEP_LOG_ERR("Failed allocating sglist array for %u entries\n",
			sge_num);
		return -ENOMEM;
	}
	/* Create (copy to) clone */
	sg_init_table(*sgl_clone_p, sge_num);
	cur_sge_source = first_sge;
	cur_sge_clone = *sgl_clone_p;
	/* Special handling on first entry */
	if (first_sge == last_sge) /* Just one entry */
		sg_set_page(cur_sge_clone, sg_page(cur_sge_source),
			clone_len,
			cur_sge_source->offset + first_sge_offset);
	else /* There are additional entries - take this one up to its end */
		sg_set_page(cur_sge_clone, sg_page(cur_sge_source),
			cur_sge_source->length - first_sge_offset,
			cur_sge_source->offset + first_sge_offset);
	cur_sge_source = sg_next(cur_sge_source);
	cur_sge_clone = sg_next(cur_sge_clone);
	/* Handle following (whole) entries, but last (which may be partial) */
	while ((cur_sge_source != NULL) && (cur_sge_source != last_sge)) {
		sg_set_page(cur_sge_clone, sg_page(cur_sge_source),
			cur_sge_source->length, cur_sge_source->offset);
		cur_sge_source = sg_next(cur_sge_source);
		cur_sge_clone = sg_next(cur_sge_clone);
	}
	/* Handle last (partial?) entry */
	if ((cur_sge_source != NULL) && (first_sge != last_sge)) {
		/* last not handled already as first */
		sg_set_page(cur_sge_clone, sg_page(cur_sge_source),
			last_sge_len, cur_sge_source->offset);
	}
	return 0;
}
	...
	struct client_dma_buffer *client_dma_buf_p)
{
	...
	/* sg_save4next may be pointing to sg_orig... */
	if ((client_dma_buf_p->sg_save4next != NULL) &&
	    (client_dma_buf_p->sg_save4next != client_dma_buf_p->sg_orig))
 ...
 */
static bool is_sgl_phys_contig(
	struct scatterlist *sgl, unsigned long *data_size_p)
{
 ...
 */
static bool is_pages_phys_contig(
	struct page *pages_list[], unsigned int num_of_pages)
{
	int i;
...
static int user_buf_to_client_dma_buf(struct llimgr_obj *llimgr_p,
	struct client_dma_buffer *client_dma_buf_p)
{
	uint8_t __user const *user_buf_ptr = client_dma_buf_p->user_buf_ptr;
	...
	client_dma_buf_p->user_pages = kmalloc(
		sizeof(struct page *) * num_of_pages, GFP_KERNEL);
	...
	client_dma_buf_p->num_of_pages = get_user_pages_fast(
		(unsigned long)user_buf_ptr, num_of_pages, is_outbuf,
		client_dma_buf_p->user_pages);
	if (client_dma_buf_p->num_of_pages != num_of_pages) {
		SEP_LOG_WARN("Failed to lock all user pages "
				"(locked %d, requested lock = %d)\n",
	...
	if (buf_size <= DLLI_BUF_LIMIT) {
		if (/* Check if possible to map buffer directly as DLLI */
#ifdef CONFIG_NOT_COHERENT_CACHE
		    /* For systems with incoherent cache the buffer must be
		       cache line aligned to be cosidered for this case */
		    (((unsigned long)user_buf_ptr & CACHE_LINE_MASK) == 0) &&
		    ((buf_end & CACHE_LINE_MASK) == CACHE_LINE_MASK) &&
#endif
		    is_pages_phys_contig(client_dma_buf_p->user_pages,
			client_dma_buf_p->num_of_pages)) {
			SEP_LOG_DEBUG("Mapping user buffer @%p (0x%08lX B) to "
				      "DLLI directly\n",
	...
	if ((main_buf_size + tail_buf_size) == 0) {
		...
		calc_aux_bufs_size((unsigned long)user_buf_ptr, buf_size,
			dma_direction, &head_buf_size, &tail_buf_size);
	...
	struct client_dma_buffer *client_dma_buf_p)
{
	...
	unsigned long next_sgl_offset = 0;
	int rc;
	...
	if (buf_size <= DLLI_BUF_LIMIT) {
		if (/* Check if possible to map buffer directly as DLLI */
		    ...
		    (((sgl->offset + buf_size) & CACHE_LINE_MASK) == 0) &&
#endif
		    is_sgl_phys_contig(sgl, &sgl_phys_contig_size)) {
			SEP_LOG_DEBUG("Mapping sgl buffer (0x%08lX B) to "
				      "DLLI directly\n", buf_size);
			main_buf_size = buf_size; /* Leave 0 for tail_buf_size*/
			/* 0 for the tail buffer indicates that we use this
			   optimization, because in any other case there must
			...
			SEP_LOG_DEBUG("Mapping sgl buffer (0x%08lX B) to DLLI "
				      "via aux. buffer\n", buf_size);
			tail_buf_size = buf_size;
			/* All data goes to "tail" in order to piggy-back over
			   the aux. buffers logic for copying data in/out of the
...
#ifdef CONFIG_NOT_COHERENT_CACHE
	rc = clone_sg_list(sgl, 0, head_buf_size, &client_dma_buf_p->sg_head);
	if (unlikely(rc != 0)) {
		SEP_LOG_ERR("Failed creating sg_head\n");
		cleanup_client_dma_buf(llimgr_p, client_dma_buf_p);
		return rc;
	}
	next_sgl_offset = head_buf_size;
#endif
	rc = clone_sg_list(sgl, next_sgl_offset, main_buf_size,
		&client_dma_buf_p->sg_main);
	if (unlikely(rc != 0)) {
		SEP_LOG_ERR("Failed creating sg_main\n");
		cleanup_client_dma_buf(llimgr_p, client_dma_buf_p);
		return rc;
	}
	next_sgl_offset += main_buf_size;
	rc = clone_sg_list(sgl, next_sgl_offset, tail_buf_size,
		&client_dma_buf_p->sg_tail);
	if (unlikely(rc != 0)) {
		SEP_LOG_ERR("Failed creating sg_tail\n");
		cleanup_client_dma_buf(llimgr_p, client_dma_buf_p);
		return rc;
	}
	next_sgl_offset += tail_buf_size;
...
#endif
	client_dma_buf_p->buf_end_aux_buf_size = tail_buf_size;
	/* Create save4next sgl */
	rc = clone_sg_list(sgl, next_sgl_offset,
		client_dma_buf_p->save4next_size,
		&client_dma_buf_p->sg_save4next);
	if (unlikely(rc != 0)) {
		SEP_LOG_ERR("Failed creating sg_save4next\n");
		cleanup_client_dma_buf(llimgr_p, client_dma_buf_p);
		return rc;
	}
	/* Sanity check... */
	next_sgl_offset += client_dma_buf_p->save4next_size;
	if (next_sgl_offset != client_dma_buf_p->buf_size)
		SEP_LOG_ERR("Reached offset %lu for a buffer of %lu B\n",
			next_sgl_offset, client_dma_buf_p->buf_size);
	return 0;
}
 ...
 */
int llimgr_register_client_dma_buf(llimgr_h llimgr,
	...
	struct client_dma_buffer *client_dma_buf_p)
{
	...
	if ((likely(rc == 0)) &&
		...
		client_dma_buf_p->buf_start_aux_buf_va =
			dma_pool_alloc(llimgr_p->edge_bufs_pool,
					GFP_KERNEL,
					&client_dma_buf_p->buf_start_aux_buf_dma);
		if (unlikely(client_dma_buf_p->buf_start_aux_buf_va == NULL)) {
			SEP_LOG_ERR("Failed allocating from edge_bufs_pool for"
				    " head\n");
			rc = -ENOMEM;
		} else {
	...
	if ((likely(rc == 0)) && (client_dma_buf_p->buf_end_aux_buf_size > 0)) {
		...
		if (client_dma_buf_p->buf_end_aux_buf_size <=
		    EDGE_BUFS_POOL_ITEM_SIZE) {
			client_dma_buf_p->buf_end_aux_buf_va = dma_pool_alloc(
				llimgr_p->edge_bufs_pool, GFP_KERNEL,
				&client_dma_buf_p->buf_end_aux_buf_dma);
		} else { /* Allocate from the dedicated DLLI buffers pool */
			client_dma_buf_p->buf_end_aux_buf_va = dma_pool_alloc(
				llimgr_p->dlli_bufs_pool, GFP_KERNEL,
				&client_dma_buf_p->buf_end_aux_buf_dma);
		}
		if (unlikely(client_dma_buf_p->buf_end_aux_buf_va == NULL)) {
			SEP_LOG_ERR("Failed allocating from aux. buf for tail"
				    "\n");
			rc = -ENOMEM;
		} else {
			SEP_LOG_DEBUG("end_aux: va=%p dma=0x%08x\n",
				client_dma_buf_p->buf_end_aux_buf_va,
				client_dma_buf_p->buf_end_aux_buf_dma);
		}
	}
 ...
 */
void llimgr_deregister_client_dma_buf(llimgr_h llimgr,
	struct client_dma_buffer *client_dma_buf_p)
{
	struct llimgr_obj *llimgr_p = (struct llimgr_obj *)llimgr;
...
static inline int copy_to_from_aux_buf(bool to_buf,
	struct scatterlist *sgl, void *buf_p, size_t buf_len)
{
	size_t copied_cnt;
 ...
 */
int llimgr_copy_from_client_buf_save4next(
	struct client_dma_buffer *client_dma_buf_p,
	uint8_t *to_buf, unsigned long buf_len)
{
	int copied_cnt;
 ...
 *
 */
static void dump_mlli_table(uint32_t *table_start_p, dma_addr_t dma_addr,
	unsigned long table_size)
{
	uint32_t lli_spad[SEP_LLI_ENTRY_WORD_SIZE];
	int i;
	uint32_t *cur_entry_p;
	...
	for (i = 0 , cur_entry_p = table_start_p + SEP_LLI_ENTRY_WORD_SIZE;
	     i < num_of_entries;
	     cur_entry_p += SEP_LLI_ENTRY_WORD_SIZE, i++) {
		SEP_LLI_COPY_FROM_SEP(lli_spad, cur_entry_p); /* LE to BE...*/
...
{
	uint32_t lli_spad[SEP_LLI_ENTRY_WORD_SIZE];
	uint32_t *cur_table_p;
	uint32_t *next_table_p;
	uint32_t *link_entry_p;
	...
	/* This loop uses "cur_table_p" as the previous table that
	   was already dumped */
	for (
		cur_table_p = mlli_tables_list_p->link_to_first_table;
		cur_table_p != NULL;
		cur_table_p = next_table_p, table_count++) {

		if (table_count > mlli_tables_list_p->table_count) {
			SEP_LOG_ERR("MLLI tables list has more tables "
				    "than table_cnt=%u. Stopping dump.\n",
...
static inline void set_dlli(struct mlli_tables_list *mlli_tables_list_p,
	uint32_t dlli_addr, uint16_t dlli_size)
{
	uint32_t lli_spad[SEP_LLI_ENTRY_WORD_SIZE]; /* LLI scratchpad */
 ...
 *
 */
static inline void set_last_lli(uint32_t *last_lli_p)
{
	uint32_t lli_spad[SEP_LLI_ENTRY_WORD_SIZE]; /* LLI scratchpad */
 ...
 *
 */
static inline void set_last_table(uint32_t *mlli_table_p)
{
	uint32_t lli_spad[SEP_LLI_ENTRY_WORD_SIZE]; /* LLI scratchpad */
...
static inline void link_to_prev_mlli(
	struct mlli_tables_list_iterator *mlli_iter_p)
{
	uint32_t lli_spad[SEP_LLI_ENTRY_WORD_SIZE]; /* LLI scratchpad */
	const uint32_t cur_mlli_table_size =
	...
	SEP_LLI_SET(lli_spad, LAST, 1);
	SEP_LLI_COPY_TO_SEP(
		mlli_iter_p->prev_mlli_table_p + SEP_LLI_ENTRY_WORD_SIZE,
		lli_spad);
 ...
 *
 */
static inline void terminate_mlli_tables_list(
	struct mlli_tables_list_iterator *mlli_iter_p)
{
	uint32_t *last_lli_p = mlli_iter_p->cur_mlli_table_p +
	...
	struct mlli_tables_list_iterator *mlli_iter_p)
{
	...
	mlli_iter_p->cur_mlli_table_p = dma_pool_alloc(
		llimgr_p->mlli_cache, GFP_KERNEL,
 ...
 * Returns void
 */
static inline void append_lli_to_mlli(
	struct mlli_tables_list_iterator *mlli_iter_p,
	dma_addr_t dma_addr, uint32_t data_size)
{
	uint32_t lli_spad[SEP_LLI_ENTRY_WORD_SIZE]; /* LLI scratchpad */
	uint32_t *next_lli_p;
 ...
 * Returns int 0 for success
 */
static int append_data_to_mlli(
	struct llimgr_obj *llimgr_p,
	struct mlli_tables_list *mlli_tables_list_p,
	struct mlli_tables_list_iterator *mlli_iter_p,
	dma_addr_t data_dma_addr, uint32_t data_size)
{
	uint32_t remaining_data_for_mlli;
	int rc;

#ifdef DEBUG
	if (data_size > llimgr_p->max_data_per_mlli) {
		SEP_LOG_ERR("Given data size (%uB) is too large for MLLI "
	...
	enum dma_data_direction dma_direction)
{
	...
	rc = sync_client_dma_buf(llimgr_p->dev,
		client_memref, true /*for device*/, dma_direction);
	if (likely(rc == 0)) {
	...
	int is_data_dirty)
{
	...
	if (mlli_tables_list_p->table_count > 0) {
		cur_mlli_p =
			SEP_MLLI_GET_NEXT_VA(
				mlli_tables_list_p->link_to_first_table);
		link_entry_p = mlli_tables_list_p->link_to_first_table +
			SEP_LLI_ENTRY_WORD_SIZE;
		SEP_LLI_COPY_FROM_SEP(lli_spad, link_entry_p); /* LE to BE...*/
		...
		cur_mlli_p = NULL; /* Skip the cleanup loop below */
	}

	/* Cleanup MLLI tables */
	while (cur_mlli_p != NULL) {
		SEP_LOG_DEBUG("Freeing MLLI table buffer at %p (%08lX)\n",
			cur_mlli_p, (unsigned long)cur_mlli_dma_addr);
	...
	dma_addr_t prepend_data, unsigned long prepend_data_size)
{
	...
	uint32_t dma_size = memref->buf_size - memref->save4next_size;
	...
	unsigned long prepend_data_size)
{
	...
	const unsigned long client_dma_size =
	...
	SEP_LOG_TRACE("buf @ 0x%08lX  , size=0x%08lX B, "
		      "prepend_size=0x%08lX B, dma_dir=%d\n",
		(unsigned long)client_memref->user_buf_ptr,
		client_memref->buf_size, prepend_data_size, dma_direction);
	...
	if (prepend_data_size > 0) {
		...
		if (unlikely(rc != 0)) {
			SEP_LOG_ERR("Failed adding LLI entry for prepend_data."
				    "\n");
	...
	if (client_memref->buf_start_aux_buf_size > 0) {
		...
		if (unlikely(rc != 0)) {
			SEP_LOG_ERR("Failed adding LLI entry for start_aux_buf."
				    "\n");
			goto mlli_create_exit; /* do cleanup */
		}
	}
	...
	for (cur_sg_entry = client_memref->sg_main,
		...
		if ((remaining_main_data > 0) &&
		    (cur_sge_len >= remaining_main_data) /*last "main" data*/ &&
			...
			rc = append_data_to_mlli(
				llimgr_p, mlli_tables_p, &mlli_iter,
				cur_sge_addr, remaining_main_data);
			if (unlikely(rc != 0)) {
				SEP_LOG_ERR("Failed adding LLI entry "
					    "for sg_main (last).\n");
				goto mlli_create_exit; /* do cleanup */
			}
			...
			rc = alloc_next_mlli(llimgr_p, mlli_tables_p,
				&mlli_iter);
			if (unlikely(rc != 0)) {
				SEP_LOG_ERR("Failed adding MLLI table "
					    "for tail.\n");
				goto mlli_create_exit; /* do cleanup */
			}
		}
		...
		if (likely(cur_sge_len > 0)) {
			...
			rc = append_data_to_mlli(llimgr_p, mlli_tables_p,
				&mlli_iter, cur_sge_addr, cur_sge_len);
			if (unlikely(rc != 0)) {
				SEP_LOG_ERR("Failed adding LLI entry for "
					    "sg_main.\n");
				goto mlli_create_exit; /* do cleanup */
			}
		}
	...
	if (client_memref->buf_end_aux_buf_size > 0) {
		...
		if (unlikely(rc != 0)) {
			SEP_LOG_ERR("Failed adding LLI entry for end_aux_buf."
				    "\n");
			goto mlli_create_exit; /* do cleanup */
		}
	}
 ...
 *
 */
void llimgr_destroy_mlli(llimgr_h llimgr,
	struct mlli_tables_list *mlli_tables_p)
{
	struct llimgr_obj *llimgr_p = (struct llimgr_obj *)llimgr;
	const bool is_dirty =
 ...
 *
 */
void llimgr_mlli_to_seprpc_memref(
	struct mlli_tables_list *mlli_tables_p,
	struct seprpc_memref *memref_p)
{
	...
	memref_p->ref_type = cpu_to_le32(table_count > 0 ?
		SEPRPC_MEMREF_MLLI : SEPRPC_MEMREF_DLLI);
...
void llimgr_get_mlli_desc_info(struct mlli_tables_list *mlli_tables_p,
	uint32_t *first_table_addr_p, uint16_t *first_table_size_p,
	uint16_t *num_of_tables_p)
{
	uint32_t link_lli_spad[SEP_LLI_ENTRY_WORD_SIZE];
	uint32_t *first_mlli_link_p;
