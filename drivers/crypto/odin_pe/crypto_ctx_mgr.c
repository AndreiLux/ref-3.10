...
struct ctxmgr_cache_entry {
	host_crypto_ctx_id_t ctx_id;/* Allocated context ID or CTX_INVALID_ID */
...
static const uint8_t des_key_parity[] = {
...
size_t ctxmgr_get_ctx_size(enum crypto_alg_class alg_class)
{
	...
	case ALG_CLASS_AUTH_ENC: return sizeof(struct host_crypto_ctx_auth_enc);
	case ALG_CLASS_MAC: return sizeof(struct host_crypto_ctx_mac);
	case ALG_CLASS_HASH: return sizeof(struct host_crypto_ctx_hash);
	default:
		return 0;
	}
}
 ...
 * @hash_mode:
 *
 * Returns uint32_t Digest size in octets (0 if unknown hash mode)
 */
static uint32_t get_hash_digest_size(enum sep_hash_mode hash_mode)
{
	switch (hash_mode) {
	case SEP_HASH_SHA1: return (160>>3); /* 160 bit */
	case SEP_HASH_SHA224: return (224>>3);
	case SEP_HASH_SHA256: return (256>>3);
	case SEP_HASH_SHA384: return (384>>3);
	case SEP_HASH_SHA512: return (512>>3);
	default:
...
static uint16_t get_hash_block_size(enum dxdi_hash_type hash_type)
{
...
int ctxmgr_map_user_ctx(struct client_crypto_ctx_info *ctx_info,
	...
	uint32_t __user *user_ctx_ptr)
{
	const unsigned long offset_in_page =
		((unsigned long)user_ctx_ptr & ~PAGE_MASK);
	const unsigned long dist_from_page_end =
	...
	if (alg_class == ALG_CLASS_NONE) {
		...
		if (dist_from_page_end < ctx_size) {
			SEP_LOG_ERR("Given user context that crosses "
				    "a page (0x%p)\n", user_ctx_ptr);
			rc = -EINVAL;
			goto ctx_cross_page;
		}
...
int ctxmgr_map_kernel_ctx(struct client_crypto_ctx_info *ctx_info,
	...
	struct sep_ctx_cache_entry *sep_ctx_p, dma_addr_t sep_ctx_dma_addr)
{
	int rc = 0;
	...
	if ((((unsigned long)sep_ctx_p + sizeof(struct sep_ctx_cache_entry))
		...
		SEP_LOG_ERR("SeP context cross page boundary (start=0x%08lX "
	...
	if (sep_ctx_p == embedded_sep_ctx_p) {
		ctx_info->sep_ctx_dma_addr = dma_map_single(mydev, sep_ctx_p,
			sizeof(struct sep_ctx_cache_entry), DMA_BIDIRECTIONAL);
		if (dma_mapping_error(mydev, ctx_info->sep_ctx_dma_addr)) {
			SEP_LOG_ERR("Mapping sep_ctx for DMA failed");
			rc = -ENOMEM;
 ...
 */
static struct hash_block_remainder *get_blk_rem_buf(
...
int ctxmgr_map2dev_hash_tail(struct client_crypto_ctx_info *ctx_info,
	      struct device *mydev)
{
	struct hash_block_remainder *blk_rem_p =
		get_blk_rem_buf(ctx_info);

#ifdef DEBUG
	if (ctx_info->hash_tail_dma_addr != 0)
		SEP_LOG_DEBUG("Invoked for a buffer that is "
			      "already mapped!\n");
#endif

	if ((blk_rem_p->size > 0) && (ctx_info->hash_tail_dma_addr == 0)) {
		/* Map only if there is any data and it is not already mapped */
		ctx_info->hash_tail_dma_addr = dma_map_single(mydev,
			(void *)blk_rem_p->data,
			blk_rem_p->size, DMA_TO_DEVICE);
		if (dma_mapping_error(mydev,
			ctx_info->hash_tail_dma_addr)) {
			SEP_LOG_ERR("Mapping hash_tail for DMA failed");
			ctx_info->hash_tail_dma_addr = 0;
			return -ENOMEM;
		}
	}
	      ...
	      struct device *mydev)
{
	...
	if ((blk_rem_p->size > 0) && (ctx_info->hash_tail_dma_addr != 0)) {
		dma_unmap_single(mydev, ctx_info->hash_tail_dma_addr,
			blk_rem_p->size, DMA_TO_DEVICE);
		ctx_info->hash_tail_dma_addr = 0; /* Mark as unmapped */
	}
}
 ...
 */
enum host_ctx_state ctxmgr_get_ctx_state(
	const struct client_crypto_ctx_info *ctx_info)
{
 ...
 */
void ctxmgr_set_ctx_id(
	struct client_crypto_ctx_info *ctx_info,
	const host_crypto_ctx_id_t ctx_id)
{
	...
	SEP_LOG_DEBUG("ctx_id=0x%16llX for ctx@0x%p\n",
		ctx_id, (ctx_info->user_ptr == NULL) ?
			(void *)ctx_info->ctx_kptr :
			(void *)ctx_info->user_ptr);
#endif
 ...
 */
host_crypto_ctx_id_t ctxmgr_get_ctx_id(struct client_crypto_ctx_info *ctx_info)
{
 ...
 * Returns uint32_t
 */
uint32_t ctxmgr_get_session_id(struct client_crypto_ctx_info *ctx_info)
{
#ifdef DEBUG
	if (ctx_info->ctx_kptr == NULL) {
		SEP_LOG_ERR("Context not mapped\n"); SEP_DRIVER_BUG();
	...
	return (uint32_t)(ctx_info->ctx_kptr->uid >> 32);
}
 ...
 */
enum crypto_alg_class ctxmgr_get_alg_class(
	const struct client_crypto_ctx_info *ctx_info)
{
 ...
 * @ctx_info:	 User context info structure
 *
 * Returns uint32_t Cypto-block size in bytes, 0 if invalid/unsupported alg.
 */
uint32_t ctxmgr_get_crypto_blk_size(struct client_crypto_ctx_info *ctx_info)
{
	uint32_t cblk_size = 0;
#ifdef DEBUG
	if (ctx_info->ctx_kptr == NULL) {
		SEP_LOG_ERR("Context not mapped\n"); SEP_DRIVER_BUG();
	}
	...
	case ALG_CLASS_SYM_CIPHER: {
		...
		} else if (((cipher_type >= _DXDI_SYMCIPHER_DES_FIRST) &&
			   (cipher_type <= _DXDI_SYMCIPHER_DES_LAST)) ||
			   ((cipher_type >= _DXDI_SYMCIPHER_C2_FIRST) &&
			   (cipher_type <= _DXDI_SYMCIPHER_C2_LAST))) {
			/* DES and C2 have the same block size */
			cblk_size = SEP_DES_BLOCK_SIZE;
		} else {
			SEP_LOG_ERR("Invalid sym.cipher type %d", cipher_type);
		}
	...
	case ALG_CLASS_AUTH_ENC: {
		...
		if (ae_type == DXDI_AUTHENC_AES_CCM)
			...
			SEP_LOG_ERR("Invalid auth.enc. type %d", ae_type);
		break;
	}
	...
	case ALG_CLASS_MAC: {
		struct host_crypto_ctx_mac *ctx_p =
			((struct host_crypto_ctx_mac *)ctx_info->ctx_kptr);
		const enum dxdi_mac_type mac_type = ctx_p->props.mac_type;
		switch (mac_type) { /* switch for block size */
		case DXDI_MAC_HMAC:
			cblk_size = get_hash_block_size(
				ctx_p->props.alg_specific.hmac.hash_type);
			break;
 ...
 * Returns bool
 */
bool ctxmgr_is_valid_adata_size(
	struct client_crypto_ctx_info *ctx_info,
	unsigned long adata_size)
{
	...
	if ((adata_size != host_ctx_p->props.adata_size) ||
	    (adata_size == 0))
		return false;
 ...
 */
bool ctxmgr_is_valid_data_unit_size(
	struct client_crypto_ctx_info *ctx_info,
	...
	bool is_finalize)
{
	...
	case ALG_CLASS_SYM_CIPHER: {
		...
		case DXDI_SYMCIPHER_AES_XTS:
			if (host_ctx_p->props.alg_specific.
				aes_xts.data_unit_size == 0) {
				/* Initialize on first data unit if not
				   provided by the user */
				if (data_unit_size < 32) {
					SEP_LOG_ERR("AES-XTS data unit size "
						    "too small (%lu). "
						    "Must be at least 32B\n",
						data_unit_size);
					return false;
				}
				host_ctx_p->props.alg_specific.
					aes_xts.data_unit_size = data_unit_size;
				aes_ctx_p = (struct sep_ctx_cipher *)
					&(host_ctx_p->sep_ctx);
				aes_ctx_p->data_unit_size =
					cpu_to_le32(host_ctx_p->props.
						alg_specific.
						aes_xts.data_unit_size);
				break;
			} else if (((is_finalize) && (data_unit_size > 0)) ||
				   (!is_finalize)) {
				...
				if (host_ctx_p->props.alg_specific.
					aes_xts.data_unit_size !=
					data_unit_size) {
					SEP_LOG_ERR("Data unit mismatch. "
					    "was %lu. now %lu.\n",
						host_ctx_p->props.alg_specific.
						aes_xts.data_unit_size,
						data_unit_size);
		...
		case DXDI_SYMCIPHER_AES_OFB:
		case DXDI_SYMCIPHER_AES_CBC_CTS:
			if (!is_finalize) { /* !finalize */
				if (!IS_MULT_OF(data_unit_size,
					SEP_AES_BLOCK_SIZE)) {
					SEP_LOG_ERR("Data unit size (%lu) is "
		...
		case DXDI_SYMCIPHER_AES_CBC:
			if (!IS_MULT_OF(data_unit_size, SEP_AES_BLOCK_SIZE)) {
				SEP_LOG_ERR("Data unit size (%lu) is "
					    "not AES block multiple\n",
					data_unit_size);
				return false;
		...
		case DXDI_SYMCIPHER_DES_CBC:
			if (!IS_MULT_OF(data_unit_size, SEP_DES_BLOCK_SIZE)) {
				SEP_LOG_ERR("Data unit size (%lu) is"
					    "not DES block multiple\n",
					data_unit_size);
				return false;
			}
			break;
		...
		case DXDI_SYMCIPHER_C2_CBC:
			if (!IS_MULT_OF(data_unit_size, SEP_C2_BLOCK_SIZE)) {
				SEP_LOG_ERR("Data unit size (%lu) is"
					    "not C2 block multiple\n",
					data_unit_size);
				return false;
			}
			break;
 ...
 */
enum dxdi_sym_cipher_type ctxmgr_get_sym_cipher_type(
	const struct client_crypto_ctx_info *ctx_info)
{
#ifdef DEBUG
	if (ctx_info->ctx_kptr == NULL) {
 ...
 */
enum dxdi_mac_type ctxmgr_get_mac_type(
	const struct client_crypto_ctx_info *ctx_info)
{
	...
	if (mac_ctx->alg_class == ALG_CLASS_MAC)
		return ((struct host_crypto_ctx_mac *)
			ctx_info->ctx_kptr)->props.mac_type;
	else
 ...
 */
enum dxdi_hash_type ctxmgr_get_hash_type(
	const struct client_crypto_ctx_info *ctx_info)
{
#ifdef DEBUG
	if (ctx_info->ctx_kptr == NULL) {
 ...
 */
int ctxmgr_save_hash_blk_remainder(
	struct client_crypto_ctx_info *ctx_info,
	...
	bool append_data)
{
	...
	rc = llimgr_copy_from_client_buf_save4next(client_dma_buf_p,
		blk_rem_p->data + copy_offset, HASH_BLK_SIZE_MAX - copy_offset);
 ...
 * Returns uint16_t Number of valid bytes/octets in tail buffer
 */
uint16_t ctxmgr_get_hash_blk_remainder_buf(
	struct client_crypto_ctx_info *ctx_info,
	dma_addr_t *hash_blk_remainder_dma_p)
{
	struct hash_block_remainder *blk_rem_p =
		get_blk_rem_buf(ctx_info);
 ...
 */
uint32_t ctxmgr_get_digest_or_mac(
	struct client_crypto_ctx_info *ctx_info,
	uint8_t *digest_or_mac)
{
	uint8_t *digest_or_mac_source;
	uint32_t digest_or_mac_size;
	dma_addr_t digest_or_mac_dma_addr;

	digest_or_mac_size = ctxmgr_get_digest_or_mac_ptr(ctx_info,
		&digest_or_mac_source, &digest_or_mac_dma_addr);
	if (digest_or_mac_source != NULL) {
		/* "Memory barrier" for DMA operations from device side */
		/* Assure any updates from the device are visible to the CPU */
		dma_sync_single_for_cpu(ctx_info->dev,
			digest_or_mac_dma_addr, digest_or_mac_size,
			DMA_BIDIRECTIONAL);
		memcpy(digest_or_mac, digest_or_mac_source, digest_or_mac_size);
	}
 ...
 * @digest_or_mac_dma_addr_p:	Returned DMA address of digest/MAC buffer
 *
 * Returns Returned digest/MAC size
 * This function may be used for the caller to reference the result instead
 * of always copying
 */
uint32_t ctxmgr_get_digest_or_mac_ptr(
	struct client_crypto_ctx_info *ctx_info,
	uint8_t **digest_or_mac_pp, dma_addr_t *digest_or_mac_dma_addr_p)
{
	struct sep_ctx_cache_entry *sep_ctx_p;
	enum crypto_alg_class alg_class;
	uint32_t digest_or_mac_size = 0;
	...
	*digest_or_mac_pp = NULL; /* default */
	...
	case SEP_CRYPTO_ALG_AEAD: {
		...
		if (le32_to_cpu(aead_ctx_p->mode) == SEP_CIPHER_CCM) {
			digest_or_mac_size = le32_to_cpu(aead_ctx_p->tag_size);
			*digest_or_mac_pp = aead_ctx_p->mac_state;
		} else {
			SEP_LOG_ERR("Invalid mode (%d) for "
				    "SEP_CRYPTO_ALG_AEAD\n",
	...
	/* DMA address at the same offset of returned pointer */
	*digest_or_mac_dma_addr_p = ctx_info->sep_ctx_dma_addr +
		(*digest_or_mac_pp - ((uint8_t *)sep_ctx_p));
	return digest_or_mac_size;
}
	...
	const enum dxdi_sym_cipher_type cipher_type)
{
	...
	case DXDI_SYMCIPHER_AES_XTS:
		...
		break;
	case DXDI_SYMCIPHER_AES_OFB:
		aes_ctx_p->mode = cpu_to_le32(SEP_CIPHER_OFB);
		break;
	case DXDI_SYMCIPHER_AES_CBC_CTS:
		aes_ctx_p->mode = cpu_to_le32(SEP_CIPHER_CBC_CTS);
		break;
	case DXDI_SYMCIPHER_DES_ECB:
 ...
 * Returns int
 */
int ctxmgr_set_symcipher_iv_user(
	uint32_t __user *user_ctx_ptr,
	uint8_t *iv_ptr)
{
	...
	switch (cipher_type) { /* Update IV in host context */
	case DXDI_SYMCIPHER_AES_CBC:
		copy_rc = copy_to_user(
			host_ctx_p->props.alg_specific.aes_cbc.iv, iv_ptr,
			SEP_AES_IV_SIZE);
		break;
	case DXDI_SYMCIPHER_AES_CTR:
		copy_rc = copy_to_user(
			host_ctx_p->props.alg_specific.aes_ctr.cntr, iv_ptr,
			SEP_AES_IV_SIZE);
		break;
	case DXDI_SYMCIPHER_AES_XTS:
		copy_rc = copy_to_user(
			host_ctx_p->props.alg_specific.aes_xts.init_tweak,
			iv_ptr, SEP_AES_IV_SIZE);
		break;
	case DXDI_SYMCIPHER_AES_OFB:
		copy_rc = copy_to_user(
			host_ctx_p->props.alg_specific.aes_ofb.cntr, iv_ptr,
			SEP_AES_IV_SIZE);
		break;
	case DXDI_SYMCIPHER_AES_CBC_CTS:
		copy_rc = copy_to_user(
			host_ctx_p->props.alg_specific.aes_cts.iv, iv_ptr,
			SEP_AES_IV_SIZE);
		break;
	default:
		SEP_LOG_ERR("Cipher type %d does not support setting IV\n",
			cipher_type);
		return -EINVAL;
	}
	if (copy_rc != 0) {
		SEP_LOG_ERR("Failed putting user host IV)");
		return -EFAULT;
	}

	/* Update IV in SeP context */
	if (copy_to_user(aes_ctx_p->block_state, iv_ptr, SEP_AES_IV_SIZE)) {
		SEP_LOG_ERR("Failed putting user sep block_state)");
		return -EFAULT;
	}
 ...
 * Returns int
 */
int ctxmgr_get_symcipher_iv_user(uint32_t __user *user_ctx_ptr, uint8_t *iv_ptr)
{
	struct host_crypto_ctx_sym_cipher __user *host_ctx_p =
		(struct host_crypto_ctx_sym_cipher __user *)user_ctx_ptr;
	struct sep_ctx_cipher *aes_ctx_p =
	...
	if (copy_from_user(&cipher_type, &host_ctx_p->props.cipher_type,
			   sizeof(enum dxdi_sym_cipher_type))) {
		SEP_LOG_ERR("Failed fetching user cipher_type");
		return -EFAULT;
	}
	...
	if ((cipher_type != DXDI_SYMCIPHER_AES_CBC) &&
	    ...
	    (cipher_type != DXDI_SYMCIPHER_AES_CBC_CTS) &&
	    (cipher_type != DXDI_SYMCIPHER_AES_OFB)) {
		return -EINVAL;
	}

	if (copy_from_user(iv_ptr, aes_ctx_p->block_state, SEP_AES_IV_SIZE)) {
		SEP_LOG_ERR("Failed fetching user block_state)");
		return -EFAULT;
	}
...
static void get_symcipher_iv_info(struct client_crypto_ctx_info *ctx_info,
	uint8_t **host_ctx_iv, uint8_t **sep_ctx_iv, unsigned long *iv_size)
{
	struct host_crypto_ctx_sym_cipher *host_ctx_p =
		(struct host_crypto_ctx_sym_cipher *)ctx_info->ctx_kptr;
	...
	case DXDI_SYMCIPHER_AES_XTS:
		*host_ctx_iv =
		...
		break;
	case DXDI_SYMCIPHER_AES_OFB:
		*host_ctx_iv = host_ctx_p->props.alg_specific.aes_ofb.cntr;
		break;
	case DXDI_SYMCIPHER_AES_CBC_CTS:
		*host_ctx_iv = host_ctx_p->props.alg_specific.aes_cts.iv;
		break;
	case DXDI_SYMCIPHER_DES_CBC:
...
int ctxmgr_get_symcipher_iv(struct client_crypto_ctx_info *ctx_info,
	uint8_t *iv_user, uint8_t *iv_current, uint8_t *iv_size_p)
{
	uint8_t *host_ctx_iv;
	uint8_t *sep_ctx_iv;
	unsigned long iv_size;
 ...
 * Returns int 0 on success
 */
int ctxmgr_set_symcipher_direction(
	struct client_crypto_ctx_info *ctx_info,
 ...
 */
enum dxdi_cipher_direction ctxmgr_get_symcipher_direction(
	struct client_crypto_ctx_info *ctx_info)
{
	struct host_crypto_ctx_sym_cipher *host_ctx_p =
		(struct host_crypto_ctx_sym_cipher *)ctx_info->ctx_kptr;
...
{
	u32 n, w;
	uint64_t *k1, *k2, *k3; /* For 3DES/2DES checks */
	...
	n  = des_key_parity[key[0]]; n <<= 4;
	n |= des_key_parity[key[1]]; n <<= 4;
	n |= des_key_parity[key[2]]; n <<= 4;
	n |= des_key_parity[key[3]]; n <<= 4;
	n |= des_key_parity[key[4]]; n <<= 4;
	n |= des_key_parity[key[5]]; n <<= 4;
	n |= des_key_parity[key[6]]; n <<= 4;
...
int ctxmgr_set_symcipher_key(struct client_crypto_ctx_info *ctx_info,
	uint8_t key_size, const uint8_t *key)
{
	struct sep_ctx_cipher *aes_ctx_p = NULL;
	struct sep_ctx_cipher *des_ctx_p = NULL;
	struct sep_ctx_c2 *c2_ctx_p = NULL;
	...
	case DXDI_SYMCIPHER_AES_CBC_CTS:
		...
		if (props->cipher_type == DXDI_SYMCIPHER_AES_XTS) {
			...
			if ((key_size != 32) &&
			    (key_size != 64)) {
				SEP_LOG_ERR("Invalid key size for AES-XTS "
					    "(%u bits)\n", key_size * 8);
				return -EINVAL;
			}
			...
			memcpy(aes_ctx_p->xex_key, key + key_size,
				key_size);
			...
			if ((key_size != 16) &&
			    (key_size != 24) &&
			    (key_size != 32)) {
				SEP_LOG_ERR("Invalid key size for AES "
					    "(%u bits)\n", key_size * 8);
				return -EINVAL;
			}
		}
 ...
 * Returns int 0 on success
 */
int ctxmgr_init_symcipher_ctx_no_props(
	struct client_crypto_ctx_info *ctx_info,
	enum dxdi_sym_cipher_type cipher_type)
{
	struct host_crypto_ctx_sym_cipher *host_ctx_p =
	...
	rc = set_sep_ctx_alg_mode(ctx_info, cipher_type);
	...
	*error_info = DXDI_ERROR_NULL; /* assume no error */
	...
	case DXDI_SYMCIPHER_AES_XTS:
		...
		memcpy(aes_ctx_p->block_state,
			props->alg_specific.aes_xts.init_tweak,
			SEP_AES_IV_SIZE);
		aes_ctx_p->data_unit_size = cpu_to_le32(
	...
	case DXDI_SYMCIPHER_AES_OFB:
		aes_ctx_p = (struct sep_ctx_cipher *)ctx_info->sep_ctx_kptr;
		memcpy(aes_ctx_p->block_state, props->alg_specific.aes_ofb.cntr,
			SEP_AES_IV_SIZE);
		break;
	case DXDI_SYMCIPHER_AES_CBC_CTS:
		aes_ctx_p = (struct sep_ctx_cipher *)ctx_info->sep_ctx_kptr;
		memcpy(aes_ctx_p->block_state, props->alg_specific.aes_cts.iv,
			SEP_AES_IV_SIZE);
		break;
	case DXDI_SYMCIPHER_DES_CBC:
		des_ctx_p = (struct sep_ctx_cipher *)ctx_info->sep_ctx_kptr;
	...
	case DXDI_SYMCIPHER_C2_CBC:
		c2_ctx_p = (struct sep_ctx_c2 *)ctx_info->sep_ctx_kptr;
#if 0 /*C2 reset interval is not supported, yet */
		c2_ctx_p->reset_interval =
			cpu_to_le32(props->alg_specific.c2_cbc.reset_interval);
		break;
#else
	...
	uint32_t *error_info)
{
	...
	*error_info = DXDI_ERROR_NULL;
	...
	memcpy(&(host_ctx_p->props), props,
		sizeof(struct dxdi_auth_enc_props));
	host_ctx_p->is_encrypted = false;
	...
	uint32_t *error_info)
{
	...
	*error_info = DXDI_ERROR_NULL;
	...
	case DXDI_MAC_HMAC:
		...
		if (get_hash_block_size(hash_type) >
		    SEP_HMAC_BLOCK_SIZE_MAX) {
			SEP_LOG_ERR("Given hash type (%d) is not supported for "
				    "HMAC\n", hash_type);
			*error_info = DXDI_ERROR_UNSUP;
			return -EINVAL;
		}
...
static void dump_sep_aes_ctx(struct sep_ctx_cipher *ctx_p)
{
	...
	dump_byte_array("Key",
			ctx_p->key, le32_to_cpu(ctx_p->key_size));
...
{
	SEP_LOG_DEBUG("Alg.=AEAD , Mode=%d , Direction=%d , Key size=%d , "
		      "header size=%d , nonce size=%d , tag size=%d , "
		      "text size=%d\n",
		      le32_to_cpu(ctx_p->mode),
		      le32_to_cpu(ctx_p->direction),
		      le32_to_cpu(ctx_p->key_size),
	...
	dump_byte_array("nonce",
			ctx_p->nonce, le32_to_cpu(ctx_p->nonce_size));
}
...
{
	SEP_LOG_DEBUG("Alg.=C2 , Mode=%d , Direction=%d , Key size=%d , "
		      "ResetInt.=%d",
		      le32_to_cpu(ctx_p->mode),
		      le32_to_cpu(ctx_p->direction),
		      le32_to_cpu(ctx_p->key_size),
...
{
	switch (mode) {
	case SEP_HASH_SHA1:   return "SHA1";
	case SEP_HASH_SHA224: return "SHA224";
	case SEP_HASH_SHA256: return "SHA256";
	case SEP_HASH_SHA384: return "SHA384";
	case SEP_HASH_SHA512: return "SHA512";
...
static void dump_sep_hmac_ctx(struct sep_ctx_hmac *ctx_p)
{
	...
	dump_byte_array("K0",
			ctx_p->k0, le32_to_cpu(ctx_p->k0_size));
}
 ...
 */
dma_addr_t ctxmgr_get_sep_ctx_dma_addr(
	const struct client_crypto_ctx_info *ctx_info)
{
#ifdef DEBUG
	if (ctx_info->sep_ctx_dma_addr == 0) {
 ...
 *
 * Returns sep_ctx_cache_t handle (NULL on failure)
 */
sep_ctx_cache_t ctxmgr_sep_cache_create(int num_of_entries)
{
	...
	new_cache = kmalloc(sizeof(struct sep_ctx_cache) +
			    (num_of_entries - 1) *
			    sizeof(struct ctxmgr_cache_entry),
			    GFP_KERNEL);
	if (new_cache == NULL) {
	...
	return (sep_ctx_cache_t)new_cache;
}
 ...
 * Returns void
 */
void ctxmgr_sep_cache_destroy(sep_ctx_cache_t sep_cache)
{
 ...
 */
int ctxmgr_sep_cache_get_size(sep_ctx_cache_t sep_cache)
{
	struct sep_ctx_cache *this_cache = (struct sep_ctx_cache *)sep_cache ;
 ...
 * Returns cache index
 */
int ctxmgr_sep_cache_alloc(sep_ctx_cache_t sep_cache,
	host_crypto_ctx_id_t ctx_id,
	int *load_required_p)
{
	struct sep_ctx_cache *this_cache = (struct sep_ctx_cache *)sep_cache ;
	int i;
	...
	*load_required_p = 1; /* until found assumre a load is required */
 ...
 * Returns void
 */
void ctxmgr_sep_cache_invalidate(sep_ctx_cache_t sep_cache,
	host_crypto_ctx_id_t ctx_id, host_crypto_ctx_id_t id_mask)
{
	struct sep_ctx_cache *this_cache = (struct sep_ctx_cache *)sep_cache ;
	int i;
