...
/* Opaque handle for descriptor queue object */
typedef void *desc_q_t;
...
enum desc_q_state {
	...
	DESC_Q_INACTIVE,	/* Queue was initialized but not activated */
	DESC_Q_ACTIVE,		/* Queue is ready to accept requests    */
	DESC_Q_ASLEEP,		/* Queue is in "sleep" state (cannot accept
				   additional requests - descs may be
				   enqueued in descs backlog queue) */
	DESC_Q_INACTIVE_ASLEEP  /* Denote when moved to sleep state before
				   a queue was activated. This cause "wakeup"
				   to return to INACTIVE state. The queue cannot
				   be activated from this state. */
};
 ...
 */
desc_q_t desc_q_create(int qid, struct queue_drvdata *drvdata);
