...
static void release_symcipher_ctx(
	struct sep_op_ctx *op_ctx, uint8_t *iv_crypto);
...
{ /* ofb(aes) */
	.cra_name = "ofb(aes)",
	.cra_driver_name = MODULE_NAME "-aes-ofb",
	.cra_blocksize		= SEP_AES_BLOCK_SIZE,
	.cra_u			= {
		.ablkcipher = {
			.min_keysize	= SEP_AES_128_BIT_KEY_SIZE,
...
{ /* cts(cbc(aes)) */
	...
	.cra_driver_name = MODULE_NAME "-aes-cts",
	.cra_blocksize		= SEP_AES_BLOCK_SIZE,
	.cra_u			= {
		.ablkcipher = {
			.min_keysize	= SEP_AES_128_BIT_KEY_SIZE,
...
static const enum dxdi_sym_cipher_type dx_algs_cipher_types[] = {
	...
	DXDI_SYMCIPHER_AES_OFB,
	DXDI_SYMCIPHER_AES_CBC_CTS,
...
static void crypto_ctx_cleanup(struct crypto_tfm *tfm)
{
	...
	rc = ctxmgr_map_kernel_ctx(ctx_info, mydev, host_ctx_p->alg_class,
		(struct host_crypto_ctx *)host_ctx_p, NULL, 0);
	if (rc != 0) {
...
static int dispatch_crypto_op(struct sep_op_ctx *op_ctx, bool may_backlog,
	bool do_init, enum sep_proc_mode proc_mode, bool keep_in_cache)
{
	...
	host_crypto_ctx_id_t ctx_id = ctxmgr_get_ctx_id(ctx_info);
	int rc = 0;
	...
	ctxmgr_set_sep_cache_idx(ctx_info,
		ctxmgr_sep_cache_alloc(drvdata->sep_cache,
			ctx_id, &sep_ctx_load_req));
	desc_q_pack_crypto_op_desc(&desc, op_ctx,
		sep_ctx_load_req, do_init, proc_mode);
...
/**
 * process_digest_completion() - Process completion event for hash operation
 *
 * @digest_req:	The async. digest request context
 *
 */
static void process_digest_completion(struct async_digest_req_ctx *digest_req)
{
	uint8_t digest_size;
	uint8_t *digest_ptr;
	...
	if (op_ctx->op_type == SEP_OP_CRYPTO_FINI) {
		...
		digest_size = ctxmgr_get_digest_or_mac_ptr(
			&op_ctx->ctx_info, &digest_ptr, &digest_dma_addr);
		if (unlikely(digest_ptr == NULL)) {
			SEP_LOG_ERR("Failed fetching digest/MAC\n");
			return;
		}
		if (digest_req->result != NULL) {
			/* Must sync DMA buffer to CPU before copying result */
			dma_sync_single_for_cpu(op_ctx->ctx_info.dev,
				digest_dma_addr, digest_size,
				DMA_BIDIRECTIONAL);
...
#ifdef DEBUG
		/* Must sync DMA buffer to CPU before dumping result */
		/* (for the "else" case in the "if" above)           */
		dma_sync_single_for_cpu(op_ctx->ctx_info.dev,
			digest_dma_addr, digest_size, DMA_BIDIRECTIONAL);
		dump_byte_array("digest", digest_req->result, digest_size);
		ahash_tfm = crypto_ahash_reqtfm(
			container_of(digest_req->async_req.initiating_req,
...
static void dx_crypto_api_handle_op_completion(struct work_struct *work)
{
	...
	int err;
	uint8_t *req_info_p; /* For state persistency in caller's context (IV)*/
	...
	if (op_ctx->op_state == USER_OP_COMPLETED) {
		...
		case CRYPTO_ALG_TYPE_ABLKCIPHER:
			...
			/* In case of CTS cipher (operation type is always
			   SEP_OP_CRYPTO_FINI) we want to initialize context for
			   next operation */
			if (op_ctx->op_type == SEP_OP_CRYPTO_FINI)
				symcipher_ctx_init(initiating_req->tfm);
			break;
		case CRYPTO_ALG_TYPE_AHASH:
			process_digest_completion(container_of(areq_ctx,
				struct async_digest_req_ctx, async_req));
			break;
		default:
 ...
 */
static enum dxdi_sym_cipher_type get_symcipher_tfm_cipher_type(
...
static int prepare_symcipher_ctx_for_processing(struct sep_op_ctx *op_ctx,
	struct host_crypto_ctx_sym_cipher *host_ctx_p,
	uint8_t *iv_crypto, enum dxdi_cipher_direction direction)
{
	struct client_crypto_ctx_info *ctx_info = &op_ctx->ctx_info;
	struct device *mydev = crypto_api_ctx.drv_data->sep_data->dev;
	...
	rc = ctxmgr_map_kernel_ctx(ctx_info, mydev,
		ALG_CLASS_SYM_CIPHER, (struct host_crypto_ctx *)host_ctx_p,
 ...
 */
static void release_symcipher_ctx(struct sep_op_ctx *op_ctx, uint8_t *iv_crypto)
{
	struct client_crypto_ctx_info *ctx_info = &op_ctx->ctx_info;
	uint8_t iv_sep_ctx[DXDI_AES_BLOCK_SIZE];
	uint8_t iv_size = DXDI_AES_BLOCK_SIZE; /* Init. to max. */
		...
		if (likely(rc == 0))  {
			if (iv_size > 0) {
				if (unlikely(iv_crypto == NULL)) {
					SEP_LOG_ERR("iv_crypto==NULL when "
						    "iv_size==%u\n", iv_size);
				} else
					memcpy(iv_crypto, iv_sep_ctx, iv_size);
			}
		} else {
			SEP_LOG_ERR("Failed getting IV information for "
				    "ctx@%p\n", ctx_info->ctx_kptr);
		}
	}
	...
	enum dxdi_cipher_direction direction)
{
	...
	/*In case of CTS operation type is always finish */
	if (host_ctx_p->props.cipher_type == DXDI_SYMCIPHER_AES_CBC_CTS)
	...
	rc = prepare_symcipher_ctx_for_processing(
		op_ctx, host_ctx_p, req->info, direction);
	if (unlikely(rc != 0)) {
		op_ctx_fini(op_ctx);
		return rc;
	}
	rc = prepare_data_for_sep(op_ctx, NULL, req->src, NULL, req->dst,
		req->nbytes, op_ctx->op_type = SEP_OP_CRYPTO_FINI ?
		CRYPTO_DATA_TEXT_FINALIZE : CRYPTO_DATA_TEXT);

	if (unlikely(rc != 0)) {
		SEP_LOG_ERR("Failed preparing DMA buffers (rc=%d, "
		...
		rc = dispatch_crypto_op(op_ctx,
			...
			true/*init.*/, op_ctx->op_type = SEP_OP_CRYPTO_FINI ?
			SEP_PROC_MODE_FIN : SEP_PROC_MODE_PROC_T,
			false/*cache*/);
	}
...
static int ablkcipher_algs_init(void)
{
	...
	for (i = 0, rc = 0; i < DX_ABLKCIPHER_NUM; i++) {
		...
		alg_spad.cra_blocksize =
			dx_ablkcipher_algs[i].cra_blocksize;
	...
	if (rc != 0) { /* Failure: cleanup algorithms that already registered */
 ...
 */
static int prepare_digest_context_for_processing(
	struct ahash_request *req, bool do_init)
{
	...
	if (!do_init) {
		...
		if (req_ctx->async_req.initiating_req == NULL) {
			SEP_LOG_ERR("Invoked for uninitialized async. req."
				    "context\n");
			return -EINVAL;
		}
	...
	rc = ctxmgr_map_kernel_ctx(ctx_info, mydev,
		...
		(struct host_crypto_ctx *)&req_ctx->host_ctx, NULL, 0);
	if (rc != 0) {
		SEP_LOG_ERR("Failed mapping context (rc=%d)\n", rc);
		return rc;
	}
	if (do_init) {
		...
		if (mac_type == DXDI_MAC_NONE) { /* Hash alg. */
			rc = ctxmgr_init_hash_ctx(ctx_info,
				digest_alg->hash_type, &error_info);
		} else { /* MAC */
	...
	struct scatterlist *src, unsigned int nbytes)
{
	...
	if ((!do_init) && (req_ctx->result != NULL)) {
		...
		if (unlikely(nbytes > 0)) {
			SEP_LOG_ERR("Invoked with %u B after "
				    "already finalized\n", nbytes);
			return -EINVAL;
		}
		if (is_last) {
	...
	if (op_ctx->op_type != SEP_OP_CRYPTO_INIT) {
		rc = prepare_data_for_sep(op_ctx, NULL, src, NULL, NULL, nbytes,
			is_last ? CRYPTO_DATA_TEXT_FINALIZE : CRYPTO_DATA_TEXT);
		if (rc == -ENOTBLK) {
	...
	rc = dispatch_crypto_op(op_ctx,
		req->base.flags & CRYPTO_TFM_REQ_MAY_BACKLOG, do_init,
		is_last ? SEP_PROC_MODE_FIN :
			do_init ? SEP_PROC_MODE_NOP : SEP_PROC_MODE_PROC_T,
		true/*cache*/);
	if (unlikely(IS_DESCQ_ENQUEUE_ERR(rc))) {
	...
	u8 *digest, unsigned int *digest_len_p)
{
	...
	op_ctx_init(&op_ctx, &crypto_api_ctx);
	rc = ctxmgr_map_kernel_ctx(
		ctx_info_p, drvdata->sep_data->dev, ALG_CLASS_HASH,
		(struct host_crypto_ctx *)&host_ctx, NULL, 0);
	if (rc != 0) {
	...
	if (likely(!IS_DESCQ_ENQUEUE_ERR(rc))) {
		...
		if (likely(op_ctx.error_info == 0)) {
			*digest_len_p = ctxmgr_get_digest_or_mac(
				ctx_info_p, digest);
		}
	}
	crypto_op_completion_cleanup(&op_ctx);
	...
	const u8 *key, unsigned int keylen)
{
	...
	if ((digest_alg->hash_type != DXDI_HASH_NONE) &&
		...
		if (key_data->key_size != digest_alg->ahash.halg.digestsize)
			SEP_LOG_ERR("Returned digest size is %u != %u "
				    "(expected)\n",
				key_data->key_size,
...
int dx_crypto_api_init(struct sep_drvdata *drvdata)
{
	...
	sep_ctx_pool = dma_pool_create("dx_sep_ctx",
		...
		sizeof(struct sep_ctx_cache_entry), L1_CACHE_BYTES, 0);
