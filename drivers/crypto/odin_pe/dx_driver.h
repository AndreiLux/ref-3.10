...
/* if "CC_INTERRUPT_BY_TIMER" is defined to !0 then interrupt handler will be
 * scheduled by host timer and NOT by SEP interrupt controller. Use this
 * option for debugging only */
/*#define CC_INTERRUPT_BY_TIMER 1 */
...
/* Maximum ID for CC interrupt cause (bit offset in cause register) */
#define CC_CAUSE_ID_MAX 31

struct sep_drvdata;

/**
 * Callback function for interrupt handlers of specific interrupt causes
 */
typedef void (*cc_interrupt_handler)(
	struct sep_drvdata *drvdata, int cause_id, void *priv_context);

/**
 * struct cc_interrupt_dispatch_entry - Entry in interrupt handlers dispatch
 *	table.
 * @handler:	The function pointer of the handler
 * @priv_context:	The private context of the handler
 */
struct cc_interrupt_dispatch_entry {
	cc_interrupt_handler handler;
	void *priv_context;
};
 ...
 * @irq_handlers:	Handlers for registered interrupts.
 ...
 * @ipsec_q_size_log2:	Log2 number of entries in each IPsec queue to SEP
 * @ipsec_qs:		Handle to object of IPsec queues pair
...
struct sep_drvdata {
	...
	unsigned int irq;
	uint32_t irq_mask;
	struct cc_interrupt_dispatch_entry irq_handlers[CC_CAUSE_ID_MAX + 1];
	uint32_t rom_ver;
	uint32_t fw_ver;
#ifdef CACHE_IMAGE_NAME
	uint8_t icache_size_log2;
	uint8_t dcache_size_log2;
...
#ifdef PE_IPSEC_QUEUES
	uint8_t ipsec_q_size_log2;
	ipsec_qs_h ipsec_qs;
	uint32_t rnd_seed_size;
	uint32_t rnd_seed[DXDI_RND_SEED_SIZE_IN_WORD];
#endif
...
struct sep_client_ctx {
	...
	struct sep_app_session sepapp_sessions[
...
enum sep_op_type {
	...
	SEP_OP_CRYPTO_FINI = (1<<2),/* CRYPTO_OP::Finalize (incl. integrated) */
 ...
 *		- Next IV for AES-CBC, AES-CTR, AES-OFB, DES-CBC
...
struct sep_op_ctx {
	...
	uint8_t ctx_info_num;
	uint8_t pending_descs_cntr;
	uint8_t backlog_descs_cntr;
...
#define READ_REGISTER(_addr) _READ_REGISTER(_addr)
 ...
 *
 */
static inline host_crypto_ctx_id_t alloc_crypto_ctx_id(
	struct sep_client_ctx *client_ctx)
{
...
#endif
	return (((uint64_t)(unsigned long)client_ctx) <<
			CRYPTO_CTX_ID_CLIENT_SHIFT) |
		(uint64_t)atomic_inc_return(&client_ctx->uid_cntr);
}
	...
	struct sep_client_ctx *client_ctx)
{
	...
	for (i = 0; i < DXDI_COMBINED_NODES_MAX; i++)
		USER_CTX_INFO_INIT(&ctx_info_p[i]);
}
...
static inline void op_ctx_fini(struct sep_op_ctx *op_ctx)
{
	...
	if (op_ctx->spad_buf_p != NULL) {
		dma_pool_free(
			op_ctx->client_ctx->drv_data->sep_data->spad_buf_pool,
			op_ctx->spad_buf_p, op_ctx->spad_buf_dma_addr);
	}
	memset(op_ctx, 0, sizeof(struct sep_op_ctx));
}
