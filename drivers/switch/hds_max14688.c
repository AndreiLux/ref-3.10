...
static struct max14688_jack_match max14688_jack_matches[] = {
    ...
    JACK_MATCH("3P",     Z(0,        50),  Z(0,   4),   false,  LOW,   LOW,   SWITCH_NAME,          LGE_HEADSET_NO_MIC, EV_SW, SW_HEADPHONE_INSERT, NONE),
    JACK_MATCH("4P",     Z(50, 2600),  Z(0,   4),   true,   HIGH,  LOW,   SWITCH_NAME,          LGE_HEADSET,        EV_SW, SW_HEADPHONE_INSERT, SW_MICROPHONE_INSERT),
#if MAX14688_ADVANCED_JACK_DET
    JACK_MATCH("3P/ADV", Z(0,        50),  Z(4,  11),   false,  LOW,   LOW,   SWITCH_NAME_ADVANCED, LGE_HEADSET_NO_MIC, EV_SW, SW_ADVANCED_HEADPHONE_INSERT, NONE),
    JACK_MATCH("4P/ADV", Z(50, 2600),  Z(4,  11),   true,   HIGH,  LOW,   SWITCH_NAME_ADVANCED, LGE_HEADSET,        EV_SW, SW_ADVANCED_HEADPHONE_INSERT, SW_MICROPHONE_INSERT),
    JACK_MATCH("ACC/AUX",Z(0,        50),  Z(11, 64),   false,  LOW,   LOW,   SWITCH_NAME_AUX,      LGE_HEADSET_NO_MIC, EV_SW, SW_AUX_ACCESSORY_INSERT, NONE),
    JACK_MATCH("ACC/AUX",Z(50, 2600),  Z(11, 64),   true,   HIGH,  LOW,   SWITCH_NAME_AUX,      LGE_HEADSET,        EV_SW, SW_AUX_ACCESSORY_INSERT, SW_MICROPHONE_INSERT),
#else
#endif
};
...
static struct max14688_button_match max14688_button_matches[] = {
    ...
    BUTTON_MATCH("VOLUP",  Z(150, 300), DONTCARE,    EV_KEY, KEY_VOLUMEUP),
    BUTTON_MATCH("VOLDN",  Z(300, 650), DONTCARE,    EV_KEY, KEY_VOLUMEDOWN),
};
...
#ifdef MAX14688_ADVANCED_JACK_DET
	struct switch_dev		sdev_adv;
	struct switch_dev		sdev_aux;
#endif
    struct mutex            lock;
    struct device          *dev;
    struct kobject         *kobj;
...
{
	earjack_detect = false;
    /* kill off pending det_work */
    cancel_delayed_work(&me->det_work);
...
{
    unsigned long det_work_delay = msecs_to_jiffies(500);// 300 -> 500
...
static void max14688_check_suspended_worker(struct work_struct *work)
{
    ...
    {
        log_vdbg("max14688 resume. me->suspended:%d\n", me->suspended);
...
*/
	rc = of_property_read_u32(np, "max14688,gpio_detect", &pdata->gpio_detect);
	rc = of_property_read_u32(np, "max14688,gpio_int", &pdata->gpio_int);
    ...
    const struct i2c_device_id *id)
{
...
#ifdef MAX14688_ADVANCED_JACK_DET
    me->sdev_adv.name = pdata->switch_name_advanced;
    rc = switch_dev_register(&me->sdev_adv);
    if (rc < 0) {
	 log_err("Failed to register switch device\n");
	 switch_dev_unregister(&me->sdev_adv);
        goto abort;
    }

    me->sdev_aux.name = pdata->switch_name_aux;
    rc = switch_dev_register(&me->sdev_aux);
    if (rc < 0) {
	 log_err("Failed to register switch device\n");
	 switch_dev_unregister(&me->sdev_aux);
        goto abort;
    }
#endif
    ...
    if (unlikely(me->irq < 0)) {
		...
		if ( me->gpio_int == 113 ) {    //112+SMS_GPIO_1(GIC)
			rc = odin_gpio_sms_request_threaded_irq(me->irq, NULL, max14688_isr,
...
{
    struct max14688 *me = dev_get_drvdata(dev);
    bool may_wakeup = device_may_wakeup(dev);
    int rc;
    ...
    if (unlikely(delayed_work_pending(&me->irq_work) ||
                 delayed_work_pending(&me->det_work))) {
        /* Device should still be 'active' */
        log_info("Deferring this suspending request "
                 "for detection working on\n");
        rc = -EBUSY;
        goto out;
    }
    rc = 0;
...
static void max14688_init_async(void *data, async_cookie_t cookie)
{
	int ret = 0;


	ret = i2c_add_driver(&max14688_i2c_driver);
	if( ret < 0 )
		pr_err("%s:failed to register max14688 i2c driven", __func__);
}

static int __init max14688_init(void)
{
	async_schedule(max14688_init_async, NULL);
	return 0;
}

static void __exit max14688_exit(void)
{
	i2c_del_driver(&max14688_i2c_driver);
}
//module_i2c_driver(max14688_i2c_driver);

late_initcall_sync(max14688_init);
//module_init(max14688_init_async);
module_exit(max14688_exit);
