...
static int com_to_audio(struct max77804k_muic_data *muic_data)
{
	max77804k_reg_ctrl1_t reg_val;
	int ret;
...
#if defined(CONFIG_MUIC_MAX77804K_SUPPROT_AUDIO_FACTORY)
...
#endif /* CONFIG_MUIC_MAX77804K_SUPPROT_AUDIO_FACTORY */
...
#endif /* CONFIG_MUIC_MAX77804K_SUPPORT_AUDIO_FACTORY */
...
static int max77804k_muic_enable_cpen(struct max77804k_muic_data *muic_data)
{
	return max77804k_muic_update_reg(muic_data->i2c, MAX77804_MUIC_REG_CTRL2,
				(MAX77804K_ENABLE_BIT << CTRL2_CPEN_SHIFT),
				CTRL2_CPEN_MASK, true);
}

static int max77804k_muic_disable_cpen(struct max77804k_muic_data *muic_data)
{
	return max77804k_muic_update_reg(muic_data->i2c, MAX77804_MUIC_REG_CTRL2,
				(MAX77804K_DISABLE_BIT << CTRL2_CPEN_SHIFT),
				CTRL2_CPEN_MASK, true);
}

static int max77804k_muic_attach_audio_path(struct max77804k_muic_data *muic_data)
{
	int ret;
	...
	ret = max77804k_muic_enable_cpen(muic_data);
	if (ret)
		goto err_cpen;
...
static int max77804k_muic_handle_detach(struct max77804k_muic_data *muic_data)
{
	...
	max77804k_muic_disable_cpen(muic_data);
					...
					muic_attached_dev_t new_dev)
{
	...
	case ATTACHED_DEV_DESKDOCK_MUIC:
		if (muic_data->attached_dev == ATTACHED_DEV_DESKDOCK_VB_MUIC)
			...
			ret = max77804k_muic_attach_audio_path(muic_data);
		break;
	case ATTACHED_DEV_DESKDOCK_VB_MUIC:
