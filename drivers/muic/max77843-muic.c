					   ...
					   char *buf)
{
	...
	pr_debug("%s:%s %s", MUIC_DEV_NAME, __func__, pdata->hv_sel ? "5V":"9V");
					  ...
					  const char *buf, size_t count)
{
	...
	if (!strncasecmp(buf, "5V", 2)) {
		...
		case ATTACHED_DEV_AFC_CHARGER_9V_MUIC:
			pr_debug("%s:%s reset muic to change AFC 9V => TA 5V case 1 attached_dev[%d]\n", MUIC_DEV_NAME, __func__, muic_data->attached_dev);
			...
			while((muic_data->attached_dev != ATTACHED_DEV_AFC_CHARGER_5V_MUIC)&&(repeat_cnt--!=0)) {
				msleep(10);
//				pr_debug("%s:%s wait until attached device changing attached_dev[%d]\n", MUIC_DEV_NAME, __func__, muic_data->attached_dev);
			}
			pr_debug("%s:%s change done attached_dev[%d] waited[%d]ms \n", MUIC_DEV_NAME, __func__, muic_data->attached_dev, (30-repeat_cnt)*10);


			break;
		case ATTACHED_DEV_QC_CHARGER_9V_MUIC:
			pr_debug("%s:%s reset muic to change QC 9V => TA 5V case 1 attached_dev[%d]\n", MUIC_DEV_NAME, __func__, muic_data->attached_dev);
...
//				pr_debug("%s:%s wait until attached device changing attached_dev[%d]\n",	MUIC_DEV_NAME, __func__, muic_data->attached_dev);
			...
			break;
		default:
			pr_debug("%s:%s reset called case 1 but charger is not connected attached_dev[%d]\n",	MUIC_DEV_NAME, __func__, muic_data->attached_dev);
			break;
		}
		...
		if(muic_data->pdata->hv_sel == 0) {
			pr_debug("%s:%s off 5V called twice [%d]\n",	MUIC_DEV_NAME, __func__, muic_data->pdata->hv_sel);
			return count;
		}
		...
		case ATTACHED_DEV_AFC_CHARGER_5V_MUIC:
			pr_debug("%s:%s reset muic to change AFC 5V => TA 9V case 2 attached_dev[%d]\n",	MUIC_DEV_NAME, __func__, muic_data->attached_dev);
...
//				pr_debug("%s:%s wait until attached device changing attached_dev[%d]\n", MUIC_DEV_NAME, __func__, muic_data->attached_dev);
		...
		case ATTACHED_DEV_QC_CHARGER_5V_MUIC:
			pr_debug("%s:%s reset muic to change QC 5V => TA 9V case 2 attached_dev[%d]\n", MUIC_DEV_NAME, __func__, muic_data->attached_dev);
...
//				pr_debug("%s:%s wait until attached device changing attached_dev[%d]\n",	MUIC_DEV_NAME, __func__, muic_data->attached_dev);
			...
			break;
		default:
			pr_debug("%s:%s reset called case 2 but charger is not connected attached_dev[%d]\n",	MUIC_DEV_NAME, __func__, muic_data->attached_dev);
			break;
		}
...
static int max77843_muic_handle_detach(struct max77843_muic_data *muic_data)
{
	...
	pr_debug("%s:%s cancel_delayed_work, Mping missing wa\n",
