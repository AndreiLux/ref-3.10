...
int find_empty_page_from_top(int start_addr, struct mtd_info *mtd)
{
	...
	for (page_offset = 0; page_offset < (block_size / page_size); page_offset++) {
		...
		if (mtd->_read_oob(mtd, (loff_t) current_add, &ops_pt) != 0) {
			pr_info("find_emp read failed %x\n", current_add);
			continue;
		} else {
	...
	if (page_offset != 0x40) {
		...
		if (mtd->_erase(mtd, &ei) != 0) {	/* no good block for used in replace pool */
			pr_info("find_emp erase mirror failed %x\n", start_addr);
...
bool find_mirror_pt_from_bottom(int *start_addr, struct mtd_info *mtd)
{
	...
	ops_pt.oobbuf = page_buf + page_size;
	ops_pt.ooboffs = 0;
	pr_info("find_mirror find begain at %x\n", mpt_start_addr);
	for (mpt_locate = ((block_size / page_size) - 1); mpt_locate >= 0; mpt_locate--) {
		...
		if (mtd->_read_oob(mtd, (loff_t) current_start_addr, &ops_pt) != 0) {
			pr_info("find_mirror read  failed %x %x\n", current_start_addr,
				mpt_locate);
		}
		...
		if (is_valid_mpt(page_buf) && is_valid_mpt(pmt_spare)) {
			...
			pr_info("find_mirror find valid pt at %x sq %x\n", current_start_addr,
				pi.sequencenumber);
			break;
		} else {
			continue;
...
void part_init_pmt(struct mtd_info *mtd, u8 *buf)
{
	...
	pr_info("offset  %llx part->size %llx %s\n", part->offset, part->size, part->name);
	...
	if (retval == ERR_NO_EXIST)	/* first run preloader before dowload */
	{
		...
		for (i = 0; i < part_num; i++) {
			pr_info("partition %s size %lx %lx\n", lastest_part[i].name,
...
int new_part_tab(u8 *buf, struct mtd_info *mtd)
{
	...
	for (change_index = 0; change_index <= part_num; change_index++) {
		if ((new_part[change_index].size != lastest_part[change_index].size)
			...
			pr_info("new_pt %x size changed from %lx to %lx\n", change_index,
		...
		if (pageoffset != 0xFFFF) {
			...
			for (i = 0; i < 2; i++) {
				...
				if (mtd->_write_oob(mtd, (loff_t) current_addr, &ops_pt) != 0) {
					pr_info("new_pt write m first page failed %x\n",
						current_addr);
				} else {
					pr_info("new_pt write mirror at %x\n", current_addr);
					...
					if ((mtd->_read_oob(mtd, (loff_t) current_addr, &ops_pt) !=
						...
						pr_info
						    ("new_pt read or verify first mirror page failed %x\n",
						     current_addr);
						ops_pt.datbuf = (uint8_t *) page_buf;
						memset(page_buf, 0, PT_SIG_SIZE);
						if (mtd->
						    _read_oob(mtd, (loff_t) current_addr,
							      &ops_pt) != 0) {
							pr_info("new_pt mark failed %x\n",
								current_addr);
						}
					} else {
...
int update_part_tab(struct mtd_info *mtd)
{
	...
	if ((pi.pt_changed == 1 || pi.pt_has_space == 0) && pi.tool_or_sd_update == 2) {
		...
		ei.addr = start_addr;
		if (mtd->_erase(mtd, &ei) != 0) {	/* no good block for used in replace pool */
			pr_info("update_pt erase failed %x\n", start_addr);
		...
		for (retry_r = 0; retry_r < RETRY_TIMES; retry_r++) {
			for (retry_w = 0; retry_w < RETRY_TIMES; retry_w++) {
				...
				*(int *)sig_buf = PT_SIG;
				...
				ops_pt.datbuf = (uint8_t *) page_buf;
				if (mtd->_write_oob(mtd, (loff_t) current_addr, &ops_pt) != 0) {	/* no good block for used in replace pool . still used the original ones */
