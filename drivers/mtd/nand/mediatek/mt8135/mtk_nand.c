...
void dump_nfi(void)
{
	...
	pr_info("NFI_PAGEFMT_REG16: 0x%x\n", DRV_Reg16(NFI_PAGEFMT_REG16));
	pr_info("NFI_CON_REG16: 0x%x\n", DRV_Reg16(NFI_CON_REG16));
	...
	pr_info("NFI_LOCK_REG16: 0x%x\n", DRV_Reg16(NFI_LOCK_REG16));
	pr_info("NFI_LOCKCON_REG32: 0x%x\n", DRV_Reg32(NFI_LOCKCON_REG32));
	pr_info("NFI_LOCKANOB_REG16: 0x%x\n", DRV_Reg16(NFI_LOCKANOB_REG16));
...
bool get_device_info(u8 *id, flashdev_info *devinfo)
{
	...
	if (target != -1) {
		...
		memcpy(devinfo->devciename, gen_FlashTable[target].devciename,
 ...
 *
 ******************************************************************************/
static bool mtk_nand_check_bch_error(struct mtd_info *mtd, u8 *pDataBuf, u32 u4SecIndex,
				     u32 u4PageAddr)
{
	bool bRet = true;
	...
	if (0 != (u4ErrorNumDebug0 & 0xFFFFF) || 0 != (u4ErrorNumDebug1 & 0xFFFFF)) {
		for (i = 0; i <= u4SecIndex; ++i) {
			...
			if (0x1F == u4ErrNum) {
				pr_err("In Kernel UnCorrectable at PageAddr=%d, Sector=%d\n",
				       u4PageAddr, i);
				bRet = false;
				mtd->ecc_stats.failed++;
			} else {
				if (u4ErrNum) {
					pr_err(" In Kernel Correct %d at PageAddr=%d, Sector=%d\n",
		...
		if ((correct_count > 2) && bRet) {
			...
			pr_info("In kernel Less than 2 bit error, ignore\n");
		}
	}
#else
	...
	if (u4ErrNum) {
		if (0xF == u4ErrNum) {
			mtd->ecc_stats.failed++;
			bRet = false;
			/* pr_err("UnCorrectable at PageAddr=%d\n", u4PageAddr); */
		} else {
			for (i = 0; i < ((u4ErrNum + 1) >> 1); ++i) {
				...
				if (0 != u4ErrBitLoc2nd) {
					if (u4ErrBitLoc2nd < 0x1000) {
						...
						mtd->ecc_stats.failed++;
						/* pr_err("UnCorrectable High ErrLoc=%d\n", au4ErrBitLoc[i]); */
					}
				}
			}
		}
...
static void mtk_nand_configure_lock(void)
{
	...
	DRV_WriteReg16(NFI_LOCKANOB_REG16,
		       (u4WriteColNOB << PROG_CADD_NOB_SHIFT) | (u4WriteRowNOB <<
								 PROG_RADD_NOB_SHIFT) |
		       (u4EraseColNOB << ERASE_CADD_NOB_SHIFT) | (u4EraseRowNOB <<
				     ...
				     u8 *buf)
{
	...
	if (full) {
		...
		if (!phys) {
			pr_err
			    ("[mt6575_nand_ready_for_write]convert virt addr (%x) to phys add fail!!!",
			     (u32) buf);
			return false;
		} else {
			DRV_WriteReg32(NFI_STRADDR_REG32, phys);
		...
		if ((T_phys > 0x700000 && T_phys < 0x800000)
			...
			{
				pr_info
				    ("[NFI_WRITE]ERROR: Forbidden AHB address wrong phys address =0x%x , right phys address=0x%x, virt  address= 0x%x (count = %d)\n",
 ...
 *
 ******************************************************************************/
bool mtk_nand_exec_read_page(struct mtd_info *mtd, u32 u4RowAddr, u32 u4PageSize, u8 *pPageBuf,
			     u8 *pFDMBuf)
{
	u8 *buf;
	bool bRet = true;
	...
	if (get_chip_eco_ver() == CHIP_E1) {
		if (((u4RowAddr * u4PageSize) >= g_exist_Partition[0].offset)
		    && ((u4RowAddr * u4PageSize) <
			(g_exist_Partition[1].offset + g_exist_Partition[1].size))) {
			u32 u4ENCODESize;
			u32 u4DECODESize;
			...
			DRV_WriteReg32(ECC_ENCCNFG_REG32,
				       ECC_CNFG_ECC8 | ENC_CNFG_NFI | (u4ENCODESize <<
								       ENC_CNFG_MSG_SHIFT));
#ifndef MANUAL_CORRECT
			NFI_SET_REG32(ECC_DECCNFG_REG32, DEC_CNFG_CORRECT);
#else
			     ...
			     u8 *pFDMBuf)
{
	...
	if (dummy_driver_debug) {
		...
		if (!((time * 123 + 59) % 32768)) {
			pr_info("[NAND_DUMMY_DRIVER] Simulate write error at page: 0x%x\n",
				u4RowAddr);
			return -EIO;
		}
	}
 ...
 *
 *****************************************************************************/
static int mtk_nand_write_page(struct mtd_info *mtd, struct nand_chip *chip, const u8 *buf,
			       int page, int cached, int raw)
{
	...
	if (mtk_nand_exec_write_page
		...
		if (update_bmt
		    ((page_in_block + mapped_block * page_per_block) << chip->page_shift,
...
static int mtk_nand_read_page(struct mtd_info *mtd, struct nand_chip *chip, u8 *buf, int page)
{
	...
	if (mtk_nand_exec_read_page
	    (mtd, page_in_block + mapped_block * page_per_block, mtd->writesize, buf,
	     chip->oob_poi))
		return 0;
	/* else
	   return -EIO; */
...
int mtk_nand_erase_hw(struct mtd_info *mtd, int page)
{
	...
	if (dummy_driver_debug) {
		unsigned long long time = sched_clock();
		if (!((time * 123 + 59) % 1024)) {
			pr_info("[NAND_DUMMY_DRIVER] Simulate erase error at page: 0x%x\n", page);
...
int mtk_nand_block_bad_hw(struct mtd_info *mtd, loff_t ofs)
{
	...
	if (mtk_nand_read_oob_raw(mtd, oob_buf, page_addr, sizeof(oob_buf))) {
		pr_warn("mtk_nand_read_oob_raw return error\n");
		return 1;
	}
...
static int mtk_nand_block_bad(struct mtd_info *mtd, loff_t ofs, int getchip)
{
	...
	if (ret) {
		...
		if (update_bmt
		    (mapped_block << chip->phys_erase_shift, UPDATE_UNMAPPED_BLOCK, NULL, NULL)) {
			MSG(INIT, "Update BMT success\n");
			ret = 0;
		} else {
			MSG(INIT, "Update BMT fail\n");
...
static int mtk_nand_verify_buf(struct mtd_info *mtd, const uint8_t *buf, int len)
{
	 ...
	 */
	/* pr_info("mtk_nand_verify_buf OK at page %d\n", g_kCMD.u4RowAddr); */
...
static int mtk_nand_proc_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	len +=
	    sprintf(page + len, "total size: %dMiB; part number: %s\n", devinfo.totalsize,
		    devinfo.devciename);
	len +=
	    sprintf(page + len, "Current working in %s mode\n",
 ...
 *
 ******************************************************************************/
static int mtk_nand_proc_write(struct file *file, const char *buffer, unsigned long count,
			       void *data)
{
...
static int mtk_nand_probe(struct platform_device *pdev)
{
	...
	pr_info("Allocate 16 byte aligned buffer: %p\n", local_buffer_16_align);
	...
	nand_chip->ecc.layout->eccbytes =
	    devinfo.sparesize - OOB_AVAI_PER_SECTOR * (devinfo.pagesize / NAND_SECTOR_SIZE);
	for (i = 0; i < nand_chip->ecc.layout->eccbytes; i++) {
		nand_chip->ecc.layout->eccpos[i] =
		    OOB_AVAI_PER_SECTOR * (devinfo.pagesize / NAND_SECTOR_SIZE) + i;
	}
	...
	mt_irq_set_sens(MT_NFI_IRQ_ID, MT_EDGE_SENSITIVE);
...
static int __init mtk_nand_init(void)
{
	...
	pr_info("MediaTek Nand driver init, version %s\n", VERSION);
