...
int hw_configure_acr(u32 pclk, hdmi_core_fs audio_fs)
{
    ...
    /*DTS2012050301959 z36904 20120503 begin, for Coverity check error*/
    switch (audio_fs) {
        case FS_32000:
            fs = 32000;
            n = 4096;
    ...
    /* Process CTS */
    /*DTS2012072001293 z00222844 20120728 modify begin*/
    cts = (pclk * (n / 128) * 10) / (fs / 100);
    /*DTS2012072001293 z00222844 20120728 end*/
    /*DTS2012050301959 z36904 20120503 end*/
...
void hw_enable(hdmi_config *cfg)
{
    ...
    BUG_ON(NULL == cfg);
    /*DTS2012121105814 z00222844 20121211 begin add request max ddr freq when 1080p60*/
...
#endif
    /*DTS2012121105814 z00222844 20121211 end*/
...
void hw_phy_power_on(void)
{
    ...
    */
    ...
    /* DTS2012050402791 z36904 20120504 end > */
    //mdelay(5);
    ...
    if (hw_support_mhl()) {
        ...
        REG_FLD_MOD(hw_res.base_phy, PHY_MHL_DRV_CTRL_ADDR, 0x58, 7, 0);
#if PHY_MHL_30BIT
...
/*DTS2012042700868 z36904 20120530 begin add request ddr freq when power on*/
#ifdef CONFIG_CPU_FREQ_GOV_K3HOTPLUG
        pm_qos_add_request(&hdmi.qos_request, PM_QOS_DDR_MIN_PROFILE, HDMI_PM_QOS_DDR_MIN_FREQ);
        logd("set ddr freq:%d\n", HDMI_PM_QOS_DDR_MIN_FREQ);
#endif
/*DTS2012042700868 z36904 20120530 end*/
...
/*DTS2012042700868 z36904 20120530 begin remove request when power off*/
#ifdef CONFIG_CPU_FREQ_GOV_K3HOTPLUG
        pm_qos_remove_request(&hdmi.qos_request);
...
void hw_free_resources(void)
{
    ...
    IN_FUNCTION;
#if HDMI_CHIP_VER
    /*DTS2012072001293 z00222844 20120728 add if init check begin*/
