...
static int enable_hpd(bool enable)
{
    ...
    if (enable) {
        ...
        k3_mhl_reset_states();
        /* < DTS2012040902319 z36904 20120419 begin */
        ret = hdmi_pw_set_video_power(HDMI_POWER_OFF);
        if (ret) {
            loge("set video power fail: %d.\n", ret);
        }
        /*  DTS2012040902319 z36904 20120419 end > */
    }
...
#define HDMI_READ_EDID_COUNT 6
    ...
    if (!hdmi.edid_set) {
        ...
        /*DTS2012050807615 z36904 20120510 begin*/
        ...
        /*DTS2012050807615 z36904 20120510 end*/
        if (ret) {
            loge("read edid fail.\n");
            memset(&edid, 0, sizeof(edid));
            hdmi.edid_set = false;
...
static ssize_t hdmi_get_timing_list(char *buf, bool mhl_check)
{
    ...
    if (INVALID_VALUE != cm.code) {
        if (HDMI_CODE_TYPE_VESA == cm.mode) {
            /*DTS2012051500792 j00140427 20120515 start*/
            ...
            /*DTS2012051500792 j00140427 20120515 end*/
        } else {
                                       ...
                                       const char *buf, size_t size)
{
    ...
    /*DTS2012050902840 j00140427 20100514 start*/
    if (hdcp_get_enable() == enable) {
        OUT_FUNCTION;
        return size;
    }
    /*DTS2012050902840 j00140427 20100514 end*/
...
/* < DTS2012042102767 z36904 20120421 begin*/
/******************************************************************************
...
/* DTS2012042102767 z36904 20120421 end >*/
...
int k3_hdmi_probe(struct platform_device *pdev)
{
    ...
    //DTS2013021805702 w00227189 20130218 begin, clear fortify tool bug
    if(ret < 0)
    {
        logw("switch device register error %d\n",ret);
    }
    //DTS2013021805702 w00227189 20130218 end, clear fortify tool bug
#endif
