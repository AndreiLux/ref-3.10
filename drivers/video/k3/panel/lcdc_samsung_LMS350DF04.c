...
#define DTS_COMP_LDI_SAMSUNG_LMS350DF04	"hisilicon,ldi_samsung_LMS350DF04"
...
static u32 gpio_lcd_power;
static u32 gpio_lcd_reset;
static u32 gpio_lcd_id0;
static u32 gpio_lcd_id1;
...
static u32 gpio_pwm0 = 149;/*GPIO_18_5*/
static u32 gpio_pwm1 = 150;/*GPIO_18_6*/
...
**
*/
static int samsung_backlight_on(struct k3_fb_data_type *k3fd)
{
	...
	if (pinfo->bl_set_type & BL_SET_BY_PWM) {
		...
		/* backlight on */
		//k3_pwm_set_backlight(k3fd);
	}

	return 0;
}

static int samsung_backlight_off(struct k3_fb_data_type *k3fd)
{
	struct k3_panel_info *pinfo = NULL;
...
static int lcdc_samsung_panel_on(struct platform_device *pdev)
{
	...
	if (pinfo->lcd_init_step == LCD_INIT_POWER_ON) {
		...
		if (ret != 0) {
			K3_FB_ERR("spi_cmds_tx failed! ret=%d\n", ret);
		}

		if (k3fd->panel_info.bl_set_type & BL_SET_BY_PWM) {
			/* backlight on */
			samsung_backlight_on(k3fd);
		}
	} else {
...
static int lcdc_samsung_probe(struct platform_device *pdev)
{
	...
	if (k3_fb_device_probe_defer(PANEL_LCDC)) {
		goto err_probe_defer;
	}

	K3_FB_DEBUG("+.\n");

	np = of_find_compatible_node(NULL, NULL, DTS_COMP_LDI_SAMSUNG_LMS350DF04);
	if (!np) {
		K3_FB_ERR("NOT FOUND device node %s!\n", DTS_COMP_LDI_SAMSUNG_LMS350DF04);
		goto err_return;
	}
...
static const struct of_device_id k3_panel_match_table[] = {
	{
		.compatible = DTS_COMP_LDI_SAMSUNG_LMS350DF04,
		.data = NULL,
	},
};
