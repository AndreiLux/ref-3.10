...
static u32 gpio_rgb2mipi_power_1v2 = 54; //GPIO_6_6
static u32 gpio_rgb2mipi_standby = 67; //GPIO_8_3
static u32 gpio_rgb2mipi_resetn = 65; //GPIO_8_1
static u32 gpio_rgb2mipi_gating = 60; //GPIO_7_4
static u32 gpio_rgb2mipi_pclk_gating = 58; //GPIO_7_2
static u32 gpio_rgb2mipi_refclk_gating = 59; //GPIO_7_3
static u32 gpio_rgb2mipi_spics = 168; //GPIO_21_0
static u32 gpio_rgb2mipi_drv_en = 73; //GPIO_9_1
...
static int rgb2mipi_swrite(struct spi_device *spi_dev, struct dsi_cmd_desc *cm)
{
	...
	if (dataLength == 1) {
		...
		cmds =*( (unsigned char *)(cm->payload + 1));
...
static int rgb2mipi_lwrite(struct spi_device *spi_dev, struct dsi_cmd_desc *cm)
{
	...
	if ((dataLength > 2) && (dataLength <= 8)) {
		...
		for(i = 0; i < dataLength - 1; i++){
			param_upto8_cmds[i].value =  *((unsigned char *)(cm->payload + i + 1));
		}
		...
		for(i = 0; i < dataLength; i++) {
			param_all_payload_cmds[0].value =  *((unsigned char *)(cm->payload + i));
...
**
*/
static int rgb2mipi_init(struct k3_fb_data_type *k3fd)
{
	struct spi_device *spi_dev = NULL;

	BUG_ON(k3fd == NULL);
	spi_dev = k3fd->panel_info.spi_dev;
	BUG_ON(spi_dev == NULL);
...
static int rgb2mipi_prep4Rec(struct k3_fb_data_type *k3fd)
{
	struct spi_device *spi_dev = NULL;
...
static int rgb2mipi_deinit(struct k3_fb_data_type *k3fd)
{
	struct spi_device *spi_dev = NULL;
...
static int rgb2mipi_set_backlight(struct platform_device *pdev)
{
	int ret = 0;
	struct k3_fb_data_type *k3fd = NULL;
...
static int rgb2mipi_probe(struct platform_device *pdev)
{
	...
	k3fd = platform_get_drvdata(pdev);
	BUG_ON(k3fd == NULL);
	BUG_ON(k3fd->panel_info.spi_dev == NULL);
	...
	/* set driver data */
	platform_set_drvdata(dpe_dev, k3fd);
	/* device add */
