		...
		int *input_endpos)
{
	...
	*input_endpos = MIN((*input_endpos) + cfg.Scl_Input_Right_Ov, layer_start_limit + layer_span - 1);
	...
	if(inc_hscl < INC_FACTOR &&
                ...
                K3_BA_ERR("err!! input_startpos[%d],input_endpos[%d],inc[%d],FirstTile[%d]"
...
{
	if (need_rot) {
		b_detail->inc_hscl = layer_info->src_rect.h * INC_FACTOR / layer_info->dst_rect.w;
		b_detail->inc_vscl = (layer_info->src_rect.w - 1) * INC_FACTOR / (layer_info->dst_rect.h - 1);
	} else {
		b_detail->inc_hscl = layer_info->src_rect.w * INC_FACTOR / layer_info->dst_rect.w;
		b_detail->inc_vscl = (layer_info->src_rect.h - 1) * INC_FACTOR / (layer_info->dst_rect.h - 1);
	}
...
int get_inc_hscl(dss_layer_t *layer_infos)
{
	...
	if (layer_infos->need_cap & CAP_ROT)
		inc_hscl = layer_infos->src_rect.h * INC_FACTOR / layer_infos->dst_rect.w;
	else
		inc_hscl = layer_infos->src_rect.w * INC_FACTOR / layer_infos->dst_rect.w;
...
uint32_t calc_dest_block_size(dss_overlay_t *dss_overlay)
{
	...
	for (i = 0; i < dss_overlay->layer_nums; i++) {
		...
		if (dss_overlay->layer_infos[i].need_cap & CAP_SCL) {
			...
			K3_BA_ERR("layer has no rot and scl, should no get here\n");
		}
	}
	...
	dss_rect_t src_rect, dss_layer_t *layer_cut_block)
{
	...
	case K3_FB_TRANSFORM_ROT_270:
		...
		break;
	default:
		K3_BA_ERR("unknow dss_layer->transform: %d\n",layer_cut_block->transform);
		return 1;
	}
	...
	uint32_t end_pos, uint32_t *fix_size)
{
	...
	*fix_size = block_size;
	...
	if (block_size <= SCF_MIN_OUTPUT * MAX_OFFLINE_SCF) {
		K3_BA_ERR("block size[%d] is too small!\n", block_size);
		return 1;
	}
	...
	for (i = 0; i < dss_overlay->layer_nums; i++) {
		...
		if (scf_layer_num > MAX_OFFLINE_SCF) {
			K3_BA_ERR("scf layer in offline [%d] is more than scf moudle [%d]\n",scf_layer_num,MAX_OFFLINE_SCF);
			return 1;
		}
	}
	...
	int *block_num, dss_rect_t *block_rects[])
{
	...
	*block_num = 0;
	...
	if (block_size == 0xFFFF) {
		K3_BA_ERR("calc_dest_block_size err\n");
		return -2;
	}
	...
	for (current_offset = block_size; last_offset < w; last_offset = current_offset, current_offset += block_size) {
		if (block_fix_scf_constraint(dss_overlay, block_size, current_offset, &fix_scf_span) != 0) {
			K3_BA_ERR("block_fix_scf_constraint err\n");
			return -3;
		}
		...
		if (current_offset - last_offset < SCF_MIN_OUTPUT) {
			K3_BA_ERR("scf fix size err,block_size[%d] fix_scf_span[%d],current_offset[%d] last_offset[%d]\n",
		...
		for (i = 0; i < dss_overlay->layer_nums; i++) {
			...
			if ((last_offset <= dss_layer->dst_rect.x + dss_layer->dst_rect.w - 1) && (dss_layer->dst_rect.x < current_offset)) {
				...
				if((*block_num) >= K3_DSS_OFFLINE_MAX_BLOCK)
					return -5;
	...
	*/
	...
	for (i = 0; i < dss_overlay->layer_nums; i++) {
		...
		if ((output_span < SCF_MIN_OUTPUT) && (dss_layer->need_cap & CAP_SCL)) {
			K3_BA_ERR("block layer[%d] scf output smaller than SCF_MIN_OUTPUT, dump some info\
