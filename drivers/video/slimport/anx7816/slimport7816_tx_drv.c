...
static struct
{
	unchar vsi_interrupt_en:1;
	unchar novsi_interrupt_en:1;
...
void sp_tx_initialization(void)
{
	...
	*/
	...
	flag.vsi_interrupt_en = 0;
	flag.novsi_interrupt_en = 0;
}
...
void system_isr_handler(void)
{
	...
	if(sp_tx_system_state >STATE_VIDEO_OUTPUT) {
		...
		if (flag.novsi_interrupt_en)
			hdmi_rx_no_vsi_int();

		if (flag.vsi_interrupt_en)
			hdmi_rx_new_vsi_int();
