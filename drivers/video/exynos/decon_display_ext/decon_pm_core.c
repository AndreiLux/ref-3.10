...
static unsigned int frame_done_count;
static unsigned int te_count;

static void display_block_clock_on(struct display_driver *dispdrv);
static void display_block_clock_off(struct display_driver *dispdrv);
static int display_hibernation_power_on(struct display_driver *dispdrv);
static int display_hibernation_power_off(struct display_driver *dispdrv);
...
static struct pm_ops display_block_ops = {
...
extern struct pm_ops decon1_pm_ops;
#ifdef CONFIG_DECON_MIC
extern struct pm_ops mic1_pm_ops;
#endif
extern struct pm_ops dsi1_pm_ops;
#endif

extern struct mipi_dsim_device *dsim_for_decon_ext;

int disp1_pm_set_plat_status(struct display_driver *dispdrv, bool platform_on)
{
	if (platform_on)
...
int init_display1_pm_status(struct display_driver *dispdrv) {
	...
	dispdrv->platform_status = DISP_STATUS_PM0;
	disp1_pm_set_plat_status(dispdrv, false);
...
int init_display1_pm(struct display_driver *dispdrv)
{
	init_display1_pm_status(dispdrv);
	...
	dispdrv->decon_driver.ops = &decon1_pm_ops;
	dispdrv->dsi_driver.ops = &dsi1_pm_ops;
#ifdef CONFIG_DECON_MIC
	dispdrv->mic_driver.ops = &mic1_pm_ops;
#endif
#endif
	return 0;
}

void disp1_debug_power_info(void)
{
...
void disp1_pm_gate_lock(struct display_driver *dispdrv, bool increase)
{
	if (increase)
...
void debug_function1(struct display_driver *dispdrv, const char *buf)
{
	long input_time;
#ifndef CONFIG_FB_HIBERNATION_DISPLAY2
...
int disp1_pm_runtime_enable(struct display_driver *dispdrv)
{
...
int disp1_pm_runtime_get_sync(struct display_driver *dispdrv)
{
...
int disp1_pm_runtime_put_sync(struct display_driver *dispdrv)
{
	if (!dispdrv->pm_status.clock_gating_on) {
...
/* disp1_pm_te_triggered - check clock gating or not.
 * this function is called in the TE interrupt handler */
void disp1_pm_te_triggered(struct display_driver *dispdrv)
{
	te_count++;
	...
	if (dispdrv->platform_status > DISP_STATUS_PM0 &&
		...
		if (dispdrv->pm_status.clock_enabled &&
			...
			if (dispdrv->pm_status.clk_idle_count > MAX_CLK_GATING_COUNT) {
				disp1_pm_gate_lock(dispdrv, true);
...
/* disp1_pm_sched_power_on - it is called in the early start of the
 * fb_ioctl to exit HDM */
int disp1_pm_sched_power_on(struct display_driver *dispdrv, unsigned int cmd)
{
	...
	if (dispdrv->platform_status < DISP_STATUS_PM1) {
		if (cmd == S3CFB_WIN_CONFIG)
			disp1_pm_set_plat_status(dispdrv, true);
	}
...
void disp1_set_pm_status(int flag)
{
	struct display_driver *dispdrv = get_display_driver_ext();
...
/* disp1_pm_add_refcount - it is called in the early start of the
 * update_reg_handler */
int disp1_pm_add_refcount(struct display_driver *dispdrv)
{
...
/* disp1_pm_dec_refcount - it is called at the DSI frame done */
int disp1_pm_dec_refcount(struct display_driver *dispdrv)
{
	save_decon1_operation_time(OPS_CALL_DSIM_FRAME_DONE);
	++frame_done_count;
...
static void decon_clock_gating_handler(struct kthread_work *work)
{
	...
	init_gating_idle_count(dispdrv);
	disp1_pm_gate_lock(dispdrv, false);
	pm_debug("display_block_clock_off -");
}
...
static int __display_hibernation_power_on(struct display_driver *dispdrv)
{
	...
	save_decon1_operation_time(OPS_CALL_DISPLAY_POWER_ON);
...
{
	save_decon1_operation_time(OPS_CALL_DISPLAY_POWER_OFF);
...
static int display_hibernation_power_off(struct display_driver *dispdrv)
{
	...
	disp1_pm_runtime_put_sync(dispdrv);
