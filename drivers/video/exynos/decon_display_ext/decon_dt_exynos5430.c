/* linux/drivers/video/exynos/decon_display2/decon_dt_exynos5430.c
 *
...
#define DISP_CTRL_NAME	"decon_ctrl1"
#define MIPI_DSI_NAME	"mipi_dsi1"

#ifdef CONFIG_DECON_MIC
#define MIC_NAME	"decon_mic1"
#endif
...
static struct s3c_fb_platdata g_decon_platdata_ext;

struct mipi_dsim_config g_dsim_config_ext = {
	.dsim_ddi_pd = &s6e3fa2_mipi_lcd_ext_driver,
};

#ifdef CONFIG_DECON_MIC
static struct mic_config g_mic_config;
#endif
...
struct mipi_dsim_lcd_config g_lcd_config_ext;
struct decon_lcd g_decon_lcd_ext;
struct display_sysreg g_disp_sysreg_ext;

static int parse_decon_platdata_ext(struct device_node *np)
{
	u32 temp;
	char *pprop;
	...
	DT_READ_U32(np, "samsung,vidcon1", g_decon_platdata_ext.vidcon1);
#endif
	DT_READ_U32(np, "samsung,default_win", g_decon_platdata_ext.default_win);
	DT_READ_U32(np, "samsung,max_bpp", g_fb_win0.max_bpp);
	DT_READ_U32(np, "samsung,default_bpp", g_fb_win0.default_bpp);

	g_decon_platdata_ext.win[0] = &g_fb_win0;
	g_decon_platdata_ext.win[1] = &g_fb_win0;
	g_decon_platdata_ext.win[2] = &g_fb_win0;
	g_decon_platdata_ext.win[3] = &g_fb_win0;
	g_decon_platdata_ext.win[4] = &g_fb_win0;
	return 0;
...
static void parse_disp_sysreg(void)
{
	...
	g_disp_sysreg_ext.disp_sysreg[0] = of_iomap(nd, 0);
	if(!g_disp_sysreg_ext.disp_sysreg[0]) {
...
static int parse_display_dt_exynos5430(struct device_node *np)
{
	...
	ret = parse_decon_platdata_ext(np);
	if (ret < 0) {
	...
	struct display_driver *ddp)
{
	...
	ddp->decon_driver_ext.irq_no = res->start;
#endif
...
{
	return &g_decon_platdata_ext;
}

struct mipi_dsim_config *get_display_dsi_drvdata_ext(void)
{
	return &g_dsim_config_ext;
}

struct mipi_dsim_lcd_config *get_display_lcd_drvdata_ext(void)
{
	return &g_lcd_config_ext;
}

#ifdef CONFIG_DECON_MIC
struct mic_config *get_display_mic_config_ext(void)
{
	return &g_mic_config;
}
#endif
...
static int parse_dsi_drvdata(struct device_node *np)
{
	...
	DT_READ_U32_OPTIONAL(np, "e_interface", g_dsim_config_ext.e_interface);
	DT_READ_U32_OPTIONAL(np, "e_pixel_format",
		g_dsim_config_ext.e_pixel_format);
	DT_READ_U32_OPTIONAL(np, "auto_flush", g_dsim_config_ext.auto_flush);
	DT_READ_U32_OPTIONAL(np, "eot_disable", g_dsim_config_ext.eot_disable);
	DT_READ_U32_OPTIONAL(np, "auto_vertical_cnt",
		g_dsim_config_ext.auto_vertical_cnt);
	DT_READ_U32_OPTIONAL(np, "hse", g_dsim_config_ext.hse);
	DT_READ_U32_OPTIONAL(np, "hfp", g_dsim_config_ext.hfp);
	DT_READ_U32_OPTIONAL(np, "hbp", g_dsim_config_ext.hbp);
	DT_READ_U32_OPTIONAL(np, "hsa", g_dsim_config_ext.hsa);
	DT_READ_U32_OPTIONAL(np, "e_no_data_lane",
		g_dsim_config_ext.e_no_data_lane);
	DT_READ_U32_OPTIONAL(np, "e_byte_clk", g_dsim_config_ext.e_byte_clk);
	DT_READ_U32_OPTIONAL(np, "e_burst_mode", g_dsim_config_ext.e_burst_mode);
	DT_READ_U32_OPTIONAL(np, "p", g_dsim_config_ext.p);
	DT_READ_U32_OPTIONAL(np, "m", g_dsim_config_ext.m);
	DT_READ_U32_OPTIONAL(np, "s", g_dsim_config_ext.s);
	DT_READ_U32_OPTIONAL(np, "pll_stable_time",
		g_dsim_config_ext.pll_stable_time);
	DT_READ_U32_OPTIONAL(np, "esc_clk", g_dsim_config_ext.esc_clk);
	DT_READ_U32_OPTIONAL(np, "stop_holding_cnt",
		g_dsim_config_ext.stop_holding_cnt);
	DT_READ_U32_OPTIONAL(np, "bta_timeout", g_dsim_config_ext.bta_timeout);
	DT_READ_U32_OPTIONAL(np, "rx_timeout", g_dsim_config_ext.rx_timeout);
...
#ifdef CONFIG_DECON_MIC
int parse_decon_mic_dt_exynos5430_ext(struct device_node *np)
{
	u32 temp;
...
struct decon_lcd *decon_get_lcd_info_ext(void)
{
	return &g_decon_lcd_ext;
}

struct display_sysreg *decon_get_sysreg_ext(void)
{
	return &g_disp_sysreg_ext;
}

int parse_lcd_drvdata_ext(struct device_node *np)
{
	int ret, idx = 0;
	u32 temp, res[3];
	...
	DT_READ_U32_OPTIONAL(node, "mode", g_decon_lcd_ext.mode);
	DT_READ_U32_OPTIONAL(node, "mode", g_dsim_config_ext.e_interface);

	ret = of_property_read_u32_array(node, "resolution", res, 2);
	g_decon_lcd_ext.xres = res[0];
	g_decon_lcd_ext.yres = res[1];

	ret = of_property_read_u32_array(node, "size", res, 2);
	g_decon_lcd_ext.width = res[0];
	g_decon_lcd_ext.height = res[1];

	DT_READ_U32_OPTIONAL(node, "timing,refresh", g_decon_lcd_ext.fps);

	ret = of_property_read_u32_array(node, "timing,h-porch", res, 3);
	g_decon_lcd_ext.hbp = res[0];
	g_decon_lcd_ext.hfp = res[1];
	g_decon_lcd_ext.hsa = res[2];

	ret = of_property_read_u32_array(node, "timing,v-porch", res, 3);
	g_decon_lcd_ext.vbp = res[0];
	g_decon_lcd_ext.vfp = res[1];
	g_decon_lcd_ext.vsa = res[2];

	DT_READ_U32_OPTIONAL(node, "timing,dsi-hs-clk", g_decon_lcd_ext.hs_clk);
	DT_READ_U32_OPTIONAL(node, "timing,dsi-escape-clk", g_decon_lcd_ext.esc_clk);
	DT_READ_U32_OPTIONAL(node, "mic", g_decon_lcd_ext.mic);
	DT_READ_U32_OPTIONAL(node, "timing,hsync-2h-cycle", g_decon_lcd_ext.hsync_2h_cycle);

	g_decon_lcd_ext.hs_clk *= 1000000;
	g_decon_lcd_ext.esc_clk *= 1000000;
...
static int parse_all_dt_exynos5430(struct device_node *parent)
{
	...
	ret = parse_decon_mic_dt_exynos5430_ext(node);
	if (ret < 0) {
		pr_err("parsing for MIC failed.\n");
		return ret;
	}
#endif
	parse_lcd_drvdata_ext(parent);

	return ret;
}

/* parse_display_driver_dt_ext_exynos5430
 * for creating all display device tree data & set up H/W info like as
 * base address and IRQ numbers of all display system IPs. */
int parse_display_driver_dt_ext(struct platform_device *pdev,
	struct display_driver *ddp)
{
	int ret = 0;
