/* linux/drivers/decon_ext_display/decon-fb.c
 *
...
int decon_hibernation_power_on_ext(struct display_driver *dispdrv);
int decon_hibernation_power_off_ext(struct display_driver *dispdrv);
#endif

extern struct mipi_dsim_device *dsim_for_decon_ext;
extern int s5p_mipi_dsi_disable_ext(struct mipi_dsim_device *dsim);
extern int s5p_mipi_dsi_enable_ext(struct mipi_dsim_device *dsim);
#ifdef CONFIG_DECON_MIC
extern struct decon_mic_ext *mic_for_decon_ext;
extern int decon_mic_enable_ext(struct decon_mic_ext *mic);
extern int decon_mic_disable_ext(struct decon_mic_ext *mic);
#endif
#ifdef CONFIG_ION_EXYNOS
...
u32 fb_visual_ext(u32 bits_per_pixel, unsigned short palette_sz)
{
...
static int s3c_fb_set_par(struct fb_info *info)
{
	...
	disp_pm_runtime_get_sync_ext(dispdrv);

	if (sfb->power_state == POWER_DOWN)
		return 0;

	decon_reg_shadow_protect_win_ext(win->index, 1);

	info->fix.visual = fb_visual_ext(var->bits_per_pixel,
	...
	if (decon_reg_is_win_enabled_ext(win_no))
	...
	win_regs.vidw_buf_size = vidw_buf_size(var->xres, info->fix.line_length,
			var->bits_per_pixel);
	decon_reg_set_regs_data_ext(win_no, &win_regs);

	decon_reg_shadow_protect_win_ext(win->index, 0);

	decon_reg_update_standalone_ext();

	disp_pm_runtime_put_sync_ext(dispdrv);
...
static void s3c_fb_disable_otf(struct s3c_fb *sfb)
{
	...
	if (test_bit(S3C_FB_LOCAL, &sfb->windows[0]->state)) {
		...
		decon_reg_shadow_protect_win_ext(sfb->windows[0]->index, 1);
		...
		decon_reg_shadow_protect_win_ext(sfb->windows[0]->index, 0);
...
static int s3c_fb_blank(int blank_mode, struct fb_info *info)
{
	...
	save_decon_ext_operation_time(OPS_CALL_FB_BLANK_IN);
	...
	case FB_BLANK_NORMAL:
		...
		s5p_mipi_dsi_disable_ext(dsim_for_decon_ext);
#ifdef CONFIG_DECON_MIC
		decon_mic_disable_ext(mic_for_decon_ext);
#endif
#if defined(CONFIG_DECON_DEVFREQ)
		exynos5_update_media_layers(TYPE_DECON_EXT, 0);
...
#if defined(CONFIG_DECON_DEVFREQ)
		exynos5_update_media_layers(TYPE_DECON_EXT, 1);
		exynos5_update_media_layers(TYPE_GSCL_LOCAL, 0);
		prev_overlap_cnt = 1;
		prev_gsc_local_cnt = 0;
#endif
#ifdef CONFIG_DECON_MIC
		decon_mic_enable_ext(mic_for_decon_ext);
#endif
		s5p_mipi_dsi_enable_ext(dsim_for_decon_ext);
	...
	save_decon_ext_operation_time(OPS_CALL_FB_BLANK_OUT);
			      ...
			      struct fb_info *info)
{
	...
	decon_reg_activate_window_ext(win->index);
...
#ifdef CONFIG_FB_I80_COMMAND_MODE
	decon_reg_set_trigger_ext(sfb->trig_mode, DECON_TRIG_ENABLE);

	s3c_fb_wait_for_vsync(sfb, VSYNC_TIMEOUT_MSEC);

	decon_reg_set_trigger_ext(sfb->trig_mode, DECON_TRIG_DISABLE);
#endif
 ...
 */
static void s3c_fb_log_fifo_underflow_locked(struct s3c_fb *sfb, ktime_t timestamp)
{
#ifdef CONFIG_DEBUG_FS
...
#ifdef CONFIG_FB_I80_COMMAND_MODE
static irqreturn_t decon_ext_fb_isr_for_eint(int irq, void *dev_id)
{
	...
	save_decon_ext_operation_time(OPS_RUN_FB_TEIRQ);
#ifdef CONFIG_FB_HIBERNATION_DISPLAY2
	/* triggering power event for PM */
	if (sfb->output_on && sfb->power_state == POWER_ON)
		disp_pm_te_triggered(get_display_driver_ext());
#endif

#ifdef CONFIG_DECON_MIPI_DSI_PKTGO
	if (sfb->output_on)
		s5p_mipi_dsi_te_triggered_ext();
#endif
	return IRQ_HANDLED;
}
#endif
...
static int s3c_fb_set_window_position(struct fb_info *info,
...
int s3c_fb_set_plane_alpha_blending2(struct fb_info *info,
...
static int s3c_fb_set_chroma_key(struct fb_info *info,
...
static int s3c_fb_set_vsync_int(struct fb_info *info,
		bool active)
{
	struct s3c_fb_win *win = info->par;
	struct s3c_fb *sfb = win->parent;
		...
		struct s3c_fb_win_config *win_config, struct s3c_reg_data *regs)
{
	...
	*/
	...
	win->fbinfo->fix.visual = fb_visual_ext(win->fbinfo->var.bits_per_pixel,
			win->variant.palette_sz);
...
#endif
	for (i = 0; i < sfb->variant.nr_windows - 1 && !ret; i++) {
		struct s3c_fb_win_config *config = &win_config[i];
		struct s3c_fb_win *win = sfb->windows[i + 1];
		 ...
		 */
		regs->wincon[i + 1] |= WINCONx_BURSTLEN_8WORD;

		regs->winmap[i + 1] = color_map;
...
#ifdef CONFIG_FB_HIBERNATION_DISPLAY2
		disp_pm_gate_lock(get_display_driver_ext(), true);
#endif
		...
		struct s3c_reg_data *regs, bool enable)
{
	...
	if (enable) {
		...
		if (atomic_read(&sfb->dsd_clk_ref_cnt) == 0) {
			GET_DISPCTL_OPS(dispdrv).enable_display_dsd_clocks_ext(sfb->dev);
...
static int gsc_subdev_wait_stop(struct s3c_fb *sfb, int i)
{
	...
	if (atomic_read(&sfb->dsd_clk_ref_cnt) == 1) {
		GET_DISPCTL_OPS(dispdrv).disable_display_dsd_clocks_ext(sfb->dev);
...
#ifdef CONFIG_FB_WINDOW_UPDATE
static void decon_ext_reg_set_win_update_config(struct s3c_fb *sfb, struct s3c_reg_data *regs)
{
	struct decon_lcd lcd_info;
	struct decon_lcd *lcd_info_org = decon_get_lcd_info_ext();

	if (regs->need_update) {
		/* TODO: Do we need to wait until the MIPI frame done */
		decon_reg_wait_linecnt_is_zero_timeout_ext(35 * 1000);
		/* Partial Command */
		s5p_mipi_dsi_partial_area_command_ext(dsim_for_decon_ext,
			regs->x[S3C_WIN_UPDATE_IDX], regs->y[S3C_WIN_UPDATE_IDX],
			regs->w[S3C_WIN_UPDATE_IDX], regs->h[S3C_WIN_UPDATE_IDX]);
#ifdef CONFIG_DECON_MIC
		dsim_reg_set_win_update_conf_ext(regs->w[S3C_WIN_UPDATE_IDX], regs->h[S3C_WIN_UPDATE_IDX], true);
		mic_reg_set_win_update_conf_ext(regs->w[S3C_WIN_UPDATE_IDX], regs->h[S3C_WIN_UPDATE_IDX]);
#else
		dsim_reg_set_win_update_conf_ext(regs->w[S3C_WIN_UPDATE_IDX], regs->h[S3C_WIN_UPDATE_IDX], false);
#endif
		...
		if (regs->h[S3C_WIN_UPDATE_IDX] + lcd_info_org->vfp < (lcd_info_org->yres / 8))
			lcd_info.vfp = (lcd_info_org->yres / 8) - regs->h[S3C_WIN_UPDATE_IDX];
		else
...
static void __s3c_fb_update_regs(struct s3c_fb *sfb, struct s3c_reg_data *regs)
{
	...
	for (i = 0; i < sfb->variant.nr_windows; i++)
		decon_reg_shadow_protect_win_ext(sfb->windows[i]->index, 1);

#ifdef CONFIG_FB_WINDOW_UPDATE
	decon_ext_reg_set_win_update_config(sfb, regs);
#endif

	for (i = 0; i < sfb->variant.nr_windows; i++) {
		if (WIN_CONFIG_DMA(i)) {
			s3c_fb_separate_regs_param(&win_regs, regs, i);
			decon_reg_set_regs_data_ext(i, &win_regs);
	...
	for (i = 0; i < sfb->variant.nr_windows; i++)
		decon_reg_shadow_protect_win_ext(sfb->windows[i]->index, 0);
...
static void s3c_fb_update_regs(struct s3c_fb *sfb, struct s3c_reg_data *regs)
{
	...
	save_decon_ext_operation_time(OPS_RUN_UPDATE_REG_QUEUE_ENTER);
	...
	if (prev_overlap_cnt < regs->win_overlap_cnt)
		exynos5_update_media_layers(TYPE_DECON_EXT, regs->win_overlap_cnt);
	...
	count = 9000;
	decon_reg_wait_for_update_timeout_ext(count * 100);
...
#endif
	save_decon_ext_operation_time(OPS_RUN_UPDATE_REG_QUEUE_EXIT);
}
...
#ifdef CONFIG_FB_HIBERNATION_DISPLAY2
		disp_pm_gate_lock(get_display_driver_ext(), false);
#endif
		list_del(&data->list);
		kfree(data);
	}
			...
			unsigned long arg)
{
...
#ifdef CONFIG_FB_HIBERNATION_DISPLAY2_POWER_GATING
	/* Try to scheduled for DISPLAY power_on */
	if (disp_pm_sched_power_on(get_display_driver_ext(), cmd) < 0)
		return 0;
#endif
	...
	case S3CFB_WIN_SET_PLANE_ALPHA:
		...
		ret = s3c_fb_set_plane_alpha_blending2(info, p.user_alpha);
		break;
	...
	case S3CFB_WIN_CONFIG:
		...
		save_decon_ext_operation_time(OPS_GET_WIN_CONFIG_IOCTL);
...
{
#ifdef CONFIG_FB_HIBERNATION_DISPLAY2
	disp_pm_sched_power_on(get_display_driver_ext(), S3CFB_PLATFORM_RESET);
#endif
	return 0;
}
...
static void decon_ext_fb_missing_pixclock(struct decon_fb_videomode *win_mode,
...
ssize_t s3c_fb_debugfs_write2(struct file *file, const char *userbuf, size_t count, loff_t *off)
{
	char buf[20], *p;
	struct display_driver *dispdrv;
...
{
	sfb->debug_dentry = debugfs_create_file("s3c-fb2", S_IRUGO|S_IWUSR, NULL, sfb,
			&s3c_fb_debugfs_fops);
...
#endif
#ifdef CONFIG_FB_I80_COMMAND_MODE
static int decon_ext_fb_config_eint_for_te(struct s3c_fb *sfb)
{
	int ret;
	int gpio;
	...
	ret = devm_request_irq(dev, sfb->irq_no, decon_ext_fb_isr_for_eint,
			  IRQF_TRIGGER_RISING, "s3c_fb1_eint", sfb);

	return ret;
}
#endif

static struct decon_lcd *decon_ext_parse_lcd_info(struct s3c_fb_platdata *pd)
{
	int i;
	struct decon_lcd *lcd_info = decon_get_lcd_info_ext();
...
#if defined(CONFIG_S5P_LCD_EXT_INIT)
static void decon_ext_set_regs_param(struct fb_info *info, struct decon_regs_data *regs)
{
	u32 line_length;
...
int create_decon_ext_display_controller(struct platform_device *pdev)
{
	...
	fbdrv = dispdrv->dt_ops.get_display_drvdata_ext();
	pd = dispdrv->dt_ops.get_display_platdata_ext();
	if (!pd) {
		dev_err(dev, "no platform data specified\n");
		return -EINVAL;
	}

	lcd_info = decon_ext_parse_lcd_info(pd);
	...
	sfb->timeline = sw_sync_timeline_create("decon-ext");
	sfb->timeline_max = 1;
	/* XXX need to cleanup on errors */
#endif
	pm_runtime_enable(sfb->dev);

	sfb->regs = ioremap(dispdrv->decon_driver.regs->start, resource_size(dispdrv->decon_driver.regs));
	if (!sfb->regs) {
	...
	s3c_fb_irq, IRQF_SHARED, "s3c_fb", sfb);
	if (ret) {
	...
	for (win = 0; win < fbdrv->variant.nr_windows; win++) {
		...
		if (!pd->win[win]->win_mode.videomode.pixclock)
			decon_ext_fb_missing_pixclock(&pd->win[win]->win_mode, sfb->psr_mode);
	}
	...
	decon_reg_shadow_protect_win_ext(default_win, 1);

	s3c_fb_to_init_param(sfb, &p);
	decon_reg_init_ext(&p);

	decon_ext_set_regs_param(fbinfo, &win_regs);
	decon_reg_set_regs_data_ext(default_win, &win_regs);

	decon_reg_shadow_protect_win_ext(default_win, 0);

	s3c_fb_to_psr_info(sfb, &psr);
	s3c_fb_vidout_start(&psr);

	decon_reg_activate_window_ext(default_win);

	decon_reg_set_winmap_ext(default_win, 0x000000 /* black */, 1);
...
#ifdef CONFIG_FB_I80_COMMAND_MODE
	ret = decon_ext_fb_config_eint_for_te(sfb);
	if (ret) {
...
#ifdef CONFIG_FB_HIBERNATION_DISPLAY2
	dispdrv->decon_driver_ext.ops->pwr_on = decon_hibernation_power_on_ext;
	dispdrv->decon_driver_ext.ops->pwr_off = decon_hibernation_power_off_ext;
#endif
...
err_create_file:
#if defined(CONFIG_ION_EXYNOS) && defined(CONFIG_S5P_LCD_EXT_INIT)
...
static int s3c_fb_remove(struct platform_device *pdev)
{
	...
	dispdrv = get_display_driver_ext();
	sfb = dispdrv->decon_driver_ext.sfb;
...
static int s3c_fb_suspend(struct device *dev)
{
	...
	if (sfb->output_on) {
		...
		GET_DISPCTL_OPS(dispdrv).disable_display_decon_ext_clocks(sfb->dev);
...
static int s3c_fb_resume(struct device *dev)
{
	...
	disp_pm_runtime_get_sync_ext(dispdrv);
#endif
	GET_DISPCTL_OPS(dispdrv).init_display_decon_clocks_ext(sfb->dev);
...
#ifdef CONFIG_PM_RUNTIME
int s3c_fb_runtime_suspend_ext(struct device *dev)
{
	struct s3c_fb *sfb;
	struct display_driver *dispdrv;
	struct decon_psr_info psr;
	...
	save_decon_ext_operation_time(OPS_CALL_RUNTIME_SUSPEND);
	if (sfb->power_state != POWER_HIBER_DOWN)
		sfb->power_state = POWER_DOWN;
...
int s3c_fb_runtime_resume_ext(struct device *dev)
{
	struct s3c_fb *sfb;
	struct display_driver *dispdrv;
	struct s3c_fb_platdata *pd;
...
#endif
	save_decon_ext_operation_time(OPS_CALL_RUNTIME_RESUME);
	GET_DISPCTL_OPS(dispdrv).init_display_decon_clocks_ext(dev);
...
{
	int ret = 0;
	struct s3c_fb *sfb = dispdrv->decon_driver_ext.sfb;
	...
	decon_reg_set_vidout_ext(sfb->psr_mode, 1);
...
#endif
	decon_reg_configure_trigger_ext(sfb->trig_mode);

	if (sfb->trig_mode == DECON_HW_TRIG)
		decon_reg_set_trigger_ext(sfb->trig_mode, DECON_TRIG_DISABLE);

	/* use platform specified window as the basis for the lcd timings */
	decon_reg_configure_lcd_ext(sfb->lcd_info);
...
static const struct dev_pm_ops s3cfb_pm_ops = {
	...
	SET_RUNTIME_PM_OPS(s3c_fb_runtime_suspend_ext, s3c_fb_runtime_resume_ext,
			   NULL)
};
