...
static DEFINE_MUTEX(dsim_rd_wr_mutex_ext);
static DECLARE_COMPLETION(dsim_ph_wr_comp_ext);
static DECLARE_COMPLETION(dsim_wr_comp_ext);
static DECLARE_COMPLETION(dsim_rd_comp_ext);
...
struct mipi_dsim_device *dsim_for_decon_ext;
EXPORT_SYMBOL(dsim_for_decon_ext);


#ifdef CONFIG_FB_HIBERNATION_DISPLAY2
static int s5p_mipi_dsi_hibernation_power_on_ext(struct display_driver *dispdrv);
static int s5p_mipi_dsi_hibernation_power_off_ext(struct display_driver *dispdrv);
#endif


static int s5p_dsim_init_d_phy_ext(struct mipi_dsim_device *dsim, unsigned int enable)
{
#ifdef CONFIG_EXYNOS_DECON_DUAL_DISPLAY  /* Need to modify */
	void __iomem *addr;
	u32 data;
#endif
...
static void s5p_mipi_dsi_long_data_wr_ext(struct mipi_dsim_device *dsim, unsigned int data0, unsigned int data1)
{
	...
	for (data_cnt = 0; data_cnt < data1; data_cnt += 4) {
		...
		if ((data1 - data_cnt) < 4) {
			...
			dsim_reg_wr_tx_payload_ext(payload);
...
int s5p_mipi_dsi_wr_data_ext(struct mipi_dsim_device *dsim, unsigned int data_id,
	unsigned int data0, unsigned int data1)
{
	...
	mutex_lock(&dsim_rd_wr_mutex_ext);
	switch (data_id) {
	...
	case MIPI_DSI_SET_MAXIMUM_RETURN_PACKET_SIZE:
		INIT_COMPLETION(dsim_ph_wr_comp_ext);
		dsim_reg_clear_int_ext(DSIM_INTSRC_SFR_PH_FIFO_EMPTY);
		dsim_reg_wr_tx_header_ext(data_id, data0, data1);
		if (!wait_for_completion_interruptible_timeout(&dsim_ph_wr_comp_ext,
			MIPI_WR_TIMEOUT)) {
	...
	case MIPI_DSI_DCS_LONG_WRITE:
	{
		...
		size = data1 * 4;
		INIT_COMPLETION(dsim_wr_comp_ext);
		dsim_reg_clear_int_ext(DSIM_INTSRC_SFR_PL_FIFO_EMPTY);
		...
		/* in case that data count is more then 4 */
			...
			s5p_mipi_dsi_long_data_wr_ext(dsim, data0, data1);

		/* put data into header fifo */
		dsim_reg_wr_tx_header_ext(data_id, data1 & 0xff, (data1 & 0xff00) >> 8);
		if (!wait_for_completion_interruptible_timeout(&dsim_wr_comp_ext,
			MIPI_WR_TIMEOUT)) {
...
exit:
	if (dsim->enabled && (ret == -ETIMEDOUT)) {
		...
		dsim_reg_set_fifo_ctrl_ext(DSIM_FIFOCTRL_INIT_SFR);
	}

#ifdef CONFIG_FB_HIBERNATION_DISPLAY2
	disp_pm_gate_lock(dispdrv, false);
#endif
	mutex_unlock(&dsim_rd_wr_mutex_ext);
	return ret;
}

static void s5p_mipi_dsi_rx_err_handler_ext(struct mipi_dsim_device *dsim,
	u32 rx_fifo)
{
...
int s5p_mipi_dsi_rd_data_ext(struct mipi_dsim_device *dsim, u32 data_id,
	 u32 addr, u32 count, u8 *buf, u8 rxfifo_done)
{
	...
	INIT_COMPLETION(dsim_rd_comp_ext);

	/* Set the maximum packet size returned */
	s5p_mipi_dsi_wr_data_ext(dsim,
		MIPI_DSI_SET_MAXIMUM_RETURN_PACKET_SIZE, count, 0);

	/* Read request */
	s5p_mipi_dsi_wr_data_ext(dsim, data_id, addr, 0);
	if (!wait_for_completion_interruptible_timeout(&dsim_rd_comp_ext,
		MIPI_RD_TIMEOUT)) {
	...
	case MIPI_DSI_RX_ACKNOWLEDGE_AND_ERROR_REPORT:
		s5p_mipi_dsi_rx_err_handler_ext(dsim, rx_fifo);
		goto rx_error;
...
rx_error:
	dsim_reg_force_dphy_stop_state_ext(1);
	usleep_range(3000, 4000);
	dsim_reg_force_dphy_stop_state_ext(0);
	ret = -EPERM;
	goto exit;
...
void s5p_mipi_dsi_te_triggered_ext(void)
{
	struct display_driver *dispdrv = get_display_driver_ext();
	...
	dsim_reg_set_pkt_go_ready_ext();
}

void s5p_mipi_dsi_trigger_unmask_ext(void)
{
	struct display_driver *dispdrv = get_display_driver_ext();
	struct mipi_dsim_device *dsim = dispdrv->dsi_driver.dsim;
	...
	dsim_reg_set_pkt_go_cnt_ext(0xff);
	dsim_reg_set_pkt_go_enable_ext(true);
...
int s5p_mipi_dsi_partial_area_command_ext(struct mipi_dsim_device *dsim,
                                u32 x, u32 y, u32 w, u32 h)
{
	...
	if (x != dsim->lcd_win.x || w != dsim->lcd_win.w) {
		...
		data[4] = right & 0xff;
		retry = 2;
        while (s5p_mipi_dsi_wr_data_ext(dsim, MIPI_DSI_DCS_LONG_WRITE,
...
#endif
static void s5p_mipi_dsi_d_phy_onoff_ext(struct mipi_dsim_device *dsim,
	unsigned int enable)
{
	s5p_dsim_init_d_phy_ext(dsim, enable);
}

static irqreturn_t s5p_mipi_dsi_interrupt_handler_ext(int irq, void *dev_id)
{
...
static void dsim_device_to_clks_ext(struct mipi_dsim_device *dsim,
...
static void clks_to_dsim_device_ext(struct dsim_clks *clks,
...
int s5p_mipi_dsi_enable_ext(struct mipi_dsim_device *dsim)
{
	struct display_driver *dispdrv;
	struct dsim_clks clks;
	...
	GET_DISPDRV_OPS(dispdrv).init_display_dsi_clocks_ext(dsim->dev);
	GET_DISPDRV_OPS(dispdrv).enable_display_driver_power_ext(dsim->dev);

	if (dsim->dsim_lcd_drv->resume)
		dsim->dsim_lcd_drv->resume(dsim);

	/* DPHY power on */
	s5p_mipi_dsi_d_phy_onoff_ext(dsim, 1);

	dsim->state = DSIM_STATE_INIT;

	dsim_reg_init_ext(dsim->lcd_info, dsim->dsim_config->e_no_data_lane + 1);

	dsim_device_to_clks_ext(dsim, &clks);
	dsim_reg_set_clocks_ext(&clks, DSIM_LANE_CLOCK | dsim->data_lane, 1);
	clks_to_dsim_device_ext(&clks, dsim);

	dsim_reg_set_lanes_ext(DSIM_LANE_CLOCK | dsim->data_lane, 1);

	dsim->state = DSIM_STATE_STOP;

	GET_DISPDRV_OPS(dispdrv).reset_display_driver_panel_ext(dsim->dev);

	dsim_reg_set_hs_clock_ext(1);

	dsim->state = DSIM_STATE_HSCLKEN;
	/* enable interrupts */
	dsim_reg_set_int_ext(1);
...
int s5p_mipi_dsi_disable_ext(struct mipi_dsim_device *dsim)
{
	struct display_driver *dispdrv;
...
#ifdef CONFIG_DECON_MIPI_DSI_PKTGO
	dsim_reg_set_pkt_go_cnt_ext(0x0);
	dsim_reg_set_pkt_go_enable_ext(false);
	...
	/* disable interrupts */
	dsim_reg_set_int_ext(0);

	/* disable HS clock */
	dsim_reg_set_hs_clock_ext(0);

	/* make CLK/DATA Lane as LP00 */
	dsim_reg_set_lanes_ext(DSIM_LANE_CLOCK | dsim->data_lane, 0);

	dsim_reg_set_clocks_ext(NULL, DSIM_LANE_CLOCK | dsim->data_lane, 0);

	dsim_reg_sw_reset_ext();

	dsim->state = DSIM_STATE_SUSPEND;

	s5p_mipi_dsi_d_phy_onoff_ext(dsim, 0);

	GET_DISPDRV_OPS(dispdrv).disable_display_driver_power_ext(dsim->dev);
...
#ifdef CONFIG_FB_HIBERNATION_DISPLAY2
static int s5p_mipi_dsi_lcd_off_ext(struct mipi_dsim_device *dsim)
{
	struct display_driver *dispdrv;
	/* get a reference of the display driver */
	dispdrv = get_display_driver_ext();
	...
	dsim->enabled = false;
	dsim->dsim1_lcd_drv->suspend(dsim);
	dsim->state = DSIM_STATE_SUSPEND;
...
#ifdef CONFIG_FB_HIBERNATION_DISPLAY2
int s5p_mipi_dsi_hibernation_power_on_ext(struct display_driver *dispdrv)
{
	struct mipi_dsim_device *dsim = dispdrv->dsi_driver.dsim;
	struct dsim1_clks clks;
	...
	/* Enable PHY PLL */
	dsim1_reg_enable_pll(1);

	/* Exit ULPS mode clk & data */
	dsim1_reg_set_ulps(0);

	dsim->state = DSIM_STATE_INIT;

	/* DSIM init */
	dsim_reg_init_ext(dsim->lcd_info, dsim->dsim1_config->e_no_data_lane + 1);
...
int s5p_mipi_dsi_hibernation_power_off_ext(struct display_driver *dispdrv)
{
	struct mipi_dsim_device *dsim = dispdrv->dsi_driver.dsim;
...
int create_mipi_dsi_controller_ext(struct platform_device *pdev)
{
	...
	dsim->dsim_config = dispdrv->dt_ops.get_display_dsi_drvdata_ext();

	dsim->lcd_info = decon_get_lcd_info_ext();
	...
	if (request_irq(dsim->irq, s5p_mipi_dsi_interrupt_handler_ext,
	...
	mutex_init(&dsim_rd_wr_mutex_ext);
	...
	GET_DISPDRV_OPS(dispdrv).enable_display_driver_power_ext(&pdev->dev);
	...
	if (dispdrv->dsi_driver.ops) {
		dispdrv->dsi_driver.ops->pwr_on = s5p_mipi_dsi_hibernation_power_on_ext;
		dispdrv->dsi_driver.ops->pwr_off = s5p_mipi_dsi_hibernation_power_off_ext;
	}
#endif
