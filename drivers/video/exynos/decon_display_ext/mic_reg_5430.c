...
static void mic_reg_sw_reset(void)
{
	int tries;
...
static void mic_reg_set_image_size(struct decon_lcd *lcd)
{
	u32 val;
...
static void mic_reg_set_mic_base_operation(struct decon_lcd *lcd, bool enable)
{
...
static void mic_reg_set_update(u32 en)
{
	u32 val = en ? ~0 : 0;
	mic_write_mask(MIC_OP, val, MIC_OP_UPDATE_REG);
}

static void mic_reg_set_porch_timing(struct decon_lcd *lcd)
{
...
static void mic_reg_set_output_timing(struct decon_lcd *lcd)
{
