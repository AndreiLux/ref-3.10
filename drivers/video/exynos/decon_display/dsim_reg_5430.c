...
void dsim_reg_set_porch(struct decon_lcd *lcd)
{
	...
	if (lcd->mic)
		width = lcd->hsync_2h_cycle ? lcd->xres : (lcd->xres >> 1) + (lcd->xres % 4);
	else
		width = lcd->hsync_2h_cycle ? (lcd->xres << lcd->hsync_2h_cycle) : lcd->xres;

	height = (lcd->mic || lcd->hsync_2h_cycle) ? lcd->yres >> lcd->hsync_2h_cycle : lcd->yres;
