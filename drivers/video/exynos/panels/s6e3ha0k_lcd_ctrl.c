...
void lcd_init(struct decon_lcd *lcd)
{
	...
	if (dsim_wr_data(MIPI_DSI_DCS_LONG_WRITE, (u32)SEQ_MIPI_SINGLE_DSI_SET1,
	...
	if (dsim_wr_data(MIPI_DSI_DCS_SHORT_WRITE_PARAM,
				(u32)SEQ_MIPI_SINGLE_DSI_SET2[0],
				(u32)SEQ_MIPI_SINGLE_DSI_SET2[1]) < 0)
	...
	if (dsim_wr_data(MIPI_DSI_DCS_LONG_WRITE, (u32)SEQ_REG_FF,
				ARRAY_SIZE(SEQ_REG_FF)) < 0)
		dsim_err("fail to write SEQ_REG_FF command.\n");
	msleep(50);

	if (dsim_wr_data(MIPI_DSI_DCS_LONG_WRITE, (u32)SEQ_TEST_KEY_OFF_FC,
	...
	if (dsim_wr_data(MIPI_DSI_DCS_SHORT_WRITE_PARAM,
				(u32)SEQ_TOUCH_HSYNC_ON[0],
				(u32)SEQ_TOUCH_HSYNC_ON[1]) < 0)
		dsim_err("fail to write SEQ_TOUCH_HSYNC_ON command.\n");
	msleep(50);

	if (dsim_wr_data(MIPI_DSI_DCS_SHORT_WRITE_PARAM,
				(u32)SEQ_TOUCH_VSYNC_ON[0],
				(u32)SEQ_TOUCH_VSYNC_ON[1]) < 0)
