...
static void muic_mhl_cb(bool otg_enable, int plim)
{
	...
	if (otg_enable) {
		psy_do_property(POWER_SUPPLY_NAME, get,
					POWER_SUPPLY_PROP_ONLINE, value);
		if (value.intval == POWER_SUPPLY_TYPE_BATTERY ||
				value.intval == POWER_SUPPLY_TYPE_WIRELESS) {
			if (!lpcharge) {
				value.intval = true;
				psy_do_property(POWER_SUPPLY_NAME, set,
		...
		psy_do_property(POWER_SUPPLY_NAME, set, POWER_SUPPLY_PROP_CHARGE_OTG_CONTROL, value);
	}
...
#ifdef CONFIG_SOC_EXYNOS5430 /* KQ-Helsinki MHL-Regulator */
