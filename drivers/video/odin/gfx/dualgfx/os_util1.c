...
/*--------------------------------------------------------------------
	Macro Definitions
---------------------------------------------------------------------*/
static struct task_struct *g_csowner1 = NULL;
static struct class *kdrv_class1 = NULL;

#define SET_CRITICAL1() 		do { g_csowner1 = current; } while (0)
#define CLEAR_CRITICAL() 	do { g_csowner1 = NULL; } while (0)
#define CHECK_CRITICAL() 	( g_csowner1 == current || in_interrupt() )

/*
 * critical resource lock/unlock
 */

#define OS_GLOBAL_LOCK1()	\
	do { spin_lock_irqsave(&_g_res_lock1, flags);} while (0)
#define OS_GLOBAL_UNLOCK1()	\
	do { spin_unlock_irqrestore(&_g_res_lock1, flags); } while (0)
...
static	spinlock_t		_g_res_lock1 = SPIN_LOCK_UNLOCKED;
#else
static DEFINE_SPINLOCK(_g_res_lock1);
#endif
...
/*--------------------------------------------------------------------
	DEVICE REGISTER
---------------------------------------------------------------------*/
void os_createdeviceclass1    ( dev_t dev, const char* fmt, ... )
{
	va_list args;
	...
	device_create( kdrv_class1, NULL, dev, NULL, dev_name );
}
EXPORT_SYMBOL(os_createdeviceclass1);

void os_destroydeviceclass1   ( dev_t dev )
{
	device_destroy( kdrv_class1, dev );
}

/*--------------------------------------------------------------------
	MUTEX OS API IMPLEMENTATION
---------------------------------------------------------------------*/
void os_initmutex_tag1	( OS_SEM_T* pSem , UINT32 attr )
{
	PARAM_UNUSED(attr);
...
int os_lockmutex_tag1	( OS_SEM_T* pSem, UINT32 timeout,
					const char* szFile, const char* szFunc, const int nLine )
{
	...
	{
		printk("Ooops. OS_LockMutex1 called at ISR. force timeout from \
			%d to 0.\n", timeout );

		timeout = 0;
	}

	/* normally, ISR should call OS_LockMutex1 with zero timeout */
	if ( timeout == 0x0 )
	{
		...
		if ( RET_OK != ret )
		{
			printk("OS_LockMutex1 timeout. ret=%d\n", ret );
		}
	}
	/* normal process seems to call OS_LockMutex1 with timoeut option */
	else if ( timeout == OS_INFINITE_WAIT )
	{
		...
		if ( RET_OK != ret )
		{
			printk("OS_LockMutex1 failed. ret=%d\n", ret );
		}
	}
	else
	/* if timeout option is enabled, */
	{
		...
		for ( ; ; )
		{
			...
			else
			{
				...
				printk("OS_LockMutex1 : timeout interrupt (ticks:%d)\n",
					(int)ticks );
				if ( signal_pending(current) )
				{
					printk("OS_LockMutex1 signal interrrupt.\n" );
...
/*		printk("OS_LockMutex1 (%p): done (tm:%d)\n", pSem, timeout); */
	}
 ...
 */
void os_unlockmutex_tag1 ( OS_SEM_T* pSem,
				const char* szFile, const char* szFunc, const int nLine )
{
	ULONG	flags;
