...
--------------------------------------------------------------------*/
extern void gfx_odin_inithal1 ( GFX_HAL_T1 *  hal );
...
/*-------------------------------------------------------------------
	global Variables
--------------------------------------------------------------------*/
static int	g_gfx_major = 152;
static int	g_gfx_minor = 0;
/* int	g_gfx_debug_fd;	*/

LX_GFX_CFG_T	g_gfx_cfg1;
GFX_HAL_T1		g_gfx_hal1;

/*-------------------------------------------------------------------
	Static Function Prototypes Declarations
--------------------------------------------------------------------*/
static int gfx_open1(struct inode *inode, struct file *filp);
static int gfx_close1(struct inode *inode, struct file *filp);

static long gfx_ioctl1(struct file *filp, u32 cmd, ULONG arg );
/* #endif */
...
/*-------------------------------------------------------------------
	Static Variables
--------------------------------------------------------------------*/
static OS_SEM_T	gfx_dev_mtx1;

/**
 * main control block for gfx device
*/
static GFX_DEVICE_T *g_gfx_device1;

/**
 * file I/O description for gfx device
 *
*/
static struct file_operations g_gfx_fops1 =
{
	.open 	= gfx_open1,
	.release= gfx_close1,
	.unlocked_ioctl = gfx_ioctl1,
	.mmap	= gfx_mmap,
};
...
/** lock GFX device */
void gfx_lockdevice1(void)
{
	OS_LockMutex1( &gfx_dev_mtx1 );
}

/** unlock GFX device */
void gfx_unlockdevice1  (void)
{
	OS_UnlockMutex1( &gfx_dev_mtx1 );
}
 ...
 *
 */
static void gfx_inithal1 ( void )
{
    memset( &g_gfx_hal1, 0x0, sizeof(GFX_HAL_T1));

    /* initialize HAL (Hardware Abstraction Layer) */
	gfx_odin_inithal1( &g_gfx_hal1 );

}

/** initialize GFX environment before the real initialization
 *
 *	@note gfx_inithal1 should be called before calling any other functions.
 *	@note gfx_initcfg should be called before calling any other functions.
 */
void gfx_preinit1(void)
{
	gfx_inithal1( );
	gfx_initcfg1( );
}
...
#ifdef CONFIG_OF
static const struct of_device_id odin_gfx_match1[] = {
	{
		.compatible	= "odin-gfx1",
	},
};
#endif
static void gfx_release(struct device *dev);

const struct file_operations gfx_fops1 = {
	.owner 			= THIS_MODULE,
	.open 			= gfx_open1,
	.release 			= gfx_close1,
	.unlocked_ioctl 	= gfx_ioctl1,
...
struct miscdevice gfx_misc_device1 = {
 ...
 *
 */
 int /*__init*/ gfx_probe1(struct platform_device *pdev)
{
	int r;
	GFX_DRVDATA_T *drv_data;
	...
	r  = misc_register(&gfx_misc_device1);
	if (r ) {
		printk("gfx1 : failed to register misc device.\n");
		return r;
	}
...
static int  gfx_remove(struct platform_device *pdev)
{
	...
	drv_data = platform_get_drvdata(pdev);
	misc_deregister(&gfx_misc_device1);
	kfree(drv_data);
 ...
 */
static struct platform_driver odin_gfx_driver1 =
{
	.probe          = gfx_probe1,
	.suspend        = gfx_suspend,
	.remove         = gfx_remove,
	...
	.driver         =
	{
		.name   = GFX1_MODULE,
#ifdef CONFIG_OF
		.of_match_table = of_match_ptr(odin_gfx_match1),
#endif
	},
};
...
int gfx_init1(void)
{
	 ...
	 */
#if 0
	g_gfx_debug_fd		= DBG_OPEN( GFX1_MODULE );
...
/*	OS_DEBUG_EnableModuleByIndex ( g_gfx_debug_fd, 3, DBG_COLOR_RED ); */
	...
	if (platform_driver_register(&odin_gfx_driver1) < 0)
	{
	...
	else
	{
		...
		{
			platform_driver_unregister(&odin_gfx_driver1);
	...
	memset( g_gfx_device1, 0x0, sizeof(GFX_DEVICE_T)*GFX_MAX_DEVICE );

	if (g_gfx_major)
	{
		dev = MKDEV( g_gfx_major, g_gfx_minor);
		err = register_chrdev_region(dev, GFX_MAX_DEVICE, GFX1_MODULE );
	}
	else
	{
		err = alloc_chrdev_region(&dev, g_gfx_minor, GFX_MAX_DEVICE, GFX1_MODULE );
	...
	OS_InitMutex1 ( &gfx_dev_mtx1, OS_SEM_ATTR_DEFAULT );
	...
	gfx_inithw1();
   	...
   	gfx_initsurfacememory1();
   		/* initialize dynamic surface memory */
    gfx_setcommanddelay1(0x0);
	...
	for ( i=0; i<GFX_MAX_DEVICE; i++ )
	{
		...
		dev = MKDEV( g_gfx_major, g_gfx_minor );
		cdev_init( &(g_gfx_device1[i].cdev), &g_gfx_fops1 );
		g_gfx_device1[i].devno = dev;
		g_gfx_device1[i].cdev.owner = THIS_MODULE;
		g_gfx_device1[i].cdev.ops = &g_gfx_fops1;

		/* TODO: initialize minor device */

		/* END */
		err = cdev_add (&(g_gfx_device1[i].cdev), dev, 1 );
		...
		os_createdeviceclass1 ( g_gfx_device1[i].devno, "%s%d", GFX1_MODULE, i );
	}
	...
	GFXINFO("gfx1 device is initialized\n");
 ...
 */
static int
gfx_open1(struct inode *inode, struct file *filp)
{
 ...
 *
 */
static int gfx_close1(struct inode *inode, struct file *file)
{
    int	major,minor;
	GFX_DEV_CTX_T *dev_ctx;
    GFX_DEVICE_T *my_dev;
...
/* static int gfx_ioctl1 ( struct inode* inode, struct file *filp,
 * u32 cmd, ULONG arg ) */
/* #else */
static long gfx_ioctl1 ( struct file* filp, u32 cmd, ULONG arg )
/* #endif */
...
/*	GFXDBG("%s : IOC 0x%08x (%s:%d)\n", __F__, cmd, gfx_ioc2str(cmd),
	...
	switch (cmd)
	{
		...
		case GFX_IOR_CHIP_REV_INFO:
		{
			...
			err = copy_to_user((void __user *)arg, &rev_info,
				sizeof(CHIP_REV_INFO_T));
		...
		case GFX_IOR_GET_CFG:
		{
			err = copy_to_user((void __user *)arg, (void *)&g_gfx_cfg1 ,
				sizeof(LX_GFX_CFG_T));
            GFX_CHECK_ERROR( err > 0, goto func_exit, "copy error\n");
		...
		case GFX_IORW_ALLOC_SURFACE :
		{
			...
			gfx_lockdevice1();
			err = gfx_allocsurface1(&param);
			gfx_unlockdevice1();

			GFX_CHECK_ERROR( RET_OK != err, goto func_exit,
				"gfx_allocsurface1 failed\n" );
		...
		case GFX_IOW_FREE_SURFACE :
		{
			...
			err = copy_from_user((void *)&surface_fd, (void __user *)arg,
				sizeof(u32));
            GFX_CHECK_ERROR( err > 0, goto func_exit, "copy error\n");

			gfx_lockdevice1();
			err = gfx_freesurface1(surface_fd);
			gfx_unlockdevice1();
			GFX_CHECK_ERROR( RET_OK != err, goto func_exit,
				"[DGFX] gfx_freesurface1 failed\n");
		...
		case GFX_IOW_SET_SURFACE_PALETTE :
		{
			...
			gfx_lockdevice1();
			gfx_setsurfacepalette1( param.surface_fd, param.palette.palette_num,
		...
		case GFX_IOR_GET_SURFACE_PALETTE :
		{
			...
			gfx_lockdevice1();
			gfx_getsurfacepalette1( param.surface_fd, param.palette.palette_num,
				param.palette.palette_data );
			gfx_unlockdevice1();
		...
		case GFX_IOW_MANUAL_BLEND :
		{
			...
			for ( retry=g_gfx_cfg1.sync_fail_retry_count ; retry>0 ; retry-- )
			{
				err = gfx_runblendop1(&manual_blend);

			    /* when batch mode is auto, driver automatically
			    	flush queue (start batch run) */
			    if (gfx_getruncommand1() == LX_GFX_BATCH_RUN_MODE_AUTO)
				{
					gfx_runflushcommand1();
				}
				 ...
				 */
				if ( gfx_getgraphicsyncmode1() == LX_GFX_GRAPHIC_SYNC_MODE_AUTO )
				{
					u32	timeout = gfx_calcsynctimeout1( &manual_blend,
						g_gfx_cfg1.sync_wait_timeout );
					err = gfx_waitsynccommand (timeout);
					err = gfx_waitsynccommand1( timeout );
			/*		printk("GFX1 is done\n" ); */
#if 0
					if ( GFX_WADesc1(scaler_read_buf_stuck)
				...
				gfx_dumpblitparam1 ( &manual_blend );
#endif
			}
			gfx_unlockdevice1();
		...
		case GFX_IOW_SET_CSC_CONV_TBL:
		{
			...
			err = gfx_setcolorspace1(csc_conv.coef);
            GFX_CHECK_CODE( err != RET_OK, goto func_exit,
            	"gfx_setcolorspace1 error\n");
		...
		case GFX_IOR_GET_CMD_DELAY :
		{
			...
			gfx_getcommanddelay1(&cmd_delay);
		...
		case GFX_IOW_SET_CMD_DELAY :
		{
			...
			gfx_setcommanddelay1(cmd_delay);
		...
		case GFX_IORW_GET_SURFACE_MEM_INFO:
		{
			...
			gfx_lockdevice1();
			err = gfx_getsurfacememory1(&mem_info);
			gfx_unlockdevice1();
            GFX_CHECK_CODE( err != RET_OK, goto func_exit,
            	"gfx_getsurfacememory1 error\n");
		...
		case GFX_IOW_SET_BATCH_RUN_MODE:
		{
			...
			err = gfx_setruncommand1(&run_mode);
            GFX_CHECK_CODE( err != RET_OK, goto func_exit,
            	"gfx_setruncommand1 error\n");
		...
		{
			err = gfx_runflushcommand1();
            GFX_CHECK_CODE( err != RET_OK, goto func_exit,
            	"gfx_runflushcommand1 error\n");
		...
		case GFX_IOW_SET_GRAPHIC_SYNC_MODE:
		{
			...
			err = gfx_setgraphicsyncmode1(mode);
            GFX_CHECK_CODE( err != RET_OK, goto func_exit,
            	"gfx_setgraphicsyncmode1 error\n");
		...
		case GFX_IOW_WAIT_FOR_SYNC:
		{
			...
			err = gfx_waitsynccommand1(msec_tm);
            GFX_CHECK_CODE( err != RET_OK, goto func_exit,
            	"gfx_waitsynccommand1 error\n");
		...
		case GFX_IOR_GET_MEM_STAT:
		{
			...
			(void)gfx_getsurfacememorystat1( &mem_stat );
...
module_init(gfx_init1);
module_exit(gfx_cleanup1);

MODULE_AUTHOR("");
MODULE_DESCRIPTION("GFX1 base driver");
