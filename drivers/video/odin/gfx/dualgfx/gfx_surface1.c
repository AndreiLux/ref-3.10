...
/*--------------------------------------------------------------------
	global Variables
---------------------------------------------------------------------*/
GFX_SURFACE_OBJ_T *g_gfx_surf_list1;
...
/*--------------------------------------------------------------------
	Static Variables
---------------------------------------------------------------------*/
static u32	g_cr_idx1;	/* create index */
static LX_GFX_PHYS_MEM_INFO_T	g_gfx_mem_pool_info1;
 ...
 *
 */
int gfx_initsurfacememory1(void)
{
	int	ret =0;
	g_cr_idx1 = 0;
	...
	g_gfx_mem_pool_info1.phys_addr 	= (void*)gmemcfggfx1.surface.base;
	g_gfx_mem_pool_info1.length	 	= gmemcfggfx1.surface.size;
	g_gfx_mem_pool_info1.offset	 	= 0;

	GFX_TRACE_BEGIN();
	g_gfx_surf_list1 = (GFX_SURFACE_OBJ_T *)
		vmalloc(sizeof(GFX_SURFACE_OBJ_T)*GFX_MAX_SURFACE);
	GFX_CHECK_ERROR( NULL == g_gfx_surf_list1, return RET_ERROR,
		"can't alloc surf_list\n");

	memset((void *)g_gfx_surf_list1, 0x0 ,
 ...
 *
 */
int gfx_allocsurface1(LX_GFX_SURFACE_SETTING_PARAM_T *surface)
{
	int i;
	int slot = -1;
	int ret = RET_ERROR;
	...
	if ( (ULONG)surface->surface_setting.phys_addr ) { i = 0x10; }

	/* search empty slot */
	for ( i = 0x10 ; i < GFX_MAX_SURFACE; i++ )
	{
		if ( !g_gfx_surf_list1[i].balloc ) { slot = i; break; }
	}
	...
	g_gfx_surf_list1[slot].balloc = 1;
	g_gfx_surf_list1[slot].surf.surface_fd = slot;
	g_gfx_surf_list1[slot].surf.type = surface->surface_setting.type;
	g_gfx_surf_list1[slot].surf.pixel_format =
				surface->surface_setting.pixel_format;
	g_gfx_surf_list1[slot].surf.width = surface->surface_setting.width;
	g_gfx_surf_list1[slot].surf.height = surface->surface_setting.height;
	g_gfx_surf_list1[slot].surf.alignment = surface->surface_setting.alignment;
	g_gfx_surf_list1[slot].cidx = g_cr_idx1++;
	g_gfx_surf_list1[slot].ctick = OS_GETMSECTICKS();

	/* get task name */
	get_task_comm( g_gfx_surf_list1[slot].psname, current );
	g_gfx_surf_list1[slot].psname[TASK_COMM_LEN] = 0x0;

	GFX_TRACE("%s:%d -- fd %d, dim %dx%d\n", __F__, __L__, slot,
		g_gfx_surf_list1[slot].surf.width, g_gfx_surf_list1[slot].surf.height );
	...
	if (surface->surface_setting.stride == 0)
	{
		switch (surface->surface_setting.pixel_format)
		{
			case LX_GFX_PIXEL_FORMAT_INDEX_0:
				g_gfx_surf_list1[slot].surf.stride = 0;
				break;

			case LX_GFX_PIXEL_FORMAT_INDEX_1:
				g_gfx_surf_list1[slot].surf.stride =
	...
	{
		g_gfx_surf_list1[slot].surf.stride = surface->surface_setting.stride;
	}
	...
	{
		g_gfx_surf_list1[slot].surf.phys_addr=
			surface->surface_setting.phys_addr;
		g_gfx_surf_list1[slot].mem.phys_addr =
			surface->surface_setting.phys_addr;
		g_gfx_surf_list1[slot].mem.length =
			surface->surface_setting.stride * surface->surface_setting.height;
		g_gfx_surf_list1[slot].mem.offset =
			(UINT32)surface->surface_setting.phys_addr -
			(UINT32)g_gfx_mem_pool_info1.phys_addr;
...
/*		ret = GFX_AllocSurfaceMem(surface->surface_fd); */
	...
	GFX_CHECK_ERROR( ret != RET_OK, g_gfx_surf_list1[slot].balloc = 0;
		goto func_exit, "fail to alloc surface (%d)\n", surface->surface_fd );

	g_gfx_surf_list1[slot].bpalette = 0;

	/* return surface_settings data */
	memcpy( &surface->surface_setting, &g_gfx_surf_list1[slot].surf,
...
func_exit:
	if ( ret != RET_OK )
	{
		...
		(void)gfx_getsurfacememorystat1( &gfx_mem_stat );
		...
		{
			if ( g_gfx_surf_list1[i].balloc ) surface_cnt++;
		}
 ...
 * @return	int	0 : OK , -1 : NOT OK
 */
int gfx_freesurface1(UINT32 surface_fd)
{
	UINT32 phys_addr;
	int ret = RET_ERROR;
	GFX_TRACE_BEGIN();
	...
	GFX_CHECK_ERROR( !g_gfx_surf_list1[surface_fd].balloc, goto func_exit,
		"[DGFX] not allocated fd 0x%x(%d)\n", surface_fd, surface_fd );

	phys_addr = (UINT32)g_gfx_surf_list1[surface_fd].surf.phys_addr;

	/* since surface is allocated with the external memory,
		we should check phys_addr range */
	if ( g_gfx_surf_list1[surface_fd].surf.type == LX_GFX_SURFACE_TYPE_MEM_BUFFER
		&& ( phys_addr >= (UINT32)g_gfx_mem_pool_info1.phys_addr
		&& phys_addr < (UINT32)g_gfx_mem_pool_info1.phys_addr +
		g_gfx_mem_pool_info1.length ) )
	{
/*       GFX_FreeSurfaceMem(surface_fd); */
	 ...
	 */
	if ( g_gfx_surf_list1[surface_fd].pal )
	{
		OS_Free( g_gfx_surf_list1[surface_fd].pal );
	}

	memset( &g_gfx_surf_list1[surface_fd], 0x0, sizeof(GFX_SURFACE_OBJ_T) );
 ...
 *
 */
int gfx_getsurfacememory1(LX_GFX_SURFACE_MEM_INFO_PARAM_T *mem)
{
	GFX_TRACE_BEGIN();
#if 1
	memcpy( &mem->surface_mem, &(g_gfx_surf_list1[mem->surface_fd].mem),
	sizeof(LX_GFX_PHYS_MEM_INFO_T));
#else
	mem->surface_mem.phys_addr  = g_gfx_surf_list1[mem->surface_fd].mem.phys_addr;
	mem->surface_mem.offset     = g_gfx_surf_list1[mem->surface_fd].mem.offset;
	mem->surface_mem.length     = g_gfx_surf_list1[mem->surface_fd].mem.length;
#endif
 ...
 *
 */
int gfx_setsurfacepalette1(int surface_fd, int size , UINT32 *data)
{
	int ret = RET_OK;

	GFX_TRACE_BEGIN();

	if ( (g_gfx_surf_list1[surface_fd].surf.width == 0) ||
		(g_gfx_surf_list1[surface_fd].surf.height == 0) )
	{
	...
	if ( !g_gfx_surf_list1[surface_fd].pal )
	{
		g_gfx_surf_list1[surface_fd].pal = (UINT32*)OS_Malloc( sizeof(UINT32)*256 );
	}

	GFX_CHECK_ERROR( !g_gfx_surf_list1[surface_fd].pal, goto func_exit,
		"out of resource. check memory usage\n");

	memcpy( (void *)g_gfx_surf_list1[surface_fd].pal , (void *)data ,
			sizeof(UINT32)*size);
	 ...
	 */
	g_gfx_surf_list1[surface_fd].bpalette 	= 1;
	g_gfx_surf_list1[surface_fd].bpaldownload= 0;
	g_gfx_surf_list1[surface_fd].palsize 	= size;

	GFXINFO("%s:%d -- fd %d, palSize %d, palData %p\n", __F__, __L__,
				surface_fd, g_gfx_surf_list1[surface_fd].palsize,
				g_gfx_surf_list1[surface_fd].pal );
func_exit:
	GFX_TRACE_END();
 ...
 *
 */
int gfx_getsurfacepalette1(int surface_fd, int size , UINT32 *data)
{
	int ret = RET_OK;
	...
	if ( (g_gfx_surf_list1[surface_fd].surf.width == 0) ||
		(g_gfx_surf_list1[surface_fd].surf.height == 0) ||
		(g_gfx_surf_list1[surface_fd].pal == NULL ) )
	{
		GFXERR("ERROR : tried to save palette data on free surface [%d]\n",
			surface_fd);
		GFX_TRACE_END();
	...
	memcpy( (void*)data, (void *)g_gfx_surf_list1[surface_fd].pal,
		sizeof(UINT32)*size);
 ...
 *
 */
int gfx_getsurfacememorystat1 ( LX_GFX_MEM_STAT_T* pMemStat )
{
	int	i;
	int	surf_cnt = 0;
...
/*	pMemStat->surface_alloc_num		= mem_info.mem_alloc_cnt; */
	...
	for ( i=0; i< GFX_MAX_SURFACE; i++ )
	{
		if ( g_gfx_surf_list1[i].balloc ) surf_cnt++;
	}
