...
struct odin_dsi_cmd lglh520wf1_dsi_flip_command[] = {
	DSI_CMD_SHORT(DSI_SRC_DATA_ID_DCS_SHORT_WRITE_1_PARA, 0x36, 0x00),
};

struct odin_dsi_cmd lglh520wf1_dsi_off_command[] = {
	...
	DSI_DLY_MS(17*9),
/*	DSI_CMD_SHORT(DSI_SRC_DATA_ID_DCS_SHORT_WRITE_1_PARA, 0xB1, 0x01),*/
};


struct odin_dsi_cmd lglh520wf1_dsi_init_cmd[] = {
/* AFTER TUNING */
/* Display Initial Set*/
	...
	DSI_DLY_MS(100),
	/* Manufacturer Command Access Protest */
...
#if defined(CONFIG_LGE_R63417_COLOR_ENGINE)
...
static int lglh520wf1_dsi_enable(struct odin_dss_device *dssdev);
static void lglh520wf1_dsi_disable(struct odin_dss_device *dssdev);
static int lglh520wf1_dsi_resume(struct odin_dss_device *dssdev);
static int lglh520wf1_dsi_suspend(struct odin_dss_device *dssdev);

static void lglh520wf1_dsi_get_timings(struct odin_dss_device *dssdev,
			struct odin_video_timings *timings)
{
...
#if 0
int lglh520wf1_dsi_panel_on(
	enum odin_mipi_dsi_index resource_index, struct odin_dss_device *dssdev);
#endif

static ssize_t lglh520wf1_dsi_enable_test(struct device *dev,
					 ...
					 const char *buf, size_t count)
{
	...
	if (on_off==1)
	{
		lglh520wf1_dsi_enable(dssdev);
		odin_set_rd_enable_plane(dssdev->channel, 1);
	}
	else
	{
		odin_set_rd_enable_plane(dssdev->channel, 0);
		lglh520wf1_dsi_disable(dssdev);
	}

	return count;
}

static ssize_t lglh520wf1_dsi_resume_test(struct device *dev,
					 struct device_attribute *attr,
					 const char *buf, size_t count)
{
	int on_off;
	...
	if (on_off==1)
	{
		lglh520wf1_dsi_resume(dssdev);
		/*odin_set_rd_enable_plane(dssdev->channel, 1);*/
	}
	else
	{
		/*odin_set_rd_enable_plane(dssdev->channel, 0);*/
		lglh520wf1_dsi_suspend(dssdev);
	}

	return count;
}
void aa_function(int enable)
{
   /* int ret; */
   return;
...
static int lglh520wf1_update(
	struct odin_dss_device *dssdev, u16 x, u16 y, u16 h, u16 w){

	struct lglh520wf1_dsi_data *data = dev_get_drvdata(&dssdev->dev);

	mutex_lock(&data->lock);
	odin_mipi_hs_command_update(0, dssdev);
	mutex_unlock(&data->lock);

	return 0;
}

static void lglh520wf1_power_on(struct odin_dss_device *dssdev)
{
	int ret;
...
void lglh520wf1_power_off(struct odin_dss_device *dssdev)
{
	gpio_set_value(LCD_RESET_N , 0);
	mdelay(2);
	...
	enum odin_mipi_dsi_index resource_index, struct odin_dss_device *dssdev)
{
	...
	lglh520wf1_dsi_flip_command[0].sp_len_dly.sp.data1 = flip_data;

	if (flip_data)
		odin_mipi_dsi_cmd(resource_index, lglh520wf1_dsi_flip_command,
					ARRAY_SIZE(lglh520wf1_dsi_flip_command));

	odin_mipi_dsi_cmd(resource_index, lglh520wf1_dsi_init_cmd,
					ARRAY_SIZE(lglh520wf1_dsi_init_cmd));

	return 0;
}

int lglh520wf1_panel_off(
	enum odin_mipi_dsi_index resource_index, struct odin_dss_device *dssdev)
{
	odin_mipi_hs_video_mode_stop(resource_index, dssdev);

	odin_mipi_dsi_cmd(resource_index, lglh520wf1_dsi_off_command,
				ARRAY_SIZE(lglh520wf1_dsi_off_command));

	lglh520wf1_power_off(dssdev);
...
static int lglh520wf1_dsi_enable(struct odin_dss_device *dssdev)
{
	struct lglh520wf1_dsi_data *td = dev_get_drvdata(&dssdev->dev);
	int r = 0;
	...
	r = odin_du_power_on(dssdev); /* default: gra0 clk enable */
	if (r) {
		dev_err(&dssdev->dev, "failed to power on du\n");
		goto err;
	}

	r = lglh520wf1_panel_on(dssdev->channel, dssdev);
...
static void lglh520wf1_dsi_disable(struct odin_dss_device *dssdev)
{
	...
	if (dssdev->state == ODIN_DSS_DISPLAY_ACTIVE) {
		...
		r =lglh520wf1_panel_off(dssdev->channel, dssdev);
		if (r) {
...
static DEVICE_ATTR(panel_enable, S_IWUSR,
		NULL, lglh520wf1_dsi_enable_test);
static DEVICE_ATTR(panel_resume, S_IWUSR,
		NULL, lglh520wf1_dsi_resume_test);

static struct attribute *lglh520wf1_attrs[] = {
	&dev_attr_panel_enable.attr,
	&dev_attr_panel_resume.attr,
	NULL,
};

static struct attribute_group lglh520wf1_attr_group = {
	.attrs = lglh520wf1_attrs,
};

static int lglh520wf1_dsi_probe(struct odin_dss_device *dssdev)
{
	int r;
	struct lglh520wf1_dsi_data *td;
	...
	r = sysfs_create_group(&dssdev->dev.kobj, &lglh520wf1_attr_group);
	if (r) {
...
err_device_create_file:
	sysfs_remove_group(&dssdev->dev.kobj, &lglh520wf1_attr_group);
...
static void __exit lglh520wf1_dsi_remove(struct odin_dss_device *dssdev)
{

	/* reset, to be sure that the panel is in a valid state */
	/*lglh520wf1_dsi_hw_reset(dssdev);*/
}

#if 0
static void lglh520wf1_dsi_power_off(struct odin_dss_device *dssdev)
{

}

static int lglh520wf1_dsi_panel_reset(struct odin_dss_device *dssdev)
{
	dev_err(&dssdev->dev, "performing LCD reset\n");

	lglh520wf1_dsi_power_off(dssdev);
	/*lglh520wf1_dsi_hw_reset(dssdev);*/
	return lglh520wf1_dsi_panel_on(dssdev->channel, dssdev);
}
#endif

static void lglh520wf1_dsi_get_resolution(
...
#if 0
static void lglh520wf1_dsi_ulps_work(struct work_struct *work)
{

}

static void lglh520wf1_dsi_esd_work(struct work_struct *work)
{

}
#endif

static int lglh520wf1_dsi_set_update_mode(struct odin_dss_device *dssdev,
...
static enum odin_dss_update_mode lglh520wf1_dsi_get_update_mode(
		struct odin_dss_device *dssdev)
{
	if (dssdev->panel.dsi_mode == ODIN_DSS_DSI_CMD_MODE)
...
static int lglh520wf1_dsi_runtime_suspend(struct device *dev)
{
	struct lglh520wf1_dsi_data *td = dev_get_drvdata(dev);

	dev_info(dev, "lglh520wf1_dsi_runtime_suspend\n");

	pm_qos_update_request(&td->mem_qos, 0);
	return 0;
}

static int lglh520wf1_dsi_runtime_resume(struct device *dev)
{
	struct lglh520wf1_dsi_data *td = dev_get_drvdata(dev);

	dev_info(dev, "lglh520wf1_dsi_runtime_resume\n");
...
static const struct dev_pm_ops lglh520wf1_dsi_pm_ops = {
	.runtime_suspend = lglh520wf1_dsi_runtime_suspend,
	.runtime_resume	 = lglh520wf1_dsi_runtime_resume,
};

static struct odin_dss_driver lglh520wf1_dsi_driver = {
	.probe		= lglh520wf1_dsi_probe,
	.remove		= __exit_p(lglh520wf1_dsi_remove),

	.enable		= lglh520wf1_dsi_enable,
	.disable	= lglh520wf1_dsi_disable,
	.suspend	= lglh520wf1_dsi_suspend,
	.resume		= lglh520wf1_dsi_resume,
	.update		= lglh520wf1_update,
	.set_update_mode = lglh520wf1_dsi_set_update_mode,
	.get_update_mode = lglh520wf1_dsi_get_update_mode,
	.get_resolution	= lglh520wf1_dsi_get_resolution,
	.get_recommended_bpp = odindss_default_get_recommended_bpp,

	.get_timings	= lglh520wf1_dsi_get_timings,

	.driver         = {
		.name   = "dsi_lglh520wf1_panel",
		.owner  = THIS_MODULE,
		.pm = &lglh520wf1_dsi_pm_ops,
	},
};

static int __init lglh520wf1_dsi_init(void)
{
	odin_dss_register_driver(&lglh520wf1_dsi_driver);

	return 0;
}

static void __exit lglh520wf1_dsi_exit(void)
{
	odin_dss_unregister_driver(&lglh520wf1_dsi_driver);
}

module_init(lglh520wf1_dsi_init);
module_exit(lglh520wf1_dsi_exit);
