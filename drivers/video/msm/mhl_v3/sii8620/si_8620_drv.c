...
#define ENABLE_FP	VAL_VID_OVRRD_3DCONV_EN_FRAME_PACK
	...
	pr_info("input_quant_range1 = [%02x]\n", input_quant_range );
#ifdef CONFIG_MHL3_SEC_FEATURE
...
/* begin BZ 32674 ( */
			...
			if (aqr_default != input_quant_range) {
				...
				pr_info("input_quant_range2 = [%02x]\n", input_quant_range );
#if 0
...
#ifdef CONFIG_MHL3_SEC_FEATURE
uint8_t *g_EDID_block_data;
uint8_t *mhl_get_edid(void)
{
	pr_info("sii8620:%s:%d: get_edid\n", __func__, __LINE__);
...
#ifdef CONFIG_MHL3_SEC_FEATURE
	/* Transmit edid info to the hdmi driver and turn on the hdmi block. -YHM*/
	g_EDID_block_data = edid;
