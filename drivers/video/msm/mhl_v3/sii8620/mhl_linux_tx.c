...
int sii8620_detection_callback(unsigned long event)
{
	...
	pr_info("sii8620:%s:%d: sii8620_detection_callback : event=%lu*****\n",
			__func__, __LINE__, event);

	if (dev_context->muic_state == event)
	{
		pr_info("sii8620:%s: already %s\n", __func__, event ? "attached" : "detached");
		return 0;
	}
...
#else
	if (event) {
		pr_info("sii8620:%s:%d: MHL-Connection\n", __func__, __LINE__);
		clk_prepare_enable(mhl_clk);
		dev_context->pdata->power(true);
		si_mhl_tx_initialize(dev_context);
		...
		pr_info("sii8620:%s:%d: MHL-Disconnection\n", __func__, __LINE__);
		clk_disable_unprepare(mhl_clk);
		disable_irq(dev_context->drv_info->irq);
		dev_context->pdata->charger_mhl_cb(false, -1);
		dev_context->pdata->power(false);
...
#else
	if (event) {
		pr_info("sii8620:detection started\n");
		dev_context->pdata->power(true);
		si_mhl_tx_initialize(dev_context);
		enable_irq(dev_context->drv_info->irq);
	} else {
		pr_info("sii8620:disconnection\n");
		disable_irq(dev_context->drv_info->irq);
		dev_context->pdata->charger_mhl_cb(false, -1);
		dev_context->pdata->power(false);
		if (dev_context->mhl_event_switch.state == 1) {
...
#endif
{
	...
	if (mhl_class == NULL) {
		...
		if (ret) {
			pr_info("sii8620:%s:%d: cdev_add %s failed %d\n", __func__, __LINE__,
				MHL_DRIVER_NAME, ret);
			goto free_chrdev;
		}
#ifdef CONFIG_MHL3_SEC_FEATURE
