...
static void sii8620_setup_charging(struct mhl_dev_context *dev_context)
{
	...
	if ((version & 0xf0) >= 0x20) {
		 ...
		 * Therefore MHL 2.X dongle and USB connection, force setting to 0x00
		 */
		...
		pr_info("%s: version: 0x%02x, plim(%d)\n", __func__, version, plim);
		...
		pr_info("%s: version: 0x%02x, id:%d, reserved:%d\n", __func__, version, adopter_id, dev_context->dev_cap_cache.mdc.reserved);

	} else {
		pr_err("%s MHL version error - 0x%02x\n", __func__,
				version);
	}
}
