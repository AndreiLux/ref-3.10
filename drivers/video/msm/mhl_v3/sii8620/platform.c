		...
		unsigned long event, void *ptr)
{
	...
	pr_info("sii8620:%s:%d:  '%s' is %s!!!\n", __func__, __LINE__, extcon_cable_name[cable->cable_type],
			event ? "attached" : "detached");

	if (!sii8620->pdata->bootup_complete) {
		pr_info("sii8620 : platform is not ready yet\n");
		return NOTIFY_DONE;
	}
...
static void sii8620_link_monitor(u8 cmd_value)
{
	...
	scm_call(_SCM_SVC_OEM, _SCM_OEM_CMD,
		&pdata->monitor_cmd,
		sizeof(pdata->monitor_cmd), NULL, 0);
	pr_info("%s() monitor_cmd.a = %d\n", __func__,
		pdata->monitor_cmd.a);
}
...
static void of_sii8620_hw_onoff(bool onoff)
{
	...
	if (onoff) {
		...
		if (pdata->vcc_1p0v) {
			...
			if (unlikely(ret < 0)) {
				pr_err("[ERROR] regulator vcc_1p0v set_vtg failed rc\n");
				return;
			}

			ret = regulator_enable(pdata->vcc_1p0v);
			if (unlikely(ret < 0)) {
				pr_err("[ERROR] regulator vcc_1p0v enable failed rc\n");
				return;
			}
		}
		...
		if (pdata->vcc_1p0v) {
			...
			if (unlikely(ret < 0)) {
				pr_err("[ERROR] regulator vcc_1p0v disable failed rc\n");
				return;
			}
		}
...
static int of_sii8620_parse_dt(struct sii8620_platform_data *pdata, struct device *dev)
{
	...
	if (of_property_read_string(np, "sii8620,gpio_mhl_reset_type",
		...
		pr_info("sii8620:%s:%d: gpio_mhl_reset_type = %d\n", __func__, __LINE__, pdata->gpio_mhl_reset_type);
	}

	pdata->gpio_mhl_wakeup = of_get_named_gpio_flags(np,
			"sii8620,gpio_mhl_wakeup",0, NULL);
	if (pdata->gpio_mhl_wakeup > 0)
		pr_info("sii8620:%s:%d: gpio_mhl_wakeup = %d\n", __func__, __LINE__, pdata->gpio_mhl_wakeup);

	pdata->gpio_mhl_spi_dvld = of_get_named_gpio_flags(np,
			"sii8620,gpio_mhl_spi_dvld",0, NULL);
	if (pdata->gpio_mhl_spi_dvld > 0)
		pr_info("sii8620:%s:%d: gpio_mhl_spi_dvld = %d\n", __func__, __LINE__, pdata->gpio_mhl_spi_dvld);
	pdata->gpio_mhl_en = of_get_named_gpio_flags(np,
			"sii8620,gpio_mhl_en", 0, NULL);
	if (pdata->gpio_mhl_en > 0)
		pr_info("sii8620:%s:%d: gpio_mhl_en = %d\n", __func__, __LINE__, pdata->gpio_mhl_en);

	if (!of_property_read_u32(np, "sii8620,swing_level_v2",
				&pdata->swing_level_v2))
		pr_info("sii8620:%s:%d: swing_level_v2 = 0x%X\n", __func__, __LINE__, pdata->swing_level_v2);

	if (!of_property_read_u32(np, "sii8620,swing_level_v3",
				&pdata->swing_level_v3))
		pr_info("sii8620:%s:%d: swing_level_v3 = 0x%X\n", __func__, __LINE__, pdata->swing_level_v3);

	pdata->vcc_1p0v = regulator_get(dev, "vcc_1p0v");
	if (IS_ERR(pdata->vcc_1p0v)) {
		pr_err("sii8620,vcc_1p0v is not exist in device tree\n");
		pdata->vcc_1p0v = NULL;
	}

	pdata->vcc_1p8v = regulator_get(dev, "vcc_1p8v");
	if (IS_ERR(pdata->vcc_1p8v)) {
		pdata->vcc_1p8v = regulator_get(dev, "max77826_ldo7");
		if (IS_ERR(pdata->vcc_1p8v)) {
			pr_err("sii8620,vcc_1p8v is not exist in device tree\n");
			pdata->vcc_1p8v = NULL;
		}
	}
	...
	if (np_charger != NULL) {
		if (!of_property_read_string(np_charger,
			...
			pr_info("sii8620:%s:%d: charger_name = %s\n", __func__, __LINE__, pdata->charger_name);
	}
...
static void of_sii8620_gpio_init(struct sii8620_platform_data *pdata)
{
	if(pdata->gpio_mhl_en > 0) {
		if (gpio_request(pdata->gpio_mhl_en, "mhl_en")) {
			pr_err("sii8620:%s:%d:[ERROR] unable to request gpio_mhl_en [%d]\n",
					__func__, __LINE__, pdata->gpio_mhl_en);
			return;
		}
		if(gpio_direction_output(pdata->gpio_mhl_en, 0)) {
			pr_err("sii8620:%s:%d:[ERROR] unable to  gpio_mhl_en low[%d]\n",
					__func__, __LINE__, pdata->gpio_mhl_en);
			return;
		}
	}
	...
	if(pdata->gpio_mhl_reset > 0) {
		if (gpio_request(pdata->gpio_mhl_reset, "mhl_reset")) {
			pr_err("sii8620:%s:%d:[ERROR] unable to request gpio_mhl_reset [%d]\n",
					__func__, __LINE__, pdata->gpio_mhl_reset);
			return;
		}
		if(gpio_direction_output(pdata->gpio_mhl_reset, 0)) {
			pr_err("sii8620:%s:%d:[ERROR] unable to gpio_mhl_reset low[%d]\n",
					__func__, __LINE__, pdata->gpio_mhl_reset);
			return;
		}
	}
