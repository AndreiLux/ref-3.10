...
static void s3c_fb_pcd_enable(struct s3c_fb *sfb);
static void s3c_fb_pcd_disable(struct s3c_fb *sfb);
#endif
/**
...
#if defined(CONFIG_FB_I80_COMMAND_MODE) && defined(CONFIG_LCD_PCD)
static void s3c_fb_pcd_work(struct work_struct *work)
{
	struct s3c_fb *sfb =
	...
	cancel_delayed_work(&sfb->pcd_work);
	if (sfb->pcd_detected)
...
static int s3c_fb_pcd_init(struct s3c_fb *sfb)
{
	struct device *dev = sfb->dev;
	int pcd;

	INIT_DELAYED_WORK(&sfb->pcd_work, s3c_fb_pcd_work);
