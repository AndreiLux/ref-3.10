...
int init_display_decon_clocks(struct device *dev)
{
	...
	if (lcd->xres * lcd->yres == 1440 * 2560) {
		exynos_display_set_rate(dev, "dout_aclk_disp_333", 333 * MHZ);
		exynos_display_set_rate(dev, "dout_sclk_decon_eclk", 275 * MHZ);
	} else {
		exynos_display_set_rate(dev, "dout_aclk_disp_333", 222 * MHZ);
		exynos_display_set_rate(dev, "dout_sclk_decon_eclk", 400 * MHZ);
	}
	exynos_display_set_parent(dev, "mout_sclk_decon_eclk_a", "mout_bus_pll_sub");
	exynos_display_set_parent(dev, "mout_aclk_disp_222_user", "aclk_disp_222");
	exynos_display_set_parent(dev, "mout_sclk_decon_eclk_user", "sclk_decon_eclk_mif");
	exynos_display_set_parent(dev, "mout_sclk_decon_vclk_user", "sclk_decon_vclk_mif");
	exynos_display_set_parent(dev, "mout_sclk_dsd_user", "sclk_dsd_mif");
	exynos_display_set_parent(dev, "mout_sclk_decon_eclk_disp", "mout_sclk_decon_eclk_user");
	exynos_display_set_parent(dev, "mout_disp_pll", "disp_pll");
	exynos_display_set_parent(dev, "mout_aclk_disp_333_user", "aclk_disp_333");

	exynos_display_set_parent(dev, "mout_sclk_dsd_a", "dout_mfc_pll");
#endif
