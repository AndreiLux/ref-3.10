...
static void s6e3ha0_read_coordinate(struct lcd_info *lcd)
{
	int ret;
...
static void s6e3ha0_read_id(struct lcd_info *lcd, u8 *buf)
{
	int ret;
...
static void s6e3ha0_read_ddi_id(struct lcd_info *lcd, u8 *buf)
{
	int ret;
...
static void s6e3ha0_update_seq(struct lcd_info *lcd)
{
	u8 id;
...
#if defined(CONFIG_DECON_MDNIE_LITE)
	mdnie_register(&lcd->ld->dev, lcd, (mdnie_w)s6e3ha0_write, (mdnie_r)s6e3ha0_read);
#endif
...
MODULE_DESCRIPTION("MIPI-DSI S6E3HA0 (1440*25600) Panel Driver");
MODULE_LICENSE("GPL");
