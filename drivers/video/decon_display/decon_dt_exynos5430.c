...
int parse_lcd_drvdata(struct device_node *np)
{
...
#if defined(CONFIG_DECON_LCD_S6E3HA0)
	int gpio;
	...
	if (idx)
		g_dsim_config.dsim_ddi_pd = &s6e3fa2_mipi_lcd_driver;
#endif
	...
	g_decon_lcd.hfp = res[0];
	g_decon_lcd.hbp = res[1];
	g_decon_lcd.hsa = res[2];

	ret = of_property_read_u32_array(node, "timing,v-porch", res, 3);
	g_decon_lcd.vfp = res[0];
	g_decon_lcd.vbp = res[1];
