...
static void stm_disable_clock(struct stm_drvdata *drvdata)
{
	clk_disable_unprepare(drvdata->clk_at);
...
static int  stm_suspend(struct platform_device *pdev, pm_message_t message)
{

	struct stm_drvdata *drvdata = platform_get_drvdata(pdev);

	dev_info(drvdata->dev, "stm_suspend+++\n");

	dev_info(drvdata->dev, "stm_suspend---\n");
	return 0;
}

static int  stm_resume(struct platform_device *pdev)
{
	struct stm_drvdata *drvdata = platform_get_drvdata(pdev);

	dev_info(drvdata->dev, "stm_resume+++\n");
