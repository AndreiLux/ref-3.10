...
int initialize_event_symlink(struct ssp_data *data)
{
	...
	iRet = sensors_create_symlink(&data->uncal_mag_input_dev->dev.kobj,
		data->uncal_mag_input_dev->name);
	if (iRet < 0)
		goto iRet_uncal_mag_sysfs_create_link;

	iRet = sensors_create_symlink(&data->sig_motion_input_dev->dev.kobj,
		data->sig_motion_input_dev->name);
	if (iRet < 0)
		goto iRet_sig_motion_sysfs_create_link;

	iRet = sensors_create_symlink(&data->uncalib_gyro_input_dev->dev.kobj,
		data->uncalib_gyro_input_dev->name);
	if (iRet < 0)
		goto iRet_uncalib_gyro_sysfs_create_link;

	iRet = sensors_create_symlink(&data->step_cnt_input_dev->dev.kobj,
		data->step_cnt_input_dev->name);
	if (iRet < 0)
		goto iRet_step_cnt_sysfs_create_link;

	iRet = sensors_create_symlink(&data->meta_input_dev->dev.kobj,
...
iRet_meta_sysfs_create_link:
	sensors_remove_symlink(&data->step_cnt_input_dev->dev.kobj,
		data->step_cnt_input_dev->name);
iRet_step_cnt_sysfs_create_link:
	sensors_remove_symlink(&data->uncalib_gyro_input_dev->dev.kobj,
		data->uncalib_gyro_input_dev->name);
iRet_uncalib_gyro_sysfs_create_link:
	sensors_remove_symlink(&data->sig_motion_input_dev->dev.kobj,
		data->sig_motion_input_dev->name);
iRet_sig_motion_sysfs_create_link:
	sensors_remove_symlink(&data->uncal_mag_input_dev->dev.kobj,
		data->uncal_mag_input_dev->name);
...
void remove_event_symlink(struct ssp_data *data)
{
	...
	sensors_remove_symlink(&data->step_cnt_input_dev->dev.kobj,
		data->step_cnt_input_dev->name);
	sensors_remove_symlink(&data->meta_input_dev->dev.kobj,
		data->meta_input_dev->name);
}
