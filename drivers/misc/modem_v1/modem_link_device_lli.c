...
static void print_pm_status(struct shmem_link_device *shmd, int level)
{
	...
	*/
	if (level > 0) {
		pr_err(LOG_TAG "[%02d:%02d:%02d.%03d] LLI PM {%d:%d:%d:%d:%X} "
			"%pf\n", t.hour, t.min, t.sec, t.msec, ap_wakeup,
			cp_wakeup, cp_status, ap_status, magic, CALLER);
	} else {
		pr_info(LOG_TAG "[%02d:%02d:%02d.%03d] LLI PM {%d:%d:%d:%d:%X} "
			"%pf\n", t.hour, t.min, t.sec, t.msec, ap_wakeup,
			cp_wakeup, cp_status, ap_status, magic, CALLER);
	}
#endif
...
{
	if (shmd->type == LLI_SHMEM)
		return (u16)mipi_lli_read_interrupt();
...
static irqreturn_t ap_wakeup_handler(int irq, void *data)
{
	...
	irq_set_irq_type(irq,
			 ap_wakeup ? IRQ_TYPE_LEVEL_LOW : IRQ_TYPE_LEVEL_HIGH);
	...
	if (ap_wakeup) {
		...
		if (/*!mipi_lli_suspended() &&*/ !ap_status)
			gpio_set_value(shmd->gpio_ap_status, 1);
	} else {
		if (wake_lock_active(&shmd->ap_wlock))
...
static irqreturn_t cp_status_handler(int irq, void *data)
{
	...
	irq_set_irq_type(irq,
			 cp_status ? IRQ_TYPE_LEVEL_LOW : IRQ_TYPE_LEVEL_HIGH);
...
static void cmd_phone_start_handler(struct shmem_link_device *shmd)
{
	...
	irq_set_irq_type(shmd->irq_ap_wakeup,
			 ap_wakeup ? IRQ_TYPE_LEVEL_LOW : IRQ_TYPE_LEVEL_HIGH);
	if (ap_wakeup && !wake_lock_active(&shmd->ap_wlock))
		wake_lock(&shmd->ap_wlock);

	irq_set_irq_type(shmd->irq_cp_status,
			 cp_status ? IRQ_TYPE_LEVEL_LOW : IRQ_TYPE_LEVEL_HIGH);
...
bad_msg:
#ifdef DEBUG_MODEM_IF
	pr_ipc(1, "LLI: ERR! CP2MIF", (src + out), (rest > 20) ? 20 : rest);
#endif
	return -EBADMSG;
}
...
/**
 * lli_irq_handler: interrupt handler for a LLI interrupt
 * @data: pointer to a data
 *
 * 1) Reads the interrupt value
 * 2) Performs interrupt handling
 ...
 *   lli_irq_handler -> udl_handler
 *   lli_irq_handler -> ipc_rx_task -> cmd_handler -> cmd_xxx_handler
 *   lli_irq_handler -> ipc_rx_task -> msg_handler -> rx_ipc_frames ...
 */
 ...
 */
static int lli_send_ipc(struct shmem_link_device *shmd, int dev)
{
	struct link_device *ld = &shmd->ld;
	int ret;
	u16 mask;
 ...
 * 2) Tries to transmit IPC messages with lli_send_ipc().
 */
static void lli_try_send_ipc(struct shmem_link_device *shmd, int dev,
			struct io_device *iod, struct sk_buff *skb)
{
	...
	ret = lli_send_ipc(shmd, dev);
	if (ret < 0) {
		mif_err("%s->%s: ERR! lli_send_ipc fail (err %d)\n",
...
static int lli_send_udl_cmd(struct shmem_link_device *shmd, int dev,
			struct io_device *iod, struct sk_buff *skb)
{
	struct link_device *ld = &shmd->ld;
	u8 *buff;
...
static int lli_send_udl_data(struct shmem_link_device *shmd, int dev)
{
	struct link_device *ld = &shmd->ld;
	struct sk_buff_head *txq = ld->skb_txq[dev];
	int copied;
 ...
 */
static void lli_send_udl(struct shmem_link_device *shmd, int dev,
			struct io_device *iod, struct sk_buff *skb)
{
	struct link_device *ld = &shmd->ld;
	struct sk_buff_head *txq = ld->skb_txq[dev];
	...
	if (iod->format == IPC_RAMDUMP || !std_udl_with_payload(udl_cmd)) {
		ret = lli_send_udl_cmd(shmd, dev, iod, skb);
		if (ret > 0)
			send_int2cp(shmd, INT_NON_CMD(mask));
		else
			mif_err("ERR! lli_send_udl_cmd fail (err %d)\n", ret);
		goto exit;
	}
	...
	while (1) {
		ret = lli_send_udl_data(shmd, dev);
		if (ret < 0) {
			mif_err("ERR! lli_send_udl_data fail (err %d)\n", ret);
			skb_queue_purge(txq);
			break;
		}
 ...
 *   lli_try_send_ipc -> lli_send_ipc -> xmit_ipc_msg -> write_ipc_to_txq
 *
 * Call flow on congestion in a IPC TXQ:
 *   lli_try_send_ipc -> lli_send_ipc -> xmit_ipc_msg ,,, queue_delayed_work
 *   => xxx_tx_work -> wait_for_res_ack
 *   => msg_handler
 *   => process_res_ack -> xmit_ipc_msg (,,, queue_delayed_work ...)
 */
static int lli_send(struct link_device *ld, struct io_device *iod,
		    struct sk_buff *skb)
{
	...
	case IPC_RAW:
		if (likely(ld->mode == LINK_MODE_IPC)) {
			if (unlikely(shmd->forced_cp_crash)) {
				...
				lli_try_send_ipc(shmd, dev, iod, skb);
		} else {
	...
	case IPC_BOOT:
	case IPC_RAMDUMP:
		lli_send_udl(shmd, IPC_RAW, iod, skb);
		break;
...
static int lli_dload_start(struct link_device *ld, struct io_device *iod)
{
	struct shmem_link_device *shmd = to_shmem_link_device(ld);
	u32 magic;
 ...
 *
 */
static int lli_set_dload_info(struct link_device *ld, struct io_device *iod,
			unsigned long arg)
{
	struct shmem_link_device *shmd = to_shmem_link_device(ld);
	struct std_dload_info *dl_info = &shmd->dl_info;
...
static int lli_force_dump(struct link_device *ld, struct io_device *iod)
{
	struct shmem_link_device *shmd = to_shmem_link_device(ld);
	mif_err("+++\n");
	trigger_forced_cp_crash(shmd);
...
static int lli_dump_start(struct link_device *ld, struct io_device *iod)
{
	struct shmem_link_device *shmd = to_shmem_link_device(ld);
...
static void lli_remap_4mb_ipc_region(struct shmem_link_device *shmd)
{
...
static int lli_init_ipc_map(struct shmem_link_device *shmd)
{
	if (shmd->size >= SHMEM_SIZE_4MB)
		lli_remap_4mb_ipc_region(shmd);
	else
		return -EINVAL;
...
static int lli_init_communication(struct link_device *ld, struct io_device *iod)
{
	struct shmem_link_device *shmd = to_shmem_link_device(ld);
...
struct link_device *lli_create_link_device(struct platform_device *pdev)
{
	...
	*/
	...
	if (!shmd) {
		mif_err("%s: ERR! shmd devm_kzalloc fail\n", modem->link_name);
		goto error;
	}
	ld = &shmd->ld;
	...
	*/
	ld->init_comm = lli_init_communication;
	ld->send = lli_send;
	ld->dload_start = lli_dload_start;
	ld->firm_update = lli_set_dload_info;
	ld->force_dump = lli_force_dump;
	ld->dump_start = lli_dump_start;
	...
	*/
	if (modem->link_types & LINKTYPE(LINKDEV_LLI)) {
		shmd->type = LLI_SHMEM;
		mif_debug("%s: shmd->type = LLI_SHMEM\n", ld->name);
	...
	phys_base = mipi_lli_get_phys_base();
	size = mipi_lli_get_phys_size();
	mif_err("%s: phys_base:0x%08X size:%d\n", ld->name, phys_base, size);

	shmd->start = phys_base;
	shmd->size = size;
	shmd->base = mipi_lli_request_sh_region(shmd->start, shmd->size);
	if (!shmd->base) {
		mif_err("%s: ERR! lli_request_sh_region fail\n", ld->name);
		goto error;
	}
	...
	*/
	err = lli_init_ipc_map(shmd);
	if (err < 0) {
		mif_err("%s: ERR! lli_init_ipc_map fail (err %d)\n",
			ld->name, err);
		goto error;
	}
	...
	*/
	...
	shmd->irq_ap_wakeup = gpio_to_irq(modem->gpio_ap_wakeup);
	...
	shmd->irq_cp_status = gpio_to_irq(modem->gpio_cp_status);
	...
	*/
	err = mipi_lli_register_handler(lli_irq_handler, shmd);
	if (err) {
		mif_err("%s: ERR! lli_register_handler fail (err %d)\n",
			ld->name, err);
		goto error;
	}
