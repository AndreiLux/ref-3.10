...
#define sp_tx_clean_hdcp_status() do{ \
	...
	mdelay(2); \
	SLIMPORT_DBG_INFO("sp_tx_clean_hdcp_status \n"); \
	}while(0)
...
#define sp_tx_set_sys_state(ss) \
	do { \
		SLIMPORT_DBG_INFO("set: clean_status: %x,\n ", (uint)g_need_clean_status); \
...
#define goto_next_system_state() \
	do { \
		SLIMPORT_DBG_INFO("next: clean_status: %x,\n ", (uint)g_need_clean_status); \
		sp_tx_system_state_bak = sp_tx_system_state; \
		sp_tx_system_state++;\
		print_sys_state(sp_tx_system_state); \
	}while(0)
...
#define redo_cur_system_state() \
	do { \
		SLIMPORT_DBG_INFO("redo: clean_status: %x,\n ", (uint)g_need_clean_status); \
		g_need_clean_status = 1; \
		sp_tx_system_state_bak = sp_tx_system_state; \
		print_sys_state(sp_tx_system_state); \
	}while(0)
...
#define system_state_change_with_case(status) \
	do{ \
		if(sp_tx_system_state >= status) { \
			SLIMPORT_DBG_INFO("change_case: clean_status: %xm,\n ", (uint)g_need_clean_status); \
...
void wait_aux_op_finish(unchar * err_flag)
{
	...
	*err_flag = 0;
	...
	while (__i2c_read_byte(TX_P0, AUX_CTRL2) & AUX_OP_EN) {
				mdelay(2);
				if ((cnt--) == 0) {
					SLIMPORT_DBG_INFO("aux operate failed!\n");
					*err_flag = 1;
					break;
				}
			}
	...
	if (c & 0x0F) {
			SLIMPORT_DBG_INFO("wait aux operation failed %.2x\n", (uint)c);
			*err_flag = 1;
	}
}
...
void print_sys_state(unchar ss)
{
	switch(ss) {
	case STATE_INIT:
		SLIMPORT_DBG_INFO(" -STATE_INIT- \n");
		break;
	case STATE_WAITTING_CABLE_PLUG:
		SLIMPORT_DBG_INFO(" -STATE_WAITTING_CABLE_PLUG- \n");
		break;
	case STATE_SP_INITIALIZED:
		SLIMPORT_DBG_INFO(" -STATE_SP_INITIALIZED- \n");
		break;
	case STATE_SINK_CONNECTION:
		SLIMPORT_DBG_INFO(" -STATE_SINK_CONNECTION- \n");
		break;
	#ifdef ENABLE_READ_EDID
	case STATE_PARSE_EDID:
		SLIMPORT_DBG_INFO(" -STATE_PARSE_EDID- \n");
		break;
	#endif
	case STATE_LINK_TRAINING:
		SLIMPORT_DBG_INFO(" -STATE_LINK_TRAINING- \n");
		break;
	case STATE_VIDEO_OUTPUT:
		SLIMPORT_DBG_INFO(" -STATE_VIDEO_OUTPUT- \n");
		break;
	#ifndef HDCP_AUTO_EN
	case STATE_HDCP_AUTH:
		SLIMPORT_DBG_INFO(" -STATE_HDCP_AUTH- \n");
		break;
	#endif
	case STATE_AUDIO_OUTPUT:
		SLIMPORT_DBG_INFO(" -STATE_AUDIO_OUTPUT- \n");
		break;
	case STATE_PLAY_BACK:
		SLIMPORT_DBG_INFO(" -STATE_PLAY_BACK- \n");
		break;
	default:
		SLIMPORT_ERR(" system state is error1 \n");
		break;
	}
}
...
//========initialized system
int slimport_block_power_ctrl(enum SP_TX_POWER_BLOCK sp_tx_pd_block, unchar power)
{
	int ret = 0;
	if(power == SP_POWER_ON)
		ret = sp_write_reg_and(TX_P2, SP_POWERD_CTRL_REG, (~sp_tx_pd_block));
	else
		ret = sp_write_reg_or(TX_P2, SP_POWERD_CTRL_REG, (sp_tx_pd_block));
	 SLIMPORT_DBG_INFO("sp_tx_power_on: %.2x\n", (uint)sp_tx_pd_block);
	return ret;
}
...
void vbus_power_ctrl(unsigned char ON)
{
	...
	if(ON == 0) {
		//kyusuk
		hdmi_rx_set_hpd(0);
//		SLIMPORT_DBG_INFO("hdmi_rx_set_hpd 0 !\n");
		...
		SLIMPORT_DBG_INFO("3.3V output disabled\n");
	} else {
		for (i = 0; i < 5; i++) {
			...
			if (!((unchar)__i2c_read_byte(TX_P2, TX_PLL_FILTER5) & 0xc0)) {
				SLIMPORT_DBG_INFO("3.3V output enabled\n");
				break;
			}else{
				SLIMPORT_DBG_INFO("VBUS power can not be supplied\n");
			}
		}
	}
}
...
void hdmi_rx_initialization(void)
{
	...
	*/
	...
	hdmi_rx_set_hpd(0);
	hdmi_rx_set_termination(0);
	SLIMPORT_DBG_INFO("HDMI Rx is initialized...\n");
}
...
{
	SLIMPORT_DBG_INFO("define XTAL_CLK:  %x\n ", (uint)XTAL_CLK_DEF);
...
bool slimport_chip_detect(void)
{
	...
	SLIMPORT_DBG_INFO("CHIPID: ANX%x\n", c & 0x0000FFFF);
...
void slimport_waitting_cable_plug_process(void)
{
	...
	/*  hdmi_rx_set_hpd(1);
		SLIMPORT_DBG_INFO("hdmi_rx_set_hpd 1 cable detected \n");
...
static unchar sp_tx_get_cable_type(CABLE_TYPE_STATUS det_cable_type_state, bool bdelay)
{
	...
	SLIMPORT_DBG_INFO("DPCD 0x005: %x \n", (int)ds_port_preset);
		...
		case GETTED_CABLE_TYPE:
			...
			case 0x00:
				...
				if((data_buf[0] == 0x00) && (data_buf[1] == 0x22)
					...
					down_sample_en = 1;
					SLIMPORT_DBG_INFO("Downstream is DP 4K dongle.\n");

				} else
					down_sample_en = 0;
				#endif
				SLIMPORT_DBG_INFO("Downstream is DP dongle.\n");
				break;
			case 0x01:
			case 0x03:
				...
				if (((data_buf[0] == 0x00) && (data_buf[1] == 0x22)
					...
					SLIMPORT_DBG_INFO("Downstream is VGA dongle.\n");
				} else {
					...
					if((data_buf[0] == 0x00) && (data_buf[1] == 0x22)
						...
						SLIMPORT_DBG_INFO("Downstream is general DP2VGA converter.\n");
					}
				}
				break;
			case 0x02:
				if(AUX_OK == sp_tx_aux_dpcdread_bytes(0x00, 0x04, 0x00, 8, data_buf)){
					if ((data_buf[0] == 0xb9) && (data_buf[1] == 0x22)
						...
						SLIMPORT_DBG_INFO("Downstream is HDMI dongle.\n");
							...
							if((data_buf[0] == 0x00) && (data_buf[1] == 0x22)
								...
								sp_write_reg_and_or(TX_P2, TX_ANALOG_DEBUG2, (~0x3c), 0x3c & (XTAL_24M << 2) );
								down_sample_en = 1;
								SLIMPORT_DBG_INFO("Downstream is HDMI 4K dongle.\n");
						...
						cur_cable_type = DWN_STRM_IS_DIGITAL;
						SLIMPORT_DBG_INFO("Downstream is general DP2HDMI converter.\n");
					}
				}else
					SLIMPORT_ERR("dpcd read error!.\n");

				break;
			default:
				cur_cable_type = DWN_STRM_IS_NULL;
				SLIMPORT_ERR("Downstream can not recognized.\n");
				break;
			}
		default:
			break;
		...
		default:
			...
			if(sp_tx_rx_type == DWN_STRM_IS_NULL) {
				...
				if(sp_tx_sc_state >= SC_WAITTING_CABLE_TYPE){
					...
					SLIMPORT_DBG_INFO("Can not get cable type!\n");
				}
				break;
			}
...
void sp_tx_enable_video_input(unchar enable)
{
	...
	if (enable) {
		if((c & VIDEO_EN) != VIDEO_EN) {
			...
			SLIMPORT_DBG_INFO("Slimport Video is enabled!\n");
		}
	} else {
		if( (c & VIDEO_EN) == VIDEO_EN){
			c &= ~VIDEO_EN;
			sp_write_reg(TX_P2, VID_CTRL1, c);
			SLIMPORT_DBG_INFO("Slimport Video is disabled!\n");
		}
	}
}
void sp_tx_send_message(enum SP_TX_SEND_MSG message)
{
	...
	case MSG_CLEAR_IRQ:
		//SLIMPORT_DBG_INFO("clear irq start!\n");
		sp_tx_aux_dpcdread_bytes(0x00, 0x04, 0x10, 1, &c);
		//SLIMPORT_DBG_INFO("clear irq middle!\n");
		c |= 0x01;
		sp_tx_aux_dpcdwrite_byte(0x00, 0x04, 0x10, c);
		//SLIMPORT_DBG_INFO("clear irq end!\n");
		break;
	}
...
static unchar get_edid_detail(unchar *data_buf)
{
	...
	//SLIMPORT_DBG_INFO("=============pixclock  via EDID : %d\n", (uint)pixclock_edid);
...
static unchar parse_edid_to_get_bandwidth(void)
{
	...
	while (4 > i && 0 != edid_blocks[0x36+desc_offset]) {
		...
		SLIMPORT_DBG_INFO("bandwidth via EDID : %x\n", (uint)temp);
		if(bandwidth < temp)
			bandwidth = temp;
...
unchar sp_tx_get_edid_block(void)
{
	...
	SLIMPORT_DBG_INFO("EDID Block = %d\n", (int)(c + 1));
...
void edid_read(unchar offset, unchar *pblock_buf)
{
	...
	while((data_cnt) < 16)
	{
		...
		//if(c != 0x10)
			//SLIMPORT_DBG_INFO("edid read: len : %x \n", (uint)c);

		if((c & 0x1f) != 0){
			data_cnt = data_cnt + c;
			do{
				sp_read_reg(TX_P0, BUF_DATA_0 + c -1, &(pblock_buf[c - 1]));
				//SLIMPORT_DBG_INFO("index: %x, data: %x ", (uint)c, (uint)pblock_buf[c]);
				if(c == 1)
					break;
			}while(c--);
			...
			//SLIMPORT_DBG_INFO("edid read : length: 0 \n");
			if(cnt++ <= 2){
				sp_tx_rst_aux();
...
//	SLIMPORT_DBG_INFO("====g_edid_checksum  ==%x \n", (uint)g_edid_checksum);
...
static void edid_header_result(unchar *pBuf)
{
	 if((pBuf[0] == 0) && (pBuf[7] == 0) && (pBuf[1] == 0xff) && (pBuf[2] == 0xff) && (pBuf[3] == 0xff)
	        ...
	        SLIMPORT_DBG_INFO("Good EDID header!\n");
	 else
	        SLIMPORT_ERR("Bad EDID header!\n");
...
void check_edid_data(unchar *pblock_buf)
{
	...
	for(i =0; i <= ((pblock_buf[0x7e] > 1) ? 1 : pblock_buf[0x7e]); i++){
			if(!edid_checksum_result(pblock_buf+ i*128))
				SLIMPORT_ERR("Block %x edid checksum error\n", (uint)i);
			else
				SLIMPORT_DBG_INFO("Block %x edid checksum OK\n", (uint)i);
		}
}
	...
	if(g_edid_break == 0) {
		...
		for(i = 0; i < 16; i++){
			if(org_buf[0x70 + i] != temp_buf[i]){
				SLIMPORT_DBG_INFO("different checksum and blocks num\n");
	...
	if(g_edid_break == 0) {
		for(i = 0; i < 16; i++){
			if(org_buf[i + 8] != temp_buf[i]){
				SLIMPORT_DBG_INFO("different edid information\n");
				return_flag = 1;
				break;
			}
		}
...
void slimport_edid_process(void)
{
	...
	SLIMPORT_DBG_INFO("edid_process\n");

	if(g_read_edid_flag == 1){
		if(check_with_pre_edid(edid_blocks))
			g_read_edid_flag = 0;
		else
			SLIMPORT_DBG_INFO("Don`t need to read edid!\n");
	}

	if(g_read_edid_flag == 0){
		sp_tx_edid_read(edid_blocks);
		if(g_edid_break)
			SLIMPORT_ERR("ERR:EDID corruption!\n");
	}
	...
	do {
		if((__i2c_read_byte(TX_P0, HDCP_KEY_STATUS) & 0x01))
			...
			SLIMPORT_DBG_INFO("waiting HDCP KEY loaddown \n");
			mdelay(1);
		}
	}while(--i);
	sp_write_reg(RX_P0, HDMI_RX_INT_MASK1_REG, 0xe2);

	/* Delay HPD when HDMI plugged again immediately after unplugged*/
	if (get_hdmi_power_status() == 1){
		SLIMPORT_DBG_INFO("HDMI_hpd delay\n");
		mdelay(2000);
	}
	// kibum.lee
	atomic_notifier_call_chain(&hdmi_hpd_notifier_list, 1, NULL);
	...
	hdmi_rx_set_hpd(1);
	SLIMPORT_DBG_INFO("hdmi_rx_set_hpd 1 !\n");
	hdmi_rx_set_termination(1);
	...
	SLIMPORT_DBG_INFO("set link bw in edid %x \n", (uint)temp_value1);
...
static void sp_tx_lvttl_bit_mapping(void)
{
	...
	if (sp_tx_test_edid ){
		...
		sp_tx_test_edid = 0;
		SLIMPORT_DBG_INFO("***color space is set to 18bit***\n");
	}
...
static unchar sp_tx_bw_lc_sel(ulong pclk)
{
	...
	if(sp_tx_get_link_bw() != c1){
		...
		SLIMPORT_DBG_INFO("It is different bandwidth between sink support and cur video!%.2x\n",(uint)c1);
		return 1;
	}
     return 0;
}
...
void sp_tx_enhancemode_set(void)
{
	...
	if (c & ENHANCED_FRAME_CAP) {
		...
		SLIMPORT_DBG_INFO("Enhance mode enabled\n");
	} else {
		...
		SLIMPORT_DBG_INFO("Enhance mode disabled\n");
	}
}
...
void slimport_link_training(void)
{
	...
	return_value = 1;
	SLIMPORT_DBG_INFO("sp_tx_LT_state : %x\n", (int)sp_tx_LT_state);
	switch(sp_tx_LT_state) {
		...
		case LT_WAIT_PLL_LOCK:
			if (!sp_tx_get_pll_lock_status()) {
				...
				temp_value &=~PLL_RST;
				sp_write_reg(TX_P0, SP_TX_PLL_CTRL_REG, temp_value);
				SLIMPORT_DBG_INFO("PLL not lock!\n");
			}else
		...
		case LT_CHECK_LINK_BW:
			...
			if(temp_value < g_changed_bandwidth){
				SLIMPORT_DBG_INFO("****Over bandwidth****\n");
		...
		case LT_FINISH:
			...
			if ((temp_value&0x07) == 0x07) {// for one lane case.
				...
				if(!sp_tx_test_lt) {
					...
					if (temp_value == g_changed_bandwidth){
						SLIMPORT_DBG_INFO("LT succeed, bw: %.2x ", (uint) temp_value);
						SLIMPORT_DBG_INFO("Lane0 Set: %.2x\n", (uint) __i2c_read_byte(TX_P0, SP_TX_LT_SET_REG));
						sp_tx_LT_state = LT_INIT;
						goto_next_system_state();
					}else {
						SLIMPORT_DBG_INFO("bw cur:%.2x, per:%.2x \n", (uint)temp_value, (uint)g_changed_bandwidth);
				...
				SLIMPORT_DBG_INFO("LANE0 Status error: %.2x\n", (uint)(temp_value&0x07));
				sp_tx_LT_state = LT_ERROR;
			}
			break;
		default:
...
void sp_tx_set_colorspace(void)
{
	...
	if(down_sample_en){
		...
		color_space &= 0x60;
		if(color_space == 0x20){
			SLIMPORT_DBG_INFO("YCbCr4:2:2 ---> PASS THROUGH.\n");
			...
			SLIMPORT_DBG_INFO("YCbCr4:4:4 ---> YCbCr4:2:2\n");
			...
			SLIMPORT_DBG_INFO("RGB4:4:4 ---> YCbCr4:2:2\n");
			sp_write_reg(TX_P2, SP_TX_VID_CTRL6_REG, 0x41);
...
void slimport_config_video_output(void)
{
	...
	switch(sp_tx_vo_state){
		...
		case VO_WAIT_VIDEO_STABLE:
			...
			{
				SLIMPORT_DBG_INFO("HDMI input video not stable!\n");
			}
		...
		case VO_WAIT_TX_VIDEO_STABLE:
			...
			if(temp_value & CHA_STA) {
				SLIMPORT_DBG_INFO("Stream clock not stable!\n");
			}else {
				...
				{
					SLIMPORT_ERR("video stream not valid!\n");
				} else{
				#ifdef DEMO_4K_2K
				if(down_sample_en)
		...
		case VO_CHECK_VIDEO_INFO:
			...
			if (!sp_tx_bw_lc_sel(sp_tx_pclk_calc())
				...
				if(g_hdmi_dvi_status != temp_value){
					SLIMPORT_DBG_INFO("Different mode of DVI or HDMI mode \n");
					g_hdmi_dvi_status = temp_value;
...
void slimport_hdcp_process(void)
{
	...
	case HDCP_WAITTING_VID_STB:
		...
		if(sp_tx_rx_type == DWN_STRM_IS_HDMI_7730) {
			//SLIMPORT_DBG_INFO("video stb : count%.2x \n",(WORD)ds_vid_stb_cntr);
			if (!sp_tx_get_ds_video_status()) {
				...
				SLIMPORT_DBG_INFO("downstream video not stable\n"); //for debug
				#endif
			} else {
				ds_vid_stb_cntr = 0;
	...
	case HDCP_FINISH:
		...
		goto_next_system_state();
		HDCP_state = HDCP_CAPABLE_CHECK;
		SLIMPORT_DBG_INFO("@@@@@@@hdcp_auth_pass@@@@@@\n");
		break;
	case HDCP_FAILE:
		if(HDCP_fail_count > 5) {
			...
			HDCP_state = HDCP_CAPABLE_CHECK;
			HDCP_fail_count = 0;
			SLIMPORT_DBG_INFO("*********hdcp_auth_failed*********\n");
		}else {
			HDCP_fail_count++;
			HDCP_state = HDCP_WAITTING_VID_STB;
		}
	...
	case HDCP_NOT_SUPPORT:
		SLIMPORT_DBG_INFO("Sink is not capable HDCP\n");
...
static void sp_tx_config_audio(void)
{
	...
	ulong AUD_Freq = 0;
	SLIMPORT_DBG_INFO("**Config audio **\n");
	...
	SLIMPORT_DBG_INFO("AUD_Freq = %ld , LS_CLK = %ld\n", AUD_Freq, LS_Clk);
...
void slimport_initialization(void)
{
	...
	g_read_edid_flag = 0;
	#endif
	status = slimport_block_power_ctrl(SP_TX_PWR_REG, SP_POWER_ON);
	if (status < 0) {
		hardware_power_ctl(0);
		return;
	}
...
static void sp_tx_pll_changed_int_handler(void)
{
	if (sp_tx_system_state >= STATE_LINK_TRAINING) {
		if (!sp_tx_get_pll_lock_status()) {
			SLIMPORT_DBG_INFO("PLL:PLL not lock!\n");
			sp_tx_set_sys_state(STATE_LINK_TRAINING);
		}
	}
}
...
static void sp_tx_phy_auto_test(void)
{
	...
	SLIMPORT_DBG_INFO("DPCD:0x00206 = %.2x\n", (uint)bytebuf[0]);
	c1 = bytebuf[0] & 0x0f;
	switch (c1) {
	case 0x00:
	sp_write_reg(TX_P0, 0xA3, (b_sw & ~0x1b) | 0x00);
	//SLIMPORT_DBG_INFO("lane0,Swing0, emp 0db.\n");
	break;
	case 0x01:
	sp_write_reg(TX_P0, 0xA3, (b_sw & ~0x1b) | 0x01);
	//SLIMPORT_DBG_INFO("lane0,Swing1, emp 0db.\n");
	break;
	case 0x02:
	sp_write_reg(TX_P0, 0xA3, (b_sw & ~0x1b) | 0x02);
	//SLIMPORT_DBG_INFO("lane0,Swing2, emp 0db.\n");
	break;
	case 0x03:
	sp_write_reg(TX_P0, 0xA3, (b_sw & ~0x1b) | 0x03);
	//SLIMPORT_DBG_INFO("lane0,Swing3, emp 0db.\n");
	break;
	case 0x04:
	sp_write_reg(TX_P0, 0xA3, (b_sw & ~0x1b) | 0x08);
	//SLIMPORT_DBG_INFO("lane0,Swing0, emp 3.5db.\n");
	break;
	case 0x05:
	sp_write_reg(TX_P0, 0xA3, (b_sw & ~0x1b) | 0x09);
	//SLIMPORT_DBG_INFO("lane0,Swing1, emp 3.5db.\n");
	break;
	case 0x06:
	sp_write_reg(TX_P0, 0xA3, (b_sw & ~0x1b) | 0x0a);
	//SLIMPORT_DBG_INFO("lane0,Swing2, emp 3.5db.\n");
	break;
	case 0x08:
	sp_write_reg(TX_P0, 0xA3, (b_sw & ~0x1b) | 0x10);
	//SLIMPORT_DBG_INFO("lane0,Swing0, emp 6db.\n");
	break;
	case 0x09:
	sp_write_reg(TX_P0, 0xA3, (b_sw & ~0x1b) | 0x11);
	//SLIMPORT_DBG_INFO("lane0,Swing1, emp 6db.\n");
	break;
	case 0x0c:
	sp_write_reg(TX_P0, 0xA3, (b_sw & ~0x1b) | 0x18);
	//SLIMPORT_DBG_INFO("lane0,Swing0, emp 9.5db.\n");
	break;
	default:
	break;
	}
...
static void sp_tx_sink_irq_int_handler(void)
{
	...
	need_return = 0;
	SLIMPORT_DBG_INFO("sp_tx_sink_irq_int_handler \n" );
	...
	if (IRQ_Vector & CP_IRQ) {
		...
		#endif
		{
			...
			if (c1 & 0x04) {
					...
					sp_tx_clean_hdcp_status();
					#endif
					SLIMPORT_DBG_INFO("IRQ:____________HDCP Sync lost!\n");
				}
		}
	...
	if (IRQ_Vector & SINK_SPECIFIC_IRQ) {
		...
		if (sp_tx_rx_type == DWN_STRM_IS_HDMI_7730) {
				...
				switch( Int_vector[0] & temp) {
					...
					case 0x01:
						if ((Int_vector[0] & 0x01) == 0x01) {//downstream HPD changed from 0 to 1
							...
							if (c & 0x01)
								SLIMPORT_DBG_INFO("Downstream HDMI is pluged!\n");
						}
						break;
					case 0x02:
						sp_tx_aux_dpcdread_bytes(0x00, 0x05, 0x18, 1, &c);
						if ((c & 0x02) != 0x02) {
							SLIMPORT_DBG_INFO("Downstream HDMI is unpluged!\n");
							vbus_power_ctrl(0);
							reg_hardware_reset();
							sp_tx_set_sys_state(STATE_SP_INITIALIZED);
							need_return = 1;
						...
						break;
					case 0x04:
						SLIMPORT_DBG_INFO("Here is check: Link is down!\n");
						if((sp_tx_system_state > STATE_VIDEO_OUTPUT)) {
							SLIMPORT_DBG_INFO("Rx specific  IRQ: Link is down!\n");
							need_return = link_down_check();
						}
						break;
					case 0x08:
						if ((Int_vector[0] & 0x10) != 0x10)
							SLIMPORT_DBG_INFO("Downstream HDCP is passed!\n");
						else {
							#ifndef HDCP_AUTO_EN
							if (sp_tx_system_state >= STATE_HDCP_AUTH) {
								sp_tx_video_mute(1);
								sp_tx_set_sys_state(STATE_HDCP_AUTH);
								SLIMPORT_DBG_INFO("Re-authentication due to downstream HDCP failure!\n");
								sp_tx_clean_hdcp_status();
							}
							#else
								//sp_tx_clean_hdcp_status();
					...
					case 0x10:
						break;
					case 0x20:
						SLIMPORT_DBG_INFO("Downstream HDCP link integrity check fail!\n");
						#ifndef HDCP_AUTO_EN
						system_state_change_with_case(STATE_HDCP_AUTH);
					...
					case 0x40:
						...
						SLIMPORT_DBG_INFO("Receive CEC command from downstream done!\n");
						break;
					case 0x80:
						SLIMPORT_DBG_INFO("CEC command transfer to downstream done!\n");
						break;
				}
			...
			if ((Int_vector[1] & 0x04) == 0x04) {
				sp_tx_aux_dpcdread_bytes(0x00, 0x05, 0x18, 1, &c);
				if ((c & 0x40) == 0x40)
					SLIMPORT_DBG_INFO("Downstream HDMI termination is detected!\n");
			}
	...
	if (IRQ_Vector & TEST_IRQ) {
		...
		if (test_vector & 0x01) {//test link training
			...
			sp_tx_test_bw = c;
			SLIMPORT_DBG_INFO("test_bw = %.2x\n", (uint)sp_tx_test_bw);

			sp_tx_aux_dpcdread_bytes(0x00, 0x02, 0x60,1,&c);
			c = c | TEST_ACK;
			sp_tx_aux_dpcdwrite_bytes(0x00, 0x02, 0x60,1, &c);

			SLIMPORT_DBG_INFO("Set TEST_ACK!\n");
			if (sp_tx_system_state >= STATE_LINK_TRAINING){
				sp_tx_LT_state = LT_INIT;
				sp_tx_set_sys_state(STATE_LINK_TRAINING);
			}
			SLIMPORT_DBG_INFO("IRQ:test-LT request!\n");
		}

		if(test_vector & 0x02){
			//SLIMPORT_DBG_INFO("-------------------test pattern\n");
			sp_tx_aux_dpcdread_bytes(0x00, 0x02, 0x60,1,&c);
			c = c | TEST_ACK;
			sp_tx_aux_dpcdwrite_bytes(0x00, 0x02, 0x60,1, &c);
		}
		if (test_vector & 0x04) {//test edid
			...
			sp_tx_test_edid = 1;
			SLIMPORT_DBG_INFO("Test EDID Requested!\n");
		}
...
static void sp_tx_auth_done_int_handler(void)
{
	...
	if(HDCP_state > HDCP_HW_ENABLE
		...
		if (bytebuf[0] & SP_TX_HDCP_AUTH_PASS) {
			...
			if ((bytebuf[1] & 0x08)||(bytebuf[0]&0x80)){
				SLIMPORT_DBG_INFO("max cascade/devs exceeded!\n");
				sp_tx_hdcp_encryption_disable();
			} else
				SLIMPORT_DBG_INFO("Authentication pass in Auth_Done\n");

			HDCP_state = HDCP_FINISH;

		}else {
			SLIMPORT_ERR("Authentication failed in AUTH_done\n");
			sp_tx_video_mute(1);
			sp_tx_clean_hdcp_status();
	...
	#endif
	SLIMPORT_DBG_INFO("sp_tx_auth_done_int_handler \n");
	...
	{
	//	unchar temp;
		SLIMPORT_DBG_INFO("sp_tx_polling_err_int_handler \n");
...
static void sp_tx_lt_done_int_handler(void)
{
	...
	if(sp_tx_LT_state == LT_WAITTING_FINISH
		...
		if (c & 0x70) {
			c = (c & 0x70) >> 4;
			SLIMPORT_DBG_INFO("LT failed in interrupt, ERR code = %.2x\n", (uint) c);
			sp_tx_LT_state = LT_ERROR;
		} else {
			SLIMPORT_DBG_INFO("lt_done: LT Finish \n");
...
{
	SLIMPORT_DBG_INFO("sp_tx_link_change_int_handler \n");
	if (sp_tx_system_state < STATE_LINK_TRAINING)
		return;
...
{
	SLIMPORT_DBG_INFO("*HDMI_RX Interrupt: Pixel Clock Change.\n");
	if (sp_tx_system_state > STATE_VIDEO_OUTPUT) {
		sp_tx_video_mute(1);
...
{
	SLIMPORT_DBG_INFO("*HDMI_RX Interrupt: Sync Detect.\n");
}
static void hdmi_rx_hdmi_dvi_int(void)
{
	unchar c;
	SLIMPORT_DBG_INFO("hdmi_rx_hdmi_dvi_int. \n");
	sp_read_reg(RX_P0, HDMI_STATUS, &c);
	if ((c & HDMI_MODE) != g_hdmi_dvi_status) {
		SLIMPORT_DBG_INFO("hdmi_dvi_int: Is HDMI MODE: %x.\n", (uint)(c & HDMI_MODE));
		hdmi_rx_mute_audio(0);
...
static void hdmi_rx_avmute_int(void)
{
	...
	if (avmute_status & MUTE_STAT) {
		SLIMPORT_DBG_INFO("HDMI_RX AV mute packet received.\n");
...
{
	SLIMPORT_DBG_INFO("*HDMI_RX Interrupt: New AVI Packet.\n");
	sp_tx_lvttl_bit_mapping();
	sp_tx_set_colorspace();
	sp_tx_avi_setup();
	sp_tx_config_packets(AVI_PACKETS);
...
static void hdmi_rx_new_vsi_int(void)
{
	...
	SLIMPORT_DBG_INFO("*HDMI_RX Interrupt: NEW VSI packet.\n");
	...
	if(!(c&INFO_FRAME_VSC_EN))
	{
		...
		{
			SLIMPORT_DBG_INFO("3D VSI packet is detected. Config VSC packet\n");
			...
			default:
				v3d_structure = 0x00;
				SLIMPORT_DBG_INFO("3D structure is not supported\n");
				break;
			}
...
static void hdmi_rx_no_vsi_int(void)
{
	...
	if(c&INFO_FRAME_VSC_EN)
	{
		SLIMPORT_DBG_INFO("No new VSI is received, disable  VSC packet\n");
		c &= ~INFO_FRAME_VSC_EN;
...
{
	SLIMPORT_DBG_INFO("WAIT_AUDIO: hdmi_rx_restart_audio_chk.\n");
...
static void hdmi_rx_cts_rcv_int(void)
{
	...
	//SLIMPORT_DBG_INFO("*hdmi_rx_cts_rcv_int. \n");
	return;
}
static void hdmi_rx_audio_rcv_int(void)
{
	if(sp_tx_ao_state == AO_INIT)
		...
		sp_tx_ao_state = AO_RCV_INT_FINISH;
	 //SLIMPORT_DBG_INFO("*hdmi_rx_audio_rcv_int\n");
...
static void hdmi_rx_audio_samplechg_int(void)
{
	...
	// SLIMPORT_DBG_INFO("*hdmi_rx_audio_samplechg_int\n");
...
{
	static unchar count = 0;
	SLIMPORT_DBG_INFO("*HDMI_RX Interrupt: hdcp error.\n");
	if(count >= 40)	{
		count = 0;
		SLIMPORT_DBG_INFO("Lots of hdcp error occured ...\n");
		hdmi_rx_mute_audio(1);
		hdmi_rx_mute_video(1);
		hdmi_rx_set_hpd(0);
		mdelay(10);
...
{
	unchar c;
       //SLIMPORT_DBG_INFO("*HDMI_RX Interrupt: New GCP Packet.\n");
...
void sp_tx_show_infomation(void)
{
	...
	switch(c){
		case 0x06:
			SLIMPORT_DBG_INFO(" BW = 1.62G\n");
			break;
		case 0x0a:
			SLIMPORT_DBG_INFO(" BW = 2.7G\n");
			break;
		case 0x14:
			SLIMPORT_DBG_INFO(" BW = 5.4G\n");
			break;
		case 0x19:
			SLIMPORT_DBG_INFO(" BW = 6.75G\n");
			break;
		default:
			break;
	}
	...
	SLIMPORT_DBG_INFO("Total resolution is %d * %d \n", h_res, v_res);

	SLIMPORT_DBG_INFO("HF=%d, HSW=%d, HBP=%d\n", h_fp, h_sw, h_bp);
	SLIMPORT_DBG_INFO("VF=%d, VSW=%d, VBP=%d\n", v_fp, v_sw, v_bp);
	SLIMPORT_DBG_INFO("Active resolution is %d * %d", h_act, v_act);
	...
	SLIMPORT_DBG_INFO("   @ %ldHz\n", fresh_rate);

	sp_read_reg(TX_P0, SP_TX_VID_CTRL, &c);

	if ((c & 0x06) == 0x00)
		SLIMPORT_DBG_INFO("ColorSpace: RGB,");
	else if ((c & 0x06) == 0x02)
		SLIMPORT_DBG_INFO("ColorSpace: YCbCr422,");
	else if ((c & 0x06) == 0x04)
		SLIMPORT_DBG_INFO("ColorSpace: YCbCr444,");

	sp_read_reg(TX_P0, SP_TX_VID_CTRL, &c);

	if ((c & 0xe0) == 0x00)
		SLIMPORT_DBG_INFO("6 BPC\n");
	else if ((c & 0xe0) == 0x20)
		SLIMPORT_DBG_INFO("8 BPC\n");
	else if ((c & 0xe0) == 0x40)
		SLIMPORT_DBG_INFO("10 BPC\n");
	else if ((c & 0xe0) == 0x60)
		SLIMPORT_DBG_INFO("12 BPC\n");


	if(sp_tx_rx_type == DWN_STRM_IS_HDMI_7730) {//assuming it is anx7730
		sp_tx_aux_dpcdread_bytes(0x00, 0x05, 0x23, 1, &c);
		SLIMPORT_DBG_INFO("ANX7730 current FW Ver %.2x \n", (uint)(c & 0x7f));
	}
...
void hdmi_rx_show_video_info(void)
{
    ...
    SLIMPORT_DBG_INFO(">HDMI_RX Info<\n");
    sp_read_reg(RX_P0,HDMI_STATUS, &c);
    if(c & HDMI_MODE)
        SLIMPORT_DBG_INFO("HDMI_RX Mode = HDMI Mode.\n");
    else
        SLIMPORT_DBG_INFO("HDMI_RX Mode = DVI Mode.\n");
    ...
    SLIMPORT_DBG_INFO("HDMI_RX Video Resolution = %d * %d ",h_res,v_res);
    sp_read_reg(RX_P0,HDMI_RX_VIDEO_STATUS_REG1, &c);
    if(c & VIDEO_TYPE)
        SLIMPORT_DBG_INFO("Interlace Video.\n");
    else
        SLIMPORT_DBG_INFO("Progressive Video.\n");


    sp_read_reg(RX_P0,HDMI_RX_SYS_CTRL1_REG, &c);
    if((c & 0x30) == 0x00)
        SLIMPORT_DBG_INFO("Input Pixel Clock = Not Repeated.\n");
    else if((c & 0x30) == 0x10)
        SLIMPORT_DBG_INFO("Input Pixel Clock = 2x Video Clock. Repeated.\n");
    else if((c & 0x30) == 0x30)
        SLIMPORT_DBG_INFO("Input Pixel Clock = 4x Vvideo Clock. Repeated.\n");

    if((c & 0xc0) == 0x00)
        SLIMPORT_DBG_INFO("Output Video Clock = Not Divided.\n");
    else if((c & 0xc0) == 0x40)
        SLIMPORT_DBG_INFO("Output Video Clock = Divided By 2.\n");
    else if((c & 0xc0) == 0xc0)
        SLIMPORT_DBG_INFO("Output Video Clock = Divided By 4.\n");

    if(c & 0x02)
        SLIMPORT_DBG_INFO("Output Video Using Rising Edge To Latch Data.\n");
    else
        SLIMPORT_DBG_INFO("Output Video Using Falling Edge To Latch Data.\n");


    SLIMPORT_DBG_INFO("Input Video Color Depth = ");
    sp_read_reg(RX_P0,0x70, &c1);
	 c1 &= 0xf0;
	 if(c1 == 0x00)
            SLIMPORT_DBG_INFO("Legacy Mode.\n");
	 else if(c1 == 0x40)
            SLIMPORT_DBG_INFO("24 Bit Mode.\n");
        else if(c1 == 0x50)
            SLIMPORT_DBG_INFO("30 Bit Mode.\n");
        else if(c1 == 0x60)
            SLIMPORT_DBG_INFO("36 Bit Mode.\n");
        else if(c1 == 0x70)
            SLIMPORT_DBG_INFO("48 Bit Mode.\n");



    SLIMPORT_DBG_INFO("Input Video Color Space = ");
    sp_read_reg(RX_P1,HDMI_RX_AVI_DATA00_REG, &c);
    c &= 0x60;
    if(c == 0x20)
        SLIMPORT_DBG_INFO("YCbCr4:2:2 .\n");
    else if(c == 0x40)
        SLIMPORT_DBG_INFO("YCbCr4:4:4 .\n");
    else if(c == 0x00)
        SLIMPORT_DBG_INFO("RGB.\n");
    else
        SLIMPORT_ERR("Unknow 0x44 = 0x%.2x\n", (int)c);



    sp_read_reg(RX_P1,HDMI_RX_HDCP_STATUS_REG, &c);
    if(c & AUTH_EN)
        SLIMPORT_DBG_INFO("Authentication is attempted.\n");
    else
        SLIMPORT_DBG_INFO("Authentication is not attempted.\n");
    ...
    if(cl < 20)
        SLIMPORT_DBG_INFO("Decryption is active.\n");
    else
        SLIMPORT_DBG_INFO("Decryption is not active.\n");
...
void clean_system_status(void)
{
	if(g_need_clean_status) {
		SLIMPORT_DBG_INFO("clean_system_status. A -> B; \n");
		SLIMPORT_DBG_INFO("A:");
...
void sp_tx_get_ddc_hdcp_BCAP(unchar *bcap)
{
	...
	//SLIMPORT_DBG_INFO("HDCP BCAP = %.2x\n", (uint)*bcap);
...
void sp_tx_get_ddc_hdcp_BKSV(unchar *bksv)
{
	...
	//SLIMPORT_DBG_INFO("HDCP BKSV0 = %.2x\n", (uint)*bksv);
...
unchar slimport_hdcp_cap_check(void)
{
	...
	if((sp_tx_rx_type == DWN_STRM_IS_VGA_9832)
		...
		if(AUX_OK == sp_tx_aux_dpcdread_bytes(0x06, 0x80, 0x28, 1, &c)) {
			...
			SLIMPORT_DBG_INFO("HDCP CAPABLE: read AUX err! \n");
...
static void hdmi_rx_cec_rcv_int(void)
{
	...
	SLIMPORT_DBG_INFO("HDMI RX received CEC\n");
#endif
...
void CEC_init_fun(void)
{
	...
	if(sp_tx_rx_type == DWN_STRM_IS_HDMI_7730){
		cec_init();//for CEC
		SLIMPORT_DBG_INFO("cec initialed!\n");
	}
}
...
void simulate_watchdog_func(void)
{
	...
	if((sp_tx_cur_states() < STATE_AUDIO_OUTPUT)
		...
		if(bak_states > 200){
			bak_states = 0;
			system_power_ctrl(0);
			SLIMPORT_ERR("SlimPort Watchdog start!\n");
		}
	}else{
		if(bak_states != 0)
			bak_states = 0;
