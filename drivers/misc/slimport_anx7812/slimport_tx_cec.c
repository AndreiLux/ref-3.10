...
//#ifdef CEC_DBG_MSG_ENABLED
    //SLIMPORT_DBG_INFO("CEC status =%.2x\n", (uint)cstatus);
//#endif
...
unsigned char downstream_cec_sendframe(unsigned char *pFrameData, unsigned char Len)
{
    ...
    for(i = 5; i; i--)
    {
        ...
        //SLIMPORT_DBG_INFO("DPCD 00511 = %.2x\n", (uint)c);
...
//#ifdef CEC_DBG_MSG_ENABLED
    //SLIMPORT_DBG_INFO("HDMI CEC status =%.2x\n", (uint)cStatus);
//#endif
    //SP_TX_AUX_DPCDRead_Bytes(0x00,0x05,0x71,1,&c);
    //SLIMPORT_DBG_INFO("CEC status =%.2x\n",(uint)c);
...
unsigned char upstream_cec_sendframe(unsigned char *pFrameData, unsigned char Len)
{
	...
	time_out_counter = 0;
    //SLIMPORT_DBG_INFO("send to up len = %.2x\n",(unsigned char)Len);
    // According to CEC 7.1, re-transmission can be attempted up to 5 times
    for(i = 5; i; i--)
    {
        for(pDataTmp = pFrameData, LenTmp = Len; LenTmp; LenTmp--)
        ...
        //SLIMPORT_DBG_INFO("0x7E:0xD2 = %.2x\n",(uint)c);
...
{
	SLIMPORT_DBG_INFO("00 set phy addr[0] = %.2x,phy addr[1] = %.2x,\n",
...
void DownStream_CEC_ParseMessage(void)
{
    ...
    case CEC_OPCODE_REPORT_PHYSICAL_ADDRESS:
		...
		SLIMPORT_DBG_INFO("init address = %.2x\n",(uint)c);

		c = (unchar)(g_CECFrame.header.dest);
		SLIMPORT_DBG_INFO("dest address = %.2x\n",(uint)c);

		c = (unchar)(g_CECFrame.msg.raw[0]);
		SLIMPORT_DBG_INFO("message raw[0] = %.2x\n",(uint)c);

		c = (unchar)(g_CECFrame.msg.raw[1]);
		SLIMPORT_DBG_INFO("message raw[1] = %.2x\n",(uint)c);

		c = (unchar)(g_CECFrame.msg.raw[2]);
		SLIMPORT_DBG_INFO("message raw[2] = %.2x\n",(uint)c);

		c = (unchar)(g_CECFrame.msg.raw[3]);
		SLIMPORT_DBG_INFO("message raw[3] = %.2x\n",(uint)c);

		c = (unchar)(g_CECFrame.msg.raw[4]);
		SLIMPORT_DBG_INFO("message raw[4] = %.2x\n",(uint)c);

		c = (unchar)(g_CECFrame.msg.raw[5]);
		SLIMPORT_DBG_INFO("message raw[5] = %.2x\n",(uint)c);
       break;
...
void uptream_cec_parsemessage(void)
{
    ...
    case CEC_OPCODE_REPORT_PHYSICAL_ADDRESS:
		SLIMPORT_DBG_INFO("parse phy addr[0] = %.2x,phy addr[1] = %.2x,\n",
