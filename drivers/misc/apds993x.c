...
unsigned int ps_real_detection = -1;
#endif
...
static void apds993x_change_ps_threshold(struct i2c_client *client)
{
	...
	if ((data->ps_data > data->pilt) && (data->ps_data >= data->piht)) {
		...
		data->ps_detection = 1;
#ifdef LIGER_SUSPEND_KNOCK_ON
		ps_real_detection = data->ps_detection;
#endif
		/* FAR-to-NEAR detection */
		pr_debug("adps993x FAR-TO-NEAR data->ps_data = %d, data->pilt = %d, data->piht = %d\n",
		...
		if(!forced_far){
			...
			apds993x_set_pilt(client, FORCED_PILT);
			apds993x_set_piht(client, 1023);
			forced_far = 0;
			pr_info("%s: forced far\n", __func__);
		}
#else
		apds993x_set_pilt(client, data->ps_hysteresis_threshold);
		apds993x_set_piht(client, 1023);
		...
		pr_debug("adps993x NEAR-TO-FAR data->ps_data = %d, data->pilt = %d, data->piht = %d\n",
					data->ps_data, data->pilt, data->piht);
...
#ifdef PS_AMBIENT_SUNLIGHT
		if(check_sunlight&&(data->ps_data > data->ps_threshold)) {
			/* near-to-far detected */
			data->ps_detection = 0;
#ifdef LIGER_SUSPEND_KNOCK_ON
			ps_real_detection = data->ps_detection;
			...
			pr_info("%s: sunlight near-to-far >>> pdata(%d), data->ps_threshold(%d)\n",
...
static void apds993x_work_handler(struct work_struct *work)
{
	...
	if ((status & enable & 0x30) == 0x30) {
		...
		if (ch0data <
			...
			if((status & 0x40) == 0x40) {
				check_sunlight = 1;
				pr_info("%s: goto FORCED_FAR_CHECK ch0data = %d\n", __FUNCTION__, ch0data);
				goto FORCED_FAR_CHECK;
			}
#endif
			apds993x_change_ps_threshold(client);
		} else {
...
FORCED_FAR_CHECK:
				pr_info("%s: background ambient noise  check_sunlight = %d\n", __func__, check_sunlight);

				data->ps_data =	i2c_smbus_read_word_data(client, CMD_WORD|APDS993X_PDATAL_REG);
				pr_debug("adps993x data->ps_data = %d, data->pilt = %d, data->piht = %d, check_sunlight = %d\n",
					data->ps_data, data->pilt, data->piht, check_sunlight);

				if(check_sunlight && (data->ps_data != 1023)){
					/* forced NEAR-to-FAR detection */
					data->ps_detection = 0;
#ifdef LIGER_SUSPEND_KNOCK_ON
					ps_real_detection = data->ps_detection;
#endif
					...
					apds993x_set_piht(client, data->ps_threshold);
					forced_far = 1;
					pr_info("apds993x forced NEAR_TO_FAR\n");
				}
					...
					apds993x_set_pilt(client, FORCED_PILT);
					apds993x_set_piht(client, 1023);
					pr_info("apds993x forced FAR_TO_NEAR (psdata = 1023)\n");
				}
				check_sunlight = 0;
#else
				pr_info("%s: background ambient noise\n", __func__);
...
static DEVICE_ATTR(ps_default_crosstalk, S_IWUSR | S_IWGRP | S_IRUGO,
...
static DEVICE_ATTR(adjust_crosstalk, S_IWUSR | S_IWGRP | S_IRUGO,
...
#if 0 /* Don't need regulator setting in odin */
	if (pdata->power_on)
