	...
	if (false == i2c->dma_en) {
		if (I2C_MASTER_WRRD != i2c->op) {
			...
			if (!trans_len || !trans_num || trans_len * trans_num > 8) {
				dev_err(i2c->dev,
					"mt6573-i2c: non-WRRD transfer length is not right. trans_len=%x, tans_num=%x, trans_auxlen=%x\n",
			...
			if (!trans_len || !trans_auxlen || trans_len > 8 || trans_auxlen > 8) {
				dev_err(i2c->dev,
					"mt6573-i2c: WRRD transfer length is not right. trans_len=%x, tans_num=%x, trans_auxlen=%x\n",
					trans_len, trans_num, trans_auxlen);
			}
		}
	} else {
		if (I2C_MASTER_WRRD != i2c->op) {
			...
			if (!trans_len || !trans_num || trans_len > 255 || trans_num > 255) {
				dev_err(i2c->dev,
					"mt6573-i2c: DMA non-WRRD transfer length is not right. trans_len=%x, tans_num=%x, trans_auxlen=%x\n",
					trans_len, trans_num, trans_auxlen);
			}
		} else {
		...
		if (i2c->poll_en) {
			while (1) {
				...
				if (i2c->irq_stat) {
					...
					I2C_CLR_INTR_STATUS(I2C_HS_NACKERR | I2C_ACKERR |
							    I2C_TRANSAC_COMP);
					break;
				}
				tmo_poll--;
...
/* dev_info(i2c->dev, "addr %.2x read byte = 0x%.2X\n", addr, *ptr); */
		...
		if (i2c->dma_en) {
			if ((u32) ptr > 0xc0000000)
				dev_err(i2c->dev,
					"mt6573-i2c: DMA mode should use physical buffer address!\n");
...
/* hwDisableClock(MT65XX_PDN_PERI_APDMA, "i2c"); */
	...
	if (tmo == 0) {
		...
		dev_err(i2c->dev,
			"I2C structure:\nMode %x\nSt_rs %x\nDma_en %x\nOp %x\nPoll_en %x\nTrans_len %x\nTrans_num %x\nTrans_auxlen %x\nData_size %x\nIrq_stat %x\nTrans_stop %d\nTrans_comp %d\nTrans_error %d\n",
			i2c->mode, i2c->st_rs, i2c->dma_en, i2c->op, i2c->poll_en, trans_len,
			trans_num, trans_auxlen, data_size, i2c->irq_stat, i2c->trans_stop,
			i2c->trans_comp, i2c->trans_err);

		dev_err(i2c->dev,
			"I2C register:\nSLAVE_ADDR %x\nINTR_MASK %x\nINTR_STAT %x\nCONTROL %x\nTRANSFER_LEN %x\nTRANSAC_LEN %x\nDELAY_LEN %x\nTIMING %x\nSTART %x\nFIFO_STAT %x\nIO_CONFIG %x\nHS %x\nDEBUGSTAT %x\n",
			(__raw_readl(mt6573_I2C_SLAVE_ADDR)), (__raw_readl(mt6573_I2C_INTR_MASK)),
			(__raw_readl(mt6573_I2C_INTR_STAT)), (__raw_readl(mt6573_I2C_CONTROL)),
			(__raw_readl(mt6573_I2C_TRANSFER_LEN)),
			(__raw_readl(mt6573_I2C_TRANSAC_LEN)), (__raw_readl(mt6573_I2C_DELAY_LEN)),
			(__raw_readl(mt6573_I2C_TIMING)), (__raw_readl(mt6573_I2C_START)),
			(__raw_readl(mt6573_I2C_FIFO_STAT)), (__raw_readl(mt6573_I2C_IO_CONFIG)),
			(__raw_readl(mt6573_I2C_HS)), (__raw_readl(mt6573_I2C_DEBUGSTAT)));

		dev_err(i2c->dev,
			"DMA register:\nINT_FLAG %x\nCON %x\nMEM_ADDR %x\nLEN %x\nINT_EN %x\nEN %x\n",
			(__raw_readl(i2c->pdmabase + OFFSET_INT_FLAG)),
			(__raw_readl(i2c->pdmabase + OFFSET_CON)),
			(__raw_readl(i2c->pdmabase + OFFSET_MEM_ADDR)),
			(__raw_readl(i2c->pdmabase + OFFSET_LEN)),
		...
		if (likely(!(addr & I2C_A_FILTER_MSG))) {
			...
			dev_err(i2c->dev,
				"I2C structure:\nMode %x\nSt_rs %x\nDma_en %x\nOp %x\nPoll_en %x\nTrans_len %x\nTrans_num %x\nTrans_auxlen %x\nData_size %x\nIrq_stat %x\nTrans_stop %d\nTrans_comp %d\nTrans_error %d\n",
				i2c->mode, i2c->st_rs, i2c->dma_en, i2c->op, i2c->poll_en,
				trans_len, trans_num, trans_auxlen, data_size, i2c->irq_stat,
				i2c->trans_stop, i2c->trans_comp, i2c->trans_err);

			dev_err(i2c->dev,
				"I2C register:\nSLAVE_ADDR %x\nINTR_MASK %x\nINTR_STAT %x\nCONTROL %x\nTRANSFER_LEN %x\nTRANSAC_LEN %x\nDELAY_LEN %x\nTIMING %x\nSTART %x\nFIFO_STAT %x\nIO_CONFIG %x\nHS %x\nDEBUGSTAT %x\n",
				(__raw_readl(mt6573_I2C_SLAVE_ADDR)),
				(__raw_readl(mt6573_I2C_INTR_MASK)),
				(__raw_readl(mt6573_I2C_INTR_STAT)),
				(__raw_readl(mt6573_I2C_CONTROL)),
				(__raw_readl(mt6573_I2C_TRANSFER_LEN)),
				(__raw_readl(mt6573_I2C_TRANSAC_LEN)),
				(__raw_readl(mt6573_I2C_DELAY_LEN)),
				(__raw_readl(mt6573_I2C_TIMING)), (__raw_readl(mt6573_I2C_START)),
				(__raw_readl(mt6573_I2C_FIFO_STAT)),
				(__raw_readl(mt6573_I2C_IO_CONFIG)), (__raw_readl(mt6573_I2C_HS)),
				(__raw_readl(mt6573_I2C_DEBUGSTAT)));
...
/* while(1){ */
		...
		number++;
		printk(KERN_INFO
		       "I2C: Fast Speed Mode + Fifo Mode Test.......................stress number = %d\n",
		       number);
...
/* printk(KERN_INFO "client->timing: %d\n", client->timing); */
		...
		number++;
		printk(KERN_INFO
		       "I2C: High Speed Mode + Fifo Mode Test..................Stess number = %d\n",
		       number);
		...
		number++;
		printk(KERN_INFO
		       "I2C: Fast Speed Mode + Fifo Mode + Write-Read Mode Test.....Stress number = %d\n",
		       number);
		...
		number++;
		printk(KERN_INFO
		       "I2C: Fast Speed Mode + Fifo Mode + Multi Transac Mode Test....Stress number =%d\n",
		       number);
		...
		number++;
		printk(KERN_INFO
		       "I2C: Fast Speed Mode + DMA Mode Test................Stress number =%d\n",
		       number);
		...
		printk(KERN_INFO
		       "I2C: High Speed Mode + DMA Mode Test...................Stress number = %d\n",
		       number);
...
*/
		number++;
		printk(KERN_INFO
		       "I2C: Fast Speed Mode + DMA Mode + Multi Transac Mode Test......Stress number =%d\n",
		       number);
		...
		number++;
		printk(KERN_INFO
		       "I2C: High Speed Mode + Fifo Mode + Write-Read Mode Test.....Stress number = %d\n",
		       number);
		...
		number++;
		printk(KERN_INFO
		       "I2C: High Speed Mode + Fifo Mode + Multi Transac Mode Test....Stress number =%d\n",
		       number);
...
*/
		...
		number++;
		printk(KERN_INFO
		       "I2C: High Speed Mode + DMA Mode + Multi Transac Mode Test....Stress number =%d\n",
		       number);
...
static void __init mt6573_test(void)
{
