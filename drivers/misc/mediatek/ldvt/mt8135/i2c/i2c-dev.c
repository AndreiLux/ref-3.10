...
static long i2cdev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case I2C_UVVF_REPEAT_N7:
		number++;
		printk(KERN_INFO
		       "I2C: Fast Speed Mode + DMA Mode + Write-Read Mode Test.....Stress number = %d\n",
		       number);
		virt = dma_alloc_coherent(NULL, 4096, &phys, GFP_KERNEL);
		printk(KERN_INFO "Address: virt = %x, phys = %x\n", virt, phys);
	...
	case I2C_UVVF_REPEAT_N9:
		number++;
		printk(KERN_INFO
		       "I2C: High Speed Mode + DMA Mode + Write-Read Mode Test.....Stress number = %d\n",
		       number);
		virt = dma_alloc_coherent(NULL, 4096, &phys, GFP_KERNEL);
		printk(KERN_INFO "Address: virt = %x, phys = %x\n", virt, phys);
		...
		client->addr =
		    0xee & I2C_MASK_FLAG | I2C_WR_FLAG | I2C_RS_FLAG | I2C_DMA_FLAG | I2C_HS_FLAG;
