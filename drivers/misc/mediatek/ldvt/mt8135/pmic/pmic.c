...
void pmic_UVVP_PMIC_TOP_WR(int test_value)
{
	...
	for (u2Cnt = 0; u2Cnt < (sizeof(aPMURegBeg_bank0_write) / sizeof(*aPMURegBeg_bank0_write));
	     ++u2Cnt) {
		/* write test value */
...
/* wake_up_pmic(); */
	...
	printk
	    ("LOWBATTERY (AUXADC) interrupt setting .. done (adc_out_lbat=%d, lbat_debounce_count_max=%d, lbat_debounce_count_min=%d)\n",
	...
	if (g_buck_num == 0) {
		...
		if (g_log_buck_vosel == 1) {
			...
			printk("Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x\n",
			       ...
			       0x214, upmu_get_reg_value(0x214), 0x21e, upmu_get_reg_value(0x21e)
			    );
		}
		...
		if (g_log_buck_vosel == 1) {
			...
			printk("Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x\n",
			       0x128, upmu_get_reg_value(0x128),
			       0x23a, upmu_get_reg_value(0x23a), 0x244, upmu_get_reg_value(0x244)
			    );
		}
		...
		if (g_log_buck_vosel == 1) {
			...
			printk("Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x\n",
			       0x128, upmu_get_reg_value(0x128),
			       0x266, upmu_get_reg_value(0x266), 0x270, upmu_get_reg_value(0x270)
			    );
		}
		...
		if (g_log_buck_vosel == 1) {
			...
			printk("Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x\n",
			       0x128, upmu_get_reg_value(0x128),
			       0x28c, upmu_get_reg_value(0x28c), 0x296, upmu_get_reg_value(0x296)
			    );
		}
		...
		if (g_log_buck_vosel == 1) {
			...
			printk("Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x\n",
			       0x128, upmu_get_reg_value(0x128),
			       0x30e, upmu_get_reg_value(0x30e), 0x318, upmu_get_reg_value(0x318)
			    );
		}
		...
		if (g_log_buck_vosel == 1) {
			...
			printk("Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x\n",
			       0x128, upmu_get_reg_value(0x128),
			       0x334, upmu_get_reg_value(0x334), 0x33e, upmu_get_reg_value(0x33e)
			    );
		}
		...
		if (g_log_buck_vosel == 1) {
			...
			printk("Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x\n",
			       0x128, upmu_get_reg_value(0x128),
			       0x35e, upmu_get_reg_value(0x35e), 0x368, upmu_get_reg_value(0x368)
			    );
		}
		...
		if (g_log_buck_vosel == 1) {
			...
			printk("Reg[%x]=0x%x,Reg[%x]=0x%x,Reg[%x]=0x%x\n",
			       0x128, upmu_get_reg_value(0x128),
			       0x388, upmu_get_reg_value(0x388), 0x392, upmu_get_reg_value(0x392)
			    );
		}
	} else {
	}
...
void do_vrf18_en_test(int index_val)
{
	...
	for (i = 0; i <= 1; i++) {
		...
		case 0:
			...
			upmu_set_vrf18_md1_en_sel(0);
			upmu_set_vrf18_md2_en_sel(0);

			printk("[upmu_set_vrf18_en(0); upmu_set_vrf18_md2_en(0);] ");
			upmu_set_vrf18_en(0);
			upmu_set_vrf18_md2_en(0);
			g_buck_num = 6;
			read_auxadc_value();
			g_buck_num = 100;
...
void do_vproc_dlc_subtest(void)
{
	...
	for (i = 0; i <= PMIC_VPROC_DLC_SLEEP_MASK; i++) {
		...
		printk
		    ("[do_vproc_dlc_subtest] upmu_set_vproc_dlc_sleep=%d, upmu_set_vproc_dlc_n_sleep=%d\n",
		     i, i);

		printk
		    ("[do_vproc_dlc_subtest] upmu_get_qi_vproc_dlc=%d, upmu_get_qi_vproc_dlc_n=%d\n",
	...
	for (i = 0; i <= PMIC_VPROC_DLC_ON_MASK; i++) {
		...
		printk
		    ("[do_vproc_dlc_subtest] upmu_set_vproc_dlc_on=%d, upmu_set_vproc_dlc_n_on=%d\n",
		     i, i);
...
void do_vsram_dlc_subtest(void)
{
	...
	for (i = 0; i <= PMIC_VSRAM_DLC_SLEEP_MASK; i++) {
		...
		printk
		    ("[do_vsram_dlc_subtest] upmu_set_vsram_dlc_sleep=%d, upmu_set_vsram_dlc_n_sleep=%d\n",
		     i, i);

		printk
		    ("[do_vsram_dlc_subtest] upmu_get_qi_vsram_dlc=%d, upmu_get_qi_vsram_dlc_n=%d\n",
	...
	for (i = 0; i <= PMIC_VSRAM_DLC_ON_MASK; i++) {
		...
		printk
		    ("[do_vsram_dlc_subtest] upmu_set_vsram_dlc_on=%d, upmu_set_vsram_dlc_n_on=%d\n",
		     i, i);
...
void do_vcore_dlc_subtest(void)
{
	...
	for (i = 0; i <= PMIC_VCORE_DLC_SLEEP_MASK; i++) {
		...
		printk
		    ("[do_vcore_dlc_subtest] upmu_set_vcore_dlc_sleep=%d, upmu_set_vcore_dlc_n_sleep=%d\n",
		     i, i);

		printk
		    ("[do_vcore_dlc_subtest] upmu_get_qi_vcore_dlc=%d, upmu_get_qi_vcore_dlc_n=%d\n",
	...
	for (i = 0; i <= PMIC_VCORE_DLC_ON_MASK; i++) {
		...
		printk
		    ("[do_vcore_dlc_subtest] upmu_set_vcore_dlc_on=%d, upmu_set_vcore_dlc_n_on=%d\n",
		     i, i);
...
void do_vio18_dlc_subtest(void)
{
	...
	for (i = 0; i <= PMIC_VIO18_DLC_SLEEP_MASK; i++) {
		...
		printk
		    ("[do_vio18_dlc_subtest] upmu_set_vio18_dlc_sleep=%d, upmu_set_vio18_dlc_n_sleep=%d\n",
		     i, i);

		printk
		    ("[do_vio18_dlc_subtest] upmu_get_qi_vio18_dlc=%d, upmu_get_qi_vio18_dlc_n=%d\n",
	...
	for (i = 0; i <= PMIC_VIO18_DLC_ON_MASK; i++) {
		...
		printk
		    ("[do_vio18_dlc_subtest] upmu_set_vio18_dlc_on=%d, upmu_set_vio18_dlc_n_on=%d\n",
		     i, i);
...
void do_vrf18_dlc_subtest(void)
{
	...
	for (i = 0; i <= PMIC_VRF18_DLC_SLEEP_MASK; i++) {
		...
		printk
		    ("[do_vrf18_dlc_subtest] upmu_set_vrf18_dlc_sleep=%d, upmu_set_vrf18_dlc_n_sleep=%d\n",
		     i, i);

		printk
		    ("[do_vrf18_dlc_subtest] upmu_get_qi_vrf18_dlc=%d, upmu_get_qi_vrf18_dlc_n=%d\n",
	...
	for (i = 0; i <= PMIC_VRF18_DLC_ON_MASK; i++) {
		...
		printk
		    ("[do_vrf18_dlc_subtest] upmu_set_vrf18_dlc_on=%d, upmu_set_vrf18_dlc_n_on=%d\n",
		     i, i);
...
void do_vrf18_2_dlc_subtest(void)
{
	...
	for (i = 0; i <= PMIC_VRF18_2_DLC_SLEEP_MASK; i++) {
		...
		printk
		    ("[do_vrf18_2_dlc_subtest] upmu_set_vrf18_2_dlc_sleep=%d, upmu_set_vrf18_2_dlc_n_sleep=%d\n",
		     i, i);

		printk
		    ("[do_vrf18_2_dlc_subtest] upmu_get_qi_vrf18_2_dlc=%d, upmu_get_qi_vrf18_2_dlc_n=%d\n",
	...
	for (i = 0; i <= PMIC_VRF18_2_DLC_ON_MASK; i++) {
		...
		printk
		    ("[do_vrf18_2_dlc_subtest] upmu_set_vrf18_2_dlc_on=%d, upmu_set_vrf18_2_dlc_n_on=%d\n",
		     i, i);
	...
	for (i = 0; i <= 1; i++) {
		...
		case 0:
			for (j = 0; j <= PMIC_VPROC_DLC_MASK; j++) {
				...
				if ((upmu_get_qi_vproc_dlc() != j)
				    || (upmu_get_qi_vproc_dlc_n() != j)) {
					printk
					    ("[do_vproc_dlc_test] fail at upmu_get_qi_vproc_dlc=%d, upmu_get_qi_vproc_dlc_n=%d\n",
					     upmu_get_qi_vproc_dlc(), upmu_get_qi_vproc_dlc_n());
				}

				printk
				    ("[do_vproc_dlc_test] upmu_set_vproc_dlc=%d, upmu_set_vproc_dlc_n=%d, upmu_get_qi_vproc_dlc=%d, upmu_get_qi_vproc_dlc_n=%d\n",
	...
	for (i = 0; i <= 1; i++) {
		...
		case 0:
			for (j = 0; j <= PMIC_VSRAM_DLC_MASK; j++) {
				...
				if ((upmu_get_qi_vsram_dlc() != j)
				    || (upmu_get_qi_vsram_dlc_n() != j)) {
					printk
					    ("[do_vsram_dlc_test] fail at upmu_get_qi_vsram_dlc=%d, upmu_get_qi_vsram_dlc_n=%d\n",
					     upmu_get_qi_vsram_dlc(), upmu_get_qi_vsram_dlc_n());
				}

				printk
				    ("[do_vsram_dlc_test] upmu_set_vsram_dlc=%d, upmu_set_vsram_dlc_n=%d, upmu_get_qi_vsram_dlc=%d, upmu_get_qi_vsram_dlc_n=%d\n",
	...
	for (i = 0; i <= 1; i++) {
		...
		case 0:
			for (j = 0; j <= PMIC_VCORE_DLC_MASK; j++) {
				...
				if ((upmu_get_qi_vcore_dlc() != j)
				    || (upmu_get_qi_vcore_dlc_n() != j)) {
					printk
					    ("[do_vcore_dlc_test] fail at upmu_get_qi_vcore_dlc=%d, upmu_get_qi_vcore_dlc_n=%d\n",
					     upmu_get_qi_vcore_dlc(), upmu_get_qi_vcore_dlc_n());
				}

				printk
				    ("[do_vcore_dlc_test] upmu_set_vcore_dlc=%d, upmu_set_vcore_dlc_n=%d, upmu_get_qi_vcore_dlc=%d, upmu_get_qi_vcore_dlc_n=%d\n",
		...
		case 0:
			for (j = 0; j <= PMIC_VM_DLC_MASK; j++) {
				...
				if ((upmu_get_qi_vm_dlc() != j) || (upmu_get_qi_vm_dlc_n() != j)) {
					printk
					    ("[do_vm_dlc_test] fail at upmu_get_qi_vm_dlc=%d, upmu_get_qi_vm_dlc_n=%d\n",
					     upmu_get_qi_vm_dlc(), upmu_get_qi_vm_dlc_n());
				}

				printk
				    ("[do_vm_dlc_test] upmu_set_vm_dlc=%d, upmu_set_vm_dlc_n=%d, upmu_get_qi_vm_dlc=%d, upmu_get_qi_vm_dlc_n=%d\n",
	...
	for (i = 0; i <= 1; i++) {
		...
		case 0:
			for (j = 0; j <= PMIC_VIO18_DLC_MASK; j++) {
				...
				if ((upmu_get_qi_vio18_dlc() != j)
				    || (upmu_get_qi_vio18_dlc_n() != j)) {
					printk
					    ("[do_vio18_dlc_test] fail at upmu_get_qi_vio18_dlc=%d, upmu_get_qi_vio18_dlc_n=%d\n",
					     upmu_get_qi_vio18_dlc(), upmu_get_qi_vio18_dlc_n());
				}

				printk
				    ("[do_vio18_dlc_test] upmu_set_vio18_dlc=%d, upmu_set_vio18_dlc_n=%d, upmu_get_qi_vio18_dlc=%d, upmu_get_qi_vio18_dlc_n=%d\n",
	...
	for (i = 0; i <= 1; i++) {
		...
		case 0:
			for (j = 0; j <= PMIC_VPA_DLC_MASK; j++) {
				...
				printk
				    ("[do_vpa_dlc_test] upmu_set_vpa_dlc=%d, upmu_get_qi_vpa_dlc=%d\n",
	...
	for (i = 0; i <= 1; i++) {
		...
		case 0:
			for (j = 0; j <= PMIC_VRF18_DLC_MASK; j++) {
				...
				if ((upmu_get_qi_vrf18_dlc() != j)
				    || (upmu_get_qi_vrf18_dlc_n() != j)) {
					printk
					    ("[do_vrf18_dlc_test] fail at upmu_get_qi_vrf18_dlc=%d, upmu_get_qi_vrf18_dlc_n=%d\n",
					     upmu_get_qi_vrf18_dlc(), upmu_get_qi_vrf18_dlc_n());
				}

				printk
				    ("[do_vrf18_dlc_test] upmu_set_vrf18_dlc=%d, upmu_set_vrf18_dlc_n=%d, upmu_get_qi_vrf18_dlc=%d, upmu_get_qi_vrf18_dlc_n=%d\n",
	...
	for (i = 0; i <= 1; i++) {
		...
		case 0:
			for (j = 0; j <= PMIC_VRF18_2_DLC_MASK; j++) {
				...
				if ((upmu_get_qi_vrf18_2_dlc() != j)
				    || (upmu_get_qi_vrf18_2_dlc_n() != j)) {
					printk
					    ("[do_vrf18_2_dlc_test] fail at upmu_get_qi_vrf18_2_dlc=%d, upmu_get_qi_vrf18_2_dlc_n=%d\n",
					     upmu_get_qi_vrf18_2_dlc(),
					     upmu_get_qi_vrf18_2_dlc_n());
				}

				printk
				    ("[do_vrf18_2_dlc_test] upmu_set_vrf18_2_dlc=%d, upmu_set_vrf18_2_dlc_n=%d, upmu_get_qi_vrf18_2_dlc=%d, upmu_get_qi_vrf18_2_dlc_n=%d\n",
...
void do_vio18_burst_subtest(void)
{
	...
	for (i = 0; i <= PMIC_VIO18_BURSTH_SLEEP_MASK; i++) {
		...
		printk
		    ("[do_vio18_burst_subtest] upmu_set_vio18_bursth_sleep=%d, upmu_set_vio18_burstl_sleep=%d\n",
		     i, i);

		printk
		    ("[do_vio18_burst_subtest] upmu_get_qi_vio18_burst=%d, upmu_get_qi_vio18_burst_n=%d\n",
	...
	for (i = 0; i <= PMIC_VIO18_BURSTH_ON_MASK; i++) {
		...
		printk
		    ("[do_vio18_burst_subtest] upmu_set_vio18_bursth_on=%d, upmu_set_vio18_burstl_on=%d\n",
		     i, i);
...
void do_vpa_burst_subtest(void)
{
	...
	for (i = 0; i <= PMIC_VPA_BURSTH_SLEEP_MASK; i++) {
		...
		printk
		    ("[do_vpa_burst_subtest] upmu_set_vpa_bursth_sleep=%d, upmu_set_vpa_burstl_sleep=%d\n",
		     i, i);

		printk
		    ("[do_vpa_burst_subtest] upmu_get_qi_vpa_burst=%d, upmu_get_qi_vpa_burst_n=%d\n",
	...
	for (i = 0; i <= PMIC_VPA_BURSTH_ON_MASK; i++) {
		...
		printk
		    ("[do_vpa_burst_subtest] upmu_set_vpa_bursth_on=%d, upmu_set_vpa_burstl_on=%d\n",
		     i, i);
...
void do_vrf18_burst_subtest(void)
{
	...
	for (i = 0; i <= PMIC_VRF18_BURSTH_SLEEP_MASK; i++) {
		...
		mdelay(1);
		printk
		    ("[do_vrf18_burst_subtest] upmu_set_vrf18_bursth_sleep=%d, upmu_set_vrf18_burstl_sleep=%d\n",
		     i, i);
		mdelay(1);
		printk
		    ("[do_vrf18_burst_subtest] upmu_get_qi_vrf18_burst=%d, upmu_get_qi_vrf18_burst_n=%d\n",
	...
	for (i = 0; i <= PMIC_VRF18_BURSTH_ON_MASK; i++) {
		...
		mdelay(1);
		printk
		    ("[do_vrf18_burst_subtest] upmu_set_vrf18_bursth_on=%d, upmu_set_vrf18_burstl_on=%d\n",
		     i, i);
		mdelay(1);
		printk
		    ("[do_vrf18_burst_subtest] upmu_get_qi_vrf18_burst=%d, upmu_get_qi_vrf18_burst_n=%d\n",
...
void do_vrf18_2_burst_subtest(void)
{
	...
	for (i = 0; i <= PMIC_VRF18_2_BURSTH_SLEEP_MASK; i++) {
		...
		mdelay(1);
		printk
		    ("[do_vrf18_2_burst_subtest] upmu_set_vrf18_2_bursth_sleep=%d, upmu_set_vrf18_2_burstl_sleep=%d\n",
		     i, i);
		mdelay(1);
		printk
		    ("[do_vrf18_2_burst_subtest] upmu_get_qi_vrf18_2_burst=%d, upmu_get_qi_vrf18_2_burst_n=%d\n",
	...
	for (i = 0; i <= PMIC_VRF18_2_BURSTH_ON_MASK; i++) {
		...
		mdelay(1);
		printk
		    ("[do_vrf18_2_burst_subtest] upmu_set_vrf18_2_bursth_on=%d, upmu_set_vrf18_2_burstl_on=%d\n",
		     i, i);
		mdelay(1);
		printk
		    ("[do_vrf18_2_burst_subtest] upmu_get_qi_vrf18_2_burst=%d, upmu_get_qi_vrf18_2_burst_n=%d\n",
		...
		case 0:
			for (j = 0; j <= PMIC_VPROC_BURST_MASK; j++) {
				...
				if ((upmu_get_qi_vproc_burst() != j)) {
					printk
					    ("[do_vproc_burst_test] fail at upmu_get_qi_vproc_burst=%d\n",
					     upmu_get_qi_vproc_burst());
				}

				printk
				    ("[do_vproc_burst_test] upmu_set_vproc_burst=%d, upmu_get_qi_vproc_burst=%d\n",
		...
		case 0:
			for (j = 0; j <= PMIC_VSRAM_BURST_MASK; j++) {
				...
				if ((upmu_get_qi_vsram_burst() != j)) {
					printk
					    ("[do_vsram_burst_test] fail at upmu_get_qi_vsram_burst=%d\n",
					     upmu_get_qi_vsram_burst());
				}

				printk
				    ("[do_vsram_burst_test] upmu_set_vsram_burst=%d, upmu_get_qi_vsram_burst=%d\n",
		...
		case 0:
			for (j = 0; j <= PMIC_VCORE_BURST_MASK; j++) {
				...
				if ((upmu_get_qi_vcore_burst() != j)) {
					printk
					    ("[do_vcore_burst_test] fail at upmu_get_qi_vcore_burst=%d\n",
					     upmu_get_qi_vcore_burst());
				}

				printk
				    ("[do_vcore_burst_test] upmu_set_vcore_burst=%d, upmu_get_qi_vcore_burst=%d\n",
		...
		case 0:
			for (j = 0; j <= PMIC_VM_BURST_MASK; j++) {
				...
				if ((upmu_get_qi_vm_burst() != j)) {
					printk
					    ("[do_vm_burst_test] fail at upmu_get_qi_vm_burst=%d\n",
					     upmu_get_qi_vm_burst());
				}

				printk
				    ("[do_vm_burst_test] upmu_set_vm_burst=%d, upmu_get_qi_vm_burst=%d\n",
	...
	for (i = 0; i <= 1; i++) {
		...
		case 0:
			for (j = 0; j <= PMIC_VIO18_BURSTH_MASK; j++) {
				...
				if ((upmu_get_qi_vio18_bursth() != j)
				    || (upmu_get_qi_vio18_burstl() != j)) {
					printk
					    ("[do_vio18_burst_test] fail at upmu_get_qi_vio18_burst=%d, upmu_get_qi_vio18_burst_n=%d\n",
					     upmu_get_qi_vio18_bursth(),
					     upmu_get_qi_vio18_burstl());
				}

				printk
				    ("[do_vio18_burst_test] upmu_set_vio18_bursth=%d, upmu_set_vio18_burstl=%d, upmu_get_qi_vio18_bursth=%d, upmu_get_qi_vio18_burstl=%d\n",
	...
	for (i = 0; i <= 1; i++) {
		...
		case 0:
			for (j = 0; j <= PMIC_VPA_BURSTH_MASK; j++) {
				...
				if ((upmu_get_qi_vpa_bursth() != j)
				    || (upmu_get_qi_vpa_burstl() != j)) {
					printk
					    ("[do_vpa_burst_test] fail at upmu_get_qi_vpa_burst=%d, upmu_get_qi_vpa_burst_n=%d\n",
					     upmu_get_qi_vpa_bursth(), upmu_get_qi_vpa_burstl());
				}

				printk
				    ("[do_vpa_burst_test] upmu_set_vpa_bursth=%d, upmu_set_vpa_burstl=%d, upmu_get_qi_vpa_bursth=%d, upmu_get_qi_vpa_burstl=%d\n",
	...
	for (i = 0; i <= 1; i++) {
		...
		case 0:
			for (j = 0; j <= PMIC_VRF18_BURSTH_MASK; j++) {
				...
				if ((upmu_get_qi_vrf18_bursth() != j)
				    || (upmu_get_qi_vrf18_burstl() != j)) {
					printk
					    ("[do_vrf18_burst_test] fail at upmu_get_qi_vrf18_burst=%d, upmu_get_qi_vrf18_burst_n=%d\n",
					     upmu_get_qi_vrf18_bursth(),
					     upmu_get_qi_vrf18_burstl());
				}

				printk
				    ("[do_vrf18_burst_test] upmu_set_vrf18_bursth=%d, upmu_set_vrf18_burstl=%d, upmu_get_qi_vrf18_bursth=%d, upmu_get_qi_vrf18_burstl=%d\n",
	...
	for (i = 0; i <= 1; i++) {
		...
		case 0:
			for (j = 0; j <= PMIC_VRF18_2_BURSTH_MASK; j++) {
				...
				if ((upmu_get_qi_vrf18_2_bursth() != j)
				    || (upmu_get_qi_vrf18_2_burstl() != j)) {
					printk
					    ("[do_vrf18_2_burst_test] fail at upmu_get_qi_vrf18_2_burst=%d, upmu_get_qi_vrf18_2_burst_n=%d\n",
					     upmu_get_qi_vrf18_2_bursth(),
					     upmu_get_qi_vrf18_2_burstl());
				}

				printk
				    ("[do_vrf18_2_burst_test] upmu_set_vrf18_2_bursth=%d, upmu_set_vrf18_2_burstl=%d, upmu_get_qi_vrf18_2_bursth=%d, upmu_get_qi_vrf18_2_burstl=%d\n",
				     j, j, upmu_get_qi_vrf18_2_bursth(),
				     upmu_get_qi_vrf18_2_burstl());
			}
			break;
...
/* write 0x5A to FLASH_CON_0~4 */
	...
	for (index = 0; index < 5; index++) {
		ret =
		    pmic_bank1_read_interface(BANK1_FLASH_CON0 + index, &read_data[index], 0xFF,
					      0x0);
	}
	/* check those value */
	for (index = 0; index < 5; index++) {
...
/* write 0x5A to KPLED_CON_0~4 */
	...
	for (index = 0; index < 5; index++) {
		ret =
		    pmic_bank1_read_interface(BANK1_KPLED_CON0 + index, &read_data[index], 0xFF,
					      0x0);
	}
	/* check those value */
	for (index = 0; index < 5; index++) {
...
/* write 0x5A to ISINK_CON0~16 */
	...
	for (index = 0; index < 17; index++) {
		ret =
		    pmic_bank1_read_interface(BANK1_ISINKS_CON0 + index, &read_data[index], 0xFF,
					      0x0);
	}
	/* check those value */
	for (index = 0; index < 17; index++) {
			...
			for (index = 0; index <= 0xF; index++) {
				...
				for (count = 0; count < count_data; count++) {
					...
					printk("[TC_2_07],%d,%d,%d,%x,%d\r\n", mode, index,
					       ret_read, ret_read_ori, dvt_number_count_ret_read);
				}
			}
			/* reset */
	...
	for (channel = 0; channel <= channel_max; channel++) {
		...
		if (channel == 0) {
			...
			for (count = 0; count < count_data; count++) {
				...
				printk("[TC_2_10],%d,%d,%d,%x,%d\r\n", channel, 0x3, ret_read,
				       ret_read_ori, dvt_number_count_ret_read);
			}
			upmu_isinks_ch0_en(0);
		} else if (channel == 1) {
...
void UVVP_PMIC_BOOST_TC_3_01_ldvt(void)
{
	...
	for (index = 0; index < 5; index++) {
		ret =
		    pmic_bank1_read_interface(BANK1_BOOST_CON0 + index, &read_data[index], 0xFF,
					      0x0);
	}
	/* check those value */
...
void UVVP_PMIC_FGADC_TC_4_01_ldvt(void)
{
	...
	for (index = 0; index < 33; index++) {
		ret =
		    pmic_bank1_read_interface(BANK1_FGADC_CON0 + index, &read_data[index], 0xFF,
					      0x0);
	}
	/* check */
	printk("Reg,WriteValue,GetValue\r\n");
...
/* 1. Set Vin as 3.4 / 3.8 / 4.2 */
	...
	printk("[CH0] TrimData=%d, NotTrimData=%d, data_63_56=0x%x, data_55_48=0x%x\r\n", trim_data,
	       not_trim_data, data_63_56, data_55_48);
	/* --------------------------------------------------------------------------------------------------------------- */
	/* 2. set OTP value */
	gain_trim = 0x4D;
	offset_trim = 0x1F;
	...
	for (VPA_CTRL = 0; VPA_CTRL <= VPA_CTRL_max; VPA_CTRL++) {
		...
		if (VPA_CTRL == 0) {
			for (VPA_VOSEL = 0; VPA_VOSEL <= VPA_VOSEL_max; VPA_VOSEL++) {
				...
				printk("%d,%d,%d,%d,%d\r\n", VPA_CTRL, VPA_VOSEL, PASEL_value,
				       PASEL_SETZ, Ext_AUXADC_value);
			}
		} else if (VPA_CTRL == 1) {
			for (PASEL_value = 0; PASEL_value <= PASEL_value_max; PASEL_value++) {
				...
				if (PASEL_value == 0) {
					for (PASEL_SETZ = 0; PASEL_SETZ <= PASEL_SETZ_max;
					     PASEL_SETZ++)
						/* PASEL_SETZ=0x0; */
					{
						...
						Ext_AUXADC_value = Get_BUCK_AUXADC_Value(BUCK_VPA);
						printk("%d,%d,%d,%d,%d\r\n", VPA_CTRL, VPA_VOSEL,
						       PASEL_value, PASEL_SETZ, Ext_AUXADC_value);
					}
	...
	for (VRF18_ON_CTRL = 0; VRF18_ON_CTRL <= VRF18_ON_CTRL_max; VRF18_ON_CTRL++) {
		...
		if (VRF18_ON_CTRL == 0) {
			for (VRF18_EN = 0; VRF18_EN <= VRF18_EN_max; VRF18_EN++) {
				...
				printk("%d,%d,%d,%d\r\n", VRF18_ON_CTRL, VRF18_EN, SRCLK_EN,
				       Ext_AUXADC_value);
			}
...
void UVVP_PMIC_BUCK_TC_6_03_ldvt(void)
{
	...
	printk
	    ("VPROC_CTRL,VPROC_VOSEL,VPROC_VOSEL_SRCLKEN0,VPROC_VOSEL_SRCLKEN1,DVS_SELECT,VPROC_VOSEL_DVS00,VPROC_VOSEL_DVS01,VPROC_VOSEL_DVS10,VPROC_VOSEL_DVS11,SRCLK_EN,VPROC_VOLTAGE,Reg[0x4e]\r\n");
		...
		if (VPROC_CTRL == 0) {
			for (VPROC_VOSEL = 0; VPROC_VOSEL <= VPROC_VOSEL_max; VPROC_VOSEL++) {
				...
				printk("%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%x\r\n",
				       VPROC_CTRL, VPROC_VOSEL, VPROC_VOSEL_SRCLKEN0,
				       VPROC_VOSEL_SRCLKEN1, DVS_SELECT, VPROC_VOSEL_DVS00,
				       VPROC_VOSEL_DVS01, VPROC_VOSEL_DVS10, VPROC_VOSEL_DVS11,
				       SRCLK_EN, Ext_AUXADC_value, read_data);
			}
			...
			for (SRCLK_EN = 0; SRCLK_EN <= SRCLK_EN_max; SRCLK_EN++) {
				...
				if (SRCLK_EN == 0) {
					for (VPROC_VOSEL_SRCLKEN0 = 0;
					     VPROC_VOSEL_SRCLKEN0 <= VPROC_VOSEL_SRCLKEN0_max;
					     VPROC_VOSEL_SRCLKEN0++) {
						upmu_buck_vosel_srclken_0(BUCK_VPROC,
									  VPROC_VOSEL_SRCLKEN0);

						ret =
						    pmic_read_interface(0x4e, &read_data, 0xFF,
									0x0);

						Ext_AUXADC_value =
						    Get_BUCK_AUXADC_Value(BUCK_VPROC);
						printk("%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%x\r\n",
						       VPROC_CTRL, VPROC_VOSEL,
						       VPROC_VOSEL_SRCLKEN0, VPROC_VOSEL_SRCLKEN1,
						       DVS_SELECT, VPROC_VOSEL_DVS00,
						       VPROC_VOSEL_DVS01, VPROC_VOSEL_DVS10,
						       VPROC_VOSEL_DVS11, SRCLK_EN,
						       Ext_AUXADC_value, read_data);
					}
				} else if (SRCLK_EN == 1) {

					for (VPROC_VOSEL_SRCLKEN1 = 0;
					     VPROC_VOSEL_SRCLKEN1 <= VPROC_VOSEL_SRCLKEN1_max;
					     VPROC_VOSEL_SRCLKEN1++) {
						upmu_buck_vosel_srclken_1(BUCK_VPROC,
									  VPROC_VOSEL_SRCLKEN1);
		...
		/*VPROC_CTRL==2 || VPROC_CTRL==3 */
			...
			for (SRCLK_EN = 0; SRCLK_EN <= SRCLK_EN_max; SRCLK_EN++) {
				...
				if (SRCLK_EN == 0) {
					...
					for (DVS_SELECT = 0; DVS_SELECT <= DVS_SELECT_max;
					     DVS_SELECT++) {
						/* set DVT0/1 */
						...
						if (DVS_SELECT == 0) {
							for (VPROC_VOSEL_DVS00 = 0;
							     VPROC_VOSEL_DVS00 <=
							     VPROC_VOSEL_DVS00_max;
							     VPROC_VOSEL_DVS00++) {
								upmu_buck_vosel_dvs_00(BUCK_VPROC,
										       VPROC_VOSEL_DVS00);

								ret =
								    pmic_read_interface(0x4e,
											&read_data,
											0xFF, 0x0);

								Ext_AUXADC_value =
								    Get_BUCK_AUXADC_Value
								    (BUCK_VPROC);
								printk
							...
							for (VPROC_VOSEL_DVS01 = 0;
							     VPROC_VOSEL_DVS01 <=
							     VPROC_VOSEL_DVS01_max;
							     VPROC_VOSEL_DVS01++) {
								upmu_buck_vosel_dvs_01(BUCK_VPROC,
										       VPROC_VOSEL_DVS01);
							...
							for (VPROC_VOSEL_DVS10 = 0;
							     VPROC_VOSEL_DVS10 <=
							     VPROC_VOSEL_DVS10_max;
							     VPROC_VOSEL_DVS10++) {
								upmu_buck_vosel_dvs_10(BUCK_VPROC,
										       VPROC_VOSEL_DVS10);
							...
							for (VPROC_VOSEL_DVS11 = 0;
							     VPROC_VOSEL_DVS11 <=
							     VPROC_VOSEL_DVS11_max;
							     VPROC_VOSEL_DVS11++) {
								upmu_buck_vosel_dvs_11(BUCK_VPROC,
										       VPROC_VOSEL_DVS11);
	...
	for (VCORE_CTRL = 0; VCORE_CTRL <= VCORE_CTRL_max; VCORE_CTRL++) {
		...
		if (VCORE_CTRL == 0) {
			for (VCORE_VOSEL = 0; VCORE_VOSEL <= VCORE_VOSEL_max; VCORE_VOSEL++) {
				...
				printk("%d,%d,%d,%d,%d\r\n", VCORE_CTRL, VCORE_VOSEL,
				       VCORE_VOSEL_CON1, SRCLK_EN, Ext_AUXADC_value);
			}
			...
			for (SRCLK_EN = 0; SRCLK_EN <= SRCLK_EN_max; SRCLK_EN++) {
				...
				if (SRCLK_EN == 0) {
					for (VCORE_VOSEL = 0; VCORE_VOSEL <= VCORE_VOSEL_max;
					     VCORE_VOSEL++) {
						vol_temp = p_upmu_entry->vol_list[VCORE_VOSEL];
						/* vol_temp=UPMU_VOLT_1_0_0_0_V; */
						upmu_buck_vosel(BUCK_VCORE, vol_temp);
						...
						Ext_AUXADC_value =
						    Get_BUCK_AUXADC_Value(BUCK_VCORE);
						printk("%d,%d,%d,%d,%d\r\n", VCORE_CTRL,
						       VCORE_VOSEL, VCORE_VOSEL_CON1, SRCLK_EN,
						       Ext_AUXADC_value);
					}
				} else if (SRCLK_EN == 1) {

					for (VCORE_VOSEL_CON1 = 0;
					     VCORE_VOSEL_CON1 <= VCORE_VOSEL_CON1_max;
					     VCORE_VOSEL_CON1++) {
...
void UVVP_PMIC_DIGLDO_TC_7_01_ldvt(void)
{
	...
	for (PMU_RESETB = 0; PMU_RESETB <= PMU_RESETB_max; PMU_RESETB++) {
		...
		if (PMU_RESETB == 0) {
			for (STRUP_VM12_1_EN = 0; STRUP_VM12_1_EN <= STRUP_VM12_1_EN_max;
			     STRUP_VM12_1_EN++) {
				/* SW can not set STRUP_VM12_1_EN */

				Ext_AUXADC_value = Get_LDO_AUXADC_Value(LDO_VM12_1);
				printk("%d,%d,%d,%d\r\n", PMU_RESETB, STRUP_VM12_1_EN, VM12_1_EN,
				       Ext_AUXADC_value);
			}
	...
	for (VM12_1_LP_SEL = 0; VM12_1_LP_SEL <= VM12_1_LP_SEL_max; VM12_1_LP_SEL++) {
		...
		if (VM12_1_LP_SEL == 0) {
			for (VM12_1_LP_SET = 0; VM12_1_LP_SET <= VM12_1_LP_SET_max; VM12_1_LP_SET++) {
				...
				Ext_AUXADC_value = Get_LDO_AUXADC_Value(LDO_VM12_1);
				printk("%d,%d,%d,%d,%x\r\n", VM12_1_LP_SEL, VM12_1_LP_SET, SRCLK_EN,
				       Ext_AUXADC_value, read_data);
			}
...
void UVVP_PMIC_DIGLDO_TC_7_03_ldvt(void)
{
	...
	for (PMU_RESETB = 0; PMU_RESETB <= PMU_RESETB_max; PMU_RESETB++) {
		...
		if (PMU_RESETB == 0) {
			for (STRUP_VM12_2_EN = 0; STRUP_VM12_2_EN <= STRUP_VM12_2_EN_max;
			     STRUP_VM12_2_EN++) {
				/* SW can not set STRUP_VM12_2_EN */

				Ext_AUXADC_value = Get_LDO_AUXADC_Value(LDO_VM12_2);
				printk("%d,%d,%d,%d\r\n", PMU_RESETB, STRUP_VM12_2_EN, VM12_2_EN,
				       Ext_AUXADC_value);
			}
		} else if (PMU_RESETB == 1) {
	...
	for (VM12_2_LP_SEL = 0; VM12_2_LP_SEL <= VM12_2_LP_SEL_max; VM12_2_LP_SEL++) {
		...
		if (VM12_2_LP_SEL == 0) {
			for (VM12_2_LP_SET = 0; VM12_2_LP_SET <= VM12_2_LP_SET_max; VM12_2_LP_SET++) {
				...
				Ext_AUXADC_value = Get_LDO_AUXADC_Value(LDO_VM12_2);
				printk("%d,%d,%d,%d,%x\r\n", VM12_2_LP_SEL, VM12_2_LP_SET, SRCLK_EN,
				       Ext_AUXADC_value, read_data);
			}
...
void UVVP_PMIC_DIGLDO_TC_7_05_ldvt(void)
{
	...
	for (PMU_RESETB = 0; PMU_RESETB <= PMU_RESETB_max; PMU_RESETB++) {
		...
		if (PMU_RESETB == 0) {
			for (STRUP_VM12_INT_EN = 0; STRUP_VM12_INT_EN <= STRUP_VM12_INT_EN_max;
			     STRUP_VM12_INT_EN++) {
				/* SW can not set STRUP_VM12_INT_EN */

				Ext_AUXADC_value = Get_LDO_AUXADC_Value(LDO_VM12_INT);
				printk("%d,%d,%d,%d\r\n", PMU_RESETB, STRUP_VM12_INT_EN,
				       VM12_INT_EN, Ext_AUXADC_value);
			}
		} else if (PMU_RESETB == 1) {
...
void UVVP_PMIC_DIGLDO_TC_7_06_ldvt(void)
{
	...
	printk
	    ("VM12_INT_CTRL_SEL,VM12_INT_CAL,SRCLK_EN,VM12_INT_SLEEP,VPROC_VOSEL,VM12_INT_LOW_BOUND,VM12_INT_VOLTAGE,Reg[0xb9]\r\n");
	...
	for (VM12_INT_CTRL_SEL = 0; VM12_INT_CTRL_SEL <= VM12_INT_CTRL_SEL_max; VM12_INT_CTRL_SEL++) {
		ret = pmic_config_interface(BANK0_DIGLDO_COND, VM12_INT_CTRL_SEL,
					    BANK_0_VM12_INT_CTRL_SEL_MASK,
					    BANK_0_VM12_INT_CTRL_SEL_SHIFT);
		...
		if (VM12_INT_CTRL_SEL == 0) {
			for (VM12_INT_CAL = 0; VM12_INT_CAL <= VM12_INT_CAL_max; VM12_INT_CAL++) {
				...
				printk("%d,%d,%d,%d,%d,%d,%d,%x\r\n",
				       VM12_INT_CTRL_SEL, VM12_INT_CAL, SRCLK_EN, VM12_INT_SLEEP,
				       VPROC_VOSEL, VM12_INT_LOW_BOUND, Ext_AUXADC_value,
				       read_data);
			}
			...
			for (SRCLK_EN = 0; SRCLK_EN <= SRCLK_EN_max; SRCLK_EN++) {
				...
				if (SRCLK_EN == 0) {
					for (VM12_INT_SLEEP = 0;
					     VM12_INT_SLEEP <= VM12_INT_SLEEP_max;
					     VM12_INT_SLEEP++) {
						ret =
						    pmic_config_interface(BANK0_DIGLDO_CON9,
									  VM12_INT_SLEEP,
									  BANK_0_VM12_INT_SLEEP_MASK,
									  BANK_0_VM12_INT_SLEEP_SHIFT);

						ret =
						    pmic_read_interface(0xb9, &read_data, 0xFF,
									0x0);

						Ext_AUXADC_value =
						    Get_LDO_AUXADC_Value(LDO_VM12_INT);
						printk("%d,%d,%d,%d,%d,%d,%d,%x\r\n",
						       VM12_INT_CTRL_SEL, VM12_INT_CAL, SRCLK_EN,
						       VM12_INT_SLEEP, VPROC_VOSEL,
						       VM12_INT_LOW_BOUND, Ext_AUXADC_value,
						       read_data);
					}
				} else if (SRCLK_EN == 1) {
					for (VPROC_VOSEL = 0; VPROC_VOSEL <= VPROC_VOSEL_max;
					     VPROC_VOSEL++) {
						vol_temp = p_upmu_entry->vol_list[VPROC_VOSEL];
						/* vol_temp=UPMU_VOLT_1_0_0_0_V; */
						upmu_buck_vosel(BUCK_VPROC, vol_temp);

						for (VM12_INT_LOW_BOUND = 0;
						     VM12_INT_LOW_BOUND <= VM12_INT_LOW_BOUND_max;
						     VM12_INT_LOW_BOUND++) {
							ret =
							    pmic_config_interface(BANK0_DIGLDO_CONA,
										  VM12_INT_LOW_BOUND,
										  BANK_0_VM12_INT_LOW_BOUND_MASK,
										  BANK_0_VM12_INT_LOW_BOUND_SHIFT);

							ret =
							    pmic_read_interface(0xb9, &read_data,
										0xFF, 0x0);

							Ext_AUXADC_value =
							    Get_LDO_AUXADC_Value(LDO_VM12_INT);
							printk("%d,%d,%d,%d,%d,%d,%d,%x\r\n",
							       VM12_INT_CTRL_SEL, VM12_INT_CAL,
							       SRCLK_EN, VM12_INT_SLEEP,
							       VPROC_VOSEL, VM12_INT_LOW_BOUND,
							       Ext_AUXADC_value, read_data);
						}
					}
				} else {
...
void UVVP_PMIC_DIGLDO_TC_7_07_ldvt(void)
{
	...
	for (PMU_RESETB = 0; PMU_RESETB <= PMU_RESETB_max; PMU_RESETB++) {
		...
		if (PMU_RESETB == 0) {
			for (STRUP_VIO28_EN = 0; STRUP_VIO28_EN <= STRUP_VIO28_EN_max;
			     STRUP_VIO28_EN++) {
				/* SW can not set STRUP_VIO28_EN */

				Ext_AUXADC_value = Get_LDO_AUXADC_Value(LDO_VIO28);
				printk("%d,%d,%d,%d\r\n", PMU_RESETB, STRUP_VIO28_EN, VIO28_EN,
				       Ext_AUXADC_value);
			}
		} else if (PMU_RESETB == 1) {
	...
	for (PMU_RESETB = 0; PMU_RESETB <= PMU_RESETB_max; PMU_RESETB++) {
		...
		if (PMU_RESETB == 0) {
			for (STRUP_VMC_EN = 0; STRUP_VMC_EN <= STRUP_VMC_EN_max; STRUP_VMC_EN++) {
				...
				printk("%d,%d,%d,%d\r\n", PMU_RESETB, STRUP_VMC_EN, VMC_EN,
				       Ext_AUXADC_value);
			}
		} else if (PMU_RESETB == 1) {
...
void UVVP_PMIC_DIGLDO_TC_7_10_ldvt(void)
{
	...
	for (VMC_LP_SEL = 0; VMC_LP_SEL <= VMC_LP_SEL_max; VMC_LP_SEL++) {
		...
		if (VMC_LP_SEL == 0) {
			for (VMC_LP_MODE_SET = 0; VMC_LP_MODE_SET <= VMC_LP_MODE_SET_max;
			     VMC_LP_MODE_SET++) {
				...
				Ext_AUXADC_value = Get_LDO_AUXADC_Value(LDO_VMC);
				printk("%d,%d,%d,%d,%x\r\n", VMC_LP_SEL, VMC_LP_MODE_SET, SRCLK_EN,
				       Ext_AUXADC_value, read_data);
			}
	...
	for (PMU_RESETB = 0; PMU_RESETB <= PMU_RESETB_max; PMU_RESETB++) {
		...
		if (PMU_RESETB == 0) {
			for (STRUP_VMCH_EN = 0; STRUP_VMCH_EN <= STRUP_VMCH_EN_max; STRUP_VMCH_EN++) {
				...
				printk("%d,%d,%d,%d\r\n", PMU_RESETB, STRUP_VMCH_EN, VMCH_EN,
				       Ext_AUXADC_value);
			}
		} else if (PMU_RESETB == 1) {
...
void UVVP_PMIC_DIGLDO_TC_7_13_ldvt(void)
{
	...
	for (VMCH_LP_SEL = 0; VMCH_LP_SEL <= VMCH_LP_SEL_max; VMCH_LP_SEL++) {
		...
		if (VMCH_LP_SEL == 0) {
			for (VMCH_LP_MODE_SET = 0; VMCH_LP_MODE_SET <= VMCH_LP_MODE_SET_max;
			     VMCH_LP_MODE_SET++) {
...
void UVVP_PMIC_ANALDO_TC_8_02_ldvt(void)
{
	...
	for (PMU_RESETB = 0; PMU_RESETB <= PMU_RESETB_max; PMU_RESETB++) {
		...
		if (PMU_RESETB == 0) {
			for (STRUP_VTCXO_EN = 0; STRUP_VTCXO_EN <= STRUP_VTCXO_EN_max;
			     STRUP_VTCXO_EN++) {
				/* SW can not set STRUP_VTCXO_EN */

				Ext_AUXADC_value = Get_LDO_AUXADC_Value(LDO_VTCXO);
				printk("%d,%d,%d,%d,%d,%d\r\n", PMU_RESETB, STRUP_VTCXO_EN,
				       VTCXO_ON_CTRL, SRCLK_EN, VTCXO_EN, Ext_AUXADC_value);
			}
		} else if (PMU_RESETB == 1) {
			for (VTCXO_ON_CTRL = 0; VTCXO_ON_CTRL <= VTCXO_ON_CTRL_max; VTCXO_ON_CTRL++) {
				...
				if (VTCXO_ON_CTRL == 0) {
					for (VTCXO_EN = 0; VTCXO_EN <= VTCXO_EN_max; VTCXO_EN++) {
						...
						Ext_AUXADC_value = Get_LDO_AUXADC_Value(LDO_VTCXO);
						printk("%d,%d,%d,%d,%d,%d\r\n", PMU_RESETB,
						       STRUP_VTCXO_EN, VTCXO_ON_CTRL, SRCLK_EN,
						       VTCXO_EN, Ext_AUXADC_value);
					}
	...
	for (PMU_RESETB = 0; PMU_RESETB <= PMU_RESETB_max; PMU_RESETB++) {
		...
		if (PMU_RESETB == 0) {
			for (STRUP_VA1_EN = 0; STRUP_VA1_EN <= STRUP_VA1_EN_max; STRUP_VA1_EN++) {
				...
				printk("%d,%d,%d,%d\r\n", PMU_RESETB, STRUP_VA1_EN, VA1_EN,
				       Ext_AUXADC_value);
			}
		} else if (PMU_RESETB == 1) {
	...
	for (VA1_LP_SEL = 0; VA1_LP_SEL <= VA1_LP_SEL_max; VA1_LP_SEL++) {
		...
		if (VA1_LP_SEL == 0) {
			for (VA1_LP_SET = 0; VA1_LP_SET <= VA1_LP_SET_max; VA1_LP_SET++) {
				...
				Ext_AUXADC_value = Get_LDO_AUXADC_Value(LDO_VA1);
				printk("%d,%d,%d,%d,%x\r\n", VA1_LP_SEL, VA1_LP_SET, SRCLK_EN,
				       Ext_AUXADC_value, read_data);
			}
...
void UVVP_PMIC_I2C_TC_9_02_ldvt(void)
{
	...
	printk("[Bank0] Reg=%x|%x|%x, WriteData=%x|%x|%x, ReadBackDate=%x|%x|%x\r\n",
	       ...
	       0x05, 0x0A, 0x30, temp_val_1, temp_val_2, temp_val_3);
	...
	printk("[Bank1] Reg=%x|%x|%x|%x, WriteData=%x|%x|%x|%x, ReadBackDate=%x|%x|%x|%x\r\n",
	       BANK1_FLASH_CON0, BANK1_FLASH_CON1, BANK1_FLASH_CON2, BANK1_FLASH_CON3,
	       0x50, 0x03, 0x05, 0x0A, temp_val_1, temp_val_2, temp_val_3, temp_val_4);
...
void UVVP_PMIC_I2C_TC_9_03_ldvt(void)
{
	...
	printk("[Bank0] Reg=%x|%x|%x, WriteData=%x|%x|%x, ReadBackDate=%x|%x|%x\r\n",
	       BANK0_INT_CON0, BANK0_INT_CON1, BANK0_INT_CON2,
	       0x05, 0x0A, 0x30, read_data[0], read_data[1], read_data[2]);
...
void print_raw_data(int buckldoNumber)
{
	...
	if (pmu_power_type == BUCK_TYPE) {
		...
		printk("%d,%s,%d,%d,%d\n",
		       pmu_power_type, buck_name[buckldoNumber],
		       pmu_dvt_buck_profile[buckldoNumber].channel,
		...
		printk("%d,%s,%d,%d,%d\n",
		       pmu_power_type, ldo_name[buckldoNumber],
		       pmu_dvt_ldo_profile[buckldoNumber].channel,
	...
	for (i = ldoNumber; i < LDO_MAX_NUMBER; i++, ldoNumber++) {
		...
		if (ldoNumber == LDO_VSIM) {
			...
			print_raw_data(ldoNumber);
			   ...
			   (ldoNumber == LDO_VMCH) ||
			   (ldoNumber == LDO_VGP) ||
			   (ldoNumber == LDO_VGP2) || (ldoNumber == LDO_VIBR)
		    ) {
			upmu_ldo_en(ldoNumber, 1);
