...
static int touch_log_en;
static int touch_interrupt;	/* hw status */
static int fav_mode_en;
...
static void dump_AUXADC_regs(void)
{
	...
	for (i = 0; i <= 0xa0 / 4; i++) {
		printk("[addr:value] 0x%8x:0x%8x\n", (AUXADC_BASE + 4 * i),
		       *(volatile u32 *)(AUXADC_BASE + 4 * i));
	}
	printk("[addr:value] 0x%8x:0x%8x\n", AUXADC_TS_RAW_CON,
	       *(volatile u32 *)(AUXADC_TS_RAW_CON));
	printk("[addr:value] 0x%8x:0x%8x\n", AUXADC_TS_AUTO_TIME_INTVL,
	       *(volatile u32 *)(AUXADC_TS_AUTO_TIME_INTVL));
}
...
unsigned int ts_udvt_read_status(void)
{
	...
	for (j = 0; j < 4; j++) {
		for (i = 0; i < 7; i++) {
			...
			printk("[ts_udvt]: AUXADC_TS_RAW_X_DAT0  = %4d, addr = 0x%04x\n", rx[i],
			       (AUXADC_TS_RAW_X_DAT0 + 0x04 * i));
			printk("[ts_udvt]: AUXADC_TS_RAW_Y_DAT0  = %4d, addr = 0x%04x\n", ry[i],
			       (AUXADC_TS_RAW_Y_DAT0 + 0x04 * i));
		}
		...
		printk("[ts_udvt]: AUXADC_TS_RAW_Z1_DAT0 = %4d, addr = 0x%04x\n", rz1[j],
		       (AUXADC_TS_RAW_Z1_DAT0 + 0x04 * j));
		printk("[ts_udvt]: AUXADC_TS_RAW_Z2_DAT0 = %4d, addr = 0x%04x\n", rz2[j],
		       (AUXADC_TS_RAW_Z2_DAT0 + 0x04 * j));
	}
...
{
	if (touch_log_en)
		printk("[ts_udvt]: interrupt trigger\n");
...
static long ts_udvt_dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case SET_RAW_ABORT:
		if (pcmd->value == 0)	/* start */
			(*(volatile u32 *)AUXADC_TS_RAW_CON) =
			    (((*(volatile u32 *)AUXADC_TS_RAW_CON)) & 0xfffe);
		else		/* abort */
			(*(volatile u32 *)AUXADC_TS_RAW_CON) =
			    (((*(volatile u32 *)AUXADC_TS_RAW_CON)) | 0x1);
		break;
	case SET_FAV_NOISE:
		if (pcmd->value == 1)	/* enable */
			(*(volatile u32 *)AUXADC_TS_CON1) =
			    (((*(volatile u32 *)AUXADC_TS_CON1)) | (1 << FAV_ASAMP));
		else		/* disable */
			(*(volatile u32 *)AUXADC_TS_CON1) =
			    (((*(volatile u32 *)AUXADC_TS_CON1)) & (~(1 << FAV_ASAMP)));
		break;
	case SET_FAV_ACC_MODE:
		...
		{
			(*(volatile u32 *)AUXADC_TS_CON1) =
			    ((((*(volatile u32 *)AUXADC_TS_CON1)) & 0xfffc));
		} else if (pcmd->value == 4)	/* 4 times */
		{
			(*(volatile u32 *)AUXADC_TS_CON1) =
			    ((((*(volatile u32 *)AUXADC_TS_CON1)) & 0xfffc) | 0x01);
		} else if (pcmd->value == 8)	/* 8 times */
		{
			(*(volatile u32 *)AUXADC_TS_CON1) =
			    ((((*(volatile u32 *)AUXADC_TS_CON1)) & 0xfffc) | 0x02);
		} else if (pcmd->value == 16)	/* 16 times */
		{
			(*(volatile u32 *)AUXADC_TS_CON1) =
			    ((((*(volatile u32 *)AUXADC_TS_CON1)) & 0xfffc) | 0x03);
		}
		break;
	case SET_FAV_LATENCY:
		(*(volatile u32 *)AUXADC_TS_CON1) =
		    (((*(volatile u32 *)AUXADC_TS_CON1) & 0x00FF) | (pcmd->value << FAV_ADEL_BIT));
		break;
	case SET_FAV_INTR_MODE:
		if (pcmd->value == 0) {	/* software mode */
			(*(volatile u32 *)AUXADC_TS_CON1) =
			    (((*(volatile u32 *)AUXADC_TS_CON1)) & (~(1 << FAV_SEL)));
		}
		if (pcmd->value == 1) {
			(*(volatile u32 *)AUXADC_TS_CON1) =
			    (((*(volatile u32 *)AUXADC_TS_CON1)) | ((1 << FAV_SEL)));
	...
	case SET_FAV_MODE:
		if (pcmd->value == 1)	/* enable */
			(*(volatile u32 *)AUXADC_MISC) =
			    (((*(volatile u32 *)AUXADC_MISC)) | ((1 << 8)));
		else
			(*(volatile u32 *)AUXADC_MISC) =
			    (((*(volatile u32 *)AUXADC_MISC)) & (~(1 << 8)));
		break;
...
int ts_udvt_local_init(void)
{
	...
	mt65xx_irq_set_sens(MT65XX_TOUCH_IRQ_LINE, MT_EDGE_SENSITIVE);
	mt65xx_irq_set_polarity(MT65XX_TOUCH_IRQ_LINE, MT_POLARITY_LOW);
	if (request_irq(MT65XX_TOUCH_IRQ_LINE, ts_udvt_handler, 0, "_TS_UDVT", NULL))
		printk("[ts_udvt]: request_irq failed.\n");

	mt65xx_irq_set_sens(MT65XX_TOUCH_BATCH_LINE, MT_EDGE_SENSITIVE);
	mt65xx_irq_set_polarity(MT65XX_TOUCH_BATCH_LINE, MT_POLARITY_LOW);
