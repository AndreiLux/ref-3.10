...
static void dlHdmiTxGenerateVideoPortTables
	...
	for (i = 0; i < 6; i++) {
		...
		/* printk("videoPortMapping_YUV422[%d][%d] = %d\n", unit, i, videoPortMapping_YUV422[unit][i]); */
		...
		/* printk("videoPortMapping_RGB444[%d][%d] = %d\n", unit, i, videoPortMapping_RGB444[unit][i]); */
		/* RGB444 */
	...
	int j = 0;
	for (j = 0; j < 6; j++) {
		printk("pConfig->pEnableVideoPortYUV422[%d]=%d\n", j,
		       pConfig->pEnableVideoPortYUV422[j]);
	}
	for (j = 0; j < 6; j++) {
		printk("pConfig->pGroundVideoPortYUV422[%d]=%d\n", j,
		       pConfig->pGroundVideoPortYUV422[j]);
	}
	for (j = 0; j < 6; j++) {
		printk("pConfig->pEnableVideoPortRGB444[%d]=%d\n", j,
		       pConfig->pEnableVideoPortRGB444[j]);
	}
	for (j = 0; j < 6; j++) {
		printk("pConfig->pGroundVideoPortRGB444[%d]=%d\n", j,
		       pConfig->pGroundVideoPortRGB444[j]);
	}
	for (j = 0; j < 6; j++) {
		printk("pConfig->pEnableVideoPortYUV444[%d]=%d\n", j,
		       pConfig->pEnableVideoPortYUV444[j]);
	}
	for (j = 0; j < 6; j++) {
		printk("pConfig->pGroundVideoPortYUV444[%d]=%d\n", j,
		       pConfig->pGroundVideoPortYUV444[j]);
	}
#endif
#ifdef TMFL_RGB_DDR_12BITS
