...
void dump_register(tmUnitSelect_t txUnit)
{
	tmHdmiTxobject_t *pDis;	/* Pointer to Device Instance Structure */
	tmErrorCode_t err;	/* Error code */
	...
	err = checkUnitSetDis(txUnit, &pDis);
	RETIF(err != TM_OK, err)
	    /* xuecheng, dump register */
	unsigned char regvalue = 0;
	getHwRegister(pDis, E_REG_P00_VIP_CNTRL_0_W, &regvalue);
	printk("E_REG_P00_VIP_CNTRL_0_W=0x%08x\n", regvalue);
	getHwRegister(pDis, E_REG_P00_VIP_CNTRL_1_W, &regvalue);
	printk("E_REG_P00_VIP_CNTRL_1_W=0x%08x\n", regvalue);
	getHwRegister(pDis, E_REG_P00_VIP_CNTRL_2_W, &regvalue);
	printk("E_REG_P00_VIP_CNTRL_2_W=0x%08x\n", regvalue);
