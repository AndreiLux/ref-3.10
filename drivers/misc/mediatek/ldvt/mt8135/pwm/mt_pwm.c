...
/* conf.PWM_MODE_FIFO_REGS.STOP_BITPOS_VALUE = 63; */
	...
	case PWM_UVVF_FIFO_HLGDUR:
		...
		while (1) {
			printk(KERN_INFO
			       "PWM: gduration = %x, lduration = %x, hduration = %x, pwm_no = %x\n",
			       conf.PWM_MODE_FIFO_REGS.GDURATION, conf.PWM_MODE_FIFO_REGS.LDURATION,
			       conf.PWM_MODE_FIFO_REGS.HDURATION, conf.pwm_no);
			pwm_set_spec_config(&conf);
	...
	case PWM_UVVF_FIFO_GIOUT:
		...
		while (1) {
			printk(KERN_INFO
			       "PWM: guard_value = %x, idle_value = %x, wave_num = %x, pwm_no = %x\n",
			       conf.PWM_MODE_FIFO_REGS.GUARD_VALUE,
			       conf.PWM_MODE_FIFO_REGS.IDLE_VALUE, conf.PWM_MODE_FIFO_REGS.WAVE_NUM,
			       conf.pwm_no);
			pwm_set_spec_config(&conf);
	...
	case PWM_UVVF_REGISTER_WR:
		...
		for (loopcnt = 0; loopcnt < 2; loopcnt++) {
			...
			for (pwm_no = 0; pwm_no < 7; pwm_no++) {
				originalvalue =
				    __raw_readl(PWM_register[pwm_no] + 4 * PWM_HDURATION);
				__raw_writel(testregvalue,
					     PWM_register[pwm_no] + 4 * PWM_HDURATION);
				readregvalue =
				    __raw_readl(PWM_register[pwm_no] + 4 * PWM_HDURATION);
				...
				__raw_writel(originalvalue,
					     PWM_register[pwm_no] + 4 * PWM_HDURATION);

				originalvalue =
				    __raw_readl(PWM_register[pwm_no] + 4 * PWM_LDURATION);
				__raw_writel(testregvalue,
					     PWM_register[pwm_no] + 4 * PWM_LDURATION);
				readregvalue =
				    __raw_readl(PWM_register[pwm_no] + 4 * PWM_LDURATION);
				if ((readregvalue & 0xffff) != (testregvalue & 0xffff)) {
					statusflag++;
				...
				originalvalue =
				    __raw_readl(PWM_register[pwm_no] + 4 * PWM_GDURATION);
				__raw_writel(testregvalue,
					     PWM_register[pwm_no] + 4 * PWM_GDURATION);
				readregvalue =
				    __raw_readl(PWM_register[pwm_no] + 4 * PWM_GDURATION);
				if ((readregvalue & 0xffff) != (testregvalue & 0xffff)) {
					statusflag++;
				...
				originalvalue =
				    __raw_readl(PWM_register[pwm_no] + 4 * PWM_BUF0_BASE_ADDR);
				__raw_writel(testregvalue,
					     PWM_register[pwm_no] + 4 * PWM_BUF0_BASE_ADDR);
				readregvalue =
				    __raw_readl(PWM_register[pwm_no] + 4 * PWM_BUF0_BASE_ADDR);
				...
				originalvalue =
				    __raw_readl(PWM_register[pwm_no] + 4 * PWM_BUF0_SIZE);
				__raw_writel(testregvalue,
					     PWM_register[pwm_no] + 4 * PWM_BUF0_SIZE);
				readregvalue =
				    __raw_readl(PWM_register[pwm_no] + 4 * PWM_BUF0_SIZE);
				if ((readregvalue & 0xffff) != (testregvalue & 0xffff)) {
					statusflag++;
				...
				originalvalue =
				    __raw_readl(PWM_register[pwm_no] + 4 * PWM_BUF1_BASE_ADDR);
				__raw_writel(testregvalue,
					     PWM_register[pwm_no] + 4 * PWM_BUF1_BASE_ADDR);
				readregvalue =
				    __raw_readl(PWM_register[pwm_no] + 4 * PWM_BUF1_BASE_ADDR);
				if ((readregvalue & 0xffffffff) != (testregvalue & 0xffffffff)) {
					statusflag++;
				...
				originalvalue =
				    __raw_readl(PWM_register[pwm_no] + 4 * PWM_BUF1_SIZE);
				__raw_writel(testregvalue,
					     PWM_register[pwm_no] + 4 * PWM_BUF1_SIZE);
				readregvalue =
				    __raw_readl(PWM_register[pwm_no] + 4 * PWM_BUF1_SIZE);
				if ((readregvalue & 0xffff) != (testregvalue & 0xffff)) {
					statusflag++;
				...
				originalvalue =
				    __raw_readl(PWM_register[pwm_no] + 4 * PWM_SEND_DATA0);
				__raw_writel(testregvalue,
					     PWM_register[pwm_no] + 4 * PWM_SEND_DATA0);
				readregvalue =
				    __raw_readl(PWM_register[pwm_no] + 4 * PWM_SEND_DATA0);
				if ((readregvalue & 0xffffffff) != (testregvalue & 0xffffffff)) {
					statusflag++;
				...
				originalvalue =
				    __raw_readl(PWM_register[pwm_no] + 4 * PWM_SEND_DATA1);
				__raw_writel(testregvalue,
					     PWM_register[pwm_no] + 4 * PWM_SEND_DATA1);
				readregvalue =
				    __raw_readl(PWM_register[pwm_no] + 4 * PWM_SEND_DATA1);
				if ((readregvalue & 0xffffffff) != (testregvalue & 0xffffffff)) {
					statusflag++;
...
/*
				...
				*/
				...
				originalvalue =
				    __raw_readl(PWM_register[pwm_no] + 4 * PWM_DATA_WIDTH);
				__raw_writel(testregvalue,
					     PWM_register[pwm_no] + 4 * PWM_DATA_WIDTH);
				readregvalue =
				    __raw_readl(PWM_register[pwm_no] + 4 * PWM_DATA_WIDTH);
	...
	case PWM_UVVF_MEMO_HLGDUR:
		...
		while (1) {
			printk(KERN_INFO
			       ...
			       conf.PWM_MODE_MEMORY_REGS.HDURATION, conf.pwm_no);
			pwm_set_spec_config(&conf);
	...
	case PWM_UVVF_MEMO_GIOUT:
		...
		while (1) {
			printk(KERN_INFO
			       ...
			       conf.PWM_MODE_MEMORY_REGS.WAVE_NUM, conf.pwm_no);
			pwm_set_spec_config(&conf);
	...
	case PWM_UVVF_OLD_DTDUR:
		...
		while (1) {
			printk(KERN_INFO "PWM: data_width = %x, gduration = %x, pwm_no = %x\n",
			       conf.PWM_MODE_OLD_REGS.DATA_WIDTH, conf.PWM_MODE_OLD_REGS.GDURATION,
			       conf.pwm_no);
			pwm_set_spec_config(&conf);
	...
	case PWM_UVVF_OLD_GIOUT:
		...
		while (1) {
			printk(KERN_INFO
			       ...
			       conf.PWM_MODE_OLD_REGS.IDLE_VALUE, conf.PWM_MODE_OLD_REGS.WAVE_NUM,
			       conf.pwm_no);
			pwm_set_spec_config(&conf);
	...
	case PWM_UVVF_RANDOM_HLDUR:
		...
		while (1) {
			printk(KERN_INFO "PWM: lduration = %x, hduration = %x, pwm_no = %x\n",
			       conf.PWM_MODE_RANDOM_REGS.LDURATION,
			       conf.PWM_MODE_RANDOM_REGS.HDURATION, conf.pwm_no);
			pwm_set_spec_config(&conf);
