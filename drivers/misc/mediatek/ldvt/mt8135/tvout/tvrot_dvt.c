...
static unsigned int *pva;
static unsigned int phy;
...
static void tvr_dvt_path_init(void)
{
	TVDVT_init_lcd();
	TVR_Init();
	TVR_PowerOn();
...
/* 3. Config TVR */
	...
	if (tvrot_param.bAuto) {
		tvrot_param.dstBufAddr[1] =
		    param->dst_img_addr + tvrot_param.srcWidth * tvrot_param.srcHeight * 2;
		tvrot_param.dstBufAddr[2] =
		    tvrot_param.dstBufAddr[1] + tvrot_param.srcWidth * tvrot_param.srcHeight * 2;
	}
...
void tvr_dvt_path_start(void)
{
	TVR_Start();
	TVDVT_update_screen();
}

static int tvr_dvt_path_wait_done(void)
{
...
static int tvr_dvt_path_stop(void)
{
	TVR_Stop();
}

static int tvr_dvt_path_deinit(void)
{
	TVR_Deinit();
}
...
static int tvr_dvt_ioctl(struct inode *stInode,
			 struct file *stFile, unsigned int cmd, unsigned long param)
{
	int ret = 0;
	/* printk("%s\n", __func__); */
