...
S32 _pwrap_event_test(void)
{
	...
	if (sub_return != 0) {	/* TERR="Error: [DrvPWRAP_EventTest][DrvPWRAP_WACS2] fail, return=%x, exp=0x0", sub_return */
...
S32 _pwrap_status_update_test(void)
{
	...
	int_en_orig = WRAP_RD32(PMIC_WRAP_INT_EN);
...
S32 _pwrap_wrap_access_test(void)
{
	...
	if (rdata != DEFAULT_VALUE_READ_TEST) {
		PWRAPERR("read test error(using WACS3),return_value=%x, rdata=%x", return_value,
			 rdata);
		res += 1;
	}
	...
	if (rdata != 0x1234) {
		PWRAPERR("write test error(using WACS3),return_value=%x, rdata=%x", return_value,
			 rdata);
		res += 1;
	}
	/* ###############################//############################### */
	...
	/* ############################### */
	PWRAPLOG("start test WACS4\n");
	return_value = pwrap_wacs4(0, DEW_READ_TEST, 0, &rdata);
	if (rdata != DEFAULT_VALUE_READ_TEST) {
		PWRAPERR("read test error(using WACS4),return_value=%x, rdata=%x", return_value,
			 rdata);
		res += 1;
	}
	...
	if (rdata != 0x1234) {
		PWRAPERR("write test error(using WACS4), return_value=%x, rdata=%x", return_value,
			 rdata);
		res += 1;
	}
...
S32 _pwrap_man_access_test(void)
{
	...
	_pwrap_switch_mux(0);	/* switch wrap mode */
...
U32 RegWriteValue[4] = { 0, 0xFFFF, 0x5555, 0xAAAA };
U32 RegWriteValue32bit[4] = { 0, 0xFFFFFFFF, 0x55555555, 0xAAAAAAAA };

#define PWRAP_REG_TEST
#define PWRAP_PERI_REG_TEST
#define DEW_REG_TEST
...
S32 tc_reg_rw_test(void)
{
	...
	PERI_PWRAP_BRIDGE_reg_size =
	    sizeof(PERI_PWRAP_BRIDGE_reg_tbl) / sizeof(PERI_PWRAP_BRIDGE_reg_tbl[0]);
...
#ifdef PWRAP_REG_TEST
	PWRAPLOG("start test pmic_wrap_reg_tbl\n");
	for (i = 0; i < pmic_wrap_reg_size; i++) {
		PWRAPLOG("%d [0]=0x%x [1]=0x%x [2]=0x%x\n", i, pmic_wrap_reg_tbl[i][0],
			 pmic_wrap_reg_tbl[i][1], pmic_wrap_reg_tbl[i][2]);
		/* Only R/W or RO should do default value test */
		if (REG_TYP_WO != pmic_wrap_reg_tbl[i][3]) {
			if ((*((volatile unsigned int *)pmic_wrap_reg_tbl[i][0]) !=
			     pmic_wrap_reg_tbl[i][1])) {
				PWRAPLOG
				    ("Register %x Default Value test fail. DEF %x, read %x \r\n",
	...
	/* test value:RegWriteValue32bit[LCD_RegWriteValueSize] = {0, 0xFFFFFFFF, 0x55555555, 0xAAAAAAAA}; */
	for (i = 0; i < pmic_wrap_reg_size; i++) {
		if (REG_TYP_RW == pmic_wrap_reg_tbl[i][3]) {
			for (j = 0; j < pmic_wrap_reg_size; j++) {
				/* write to register */
				*((volatile unsigned int *)pmic_wrap_reg_tbl[i][0]) =
				    (RegWriteValue32bit[j] & pmic_wrap_reg_tbl[i][2]);

				if (((*((volatile unsigned int *)pmic_wrap_reg_tbl[i][0]) &
				      pmic_wrap_reg_tbl[i][2]) !=
				     (RegWriteValue32bit[j] & pmic_wrap_reg_tbl[i][2]))) {
					PWRAPLOG
					    ("Register %x R/W test fail. write %x, read %x \r\n",
	...
	for (i = 0; i < PERI_PWRAP_BRIDGE_reg_size; i++) {
		...
		if (REG_TYP_WO != PERI_PWRAP_BRIDGE_reg_tbl[i][3]) {
			if ((*((volatile unsigned int *)PERI_PWRAP_BRIDGE_reg_tbl[i][0]) !=
			     PERI_PWRAP_BRIDGE_reg_tbl[i][1])) {
				PWRAPLOG
				    ("Register %x Default Value test fail. DEF %x, read %x \r\n",
	...
	for (i = 0; i < PERI_PWRAP_BRIDGE_reg_size; i++) {
		if (REG_TYP_RW == PERI_PWRAP_BRIDGE_reg_tbl[i][3]) {
			for (j = 0; j < PERI_PWRAP_BRIDGE_reg_size; j++) {
				*((volatile unsigned int *)PERI_PWRAP_BRIDGE_reg_tbl[i][0]) =
				    (RegWriteValue32bit[j] & PERI_PWRAP_BRIDGE_reg_tbl[i][2]);

				if (((*((volatile unsigned int *)PERI_PWRAP_BRIDGE_reg_tbl[i][0]) &
				      PERI_PWRAP_BRIDGE_reg_tbl[i][2]) !=
				     (RegWriteValue32bit[j] & PERI_PWRAP_BRIDGE_reg_tbl[i][2]))) {
					PWRAPLOG
					    ("Register %x R/W test fail. write %x, read %x \r\n",
					     PERI_PWRAP_BRIDGE_reg_tbl[i][0],
...
/* init wrap */
...
#ifdef DEW_REG_TEST
	PWRAPLOG("start test DEW_reg_tbl_size\n");
	for (i = 0; i < DEW_reg_tbl_size; i++) {
		PWRAPLOG("%d [0]=0x%x [1]=0x%x [2]=0x%x [3]=%d\n", i, DEW_reg_tbl[i][0],
			 DEW_reg_tbl[i][1], DEW_reg_tbl[i][2], DEW_reg_tbl[i][3]);

		/* Only R/W or RO should do default value test */
		if (REG_TYP_WO != DEW_reg_tbl[i][3]) {
			sub_return = pwrap_read(DEW_reg_tbl[i][0], &regValue);
			if (sub_return == 0) {
				PWRAPREG("0x%x = 0x%x\n", DEW_reg_tbl[i][0], regValue);

				if ((regValue != DEW_reg_tbl[i][1])) {
					PWRAPLOG
					    ("Register %x Default Value test fail. DEF %x, read %x \r\n",
					     DEW_reg_tbl[i][0], DEW_reg_tbl[i][1], regValue);
					test_result++;
				}
			}
		}
	...
	/* test value:RegWriteValue[LCD_RegWriteValueSize] = {0, 0xFFFF, 0x5555, 0xAAAA}; */
	for (i = 0; i < DEW_reg_tbl_size; i++) {
		if (REG_TYP_RW == DEW_reg_tbl[i][3]) {
			PWRAPLOG
			    ("%d [reg_offset]=0x%x [default value]=0x%x [mask]=0x%x [type]=%d\n", i,
			     DEW_reg_tbl[i][0], DEW_reg_tbl[i][1], DEW_reg_tbl[i][2],
			     DEW_reg_tbl[i][3]);

			if (DEW_reg_tbl[i][0] == DEW_DIO_EN)	/* skip DIO registers */
				continue;

			for (j = 0; j < 4; j++) {
				reg_data = RegWriteValue[j] & DEW_reg_tbl[i][2];
				PWRAPREG("write 0x%x = 0x%x(orig value = 0x%x, mask = 0x%x)\n",
					 DEW_reg_tbl[i][0], reg_data, RegWriteValue[j],
					 DEW_reg_tbl[i][2]);

				sub_return = pwrap_write(DEW_reg_tbl[i][0], reg_data);
				mdelay(1);
				sub_return1 = pwrap_read(DEW_reg_tbl[i][0], &regValue);
				if ((sub_return == 0) && (sub_return1 == 0)) {
					regValue &= DEW_reg_tbl[i][2];
					PWRAPREG("0x%x = 0x%x\n", DEW_reg_tbl[i][0], regValue);

					if (regValue != reg_data) {
						PWRAPLOG
						    ("Register %x R/W test fail. write %x, read %x \r\n",
						     DEW_reg_tbl[i][0], reg_data, regValue);
						test_result++;
					}
				} else {
					PWRAPREG("sub_return=%d sub_return1=%d\n", sub_return,
						 sub_return1);
				}
			}
			PWRAPREG("\n");
...
static U32 peri_int_test_bit;
static U32 wait_peri_int_flag;
static U32 peri_int_test_fail_count;
...
static U32 peri_wdt_test_bit;
static volatile U32 wait_for_peri_wdt_flag;
static U32 peri_wdt_test_fail_count;
...
S32 _wdt_test_bit9(void)
{
	...
	for (i = 0; i < EINT_MAX_CHANNEL; i++) {
		eint_registration(eint_in_cpu0, 0, HIGH_LEVEL_TRIGGER,
				  _concurrence_eint_test_callback1, 1);
...
{
	PWRAPLOG("_int_test_bit1 test combine with status update test.\n");
}
...
/* #define INT_TEST_ALL */
#define INT_TEST_BIT1
/* #define INT_TEST_BIT3 */
/* #define INT_TEST_BIT20 */
...
S32 tc_int_test(void)
{
...
#if (defined(INT_TEST_BIT1) || defined(INT_TEST_ALL))
	/* bit1 */
	pwrap_init();
...
#if (defined(INT_TEST_BIT20) || defined(INT_TEST_ALL))
	/* bit20 */
	pwrap_init();
	_int_test_disable_watch_dog();
...
#if (defined(INT_TEST_BIT3) || defined(INT_TEST_ALL))
	/* bit3 */
	pwrap_init();
	_int_test_disable_watch_dog();
	...
	/* re-init pwrap for further test */
	pwrap_init();
...
/* [8]: WACS4_UNEXP_VLDCLR: WACS4 unexpected VLDCLR */
S32 _peri_int_test_bit8(void)
{
	U32 rdata = 0;
	U32 reg_rdata = 0;
	U32 res = 0;
...
/* [10]: WACS4_CMD_MISS: A WACS4 CMD is written while MAN is disabled. */
/* disable wacs4,send a wacs4 command */
S32 _peri_int_test_bit10(void)
{
	U32 rdata = 0;
	U32 reg_rdata = 0;
	U32 res = 0;
...
#endif
		{
			if ((wacs3_read_write_flag == 0) && (wacs3_send_write_cmd_done == 0)) {
				...
				if (rdata != wacs3_test_value) {
					PWRAPERR
					    ("read test error(using WACS3),wacs3_test_value=%x, rdata=%x\n",
...
#endif
		{
			if ((wacs4_read_write_flag == 0) && (wacs4_send_write_cmd_done == 0)) {
				...
				if (rdata != wacs4_test_value) {
					/* TERR="Error: [WriteTest] fail, rdata=%x, exp=0x1234", rdata */
					PWRAPERR
					    ("read test error(using WACS4),wacs4_test_value=%x, rdata=%x\n",
