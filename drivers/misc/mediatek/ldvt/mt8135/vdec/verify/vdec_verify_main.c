...
BOOL fgVDecVerify_PrepareInfo_RM(UINT32 u4InstID)
{
	...
	fgOpen =
	    fgOpenFile(u4InstID, _bFileStr1[u4InstID][RM_FRMINFO_INDEX], "r+b",
	...
	sprintf(strEmuFileName, "%s/%s%07d%s", _bFileStr1[u4InstID][RM_GOLDENPATH_INDEX],
		strRMGoldenFileName, _u4FileCnt[u4InstID], strGoldenFileNameExt_FileList);
#else				/* RM_CRCCHECK_ENABLE */
	{
		UINT32 u4PicCnt = _u4FileCnt[u4InstID];
		if (u4PicCnt / 5000 > 0) {
			sprintf(strEmuFileName, "%s%05d/%s%07d%s",
				_bFileStr1[u4InstID][RM_GOLDENPATH_INDEX], u4PicCnt / 5000 * 5000,
				strRMGoldenFileName, _u4FileCnt[u4InstID],
				strGoldenFileNameExt_FileList);
		} else {
			/* sprintf(strEmuFileName, "%s/%s%07d%s", _bFileStr1[u4InstID][RM_GOLDENPATH_INDEX], */
			sprintf(strEmuFileName, "%s%s%07d%s",
				_bFileStr1[u4InstID][RM_GOLDENPATH_INDEX], strRMGoldenFileName,
				_u4FileCnt[u4InstID], strGoldenFileNameExt_FileList);
		}
	}
...
#ifdef SATA_HDD_FS_SUPPORT
	{
		UINT32 u4PicCnt = _u4FileCnt[u4InstID];
		if (u4PicCnt / 5000 > 0) {
			sprintf(_bFileStr1[u4InstID][1], "%s%05d/%s%07d%s",
				_bFileStr1[u4InstID][RM_SOURCEPATH_INDEX], u4PicCnt / 5000 * 5000,
				strRMSrcFileName, _u4FileCnt[u4InstID], strSrcFileNameExt_FileList);
		} else {
			/* sprintf(strEmuFileName, "%s/%s%07d%s", _bFileStr1[u4InstID][RM_SOURCEPATH_INDEX], */
			sprintf(_bFileStr1[u4InstID][1], "%s%s%07d%s",
				_bFileStr1[u4InstID][RM_SOURCEPATH_INDEX], strRMSrcFileName,
				_u4FileCnt[u4InstID], strSrcFileNameExt_FileList);
		}
	}
	...
	fgOpen =
	    fgOpenFile(u4InstID, _bFileStr1[u4InstID][RM_AUFIFO_INDEX], "r+b",
		       &_tInFileInfo[u4InstID]);
	if (fgOpen == FALSE) {
		vVDecOutputDebugString("Open bit-stream fail\n");
		strcpy(_tFileListRecInfo[u4InstID].bFileName, _FileList_Rec[u4InstID]);
	...
	fgOpen =
	    fgOpenFile(u4InstID, _bFileStr1[u4InstID][RM_SUMINFO_INDEX], "r+b",
		       &_tInFileInfo[u4InstID]);
	if (fgOpen == FALSE) {
		vVDecOutputDebugString("Open bit-stream fail\n");
		strcpy(_tFileListRecInfo[u4InstID].bFileName, _FileList_Rec[u4InstID]);
	...
	if (fgRMCheckCRCResult) {
		...
		fgOpen =
		    fgOpenFile(u4InstID, _bFileStr1[u4InstID][RM_CRCINFO_INDEX], "r+b",
			       &_tInFileInfo[u4InstID]);
		if (fgOpen == FALSE) {
			vVDecOutputDebugString("Open bit-stream fail\n");
			strcpy(_tFileListRecInfo[u4InstID].bFileName, _FileList_Rec[u4InstID]);
...
/* u4WmvMode  =  pparam->u4Mode; */
	...
	if (!_fgSemaCreated[u4InstID]) {
		VERIFY(x_sema_create
		       (&_ahVDecEndSema[u4InstID], X_SEMA_TYPE_BINARY,
	...
	while (fgVdecReadFileName
	       (u4InstID, &_tFileListInfo[u4InstID], &_tFileListRecInfo[u4InstID],
		&_u4StartCompPicNum[u4InstID], &_u4EndCompPicNum[u4InstID],
		&_u4DumpRegPicNum[u4InstID])) {
		...
		if (_u4CodecVer[u4InstID] == VDEC_H264) {
			...
			fgOpen =
			    fgOpenFile(u4InstID, _bFileStr1[u4InstID][12], "r+b", &tInFileInfo);
			if (fgOpen == FALSE) {
			...
			fgOpen =
			    fgOpenFile(u4InstID, _bFileStr1[u4InstID][13], "r+b", &tInFileInfo);
			if (fgOpen == FALSE) {
		...
		if ((_u4CodecVer[u4InstID] == VDEC_H264) || (_u4CodecVer[u4InstID] == VDEC_MPEG4)) {
			vAddStartCode2Dram(_pucVFifo[u4InstID] +
					   _tInFileInfo[u4InstID].u4FileLength);
		}
#if VDEC_CACHE_PROC		/* update physical memory */
		{
			...
			printk
			    ("vVDecVerifyThread, update cache to physical mem, addr:0x%x, size:0x%x\n",
			...
			u4MappedVFifoSa =
			    dma_map_single(NULL, (UINT32) (_pucVFifo[u4InstID]), u4FifoDataSize,
					   DMA_TO_DEVICE);
		}
#endif
		...
		strcpy(_tFileListRecInfo[u4InstID].bFileName, _FileList_Rec[u4InstID]);
		sprintf(strMessage,
			"The bitstream file length / real returns is %d/%d (0x%.8x/0x%.8x)\n",
			_tInFileInfo[u4InstID].u4FileLength, _tInFileInfo[u4InstID].u4RealGetBytes,
			_tInFileInfo[u4InstID].u4FileLength, _tInFileInfo[u4InstID].u4RealGetBytes);
		fgWrMsg2PC(strMessage, strlen(strMessage), 8, &_tFileListRecInfo[u4InstID]);
		...
		if (_u4CodecVer[u4InstID] == VDEC_VP6) {
			...
			printk("<vdec> _fgVP6CRCExist[%u]=%d, _fgVP6SmallFlolder[%u]=%d\n",
			       u4InstID, _fgVP6CRCExist[u4InstID], u4InstID,
			       _fgVP6SmallFlolder[u4InstID]);
		}

		printk("_u4CodecVer is 0x%x\n", _u4CodecVer[u4InstID]);
		...
		if (_u4CodecVer[u4InstID] == VDEC_VP6) {
			...
			for (i = 0;; i++) {
				...
				if (_bFileStr1[u4InstID][1][i] == 'b'
				    || _bFileStr1[u4InstID][1][i] == 'B') {
			...
			u4Temp =
			    sprintf((char *)_bFileStr1[u4InstID][3], "%s", (char *)_bPatternPath);
			u4Temp +=
			    sprintf((char *)_bFileStr1[u4InstID][3] + u4Temp, "%s", _bFileCnt);

			printk("Filecnt path  %s\n", _bFileStr1[u4InstID][3]);
...
#endif
			{
				...
				if (ucNum >= 0x30 && ucNum <= 0x39) {
					if (i == 0) {
						...
						_u4PicTotalCnt[u4InstID] +=
						    ((ucNum - 0x30) * u4Get10Value(i));
						i += 1;
					}
				...
				printk("_u4PicTotalCnt  %d, i is %d\n", _u4PicTotalCnt[u4InstID],
				       i);
			}
		...
		if (_u4CodecVer[u4InstID] == VDEC_VP8) {
			...
			for (i = 0;; i++) {
				...
				if (_bFileStr1[u4InstID][1][i] == 'v'
				    || _bFileStr1[u4InstID][1][i] == 'V') {
			...
			u4Temp =
			    sprintf(_bPatternPath + path_byte_addr + 14, "%s", _bPathAddStr) + 14;
			u4Temp +=
			    sprintf(_bPatternPath + path_byte_addr + u4Temp, "%s", _bFileName);
			/* u4Temp += sprintf(_bPatternPath+path_byte_addr+u4Temp, "%s", _bGoldFileName); */
			_bPatternPath[path_byte_addr + u4Temp] = '\0';
