...
UINT32 u4VParserMPEG4(UINT32 u4InstID, BOOL fgInquiry)
{
	...
	case VDEC_DIVX3:
		...
		_u4Divx3SetPos[u4InstID] =
		    _u4Divx3SetPos[u4InstID] + 4 +
		    u4VDEC_HAL_MPEG_GetBitStreamShift(_u4BSID[u4InstID], u4InstID, 32);
...
UINT32 u4ShVParser(UINT32 u4InstID, BOOL fgInquiry)
{
	...
	if (_u4CodecVer[u4InstID] == VDEC_H263) {
		...
		case 0:
			_u4Datain[u4InstID] =
			    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 16);
		...
		_u4Datain[u4InstID] =
		    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 9);
		...
		_u4Datain[u4InstID] =
		    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 18);
...
void vVerifyCalGmcMv(UINT32 u4InstID)
{
	...
	BOOL fgBadDivX = ((_u4UserDataCodecVersion[u4InstID] == 500) ||
	    ...
	    (_u4UserDataBuildNumber[u4InstID] >= 370) && (_u4UserDataBuildNumber[u4InstID] <= 413);
	long i0_pla, j0_pla;
...
void vVerifyMp4SetDecPrm(UINT32 u4InstID)
{
	...
	VDEC_INFO_MPEG4_DEC_PRM_T * ptMp4Prm =
	...
	if ((_u4UserDataCodecVersion[u4InstID] < 500) &&
	    ...
	    _fgVerUDivXM4v[u4InstID] && !fgPureIsoM4v()) {
...
/* _fgVerUDivXM4v[u4InstID] && */
	...
	if (fgPureIsoM4v()) {
		...
		if (_ucVopCdTp[u4InstID] != VCT_B) {
			if (_u4CodecVer[u4InstID] == VDEC_DIVX3) {
				...
				ptMp4Prm->u4CMvType =
				    (ptMp4Prm->u4CMvType & 0xfffffffe) | QPEL_4MV_CMV_14496;
			} else if ((_u4UserDataCodecVersion[u4InstID] == 0)
			...
			if ((_u4UserDataCodecVersion[u4InstID] == 0)
			    || (_u4UserDataCodecVersion[u4InstID] >= 500)) {
				ptMp4Prm->u4CMvType |= QPEL_FRAME_CMV_14496;
				/* ptMp4Prm->u4CMvType |= QPEL_FRAME_CMV_DIVXOLD; */
			} else {
				ptMp4Prm->u4CMvType |= QPEL_FRAME_CMV_DIVXOLD;
...
#else
	u4DivxCodecVersion = _u4UserDataCodecVersion[u4InstID];
	if (u4DivxCodecVersion == 0)	/* only for verification */
	...
	{
		u4MVWorkaround |=
		    BUG_SEDG_QPEL_CHROMA | BUG_MMS_FCODE_FOR | BUG_MMS_INTER4V_CHR |
		    BUG_MMS_BVOP_CHR;
	...
	if (_ucVopCdTp[u4InstID] != VCT_B) {
		...
		if (u4MVWorkaround & BUG_MMS_INTER4V_CHR) {
			...
			ptMp4Prm->u4CMvType |= QPEL_4MV_CMV_DIVX503;
		}

		ptMp4Prm->u4CMvType |= QPEL_FIELD_CMV_DIVX503;
	} else {
		if (u4MVWorkaround & BUG_MMS_BVOP_CHR
		    && ptMp4Prm->rDep.rM4vDecPrm.prVol->ucQuarterSample) {
			ptMp4Prm->u4CMvType |= QPEL_FRAME_CMV_14496;
		} else {
			ptMp4Prm->u4CMvType |= QPEL_FRAME_CMV_DIVXOLD;
		}
		...
		if ((!(u4MVWorkaround & BUG_MMS_BVOP_CHR))
		    && ptMp4Prm->rDep.rM4vDecPrm.prVol->ucQuarterSample) {
			printk
			    ("HW not support this mode for chrome MV calculation in inter4mv B VOP\n");
		} else {
			ptMp4Prm->u4CMvType |= QPEL_4MV_CMV_14496;
		}
		ptMp4Prm->u4CMvType |= QPEL_FIELD_CMV_14496;
...
UINT32 u4Dx3VParser(UINT32 u4InstID, BOOL fgInquiry)
{
	...
	if (_ucVopCdTp[u4InstID] == VCT_I) {
		...
		_u4Datain[u4InstID] =
		    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 5);
		...
		_u4Datain[u4InstID] =
		    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 2);
	}
...
void vVerifyDx3SetPicLayerPrm(UINT32 u4InstID)
{
	VDEC_INFO_DIVX3_PIC_PRM_T *ptDx3Prm =
	    &_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecMPEGDecPrm.rPicLayer.rMp4DecPrm.rDep.
	    rDx3DecPrm;
...
UINT32 u4M4VParser(UINT32 u4InstID, BOOL fgInquiry)
{
	...
	while ((_u4Datain[u4InstID] =
		u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID,
						  0)) == USER_DATA_START_CODE) {
	...
	if (_fgVerResyncMarkerDisable[u4InstID] == 0 && (u4MbW * u4MbH) > 4096) {
		...
		u4BytePos =
		    u4VDEC_HAL_MPEG_ReadRdPtr(_u4BSID[u4InstID], u4InstID,
					      (UINT32) _pucVFifo[u4InstID], &u4BitsPos);
		...
		while (!fgMPEG4ValidStartCode(_u4Datain[u4InstID])) {
			if ((_u4Datain[u4InstID] & u4ResyncMask) == u4ResyncPattern) {
				...
				_u4ResyncMarkMbx[u4InstID][u4ResyncMarkerCnt] =
				    (u4MbNum -
				     (_u4ResyncMarkMby[u4InstID][u4ResyncMarkerCnt] * u4MbW));
...
UINT32 u4NormalVOL(UINT32 u4InstID)
{
	...
	{
		_u4Datain[u4InstID] =
		    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 7);
	...
	_ucAspRatInf[u4InstID] =
	    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 4) & 0xf;
...
*/
	...
	{
		_u2FixedVopTimeIncrement[u4InstID] =
		    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID,
						      _ucVopTimeIncrementResolutionBits[u4InstID]);
		_u2FixedVopTimeIncrement[u4InstID] &=
		    u4VerifyLSBMask(_ucVopTimeIncrementResolutionBits[u4InstID]);
	}
	...
	if (_ucVideoObjectLayerVerid[u4InstID] == 1) {
		_ucSpriteEnable[u4InstID] =
		    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 1) & 0x1;
	} else {
		_ucSpriteEnable[u4InstID] =
		    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 2) & 0x3;
	}
	...
	if (_fgVerQuantType[u4InstID]) {
		...
		if (_fgVerLoadIntraMatrix[u4InstID]) {
			...
			vVDEC_HAL_MPEG4_FW_LoadQuantMatrix(u4InstID, TRUE,
							   &_u4QuanMatrixLen[u4InstID],
							   (UINT32 *) (rQuantMatrix[u4InstID].
								       ucQuantMatrix));
#else
			if (!fgVDEC_HAL_MPEG4_LoadQuantMatrix
			    (u4InstID, TRUE, &_u4QuanMatrixLen[u4InstID], &rMpegBSInitPrm,
			     &rQuantMatrix[u4InstID])) {
...
#ifdef FW_WRITE_QUANTIZATION
			vVerifyParseQtable(u4InstID);
			vVDEC_HAL_MPEG4_FW_LoadQuantMatrix(u4InstID, FALSE,
							   &_u4QuanMatrixLen[u4InstID],
							   (UINT32 *) (rQuantMatrix[u4InstID].
								       ucQuantMatrix));
#else
			if (!fgVDEC_HAL_MPEG4_LoadQuantMatrix
			    (u4InstID, FALSE, &_u4QuanMatrixLen[u4InstID], &rMpegBSInitPrm,
			     &rQuantMatrix[u4InstID])) {
...
void vVerifyGetPxlAspRat(UINT32 u4InstID, BYTE bAspRatInf)
{
	...
	case 0xf:
		_ucParW[u4InstID] =
		    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 8) & 0xff;
		_ucParH[u4InstID] =
		    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 8) & 0xff;
		break;
	}
...
UINT32 u4VOP(UINT32 u4InstID)
{
	...
	_u4Datain[u4InstID] =
	    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID,
					      _ucVopTimeIncrementResolutionBits[u4InstID] + 1);
	_u2VopTimeIncrement[u4InstID] =
	    _u4Datain[u4InstID] & u4VerifyLSBMask(_ucVopTimeIncrementResolutionBits[u4InstID]);
	_u4CurrDispTime[u4InstID] =
	    (_u4CurrDispTime[u4InstID] * _u2VopTimeIncrementResolution[u4InstID]) +
	    _u2VopTimeIncrement[u4InstID];
	if (_ucVopCdTp[u4InstID] != VCT_B) {
		...
		if (_fgVerInterlaced[u4InstID]) {
			if (_rDirMode[u4InstID].u4TFrm == 0xffffffff) {
				_rDirMode[u4InstID].u4TFrm =
				    _u4NextDispTime[u4InstID] - _u4CurrDispTime[u4InstID];
			}
			dwRetVal =
			    u4Div2Slash(_u4PrevDispTime[u4InstID], _rDirMode[u4InstID].u4TFrm);
			_rDirMode[u4InstID].u4Trbi =
			    2 *
			    (u4Div2Slash(_u4CurrDispTime[u4InstID], _rDirMode[u4InstID].u4TFrm) -
			     dwRetVal);
			_rDirMode[u4InstID].u4Trdi =
			    2 *
			    (u4Div2Slash(_u4NextDispTime[u4InstID], _rDirMode[u4InstID].u4TFrm) -
			     dwRetVal);
		}
	}
	_u4Datain[u4InstID] = u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 2);
	...
	if (_fgVerReducedResolutionVopEnable[u4InstID] &&
		...
		_fgVerVopReducedResolution[u4InstID] =
		    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 1) & 0x1;
	}

	_ucIntraDcVlcThr[u4InstID] =
	    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 3) & 0x7;
	if (_fgVerInterlaced[u4InstID]) {
		_u4Datain[u4InstID] =
		    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 2);
	...
	_ucVopQuant[u4InstID] =
	    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 5) & 0x1f;
	if (_ucVopCdTp[u4InstID] != VCT_I) {
		_ucFordFCode[u4InstID] =
		    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 3) & 0x7;
	}
	if (_ucVopCdTp[u4InstID] == VCT_B) {
		_ucBackFCode[u4InstID] =
		    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 3) & 0x7;
	}
	u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 32);
	if ((dwRetVal = u4VopData()) != 0) {
...
INT32 i4WarpingMvCode(UINT32 u4InstID)
{
	...
	code +=
	    (u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, dwLen - 1) &
	     u4VerifyLSBMask(dwLen - 1));
	return (code);
}
...
void vVerifyUserData(UINT32 u4InstID)
{
	...
	if ((pbUdBuf[0] != 'D') ||
	    (pbUdBuf[1] != 'i') || (pbUdBuf[2] != 'v') || (pbUdBuf[3] != 'X')) {
		return;
	}
	...
	if ((pbUdBuf[7] == 'B') &&
	    (pbUdBuf[8] == 'u') &&
	    (pbUdBuf[9] == 'i') && (pbUdBuf[10] == 'l') && (pbUdBuf[11] == 'd')) {
...
UINT32 u4VerifyGetDivXNo(BYTE *pbNo)
{
	...
	dwRetVal = ((pbNo[0] & 0xf) * 100) + ((pbNo[1] & 0xf) * 10) + (pbNo[2] & 0xf);
	return (dwRetVal);
}
...
BOOL fgVPrsValidStartCode(UINT32 u4InstID, BOOL fgMp4, UINT32 *pdwSC)
{
	if (fgMp4) {
		if (fgMPEG4ValidStartCode(*pdwSC))
			return TRUE;
		else {		/* check video plane with short header */
...
#else
				else if ((dwDatain & SHORT_VIDEO_START_MASK) ==
...
#else
			else if (/*(_fgShortHeader[u4InstID])&& */
			    (((*pdwSC) & SHORT_VIDEO_START_MASK) == SHORT_VIDEO_START_MARKER))
#endif
			{
				return TRUE;
			}
		...
		if ((SEQUENCE_HEADER_CODE == (*pdwSC)) ||
		    ...
		    (PICTURE_START_CODE == (*pdwSC)) || (SEQUENCE_END_CODE == (*pdwSC)))
			return TRUE;
	}
	return FALSE;
}
...
static void vVerifyParseQtable(UINT32 u4InstID)
{
	...
	for (ucMatrixIdx = 0; ucMatrixIdx < MPV_MATRIX_SIZE; ucMatrixIdx++) {
		...
		if (u4Byte0 == 0x0 || u4Byte1 == 0x0 || u4Byte2 == 0x0 || u4Byte3 == 0x0) {
			if (u4Byte0 == 0x0) {
				u4RemainData =
				    (u4LastQuant << 24) + (u4LastQuant << 16) + (u4LastQuant << 8) +
				    u4LastQuant;
				u4Datain =
				    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID,
								      8);
				...
				u4RemainData =
				    (u4Byte0 << 24) + (u4Byte0 << 16) + (u4Byte0 << 8) + u4Byte0;
				...
				u4RemainData =
				    (u4Byte1 << 24) + (u4Byte1 << 16) + (u4Byte1 << 8) + u4Byte0;
				pu4Addr[ucMatrixIdx] = u4RemainData;
				...
				u4RemainData =
				    (u4Byte2 << 24) + (u4Byte2 << 16) + (u4Byte1 << 8) + u4Byte0;
				pu4Addr[ucMatrixIdx] = u4RemainData;
