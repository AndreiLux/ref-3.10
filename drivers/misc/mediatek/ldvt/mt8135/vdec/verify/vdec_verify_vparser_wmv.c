...
/* Robert Adds. */
void reset_pic_hdr_bits(UINT32 u4InstID)
{
	_u4PicHdrBits[u4InstID] = 0;
	_fgCounting[u4InstID] = TRUE;
}

UINT32 pic_hdr_bitcount(UINT32 u4InstID)
{
	return _u4PicHdrBits[u4InstID];
}

void set_pic_hdr_bits(UINT32 u4InstID, UINT32 n)
{
...
void after_bp(UINT32 u4InstID)
{
	...
	if (_u4BSID[u4InstID] == 1) {
		u4Bytes =
		    u4VDEC_HAL_WMV_ReadRdPtr(0, u4InstID, (UINT32) _pucVFifo[u4InstID], &u4Bits);
		rWmvBSInitPrm.u4VFifoSa = (UINT32) _pucVFifo[u4InstID];
		rWmvBSInitPrm.u4VFifoEa = (UINT32) _pucVFifo[u4InstID] + V_FIFO_SZ;
		rWmvBSInitPrm.u4ReadPointer = (UINT32) _pucVFifo[u4InstID] + u4Bytes;
#ifndef RING_VFIFO_SUPPORT
...
void vRCVFileHeader(UINT32 u4InstID)
{
	...
	if (i4hdrext != 0) {
		...
		_iSeqHdrData1[u4InstID] =
		    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 32);
		AddPicHdrBitCount(u4InstID, 32);
		if (_iSeqHdrDataLen[u4InstID] == 5) {
			_iSeqHdrData2[u4InstID] =
			    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 8);
...
UINT32 u4DecodeVOLHead_WMV3(UINT32 u4InstID)
{
	...
	prWMVEPS->fgExplicitQuantizer = _rWMVEPS[u4InstID].fgExplicitSeqQuantizer
	    || _rWMVEPS[u4InstID].fgExplicitFrameQuantizer;
...
UINT32 WMVideoDecDecodeClipInfo(UINT32 u4InstID)
{
	...
	if (_i4CodecVersion[u4InstID] >= VDEC_WMV2) {
		if (prWMVSPS->fgXintra8Switch) {
			prWMVSPS->fgXintra8 =
			    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 1);
	...
	} else			/* WMV1 */
	{
		...
		prWMVSPS->i4BitRate =
		    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 11);
...
void decodeVOPHead_WMV2(UINT32 u4InstID)
{
	...
	if (!prWMVSPS->fgSkipBitCoding) {
		prWMVSPS->fgCODFlagOn =
		    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 1);
		AddPicHdrBitCount(u4InstID, 1);
		prWMVSPS->i4SkipBitModeV87 = prWMVSPS->fgCODFlagOn;	/* 0:no_skip_bit, 1:raw_mode */
	} else {
		INT32 iSkipBitCode =
		    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 2);
	...
	if (prWMVSPS->fgNewPcbPcyTable) {
		if (prWMVPPS->i4StepSize <= 10) {
			if (!u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 1)) {	/* 0 High */
				prWMVSPS->i4HufNewPCBPCYDec = HighRate;
				AddPicHdrBitCount(u4InstID, 1);
			} else if (!u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 1)) {	/* 10 Low */
			...
			if (!u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 1)) {	/* 0 Mid */
				prWMVSPS->i4HufNewPCBPCYDec = MidRate;
				AddPicHdrBitCount(u4InstID, 1);
			} else if (!u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 1)) {	/* 10 High */
				prWMVSPS->i4HufNewPCBPCYDec = HighRate;
				AddPicHdrBitCount(u4InstID, 2);
			} else {	/* 11 Low */
				prWMVSPS->i4HufNewPCBPCYDec = LowRate;
			...
			if (!u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 1)) {	/* 0 Low */
				prWMVSPS->i4HufNewPCBPCYDec = LowRate;
				AddPicHdrBitCount(u4InstID, 1);
			} else if (!u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 1)) {	/* 10 Mid */
				prWMVSPS->i4HufNewPCBPCYDec = MidRate;
				AddPicHdrBitCount(u4InstID, 2);
			} else {	/* 11 High */
				prWMVSPS->i4HufNewPCBPCYDec = HighRate;
	...
	if (prWMVEPS->i4DQuantCodingOn == 2) {
		...
		if (prWMVPPS->fgDQuantOn) {
			...
			if (prWMVPPS->i4Panning || prWMVPPS->fgDQuantBiLevel) {
				bPicQtDiff =
				    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 3);
				AddPicHdrBitCount(u4InstID, 3);
				if (bPicQtDiff == 7) {
					bAltPicQt =
					    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID],
									    u4InstID, 5);
					AddPicHdrBitCount(u4InstID, 5);
				} else {
...
INT32 iGetPMvMode(UINT32 u4InstID, INT32 iPQuant, BOOL fgRepeat)
{
	...
	if (iPQuant > 12) {	/* P Picture Low Rate */
...
UINT32 WMVideoDecDecodeFrameHead(UINT32 u4InstID)
{
	...
	if (prWMVSPS->fgSeqFrameInterpolation) {
		/*fgInterpolateCurrentFrame = */ u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID],
										 u4InstID, 1);
		AddPicHdrBitCount(u4InstID, 1);
	}
	/*iFrmCntMod4 = */
	...
	if ((prWMVPPS->ucPicType == BVOP) || (prWMVPPS->ucPicType == BIVOP)) {
		...
		if (iShort == 0x7) {
			...
			if (iLong == 0xf) {
				...
				prWMVPPS->i4BFrameReciprocal =
				    _iBInverse[prWMVPPS->i4BDenominator - 1];
				if (prWMVSPS->i4NumBFrames == 0) {
	...
	prWMVPPS->prDQuantParam =
	    prWMVPPS->fgUse3QPDZQuantizer ? prWMVPPS->rDQuantParam3QPDeadzone : prWMVPPS->
	    rDQuantParam5QPDeadzone;

	prWMVPPS->i4Overlap = 0;
	if (prWMVEPS->fgSequenceOverlap && (prWMVPPS->ucPicType != BVOP)) {
		if (prWMVPPS->i4StepSize >= 9)
			prWMVPPS->i4Overlap = 1;
		else if ((prWMVSPS->i4WMV3Profile == WMV3_ADVANCED_PROFILE)
			 && ((prWMVPPS->ucPicType == IVOP) || (prWMVPPS->ucPicType == BIVOP)))
	...
	{
		prWMVSPS->u4PicWidthDec =
		    prWMVSPS->rMultiResParams[prWMVSPS->i4ResIndex].i4WidthDec;
		prWMVSPS->u4PicHeightDec =
		    prWMVSPS->rMultiResParams[prWMVSPS->i4ResIndex].iHeightDec;
		...
		prWMVSPS->u4PicWidthCmp =
		    prWMVSPS->rMultiResParams[prWMVSPS->i4ResIndex].i4FrmWidthSrc;
		prWMVSPS->u4PicHeightCmp =
		    prWMVSPS->rMultiResParams[prWMVSPS->i4ResIndex].i4FrmHeightSrc;
	...
	if ((prWMVPPS->ucPicType == IVOP) || (prWMVPPS->ucPicType == BIVOP)) {
		...
		prWMVPPS->u4DCTACInterTableIndx =
		    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 1);
		AddPicHdrBitCount(u4InstID, 1);
		...
		prWMVPPS->u4DCTACIntraTableIndx =
		    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 1);
		AddPicHdrBitCount(u4InstID, 1);
		...
		if ((prWMVPPS->ucPicType == BVOP) && (!prWMVPPS->fgFieldMode)) {
			...
			if (prWMVPPS->i4X9MVMode == INTENSITY_COMPENSATION) {
				...
				prWMVPPS->i4X9MVMode =
				    iGetPMvMode(u4InstID, prWMVPPS->i4StepSize, TRUE);
				prWMVPPS->i4LumScale =
				    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 6);
				prWMVPPS->i4LumShift =
				    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 6);
				AddPicHdrBitCount(u4InstID, 12);
			}
...
UINT32 WMV78DecodePicture(UINT32 u4InstID)
{
	...
	if (prWMVPPS->ucPicType == IVOP) {
		...
		{
			prWMVSPS->i4SliceCode =
			    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID,
							    NUMBITS_SLICE_SIZE);
		...
		{
			prWMVSPS->fgDCTTableMBEnabled =
			    (prWMVSPS->i4BitRate > MIN_BITRATE_MB_TABLE);
			prWMVPPS->fgDCPredIMBInPFrame =
			    ((prWMVSPS->i4BitRate <= MAX_BITRATE_DCPred_IMBInPFrame)
...
void vWMVSearchSliceStartCode(UINT32 u4InstID)
{
	...
	while (1) {
		...
		} else if (((UINT32) pVideoData) > /*=*/ ((_tInFileInfo[u4InstID].u4FileLength + 64 /*<< 3 */) + ((UINT32) _pucVFifo[u4InstID])))	/* tmp, enlarge 64 bytes for judgment */
		{
			printk
			    ("[WMV] SearchSliceStartCode to stream end, len:0x%x, Pic count to [%d]\n",
...
BOOL fgVParserProcWMV(UINT32 u4InstID)
{
	...
	if (_i4CodecVersion[u4InstID] != VDEC_VC1) {
		...
		if (_i4RcvVersion[u4InstID] == 0) {
			...
			/*u4TimeStamp = */
			iRCVRead4Bytes(u4InstID);	/* u4TimeStamp */
			_i4HeaderLen[u4InstID] += 4;
		} else {
		...
		if ((prWMVPPS->ucFrameCodingMode != INTERLACEFIELD)
		    || (prWMVPPS->i4CurrentTemporalField == 0)) {
			switch (prWMVPPS->ucPicType) {
			case IVOP:
			case PVOP:
				vVPrsIPProc(u4InstID);
...
BOOL fgWMVNextStartCode(UINT32 u4InstID)
{
	...
	vVDEC_HAL_WMV_AlignRdPtr(_u4BSID[u4InstID], u4InstID, (UINT32) _pucVFifo[u4InstID],
				 BYTE_ALIGN);
...
UINT32 u4VParserWMVA(UINT32 u4InstID)
{
	...
	} while ((u4Datain != WMV_SC_FRAME) && (u4Datain != WMV_SC_FIELD)
		 && (u4Datain != WMV_SC_ENDOFSEQ));
	...
	if (_i4CodecVersion[u4InstID] != VDEC_VC1) {	/* simple or main profile */
		...
		if (prWMVPPS->ucFrameCodingMode == PROGRESSIVE) {
			...
			prWMVSPS->u4PicHeightDec =
			    prWMVSPS->rMultiResParams[PROGRESSIVE].iHeightDec;
			prWMVSPS->u4NumMBX = prWMVSPS->u4PicWidthDec >> 4;
			prWMVSPS->u4NumMBY = prWMVSPS->u4PicHeightDec >> 4;
			prWMVSPS->u4PicWidthCmp =
			    prWMVSPS->rMultiResParams[PROGRESSIVE].i4FrmWidthSrc;
			prWMVSPS->u4PicHeightCmp =
			    prWMVSPS->rMultiResParams[PROGRESSIVE].i4FrmHeightSrc;
			...
			prWMVSPS->u4PicWidthDec =
			    prWMVSPS->rMultiResParams[INTERLACEFRAME].i4WidthDec;
			prWMVSPS->u4PicHeightDec =
			    prWMVSPS->rMultiResParams[INTERLACEFRAME].iHeightDec;
			prWMVSPS->u4NumMBX = prWMVSPS->u4PicWidthDec >> 4;
			prWMVSPS->u4NumMBY = prWMVSPS->u4PicHeightDec >> 4;
			prWMVSPS->u4PicWidthCmp =
			    prWMVSPS->rMultiResParams[INTERLACEFRAME].i4FrmWidthSrc;
			prWMVSPS->u4PicHeightCmp =
			    prWMVSPS->rMultiResParams[INTERLACEFRAME].i4FrmHeightSrc;
 ...
 INTERLACE_FIELD :
			...
			prWMVSPS->u4PicWidthDec =
			    prWMVSPS->rMultiResParams[INTERLACEFIELD].i4WidthDec;
			prWMVSPS->u4PicHeightDec =
			    prWMVSPS->rMultiResParams[INTERLACEFIELD].iHeightDec;
			prWMVSPS->u4NumMBX = prWMVSPS->u4PicWidthDec >> 4;
			prWMVSPS->u4NumMBY = prWMVSPS->u4PicHeightDec >> 4;
			prWMVSPS->u4PicWidthCmp =
			    prWMVSPS->rMultiResParams[INTERLACEFIELD].i4FrmWidthSrc;
			prWMVSPS->u4PicHeightCmp =
			    prWMVSPS->rMultiResParams[INTERLACEFIELD].i4FrmHeightSrc;
			...
			if (prWMVPPS->i4CurrentTemporalField == 0) {	/* 1st field */
				...
				printk("@@@@@ prWMVPPS->u4SlicePicHeaderNumField bit cnt %d",
				       prWMVPPS->u4SlicePicHeaderNumField);
				...
				if (prWMVPPS->ucPicType == PVOP) {
					...
					prWMVPPS->iForwardRefDistance =
					    prWMVEPS->i4RefFrameDistance;
#endif
					SetupFieldPictureMVScaling(u4InstID,
								   prWMVEPS->i4RefFrameDistance);
					decodePFieldMode(u4InstID);
				} else if (prWMVPPS->ucPicType == BVOP) {
					INT32 iForwardRefDistance =
					    (prWMVPPS->i4BNumerator * prWMVEPS->i4RefFrameDistance *
					     prWMVPPS->i4BFrameReciprocal) >> 8;
					INT32 iBackwardRefDistance =
					    prWMVEPS->i4RefFrameDistance - iForwardRefDistance - 1;
					...
					SetupBackwardBFieldPictureMVScaling(u4InstID,
									    iBackwardRefDistance);
...
UINT32 decodeSequenceHead_Advanced(UINT32 u4InstID)
{
	...
	if (fgDisplay_Ext) {
		...
		if (fgAspect_Ratio_Flag) {
			prWMVSPS->i4AspectRatio =
			    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 4);
			AddPicHdrBitCount(u4InstID, 4);
...
void vResetConditionalVariablesForSequenceSwitch(UINT32 u4InstID)
{
	...
	_rWMVEPS[u4InstID].fgExplicitSeqQuantizer = prWMVEPS->fgExplicitFrameQuantizer =
	    prWMVPPS->fgUse3QPDZQuantizer = FALSE;
...
UINT32 DecodeEntryPointHeader(UINT32 u4InstID)
{
	...
	prWMVEPS->fgExplicitQuantizer = prWMVEPS->fgExplicitSeqQuantizer
	    || prWMVEPS->fgExplicitFrameQuantizer;
	prWMVPPS->i4BFrameReciprocal = _iBInverse[prWMVSPS->i4NumBFrames];
	prWMVPPS->ucPicType = IVOP;
	...
	if (prWMVEPS->fgExtendedMvMode) {
		prWMVEPS->fgExtendedDeltaMvMode =
		    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 1);
		AddPicHdrBitCount(u4InstID, 1);
	}
	...
	if (prWMVEPS->fgRangeRedYFlag) {
		prWMVEPS->i4RangeRedY =
		    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 3) + 1;
		AddPicHdrBitCount(u4InstID, 3);
	} else {
		prWMVEPS->i4RangeRedY = 0;
	}

	prWMVEPS->fgRangeRedUVFlag =
	    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 1);
	AddPicHdrBitCount(u4InstID, 1);
	if (prWMVEPS->fgRangeRedUVFlag) {
		prWMVEPS->i4RangeMapUV =
		    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 3) + 1;
		AddPicHdrBitCount(u4InstID, 3);
	} else {
...
UINT32 decodeVOPHeadProgressiveWMVA(UINT32 u4InstID)
{
	...
	if (prWMVEPS->fgPanScanPresent) {
		...
		if (fgPanScanPresent) {
			...
			for (i = 0; i < u4NumberOfPanScanWindows; i++) {
				prWMVPPS->rPanScanWindowInfo[i].u4PanScanHorizOffset =
				    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID,
								    18);
				prWMVPPS->rPanScanWindowInfo[i].u4PanScanVertOffset =
				    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID,
								    18);
				prWMVPPS->rPanScanWindowInfo[i].u4PanScanWidth =
				    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID,
								    14);
				prWMVPPS->rPanScanWindowInfo[i].u4PanScanHeight =
				    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID,
								    14);
	...
	if (prWMVSPS->fgInterlacedSource) {
		/*fgUVProgressiveSubsampling = */ u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID],
										  u4InstID, 1);
		AddPicHdrBitCount(u4InstID, 1);
	}
	if (prWMVSPS->fgSeqFrameInterpolation) {
...
UINT32 decodeVOPHeadFieldPicture(UINT32 u4InstID)
{
	...
	if (prWMVEPS->fgRefDistPresent
	    && ((prWMVPPS->ucFirstFieldType == IVOP) || (prWMVPPS->ucFirstFieldType == PVOP))) {
		prWMVEPS->i4RefFrameDistance =
		    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 2);
		AddPicHdrBitCount(u4InstID, 2);
...
UINT32 decodeFieldHeadFieldPicture(UINT32 u4InstID)
{
	...
	if ((prWMVPPS->ucPicType == IVOP) || (prWMVPPS->ucPicType == BIVOP)) {
		...
		if (prWMVEPS->fgExtendedDeltaMvMode) {
			...
			prWMVEPS->i4ExtendedDMVY =
			    (_rWMVEPS[u4InstID].i4DeltaMVRangeIndex & 2) >> 1;
		}
		...
		if (prWMVPPS->i4X9MVMode == INTENSITY_COMPENSATION)	/* PVOP only. */
		{
			...
			if (u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 1)) {
				...
				prWMVPPS->fgLuminanceWarpTop = prWMVPPS->fgLuminanceWarpBottom =
				    TRUE;
				prWMVPPS->i4LumScaleTop =
				    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 6);
				prWMVPPS->i4LumShiftTop =
				    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 6);
				prWMVPPS->i4LumScaleBottom =
				    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 6);
				prWMVPPS->i4LumShiftBottom =
				    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 6);
				AddPicHdrBitCount(u4InstID, 24);
			} else if (u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 1)) {
				AddPicHdrBitCount(u4InstID, 2);
		...
		prWMVPPS->i4MBModeTable =
		    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 3);
		AddPicHdrBitCount(u4InstID, 3);
		prWMVPPS->i4MvTable =
		    (prWMVPPS->
		     fgTwoRefPictures) ? u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID],
									 u4InstID,
									 3) :
		    u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 2);
		if (prWMVPPS->fgTwoRefPictures) {
...
INT32 iGetIFMvMode(UINT32 u4InstID, INT32 iPQuant, BOOL fgRepeat)	/* for Interlace Field */
{
	...
	if (iPQuant > 12) {	/* P/B Field Picture Low Rate */
		if (u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 1)) {
			...
			if (prWMVPPS->ucPicType == BVOP) {
				...
				if (fgRepeat
				    ||
				    (!u4VDEC_HAL_WMV_GetRealBitStream
				     (_u4BSID[u4InstID], u4InstID, 1))) {
	...
	} else {		/* P/B Field Picture High rate */
		if (u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 1)) {
			iMvMode = ALL_1MV;	/* 1b */
			AddPicHdrBitCount(u4InstID, 1);
		} else if (u4VDEC_HAL_WMV_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 1)) {
...
void SetupFieldPictureMVScaling(UINT32 u4InstID, INT32 i4RefFrameDistance)
{
	...
	if (prWMVPPS->i4CurrentTemporalField == 0) {
		prWMVPPS->i4MaxZone1ScaledFarMVX =
		    s_sMVScaleValuesFirstField_verify[i4RefFrameDistance].i4MaxZone1ScaledFarMVX;
		prWMVPPS->i4MaxZone1ScaledFarMVY =
		    s_sMVScaleValuesFirstField_verify[i4RefFrameDistance].i4MaxZone1ScaledFarMVY;
		prWMVPPS->i4Zone1OffsetScaledFarMVX =
		    s_sMVScaleValuesFirstField_verify[i4RefFrameDistance].i4Zone1OffsetScaledFarMVX;
		prWMVPPS->i4Zone1OffsetScaledFarMVY =
		    s_sMVScaleValuesFirstField_verify[i4RefFrameDistance].i4Zone1OffsetScaledFarMVY;
		prWMVPPS->i4FarFieldScale1 =
		    s_sMVScaleValuesFirstField_verify[i4RefFrameDistance].i4FarFieldScale1;
		prWMVPPS->i4FarFieldScale2 =
		    s_sMVScaleValuesFirstField_verify[i4RefFrameDistance].i4FarFieldScale2;
		prWMVPPS->i4NearFieldScale =
		    s_sMVScaleValuesFirstField_verify[i4RefFrameDistance].i4NearFieldScale;
	} else {
		prWMVPPS->i4MaxZone1ScaledFarMVX =
		    s_sMVScaleValuesSecondField_verify[i4RefFrameDistance].i4MaxZone1ScaledFarMVX;
		prWMVPPS->i4MaxZone1ScaledFarMVY =
		    s_sMVScaleValuesSecondField_verify[i4RefFrameDistance].i4MaxZone1ScaledFarMVY;
		prWMVPPS->i4Zone1OffsetScaledFarMVX =
		    s_sMVScaleValuesSecondField_verify[i4RefFrameDistance].
		    i4Zone1OffsetScaledFarMVX;
		prWMVPPS->i4Zone1OffsetScaledFarMVY =
		    s_sMVScaleValuesSecondField_verify[i4RefFrameDistance].
		    i4Zone1OffsetScaledFarMVY;
		prWMVPPS->i4FarFieldScale1 =
		    s_sMVScaleValuesSecondField_verify[i4RefFrameDistance].i4FarFieldScale1;
		prWMVPPS->i4FarFieldScale2 =
		    s_sMVScaleValuesSecondField_verify[i4RefFrameDistance].i4FarFieldScale2;
		prWMVPPS->i4NearFieldScale =
		    s_sMVScaleValuesSecondField_verify[i4RefFrameDistance].i4NearFieldScale;
	}
}
...
void SetupBackwardBFieldPictureMVScaling(UINT32 u4InstID, INT32 i4RefFrameDistance)
{
	...
	if (prWMVPPS->i4CurrentTemporalField == 0) {
		prWMVPPS->i4MaxZone1ScaledFarBackMVX =
		    s_sMVScaleValuesFirstFieldB_verify[i4RefFrameDistance].i4MaxZone1ScaledFarMVX;
		prWMVPPS->i4MaxZone1ScaledFarBackMVY =
		    s_sMVScaleValuesFirstFieldB_verify[i4RefFrameDistance].i4MaxZone1ScaledFarMVY;
		prWMVPPS->i4Zone1OffsetScaledFarBackMVX =
		    s_sMVScaleValuesFirstFieldB_verify[i4RefFrameDistance].
		    i4Zone1OffsetScaledFarMVX;
		prWMVPPS->i4Zone1OffsetScaledFarBackMVY =
		    s_sMVScaleValuesFirstFieldB_verify[i4RefFrameDistance].
		    i4Zone1OffsetScaledFarMVY;
		prWMVPPS->i4FarFieldScaleBack1 =
		    s_sMVScaleValuesFirstFieldB_verify[i4RefFrameDistance].i4FarFieldScale1;
		prWMVPPS->i4FarFieldScaleBack2 =
		    s_sMVScaleValuesFirstFieldB_verify[i4RefFrameDistance].i4FarFieldScale2;
		prWMVPPS->i4NearFieldScaleBack =
		    s_sMVScaleValuesFirstFieldB_verify[i4RefFrameDistance].i4NearFieldScale;
	} else {
		prWMVPPS->i4MaxZone1ScaledFarBackMVX =
		    s_sMVScaleValuesSecondFieldB_verify[i4RefFrameDistance].i4MaxZone1ScaledFarMVX;
		prWMVPPS->i4MaxZone1ScaledFarBackMVY =
		    s_sMVScaleValuesSecondFieldB_verify[i4RefFrameDistance].i4MaxZone1ScaledFarMVY;
		prWMVPPS->i4Zone1OffsetScaledFarBackMVX =
		    s_sMVScaleValuesSecondFieldB_verify[i4RefFrameDistance].
		    i4Zone1OffsetScaledFarMVX;
		prWMVPPS->i4Zone1OffsetScaledFarBackMVY =
		    s_sMVScaleValuesSecondFieldB_verify[i4RefFrameDistance].
		    i4Zone1OffsetScaledFarMVY;
		prWMVPPS->i4FarFieldScaleBack1 =
		    s_sMVScaleValuesSecondFieldB_verify[i4RefFrameDistance].i4FarFieldScale1;
		prWMVPPS->i4FarFieldScaleBack2 =
		    s_sMVScaleValuesSecondFieldB_verify[i4RefFrameDistance].i4FarFieldScale2;
		prWMVPPS->i4NearFieldScaleBack =
		    s_sMVScaleValuesSecondFieldB_verify[i4RefFrameDistance].i4NearFieldScale;
	}
}
...
{
	VDEC_INFO_WMV_PIC_PRM_T *prWMVPPS = &_rWMVPPS[u4InstID];
	BOOL fgHalfPelMode = ((prWMVPPS->i4X9MVMode == ALL_1MV_HALFPEL)
			      || (prWMVPPS->i4X9MVMode == ALL_1MV_HALFPEL_BILINEAR));
...
void UpdateVopheaderParam(UINT32 u4InstID)
{
	...
	if (((prWMVPPS->ucPicType == IVOP) || (prWMVPPS->ucPicType == PVOP) || (prWMVPPS->ucPicType == SKIPFRAME)) && (/* first field picture */
	...
	/*  */
	if (_i4CodecVersion[u4InstID] == VDEC_VC1) {
		if ((prWMVPPS->ucPicType == PVOP) || (prWMVPPS->ucPicType == IVOP)
		    || (prWMVPPS->ucPicType == SKIPFRAME)) {
			if (prWMVPPS->ucPicType == IVOP) {
			...
			if (prWMVPPS->fgFieldMode == TRUE) {	/* field picture */
				if (prWMVPPS->i4CurrentTemporalField == 0) {	/* the first P field picture */
					...
					if (prWMVPPS->i4CurrentField == 0) {	/* current decode field is top field */
						...
						prWMVICOMPPS->OldBotField.Old =
						    prWMVICOMPPS->NewBotField.Old;
						prWMVICOMPPS->OldBotField.New =
						    prWMVICOMPPS->NewBotField.New;
						...
						prWMVICOMPPS->NewTopField.Old =
						    prWMVICOMPPS->NewTopField.New;

						if (/* forward reference picture is frame picture *//* and decode the first field picture now */
							   (prWMVPPS->u4ForwardRefPicType ==
							    PROGRESSIVE)
							   || (prWMVPPS->u4ForwardRefPicType ==
							       INTERLACEFRAME)) {
						...
						prWMVICOMPPS->OldTopField.Old =
						    prWMVICOMPPS->NewTopField.Old;
						prWMVICOMPPS->OldTopField.New =
						    prWMVICOMPPS->NewTopField.New;
					...
					if ((prWMVPPS->u4ForwardRefPicType == PROGRESSIVE)
					    && (prWMVPPS->u4BackwardRefPicType == INTERLACEFIELD)) {
					...
					prWMVICOMPPS->i4SecondFieldParity =
					    prWMVPPS->i4CurrentField;
				...
				if (/* forward reference picture is frame picture */
					   (prWMVPPS->u4ForwardRefPicType == PROGRESSIVE)
					   || (prWMVPPS->u4ForwardRefPicType == INTERLACEFRAME)) {
					prWMVICOMPPS->NewTopField.Old.i4Enable = 0;
					prWMVICOMPPS->NewBotField.Old.i4Enable = 0;
				...
				if (prWMVPPS->fgLuminanceWarp == TRUE) {
					cal_icomp(&iScaleTop, &iShiftTop, prWMVPPS->i4LumScale,
						  prWMVPPS->i4LumShift);
					prWMVICOMPPS->NewTopField.New.i4Shift = iShiftTop;
					prWMVICOMPPS->NewTopField.New.i4Scale = iScaleTop;
					prWMVICOMPPS->NewTopField.New.i4Enable = 1;
				...
				if (prWMVPPS->fgLuminanceWarp == TRUE) {
					cal_icomp(&iScaleBot, &iShiftBot, prWMVPPS->i4LumScale,
						  prWMVPPS->i4LumShift);
					prWMVICOMPPS->NewBotField.New.i4Shift = iShiftBot;
					prWMVICOMPPS->NewBotField.New.i4Scale = iScaleBot;
					prWMVICOMPPS->NewBotField.New.i4Enable = 1;
	...
	if (((prWMVPPS->ucPicType == PVOP) || (prWMVPPS->ucPicType == SKIPFRAME))
				...
				if (prWMVPPS->i4CurrentField == 0) {	/* current field is top field */
					if (prWMVICOMPPS->i4SecondFieldParity == 0) {
						...
						if (prWMVICOMPPS->i4BoundaryUMVIcomp == 1) {
							/* top field picture */
							if (prWMVICOMPPS->i4SecondFieldParity == 0) {
								if (prWMVICOMPPS->OldBotField.New.
								    i4Enable == 1) {
									prWMVICOMPPS->
								...
								if (prWMVICOMPPS->OldTopField.New.
								    i4Enable == 1) {
									prWMVICOMPPS->
									    i4BoundaryUMVIcompEnable
									    = 1;
	...
	/* ----------------------------------------------------- */
	{
		if ((_i4CodecVersion[u4InstID] == VDEC_WMV1)
			...
			if (prWMVICOMPPS->ucFrameTypeLast == SKIPFRAME) {
				if ((prWMVICOMPPS->i4ResetMvDram == 1)
				    && ((prWMVPPS->ucPicType == IVOP)
					|| (prWMVPPS->ucPicType == PVOP))) {
					prWMVICOMPPS->i4ResetMvDram = 0;
				} else {
				...
				if (((prWMVPPS->ucFrameCodingMode == INTERLACEFIELD)
				     &&
				     (((prWMVPPS->i4CurrentField == 0)
				       && (prWMVICOMPPS->ucFrameTypeLastTop == IVOP))
				      || ((prWMVPPS->i4CurrentField == 1)
					  && (prWMVICOMPPS->ucFrameTypeLastBot == IVOP))))
				    || ((prWMVPPS->ucFrameCodingMode != INTERLACEFIELD)
					&& (prWMVICOMPPS->ucFrameTypeLast == IVOP))) {
					if (prWMVPPS->ucPicType == PVOP) {
