...
void vInsertRefPicList(UINT32 u4InstID, VDEC_INFO_H264_REF_PIC_LIST_T *ptRefPicList,
		       INT32 iCurrPOC, UINT32 u4RefPicListInfo);
...
void vAssignQuantParam(UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *ptVerMpvDecPrm)
{
	...
	if ((!ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prPPS->fgPicScalingMatrixPresentFlag) &&
		...
		if (ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->fgSeqScalingMatrixPresentFlag) {	/* check sps first */
			for (i = 0; i < 8; i++) {
				ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.
				    fgUserScalingListPresentFlag[i] = TRUE;
				if (i < 6) {
					if (!ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->fgSeqScalingListPresentFlag[i]) {	/* fall-back rule A */
						if ((i == 0) || (i == 3)) {
							ptList[i] =
							    (i ==
							     0) ? quant_intra_default :
							    quant_inter_default;
							vVDEC_HAL_H264_WriteScalingList(u4InstID, i,
											ptList[i]);
						} else {
							ptList[i] = ptList[i - 1];
							ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.
							    fgUserScalingListPresentFlag[i] =
							    ptVerMpvDecPrm->SpecDecPrm.
							    rVDecH264DecPrm.
							    fgUserScalingListPresentFlag[i - 1];
							/* if(ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.fgUserScalingListPresentFlag[i]) */
							{
								vVDEC_HAL_H264_WriteScalingList
								    (u4InstID, i, ptList[i]);
							}
						}
					} else {	/* fall-back rule A */
						if (ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.
						    prSPS->fgUseDefaultScalingMatrix4x4Flag[i]) {
							ptList[i] =
							    (i <
							     3) ? quant_intra_default :
							    quant_inter_default;
							vVDEC_HAL_H264_WriteScalingList(u4InstID, i,
											ptList[i]);
						} else {
					...
					if (!ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->fgSeqScalingListPresentFlag[i] || ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->fgUseDefaultScalingMatrix8x8Flag[i - 6]) {	/* fall-back rule A */
						ptList[i] =
						    (i ==
						     6) ? quant8_intra_default :
			...
			for (i = 0; i < 8; i++) {
				if (i < 6) {
					if (!ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prPPS->fgPicScalingListPresentFlag[i]) {	/* fall-back rule A */
						if ((i == 0) || (i == 3)) {
							if (!ptVerMpvDecPrm->SpecDecPrm.
							    rVDecH264DecPrm.prSPS->
							    fgSeqScalingMatrixPresentFlag) {
								ptList[i] =
								    (i ==
								     0) ? quant_intra_default :
								    quant_inter_default;
						...
						if (ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.
						    prPPS->fgUseDefaultScalingMatrix4x4Flag[i]) {
							/* printk("\n"); */
							ptList[i] =
							    (i <
							     3) ? quant_intra_default :
					...
					} else if (ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.
						   prPPS->fgUseDefaultScalingMatrix8x8Flag[i - 6]) {
						ptList[i] =
						    (i ==
						     6) ? quant8_intra_default :
						    quant8_inter_default;
...
void vVerifyVDecSetPicInfo(UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *ptVerMpvDecPrm)
{
	...
	vVDEC_HAL_H264_SetPPSAVLD(u4InstID,
				  _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.
				  fgUserScalingMatrixPresentFlag,
				  &(_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.
				    fgUserScalingListPresentFlag[0]),
				  ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prPPS);
	vVDEC_HAL_H264_SetSHDRAVLD1(u4InstID,
				    ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr);
}
...
{
	tVerMpvDecPrm->u4PicW =
	    (tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->u4PicWidthInMbsMinus1 + 1) << 4;
	...
	if ((tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rLastInfo.u4LastPicW !=
	     tVerMpvDecPrm->u4PicW)
	    || (tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rLastInfo.u4LastPicH !=
		tVerMpvDecPrm->u4PicH)) {
	...
	if (tVerMpvDecPrm->ucPicStruct & TOP_FIELD) {
		_ptCurrFBufInfo[u4InstID]->u4TFldPara =
		    ((fgIsFrmPic(u4InstID) ? 0 : 1) << 19) +
		    ((fgIsFrmPic(u4InstID)
		      && tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->
		      fgMbAdaptiveFrameFieldFlag) << 18);
	}
	if (tVerMpvDecPrm->ucPicStruct & BOTTOM_FIELD) {
		_ptCurrFBufInfo[u4InstID]->u4BFldPara =
		    ((fgIsFrmPic(u4InstID) ? 0 : 1) << 19) +
		    ((fgIsFrmPic(u4InstID)
		      && tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->
		      fgMbAdaptiveFrameFieldFlag) << 18);
...
void vVDecSetPRefPicList(UINT32 u4InstID)
{
	...
	_ptCurrFBufInfo[u4InstID]->u4FrameNum =
	    _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.prSliceHdr->u4FrameNum;
	u4CurrPicNum =
	    (fgIsFrmPic(u4InstID)) ? _ptCurrFBufInfo[u4InstID]->
	    u4FrameNum : ((_ptCurrFBufInfo[u4InstID]->u4FrameNum << 1) + 1);
	...
	{
		for (i = 0; i < _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.ucMaxFBufNum;
		     i++) {
			...
			if (fgChkRefInfo(u4InstID, i, SREF_PIC)) {
				if (_ptFBufInfo[u4InstID][i].u4FrameNum >
				    _ptCurrFBufInfo[u4InstID]->u4FrameNum) {
					_ptFBufInfo[u4InstID][i].i4FrameNumWrap =
					    _ptFBufInfo[u4InstID][i].u4FrameNum -
					    _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.
					    prSPS->u4MaxFrameNum;
				} else {
					_ptFBufInfo[u4InstID][i].i4FrameNumWrap =
				...
				if (fgIsFrmPic(u4InstID)) {
					_ptFBufInfo[u4InstID][i].i4PicNum =
					    _ptFBufInfo[u4InstID][i].i4FrameNumWrap;
				} else {
					_ptFBufInfo[u4InstID][i].i4PicNum =
					    (_ptFBufInfo[u4InstID][i].i4FrameNumWrap << 1) + 1;
				}

				if (_ptFBufInfo[u4InstID][i].ucTFldRefType == SREF_PIC) {
					_ptFBufInfo[u4InstID][i].i4TFldPicNum =
					    (_ptFBufInfo[u4InstID][i].i4FrameNumWrap << 1) +
					    u4AddTop;
					vInsertRefPicList(u4InstID, &_ptRefPicList[u4InstID][0], i,
							  SREF_PIC + (0 << 8) + (i << 16));
				}
				if (_ptFBufInfo[u4InstID][i].ucBFldRefType == SREF_PIC) {
					_ptFBufInfo[u4InstID][i].i4BFldPicNum =
					    (_ptFBufInfo[u4InstID][i].i4FrameNumWrap << 1) +
					    u4AddBot;
					vInsertRefPicList(u4InstID, &_ptRefPicList[u4InstID][1], i,
							  SREF_PIC + (1 << 8) + (i << 16));
				}
			} else if (fgChkRefInfo(u4InstID, i, LREF_PIC)) {
				_ptFBufInfo[u4InstID][i].i4LongTermPicNum =
				    _ptFBufInfo[u4InstID][i].u4LongTermFrameIdx;

				if (_ptFBufInfo[u4InstID][i].ucTFldRefType == LREF_PIC) {
					u4Idx = _ptFBufInfo[u4InstID][i].u4TFldLongTermFrameIdx;
					_ptFBufInfo[u4InstID][i].i4TFldLongTermPicNum =
					    (u4Idx << 1) + u4AddTop;
					vInsertRefPicList(u4InstID, &_ptRefPicList[u4InstID][4], i,
							  LREF_PIC + (6 << 8) + (i << 16));
				}
				if (_ptFBufInfo[u4InstID][i].ucBFldRefType == LREF_PIC) {
					_ptFBufInfo[u4InstID][i].i4BFldLongTermPicNum =
					    (_ptFBufInfo[u4InstID][i].u4BFldLongTermFrameIdx << 1) +
					    u4AddBot;
					vInsertRefPicList(u4InstID, &_ptRefPicList[u4InstID][5], i,
							  LREF_PIC + (7 << 8) + (i << 16));
				}
			}
		}
	...
	vVDEC_HAL_H264_InitPRefList(u4InstID, fgIsFrmPic(u4InstID),
				    _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.prSPS->
				    u4MaxFrameNum, u4CurrPicNum);
...
/* Return      : None */
/* ********************************************************************* */
void vSetupPRefPicList(UINT32 u4InstID, UINT32 *pu4RefIdx, UINT32 u4TFldListIdx,
		       UINT32 u4BFldListIdx)
{
	INT32 i;
	UINT32 u4TotalFBuf;
	...
	u4DpbBaseOffset = (_ucMVCType[u4InstID] == 2) ?
	    _tVerMpvDecPrm[0].SpecDecPrm.rVDecH264DecPrm.ucMaxFBufNum << 1 : 0;
#endif
	/* VDEC_INFO_H264_P_REF_PRM_T rPRefPicListInfo; */


	u4TotalFBuf =
	    (_ptRefPicList[u4InstID][u4TFldListIdx].u4RefPicCnt >=
	     _ptRefPicList[u4InstID][u4BFldListIdx].
	     u4RefPicCnt) ? _ptRefPicList[u4InstID][u4TFldListIdx].
	    u4RefPicCnt : _ptRefPicList[u4InstID][u4BFldListIdx].u4RefPicCnt;
	if (fgIsFrmPic(u4InstID)) {
		u4TFldListIdx =
		    (_ptRefPicList[u4InstID][u4TFldListIdx].u4RefPicCnt >=
		     _ptRefPicList[u4InstID][u4BFldListIdx].
		     u4RefPicCnt) ? u4TFldListIdx : u4BFldListIdx;
	}
	for (i = 0; i < u4TotalFBuf; i++) {
		if (fgIsFrmPic(u4InstID)) {
			prPRefPicListInfo->ucFBufIdx =
			    _ptRefPicList[u4InstID][u4TFldListIdx].u4FBufIdx[i];
			prPRefPicListInfo->i4BFldLongTermPicNum =
			    _ptFBufInfo[u4InstID][prPRefPicListInfo->ucFBufIdx].
			    i4BFldLongTermPicNum;
			prPRefPicListInfo->i4BFldPicNum =
			    _ptFBufInfo[u4InstID][prPRefPicListInfo->ucFBufIdx].i4BFldPicNum;
			prPRefPicListInfo->i4BFldPOC =
			    _ptFBufInfo[u4InstID][prPRefPicListInfo->ucFBufIdx].i4BFldPOC;
			prPRefPicListInfo->i4PicNum =
			    _ptFBufInfo[u4InstID][prPRefPicListInfo->ucFBufIdx].i4PicNum;
			prPRefPicListInfo->i4LongTermPicNum =
			    _ptFBufInfo[u4InstID][prPRefPicListInfo->ucFBufIdx].i4LongTermPicNum;
			prPRefPicListInfo->i4TFldLongTermPicNum =
			    _ptFBufInfo[u4InstID][prPRefPicListInfo->ucFBufIdx].
			    i4TFldLongTermPicNum;
			prPRefPicListInfo->i4TFldPicNum =
			    _ptFBufInfo[u4InstID][prPRefPicListInfo->ucFBufIdx].i4TFldPicNum;
			prPRefPicListInfo->i4TFldPOC =
			    _ptFBufInfo[u4InstID][prPRefPicListInfo->ucFBufIdx].i4TFldPOC;
			prPRefPicListInfo->u4BFldPara =
			    _ptFBufInfo[u4InstID][prPRefPicListInfo->ucFBufIdx].u4BFldPara;
			prPRefPicListInfo->u4FBufYStartAddr =
			    _ptFBufInfo[u4InstID][prPRefPicListInfo->ucFBufIdx].u4Addr;
			prPRefPicListInfo->u4FBufInfo = FRAME + (i << 8) + (pu4RefIdx[0] << 16);
			prPRefPicListInfo->u4ListIdx = u4TFldListIdx;
			prPRefPicListInfo->u4TFldPara =
			    _ptFBufInfo[u4InstID][prPRefPicListInfo->ucFBufIdx].u4TFldPara;
			...
			if (i < _ptRefPicList[u4InstID][u4TFldListIdx].u4RefPicCnt) {
				...
				prPRefPicListInfo->u4FBufYStartAddr =
				    _ptFBufInfo[u4InstID][prPRefPicListInfo->ucFBufIdx].u4Addr;
				prPRefPicListInfo->u4FBufInfo =
				    TOP_FIELD + (i << 8) + (pu4RefIdx[0] << 16);
				prPRefPicListInfo->u4ListIdx = u4TFldListIdx;
				prPRefPicListInfo->u4TFldPara =
				    _ptFBufInfo[u4InstID][prPRefPicListInfo->ucFBufIdx].u4TFldPara;
				prPRefPicListInfo->ucFBufIdx += u4DpbBaseOffset;
			...
			if (i < _ptRefPicList[u4InstID][u4BFldListIdx].u4RefPicCnt) {
				prPRefPicListInfo->ucFBufIdx =
				    _ptRefPicList[u4InstID][u4BFldListIdx].u4FBufIdx[i];
				prPRefPicListInfo->i4BFldLongTermPicNum =
				    _ptFBufInfo[u4InstID][prPRefPicListInfo->ucFBufIdx].
				    i4BFldLongTermPicNum;
				prPRefPicListInfo->i4BFldPicNum =
				...
				prPRefPicListInfo->u4FBufYStartAddr =
				    _ptFBufInfo[u4InstID][prPRefPicListInfo->ucFBufIdx].u4Addr;
				prPRefPicListInfo->u4FBufInfo =
				    BOTTOM_FIELD + (i << 8) + (pu4RefIdx[0] << 16);
...
/* else */
			{
				if ((_tVerMpvDecPrm[u4InstID].ucPicStruct == FRAME)) {
					prPRefPicListInfo->ucFBufIdx =
					    _ptRefPicList[u4InstID][u4TFldListIdx].u4FBufIdx[0];
					prPRefPicListInfo->i4BFldLongTermPicNum =
					    _ptFBufInfo[u4InstID][prPRefPicListInfo->ucFBufIdx].
					    i4BFldLongTermPicNum;
					prPRefPicListInfo->i4BFldPicNum =
					...
					prPRefPicListInfo->u4FBufYStartAddr =
					    _ptFBufInfo[u4InstID][prPRefPicListInfo->ucFBufIdx].
					    u4YStartAddr;
					prPRefPicListInfo->u4FBufCAddrOffset =
					    _ptFBufInfo[u4InstID][prPRefPicListInfo->ucFBufIdx].
					    u4CAddrOffset;
					prPRefPicListInfo->u4FBufMvStartAddr =
					    _ptFBufInfo[u4InstID][prPRefPicListInfo->ucFBufIdx].
					    u4MvStartAddr;
					...
					prPRefPicListInfo->u4TFldPara =
					    ...
					    u4TFldPara;
					prPRefPicListInfo->ucFBufIdx += u4DpbBaseOffset;
					vVDEC_HAL_H264_SetPRefPicListReg(u4InstID,
									 prPRefPicListInfo);
					/* pu4RefIdx[0] ++; */
				} else if (_tVerMpvDecPrm[u4InstID].ucPicStruct == TOP_FIELD) {
					...
					{
						prPRefPicListInfo->ucFBufIdx =
						    _ptRefPicList[u4InstID][u4TFldListIdx].
						    u4FBufIdx[0];
						prPRefPicListInfo->i4BFldLongTermPicNum =
						    _ptFBufInfo[u4InstID][prPRefPicListInfo->
									  ucFBufIdx].
						    i4BFldLongTermPicNum;
						prPRefPicListInfo->i4BFldPicNum =
						    _ptFBufInfo[u4InstID][prPRefPicListInfo->
									  ucFBufIdx].i4BFldPicNum;
						prPRefPicListInfo->i4BFldPOC =
						    _ptFBufInfo[u4InstID][prPRefPicListInfo->
									  ucFBufIdx].i4BFldPOC;
						prPRefPicListInfo->i4PicNum =
						...
						prPRefPicListInfo->i4TFldPicNum =
						    _ptFBufInfo[u4InstID][prPRefPicListInfo->
									  ucFBufIdx].i4TFldPicNum;
						prPRefPicListInfo->i4TFldPOC =
						    _ptFBufInfo[u4InstID][prPRefPicListInfo->
									  ucFBufIdx].i4TFldPOC;
						prPRefPicListInfo->u4BFldPara =
						    _ptFBufInfo[u4InstID][prPRefPicListInfo->
									  ucFBufIdx].u4BFldPara;
						prPRefPicListInfo->u4FBufYStartAddr =
						    _ptFBufInfo[u4InstID][prPRefPicListInfo->
									  ucFBufIdx].u4YStartAddr;
						prPRefPicListInfo->u4FBufCAddrOffset =
						    _ptFBufInfo[u4InstID][prPRefPicListInfo->
									  ucFBufIdx].u4CAddrOffset;
						prPRefPicListInfo->u4FBufMvStartAddr =
						    _ptFBufInfo[u4InstID][prPRefPicListInfo->
									  ucFBufIdx].u4MvStartAddr;
						prPRefPicListInfo->u4FBufInfo =
						    TOP_FIELD + (0 << 16);
						prPRefPicListInfo->u4ListIdx = u4TFldListIdx;
						prPRefPicListInfo->u4TFldPara =
						    _ptFBufInfo[u4InstID][prPRefPicListInfo->
									  ucFBufIdx].u4TFldPara;
						prPRefPicListInfo->ucFBufIdx += u4DpbBaseOffset;
						vVDEC_HAL_H264_SetPRefPicListReg(u4InstID,
										 prPRefPicListInfo);
						/* pu4RefIdx[0] ++; */
	...
	if (fgIsFrmPic(u4InstID)) {
		...
		iCurrPOC =
		    (_tVerMpvDecPrm[u4InstID].ucPicStruct ==
		     TOP_FIELD) ? _ptCurrFBufInfo[u4InstID]->i4TFldPOC : _ptCurrFBufInfo[u4InstID]->
		    i4BFldPOC;
	}
...
/* && !((_tVerMpvDecPrm.SpecDecPrm.rVDecH264DecPrm.prSPS->u4PicOrderCntType == 0) && (_ptFBufInfo[i].fgNonExisting))) */
				{
					/* B0 */
					vInsertRefPicList(u4InstID, &_ptRefPicList[u4InstID][0],
							  iCurrPOC,
							  SREF_PIC + (2 << 8) + (i << 16));
					/* B1 */
					vInsertRefPicList(u4InstID, &_ptRefPicList[u4InstID][2],
							  iCurrPOC,
							  SREF_PIC + (4 << 8) + (i << 16));
				}
				if ((_ptFBufInfo[u4InstID][i].ucBFldRefType == SREF_PIC)
				    ...
				    !((_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.prSPS->
				       u4PicOrderCntType == 0)
				      && (_ptFBufInfo[u4InstID][i].fgNonExisting))) {
					/* B0 */
					vInsertRefPicList(u4InstID, &_ptRefPicList[u4InstID][1],
							  iCurrPOC,
							  SREF_PIC + (3 << 8) + (i << 16));
					/* B1 */
					vInsertRefPicList(u4InstID, &_ptRefPicList[u4InstID][3],
							  iCurrPOC,
							  SREF_PIC + (5 << 8) + (i << 16));
				}
			} else if (fgChkRefInfo(u4InstID, i, LREF_PIC)) {
				if (_ptFBufInfo[u4InstID][i].ucTFldRefType == LREF_PIC) {
					vInsertRefPicList(u4InstID, &_ptRefPicList[u4InstID][4],
							  iCurrPOC,
							  LREF_PIC + (8 << 8) + (i << 16));
				}
				if (_ptFBufInfo[u4InstID][i].ucBFldRefType == LREF_PIC) {
					vInsertRefPicList(u4InstID, &_ptRefPicList[u4InstID][5],
							  iCurrPOC,
							  LREF_PIC + (9 << 8) + (i << 16));
				}
			}
	...
	if (fgIsFrmPic(u4InstID)) {
		u4TotalRPIdx =
		    (_ptRefPicList[u4InstID][0].u4RefPicCnt <
		     _ptRefPicList[u4InstID][1].u4RefPicCnt) ? _ptRefPicList[u4InstID][1].
		    u4RefPicCnt : _ptRefPicList[u4InstID][0].u4RefPicCnt;
		u4TotalRPIdx +=
		    (_ptRefPicList[u4InstID][4].u4RefPicCnt <
		     _ptRefPicList[u4InstID][5].u4RefPicCnt) ? _ptRefPicList[u4InstID][5].
		    u4RefPicCnt : _ptRefPicList[u4InstID][4].u4RefPicCnt;
	} else {
		u4TotalRPIdx =
		    _ptRefPicList[u4InstID][0].u4RefPicCnt +
		    _ptRefPicList[u4InstID][1].u4RefPicCnt +
		    _ptRefPicList[u4InstID][4].u4RefPicCnt + _ptRefPicList[u4InstID][5].u4RefPicCnt;
	}
...
/* Return      : None */
/* ********************************************************************* */
void vSetupBRefPicList(UINT32 u4InstID, UINT32 *pu4RefIdx, UINT32 u4TFldListIdx,
		       UINT32 u4BFldListIdx, BOOL *fgDiff)
{
	INT32 i;
	UINT32 u4TotalFBuf;
	UINT32 u4Cnt[2];
	...
	for (i = 0; i < u4TotalFBuf; i++) {
		if (fgIsFrmPic(u4InstID)) {
			prBRefPicListInfo->ucFBufIdx =
			    _ptRefPicList[u4InstID][u4TFldListIdx].u4FBufIdx[i];
			prBRefPicListInfo->i4BFldLongTermPicNum =
			    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx].
			    i4BFldLongTermPicNum;
			prBRefPicListInfo->i4BFldPicNum =
			    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx].i4BFldPicNum;
			prBRefPicListInfo->i4BFldPOC =
			    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx].i4BFldPOC;
			prBRefPicListInfo->i4PicNum =
			    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx].i4PicNum;
			prBRefPicListInfo->i4LongTermPicNum =
			    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx].i4LongTermPicNum;
			prBRefPicListInfo->i4TFldLongTermPicNum =
			    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx].
			    i4TFldLongTermPicNum;
			prBRefPicListInfo->i4TFldPicNum =
			    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx].i4TFldPicNum;
			prBRefPicListInfo->i4TFldPOC =
			    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx].i4TFldPOC;
			/* prBRefPicListInfo->u4DramPicSize = _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx].u4DramPicSize; */
			prBRefPicListInfo->u4BFldPara =
			    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx].u4BFldPara;
			prBRefPicListInfo->u4FBufYStartAddr =
			    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx].u4Addr;
#if VDEC_H264_REDUCE_MV_BUFF
			prBRefPicListInfo->u4FBufMvStartAddr =
			    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx].u4MvStartAddr;
#else
			prBRefPicListInfo->u4FBufMvStartAddr =
			    (prBRefPicListInfo->u4FBufYStartAddr +
			     ((_ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx].u4DramPicSize *
			       3) >> 1));
#endif
			prBRefPicListInfo->u4FBufInfo = FRAME + (i << 8) + (pu4RefIdx[0] << 16);
			prBRefPicListInfo->u4ListIdx = u4TFldListIdx;
			prBRefPicListInfo->u4TFldPara =
			    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx].u4TFldPara;
			...
			prBRefPicListInfo->ucFBufIdx1 =
			    _ptRefPicList[u4InstID][prBRefPicListInfo->u4ListIdx1].u4FBufIdx[i];
			prBRefPicListInfo->u4FBufYStartAddr1 =
			    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx1].u4Addr;
			prBRefPicListInfo->i4LongTermPicNum1 =
			    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx1].i4LongTermPicNum;
			prBRefPicListInfo->i4PicNum1 =
			    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx1].i4PicNum;
			prBRefPicListInfo->i4TFldPOC1 =
			    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx1].i4TFldPOC;
			prBRefPicListInfo->i4BFldPOC1 =
			    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx1].i4BFldPOC;
			prBRefPicListInfo->u4TFldPara1 =
			    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx1].u4TFldPara;
			prBRefPicListInfo->u4BFldPara1 =
			    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx1].u4BFldPara;
#if VDEC_H264_REDUCE_MV_BUFF
			prBRefPicListInfo->u4FBufMvStartAddr1 =
			    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx1].u4MvStartAddr;
#else
			...
			prBRefPicListInfo->u4FBufMvStartAddr1 =
			    (prBRefPicListInfo->u4FBufYStartAddr1 +
			     ((_ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx1].u4DramPicSize *
			       3) >> 1));
#endif
			...
			if (i < _ptRefPicList[u4InstID][u4TFldListIdx].u4RefPicCnt) {
				...
				prBRefPicListInfo->u4FBufMvStartAddr =
				    (prBRefPicListInfo->u4FBufYStartAddr +
				     ((_ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx].
				       u4DramPicSize * 3) >> 1));
#endif
				prBRefPicListInfo->u4FBufInfo =
				    TOP_FIELD + (i << 8) + (pu4RefIdx[0] << 16);
				prBRefPicListInfo->u4ListIdx = u4TFldListIdx;
				prBRefPicListInfo->u4TFldPara =
				    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx].u4TFldPara;
				if (prBRefPicListInfo->u4ListIdx < 4) {
					prBRefPicListInfo->u4ListIdx1 =
					    prBRefPicListInfo->u4ListIdx + 2;
				} else {
					prBRefPicListInfo->u4ListIdx1 =
					    prBRefPicListInfo->u4ListIdx;
				}
				prBRefPicListInfo->ucFBufIdx1 =
				    _ptRefPicList[u4InstID][prBRefPicListInfo->u4ListIdx1].
				    u4FBufIdx[i];
				prBRefPicListInfo->u4FBufYStartAddr1 =
				    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx1].u4Addr;
				prBRefPicListInfo->i4LongTermPicNum1 =
				    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx1].
				    i4LongTermPicNum;
				prBRefPicListInfo->i4PicNum1 =
				    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx1].
				    i4BFldPicNum;
				...
				prBRefPicListInfo->ucFBufIdx1 += u4DpbBaseOffset;
				fgIsDiff =
				    bVDEC_HAL_H264_SetBRefPicListReg(u4InstID, prBRefPicListInfo);
				/* vVDecSetBRefPicListReg(TOP_FIELD + (i << 8) + (pu4RefIdx[0] << 16), u4TFldListIdx); */
				if (u4TFldListIdx < 4)	/* Short-term only */
				{
					prBRefPicListInfo->ucFBufIdx =
					    _ptRefPicList[u4InstID][u4TFldListIdx + 2].u4FBufIdx[i];
					prBRefPicListInfo->i4BFldLongTermPicNum =
					    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx].
					    i4BFldLongTermPicNum;
					prBRefPicListInfo->i4BFldPicNum =
					...
					prBRefPicListInfo->ucFBufIdx1 += u4DpbBaseOffset;
					fgIsDiff =
					    bVDEC_HAL_H264_SetBRefPicListReg(u4InstID,
									     prBRefPicListInfo);
			...
			if (i < _ptRefPicList[u4InstID][u4BFldListIdx].u4RefPicCnt) {
				...
				{
					prBRefPicListInfo->ucFBufIdx =
					    _ptRefPicList[u4InstID][u4BFldListIdx + 2].u4FBufIdx[i];
					prBRefPicListInfo->i4BFldLongTermPicNum =
					    _ptFBufInfo[u4InstID][prBRefPicListInfo->ucFBufIdx].
					    i4BFldLongTermPicNum;
					prBRefPicListInfo->i4BFldPicNum =
...
void vPartitionDPB(UINT32 u4InstID)
{
	...
	u4PicYCSize =
	    ((_tVerMpvDecPrm[u4InstID].u4PicW * _tVerMpvDecPrm[u4InstID].u4PicH) * 3) >> 1;
...
/* _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.ucMaxFBufNum = 7; */
...
#else
	_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.ucMaxFBufNum =
	    DPB_SZ / ((_tVerMpvDecPrm[u4InstID].u4PicW * _tVerMpvDecPrm[u4InstID].u4PicH) * 7 / 4);
	printk("[VDEC_VER] MaxFBufNum = 0x%x\n",
	       _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.ucMaxFBufNum);
	if (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.prSPS->u4NumRefFrames >
	    _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.ucMaxFBufNum) {
		_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.ucMaxFBufNum =
		...
		printk
		    ("==============================================================================\n");
		printk
		    ("[VDEC-H264-VFY] DPB Buffer smaller than file needed,should remalloc DPB buffer!\n");
		printk("[VDEC_H264_VFY] DPB Size need about 0x%x B\n",
		       (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.prSPS->u4NumRefFrames +
			1) * (_tVerMpvDecPrm[u4InstID].u4PicW * _tVerMpvDecPrm[u4InstID].u4PicH));
		printk
		    ("==============================================================================\n");
	}
#if (!VDEC_MVC_SUPPORT)
	...
	if (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.ucMaxFBufNum <
	    _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.prSPS->u4NumRefFrames) {
		printk("[VDEC-MVC-VFY]Re-Assigne MVC refnum\n");
		_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.ucMaxFBufNum =
		    _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.prSPS->u4NumRefFrames + 1;
	}
	if ((_ucMVCType[u4InstID] == 1)) {
		_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.ucMaxFBufNum++;
	...
	u4DramPicSize =
	    ((((_tVerMpvDecPrm[u4InstID].u4PicW + 15) >> 4) * ((_tVerMpvDecPrm[u4InstID].u4PicH +
								31) >> 5)) << 9);
	/* 1 pic area = Y + CbCr +MV */
	u4DramPicArea = ((((u4DramPicSize * 7) >> 2) + 511) >> 9) << 9;

	if (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.ucMaxFBufNum * u4DramPicArea >
	    DPB_SZ) {
	...
	u4DramPicSize =
	    ((((_tVerMpvDecPrm[u4InstID].u4PicW + 63) >> 6) * ((_tVerMpvDecPrm[u4InstID].u4PicH +
								31) >> 5)) << 11);
	/* For Swap mode */
	/* 1 pic area = Y + CbCr +MV */
	...
	for (i = 0; i < _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.ucMaxFBufNum; i++) {
		...
		_ptFBufInfo[u4InstID][i].u4Addr =
		    ((UINT32) _pucDPB[u4InstID]) + (i * u4DramPicArea);
		...
		{
			if (u4InstID == 0) {
				_ptFBufInfo[u4InstID][i].u4MvStartAddr =
				    (UINT32) _pucAVCMVBuff_Main[i];
			} else {
				_ptFBufInfo[u4InstID][i].u4MvStartAddr =
				    (UINT32) _pucAVCMVBuff_Sub[i];
			}
		...
		{
			_ptFBufInfo[u4InstID][i].u4MvStartAddr =
			    _ptFBufInfo[u4InstID][i].u4Addr + ((u4DramPicSize * 3) >> 1);
...
void vFillFrameNumGap(UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *tVerMpvDecPrm)
{
	...
	u4UnusedShortTermFrameNum =
	    (tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rLastInfo.u4LastFrameNum +
	     1) % tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->u4MaxFrameNum;
	u4CurrFrameNum = tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->u4FrameNum;

	if ((tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->u4FrameNum !=
	     tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rLastInfo.u4LastFrameNum)
		...
		while (u4CurrFrameNum != u4UnusedShortTermFrameNum) {
			...
			tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->
			    fgAdaptiveRefPicMarkingModeFlag = 0;

			if (tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->u4PicOrderCntType == 0) {
				_ptCurrFBufInfo[u4InstID]->i4POC =
				    tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rLastInfo.i4LastPOC;
				_ptCurrFBufInfo[u4InstID]->i4TFldPOC =
				    tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rLastInfo.
				    i4LastTFldPOC;
				_ptCurrFBufInfo[u4InstID]->i4BFldPOC =
				    tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rLastInfo.
				    i4LastBFldPOC;
			} else {
				vVDecSetCurrPOC(u4InstID);
...
BOOL fgChkRefInfo(UINT32 u4InstID, UINT32 u4FBufIdx, UINT32 u4RefType)
{
	if (fgIsFrmPic(u4InstID)) {
		...
		if ((_ptFBufInfo[u4InstID][u4FBufIdx].ucTFldRefType == u4RefType)
		    && (_ptFBufInfo[u4InstID][u4FBufIdx].ucBFldRefType == u4RefType)) {
			return TRUE;
		} else {
			return FALSE;
		}
		       ...
		       INT32 iCurrPOC, UINT32 u4RefPicListInfo)
{
	...
	if (ucRefType == SREF_PIC) {
		...
		for (j = ptRefPicList->u4RefPicCnt - 1; j >= 0; j--) {
			fgSwitch = FALSE;
			if (bListType == 0) {
				if (_ptFBufInfo[u4InstID][ptRefPicList->u4FBufIdx[j + 1]].
				    i4TFldPicNum >
				    _ptFBufInfo[u4InstID][ptRefPicList->u4FBufIdx[j]].
				    i4TFldPicNum) {
					fgSwitch = TRUE;
				}
			} else if (bListType == 1) {
				...
				iComp0 =
				    (bListType ==
				     2) ? _ptFBufInfo[u4InstID][ptRefPicList->u4FBufIdx[j]].
				    i4TFldPOC : _ptFBufInfo[u4InstID][ptRefPicList->u4FBufIdx[j]].
				    i4BFldPOC;
				iComp1 =
				    (bListType ==
				     2) ? _ptFBufInfo[u4InstID][ptRefPicList->u4FBufIdx[j +
											1]].
				    i4TFldPOC : _ptFBufInfo[u4InstID][ptRefPicList->
								      u4FBufIdx[j + 1]].i4BFldPOC;
				if ((fgIsFrmPic(u4InstID)
				     ? (((iComp0 < iCurrPOC) && (iComp1 < iCurrPOC)
					 && (iComp1 > iComp0))
					|| ((iComp0 >= iCurrPOC) && (iComp1 < iComp0))
					|| ((iComp0 >= iCurrPOC) && (iComp1 < iCurrPOC)))
				     :
				     (((iComp0 <= iCurrPOC) &&
				       (iComp1 <= iCurrPOC) && (iComp1 > iComp0))
				      || ((iComp0 > iCurrPOC) && (iComp1 < iComp0))
				      || ((iComp0 > iCurrPOC) && (iComp1 <= iCurrPOC))))) {
					fgSwitch = TRUE;
				}
				...
				iComp0 =
				    (bListType ==
				     4) ? _ptFBufInfo[u4InstID][ptRefPicList->u4FBufIdx[j]].
				    i4TFldPOC : _ptFBufInfo[u4InstID][ptRefPicList->u4FBufIdx[j]].
				    i4BFldPOC;
				iComp1 =
				    (bListType ==
				    ...
				    i4TFldPOC : _ptFBufInfo[u4InstID][ptRefPicList->
								      u4FBufIdx[j + 1]].i4BFldPOC;
				if (((iComp0 <= iCurrPOC) && (iComp1 > iComp0))
				    ||
				    ((iComp0 > iCurrPOC) &&
				     (iComp1 > iCurrPOC) && (iComp1 < iComp0))
				    || ((iComp0 <= iCurrPOC) && (iComp1 > iCurrPOC))) {
		...
		for (j = (INT32) (ptRefPicList->u4RefPicCnt - 1); j >= 0; j--) {
			if (bListType == 6) {
				iComp0 =
				    ...
				    i4TFldLongTermPicNum;
				iComp1 =
				    _ptFBufInfo[u4InstID][ptRefPicList->u4FBufIdx[j + 1]].
				    i4TFldLongTermPicNum;
			} else if (bListType == 7) {
				iComp0 =
				    _ptFBufInfo[u4InstID][ptRefPicList->u4FBufIdx[j]].
...
void vVDecSetCurrPOC(UINT32 u4InstID)
{
	...
	iMaxPicOrderCntLsb =
	    1 << (tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->u4Log2MaxPicOrderCntLsbMinus4 +
		  4);
	...
	case 0:
		if (fgIsIDRPic(u4InstID)) {
			...
			if (tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rLastInfo.fgLastMmco5) {
				if (tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rLastInfo.
				    ucLastPicStruct != BOTTOM_FIELD) {
					iPrevPOCMsb = 0;
					iPrevPOCLsb =
					    tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rLastInfo.
					    i4LastRefTFldPOC;
		...
		/* Calculate POCMsb */
		if ((tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->i4PicOrderCntLsb <
		     iPrevPOCLsb)
		    &&
		    ((iPrevPOCLsb -
		      tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->i4PicOrderCntLsb) >=
		     (iMaxPicOrderCntLsb >> 1))) {
			tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->i4PicOrderCntMsb =
			    iPrevPOCMsb + iMaxPicOrderCntLsb;
		} else
		    if ((tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->i4PicOrderCntLsb >
			 iPrevPOCLsb)
			&&
			((tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->i4PicOrderCntLsb -
			  iPrevPOCLsb) > (iMaxPicOrderCntLsb >> 1))) {
			tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->i4PicOrderCntMsb =
			    iPrevPOCMsb - iMaxPicOrderCntLsb;
		} else {
			tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->i4PicOrderCntMsb =
		...
		if ((!tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->fgFieldPicFlag)
		    || (!tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->fgBottomFieldFlag)) {
			_ptCurrFBufInfo[u4InstID]->i4TFldPOC =
			    tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->i4PicOrderCntMsb +
			    tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->i4PicOrderCntLsb;
		}
	...
	case 1:
		if (tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rLastInfo.fgLastMmco5) {
			...
			iPrevFrameNumOffset =
			    tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rLastInfo.
			    i4LastFrameNumOffset;
		}
		printk("u4LastFrameNum %d, u4FrameNum %d\n",
		       tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rLastInfo.u4LastFrameNum,
		       tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->u4FrameNum);
		if (fgIsIDRPic(u4InstID)) {
			tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.i4FrmNumOffset = 0;
		} else if (tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rLastInfo.u4LastFrameNum >
			   tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->u4FrameNum) {
			tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.i4FrmNumOffset =
			    iPrevFrameNumOffset +
			    (INT32) tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->u4MaxFrameNum;
		} else {
			tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.i4FrmNumOffset =
			    iPrevFrameNumOffset;
		}

		if (tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->
		    u4NumRefFramesInPicOrderCntCycle != 0) {
			iAbsFrameNum =
			    tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.i4FrmNumOffset +
			    (INT32) tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->
			    u4FrameNum;
		} else {
			iAbsFrameNum = 0;
		}
		...
		if (iAbsFrameNum > 0) {
			iPicOrderCntCycleCnt =
			    (iAbsFrameNum -
			     1) /
			    tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->
			    u4NumRefFramesInPicOrderCntCycle;
			iFrameNumInPicOrderCntCycle =
			    (iAbsFrameNum -
			     1) %
			    tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->
			    u4NumRefFramesInPicOrderCntCycle;
		...
		for (i = 0;
		     i <
		     tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->
		     u4NumRefFramesInPicOrderCntCycle; i++) {
			iExpectedDeltaPerPicOrderCntCycle +=
			    tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->i4OffsetForRefFrame[i];
		}
		if (iAbsFrameNum > 0) {
			iExpectedDeltaPerPicOrderCnt =
			    (INT32) iPicOrderCntCycleCnt * iExpectedDeltaPerPicOrderCntCycle;
			for (i = 0; i <= iFrameNumInPicOrderCntCycle; i++) {
				iExpectedDeltaPerPicOrderCnt =
				    iExpectedDeltaPerPicOrderCnt +
				    tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->
				    i4OffsetForRefFrame[i];
			}
		} else {
		...
		if (_u4NalRefIdc[u4InstID] == 0) {
			iExpectedDeltaPerPicOrderCnt =
			    iExpectedDeltaPerPicOrderCnt +
			    tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->i4OffsetForNonRefPic;
		}
		if (!tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->fgFieldPicFlag) {
			_ptCurrFBufInfo[u4InstID]->i4TFldPOC =
			    iExpectedDeltaPerPicOrderCnt +
			    tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->
			    i4DeltaPicOrderCnt[0];
			iTemp =
			    _ptCurrFBufInfo[u4InstID]->i4TFldPOC +
			    tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->
			    i4OffsetForTopToBottomField;
			_ptCurrFBufInfo[u4InstID]->i4BFldPOC =
			    iTemp +
			    tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->
			    i4DeltaPicOrderCnt[1];
			_ptCurrFBufInfo[u4InstID]->i4BFldPOC =
			    _ptCurrFBufInfo[u4InstID]->i4TFldPOC +
			    tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->
			    i4OffsetForTopToBottomField +
	...
	_ptCurrFBufInfo[u4InstID]->i4POC =
	    (_ptCurrFBufInfo[u4InstID]->i4TFldPOC <
	     _ptCurrFBufInfo[u4InstID]->i4BFldPOC) ? _ptCurrFBufInfo[u4InstID]->
	    i4TFldPOC : _ptCurrFBufInfo[u4InstID]->i4BFldPOC;
	/*
...
void vVerifySlidingWindowProce(UINT32 u4InstID)
{
	...
	if ((_ptCurrFBufInfo[u4InstID]->ucFBufStatus == FRAME)
	    && (_tVerMpvDecPrm[u4InstID].ucPicStruct == BOTTOM_FIELD)
	    && (_ptCurrFBufInfo[u4InstID]->ucTFldRefType == SREF_PIC)) {
		_ptCurrFBufInfo[u4InstID]->ucBFldRefType = SREF_PIC;
	} else if ((_ptCurrFBufInfo[u4InstID]->ucFBufStatus == FRAME)
		...
		for (i = 0; i < _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.ucMaxFBufNum;
		     i++) {
			if ((_ptFBufInfo[u4InstID][i].ucTFldRefType == SREF_PIC)
			    || (_ptFBufInfo[u4InstID][i].ucBFldRefType == SREF_PIC)) {
				i4FrameNumWrap =
				    (_ptFBufInfo[u4InstID][i].u4FrameNum >
				     _ptCurrFBufInfo[u4InstID]->
				     u4FrameNum) ? (_ptFBufInfo[u4InstID][i].u4FrameNum -
						    _tVerMpvDecPrm[u4InstID].SpecDecPrm.
						    rVDecH264DecPrm.prSPS->
						    u4MaxFrameNum) : _ptFBufInfo[u4InstID][i].
				    u4FrameNum;
		...
		if ((u4NumShortTerm + u4NumLongTerm) >=
		    ((_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.prSPS->u4NumRefFrames >
		      0) ? _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.prSPS->
		     u4NumRefFrames : 1)) {
...
void vSearchRealPic(UINT32 u4InstID)
{
	...
	do {
		...
		_u4CurrPicStartAddr[u4InstID] =
		    u4VDEC_HAL_H264_ReadRdPtr(_u4BSID[u4InstID], u4InstID,
					      (UINT32) _pucVFifo[u4InstID], &u4Bits);
		rH264BSInitPrm.u4VFifoSa = (UINT32) _pucVFifo[u4InstID];
		rH264BSInitPrm.u4VFifoEa = (UINT32) _pucVFifo[u4InstID] + V_FIFO_SZ;
		rH264BSInitPrm.u4VLDRdPtr =
		    (UINT32) _pucVFifo[u4InstID] + _u4CurrPicStartAddr[u4InstID];
		...
		rH264BSInitPrm.u4VLDWrPtr =
		    (UINT32) _pucVFifo[u4InstID] +
		    ((_u4LoadBitstreamCnt[u4InstID] % 2) ? (V_FIFO_SZ) : (V_FIFO_SZ >> 1));
#endif
		...
		if ((_ucMVCType[u4InstID] == 1) && (_ucNalUnitType[u4InstID] == MVC_PREFIX_NAL)) {
			...
			_ucNalUnitType[u4InstID] =
			    _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.rMvcExtInfo.
			    fgIdrFlag ? NON_IDR_SLICE : IDR_SLICE;
#else
			_ucNalUnitType[u4InstID] =
			    _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.rMvcExtInfo.
			    fgNonIdrFlag ? NON_IDR_SLICE : IDR_SLICE;
#endif
#endif
			...
			_ucNalUnitType[u4InstID] =
			    ...
			    fgNonIdrFlag ? NON_IDR_SLICE : IDR_SLICE;
#endif
		} else if ((_ucMVCType[u4InstID] == 2)
			   && ((_ucNalUnitType[u4InstID] == NON_IDR_SLICE)
			       || (_ucNalUnitType[u4InstID] == IDR_SLICE))) {
			...
			if (_u4BSID[u4InstID] != 0) {
				...
				rH264BSInitPrm.u4VLDWrPtr =
				    ...
				    ((_u4LoadBitstreamCnt[u4InstID] %
				      2) ? (V_FIFO_SZ) : (V_FIFO_SZ >> 1));
#endif
				rH264BSInitPrm.u4PredSa =
				    /*PHYSICAL*/((UINT32) _pucPredSa[u4InstID]);
...
void vVerifySeq_Par_Set_Rbsp(UINT32 u4InstID)
{
	...
	if (u4SeqParameterSetId < 32) {
		...
		_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.rLastInfo.ucLastSPSId =
	...
	if ((prSPS->u4ProfileIdc == FREXT_HP)
		...
		if (prSPS->u4ChromaFormatIdc == 3) {
			prSPS->fgResidualColorTransformFlag =
			    bVDEC_HAL_H264_GetBitStreamFlg(_u4BSID[u4InstID], u4InstID);
			/* //*fprintf(_tRecFileInfo.fpFile, "residual_color_transform_flag %d\n", prSPS->fgResidualColorTransformFlag); */
		}
		prSPS->u4BitDepthLumaMinus8 =
		    u4VDEC_HAL_H264_UeCodeNum(_u4BSID[u4InstID], u4InstID);
		...
		if (prSPS->fgSeqScalingMatrixPresentFlag) {
			for (i = 0; i < 8; i++) {
				prSPS->fgSeqScalingListPresentFlag[i] =
				    bVDEC_HAL_H264_GetBitStreamFlg(_u4BSID[u4InstID], u4InstID);
				...
				if (prSPS->fgSeqScalingListPresentFlag[i]) {
					if (i < 6) {
						vVDEC_HAL_H264_ScalingList(_u4BSID[u4InstID],
									   u4InstID,
									   prSPS->
									   cScalingList4x4[i], 16,
									   &prSPS->
									   fgUseDefaultScalingMatrix4x4Flag
									   [i]);
					} else {
						vVDEC_HAL_H264_ScalingList(_u4BSID[u4InstID],
									   u4InstID,
									   ...
									   64,
									   &prSPS->
									   fgUseDefaultScalingMatrix8x8Flag
									   [i - 6]);
					}
				}
			}
	...
	if (prSPS->u4PicOrderCntType == 0) {
		...
		prSPS->i4OffsetForNonRefPic =
		    i4VDEC_HAL_H264_SeCodeNum(_u4BSID[u4InstID], u4InstID);
	...
	if (prSPS->fgVuiParametersPresentFlag) {
		...
		if (prSPS->rVUI.fgAspectRatioInfoPresentFlag) {
			prSPS->rVUI.u4AspectRatioIdc =
			    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 8);
			if (prSPS->rVUI.u4AspectRatioIdc == 255)	/* Extended_SAR */
			{
				prSPS->rVUI.u4SarWidth =
				    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID], u4InstID,
								     16);
		...
		if (prSPS->rVUI.fgVideoSignalTypePresentFlag) {
			prSPS->rVUI.u4VideoFormat =
			    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 3);
		...
		if (prSPS->rVUI.fgTimingInfoPresentFlag) {
			prSPS->rVUI.u4NumUnitsInTick =
			    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 32);
			prSPS->rVUI.u4TimeScale =
			    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 32);
			prSPS->rVUI.fgFixedFrameRateFlag =
			    bVDEC_HAL_H264_GetBitStreamFlg(_u4BSID[u4InstID], u4InstID);
		}
		prSPS->rVUI.fgNalHrdParametersPresentFlag =
		    bVDEC_HAL_H264_GetBitStreamFlg(_u4BSID[u4InstID], u4InstID);
		if (prSPS->rVUI.fgNalHrdParametersPresentFlag) {
			vVerifyHrdParameters(u4InstID, &prSPS->rVUI.tNalHrdParameters);
		}
		prSPS->rVUI.fgVclHrdParametersPresentFlag =
		    bVDEC_HAL_H264_GetBitStreamFlg(_u4BSID[u4InstID], u4InstID);
		...
		if (prSPS->rVUI.fgNalHrdParametersPresentFlag
		    || prSPS->rVUI.fgVclHrdParametersPresentFlag) {
			prSPS->rVUI.fgLowDelayHrdFlag =
			    bVDEC_HAL_H264_GetBitStreamFlg(_u4BSID[u4InstID], u4InstID);
		}
		prSPS->rVUI.fgPicStructPresentFlag =
		    bVDEC_HAL_H264_GetBitStreamFlg(_u4BSID[u4InstID], u4InstID);
		prSPS->rVUI.fgBitstreamRestrictionFlag =
		    bVDEC_HAL_H264_GetBitStreamFlg(_u4BSID[u4InstID], u4InstID);
		if (prSPS->rVUI.fgBitstreamRestrictionFlag) {
			prSPS->rVUI.fgMotionVectorsOverPicBoundariesFlag =
			    bVDEC_HAL_H264_GetBitStreamFlg(_u4BSID[u4InstID], u4InstID);
			prSPS->rVUI.u4MaxBytesPerPicDenom =
			    u4VDEC_HAL_H264_UeCodeNum(_u4BSID[u4InstID], u4InstID);
			prSPS->rVUI.u4MaxBitsPerMbDenom =
			    u4VDEC_HAL_H264_UeCodeNum(_u4BSID[u4InstID], u4InstID);
			prSPS->rVUI.u4Log2MaxMvLengthHorizontal =
			    u4VDEC_HAL_H264_UeCodeNum(_u4BSID[u4InstID], u4InstID);
			prSPS->rVUI.u4Log2MaxMvLengthVertical =
			    u4VDEC_HAL_H264_UeCodeNum(_u4BSID[u4InstID], u4InstID);
			prSPS->rVUI.u4NumReorderFrames =
			    u4VDEC_HAL_H264_UeCodeNum(_u4BSID[u4InstID], u4InstID);
			prSPS->rVUI.u4MaxDecFrameBuffering =
			    u4VDEC_HAL_H264_UeCodeNum(_u4BSID[u4InstID], u4InstID);
		}
	}
#endif
...
void vVerifyPic_Par_Set_Rbsp(UINT32 u4InstID)
{
	...
	if (ptPPS->u4NumSliceGroupsMinus1 > 255) {
		vVDecOutputDebugString
		    ("///Warning!!! num_slice_groups_minus1 size isn't enough ///\n");
	}
	...
	if (ptPPS->u4NumSliceGroupsMinus1 > 0) {
		...
		if (ptPPS->u4SliceGroupMapType == 0) {
			...
			for (i = 0; i < ptPPS->u4NumSliceGroupsMinus1; i++) {
			   ...
			   (ptPPS->u4SliceGroupMapType == 4) || (ptPPS->u4SliceGroupMapType == 5)) {
	...
	ptPPS->u4WeightedBipredIdc =
	    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 2);
	...
	if (bVDEC_HAL_H264_IsMoreRbspData(_u4BSID[u4InstID], u4InstID)) {
		...
		if (ptPPS->fgPicScalingMatrixPresentFlag) {
			for (i = 0; i < ((ptPPS->fgTransform8x8ModeFlag << 1) + 6); i++) {
				ptPPS->fgPicScalingListPresentFlag[i] =
				    bVDEC_HAL_H264_GetBitStreamFlg(_u4BSID[u4InstID], u4InstID);
...
void vParseSliceHeader(UINT32 u4InstID)
{
	...
	if ((u4PPSID < 256)
		...
		ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS =
		    &_rH264SPS[u4InstID][_rH264PPS[u4InstID][u4PPSID].u4SeqParameterSetId];
		/* *fprintf(_tRecFileInfo.fpFile, "pic_parameter_set_id 0x%.8x\n", u4PPSID); */
	} else {
	...
	if (ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->fgFrameMbsOnlyFlag) {
		...
		if (prSliceHdr->fgFieldPicFlag) {
			...
			ptVerMpvDecPrm->ucPicStruct =
			    (prSliceHdr->fgBottomFieldFlag) ? BOTTOM_FIELD : TOP_FIELD;
		} else {
			ptVerMpvDecPrm->ucPicStruct = FRAME;
		}
	}
	...
	if (ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->u4PicOrderCntType == 0) {
		...
		if (ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prPPS->fgPicOrderPresentFlag
		    && (!prSliceHdr->fgFieldPicFlag)) {
	...
	if ((ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->u4PicOrderCntType == 1)
	    && (!ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->fgDeltaPicOrderAlwaysZeroFlag)) {
	...
	if ((ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prPPS->fgWeightedPredFlag
	     && fgIsPSlice(prSliceHdr->u4SliceType))
	    || ((ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prPPS->u4WeightedBipredIdc == 1)
		&& fgIsBSlice(prSliceHdr->u4SliceType))) {
	...
	if (ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prPPS->fgEntropyCodingModeFlag
	    && (!fgIsISlice(prSliceHdr->u4SliceType))) {
...
void vVerifyDec_Ref_Pic_Marking(UINT32 u4InstID, VDEC_INFO_H264_SLICE_HDR_T *prSliceHdr)
{
	...
	if (fgIsIDRPic(u4InstID)) {
		...
		if (prSliceHdr->fgAdaptiveRefPicMarkingModeFlag) {
			u4Cnt = 0;
			do {
				prSliceHdr->u4MemoryManagementControlOperation[u4Cnt] =
				    u4VDEC_HAL_H264_UeCodeNum(_u4BSID[u4InstID], u4InstID);
				...
				} else if (prSliceHdr->u4MemoryManagementControlOperation[u4Cnt] ==
					   2) {
					prSliceHdr->u4MemoryManagementControlOperation[u4Cnt] |=
					    (u4VDEC_HAL_H264_UeCodeNum(_u4BSID[u4InstID], u4InstID)
					     << 8);
...
void vVerifyHrdParameters(UINT32 u4InstID, VDEC_INFO_H264_HRD_PRM_T *tHrdPara)
{
	...
	for (u4SchedSelIdx = 0; u4SchedSelIdx <= tHrdPara->u4CpbCntMinus1; u4SchedSelIdx++) {
		tHrdPara->u4BitRateValueMinus1[u4SchedSelIdx] =
		    u4VDEC_HAL_H264_UeCodeNum(_u4BSID[u4InstID], u4InstID);
		tHrdPara->u4CpbSizeValueMinus1[u4SchedSelIdx] =
		    u4VDEC_HAL_H264_UeCodeNum(_u4BSID[u4InstID], u4InstID);
		tHrdPara->fgCbrFlag[u4SchedSelIdx] =
		    bVDEC_HAL_H264_GetBitStreamFlg(_u4BSID[u4InstID], u4InstID);
	}
	tHrdPara->u4InitialCpbRemovalDelayLengthMinus1 =
	    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 5);
	tHrdPara->u4CpbRemovalDelayLengthMinus1 =
	    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 5);
	tHrdPara->u4DpbOutputDelayLengthMinus1 =
	    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 5);
	tHrdPara->u4TimeOffsetLength =
	    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 5);
}
...
void vVerifyInitSliceHdr(VDEC_INFO_DEC_PRM_T *tVerMpvDecPrm)
{
	...
	tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->u4NumRefIdxL0ActiveMinus1 =
	    tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prPPS->u4NumRefIdxL0ActiveMinus1;
	tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->u4NumRefIdxL1ActiveMinus1 =
	    tVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prPPS->u4NumRefIdxL1ActiveMinus1;
...
void vVerifySEI_Rbsp(UINT32 u4InstID)
{
	...
	do {
		...
		u4PayloadType = 0;
		while ((bTmpByte =
			(u4VDEC_HAL_H264_GetBitStreamShift(_u4BSID[u4InstID], u4InstID, 8) >> 24))
		       == 0xFF) {
			u4PayloadType += 255;
		}
		...
		default:
			...
			while (u4PayloadSize > 0) {
				bTmpByte =
				    u4VDEC_HAL_H264_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID,
								      8);
				u4PayloadSize--;
			}
			break;
...
void vInterpretFilmGrainCharacteristicsInfo(UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *ptVerMpvDecPrm)
{
	...
	if (!ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSEI->fgFilmGrainCharacteristicsCancelFlag) {
		...
		ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSEI->
		    fgSeparateColourDescriptionPresentFlag =
		    bVDEC_HAL_H264_GetBitStreamFlg(_u4BSID[u4InstID], u4InstID);
		/* printf("separate_colour_description_present_flag = %d\n", ptVerMpvDecPrm->prSEI->fgSeparateColourDescriptionPresentFlag); */
		if (ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSEI->
		    fgSeparateColourDescriptionPresentFlag) {
			ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSEI->
			    u4FilmGrainBitDepthLumaMinus8 =
			    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 3);
			ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSEI->
			    u4FilmGrainBitDepthChromaMinus8 =
			    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 3);
			ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSEI->fgFilmGrainFullRangeFlag =
			    bVDEC_HAL_H264_GetBitStreamFlg(_u4BSID[u4InstID], u4InstID);
			ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSEI->
			    u4FilmGrainColourPrimaries =
			    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 8);
			ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSEI->
			    u4FilmGrainTransferCharacteristics =
			    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 8);
			ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSEI->
			    u4FilmGrainMatrixCoefficients =
			    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 8);
		}
		...
		ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSEI->u4Log2ScaleFactor =
		    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 4);
		/* printf("log2_scale_factor = %d\n", ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSEI->u4Log2ScaleFactor); */
		for (c = 0; c < 3; c++) {
			ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSEI->
			    fgCompModelPresentFlag[c] =
			    bVDEC_HAL_H264_GetBitStreamFlg(_u4BSID[u4InstID], u4InstID);
		...
		for (c = 0; c < 3; c++) {
			if (ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSEI->
				...
				{
					ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSEI->
					    pucCompModelValue[(c << 9) + (i << 1)] = 0;
					ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSEI->
					    pucCompModelValue[(c << 9) + (i << 1) + 1] = 0;
				}
				for (i = 0;
				     i <=
				     ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSEI->
				     u4NumIntensityIntervalsMinus1[c]; i++) {
					ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSEI->
					    u4IntensityIntervalLowerBound[c][i] =
					    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID],
									     u4InstID, 8);
					/* printf("intensity_interval_lower_bound[%d][%d] = %d\n", c, i, ptVerMpvDecPrm->prSEI->u4IntensityIntervalLowerBound[c][i]); */
					ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSEI->
					    u4IntensityIntervalUpperBound[c][i] =
					    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID],
									     u4InstID, 8);
					...
					{
						u4Temp =
						    i4VDEC_HAL_H264_SeCodeNum(_u4BSID[u4InstID],
									      u4InstID);
						/* printf("comp_model_value[%d][%d][%d] = %d\n", c, i, j, u4Temp); */
						if (j == 0) {
							for (k =
							     ptVerMpvDecPrm->SpecDecPrm.
							     rVDecH264DecPrm.prSEI->
							     u4IntensityIntervalLowerBound[c][i];
							     k <=
							     ptVerMpvDecPrm->SpecDecPrm.
							     rVDecH264DecPrm.prSEI->
		...
		ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSEI->
		    u4FilmGrainCharacteristicsRepetitionPeriod =
		    u4VDEC_HAL_H264_UeCodeNum(_u4BSID[u4InstID], u4InstID);
...
void vInterpretBufferingPeriodInfo(UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *ptVerMpvDecPrm)
{
	...
	if (_rH264SPS[u4InstID][u4SPSID].fgSPSValid) {
		if (((!ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr)
		     || !ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSliceHdr->
		     fgNoOutputOfPriorPicsFlag)
		    && (ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS !=
			(&_rH264SPS[u4InstID][u4SPSID]))) {
		...
		if (ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->fgVuiParametersPresentFlag) {
			if (ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->rVUI.
			    fgNalHrdParametersPresentFlag) {
				for (k = 0;
				     k <
				     ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->rVUI.
				     tNalHrdParameters.u4CpbCntMinus1 + 1; k++) {
					/*u4InitCpbRemovalDelay        = */
					    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID],
									     u4InstID,
									     ptVerMpvDecPrm->
									     SpecDecPrm.
									     rVDecH264DecPrm.prSPS->
									     rVUI.tNalHrdParameters.
									     u4InitialCpbRemovalDelayLengthMinus1
									     + 1);
					/*u4InitCpbRemovalDelayOffset = */
					    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID],
									     u4InstID,
									     ptVerMpvDecPrm->
									     SpecDecPrm.
			...
			if (ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->rVUI.
			    fgVclHrdParametersPresentFlag) {
				for (k = 0;
				     k <
				     ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.prSPS->rVUI.
				     tVclHrdParameters.u4CpbCntMinus1 + 1; k++) {
					/*u4InitCpbRemovalDelay        = */
					    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID],
									     u4InstID,
									     ptVerMpvDecPrm->
...
/* Return      : None */
/* ********************************************************************* */
void vSetupInterViewRefPicList(UINT32 u4InstID, UINT32 u4BaseViewDpbId,
			       VDEC_INFO_H264_FBUF_INFO_T * ptInterViewDpbInfo, UCHAR ucRefListId,
			       UINT32 *pu4RefIdx)
{
...
void vAppendInterviewRefPicList(UINT32 u4InstID, UINT32 *pu4RefIdx, UINT32 u4PicListIdx)
{
	...
	for (i4Idx = prVDecH264DecPrm->prSPS->rMvcSPS.ucNumViewsMinus1; i4Idx >= 0; i4Idx--) {
		if (prVDecH264DecPrm->prSPS->rMvcSPS.aucViewId[i4Idx] ==
		    prVDecH264DecPrm->rMvcExtInfo.u4ViewId) {
			u4VOIdx = i4Idx;
			break;
		}
	}
	...
	if ((u4VOIdx < MAX_MVC_VIEW_ID) && (prBaseLastInfo->ucLastDpbId < 17)) {
		...
		if (prVDecH264DecPrm->rMvcExtInfo.fgAnchorPicFlag) {
			if (((u4PicListIdx == 0) || (u4PicListIdx == 1))
			    && prVDecH264DecPrm->prSPS->rMvcSPS.aucNumAnchorRefsL0[u4VOIdx]) {
				ucNumRefListNum =
				    prVDecH264DecPrm->prSPS->rMvcSPS.aucNumAnchorRefsL0[u4VOIdx];
			} else if ((u4PicListIdx == 2)
				   && prVDecH264DecPrm->prSPS->rMvcSPS.
				   aucNumAnchorRefsL1[u4VOIdx]) {
				ucNumRefListNum =
				    prVDecH264DecPrm->prSPS->rMvcSPS.aucNumAnchorRefsL1[u4VOIdx];
			}

			if (ucNumRefListNum) {
				for (i4Idx = 0; i4Idx < ucNumRefListNum; i4Idx++) {
					if ((((u4PicListIdx == 0) || (u4PicListIdx == 1))
					     && (prBaseLastInfo->u4LastViewId ==
						 prVDecH264DecPrm->prSPS->rMvcSPS.
						 aucAnchorRefL0[u4VOIdx][i4Idx]))
					    || ((u4PicListIdx == 2)
						&& (prBaseLastInfo->u4LastViewId ==
						    prVDecH264DecPrm->prSPS->rMvcSPS.
						    aucAnchorRefL1[u4VOIdx][i4Idx]))) {
						/* Set this one to ref pic list */
						vSetupInterViewRefPicList(u4InstID,
									  prBaseLastInfo->
									  ucLastDpbId,
									  prBaseLastDpbInfo,
									  u4PicListIdx, pu4RefIdx);
						break;
					}
				}
			}
		...
		{
			if (((u4PicListIdx == 0) || (u4PicListIdx == 1))
			    && prVDecH264DecPrm->prSPS->rMvcSPS.aucNumNonAnchorRefsL0[u4VOIdx]) {
				ucNumRefListNum =
				    prVDecH264DecPrm->prSPS->rMvcSPS.aucNumNonAnchorRefsL0[u4VOIdx];
			} else if ((u4PicListIdx == 2)
				   && prVDecH264DecPrm->prSPS->rMvcSPS.
				   aucNumNonAnchorRefsL1[u4VOIdx]) {
				ucNumRefListNum =
				    prVDecH264DecPrm->prSPS->rMvcSPS.aucNumNonAnchorRefsL1[u4VOIdx];
			}
			...
			if (ucNumRefListNum) {
				for (i4Idx = 0; i4Idx < ucNumRefListNum; i4Idx++) {
					if ((((u4PicListIdx == 0) || (u4PicListIdx == 1))
					     && (prBaseLastInfo->u4LastViewId ==
						 prVDecH264DecPrm->prSPS->rMvcSPS.
						 aucNonAnchorRefL0[u4VOIdx][i4Idx]))
					    || ((u4PicListIdx == 2)
						&& (prBaseLastInfo->u4LastViewId ==
						    prVDecH264DecPrm->prSPS->rMvcSPS.
						    aucNonAnchorRefL1[u4VOIdx][i4Idx]))) {
						/* Set this one to ref pic list */
						vSetupInterViewRefPicList(u4InstID,
									  prBaseLastInfo->
									  ucLastDpbId,
...
/* if (_u4FileCnt[1] >0x09 && (_ucMVCType[u4InstID] == 2) && (_tVerMpvDecPrm[1].SpecDecPrm.rVDecH264DecPrm.rMvcExtInfo.fgNonIdrFlag == 0)) */
	if ((_ucMVCType[u4InstID] == 2)
	    && (_tVerMpvDecPrm[1].SpecDecPrm.rVDecH264DecPrm.rMvcExtInfo.fgNonIdrFlag == 0)) {
		if ((_tVerMpvDecPrm[u4InstID].ucPicStruct == BOTTOM_FIELD)
		    || (_tVerMpvDecPrm[u4InstID].ucPicStruct == TOP_FIELD)) {
			for (i4Idx = pu4RefIdx[0];
			     i4Idx <
			     _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.ucMaxFBufNum * 2;
			     i4Idx++) {
				/* Set this one to ref pic list */
				vSetupInterViewRefPicList(u4InstID, prBaseLastInfo->ucLastDpbId,
							  prBaseLastDpbInfo, u4PicListIdx,
							  pu4RefIdx);
			}
		} else {
			for (i4Idx = pu4RefIdx[0];
...
void vPrefix_Nal_Unit_Rbsp_Verify(UINT32 u4InstID)
{
	...
	ptVerMpvDecPrm = &_tVerMpvDecPrm[u4InstID];
	ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rMvcExtInfo.fgSvcExtensionFlag =
	    bVDEC_HAL_H264_GetBitStreamFlg(_u4BSID[u4InstID], u4InstID);
	...
	{
#if MVC_PATCH_1
		ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rMvcExtInfo.fgIdrFlag =
		    bVDEC_HAL_H264_GetBitStreamFlg(_u4BSID[u4InstID], u4InstID);
#else
		ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rMvcExtInfo.fgNonIdrFlag =
		    bVDEC_HAL_H264_GetBitStreamFlg(_u4BSID[u4InstID], u4InstID);
#endif
		ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rMvcExtInfo.ucPriorityId =
		    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 6);
		ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rMvcExtInfo.u4ViewId =
		    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 10);
		ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rMvcExtInfo.ucTemporalId =
		    u4VDEC_HAL_H264_GetRealBitStream(_u4BSID[u4InstID], u4InstID, 3);
		ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rMvcExtInfo.fgAnchorPicFlag =
		    bVDEC_HAL_H264_GetBitStreamFlg(_u4BSID[u4InstID], u4InstID);
		ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rMvcExtInfo.fgInterViewFlag =
		    bVDEC_HAL_H264_GetBitStreamFlg(_u4BSID[u4InstID], u4InstID);
		ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rMvcExtInfo.fgReservedOneBit =
		    bVDEC_HAL_H264_GetBitStreamFlg(_u4BSID[u4InstID], u4InstID);
	}
}
...
void vSubset_Seq_Parameter_Set_Rbsp_Verify(UINT32 u4VDecID)
{
	...
	if (ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rLastInfo.ucLastSPSId < 32) {
		prSPS =
		    &_rH264SPS[u4VDecID][ptVerMpvDecPrm->SpecDecPrm.rVDecH264DecPrm.rLastInfo.
					 ucLastSPSId];
		if ((prSPS->u4ProfileIdc == 118) || (prSPS->u4ProfileIdc == 128)) {
			...
			prSPS->rMvcSPS.ucNumViewsMinus1 =
			    u4VDEC_HAL_H264_UeCodeNum(u4BSID, u4VDecID);
			H264PsrRangeChk(prSPS->rMvcSPS.ucNumViewsMinus1, MAX_MVC_VIEW_ID);
			for (u4I = 0; u4I <= prSPS->rMvcSPS.ucNumViewsMinus1; u4I++) {
				prSPS->rMvcSPS.aucViewId[u4I] =
				    u4VDEC_HAL_H264_UeCodeNum(u4BSID, u4VDecID);
			}
			for (u4I = 1; u4I <= prSPS->rMvcSPS.ucNumViewsMinus1; u4I++) {
				prSPS->rMvcSPS.aucNumAnchorRefsL0[u4I] =
				    u4VDEC_HAL_H264_UeCodeNum(u4BSID, u4VDecID);
				H264PsrRangeChk(prSPS->rMvcSPS.aucNumAnchorRefsL0[u4I],
						MAX_MVC_REF_FRM_NUM);
				for (u4J = 0; u4J < prSPS->rMvcSPS.aucNumAnchorRefsL0[u4I]; u4J++) {
					prSPS->rMvcSPS.aucAnchorRefL0[u4I][u4J] =
					    u4VDEC_HAL_H264_UeCodeNum(u4BSID, u4VDecID);
				}
				prSPS->rMvcSPS.aucNumAnchorRefsL1[u4I] =
				    u4VDEC_HAL_H264_UeCodeNum(u4BSID, u4VDecID);
				H264PsrRangeChk(prSPS->rMvcSPS.aucNumAnchorRefsL1[u4I],
						MAX_MVC_REF_FRM_NUM);
				for (u4J = 0; u4J < prSPS->rMvcSPS.aucNumAnchorRefsL1[u4I]; u4J++) {
					prSPS->rMvcSPS.aucAnchorRefL1[u4I][u4J] =
					    u4VDEC_HAL_H264_UeCodeNum(u4BSID, u4VDecID);
				}
			}
			for (u4I = 1; u4I <= prSPS->rMvcSPS.ucNumViewsMinus1; u4I++) {
				prSPS->rMvcSPS.aucNumNonAnchorRefsL0[u4I] =
				    u4VDEC_HAL_H264_UeCodeNum(u4BSID, u4VDecID);
				H264PsrRangeChk(prSPS->rMvcSPS.aucNumNonAnchorRefsL0[u4I],
						MAX_MVC_REF_FRM_NUM);
				for (u4J = 0; u4J < prSPS->rMvcSPS.aucNumNonAnchorRefsL0[u4I];
				     u4J++) {
					prSPS->rMvcSPS.aucNonAnchorRefL0[u4I][u4J] =
					    u4VDEC_HAL_H264_UeCodeNum(u4BSID, u4VDecID);
				}
				prSPS->rMvcSPS.aucNumNonAnchorRefsL1[u4I] =
				    u4VDEC_HAL_H264_UeCodeNum(u4BSID, u4VDecID);
				H264PsrRangeChk(prSPS->rMvcSPS.aucNumNonAnchorRefsL1[u4I],
						MAX_MVC_REF_FRM_NUM);
				for (u4J = 0; u4J < prSPS->rMvcSPS.aucNumNonAnchorRefsL1[u4I];
				     u4J++) {
					prSPS->rMvcSPS.aucNonAnchorRefL1[u4I][u4J] =
					    u4VDEC_HAL_H264_UeCodeNum(u4BSID, u4VDecID);
				}
			}
			prSPS->rMvcSPS.ucNumLevelValuesSignalledMinus1 =
			    u4VDEC_HAL_H264_UeCodeNum(u4BSID, u4VDecID);
			...
			for (u4I = 0; u4I <= prSPS->rMvcSPS.ucNumLevelValuesSignalledMinus1; u4I++) {
				prSPS->rMvcSPS.aucLevelIdc[u4I] =
				    u4VDEC_HAL_H264_GetRealBitStream(u4BSID, u4VDecID, 8);
				prSPS->rMvcSPS.au2NumApplicableOpsMinus1[u4I] =
				    u4VDEC_HAL_H264_UeCodeNum(u4BSID, u4VDecID);
				for (u4J = 0; u4J <= prSPS->rMvcSPS.au2NumApplicableOpsMinus1[u4I];
				     u4J++) {
					prSPS->rMvcSPS.aucApplicableOpTemporalId[u4I][u4J] =
					    u4VDEC_HAL_H264_GetRealBitStream(u4BSID, u4VDecID, 3);
					prSPS->rMvcSPS.
					    au2ApplicableOpNumTargetViewsMinus1[u4I][u4J] =
					    u4VDEC_HAL_H264_UeCodeNum(u4BSID, u4VDecID);
					H264PsrRangeChk(prSPS->rMvcSPS.aucNumNonAnchorRefsL1[u4I],
							MAX_MVC_APPICABLE_OP_NUM);
					for (u4K = 0;
					     u4K <=
					     prSPS->rMvcSPS.
					     au2ApplicableOpNumTargetViewsMinus1[u4I][u4J]; u4K++) {
						prSPS->rMvcSPS.
						    au2ApplicableOpTargetViewsId[u4I][u4J][u4K] =
						    u4VDEC_HAL_H264_UeCodeNum(u4BSID, u4VDecID);
					}
					prSPS->rMvcSPS.au2ApplicableOpNumViewsMinus1[u4I][u4J] =
					    u4VDEC_HAL_H264_UeCodeNum(u4BSID, u4VDecID);
				}
			}

			prSPS->fgMvcVuiParametersPresentFlag =
			    bVDEC_HAL_H264_GetBitStreamFlg(u4BSID, u4VDecID);
			if (prSPS->fgMvcVuiParametersPresentFlag) {
				/* mvc_vui_parameters_Extension */
				prSPS->rMvcVUI.u4NumOpsMinus1 =
				    u4VDEC_HAL_H264_UeCodeNum(u4BSID, u4VDecID);
				H264PsrRangeChk(prSPS->rMvcSPS.aucNumNonAnchorRefsL1[u4I],
						MAX_MVC_APPICABLE_OP_NUM);
				for (u4I = 0; u4I <= prSPS->rMvcVUI.u4NumOpsMinus1; u4I++) {
					prSPS->rMvcVUI.ucTemporalId[u4I] =
					    u4VDEC_HAL_H264_GetRealBitStream(u4BSID, u4VDecID, 3);
					prSPS->rMvcVUI.ucNumTargetOutputViewsMinus1[u4I] =
					    u4VDEC_HAL_H264_UeCodeNum(u4BSID, u4VDecID);
					for (u4J = 0;
					     u4J <= prSPS->rMvcSPS.au2NumApplicableOpsMinus1[u4I];
					     u4J++) {
						prSPS->rMvcVUI.aucViewId[u4I][u4J] =
						    u4VDEC_HAL_H264_UeCodeNum(u4BSID, u4VDecID);
					}
					prSPS->rMvcVUI.fgTimingInfoPresentFlag[u4I] =
					    bVDEC_HAL_H264_GetBitStreamFlg(u4BSID, u4VDecID);
					if (prSPS->rMvcVUI.fgTimingInfoPresentFlag[u4I]) {
						prSPS->rMvcVUI.u4NumUnitsInTick[u4I] =
						    u4VDEC_HAL_H264_GetRealBitStream(u4BSID,
										     u4VDecID, 32);
						prSPS->rMvcVUI.u4TimeScale[u4I] =
						    u4VDEC_HAL_H264_GetRealBitStream(u4BSID,
										     u4VDecID, 32);
						prSPS->rMvcVUI.fgFixedFrameRateFlag[u4I] =
						    bVDEC_HAL_H264_GetBitStreamFlg(u4BSID,
										   u4VDecID);
					}
					prSPS->rMvcVUI.fgNalHrdParametersPresentFlag[u4I] =
					    bVDEC_HAL_H264_GetBitStreamFlg(u4BSID, u4VDecID);
					if (prSPS->rMvcVUI.fgNalHrdParametersPresentFlag[u4I]) {
						vVerifyHrdParameters(u4VDecID,
								     &prSPS->rVUI.
								     tNalHrdParameters);
					}
					prSPS->rMvcVUI.fgVclHrdParametersPresentFlag[u4I] =
					    bVDEC_HAL_H264_GetBitStreamFlg(u4BSID, u4VDecID);
					...
					if (prSPS->rMvcVUI.fgNalHrdParametersPresentFlag[u4I]
					    || prSPS->rMvcVUI.fgVclHrdParametersPresentFlag[u4I]) {
						prSPS->rMvcVUI.fgLowDelayHrdFlag[u4I] =
						    bVDEC_HAL_H264_GetBitStreamFlg(u4BSID,
										   u4VDecID);
					}
					prSPS->rMvcVUI.fgPicStructPresetFlag[u4I] =
					    bVDEC_HAL_H264_GetBitStreamFlg(u4BSID, u4VDecID);
				}
			}

			fgAdditionalExtension2Flag =
			    bVDEC_HAL_H264_GetBitStreamFlg(u4BSID, u4VDecID);
