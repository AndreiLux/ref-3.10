...
INT32 i4VP8_Parse_Frame_Header(UINT32 u4BsId, UINT32 u4InstID,
			       VDEC_INFO_VP8_FRM_HDR_T *prVDecVp8FrmHdr);
INT32 i4VP8_Parse_Intra_Header(UINT32 u4BsId, UINT32 u4InstID,
			       VDEC_INFO_VP8_FRM_HDR_T *prVDecVp8FrmHdr);
INT32 i4VP8_Parse_Inter_Header(UINT32 u4BsId, UINT32 u4InstID,
			       VDEC_INFO_VP8_FRM_HDR_T *prVDecVp8FrmHdr);
...
static const UINT8
    default_coef_probs[VP8_BLOCK_TYPES][VP8_COEF_BANDS][VP8_PREV_COEF_CONTEXTS][VP8_COEF_TOKENS -
										1] = {
	{
	 {
...
static const UINT8
    _uVp8CoefUpdateProbs[VP8_BLOCK_TYPES][VP8_COEF_BANDS][VP8_PREV_COEF_CONTEXTS][VP8_COEF_TOKENS -
										  1] = {
	{
	 {
...
static VOID _VDEC_Vp8InitDeQTable(VDEC_PARAM_VP8DEC_T *pVp8DecParam, BOOL *fgChange)
{
	...
	for (u4QType = VP8_QTYPE_Y1AC; u4QType < VP8_QTYPE_MAX; u4QType++) {
		...
		if (u4QType == VP8_QTYPE_Y1DC || u4QType == VP8_QTYPE_Y2DC
		    || u4QType == VP8_QTYPE_UVDC) {
			...
			for (u4Index = 0; u4Index < VP8_QINDEX_RANGE; u4Index++) {
				for (u4AcIndex = 1; u4AcIndex < 16; u4AcIndex++) {
					...
					_rVp8Dequant[u4QType >> 1][u4Index][u4Row][u4Col] =
					    i2Qvalue;
				}
			}
#else
	...
	if (1) {
		for (u4SegIndex = 0; u4SegIndex < MAX_MB_SEGMENTS; u4SegIndex++) {
			if (VDEC_FLGSET(prVp8DecParam->u4FlagParam, VP8PARAM_SEGMENT_ENABLE)) {
				if (VDEC_FLGSET
				    (prVp8DecParam->u4FlagParam, VP8PARAM_SEGMENT_ABSDATA)) {
					i4QIndex =
					    prVp8DecParam->
					    SegmentFeatureData[VP8_MBLVL_ALT_Q][u4SegIndex];
				} else {
					i4QIndex =
					    prVp8DecParam->QIndexInfo[VP8_QTYPE_Y1AC] +
					    prVp8DecParam->
					    SegmentFeatureData[VP8_MBLVL_ALT_Q][u4SegIndex];
			...
			u4Value =
			    (u4Value << 8) | (_rVp8Dequant[VP8_QTABLE_Y2][i4QIndex][0][0] & 0xff);
			u4Value =
			    (u4Value << 8) | ((_rVp8Dequant[VP8_QTABLE_Y2][i4QIndex][0][1] >> 8) &
					      0xff);
			u4Value =
			    (u4Value << 8) | (_rVp8Dequant[VP8_QTABLE_Y2][i4QIndex][0][1] & 0xff);
			...
			u4Value =
			    (u4Value << 8) | (_rVp8Dequant[VP8_QTABLE_Y1][i4QIndex][0][0] & 0xff);
			u4Value =
			    (u4Value << 8) | ((_rVp8Dequant[VP8_QTABLE_Y1][i4QIndex][0][1] >> 8) &
					      0xff);
			u4Value =
			    (u4Value << 8) | (_rVp8Dequant[VP8_QTABLE_Y1][i4QIndex][0][1] & 0xff);
			vVDecWriteVLD(u4InstID, RW_VLD_SCL_ADDR, (u4SegIndex << 4) + 4);
			vVDecWriteVLD(u4InstID, RW_VLD_SCL_DATA, u4Value);

			u4Value = (_rVp8Dequant[VP8_QTABLE_UV][i4QIndex][0][0] >> 8) & 0xff;
			u4Value =
			    (u4Value << 8) | (_rVp8Dequant[VP8_QTABLE_UV][i4QIndex][0][0] & 0xff);
			u4Value =
			    (u4Value << 8) | ((_rVp8Dequant[VP8_QTABLE_UV][i4QIndex][0][1] >> 8) &
					      0xff);
			u4Value =
			    (u4Value << 8) | (_rVp8Dequant[VP8_QTABLE_UV][i4QIndex][0][1] & 0xff);
	...
	for (u4BlockType = 0; u4BlockType < VP8_BLOCK_TYPES; u4BlockType++) {
		for (u4Band = 0; u4Band < VP8_COEF_BANDS; u4Band++) {
			for (u4Context = 0; u4Context < VP8_PREV_COEF_CONTEXTS; u4Context++) {
				...
				prCtxt->CoefProbs[u4BlockType][u4Band][u4Context][0] =
				    (UINT8) (u4CoefProb & 0xff);
				prCtxt->CoefProbs[u4BlockType][u4Band][u4Context][1] =
				    (UINT8) ((u4CoefProb >> 8) & 0xff);
				prCtxt->CoefProbs[u4BlockType][u4Band][u4Context][2] =
				    (UINT8) ((u4CoefProb >> 16) & 0xff);
				prCtxt->CoefProbs[u4BlockType][u4Band][u4Context][3] =
				    (UINT8) ((u4CoefProb >> 24) & 0xff);
				u4CoefProb = u4VDecReadVP8VLD(0, RW_VP8_BSDSET);
				prCtxt->CoefProbs[u4BlockType][u4Band][u4Context][4] =
				    (UINT8) (u4CoefProb & 0xff);
				prCtxt->CoefProbs[u4BlockType][u4Band][u4Context][5] =
				    (UINT8) ((u4CoefProb >> 8) & 0xff);
				prCtxt->CoefProbs[u4BlockType][u4Band][u4Context][6] =
				    (UINT8) ((u4CoefProb >> 16) & 0xff);
				prCtxt->CoefProbs[u4BlockType][u4Band][u4Context][7] =
				    (UINT8) ((u4CoefProb >> 24) & 0xff);
				u4CoefProb = u4VDecReadVP8VLD(0, RW_VP8_BSDSET);
				prCtxt->CoefProbs[u4BlockType][u4Band][u4Context][8] =
				    (UINT8) (u4CoefProb & 0xff);
				prCtxt->CoefProbs[u4BlockType][u4Band][u4Context][9] =
				    (UINT8) ((u4CoefProb >> 8) & 0xff);
				prCtxt->CoefProbs[u4BlockType][u4Band][u4Context][10] =
				    (UINT8) ((u4CoefProb >> 16) & 0xff);
				u4BusSramAddr += 4;
			}
		}
		...
		for (u4Band = 0; u4Band < VP8_COEF_BANDS; u4Band++) {
			for (u4Context = 0; u4Context < VP8_PREV_COEF_CONTEXTS; u4Context++) {
				...
				for (u4Tolen = 0; u4Tolen < VP8_COEF_TOKENS - 1; u4Tolen++) {
					...
					u4CoefProb +=
					    prCtxt->
					    CoefProbs[u4BlockType][u4Band][u4Context][u4Tolen] <<
					    (u4Mod << 3);
					if (u4Mod == 3) {

						vVDecWriteVP8VLD(u4InstID, RW_VP8_BSDSET,
								 u4CoefProb, 0);
					}
				}
...
static VOID _VDEC_Vp8FlushCtxProbs(UINT32 u4InstID, VDEC_VP8FRAME_CONTEXT_T *prCtxt,
				   UINT32 u4FrameType)
{
	UINT32 u4HalValue = 0;
	UINT8 *prMvc = (UINT8 *) prCtxt->MVC;
...
static UINT32 u4VDEC_Vp8DecInit(VDEC_INFO_VP8_FRM_HDR_T *pVp8DecInfo)
{
	...
	memcpy(pVp8DecInfo->rVp8DecParam.rCurFc.CoefProbs, default_coef_probs,
	       sizeof(default_coef_probs));
	memcpy(&pVp8DecInfo->rVp8DecParam.rLastFC, &pVp8DecInfo->rVp8DecParam.rCurFc,
	       sizeof(VDEC_VP8FRAME_CONTEXT_T));
			       ...
			       VDEC_INFO_VP8_FRM_HDR_T *prVDecVp8FrmHdr)
{
	...
	u4HalValue = 0;
	VDEC_REG_SET_VALUE(u4HalValue, ((prVDecVp8FrmHdr->u4Width + 15) >> 4) - 1,
			   RW_PIC_WIDTH_IN_MBS_S, RW_PIC_WIDTH_IN_MBS_E);
	VDEC_REG_SET_VALUE(u4HalValue, ((prVDecVp8FrmHdr->u4Height + 15) >> 4) - 1,
			   RW_PIC_HEIGHT_IN_MBS_S, RW_PIC_HEIGHT_IN_MBS_E);
	vVDecWriteVLDTOP(u4InstID, RW_VLD_TOP_PIC_MB_SIZE_M1, u4HalValue);

	u4HalValue = 0;
	VDEC_REG_SET_VALUE(u4HalValue, (prVDecVp8FrmHdr->u4FirstPartLen + u4ReadOffset),
			   RW_VP8_FSTPB_S, RW_VP8_FSTPB_E);
	VDEC_REG_SET_VALUE(u4HalValue, prVDecVp8FrmHdr->uVersion, RW_VP8_VERSION_S,
			   RW_VP8_VERSION_E);
	...
	VDEC_RPOS_INC(prVDecVp8FrmHdr->u4VldStartPos, u4ReadOffset, prVDecVp8FrmHdr->u4FifoStart,
		      prVDecVp8FrmHdr->u4FifoEnd);
...
/* prVDecVp8FrmHdr->u4FifoEnd = VIRTUAL(prVDecVp8FrmHdrPhysical.u4FifoEnd); */
	...
	if (prVDecVp8FrmHdr->uFrameType == VP8_I_FRM) {
		VDEC_SETFLG_COND(u4FlagParam, VP8PARAM_COLOR,
				 u4VDEC_VP8_VLDReadLiteral(u4InstID, 1));
		VDEC_SETFLG_COND(u4FlagParam, VP8PARAM_CLAMP_TYPE,
				 u4VDEC_VP8_VLDReadLiteral(u4InstID, 1));
		memcpy(prVp8DecParam->rCurFc.MVC, vp8_default_mv_context,
		       sizeof(vp8_default_mv_context));
		memcpy(prVDecVp8FrmHdr->rVp8DecParam.rCurFc.CoefProbs, default_coef_probs,
		       sizeof(default_coef_probs));
		...
		memset(prVp8DecParam->SegmentFeatureData, 0,
		       sizeof(prVp8DecParam->SegmentFeatureData));
	...
	VDEC_SETFLG_COND(u4FlagParam, VP8PARAM_SEGMENT_ENABLE,
			 u4VDEC_VP8_VLDReadLiteral(u4InstID, 1));

	if (VDEC_FLGSET(u4FlagParam, VP8PARAM_SEGMENT_ENABLE)) {
		VDEC_SETFLG_COND(u4FlagParam, VP8PARAM_SEGMENT_UPDATE_MAP,
				 u4VDEC_VP8_VLDReadLiteral(u4InstID, 1));
		VDEC_SETFLG_COND(u4FlagParam, VP8PARAM_SEGMENT_UPDATE_DATA,
				 u4VDEC_VP8_VLDReadLiteral(u4InstID, 1));

		if (VDEC_FLGSET(u4FlagParam, VP8PARAM_SEGMENT_UPDATE_DATA)) {
			UINT32 u4LvlType, u4SegIndex;
			VDEC_SETFLG_COND(u4FlagParam, VP8PARAM_SEGMENT_ABSDATA,
					 u4VDEC_VP8_VLDReadLiteral(u4InstID, 1));
			for (u4LvlType = 0; u4LvlType < VP8_MBLVL_MAX; u4LvlType++) {
				for (u4SegIndex = 0; u4SegIndex < MAX_MB_SEGMENTS; u4SegIndex++) {
					prVp8DecParam->SegmentFeatureData[u4LvlType][u4SegIndex] =
					    0;
					if (u4VDEC_VP8_VLDReadLiteral(u4InstID, 1)) {
						prVp8DecParam->
						    SegmentFeatureData[u4LvlType][u4SegIndex] =
						    u4VDEC_VP8_VLDReadLiteral(u4InstID,
									      _Vp8MbFeatureDataBits
									      [u4LvlType]);
						if (u4VDEC_VP8_VLDReadLiteral(u4InstID, 1)) {
							prVp8DecParam->
							    SegmentFeatureData[u4LvlType]
							    [u4SegIndex] =
							    -prVp8DecParam->
							    SegmentFeatureData[u4LvlType]
							    [u4SegIndex];
			...
			VDEC_SETFLG_COND(u4FlagParam, VP8PARAM_SEGMENT_ABSDATA,
					 VDEC_FLGSET(prVp8DecParam->u4LastParam,
						     VP8PARAM_SEGMENT_ABSDATA));
		}
		...
		if (VDEC_FLGSET(u4FlagParam, VP8PARAM_SEGMENT_UPDATE_MAP)) {
			...
			for (u4ProbIndex = 0; u4ProbIndex < MB_FEATURE_TREE_PROBS; u4ProbIndex++) {
				...
				if (u4VDEC_VP8_VLDReadLiteral(u4InstID, 1)) {
					prVp8DecParam->SegmentTreeProbs[u4ProbIndex] =
					    (UINT8) u4VDEC_VP8_VLDReadLiteral(u4InstID, 8);
				}
			}
		}

		u4HalValue =
		    (prVp8DecParam->SegmentTreeProbs[2] << 16) +
		    (prVp8DecParam->SegmentTreeProbs[1] << 8)
	...
	prVp8DecParam->eLoopFilterType =
	    (VDEC_VP8_FILTER_TYPE_T) u4VDEC_VP8_VLDReadLiteral(u4InstID, 1);
	...
	VDEC_SETFLG_COND(u4FlagParam, VP8PARAM_MODEREF_LFDELTA_ENABLE,
			 u4VDEC_VP8_VLDReadLiteral(u4InstID, 1));

	if (VDEC_FLGSET(u4FlagParam, VP8PARAM_MODEREF_LFDELTA_ENABLE)) {
		VDEC_SETFLG_COND(u4FlagParam, VP8PARAM_MODEREF_IFDELTA_UPDATE,
				 u4VDEC_VP8_VLDReadLiteral(u4InstID, 1));
		if (VDEC_FLGSET(u4FlagParam, VP8PARAM_MODEREF_IFDELTA_UPDATE)) {
			...
			for (u4DeltaIndex = 0; u4DeltaIndex < MAX_REF_LF_DELTAS; u4DeltaIndex++) {
				if (u4VDEC_VP8_VLDReadLiteral(u4InstID, 1)) {
					prVp8DecParam->RefLFDeltas[u4DeltaIndex] =
					    (INT8) u4VDEC_VP8_VLDReadLiteral(u4InstID, 6);
					if (u4VDEC_VP8_VLDReadLiteral(u4InstID, 1)) {
						prVp8DecParam->RefLFDeltas[u4DeltaIndex] =
						    prVp8DecParam->RefLFDeltas[u4DeltaIndex] * (-1);
					}
				}
			}
			...
			for (u4DeltaIndex = 0; u4DeltaIndex < MAX_MODE_LF_DELTAS; u4DeltaIndex++) {
				if (u4VDEC_VP8_VLDReadLiteral(u4InstID, 1)) {
					prVp8DecParam->ModeLFDeltas[u4DeltaIndex] =
					    (INT8) u4VDEC_VP8_VLDReadLiteral(u4InstID, 6);
					if (u4VDEC_VP8_VLDReadLiteral(u4InstID, 1)) {
						prVp8DecParam->ModeLFDeltas[u4DeltaIndex] =
						    prVp8DecParam->ModeLFDeltas[u4DeltaIndex] *
	...
	VDEC_RPOS_INC(prVDecVp8FrmHdr->u4VldStartPos, prVDecVp8FrmHdr->u4FirstPartLen,
		      prVDecVp8FrmHdr->u4FifoStart, prVDecVp8FrmHdr->u4FifoEnd);
	...
	ASSERT(prVp8DecParam->uDataPartitionNum <= MAX_VP8_DATAPARTITION);
	{
		...
		for (u4Index = 0; u4Index < prVp8DecParam->uDataPartitionNum; u4Index++) {
			if (u4Index < prVp8DecParam->uDataPartitionNum - 1) {
				...
				u4PartionOffset =
				    u4VDecReadVP8VLD(u4InstID, RW_VP8_COEFR0 + u4Index * 4);
				VDEC_RPOS_INC(u4PartionOffset, u4HalValue,
					      prVDecVp8FrmHdr->u4FifoStart,
					      prVDecVp8FrmHdr->u4FifoEnd);
/* VDEC_RPOS_INC(u4PartionOffset,u4HalValue,prVDecVp8FrmHdrPhysical.u4FifoStart, prVDecVp8FrmHdrPhysical.u4FifoEnd); */
				...
				vVDecWriteVP8VLD(u4InstID, RW_VP8_COEFPB0 + u4Index * 4,
						 prVDecVp8FrmHdr->u4FrameSize, 0);
			}
		}
	}
	/* prVDecVp8FrmHdr->u4FifoStart = VIRTUAL(prVDecVp8FrmHdrPhysical.u4FifoStart); */
/* prVDecVp8FrmHdr->u4FifoEnd = VIRTUAL(prVDecVp8FrmHdrPhysical.u4FifoEnd); */
...

	{
		...
		INT16 i2QValue = 0;
		prVp8DecParam->QIndexInfo[VP8_QTYPE_Y1AC] =
		    (INT16) u4VDEC_VP8_VLDReadLiteral(u4InstID, 7);
	...
	if (prVDecVp8FrmHdr->uFrameType != VP8_I_FRM) {
		VDEC_SETFLG_COND(u4FlagParam, VP8PARAM_REFRESH_GOLDEN,
				 u4VDEC_VP8_VLDReadLiteral(u4InstID, 1));
		VDEC_SETFLG_COND(u4FlagParam, VP8PARAM_REFRESH_ALTRF,
				 u4VDEC_VP8_VLDReadLiteral(u4InstID, 1));
		prVp8DecParam->uCopyBuf2Gf = 0;
		...
		prVp8DecParam->RefFrameSignBias[VP8_MVREF_GOLDEN_FRAME] =
		    u4VDEC_VP8_VLDReadLiteral(u4InstID, 1);
		prVp8DecParam->RefFrameSignBias[VP8_MVREF_ALTREF_FRAME] =
		    u4VDEC_VP8_VLDReadLiteral(u4InstID, 1);
	...
	VDEC_SETFLG_COND(u4FlagParam, VP8PARAM_REFRESH_PROBS,
			 u4VDEC_VP8_VLDReadLiteral(u4InstID, 1));

	if (!VDEC_FLGSET(u4FlagParam, VP8PARAM_REFRESH_PROBS)) {
		memcpy(&prVp8DecParam->rLastFC, &prVp8DecParam->rCurFc,
		       sizeof(VDEC_VP8FRAME_CONTEXT_T));
	}
	...
	if (prVDecVp8FrmHdr->uFrameType == VP8_I_FRM) {
		...
		VDEC_SETFLG_COND(u4FlagParam, VP8PARAM_REFRESH_LASTFRAME,
				 u4VDEC_VP8_VLDReadLiteral(u4InstID, 1));
	}
		...
		for (u4BlockType = 0; u4BlockType < VP8_BLOCK_TYPES; u4BlockType++) {
			...
			for (u4Band = 0; u4Band < VP8_COEF_BANDS; u4Band++) {
				...
				for (u4Context = 0; u4Context < VP8_PREV_COEF_CONTEXTS; u4Context++) {
					...
					puWriteProb =
					    prVp8DecParam->rCurFc.
					    CoefProbs[u4BlockType][u4Band][u4Context];
					for (u4Tolen = 0; u4Tolen < VP8_COEF_TOKENS - 1; u4Tolen++) {
						if (u4VDec_VP8_VLDReadBit
						    (u4InstID, puContext[u4Tolen])) {
							puWriteProb[u4Tolen] =
							    u4VDEC_VP8_VLDReadLiteral(u4InstID, 8);
						}
					}
				}
			}
	...
	u4HalValue = 0;
	VDEC_REG_SET_FLAG(u4HalValue, VDEC_FLGSET(u4FlagParam, VP8PARAM_CLAMP_TYPE),
			  RW_VP8_CLTYPE_FLAG);
	VDEC_REG_SET_FLAG(u4HalValue, VDEC_FLGSET(u4FlagParam, VP8PARAM_SEGMENT_ENABLE),
			  RW_VP8_SEGEN_FLAG);
	VDEC_REG_SET_FLAG(u4HalValue, VDEC_FLGSET(u4FlagParam, VP8PARAM_SEGMENT_UPDATE_MAP),
			  RW_VP8_UMBSM_FLAG);
	VDEC_REG_SET_FLAG(u4HalValue, VDEC_FLGSET(u4FlagParam, VP8PARAM_NOCOEF_SKIP),
			  RW_VP8_MBSKIP_FLAG);
	VDEC_REG_SET_VALUE(u4HalValue, prVp8DecParam->uDataPartitionToken, RW_VP8_MTP_S,
			   RW_VP8_MTP_E);
	...
	if (prVDecVp8FrmHdr->uFrameType == VP8_I_FRM) {
		if (VDEC_FLGSET(u4FlagParam, VP8PARAM_NOCOEF_SKIP)) {
			...
			vVDecWriteVP8VLD(u4InstID, RW_VP8_PICPROB,
					 (UINT32) prVp8DecParam->uSkipFalseProb, 0);
		...
		if (u4VDEC_VP8_VLDReadLiteral(u4InstID, 1)) {
			INT32 i4YMode = 0;
			do {
				prVp8DecParam->rCurFc.YModeProb[i4YMode] =
				    (UINT8) u4VDEC_VP8_VLDReadLiteral(u4InstID, 8);
		...
		if (u4VDEC_VP8_VLDReadLiteral(u4InstID, 1)) {
			INT32 i4UVMode = 0;
			do {
				prVp8DecParam->rCurFc.UVModeProb[i4UVMode] =
				    (UINT8) u4VDEC_VP8_VLDReadLiteral(u4InstID, 8);
			...
			do {
				...
				do {
					if (u4VDec_VP8_VLDReadBit(u4InstID, *pMvUpdateProb++)) {
						const UINT8 x =
						    (UINT8) u4VDEC_VP8_VLDReadLiteral(u4InstID, 7);
	...
	case VP8_FRAME_CURRENT:
		{
			...
			case VP8_FRAME_CURRENT:
				{
					prVDecVp8FrmHdr->u4GldYAddr =
					    (UINT32) _pucWorkYBuf[u4InstID];
					switch (_u4VP8LastBufIdx[u4InstID]) {
					case VP8_FRAME_CURRENT:
						prVDecVp8FrmHdr->u4LstYAddr =
						    (UINT32) _pucWorkYBuf[u4InstID];
						_u4VP8CurrBufIdx[u4InstID] = VP8_FRAME_LAST;
						prVDecVp8FrmHdr->u4CurYAddr =
						    (UINT32) _pucDumpLstYBuf[u4InstID];
					...
					case VP8_FRAME_LAST:
						...
						prVDecVp8FrmHdr->u4CurYAddr =
						    (UINT32) _pucDumpGldYBuf[u4InstID];
					...
					case VP8_FRAME_ALT:
						prVDecVp8FrmHdr->u4LstYAddr =
						    (UINT32) _pucDumpArfYBuf[u4InstID];
						_u4VP8CurrBufIdx[u4InstID] = VP8_FRAME_LAST;
						prVDecVp8FrmHdr->u4CurYAddr =
						    (UINT32) _pucDumpLstYBuf[u4InstID];
						/* prVDecVp8FrmHdr->u4CurCAddr= (UINT32)_pucDumpLstCBuf[u4InstID]; */
...
/* VDEC_INFO_DEC_PRM_T *prDecPrm = &_tVerMpvDecPrm[u4InstID]; */
	VDEC_INFO_VP8_DEC_PRM_T * prVDecVP8DecPrm =
	    &(_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecVP8DecPrm);
	VDEC_INFO_VP8_FRM_HDR_T *prVDecVp8FrmHdr = &_rVDecVp8FrmHdr[u4InstID];
...
/* Searh path name */
	...
	for (i = 0;; i++) {
		...
		if (_bFileStr1[u4InstID][1][i] == 'v' || _bFileStr1[u4InstID][1][i] == 'V') {
			if ((_bFileStr1[u4InstID][1][i + 1] == 'p'
			     || _bFileStr1[u4InstID][1][i + 1] == 'P')
			    && (_bFileStr1[u4InstID][1][i + 2] == '8'
				|| _bFileStr1[u4InstID][1][i + 2] == '8')
			    && (_bFileStr1[u4InstID][1][i + 3] == '_')
			    && (_bFileStr1[u4InstID][1][i + 4] == 'b'
				|| _bFileStr1[u4InstID][1][i + 4] == 'B')
			    && (_bFileStr1[u4InstID][1][i + 5] == 'i'
				|| _bFileStr1[u4InstID][1][i + 5] == 'I')
			    && (_bFileStr1[u4InstID][1][i + 6] == 't'
				|| _bFileStr1[u4InstID][1][i + 6] == 'T')
			    && (_bFileStr1[u4InstID][1][i + 7] == 's'
				|| _bFileStr1[u4InstID][1][i + 7] == 'S')
			    && (_bFileStr1[u4InstID][1][i + 8] == 't'
				|| _bFileStr1[u4InstID][1][i + 8] == 'T')) {
				path_byte_addr = i;
			}
		} else if (_bFileStr1[u4InstID][1][i] == '.') {
			if ((_bFileStr1[u4InstID][1][i + 1] == 'i'
			     || _bFileStr1[u4InstID][1][i + 1] == 'I')
			    && (_bFileStr1[u4InstID][1][i + 2] == 'v'
				|| _bFileStr1[u4InstID][1][i + 2] == 'V')
			    && (_bFileStr1[u4InstID][1][i + 3] == 'f'
				|| _bFileStr1[u4InstID][1][i + 3] == 'F')) {
	...
	u4Temp = sprintf(_bPatternPath + path_byte_addr + 14, "%s", _bPathAddStr) + 14;
...
#if VDEC_SUPPORT_MERGE_CRC
	{
		...
		u4CRCValueY0 =
		    (u4CRCTmp3 << 24) | (u4CRCTmp2 << 16) | (u4CRCTmp1 << 8) | (u4CRCTmp0 << 0);
	...
	vConcateStr((char *)_bFileStr1[u4InstID][3], (char *)_bPatternPath, (char *)_bFileNameCRC,
		    (UINT32) _u4FileCnt[u4InstID]);

#if VDEC_DRAM_BUSY_TEST
	i4VDEC_HAL_Dram_Busy_Off(u4InstID);
	fgOpenFile(u4InstID, (char *)_bFileStr1[u4InstID][3], (char *)"r+b",
...
/*		   for (j=filename_byte_addr; j>0 ; j--)
	...
	if (fgCompare) {
		...
		vConcateStr(_bFileStr1[u4InstID][3], _bPatternPath, _bFileAddStrY,
			    _u4FileCnt[u4InstID]);

#ifdef EXT_COMPARE
		_tFBufFileInfo[u4InstID].u4FileLength =
		    (((prVDecVp8FrmHdr->u2WidthDec +
		       15) >> 4) << 4) * (((prVDecVp8FrmHdr->u2HeightDec + 31) >> 5) << 5);
#else
#ifdef DIRECT_DEC
...
#endif
		{
			...
			if (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecVP8DecPrm.rVP8PpInfo.
			    fgPpEnable) {
...
/* HalFlushInvalidateDCache(); */
		...
		/* if(!fgIsMonoPic(_u4VDecID)) */
		{
			...
			_tFBufFileInfo[u4InstID].u4FileLength = 0;
			vConcateStr(_bFileStr1[u4InstID][4], _bPatternPath, _bFileAddStrC,
				    _u4FileCnt[u4InstID]);
#ifdef EXT_COMPARE
			_tFBufFileInfo[u4InstID].u4FileLength =
			    (((prVDecVp8FrmHdr->u2WidthDec +
			       15) >> 4) << 4) * (((prVDecVp8FrmHdr->u2HeightDec +
						    31) >> 5) << 5) >> 1;
#else
#ifdef DIRECT_DEC
			if ((_u4FileCnt[u4InstID] >= _u4StartCompPicNum[u4InstID])
...
#endif
			{
								...
								if (1) {
									if ((mbw == u4Tx0
										     ...
										     (pbGoldenBuf)))
										{
											...
											u4Cnt++;
											vVDecOutputDebugString
											    ("Cb Data Mismatch at [x= 0x%d, y=0x%d] = 0x%x, Golden = 0x%x !!!\n",
											     u4XPix,
											     u4YPix,
											     (*pbDecBuf),
											     (*pbGoldenBuf));
										     ...
										     (pbGoldenBuf)))
										{
											...
											u4Cnt++;
											vVDecOutputDebugString
											    ("Cr Data Mismatch at [x= 0x%d, y=0x%d] = 0x%x, Golden = 0x%x !!!\n",
											     u4XPix,
											     u4YPix,
											     (*pbDecBuf),
											     (*pbGoldenBuf));
				...
				for (u4Pix = 0; u4Pix < u4Width * (u4Height >> 1); u4Pix++) {
					if ((_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecVP8DecPrm.
					     fgDec2ndFld)
	...
	if ((_u4FileCnt[u4InstID] >= _u4StartCompPicNum[u4InstID])
		...
		if (fgCompare) {
			...
			vConcateStr(_bFileStr1[u4InstID][3], _bFileStr1[u4InstID][0],
				    "_chksum.bin\0", _u4FileCnt[u4InstID]);
#ifdef IDE_READ_SUPPORT
			fgOpen =
			    fgPureOpenIdeFile(_bTempStr1[u4InstID], "r+b",
					      &_tTempFileInfo[u4InstID]);
#else
			fgOpen =
			    fgOpenFile(u4InstID, _bTempStr1[u4InstID], "r+b",
				       &_tTempFileInfo[u4InstID]);
#endif
#else
			_tFBufFileInfo[u4InstID].fgGetFileInfo = FALSE;
...
#ifdef IDE_READ_SUPPORT
			fgOpen =
			    fgPureOpenIdeFile(_bFileStr1[u4InstID][3], "r+b",
					      &_tFBufFileInfo[u4InstID]);
#else
			fgOpen =
			    fgOpenFile(u4InstID, _bFileStr1[u4InstID][3], "r+b",
...
#if defined(CAPTURE_ESA_LOG) && defined(CAPTURE_ALL_IN_ONE) && (!WRITE_ESA_PER_FRAME)
					fgWrData2PC(_pucESATotalBuf[u4InstID],
						    _u4ESATotalLen[u4InstID], 7,
						    _ESAFileName[u4InstID]);
#endif
					sprintf(strMessage,
						" Compare Finish==> Pic count to [%d]\n",
						_u4FileCnt[u4InstID] - 1);
					fgWrMsg2PC(strMessage, strlen(strMessage), 8,
						   &_tFileListRecInfo[u4InstID]);
					/* fprintf(_tFileListRecInfo.fpFile, " Compare Finish==> Pic count to [%d]\n", _u4FileCnt[_u4VDecID] - 1); */
	...
	if (fgCompare) {
...
#endif
		{
			...
			for (mbw = 0; mbw < u4Tx1; mbw++) {
				...
				if (_u4MbH % 2) {
					u4StartGld =
					    ((_u4MbH % 2) * (16 * 16)) + ((_u4MbH >> 1) * u4Tx1 +
									  mbw) * (16 * 32);
				} else {
					u4StartGld =
					    (_u4MbH ==
					     0) ? ((_u4MbH * u4Tx1 +
						    mbw) * (16 * 32)) : (((_u4MbH >> 1) * u4Tx1 +
									  mbw) * (16 * 32));
				}
				pbGoldenBuf =
				    (UCHAR *) (((UINT32) (_pucDumpYBuf[u4InstID])) + u4StartGld);
				pbDecBuf = (UCHAR *) (u4YBase + u4Start);
...
#endif
		{
			...
			for (mbw = 0; mbw < u4Tx1; mbw++) {
				...
				if (_u4MbH % 2) {
					u4StartGld =
					    ((_u4MbH % 2) * (16 * 8)) + ((_u4MbH >> 1) * u4Tx1 +
									 mbw) * (16 * 16);
				} else {
					u4StartGld =
					    (_u4MbH ==
					     0) ? ((_u4MbH * u4Tx1 +
						    mbw) * (16 * 16)) : (((_u4MbH >> 1) * u4Tx1 +
									  mbw) * (16 * 16));
				}
				pbGoldenBuf =
				    (UCHAR *) (((UINT32) (_pucDumpCBuf[u4InstID])) + u4StartGld);
				pbDecBuf = (UCHAR *) (u4CBase + u4Start);
...
/* VDEC_INFO_VP8_FRM_HDR_T *prVDecVp8FrmHdr = &_rVDecVp8FrmHdr[u4InstID]; */
	...
	u4VldByte =
	    u4VDEC_HAL_VP8_ReadRdPtr(0, u4InstID, (UINT32) _pucVFifo[u4InstID], &u4VldBit) - 4;

#if VDEC_VER_COMPARE_CRC
	vVP8_CheckCRCResult(u4InstID);
#else
	vVP8WrData2PC(u4InstID, _pucDumpYBuf[u4InstID],
		      ((((_rVDecVp8FrmHdr[u4InstID].u4Width +
			  15) >> 4) * ((_rVDecVp8FrmHdr[u4InstID].u4Height + 31) >> 5)) << 9),
		      &fgNextFrameExist);
#endif
	...
	if (_u4ReDecCnt[u4InstID] > 0) {
		...
		_rVDecVp8FrmHdr[u4InstID].u4VldStartPos =
		    (UINT32) _pucVFifo[u4InstID] + _u4VP8ByteCount[u4InstID];
#ifndef RING_VFIFO_SUPPORT
...
/* _rVDecVp8FrmHdr[u4InstID].u4WritePos = (UINT32)_pucVFifo[u4InstID] + (V_FIFO_SZ*(0.5 + 0.5 *(_u4LoadBitstreamCnt[u4InstID]%2))); */
		_rVDecVp8FrmHdr[u4InstID].u4WritePos =
		    (UINT32) _pucVFifo[u4InstID] +
		    ((_u4LoadBitstreamCnt[u4InstID] % 2) ? (V_FIFO_SZ) : (V_FIFO_SZ >> 1));
#endif
...
/* u4Size3 = (UINT32*)((UINT32)_pucVFifo[u4InstID] +_u4VP8ByteCount[u4InstID] + 3); */
		...
		_u4VP8FrmSZ[u4InstID] =
		    (((u4Size0 & 0xFF)) | ((u4Size1 & 0xFF) << 8) | ((u4Size2 & 0xFF) << 16) |
		     (u4Size3 & 0xFF) << 24);
...
/* i4VDEC_HAL_VP8_InitBarrelShifter(_u4BSID[u4InstID], u4InstID, &_rVDecVp8FrmHdr[u4InstID]); */
...
#if (VDEC_VP8_ERR_TEST)
		u4Size0 =
		    *((UINT32 *) _pucSizeFileBuf[u4InstID] + (((_u4FileCnt[u4InstID] + 1) * 4)));
		u4Size1 =
		    *((UINT32 *) _pucSizeFileBuf[u4InstID] +
		      (((_u4FileCnt[u4InstID] + 1) * 4) + 1));
		u4Size2 =
		    *((UINT32 *) _pucSizeFileBuf[u4InstID] +
		      (((_u4FileCnt[u4InstID] + 1) * 4) + 2));
		u4Size3 =
		    *((UINT32 *) _pucSizeFileBuf[u4InstID] +
		      (((_u4FileCnt[u4InstID] + 1) * 4) + 3));
		u4BsSize =
		    (((u4Size3 & 0xFF) << 24) | ((u4Size2 & 0xFF) << 16) | ((u4Size1 & 0xFF) << 8) |
		     (u4Size0 & 0xFF));
		...
		if ((_u4LoadBitstreamCnt[u4InstID] & 0x1)
		    && (_rVDecVp8FrmHdr[u4InstID].u4ReadPointer >
...
#ifdef SATA_HDD_READ_SUPPORT
			if (!fgOpenHDDFile
			    (u4InstID, (char *)_bFileStr1[u4InstID][1], "r+b",
			     &_tInFileInfo[u4InstID])) {
				fgOpenPCFile(u4InstID, (char *)_bFileStr1[u4InstID][1], "r+b",
					     &_tInFileInfo[u4InstID]);
			}
#else
			fgOpenPCFile(u4InstID, (char *)_bFileStr1[u4InstID][1], "r+b",
			...
			_u4LoadBitstreamCnt[u4InstID]++;
			   ...
			   && (_rVDecVp8FrmHdr[u4InstID].u4ReadPointer <
			       ((UINT32) _pucVFifo[u4InstID] + (V_FIFO_SZ / 2)))) {
			_tInFileInfo[u4InstID].fgGetFileInfo = TRUE;
			_tInFileInfo[u4InstID].pucTargetAddr =
			    _pucVFifo[u4InstID] + (V_FIFO_SZ / 2);
			_tInFileInfo[u4InstID].u4FileOffset =
			    ((V_FIFO_SZ * (_u4LoadBitstreamCnt[u4InstID] + 1)) / 2);
			_tInFileInfo[u4InstID].u4TargetSz = (V_FIFO_SZ / 2);
			_tInFileInfo[u4InstID].u4FileLength = 0;
...
static VOID vVDEC_Vp8DecFinish(VDEC_INFO_VP8_FRM_HDR_T *pVp8DecInfo)
{
	...
	if (!VDEC_FLGSET(prVp8DecParam->u4FlagParam, VP8PARAM_REFRESH_PROBS)) {
		memcpy(&prVp8DecParam->rCurFc, &prVp8DecParam->rLastFC,
		       sizeof(VDEC_VP8FRAME_CONTEXT_T));
...
void vVerVP8DecEndProc(UINT32 u4InstID)
{
...

	{
		while (u4CntTimeChk < DEC_RETRY_NUM) {
			...
			if ((u4Cnt & 0x3f) == 0x3f) {
				...
				if (fgIsVP8VDecComplete(u4InstID)) {
					...
					vVDEC_HAL_VP8_GetMbxMby(u4InstID, &u4MbX, &u4MbY);
					printk
					    ("u4CntTimeChk is %d, u4MbX = 0x%x, u4MbY = 0x%x, Misc41 value is 0x%lx\n",
					     u4CntTimeChk, u4MbX, u4MbY, u4VDecReadDV(u4InstID,
										      41 * 4));
					msleep(10);
					if ((u4MbX == u4MbX_last) && (u4MbY == u4MbY_last)) {
						u4CntTimeChk++;
		...
		dma_unmap_single(NULL, (UINT32) _pu1VP8VDecYAddrPhy, GOLD_Y_SZ + GOLD_C_SZ,
				 DMA_FROM_DEVICE);
		dma_unmap_single(NULL, (UINT32) _pu1VP8VDecYAltPhy, GOLD_Y_SZ + GOLD_C_SZ,
				 DMA_TO_DEVICE);
		dma_unmap_single(NULL, (UINT32) _pu1VP8VDecYCurPhy, GOLD_Y_SZ + GOLD_C_SZ,
				 DMA_TO_DEVICE);
		dma_unmap_single(NULL, (UINT32) _pu1VP8VDecYGldPhy, GOLD_Y_SZ + GOLD_C_SZ,
				 DMA_TO_DEVICE);
#endif
		...
		if ((u4CntTimeChk == DEC_RETRY_NUM) || (u4VP8ErrType != 0))	/* || (rVp8ErrInfo.u4Vp8ErrCnt != 0)) */
		{
			...
			fgWrMsg2PC((void *)strMessage, strlen(strMessage), 8,
				   &_tFileListRecInfo[u4InstID]);
			sprintf(strMessage, "the length is %d (0x%.8x)\n",
