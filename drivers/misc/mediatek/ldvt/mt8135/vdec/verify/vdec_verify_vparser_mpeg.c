...
UINT32 u4VParserMPEG12(UINT32 u4InstID, BOOL fgInquiry)
{
	...
	do {
		...
		if ((_u4Datain[u4InstID] != SEQUENCE_HEADER_CODE)
		    && (_u4Datain[u4InstID] != GROUP_START_CODE)
	...
	if (_u4Datain[u4InstID] == SEQUENCE_HEADER_CODE) {
		...
		do {
			...
			if (!fgInquiry) {
				...
				if (!fgVerifyMpvPicDimChk
				    (u4InstID, _u4HSize[u4InstID], _u4VSize[u4InstID], TRUE)) {
					return (PIC_DIM_ERR);
				}
...
INT32 i4ExtUsrData(UINT32 u4InstID, BYTE bIndex)	/* extension_and_user_data(BYTE i) */
{
	...
	while ((_u4Datain[u4InstID] == EXTENSION_START_CODE)
	       || (_u4Datain[u4InstID] == USER_DATA_START_CODE)) {
		if (_u4Datain[u4InstID] == EXTENSION_START_CODE) {
			if (bIndex != 1) {
...
#if 0
	if (((_u4Datain[u4InstID] >> 16) == LINE21_INDICATOR) && (bIdx == 1) && fgCCOn()) {
...
INT32 i4QuantMatrixExt(UINT32 u4InstID)
{
	...
	if (_fgVerLoadIntraMatrix[u4InstID] == TRUE) {
		if (_u4BSID[u4InstID] == 1) {
			...
			u4Bytes =
			    u4VDEC_HAL_MPEG_ReadRdPtr(1, u4InstID, (UINT32) _pucVFifo[u4InstID],
						      &u4Bits);
...
/* rMpegBSInitPrm.u4WritePointer = (UINT32)_pucVFifo[u4InstID] + (V_FIFO_SZ*(0.5 + 0.5 *(_u4LoadBitstreamCnt[u4InstID]%2))); */
		...
		vVDEC_HAL_MPEG12_FW_LoadQuantMatrix(u4InstID, TRUE,
	...
	if (_fgVerLoadNonIntraMatrix[u4InstID] == TRUE) {
		...
		vVDEC_HAL_MPEG12_FW_LoadQuantMatrix(u4InstID, FALSE,
						    (UINT32 *) (rQuantMatrix[u4InstID].
								ucQuantMatrix));
#else
...
INT32 i4PictureDisExt(UINT32 u4InstID)	/* picture_display_extension() */
{
	...
	for (i = 0; i < bNumFrmCenterOfst; i++) {
		...
		_u4Datain[u4InstID] =
		    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 17);
...
INT32 i4PicHdr(UINT32 u4InstID)
{
	...
	if ((_u4PicCdTp[u4InstID] == B_TYPE) && _u4BPicIniFlag[u4InstID] == 0
	    && _u4BPicIniFlag0[u4InstID] == 0) {
	...
	if ((_u4PicCdTp[u4InstID] == P_TYPE) || (_u4PicCdTp[u4InstID] == B_TYPE)) {
		...
		_u4Datain[u4InstID] =
		    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 4);
	}
...
/* fprintf(outfile, "Picture_Coding_Extension_ID ERROR !!!"); */
	...
	if ((_pucfcode[u4InstID][0][0] == 0)
	    || ((_pucfcode[u4InstID][0][0] >= 10) && (_pucfcode[u4InstID][0][0] < 15))) {
#if (F_CODE_00_ERR)
	...
	if ((_pucfcode[u4InstID][0][1] == 0)
	    || ((_pucfcode[u4InstID][0][1] >= 10) && (_pucfcode[u4InstID][0][1] < 15))) {
#if (F_CODE_01_ERR)
	...
	if ((_pucfcode[u4InstID][1][0] == 0)
	    || ((_pucfcode[u4InstID][1][0] >= 10) && (_pucfcode[u4InstID][1][0] < 15))) {
#if (F_CODE_10_ERR)
	...
	if ((_pucfcode[u4InstID][1][1] == 0)
	    || ((_pucfcode[u4InstID][1][1] >= 10) && (_pucfcode[u4InstID][1][1] < 15))) {
#if (F_CODE_11_ERR)
	...
	if (fgCompositeDis) {
		...
		_u4Datain[u4InstID] =
		    u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 22);
		if (fgMPEGNextStartCode(u4InstID) == FALSE) {
...
void vVerifyMp2SetPicLayerDecPrm(UINT32 u4InstID)
{
	VDEC_INFO_MPEG2_PIC_PRM_T *ptPicPrm =
	    &_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecMPEGDecPrm.rPicLayer.rMp2PicPrm;
...
BOOL fgMPEGNextStartCode(UINT32 u4InstID)
{
	UINT32 u4Retry = 0;
	UINT32 u4NextStart;
	vVDEC_HAL_MPEG_AlignRdPtr(_u4BSID[u4InstID], u4InstID, (UINT32) _pucVFifo[u4InstID],
				  BYTE_ALIGN);
	...
	while ((u4NextStart >> 8) != 1) {
		if (u4InstID == 0) {
			vVDecWriteVLD(u4InstID, RW_VLD_PROC,
				      u4VDecReadVLD(u4InstID, RW_VLD_PROC) | VLD_SSCBYTE);
		}
		u4NextStart = u4VDEC_HAL_MPEG_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 0);
		if (++u4Retry > 2) {
			return FALSE;
