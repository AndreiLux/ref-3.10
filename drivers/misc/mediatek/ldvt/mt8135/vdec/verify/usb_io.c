...
int vdecreadFile(int fp, char *buf, int readlen)
{
	...
	if (op_res != sizeof(int))
		printk("[READ] Command FILE_READ send fail %d\n", op_res);

	/* Send file descriptor index */
	opRes = fd->f_op->write(fd, (void *)&fp, sizeof(int), &fd->f_pos);
	if (op_res != sizeof(int))
		printk("[READ] File descriptor index send fail %d\n", op_res);
	...
	num = fd->f_op->write(fd, (const void *)&cmd, sizeof(int), &fd->f_pos);
	if (num != sizeof(int))
		printk("[USBIO][READ] E1\n");
	num = fd->f_op->read(fd, (void *)&ack, sizeof(int), &fd->f_pos);
	if (num != sizeof(int) || ack != ACK_GET_PARAM_NUM)
		printk("[USBIO][READ] E1-A\n");

	/* Send int 2 */
	cmd = 2;
	num = fd->f_op->write(fd, (const void *)&cmd, sizeof(int), &fd->f_pos);
	if (num != sizeof(int))
		printk("[USBIO][READ] E2\n");
	num = fd->f_op->read(fd, (void *)&ack, sizeof(int), &fd->f_pos);
	if (num != sizeof(int) || ack != ACK_GET_PARAM)
		printk("[USBIO][READ] E2-A\n");

	/* Send int fp */
	num = fd->f_op->write(fd, (const void *)&fp, sizeof(int), &fd->f_pos);
	if (num != sizeof(int))
		printk("[USBIO][READ] E3\n");
	num = fd->f_op->read(fd, (void *)&ack, sizeof(int), &fd->f_pos);
	if (num != sizeof(int) || ack != ACK_GET_PARAM)
		printk("[USBIO][READ] E3-A\n");
	...
	num = fd->f_op->write(fd, (const void *)&ack, sizeof(int), &fd->f_pos);
	if (num != sizeof(int))
		printk("[USBIO][READ] E5\n");
	num = fd->f_op->read(fd, (void *)&cmd, sizeof(int), &fd->f_pos);
	if (num != sizeof(int) || cmd != 2)
		printk("[USBIO][READ] E5-A\n");

	ack = ACK_GET_PARAM;
	num = fd->f_op->write(fd, (const void *)&ack, sizeof(int), &fd->f_pos);
	if (num != sizeof(int))
		printk("[USBIO][READ] E6\n");

	while (bytesRead < readlen) {
		/* Get bytes to read */
		num = fd->f_op->read(fd, (void *)&bytesToRead, sizeof(int), &fd->f_pos);
		if (num != sizeof(int) || bytesToRead < 0)
			printk("[USBIO][READ] E6-A\n");
		if (0 == bytesToRead) {
			ack = ACK_CMD_COMPLETE;
			num = fd->f_op->write(fd, (const void *)&ack, sizeof(int), &fd->f_pos);
			if (num != sizeof(int))
				printk("[USBIO][READ] E6-B\n");
			break;
		}
		/* Get data */
		ack = ACK_GET_PARAM;
		...
		if (bytesRead < readlen) {
			...
			num = fd->f_op->write(fd, (const void *)&ack, sizeof(int), &fd->f_pos);
			if (num != sizeof(int))
				printk("[USBIO][READ] E8-A\n");
		} else {
			ack = ACK_CMD_COMPLETE;
			num = fd->f_op->write(fd, (const void *)&ack, sizeof(int), &fd->f_pos);
			if (num != sizeof(int))
				printk("[USBIO][READ] E8-B\n");
		}
	}
...
int vdecwriteFile(int fp, char *buf, int writelen)
{
	...
	num = fd->f_op->write(fd, (const void *)&cmd, sizeof(int), &fd->f_pos);
	if (num != sizeof(int))
		printk("[USBIO][WRITE] E1\n");
	num = fd->f_op->read(fd, (void *)&ack, sizeof(int), &fd->f_pos);
	if (num != sizeof(int) || ack != ACK_GET_PARAM_NUM)
		printk("[USBIO][WRITE] E1-A\n");

	/* Send int 4 */
	cmd = 4;
	num = fd->f_op->write(fd, (const void *)&cmd, sizeof(int), &fd->f_pos);
	if (num != sizeof(int))
		printk("[USBIO][WRITE] E2\n");
	num = fd->f_op->read(fd, (void *)&ack, sizeof(int), &fd->f_pos);
	if (num != sizeof(int) || ack != ACK_GET_PARAM)
		printk("[USBIO][WRITE] E2-A\n");

	/* Send int fp */
	num = fd->f_op->write(fd, (const void *)&fp, sizeof(int), &fd->f_pos);
	if (num != sizeof(int))
		printk("[USBIO][WRITE] E3\n");
	num = fd->f_op->read(fd, (void *)&ack, sizeof(int), &fd->f_pos);
	if (num != sizeof(int) || ack != ACK_GET_PARAM)
		printk("[USBIO][WRITE] E3-A\n");

	/* Send int writelen */
	num = fd->f_op->write(fd, (const void *)&writelen, sizeof(int), &fd->f_pos);
	if (num != sizeof(int))
		printk("[USBIO][WRITE] E4\n");
	num = fd->f_op->read(fd, (void *)&ack, sizeof(int), &fd->f_pos);
	if (num != sizeof(int) || ack != ACK_GET_PARAM)
		printk("[USBIO][WRITE] E4-A\n");


	bytesToWriteTotal = writelen;
	while (bytesToWriteTotal > 0) {
		bytesToWrite =
		    (bytesToWriteTotal > SERIAL_BUFFER_SZ) ? SERIAL_BUFFER_SZ : bytesToWriteTotal;

		/* Send int bytesToWrite */
		num = fd->f_op->write(fd, (const void *)&bytesToWrite, sizeof(int), &fd->f_pos);
		if (num != sizeof(int))
			printk("[USBIO][WRITE] E5\n");
		num = fd->f_op->read(fd, (void *)&ack, sizeof(int), &fd->f_pos);
		if (num != sizeof(int) || ack != ACK_GET_PARAM)
			printk("[USBIO][WRITE] E5-A\n");
		...
		if (bytesWrite != bytesToWrite)
			printk("[USBIO][WRITE] E6\n");
		num = fd->f_op->read(fd, (void *)&ack, sizeof(int), &fd->f_pos);
		if (num != sizeof(int))
			printk("[USBIO][WRITE] E6-A\n");

		bufCur += bytesWrite;
		bytesToWriteTotal -= bytesWrite;

		if (bytesToWriteTotal > 0) {
			if (ack != ACK_CMD_CONTINUE)
				printk("[USBIO][WRITE] E6-B\n");
			if (ack == ACK_CMD_COMPLETE)
				break;
		} else {
			if (ack != ACK_CMD_COMPLETE)
				printk("[USBIO][WRITE] E6-C\n");
		}
	}

	num = fd->f_op->read(fd, (void *)&ack, sizeof(int), &fd->f_pos);
	if (num != sizeof(int))
		printk("[USBIO][WRITE] E7\n");
	if (ack != 0) {
		/* set_fs(vdecoldfs); */
		return -1;
	}
...
int vdeccloseFile(int fp)
{
	...
	num = fd->f_op->write(fd, (const void *)&cmd, sizeof(int), &fd->f_pos);
	if (num != sizeof(int))
		printk("[USBIO][CLOSE] E1\n");
	num = fd->f_op->read(fd, (void *)&ack, sizeof(int), &fd->f_pos);
	if (num != sizeof(int) || ack != ACK_GET_PARAM_NUM)
		printk("[USBIO][CLOSE] E1-A\n");

	cmd = 1;
	num = fd->f_op->write(fd, (const void *)&cmd, sizeof(int), &fd->f_pos);
	if (num != sizeof(int))
		printk("[USBIO][CLOSE] E2\n");
	num = fd->f_op->read(fd, (void *)&ack, sizeof(int), &fd->f_pos);
	if (num != sizeof(int) || ack != ACK_GET_PARAM)
		printk("[USBIO][CLOSE] E2-A\n");

	num = fd->f_op->write(fd, (const void *)&fp, sizeof(int), &fd->f_pos);
	if (num != sizeof(int))
		printk("[USBIO][CLOSE] E3\n");
	num = fd->f_op->read(fd, (void *)&ack, sizeof(int), &fd->f_pos);
	if (num != sizeof(int) || ack != ACK_CMD_COMPLETE)
		printk("[USBIO][CLOSE] E3-A\n");

	num = fd->f_op->read(fd, (void *)&ack, sizeof(int), &fd->f_pos);
	if (num != sizeof(int))
		printk("[USBIO][CLOSE] E3-B\n");
	if (ack == -1) {
		num = fd->f_op->read(fd, (void *)&ack, sizeof(int), &fd->f_pos);
		/* set_fs(vdecoldfs); */
		return -1;
...
/* UTIL_Printf(" i4Fd(%x)\n", i4Fd); */
	...
	*pu4TotalFileLength = u4FileSize;
	...
	/* UTIL_Printf(" pvAddr (0x%lx), u4ReadSize(%d), filesize(%d), i4Fd is 0x%lx\n",pvAddr, u4ReadSize, u4FileSize, i4Fd); */
...
BOOL fgHDDFsWriteFile(CHAR *strFileName, void *pvAddr, UINT32 u4Length)
{
	...
	printk(" ===> fgHDDFsWriteFile %s, len:0x%x <===\n", strFileName, u4Length);
