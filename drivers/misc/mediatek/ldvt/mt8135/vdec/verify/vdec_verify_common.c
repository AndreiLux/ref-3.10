...
UCHAR ucVDecGetMinPOCFBuf(UINT32 u4InstID, VDEC_INFO_DEC_PRM_T * tVerMpvDecPrm, BOOL fgWithEmpty)
{
	...
	for (u4Idx = 0; u4Idx < _tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecH264DecPrm.ucMaxFBufNum;
	     u4Idx++) {
		...
		if (fgWithEmpty) {
			if ((i4MinPOC != 0x80000001)
			    && (_ptFBufInfo[u4InstID][u4Idx].eH264DpbStatus ==
				H264_DPB_STATUS_EMPTY)) {
				i4MinPOC = 0x80000001;
				u4MinPOCFBufIdx = u4Idx;
			} else if ((i4MinPOC > _ptFBufInfo[u4InstID][u4Idx].i4POC)
				   && (_ptFBufInfo[u4InstID][u4Idx].eH264DpbStatus ==
...
#if 0
		if (fgWithEmpty && (i4MinPOC != 0x80000001)
