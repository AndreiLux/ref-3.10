...
void vRprImgResz(UINT32 u4InstID,
		 ...
		 UINT32 u4CurrPicDispWidth, UINT32 u4CurrPicDispHeight)
{
	...
	if (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.fgRPRRacingModeEnable) {
		vRM_VDecWrite_ImgResz(0, RZ_SRC_Y_OW_ADDR_BASE1 * 4,
				      PHYSICAL(prParsingPic->u4OutBufY) / 16);
		vRM_VDecWrite_ImgResz(0, RZ_SRC_CB_OW_ADDR_BASE1 * 4,
				      PHYSICAL(prParsingPic->u4OutBufC) / 16);
	} else {
		vRM_VDecWrite_ImgResz(0, RZ_SRC_Y_OW_ADDR_BASE1 * 4,
				      (UINT32) (PHYSICAL(prParsingPic->u4FwdBufY) / 16));
		vRM_VDecWrite_ImgResz(0, RZ_SRC_CB_OW_ADDR_BASE1 * 4,
				      (UINT32) (PHYSICAL(prParsingPic->u4FwdBufC) / 16));
	}

	vRM_VDecWrite_ImgResz(0, RZ_TG_Y_OW_ADDR_BASE * 4,
			      (UINT32) ((UINT32) PHYSICAL((UINT32) _pucPic3Y[u4InstID]) / 16));
	vRM_VDecWrite_ImgResz(0, RZ_TG_C_OW_ADDR_BASE * 4,
			      (UINT32) ((UINT32) PHYSICAL((UINT32) _pucPic3C[u4InstID]) / 16));

	vRM_VDecWrite_ImgResz(0, RZ_TMP_Y_OW_ADDR_BASE * 4,
			      ((UINT32) PHYSICAL((UINT32) _pucRMReszWorkBuf[u4InstID])) / 16);
...
/* qw = (u4CurrPicDecWidth) << 12 | u4CurrPicDecHeight; */
...
#ifdef RM_ADDRSWAP_ENABLE
	{
		...
		u4AddressSwapMode =
		    auImgRszAddrSwapMapTable[_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecRMDecPrm.
					     u4RMAddrSwapMode];
		...
		u4MIFMODRegValue =
		    u4MIFMODRegValue | (u4AddressSwapMode << 4) | (u4AddressSwapMode << 0);
