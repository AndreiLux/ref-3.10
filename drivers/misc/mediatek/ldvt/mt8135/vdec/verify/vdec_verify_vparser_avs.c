...
INT32 i4VDECVerAvsPicHdr(UINT32 u4BsId, UINT32 u4InstID, VDEC_INFO_AVS_SEQ_HDR_T *prVDecAvsSeqHdr,
			 VDEC_INFO_AVS_PIC_HDR_T *prVDecAvsPicHdr);
...
void vVerInitAVS(UINT32 u4InstID)
{
	...
	VDEC_INFO_AVS_DEC_PRM_T *prVDecAvsDecPrm =
	    &(_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecAVSDecPrm);
	UINT32 i;
	...
	prVDecAvsDecPrm->arFBufInfo[AVS_PREV_FW_IDX].u4CAddrOffset =
	    ((UINT32) _pucPic3C[u4InstID] - (UINT32) _pucPic3Y[u4InstID]);
	prVDecAvsDecPrm->arFBufInfo[AVS_FW_REF_IDX].u4YAddr = (UINT32) _pucPic2Y[u4InstID];
	prVDecAvsDecPrm->arFBufInfo[AVS_FW_REF_IDX].u4CAddrOffset =
	    ((UINT32) _pucPic2C[u4InstID] - (UINT32) _pucPic2Y[u4InstID]);
	prVDecAvsDecPrm->arFBufInfo[AVS_BW_REF_IDX].u4YAddr = (UINT32) _pucPic1Y[u4InstID];
	prVDecAvsDecPrm->arFBufInfo[AVS_BW_REF_IDX].u4CAddrOffset =
	    ((UINT32) _pucPic1C[u4InstID] - (UINT32) _pucPic1Y[u4InstID]);
	prVDecAvsDecPrm->arFBufInfo[AVS_CURR_WORK_IDX].u4YAddr = (UINT32) _pucPic0Y[u4InstID];
	prVDecAvsDecPrm->arFBufInfo[AVS_CURR_WORK_IDX].u4CAddrOffset =
	    ((UINT32) _pucPic0C[u4InstID] - (UINT32) _pucPic0Y[u4InstID]);
...
UINT32 u4VerVParserAVS(UINT32 u4InstID, BOOL fgInquiry)
{
	...
	do {
		do {
			...
			_u4Datain[u4InstID] =
			    u4VDEC_HAL_AVS_ShiftGetBitStream(_u4BSID[u4InstID], u4InstID, 0);
			if ((_u4Datain[u4InstID] != AVS_SEQ_HDR_SC)
			    && (_u4Datain[u4InstID] != AVS_I_PICTURE_SC)
			    && (_u4Datain[u4InstID] != AVS_PB_PICTURE_SC)) {
				/* Keep search next start code */
				if (prVDecAvsPicHdr->fgSecField
				    && _u4Datain[u4InstID] >= AVS_SLICE_SC_MIN
				    && _u4Datain[u4InstID] <= AVS_SLICE_SC_MAX) {
					printk("AVS_2nd Field Find Slice Start Code\n");
					return (i4RetVal);
				} else {
					_u4Datain[u4InstID] =
					    u4VDEC_HAL_AVS_ShiftGetBitStream(_u4BSID[u4InstID],
									     u4InstID, 8);
				}
			} else {
				if (prVDecAvsPicHdr->fgSecField
...
INT32 i4VDECVerAvsSeqHdr(UINT32 u4BsId, UINT32 u4InstID, VDEC_INFO_AVS_SEQ_HDR_T *prVDecAvsSeqHdr)
{
	...
	if ((prVDecAvsSeqHdr->u4HSize == 0) || (prVDecAvsSeqHdr->u4VSize == 0)) {
		...
		prVDecAvsSeqHdr->u2WidthDec =
		    (UINT16) (16 * ((prVDecAvsSeqHdr->u4HSize + 15) / 16));
		if (prVDecAvsSeqHdr->u4IsProgSeq == 1) {
			prVDecAvsSeqHdr->u2HeightDec =
			    (UINT16) (16 * ((prVDecAvsSeqHdr->u4VSize + 15) / 16));
		} else {
			prVDecAvsSeqHdr->u2HeightDec =
			    (UINT16) (32 * ((prVDecAvsSeqHdr->u4VSize + 31) / 32));
		}
			 ...
			 VDEC_INFO_AVS_PIC_HDR_T *prVDecAvsPicHdr)
{
	...
	if (!prVDecAvsPicHdr->fgIsIPic) {
		if (!
		    ((prVDecAvsPicHdr->u4PicCodingType == B_PIC)
		     && (prVDecAvsPicHdr->u4PicStruct == FRM_PIC)
	...
	if ((!prVDecAvsPicHdr->u4ProgFrm && !prVDecAvsPicHdr->u4PicStruct)
	    || (!prVDecAvsPicHdr->fgIsIPic)) {
		...
		if (prVDecAvsPicHdr->u4LoopFilterParamFg) {
			...
			if (u4Temp & 0x10000) {
				...
				prVDecAvsPicHdr->i4AlphaCOffset =
				    (INT32) ((((UINT32) (prVDecAvsPicHdr->i4AlphaCOffset) ^
					       (0xFFFFFFFF)) + 1) & 0xFFFF) * (-1);
			} else {
				prVDecAvsPicHdr->i4AlphaCOffset = (INT32) (u4Temp & 0xFFFF);
			}
			...
			if (u4Temp & 0x10000) {
				...
				prVDecAvsPicHdr->i4BetaOffset =
				    (INT32) ((((UINT32) (prVDecAvsPicHdr->i4BetaOffset) ^
					       (0xFFFFFFFF)) + 1) & 0xFFFF) * (-1);
			} else {
				prVDecAvsPicHdr->i4BetaOffset = (INT32) (u4Temp & 0xFFFF);
			}
...
void vVerifyAvsPrepareFBufInfo(UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *prDecPrm)
{
	...
	if (prVDecAvsDecPrm->prPicHdr->u4PicCodingType == I_PIC) {
		/* Decode Buffer */
		prVDecAvsDecPrm->rAvsFrameBufSa.u4Pic0YSa =
		    prVDecAvsDecPrm->arFBufInfo[AVS_CURR_WORK_IDX].u4YAddr;
		prVDecAvsDecPrm->rAvsFrameBufSa.u4Pic0CSa =
		    prVDecAvsDecPrm->arFBufInfo[AVS_CURR_WORK_IDX].u4YAddr +
		    prVDecAvsDecPrm->arFBufInfo[AVS_CURR_WORK_IDX].u4CAddrOffset;
		/* Backward Buffer */
		prVDecAvsDecPrm->rAvsFrameBufSa.u4Pic1YSa =
		    prVDecAvsDecPrm->arFBufInfo[AVS_BW_REF_IDX].u4YAddr;
		prVDecAvsDecPrm->rAvsFrameBufSa.u4Pic1CSa =
		    prVDecAvsDecPrm->arFBufInfo[AVS_BW_REF_IDX].u4YAddr +
		    prVDecAvsDecPrm->arFBufInfo[AVS_BW_REF_IDX].u4CAddrOffset;
		/* Forward Buffer */
		prVDecAvsDecPrm->rAvsFrameBufSa.u4Pic2YSa =
		    prVDecAvsDecPrm->arFBufInfo[AVS_FW_REF_IDX].u4YAddr;
		prVDecAvsDecPrm->rAvsFrameBufSa.u4Pic2CSa =
		    prVDecAvsDecPrm->arFBufInfo[AVS_FW_REF_IDX].u4YAddr +
		    prVDecAvsDecPrm->arFBufInfo[AVS_FW_REF_IDX].u4CAddrOffset;
		...
		/* Backward Buffer */
		prVDecAvsDecPrm->rAvsFrameBufSa.u4Pic1YSa =
		    prVDecAvsDecPrm->arFBufInfo[AVS_LAST_P_IDX].u4YAddr;
		prVDecAvsDecPrm->rAvsFrameBufSa.u4Pic1CSa =
		    prVDecAvsDecPrm->arFBufInfo[AVS_LAST_P_IDX].u4YAddr +
		    prVDecAvsDecPrm->arFBufInfo[AVS_LAST_P_IDX].u4CAddrOffset;
		/* Forward Buffer */
		prVDecAvsDecPrm->rAvsFrameBufSa.u4Pic2YSa =
		    prVDecAvsDecPrm->arFBufInfo[AVS_PREV_P_IDX].u4YAddr;
		prVDecAvsDecPrm->rAvsFrameBufSa.u4Pic2CSa =
		    prVDecAvsDecPrm->arFBufInfo[AVS_PREV_P_IDX].u4YAddr +
		    prVDecAvsDecPrm->arFBufInfo[AVS_PREV_P_IDX].u4CAddrOffset;
...
void vVerifySetAvsBsInfo(UINT32 u4BSID, UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *prDecParam,
			 VDEC_INFO_AVS_BS_INIT_PRM_T rAvsBSInitPrm)
{
...
void vVerAVSUpdateBufStatus(UINT32 u4InstID)
{
	...
	if (prVDecAvsPicHdr->u4PicStruct == FRM_PIC || prVDecAvsDecPrm->prPicHdr->fgSecField) {
		if (prVDecAvsPicHdr->u4PicCodingType == I_PIC
		    || prVDecAvsPicHdr->u4PicCodingType == P_PIC) {
			/* Maintain DPB */
			memcpy(&rTempFBufInfo, &prVDecAvsDecPrm->arFBufInfo[AVS_PREV_FW_IDX],
			       sizeof(VDEC_INFO_AVS_FBUF_INFO_T));
			memcpy(&prVDecAvsDecPrm->arFBufInfo[AVS_PREV_FW_IDX],
			       &prVDecAvsDecPrm->arFBufInfo[AVS_PREV_P_IDX],
			       sizeof(VDEC_INFO_AVS_FBUF_INFO_T));
			memcpy(&prVDecAvsDecPrm->arFBufInfo[AVS_PREV_P_IDX],
			       &prVDecAvsDecPrm->arFBufInfo[AVS_LAST_P_IDX],
			       sizeof(VDEC_INFO_AVS_FBUF_INFO_T));
			memcpy(&prVDecAvsDecPrm->arFBufInfo[AVS_LAST_P_IDX],
			       &prVDecAvsDecPrm->arFBufInfo[AVS_CURR_WORK_IDX],
			       sizeof(VDEC_INFO_AVS_FBUF_INFO_T));
...
void vVerifyVDecSetAVSInfo(UINT32 u4InstID)
{
	...
	if ((prVDecAvsDecPrm->prSeqHdr->u4IsProgSeq)
	    || ((!prVDecAvsDecPrm->prSeqHdr->u4IsProgSeq)
		&& (!prVDecAvsDecPrm->prPicHdr->fgSecField))) {
	...
	prVDecAvsDecPrm->prCurrFBufInfo->u1PicCodingType =
	    prVDecAvsDecPrm->prPicHdr->u4PicCodingType;
...
/* Searh path name */
	...
	for (i = 0;; i++) {
		...
		if (_bFileStr1[u4InstID][1][i] == 'b' || _bFileStr1[u4InstID][1][i] == 'B') {
			...
			if ((_bFileStr1[u4InstID][1][i + 1] == 'a'
			     || _bFileStr1[u4InstID][1][i + 1] == 'A')
	...
	if (fgCompare) {
		...
		_tFBufFileInfo[u4InstID].u4FileLength =
		    (((prVDecAvsSeqHdr->u2WidthDec +
		       15) >> 4) << 4) * (((prVDecAvsSeqHdr->u2HeightDec + 31) >> 5) << 5);
#else
#ifdef DIRECT_DEC
...
#endif
		{
			...
			if (_tVerMpvDecPrm[u4InstID].ucAddrSwapMode != ADDRSWAP_OFF) {
				...
				vAVS_InvAddressSwap(u4InstID,
						    ...
						    (BYTE *) u4NonSwapTargY,
						    (BYTE *) u4NonSwapTargC, u4AlignWidth,
			...
			if (fgOpenFile
			    (u4InstID, _bFileStr1[u4InstID][3], "r+b",
			     &_tFBufFileInfo[u4InstID]) == FALSE) {
		...
		/* if(!fgIsMonoPic(_u4VDecID)) */
		{
...
#endif
			{
				...
				if (fgOpenFile
				    (u4InstID, _bFileStr1[u4InstID][4], "r+b",
				     &_tFBufFileInfo[u4InstID]) == FALSE) {
...
#endif
			{
				...
				for (u4Pix = 0; u4Pix < u4Width * (u4Height >> 1); u4Pix++) {
					if ((_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecVP6DecPrm.
					     fgDec2ndFld)
...
#if !VDEC_AVS_ERR_TEST
		vAVSWrData2PC(u4InstID, _pucDumpYBuf[u4InstID],
			      ((((prVDecAvsSeqHdr->u2WidthDec +
				  15) >> 4) * ((prVDecAvsSeqHdr->u2HeightDec + 31) >> 5)) << 9),
			      &fgNextFrameExist);
#else
		_u4FileCnt[u4InstID]++;

		if (_tInFileInfo[u4InstID].u4FileLength - u4VldByte < 1024
		    || (_u4FileCnt[u4InstID] >= _u4EndCompPicNum[u4InstID])) {
...
BOOL fgIsAVSVDecComplete(UINT32 u4InstID)
{
	...
	if (_fgVDecComplete[u4InstID]) {
		...
		{
			i4VDEC_HAL_Common_Gcon_Enable_After_Complete(u4InstID, VDEC_AVS);
		}
#endif
...
#if !VDEC_AVS_ERR_TEST
	if ((u4CntTimeChk == DEC_RETRY_NUM) || (u4AvsErrType != 0)
	    || (rAvsErrInfo.u4AvsErrCnt != 0)) {
#ifndef INTERGRATION_WITH_DEMUX
		/* #ifdef EXT_COMPARE */
		_fgVDecErr[u4InstID] = TRUE;
		...
		sprintf(strMessage,
			...
			u4AvsErrType, rAvsErrInfo.u4AvsErrType, rAvsErrInfo.u4AvsErrRow, u4MbX,
			prSeqHdr->u4HSize - 1, u4MbY, prSeqHdr->u4VSize - 1);
