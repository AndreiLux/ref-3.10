...
UINT32 u4CalculatePixelAddress_Y(UINT32 u4YBase,	/* /< [IN] frame buffer Y component address */
				 ...
				 UCHAR bBlockW)
{
	...
	if (fgBlock)
		u4YAdr =
		    u4YBase + ((u4FrameWidth * u4YBlockNum) << 5) +
		    ((u4YOffsetInBlock + (u4XBlockNum << 5)) << bBlockW) + u4XOffsetInBlock;
	else
...
UINT32 u4CalculatePixelAddress_C(UINT32 u4CBase,	/* /< [IN] frame buffer CbCr component address */
				 UINT32 u4XPos,	/* /< [IN] x position of pixel in frame buffer */
				 UINT32 u4YPos,	/* /< [IN] y position of pixel in frame buffer */
				 UINT32 u4FrameWidth,	/* /< [IN] Frame buffer width */
				 BOOL fgBlock,	/* /< [IN] MTK block / raster scan */
				 ...
				 UCHAR bBlockW)
{
	...
	if (fg420) {
		...
		if (fgBlock)
			u4CAdr =
			    u4CBase + ((u4FrameWidth * u4YBlockNum) << 4) +
			    ((u4YOffsetInBlock + (u4XBlockNum << 4)) << bBlockW) + u4XOffsetInBlock;
		else
		...
		if (fgBlock)
			u4CAdr =
			    u4CBase + ((u4FrameWidth * u4YBlockNum) << 5) +
			    ((u4YOffsetInBlock + (u4XBlockNum << 4)) << bBlockW) + u4XOffsetInBlock;
		else
			u4CAdr = u4CBase + (u4Y * u4FrameWidth) + (u4X);
	}
...
UINT32 u4InverseAddrSwap(UINT32 u4AddrSwapMode, UINT32 u4SwappedAddr, BOOL fgIsYComponent)
{
	...
	case 0x1:		/* MT8520_SWAP_MODE_1 */
		u4NonSwappedAddr =
		    ((u4SwappedAddr & 0xFFFFFFC0) | ((u4SwappedAddr & 0x20) >> 5) |
		     ((u4SwappedAddr & 0x10) >> 2) | ((u4SwappedAddr & 0x8) >> 2) |
		     ((u4SwappedAddr & 0x7) << 3));
		break;
	case 0x2:		/* MT8520_SWAP_MODE_2 */
		u4NonSwappedAddr =
		    ((u4SwappedAddr & 0xFFFFFFE0) | ((u4SwappedAddr & 0x10) >> 4) |
		     ((u4SwappedAddr & 0xF) << 1));
		break;
	case 0x4:		/* MT5351_SWAP_MODE_0 */
		if (fgIsYComponent) {
			u4TempAddr =
			    ((u4SwappedAddr & 0xFFFFFF80) | ((u4SwappedAddr & 0x40) >> 4) |
			     ((u4SwappedAddr & 0x3C) << 1) | (u4SwappedAddr & 0x3));
			u4NonSwappedAddr =
			    ((u4TempAddr & 0xFFFFFF80) | ((u4TempAddr & 0x7C) >> 2) |
			     ((u4TempAddr & 0x3) << 5));
		} else {
			u4TempAddr =
			    ((u4SwappedAddr & 0xFFFFFFC0) | ((u4SwappedAddr & 0x20) >> 3) |
			     ((u4SwappedAddr & 0x1C) << 1) | (u4SwappedAddr & 0x3));
			u4NonSwappedAddr =
			    ((u4TempAddr & 0xFFFFFFC0) | ((u4TempAddr & 0x3C) >> 2) |
			     ((u4TempAddr & 0x3) << 4));
		}
		break;
	case 0x5:		/* MT5351_SWAP_MODE_1 */
		if (fgIsYComponent) {
			u4TempAddr =
			    ((u4SwappedAddr & 0xFFFFFF00) | ((~u4SwappedAddr) & 0x80) |
			     (u4SwappedAddr & 0x7F));
			u4NonSwappedAddr =
			    ((u4TempAddr & 0xFFFFFF80) | ((u4TempAddr & 0x7C) >> 2) |
			     ((u4TempAddr & 0x3) << 5));
			...
			u4TempAddr =
			    ((u4SwappedAddr & 0xFFFFFF80) | ((~u4SwappedAddr) & 0x40) |
			     (u4SwappedAddr & 0x3F));
			u4NonSwappedAddr =
			    ((u4TempAddr & 0xFFFFFFC0) | ((u4TempAddr & 0x3C) >> 2) |
			     ((u4TempAddr & 0x3) << 4));
	...
	case 0x6:		/* MT5351_SWAP_MODE_2 */
		if (fgIsYComponent) {
			u4NonSwappedAddr =
			    ((u4SwappedAddr & 0xFFFFFF80) | ((u4SwappedAddr & 0x7C) >> 2) |
			     ((u4SwappedAddr & 0x3) << 5));
		} else {
			u4NonSwappedAddr =
			    ((u4SwappedAddr & 0xFFFFFFC0) | ((u4SwappedAddr & 0x3C) >> 2) |
			     ((u4SwappedAddr & 0x3) << 4));
		}
		break;
	...
	case 0xF:		/* MT8320_FIELD_COMPACT */
		if (fgIsYComponent) {
			...
			u4NonSwappedAddr =
			    ((u4SwappedAddr & 0xFFFFFFE0) | ((u4SwappedAddr & 0x0F) << 1) |
			     ((u4SwappedAddr & 0x10) >> 4));
		} else {
			/* u4NonSwappedAddr = ((u4SwappedAddr&0xFFFFFFF0) | ((u4SwappedAddr&0x0E)>>1) | ((u4SwappedAddr&0x1)<<3)); */
			u4NonSwappedAddr =
			    ((u4SwappedAddr & 0xFFFFFFF0) | ((u4SwappedAddr & 0x07) << 1) |
			     ((u4SwappedAddr & 0x8) >> 3));
		}
		break;
#endif
...
/* #ifndef DOWN_SCALE_SUPPORT */
	...
	printk("<vdec> %s, %d\n", __func__, __LINE__);
	strncpy(_tFileListRecInfo[u4InstID].bFileName, _FileList_Rec[u4InstID],
		sizeof(_FileList_Rec[u4InstID]));
	...
	if (_u4FileCnt[u4InstID] == _u4ReDecPicNum[u4InstID]) {
		if (_u4ReDecNum[u4InstID] != 0) {
			...
			vVDecOutputDebugString
			    ("/n!!!!!!!!!!!!!! Re-Decode and Wait for debug !!!!!!!!!!!!!!!!\n");
		}
	}
	...
	if (!(
		...
		if (_u4CodecVer[u4InstID] != VDEC_MPEG2) {
			vConcateStr(_bFileStr1[u4InstID][3], _bFileStr1[u4InstID][0],
				    _bFileAddStrY[0], _u4FileCnt[u4InstID]);
		} else {
#if 1				/* 8320 */
#if VDEC_PP_ENABLE
			vConcateStr(_bFileStr1[u4InstID][3], _bFileStr1[u4InstID][0],
				    "_y_dbk_8320.out\0", _u4FileCnt[u4InstID]);
#else
			vConcateStr(_bFileStr1[u4InstID][3], _bFileStr1[u4InstID][0],
				    "_y_gold.raw\0", _u4FileCnt[u4InstID]);
#endif
#else
			vConcateStr(_bFileStr1[u4InstID][3], _bFileStr1[u4InstID][0], ".ramY\0",
				    _u4FileCnt[u4InstID]);
#endif
		}

#ifdef EXT_COMPARE
		_tFBufFileInfo[u4InstID].u4FileLength =
		    (((_u4RealHSize[u4InstID] + 15) >> 4) << 4) * (((_u4RealVSize[u4InstID] +
								     31) >> 5) << 5);
#else
#ifdef DIRECT_DEC
...
#endif
		{
			...
			if (_u4CodecVer[u4InstID] == VDEC_MPEG2) {
				if (!
				    (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecMPEGDecPrm.
				     fgDec2ndFld)) {
...
/* add flush cache here qiguo 29/4 */
			...
			if (_tVerMpvDecPrm[u4InstID].ucAddrSwapMode != 4) {
				...
				if ((_tVerMpvDecPrm[u4InstID].rDownScalerPrm.ucPicStruct ==
				     TOP_FIELD)
				    || (_tVerMpvDecPrm[u4InstID].rDownScalerPrm.ucPicStruct ==
					BOTTOM_FIELD)) {
					u4AlignHeight =
					    _tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4TrgOffV +
					    (_tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4TrgHeight *
					     2);
				} else {
					u4AlignHeight =
					    _tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4TrgOffV +
				...
				vMPEG_InvAddressSwap(u4InstID,
						     (BYTE *) _pucVDSCLBuf[u4InstID],
						     (BYTE *) (_pucVDSCLBuf[u4InstID] + 0x1FE000),
						     (BYTE *) _pucDumpYBuf[u4InstID],
						     (BYTE *) _pucDumpCBuf[u4InstID], u4AlignWidth,
						     u4AlignHeight, u4AlignSize,
				...
				memcpy((UCHAR *) u4NonSwapYBase, _pucDumpYBuf[u4InstID],
				       (u4AlignWidth * u4AlignHeight) + u4AlignSize);
				memcpy((UCHAR *) u4NonSwapCBase, _pucDumpCBuf[u4InstID],
				       (u4AlignWidth * u4AlignHeight / 2) + u4AlignSize);
			...
			if (_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecMPEGDecPrm.rMpegPpInfo.
			    fgPpEnable) {
				vGenerateDownScalerGolden(u4InstID, (UINT32) _pucPpYSa[u4InstID],
							  (UINT32) (_pucPpCSa[u4InstID]), u4Size);
			} else {
				vGenerateDownScalerGolden(u4InstID, u4NonSwapYBase, u4NonSwapCBase,
							  u4Size);
			}
#else
#if (VDEC_PP_ENABLE)
			...
			fgOpenFile(u4InstID, _bFileStr1[u4InstID][3], "r+b",
				   &_tFBufFileInfo[u4InstID]);
		...
		if ((_tVerMpvDecPrm[u4InstID].rDownScalerPrm.ucPicStruct == TOP_FIELD)
		    || (_tVerMpvDecPrm[u4InstID].rDownScalerPrm.ucPicStruct == BOTTOM_FIELD)) {
			u4Height =
			    _tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4TrgOffV +
			    (_tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4TrgHeight * 2);
		} else {
			u4Height =
			    _tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4TrgOffV +
			    _tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4TrgHeight;
...
/* printk("Compare Y data start .......\n"); */
			for (mbh = 0; mbh < u4Ty1; mbh++) {
				for (mbw = 0; mbw < u4Tx1; mbw++) {
					...
					pbGoldenBuf =
					    (UCHAR *) (((UINT32) (_pucDumpYBuf[u4InstID])) +
						       u4Start);
...
/* } */
					...
					for (j = 0; j < 32; j++) {
						for (i = 0; i < 16; i++) {
							if ((_tVerMpvDecPrm[u4InstID].SpecDecPrm.
							     rVDecMPEGDecPrm.fgDec2ndFld)
							    || (_tVerMpvDecPrm[u4InstID].
								ucPicStruct == FRM_PIC)) {
								if ((mbw == u4Tx0 && i >= u4X)
								    || (mbh == u4Ty0 && j >= u4Y)) {
									/* Do not compare */
									...
									if ((*(pbDecBuf)) !=
										...
										vVDecOutputDebugString
										    ("Pic count to [%d]\n",
										     ...
										     [u4InstID]);
										vVDecOutputDebugString
										    ("Y Data Mismatch at [x=%d, y=%d] = 0x%x, Golden = 0x%x !!!\n",
										     u4XPix, u4YPix,
										     (*pbDecBuf),
										     (*pbGoldenBuf));
										sprintf(strMessage,
										...
										vWrite2PC(u4InstID, 16, (UCHAR *) &_tVerMpvDecPrm[u4InstID].rDownScalerPrm);
#endif
#endif
										break;
									}
			...
			for (u4Pix = 0; u4Pix < u4Width * u4Height; u4Pix++) {
				if ((_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecMPEGDecPrm.
				     fgDec2ndFld)
				    || (_tVerMpvDecPrm[u4InstID].ucPicStruct == FRM_PIC)) {
					...
					pbGoldenBuf =
					    (UCHAR *) ((UINT32) _pucDumpYBuf[u4InstID] + u4Pix);
					if ((*(pbDecBuf)) != (*(pbGoldenBuf))) {
						...
						vVDecOutputDebugString
						    ("Y Data Mismatch at [%d] = 0x%.2x, Golden = 0x%.2x !!!\n",
						     u4Pix, (*pbDecBuf), (*pbGoldenBuf));
						sprintf(strMessage,
							"Y Data Mismatch at [%d] = 0x%.2x, Golden = 0x%.2x !!!\n",
							u4Pix, (*pbDecBuf), (*pbGoldenBuf));
						fgWrMsg2PC(strMessage, strlen(strMessage), 8,
							   &_tFileListRecInfo[u4InstID]);
						...
						_tFBufFileInfo[u4InstID].u4FileLength =
						    (((_u4RealHSize[u4InstID] +
						       15) >> 4) << 4) * (((_u4RealVSize[u4InstID] +
									    32) >> 5) << 5);
						vWrite2PC(u4InstID, 2, _pucDecWorkBuf[u4InstID]);
#if defined(DOWN_SCALE_SUPPORT)
						_tFBufFileInfo[u4InstID].u4FileLength =
						    (((u4Width + 15) >> 4) << 4) * (((u4Height +
										      32) >> 5) <<
										    5);
		...
		/* if(!fgIsMonoPic(_u4VDecID)) */
		{
			...
			_tFBufFileInfo[u4InstID].u4FileLength = 0;
			if (_u4CodecVer[u4InstID] != VDEC_MPEG2) {
				vConcateStr(_bFileStr1[u4InstID][4], _bFileStr1[u4InstID][0],
					    _bFileAddStrC[0], _u4FileCnt[u4InstID]);
			} else {
#if 1				/* 8320 */
#if VDEC_PP_ENABLE
				vConcateStr(_bFileStr1[u4InstID][4], _bFileStr1[u4InstID][0],
					    "_c_dbk_8320.out\0", _u4FileCnt[u4InstID]);
#else
				vConcateStr(_bFileStr1[u4InstID][4], _bFileStr1[u4InstID][0],
					    "_c_gold.raw\0", _u4FileCnt[u4InstID]);
#endif
#else
				vConcateStr(_bFileStr1[u4InstID][4], _bFileStr1[u4InstID][0],
					    ".ramCbCr\0", _u4FileCnt[u4InstID]);
#endif
			}
#ifdef EXT_COMPARE
			_tFBufFileInfo[u4InstID].u4FileLength =
			    (((_u4RealHSize[u4InstID] +
			       15) >> 4) << 4) * (((_u4RealVSize[u4InstID] + 31) >> 5) << 5) >> 1;
#else
#ifdef DIRECT_DEC
			if ((_u4FileCnt[u4InstID] >= _u4StartCompPicNum[u4InstID])
			    && (_u4FileCnt[u4InstID] <= _u4EndCompPicNum[u4InstID]))
			...
			{
#ifndef DOWN_SCALE_SUPPORT
				fgOpenFile(u4InstID, _bFileStr1[u4InstID][4], "r+b",
					   &_tFBufFileInfo[u4InstID]);
...
/* printk("Compare CbCr data start .......\n"); */
				for (mbh = 0; mbh < u4Ty1; mbh++) {
					for (mbw = 0; mbw < u4Tx1; mbw++) {
						...
						pbGoldenBuf =
						    (UCHAR *) (((UINT32) (_pucDumpCBuf[u4InstID])) +
							       u4Start);
						...
						for (j = 0; j < 16; j++) {
							for (i = 0; i < 8; i++) {
								if ((_tVerMpvDecPrm[u4InstID].
								     SpecDecPrm.rVDecMPEGDecPrm.
								     fgDec2ndFld)
								    || (_tVerMpvDecPrm[u4InstID].
									ucPicStruct == FRM_PIC)) {
									if ((mbw == u4Tx0
										     ...
										     (pbGoldenBuf)))
										{
											...
											u4Cnt++;
											vVDecOutputDebugString
											    ("Cb Data Mismatch at [x=%d, y=%d] = 0x%x, Golden = 0x%x !!!\n",
											     u4XPix,
											     u4YPix,
											     (*pbDecBuf),
											     (*pbGoldenBuf));
											sprintf
											    (strMessage,
											     "<vdec> Cb Data Mismatch at [x=%d, y=%d] = 0x%x, Golden = 0x%x !!!\n",
											     u4XPix,
											     u4YPix,
											     (*pbDecBuf),
											     (*pbGoldenBuf));
											...
											_tFBufFileInfo
											    ...
											    u4FileLength
											    =
											    ((((_tVerPic[u4InstID].u4W + 15) >> 4) << 4) * (((_tVerPic[u4InstID].u4H + 32) >> 5) << 5)) / 2;
											vWrite2PC
											    (u4InstID,
											     3,
											     _pucDecCWorkBuf
											...
											_tFBufFileInfo
											    ...
											    u4FileLength
											    =
											    ((((u4Width + 15) >> 4) << 4) * (((u4Height + 32) >> 5) << 5)) / (2 - _tVerMpvDecPrm[u4InstID].rDownScalerPrm.ucVdoFmt);
											vWrite2PC
											    (u4InstID,
											     6,
											     _pucVDSCLBuf
										     ...
										     (pbGoldenBuf)))
										{
											...
											u4Cnt++;
											vVDecOutputDebugString
											    ("Cr Data Mismatch at [x=%d, y=%d] = 0x%x, Golden = 0x%x !!!\n",
											     u4XPix,
											     u4YPix,
											     (*pbDecBuf),
											     (*pbGoldenBuf));
					...
					if ((_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecMPEGDecPrm.
						...
						if ((*(pbDecBuf)) != (*(pbGoldenBuf))) {
...
#endif
							{
								u4Cnt++;
								vVDecOutputDebugString
								    ("C Data Mismatch at [%d] = 0x%.2x, Golden = 0x%.2x !!!\n",
								     u4Pix, (*pbDecBuf),
								     (*pbGoldenBuf));
								sprintf(strMessage,
									"C Data Mismatch at [%d] = 0x%.2x, Golden = 0x%.2x !!!\n",
								...
								/* vWrite2PC(_u4VDecID, 2, _pucDecWorkBuf); */
								vWrite2PC(u4InstID, 12, _pucDecWorkBuf[u4InstID]);	/* dump check sum */
								vWrite2PC(u4InstID, 1,
									  _pucVFifo[u4InstID]);
								_tFBufFileInfo[u4InstID].
								    u4FileLength =
								    ((((_u4RealHSize[u4InstID] +
									15) >> 4) << 4) *
								     (((_u4RealVSize[u4InstID] +
									32) >> 5) << 5)) / 2;
								vWrite2PC(u4InstID, 3,
									  _pucDecCWorkBuf
									  [u4InstID]);
#endif
								break;
...
#ifdef MPEG_FORCE_DUMP_OUTPUT	/* force dump */
	fgDecErr = TRUE;
	printk("PP 0x40: 0x%x\n", u4ReadReg(RM_VDEC_PP_BASE + 0x40));
#endif
...
#endif
	{
		if ((_tVerMpvDecPrm[u4InstID].SpecDecPrm.rVDecMPEGDecPrm.fgDec2ndFld)
			...
			if (fgDecErr) {
				...
				/* dump Y data to PC */
				if (_u4CodecVer[u4InstID] != VDEC_MPEG2) {
					vConcateStr(_bFileStr1[u4InstID][3],
						    _bFileStr1[u4InstID][0], _bFileAddStrY[0],
						    _u4FileCnt[u4InstID]);
				} else {
#if 1				/* 8320 */
#if VDEC_PP_ENABLE
					vConcateStr(_bFileStr1[u4InstID][3],
						    ...
						    "_y_dbk.out.8135.out\0", _u4FileCnt[u4InstID]);
#else
					vConcateStr(_bFileStr1[u4InstID][3],
						    _bFileStr1[u4InstID][0],
						    "_y_gold.raw.8135.raw\0", _u4FileCnt[u4InstID]);
#endif
#else
					vConcateStr(_bFileStr1[u4InstID][3],
						    _bFileStr1[u4InstID][0], ".ramY\0",
						    _u4FileCnt[u4InstID]);
#endif
				}

				printk("<vdec> dump pattern: %s, W:%d, H:%d\n",
				       _bFileStr1[u4InstID][3], u4Width, u4Height);
				fgWrData2PC(u4YBase, u4Width * u4Height, 7,
				...
				/* dump C data to PC */
				if (_u4CodecVer[u4InstID] != VDEC_MPEG2) {
					vConcateStr(_bFileStr1[u4InstID][4],
						    _bFileStr1[u4InstID][0], _bFileAddStrC[0],
						    _u4FileCnt[u4InstID]);
				} else {
#if 1				/* 8320 */
#if VDEC_PP_ENABLE
					vConcateStr(_bFileStr1[u4InstID][4],
						    _bFileStr1[u4InstID][0],
						    "_c_dbk.out.8135.out\0", _u4FileCnt[u4InstID]);
#else
					vConcateStr(_bFileStr1[u4InstID][4],
						    _bFileStr1[u4InstID][0],
						    "_c_gold.raw.8135.raw\0", _u4FileCnt[u4InstID]);
#endif
#else
					vConcateStr(_bFileStr1[u4InstID][4],
						    _bFileStr1[u4InstID][0], ".ramCbCr\0",
						    _u4FileCnt[u4InstID]);
#endif
				}
				printk("<vdec> dump pattern: %s\n", _bFileStr1[u4InstID][4]);
				fgWrData2PC(u4CBase, u4Width * u4Height / 2, 7,
					    _bFileStr1[u4InstID][4]);
#ifdef MPEG4_FORCE_DUMP_OUTPUT
				/* fgWrData2PC(u4CBase, u4Width*u4Height/2, 7, _bFileStr1[u4InstID][4]); */
#endif
			}
			printk("_u4FileCnt = %d, u4CompareCnt = %d, Y base = 0x%x, C base = 0x%x\n",
			       _u4FileCnt[u4InstID], u4CompareCnt, u4YBase, u4CBase);
...
#ifdef MPEG_CRC_COMPOSITE_CHECK_ENABLE
	/* only for integrated CRC bin */
	if (_u4CRCCnt[u4InstID] * 32 < _u4CRCFileSize[u4InstID]) {
		fgOpen = TRUE;
	} else {
		if ((fgDecErr == FALSE) && (_fgVDecErr[u4InstID] == FALSE)) {
			fgOpen = FALSE;	/* last frame, finish */
		} else {
			fgOpen = TRUE;	/* last frame exist error, NG */
		}
	}
#else
#if (defined(COMP_HW_CHKSUM) && (!defined(DOWN_SCALE_SUPPORT)))
	...
	vConcateStr(_bFileStr1[u4InstID][3], _bFileStr1[u4InstID][0], "_chksum.bin\0",
		    _u4FileCnt[u4InstID]);
	...
	if (_u4CodecVer[u4InstID] != VDEC_MPEG2) {
		vConcateStr(_bFileStr1[u4InstID][3], _bFileStr1[u4InstID][0], _bFileAddStrY[0],
			    _u4FileCnt[u4InstID]);
	} else {
#if 1				/* 8320 */
		vConcateStr(_bFileStr1[u4InstID][3], _bFileStr1[u4InstID][0], "_y_gold.raw\0",
			    _u4FileCnt[u4InstID]);
#else
		vConcateStr(_bFileStr1[u4InstID][3], _bFileStr1[u4InstID][0], ".ramY\0",
			    _u4FileCnt[u4InstID]);
	...
	if ((fgOpen == FALSE) || (fgDecErr == TRUE) || (_fgVDecErr[u4InstID] == TRUE)) {
		printk("fgDecErr:[%d, %d], W:%d, H:%d, MC 0x920:0x%x, PP 0x3C:0x%x\n",
		       fgDecErr, _fgVDecErr[u4InstID], _tVerPic[u4InstID].u4W,
		       _tVerPic[u4InstID].u4H, u4VDecReadMC(u4InstID, 0x920),
		       u4ReadReg(RM_VDEC_PP_BASE + 0x3C));
	}
#ifdef MPEG_FORCE_DUMP_OUTPUT
	if (fgOpen == FALSE) {
		printk("_u4WMVByteCount:0x%x, u4FileLength:0x%x\n", _u4WMVByteCount[u4InstID],
		       _tInFileInfo[u4InstID].u4FileLength);

		if ((_u4WMVByteCount[u4InstID] + 4) < _tInFileInfo[u4InstID].u4FileLength) {
			fgOpen = TRUE;	/* continue even if no YC golden data file */
		}
	}
#endif
...
#endif
	{
		...
		/* break decode */
		if (fgOpen == FALSE) {
			printk(strMessage, " Compare Finish[OK] ==> Pic count to [%d]\n",
			...
			printk(strMessage, "W:%d, H:%d, MC 0x920:0x%x, PP 0x3C:0x%x\n",
			       _tVerPic[u4InstID].u4W, _tVerPic[u4InstID].u4H,
			       u4VDecReadMC(u4InstID, 0x920), u4ReadReg(RM_VDEC_PP_BASE + 0x3C));

			vVDEC_HAL_MPEG_VDec_DumpReg(u4InstID, FALSE);

			/* dump YC */
#if 0				/* todo: */
			_u4FileCnt[u4InstID]--;	/* for recording file name */

			_tFBufFileInfo[u4InstID].u4FileLength =
			    (((_tVerPic[u4InstID].u4W +
			       15) >> 4) << 4) * (((_tVerPic[u4InstID].u4H + 32) >> 5) << 5);
			...
			_tFBufFileInfo[u4InstID].u4FileLength =
			    ((((_tVerPic[u4InstID].u4W +
				15) >> 4) << 4) * (((_tVerPic[u4InstID].u4H + 32) >> 5) << 5)) / 2;
			...
			printk(strMessage, " Compare Finish[NG] ==> Pic count to [%d]\n",
			       _u4FileCnt[u4InstID] - 1);
			/* fgWrMsg2PC(strMessage,strlen(strMessage),8,&_tFileListRecInfo[u4InstID]); */
			/* fprintf(_tFileListRecInfo.fpFile, " Error ==> Pic count to [%d]\n", _u4FileCnt[_u4VDecID] - 1); */
		}
#ifndef MPEG4_8130_ERROR_CONCEAL
...
void vMPEG4CrcCmp(UINT32 u4InstID, UCHAR *ptAddr, UINT32 u4Size)
{
	...
	if (fgYCrcOpen == FALSE) {
		...
		/* sprintf(_bFileStr1[u4InstID][14] + u4FileNameLen, "%s", "_Ycrc.bin"); */
		/* sprintf(_bFileStr1[u4InstID][15] + u4FileNameLen, "%s", "_Ccrc.bin"); */
#endif
	...
	if ((u4FilePicNum < u4FileTotalPicNum) && fgCmpRet) {
		...
		if ((u4FilePicNum == u4FileTotalPicNum)
		    || (u4FilePicNum > _u4EndCompPicNum[u4InstID])) {
			...
			fgWrData2PC(_pucESATotalBuf[u4InstID], _u4ESATotalLen[u4InstID], 7,
				    _ESAFileName[u4InstID]);
#endif
			u4FilePicNum = 0;
...
void vWMVWrData2PC(UINT32 u4InstID, UCHAR *ptAddr, UINT32 u4Size)
{
	...
	if (prWMVPPS->ucPicType != SKIPFRAME) {
...
#endif
		{
			...
			_tFBufFileInfo[u4InstID].u4FileLength =
			    (((_tVerPic[u4InstID].u4W +
			       15) >> 4) << 4) * (((_tVerPic[u4InstID].u4H + 31) >> 5) << 5);
#else
#ifdef DIRECT_DEC
			if ((_u4FileCnt[u4InstID] >= _u4StartCompPicNum[u4InstID])
			    && (_u4FileCnt[u4InstID] <= _u4EndCompPicNum[u4InstID]))
			...
			{
#ifdef DOWN_SCALE_SUPPORT
				vGenerateDownScalerGolden(u4InstID,
...
#ifndef GOLDEN_128BIT_COMP
			u4BufferWidth =
			    ((_tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4DispW + 15) >> 4) << 4;
			u4GoldenBufferWidth =
			    ((_tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4DispW + 15) >> 4) << 4;
#endif
			u4Width = _tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4DispW;
			...
			if (_i4CodecVersion[u4InstID] != VDEC_VC1) {
				...
				u4GoldenBufferWidth =
				    ((_rWMVSPS[u4InstID].u4MaxPicWidthSrc + 15) >> 4) << 4;
			}
#endif
			if (_rWMVSPS[u4InstID].u4PicWidthDec > _tVerPic[u4InstID].u4W) {
...
#endif
			{
...
#endif
						u4GoldenStart =
						    (mbh * u4GoldenTx1 + mbw) * (16 * 32);
						u4GoldenBuf =
						    (UINT32 *) (((UINT32) (_pucDumpYBuf[u4InstID]))
								+ u4GoldenStart);
										...
										if (u4DecData !=
											...
											fgWrMsg2PC
											    (strMessage,
											     ...
											     [u4InstID]);
											vVDecOutputDebugString
											    ("Y Data Mismatch at [x= 0x%d, y=0x%d] = 0x%x, Golden = 0x%x !!!\n",
											     u4XPix,
											     u4YPix,
											     (*u4DecBuf),
											     (*u4GoldenBuf));
											...
											_tFBufFileInfo
											    ...
											    u4FileLength
											    =
											    (((_tVerPic[u4InstID].u4W + 15) >> 4) << 4) * (((_tVerPic[u4InstID].u4H + 32) >> 5) << 5);
											vWrite2PC
											    (u4InstID,
											     2,
											     _pucDecWorkBuf
											...
											_tFBufFileInfo
											    ...
											    u4FileLength
											    =
											    (((u4Width + 15) >> 4) << 4) * (((u4Height + 32) >> 5) << 5);
											vWrite2PC
											    (u4InstID,
											     5,
											     _pucVDSCLBuf
				...
				for (u4XPix = 0; u4XPix < u4Width; u4XPix++) {
					for (u4YPix = 0; u4YPix < u4Height; u4YPix++) {
						if (((_rWMVPPS[u4InstID].ucFrameCodingMode ==
						      INTERLACEFIELD)
						     && ((u4YPix % 2) ==
							 _rWMVPPS[u4InstID].i4CurrentField))
						    || (_rWMVPPS[u4InstID].ucFrameCodingMode !=
							INTERLACEFIELD)) {
							/* compare Y */
#ifdef DOWN_SCALE_SUPPORT
							pbDecBuf =
							    (UCHAR *)
							    u4CalculatePixelAddress_Y(u4YBase,
										      u4XPix,
										      u4YPix,
										      u4BufferWidth,
										      (_tVerMpvDecPrm
							...
							pbGoldenBuf =
							    (UCHAR *)
							    u4CalculatePixelAddress_Y((UINT32)
										      _pucDumpYBuf
										      [u4InstID],
										      u4XPix,
										      u4YPix,
							...
							if ((*(pbDecBuf)) != (*(pbGoldenBuf))) {
								...
								fgWrMsg2PC(strMessage,
									   ...
									   [u4InstID]);
								vVDecOutputDebugString
								    ("Y Data Mismatch at [x= 0x%.8x, y=0x%.8x] = 0x%.2x, Golden = 0x%.2x !!!\n",
								     u4XPix, u4YPix, (*pbDecBuf),
								     (*pbGoldenBuf));
								sprintf(strMessage,
									"Y Data Mismatch at [x= 0x%.8x, y=0x%.8x] = 0x%.2x, Golden = 0x%.2x !!!\n",
									u4XPix, u4YPix, (*pbDecBuf),
								...
								_tFBufFileInfo[u4InstID].
								    ...
								    (((_tVerPic[u4InstID].u4W +
								       15) >> 4) << 4) *
								    (((_tVerPic[u4InstID].u4H +
								       32) >> 5) << 5);
								vWrite2PC(u4InstID, 2,
									  _pucDecWorkBuf[u4InstID]);
#if defined(DOWN_SCALE_SUPPORT)
								_tFBufFileInfo[u4InstID].
								    u4FileLength =
								    (((u4Width +
			...
			/* if(!fgIsMonoPic(_u4VDecID)) */
			{
...
#endif
				{
					...
					fgOpenFile(u4InstID, _bFileStr1[u4InstID][4], "r+b",
						   &_tFBufFileInfo[u4InstID]);
					_u4GoldenCSize[u4InstID] =
					    _tFBufFileInfo[u4InstID].u4RealGetBytes;
#endif
				}
#endif
...
#endif
				{
					...
					u4GoldenTx1 =
					    (u4GoldenX == 0) ? u4GoldenTx0 : (u4GoldenTx0 + 1);
...
#if VDEC_DDR3_SUPPORT
							u4DDR3Start =
							    (mbh * u4TxDDR3 + mbw) * (16 * 16);
							/* u4DDR3Start = (mbh*u4GoldenTx1 + mbw) *(16*16); */
#endif
							u4GoldenStart =
							    (mbh * u4GoldenTx1 + mbw) * (16 * 16);
							u4GoldenBuf =
							    (UINT32
...
#endif
									{
										if ((mbw == u4Tx0
											...
											if ((mbw ==
												...
												u4DecData
												    =
												    (((*(u4DecBuf)) << (8 * (4 - (2 * u4X)))) >> (8 * (4 - (2 * u4X))));
												u4GoldenData
												    =
												    (((*(u4GoldenBuf)) << (8 * (4 - (2 * u4X)))) >> (8 * (4 - (2 * u4X))));
											} else {
												u4DecData
												    =
												    (*
											...
											if (u4DecData != u4GoldenData) {
												...
												fgWrMsg2PC
												    (strMessage,
												     ...
												     [u4InstID]);
												vVDecOutputDebugString
												    ("CbCr Data Mismatch at [x= 0x%d, y=0x%d] = 0x%x, Golden = 0x%x !!!\n",
												     u4XPix,
												     u4YPix,
												     (*u4DecBuf),
												     (*u4GoldenBuf));
					...
					for (u4XPix = 0; u4XPix < u4Width; u4XPix++) {
						for (u4YPix = 0; u4YPix < u4Height; u4YPix++) {
#if defined(DOWN_SCALE_SUPPORT)
							if (((_rWMVPPS[u4InstID].
							      ucFrameCodingMode == INTERLACEFIELD)
							     &&
							     (((u4YPix >>
								(1 -
								 _tVerMpvDecPrm[u4InstID].
								 rDownScalerPrm.ucVdoFmt)) % 2) ==
							      _rWMVPPS[u4InstID].i4CurrentField))
							    || (_rWMVPPS[u4InstID].
								ucFrameCodingMode !=
								INTERLACEFIELD))
...
#endif
							{
								...
								pbDecBuf =
								    (UCHAR *)
								    u4CalculatePixelAddress_C
								    (u4CBase, u4XPix, u4YPix,
								     u4BufferWidth,
								     (_tVerMpvDecPrm[u4InstID].
								      rDownScalerPrm.ucScanType ==
								      0),
								     (_tVerMpvDecPrm[u4InstID].
								      rDownScalerPrm.ucVdoFmt == 0),
								     4);
								pbGoldenBuf =
								    (UCHAR *)
								    u4CalculatePixelAddress_C(((UINT32) _pucDumpCBuf[u4InstID]), u4XPix, u4YPix, u4GoldenBufferWidth, (_tVerMpvDecPrm[u4InstID].rDownScalerPrm.ucScanType == 0), (_tVerMpvDecPrm[u4InstID].rDownScalerPrm.ucVdoFmt == 0), 4);
#else
								pbDecBuf =
								    (UCHAR *)
								    u4CalculatePixelAddress_C
								...
								pbGoldenBuf =
								    (UCHAR *)
								    u4CalculatePixelAddress_C((UINT32) _pucDumpCBuf[u4InstID], u4XPix, u4YPix, u4GoldenBufferWidth, 1, 1, 3);
#endif
								if ((*(pbDecBuf)) !=
								    (*(pbGoldenBuf))) {
									u4Cnt++;
									...
									fgWrMsg2PC(strMessage,
										   ...
										   [u4InstID]);
									vVDecOutputDebugString
									    ("Cb Data Mismatch at [x= 0x%.8x, y=0x%.8x] = 0x%.2x, Golden = 0x%.2x !!!\n",
									     u4XPix, u4YPix,
									     (*pbDecBuf),
									     (*pbGoldenBuf));
									sprintf(strMessage,
								...
								if ((*(pbDecBuf)) !=
									...
									fgWrMsg2PC(strMessage,
										   ...
										   [u4InstID]);
									vVDecOutputDebugString
									    ("Cr Data Mismatch at [x= 0x%.8x, y=0x%.8x] = 0x%.2x, Golden = 0x%.2x !!!\n",
									     u4XPix, u4YPix,
									     (*pbDecBuf),
									     (*pbGoldenBuf));
									sprintf(strMessage,
	...
	if (((_rWMVPPS[u4InstID].ucFrameCodingMode == INTERLACEFIELD)
	     && (_rWMVPPS[u4InstID].i4CurrentTemporalField == 1))
	    || (_rWMVPPS[u4InstID].ucFrameCodingMode != INTERLACEFIELD)) {
#ifdef REDEC
		if (_u4ReDecCnt[u4InstID] == 0)
#endif
			_u4FileCnt[u4InstID]++;
	...
	if ((fgOpen == FALSE) || (fgDecErr == TRUE) || (_fgVDecErr[u4InstID] == TRUE)) {
		...
		/* break decode */
		if (fgOpen == FALSE) {
			sprintf(strMessage, "Compare Finish[OK] ==> %s  Pic count to [%d]\n",
			...
			sprintf(strMessage, "Compare Finish[NG] ==> %s  Pic count to [%d]\n",
				_bFileStr1[u4InstID][1], _u4FileCnt[u4InstID] - 1);
			fgWrMsg2PC(strMessage, strlen(strMessage), 8, &_tFileListRecInfo[u4InstID]);
			/* ASSERT(0); */
			/* fprintf(_tFileListRecInfo.fpFile, " Error ==> Pic count to [%d]\n", _u4FileCnt[_u4VDecID] - 1); */
...
void vLBDParaParsing(UINT32 u4InstID)
{
	...
	} while (((*_pcLBDSettingFile[u4InstID]) != ' ')
		 && ((*_pcLBDSettingFile[u4InstID]) != '\n'));
	_pcLBDSettingFile[u4InstID]++;
	...
	do {
		...
		u4Temp += CharToInt((*_pcLBDSettingFile[u4InstID]));
		_pcLBDSettingFile[u4InstID]++;
	} while (((*_pcLBDSettingFile[u4InstID]) != '\r')
		 && ((*_pcLBDSettingFile[u4InstID]) != '\n'));
	_pcLBDSettingFile[u4InstID]++;
	_pcLBDSettingFile[u4InstID]++;
...
void vCheckLBDResult(UINT32 u4InstID)
{
	...
	if (_u4CodecVer[u4InstID] == VDEC_H264) {
		...
		} while (((*_pcLBDGoldenFile[u4InstID]) != ' ')
			 && ((*_pcLBDGoldenFile[u4InstID]) != '\n'));
		_pcLBDGoldenFile[u4InstID]++;
		...
		do {
			...
			_pcLBDGoldenFile[u4InstID]++;
		} while (((*_pcLBDGoldenFile[u4InstID]) != '\r')
			 && ((*_pcLBDGoldenFile[u4InstID]) != '\n'));
		_pcLBDGoldenFile[u4InstID]++;
		_pcLBDGoldenFile[u4InstID]++;
		...
		if ((*_pcLBDGoldenFile[u4InstID]) == '@') {
			...
			if (_u4FileCnt[u4InstID] != 1)	/* flush DPB buffer */
			{
				if (fgIsDecFlagSet(u4InstID, DEC_FLAG_CHG_FBUF)) {
					_ptCurrFBufInfo[u4InstID]->eH264DpbStatus =
					    H264_DPB_STATUS_DECODED;
					_ptCurrFBufInfo[u4InstID]->u4DecOrder =
...
BOOL vH264_CheckCRCResult(UINT32 u4InstID)
{
	...
	if (fgH264YCrcOpen == FALSE) {
		...
		fgOpen =
		    fgOpenFile(u4InstID, _bFileStr1[u4InstID][16], "r+b", &_tInFileInfo[u4InstID]);
	...
	for (i = 0; i < 4; i++) {
		u4HW_CbCr_Result[i] = u4VDecReadCRC(u4InstID, (i + 6) << 2);
		u4Golden =
		    ((UINT32 *) _pucH264CRCYBuf[u4InstID])[u4H264Cnt * u4GoldenOffset + 4 + i];
		...
		if (u4H264Cnt == u4H264FileTotalPicNum) {
			...
			for (i = 0; i < 2; i++) {
				...
				if (fgIsDecFlagSet(i, DEC_FLAG_CHG_FBUF)) {
					_ptCurrFBufInfo[i]->eH264DpbStatus =
					    H264_DPB_STATUS_DECODED;
...
/* REG_9: c_crc_checksum[127:96] */
	...
	_tFBufFileInfo[u4InstID].u4FileLength = 0;
	/* Y decoded data Compare */
	vConcateStr(_bFileStr1[u4InstID][3], _bFileStr1[u4InstID][0], _bFileNameCRC,
		    _u4FileCnt[u4InstID]);
	fgOpen = fgOpenFile(u4InstID, _bFileStr1[u4InstID][3], "r+b", &_tFBufFileInfo[u4InstID]);
	if (fgOpen == FALSE) {
...
void vH264WrData2PC(UINT32 u4InstID, UCHAR *ptAddr, UINT32 u4Size)
{
	...
	if (_ucMVCType[u4InstID] == 1 && _u4FileCnt[u4InstID] % 10 == 0)
		printk("[Base] FrmCnt = %d\n",
		       (_u4FileCnt[u4InstID] << _fgMVCReady[u4InstID]) + (_ucMVCType[u4InstID] -
									  _fgMVCReady[u4InstID]));
...
/* /TODO:    BSP_InvDCacheRange((UINT32)_pucDPB[u4InstID],DPB_SZ); */
	...
	u4NonSwapYBase = (UINT32) _pucDecWorkBuf[u4InstID];
	u4NonSwapCBase =
	    (UINT32) (_pucDecWorkBuf[u4InstID] + _ptCurrFBufInfo[u4InstID]->u4DramPicSize);
...
#ifdef EXT_COMPARE
	_tFBufFileInfo[u4InstID].u4FileLength =
	    (((_ptCurrFBufInfo[u4InstID]->u4W +
	       15) >> 4) << 4) * (((_ptCurrFBufInfo[u4InstID]->u4H + 31) >> 5) << 5);
#else
#ifdef DIRECT_DEC
	if ((_u4FileCnt[u4InstID] >= _u4StartCompPicNum[u4InstID])
	    && (_u4FileCnt[u4InstID] <= _u4EndCompPicNum[u4InstID]))
#endif
	{
#ifdef DOWN_SCALE_SUPPORT
#ifdef FGT_SUPPORT
		vGenerateDownScalerGolden(u4InstID, (UINT32) _pucFGTBuf[u4InstID],
					  (UINT32) (_pucFGTBuf[u4InstID] +
						    _ptCurrFBufInfo[u4InstID]->u4DramPicSize),
					  u4Size);
#else
		vGenerateDownScalerGolden(u4InstID, (UINT32) _pucDecWorkBuf[u4InstID],
					  (UINT32) (_pucDecWorkBuf[u4InstID] +
						    _ptCurrFBufInfo[u4InstID]->u4DramPicSize),
					  u4Size);
#endif
		...
		if (_ucMVCType[u4InstID] > 0 && _tVerMpvDecPrm[u4InstID].u4PicW == 1280
		    && _tVerMpvDecPrm[u4InstID].u4PicH == 736) {
	...
	if ((_tVerMpvDecPrm[u4InstID].rDownScalerPrm.ucPicStruct == TOP_FIELD)
		...
		u4Pic32XSize =
		    _tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4TrgWidth *
		    ((_tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4TrgHeight * 2) - u4Upd);
	} else {
		u4Upd = _tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4TrgHeight % 32;
		u4Pic32XSize =
		    _tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4TrgWidth *
		    (_tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4TrgHeight - u4Upd);
	}
#else
	...
	if ((_tVerMpvDecPrm[u4InstID].rDownScalerPrm.ucPicStruct == TOP_FIELD)
	    || (_tVerMpvDecPrm[u4InstID].rDownScalerPrm.ucPicStruct == BOTTOM_FIELD)) {
		_u4GoldenYSize[u4InstID] =
		    _tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4DispW *
		    ((_tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4TrgHeight * 2) +
		     _tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4TrgOffV);
	} else {
		_u4GoldenYSize[u4InstID] =
		...
		if (_tVerMpvDecPrm[u4InstID].ucAddrSwapMode == ADDRSWAP_OFF) {
			for (u4Idx = 0; u4Idx < _u4GoldenYSize[u4InstID] >> 2; u4Idx++) {
				...
				if (1 == _tVerMpvDecPrm[u4InstID].rDownScalerPrm.ucScanType) {
					if ((_tVerMpvDecPrm[u4InstID].ucPicStruct == BOTTOM_FIELD)
					    &&
					    ((i %
					      _tVerMpvDecPrm[u4InstID].rDownScalerPrm.
					      u4TrgBufLen) == 0)) {
						/* Skip Top lines */
						i += _tVerMpvDecPrm[u4InstID].rDownScalerPrm.
						    u4TrgBufLen;
					}
				} else {
					if ((_tVerMpvDecPrm[u4InstID].ucPicStruct == BOTTOM_FIELD)
				...
				if ((pu4GoldenBuf[u4Idx] != pu4WorkBuf[u4Idx])) {
					u4Cnt++;
					sprintf(strMessage,
						"Y Data Mismatch at [%d] = 0x%X, Golden = 0x%X !!\n",
				...
				if (1 == _tVerMpvDecPrm[u4InstID].rDownScalerPrm.ucScanType) {
					if ((_tVerMpvDecPrm[u4InstID].ucPicStruct == TOP_FIELD)
					    ...
					    ((i %
					      _tVerMpvDecPrm[u4InstID].rDownScalerPrm.
					      u4TrgBufLen) ==
					     (_tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4TrgBufLen -
					      1))) {
						/* Skip bottom lines */
						i += _tVerMpvDecPrm[u4InstID].rDownScalerPrm.
						    u4TrgBufLen;
					...
					if (u4Upd && (i > u4Pic32XSize)
					    && ((i - u4Pic32XSize) % 512 == ((u4Upd << 4) - 1))
					    && (fgIsFrmPic(u4InstID))) {
						i += ((32 - u4Upd) << 4);
					} else if (u4Upd && (i > u4Pic32XSize) && (i % 256 == 255)
						   && (!fgIsFrmPic(u4InstID))) {
						i += 256;
					}
					if ((_tVerMpvDecPrm[u4InstID].ucPicStruct == TOP_FIELD)
			...
			if (_tVerMpvDecPrm[u4InstID].ucPicStruct == FRAME) {
				...
				for (u4XPix = 0; u4XPix < _tVerMpvDecPrm[u4InstID].u4PicW; u4XPix++) {
					for (u4YPix = 0; u4YPix < _tVerMpvDecPrm[u4InstID].u4PicH;
					     u4YPix++) {
#ifndef VDEC_DDR3_SUPPORT
						pucWorkBuf =
						    (UCHAR *) u4CalculatePixelAddress_Y((UINT32)
											pbDecBuf,
											u4XPix,
											u4YPix,
											u4AlignWidth,
						...
						if ((*(pucWorkBuf)) != (*(pucGoldenBuf))) {
							...
							sprintf(strMessage,
								"Current Pic Cnt = %d !!!\n",
								_u4FileCnt[u4InstID]);
							fgWrMsg2PC(strMessage, strlen(strMessage),
								   8, &_tFileListRecInfo[u4InstID]);
							vVDecOutputDebugString
							    ("Y Data Mismatch at [x= 0x%.8x, y=0x%.8x] = 0x%.2x, Golden = 0x%.2x !!!\n",
							     u4XPix, u4YPix, (*pucWorkBuf),
							     (*pucGoldenBuf));
							sprintf(strMessage,
								"Y Data Mismatch at [x= 0x%.8x, y=0x%.8x] = 0x%.2x, Golden = 0x%.2x !!!\n",
								u4XPix, u4YPix, (*pucWorkBuf),
	...
	if (!fgIsMonoPic(u4InstID)) {
		...
		_tFBufFileInfo[u4InstID].u4FileLength = 0;
#if (!VDEC_MVC_SUPPORT)
		vConcateStr(_bFileStr1[u4InstID][4], _bFileStr1[u4InstID][0], _bFileAddStrC[0],
			    _u4FileCnt[u4InstID]);
#else
		vConcateStr(_bFileStr1[u4InstID][4], _bFileStr1[u4InstID][0], _bFileAddStrC[0],
			    (_u4FileCnt[u4InstID] << _fgMVCReady[u4InstID]) +
			    (_ucMVCType[u4InstID] - _fgMVCReady[u4InstID]));
#endif
#if defined(NO_COMPARE)
#elif defined(EXT_COMPARE)
		_tFBufFileInfo[u4InstID].u4FileLength =
		    (((_ptCurrFBufInfo[u4InstID]->u4W +
		       15) >> 4) << 4) * (((_ptCurrFBufInfo[u4InstID]->u4H + 31) >> 5) << 5) >> 1;
#else
#ifdef DIRECT_DEC
		if ((_u4FileCnt[u4InstID] >= _u4StartCompPicNum[u4InstID])
		    && (_u4FileCnt[u4InstID] <= _u4EndCompPicNum[u4InstID]))
		...
		if (_tVerMpvDecPrm[u4InstID].rDownScalerPrm.ucVdoFmt == 0) {
			if ((_tVerMpvDecPrm[u4InstID].rDownScalerPrm.ucPicStruct == TOP_FIELD)
				...
				_u4GoldenCSize[u4InstID] =
				    _tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4DispW *
				    ((_tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4TrgHeight * 2) +
				     _tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4TrgOffV) / 2;
			} else {
				_u4GoldenCSize[u4InstID] =
				    _tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4DispW *
				    (_tVerMpvDecPrm[u4InstID].rDownScalerPrm.u4TrgHeight +
...
#endif
		{
			...
			if (_tVerMpvDecPrm[u4InstID].ucAddrSwapMode == ADDRSWAP_OFF) {
				...
				for (u4Idx = 0; u4Idx < _u4GoldenCSize[u4InstID] >> 2; u4Idx++) {
					...
					/* raster scan mode */
					if (1 == _tVerMpvDecPrm[u4InstID].rDownScalerPrm.ucScanType) {
						if ((_tVerMpvDecPrm[u4InstID].ucPicStruct ==
						     BOTTOM_FIELD)
						    &&
						    ((i %
						      _tVerMpvDecPrm[u4InstID].rDownScalerPrm.
						      u4TrgBufLen) == 0)) {
							/* Skip Top lines */
							i += _tVerMpvDecPrm[u4InstID].
							    rDownScalerPrm.u4TrgBufLen;
						}
					} else {
						if ((_tVerMpvDecPrm[u4InstID].ucPicStruct ==
						     BOTTOM_FIELD) && ((i % 16) == 0)
					...
					u4Idx = i >> 2;
#if defined(DOWN_SCALE_SUPPORT)
					if (pu4GoldenBuf[u4Idx] !=
					    pu4WorkBuf[u4Idx + (u4Size >> 2)])
#else
					if (pu4GoldenBuf[u4Idx] !=
					    pu4WorkBuf[u4Idx + (u4Size >> 2)])
#endif
					{
						...
						u4Cnt++;
						sprintf(strMessage,
							"CbCr Data Mismatch at [%d] = 0x%X, Golden = 0x%X !!!\n",
							i, pu4WorkBuf[u4Idx + (u4Size >> 2)],
							pu4GoldenBuf[u4Idx]);
						printk("%s", strMessage);
						fgDecErr = TRUE;
						vVDEC_HAL_H264_VDec_DumpReg(u4InstID);	/* mark by ginny */
...
#else
					if (u4Upd && (i > u4Pic32XSize)
					    && ((i - u4Pic32XSize) % 256 == ((u4Upd << 3) - 1))
					    && fgIsFrmPic(u4InstID)) {
						i += ((16 - (u4Upd >> 1)) << 4);	/* remainder x 16 */
					} else if (u4Upd && (i > u4Pic32XSize) && (i % 128 == 127)
						   && (!fgIsFrmPic(u4InstID))) {
						i += 128;
					}
					/* odd lines for top, even lines for bottom */
				...
				if (_tVerMpvDecPrm[u4InstID].ucPicStruct == FRAME) {
#ifndef VDEC_DDR3_SUPPORT
					u4AlignWidth =
					    ((_tVerMpvDecPrm[u4InstID].u4PicW + 15) >> 4) << 4;
#else
					u4AlignWidth =
					    ((_tVerMpvDecPrm[u4InstID].u4PicW + 15) >> 4) << 4;
					u4DDR3AlignWidth = _tVerMpvDecPrm[u4InstID].u4PicW;
					u4DDR3AlignWidth = (((u4DDR3AlignWidth + 63) >> 6) << 6);	/* Align to 4MB width */
#endif
					for (u4XPix = 0; u4XPix < _tVerMpvDecPrm[u4InstID].u4PicW;
					     u4XPix++) {
						for (u4YPix = 0;
						     u4YPix < _tVerMpvDecPrm[u4InstID].u4PicH;
						     u4YPix++) {
							/* compare Cb */
#ifndef VDEC_DDR3_SUPPORT
							pucWorkBuf =
							    (UCHAR *)
							    u4CalculatePixelAddress_C((INT32)
										      (pbDecBuf +
										       u4Size),
										      u4XPix,
										      u4YPix,
							...
							pucGoldenBuf =
							    (UCHAR *)
							    u4CalculatePixelAddress_C((UINT32)
										      _pucDumpCBuf
										      [u4InstID],
										      u4XPix,
										      u4YPix,
	...
	printk("open file = %s  fgOpen = %d ==>%d\n", _bFileStr1[u4InstID][3], fgOpen, u4InstID);
	if (_ucMVCType[u4InstID] == 0) {
		if ((fgOpen == FALSE) || (fgDecErr == TRUE) || (_fgVDecErr[u4InstID] == TRUE)) {
			sprintf(strMessage, "%s", "\n");
			fgWrMsg2PC(strMessage, strlen(strMessage), 8, &_tFileListRecInfo[u4InstID]);
...
void vVDecVerify_PrepareFileName_RM(UINT32 u4InstID,
				    ...
				    UINT32 u4PathNumCount, UINT32 u4SubNameCnt)
{
	UINT32 u4Loop = 0;
...
/* Return      : None */
/* ********************************************************************* */
BOOL fgVdecReadFileName(UINT32 u4InstID, VDEC_INFO_VERIFY_FILE_INFO_T * ptFileListInfo,
			VDEC_INFO_VERIFY_FILE_INFO_T * ptFileRecInfo, UINT32 *u4StartComp,
			UINT32 *u4EndComp, UINT32 *DumpPic)
{
	...
	if (((*_pcBitstreamFileName[u4InstID]) == 'H')
	    && ((*(_pcBitstreamFileName[u4InstID] + 1)) == '2')
	    && ((*(_pcBitstreamFileName[u4InstID] + 2)) == '6')
	    && ((*(_pcBitstreamFileName[u4InstID] + 3)) == '4')) {
		_u4CodecVer[u4InstID] = VDEC_H264;
		_pcBitstreamFileName[u4InstID] += 4;
		_ucMVCType[u4InstID] = 0;
		_fgMVCError[u4InstID] = 0;
	} else if (((*_pcBitstreamFileName[u4InstID]) == 'M')
		   && ((*(_pcBitstreamFileName[u4InstID] + 1)) == 'V')
		   && ((*(_pcBitstreamFileName[u4InstID] + 2)) == 'C')) {
		_u4CodecVer[u4InstID] = VDEC_H264;
		_pcBitstreamFileName[u4InstID] += 3;
		_fgMVCReady[u4InstID] = (u4InstID == 0) ? TRUE : FALSE;
		_ucMVCType[u4InstID] = (u4InstID == 0) ? 1 : 2;
		_fgMVCError[u4InstID] = 0;
	} else if (((*_pcBitstreamFileName[u4InstID]) == 'W')
		   && ((*(_pcBitstreamFileName[u4InstID] + 1)) == 'M')
		   && ((*(_pcBitstreamFileName[u4InstID] + 2)) == 'V')) {
		_u4CodecVer[u4InstID] = VDEC_WMV;
		_pcBitstreamFileName[u4InstID] += 3;
	} else if (((*_pcBitstreamFileName[u4InstID]) == 'M')
		   && ((*(_pcBitstreamFileName[u4InstID] + 1)) == 'P')
		   && ((*(_pcBitstreamFileName[u4InstID] + 2)) == 'E')
		   && ((*(_pcBitstreamFileName[u4InstID] + 3)) == 'G')
		   && ((*(_pcBitstreamFileName[u4InstID] + 4)) == '2')) {
		...
		_pcBitstreamFileName[u4InstID] += 5;
		   ...
		   && ((*(_pcBitstreamFileName[u4InstID] + 2)) == 'E')
		   && ((*(_pcBitstreamFileName[u4InstID] + 3)) == 'G')
		   && ((*(_pcBitstreamFileName[u4InstID] + 4)) == '4')) {
		_u4CodecVer[u4InstID] = VDEC_MPEG4;
		_pcBitstreamFileName[u4InstID] += 5;
	} else if (((*_pcBitstreamFileName[u4InstID]) == 'S')
		   && ((*(_pcBitstreamFileName[u4InstID] + 1)) == 'H')
		   && ((*(_pcBitstreamFileName[u4InstID] + 2)) == 'O')
		   && ((*(_pcBitstreamFileName[u4InstID] + 3)) == 'R')
		   && ((*(_pcBitstreamFileName[u4InstID] + 4)) == 'T')) {
		_u4CodecVer[u4InstID] = VDEC_MPEG4;
		_fgShortHeader[0] = TRUE;
		_fgShortHeader[1] = TRUE;
		_pcBitstreamFileName[u4InstID] += 5;
	} else if (((*_pcBitstreamFileName[u4InstID]) == 'D')
		   && ((*(_pcBitstreamFileName[u4InstID] + 1)) == 'I')
		   && ((*(_pcBitstreamFileName[u4InstID] + 2)) == 'V')
		   && ((*(_pcBitstreamFileName[u4InstID] + 3)) == 'X')
		   && ((*(_pcBitstreamFileName[u4InstID] + 4)) == '3')) {
	...
	/* PANDA H263 */
	else if (((*_pcBitstreamFileName[u4InstID]) == 'H')
		 && ((*(_pcBitstreamFileName[u4InstID] + 1)) == '2')
		 && ((*(_pcBitstreamFileName[u4InstID] + 2)) == '6')
		 && ((*(_pcBitstreamFileName[u4InstID] + 3)) == '3')) {
...
#ifdef VDEC_VIDEOCODEC_RM
	else if (((*_pcBitstreamFileName[u4InstID]) == 'R')
		 && ((*(_pcBitstreamFileName[u4InstID] + 1)) == 'M')) {
		_u4CodecVer[u4InstID] = VDEC_RM;
		_pcBitstreamFileName[u4InstID] += 2;
	}
#endif				/* VDEC_VIDEOCODEC_RM */
	else if (((*_pcBitstreamFileName[u4InstID]) == 'V')
		 && ((*(_pcBitstreamFileName[u4InstID] + 1)) == 'P')
		 && ((*(_pcBitstreamFileName[u4InstID] + 2)) == '6')
		 && ((*(_pcBitstreamFileName[u4InstID] + 3)) == ' ')) {
		...
		_u1AlphaFlag[u4InstID] = 0;
		   ...
		   && ((*(_pcBitstreamFileName[u4InstID] + 1)) == 'P')
		   && ((*(_pcBitstreamFileName[u4InstID] + 2)) == '6')
		   && ((*(_pcBitstreamFileName[u4InstID] + 3)) == 'A')) {
		_u4CodecVer[u4InstID] = VDEC_VP6;
		_pcBitstreamFileName[u4InstID] += 4;
		...
		_u4AdobeMode[u4InstID] = 0;
		   ...
		   && ((*(_pcBitstreamFileName[u4InstID] + 1)) == 'P')
		   && ((*(_pcBitstreamFileName[u4InstID] + 2)) == '6')
		   && ((*(_pcBitstreamFileName[u4InstID] + 3)) == 'O')) {
		_u4CodecVer[u4InstID] = VDEC_VP6;
		_pcBitstreamFileName[u4InstID] += 4;
		_u1AlphaBitstream[u4InstID] = 0;
		_u1AlphaFlag[u4InstID] = 0;
		_u4AdobeMode[u4InstID] = 0;
		   ...
		   && ((*(_pcBitstreamFileName[u4InstID] + 1)) == 'P')
		   && ((*(_pcBitstreamFileName[u4InstID] + 2)) == '8')) {
		_u4CodecVer[u4InstID] = VDEC_VP8;
		_pcBitstreamFileName[u4InstID] += 3;
	} else if (((*_pcBitstreamFileName[u4InstID]) == 'A')
		   && ((*(_pcBitstreamFileName[u4InstID] + 1)) == 'V')
		   && ((*(_pcBitstreamFileName[u4InstID] + 2)) == 'S')) {
	...
	while (((*_pcBitstreamFileName[u4InstID]) != '\n')
		...
		if (((*_pcBitstreamFileName[u4InstID]) != ' ')
		    || ((_u4CodecVer[u4InstID] == VDEC_VP8) && (_u4StringCnt[u4InstID] > 0))) {
			...
			if ((!fgGetStartFrmCnt) && (!fgGetEndFrmCnt) & (!fgGetDumpFrmCnt)) {
				...
				if (bFileName == '.') {
					...
					if (_ucMVCType[u4InstID] == 0)
						...
						if ((*(_pcBitstreamFileName[u4InstID] + 1)) == '2'
						    && (*(_pcBitstreamFileName[u4InstID] + 2)) ==
						    '6')
							fgGetName = TRUE;
						else
					...
					if (!fgGetName) {
...
#ifdef LETTERBOX_SUPPORT
						_bFileStr1[u4InstID][12][u4FileNameCnt1] =
						    bFileName;
						_bFileStr1[u4InstID][13][u4FileNameCnt1] =
						    bFileName;
#endif
						...
						if (_u4CodecVer[u4InstID] == VDEC_RM) {
							/* Frame Info File Path */
							_bFileStr1[u4InstID][8][u4FileNameCnt1] =
							    bFileName;

							/* Golden file Path */
							_bFileStr1[u4InstID][5][u4FileNameCnt1] =
							    bFileName;

							/* Path */
							_bFileStr1[u4InstID][9][u4FileNameCnt1] =
							    bFileName;
							_bFileStr1[u4InstID][10][u4FileNameCnt1] =
							    bFileName;

							_bFileStr1[u4InstID][2][u4FileNameCnt1] =
							    bFileName;
							_bFileStr1[u4InstID][3][u4FileNameCnt1] =
							    bFileName;
							_bFileStr1[u4InstID][4][u4FileNameCnt1] =
							    bFileName;

							_bFileStr1[u4InstID][RM_CRCINFO_INDEX]
							    [u4FileNameCnt1] = bFileName;
						}
#endif				/* VDEC_VIDEOCODEC_RM */
					} else {
						u4SubNameCnt++;
			...
			sprintf(_bFileStr1[u4InstID][7] + u4FileNameCnt1, "%c", '\0');
			   ...
			   && (fgGetName) && (fgGetSubName)
			   && (!fgGetStartFrmCnt) & (!fgGetDumpFrmCnt)) {
			fgGetStartFrmCnt = TRUE;
		} else if (((*_pcBitstreamFileName[u4InstID]) == ' ') && (u4FileNameCnt != 0)
			   && (fgGetName) && (fgGetSubName) && (fgGetStartFrmCnt)
			   && (!fgGetEndFrmCnt) & (!fgGetDumpFrmCnt)) {
			fgGetEndFrmCnt = TRUE;
		} else if (((*_pcBitstreamFileName[u4InstID]) == ' ') && (u4FileNameCnt != 0)
			   && (fgGetName) && (fgGetSubName) && (fgGetStartFrmCnt)
			   && (fgGetEndFrmCnt) & (!fgGetDumpFrmCnt)) {
	...
	if ((_u4CodecVer[u4InstID] == VDEC_DIVX3) || (_u4CodecVer[u4InstID] == VDEC_MPEG4)
	    || (_u4CodecVer[u4InstID] == VDEC_H263)) {
		sprintf(_bFileStr1[u4InstID][0] + u4FileNameCnt1 - u4SubNameCnt, "%s",
			_bFileStr1[u4InstID][8]);
		sprintf(_bFileStr1[u4InstID][2] + u4FileNameCnt1 - u4SubNameCnt, "%s",
			_bFileStr1[u4InstID][8]);
		sprintf(_bFileStr1[u4InstID][6] + u4FileNameCnt1 - u4SubNameCnt, "%s",
			_bFileStr1[u4InstID][8]);
		sprintf(_bFileStr1[u4InstID][7] + u4FileNameCnt1 - u4SubNameCnt, "%s",
			_bFileStr1[u4InstID][8]);
	...
	if (u4FileNameCnt != 0) {
		if (_u4CodecVer[u4InstID] == VDEC_MPEG2) {
			...
			sprintf(_bFileStr1[u4InstID][0] + u4FileNameCnt1 - (u4SubNameCnt - 1),
				"%s_", p + 1);
			/* printk("<vdec> _bFileStr1[%d][%d] = (%s), (%s, %d)\n", u4InstID, 0, _bFileStr1[u4InstID][0], __FUNCTION__, __LINE__); */
#else
			_bFileStr1[u4InstID][0][u4FileNameCnt1 - (u4SubNameCnt)] = '\\';
...
#ifdef VDEC_VIDEOCODEC_RM
		if (_u4CodecVer[u4InstID] == VDEC_RM) {
			vVDecVerify_PrepareFileName_RM(u4InstID, u4FileNameCnt1, u4PathNumCount,
						       u4SubNameCnt);
		} else {
...
#if 1				/* #ifdef WMV_CRC_COMPOSITE_CHECK_ENABLE || MPEG_CRC_COMPOSITE_CHECK_ENABLE */
...
#if AVC_NEW_CRC_COMPARE
		sprintf(_bFileStr1[u4InstID][16], "%s\n", _bFileStr1[u4InstID][0]);
#endif
...
void vWrite2PC(UINT32 u4InstID, UCHAR bType, UCHAR *pbAddr)
{
	...
	case 1:
		u4Temp +=
		    sprintf(_bTempStr1[u4InstID] + u4Temp, "%s%d%s", "dump_bitstream_VDec",
			    u4InstID, ".err");
		fgWrData2PC((void *)pbAddr, _tInFileInfo[u4InstID].u4FileLength, 7,
			    _bTempStr1[u4InstID]);
		break;
	case 2:
		u4Temp +=
		    sprintf(_bTempStr1[u4InstID] + u4Temp, "dump_VDec%d_%d_Y.err", u4InstID,
			    _u4FileCnt[u4InstID]);
		fgWrData2PC((void *)pbAddr, _tFBufFileInfo[u4InstID].u4FileLength, 7,
			    _bTempStr1[u4InstID]);
		break;
	case 3:
		u4Temp +=
		    sprintf(_bTempStr1[u4InstID] + u4Temp, "dump_VDec%d_%d_C.err", u4InstID,
			    _u4FileCnt[u4InstID]);
		fgWrData2PC((void *)pbAddr, _tFBufFileInfo[u4InstID].u4FileLength, 7,
			    _bTempStr1[u4InstID]);
		break;
	case 4:
		u4Temp +=
		    sprintf(_bTempStr1[u4InstID] + u4Temp, "dump_VDec%d_%d_MV.err", u4InstID,
			    _u4FileCnt[u4InstID]);
		fgWrData2PC((void *)pbAddr, _tFBufFileInfo[u4InstID].u4FileLength, 7,
			    _bTempStr1[u4InstID]);
		break;
	case 5:
		u4Temp +=
		    sprintf(_bTempStr1[u4InstID] + u4Temp, "dump_VDec%d_%d_Y_PP_8135.bin", u4InstID,
			    _u4FileCnt[u4InstID]);
		fgWrData2PC((void *)pbAddr, _tFBufFileInfo[u4InstID].u4FileLength, 7,
			    _bTempStr1[u4InstID]);
		break;
	case 6:
		u4Temp +=
		    sprintf(_bTempStr1[u4InstID] + u4Temp, "dump_VDec%d_%d_C_PP_8135.bin", u4InstID,
			    _u4FileCnt[u4InstID]);
		fgWrData2PC((void *)pbAddr, _tFBufFileInfo[u4InstID].u4FileLength, 7,
			    _bTempStr1[u4InstID]);
		break;
	...
	case 9:
		u4Temp +=
		    sprintf(_bTempStr1[u4InstID] + u4Temp, "%d_chksum_8530.bin",
			    _u4FileCnt[u4InstID]);
		...
		fgWrData2PC(&_u4DumpChksum[u4InstID][0], MAX_CHKSUM_NUM * 4, u4Mode,
			    _bTempStr1[u4InstID]);
		break;
	case 10:
		u4Temp +=
		    sprintf(_bTempStr1[u4InstID] + u4Temp, "%d_chksum.bin", _u4FileCnt[u4InstID]);
	...
	case 11:
		u4Temp +=
		    sprintf(_bTempStr1[u4InstID] + u4Temp, "VDec%d_%d_error_chksum.bin", u4InstID,
			    _u4FileCnt[u4InstID]);
		if ((!fgIsFrmPic(u4InstID)) && (_ptCurrFBufInfo[u4InstID]->ucFBufStatus == FRAME))	/* Only 2nd field pic needs offset 256 */
		{
			u4Mode = 8;
	...
	case 13:
		u4Temp +=
		    sprintf(_bTempStr1[u4InstID] + u4Temp, "VDec%d_dump_%d_bitstream.err", u4InstID,
			    _u4FileCnt[u4InstID]);
		fgWrData2PC((void *)pbAddr,
			    (_u4WMVByteCount[u4InstID] - _u4CurrPicStartAddr[u4InstID]), 7,
			    _bTempStr1[u4InstID]);
		break;
	case 14:
		u4Temp +=
		    sprintf(_bTempStr1[u4InstID] + u4Temp, "dump_%d_Y_Golden_PP.bin",
			    _u4FileCnt[u4InstID]);
		fgWrData2PC((void *)pbAddr, _tFBufFileInfo[u4InstID].u4FileLength, 7,
			    _bTempStr1[u4InstID]);
	...
	case 15:
		u4Temp +=
		    sprintf(_bTempStr1[u4InstID] + u4Temp, "dump_%d_C_Golden_PP.bin",
			    _u4FileCnt[u4InstID]);
		fgWrData2PC((void *)pbAddr, _tFBufFileInfo[u4InstID].u4FileLength, 7,
			    _bTempStr1[u4InstID]);
	case 16:
		u4Temp +=
		    sprintf(_bTempStr1[u4InstID] + u4Temp, "%d_vdscl_seeting.bin",
			    _u4FileCnt[u4InstID]);
		fgWrData2PC(&_tVerMpvDecPrm[u4InstID].rDownScalerPrm, 16 * 4, 7,
			    _bTempStr1[u4InstID]);
		break;
	...
	case 17:
		...
		_tFBufFileInfo[u4InstID].u4FileLength = 0;
		if (fgOpenFile(u4InstID, _ESAFileName[u4InstID], "r+b", &_tFBufFileInfo[u4InstID])
		    == 0) {
		...
		fgWrData2PC((void *)pbAddr, u4VDEC_HAL_Read_ESA(u4InstID, u4Temp), 7,
			    _ESAFileName[u4InstID]);
#else
		u4Temp = 0;
		u4Temp += sprintf(_pucESALog[u4InstID] + u4Temp, "%s,", _bTempStr1[u4InstID]);
		u4Temp = u4VDEC_HAL_Read_ESA(u4InstID, u4Temp);
		_u4ESATotalLen[u4InstID] +=
		    sprintf(_pucESATotalBuf[u4InstID] + _u4ESATotalLen[u4InstID], "%s",
			    _pucESALog[u4InstID]);
		...
		_tFBufFileInfo[u4InstID].u4TargetSz = ESALOG_SZ;
		_tFBufFileInfo[u4InstID].u4FileLength = 0;
		if (fgOpenFile(u4InstID, _bTempStr1[u4InstID], "r+b", &_tFBufFileInfo[u4InstID]) ==
		    0) {
			u4Temp = sprintf(_pucESALog[u4InstID], "%s\n", _bESAStr1[u4InstID]);
		} else {
			u4Temp = 0;
...
BOOL fgCompH264ChkSumGolden(UINT32 u4InstID)
{
	...
	if (fgOpen) {
		...
		/* ???????? */
		fgIsCompNoErr =
		    fgVDEC_HAL_H264_VDec_CompCheckSum(&_u4DumpChksum[u4InstID][0],
						      pu4GoldenCheckSum);
	} else {
		fgIsCompNoErr = FALSE;
	}
...
BOOL fgCompWMVChkSumGolden(UINT32 u4InstID)
{
	...
	if (_rWMVPPS[u4InstID].ucFrameCodingMode == INTERLACEFIELD) {
		u4Temp +=
		    sprintf(_bTempStr1[u4InstID] + u4Temp - 1, "crc_g_%d_%d.dat",
			    _u4FileCnt[u4InstID], _rWMVPPS[u4InstID].i4CurrentTemporalField);
		/* if (_rWMVPPS[u4InstID].i4CurrentTemporalField == 1) */
	} else {
		u4Temp +=
		    sprintf(_bTempStr1[u4InstID] + u4Temp - 1, "crc_g_%d.dat",
			    _u4FileCnt[u4InstID]);
	}
#endif
	...
	if (strcmp(_bTempStr1[u4InstID], _bPrevfileStrl)) {
		fgOpen =
		    fgOpenFile(u4InstID, _bTempStr1[u4InstID], "r+b", &_tTempFileInfo[u4InstID]);

		if (fgOpen) {
			strcpy(_bPrevfileStrl, _bTempStr1[u4InstID]);
		} else {
			printk("[WMV%d] Open CRC file fail!!\n", u4InstID);
		}

#ifdef WMV_CRC_COMPOSITE_CHECK_ENABLE
		_u4CRCFileSize[u4InstID] = _tTempFileInfo[u4InstID].u4FileLength;

		printk("[WMV%d] CRC file size:%d, fgFieldMode:%d\n",
		       u4InstID, _u4CRCFileSize[u4InstID], _rWMVPPS[u4InstID].fgFieldMode);
#endif
	} else {
		fgOpen = TRUE;
	}
...
#ifdef WMV_CRC_COMPOSITE_CHECK_ENABLE
		fgIsCompNoErr =
		    fgWMV_CheckCRCResult(u4InstID, _u4CRCCnt[u4InstID],
	...
	if (fgOpen) {
		...
		fgIsCompNoErr =
		    fgVDEC_HAL_WMV_VDec_CompCheckSum(&_u4DumpChksum[u4InstID][0],
						     pu4GoldenCheckSum);
	} else {
		fgIsCompNoErr = FALSE;
	}
...
BOOL fgCompMPEGChkSumGolden(UINT32 u4InstID)
{
	...
	u4Temp += sprintf(_bTempStr1[u4InstID] + u4Temp - 1, "_crc.bin");
#else
	vConcateStr(_bTempStr1[u4InstID], _bFileStr1[u4InstID][0], "_0.crc\0",
		    _u4FileCnt[u4InstID]);
#endif
	_tTempFileInfo[u4InstID].fgGetFileInfo = TRUE;
	_tTempFileInfo[u4InstID].pucTargetAddr = _pucDumpYBuf[u4InstID];
	...
	if (strcmp(_bTempStr1[u4InstID], _bPrevfileStrl)) {
		...
		printk("[WMV%d] CRC file size:%d\n", u4InstID, _u4CRCFileSize[u4InstID]);
	} else {
		fgOpen = TRUE;
	}
#else
	...
	pu4GoldenCheckSum = (UINT32 *) _pucDumpYBuf[u4InstID];
	if (fgOpen) {
#if MPEG_CRC_COMPOSITE_CHECK_ENABLE
		pu4GoldenCheckSum += (_u4CRCCnt * 32);
#endif

#if 1
		fgIsCompNoErr =
		    fgVDEC_HAL_MPEG_VDec_CompCheckSum(&_u4DumpChksum[u4InstID][0],
						      pu4GoldenCheckSum);
#else
...
void vGenerateDownScalerGolden(UINT32 u4InstID, UINT32 DecYAddr, UINT32 DecCAddr, UINT32 u4Size)
{
			...
			for (j = 0; j < 32; j++)	/* line count */
				for (l = 0; l < 16; l++) {
					if ((i * 32 + j) >= srcHeight) {
						...
						tmpbuf = *(DecDataY++);
						srcDataY[(i * 32 + j) * srcbufLen + k * 16 + l] =
						    (UCHAR) tmpbuf;
					}
				}
			...
			for (j = 0; j < 16; j++)	/* line count */
				for (l = 0; l < 16; l++) {
					if ((i * 16 + j) >= (srcHeight / 2)) {
						...
						tmpbuf = *(DecDataC++);
						srcDataC[(i * 16 + j) * srcbufLen + k * 16 + l] =
						    (UCHAR) tmpbuf;
					}
				}
	...
	if (lk_en == 1) {
		for (j = 0; j < srcHeight / 2; j++)
			for (i = 0; i < srcWidth / 2; i++) {
				...
				if ((lk_region0 > 0) && (lk_region1 > 0) && (lk_region2 > 0)
				    && (lk_region3 > 0)) {
		...
		if ((pic_stru != 3) && (pic_stru != 4)) {
			for (j = 0; j < srcHeight; j++)
				for (i = 0; i < srcWidth; i++) {
					...
					if ((i % 2) == 0) {
						Cb709 =
						    ((int)srcDataC[srcbufLen * (j / 2) + i]) - 128;
						Cr709 =
						    ((int)srcDataC[srcbufLen * (j / 2) + i + 1]) -
						    128;
					} else {
						Cb709 =
						    ((int)srcDataC[srcbufLen * (j / 2) + i - 1]) -
					...
					/* modified @07012008 */
					Y601 =
					    (Y709 * 1024 + Cb709 * 102 + Cr709 * 196 + 512) >> 10;
					if (Y601 > 255)
		...
		} else		/* interlaced frame */
		{
			for (j = 0; j < srcHeight; j = j + 2)
				for (i = 0; i < srcWidth; i++) {
					...
					if ((i % 2) == 0) {
						Cb709 =
						    ((int)srcDataC[srcbufLen * 2 * (j / 4) + i]) -
						    128;
						Cr709 =
						    ((int)srcDataC[srcbufLen * 2 * (j / 4) + i + 1])
						    - 128;
					} else {
						Cb709 =
						    ((int)srcDataC[srcbufLen * 2 * (j / 4) + i - 1])
    ...
    /**** do down scaling ****/
#if (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8520)
	vdscl(u4InstID, srcWidth, srcHeight, targetWidth, targetHeight, srcDataY, targetData, 0,
	      pic_stru, tgbufLen, h_ofst, v_ofst, src_agent, spec_type, mbaff, fg_src);
#else
	vdscl(u4InstID, srcWidth, srcHeight, targetWidth, targetHeight, srcDataY, targetData, 0,
	      pic_stru, tgbufLen, h_ofst, v_ofst, src_agent, spec_type, mbaff, fg_src, y_src_h_ofst,
	      y_src_v_ofst, y_scl_h_ofst, y_scl_v_ofst, lk_en, luma_key);
#endif
				...
				for (k = 0; k < (tgbufLen / 16); k++) {
					for (j = 0; j < 32; j++)	/* line count */
					{
						if ((i * 32 + j) >= tgbufHgh) {
						    ...
						    if (((_tVerMpvDecPrm[u4InstID].rDownScalerPrm.
							  ucPicStruct == TOP_FIELD) && (j % 2 == 1))
							||
							((_tVerMpvDecPrm[u4InstID].rDownScalerPrm.
							  ucPicStruct == BOTTOM_FIELD)
							 && (j % 2 == 0))) {
							for (h = 0; h < 16; h++) {
								VDSCLData++;
								/* *(VDSCLData++); */
...
#if (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8520)
	vdscl(u4InstID, srcWidth, srcHeight / 2, targetWidth, targetHeight, srcDataC, targetData, 1,
	      pic_stru, tgbufLen, h_ofst, v_ofst, src_agent, spec_type, mbaff, fg_src);
#else
	vdscl(u4InstID, srcWidth, srcHeight / 2, targetWidth, targetHeight, srcDataC, targetData, 1,
	      pic_stru, tgbufLen, h_ofst, v_ofst, src_agent, spec_type, mbaff, fg_src, c_src_h_ofst,
	      c_src_v_ofst, c_scl_h_ofst, c_scl_v_ofst, 0, 0);
#endif
		...
		if (component == 0)	/* Y */
		{
			...
			while (tg_pxl_cnt < targetWidth) {
				...
				if (OF == 0 && next_OF == 0)
					...
					temp =
					    (2048 - weight + factor) * srcData[j * srcbufLen +
									       srcWidth - 1];
				else if (OF == 1 && next_OF == 0)
					temp =
					    weight * srcData[j * srcbufLen + srcWidth - 1] +
					    128 * hbuff;
				else
					temp =
					    factor * srcData[j * srcbufLen + srcWidth - 1] +
					    128 * hbuff;
		...
		} else		/* C */
		{
			for (i = 2; i < srcWidth; i = i + 2) {
				...
				if (OF == 0 && next_OF == 0) {
					...
					temp = weight * srcData[j * srcbufLen + i] + 128 * hbuff;
					temp2 =
					    weight * srcData[j * srcbufLen + i + 1] + 128 * hbuff2;
				} else {
					temp = factor * srcData[j * srcbufLen + i] + 128 * hbuff;
					temp2 =
					    factor * srcData[j * srcbufLen + i + 1] + 128 * hbuff2;
				}
			...
			while (tg_pxl_cnt < targetWidth) {
				...
				if (OF == 0 && next_OF == 0) {
					...
					temp =
					    weight * srcData[j * srcbufLen + srcWidth - 2] +
					    128 * hbuff;
					temp2 =
					    weight * srcData[j * srcbufLen + srcWidth - 1] +
					    128 * hbuff2;
					...
					temp =
					    factor * srcData[j * srcbufLen + srcWidth - 2] +
					    128 * hbuff;
					temp2 =
					    factor * srcData[j * srcbufLen + srcWidth - 1] +
					    128 * hbuff2;
	...
	if ((pic_stru != 3) && (pic_stru != 4))	/* single picture */
	{
			...
			for (j = 0; j < targetWidth; j++) {
				if (OF == 0 && next_OF == 0)
					temp = weight * tempbuffer[i * tgbufLen + h_ofst + j] +
					    (2048 - weight) * tempbuffer[(i - 1) * tgbufLen +
									 h_ofst + j];
				else if (OF == 0 && next_OF == 1)
					...
					temp =
					    weight * tempbuffer[i * tgbufLen + h_ofst + j] +
					    128 * vbuff[j];
				else
					temp =
					    factor * tempbuffer[i * tgbufLen + h_ofst + j] +
					    128 * vbuff[j];
		...
		while (tg_line_cnt < targetHeight) {
			...
			for (j = 0; j < targetWidth; j++) {
				if (OF == 0 && next_OF == 0)
					temp =
					    weight * tempbuffer[(srcHeight - 1) * tgbufLen +
								h_ofst + j] + (2048 -
									       weight) *
					    tempbuffer[(srcHeight - 1) * tgbufLen + h_ofst + j];
				else if (OF == 0 && next_OF == 1)
					temp =
					    factor * tempbuffer[(srcHeight - 1) * tgbufLen +
								h_ofst + j] + (2048 -
									       weight) *
					    tempbuffer[(srcHeight - 1) * tgbufLen + h_ofst + j];
				else if (OF == 1 && next_OF == 0)
		...
		for (round = 2; round <= 3; round++) {
				...
				for (j = 0; j < targetWidth; j++) {
					if (OF == 0 && next_OF == 0)
						temp =
						    weight * tempbuffer[i * tgbufLen + h_ofst + j] +
						    (2048 -
						     weight) * tempbuffer[(i - 2) * tgbufLen +
									  h_ofst + j];
					else if (OF == 0 && next_OF == 1)
						temp =
						    factor * tempbuffer[i * tgbufLen + h_ofst + j] +
					...
					if (next_OF == 0)
						...
						if (src_agent == 4)	/* fgt */
						{
							if (fg_src == 1)	/* pp */
							{
								if ((((i % 16) == 7) || ((i % 16) == 6)) && (i < (srcHeight - 10)) && (component == 0))	/* Y */
									vbuff[j] = v_result * 0x10;	/* 12 bit */
								else if ((((i % 8) == 7) || ((i % 8) == 6)) && (i < (srcHeight - 10)) && (component == 1))	/* C */
									vbuff[j] = v_result * 0x10;	/* 12 bit */
								else
									vbuff[j] = temp_round;	/* 12 bit */
							} else	/* mc */
							{
								if ((((i % 16) == 15) || ((i % 16) == 14)) && (i < (srcHeight - 2)) && (component == 0))	/* Y */
									vbuff[j] = v_result * 0x10;	/* 12 bit */
								else if ((((i % 8) == 7)
									  || ((i % 8) == 6))
									 && (i < (srcHeight - 2))
						...
						} else if (src_agent == 2)	/* pp */
						{
							...
							{
								if ((((i % 32) == 25) || ((i % 32) == 24)) && (i < (srcHeight - 8)) && (component == 0))	/* Y */
									vbuff[j] = v_result * 0x10;	/* 12 bit */
								else if ((((i % 16) == 13) || ((i % 16) == 12)) && (i < (srcHeight - 4)) && (component == 1))	/* C */
									vbuff[j] = v_result * 0x10;	/* 12 bit */
								else
									vbuff[j] = temp_round;	/* 12 bit */
							} else	/* 264normal */
							{
								if ((((i % 16) == 11) || ((i % 16) == 10)) && (i < (srcHeight - 6)) && (component == 0))	/* Y */
									vbuff[j] = v_result * 0x10;	/* 12 bit */
								else if ((((i % 8) == 5) || ((i % 8) == 4)) && (i < (srcHeight - 4)) && (component == 1))	/* C */
									vbuff[j] = v_result * 0x10;	/* 12 bit */
								else
									vbuff[j] = temp_round;	/* 12 bit */
							}
						} else	/* mc */
						{
							if ((spec_type == 2) && (mbaff == 1))	/* 264mbaff */
							{
								if ((((i % 32) == 31) || ((i % 32) == 30)) && (i < (srcHeight - 2)) && (component == 0))	/* Y */
									vbuff[j] = v_result * 0x10;	/* 12 bit */
								else if ((((i % 16) == 15)
									  || ((i % 16) == 14))
									 && (i < (srcHeight - 2))
					...
					} else	/* both top & bottom fields, modified by cyc118 at 01202006 */
					{
						if (src_agent == 4)	/* fgt */
						{
							if (fg_src == 1)	/* pp */
							{
								if ((((i % 32) == 15) || ((i % 32) == 14)) && (i < (srcHeight - 18)) && (component == 0))	/* Y */
									vbuff[j] = v_result * 0x10;	/* 12 bit */
								else if ((((i % 16) == 15) || ((i % 16) == 14)) && (i < (srcHeight - 18)) && (component == 1))	/* C */
									vbuff[j] = v_result * 0x10;	/* 12 bit */
								else
									vbuff[j] = temp_round;	/* 12 bit */
							} else	/* mc */
						...
						} else if (src_agent == 2)	/* pp */
						{
							...
							{
								if ((((i % 64) == 51) || ((i % 64) == 50)) && (i < (srcHeight - 14)) && (component == 0))	/* Y */
									vbuff[j] = v_result * 0x10;	/* 12 bit */
								else if ((((i % 32) == 27) || ((i % 32) == 26)) && (i < (srcHeight - 6)) && (component == 1))	/* C */
									vbuff[j] = v_result * 0x10;	/* 12 bit */
								else
									vbuff[j] = temp_round;	/* 12 bit */
							} else	/* 264normal */
							{
								if ((((i % 32) == 23) || ((i % 32) == 22)) && (i < (srcHeight - 10)) && (component == 0))	/* Y */
									vbuff[j] = v_result * 0x10;	/* 12 bit */
								else if ((((i % 16) == 11) || ((i % 16) == 10)) && (i < (srcHeight - 6)) && (component == 1))	/* C */
									vbuff[j] = v_result * 0x10;	/* 12 bit */
								else
									vbuff[j] = temp_round;	/* 12 bit */
							}
						} else	/* mc */
						{
							if ((spec_type == 2) && (mbaff == 1))	/* 264mbaff */
							{
								if ((((i % 64) == 63) || ((i % 64) == 62)) && (i < (srcHeight - 2)) && (component == 0))	/* Y */
									vbuff[j] = v_result * 0x10;	/* 12 bit */
								else if ((((i % 32) == 31)
									  || ((i % 32) == 30))
									 && (i < (srcHeight - 2))
			...
			while (tg_line_cnt < targetHeight) {
				...
				for (j = 0; j < targetWidth; j++) {
					if (OF == 0 && next_OF == 0)
						temp =
						    weight * tempbuffer[(srcHeight + round - 4) *
									tgbufLen + h_ofst + j] +
						    (2048 -
						     weight) * tempbuffer[(srcHeight + round -
									   4) * tgbufLen + h_ofst +
									  j];
					else if (OF == 0 && next_OF == 1)
						temp =
						    factor * tempbuffer[(srcHeight + round - 4) *
									tgbufLen + h_ofst + j] +
						    (2048 -
						     weight) * tempbuffer[(srcHeight + round -
									   4) * tgbufLen + h_ofst +
		...
		if (component == 0)	/* Y */
		{
			...
			for (i = (1 + src_h_ofst); i < srcWidth; i++) {
				...
				if (OF == 0 && next_OF == 0)
					...
					temp =
					    factor_eval * srcData[j * srcbufLen + i] + 128 * hbuff;
				...
				if (tg_pxl_cnt < targetWidth) {
					if (next_OF == 0) {
						if (lk_en && (OF == 1) && (rgn_change == 1)) {
							tempbuffer[j * tgbufLen + tg_pxl_cnt +
								   h_ofst] =
							    srcData[j * srcbufLen + i];
							rgn_change = 0;
						} else if (lk_en && (rgn_change == 1)) {
							tempbuffer[j * tgbufLen + tg_pxl_cnt +
								   h_ofst] =
							    srcData[j * srcbufLen + i - 1];
							rgn_change = 0;
						} else
			...
			while (tg_pxl_cnt < targetWidth) {
				...
				if (OF == 0 && next_OF == 0)
					...
					temp =
					    (2048 - weight + factor_eval) * srcData[j * srcbufLen +
										    srcWidth - 1];
				else if (OF == 1 && next_OF == 0)
					temp =
					    weight * srcData[j * srcbufLen + srcWidth - 1] +
					...
					temp =
					    factor_eval * srcData[j * srcbufLen + srcWidth - 1] +
					    128 * hbuff;
				...
				if (next_OF == 0) {
					if (lk_en && (OF == 1) && (rgn_change == 1)) {
						tempbuffer[j * tgbufLen + tg_pxl_cnt + h_ofst] =
						    srcData[j * srcbufLen + srcWidth - 1];
						rgn_change = 0;
					} else if (lk_en && (rgn_change == 1)) {
						tempbuffer[j * tgbufLen + tg_pxl_cnt + h_ofst] =
						    srcData[j * srcbufLen + srcWidth - 1];
		...
		} else		/* C */
		{
			for (i = 2 * (1 + src_h_ofst); i < srcWidth; i = i + 2) {
				...
				if (OF == 0 && next_OF == 0) {
					...
					temp =
					    factor_eval * srcData[j * srcbufLen + i] + 128 * hbuff;
					temp2 =
					    factor_eval * srcData[j * srcbufLen + i + 1] +
					    128 * hbuff2;
				}
			...
			while (tg_pxl_cnt < targetWidth) {
				...
				if (OF == 0 && next_OF == 0) {
					...
					temp =
					    factor_eval * srcData[j * srcbufLen + srcWidth - 2] +
					    128 * hbuff;
					temp2 =
					    factor_eval * srcData[j * srcbufLen + srcWidth - 1] +
					    128 * hbuff2;
		...
		for (i = (1 + src_v_ofst); i < srcHeight; i++) {
			...
			for (j = 0; j < targetWidth; j++) {
				if (OF == 0 && next_OF == 0)
					...
					temp =
					    factor_eval * tempbuffer[i * tgbufLen + h_ofst + j] +
					    128 * vbuff[j];
		...
		while (tg_line_cnt < targetHeight) {
			...
			for (j = 0; j < targetWidth; j++) {
				if (OF == 0 && next_OF == 0)
					...
					temp =
					    factor_eval * tempbuffer[(srcHeight - 1) * tgbufLen +
								     h_ofst + j] + (2048 -
										    weight) *
					    tempbuffer[(srcHeight - 1) * tgbufLen + h_ofst + j];
				else if (OF == 1 && next_OF == 0)
			...
			for (i = (round + src_v_ofst); i < srcHeight; i = i + 2) {
				...
				for (j = 0; j < targetWidth; j++) {
					if (OF == 0 && next_OF == 0)
						...
						temp =
						    factor_eval * tempbuffer[i * tgbufLen + h_ofst +
									     j] + (2048 -
										   weight) *
						    tempbuffer[(i - 2) * tgbufLen + h_ofst + j];
					else if (OF == 1 && next_OF == 0)
						temp =
						    weight * tempbuffer[i * tgbufLen + h_ofst + j] +
						    128 * vbuff[j];
					...
					if (next_OF == 0)
						...
						} else if (src_agent == 2)	/* pp */
						{
							...
							} else	/* mp2, mp4 */
							{
								...
								if ((((i % 16) == 11) || ((i % 16) == 10)) && (i < (srcHeight - 6)) && (component == 0))	/* Y */
									vbuff[j] = v_result * 0x10;	/* 12 bit */
								else if ((((i % 8) == 3) || ((i % 8) == 2)) && (i < (srcHeight - 6)) && (component == 1))	/* C */
									vbuff[j] = v_result * 0x10;	/* 12 bit */
								else
									vbuff[j] = temp_round;	/* 12 bit */
#else
						...
						} else if (src_agent == 2)	/* pp */
						{
							...
							} else if ((spec_type == 2) && (mbaff == 1))	/* 264mbaff */
							{
								...
								if ((((i % 64) == 51) || ((i % 64) == 50)) && (i < (srcHeight - 14)) && (component == 0))	/* Y */
									...
									vbuff[j] = temp_round;	/* 12 bit */
#else
								if ((((i % 64) == 47) || ((i % 64) == 46)) && (i < (srcHeight - 18)) && (component == 0))	/* Y */
									vbuff[j] = v_result * 0x10;	/* 12 bit */
								else if ((((i % 32) == 23) || ((i % 32) == 22)) && (i < (srcHeight - 10)) && (component == 1))	/* C */
									vbuff[j] = v_result * 0x10;	/* 12 bit */
								else
									vbuff[j] = temp_round;	/* 12 bit */
#endif
							...
							} else	/* mp2, mp4 */
							{
								...
								if ((((i % 32) == 23) || ((i % 32) == 22)) && (i < (srcHeight - 10)) && (component == 0))	/* Y */
									vbuff[j] = v_result * 0x10;	/* 12 bit */
								else if ((((i % 16) == 7) || ((i % 16) == 6)) && (i < (srcHeight - 10)) && (component == 1))	/* C */
									vbuff[j] = v_result * 0x10;	/* 12 bit */
								else
									vbuff[j] = temp_round;	/* 12 bit */
#else
			...
			while (tg_line_cnt < targetHeight) {
				...
				for (j = 0; j < targetWidth; j++) {
					if (OF == 0 && next_OF == 0)
						...
						temp =
						    factor_eval *
						    tempbuffer[(srcHeight + round - 4) * tgbufLen +
							       h_ofst + j] + (2048 -
									      weight) *
						    tempbuffer[(srcHeight + round - 4) * tgbufLen +
							       h_ofst + j];
...
void vDvWrData2PC(UINT32 u4InstID, UCHAR *ptAddr)
{
...
#endif
	{
		for (i = 0; i < _u4GoldenYSize[u4InstID]; i++) {
			if ((_pucDumpYBuf[u4InstID][i] != pbDecBuf[i])) {
				u4Cnt++;
				vVDecOutputDebugString
				    ("Y Data Mismatch at [0x%.8x] = 0x%.2x, Golden = 0x%.2x !!!\n",
	...
	/* if(!fgIsMonoPic(_u4VDecID)) */
	{
		...
		{
#ifdef SATA_HDD_READ_SUPPORT
			fgOpenHDDFile(u4InstID, _bFileStr1[u4InstID][4], "r+b",
				      &_tFBufFileInfo[u4InstID]);
#else
			fgOpenPCFile(u4InstID, _bFileStr1[u4InstID][4], "r+b",
				     &_tFBufFileInfo[u4InstID]);
#endif
			_u4GoldenCSize[u4InstID] = _tFBufFileInfo[u4InstID].u4RealGetBytes;
		}
...
#endif
		{
			for (i = 0; i < _u4GoldenCSize[u4InstID]; i++) {
				if (_pucDumpCBuf[u4InstID][i] != pbDecBuf[i]) {
					u4Cnt++;
					vVDecOutputDebugString
					    ("CbCr Data Mismatch at [0x%.8x] = 0x%.2x, Golden = 0x%.2x !!!\n",
			...
			vVDecOutputDebugString
			    ("CbCr Data Compare Over!!! Total bytes [0x%.8x] & error [%d]\n",
			     _u4GoldenCSize[u4InstID], u4Cnt);
		}
#endif
	}
