...
/* Return      : TRUE/FALSE */
/* ********************************************************************* */
BOOL fgReadPCFile(void *pvAddr, UINT32 u4Size, char *strFileName, UINT32 u4OffSet,
		  UINT32 *pu4RealReadSize, UINT32 *pu4FileLength)
{
	FILE *pFile = NULL;
	UINT32 u4FileSize = 0;
...
BOOL fgReadHDDFile(UINT32 u4InstID, BOOL fgReadInfo, void *pvAddr, UINT32 u4Size, char *strFileName,
		   UINT32 u4OffSet, UINT32 *pu4RealReadSize, UINT32 *pu4FileLength,
		   INT32 *pi4FileId)
{
	...
	*pu4RealReadSize = 0;
	...
	if (fgReadInfo) {
		u4FILE = i4ReadFileFormHDD(u4InstID,
					   strFileName,
					   pvAddr,
					   u4OffSet, u4Size, pu4RealReadSize, pu4FileLength);
	} else {
		if ((pFile = linux_fopen(strFileName, "rb")) == NULL) {
			return FALSE;
		} else {
	...
	*pu4RealReadSize = 0;
	...
	if (fgReadInfo) {
		...
		fgRet = fgHDDFsReadFile(u4InstID,
					strFileName,
					pvAddr,
					u4OffSet,
					u4Size, pu4RealReadSize, pu4FileLength, pi4FileId);

		if (!fgRet)
			return FALSE;

		if (!
		    ((u4Size == *pu4RealReadSize)
		     || ((*pu4FileLength - u4OffSet) == *pu4RealReadSize))) {
			vVDecOutputDebugString("\n read fail: %s", strFileName);
			return FALSE;
		}

	} else {
		fgRet = fgHDDFsOpenFile(u4InstID, strFileName, pi4FileId);
...
BOOL fgOpenHDDFileEx(UINT32 u4InstID, char *strFileName, char *strMode,
		     VDEC_INFO_VERIFY_FILE_INFO_T *ptFileInfo, const char *szFile, INT32 i4Line)
{
	UINT32 u4Offset;
	...
	if (fgReadHDDFile
	    (u4InstID, ptFileInfo->fgGetFileInfo, ptFileInfo->pucTargetAddr, ptFileInfo->u4TargetSz,
	     strFileName, u4Offset, &(ptFileInfo->u4RealGetBytes), &(ptFileInfo->u4FileLength),
	     &(ptFileInfo->i4FileId))) {
...
/* Return      : None */
/* ********************************************************************* */
BOOL fgWrMsg2PC(void *pvAddr, UINT32 u4Size, UINT32 u4Mode,
...
BOOL fgOpenPCFile(UINT32 u4InstID, char *strFileName, char *strMode,
		  VDEC_INFO_VERIFY_FILE_INFO_T *ptFileInfo)
{
	FILE *pFile = NULL;
	UINT32 u4Offset;
	...
	if (ptFileInfo->fgGetFileInfo) {
		...
		if (fgReadPCFile
		    (ptFileInfo->pucTargetAddr, ptFileInfo->u4TargetSz, strFileName, u4Offset,
		     &(ptFileInfo->u4RealGetBytes), &(ptFileInfo->u4FileLength))) {
			/* vVDecOutputDebugString("The file's size is 0x%.8x bytes\n", ptFileInfo->u4FileLength); */
...
/* Return      : None */
/* ********************************************************************* */
BOOL fgOpenFileEx(UINT32 u4InstID, char *strFileName, char *strMode,
		  VDEC_INFO_VERIFY_FILE_INFO_T *ptFileInfo, const char *szFunction, INT32 i4Line)
{
	BOOL fgOpen = TRUE;
...
BOOL fgReadIdeFile(void *pvAddr, UINT32 u4Size, char *strFileName, UINT32 u4OffSet,
		   UINT32 *pu4RealReadSize, UINT32 *pu4FileLength)
{
	if ((NULL == pvAddr) || (0 == u4Size) || (NULL == strFileName) || (NULL == pu4RealReadSize))
		return FALSE;
