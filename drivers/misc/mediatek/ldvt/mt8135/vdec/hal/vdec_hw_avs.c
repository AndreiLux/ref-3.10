...
/* ********************************************************************* */
BOOL fgAVSVLDInitBarrelShifter(UINT32 u4BSID, UINT32 u4VDecID, UINT32 u4FIFOSa, UINT32 u4VLDRdPtr,
			       UINT32 u4VLDWrPtr)
{
	UINT32 u4ByteAddr;
...
BOOL fgInitAVSBarrelShift1(UINT32 u4VDecID, VDEC_INFO_AVS_BS_INIT_PRM_T * prAvsBSInitPrm)
{
	...
	vVDecSetVLDVFIFO(0, u4VDecID, PHYSICAL(prAvsBSInitPrm->u4VFifoSa),
			 PHYSICAL((UINT32) prAvsBSInitPrm->u4VFifoEa));

	if (!fgAVSVLDInitBarrelShifter
	    (0, u4VDecID, PHYSICAL((UINT32) prAvsBSInitPrm->u4VFifoSa),
	     PHYSICAL((UINT32) prAvsBSInitPrm->u4VLDRdPtr), PHYSICAL(prAvsBSInitPrm->u4VLDWrPtr))) {
		return FALSE;
	}
	return TRUE;
}
