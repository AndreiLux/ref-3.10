...
void vInitFgtHWSetting(UINT32 u4VDecID, VDEC_INFO_H264_INIT_PRM_T * prH264VDecInitPrm)
{
	...
	vVDecWriteAVCFG(u4VDecID, RW_FGT_SEED_ADDR,
			(PHYSICAL((UINT32) prH264VDecInitPrm->u4FGSeedbase)) >> 4);
	vVDecWriteAVCFG(u4VDecID, RW_FGT_SEI_ADDR_A,
			(PHYSICAL((UINT32) prH264VDecInitPrm->u4CompModelValue)) >> 4);
	vVDecWriteAVCFG(u4VDecID, RW_FGT_DATABASE_ADDR,
			(PHYSICAL((UINT32) prH264VDecInitPrm->u4FGDatabase)) >> 4);
#else
	vVDecWriteAVCFG(u4VDecID, RW_FGT_SEED_ADDR,
			(u4AbsDramANc((UINT32) prH264VDecInitPrm->u4FGSeedbase)) >> 4);
	vVDecWriteAVCFG(u4VDecID, RW_FGT_SEI_ADDR_A,
			(u4AbsDramANc((UINT32) prH264VDecInitPrm->u4CompModelValue)) >> 4);
	vVDecWriteAVCFG(u4VDecID, RW_FGT_DATABASE_ADDR,
			(u4AbsDramANc((UINT32) prH264VDecInitPrm->u4FGDatabase)) >> 4);
#endif
}
...
/* ********************************************************************* */
BOOL fgH264VLDInitBarrelShifter(UINT32 u4BSID, UINT32 u4VDecID, UINT32 u4FIFOSa, UINT32 u4VLDRdPtr,
				UINT32 u4VLDWrPtr)
{
	UINT32 u4ByteAddr;
	...
	if (u4BSID == 0) {
		...
		while (0x01000001 !=
		       (u4VDecReadVLD(u4VDecID, RO_VLD_VBAR + (u4BSID << 10)) & 0x03000003));
#else
		while (0x00000001 !=
		       (u4VDecReadVLD(u4VDecID, RO_VLD_VBAR + (u4BSID << 10)) & 0x03000003));
#endif
...
BOOL fgInitH264BarrelShift1(UINT32 u4VDecID, VDEC_INFO_H264_BS_INIT_PRM_T * prH264BSInitPrm)
{
...
#if (CONFIG_CHIP_VER_CURR < CONFIG_CHIP_VER_MT8560)
	vVDecWriteAVCVLD(u4VDecID, RW_AVLD_CTRL, AVC_EN | AVC_RDY_WITH_CNT | AVC_RDY_CNT_THD | AVLD_MEM_PAUSE_MOD_EN | AVC_SUM6_APPEND_INV | AVC_NOT_CHK_DATA_VALID | AVC_RBSP_CHK_INV | AVC_READ_FLAG_CHK_INV | AVC_ERR_BYPASS	/*| AVC_ERR_CONCEALMENT */
#if (CONFIG_DRV_VERIFY_SUPPORT)
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8530)
#else
			 | AVC_NON_SPEC_SWITCH
...
#else
	vVDecWriteAVCVLD(u4VDecID, RW_AVLD_CTRL, AVC_EN | AVC_RDY_WITH_CNT | AVC_RDY_CNT_THD | AVLD_MEM_PAUSE_MOD_EN | AVC_SUM6_APPEND_INV | AVC_NOT_CHK_DATA_VALID | AVC_RBSP_CHK_INV | AVC_ERR_BYPASS	/*| AVC_ERR_CONCEALMENT */
#if (CONFIG_DRV_VERIFY_SUPPORT)
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8530)
#else
			 | AVC_NON_SPEC_SWITCH
...
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
	vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_TIMEOUT_SW,
			 u4VDecReadVLDTOP(u4VDecID, RW_VLD_TOP_TIMEOUT_SW) | VLD_TOP_DEC_CYCLE_EN);
#endif
#endif
	vVDecWriteMC(u4VDecID, RW_MC_OPBUF, 6);

	vVDecSetVLDVFIFO(0, u4VDecID, PHYSICAL(prH264BSInitPrm->u4VFifoSa),
			 PHYSICAL((UINT32) prH264BSInitPrm->u4VFifoEa));
#if CONFIG_DRV_VIRTUAL_ADDR
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
	vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PRED_ADDR,
			 (prH264BSInitPrm->
			  u4PredSa ? PHYSICAL((UINT32) prH264BSInitPrm->u4PredSa) : 0));
#else
	vVDecWriteAVCVLD(u4VDecID, RW_AVLD_PRED_ADDR,
			 (prH264BSInitPrm->
			  u4PredSa ? PHYSICAL((UINT32) prH264BSInitPrm->u4PredSa) : 0));
#endif
#else
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
	vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PRED_ADDR,
			 u4AbsDramANc((UINT32) prH264BSInitPrm->u4PredSa));
#else
	vVDecWriteAVCVLD(u4VDecID, RW_AVLD_PRED_ADDR,
			 u4AbsDramANc((UINT32) prH264BSInitPrm->u4PredSa));
#endif
	...
	if (!fgH264VLDInitBarrelShifter
	    (0, u4VDecID, PHYSICAL((UINT32) prH264BSInitPrm->u4VFifoSa),
	     PHYSICAL((UINT32) prH264BSInitPrm->u4VLDRdPtr),
	     PHYSICAL(prH264BSInitPrm->u4VLDWrPtr))) {
		return FALSE;
	}
	return TRUE;
}
...
BOOL fgInitH264BarrelShift2(UINT32 u4VDecID, VDEC_INFO_H264_BS_INIT_PRM_T * prH264BSInitPrm)
{
	...
	vVDecSetVLDVFIFO(1, u4VDecID, PHYSICAL(prH264BSInitPrm->u4VFifoSa),
			 PHYSICAL((UINT32) prH264BSInitPrm->u4VFifoEa));

	if (!fgH264VLDInitBarrelShifter
	    (1, u4VDecID, PHYSICAL((UINT32) prH264BSInitPrm->u4VFifoSa),
	     PHYSICAL((UINT32) prH264BSInitPrm->u4VLDRdPtr),
	     PHYSICAL(prH264BSInitPrm->u4VLDWrPtr))) {
		return FALSE;
	}
...
#ifdef VDEC_SIM_DUMP
	printk
	    ("<vdec> ReadH264VldRPtr, dRptr:0x%08X, sra:0x%08X, swa:0x%08X, scnt:0x%08X, sum:0x%08X\n",
...
/* u4SramRptr = (u4VDecReadVLD(u4VDecID, RO_VLD_VBAR + (u4BSID << 10)) & 0xf) * 4; //count in 128bits */
	u4SramRptr = ((u4VDecReadVLD(u4VDecID, RO_VLD_VBAR + (u4BSID << 10))) & 0xf) * 4 + (((u4VDecReadVLD(u4VDecID, RO_VLD_VBAR + (u4BSID << 10)) >> 24)) & 0x3);	/* count in 128bits */
