...
#endif
	vVDecSetVLDVFIFO(0, u4VDecID, PHYSICAL(prMpegVFifoPrm->u4VFifoSa),
			 PHYSICAL(prMpegVFifoPrm->u4VFifoEa));

	/* for MT8320 */
	if ((prMpegVFifoPrm->u4CodeType == VDEC_MPEG) || (prMpegVFifoPrm->u4CodeType == VDEC_MPEG1)
	    || (prMpegVFifoPrm->u4CodeType == VDEC_MPEG2)) {
...
/* <0: fail. */
/* ************************************************************************** */
INT32 i4VDEC_HAL_MPEG_InitBarrelShifter(UINT32 u4BSID, UINT32 u4VDecID,
					VDEC_INFO_MPEG_BS_INIT_PRM_T * prMpegBSInitPrm)
{
	...
	vVDecSetVLDVFIFO(u4BSID, u4VDecID, PHYSICAL((UINT32) prMpegBSInitPrm->u4VFifoSa),
			 PHYSICAL((UINT32) prMpegBSInitPrm->u4VFifoEa));

	u4VLDRemainByte = ((prMpegBSInitPrm->u4ReadPointer)) & 0xf;


	vVDecWriteVLD(u4VDecID, RW_VLD_RPTR + (u4BSID << 10),
		      PHYSICAL((UINT32) prMpegBSInitPrm->u4ReadPointer));
	vVDecWriteVLD(u4VDecID, WO_VLD_WPTR + (u4BSID << 10),
		      PHYSICAL((UINT32) prMpegBSInitPrm->u4WritePointer));
	...
	if (_u4CodecVer[u4VDecID] == VDEC_MPEG4 || _u4CodecVer[u4VDecID] == VDEC_H263
	    || _u4CodecVer[u4VDecID] == VDEC_DIVX3) {
	...
	vVDecSetVLDVFIFO(u4BSID, u4VDecID, PHYSICAL(prMpegBSInitPrm->u4VFifoSa),
			 PHYSICAL(prMpegBSInitPrm->u4VFifoEa));
	...
	for (i = 0; i < 5; i++) {
		...
		vVDecWriteVLD(u4VDecID, RW_VLD_RPTR + (u4BSID << 10),
			      PHYSICAL(prMpegBSInitPrm->u4ReadPointer));
		vVDecWriteVLD(u4VDecID, RW_VLD_RPTR + (u4BSID << 10),
			      PHYSICAL(prMpegBSInitPrm->u4ReadPointer));
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8530)
		vVDecWriteVLD(u4VDecID, WO_VLD_WPTR + (u4BSID << 10),
			      PHYSICAL(prMpegBSInitPrm->u4WritePointer));
		vVDecWriteVLD(u4VDecID, RW_VLD_ASYNC + (u4BSID << 10),
			      u4VDecReadVLD(u4VDecID, RW_VLD_ASYNC) | VLD_WR_ENABLE);
#else
		vVDecWriteVLD(u4VDecID, WO_VLD_WPTR + (u4BSID << 10),
			      ((PHYSICAL(prMpegBSInitPrm->u4WritePointer) << 4) | 0x2));
#endif
		/* start to fetch data */
		vVDecWriteVLD(u4VDecID, RW_VLD_PROC + (u4BSID << 10), VLD_INIFET);
...
/* ************************************************************************** */
#if (CONFIG_DRV_ONLY || CONFIG_DRV_VERIFY_SUPPORT)
void vVDEC_HAL_MPEG12_LoadQuantMatrix(UINT32 u4VDecID, BOOL fgIntra,
				      VDEC_INFO_MPEG_QANTMATRIX_T * prQuantMatrix)
{
...
{
	vVDecWriteVLD(u4VDecID, WO_VLD_FDEC,
		      (u4VDecReadVLD(u4VDecID, WO_VLD_FDEC) & VLD_PIC_COMPLETE) | VLD_RST_QMATRIX);
}
...
/* Return      : None */
/* ************************************************************************** */
void vVDEC_HAL_MPEG_ReStoreQuantMatrix(UINT32 u4VDecID, BOOL fgIntra,
				       VDEC_INFO_MPEG_QANTMATRIX_T * prQuantMatrix)
{
	return;
}
...
void vVDEC_HAL_MPEG_SetMPEG4Flag(UINT32 u4VDecID, BOOL fgMp4)
{
	...
	if (fgMp4) {
...

		{
			...
			u4RegVal |=
			    (0x1 << 16) | ((_u4CodecVer[u4VDecID] == VDEC_DIVX3) ? (0x1 << 9)
					   : (0x0 << 9));
			vVDecWriteVLD(u4VDecID, RW_VLD_MP4_FLG, u4RegVal);
			/* printk("Test1 RW_VLD_MP4_FLG value is 0x%x\n", u4RegVal); */
		}
...
{
	vVDecWriteVLD(u4VDecID, WO_VLD_BBUFRST,
		      u4VDecReadVLD(u4VDecID, WO_VLD_BBUFRST) | VLD_MVOF_DET_DISABLE);
	return HAL_HANDLE_OK;
}
...
void vVDEC_HAL_MPEG_VDec_DumpReg(UINT32 u4VDecID, BOOL fgBefore)
{
	...
	{
		u4Temp = i;
		sprintf(strMessage, "%d (0x%.4x) = 0x%.8x\n", u4Temp, 0x2b800 + (u4Temp << 2),
			u4ReadVDSCL(u4VDecID, (u4Temp << 2)));
		fgWrMsg2PC(strMessage, strlen(strMessage), 8, &_tRecFileInfo[u4VDecID]);
	...
	for (i = i4VldStart; i < i4VldEnd; i++)	/*  */
	{
		u4Temp = i;
		sprintf(strMessage, "%d (0x%.4x) = 0x%.8x\n", u4Temp, (u4Temp << 2),
			u4VDecReadVLD(u4VDecID, (u4Temp << 2)));
		fgWrMsg2PC(strMessage, strlen(strMessage), 8, &_tRecFileInfo[u4VDecID]);
		/* fprintf(_tRecFileInfo.fpFile, "%d (0x%.4x) = 0x%.8x\n", u4Temp, 0xf800+(u4Temp << 2), u4VDecReadVLD(u4VDecID, (u4Temp << 2))); */
	}
	...
	for (i = i4McStart; i < i4McEnd; i++)	/*  */
	{
		u4Temp = i;
		sprintf(strMessage, "%d (0x%.4x) = 0x%.8x\n", u4Temp, (u4Temp << 2),
			u4VDecReadMC(u4VDecID, (u4Temp << 2)));
		fgWrMsg2PC(strMessage, strlen(strMessage), 8, &_tRecFileInfo[u4VDecID]);
		/* fprintf(_tRecFileInfo.fpFile, "%d (0x%.4x) = 0x%.8x\n", u4Temp, 0xf800+(u4Temp << 2), u4VDecReadVLD(u4VDecID, (u4Temp << 2))); */
	}
#endif
