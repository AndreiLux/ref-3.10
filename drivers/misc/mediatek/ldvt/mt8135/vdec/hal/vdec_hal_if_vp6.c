...
static void vBoolTreeToHuffCodes(UINT8 *coeff_model, const UINT8 *map, UINT32 size,
				 UINT32 *huff_probs)
{
	int a, b, i;
	UINT32 *tmp;
...
void vVDEC_HAL_VP6_InitCtx(UINT32 u4VDecID, UINT32 u4BSID, VDEC_INFO_VP6_FRM_HDR_T *prFrmHdr)
{
	...
	if (prFrmHdr->fgUseHuffman) {
		...
		/* Trigger Prob Update */
		u4Cnt = 0;
		vVDecWriteVP6VLD(u4VDecID, RW_VP6_VLD_PROB_UPD,
				 (u4VDecReadVP6VLD(u4VDecID, RW_VP6_VLD_PROB_UPD) | 0x1), u4BSID);
		...
		for (j = 0; j < 36; j++) {
			...
			for (i = 0; i < 4; i++) {
				u4Addr =
				    MC_VLD_WRAPPER_READ | ((VLD_WRAPPER_AC_BASE + j) << 2) | (3 -
											      i);
			...
			for (i = 0; i < 6; i++) {
				...
				code1 =
				    (huff_code[2 * i + 1].len << 12) | huff_code[2 * i + 1].hcode;
				vVDecWriteVP6VLD(u4VDecID, RW_VP6_VLD_FCVR1 + 4 * i,
						 ((code0 << 16) | code1), u4BSID);
			}
			u4Addr2++;
		}
		...
		for (j = 0; j < 4; j++) {
			for (i = 0; i < 4; i++) {
				u4Addr =
				    MC_VLD_WRAPPER_READ | ((VLD_WRAPPER_DC_Y + j) << 2) | (3 - i);
				vVDecWriteMC(u4VDecID, RW_MC_VLD_WRAPPER_ADDR, u4Addr);
				u4Tmp = u4VDecReadMC(u4VDecID, RW_MC_VLD_WRAPPER_DATA);
				coeff_model[i] = ((u4Tmp & 0x000000ff) << 24) |
				    ((u4Tmp & 0x0000ff00) << 8) |
			...
			if (j < 2) {
				...
				vBoolTreeToHuffCodes((UINT8 *) coeff_model, vp6_huff_run_map, 9,
						     huff_prob);
		...
		for (i = 0; i < 6; i++) {
			...
			code1 =
			    (eob_huff_code[2 * i + 1].len << 12) | eob_huff_code[2 * i + 1].hcode;
			vVDecWriteVP6VLD(u4VDecID, RW_VP6_VLD_FCVR1 + 4 * i,
					 ((code0 << 16) | code1), u4BSID);
		}
	} else {
		for (i = 0; i < 40; i++) {
			for (j = 0; j < 4; j++) {
				u4Addr =
				    MC_VLD_WRAPPER_READ | ((VLD_WRAPPER_DC_Y + i) << 2) | (3 - j);
				vVDecWriteMC(u4VDecID, RW_MC_VLD_WRAPPER_ADDR, u4Addr);
			...
			vVDecWriteVP6VLD(u4VDecID, RW_VP6_VLD_WARR, u4Addr, u4BSID);
			for (j = 0; j < 4; j++) {
				vVDecWriteVP6VLD(u4VDecID, RW_VP6_VLD_FCVR1 + 4 * j, coeff_model[j],
						 u4BSID);
			}
		}
	}
...
INT32 i4VDEC_HAL_VP6_InitVDecHW(UINT32 u4VDecID, VDEC_INFO_VP6_VFIFO_PRM_T * prVp6VFifoInitPrm)
{
	...
	vVDecSetVLDVFIFO(0, u4VDecID, PHYSICAL((UINT32) prVp6VFifoInitPrm->u4VFifoSa),
			 PHYSICAL((UINT32) prVp6VFifoInitPrm->u4VFifoEa));
...
UINT32 u4VDEC_HAL_VP6_GetBitStreamShift(UINT32 u4BSID, UINT32 u4VDecID, UINT32 u4ShiftBits)
{
    ...
    /* <0: fail. */
    /* ************************************************************************** */
INT32 i4VDEC_HAL_VP6_InitBarrelShifter(UINT32 u4BSID, UINT32 u4VDecID,
				       VDEC_INFO_VP6_BS_INIT_PRM_T * prVp6BSInitPrm)
{
	...
	vVDecSetVLDVFIFO(u4BSID, u4VDecID, PHYSICAL((UINT32) prVp6BSInitPrm->u4VFifoSa),
			 PHYSICAL((UINT32) prVp6BSInitPrm->u4VFifoEa));
	...
	vVDecWriteVLD(u4VDecID, RW_VLD_RPTR + (u4BSID << 10),
		      PHYSICAL((UINT32) prVp6BSInitPrm->u4ReadPointer));
	vVDecWriteVLD(u4VDecID, WO_VLD_WPTR + (u4BSID << 10),
		      PHYSICAL((UINT32) prVp6BSInitPrm->u4WritePointer));
    ...
    /* <0: fail. */
    /* ************************************************************************** */
INT32 i4VDEC_HAL_VP6_InitBarrelShifter2(UINT32 u4BSID, UINT32 u4VDecID,
					VDEC_INFO_VP6_BS_INIT_PRM_T * prVp6BSInitPrm,
					VDEC_INFO_VP6_FRM_HDR_T *prVDecVp6FrmHdr)
{
	...
	vVDecVP6SetVLD2VFIFO(PHYSICAL((UINT32) prVp6BSInitPrm->u4VFifoSa),
			     PHYSICAL((UINT32) prVp6BSInitPrm->u4VFifoEa));
	...
	if (prVDecVp6FrmHdr) {
		...
		vVDecWriteVP6VLD(u4VDecID, RW_VP6_PCI_PAR,
				 (u4KeyFrame | u4Huffman | u4Multi | RW_VP6_FLAG), u4BSID);
	} else {
...
INT32 i4VDEC_HAL_VP6_DecStart(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T * prDecPrm)
{
	...
	VDEC_INFO_VP6_DEC_PRM_T *prVDecVP6DecPrm =
	    (VDEC_INFO_VP6_DEC_PRM_T *) &(prDecPrm->SpecDecPrm.rVDecVP6DecPrm);
#else
	VDEC_INFO_VP6_DEC_PRM_T *prVDecVP6DecPrm =
	    (VDEC_INFO_VP6_DEC_PRM_T *) prDecPrm->prVDecCodecHalPrm;
#endif
	...
	vVDecWriteVLDTOP(u4VDecID, RW_PIC_MB_SIZE_M1,
			 (((u4HoriMb - 1) & 0x3FF) | (((u4VertMb - 1) & 0x3FF) << 16)));
	vVDecWriteVLDTOP(u4VDecID, RW_PIC_PIX_SIZE,
			 (((u4HoriMb << 4) & 0x3FFF) | (((u4VertMb << 4) & 0x3FFF) << 16)));


	/* Write IQ_QUANT */
	u4Pp200 =
	    ((prVDecVP6DecPrm->prFrmHdr->u4DQuant_Ac << 8) | prVDecVP6DecPrm->prFrmHdr->
	     u4DQuant_Dc);
...
#if 0
	u4Mc864 =
	    ((prVDecVP6DecPrm->prFrmHdr->u4Mv_Thr_En << 16) | (prVDecVP6DecPrm->prFrmHdr->
							       u4Max_Vector_Length));
	vVDecWriteMC(u4VDecID, RW_VP6_MV_TH, u4Mc864);

	u4Mc868 =
	    ((prVDecVP6DecPrm->prFrmHdr->u4BilinearFilter << 28) | (prVDecVP6DecPrm->prFrmHdr->
								    u4Var_Thr_En << 20)
	...
	if (prVDecVP6DecPrm->prFrmHdr->ucFilter_Mode != 2) {
		...
		u4Mc868 =
		    ((prVDecVP6DecPrm->prFrmHdr->u4Var_Thr_En << 20) | (prVDecVP6DecPrm->prFrmHdr->
									u4Sample_Variance_Threshold));
		vVDecWriteMC(u4VDecID, RW_VP6_AUTOSEL, u4Mc868);
	...
	vVDecWriteVP6VLD(u4VDecID, RW_VP6_VLD_ETC,
			 ETC_COEFF_ERROR | ETC_MC_TIMEOUT | ETC_MV_TIMEOUT | ETC_DCAC_TIMEOUT |
			 ETC_HUFF_ERR, u4BSID);
...
#if VMMU_SUPPORT
	{
		...
		u4Page_addr =
		    ...
		    ((((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.u4Pic1YSa) / (4 * 1024)) * 4);
		printk("[VP6] i4VDEC_HAL_VP6_DecStart, 1, Page Addr = 0x%x\n", u4Page_addr);
		vPage_Table(u4VDecID, u4Page_addr,
			    PHYSICAL((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.u4Pic1YSa),
			    PHYSICAL((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.u4Pic1YSa) +
			    PIC_Y_SZ);
		vVDecWriteMC(u4VDecID, RW_VP6_MC_RY,
			     ((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.
			      u4Pic1YSa) >> prVDecVP6DecPrm->i4MemBase >> 9);

		u4Page_addr =
		    (UINT32) _pucVMMUTable[u4VDecID] +
		    ((((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.u4Pic1CSa) / (4 * 1024)) * 4);
		printk("[VP6] i4VDEC_HAL_VP6_DecStart, 2, Page Addr = 0x%x\n", u4Page_addr);
		vPage_Table(u4VDecID, u4Page_addr,
			    PHYSICAL((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.u4Pic1CSa),
			    PHYSICAL((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.u4Pic1CSa) +
			    PIC_C_SZ);
		vVDecWriteMC(u4VDecID, RW_VP6_MC_RC,
			     ((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.
			      u4Pic1CSa) >> prVDecVP6DecPrm->i4MemBase >> 8);

		u4Page_addr =
		    (UINT32) _pucVMMUTable[u4VDecID] +
		    ((((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.u4Pic2YSa) / (4 * 1024)) * 4);
		printk("[VP6] i4VDEC_HAL_VP6_DecStart, 3, Page Addr = 0x%x\n", u4Page_addr);
		vPage_Table(u4VDecID, u4Page_addr,
			    PHYSICAL((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.u4Pic2YSa),
			    PHYSICAL((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.u4Pic2YSa) +
			    PIC_Y_SZ);
		vVDecWriteMC(u4VDecID, RW_VP6_MC_GY,
			     ((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.
			      u4Pic2YSa) >> prVDecVP6DecPrm->i4MemBase >> 9);

		u4Page_addr =
		    (UINT32) _pucVMMUTable[u4VDecID] +
		    ((((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.u4Pic2CSa) / (4 * 1024)) * 4);
		printk("[VP6] i4VDEC_HAL_VP6_DecStart, 4, Page Addr = 0x%x\n", u4Page_addr);
		vPage_Table(u4VDecID, u4Page_addr,
			    PHYSICAL((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.u4Pic2CSa),
			    PHYSICAL((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.u4Pic2CSa) +
			    PIC_C_SZ);
		vVDecWriteMC(u4VDecID, RW_VP6_MC_GC,
			     ((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.
			      u4Pic2CSa) >> prVDecVP6DecPrm->i4MemBase >> 8);

		u4Page_addr =
		    (UINT32) _pucVMMUTable[u4VDecID] +
		    ((((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.u4Pic0YSa) / (4 * 1024)) * 4);
		printk("[VP6] i4VDEC_HAL_VP6_DecStart, 5, Page Addr = 0x%x\n", u4Page_addr);
		vPage_Table(u4VDecID, u4Page_addr,
			    PHYSICAL((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.u4Pic0YSa),
			    PHYSICAL((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.u4Pic0YSa) +
			    PIC_Y_SZ);
		vVDecWriteMC(u4VDecID, RW_VP6_MC_DY,
			     ((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.
			      u4Pic0YSa) >> prVDecVP6DecPrm->i4MemBase >> 9);

		u4Page_addr =
		    (UINT32) _pucVMMUTable[u4VDecID] +
		    ((((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.u4Pic0CSa) / (4 * 1024)) * 4);
		printk("[VP6] i4VDEC_HAL_VP6_DecStart, 6, Page Addr = 0x%x\n", u4Page_addr);
		vPage_Table(u4VDecID, u4Page_addr,
			    PHYSICAL((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.u4Pic0CSa),
			    PHYSICAL((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.u4Pic0CSa) +
			    PIC_C_SZ);
		vVDecWriteMC(u4VDecID, RW_VP6_MC_DC,
			     ((UINT32) prVDecVP6DecPrm->rVp6FrameBufSa.
			      u4Pic0CSa) >> prVDecVP6DecPrm->i4MemBase >> 8);
	...
	if (prVDecVP6DecPrm->rVp6PpInfo.fgPpEnable) {
		...
		vVDecWriteMC(u4VDecID, RW_MC_PP_Y_ADDR,
			     PHYSICAL(prVDecVP6DecPrm->rVp6PpInfo.u4PpYBufSa) >> 9);
		vVDecWriteMC(u4VDecID, RW_MC_PP_C_ADDR,
			     PHYSICAL(prVDecVP6DecPrm->rVp6PpInfo.u4PpCBufSa) >> 8);
		/* ddr3 -> 4x */
		vVDecWriteMC(u4VDecID, RW_MC_PP_MB_WIDTH, (prDecPrm->u4PicW + 15) >> 4);


		u4MBqp =
		    (prVDecVP6DecPrm->rVp6PpInfo.
		     au1MBqp[0] & 0x1F) | ((UINT32) (prVDecVP6DecPrm->rVp6PpInfo.
						     au1MBqp[1] & 0x1F) << 8)
		    | ((UINT32) (prVDecVP6DecPrm->rVp6PpInfo.au1MBqp[2] & 0x1F) << 16) |
		    ((UINT32) (prVDecVP6DecPrm->rVp6PpInfo.au1MBqp[3] & 0x1F) << 24);
		...
		vVDecWriteMC(u4VDecID, RW_MC_PP_MB_WIDTH,
			     prVDecVP6DecPrm->prFrmHdr->u2HFragments - 1);
	}
	...
	vVDecWriteMC(u4VDecID, RW_MC_PP_X_RANGE, ((prDecPrm->u4PicW + 15) >> 4) - 1);
	vVDecWriteMC(u4VDecID, RW_MC_PP_Y_RANGE,
		     (((prDecPrm->u4PicH + 15) >> 4) >> (prDecPrm->ucPicStruct != FRM_PIC)) - 1);
	...
	if (prVDecVP6DecPrm->prFrmHdr->u2Buff2Offset
	    || (prVDecVP6DecPrm->u1AlphaFlag & VP6_ALPHA_ENABLE)) {
...
UINT32 u4VDEC_HAL_VP6_Parse_Mb_Type_Models(UINT32 u4BSID, UINT32 u4VDecID)
{
	...
	for (idx = 0; idx < 16; idx++) {
		for (ctx = 0; ctx < 3; ctx++) {
			for (i = 0; i < 2; i++) {
				u4Addr =
				    MC_VLD_WRAPPER_WRITE | ((idx * 3 * 2 + ctx * 2 + i + pred_range)
							    << 2);
				...
				u4Tmp0 = vp56_pre_def_mb_type_stats[idx][ctx][0][i],
				    ...
				    u4Val =
				    ((u4Tmp0 << 24) | (u4Tmp1 << 16) | (u4Tmp2 << 8) | (u4Tmp3));
				vVDecWriteMC(u4VDecID, RW_MC_VLD_WRAPPER_ADDR, (u4Addr | 0x3));
				vVDecWriteMC(u4VDecID, RW_MC_VLD_WRAPPER_DATA, u4Val);
...
UINT32 u4VDEC_HAL_VP6_VDec_SetWorkspace(UINT32 u4VDecID, UINT32 u4VLDWrapperWorkspace,
...
UINT32 u4VDec_HAL_VP6_VDec_BackupSram(UINT32 u4VDecID, VDEC_INFO_VP6_DEC_PRM_T *prDecPrm)
{
	...
	for (i = 0; i < (VLD_WRAPPER_AC_BASE_END - VLD_WRAPPER_UPD_MP_TYPE + 1); i++) {
		for (j = 0; j < 4; j++) {
			u4Addr =
			    MC_VLD_WRAPPER_READ | ((VLD_WRAPPER_UPD_MP_TYPE + i) << 2) | (3 - j);
			vVDecWriteMC(u4VDecID, RW_MC_VLD_WRAPPER_ADDR, u4Addr);
...
UINT32 u4VDec_HAL_VP6_VDec_RestoreSram(UINT32 u4VDecID, VDEC_INFO_VP6_DEC_PRM_T *prDecPrm)
{
	...
	for (i = 0; i < (VLD_WRAPPER_AC_BASE_END - VLD_WRAPPER_UPD_MP_TYPE + 1); i++) {
		for (j = 0; j < 4; j++) {
			u4Addr =
			    MC_VLD_WRAPPER_WRITE | ((VLD_WRAPPER_UPD_MP_TYPE + i) << 2) | (3 - j);
			vVDecWriteMC(u4VDecID, RW_MC_VLD_WRAPPER_ADDR, u4Addr);
