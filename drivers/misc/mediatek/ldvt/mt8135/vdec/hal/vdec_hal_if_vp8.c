...
INT32 i4VDEC_HAL_VP8_InitVDecHW(UINT32 u4VDecID, VDEC_INFO_VP8_VFIFO_PRM_T * prVp8VFifoInitPrm)
{
	...
	vVDecSetVLDVFIFO(0, u4VDecID, PHYSICAL((UINT32) prVp8VFifoInitPrm->u4VFifoSa),
			 PHYSICAL((UINT32) prVp8VFifoInitPrm->u4VFifoEa));
...
BOOL fgVDEC_Vp8DecReset(UINT32 u4InstID, VDEC_INFO_VP8_FRM_HDR_T *pVp8DecInfo, BOOL fgInit)
{
	...
	if (!fgInit) {
		...
		vVDecWriteVP8VLD2(0, RW_VLD_VSTART,
				  PHYSICAL((UINT32) (pVp8DecInfo->u4FifoStart)) >> 6);
...
{
	return (BOOL) ((u4VDecReadVP8VLD(0, RO_VP8_PFR) & 1)
		       || (u4VDecReadVP8VLD(0, RO_VP8_VOKR) & 1));
}
...
/* <0: fail. */
/* ************************************************************************** */
INT32 i4VDEC_HAL_VP8_InitBarrelShifter(UINT32 u1VP8VLD, UINT32 u4VDecID,
				       VDEC_INFO_VP8_FRM_HDR_T *pVp8DecInfo)
{
	...
	if (u1VP8VLD == VP8_VLD1) {
		...
		vVDecWriteVLD(u4VDecID, RW_VLD_VSTART,
			      PHYSICAL((UINT32) pVp8DecInfo->u4FifoStart) >> 6);
		vVDecWriteVLD(u4VDecID, RW_VLD_VEND,
			      PHYSICAL((UINT32) pVp8DecInfo->u4FifoEnd) >> 6);
	} else {
		vVDecWriteVP8VLD2(u4VDecID, RW_VLD_RDY_SWTICH, READY_TO_RISC_1);
		vVDecWriteVP8VLD2(u4VDecID, RW_VLD_VSTART,
				  PHYSICAL((UINT32) pVp8DecInfo->u4FifoStart) >> 6);
		vVDecWriteVP8VLD2(u4VDecID, RW_VLD_VEND,
				  PHYSICAL((UINT32) pVp8DecInfo->u4FifoEnd) >> 6);
	}
	...
	for (i = 0; i < 5; i++) {
		if (u1VP8VLD == VP8_VLD1) {
			...
			vVDecWriteVLD(u4VDecID, RW_VLD_RPTR,
				      PHYSICAL((UINT32) pVp8DecInfo->u4VldStartPos));
			vVDecWriteVLD(u4VDecID, RW_VLD_RPTR,
				      PHYSICAL((UINT32) pVp8DecInfo->u4VldStartPos));
			vVDecWriteVLD(u4VDecID, WO_VLD_WPTR,
				      PHYSICAL((UINT32) pVp8DecInfo->u4WritePos));
			...
			vVDecWriteVP8VLD2(u4VDecID, WO_VLD_WPTR,
					  u4VDecReadVP8VLD2(u4VDecID,
							    WO_VLD_WPTR) | VLD_CLEAR_PROCESS_EN);
			vVDecWriteVP8VLD2(u4VDecID, RW_VLD_RPTR,
					  PHYSICAL((UINT32) (pVp8DecInfo->u4VldStartPos)));
			vVDecWriteVP8VLD2(u4VDecID, RW_VLD_RPTR,
					  PHYSICAL((UINT32) (pVp8DecInfo->u4VldStartPos)));
			vVDecWriteVP8VLD2(u4VDecID, WO_VLD_WPTR,
...
/*
   ...
   */
	...
	vVDecWriteVP8MC(u4VDecID, RW_MC_NBM_CTRL,
			((u4VDecReadVP8MC(u4VDecID, RW_MC_NBM_CTRL) & 0xFFFFFFF8) | ADDRSWAP_DDR3));
#else
	...
	vVDecWriteVP8MC(u4VDecID, RW_MC_NBM_CTRL,
			((u4VDecReadVP8MC(u4VDecID, RW_MC_NBM_CTRL) & 0xFFFFFFF8) | ADDRSWAP_OFF));
#endif

	/* RW_MC_VP8SETTING */
	u4HalValue = u4VDecReadVP8MC(u4VDecID, RW_MC_VP8SETTING);
	u4HalValue = u4HalValue & 0x1;
	VDEC_REG_SET_FLAG(u4HalValue, VDEC_FLGSET(u4FlagParam, VP8PARAM_FULL_PIXEL),
			  RW_MC_VP8FULLPEL_FLAG);
	...
	vVDecWriteVP8MC(u4VDecID, RW_MC_LUMA_SIZE,
			(pVp8DecInfo->u4CurCAddr - pVp8DecInfo->u4CurYAddr));
#if VMMU_SUPPORT
	u4Page_addr =
	    (UINT32) _pucVMMUTable[u4VDecID] + (((pVp8DecInfo->u4LstYAddr) / (4 * 1024)) * 4);
	/* printk("Page Addr = 0x%x\n", u4Page_addr); */
	vPage_Table(u4VDecID, u4Page_addr, PHYSICAL((UINT32) pVp8DecInfo->u4LstYAddr),
		    PHYSICAL((UINT32) pVp8DecInfo->u4LstYAddr) + PIC_Y_SZ + PIC_C_SZ);
	vVDecWriteVP8MC(u4VDecID, RW_MC_PIC1Y_ADD, (pVp8DecInfo->u4LstYAddr));

	u4Page_addr =
	    (UINT32) _pucVMMUTable[u4VDecID] + (((pVp8DecInfo->u4GldYAddr) / (4 * 1024)) * 4);
	/* printk("Page Addr = 0x%x\n", u4Page_addr); */
	vPage_Table(u4VDecID, u4Page_addr, PHYSICAL((UINT32) pVp8DecInfo->u4GldYAddr),
		    PHYSICAL((UINT32) pVp8DecInfo->u4GldYAddr) + PIC_Y_SZ + PIC_C_SZ);
	vVDecWriteVP8MC(u4VDecID, RW_MC_PIC2Y_ADD, (pVp8DecInfo->u4GldYAddr));

	u4Page_addr =
	    (UINT32) _pucVMMUTable[u4VDecID] + (((pVp8DecInfo->u4AlfYAddr) / (4 * 1024)) * 4);
	/* printk("Page Addr = 0x%x\n", u4Page_addr); */
	vPage_Table(u4VDecID, u4Page_addr, PHYSICAL((UINT32) pVp8DecInfo->u4AlfYAddr),
		    PHYSICAL((UINT32) pVp8DecInfo->u4AlfYAddr) + PIC_Y_SZ + PIC_C_SZ);
...
#endif
	vVDecWriteVP8MC(u4VDecID, RW_VLD_MBDRAM_SEL,
			u4VDecReadVP8MC(u4VDecID, RW_VLD_MBDRAM_SEL) | (1 << 16));
...
/* #endif */
	{
		...
		u4Page_addr =
		    ...
		    (((pVp8DecInfo->u4CurYAddr) / (4 * 1024)) * 4);
		/* printk("Page Addr = 0x%x\n", u4Page_addr); */
		vPage_Table(u4VDecID, u4Page_addr, PHYSICAL((UINT32) pVp8DecInfo->u4CurYAddr),
			    PHYSICAL((UINT32) pVp8DecInfo->u4CurYAddr) + PIC_Y_SZ);
		vVDecWriteVP8MC(u4VDecID, RW_MC_PP_Y_ADDR, (pVp8DecInfo->u4CurYAddr) >> 9);

		u4Page_addr =
		    (UINT32) _pucVMMUTable[u4VDecID] +
		    (((pVp8DecInfo->u4CurCAddr) / (4 * 1024)) * 4);
		/* printk("Page Addr = 0x%x\n", u4Page_addr); */
		vPage_Table(u4VDecID, u4Page_addr, PHYSICAL((UINT32) pVp8DecInfo->u4CurCAddr),
			    PHYSICAL((UINT32) pVp8DecInfo->u4CurCAddr) + PIC_C_SZ);
		vVDecWriteVP8MC(u4VDecID, RW_MC_PP_C_ADDR, (pVp8DecInfo->u4CurCAddr) >> 8);

		vVDecVMMUEnable(PHYSICAL((UINT32) _pucVMMUTable[u4VDecID]));
#else
		vVDecWriteVP8MC(u4VDecID, RW_MC_PP_Y_ADDR,
				PHYSICAL((UINT32) pVp8DecInfo->u4CurYAddr) >> 9);
		vVDecWriteVP8MC(u4VDecID, RW_MC_PP_C_ADDR,
				PHYSICAL((UINT32) pVp8DecInfo->u4CurCAddr) >> 8);
#endif
#if VDEC_DDR3_SUPPORT
		vVDecWriteVP8MC(u4VDecID, RW_MC_PP_MB_WIDTH,
				((pVp8DecInfo->u4Width + 63) >> 6) << 2);
#else
		...
		vVDecWriteVP8MC(u4VDecID, RW_MC_PP_Y_RANGE,
				((pVp8DecInfo->u4Height + 15) >> 4) - 1);
		/* how set */
...
/* else */
	...
	vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_SEGID_DRAM_ADDR,
			 PHYSICAL(pVp8DecInfo->u4SegIdWrapper));
...
/* vVDecWriteVP8PP(u4VDecID, RW_PP_SPECIAL_MODE, u4VDecReadVP8PP(u4VDecID, RW_PP_SPECIAL_MODE) | 0x101000); /// for webp get-lucky.webp.ivf use. */
	vVDecWriteVP8PP(u4VDecID, RW_PP_SPECIAL_MODE,
			u4VDecReadVP8PP(u4VDecID, RW_PP_SPECIAL_MODE) | 0x1000);
	...
	vVDecWriteVP8MC(u4VDecID, RW_MC_MODE_CTL,
			(u4VDecReadVP8MC(u4VDecID, RW_MC_MODE_CTL) | (0x1 << 12)));
...
/* vVDecWriteVP8MC(u4VDecID, 0x660, (u4VDecReadVP8MC(u4VDecID, 0x660) |(0x80000000))); */
	...
	if ((prVp8DecParam->iLoopFilterLvl) > 0) {
		...
		VDEC_REG_SET_FLAG(u4HalValue,
				  VDEC_FLGSET(u4FlagParam, VP8PARAM_MODEREF_LFDELTA_ENABLE),
				  RW_PP_MODEREF_DELTA_ENABLE_FLAG);
		VDEC_REG_SET_FLAG(u4HalValue, pVp8DecInfo->uFrameType, RW_PP_FRAMETYPE_FLAG);
		VDEC_REG_SET_VALUE(u4HalValue, prVp8DecParam->iSharpLvl, RW_PP_SHARPLVL_S,
				   RW_PP_SHARPLVL_E);
		...
		for (u4Index = 0; u4Index < MAX_MB_SEGMENTS; u4Index++) {
			if (VDEC_FLGSET(u4FlagParam, VP8PARAM_SEGMENT_ENABLE)) {
				if (VDEC_FLGSET(u4FlagParam, VP8PARAM_SEGMENT_ABSDATA)) {
					i4Level =
					    prVp8DecParam->
					    SegmentFeatureData[VP8_MBLVL_ALT_LF][u4Index];
				} else {
		...
		for (u4Index = 0; u4Index < MAX_REF_LF_DELTAS; u4Index++) {
			u4HalValue |=
			    (((UINT8) prVp8DecParam->RefLFDeltas[u4Index]) << (u4Index * 8));
		}
		vVDecWriteVP8PP(u4VDecID, RW_PP_VP8REFDELTA, u4HalValue);
		u4HalValue = 0;
		for (u4Index = 0; u4Index < MAX_MODE_LF_DELTAS; u4Index++) {
			u4HalValue |=
			    (((UINT8) prVp8DecParam->ModeLFDeltas[u4Index]) << (u4Index * 8));
		}
		vVDecWriteVP8PP(u4VDecID, RW_PP_VP8MODEDELTA, u4HalValue);
	}
#if VDEC_DDR3_SUPPORT
	/* Turn on DDR3 mode */
	vVDecWriteVP8MC(u4VDecID, RW_MC_DDR_CTRL0,
			((u4VDecReadVP8MC(u4VDecID, RW_MC_DDR_CTRL0) & 0xFFFFFFFE) | 0x1));
	vVDecWriteVP8MC(u4VDecID, RW_MC_DDR_CTRL1,
			((u4VDecReadVP8MC(u4VDecID, RW_MC_DDR_CTRL1) & 0xFFFFFFFE) | 0x1));
	vVDecWriteVP8MC(u4VDecID, (406 << 2), u4VDecReadVP8MC(u4VDecID, (406 << 2)) & 0xFFFFFFEF);

	/* Turn-on DDR3, Set 0x834[0] = 0 */
	vVDecWriteVP8MC(u4VDecID, RW_MC_DDR3_EN,
			(u4VDecReadVP8MC(u4VDecID, RW_MC_DDR3_EN) & 0xFFFFFFFE));
	...
	/* Post-process enable */
	vVDecWriteVP8MC(u4VDecID, RW_MC_PP_ENABLE,
			(u4VDecReadVP8MC(u4VDecID, RW_MC_PP_ENABLE) | 0x1));
	/* Writeback by PP */
...
#if VP8_MB_ROW_MODE_SUPPORT
	{
		...
		if (prVp8DecParam->iLoopFilterLvl > 0
		    && prVp8DecParam->eLoopFilterType == VP8_LF_TYPE_SIMPLE) {
		...
		if (pVp8DecInfo->u4Width > 16 /* && pVp8DecInfo->u2Height>=16 */) {
			...
			vVDecWriteVP8PP(u4VDecID, RW_PP_BRC_OFFSET,
					((pVp8DecInfo->u4Width + 15) >> 4) * 32);
		}
...
/* printk("RW_PP_SPECIAL_MODE: 0x%08x\n", u4VDecReadVP8PP(u4VDecID, RW_PP_SPECIAL_MODE)); */
	...
	vVDecWriteVP8MC(u4VDecID, RW_MC_VP8RANDOMERRSET,
			(u4VDecReadVP8MC(u4VDecID, RW_MC_VP8RANDOMERRSET) | RW_MC_VP8RANDOM_ERR));
	/* memset(_pucWorkYBuf[u4VDecID] , 0x0A, PIC_Y_SZ+PIC_C_SZ); */

#if (CONFIG_DRV_VERIFY_SUPPORT && !MEM_ALLOCATE_IOREMAP)
	_pu1VP8VDecYAddrPhy =
	    (UINT8 *) dma_map_single(NULL, pVp8DecInfo->u4CurYAddr, GOLD_Y_SZ + GOLD_C_SZ,
				     DMA_FROM_DEVICE);
	_pu1VP8VDecYAltPhy =
	    (UINT8 *) dma_map_single(NULL, pVp8DecInfo->u4LstYAddr, GOLD_Y_SZ + GOLD_C_SZ,
				     DMA_TO_DEVICE);
	_pu1VP8VDecYCurPhy =
	    (UINT8 *) dma_map_single(NULL, pVp8DecInfo->u4AlfYAddr, GOLD_Y_SZ + GOLD_C_SZ,
				     DMA_TO_DEVICE);
	_pu1VP8VDecYGldPhy =
	    (UINT8 *) dma_map_single(NULL, pVp8DecInfo->u4GldYAddr, GOLD_Y_SZ + GOLD_C_SZ,
				     DMA_TO_DEVICE);
#endif
