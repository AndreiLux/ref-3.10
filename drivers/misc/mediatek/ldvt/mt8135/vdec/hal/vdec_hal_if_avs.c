...
/* <0: fail. */
/* ************************************************************************** */
INT32 i4VDEC_HAL_AVS_InitBarrelShifter(UINT32 u4BSID, UINT32 u4VDecID,
				       VDEC_INFO_AVS_BS_INIT_PRM_T * prAVSBSInitPrm)
{
	BOOL fgInitBSResult;
...
UINT32 u4VDEC_AVS_Search_SC(UINT32 u4BSID, UINT32 u4VDecID)
{
	...
	while (1) {
		...
		if (((u4Temp >> 8) == 0x000001) && ((u4Temp & 0x000000FF) <= 0xB7)) {
			/* Find Start Code. */
			break;
		}
...
UINT32 u4VDEC_AVS_Search_SliceSC(UINT32 u4BSID, UINT32 u4VDecID)
{
	...
	while (1) {
		...
		/* Shift 0x000001 for read pointer */
		u4Temp = u4VDEC_HAL_AVS_ShiftGetBitStream(u4BSID, u4VDecID, 0);
		if (((u4Temp >> 8) == 0x000001) && ((u4Temp & 0x000000FF) <= 0xAF)) {
...
INT32 i4VDEC_HAL_AVS_SetBSInfo(UINT32 u4BSID, UINT32 u4InstID, VDEC_INFO_DEC_PRM_T *prDecParam,
...
static void vVDEC_HAL_AVS_SetMvReg(UINT32 u4VDecID, const VDEC_INFO_AVS_DEC_PRM_T *prVDecAvsDecPrm)
{
	...
	if (prPicHdr->u4PicCodingType == I_PIC) {
		if ((prPicHdr->u4PicStruct == FRM_PIC) || (!prPicHdr->fgSecField)) {
			...
			ai4BlkDistFw[0] = 1;
			ai4BlkDistFw[1] =
			    (INT32) ((INT32) (prPicHdr->u4PicDistance) * 2 -
				     (INT32) (prVDecAvsDecPrm->arFBufInfo[AVS_LAST_P_IDX].
					      u4PicDistance) * 2);
			ai4BlkDistFw[2] =
			    (INT32) ((INT32) (prPicHdr->u4PicDistance) * 2 -
				     (INT32) (prVDecAvsDecPrm->arFBufInfo[AVS_LAST_P_IDX].
			...
			/* if (AVS_LAST_P_IDX >= REF_LIST_1) */
			{
				ai4BlkDistFw[3] =
				    (INT32) ((INT32) (prPicHdr->u4PicDistance) * 2 -
					     (INT32) (prVDecAvsDecPrm->arFBufInfo[AVS_PREV_P_IDX].
						      u4PicDistance) * 2);
			}
		...
		if (prPicHdr->u4PicStruct == FRM_PIC) {
			...
			if (prPicHdr->u4TFT) {
				if (!prPicHdr->fgSecField) {
					ai4BlkDistFw[0] =
					    (INT32) ((INT32) (prPicHdr->u4PicDistance) * 2 -
						     (INT32) (prVDecAvsDecPrm->
							      arFBufInfo[AVS_LAST_P_IDX].
							      u4PicDistance) * 2) - 1;
					ai4BlkDistFw[1] =
					    (INT32) ((INT32) (prPicHdr->u4PicDistance) * 2 -
						     (INT32) (prVDecAvsDecPrm->
		...
		if (prPicHdr->u4PicStruct == FRM_PIC) {
			ai4BlkDistFw[0] =
			    (INT32) ((INT32) (prPicHdr->u4PicDistance) * 2 -
				     (INT32) (prVDecAvsDecPrm->arFBufInfo[AVS_FW_REF_IDX].
					      u4PicDistance) * 2);
			ai4BlkDistBw[0] =
			    (INT32) ((INT32)
				     (prVDecAvsDecPrm->arFBufInfo[AVS_BW_REF_IDX].u4PicDistance) *
				     2 - (INT32) (prPicHdr->u4PicDistance) * 2);
			...
			/* f (AVS_BW_REF_IDX > REF_LIST_1) */
			{
				ai4DirectRatio[1] =
				    (INT32) ((INT32)
					     (prVDecAvsDecPrm->arFBufInfo[AVS_BW_REF_IDX].
					      u4PicDistance) * 2 -
					     (INT32) (prVDecAvsDecPrm->arFBufInfo[AVS_PREV_FW_IDX].
						      u4PicDistance) * 2);
			}
				...
				if (!prPicHdr->fgSecField) {
					...
					ai4BlkDistBw[0] =
					    (INT32) ((INT32)
						     (prVDecAvsDecPrm->arFBufInfo[AVS_BW_REF_IDX].
						      u4PicDistance) * 2 -
						     (INT32) (prPicHdr->u4PicDistance) * 2);
					ai4BlkDistBw[1] =
					    (INT32) ((INT32)
						     (prVDecAvsDecPrm->arFBufInfo[AVS_BW_REF_IDX].
						      u4PicDistance) * 2 -
						     (INT32) (prPicHdr->u4PicDistance) * 2) + 1;
					...
					/* if (AVS_BW_REF_IDX > REF_LIST_1) */
					{
						ai4DirectRatio[2] =
						    (INT32) ((INT32)
							     (prVDecAvsDecPrm->
							      arFBufInfo[AVS_BW_REF_IDX].
							      u4PicDistance * 2) -
							     (INT32) (prVDecAvsDecPrm->
								      arFBufInfo[AVS_PREV_FW_IDX].
								      u4PicDistance) * 2) - 1;
						ai4DirectRatio[3] =
						    (INT32) ((INT32)
							     (prVDecAvsDecPrm->
					...
					if (prVDecAvsDecPrm->arFBufInfo[AVS_BW_REF_IDX].
					    u1PicCodingType == I_PIC) {
						vVDecWriteAVSMV(u4VDecID, RW_AVS_MV_BW_I_REF, 1);
					} else {
						vVDecWriteAVSMV(u4VDecID, RW_AVS_MV_BW_I_REF, 0);
					}
					...
					ai4BlkDistBw[0] =
					    (INT32) (prVDecAvsDecPrm->arFBufInfo[AVS_BW_REF_IDX].
						     u4PicDistance) * 2 -
					    (INT32) (prPicHdr->u4PicDistance) * 2 - 1;
					ai4BlkDistBw[1] =
					    (INT32) (prVDecAvsDecPrm->arFBufInfo[AVS_BW_REF_IDX].
...
static void vVDEC_HAL_AVS_SetMcReg(UINT32 u4VDecID, const VDEC_INFO_AVS_DEC_PRM_T *prVDecAvsDecPrm)
{
	...
	if (prPicHdr->u4PicStruct == FRM_PIC) {
		...
		if ((prPicHdr->u4PicCodingType == I_PIC) && (prPicHdr->fgSecField)) {
			vVDecWriteMC(u4VDecID, RW_AVS_MC_P_LIST0_PIC0_Y_ADDR,
				     PHYSICAL((UINT32) prVDecAvsDecPrm->pucDecWorkBufY));
			vVDecWriteMC(u4VDecID, RW_AVS_MC_P_LIST0_PIC1_Y_ADDR,
				     PHYSICAL(prVDecAvsDecPrm->arFBufInfo[AVS_LAST_P_IDX].u4YAddr));
			...
			/* if (prVDecAvsDecPrm->u1LastRefIdx >= REF_LIST_1) */
			{
				vVDecWriteMC(u4VDecID, RW_AVS_MC_P_LIST0_PIC3_Y_ADDR,
					     PHYSICAL(prVDecAvsDecPrm->arFBufInfo[AVS_PREV_P_IDX].
						      u4YAddr));
			}
			/* bottom Field */
			...
			if (prPicHdr->u4TFT) {
				if (!prPicHdr->fgSecField) {
					vVDecWriteMC(u4VDecID, RW_AVS_MC_P_LIST0_PIC0_Y_ADDR,
						     PHYSICAL(prVDecAvsDecPrm->
							      arFBufInfo[AVS_LAST_P_IDX].u4YAddr));
					vVDecWriteMC(u4VDecID, RW_AVS_MC_P_LIST0_PIC1_Y_ADDR,
						     PHYSICAL(prVDecAvsDecPrm->
							      arFBufInfo[AVS_LAST_P_IDX].u4YAddr));
					...
					/* if (prVDecAvsDecPrm->u1LastRefIdx >= REF_LIST_1) */
					{
						vVDecWriteMC(u4VDecID,
							     RW_AVS_MC_P_LIST0_PIC2_Y_ADDR,
							     PHYSICAL(prVDecAvsDecPrm->
								      arFBufInfo[AVS_PREV_P_IDX].
								      u4YAddr));
						vVDecWriteMC(u4VDecID,
							     RW_AVS_MC_P_LIST0_PIC3_Y_ADDR,
							     PHYSICAL(prVDecAvsDecPrm->
								      arFBufInfo[AVS_PREV_P_IDX].
								      u4YAddr));
					}
					...
					vVDecWriteMC(u4VDecID, RW_AVS_MC_P_LIST0_PIC0_Y_ADDR,
						     PHYSICAL((UINT32) prVDecAvsDecPrm->
							      pucDecWorkBufY));
					vVDecWriteMC(u4VDecID, RW_AVS_MC_P_LIST0_PIC1_Y_ADDR,
						     PHYSICAL(prVDecAvsDecPrm->
							      arFBufInfo[AVS_LAST_P_IDX].u4YAddr));
...
static void vVDEC_HAL_AVS_SetPpReg(UINT32 u4VDecID, const VDEC_INFO_AVS_DEC_PRM_T *prVDecAvsDecPrm)
{
	...
	/* Set PP buffer */
	vVDecWriteMC(u4VDecID, RW_AVS_PP_LUMA_ADDR,
		     PHYSICAL((UINT32) prVDecAvsDecPrm->pucDecWorkBufY) >> 9);
	vVDecWriteMC(u4VDecID, RW_AVS_PP_CHROM_ADDR,
		     PHYSICAL((UINT32) prVDecAvsDecPrm->pucDecWorkBufC) >> 8);
...
INT32 i4VDEC_HAL_AVS_DecStart(UINT32 u4BsID, UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T *prDecPrm)
{
	...
	VDEC_INFO_AVS_DEC_PRM_T *prVDecAvsDecPrm =
	    (VDEC_INFO_AVS_DEC_PRM_T *) &(prDecPrm->SpecDecPrm.rVDecAVSDecPrm);
#else
	VDEC_INFO_AVS_DEC_PRM_T *prVDecAvsDecPrm =
	    (VDEC_INFO_AVS_DEC_PRM_T *) prDecPrm->prVDecCodecHalPrm;
#endif
	VDEC_INFO_AVS_SEQ_HDR_T *prSeqHdr = prVDecAvsDecPrm->prSeqHdr;
	VDEC_INFO_AVS_PIC_HDR_T *prPicHdr = prVDecAvsDecPrm->prPicHdr;
	...
	vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_TOTAL_MBS_IN_PIC,
			 (u4MbWidthMinusOne + 1) * (u4MbHeightMinusOne + 1));
	vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PIC_MB_SIZE_M1,
			 ((u4MbHeightMinusOne << 16) | u4MbWidthMinusOne));
#else
	vVDecWriteAVSVLD(u4VDecID, RW_AVS_VLD_PIC_SIZE,
	...
	vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PRED_ADDR,
			 PHYSICAL(prVDecAvsDecPrm->rAvsWorkBufSa.u4PredSa));
#else
	...
	vVDecWriteAVSVLD(u4VDecID, RW_AVS_VLD_PRED_BASE_ADDR,
			 PHYSICAL(prVDecAvsDecPrm->rAvsWorkBufSa.u4PredSa));
#else
...
/* ************************************************************************** */
BOOL fgVDEC_HAL_AVS_ChkErrInfo(UINT32 ucBsId, UINT32 u4VDecID, UINT32 u4DecErrInfo,
			       UINT32 u4ECLevel)
{
