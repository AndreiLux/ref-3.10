...
const CHAR ZZ_SCAN[16] = { 0, 1, 4, 8, 5, 2, 3, 6, 9, 12, 13, 10, 7, 11, 14, 15
};

const CHAR ZZ_SCAN8[64] = { 0, 1, 8, 16, 9, 2, 3, 10, 17, 24, 32, 25, 18, 11, 4, 5,
...
/* <0: fail. */
/* ************************************************************************** */
INT32 i4VDEC_HAL_H264_InitBarrelShifter(UINT32 u4BSID, UINT32 u4VDecID,
...
/* Return      : None */
/* ************************************************************************** */
void vVDEC_HAL_H264_ScalingList(UINT32 u4BSID, UINT32 u4VDecID, CHAR *pcScalingList,
				UINT32 u4SizeOfScalingList, BOOL *pfgUseDefaultScalingMatrixFlag)
{
	UINT32 i;
	UINT32 u4Scanj;
	INT32 i4LastScale;
	...
	for (i = 0; i < u4SizeOfScalingList; i++) {
		...
		if (i4NextScale != 0) {
			...
			*pfgUseDefaultScalingMatrixFlag = ((u4Scanj == 0)
							   && (i4NextScale == 0)) ? TRUE : FALSE;
		}
...
void vVDEC_HAL_H264_WriteScalingList(UINT32 u4VDecID, UINT32 u4Idx, CHAR *pcSlicePtr)
{
	...
	if (u4Idx < 6) {
		...
		for (i = 0; i < 4; i++) {
			...
			u4Temp =
			    (((UCHAR) pcSlicePtr[i << 2]) << 24) +
			    (((UCHAR) pcSlicePtr[(i << 2) + 1]) << 16) +
			    (((UCHAR) pcSlicePtr[(i << 2) + 2]) << 8) +
			    (((UCHAR) pcSlicePtr[(i << 2) + 3]));
...
/* Return      : None */
/* ************************************************************************** */
void vVDEC_HAL_H264_InitPRefList(UINT32 u4VDecID, BOOL fgPicFrm, UINT32 u4MaxFrameNum,
				 UINT32 u4CurrPicNum)
{
...
#endif
	vVDecWriteAVCVLD(u4VDecID, RW_AVLD_MAX_PIC_NUM,
			 fgPicFrm ? u4MaxFrameNum : (u4MaxFrameNum << 1));
...
/* Return      : None */
/* ************************************************************************** */
void vVDEC_HAL_H264_SetPRefPicListReg(UINT32 u4VDecID,
				      VDEC_INFO_H264_P_REF_PRM_T * prPRefPicListInfo)
{
	...
	if (ucFld == FRAME) {
		...
		vVDecWriteMC(u4VDecID, RW_MC_P_LIST0 + (ucRegIdx << 2),
			     PHYSICAL((UINT32) prPRefPicListInfo->u4FBufYStartAddr));
#else
		vVDecWriteMC(u4VDecID, RW_MC_P_LIST0 + (ucRegIdx << 2),
			     u4AbsDramANc((UINT32) prPRefPicListInfo->u4FBufYStartAddr));
#endif
#if VDEC_MVC_SUPPORT
		if (fgIsMVCDecode(u4VDecID)) {
			...
			u4Param =
			    (1 << 19) + (fgLRefPic << 20) +
			    (fgLRefPic ? (prPRefPicListInfo->i4LongTermPicNum & 0x7ffff)
			     : (prPRefPicListInfo->i4PicNum & 0x7ffff));
		...
		u4Temp =
		    (((prPRefPicListInfo->i4TFldPOC <= prPRefPicListInfo->i4BFldPOC)
		      && (prPRefPicListInfo->i4TFldPOC != 0x7fffffff)) ? 1 : 0) << 21;
		u4Temp |= (fgLRefPic << 20);
		...
		if (ucFld == TOP_FIELD) {
			...
			if (fgIsMVCDecode(u4VDecID)) {
				...
				u4Param =
				    (1 << 19) + (fgLRefPic << 20) +
				    (fgLRefPic ? (prPRefPicListInfo->i4TFldLongTermPicNum & 0x7ffff)
				     : (prPRefPicListInfo->i4TFldPicNum & 0x7ffff));
				vVDecWriteAVCVLD(u4VDecID, RW_AVLD_REORD_P0_RPL + (ucRegIdx << 2),
						 u4Param);
			...
			if (fgIsMVCDecode(u4VDecID)) {
				...
				u4Param =
				    (1 << 19) + (fgLRefPic << 20) +
				    (fgLRefPic ? (prPRefPicListInfo->i4BFldLongTermPicNum & 0x7ffff)
				     : (prPRefPicListInfo->i4BFldPicNum & 0x7ffff));
				vVDecWriteAVCVLD(u4VDecID, RW_AVLD_REORD_P0_RPL + (ucRegIdx << 2),
						 u4Param);
				/* printk("SetPRefPicList4: [0x%X] = 0x%X\n", RW_AVLD_REORD_P0_RPL + (ucRegIdx<<2),  u4Param); */
			} else
			...
			vVDecWriteMC(u4VDecID, RW_AMC_P_LIST0_FLD,
				     u4VDecReadMC(u4VDecID,
						  RW_AMC_P_LIST0_FLD) | (0x1 << ucRegIdx));
		}
	}
}
...
void vVDEC_HAL_H264_SetPOC(UINT32 u4VDecID, VDEC_INFO_H264_POC_PRM_T * prPOCInfo)
{
	...
	if (prPOCInfo->fgIsFrmPic) {
		...
		i4CurrPOC =
		    (prPOCInfo->ucPicStruct ==
		     TOP_FIELD) ? prPOCInfo->i4TFldPOC : prPOCInfo->i4BFldPOC;
...
/* Return      : None */
/* ************************************************************************** */
BOOL bVDEC_HAL_H264_SetBRefPicListReg(UINT32 u4VDecID,
				      VDEC_INFO_H264_B_REF_PRM_T * prBRefPicListInfo)
{
	...
	VDEC_INFO_H264_DEC_PRM_T *prH264DecPrm =
	    (VDEC_INFO_H264_DEC_PRM_T *) &(_tVerMpvDecPrm[u4VDecID].SpecDecPrm.rVDecH264DecPrm);
	...
	prH264DecPrm->prCurrFBufInfo->u4MvStartAddr =
	    (prH264DecPrm->prCurrFBufInfo->u4YStartAddr +
	     ((prH264DecPrm->prCurrFBufInfo->u4DramPicSize * 3) >> 1));
#endif
#else
	...
	if (prH264DecPrm->fgIsReduceMVBuffer) {
		vVDecWriteAVCMV(u4VDecID, RW_AMV_REDUCE_BMV,
				(u4VDecReadAVCMV(u4VDecID, RW_AMV_REDUCE_BMV) | EN_AMV_REDUCE_BMV));
...
/* printk("AVC use reduced mv buffer\n"); */
	...
	if (ucFld == FRAME) {
		/* B_0 */
#if CONFIG_DRV_VIRTUAL_ADDR
		vVDecWriteMC(u4VDecID, RW_MC_B_LIST0 + (ucRegIdx << 2),
			     PHYSICAL((UINT32) prBRefPicListInfo->u4FBufYStartAddr));
#else
		vVDecWriteMC(u4VDecID, RW_MC_B_LIST0 + (ucRegIdx << 2),
			     u4AbsDramANc((UINT32) prBRefPicListInfo->u4FBufYStartAddr));
#endif
#if VDEC_MVC_SUPPORT
		if (fgIsMVCDecode(u4VDecID)) {
			vVDecWriteAVCVLD(u4VDecID, RW_AVLD_REORD_B0_RPL + (ucRegIdx << 2),
					 (1 << 19) + (fgLRefPic << 20) +
					 (fgLRefPic
					  ? (prBRefPicListInfo->
					     i4LongTermPicNum & 0x7ffff) : (prBRefPicListInfo->
									    i4PicNum & 0x7ffff)));
		} else
#endif
		{
		...
		u4Temp =
		    (((prBRefPicListInfo->i4TFldPOC <= prBRefPicListInfo->i4BFldPOC)
		      && (prBRefPicListInfo->i4TFldPOC != 0x7fffffff)) ? 1 : 0) << 21;
		u4Temp |= (fgLRefPic << 20);
		u4Temp +=
		    ((ucFBufIdx) << 1 << 22) + prBRefPicListInfo->u4TFldPara +
		    (prBRefPicListInfo->i4TFldPOC & 0x3ffff);
		...
		u4Temp |= (fgLRefPic << 20);
		u4Temp +=
		    ((((ucFBufIdx) << 1) + 1) << 22) + prBRefPicListInfo->u4BFldPara +
		    (prBRefPicListInfo->i4BFldPOC & 0x3ffff);
...
#if CONFIG_DRV_VIRTUAL_ADDR
		vVDecWriteMC(u4VDecID, RW_MC_B_LIST1 + (ucRegIdx << 2),
			     PHYSICAL((UINT32) prBRefPicListInfo->u4FBufYStartAddr1));
#else
		vVDecWriteMC(u4VDecID, RW_MC_B_LIST1 + (ucRegIdx << 2),
			     u4AbsDramANc((UINT32) prBRefPicListInfo->u4FBufYStartAddr1));
#endif
#if VDEC_MVC_SUPPORT
		if (fgIsMVCDecode(u4VDecID)) {
			vVDecWriteAVCVLD(u4VDecID, RW_AVLD_REORD_B1_RPL + (ucRegIdx << 2),
					 (1 << 19) + (fgLRefPic << 20) +
					 (fgLRefPic
					  ? (prBRefPicListInfo->
					     i4LongTermPicNum1 & 0x7ffff) : (prBRefPicListInfo->
									     i4PicNum1 & 0x7ffff)));
		} else
#endif
		{
		...
		u4Temp =
		    (((prBRefPicListInfo->i4TFldPOC1 <= prBRefPicListInfo->i4BFldPOC1)
		      && (prBRefPicListInfo->i4TFldPOC1 != 0x7fffffff)) ? 1 : 0) << 21;
		u4Temp |= (fgLRefPic << 20);
		u4Temp +=
		    ((ucFBufIdx) << 1 << 22) + prBRefPicListInfo->u4TFldPara1 +
		    (prBRefPicListInfo->i4TFldPOC1 & 0x3ffff);
		...
		u4Temp |= (fgLRefPic << 20);
		u4Temp +=
		    ((((ucFBufIdx) << 1) + 1) << 22) + prBRefPicListInfo->u4BFldPara1 +
		    (prBRefPicListInfo->i4BFldPOC1 & 0x3ffff);
		...
		vVDecWriteAVCMV(u4VDecID, RW_AMV_B1_REF_ADDR + (ucRegIdx << 3) + 4,
				(u4Temp >> 4) + u4FieldDistance);
	} else {
		/* B_0 */
		if ((prBRefPicListInfo->u4ListIdx == 0) || (prBRefPicListInfo->u4ListIdx == 1)
		    || (prBRefPicListInfo->u4ListIdx == 4) || (prBRefPicListInfo->u4ListIdx == 5)) {
			/* u4Temp = (((_ptFBufInfo[ucFBufIdx].i4TFldPOC <= _ptFBufInfo[ucFBufIdx].i4BFldPOC) && (_ptFBufInfo[ucFBufIdx].i4TFldPOC != 0x7fffffff))? 1: 0) << 21; */
			if (ucFld == TOP_FIELD) {
#if CONFIG_DRV_VIRTUAL_ADDR
				vVDecWriteMC(u4VDecID, RW_MC_B_LIST0 + (ucRegIdx << 2),
					     PHYSICAL((UINT32) prBRefPicListInfo->
						      u4FBufYStartAddr));
#else
				vVDecWriteMC(u4VDecID, RW_MC_B_LIST0 + (ucRegIdx << 2),
					     u4AbsDramANc((UINT32) prBRefPicListInfo->
							  u4FBufYStartAddr));
#endif
#if VDEC_MVC_SUPPORT
				if (fgIsMVCDecode(u4VDecID)) {
					vVDecWriteAVCVLD(u4VDecID,
							 RW_AVLD_REORD_B0_RPL + (ucRegIdx << 2),
							 (1 << 19) + (fgLRefPic << 20) +
							 (fgLRefPic
							  ? (prBRefPicListInfo->
							     i4TFldLongTermPicNum & 0x7ffff)
							  : (prBRefPicListInfo->
							     i4TFldPicNum & 0x7ffff)));
				} else
#endif
				{
					vVDecWriteAVCVLD(u4VDecID,
				...
				u4Temp =
				    ...
				    (prBRefPicListInfo->i4TFldPOC & 0x3ffff);
				u4Temp |= (fgLRefPic << 20);
				vVDecWriteAVCMV(u4VDecID, RW_AMV_B0_REF_PARA + (ucRegIdx << 2),
						u4Temp);
			} else {
#if CONFIG_DRV_VIRTUAL_ADDR
				vVDecWriteMC(u4VDecID, RW_MC_B_LIST0 + (ucRegIdx << 2),
				...
				if (fgIsMVCDecode(u4VDecID)) {
					vVDecWriteAVCVLD(u4VDecID,
							 ...
							 (fgLRefPic
							  ? (prBRefPicListInfo->
							     i4BFldLongTermPicNum & 0x7ffff)
							  : (prBRefPicListInfo->
							     i4BFldPicNum & 0x7ffff)));
				} else
#endif
				{
					vVDecWriteAVCVLD(u4VDecID,
				...
				vVDecWriteMC(u4VDecID, RW_AMC_B_LIST0_FLD,
					     u4VDecReadMC(u4VDecID,
							  RW_AMC_B_LIST0_FLD) | (0x1 << ucRegIdx));
			}
		}
		/* B_1 */
		if ((prBRefPicListInfo->u4ListIdx == 2) || (prBRefPicListInfo->u4ListIdx == 3)
		    || (prBRefPicListInfo->u4ListIdx == 4) || (prBRefPicListInfo->u4ListIdx == 5)) {
			/* u4Temp = (((_ptFBufInfo[ucFBufIdx].i4TFldPOC <= _ptFBufInfo[ucFBufIdx].i4BFldPOC) && (_ptFBufInfo[ucFBufIdx].i4TFldPOC != 0x7fffffff))? 1: 0) << 21; */
			if (ucFld == TOP_FIELD) {
#if CONFIG_DRV_VIRTUAL_ADDR
				...
				if (fgIsMVCDecode(u4VDecID)) {
					vVDecWriteAVCVLD(u4VDecID,
							 RW_AVLD_REORD_B1_RPL + (ucRegIdx << 2),
							 (1 << 19) + (fgLRefPic << 20) +
							 (fgLRefPic
							  ? (prBRefPicListInfo->
							     i4TFldLongTermPicNum & 0x7ffff)
...
#else
				u4Temp =
				    u4AbsDramANc((UINT32) prBRefPicListInfo->u4FBufMvStartAddr);
#endif
				vVDecWriteAVCMV(u4VDecID, RW_AMV_B1_REF_ADDR + (ucRegIdx << 2),
						(u4Temp >> 4));
				...
				u4Temp =
				    ...
				    (prBRefPicListInfo->i4TFldPOC & 0x3ffff);
				u4Temp |= (fgLRefPic << 20);
				vVDecWriteAVCMV(u4VDecID, RW_AMV_B1_REF_PARA + (ucRegIdx << 2),
						u4Temp);
			} else {
#if CONFIG_DRV_VIRTUAL_ADDR
				vVDecWriteMC(u4VDecID, RW_MC_B_LIST1 + (ucRegIdx << 2),
				...
				vVDecWriteMC(u4VDecID, RW_AMC_B_LIST1_FLD,
					     u4VDecReadMC(u4VDecID,
							  RW_AMC_B_LIST1_FLD) | (0x1 << ucRegIdx));
			}
		}
	}
	if (u4VDecReadAVCMV(u4VDecID, RW_AMV_B0_REF_PARA + (ucRegIdx << 2)) !=
	    u4VDecReadAVCMV(u4VDecID, RW_AMV_B1_REF_PARA + (ucRegIdx << 2))) {
		return TRUE;
	} else {
		return FALSE;
	}
...
void vVDEC_HAL_H264_B1ListSwap(UINT32 u4VDecID, BOOL fgIsFrmPic)
{
	...
	if (fgIsFrmPic) {
		...
		vVDecWriteAVCVLD(u4VDecID, RW_AVLD_REORD_B1_RPL,
				 u4VDecReadAVCVLD(u4VDecID, RW_AVLD_REORD_B1_RPL + 4));
		vVDecWriteAVCVLD(u4VDecID, RW_AVLD_REORD_B1_RPL + 4, u4Temp);

		u4Temp = u4VDecReadAVCMV(u4VDecID, RW_AMV_B1_REF_PARA);
		vVDecWriteAVCMV(u4VDecID, RW_AMV_B1_REF_PARA,
				u4VDecReadAVCMV(u4VDecID, RW_AMV_B1_REF_PARA + 8));
		vVDecWriteAVCMV(u4VDecID, RW_AMV_B1_REF_PARA + 8, u4Temp);

		u4Temp = u4VDecReadAVCMV(u4VDecID, RW_AMV_B1_REF_PARA + 4);
		vVDecWriteAVCMV(u4VDecID, RW_AMV_B1_REF_PARA + 4,
				u4VDecReadAVCMV(u4VDecID, RW_AMV_B1_REF_PARA + 12));
		vVDecWriteAVCMV(u4VDecID, RW_AMV_B1_REF_PARA + 12, u4Temp);

		u4Temp = u4VDecReadAVCMV(u4VDecID, RW_AMV_B1_REF_ADDR);
		vVDecWriteAVCMV(u4VDecID, RW_AMV_B1_REF_ADDR,
				u4VDecReadAVCMV(u4VDecID, RW_AMV_B1_REF_ADDR + 8));
		vVDecWriteAVCMV(u4VDecID, RW_AMV_B1_REF_ADDR + 8, u4Temp);

		u4Temp = u4VDecReadAVCMV(u4VDecID, RW_AMV_B1_REF_ADDR + 4);
		vVDecWriteAVCMV(u4VDecID, RW_AMV_B1_REF_ADDR + 4,
				u4VDecReadAVCMV(u4VDecID, RW_AMV_B1_REF_ADDR + 12));
		...
		u4Temp = u4VDecReadAVCMV(u4VDecID, RW_AMV_B1_REF_PARA);
		vVDecWriteAVCMV(u4VDecID, RW_AMV_B1_REF_PARA,
				u4VDecReadAVCMV(u4VDecID, RW_AMV_B1_REF_PARA + 4));
		vVDecWriteAVCMV(u4VDecID, RW_AMV_B1_REF_PARA + 4, u4Temp);

		u4Temp = u4VDecReadAVCMV(u4VDecID, RW_AMV_B1_REF_ADDR);
		vVDecWriteAVCMV(u4VDecID, RW_AMV_B1_REF_ADDR,
				u4VDecReadAVCMV(u4VDecID, RW_AMV_B1_REF_ADDR + 4));
		...
		vVDecWriteMC(u4VDecID, RW_AMC_B_LIST1_FLD,
			     (u4VDecReadMC(u4VDecID, RW_AMC_B_LIST1_FLD) & (~1)) |
			     ((u4VDecReadMC(u4VDecID, RW_AMC_B_LIST1_FLD) >> 1) & 1));
		vVDecWriteMC(u4VDecID, RW_AMC_B_LIST1_FLD,
			     (u4VDecReadMC(u4VDecID, RW_AMC_B_LIST1_FLD) & (~2)) | (u4Temp << 1));
	}
}
...
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
	vVDecWriteVLDTOP(u4VDecID, RW_VLD_PIC_MB_SIZE_M1,
			 ((prSPS->
			   u4PicHeightInMapUnitsMinus1) << VLD_TOP_PIC_HEIGHT_IN_MBS_POS) | prSPS->
			 u4PicWidthInMbsMinus1);
#else
	vVDecWriteAVCVLD(u4VDecID, RW_AVLD_PIC_SIZE,
			 ((prSPS->
			   u4PicHeightInMapUnitsMinus1) << AVLD_PIC_HEIGHT_IN_MBS_POS) | prSPS->
			 u4PicWidthInMbsMinus1);
#endif
}
...
void vVDEC_HAL_H264_SetPPSAVLD(UINT32 u4VDecID, BOOL fgUserScalingMatrixPresentFlag,
			       ...
			       VDEC_INFO_H264_PPS_T * prVDecH264PPS)
{
	UINT32 u4PPSInfo;
	INT32 i;
...
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
	vVDecWriteAVCVLD(u4VDecID, RW_AVLD_PIC_SIZE,
			 (u4VDecReadAVCVLD(u4VDecID, RW_AVLD_PIC_SIZE) & (~0x1FFFF)) | (u4SHDRInfo &
											0x1FFF));
	vVDecWriteAVCVLD(u4VDecID, RW_AVLD_SHDR_1,
			 (u4VDecReadAVCVLD(u4VDecID, RW_AVLD_SHDR_1) & (~0xFFFFE000)) | (u4SHDRInfo
											 &
											 0xFFFFE000));
#else
...
#if CONFIG_DRV_VIRTUAL_ADDR
		vVDecWriteAVCFG(u4VDecID, RW_FGT_OUT_Y_ADDR,
				PHYSICAL((UINT32) prFGTPrm->pucFGTTrgYAddr) >> 4);
		vVDecWriteAVCFG(u4VDecID, RW_FGT_OUT_C_ADDR,
				PHYSICAL((UINT32) prFGTPrm->pucFGTTrgCAddr) >> 4);
		vVDecWriteAVCFG(u4VDecID, RW_FGT_IN_Y_ADDR,
				PHYSICAL((UINT32) prFGTPrm->pucFGTScrYAddr) >> 4);
		vVDecWriteAVCFG(u4VDecID, RW_FGT_IN_C_ADDR,
				PHYSICAL((UINT32) prFGTPrm->pucFGTScrCAddr) >> 4);
#else
		vVDecWriteAVCFG(u4VDecID, RW_FGT_OUT_Y_ADDR,
				u4AbsDramANc((UINT32) prFGTPrm->pucFGTTrgYAddr) >> 4);
		vVDecWriteAVCFG(u4VDecID, RW_FGT_OUT_C_ADDR,
				u4AbsDramANc((UINT32) prFGTPrm->pucFGTTrgCAddr) >> 4);
		vVDecWriteAVCFG(u4VDecID, RW_FGT_IN_Y_ADDR,
				u4AbsDramANc((UINT32) prFGTPrm->pucFGTScrYAddr) >> 4);
		vVDecWriteAVCFG(u4VDecID, RW_FGT_IN_C_ADDR,
				u4AbsDramANc((UINT32) prFGTPrm->pucFGTScrCAddr) >> 4);
#endif
		vVDecWriteAVCFG(u4VDecID, RW_FGT_MB_SIZE,
				(prFGTPrm->ucMBYSize << 8) + prFGTPrm->ucMBXSize);
...
INT32 i4VDEC_HAL_H264_DecStart(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T * prDecPrm)
{
...
#if (CONFIG_DRV_VERIFY_SUPPORT) && (!VDEC_DRV_PARSER)
	VDEC_INFO_H264_DEC_PRM_T *prH264DecPrm =
	    (VDEC_INFO_H264_DEC_PRM_T *) &(prDecPrm->SpecDecPrm.rVDecH264DecPrm);
	/* prH264DecPrm->prCurrFBufInfo->u4YStartAddr = prH264DecPrm->u4DecWorkBuf; */
	prH264DecPrm->prCurrFBufInfo->u4CAddrOffset = prH264DecPrm->prCurrFBufInfo->u4DramPicSize;
#if (!VDEC_H264_REDUCE_MV_BUFF)
	prH264DecPrm->prCurrFBufInfo->u4MvStartAddr =
...
#else
	VDEC_INFO_H264_DEC_PRM_T *prH264DecPrm =
	    (VDEC_INFO_H264_DEC_PRM_T *) prDecPrm->prVDecCodecHalPrm;
#endif
	...
	if (prH264DecPrm->ucECLevel == 0) {
		...
		vVDecWriteAVCVLD(u4VDecID, RW_AVLD_ERR_MASK,
				 (~(CABAC_ALIGN_BIT_ERR | CABAC_ZERO_WORD_ERR)));
	}
#else
	vVDecWriteAVCVLD(u4VDecID, RW_AVLD_ERR_MASK, (~(AVLD_MB_END_CHK | AVLD_4BLOCKS_SKIP_CHK)));
#endif
	...
	if (0)			/* prH264DecPrm->prSliceHdr->u4DisableDeblockingFilterIdc == 1) */
	{
		...
		vVDecWriteMC(u4VDecID, RW_MC_PP_Y_ADDR,
			     PHYSICAL((UINT32) prH264DecPrm->prCurrFBufInfo->u4YStartAddr) >> 9);
		vVDecWriteMC(u4VDecID, RW_MC_PP_C_ADDR,
			     PHYSICAL((UINT32)
				      (prH264DecPrm->prCurrFBufInfo->u4YStartAddr +
				       prH264DecPrm->prCurrFBufInfo->u4CAddrOffset)) >> 8);
#else
		vVDecWriteMC(u4VDecID, RW_MC_PP_Y_ADDR,
			     u4AbsDramANc((UINT32) prH264DecPrm->prCurrFBufInfo->
					  u4YStartAddr) >> 9);
		vVDecWriteMC(u4VDecID, RW_MC_PP_C_ADDR,
			     u4AbsDramANc((UINT32)
					  (prH264DecPrm->prCurrFBufInfo->u4YStartAddr +
		...
		vVDecWriteMC(u4VDecID, RW_MC_PP_Y_RANGE,
			     (((prDecPrm->u4PicH >> (1 - (prH264DecPrm->fgIsFrmPic))) + 15) >> 4) -
			     1);
...
#if CONFIG_DRV_VIRTUAL_ADDR
	vVDecWriteMC(u4VDecID, RW_AMC_Y_OUT_ADDR,
		     PHYSICAL((UINT32) prH264DecPrm->prCurrFBufInfo->u4YStartAddr));
#else
	vVDecWriteMC(u4VDecID, RW_AMC_Y_OUT_ADDR,
		     u4AbsDramANc((UINT32) prH264DecPrm->prCurrFBufInfo->u4YStartAddr));
#endif
	...
	vVDecWriteAVCVLD(u4VDecID, RW_AVLD_RM03R,
			 ((u4VDecReadAVCVLD(u4VDecID, RW_AVLD_RM03R)) & 0xFFFFFFDF));
	/* vVDecWriteAVCVLD(u4VDecID, RW_AVLD_RM03R, ((u4VDecReadAVCVLD(u4VDecID,  RW_AVLD_RM03R)) & 0xFFFFFCDF) ); */
#else
	vVDecWriteAVCVLD(u4VDecID, RW_AVLD_RM03R,
			 ((u4VDecReadAVCVLD(u4VDecID, RW_AVLD_RM03R)) & 0xFFFFFCDD));
#endif
	...
	vVDecWriteAVCMV(u4VDecID, RW_AMV_WR_ADDR,
			((PHYSICAL((UINT32) prH264DecPrm->prCurrFBufInfo->u4MvStartAddr) >> 4)) +
			((prDecPrm->ucPicStruct == BOTTOM_FIELD) ? u4FieldDistance : 0));
#else
	vVDecWriteAVCMV(u4VDecID, RW_AMV_WR_ADDR,
			((u4AbsDramANc((UINT32) prH264DecPrm->prCurrFBufInfo->u4MvStartAddr) >> 4))
			+ ((prDecPrm->ucPicStruct == BOTTOM_FIELD) ? u4FieldDistance : 0));
#endif

#if VDEC_MVC_SUPPORT
	vVDecWriteAVCMV(u4VDecID, RW_AMV_MVC_CTRL,
			(u4VDecReadAVCMV(u4VDecID, RW_AMV_MVC_CTRL)) | FW_NO_MV_MVC);
#endif
...
/* turn off test mode 0x834[4] */
	...
	if (prDecPrm->u4PicBW < 176) {
		vVDecWriteDV(u4VDecID, VDEC_DV_LAT_BUF_BYPASS,
			     u4VDecReadDV(u4VDecID, VDEC_DV_LAT_BUF_BYPASS) | 0x1);
	} else {
		vVDecWriteDV(u4VDecID, VDEC_DV_LAT_BUF_BYPASS,
			     u4VDecReadDV(u4VDecID, VDEC_DV_LAT_BUF_BYPASS) & 0xFFFFFFFE);
	}
...
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
	if (prH264DecPrm->prSPS->fgFrameMbsOnlyFlag == 0
	    && prH264DecPrm->prSliceHdr->fgFieldPicFlag == 0) {
		u4total_mbs_in_pic =
		    ((prH264DecPrm->prSPS->u4PicHeightInMapUnitsMinus1 << 1) +
		     2) * (prH264DecPrm->prSPS->u4PicWidthInMbsMinus1 + 1);
	} else {
		u4total_mbs_in_pic =
		    (prH264DecPrm->prSPS->u4PicHeightInMapUnitsMinus1 +
		     1) * (prH264DecPrm->prSPS->u4PicWidthInMbsMinus1 + 1);
	}
	...
	vVDecWriteMC(u4VDecID, RW_MC_VLD_WRAPPER,
		     PHYSICAL(_tVerMpvDecPrm[u4VDecID].SpecDecPrm.rVDecH264DecPrm.
			      u4VLDWrapperWrok));
	vVDecWriteMC(u4VDecID, RW_MC_PP_WRAPPER,
		     PHYSICAL(_tVerMpvDecPrm[u4VDecID].SpecDecPrm.rVDecH264DecPrm.u4PPWrapperWrok));
#endif

	printk("{H264 DEC} u4InstID = 0x%x, Input Window: 0x%08x\n", u4VDecID,
	       u4VDecReadAVCVLD(u4VDecID, RO_AVLD_BARL));
...
/* Return      : H264 decode error message */
/* ************************************************************************** */
BOOL fgVDEC_HAL_H264_ChkErrInfo(UINT32 ucBsId, UINT32 u4VDecID, UINT32 u4DecErrInfo,
				UINT32 u4ECLevel)
{
	UINT32 u4Data;
	BOOL fgIsVDecErr;
	...
	default:
		if (u4DecErrInfo == CABAC_ZERO_WORD_ERR) {
			...
			if (((u4Data >> 8) == START_CODE) || (u4Data == 0x00000000)
			    || (u4Data == START_CODE)) {
				fgIsVDecErr = FALSE;
			}
...
void vVDEC_HAL_H264_MVC_Switch(UINT32 u4VDecID, BOOL fgIsMVCDec)
{
	if (fgIsMVCDec) {
		vVDecWriteAVCVLD(u4VDecID, RW_AVLD_RM03R,
				 u4VDecReadAVCVLD(u4VDecID,
						  RW_AVLD_RM03R) | (MVC_SWITCH | HEADER_EXT_SWITCH |
								    REORDER_MVC_SWITCH));
	} else {
		vVDecWriteAVCVLD(u4VDecID, RW_AVLD_RM03R,
				 u4VDecReadAVCVLD(u4VDecID,
	...
	vVDecWriteAVCVLD(u4VDecID, RW_AVLD_RM03R,
			 u4VDecReadAVCVLD(u4VDecID, RW_AVLD_RM03R) & (~VIEW_REORDER_SWITCH));
...
/* Return      : None */
/* ************************************************************************** */
void vVDEC_HAL_H264_SetInterViewPRefPicListReg(UINT32 u4VDecID,
					       VDEC_INFO_H264_P_REF_PRM_T * prPRefPicListInfo)
{
	UCHAR ucFBufIdx;
	UCHAR ucFld;
...
/* Return      : None */
/* ************************************************************************** */
void vVDEC_HAL_H264_SetInterViewB0RefPicListReg(UINT32 u4VDecID,
						VDEC_INFO_H264_P_REF_PRM_T * prPRefPicListInfo)
{
	UCHAR ucFBufIdx;
	UCHAR ucFld;
...
#endif
		vVDecWriteAVCVLD(u4VDecID, RW_AVLD_REORD_B0_RPL + (ucRegIdx << 2),
				 (1 << 19) + (2 << 20) + (prPRefPicListInfo->u4ViewId & 0x7ffff));

		u4Temp =
		    (((prPRefPicListInfo->i4TFldPOC <= prPRefPicListInfo->i4BFldPOC)
		      && (prPRefPicListInfo->i4TFldPOC != 0x7fffffff)) ? 1 : 0) << 21;
		u4Temp +=
		    ((ucFBufIdx) << 1 << 22) + prPRefPicListInfo->u4TFldPara +
		    (prPRefPicListInfo->i4TFldPOC & 0x3ffff);
		vVDecWriteAVCMV(u4VDecID, RW_AMV_B0_REF_PARA + (ucRegIdx << 3), u4Temp);

		/* Always set Tfld first & !LRef in Interview */
		u4Temp = (1 << 21);
		u4Temp +=
		    ((((ucFBufIdx) << 1) + 1) << 22) + prPRefPicListInfo->u4BFldPara +
		    (prPRefPicListInfo->i4BFldPOC & 0x3ffff);
		vVDecWriteAVCMV(u4VDecID, RW_AMV_B0_REF_PARA + (ucRegIdx << 3) + 4, u4Temp);
	} else {
		if (ucFld == TOP_FIELD) {
#if CONFIG_DRV_VIRTUAL_ADDR
			...
			vVDecWriteAVCVLD(u4VDecID, RW_AVLD_REORD_B0_RPL + (ucRegIdx << 2),
					 (1 << 19) + (2 << 20) +
					 (prPRefPicListInfo->u4ViewId & 0x7ffff));
...
/* Return      : None */
/* ************************************************************************** */
void vVDEC_HAL_H264_SetInterViewB1RefPicListReg(UINT32 u4VDecID,
						VDEC_INFO_H264_P_REF_PRM_T * prPRefPicListInfo)
{
	UCHAR ucFBufIdx;
	UCHAR ucFld;
