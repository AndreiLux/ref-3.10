...
extern BOOL fgWrMsg2PC(void *pvAddr, UINT32 u4Size, UINT32 u4Mode,
		       VDEC_INFO_VERIFY_FILE_INFO_T *pFILE_INFO);
extern void vVDecOutputDebugString(const CHAR *format, ...);
#else
...
void vVLDMp4DecPrmProc(UINT32 u4VDecID, VDEC_INFO_MPEG4_DEC_PRM_T *prMp4Prm,
		       UINT32 u4MBx,
		       UCHAR ucIntraDcVlcThr, UCHAR ucQuarterSample, UCHAR ucDataPartitioned);
...
/* vVDecWriteVLD(u4VDecID, RW_VLD_PIC_SIZE, u4PicH<<16|u4PicW); */
	...
	vVDecWriteMC(u4VDecID, RW_MC_MVDCAC_SEL,
		     u4VDecReadMC(u4VDecID, RW_MC_MVDCAC_SEL) | (1 << 20));
#endif
...
/* printk("\n MPV Dump register end\n"); */
	...
	vVDecWriteMC(u4VDecID, RW_MC_DDR3_EN,
		     (u4VDecReadMC(u4VDecID, RW_MC_DDR3_EN) & (~(1 << 4))));
#endif
...
#if VDEC_8130_SUPPORT && VDEC_PP_ENABLE
		vVDecWriteMP4PP(u4VDecID, 0x40, (u4VDecReadMP4PP(u4VDecID, 0x40) | (0x1 << 12)));
#endif
...
void vVLDDecPrmProc(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T *prDecPrm)
{
	...
	u4RegVal1 = (prMp2Prm->ucPicStruct << 30) +
	    ...
	    (prMp2Prm->ucIntraDcPre << 18) + (prMp2Prm->ucAltScan << 17);

	if (prMpegDecPrm->ucMpegVer == 1) {
		u4RegVal1 = u4RegVal1 +
		    (prMp2Prm->ucFordFCode << 12) +
		    (prMp2Prm->ucFordFCode << 8) +
		    (prMp2Prm->ucBackFCode << 4) + (prMp2Prm->ucBackFCode);
	} else {
		u4RegVal1 = u4RegVal1 +
		    (prMp2Prm->pucfcode[0][0] << 12) +
		    (prMp2Prm->pucfcode[0][1] << 8) +
		    (prMp2Prm->pucfcode[1][0] << 4) + (prMp2Prm->pucfcode[1][1]);
	}
...
void vVLDSetPicType(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T *prDecPrm)
{
	...
	if ((VDEC_MPEG1 == prMpegDecPrm->ucMpegVer) || (VDEC_MPEG2 == prMpegDecPrm->ucMpegVer)) {
		...
		if (I_TYPE == prMp2Prm->ucPicCdTp)
			...
			u4Value |= (1 << 2);
	} else if ((VDEC_DIVX3 == prMpegDecPrm->ucMpegVer)
		   || (VDEC_MPEG4 == prMpegDecPrm->ucMpegVer)) {
		...
		if (VDEC_MPEG4 == prMpegDecPrm->ucMpegVer) {
			VDEC_INFO_MPEG4_VOL_PRM_T *prVolPrm =
			    prMpegDecPrm->rPicLayer.rMp4DecPrm.rDep.rM4vDecPrm.prVol;
...
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
	u4RegVal2 =
	    ((prMpegDecPrm->ucMpegVer == VDEC_MPEG1) << 31) + ((prMpegDecPrm->ucMpegVer ==
								VDEC_MPEG2) << 30) +
	    ((prDecPrm->u4PicH) << 16) + (u4MaxMblMod & 0xFF);
#else
	u4RegVal2 =
	    ((prMpegDecPrm->ucMpegVer ==
	      VDEC_MPEG1) << 31) + ((prDecPrm->u4PicH + 15) << 16) + ((u4MaxMblMod & 0xFF) << 8) +
	    u4WidthMb;
#endif
...
void vVLDSetMBRowPrm(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T *prDecPrm, BOOL fgBPic, BOOL fgFrmPic)
{
	...
	u4EndY =
	    (prMpegDecPrm->u4DecYOff + u4VLDPicH(prMpegDecPrm->ucMpegVer, prMpegDecPrm->u4DecH) -
	     1) >> 4;

	if ((prMpegDecPrm->u4DecH == prDecPrm->u4PicH) && (prMpegDecPrm->u4DecYOff == 0)) {
	...
	if (fgBPic) {
		if (fgFrmPic) {
			...
			u4RegVal3 = ((u4StartY / 2) << 16) + ((u4EndY - 1) / 2);
		}
	} else {
		u4RegVal3 = 0x1ff;
	}
	...
	if (fgBPic) {
		if (fgFrmPic) {
			...
			u4RegVal3 = ((u4StartY / 2) << 24) +
			    (((u4EndY - 1) / 2) << 16) + (prMpegDecPrm->u4BBufStart << 8);
		}
	} else {
...
void vVLDM4vDecPrmProc(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T *prDecPrm)
{
	...
	u4RegValue |= (prMp4Prm->ucVopCdTp == VCT_B &&
	    ...
	    ? 0x1 : 0x0;
	u4RegValue |= (prMp4Prm->ucVopCdTp == VCT_B && prVopPrm->ucBRefCdTp == VCT_I)
	    ? (0x1 << 1) : (0x0 << 1);
#endif
	...
	u4RegVal1 = (prVopPrm->fgTopFldFirst << 26) +
	    ...
	    (prVopPrm->ucFordFCode << 12) + (prVopPrm->ucBackFCode << 4);
	vVDecWriteVLD(u4VDecID, RW_VLD_PARA, u4RegVal1);
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
	...
	vVLDMp4DecPrmProc(u4VDecID, prMp4Prm, u4MBx, prVopPrm->ucIntraDcVlcThr,
			  prVolPrm->ucQuarterSample, prVolPrm->ucDataPartitioned);


	u4RegVal1 = u4VDecReadVLD(u4VDecID, RW_VLD_MP4_HDR) & ~(0x0778FFFF);
	/* GSLin's */
	u4Mp4Hdr = (prVolPrm->ucResyncMarkerDisable << 26) + (prVolPrm->ucQuarterSample << 25) + (prVolPrm->ucSorenson << 23) +	/* PANDA H263 */
	    (prVolPrm->ucQuantType << 24) +
	    (prVolPrm->ucInterlaced << 22) +
	    (prVolPrm->ucDataPartitioned << 21) +
	    (prVolPrm->ucReversibleVlc << 20) +
	    (prVolPrm->ucShortVideoHeader << 19) + (prVolPrm->u2VopTimeIncrementResolution);
	/* 8130NEW (2) */
#if (MPEG4_8130_SUPPORT)
	...
	if (prVolPrm->ucInterlaced) {
		vVDecWriteVLD(u4VDecID, RW_VLD_DIRE_MD_IL,
			      prDirMdPrm->u4Trbi | (prDirMdPrm->u4Trdi << 16));
	}
#endif
	...
	{
		vVDecWriteMC(u4VDecID, RW_MC_CBCR_MV_TYPE,
			     u4VDecReadMC(u4VDecID, RW_MC_CBCR_MV_TYPE) & (~(1 << 8)));
	}
	/* 8130NEW 3.1.2 */
	...
	/* 8130NEW 4.5 - 4.12 */
	vVDecWriteAVCMV(u4VDecID, RW_AMV_MP4_VTRBD,
			(prDirMdPrm->u4Trb << 16) | (prDirMdPrm->u4Trd & 0xFFFF));
	vVDecWriteAVCMV(u4VDecID, RW_AMV_MP4_VTRBD_I,
			(prDirMdPrm->u4Trbi << 16) | (prDirMdPrm->u4Trdi & 0xFFFF));
	vVDecWriteAVCMV(u4VDecID, RW_AMV_MP4_VTRBD_I_PLUS_1,
			((prDirMdPrm->u4Trbi + 1) << 16) | ((prDirMdPrm->u4Trdi + 1) & 0xFFFF));
	vVDecWriteAVCMV(u4VDecID, RW_AMV_MP4_VTRBD_I_MINUS_1,
			((prDirMdPrm->u4Trbi - 1) << 16) | ((prDirMdPrm->u4Trdi - 1) & 0xFFFF));
	...
	if (i4Trd == 0) {
		...
		vVDecWriteAVCMV(u4VDecID, RW_AMV_MP4_VTRBD_RATIO,
				(((i4Trb << 14) / i4Trd) << 16) | ((((i4Trd - i4Trb) << 14) /
								    i4Trd) & 0xFFFF));
	}
	if (i4Trdi == 0) {
		vVDecWriteAVCMV(u4VDecID, RW_AMV_MP4_VTRBD_I_RATIO, 0);
	} else {
		vVDecWriteAVCMV(u4VDecID, RW_AMV_MP4_VTRBD_I_RATIO,
				(((i4Trbi << 14) / i4Trdi) << 16) | ((((i4Trdi - i4Trbi) << 14) /
								      i4Trdi) & 0xFFFF));
	}
	if (i4Trdi == -1) {
		vVDecWriteAVCMV(u4VDecID, RW_AMV_MP4_VTRBD_I_RATIO_2, 0);
	} else {
		vVDecWriteAVCMV(u4VDecID, RW_AMV_MP4_VTRBD_I_RATIO_2,
				((((i4Trbi + 1) << 14) / (i4Trdi +
							  1)) << 16) | ((((i4Trdi -
									   i4Trbi) << 14) /
									 (i4Trdi + 1)) & 0xFFFF));
	...
	if (1 == i4Trdi) {
		...
		vVDecWriteAVCMV(u4VDecID, RW_AMV_MP4_VTRBD_I_RATIO_3,
				((((i4Trbi - 1) << 14) / (i4Trdi -
							  1)) << 16) | ((((i4Trdi -
									   i4Trbi) << 14) /
									 (i4Trdi - 1)) & 0xFFFF));
	...
	vVDecWriteMC(u4VDecID, RW_MC_GMC_Y_MV,
		     (prGmcPrm->i4GmcYMvX << 16) | (prGmcPrm->i4GmcYMvY & 0xffff));
	...
	{
		vVDecWriteMC(u4VDecID, RW_MC_GMC_C_MV,
			     (prGmcPrm->i4GmcCMvX << 16) | (prGmcPrm->i4GmcCMvY & 0xffff));
	} else if (3 == prDecPrm->ucMpegSpecType)	/* divx mode */
	{
		vVDecWriteMC(u4VDecID, RW_MC_GMC_C_MV,
			     (prGmcPrm->i4GmcYMvX << 15) | ((prGmcPrm->i4GmcYMvY >> 1) & 0xffff));
	}
#if (MPEG4_8130_SUPPORT)
	/* 8130NEW 4.2 */
	...
	u4RegVal1 |= ((prVopPrm->fgTopFldFirst & 0x1) << 1) | (prVolPrm->ucQuarterSample & 0x1);
	...
	if (1 == (prVolPrm->ucQuarterSample & 0x1)) {
		vVDecWriteAVCMV(u4VDecID, RW_AMV_MP4_GMC_Y_MV,
				(prGmcPrm->i4GmcYMvX << 16) | (prGmcPrm->i4GmcYMvY & 0xffff));
	} else {
		vVDecWriteAVCMV(u4VDecID, RW_AMV_MP4_GMC_Y_MV,
				((prGmcPrm->i4GmcYMvX >> 1) << 16) | ((prGmcPrm->
								       i4GmcYMvY >> 1) & 0xffff));
	}
#endif
...
void vVLDMp4TriggerDec(UINT32 u4VDecID)
{
	...
	printk("<SMALL PIC> DecW %d DecH %d, Mbx %d, Mby %d\n",
	       _tVerMpvDecPrm[u4VDecID].SpecDecPrm.rVDecMPEGDecPrm.u4DecW,
	       _tVerMpvDecPrm[u4VDecID].SpecDecPrm.rVDecMPEGDecPrm.u4DecH, u4Mbx, u4Mby);
	...
	vVDecWriteMC(u4VDecID, 0x820,
		     (u4VDecReadMC(u4VDecID, 0x820) & ~(0x0FFF0FFF)) | u4Addr_1 | (u4Addr_2 << 16));
	...
	vVDecWriteVLD(u4VDecID, RW_VLD_MP4_ERR_MASK,
		      (u4VDecReadVLD(u4VDecID, RW_VLD_MP4_ERR_MASK) | (0xff << 16)));
/* vVDecWriteVLD(u4VDecID,RW_VLD_MP4_ERR_DET1,0xffffffff); */
/* vVDecWriteVLD(u4VDecID,RW_VLD_MP4_ERR_DET2,0xffffffff); */
	/* L buffer off */
	/* vVDecWriteCRC(u4VDecID, 60*4,u4VDecReadCRC(u4VDecID,60*4)|0x01);  //Yi Feng hide for performance */
	/* pre-fetch priority */
...
#ifndef VDEC_SRAM
	/* bcoded dram mode */
#ifdef MPEG4_BYPASS_MODE
	vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PRED_SRAM_CFG,
			 u4VDecReadVLDTOP(u4VDecID, RW_VLD_TOP_PRED_SRAM_CFG) | (1 << 30));
#else
	vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PRED_SRAM_CFG,
			 u4VDecReadVLDTOP(u4VDecID, RW_VLD_TOP_PRED_SRAM_CFG) & (0xBFFFFFFF));
#endif
#endif
	/* bypass VLD prefetch */
#ifdef MPEG4_DATA_PARTITION_PREFETCH_MODE
	vVDecWriteMC(u4VDecID, RW_MC_PREFETCH, u4VDecReadMC(u4VDecID, RW_MC_PREFETCH) | (1 << 28));
#else
	vVDecWriteMC(u4VDecID, RW_MC_PREFETCH,
		     u4VDecReadMC(u4VDecID, RW_MC_PREFETCH) & (0xEFFFFFFF));
#endif
	...
	if (_tVerMpvDecPrm[u4VDecID].u4PicW < 176) {
		...
		vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PRED_SRAM_CFG,
				 u4VDecReadVLDTOP(u4VDecID, RW_VLD_TOP_PRED_SRAM_CFG) | (1 << 30));
		vVDecWriteMC(u4VDecID, RW_MC_PREFETCH,
			     u4VDecReadMC(u4VDecID, RW_MC_PREFETCH) | (1 << 28));
		vVDecWriteVLDTOP(u4VDecID, RW_VLD_TOP_PRED_MODE,
				 (u4VDecReadVLDTOP(u4VDecID, RW_VLD_TOP_PRED_MODE) & (~(0x1 << 4)))
				 | (2 << 4));
		vVDecWriteVLD(u4VDecID, RW_VLD_MP4_DATA_PARTITION,
			      u4VDecReadVLD(u4VDecID,
					    RW_VLD_MP4_DATA_PARTITION) | (0x1 << 2) | (0x1 << 7));
		vVDecWriteVLD(u4VDecID, RW_VLD_MP4_DCAC_NOT_CODED_SZ,
			      (u4VDecReadVLD(u4VDecID, RW_VLD_MP4_DCAC_NOT_CODED_SZ) &
			       (~(0xFFFF << 16))) | ((6 *
						      ((_tVerMpvDecPrm[u4VDecID].u4PicW +
							15) >> 4)) << 16));
	}
#endif

#ifdef MPEG4_8130_ERROR_CONCEAL
	vVDecWriteVLD(u4VDecID, RW_VLD_TIMEOUT, 0x10000);
	...
	vVDecWriteVLD(u4VDecID, RW_VLD_MP4_ERR_MASK,
		      u4VDecReadVLD(u4VDecID, RW_VLD_MP4_ERR_MASK) & (~(0x7 << 11)));
	...
	u4Reg |=
	    (_tVerMpvDecPrm[u4VDecID].SpecDecPrm.rVDecMPEGDecPrm.rPicLayer.rMp4DecPrm.rDep.
	     rM4vDecPrm.prVol->ucSorenson & 1) << 20;
	vVDecWriteVLD(u4VDecID, RW_VLD_MP4_ERR_DET1, u4Reg);
	u4Reg =
	    u4VDecReadVLD(u4VDecID,
			  RW_VLD_MP4_ERR_DET2) | ((_tVerMpvDecPrm[u4VDecID].SpecDecPrm.
						   rVDecMPEGDecPrm.rPicLayer.rMp4DecPrm.rDep.
						   rM4vDecPrm.prVol->ucShortVideoHeader & 1) << 11);
	...
	{
		/* vVDecWriteMC(u4VDecID, 661 * 4, u4VDecReadMC(u4VDecID, 661 * 4)|(1<<4)); */
		/* vVDecWriteMC(u4VDecID, 661 * 4, u4VDecReadMC(u4VDecID, 661 * 4)&(0xFFFFFFFE)); */

		/* printk("******* INPUT Window *******\n"); */
		/* printk("MPEG4 RO_VLD_BARL: 0x%08x\n", u4VDecReadVLD(u4VDecID, RO_VLD_BARL)); */
	}
		       ...
		       UCHAR ucIntraDcVlcThr, UCHAR ucQuarterSample, UCHAR ucDataPartitioned)
{
...
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
	u4DcacPitch = (prMp4Prm->rMpeg4WorkBufSize.u4DcacSize >> 4) | (8 << 24) |	/* bit per pixel */
	...
	vVDecWriteVLD(u4VDecID, RW_VLD_MVDRAM,
		      (u4AbsDramANc(prMp4Prm->rMpeg4WorkBufSa.u4MvecSa) >> 2));
	vVDecWriteVLD(u4VDecID, RW_VLD_MB1SA,
		      (u4AbsDramANc(prMp4Prm->rMpeg4WorkBufSa.u4Bmb1Sa) >> 2));
	vVDecWriteVLD(u4VDecID, RW_VLD_MB2SA,
		      (u4AbsDramANc(prMp4Prm->rMpeg4WorkBufSa.u4Bmb2Sa) >> 2));
	vVDecWriteVLD(u4VDecID, RW_VLD_COSA,
		      (u4AbsDramANc(prMp4Prm->rMpeg4WorkBufSa.u4BcodeSa) >> 2));
...
#else
	vVDecWriteVLD(u4VDecID, RW_VLD_MVSA,
		      ((prMp4Prm->rMpeg4WorkBufSize.u4MVSize >> 4) << 16) | (u4MBx * 4));
	vVDecWriteVLD(u4VDecID, RW_VLD_BMB1,
		      ((prMp4Prm->rMpeg4WorkBufSize.u4MB1Size >> 4) << 16) | (u4MBx * 4));
	vVDecWriteVLD(u4VDecID, RW_VLD_BMB2,
		      ((prMp4Prm->rMpeg4WorkBufSize.u4MB2Size >> 4) << 16) | (u4MBx * 4));
#endif
	vVDecWriteVLD(u4VDecID, RW_VLD_BCODE_SA, ((prMp4Prm->rMpeg4WorkBufSize.u4BcodeSize >> 4) << 16) | 0 << 1);	/* ???? */
	vVDecWriteMC(u4VDecID, RW_MC_VLD_WRAPPER,
		     PHYSICAL(prMp4Prm->rMpeg4WorkBufSa.u4VldWrapperSa));
...
/* vVDecWriteVLD(u4VDecID, RW_VLD_BCODE_SA,  0 << 1);//???? */
	...
	vVDecWriteVLD(u4VDecID, RW_VLD_MVSA,
		      ((u4AbsDramANc(prMp4Prm->rMpeg4WorkBufSa.u4MvecSa) >> 2) & 0x3FFFFF) |
		      ((u4MBx * 4) << 22));
	...
	vVDecWriteVLD(u4VDecID, RW_VLD_BMB1,
		      ((u4AbsDramANc(prMp4Prm->rMpeg4WorkBufSa.u4Bmb1Sa) >> 2) & 0x3FFFFF) |
		      ((u4MBx * 4) << 22));
	...
	vVDecWriteVLD(u4VDecID, RW_VLD_BMB2,
		      ((u4AbsDramANc(prMp4Prm->rMpeg4WorkBufSa.u4Bmb2Sa) >> 2) & 0x3FFFFF) |
		      ((u4MBx * 4) << 22));
	...
	vVDecWriteVLD(u4VDecID, RW_VLD_BCODE_SA,
		      ((u4AbsDramANc(prMp4Prm->rMpeg4WorkBufSa.u4BcodeSa) >> 2) & 0xFFFFFF) | (0 <<
											       25)
		      | (((u4AbsDramANc(prMp4Prm->rMpeg4WorkBufSa.u4BcodeSa) >> 2) & 0xF000000) <<
			 4));
	...
	u4DcacPitch = (0 << 0) |	/* turn on reset direction_busy */
	    ...
	    (1 << 11) | (0 << 16) |	/* mv_dram reset by 2D reset */
	...
	vVDecWriteVLD(u4VDecID, RW_VLD_MP4_NOT_CODED_SA,
		      u4AbsDramANc(prMp4Prm->rMpeg4WorkBufSa.u4NotCodedSa));
	vVDecWriteVLD(u4VDecID, RW_VLD_MP4_DATA_PARTITION_SA,
		      u4AbsDramANc(prMp4Prm->rMpeg4WorkBufSa.u4DataPartitionSa));
	vVDecWriteVLD(u4VDecID, RW_VLD_MP4_DCAC_NOT_CODED_SZ,
		      (((prMp4Prm->rMpeg4WorkBufSize.u4NotCodedSize >> 4) & 0xFFFF) | ((prMp4Prm->
											rMpeg4WorkBufSize.
											u4DcacSize
											>> 4) <<
										       16)));
	...
	/* 8130NEW 4.1 */
	vVDecWriteAVCMV(u4VDecID, RW_AMV_MP4_MV_DIRECT_SA,
			((u4AbsDramANc(prMp4Prm->rMpeg4WorkBufSa.u4MvDirectSa) >> 4) & 0x0FFFFFFF));
#endif
...
void vVLDDx3DecPrmProc(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T *prDecPrm)
{
	...
	if (2 == prDecPrm->ucMpegSpecType) {
		...
		/* printk("Test4 RW_VLD_MP4_FLG value is 0x%x\n", u4RegVal1); */
	} else if (3 == prDecPrm->ucMpegSpecType) {
		vVDecWriteVLD(u4VDecID, RW_VLD_MP4_FLG,
			      u4VDecReadVLD(u4VDecID, RW_VLD_MP4_FLG) | (0x1 << 16));

		/* printk("Test5 RW_VLD_MP4_FLG value is 0x%x\n", u4RegVal1); */
	}
	...
	u4Dx3Hdr = (prDx3Prm->ucAltIAcChromDctIdx) |
	    ...
	    (prDx3Prm->ucAltMv << 11) | (prDx3Prm->ucHasSkip << 16) | (prDx3Prm->ucFrameMode << 20);
	vVDecWriteVLD(u4VDecID, RW_VLD_DX3, u4Dx3Hdr);
	vVDecWriteVLD(u4VDecID, RW_VLD_DVX_BOUND_MODE, 0);
	vVDecWriteVLD(u4VDecID, RW_VLD_DVX_BOUND_R1,
		      (prDx3Prm->ucSliceBoundary[0] | (prDx3Prm->ucSliceBoundary[1] << 16)));
	vVDecWriteVLD(u4VDecID, RW_VLD_DVX_BOUND_R2,
		      (prDx3Prm->ucSliceBoundary[2] | (prDx3Prm->ucSliceBoundary[3] << 16)));
	...
	/* 8130NEW 4.2 DIVX3 */
	vVDecWriteAVCMV(u4VDecID, RW_AMV_MP4_COL_IVOP,
			u4VDecReadAVCMV(u4VDecID, RW_AMV_MP4_COL_IVOP) & (~0x1));

	/* 8130NEW 4.3 DIVX3 */
	vVDecWriteAVCMV(u4VDecID, RW_AMV_MP4_TOP_FIELD_QUART_SAMPLE,
			u4VDecReadAVCMV(u4VDecID, RW_AMV_MP4_TOP_FIELD_QUART_SAMPLE) & (~0x3));
...
/* VDEC_INFO_MPEG_DEC_PRM_T *prMpegDecPrm = (VDEC_INFO_MPEG_DEC_PRM_T *)prDecPrm->prVDecCodecHalPrm; */
...
#if VMMU_SUPPORT
	{
		...
		u4Page_addr =
		    ...
		    (((prMpegDecPrm->rMpegFrameBufSa.u4Pic0YSa) / (4 * 1024)) * 4);
		printk("[MPEG] vVDecMpegCommSetting, 1, Page Addr = 0x%x\n", u4Page_addr);
		vPage_Table(u4VDecID, u4Page_addr,
			    PHYSICAL(prMpegDecPrm->rMpegFrameBufSa.u4Pic0YSa),
			    PHYSICAL(prMpegDecPrm->rMpegFrameBufSa.u4Pic0YSa) + PIC_Y_SZ);
		vVDecWriteMC(u4VDecID, RW_MC_R1Y, (prMpegDecPrm->rMpegFrameBufSa.u4Pic0YSa) >> 9);

		u4Page_addr =
		    (UINT32) _pucVMMUTable[u4VDecID] +
		    (((prMpegDecPrm->rMpegFrameBufSa.u4Pic0CSa) / (4 * 1024)) * 4);
		printk("[MPEG] vVDecMpegCommSetting, 2, Page Addr = 0x%x\n", u4Page_addr);
		vPage_Table(u4VDecID, u4Page_addr,
			    PHYSICAL(prMpegDecPrm->rMpegFrameBufSa.u4Pic0CSa),
			    PHYSICAL(prMpegDecPrm->rMpegFrameBufSa.u4Pic0CSa) + PIC_C_SZ);
		vVDecWriteMC(u4VDecID, RW_MC_R1C, (prMpegDecPrm->rMpegFrameBufSa.u4Pic0CSa) >> 8);

		u4Page_addr =
		    (UINT32) _pucVMMUTable[u4VDecID] +
		    (((prMpegDecPrm->rMpegFrameBufSa.u4Pic1YSa) / (4 * 1024)) * 4);
		printk("[MPEG] vVDecMpegCommSetting, 3, Page Addr = 0x%x\n", u4Page_addr);
		vPage_Table(u4VDecID, u4Page_addr,
			    PHYSICAL(prMpegDecPrm->rMpegFrameBufSa.u4Pic1YSa),
			    PHYSICAL(prMpegDecPrm->rMpegFrameBufSa.u4Pic1YSa) + PIC_Y_SZ);
		vVDecWriteMC(u4VDecID, RW_MC_R2Y, (prMpegDecPrm->rMpegFrameBufSa.u4Pic1YSa) >> 9);

		u4Page_addr =
		    (UINT32) _pucVMMUTable[u4VDecID] +
		    (((prMpegDecPrm->rMpegFrameBufSa.u4Pic1CSa) / (4 * 1024)) * 4);
		printk("[MPEG] vVDecMpegCommSetting, 4, Page Addr = 0x%x\n", u4Page_addr);
		vPage_Table(u4VDecID, u4Page_addr,
			    PHYSICAL(prMpegDecPrm->rMpegFrameBufSa.u4Pic1CSa),
			    PHYSICAL(prMpegDecPrm->rMpegFrameBufSa.u4Pic1CSa) + PIC_C_SZ);
		vVDecWriteMC(u4VDecID, RW_MC_R2C, (prMpegDecPrm->rMpegFrameBufSa.u4Pic1CSa) >> 8);

		u4Page_addr =
		    (UINT32) _pucVMMUTable[u4VDecID] +
		    (((prMpegDecPrm->rMpegFrameBufSa.u4Pic2YSa) / (4 * 1024)) * 4);
		printk("[MPEG] vVDecMpegCommSetting, 5, Page Addr = 0x%x\n", u4Page_addr);
		vPage_Table(u4VDecID, u4Page_addr,
			    PHYSICAL(prMpegDecPrm->rMpegFrameBufSa.u4Pic2YSa),
			    PHYSICAL(prMpegDecPrm->rMpegFrameBufSa.u4Pic2YSa) + PIC_Y_SZ);
		...
		u4Page_addr =
		    (UINT32) _pucVMMUTable[u4VDecID] +
		    (((prMpegDecPrm->rMpegFrameBufSa.u4Pic2CSa) / (4 * 1024)) * 4);
		printk("[MPEG] vVDecMpegCommSetting, 6, Page Addr = 0x%x\n", u4Page_addr);
		vPage_Table(u4VDecID, u4Page_addr,
			    PHYSICAL(prMpegDecPrm->rMpegFrameBufSa.u4Pic2CSa),
			    PHYSICAL(prMpegDecPrm->rMpegFrameBufSa.u4Pic2CSa) + PIC_C_SZ);
	...
	if (_u4PicCdTp[u4VDecID] == I_TYPE) {
		printk("//Dec0_Y_0x%x.bin_Size_0x%x\n",
		       PHYSICAL((UINT32) (_u4WorkBuffer[_u4DecBufIdx[u4VDecID] * 2])),
		       ((prDecPrm->u4PicH + 15) >> 4) * ((prDecPrm->u4PicW + 15) >> 4) << 8);
		printk("//Dec0_C_0x%x.bin_Size_0x%x\n",
		       PHYSICAL(_u4WorkBuffer[_u4DecBufIdx[u4VDecID] * 2 + 1]),
		       ((prDecPrm->u4PicH + 15) >> 4) * ((prDecPrm->u4PicW + 15) >> 4) << 7);
	} else if (_u4PicCdTp[u4VDecID] == P_TYPE) {
		printk("//Dec0_Y_0x%x.bin_Size_0x%x\n",
		       PHYSICAL(_u4WorkBuffer[_u4DecBufIdx[u4VDecID] * 2]),
		       ((prDecPrm->u4PicH + 15) >> 4) * ((prDecPrm->u4PicW + 15) >> 4) << 8);
		printk("//Dec0_C_0x%x.bin_Size_0x%x\n",
		       PHYSICAL(_u4WorkBuffer[_u4DecBufIdx[u4VDecID] * 2 + 1]),
		       ((prDecPrm->u4PicH + 15) >> 4) * ((prDecPrm->u4PicW + 15) >> 4) << 7);
		printk("//Ref0_Y_0x%x.bin_Size_0x%x\n",
		       PHYSICAL(_u4WorkBuffer[_u4FRefBufIdx[u4VDecID] * 2]),
		       ((prDecPrm->u4PicH + 15) >> 4) * ((prDecPrm->u4PicW + 15) >> 4) << 8);
		printk("//Ref0_C_0x%x.bin_Size_0x%x\n",
		       PHYSICAL(_u4WorkBuffer[_u4FRefBufIdx[u4VDecID] * 2 + 1]),
		       ((prDecPrm->u4PicH + 15) >> 4) * ((prDecPrm->u4PicW + 15) >> 4) << 7);
	} else {
		printk("//Dec0_Y_0x%x.bin_Size_0x%x\n",
		       PHYSICAL(_u4WorkBuffer[_u4DecBufIdx[u4VDecID] * 2]),
		...
		printk("//Ref1_Y_0x%x.bin_Size_0x%x\n",
		       PHYSICAL(_u4WorkBuffer[_u4BRefBufIdx[u4VDecID] * 2]),
		       ((prDecPrm->u4PicH + 15) >> 4) * ((prDecPrm->u4PicW + 15) >> 4) << 8);
		printk("//Ref1_C_0x%x.bin_Size_0x%x\n",
		       PHYSICAL(_u4WorkBuffer[_u4BRefBufIdx[u4VDecID] * 2 + 1]),
		       ((prDecPrm->u4PicH + 15) >> 4) * ((prDecPrm->u4PicW + 15) >> 4) << 7);
	}
#endif
	...
	/* max */
#endif
	if (prMpegDecPrm->rMpegPpInfo.fgPpEnable) {	/* Qing Li fix here for 8550 pp reg set */
		UINT32 u4MBqp = 0;
		...
		if ((prDecPrm->u4PicW > 16) && (prDecPrm->u4PicH > 16))	/* when width or height is less than 16,should be set MC out @mtk40343 */
		{
			...
			if (prDecPrm->ucDecFBufIdx == 0) {
				vVDecWriteMC(u4VDecID, RW_MC_PP_Y_ADDR,
					     (u4AbsDramANc(prMpegDecPrm->rMpegFrameBufSa.u4Pic0YSa))
					     >> 9);
				vVDecWriteMC(u4VDecID, RW_MC_PP_C_ADDR,
					     (u4AbsDramANc(prMpegDecPrm->rMpegFrameBufSa.u4Pic0CSa))
					     >> 8);
			} else if (prDecPrm->ucDecFBufIdx == 1) {
				vVDecWriteMC(u4VDecID, RW_MC_PP_Y_ADDR,
					     (u4AbsDramANc(prMpegDecPrm->rMpegFrameBufSa.u4Pic1YSa))
					     >> 9);
				vVDecWriteMC(u4VDecID, RW_MC_PP_C_ADDR,
					     (u4AbsDramANc(prMpegDecPrm->rMpegFrameBufSa.u4Pic1CSa))
					     >> 8);
			} else if (prDecPrm->ucDecFBufIdx == 2) {
				vVDecWriteMC(u4VDecID, RW_MC_PP_Y_ADDR,
					     (u4AbsDramANc(prMpegDecPrm->rMpegFrameBufSa.u4Pic2YSa))
					     >> 9);
				vVDecWriteMC(u4VDecID, RW_MC_PP_C_ADDR,
					     (u4AbsDramANc(prMpegDecPrm->rMpegFrameBufSa.u4Pic2CSa))
					     >> 8);
			}
			vVDecWriteMC(u4VDecID, RW_MC_PP_MB_WIDTH,
				     ((prDecPrm->u4PicW + 63) >> 6) << 2);
			vVDecWriteMC(u4VDecID, RW_MC_PP_X_RANGE,
				     ((prDecPrm->u4PicW + 15) >> 4) - 1);
			vVDecWriteMC(u4VDecID, RW_MC_PP_Y_RANGE,
				     (((prDecPrm->u4PicH + 15) >> 4) >> (prDecPrm->ucPicStruct !=
									 3)) - 1);
		} else {
	...
	vVDecWriteVLD(u4VDecID, 0x270,
		      (u4VDecReadVLD(u4VDecID, 0x270) | (0x1 << 18) | (0x1 << 20)));
#endif
...
void vSetMcDecBuf(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T *prDecPrm)
{
	...
	case B_TYPE:
		...
		{
			vMCSetOutputBuf(u4VDecID, prDecPrm->ucDecFBufIdx + 2,
					prMpegDecPrm->u4FRefBufIdx);
			vMCWriteToDigBuf(u4VDecID, OFF);
		}
		break;
	}
...
void VDecDumpMP4Register(UINT32 u4VDecID)
{
	...
	INT32 i;
#ifdef REG_LOG_NEW
	_u4RegisterLogLen[u4VDecID] +=
	    sprintf(_pucRegisterLog[u4VDecID] + _u4RegisterLogLen[u4VDecID],
		    "// <DUMP_REG> Start dumping\n");
	_u4RegisterLogLen[u4VDecID] +=
	    sprintf(_pucRegisterLog[u4VDecID] + _u4RegisterLogLen[u4VDecID], "// VDEC_TOP\n");
	for (i = 0; i < 101; i++) {
		u4Val = u4VDecReadDV(0, i * 4);
	}

	_u4RegisterLogLen[u4VDecID] +=
	    sprintf(_pucRegisterLog[u4VDecID] + _u4RegisterLogLen[u4VDecID], "// VLD_TOP\n");
	for (i = 0; i < 101; i++) {
		u4Val = u4VDecReadVLDTOP(0, i * 4);
	}

	_u4RegisterLogLen[u4VDecID] +=
	    sprintf(_pucRegisterLog[u4VDecID] + _u4RegisterLogLen[u4VDecID], "// VLD\n");
	for (i = 33; i < 256; i++) {
	...
	_u4RegisterLogLen[u4VDecID] +=
	    sprintf(_pucRegisterLog[u4VDecID] + _u4RegisterLogLen[u4VDecID], "// MC\n");
	for (i = 0; i < 701; i++) {
		u4Val = u4VDecReadMC(0, i * 4);
	}

	_u4RegisterLogLen[u4VDecID] +=
	    sprintf(_pucRegisterLog[u4VDecID] + _u4RegisterLogLen[u4VDecID], "// AVC_MV\n");
	for (i = 0; i < 256; i++) {
	...
	_u4RegisterLogLen[u4VDecID] +=
	    sprintf(_pucRegisterLog[u4VDecID] + _u4RegisterLogLen[u4VDecID], "// IS\n");
	for (i = 0; i < 65; i++) {
		u4Val = u4VDecReadMP4PP(0, 0x200 + i * 4);
	}

	_u4RegisterLogLen[u4VDecID] +=
	    sprintf(_pucRegisterLog[u4VDecID] + _u4RegisterLogLen[u4VDecID], "// DCAC\n");
	for (i = 0; i < 65; i++) {
		u4Val = u4VDecReadMP4PP(0, 0x700 + i * 4);
	}

	_u4RegisterLogLen[u4VDecID] +=
	    sprintf(_pucRegisterLog[u4VDecID] + _u4RegisterLogLen[u4VDecID], "// IQ\n");
	for (i = 0; i < 65; i++) {
		u4Val = u4VDecReadMP4PP(0, 0x500 + i * 4);
	}

	_u4RegisterLogLen[u4VDecID] +=
	    sprintf(_pucRegisterLog[u4VDecID] + _u4RegisterLogLen[u4VDecID], "// IT\n");
	for (i = 0; i < 65; i++) {
		u4Val = u4VDecReadMP4PP(0, 0x900 + i * 4);
	}

	_u4RegisterLogLen[u4VDecID] +=
	    sprintf(_pucRegisterLog[u4VDecID] + _u4RegisterLogLen[u4VDecID], "// ORED_WR\n");
	for (i = 0; i < 65; i++) {
