...
extern BOOL fgWrMsg2PCEx(void *pvAddr, UINT32 u4Size, UINT32 u4Mode,
			 VDEC_INFO_VERIFY_FILE_INFO_T *pFILE_INFO, const char *szFunction,
			 INT32 i4Line);
...
INT32 i4VDEC_HAL_WMV_InitVDecHW(UINT32 u4VDecID, VDEC_INFO_WMV_VFIFO_PRM_T * prWmvVFifoInitPrm)
{
	...
	/* printk("u4CodeType:0x%x\n", prWmvVFifoInitPrm->u4CodeType); */
	...
	/* printk("_i4CodecVersion:0x%x, u4CodeType:0x%x\n", _i4CodecVersion[u4VDecID], prWmvVFifoInitPrm->u4CodeType); */

	vVDecSetVLDVFIFO(0, u4VDecID, PHYSICAL((UINT32) prWmvVFifoInitPrm->u4VFifoSa),
			 PHYSICAL((UINT32) prWmvVFifoInitPrm->u4VFifoEa));
...
/* <0: fail. */
/* ************************************************************************** */
INT32 i4VDEC_HAL_WMV_InitBarrelShifter(UINT32 u4BSID, UINT32 u4VDecID,
				       VDEC_INFO_WMV_BS_INIT_PRM_T * prWmvBSInitPrm, BOOL fgIsVC1)
{
...
/* HalFlushInvalidateDCache(); */
	...
	vVDecSetVLDVFIFO(u4BSID, u4VDecID, PHYSICAL((UINT32) prWmvBSInitPrm->u4VFifoSa),
			 PHYSICAL((UINT32) prWmvBSInitPrm->u4VFifoEa));

	u4VLDRemainByte = ((prWmvBSInitPrm->u4ReadPointer)) & 0xf;


	vVDecWriteVLD(u4VDecID, RW_VLD_RPTR + (u4BSID << 10),
		      PHYSICAL((UINT32) prWmvBSInitPrm->u4ReadPointer));
	vVDecWriteVLD(u4VDecID, WO_VLD_WPTR + (u4BSID << 10),
		      PHYSICAL((UINT32) prWmvBSInitPrm->u4WritePointer));
	...
	for (i = 0; i < 5; i++)	/* mark */
	{
		vVDecWriteVLD(u4VDecID, WO_VLD_WPTR,
			      u4VDecReadVLD(u4VDecID, WO_VLD_WPTR) | VLD_CLEAR_PROCESS_EN);
		vVDecWriteVLD(u4VDecID, RW_VLD_RPTR + (u4BSID << 10),
			      PHYSICAL((UINT32) prWmvBSInitPrm->u4ReadPointer));
		vVDecWriteVLD(u4VDecID, RW_VLD_RPTR + (u4BSID << 10),
			      PHYSICAL((UINT32) prWmvBSInitPrm->u4ReadPointer));
		...
		vVDecWriteVLD(u4VDecID, WO_VLD_WPTR + (u4BSID << 10),
			      PHYSICAL((UINT32) prWmvBSInitPrm->u4WritePointer));
		vVDecWriteVLD(u4VDecID, RW_VLD_ASYNC + (u4BSID << 10),
			      u4VDecReadVLD(u4VDecID, RW_VLD_ASYNC) | VLD_WR_ENABLE);
#else
		vVDecWriteVLD(u4VDecID, WO_VLD_WPTR + (u4BSID << 10),
			      ((PHYSICAL((UINT32) prWmvBSInitPrm->u4WritePointer) << 4) | 0x2));
#endif
#if (WMV_8320_SUPPORT)
		{
			UINT32 u4VldVal = u4VDecReadVLD(u4VDecID, RW_VLD_LDSH);
...
#if 0				/* WMV_LOG_TMP // only for verify 32 bit shift */
	/* u4VDEC_HAL_WMV_GetRealBitStream(u4BSID, u4VDecID, 32); */
	/* u4VDEC_HAL_WMV_GetRealBitStream(u4BSID, u4VDecID, 32); */
	/* u4VDEC_HAL_WMV_GetRealBitStream(u4BSID, u4VDecID, 2); */

	printk("GetBS %d, shift 32, value:0x%x\n", u4BSID,
	       u4VDEC_HAL_WMV_ShiftGetBitStream(u4BSID, u4VDecID, 32));
	printk("GetBS %d, shift 32, value:0x%x\n", u4BSID,
	       u4VDEC_HAL_WMV_ShiftGetBitStream(u4BSID, u4VDecID, 32));
	printk("GetBS %d, shift 02, value:0x%x\n", u4BSID,
	       u4VDEC_HAL_WMV_ShiftGetBitStream(u4BSID, u4VDecID, 2));
	...
	if (fgIsVC1) {
		u4VLDRemainByte -=
		    u4VDec03Number((prWmvBSInitPrm->u4ReadPointer) & (0xFFFFFFF0), u4VLDRemainByte);
	}
	/* ming add for 03 issue:begin */
	for (j = 0; j < u4VLDRemainByte; j++) {
		u4VDecVLDGetBitS(u4BSID, u4VDecID, 8);
		u4VLDByte =
		    u4VDecReadVldRPtr(u4BSID, u4VDecID, &u4VLDBit,
				      PHYSICAL((UINT32) prWmvBSInitPrm->u4VFifoSa));
...
#if 0				/* WMV_LOG_TMP */
	printk("InitBS, rd:0x%x, input window:0x%x, 0x%x\n",
...
/* <0: fail. */
/* ************************************************************************** */
INT32 i4VDEC_HAL_WMV_HWDecBP(UINT32 u4VDecID, UINT32 u4BpNum,
			     VDEC_INFO_WMV_DEC_BP_PRM_T * prWmvDecBpPrm)
{
	...
	UINT32 u4Bits = 0;
	UINT32 u4VLDByte =
	    u4VDEC_HAL_WMV_ReadRdPtr(0, u4VDecID, (UINT32) _pucVFifo[u4VDecID], &u4Bits);
#endif
...
#else
	INT32 height =
	    (prWmvDecBpPrm->ucFrameCodingMode ==
	     INTERLACEFIELD) ? prWmvDecBpPrm->u4PicHeightSrc /
	    2 : prWmvDecBpPrm->u4PicHeightDec /*Main Profile only */;
	...
	vVDecWriteVLD(u4VDecID, RW_VLD_ADDREXTEND, (u4VDecReadVLD(u4VDecID, RW_VLD_ADDREXTEND) & 0xFFFFFFC0) | (((u4AbsDramANc((UINT32) prWmvDecBpPrm->rWmvWorkBufSa.u4Mv1Sa) >> 2) >> 22) & 0x3F));	/* 194 Mv_1#1(frame/top field) */
	/* MPEG-4 use    WriteREG(RW_VLD_BMB1, 0x2d124f80); //196 set once */
	/* MPEG-4 use    WriteREG(RW_VLD_BMB2, 0x2d13d620); //197 set once */
	a198 = ((u4AbsDramANc((UINT32) prWmvDecBpPrm->rWmvWorkBufSa.u4Bp1Sa) >> 2) & 0xFFFFFF) + (((u4AbsDramANc((UINT32) prWmvDecBpPrm->rWmvWorkBufSa.u4Bp1Sa) >> 2) & 0xF000000) << 4) + (pred_use_wdle << 25) +	/* wdle */
	    (1 << 26);
	...
	if ((prWmvDecBpPrm->i4CodecVersion == VDEC_WMV1)
	    || (prWmvDecBpPrm->i4CodecVersion == VDEC_WMV2)) {
		a201 = 0x30900002 + (1 << 16) +	/* ming add for test */
	...
	/* MC post process */
	vVDecWriteMC(u4VDecID, RW_MC_PP_DBLK_Y_ADDR,
		     u4AbsDramANc((UINT32) prWmvDecBpPrm->rWmvWorkBufSa.u4Pp1Sa) >> 4);
	vVDecWriteMC(u4VDecID, RW_MC_PP_DBLK_C_ADDR,
		     u4AbsDramANc((UINT32) prWmvDecBpPrm->rWmvWorkBufSa.u4Pp2Sa) >> 4);
	/* ~initialize prediction */

	if (prWmvDecBpPrm->i4CodecVersion == VDEC_WMV2) {
		a137 = (u4BpNum) + (1 << 16) +	/* wmv8 */
	...
	vop_type =
	    (prWmvDecBpPrm->ucPicType == PVOP) ? 1 : (prWmvDecBpPrm->ucPicType == BVOP) ? 2 : 0;
...
*/
		...
		/* VLD part */
		a163 =
		    (WMV_NEW_FLG_EN | WMV_PRED_MOD_EN | WMV_PRED_RD_SYNC_DISABLE |
		     WMV_DRAM_BURST_MODE_EN | WMV_BP_USE_PRED_RD_EN);
		...
		/* a167, a168, a169 */
		vVDecWriteVLD(u4VDecID, RW_VLD_WMV_BP0_NEW_ADDR,
			      u4AbsDramANc(prWmvDecBpPrm->rWmvWorkBufSa.u4Bp0NewSa));
		vVDecWriteVLD(u4VDecID, RW_VLD_WMV_BP1_NEW_ADDR,
			      u4AbsDramANc(prWmvDecBpPrm->rWmvWorkBufSa.u4Bp1NewSa));
		vVDecWriteVLD(u4VDecID, RW_VLD_WMV_BP2_NEW_ADDR,
			      u4AbsDramANc(prWmvDecBpPrm->rWmvWorkBufSa.u4Bp2NewSa));
		...
		a171 =
		    ((i4OwBp << WMV_NUM_OW_BP_0) | (i4OwBp << WMV_NUM_OW_BP_1) |
		     (i4OwBp << WMV_NUM_OW_BP_2));
...
#if 0				/* WMV_LOG_TMP // print input window */
		printk("Input window :0x%x, 0x%x\n", u4VDecVLDGetBitS(0, 0, 0),
...
INT32 i4VDEC_HAL_WMV_DecStart(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T * prDecPrm)
{
	...
	VDEC_INFO_WMV_DEC_PRM_T *prWmvDecPrm =
	    (VDEC_INFO_WMV_DEC_PRM_T *) &(prDecPrm->SpecDecPrm.rVDecWMVDecPrm);
#else
	VDEC_INFO_WMV_DEC_PRM_T *prWmvDecPrm =
	    (VDEC_INFO_WMV_DEC_PRM_T *) prDecPrm->prVDecCodecHalPrm;
#endif
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8530)
	long a134;
#endif
...
#if VMMU_SUPPORT
	{
		...
		u4Page_addr =
		    ...
		    (((prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) / (4 * 1024)) * 4);
		printk("[WMV] i4VDEC_HAL_WMV_DecStart, 1, Page Addr = 0x%x\n", u4Page_addr);
		vPage_Table(u4VDecID, u4Page_addr,
			    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa),
			    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) + PIC_Y_SZ);
		vVDecWriteMC(u4VDecID, RW_MC_R1Y,
			     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) >> prWmvDecPrm->
			     i4MemBase >> 9);

		u4Page_addr =
		    (UINT32) _pucVMMUTable[u4VDecID] +
		    (((prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) / (4 * 1024)) * 4);
		printk("[WMV] i4VDEC_HAL_WMV_DecStart, 2, Page Addr = 0x%x\n", u4Page_addr);
		vPage_Table(u4VDecID, u4Page_addr,
			    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa),
			    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) + PIC_C_SZ);
		vVDecWriteMC(u4VDecID, RW_MC_R1C,
			     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) >> prWmvDecPrm->
			     i4MemBase >> 8);

		u4Page_addr =
		    (UINT32) _pucVMMUTable[u4VDecID] +
		    (((prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) / (4 * 1024)) * 4);
		printk("[WMV] i4VDEC_HAL_WMV_DecStart, 3, Page Addr = 0x%x\n", u4Page_addr);
		vPage_Table(u4VDecID, u4Page_addr,
			    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa),
			    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) + PIC_Y_SZ);
		vVDecWriteMC(u4VDecID, RW_MC_R2Y,
			     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) >> prWmvDecPrm->
			     i4MemBase >> 9);

		u4Page_addr =
		    (UINT32) _pucVMMUTable[u4VDecID] +
		    (((prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) / (4 * 1024)) * 4);
		printk("[WMV] i4VDEC_HAL_WMV_DecStart, 4, Page Addr = 0x%x\n", u4Page_addr);
		vPage_Table(u4VDecID, u4Page_addr,
			    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa),
			    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) + PIC_C_SZ);
		vVDecWriteMC(u4VDecID, RW_MC_R2C,
			     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) >> prWmvDecPrm->
			     i4MemBase >> 8);

		u4Page_addr =
		    (UINT32) _pucVMMUTable[u4VDecID] +
		    (((prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) / (4 * 1024)) * 4);
		printk("[WMV] i4VDEC_HAL_WMV_DecStart, 5, Page Addr = 0x%x\n", u4Page_addr);
		vPage_Table(u4VDecID, u4Page_addr,
			    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa),
			    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) + PIC_Y_SZ);
		vVDecWriteMC(u4VDecID, RW_MC_BY,
			     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) >> prWmvDecPrm->
			     i4MemBase >> 8);
		vVDecWriteMC(u4VDecID, RW_MC_BY1,
			     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) >> prWmvDecPrm->
			     i4MemBase >> 9);

		u4Page_addr =
		    (UINT32) _pucVMMUTable[u4VDecID] +
		    (((prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) / (4 * 1024)) * 4);
		printk("[WMV] i4VDEC_HAL_WMV_DecStart, 6, Page Addr = 0x%x\n", u4Page_addr);
		vPage_Table(u4VDecID, u4Page_addr,
			    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa),
			    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) + PIC_C_SZ);
		vVDecWriteMC(u4VDecID, RW_MC_BC,
			     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) >> prWmvDecPrm->
			     i4MemBase >> 7);
		vVDecWriteMC(u4VDecID, RW_MC_BC1,
			     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) >> prWmvDecPrm->
			     i4MemBase >> 8);
	}
#else
	...
	vVDecWriteMC(u4VDecID, RW_MC_MODE_CTL,
		     MC_QIU_BANK4 | MC_QIU_BANK8 | MC_DRAM_REQ_DELAY_1T | MC_DRAM_REQ_MERGE_OFF |
		     MC_MV_MERGE_OFF);
#endif
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8530)
	...
	vVDecWriteVLD(u4VDecID, RW_VLD_ADDREXTEND, (u4VDecReadVLD(u4VDecID, RW_VLD_ADDREXTEND) & 0xFFFFFFC0) | (((u4AbsDramANc((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Mv1Sa) >> 2) >> 22) & 0x3F));	/* 194 Mv_1#1(frame/top field) */
	/* MPEG-4 use    WriteREG(RW_VLD_BMB1, 0x2d124f80); //196 set once */
	/* MPEG-4 use    WriteREG(RW_VLD_BMB2, 0x2d13d620); //197 set once */
	a198 = ((u4AbsDramANc((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Bp1Sa) >> 2) & 0xFFFFFF) + (((u4AbsDramANc((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Bp1Sa) >> 2) & 0xF000000) << 4) + (pred_use_wdle << 25) +	/* wdle */
	    (1 << 26);
	vVDecWriteVLD(u4VDecID, RW_VLD_BCODE_SA, a198);	/* (UINT32)prWmvDecPrm->u4Bp1Sa >> 3); //198 Bp_1 */
	/* MPEG-4 use    WriteREG(RW_VLD_DIRE_MD_IL, 0x00000000); //199 set once */

	if ((prWmvDecPrm->i4CodecVersion == VDEC_WMV1)
	    || (prWmvDecPrm->i4CodecVersion == VDEC_WMV2)) {
		a201 = 0x30900002 + (1 << 16) +	/* ming add for test */
	...
	/* MC post process */
	vVDecWriteMC(u4VDecID, RW_MC_PP_DBLK_Y_ADDR,
		     u4AbsDramANc((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pp1Sa) >> 4);
	vVDecWriteMC(u4VDecID, RW_MC_PP_DBLK_C_ADDR,
		     u4AbsDramANc((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pp2Sa) >> 4);
	...
	vVDecWriteVLD(u4VDecID, RW_VLD_MBDRAM_SEL,
		      u4VDecReadVLD(u4VDecID, RW_VLD_MBDRAM_SEL) | (1 << 16));
	/* ------------------------------ */
	...
	} else			/* if (prWmvDecPrm->ucPicType == BVOP) , Ju said both for BIVOP and BVOP */
	{
		...
		vVDecWriteMC(u4VDecID, RW_MC_WMV9_PRE_PROC,
			     prWmvDecPrm->prICOMPS->ucPreProcessFrameStatus);
	}
	...
	if ((prWmvDecPrm->prPPS->ucPicType == IVOP) && (prWmvDecPrm->i4CodecVersion == VDEC_VC1)) {
	    ...
	    if (((prWmvDecPrm->prPPS->ucPicType == PVOP)
		 || (prWmvDecPrm->prPPS->ucPicType == SKIPFRAME))
		&& (prWmvDecPrm->i4CodecVersion == VDEC_VC1)) {
		...
		if (prWmvDecPrm->prICOMPS->NewTopField.New.i4Enable == 1) {
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE1_X1,
				     (prWmvDecPrm->prICOMPS->NewTopField.New.i4Scale * 1));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE1_X3,
				     (prWmvDecPrm->prICOMPS->NewTopField.New.i4Scale * 3));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE1_X5,
				     (prWmvDecPrm->prICOMPS->NewTopField.New.i4Scale * 5));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE1_X7,
				     (prWmvDecPrm->prICOMPS->NewTopField.New.i4Scale * 7));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE1_X9,
				     (prWmvDecPrm->prICOMPS->NewTopField.New.i4Scale * 9));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE1_X11,
				     (prWmvDecPrm->prICOMPS->NewTopField.New.i4Scale * 11));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE1_X13,
				     (prWmvDecPrm->prICOMPS->NewTopField.New.i4Scale * 13));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE1_X15,
				     (prWmvDecPrm->prICOMPS->NewTopField.New.i4Scale * 15));
			vVDecWriteMC(u4VDecID, RW_MC_YSHIFT_OFF1,
				     (prWmvDecPrm->prICOMPS->NewTopField.New.i4Shift + 32));
			vVDecWriteMC(u4VDecID, RW_MC_CSHIFT_OFF1,
				     ((128 * 64) + 32 -
				      (128 * prWmvDecPrm->prICOMPS->NewTopField.New.i4Scale)));
		}
		/* icomp_set2 is forward bottom field new icomp parameters */
		if (prWmvDecPrm->prICOMPS->NewBotField.New.i4Enable == 1) {
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE2_X1,
				     (prWmvDecPrm->prICOMPS->NewBotField.New.i4Scale * 1));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE2_X3,
				     (prWmvDecPrm->prICOMPS->NewBotField.New.i4Scale * 3));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE2_X5,
				     (prWmvDecPrm->prICOMPS->NewBotField.New.i4Scale * 5));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE2_X7,
				     (prWmvDecPrm->prICOMPS->NewBotField.New.i4Scale * 7));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE2_X9,
				     (prWmvDecPrm->prICOMPS->NewBotField.New.i4Scale * 9));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE2_X11,
				     (prWmvDecPrm->prICOMPS->NewBotField.New.i4Scale * 11));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE2_X13,
				     (prWmvDecPrm->prICOMPS->NewBotField.New.i4Scale * 13));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE2_X15,
				     (prWmvDecPrm->prICOMPS->NewBotField.New.i4Scale * 15));
			vVDecWriteMC(u4VDecID, RW_MC_YSHIFT_OFF2,
				     (prWmvDecPrm->prICOMPS->NewBotField.New.i4Shift + 32));
			vVDecWriteMC(u4VDecID, RW_MC_CSHIFT_OFF2,
				     ((128 * 64) + 32 -
				      (128 * prWmvDecPrm->prICOMPS->NewBotField.New.i4Scale)));
		}

		if (prWmvDecPrm->prICOMPS->NewTopField.Old.i4Enable == 1) {
			/* icomp_set3 is forward top field old icomp parameters */
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE3_X1,
				     (prWmvDecPrm->prICOMPS->NewTopField.Old.i4Scale * 1));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE3_X3,
				     (prWmvDecPrm->prICOMPS->NewTopField.Old.i4Scale * 3));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE3_X5,
				     (prWmvDecPrm->prICOMPS->NewTopField.Old.i4Scale * 5));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE3_X7,
				     (prWmvDecPrm->prICOMPS->NewTopField.Old.i4Scale * 7));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE3_X9,
				     (prWmvDecPrm->prICOMPS->NewTopField.Old.i4Scale * 9));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE3_X11,
				     (prWmvDecPrm->prICOMPS->NewTopField.Old.i4Scale * 11));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE3_X13,
				     (prWmvDecPrm->prICOMPS->NewTopField.Old.i4Scale * 13));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE3_X15,
				     (prWmvDecPrm->prICOMPS->NewTopField.Old.i4Scale * 15));
			vVDecWriteMC(u4VDecID, RW_MC_YSHIFT_OFF3,
				     (prWmvDecPrm->prICOMPS->NewTopField.Old.i4Shift + 32));
			vVDecWriteMC(u4VDecID, RW_MC_CSHIFT_OFF3,
				     ((128 * 64) + 32 -
				      (128 * prWmvDecPrm->prICOMPS->NewTopField.Old.i4Scale)));
		}

		if (prWmvDecPrm->prICOMPS->NewBotField.Old.i4Enable == 1) {
			/* icomp_set4 is forward bottom field old icomp parameters */
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X1,
				     (prWmvDecPrm->prICOMPS->NewBotField.Old.i4Scale * 1));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X3,
				     (prWmvDecPrm->prICOMPS->NewBotField.Old.i4Scale * 3));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X5,
				     (prWmvDecPrm->prICOMPS->NewBotField.Old.i4Scale * 5));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X7,
				     (prWmvDecPrm->prICOMPS->NewBotField.Old.i4Scale * 7));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X9,
				     (prWmvDecPrm->prICOMPS->NewBotField.Old.i4Scale * 9));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X11,
				     (prWmvDecPrm->prICOMPS->NewBotField.Old.i4Scale * 11));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X13,
				     (prWmvDecPrm->prICOMPS->NewBotField.Old.i4Scale * 13));
			vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X15,
				     (prWmvDecPrm->prICOMPS->NewBotField.Old.i4Scale * 15));
			vVDecWriteMC(u4VDecID, RW_MC_YSHIFT_OFF4,
				     (prWmvDecPrm->prICOMPS->NewBotField.Old.i4Shift + 32));
			vVDecWriteMC(u4VDecID, RW_MC_CSHIFT_OFF4,
				     ((128 * 64) + 32 -
				      (128 * prWmvDecPrm->prICOMPS->NewBotField.Old.i4Scale)));
		}
		...
		if (prWmvDecPrm->prICOMPS->i4BoundaryUMVIcomp == 1) {
			...
			if (prWmvDecPrm->prICOMPS->i4SecondFieldParity == 0) {
				if (prWmvDecPrm->prICOMPS->OldBotField.New.i4Enable == 1) {
					...
					vVDecWriteMC(u4VDecID, RW_MC_ISCALE6_X1,
						     (prWmvDecPrm->prICOMPS->OldBotField.New.
						      i4Scale * 1));
					vVDecWriteMC(u4VDecID, RW_MC_ISCALE6_X3,
						     (prWmvDecPrm->prICOMPS->OldBotField.New.
						      i4Scale * 3));
					vVDecWriteMC(u4VDecID, RW_MC_ISCALE6_X5,
						     (prWmvDecPrm->prICOMPS->OldBotField.New.
						      i4Scale * 5));
					vVDecWriteMC(u4VDecID, RW_MC_ISCALE6_X7,
						     (prWmvDecPrm->prICOMPS->OldBotField.New.
						      i4Scale * 7));
					vVDecWriteMC(u4VDecID, RW_MC_ISCALE6_X9,
						     (prWmvDecPrm->prICOMPS->OldBotField.New.
						      i4Scale * 9));
					vVDecWriteMC(u4VDecID, RW_MC_ISCALE6_X11,
						     (prWmvDecPrm->prICOMPS->OldBotField.New.
						      i4Scale * 11));
					vVDecWriteMC(u4VDecID, RW_MC_ISCALE6_X13,
						     (prWmvDecPrm->prICOMPS->OldBotField.New.
						      i4Scale * 13));
					vVDecWriteMC(u4VDecID, RW_MC_ISCALE6_X15,
						     (prWmvDecPrm->prICOMPS->OldBotField.New.
						      i4Scale * 15));
					vVDecWriteMC(u4VDecID, RW_MC_YSHIFT_OFF6,
						     (prWmvDecPrm->prICOMPS->OldBotField.New.
						      i4Shift + 32));
					vVDecWriteMC(u4VDecID, RW_MC_CSHIFT_OFF6,
						     ((128 * 64) + 32 -
						      (128 *
						       prWmvDecPrm->prICOMPS->OldBotField.New.
						       i4Scale)));
			...
			/* bottom field picture */
				...
				if (prWmvDecPrm->prICOMPS->OldTopField.New.i4Enable == 1) {
					/* icomp_set6 is forward bottom field old icomp parameters */
					vVDecWriteMC(u4VDecID, RW_MC_ISCALE6_X1,
						     (prWmvDecPrm->prICOMPS->OldTopField.New.
						      i4Scale * 1));
					vVDecWriteMC(u4VDecID, RW_MC_ISCALE6_X3,
						     (prWmvDecPrm->prICOMPS->OldTopField.New.
						      i4Scale * 3));
		...
		if ((prWmvDecPrm->prICOMPS->NewTopField.New.i4Enable == 1) ||
			...
			/* Icomp enable bits */
			{
				...
				vVDecWriteMC(u4VDecID, RW_MC_WMV9_ICOMP_EN,
					     ICOMP_C_EN + ICOMP_Y_EN);
				vVDecWriteMC(u4VDecID, RW_MC_ICOMP2_EN, b208);
				vVDecWriteMC(u4VDecID, RW_MC_SAME_ICOMP,
		...
		if (prWmvDecPrm->prPPS->fgFieldMode == TRUE)	/* field picture */
		{
			if (prWmvDecPrm->prPPS->i4CurrentTemporalField == 0)	/* the first B field picture */
			{
				...
				if (prWmvDecPrm->prICOMPS->i4SecondFieldParity == 0) {
					...
					if (prWmvDecPrm->prICOMPS->NewTopField.New.i4Enable == 1) {
						vVDecWriteMC(u4VDecID, RW_MC_ISCALE1_X1,
							     (prWmvDecPrm->prICOMPS->NewTopField.
							      New.i4Scale * 1));
						vVDecWriteMC(u4VDecID, RW_MC_ISCALE1_X3,
							     (prWmvDecPrm->prICOMPS->NewTopField.
							      New.i4Scale * 3));
					...
					if (prWmvDecPrm->prICOMPS->OldBotField.New.i4Enable == 1) {
						vVDecWriteMC(u4VDecID, RW_MC_ISCALE2_X1,
							     (prWmvDecPrm->prICOMPS->OldBotField.
							      New.i4Scale * 1));
						vVDecWriteMC(u4VDecID, RW_MC_ISCALE2_X3,
							     (prWmvDecPrm->prICOMPS->OldBotField.
							      New.i4Scale * 3));
					...
					if (prWmvDecPrm->prICOMPS->NewBotField.New.i4Enable == 1) {
						vVDecWriteMC(u4VDecID, RW_MC_ISCALE5_X1,
							     (prWmvDecPrm->prICOMPS->NewBotField.
							      New.i4Scale * 1));
						vVDecWriteMC(u4VDecID, RW_MC_ISCALE5_X3,
							     (prWmvDecPrm->prICOMPS->NewBotField.
							      New.i4Scale * 3));
						vVDecWriteMC(u4VDecID, RW_MC_ISCALE5_X5,
							     (prWmvDecPrm->prICOMPS->NewBotField.
							      New.i4Scale * 5));
						vVDecWriteMC(u4VDecID, RW_MC_ISCALE5_X7,
							     (prWmvDecPrm->prICOMPS->NewBotField.
							      New.i4Scale * 7));
						vVDecWriteMC(u4VDecID, RW_MC_ISCALE5_X9,
							     (prWmvDecPrm->prICOMPS->NewBotField.
							      New.i4Scale * 9));
						vVDecWriteMC(u4VDecID, RW_MC_ISCALE5_X11,
							     (prWmvDecPrm->prICOMPS->NewBotField.
							      New.i4Scale * 11));
						vVDecWriteMC(u4VDecID, RW_MC_ISCALE5_X13,
							     (prWmvDecPrm->prICOMPS->NewBotField.
							      New.i4Scale * 13));
						vVDecWriteMC(u4VDecID, RW_MC_ISCALE5_X15,
							     (prWmvDecPrm->prICOMPS->NewBotField.
							      New.i4Scale * 15));
						vVDecWriteMC(u4VDecID, RW_MC_YSHIFT_OFF5,
							     (prWmvDecPrm->prICOMPS->NewBotField.
							      New.i4Shift + 32));
						vVDecWriteMC(u4VDecID, RW_MC_CSHIFT_OFF5,
							     ((128 * 64) + 32 -
							      (128 *
							       prWmvDecPrm->prICOMPS->NewBotField.
							       New.i4Scale)));
						vVDecWriteMC(u4VDecID, RW_MC_BWD_ICOMP_FLD,
							     BWD_BTM_FLD);
					}

					if ((prWmvDecPrm->prICOMPS->NewTopField.New.i4Enable == 1)
					    || (prWmvDecPrm->prICOMPS->OldBotField.New.i4Enable ==
						1)
					    || (prWmvDecPrm->prICOMPS->NewTopField.Old.i4Enable ==
						1)
					    || (prWmvDecPrm->prICOMPS->OldBotField.Old.i4Enable ==
						1)
					    || (prWmvDecPrm->prICOMPS->NewBotField.New.i4Enable ==
						1)
					    ) {
						/* Icomp enable bits */
						{
							INT32 b208 = (0) +
							    (prWmvDecPrm->prICOMPS->NewBotField.New.
							     i4Enable << 4) +
							    (prWmvDecPrm->prICOMPS->OldBotField.Old.
							     i4Enable << 8) +
							    (prWmvDecPrm->prICOMPS->NewTopField.Old.
							     i4Enable << 12) +
							    (prWmvDecPrm->prICOMPS->OldBotField.New.
							     i4Enable << 16) +
							    (prWmvDecPrm->prICOMPS->NewTopField.New.
							     i4Enable << 20);

							vVDecWriteMC(u4VDecID, RW_MC_WMV9_ICOMP_EN,
								     ICOMP_C_EN + ICOMP_Y_EN);
							vVDecWriteMC(u4VDecID, RW_MC_ICOMP2_EN,
								     b208);
							vVDecWriteMC(u4VDecID, RW_MC_SAME_ICOMP,
								     NO_USE_SAME_ICOPM1_FOR_FRAME);
						}
					...
					if (prWmvDecPrm->prICOMPS->OldTopField.New.i4Enable == 1) {
						vVDecWriteMC(u4VDecID, RW_MC_ISCALE1_X1,
							     (prWmvDecPrm->prICOMPS->OldTopField.
							      New.i4Scale * 1));
						vVDecWriteMC(u4VDecID, RW_MC_ISCALE1_X3,
							     (prWmvDecPrm->prICOMPS->OldTopField.
							      New.i4Scale * 3));
					...
					if ((prWmvDecPrm->prICOMPS->OldTopField.New.i4Enable == 1)
					    || (prWmvDecPrm->prICOMPS->NewBotField.New.i4Enable ==
						1)
					    || (prWmvDecPrm->prICOMPS->OldTopField.Old.i4Enable ==
						1)
					    || (prWmvDecPrm->prICOMPS->NewBotField.Old.i4Enable ==
						1)
					    || (prWmvDecPrm->prICOMPS->NewTopField.New.i4Enable ==
						1)
					    ) {
						/* Icomp enable bits */
						{
							INT32 b208 = (0) +
							    (prWmvDecPrm->prICOMPS->NewTopField.New.
							     i4Enable << 4) +
							    (prWmvDecPrm->prICOMPS->NewBotField.Old.
							     i4Enable << 8) +
							    (prWmvDecPrm->prICOMPS->OldTopField.Old.
							     i4Enable << 12) +
							    (prWmvDecPrm->prICOMPS->NewBotField.New.
							     i4Enable << 16) +
							    (prWmvDecPrm->prICOMPS->OldTopField.New.
			...
			else	/* Second B field picture */
			{
				...
				if (prWmvDecPrm->prPPS->i4CurrentField == 0)	/* current field is top field */
				{
					...
					if (prWmvDecPrm->prICOMPS->i4SecondFieldParity == 0) {
						/* icomp_set1 is forward top field new icomp parameters */
						if (prWmvDecPrm->prICOMPS->NewTopField.New.
						    i4Enable == 1) {
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE1_X1,
								     (prWmvDecPrm->prICOMPS->
								      NewTopField.New.i4Scale * 1));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE1_X3,
								     (prWmvDecPrm->prICOMPS->
								      NewTopField.New.i4Scale * 3));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE1_X5,
								     (prWmvDecPrm->prICOMPS->
								      NewTopField.New.i4Scale * 5));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE1_X7,
								     (prWmvDecPrm->prICOMPS->
								      NewTopField.New.i4Scale * 7));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE1_X9,
								     (prWmvDecPrm->prICOMPS->
								      NewTopField.New.i4Scale * 9));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE1_X11,
								     (prWmvDecPrm->prICOMPS->
								      NewTopField.New.i4Scale *
								      11));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE1_X13,
								     (prWmvDecPrm->prICOMPS->
								      NewTopField.New.i4Scale *
						...
						/* icomp_set3 is forward top field old icomp parameters */
						if (prWmvDecPrm->prICOMPS->NewTopField.Old.
						    i4Enable == 1) {
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE3_X1,
								     (prWmvDecPrm->prICOMPS->
								      NewTopField.Old.i4Scale * 1));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE3_X3,
								     (prWmvDecPrm->prICOMPS->
								      NewTopField.Old.i4Scale * 3));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE3_X5,
								     (prWmvDecPrm->prICOMPS->
								      NewTopField.Old.i4Scale * 5));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE3_X7,
								     (prWmvDecPrm->prICOMPS->
								      NewTopField.Old.i4Scale * 7));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE3_X9,
								     (prWmvDecPrm->prICOMPS->
								      NewTopField.Old.i4Scale * 9));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE3_X11,
								     (prWmvDecPrm->prICOMPS->
								      NewTopField.Old.i4Scale *
								      11));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE3_X13,
								     (prWmvDecPrm->prICOMPS->
								      NewTopField.Old.i4Scale *
						...
						/* icomp_set5 is backward top or bottom field icomp parameters */
						if (prWmvDecPrm->prICOMPS->NewBotField.New.
						    i4Enable == 1) {
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE5_X1,
								     (prWmvDecPrm->prICOMPS->
								      NewBotField.New.i4Scale * 1));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE5_X3,
								     (prWmvDecPrm->prICOMPS->
								      NewBotField.New.i4Scale * 3));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE5_X5,
								     (prWmvDecPrm->prICOMPS->
								      NewBotField.New.i4Scale * 5));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE5_X7,
								     (prWmvDecPrm->prICOMPS->
								      NewBotField.New.i4Scale * 7));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE5_X9,
								     (prWmvDecPrm->prICOMPS->
								      NewBotField.New.i4Scale * 9));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE5_X11,
								     (prWmvDecPrm->prICOMPS->
								      NewBotField.New.i4Scale *
								      11));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE5_X13,
								     (prWmvDecPrm->prICOMPS->
								      NewBotField.New.i4Scale *
						...
						if (prWmvDecPrm->prICOMPS->i4BoundaryUMVIcomp == 1) {
							/* top field picture */
							if (prWmvDecPrm->prICOMPS->
							    i4SecondFieldParity == 0) {
								if (prWmvDecPrm->prICOMPS->
								    OldBotField.New.i4Enable == 1) {
									/* icomp_set4 is forward bottom field old icomp parameters */
									vVDecWriteMC(u4VDecID,
										     RW_MC_ISCALE6_X1,
										     (prWmvDecPrm->
										      prICOMPS->
										      OldBotField.
						...
						if ((prWmvDecPrm->prICOMPS->NewTopField.New.
							...
							i4Enable == 1)
						    || (prWmvDecPrm->prICOMPS->
							i4BoundaryUMVIcompEnable == 1)
						    ) {
							/* Icomp enable bits */
							{
								INT32 b208 =
								    (prWmvDecPrm->prICOMPS->
								     i4BoundaryUMVIcompEnable) +
								    (prWmvDecPrm->prICOMPS->
								     NewBotField.New.
								     i4Enable << 4) + (0 << 8) +
								    (prWmvDecPrm->prICOMPS->
						...
						/* icomp_set4 is forward bottom field old icomp parameters */
						if (prWmvDecPrm->prICOMPS->NewBotField.Old.
						    i4Enable == 1) {
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X1,
								     (prWmvDecPrm->prICOMPS->
								      NewBotField.Old.i4Scale * 1));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X3,
								     (prWmvDecPrm->prICOMPS->
								      NewBotField.Old.i4Scale * 3));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X5,
								     (prWmvDecPrm->prICOMPS->
								      NewBotField.Old.i4Scale * 5));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X7,
								     (prWmvDecPrm->prICOMPS->
								      NewBotField.Old.i4Scale * 7));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X9,
								     (prWmvDecPrm->prICOMPS->
								      NewBotField.Old.i4Scale * 9));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X11,
								     (prWmvDecPrm->prICOMPS->
								      NewBotField.Old.i4Scale *
								      11));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X13,
								     (prWmvDecPrm->prICOMPS->
								      NewBotField.Old.i4Scale *
				...
				} else	/* current field is bottom field */
				{
					...
					if (prWmvDecPrm->prICOMPS->i4SecondFieldParity == 0) {
						/* icomp_set2 is forward bottom field new icomp parameters */
						if (prWmvDecPrm->prICOMPS->OldBotField.New.
						    i4Enable == 1) {
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE2_X1,
								     (prWmvDecPrm->prICOMPS->
								      OldBotField.New.i4Scale * 1));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE2_X3,
								     (prWmvDecPrm->prICOMPS->
								      OldBotField.New.i4Scale * 3));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE2_X5,
								     (prWmvDecPrm->prICOMPS->
								      OldBotField.New.i4Scale * 5));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE2_X7,
								     (prWmvDecPrm->prICOMPS->
								      OldBotField.New.i4Scale * 7));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE2_X9,
								     (prWmvDecPrm->prICOMPS->
								      OldBotField.New.i4Scale * 9));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE2_X11,
								     (prWmvDecPrm->prICOMPS->
								      OldBotField.New.i4Scale *
								      11));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE2_X13,
								     (prWmvDecPrm->prICOMPS->
								      OldBotField.New.i4Scale *
						...
						/* icomp_set4 is forward bottom field old icomp parameters */
						if (prWmvDecPrm->prICOMPS->OldBotField.Old.
						    i4Enable == 1) {
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X1,
								     (prWmvDecPrm->prICOMPS->
								      OldBotField.Old.i4Scale * 1));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X3,
								     (prWmvDecPrm->prICOMPS->
								      OldBotField.Old.i4Scale * 3));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X5,
								     (prWmvDecPrm->prICOMPS->
								      OldBotField.Old.i4Scale * 5));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X7,
								     (prWmvDecPrm->prICOMPS->
								      OldBotField.Old.i4Scale * 7));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X9,
								     (prWmvDecPrm->prICOMPS->
								      OldBotField.Old.i4Scale * 9));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X11,
								     (prWmvDecPrm->prICOMPS->
								      OldBotField.Old.i4Scale *
								      11));
							vVDecWriteMC(u4VDecID, RW_MC_ISCALE4_X13,
								     (prWmvDecPrm->prICOMPS->
								      OldBotField.Old.i4Scale *
		...
		} else		/* B frame pciture */
		{
			...
			if (prWmvDecPrm->prICOMPS->NewTopField.New.i4Enable == 1) {
				...
				vVDecWriteMC(u4VDecID, RW_MC_CSHIFT_OFF1,
					     ((128 * 64) + 32 -
					      (128 *
					       prWmvDecPrm->prICOMPS->NewTopField.New.i4Scale)));
			}
			/* icomp_set2 is forward bottom field new icomp parameters */
			if (prWmvDecPrm->prICOMPS->NewBotField.New.i4Enable == 1) {
				vVDecWriteMC(u4VDecID, RW_MC_ISCALE2_X1,
				...
				vVDecWriteMC(u4VDecID, RW_MC_CSHIFT_OFF2,
					     ((128 * 64) + 32 -
					      (128 *
					       prWmvDecPrm->prICOMPS->NewBotField.New.i4Scale)));
			}
			...
			if (prWmvDecPrm->prICOMPS->NewTopField.Old.i4Enable == 1) {
				...
				vVDecWriteMC(u4VDecID, RW_MC_CSHIFT_OFF3,
					     ((128 * 64) + 32 -
					      (128 *
					       prWmvDecPrm->prICOMPS->NewTopField.Old.i4Scale)));
			}
			...
			if (prWmvDecPrm->prICOMPS->NewBotField.Old.i4Enable == 1) {
				...
				vVDecWriteMC(u4VDecID, RW_MC_CSHIFT_OFF4,
					     ((128 * 64) + 32 -
					      (128 *
					       prWmvDecPrm->prICOMPS->NewBotField.Old.i4Scale)));
			}
	...
	if (prWmvDecPrm->i4CodecVersion >= VDEC_WMV3) {
		...
		if ((prWmvDecPrm->i4CodecVersion == VDEC_WMV2)
		    && (prWmvDecPrm->prPPS->fgMvResolution == 1)) {
			vVDecWriteMC(u4VDecID, RW_MC_FILTER_TYPE, (Y_BICUBIC + C_BILINEAR));
		} else {
			vVDecWriteMC(u4VDecID, RW_MC_FILTER_TYPE, (Y_BILINEAR + C_BILINEAR));
		}
	...
	/* ------------------------------------------------------- */
	if (prWmvDecPrm->prPPS->i4Overlap & 1) {	/* clip operation will be handled by overlap_smooth module */
	...
	if (prWmvDecPrm->i4CodecVersion >= VDEC_WMV3) {
		/* simple/main I */
		if ((prWmvDecPrm->prPPS->ucPicType == IVOP
		     || prWmvDecPrm->prPPS->ucPicType == BIVOP)
		    && (prWmvDecPrm->i4CodecVersion == VDEC_WMV3)) {
	...
	if ((prWmvDecPrm->prPPS->ucPicType == BVOP) || (prWmvDecPrm->prPPS->ucPicType == BIVOP))	/* prog_B */
	{
#if VMMU_SUPPORT
		{
			...
			vPage_Table(u4VDecID, u4Page_addr,
				    ...
				    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) +
				    PIC_Y_SZ);
			vVDecWriteMC(u4VDecID, RW_MC_PP_Y_ADDR,
				     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.
				      u4Pic2YSa) >> prWmvDecPrm->i4MemBase >> 9);
			...
			vPage_Table(u4VDecID, u4Page_addr,
				    ...
				    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) +
				    PIC_C_SZ);
			vVDecWriteMC(u4VDecID, RW_MC_PP_C_ADDR,
				     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.
				      u4Pic2CSa) >> prWmvDecPrm->i4MemBase >> 8);
		}
#else
		vVDecWriteMC(u4VDecID, RW_MC_PP_Y_ADDR,
			     (u4AbsDramANc((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) >>
			      prWmvDecPrm->i4MemBase) >> 9);
		vVDecWriteMC(u4VDecID, RW_MC_PP_C_ADDR,
			     (u4AbsDramANc((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) >>
			      prWmvDecPrm->i4MemBase) >> 8);
#endif
	} else if (prDecPrm->ucDecFBufIdx == 0)	/* pic1 */
	{
#if VMMU_SUPPORT
		{
			...
			vVDecWriteMC(u4VDecID, RW_MC_PP_Y_ADDR,
				     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.
				      u4Pic0YSa) >> prWmvDecPrm->i4MemBase >> 9);
			...
			vVDecWriteMC(u4VDecID, RW_MC_PP_C_ADDR,
				     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.
				      u4Pic0CSa) >> prWmvDecPrm->i4MemBase >> 8);
		}
#else
		vVDecWriteMC(u4VDecID, RW_MC_PP_Y_ADDR,
			     (u4AbsDramANc((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) >>
			      prWmvDecPrm->i4MemBase) >> 9);
		vVDecWriteMC(u4VDecID, RW_MC_PP_C_ADDR,
			     (u4AbsDramANc((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) >>
			      prWmvDecPrm->i4MemBase) >> 8);
#endif
	} else			/* pic2 */
	{
#if VMMU_SUPPORT
		{
			...
			vVDecWriteMC(u4VDecID, RW_MC_PP_Y_ADDR,
				     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.
				      u4Pic1YSa) >> prWmvDecPrm->i4MemBase >> 9);
			...
			vVDecWriteMC(u4VDecID, RW_MC_PP_C_ADDR,
				     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.
				      u4Pic1CSa) >> prWmvDecPrm->i4MemBase >> 8);
		}
#else
		vVDecWriteMC(u4VDecID, RW_MC_PP_Y_ADDR,
			     (u4AbsDramANc((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) >>
			      prWmvDecPrm->i4MemBase) >> 9);
		vVDecWriteMC(u4VDecID, RW_MC_PP_C_ADDR,
			     (u4AbsDramANc((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) >>
			      prWmvDecPrm->i4MemBase) >> 8);
#endif
	}
	...
	if (prWmvDecPrm->i4CodecVersion == VDEC_VC1) {
		...
		/* ----------------------------------------------------- */
		vVDecWriteMC(u4VDecID, RW_MC_COND_OVL_FILTER,
			     (prWmvDecPrm->prPPS->i4Overlap & 4) ? COND_OVL_EN : COND_OVL_OFF);
		...
		vVDecWriteMC(u4VDecID, RW_MC_RNG_PARA,
			     ((prWmvDecPrm->prEPS->i4RangeRedY >
			       0) ? prWmvDecPrm->prEPS->i4RangeRedY - 1 : 0) * 256 +
			     ((prWmvDecPrm->prEPS->i4RangeMapUV >
			       0) ? prWmvDecPrm->prEPS->i4RangeMapUV - 1 : 0));
	} else {
		/* ----------------------------------------------------- */
		/* MC_reg_177 : for fix MS ref_code bug (bit[0] [8] [16]) */
		/* ----------------------------------------------------- */
	...
	/* --------------------------------------------------------- */
	if (prWmvDecPrm->i4CodecVersion >= VDEC_WMV3) {
		INT32 b231 = ((prWmvDecPrm->prPPS->u4BackwardRefPicType == 0) ? BWD_PROG_REF_PIC : BWD_INTLCE_REF_PIC) + (((prWmvDecPrm->prPPS->u4ForwardRefPicType == 0) ? FWD_PROG_REF_PIC : FWD_INTLCE_REF_PIC));	/* 0: 1) << 16); */
	...
	if ((prWmvDecPrm->i4CodecVersion == VDEC_WMV1)
	    || (prWmvDecPrm->i4CodecVersion == VDEC_WMV2)) {
		use_interpredictor =
		    (prWmvDecPrm->prSPS->i4BitRate <= MAX_BITRATE_DCPred_IMBInPFrame
		     && (prWmvDecPrm->prSPS->u4PicWidthSrc * prWmvDecPrm->prSPS->u4PicHeightSrc <
			 320 * 240)) && (prWmvDecPrm->prPPS->ucPicType == PVOP)
		    && (prWmvDecPrm->i4CodecVersion == VDEC_WMV1);
		use_rule_b = (((prWmvDecPrm->prPPS->i4Overlap & 0x1) == 0)
			      && (prWmvDecPrm->prPPS->ucPicType == IVOP)
			      && ((prWmvDecPrm->prSPS->i4WMV3Profile == WMV3_SIMPLE_PROFILE)
				  || (prWmvDecPrm->prSPS->i4WMV3Profile == WMV3_MAIN_PROFILE)))
		    || use_interpredictor | (prWmvDecPrm->i4CodecVersion <= VDEC_WMV2);
		use_quarter_pel_mv = 0;

		a193 =
		    /* 0x0021c0b4 + */
		    (((prWmvDecPrm->prSPS->u4PicWidthSrc + 15) / 16) * 4) + ((((prWmvDecPrm->prSPS->u4PicWidthSrc + 15) / 16) * 4 * 3) << 12) + (use_rule_b << 27) + (use_interpredictor << 28) + (use_quarter_pel_mv /*(pWMVDec->m_pAltTables->m_iHalfPelMV == 0) */  << 29) +	/* quarter-pel */
		    (0 << 30);	/* dcac overlap */
	...
	} else			/* >= VDEC_WMV3 */
	{
		...
		use_rule_b = ((prWmvDecPrm->prPPS->i4Overlap & 0x1) == 0) &&
			...
			|| (prWmvDecPrm->prSPS->i4WMV3Profile == WMV3_MAIN_PROFILE));
		use_quarter_pel_mv =
		    (prWmvDecPrm->prPPS->ucFrameCodingMode ==
		     INTERLACEFRAME) ? 1 : ((prWmvDecPrm->prPPS->i4X9MVMode == ALL_1MV_HALFPEL
					     || prWmvDecPrm->prPPS->i4X9MVMode ==
	...
	/* ----------------------------------------------------- */
	a198 = ((u4AbsDramANc((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Bp1Sa) >> 2) & 0xFFFFFF) + (((u4AbsDramANc((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Bp1Sa) >> 2) & 0xF000000) << 4) + (pred_use_wdle << 25) + (1 << 26);	/* wdle */
	...
	/* ----------------------------------------------------- */
	{
		...
		if ((prWmvDecPrm->i4CodecVersion == VDEC_WMV1)
			...
			a200 = 0x0020003a + (pred_use_wdle << 6) + (pred_use_wdle << 7) + (pred_use_wdle << 20) + (1 << 11) + (((prWmvDecPrm->prPPS->i4Panning & 0xf) != 0) << 9) + ((prWmvDecPrm->i4CodecVersion == VDEC_WMV1 || prWmvDecPrm->i4CodecVersion == VDEC_WMV2) << 15) + (prWmvDecPrm->prICOMPS->i4ResetMvDram << 18) + ((!(prWmvDecPrm->prSPS->u4NumMBY == (prWmvDecPrm->prSPS->u4NumMBY / (prWmvDecPrm->prSPS->i4SliceCode - sliceoffset)))) << 22) + (mv_sram_use << 31);	/* + */
		...
		{
			mv_sram_use_ok = ((prWmvDecPrm->prPPS->ucPicType == PVOP)
					  || ((prWmvDecPrm->prPPS->ucPicType == BVOP)
					      && (prWmvDecPrm->prPPS->ucFrameCodingMode !=
						  INTERLACEFIELD))) &&
...
#if VMMU_SUPPORT
			{
				...
				vPage_Table(u4VDecID, u4Page_addr,
					    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa),
					    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.
						     u4Pic0YSa) + PIC_Y_SZ);
				vVDecWriteMC(u4VDecID, RW_MC_R1Y,
					     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.
					      u4Pic0YSa) >> prWmvDecPrm->i4MemBase >> 9);
				...
				vVDecWriteMC(u4VDecID, RW_MC_BY,
					     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.
					      u4Pic2YSa) >> prWmvDecPrm->i4MemBase >> 8);
				vVDecWriteMC(u4VDecID, RW_MC_BY1,
					     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.
					      u4Pic2YSa) >> prWmvDecPrm->i4MemBase >> 9);
				...
				vVDecWriteMC(u4VDecID, RW_MC_BC,
					     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.
					      u4Pic2CSa) >> prWmvDecPrm->i4MemBase >> 7);
				vVDecWriteMC(u4VDecID, RW_MC_BC1,
					     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.
					      u4Pic2CSa) >> prWmvDecPrm->i4MemBase >> 8);
			}
			...
			if ((prWmvDecPrm->prICOMPS->ucFrameTypeLast == SKIPFRAME)
			    && (prWmvDecPrm->prPPS->ucPicType == BVOP)) {
				if (my_forward == 1) {
#if VMMU_SUPPORT
					{
						...
						u4Page_addr =
						    (UINT32) _pucVMMUTable[u4VDecID] +
						    (((prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) /
						      (4 * 1024)) * 4);
						printk
						    ("[WMV] i4VDEC_HAL_WMV_DecStart, 19, Page Addr = 0x%x\n",
						     u4Page_addr);
						vPage_Table(u4VDecID, u4Page_addr,
							    PHYSICAL((UINT32) prWmvDecPrm->
								     rWmvWorkBufSa.u4Pic0YSa),
							    PHYSICAL((UINT32) prWmvDecPrm->
								     rWmvWorkBufSa.u4Pic0YSa) +
							    PIC_Y_SZ);
						vVDecWriteMC(u4VDecID, RW_MC_R1Y,
							     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.
							      u4Pic0YSa) >> prWmvDecPrm->
							     i4MemBase >> 9);
						vVDecWriteMC(u4VDecID, RW_MC_R2Y,
							     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.
							      u4Pic0YSa) >> prWmvDecPrm->
						...
						u4Page_addr =
						    (UINT32) _pucVMMUTable[u4VDecID] +
						    (((prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) /
						      (4 * 1024)) * 4);
						printk
						    ("[WMV] i4VDEC_HAL_WMV_DecStart, 20, Page Addr = 0x%x\n",
						     u4Page_addr);
						vPage_Table(u4VDecID, u4Page_addr,
							    PHYSICAL((UINT32) prWmvDecPrm->
								     rWmvWorkBufSa.u4Pic0CSa),
							    PHYSICAL((UINT32) prWmvDecPrm->
								     rWmvWorkBufSa.u4Pic0CSa) +
							    PIC_C_SZ);
						vVDecWriteMC(u4VDecID, RW_MC_R1C,
							     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.
							      u4Pic0CSa) >> prWmvDecPrm->
							     i4MemBase >> 8);
						vVDecWriteMC(u4VDecID, RW_MC_R2C,
							     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.
							      u4Pic0CSa) >> prWmvDecPrm->
...
#if VMMU_SUPPORT
					{
						...
						u4Page_addr =
						    (UINT32) _pucVMMUTable[u4VDecID] +
						    (((prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) /
						      (4 * 1024)) * 4);
						printk
						    ("[WMV] i4VDEC_HAL_WMV_DecStart, 21, Page Addr = 0x%x\n",
						     u4Page_addr);
						vPage_Table(u4VDecID, u4Page_addr,
							    PHYSICAL((UINT32) prWmvDecPrm->
								     rWmvWorkBufSa.u4Pic1YSa),
							    PHYSICAL((UINT32) prWmvDecPrm->
								     rWmvWorkBufSa.u4Pic1YSa) +
							    PIC_Y_SZ);
						vVDecWriteMC(u4VDecID, RW_MC_R1Y,
							     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.
							      u4Pic1YSa) >> prWmvDecPrm->
							     i4MemBase >> 9);
						vVDecWriteMC(u4VDecID, RW_MC_R2Y,
							     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.
							      u4Pic1YSa) >> prWmvDecPrm->
						...
						u4Page_addr =
						    (UINT32) _pucVMMUTable[u4VDecID] +
						    (((prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) /
						      (4 * 1024)) * 4);
						printk
						    ("[WMV] i4VDEC_HAL_WMV_DecStart, 22, Page Addr = 0x%x\n",
						     u4Page_addr);
						vPage_Table(u4VDecID, u4Page_addr,
							    PHYSICAL((UINT32) prWmvDecPrm->
								     rWmvWorkBufSa.u4Pic1CSa),
							    PHYSICAL((UINT32) prWmvDecPrm->
								     rWmvWorkBufSa.u4Pic1CSa) +
							    PIC_C_SZ);
						vVDecWriteMC(u4VDecID, RW_MC_R1C,
							     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.
							      u4Pic1CSa) >> prWmvDecPrm->
							     i4MemBase >> 8);
						vVDecWriteMC(u4VDecID, RW_MC_R2C,
							     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.
							      u4Pic1CSa) >> prWmvDecPrm->
			...
			a200 = 0x00200032 + (pred_use_wdle << 6) + (pred_use_wdle << 7) + (pred_use_wdle << 20) + (0 << 11) + (((prWmvDecPrm->prPPS->i4Panning & 0xf) != 0) << 9) + ((prWmvDecPrm->i4CodecVersion == VDEC_WMV1 || prWmvDecPrm->i4CodecVersion == VDEC_WMV2) << 15) + (prWmvDecPrm->prICOMPS->i4ResetMvDram << 18) + (mv_sram_use << 31);	/* + */
			/* ((prWmvDecPrm->prPPS->i4X9MVMode == ALL_1MV_HALFPEL_BILINEAR) << 25) + */
			/* ((prWmvDecPrm->fgUVHpelBilinear == 1) << 26)+ */
			/* (prWmvDecPrm->i4CurrentTemporalField << 27) + */
			/* (prWmvDecPrm->fgVC1 << 28) + */
		...
		printk("[WMV] i4VDEC_HAL_WMV_DecStart, VMMUTable:[0x%x, 0x%x]\n",
		       ((UINT32) _pucVMMUTable[u4VDecID]),
		       PHYSICAL((UINT32) _pucVMMUTable[u4VDecID]));
	...
	if ((prWmvDecPrm->i4CodecVersion == VDEC_WMV1)
		...
		a206 = (prWmvDecPrm->i4CodecVersion <= VDEC_WMV2) +	/* wmv7/8 */
		    ...
		    (use_interpredictor << 21) + (prWmvDecPrm->prPPS->fgInterlaceV2 << 23);
	} else			/* >= VDEC_WMV3 */
	{
		a206 = 0 +	/* wmv7 */
		    (0 << 1) +	/* wmv8 */
		    ...
		    ((prWmvDecPrm->i4CodecVersion == VDEC_VC1
		      ...
		      BVOP) << 24) + (((prWmvDecPrm->prPPS->ucFrameCodingMode == INTERLACEFIELD)
				       || (prWmvDecPrm->prPPS->ucFrameCodingMode ==
					   INTERLACEFRAME)) << 25);
	...
	/* ----------------------------------------------------- */
	{
		...
		if ((prWmvDecPrm->prPPS->ucFrameCodingMode == INTERLACEFIELD) && (prWmvDecPrm->prPPS->i4CurrentField == 1) && (prWmvDecPrm->i4CodecVersion >= VDEC_WMV3))	/* bot */
		{
			...
			a194_1 =
			    (((u4AbsDramANc((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Mv12Sa) >> 2) >>
			      22) & 0x3F);
		} else {
			a194 = ((u4AbsDramANc((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Mv1Sa) >> 2) & 0x3FFFFF) +	/* dram_addr_base */
			    ((prWmvDecPrm->prSPS->u4NumMBX * 4) << 22);	/* Mv_1 */
			a194_1 =
			    (((u4AbsDramANc((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Mv1Sa) >> 2) >>
			      22) & 0x3F);
		}
		vVDecWriteVLD(u4VDecID, RW_VLD_MVSA, a194);	/* Robert: Temporary Marked! Interlace Field Needs it!!!! */
		vVDecWriteVLD(u4VDecID, RW_VLD_ADDREXTEND, (u4VDecReadVLD(u4VDecID, RW_VLD_ADDREXTEND) & 0xFFFFFFC0) | a194_1);	/* Robert: Temporary Marked! Interlace Field Needs it!!!! */
	...
	if (prWmvDecPrm->i4CodecVersion >= VDEC_WMV3) {
		...
		if (prWmvDecPrm->i4CodecVersion == VDEC_WMV2)
			wmv78_slice_num =
			    prWmvDecPrm->prSPS->u4NumMBY / prWmvDecPrm->prSPS->i4SliceCode;
		else
			wmv78_slice_num =
			    (prWmvDecPrm->prSPS->u4NumMBY / (prWmvDecPrm->prSPS->i4SliceCode - 22));
	...
	if (prWmvDecPrm->i4CodecVersion >= VDEC_WMV3) {
		...
		a209 = ((use_half * prWmvDecPrm->prPPS->i4MaxZone1ScaledFarBackMVX) & 0x7f) +
		    ...
		    (((use_half *
		       prWmvDecPrm->prPPS->i4Zone1OffsetScaledFarBackMVX) & 0x7f) << 16) +
	...
	height =
	    (prWmvDecPrm->prPPS->ucFrameCodingMode ==
	     INTERLACEFIELD) ? prWmvDecPrm->prSPS->u4PicHeightSrc /
	    2 : prWmvDecPrm->prSPS->u4PicHeightDec /*Main Profile only */;
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
	...
	/* ------------------------------------------------- */
	a132 = ((prWmvDecPrm->prPPS->fgUse3QPDZQuantizer) << 0) + ((prWmvDecPrm->prPPS->fgHalfStep) << 8)	/* + */
	    /* ((_fgSliceVC1) << 16) */
	...
	if ((prWmvDecPrm->i4CodecVersion == VDEC_WMV1)
	    || (prWmvDecPrm->i4CodecVersion == VDEC_WMV2)) {
		high_rate_switch = (prWmvDecPrm->prPPS->i4PicQtIdx <= MAXHIGHRATEQP)
		    && (prWmvDecPrm->i4CodecVersion == VDEC_WMV3);
		ttfrm =
		    (prWmvDecPrm->prPPS->fgMBXformSwitching ==
		     1) ? 0 : (prWmvDecPrm->prPPS->i4FrameXformMode ==
			       4) ? 3 : (prWmvDecPrm->prPPS->i4FrameXformMode & 0x3);

		a133 = (prWmvDecPrm->prPPS->i4CBPTable << 0) + ((prWmvDecPrm->prPPS->fgDCPredIMBInPFrame) << 3) + ((prWmvDecPrm->prPPS->fgMBXformSwitching == 1) << 4) +	/* TTMBF */
		    ((ttfrm) << 5) + ((prWmvDecPrm->prPPS->fgMvResolution) << 7) + ((prWmvDecPrm->prPPS->fgIntraDCTDCTable) << 8) + ((prWmvDecPrm->prPPS->fgDCTTableMB) << 9) + ((high_rate_switch) << 10) + ((prWmvDecPrm->prSPS->fgRotatedIdct == 0) << 11) + ((prWmvDecPrm->prPPS->u4DCTACIntraTableIndx) << 12) + ((prWmvDecPrm->prPPS->u4DCTACInterTableIndx) << 14) +	/* ming added */
		...
		ttfrm = (prWmvDecPrm->prPPS->fgMBXformSwitching == 1) ? 0 :
		    (prWmvDecPrm->prPPS->i4FrameXformMode ==
		     4) ? 3 : (prWmvDecPrm->prPPS->i4FrameXformMode & 0x3);

		a133 = (prWmvDecPrm->prPPS->i4CBPTable << 0) + ((prWmvDecPrm->prPPS->fgMBXformSwitching == 0) << 4) +	/* TTMBF */
		    ((ttfrm) << 5) + ((prWmvDecPrm->prPPS->fgIntraDCTDCTable) << 8) + ((prWmvDecPrm->prPPS->fgDCTTableMB) << 9) + ((high_rate_switch) << 10) + ((prWmvDecPrm->prSPS->fgRotatedIdct == 0) << 11) + ((prWmvDecPrm->prPPS->u4DCTACIntraTableIndx) << 12) + ((prWmvDecPrm->prPPS->u4DCTACInterTableIndx) << 14) +	/* ming added */
		    ((prWmvDecPrm->prPPS->i4DCStepSize) << 16) +
		    ((prWmvDecPrm->prPPS->i4StepSize) << 24);
	}
	...
	/* ------------------------------------------------- */
	b_fraction_chk =
	    ((prWmvDecPrm->prPPS->i4BNumerator *
	      prWmvDecPrm->prPPS->i4BFrameReciprocal) >> 7) ? 1 : 0;

	if ((prWmvDecPrm->i4CodecVersion == VDEC_WMV1)
	    || (prWmvDecPrm->i4CodecVersion == VDEC_WMV2)) {
		a136 = ((prWmvDecPrm->prEPS->i4MVRangeIndex) << 0) + ((prWmvDecPrm->prPPS->i4X9MVMode) << 8) + ((b_fraction_chk) << 16) + (0 << 17) + (0 << 18) + (0 << 19) + (0 << 24);	/* ((lookup_mv_tbl_number)     << 24); */
	} else {
		a136 = ((prWmvDecPrm->prEPS->i4MVRangeIndex) << 0) +
		    ((prWmvDecPrm->prPPS->i4X9MVMode) << 8) +
		    ((b_fraction_chk) << 16) +
		    ((prWmvDecPrm->prPPS->fgTwoRefPictures
		      && (prWmvDecPrm->prPPS->ucFrameCodingMode ==
			  INTERLACEFIELD)) << 17) + (prWmvDecPrm->prEPS->i4ExtendedDMVX << 18) +
	...
	/* ------------------------------------------------- */
	a142 = ((prWmvDecPrm->prPPS->i4X9MVMode == MIXED_MV)
		&& ((prWmvDecPrm->prPPS->ucPicType == PVOP)
		    || (prWmvDecPrm->prPPS->ucPicType == BVOP))) + (((prWmvDecPrm->i4CodecVersion <=
								      VDEC_WMV2)
								     && (prWmvDecPrm->prPPS->
									 i4MvTable)) << 1) +
	    (((prWmvDecPrm->prPPS->i4Overlap & 4) !=
	      0) << 8) + (prWmvDecPrm->prPPS->i4MBModeTable << 16) +
	    (prWmvDecPrm->prPPS->i42MVBPTable << 24) + (prWmvDecPrm->prPPS->i44MVBPTable << 28);
	...
	vVDecWriteVLD(u4VDecID, RW_VLD_VDOUFM,
		      VLD_VDOUFM + VLD_ENSTCNT + VLD_AATO + VLD_ERR_MONITOR);

#if (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8520)
	if ((prDecPrm->rDownScalerPrm.fgDSCLEn)
	    || (u4VDecReadMC(u4VDecID, RW_MC_WRAPPER_SWITCH) == 0)) {
		/* turn off error concealment because VDSCL and PP Wrapper  can not work with it */
		vVDecWriteVLD(u4VDecID, RW_VLD_PROC,
			      u4VDecReadVLD(u4VDecID, RW_VLD_PROC) | VLD_PERRCON);
	}
#endif
	...
	if (prWmvDecPrm->fgWmvMode) {
		...
		if (prWmvDecPrm->prPPS->ucFrameCodingMode != INTERLACEFIELD) {
			...
			if (prWmvDecPrm->prPPS->i4CurrentField == 0)
				...
				aAvcMAddr = prWmvDecPrm->rWmvWorkBufSa.u4MvNewSa
				    +
				    (prWmvDecPrm->prSPS->u4NumMBX * prWmvDecPrm->prSPS->u4NumMBY *
				     16);
			}
		}
		...
		if (prWmvDecPrm->prPPS->fgFieldMode == TRUE
		    && (prWmvDecPrm->i4CodecVersion == VDEC_VC1)) {
			if (prWmvDecPrm->prPPS->i4CurrentField == 0)	/* top */
			{
				aAvcMv134 =
				    ((prWmvDecPrm->prICOMPS->ucFrameTypeLastTop != PVOP) ? 1 : 0);
			} else {
				aAvcMv134 =
				    ((prWmvDecPrm->prICOMPS->ucFrameTypeLastBot != PVOP) ? 1 : 0);
			}
		} else {
		...
		i4Ratio0 =
		    prWmvDecPrm->prPPS->i4BNumerator * prWmvDecPrm->prPPS->i4BFrameReciprocal;
		...
		/* a167, a168, a169 */
		vVDecWriteVLD(u4VDecID, RW_VLD_WMV_BP0_NEW_ADDR,
			      u4AbsDramANc(prWmvDecPrm->rWmvWorkBufSa.u4Bp0NewSa));
		vVDecWriteVLD(u4VDecID, RW_VLD_WMV_BP1_NEW_ADDR,
			      u4AbsDramANc(prWmvDecPrm->rWmvWorkBufSa.u4Bp1NewSa));
		vVDecWriteVLD(u4VDecID, RW_VLD_WMV_BP2_NEW_ADDR,
			      u4AbsDramANc(prWmvDecPrm->rWmvWorkBufSa.u4Bp2NewSa));
		...
		if (prWmvDecPrm->prPPS->i4DCStepSize > 0) {
			i4Predictor =
			    (1024 +
			     (prWmvDecPrm->prPPS->i4DCStepSize >> 1)) /
	...
	vVDecWriteMC(u4VDecID, RW_MC_NBM_CTRL,
		     ((u4VDecReadMC(u4VDecID, RW_MC_NBM_CTRL) & 0xFFFFFFF8) | prDecPrm->
		      ucAddrSwapMode));
		...
		if (prWmvDecPrm->fgWmvMode)	/* For WMV7&8 new mode. */
		{
			if (prWmvDecPrm->i4CodecVersion < VDEC_WMV3) {
				...
				/* if(prWmvDecPrm->prPPS->i4MvTable) */
				{
					AVCMV_reg_180 |=
					    (prWmvDecPrm->prSPS->
					     fgFrmHybridMVOn) & ((!prWmvDecPrm->prPPS->
								  fgMvResolution) ? 1 : 0);
				}
				...
				/* if(u4NumMBY > prWmvDecPrm->prSPS->i4SliceCode) */
				{
					if (prWmvDecPrm->i4CodecVersion == VDEC_WMV2)
						u4NumMBYSlice =
						    prWmvDecPrm->prSPS->u4NumMBY /
						    prWmvDecPrm->prSPS->i4SliceCode;
					else
						u4NumMBYSlice =
						    (prWmvDecPrm->prSPS->u4NumMBY /
						     (prWmvDecPrm->prSPS->i4SliceCode - 22));
					if (u4NumMBYSlice != 0) {
						for (u4index = 1; u4index < u4NumMBY; u4index++) {
							if (u4index < 31)
								VLD_reg_157 +=
								    ((u4index % u4NumMBYSlice) ==
								     0) << (u4index + 1);
							else if (u4index < 63)
								VLD_reg_158 +=
								    ((u4index % u4NumMBYSlice) ==
				...
				if (prWmvDecPrm->prPPS->ucPicType == PVOP)
					DCAC_reg_0 |=
					    (prWmvDecPrm->prPPS->fgDCPredIMBInPFrame << 2);
...
#if WMV_LOG_TMP
	if ((_u4FileCnt[u4VDecID] % 10) == 0) {
		printk("frame %d\n", _u4FileCnt[u4VDecID]);
	}

	printk("input window: 0x%08x\n",
	       /* u4VDecVLDGetBitS(0, 0, 0), */
	       u4VDecReadVLD(u4VDecID, 0xf0));
...
#if 1
	mb();
	vVDecWriteVLD(u4VDecID, RW_VLD_WMV_LOAD_SUM, 0x00000001);	/* FW sync barrel shifter with HW */
	mb();
	vVDecWriteVLD(u4VDecID, RW_VLD_WMV_LOAD_SUM, 0x00000000);
	mb();
#endif
...
INT32 i4VDEC_HAL_WMV_VPStart(UINT32 u4VDecID, VDEC_INFO_DEC_PRM_T * prDecPrm)
{
...
#if VMMU_SUPPORT
	{
		...
		vPage_Table(u4VDecID, u4Page_addr,
			    ...
			    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) + PIC_Y_SZ);
		vVDecWriteMC(u4VDecID, RW_MC_R1Y,
			     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) >> 9);
		vVDecWriteMC(u4VDecID, RW_MC_DIGY,
			     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0YSa) >> 9);
		...
		vPage_Table(u4VDecID, u4Page_addr,
			    ...
			    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) + PIC_C_SZ);
		vVDecWriteMC(u4VDecID, RW_MC_R1C,
			     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) >> 8);
		vVDecWriteMC(u4VDecID, RW_MC_DIGC,
			     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic0CSa) >> 8);
		...
		vPage_Table(u4VDecID, u4Page_addr,
			    ...
			    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) + PIC_Y_SZ);
		vVDecWriteMC(u4VDecID, RW_MC_R2Y,
			     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic1YSa) >> 9);
		...
		vPage_Table(u4VDecID, u4Page_addr,
			    ...
			    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) + PIC_C_SZ);
		vVDecWriteMC(u4VDecID, RW_MC_R2C,
			     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic1CSa) >> 8);
		...
		vPage_Table(u4VDecID, u4Page_addr,
			    ...
			    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) + PIC_Y_SZ);
		vVDecWriteMC(u4VDecID, RW_MC_BY,
			     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) >> 8);
		vVDecWriteMC(u4VDecID, RW_MC_BY1,
			     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2YSa) >> 9);
		...
		vPage_Table(u4VDecID, u4Page_addr,
			    ...
			    PHYSICAL((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) + PIC_C_SZ);
		vVDecWriteMC(u4VDecID, RW_MC_BC,
			     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) >> 7);
		vVDecWriteMC(u4VDecID, RW_MC_BC1,
			     ((UINT32) prWmvDecPrm->rWmvWorkBufSa.u4Pic2CSa) >> 8);
	}
#else
...
/* vVDecWriteTopVLD(u4VDecID,RW_TOPVLD_WMV_PICSIZE_MB,(((prDecPrm->u4PicW+ 15)>>4) -1) | ((((prDecPrm->u4PicH + 15)>>4) - 1)<<16)); */
	...
	vVDecWriteDV(u4VDecID, VDEC_DV_INT_CFG,
		     u4VDecReadDV(u4VDecID, VDEC_DV_INT_CFG) & (~(1 << 12)));
#else
	vVDecWriteVLD(u4VDecID, RW_VLD_PICSZ,
		      ((prDecPrm->u4PicH + 15) << 16) + (prDecPrm->u4PicW >> 4));
...
void vVDEC_HAL_WMV_VDec_DumpReg(UINT32 u4VDecID, BOOL fgBefore)
{
	...
	printk("RW_VLD_WMV_MODE = 0x%08X\n", u4VDecReadVLD(u4VDecID, RW_VLD_WMV_MODE));
