...
#endif
{
	...
	u4Cnt = 50000;
#ifdef VDEC_SIM_DUMP
	printk("RISCRead if(`VDEC_PROCESS_FLAG == 1) wait(`VDEC_BITS_PROC_NOP == 1);\n");
#endif
	if (u4VDecReadVLD(u4VDecID, RO_VLD_SRAMCTRL) & PROCESS_FLAG) {
		while ((!(u4VDecReadVLD(u4VDecID, RO_VLD_SRAMCTRL) & AA_FIT_TARGET_SCLK))
		       && (u4Cnt--));
	}
#else
...
UINT32 u4VDecReadVldRPtrEx(UINT32 u4BSID, UINT32 u4VDecID, UINT32 *pu4Bits, UINT32 u4VFIFOSa,
...
#ifdef VDEC_SIM_DUMP
	printk
	    ("<vdec> ReadVldRPtr, dRptr:0x%08X, sra:0x%08X, swa:0x%08X, scnt:0x%08X, sum:0x%08X, u4BsBufLen=%u\n",
	...
	*pu4Bits &= 0x7;
	...
	if (u4ByteAddr < u4VFIFOSa) {
		u4ByteAddr = u4ByteAddr
		    + ((u4VDecReadVLD(u4VDecID, RW_VLD_VEND + (u4BSID << 10)) << 6) -
		       ((UINT32) u4VFIFOSa))
		    - u4VFIFOSa;
	} else {
...
#ifdef VDEC_SIM_DUMP
	printk("RISCRead wait(`VDEC_INI_FETCH_RDY == 1);\n");
#endif
...
{
	if (fgEnable) {
		vWriteVDSCL(u4VDecID, RW_VDSCL_ACTIVATE,
			    u4ReadVDSCL(u4VDecID, RW_VDSCL_ACTIVATE) | RW_VDSCL_EN);
	} else {
		vWriteVDSCL(u4VDecID, RW_VDSCL_ACTIVATE,
			    u4ReadVDSCL(u4VDecID, RW_VDSCL_ACTIVATE) & (~RW_VDSCL_EN));
	}
...
void vVDSCLSetConvert(UINT32 u4VDecID, BOOL fgConvert)
{
	if (fgConvert) {
		vWriteVDSCL(u4VDecID, RW_VDSCL_ACTIVATE,
			    u4ReadVDSCL(u4VDecID, RW_VDSCL_ACTIVATE) | RW_VDSCL_CONVERT);
	} else {
		vWriteVDSCL(u4VDecID, RW_VDSCL_ACTIVATE,
			    u4ReadVDSCL(u4VDecID, RW_VDSCL_ACTIVATE) & (~RW_VDSCL_CONVERT));
	}
...
void vVDSCLSetPicStruct(UINT32 u4VDecID, EPicStruct ePicStruct)
{
	...
	vWriteVDSCL(u4VDecID, RW_VDSCL_TYPE,
		    (u4ReadVDSCL(u4VDecID, RW_VDSCL_TYPE) & (~RW_VDSCL_PIC_STRU)) | ((UINT32)
										     ePicStruct));
}
...
void vVDSCLSetRasterScanOut(UINT32 u4VDecID, BOOL fgRasterScan)
{
	if (fgRasterScan) {
		vWriteVDSCL(u4VDecID, RW_VDSCL_TYPE,
			    u4ReadVDSCL(u4VDecID, RW_VDSCL_TYPE) | RW_VDSCL_RASTER_SCAN);
	} else {
		vWriteVDSCL(u4VDecID, RW_VDSCL_TYPE,
			    u4ReadVDSCL(u4VDecID, RW_VDSCL_TYPE) & (~RW_VDSCL_RASTER_SCAN));
	}
...
#endif
	vWriteVDSCL(u4VDecID, RW_VDSCL_MEM_IF,
		    (u4ReadVDSCL(u4VDecID, RW_VDSCL_MEM_IF) & (~RW_VDSCL_ADDR_SWAP_MODE2)) |
		    u4Mode);
}


void vVDSCLSetBoundPad(UINT32 u4VDecID, EPadMode ePadMode, UCHAR ucYColor, UCHAR ucCbColor,
		       UCHAR ucCrColor)
{
	switch (ePadMode) {
	case PAD_MODE_NO_PAD:
		vWriteVDSCL(u4VDecID, RW_VDSCL_BOUND,
			    u4ReadVDSCL(u4VDecID, RW_VDSCL_BOUND) | RW_VDSCL_BOUND_NO_PAD);
		break;
	case PAD_MODE_EXTEND:
		vWriteVDSCL(u4VDecID, RW_VDSCL_BOUND,
			    u4ReadVDSCL(u4VDecID, RW_VDSCL_BOUND) & (~RW_VDSCL_BOUND_NO_PAD));
		vWriteVDSCL(u4VDecID, RW_VDSCL_BOUND,
			    u4ReadVDSCL(u4VDecID, RW_VDSCL_BOUND) & (~RW_VDSCL_BOUND_PAD_EN));
		break;
	case PAD_MODE_COLOR:
		vWriteVDSCL(u4VDecID, RW_VDSCL_BOUND,
			    u4ReadVDSCL(u4VDecID, RW_VDSCL_BOUND) & (~RW_VDSCL_BOUND_NO_PAD));
			    ...
			    u4ReadVDSCL(u4VDecID, RW_VDSCL_BOUND) | RW_VDSCL_BOUND_PAD_EN);
		{
			...
			vWriteVDSCL(u4VDecID, RW_VDSCL_BOUND,
				    (u4ReadVDSCL(u4VDecID, RW_VDSCL_BOUND) & (~RW_VDSCL_BOUND_PAD_Y)
				     & (~RW_VDSCL_BOUND_PAD_CB) & (~RW_VDSCL_BOUND_PAD_CR)) | (u4Y
											       <<
											       16) |
				    (u4Cb << 8) | u4Cr);
		}
...
void vVDSCLSetSrcPicSize(UINT32 u4VDecID, UINT32 u4SrcWidth, UINT32 u4SrcHeight)
{
	...
	vWriteVDSCL(u4VDecID, RW_VDSCL_SRC_SIZE,
		    (u4ReadVDSCL(u4VDecID, RW_VDSCL_SRC_SIZE) & (~RW_VDSCL_SRC_WIDTH) &
		     (~RW_VDSCL_SRC_HEIGHT)) | (u4SrcWidth << 16) | u4SrcHeight);
}
...
void vVDSCLSetTrgPicSize(UINT32 u4VDecID, UINT32 u4TrgWidth, UINT32 u4TrgHeight)
{
	...
	vWriteVDSCL(u4VDecID, RW_VDSCL_TG_SIZE,
		    (u4ReadVDSCL(u4VDecID, RW_VDSCL_TG_SIZE) & (~RW_VDSCL_TG_WIDTH) &
		     (~RW_VDSCL_TG_HEIGHT)) | (u4TrgWidth << 16) | u4TrgHeight);
}
...
void vVDSCLSetTrgBufWidth(UINT32 u4VDecID, UINT32 u4TrgBufWidth)
{
	...
	vWriteVDSCL(u4VDecID, RW_VDSCL_TG_BUF_LEN,
		    (u4ReadVDSCL(u4VDecID, RW_VDSCL_TG_BUF_LEN) & (~RW_VDSCL_TRG_BUF_LEN)) |
		    (u4TrgBufWidth >> 4));
}
...
void vVDSCLSetTrgYBufBaseAddr(UINT32 u4VDecID, UINT32 u4YBufBaseAddr)
{
	...
	vWriteVDSCL(u4VDecID, RW_VDSCL_TG_Y_OW_BASE,
		    (u4ReadVDSCL(u4VDecID, RW_VDSCL_TG_Y_OW_BASE) & (~RW_VDSCL_TG_Y_OW_ADDR_BASE)) |
		    (u4YBufBaseAddr >> 4));
}
...
void vVDSCLSetTrgCBufBaseAddr(UINT32 u4VDecID, UINT32 u4CBufBaseAddr)
{
	...
	vWriteVDSCL(u4VDecID, RW_VDSCL_TG_C_OW_BASE,
		    (u4ReadVDSCL(u4VDecID, RW_VDSCL_TG_C_OW_BASE) & (~RW_VDSCL_TG_C_OW_ADDR_BASE)) |
		    (u4CBufBaseAddr >> 4));
}
...
{
	vWriteVDSCL(u4VDecID, RW_VDSCL_H_SCL_Y,
		    (u4ReadVDSCL(u4VDecID, RW_VDSCL_H_SCL_Y) & (~RW_VDSCL_H_SCL_Y_FACTOR)) |
		    u4HScaleFactorY);
	vWriteVDSCL(u4VDecID, RW_VDSCL_H_SCL_C,
		    (u4ReadVDSCL(u4VDecID, RW_VDSCL_H_SCL_C) & (~RW_VDSCL_H_SCL_C_FACTOR)) |
		    u4HScaleFactorC);
}
...
{
	vWriteVDSCL(u4VDecID, RW_VDSCL_H_SCL_Y,
		    (u4ReadVDSCL(u4VDecID, RW_VDSCL_H_SCL_Y) & (~RW_VDSCL_H_SCL_Y_OFFSET)) |
		    (u4HScaleOffsetY << 16));
	vWriteVDSCL(u4VDecID, RW_VDSCL_H_SCL_C,
		    (u4ReadVDSCL(u4VDecID, RW_VDSCL_H_SCL_C) & (~RW_VDSCL_H_SCL_C_OFFSET)) |
		    (u4HScaleOffsetC << 16));
}
...
{
	vWriteVDSCL(u4VDecID, RW_VDSCL_V_SCL_Y,
		    (u4ReadVDSCL(u4VDecID, RW_VDSCL_V_SCL_Y) & (~RW_VDSCL_V_SCL_Y_FACTOR)) |
		    u4VScaleFactorY);
	vWriteVDSCL(u4VDecID, RW_VDSCL_V_SCL_C,
		    (u4ReadVDSCL(u4VDecID, RW_VDSCL_V_SCL_C) & (~RW_VDSCL_V_SCL_C_FACTOR)) |
		    u4VScaleFactorC);
}
...
{
	vWriteVDSCL(u4VDecID, RW_VDSCL_V_SCL_Y,
		    (u4ReadVDSCL(u4VDecID, RW_VDSCL_V_SCL_Y) & (~RW_VDSCL_V_SCL_Y_OFFSET)) |
		    (u4VScaleOffsetY << 16));
	vWriteVDSCL(u4VDecID, RW_VDSCL_V_SCL_C,
		    (u4ReadVDSCL(u4VDecID, RW_VDSCL_V_SCL_C) & (~RW_VDSCL_V_SCL_C_OFFSET)) |
		    (u4VScaleOffsetC << 16));
}
...
#if (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8520)
void vVDSCLSetScalerVParm(UINT32 u4VDecID, UINT32 u4SrcHeight, UINT32 u4TrgHeight, BOOL fgIs422,
			  BOOL fgIsMbaff, UINT32 u4OffY, UINT32 u4OffC)
{
	...
	if (fgIs422) {
		...
		if (0 == fgIsMbaff) {
			u4VScalerFactorC =
			    ((4096 * u4TrgHeight) + (u4SrcHeight >> 1)) / u4SrcHeight;
		} else {
			/* If has remainder, plus 1 */
			  ...
			  VDEC_INFO_VDSCL_PRM_T *prDownScalerPrm)
{
	...
	if (fgIs422) {
		...
		if (0 == fgIsMbaff) {
			u4VScalerFactorC =
			    ((0x10000 * u4TrgHeight) + (u4SrcHeight >> 1)) / u4SrcHeight;
		} else {
...
void vVDSCLSetSrcOffset(UINT32 u4VDecID, UINT32 u4SrcYOffH, UINT32 u4SrcYOffV, UINT32 u4SrcCOffH,
			UINT32 u4SrcCOffV)
{
	vWriteVDSCL(u4VDecID, RW_VDSCL_SRC_OFFSET,
		    (((u4SrcCOffH & 0xF) << 12) | ((u4SrcCOffV & 0xF) << 8) |
		     ((u4SrcYOffH & 0xF) << 4) | (u4SrcYOffV & 0xF)));
}

void vVDSCLSetTmpBufBaseAddr(UINT32 u4VDecID, UINT32 u4TmpBufBaseAddr)
{
	vWriteVDSCL(u4VDecID, RW_VDSCL_TMP_OW_BASE,
		    (u4ReadVDSCL(u4VDecID, RW_VDSCL_TMP_OW_BASE) & (~RW_VDSCL_TMP_OW_ADDR_BASE)) |
		    (u4TmpBufBaseAddr >> 4));
}
...
void vVDECSetDownScalerPrm(UINT32 u4VDecID, VDEC_INFO_VDSCL_PRM_T *prDownScalerPrm)
{
	...
	if (prDownScalerPrm->fgDSCLEn) {
		...
		vVDSCLSetTrgYBufBaseAddr(u4VDecID,
					 u4AbsDramANc((UINT32) prDownScalerPrm->u4TrgYAddr));
		vVDSCLSetTrgCBufBaseAddr(u4VDecID,
					 u4AbsDramANc((UINT32) prDownScalerPrm->u4TrgCAddr));
		vVDSCLSetTmpBufBaseAddr(u4VDecID,
					u4AbsDramANc((UINT32) prDownScalerPrm->u4WorkAddr));
#if (CONFIG_CHIP_VER_CURR == CONFIG_CHIP_VER_MT8520)
...
#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8550)
		vVDSCLSetLumaKey(u4VDecID, prDownScalerPrm->fgLumaKeyEn,
				 prDownScalerPrm->u2LumaKeyValue);
#endif
	}
#endif
	return;
...
{
	vVDecWriteDV(u4VDecID, RW_VEC_LBOX_UP_CFG,
		     (prLBDPrm->u4UpBoundEnd << 16) | (prLBDPrm->u4UpBoundStart));
	vVDecWriteDV(u4VDecID, RW_VEC_LBOX_LOW_CFG,
		     (prLBDPrm->u4LowBoundStart << 16) | (prLBDPrm->u4LowBoundEnd));
	vVDecWriteDV(u4VDecID, RW_VEC_LBOX_SIDE_CFG,
		     (prLBDPrm->u4LeftBound << 16) | (prLBDPrm->u4RightBound));
	vVDecWriteDV(u4VDecID, RW_VEC_LBOX_THD_OFFSET,
		     (prLBDPrm->u4YOffset << 24) | (prLBDPrm->u4COffset << 16)
		     | (prLBDPrm->u4YValueThd << 8) | (prLBDPrm->u4CValueThd << 0));
	vVDecWriteDV(u4VDecID, RW_VEC_LBOX_CNT_THD,
		     (prLBDPrm->u4YCntThd << 16) | (prLBDPrm->u4CCntThd));
}
...
void vVDecSimWDump(const char *szFunction, UINT32 u4VDecID, const char *szOffset, UINT32 u4Addr,
		   UINT32 u4Val, UINT32 u4BaseReg)
{
	if (szFunction) {
		if (DUMP_FOR_SIM) {
...
#endif
			{
				...
				case DV_REG_OFFSET0:
					_u4RegisterLogLen[u4VDecID] +=
					    sprintf(_pucRegisterLog[u4VDecID] +
						    _u4RegisterLogLen[u4VDecID],
						    "RISCWrite_VDEC_TOP( %d, 32'h%x);\n",
						    u4Addr >> 2, u4Val);
					break;
				case VLD_REG_OFFSET0:
					_u4RegisterLogLen[u4VDecID] +=
				...
				default:
					_u4RegisterLogLen[u4VDecID] +=
					    sprintf(_pucRegisterLog[u4VDecID] +
						    _u4RegisterLogLen[u4VDecID],
						    "RISCWrite_Others( %d, 32'h%x); /* base 0x%x */\n",
						    u4Addr >> 2, u4Val, u4BaseReg);
					break;
				}
			}
...
void vVDecSimRDump(const char *szFunction, UINT32 u4VDecID, const char *szOffset, UINT32 u4Addr,
		   UINT32 u4Val, UINT32 u4BaseReg)
{
	if (szFunction) {
		if (DUMP_FOR_SIM) {
...
#endif
			{
				...
				default:
					printk
					    ("RISCRead_Others( %d);  /* return 0x%x */ /* base 0x%x */\n",
					     u4Addr >> 2, u4Val, u4BaseReg);
					break;
				}
			}
				...
				case DV_REG_OFFSET0:
					_u4RegisterLogLen[u4VDecID] +=
					    sprintf(_pucRegisterLog[u4VDecID] +
						    _u4RegisterLogLen[u4VDecID],
						    "RISCRead_VDEC_TOP( %d); /* return 0x%x */\n",
						    u4Addr >> 2, u4Val);
					break;
				case VLD_REG_OFFSET0:
					_u4RegisterLogLen[u4VDecID] +=
				...
				case VLD_TOP_REG_OFFSET0:
					_u4RegisterLogLen[u4VDecID] +=
					    sprintf(_pucRegisterLog[u4VDecID] +
						    _u4RegisterLogLen[u4VDecID],
						    "RISCRead_VLD_TOP( %d); /* return 0x%x */\n",
						    u4Addr >> 2, u4Val);
					break;
				case MC_REG_OFFSET0:
					_u4RegisterLogLen[u4VDecID] +=
