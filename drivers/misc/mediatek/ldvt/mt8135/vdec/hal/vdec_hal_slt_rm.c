...
void vRM_SLT_InvAddressSwap(UINT32 u4InstID,
			    UINT32 u4SrcType,
			    VDEC_INFO_RM_PICINFO_T *prParsingPic,
			    UINT32 u4InvsAddrBufY, UINT32 u4InvsAddrBufC)
{
	UINT32 i;
	UINT32 u4DataLength;
...
BOOL fgRM_SLT_CheckYCGolden(UINT32 u4InstID,
			    VDEC_INFO_RM_PICINFO_T *prParsingPic,
			    UINT32 u4InvsAddrBufY, UINT32 u4InvsAddrBufC, UINT32 u4YCGoldenBuf)
{
	...
	for (y = 0; y < u4Height / 2; y++) {
		for (x = 0; x < u4Width / 2; x += 4) {
			...
			vRM_SLT_mb_to_line_c(&u4ErrVal, &u4ErrVal2, u4OutBufC, u4DecWidthAlign, x,
					     y);
			u4GolVal =
			    ((u4VVal << 16) & 0xFF000000) | ((u4UVal << 8) & 0x00FF0000) |
...
/* Add Code for RM SLT */
void vDEC_SLT_RM(const char *pFile1Name, const char *pFile2Name, const char *pFile3Name,
		 const char *pFile4Name)
{
	UINT32 i;
	UINT32 u4InstID = 0;
...
/* Memory Alloc for VFIFO, HW Work Buffer, Y/C Buffer */
	...
	if ((pbRMVFIFOBuf == 0) ||
	    ...
	    (pbRMYCGoldenBuf[2] == 0) ||
	    (prPic == 0) ||
	    (prParsingPic == 0) || (pbRMInvAddrSwapBufY == 0) || (pbRMInvAddrSwapBufC == 0)) {
		ASSERT(0);
	}
...
/* Load SLT Verification Source */
	...
	for (i = 0; i < 3; i++) {
		x_sprintf(strEmuFileName, "%s%s%07d%s", strSLTBaseFolder, "g", i, ".yuv");
		fFileHandle = fopen(strEmuFileName, "rb");
...
/* Parsing Data */
	while (u4ProcessFrameCnt < u4ClipTotalFrameCnt) {
		...
		if (prPic->fgRV9) {
			...
			prParsingPic->fgDeblockPassThru =
			...
			if ((prParsingPic->ePtype == RM_INTERPIC)
			    || (prParsingPic->ePtype == RM_TRUEBPIC)) {
				prParsingPic->fgUserPrevWidth =
				    u4VDEC_HAL_RM_ShiftGetBitStream(0, u4InstID, 1);

				if (!(prParsingPic->fgUserPrevWidth)) {
					vRM_SLT_GetPicSize(0, u4InstID, &prParsingPic->u4OrgWidth,
			...
			/* RV8 Syntax */
			prParsingPic->u4RvBistreamVersion =
			    u4VDEC_HAL_RM_ShiftGetBitStream(0, u4InstID, 3);
			prParsingPic->ePtype =
			    (EnumRMPicCodType) u4VDEC_HAL_RM_ShiftGetBitStream(0, u4InstID, 2);
			prParsingPic->fgECC = u4VDEC_HAL_RM_ShiftGetBitStream(0, u4InstID, 1);
			prParsingPic->u4PSQuant = u4VDEC_HAL_RM_ShiftGetBitStream(0, u4InstID, 5);
			...
			if (prPic->u4PctszSize > 0) {
				prParsingPic->u4PicSz =
				    u4VDEC_HAL_RM_ShiftGetBitStream(0, u4InstID,
								    prParsingPic->u4PctszSize);
			}
...
/* Check Golden Data */
		fgVdecRMSLTResult = fgRM_SLT_CheckYCGolden(u4InstID, prParsingPic,
							   ...
							   (UINT32)
							   pbRMYCGoldenBuf[u4ProcessFrameCnt]);
