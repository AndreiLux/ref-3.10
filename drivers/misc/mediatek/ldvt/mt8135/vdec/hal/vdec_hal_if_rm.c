...
#endif
	vVDecSetVLDVFIFO(0, u4VDecID, PHYSICAL(prRMVFifoPrm->u4VFifoSa),
...
void vRM_HAL_InitVDecHW(UINT32 u4VDecID, VDEC_INFO_RM_VFIFO_PRM_T *prRMVFifoPrm)
{
	...
	vVDecSetVLDVFIFO(0, u4VDecID,
			 PHYSICAL(prRMVFifoPrm->u4VFifoSa), PHYSICAL(prRMVFifoPrm->u4VFifoEa));
}



INT32 i4RM_HAL_InitBarrelShifter(UINT32 u4BSID,
				 UINT32 u4VDecID, VDEC_INFO_RM_BS_INIT_PRM_T *prMpegBSInitPrm)
{
	INT32 iRet = 0;
...
#endif
{
	...
	if (prParsingPic->fgEnableMCOutput) {
...
#if VMMU_SUPPORT
		{
			...
			u4Page_addr =
			    ...
			    ((((UINT32) prParsingPic->u4MCOutBufY) / (4 * 1024)) * 4);
			printk("[RM] vVDEC_HAL_RM_TriggerDecode, 1, Page Addr = 0x%x\n",
			       u4Page_addr);
			vPage_Table(u4VDecID, u4Page_addr,
				    PHYSICAL((UINT32) prParsingPic->u4MCOutBufY),
				    PHYSICAL((UINT32) prParsingPic->u4MCOutBufY) + PIC_Y_SZ);
...
#if VMMU_SUPPORT
		{
			...
			u4Page_addr =
			    (UINT32) _pucVMMUTable[u4VDecID] +
			    ((((UINT32) prParsingPic->u4MCOutBufC) / (4 * 1024)) * 4);
			printk("[RM] vVDEC_HAL_RM_TriggerDecode, 2, Page Addr = 0x%x\n",
			       u4Page_addr);
			vPage_Table(u4VDecID, u4Page_addr,
				    PHYSICAL((UINT32) prParsingPic->u4MCOutBufC),
				    PHYSICAL((UINT32) prParsingPic->u4MCOutBufC) + PIC_C_SZ);
...
#if VMMU_SUPPORT
		{
			...
			u4Page_addr =
			    (UINT32) _pucVMMUTable[u4VDecID] +
			    ((((UINT32) prParsingPic->u4OutBufY) / (4 * 1024)) * 4);
			printk("[RM] vVDEC_HAL_RM_TriggerDecode, 3, Page Addr = 0x%x\n",
			       u4Page_addr);
			vPage_Table(u4VDecID, u4Page_addr,
				    PHYSICAL((UINT32) prParsingPic->u4OutBufY),
				    PHYSICAL((UINT32) prParsingPic->u4OutBufY) + PIC_Y_SZ);
...
#if VMMU_SUPPORT
		{
			...
			u4Page_addr =
			    (UINT32) _pucVMMUTable[u4VDecID] +
			    ((((UINT32) prParsingPic->u4OutBufC) / (4 * 1024)) * 4);
			printk("[RM] vVDEC_HAL_RM_TriggerDecode, 4, Page Addr = 0x%x\n",
			       u4Page_addr);
			vPage_Table(u4VDecID, u4Page_addr,
				    PHYSICAL((UINT32) prParsingPic->u4OutBufC),
				    PHYSICAL((UINT32) prParsingPic->u4OutBufC) + PIC_C_SZ);
...
#if VMMU_SUPPORT
	{
		...
		vPage_Table(u4VDecID, u4Page_addr, PHYSICAL((UINT32) prParsingPic->u4OutBufY),
			    PHYSICAL((UINT32) prParsingPic->u4OutBufY) + PIC_Y_SZ);
		u4RegValue = (UINT32) (prParsingPic->u4OutBufY) >> 9;
	}
#else
...
#if VMMU_SUPPORT
	{
		...
		vPage_Table(u4VDecID, u4Page_addr, PHYSICAL((UINT32) prParsingPic->u4OutBufC),
			    PHYSICAL((UINT32) prParsingPic->u4OutBufC) + PIC_C_SZ);
		u4RegValue = (UINT32) (prParsingPic->u4OutBufC) >> 8;
	}
#else
	...
	u4RegValue = (prParsingPic->fgDeblockPassThru == 0 ?
		      ...
		      ((!prParsingPic->fgRV9 &&
			(prParsingPic->ePtype != RM_INTRAPIC
			 && prParsingPic->ePtype != RM_FORCED_INTRAPIC)) ? 2 : 3) : 0);
	...
	if (prParsingPic->ePtype == RM_INTERPIC || prParsingPic->ePtype == RM_TRUEBPIC) {
...
#if VMMU_SUPPORT
		{
			...
			u4Page_addr =
			    (UINT32) _pucVMMUTable[u4VDecID] +
			    ((((UINT32) prParsingPic->u4FwdBufY) / (4 * 1024)) * 4);
			printk("[RM] vVDEC_HAL_RM_TriggerDecode, 7, Page Addr = 0x%x\n",
			       u4Page_addr);
			vPage_Table(u4VDecID, u4Page_addr,
				    PHYSICAL((UINT32) prParsingPic->u4FwdBufY),
				    PHYSICAL((UINT32) prParsingPic->u4FwdBufY) + PIC_Y_SZ);
...
#if VMMU_SUPPORT
		{
			...
			u4Page_addr =
			    (UINT32) _pucVMMUTable[u4VDecID] +
			    ((((UINT32) prParsingPic->u4FwdBufC) / (4 * 1024)) * 4);
			printk("[RM] vVDEC_HAL_RM_TriggerDecode, 8, Page Addr = 0x%x\n",
			       u4Page_addr);
			vPage_Table(u4VDecID, u4Page_addr,
				    PHYSICAL((UINT32) prParsingPic->u4FwdBufC),
				    PHYSICAL((UINT32) prParsingPic->u4FwdBufC) + PIC_C_SZ);
	...
	if (prParsingPic->ePtype == RM_TRUEBPIC) {
...
#if VMMU_SUPPORT
		{
			...
			u4Page_addr =
			    (UINT32) _pucVMMUTable[u4VDecID] +
			    ((((UINT32) prParsingPic->u4BwdBufY) / (4 * 1024)) * 4);
			printk("[RM] vVDEC_HAL_RM_TriggerDecode, 9, Page Addr = 0x%x\n",
			       u4Page_addr);
			vPage_Table(u4VDecID, u4Page_addr,
				    PHYSICAL((UINT32) prParsingPic->u4BwdBufY),
				    PHYSICAL((UINT32) prParsingPic->u4BwdBufY) + PIC_Y_SZ);
...
#if VMMU_SUPPORT
		{
			...
			u4Page_addr =
			    (UINT32) _pucVMMUTable[u4VDecID] +
			    ((((UINT32) prParsingPic->u4BwdBufC) / (4 * 1024)) * 4);
			printk("[RM] vVDEC_HAL_RM_TriggerDecode, 10, Page Addr = 0x%x\n",
			       u4Page_addr);
			vPage_Table(u4VDecID, u4Page_addr,
				    PHYSICAL((UINT32) prParsingPic->u4BwdBufC),
				    PHYSICAL((UINT32) prParsingPic->u4BwdBufC) + PIC_C_SZ);
	...
	u4RegValue = (prParsingPic->u4Height << 16) | (prParsingPic->u4Width / 16) | 0xFF00;
...
#else
	u4RegValue =
	    u4CycleCnt | u4ErrConceal | u4FastParsing | 0x3 | (prParsingPic->fgRV9 ? 4 : 0);
	vVDecWriteRMVLD(u4VDecID, RW_RMVLD_CTRL, u4RegValue);	/* RV8 or RV9 */
#endif
	...
	u4RegValue =
	    (prParsingPic->u4Width / 16 -
	     1) << 24 | prParsingPic->u4OrgHeight << 12 | prParsingPic->u4OrgWidth;
	vVDecWriteRMVLD(u4VDecID, RW_RMVLD_PICSIZE1, u4RegValue);
#endif

#if (CONFIG_CHIP_VER_CURR >= CONFIG_CHIP_VER_MT8580)
	u4RegValue = (prParsingPic->u4Height / 16 - 1) << 16 | (prParsingPic->u4Width / 16 - 1);
	...
	u4RegValue = (UINT32) (((UINT8) (prParsingPic->fgRType) << 26) |
			       ...
			       prParsingPic->u4RvTr << 10 | prParsingPic->fgDeblockPassThru << 9 |
			       /* prParsingPic->u4Tr << 10 | */
			       /* prParsingPic->u4DFP << 9 | */
			       prParsingPic->u4OsvQuant << 7 | prParsingPic->u4PSQuant << 2 |
...
void vVDEC_HAL_RM_TriggerDecode(UINT32 u4VDecID, VDEC_INFO_RM_PICINFO_T *prParsingPic)
{
	...
	u4RegValue = prParsingPic->u4Rtype << 26 | prParsingPic->u4PicSz << 23 |
	    /* prParsingPic->u4Pctsz << 23 | */
	    prParsingPic->u4RvTr << 10 | prParsingPic->fgDeblockPassThru << 9 |
	    /* prParsingPic->u4Tr << 10 | */
	    /* prParsingPic->u4DFP << 9 | */
