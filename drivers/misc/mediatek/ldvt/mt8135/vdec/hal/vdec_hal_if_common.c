...
void MPV_PllInit(void)
{
	...
	u4DramClk = BSP_GetClock(CAL_SRC_DMPLL, &u1Band, &u2MS, &u2NS, &u2Counter);
...
*/
	ucDelay = 0;
	*((UINT32 *) 0x2000D03c) =
	    (*((UINT32 *) 0x2000D03c) & 0xC0FFC0FF) | ((ucDelay << 24) | (ucDelay << 8));
...
UINT32 u4VDEC_HAL_Read_ESA(UINT32 u4InstID, UINT32 u4Temp)
{
	...
	u4Temp1 = u4Temp + sprintf(_pucESALog[u4InstID] + u4Temp, "%d,%d,%d,%d,%d,%d,%d,%d,\n",
				   ...
				   _u4ESAValue[u4InstID][6], _u4ESAValue[u4InstID][7]);
	/* printk("u4Temp = %d\n", u4Temp1); */
	return u4Temp1;
}
...
BOOL fgBreakVDec(UINT32 u4InstID)
{
	...
	if (!(u4VDecReadDV(u4InstID, VDEC_DV_DEC_BREAK) & 0x01)) {
		while (u4Cnt--);
		vVDecWriteDV(u4InstID, VDEC_DV_DEC_BREAK,
			     u4VDecReadDV(u4InstID, VDEC_DV_DEC_BREAK) | VDEC_DV_DEC_BREAK_EN);
		u4Cnt = 0;
		while (!
		       ((u4VDecReadDV(u4InstID, VDEC_DV_DEC_BREAK_STATUS) & VDEC_BREAK_OK_0)
			&& (u4VDecReadDV(u4InstID, VDEC_DV_DEC_BREAK_STATUS) & VDEC_BREAK_OK_1))) {
			u4Cnt++;
			if (u4Cnt >= 0x5000) {
				printk("not break\n");
				break;
