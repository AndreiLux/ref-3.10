...
static unsigned long av_adler32_update(unsigned long adler, const unsigned char *buf, int ylen,
				       int xlen)
{
...
static int __init readfile_init(void)
{
	...
	printk("buf:%x\n", buf);
	fp = openFile
	    ("/mnt/udiska/VP8_bitstream/gp/webm_youtube/zilker park WebM format test/post_0.y.dat",
	     O_RDONLY, 0);
	if (fp != NULL) {
		memset(buf, 0, 1024);
...
static int remote_open(struct file *fd, const char *filename, int filename_sz, const char *mode,
		       int mode_sz)
{
	int num = 0;
	int cmd = 0;
...
static int remote_read(struct file *fd, int file_idx, unsigned char *buf, int len)
{
	...
	do_gettimeofday(&tv2);
	if (op_res != sizeof(int))
		printk("[READ] Read length send fail %d\n", op_res);
	tv2.tv_sec -= tv1.tv_sec;
	if (tv2.tv_usec < tv1.tv_usec) {
		if (tv2.tv_sec < 1)
			printk("TIME ERROR\n");
	...
	do_gettimeofday(&tv2);
	if (op_res != sizeof(int))
		printk("[READ] Get command response fail %d\n", op_res);
	tv2.tv_sec -= tv1.tv_sec;
	if (tv2.tv_usec < tv1.tv_usec) {
		if (tv2.tv_sec < 1)
			printk("TIME ERROR\n");
...
static int uvvp_vdec_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	int ret = 0;
	int i, j;
	/* int recv_buf[256]; */
...
#else
		MPV_thread =
		    kernel_thread(vVDecVerifyThread, param,
...
/* case UVVP_VDEC_TEST_VERSION_1: */
		...
		msleep(2);
		MPV_thread =
		    kernel_thread(vVDecVerifyThread, param1,
				  CLONE_FS | CLONE_FILES | CLONE_SIGHAND | SIGCHLD);
		if (IS_ERR(MPV_thread)) {
			printk("[%s]: failed to create MPV thread\n", __func__);
			return 0;
	...
	case UVVP_VDEC_TEST_VERSION_1:
		...
		printk("OFFSET_R1Y_ADD write 0x%x, read 0x%x, mask 0x%x, %s\n", reg_in, reg_out,
		       Y_MASK, (reg_in & Y_MASK) == reg_out ? "Pass" : "Fail");
		...
		printk("OFFSET_R1C_ADD write 0x%x, read 0x%x, mask 0x%x, %s\n", reg_in, reg_out,
		       C_MASK, (reg_in & C_MASK) == reg_out ? "Pass" : "Fail");
...
*/
	...
	case UVVP_VDEC_TEST_VERSION_3:
		...
		printk("Data len %d, elapsed time %d ms, avg speed %d bytes/s\n", read_len,
		       (tv2.tv_sec * 1000000 + tv2.tv_usec) / 1000,
		       read_len * 1000 * 1000 / ((tv2.tv_sec * 1000000 + tv2.tv_usec)));
