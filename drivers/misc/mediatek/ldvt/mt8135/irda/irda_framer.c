...
static ssize_t mt_irda_store_stat(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t count);
static int mt_irda_change_speed(struct mt_irda_framer *framer);
struct mt_irda_framer *framer_dev;
static int rx_invert;
...
static ssize_t mt_irda_show_stat(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	count = sprintf(buf, "********************mt65xx-irda-status******************\n\
...
*********************mt65xx-irda-status**********************\n\
\n", speed_str(framer->speed), inw(IR_DIV), framer->rx_invert == IR_RX_EN_INVERT ? "yes" : "no", framer->tx_invert == IR_TX_EN_INVERT ? "yes" : "no", framer->sir_mode == IR_DIV_16 ? "3/16" : "1.61us", framer->bof_type == BOF_TYPE_C0H ? "C0H" : "FFH", framer->sir_framing_set == SIR_FRAMING_IGN_STOP_BIT ? "ignore" : "check", framer->mir_576_tune == IR_TX_MIR576_NONE ? "none" : (framer->mir_576_tune == IR_TX_MIR576_RX_HALF ? "half" : "quter"));
	{
		unsigned int i;
...
static void parse_mir_576_tune(const char *op)
{
	...
	if (old != framer_dev->mir_576_tune
	    && (framer_dev->speed == 576000 || framer_dev->speed == 1152000))
		mt_irda_start_rx(framer_dev);
	return;
}
...
/* const char *option[]={"speed=","rx_invert=","tx_invert=","modulation_type=","bof_type=","sir_framing=","mir_576_tune=",NULL}; */
	...
	const char *op = NULL;
	spin_lock_irqsave(&framer->lock, flag);
	if (framer->direction == IO_XMIT || (inw(IR_FIFO_STA) & 0x5) != 0x5
	    || (inw(IR_FIFO_STA) & 0x10)) {
		spin_unlock_irqrestore(&framer->lock, flag);
		return -EBUSY;
	} else {
		int i = 0;
...
static int mt_irda_change_speed(struct mt_irda_framer *framer)	/* this one */
{
	...
	if (speed <= 0
	    || (speed > 115200 && speed != 1152000 && speed != 576000 && speed != 4000000))
		return -1;
	if (framer->new_speed && (framer->speed != framer->new_speed)) {
		dbg("%s(),change speed %d-->%d\n", __func__, framer->speed, framer->new_speed);
		...
		dbg("%s(),speed=%d IR_DIV=%d\n", __func__, framer->speed, inw(IR_DIV));
		return 1;
	}
	return 0;
}
...
/* framer->dma->callback=irda_dma_callback; */
	...
	outw(IR_RX_EN_BIT /*|IR_RX_EN_RXONE */  |
	     (hard_ware ? (framer->speed <= 115200 ? 0 : IR_RX_EN_INVERT)
	      : (framer->speed <= 115200 ? IR_RX_EN_INVERT : 0)), IR_RX_EN);
#ifdef MT6573
...
void display_skb(struct sk_buff *skb, int rx)
{
	...
	if (skb->len == 0)
		return;
	printk("*********%s skb display begin(size=%dbytes speed=%d id=%lu)***********",
	       rx ? "rx" : "tx", skb->len, framer_dev->speed,
...
\nIR_FRAME1_SIZE=%d IR_FRAME2_SIZE=%d\n", status, err, framer_1_err, framer_2_err, size_1, size_2);
	...
	if (status & IR_IRQ_ENABLE_ERROR) {
		...
		if (framer->direction == IO_XMIT) {
			...
			framer_err =
			    (err & IR_FRAME1_ERROR) ? framer_1_err : ((err & IR_FRAME2_ERROR) ?
								      framer_2_err : 0);
			...
			if (framer_err) {
				if (framer_err &
				    (IR_FRAME_FRAMING | IR_FRAME_MIR | IR_FRAME_FIR_4PPM |
				     IR_FRAME_FIR_STO | IR_FRAME_UNKNOW))
		...
		dbg("%s(),rx abort(%s) !\n", __func__,
		    framer->direction == IO_RECV ? "rx" : "rx abort with tx dir");
...
static int mt_irda_ioctl(struct net_device *dev, struct ifreq *ifrq, int cmd)
{
	...
	case SIOCSMODE:
		...
		if (ifr->
		    ifr_mode & ~(IOCTL_SIR_DIV_16 | IOCTL_SIR_1_61 | IOCTL_BOF_C0H | IOCTL_BOF_FFH |
				 IOCTL_HAVEHARDWARE | IOCTL_NOHARDWARE)) {
			ret = -EINVAL;
			goto out;
		}
		spin_lock_irq(&framer_dev->lock);
		...
		if (ifr->ifr_mode & IOCTL_SIR_DIV_16)
			parse_modulation_type("3/16");
		else if (ifr->ifr_mode & IOCTL_SIR_1_61)
			parse_modulation_type("1.61us");
		if (ifr->ifr_mode & IOCTL_BOF_C0H)
			parse_bof_type("c0h");
		else if (ifr->ifr_mode & IOCTL_BOF_FFH)
			parse_bof_type("ffh");
		if (ifr->ifr_mode & IOCTL_HAVEHARDWARE) {
			hard_ware = 1;
			mt_irda_start_rx(framer_dev);
		} else if (ifr->ifr_mode & IOCTL_NOHARDWARE) {
			hard_ware = 0;
			mt_irda_start_rx(framer_dev);
		}
		spin_unlock_irq(&framer_dev->lock);
