...
.buffer_size_record = MMProfileDefaultBufferSize, .new_buffer_size_record =
	    MMProfileDefaultBufferSize, .buffer_size_bytes =
	    ((sizeof(MMProfile_Event_t) * MMProfileDefaultBufferSize +
		      (PAGE_SIZE - 1)) & (~(PAGE_SIZE - 1))), .record_size =
	    sizeof(MMProfile_Event_t), .meta_buffer_size =
	    MMProfileDefaultMetaBufferSize, .new_meta_buffer_size =
	    MMProfileDefaultMetaBufferSize, .selected_buffer =
	    MMProfilePrimaryBuffer, .reg_event_index =
	    sizeof(MMProfileStaticEvents) / sizeof(MMP_StaticEvent_t), .max_event_count =
	    MMProfileMaxEventCount,};
...
static void MMProfileInitBuffer(void)
{
	...
	if ((!bMMProfileInitBuffer) ||
		...
		MMP_LOG(ANDROID_LOG_DEBUG, "pMMProfileRingBuffer=0x%08x",
			(unsigned int)pMMProfileRingBuffer);
		...
		MMP_LOG(ANDROID_LOG_DEBUG, "pMMProfileMetaBuffer=0x%08x",
			(unsigned int)pMMProfileMetaBuffer);
...
static void MMProfileLog_Int(MMP_Event event, MMP_LogType type, unsigned int data1,
			     unsigned int data2, unsigned int meta_data_cookie)
{
	if (!MMProfileGlobals.enable)
		return;
	if (bMMProfileInitBuffer && MMProfileGlobals.start && MMProfileGlobals.event_state[event]) {
		...
		if (unlikely(event < 2))
			return;
		index =
		    (atomic_inc_return((atomic_t *) &(MMProfileGlobals.write_pointer)) -
		     1) % (MMProfileGlobals.buffer_size_record);
		...
		if (unlikely(lock > 1)) {
			...
			while (1) {
				index =
				    (atomic_inc_return
				     ((atomic_t *) &(MMProfileGlobals.write_pointer)) -
				     1) % (MMProfileGlobals.buffer_size_record);
				lock =
...
static int MMProfileLogMetaInt(MMP_Event event, MMP_LogType type, MMP_MetaData_t *pMetaData,
			       int bFromUser)
{
	unsigned long retn;
	if (!MMProfileGlobals.enable)
	...
	if (bMMProfileInitBuffer && MMProfileGlobals.start && MMProfileGlobals.event_state[event]) {
		...
		if (((unsigned int)pNode + block_size) <
		    ((unsigned int)pMMProfileMetaBuffer + MMProfileGlobals.meta_buffer_size)
		    && ((unsigned int)pNode + block_size) >
		    ((unsigned int)pMMProfileMetaBuffer + MMProfileGlobals.meta_buffer_size -
		...
		if ((pNode->block_size - block_size) >=
			...
			MMProfile_MetaDataBlock_t *pNewNode =
			    (MMProfile_MetaDataBlock_t *) ((unsigned int)(pNode) + block_size);
			if ((unsigned int)pNewNode >=
			    ((unsigned int)pMMProfileMetaBuffer +
			     MMProfileGlobals.meta_buffer_size))
				pNewNode =
				    (MMProfile_MetaDataBlock_t *) ((unsigned int)pNewNode -
								   MMProfileGlobals.
								   meta_buffer_size);
			      ...
			      MMP_MetaDataStructure_t *pMetaData)
{
	...
	if (bMMProfileInitBuffer && MMProfileGlobals.start && MMProfileGlobals.event_state[event]) {
		...
		memcpy((void *)((unsigned int)(MetaData.pData) + 32), pMetaData->pData,
...
static long mmprofile_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case MMP_IOC_DUMPMETADATA:
		{
				...
				if (pMetaDataBlock->data_size > 0) {
					...
					if (((unsigned int)(pMetaDataBlock->meta_data) +
						...
						unsigned int left_size =
						    (unsigned int)pMMProfileMetaBuffer +
						    MMProfileGlobals.meta_buffer_size -
						    (unsigned int)(pMetaDataBlock->meta_data);
						retn =
			...
			/* pr_info("[mmprofile_ioctl] Finished: offset=%x\n", offset-8); */
			mutex_unlock(&MMProfile_MetaBufferMutex);
		}
		break;
	...
	case MMP_IOC_TRYLOG:
		if ((!MMProfileGlobals.enable) ||
		    ...
		    (!MMProfileGlobals.start) ||
		    (arg >= MMProfileMaxEventCount) || (!MMProfileGlobals.event_state[arg]))
			ret = -EINVAL;
		break;
...
static int mmprofile_probe(void)
{
	...
	/* if (IS_ERR(filp)) */
	/* { */
	/* pr_info("[mmprofile] NVM: Cannot open configuration file %s\n", CONFIG_MMPROFILE_PATH); */
	/* MMProfileEnable(0); */
	/* } */
	/* else */
	/* { */
	...
	/* pr_info("[mmprofile] NVM: enable=%d start=%d.\n", enable, start); */
	/* if (enable == 1) */
	/* { */
	/* MMProfileEnable(1); */
	/* if (start == 1) */
