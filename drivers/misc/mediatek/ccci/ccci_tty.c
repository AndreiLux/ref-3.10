...
static void ccci_tty_read(unsigned long arg)
{
	...
	if (tty_instance->tty == NULL) {
		...
		return;
	} else if ((tty_instance->tty->index == CCCI_TTY_MODEM)
		   && (is_meta_mode() || is_advanced_meta_mode())) {
	...
	if (tty_debug_enable & (1UL << tty_instance->tty->index))
		CCCI_MSG_INF("tty",
			     "[before Read]:[RX] tty=%04d data_len=%04d write=%04d read=%04d\n",
 ...
 __ccci_read_ack:
	...
	if (tty_debug_enable & (1UL << tty_instance->tty->index))
		CCCI_MSG_INF("tty",
			     "[after  Read]:[RX] tty=%04d data_len=%04d write=%04d read=%4d\n",
			     tty_instance->tty->index, accept,
	...
	case CCCI_UART1_TX_ACK:
		{
			...
			wake_up_interruptible_poll(&ccci_tty_driver->ttys[CCCI_TTY_META]->
						   write_wait, POLLOUT);
		}
		break;
	...
	case CCCI_UART2_TX_ACK:
		{
			...
			wake_up_interruptible_poll(&ccci_tty_driver->ttys[CCCI_TTY_MODEM]->
						   write_wait, POLLOUT);
		}
		break;
	...
	case CCCI_IPC_UART_TX_ACK:
		{
			...
			wake_up_interruptible_poll(&ccci_tty_driver->ttys[CCCI_TTY_IPC]->write_wait,
						   POLLOUT);
		}
		break;
...
static int ccci_tty_write(struct tty_struct *tty, const unsigned char *buf, int len)
{
	...
	if (tty_debug_enable & (1UL << tty_instance->tty->index))
		CCCI_MSG_INF("tty",
			     "[before Write]:[TX] tty=%04d data_len=%04d write=%04d read=%04d over=%04d\n",
	...
	if (0 == size) {
		...
		while (1) {
			if (wait_event_interruptible_timeout(*tty_instance->write_waitq,
							     read !=
							     tty_instance->shared_mem->tx_control.
							     read, time_out) == 0) {
				if (retry_times) {
					CCCI_MSG_INF("tty",
						     "ttyC%d write timeout, rp=%04d wp=%04d len=%04d\n",
						     tty_instance->tty->index,
					...
					time_out = 60 * 60 * HZ;
					CCCI_MSG_INF("tty",
						     "ttyC%d change write timeout to 1 hour\n",
						     tty_instance->tty->index);
				}
			} else {
	...
	if (has_free_space) {
		...
		if (tty_debug_enable & (1UL << tty_instance->tty->index))
			CCCI_MSG_INF("tty",
				     "[after  Write]:[TX] tty=%04d data_len=%04d write=%04d read=%4d\n",
				     tty_instance->tty->index, len,
				     tty_instance->shared_mem->tx_control.write,
				     tty_instance->shared_mem->tx_control.read);
		...
		do {
			...
			if (ret == CCCI_NO_PHY_CHANNEL) {
				...
				if ((xmit_retry % 10) == 0) {
					CCCI_MSG_INF("tty",
						     "[No Physical Channel]ttyC%d retry %d times fail\n",
		...
		if (ret != CCCI_SUCCESS) {
			CCCI_MSG_INF("tty",
				     "ttyC%d write stream fail, tx write pointer will roll back[%d-->%d]\n",
				     tty_instance->tty->index,
...
int __init ccci_tty_init(void)
{
	...
	ccci_tty_driver->flags = TTY_DRIVER_RESET_TERMIOS | TTY_DRIVER_REAL_RAW;
...
/* tty_register_device(ccci_tty_driver, CCCI_TTY_IPC,  0); */

	{
		...
		ASSERT(ccci_uart_setup(0, (int *)&uart1_shared_mem, &smem_phy, &smem_size) ==
		       CCCI_SUCCESS);
	...
	{
		int smem_phy = 0, smem_size = 0;
		ASSERT(ccci_uart_setup(1, (int *)&uart2_shared_mem, &smem_phy, &smem_size) ==
		       CCCI_SUCCESS);
...

	{
		int smem_phy = 0, smem_size = 0;
		/* for IPC uart */
		ASSERT(ccci_uart_setup(5, (int *)&uart3_shared_mem, &smem_phy, &smem_size) ==
		       CCCI_SUCCESS);
		...
		ASSERT(ccci_register(CCCI_IPC_UART_RX_ACK, ccci_tty_callback, NULL) ==
		       CCCI_SUCCESS);
		ASSERT(ccci_register(CCCI_IPC_UART_TX, ccci_tty_callback, NULL) == CCCI_SUCCESS);
		ASSERT(ccci_register(CCCI_IPC_UART_TX_ACK, ccci_tty_callback, NULL) ==
		       CCCI_SUCCESS);
