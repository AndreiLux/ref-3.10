...
#define CCCI_LOGE(args...) pr_err(args)
...
/* struct logical_channel *(*find)(struct logical_layer *lg_layer,int num); */
	int (*add_client) (struct logical_layer *lg_layer, int num, int buf_num,
			   struct logical_channel **);
...
extern int ccci_logical_layer_init(struct physical_channel *, struct logical_layer *lg_layer,
				   int ch_num);
