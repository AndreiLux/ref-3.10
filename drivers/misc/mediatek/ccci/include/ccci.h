...
/*
#ifdef CCCI_DEBUG_ON
#define CCCI_DEBUG(format,arg...)   pr_info("CCCI:%s(),%d:" format,\
	__FUNCTION__,__LINE__,##arg)
#define CCCI_DEBUG_MSG(format, ...) pr_info("CCCI :" format, __VA_ARGS__)
#define CCCI_DEBUG_FUNC pr_err("%s\n",__FUNCTION__)
#else
...
extern void aed_md_exception1(int *log, int log_size, int *phy, int phy_size,
			      char *assert_type, char *filename, unsigned int line,
			      unsigned int fata1, unsigned int fata2);
#endif
