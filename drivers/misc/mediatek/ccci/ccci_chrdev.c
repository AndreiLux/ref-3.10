		...
		*out_buff = array->array[array->out_idx];
		...
		if (recv_array_is_empty(array)) {
			if (ccci_is_last_recv(&client->buff_list)) {
				...
				if (client->free_nr <= client->used_nr
				    || (client->free_nr + array->nr) < (CCCI_FIFO_MAX_LEN * 20)) {
...
static int ccci_push_buff(struct ccci_dev_client *client, CCCI_BUFF_T *buff)
{
	...
	if (array == NULL || recv_array_is_full(array)) {
		...
		if (array == NULL) {
			if (client->used_nr
			    && (client->used_nr + client->used_nr / 2) > MAX_BUFFER_MESSAGES) {
			...
			array =
			    ccci_create_array((client->used_nr
					       && (client->used_nr / 2) >
					       CCCI_FIFO_MAX_LEN) ? (client->used_nr /
								     2) : CCCI_FIFO_MAX_LEN * 5,
					      GFP_ATOMIC);
			if (array == NULL) {
				CCCI_MSG_INF("chr",
					     "Fail alloc recv_array FIFO(%d,%d): 0x%x,0x%x,%d,0x%x\n",
					     client->used_nr, client->free_nr, buff->data[0],
					     buff->data[1], buff->channel, buff->reserved);
				ret = -ENOMEM;
				push_data_fail = 1;
				goto out;
			}
...
static struct ccci_dev_client *find_get_client(int ch, pid_t pid)
{
	...
	if (&client->dev_list == &ccci_dev_list) {
		...
		ret =
		    ccci_register_with_name_size(ch, "CCCI_CHRDEV", CCCI_FIFO_MAX_LEN * 2,
						 ccci_chrdev_callback, client);
		if (ret && (ret != -EEXIST)) {
...
static ssize_t ccci_dev_write(struct file *file, const char __user *buf, size_t count,
			      loff_t *ppos)
{
	struct ccci_dev_client *client = (struct ccci_dev_client *)file->private_data;
		...
		if (ret == sizeof(buff)) {
			if (copy_to_user(u_buff + i, &buff, sizeof(buff))) {
				CCCI_MSG_INF("chr",
					     ...
					     client->pid, current->comm, buff.data[0], buff.data[1],
			...
			if (push_data_fail || (lg_ch_rx_debug_enable & (1ULL << buff.channel))) {
				CCCI_MSG_INF("chr", "read: [%d:%s] %08X, %08X, %02d, %08X\n",
					     ...
					     buff.data[0], buff.data[1], buff.channel,
					     buff.reserved);
				push_data_fail = 0;
			}
		} else {
			...
			if (value == -ERESTARTSYS) {
				...
				client->wakeup_waitq = 0;
				if (push_data_fail
				    || (lg_ch_rx_debug_enable & (1ULL << buff.channel))) {
