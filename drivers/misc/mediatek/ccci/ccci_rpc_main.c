...
#define RPC_DBG(format, args...)	pr_info("ccci_rpc: " format, ##args)
#else
...
extern void ccci_rpc_work_helper(int *p_pkt_num, RPC_PKT pkt[], RPC_BUF *p_rpc_buf,
...
/* int m = 0; */
		...
		for (m = 0; m < IPC_RPC_MAX_BUF_SIZE + sizeof(unsigned int); m++) {
			if (m % 16 == 0)
				pr_info("\nSHM: ");
			pr_info("%02X ", *((unsigned char *)rpc_buf_tmp + m));
			msleep(1);
		}
		pr_info("\n\n");
#endif
...
int __init ccci_rpc_init(void)
{
	...
	if (rpc_work_queue == NULL) {
		pr_err("Fail create rpc_work_queue!!\n");
		return -EFAULT;
	}
#endif
...
void __exit ccci_rpc_exit(void)
{
	...
	if (cancel_work_sync(&rpc_work) < 0) {
		pr_info("ccci_rpc: Cancel rpc work fail!\n");
	}
