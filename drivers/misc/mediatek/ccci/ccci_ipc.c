...
#undef WARN_ON
#ifdef CCCI_DEBUG_ON
#define DBG(format, ...) pr_warn("CCCI_IPC_DEBUG:%s()," format, __func__, ##__VA_ARGS__)

#define WARN_ON(cond, fmt, arg...)   ({if (!!(cond))\
	pr_warn("CCCI_IPC_WARNING:file(%s) line(%d):\n" fmt , __FILE__, __LINE__, ##arg); })
#else
#define DBG(format, ...)
...
{
	uint32 i=0;
	pr_info("Dump data %d bytes.\n",size);

	for (i=0;i<size;i+=4)
	{
		if (i%16==0)pr_info("\n%d----%d :",i,i+16);
		pr_info("%#x\t",*((uint32*)(data+i)));
	}
	pr_info("\n");
...
{
	ipc_ilm_t *ilm =
	    (ipc_ilm_t *) ((uint32) ccci_ipc_mapping_addr + (addr - MODEM_REGION_BASE));
	CCCI_RECV_ITEM *item;
	unsigned long flags;
	...
	if (ilm->local_para_ptr) {
		...
		if ((uint32) ilm->local_para_ptr < (uint32) ccci_ipc_buffer_phy_rd ||
		    ...
		    (uint32) ccci_ipc_buffer_phy_rd + CCCI_IPC_BUFFER_SIZE)
...
static int write_to_ring_buffer(void *data, int count, IPC_TASK *task, BUFF *ipc_buffer)
{
	...
	if (((local_para ? local_para->msg_len : 0) + (peer_buff ? peer_buff->pdu_len : 0)) !=
	    count)
...
/* WARN_ON(count<sizeof(local_para_struct),"count<sizeof(local_para_struct).\n"); */
	ilm->local_para_ptr =
	    local_para ? (local_para_struct *) (ccci_ipc_buffer_phy_wr + write_begin) : NULL;
	ilm->peer_buff_ptr =
	    peer_buff ? (peer_buff_struct *) (ccci_ipc_buffer_phy_wr +
					      ((write_begin +
						(local_para ? local_para->msg_len : 0)) & (size -
											   1))) :
	    NULL;
...
/* task=(IPC_TASK *)data+buff->reserved; */
	...
	if (buff->channel == CCCI_IPC_RX) {
		...
		recv_item(((CCCI_STREAM_T *) buff)->addr, ((CCCI_STREAM_T *) buff)->len, task,
			  &ipc_mem->buffer.buff_rd);
...
static ssize_t ccci_ipc_write(struct file *file, const char __user *buf, size_t count,
			      loff_t *ppos)
{
/* int loop=0; */
	int ret = 0;
	...
	if (test_and_set_bit(CCCI_TASK_PENDING, &task->flag)) {
		...
		if (file->f_flags & O_NONBLOCK) {
			...
			goto out_free;
							      ...
							      !test_and_set_bit(CCCI_TASK_PENDING,
										&task->flag)
							      || hard_ware_ok == 0) ==
			   -ERESTARTSYS) {
			ret = -EINTR;
	...
	*task->ilm_p = *ilm;
	...
	if (count > sizeof(ipc_ilm_t)) {
		if (write_to_ring_buffer
		    (ilm + 1, count - sizeof(ipc_ilm_t), task,
		     &ipc_mem->buffer.buff_wr) != count - sizeof(ipc_ilm_t)) {
...
/* loop++; */
	...
	ret =
	    ccci_ipc_write_stream(CCCI_IPC_TX, task->ilm_phy_addr, sizeof(ipc_ilm_t),
				  id_map->extq_id);
/* cond_resched(); */
/* }while(ret==CCCI_NO_PHY_CHANNEL) ; */
/* dump(ilm,count); */
	if (ret != CCCI_SUCCESS) {	/* ret==CCCI_IN_USE  this should not happened. */
...
int __init ccci_ipc_init(void)
{
	...
	CCCI_MSG_INF("ipc", "virt:%p, phy:%#x, len:%#x\n", ipc_mem, ccci_ipc_smem_base_phy,
		     ccci_ipc_smem_size);

	ccci_ipc_buffer_phy_wr =
	    ccci_ipc_smem_base_phy + offset_of(CCCI_IPC_MEM, buffer.buff_wr.buffer);
	ccci_ipc_buffer_phy_rd =
	    ccci_ipc_smem_base_phy + offset_of(CCCI_IPC_MEM, buffer.buff_rd.buffer);
	...
	if (register_chrdev_region(MKDEV(CCCI_IPC_DEV_MAJOR, 0), MAX_NUM_IPC_TASKS, "CCCI_IPC_DEV")
	    != 0) {
