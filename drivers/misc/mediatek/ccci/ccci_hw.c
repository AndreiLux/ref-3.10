	...
	while ((r_chnum = *CCIF_RCHNUM(pc_ch->pc_base_addr)) || arb == 0) {
		...
		for (i = 0; i < max_ch; i++) {
			...
			if (r_chnum & (1 << num) || arb == 0) {
				rx_idx++;
				if (unlikely(arb == 0))
					ccci_buff_t_assign(&buf,
							   (CCCI_BUFF_T *) CCIF_RXCHDATA(pc_ch->
											 pc_base_addr)
							   + r_chnum);
				else
					ccci_buff_t_assign(&buf,
				...
				if ((lg_ch_rx_debug_enable & ENABLE_ALL_RX_LOG) ||
					...
					CCCI_MSG_INF("cci",
						     "[RX](%d): %08X, %08X, %02d, %08X (%02d)\n",
						     irq_cnt, buf.data[0], buf.data[1], buf.channel,
						     buf.reserved, num);
				}
				...
				if (buf.channel == CCCI_SYSTEM_RX) {
					if (buf.data[1] == MD_SLP_REQUEST) {
						if (buf.reserved == LOCK_MD_SLP) {
							...
							CCCI_MSG_INF("cci",
								     "request md sleep fail!\n");
						}
					} else if (buf.data[1] == MD_WDT_MONITOR)
						CCCI_MSG_INF("cci", "MD WDT monitor message\n");
				} else if (pc_ch->lg_layer.
					   process_data(&pc_ch->lg_layer, &buf, 1, 0)
					   != sizeof(CCCI_BUFF_T)) {
...
static void ccif_debug_dump(struct physical_channel *pc_ch, DEBUG_INFO_T *debug_info)
{
	...
	if (debug_info->platform_call)
		debug_info->platform_call(debug_info->platform_data);
...
/* #ifdef CONFIG_MTK_AEE_FEATURE */
	aed_md_exception1(debug_info->ext_mem, debug_info->ext_size,
			  ...
			  (debug_info->type == MD_EX_TYPE_ASSERT_DUMP
			   || debug_info->type ==
			   MD_EX_TYPE_ASSERT) ? debug_info->assert.file_name : NULL,
			  (debug_info->type == MD_EX_TYPE_ASSERT_DUMP
			   || debug_info->type ==
			   MD_EX_TYPE_ASSERT) ? debug_info->assert.line_num : 0,
			  (debug_info->type == MD_EX_TYPE_FATALERR_BUF
			   || debug_info->type ==
			   MD_EX_TYPE_FATALERR_TASK) ? debug_info->fatal_error.err_code1 : 0,
			  (debug_info->type == MD_EX_TYPE_FATALERR_BUF
			   || debug_info->type ==
			   MD_EX_TYPE_FATALERR_TASK) ? debug_info->fatal_error.
			  err_code2 : (debug_info->type ==
				       MD_EX_TYPE_EMI_CHECK ? debug_info->data.reserved : 0));
...
static void ccif_tx_fifo_dump(void)
{
	...
	for (i = 0; i < ccif_ch.max_pc_num; i++) {
		CCCI_MSG_INF("cci", "[phy_ch:%02d] %08X, %08X, %02d, %08X\n", i,
			     *CCIF_TXCHDATA(addr + 4 * 4 * i),
			     *CCIF_TXCHDATA(addr + 4 * (4 * i + 1)),
			     *CCIF_TXCHDATA(addr + 4 * (4 * i + 2)),
			     *CCIF_TXCHDATA(addr + 4 * (4 * i + 3)));
	}
		...
		for (i = 1; i < dump_num; i++) {
			if (xmit_log[j].tv.tv_sec == ref_tv.tv_sec) {
				...
				if (xmit_log[j].tv.tv_usec >= ref_tv.tv_usec) {
					...
					us_delta =
					    (int)(xmit_log[j].tv.tv_usec - ref_tv.tv_usec +
						  1000 * 1000);
			...
			s_to_date(xmit_log[j].tv.tv_sec, xmit_log[j].tv.tv_usec, &us, &sec, &min,
				  &hour, &day, &month, &year);
			CCCI_MSG_INF("cci",
				     "%08X %08X %02d %08X   %d-%02d-%02d %02d:%02d:%02d.%06d   %03x   %d.%06ds\n",
				     xmit_log[j].data[0], xmit_log[j].data[1], xmit_log[j].data[2],
				     xmit_log[j].data[3], year, month, day, hour, min, sec, us,
...
int get_md_wakeup_src(char *buf, unsigned int len)
{
	...
	CCCI_BUFF_T *data;
	unsigned int rx_ch[CCIF_MAX_PHY][2] =
	...
	for (i = 0; i < CCIF_MAX_PHY; i++) {
		if (rx_ch[i][1]) {
			if ((rx_ch[i][0] >= 0) && (rx_ch[i][0] < CCCI_MAX_CHANNEL))
				sprintf(str, "%s(%d,%d) ", lg_ch_str[rx_ch[i][0]], rx_ch[i][0],
					rx_ch[i][1]);
			else
	...
	pr_info("CCIF_MD wakeup source: %s\n", log_buf);
 ...
 retry:
	...
	if (busy & (1 << tx_idx)) {
		...
		CCCI_MSG_INF("cci", "No physical channel(%s):%08X, %08X, %02d, %08X\n",
			     current->comm, data->data[0], data->data[1], data->channel,
			     data->reserved);
...
void ccif_send_wakeup_md_msg(void)
{
	...
	unsigned int busy;
	int tx_idx;
	CCCI_BUFF_T msg = {.data[0] = 0xFFFFFFFF, .data[1] = MD_WDT_MONITOR, .channel =
		    CCCI_SYSTEM_TX, .reserved = 0 };
	...
	if (busy == 0) {
		...
		*CCIF_BUSY(ccif_ch.pc_base_addr) = 1 << tx_idx;
		ccci_buff_t_assign((CCCI_BUFF_T *) CCIF_TXCHDATA(ccif_ch.pc_base_addr) + tx_idx,
				   &msg);
