...
static int lg_add_client(struct logical_layer *lg_layer, int num, int buf_num,
			 struct logical_channel **lg_chptr)
{
	int ret = 0;
	struct logical_channel *client;
	int i = 0;
	...
	if (lg_layer->lg_array[num]) {
		...
		*lg_chptr = lg_layer->lg_array[num];
		...
		if (client == NULL) {
			...
			/* pr_info("[CCCI] allocate client %x for num:%d\n", client, num); */
...
static int lg_ch_log_dump(CCCI_LOG_T *log)
{
	...
	if (log->sec == 0 && log->nanosec == 0)
		return 0;
	s_to_date(log->sec, log->nanosec / 1000, &ms, &sec, &min, &hour, &day, &month, &year);
...
static int lg_process_data(struct logical_layer *lg_layer, CCCI_BUFF_T *data, int rx, int drop)
{
	...
	if (lg_layer->lg_array[ch] == NULL) {
		...
		if ((rx == 0) || lg_ch_is_mailbox(ch))
			...
			if (error[ch][CLIENT_FAIL] > ISR_ERR_THREASH) {
				CCCI_MSG("Wrong data(%s): %08x, %08x, %02d, %08x\n",
					 (rx ? "RX" : "TX"), data->data[0], data->data[1],
					 data->channel, data->reserved);
	...
	if (rx) {
		if (lg_ch->have_fifo == 0) {
			ret =
			    kfifo_alloc(&lg_ch->fifo, lg_ch->buf_num * sizeof(CCCI_BUFF_T),
					GFP_ATOMIC);
			if (ret) {
		...
		if (kfifo_is_full(&lg_ch->fifo)) {
			...
			if (unlikely(lg_ch_is_mailbox(ch)))
				...
				if (error[ch][KFIFO_FULL] > ISR_ERR_THREASH) {
					CCCI_MSG
					    ("kfifo full: Owner:%s size:%d (%08X %08X %02d %08X)\n",
					     lg_ch->owner, kfifo_size(&lg_ch->fifo), data->data[0],
					     data->data[1], data->channel, data->reserved);
	...
	if (client) {
		...
		do {
			...
			for (i = 0; i < ARRAY_SIZE(client->lc_statics.log); i++) {
				if (!lg_ch_log_dump
				    (client->lc_statics.log + ((client->lc_statics.log_idx - i - 1)
							       %
							       ARRAY_SIZE(client->lc_statics.
									  log)))) {
...
int ccci_logical_layer_init(struct physical_channel *p_ch, struct logical_layer *lg_layer,
			    int ch_num)
{
	int ret = 0;
	WARN_ON(lg_layer->lg_array);
