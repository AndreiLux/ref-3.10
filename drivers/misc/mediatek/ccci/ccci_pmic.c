...
static void ccci_pmic_read(struct work_struct *ws)
{
	...
	/* pr_info("************* ccci_pmic_callback : exec_time_1 = %ld ms\r\n", exec_time_1); */
	...
	case PMIC6326_VSIM_ENABLE:
		/* #ifdef CONFIG_CCCI_PMIC_DEBUG_MSG */
		/* pr_info("ccci_pmic_callback : PMIC6326_VSIM_ENABLE\n"); */
	...
	case PMIC6326_VSIM_SET_AND_ENABLE:
		/* #ifdef CONFIG_CCCI_PMIC_DEBUG_MSG */
		/* pr_info("ccci_pmic_callback : PMIC6326_VSIM_SET_AND_ENABLE\n"); */
	...
	case PMIC6236_LOCK:
		/* #ifdef CONFIG_CCCI_PMIC_DEBUG_MSG */
		/* pr_info("ccci_pmic_callback : PMIC6236_LOCK\n"); */
	...
	case PMIC6326_UNLOCK:
		/* #ifdef CONFIG_CCCI_PMIC_DEBUG_MSG */
		/* pr_info("ccci_pmic_callback : PMIC6326_UNLOCK\n"); */
	...
	case PMIC6326_VSIM2_ENABLE:
		/* #ifdef CONFIG_CCCI_PMIC_DEBUG_MSG */
		/* pr_info("ccci_pmic_callback : PMIC6326_VSIM2_ENABLE\n"); */
	...
	case PMIC6326_VSIM2_SET_AND_ENABLE:
		/* #ifdef CONFIG_CCCI_PMIC_DEBUG_MSG */
		/* pr_info("ccci_pmic_callback : PMIC6326_VSIM2_SET_AND_ENABLE\n"); */
	...
	default:
		/* #ifdef CONFIG_CCCI_PMIC_DEBUG_MSG */
		/* pr_info("ccci_pmic_callback : ERROR op !\n"); */
	...
	/* #ifdef CONFIG_CCCI_PMIC_DEBUG_MSG */
	/* pr_info("************* ccci_pmic_callback : exec_time_2 = %ld ms\r\n", exec_time_2); */
	CCCI_PMIC_MSG("exec_time_2 = %ld ms\r\n", exec_time_2);
	/* #endif */

	exec_time_2 = (exec_time_2 - exec_time_1) / 1000;
	/* #ifdef CONFIG_CCCI_PMIC_DEBUG_MSG */
	/* pr_info("************* ccci_pmic_callback : exec_time = %ld ms\r\n", exec_time_2); */
...
static void ccci_pmic_callback(CCCI_BUFF_T *buff, void *private_data)
{
	unsigned long flag;
	/* #ifdef CONFIG_CCCI_PMIC_DEBUG_MSG */
	/* pr_info("CCCI_PMIC: ccci_pmic_callback\n"); */
	CCCI_PMIC_MSG("callback++\n");
	/* #endif */

	spin_lock_irqsave(&pmic_spinlock, flag);
	if (buff->channel == CCCI_PMIC_RX) {
		if (kfifo_in(&pmic_fifo, (unsigned char *)&buff->data[1], sizeof(unsigned long)) !=
		    sizeof(unsigned long)) {
			spin_unlock_irqrestore(&pmic_spinlock, flag);
...
static int ccci_pmic_start(void)
{
	...
	/* #ifdef CONFIG_CCCI_PMIC_DEBUG_MSG */
	/* pr_info("CCCI_PMIC: ccci_pmic_start ****************************\n"); */
	...
	ASSERT(ccci_pmic_setup((int *)&ccci_pmic_shared_mem, &ccci_pmic_shared_mem_phys_addr, &size)
	       == CCCI_SUCCESS);
	/* pr_info("CCCI PMIC %x:%x:%d\n", (unsigned int)ccci_pmic_shared_mem, (unsigned int)ccci_pmic_shared_mem_phys_addr, size); */
...
{
	/* #ifdef CONFIG_CCCI_PMIC_DEBUG_MSG */
	/* pr_info("CCCI_PMIC: ccci_pmic_stop ****************************\n"); */
...
int __init ccci_pmic_init(void)
{
	...
	if (alloc_chrdev_region(&pmic_dev_num, 0, 1, CCCI_PMIC_DEVNAME)) {
		/* pr_err("CCCI_PMIC: Device major number allocation failed\n"); */
	...
	if (ret) {
		/* pr_err("CCCI_PMIC: Char device add failed\n"); */
	...
	ret = ccci_pmic_start();
	if (ret) {
		/* pr_err("CCCI_PMIC: Unable to initialize environment\n"); */
		CCCI_MSG("<pmic>Unable to initialize environment\n");
		return ret;
	}
	/* pr_info("CCCI_PMIC: Init complete, device major number = %d\n", MAJOR(pmic_dev_num)); */
...
#if 0
module_init(ccci_pmic_init);
module_exit(ccci_pmic_exit);

MODULE_DESCRIPTION("MT65XX CCCI PMIC Driver");
