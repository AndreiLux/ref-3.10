...
#define CF_DEBUG(format, ...) pr_info(format, __VA_ARGS__)
#define CF_DEBUG_FUNC pr_info("%s\n", __func__)
#define CF_INFO(format, ...) pr_info(format, __VA_ARGS__)
#else
...
static void ccci_fs_callback(CCCI_BUFF_T * buff, void *private_data)
{
	...
	if (buff->channel == CCCI_FS_RX) {
		...
		if (kfifo_in(&fs_fifo, (unsigned char *)&buff->reserved, sizeof(buff->reserved)) ==
		    sizeof(buff->reserved)) {
...
static int ccci_fs_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	return remap_pfn_range(vma, vma->vm_start, off >> PAGE_SHIFT, vma->vm_end - vma->vm_start,
			       vma->vm_page_prot);
}
...
static int ccci_fs_start(void)
{
	...
	CCCI_MSG_INF("fs ", "%x:%x:%d\n", (unsigned int)fs_buffers,
		     (unsigned int)fs_buffers_phys_addr, size);
