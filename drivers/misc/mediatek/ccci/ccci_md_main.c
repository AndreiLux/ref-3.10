...
{
    int i;
    pr_err("CCCI_MD: [Exception] Mem dump:\n");
    ...
    pr_err("CCCI_MD: [Exception] DSP dump:\n");
    for(i = 0; i < (MD_IMG_DUMP_SIZE / 4); i += 4)
    {
	pr_err("0x%08X %08X %08X %08X %08X\n",
...
static void ex_monitor_func(unsigned long data __always_unused)
{
	...
	while (1) {
		...
		if (md_ex_get == 1 && md_ex_ok_get == 0) {
			...
			atomic_set(&md_ex_ok, 0);
			CCCI_MSG_INF("ctl",
				     "Only Receive MD_EX_REC_OK, this may be appear after time out\n");
		} else
...
static volatile int wakeup_md_is_safe;
void ccci_md_ctrl_cb(CCCI_BUFF_T *buff, void *private_data)
{
	...
	if (CCCI_MAILBOX_ID(buff) == MD_INIT_START_BOOT &&
		...
		ccci_system_message(&sys_msg);
	} else if (CCCI_MAILBOX_ID(buff) == NORMAL_BOOT_ID && md_boot_stage == MD_BOOT_STAGE_1) {
		del_timer(&md_boot_up_check_timer);
...
static int boot_md(void)
{
	...
	do {
		...
		if (time_after(jiffies, time_out)) {
			CCCI_MSG_INF("ctl",
				     "wait MD_INIT_START_BOOT time out, try to reboot MD again\n");
	...
	/* pr_info("echo MD_INIT_START_BOOT\n"); */
	CCCI_BUFF_T buff;
...
static int __init ccci_alloc_smem(void)
{
	...
	ccci_smem_size = CCCI_PCM_SMEM_SIZE +
	    CCCI_MD_LOG_SIZE +
	    CCCI_PMIC_SMEM_SIZE +
	    CCCI_RPC_SMEM_SIZE + CCCI_FS_SMEM_SIZE + (CCCI_TTY_SMEM_SIZE * CCCI_TTY_PORT_COUNT) +
#ifdef AP_MD_EINT_SHARE_DATA
	    CCCI_SYS_SMEM_SIZE +
#endif
	    MD_EX_LOG_SIZE + CCCI_IPC_SMEM_SIZE;
...
size_t ccci_ch_filter_store(char buf[], size_t len)
{
	...
	ret =
	    sscanf(buf, "-c=0x%x 0x%llx 0x%llx 0x%x 0x%x 0x%x ", &key, &lg_ch_tx_mask,
		   &lg_ch_rx_mask, &fs_tx_mask, &fs_rx_mask, &tty_mask);
	if (ret != 6) {
...
size_t ccci_ch_filter_show(char buf[], size_t len)
{
	...
	ret =
	    snprintf(buf, len,
		     "tx_mask: %llX\nrx_mask: %llX\nfs_tx_mask: %X\nfs_rx_mask: %X\ntty_msk: %X\n",
		     lg_ch_tx_debug_enable, lg_ch_rx_debug_enable, fs_tx_debug_enable,
		     fs_rx_debug_enable, tty_debug_enable);
	return ret;
}
...
/* pr_err("----------->CCCI_MD: md_img_vir=%p.\n",md_img_vir); */
	/* if (!md_img_vir) */
	/* { */
	/* pr_err("[CCCI]:CCCI_MD[Exception] Mem dump ioremap failed\n"); */
	/* return -ENOMEM; */
	/* } */
