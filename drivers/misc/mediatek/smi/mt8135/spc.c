...
void spc_config(struct MTK_SPC_CONFIG *pCfg)
{
	...
	SPCMSG("spc config prot=(%d,%d,%d,%d), start=0x%x, end=0x%x\n",
	       ...
	       pCfg->domain_2_prot, pCfg->domain_3_prot, pCfg->start, pCfg->end);
	...
	regval = (pCfg->domain_0_prot << (0 * 2)) |
	    ...
	    (pCfg->domain_2_prot << (2 * 2)) | (pCfg->domain_3_prot << (3 * 2));
...
unsigned int spc_register_isr(void *dev)
{
	...
	ret =
	    request_irq(MT_APARM_DOMAIN_IRQ_ID, (irq_handler_t) MTK_SPC_isr,
			IRQF_TRIGGER_LOW | IRQF_SHARED, "spc", dev);
	if (ret != 0) {
...
int MTK_SPC_Init(void *dev)
{
	...
	/* 0xfb:MM, 0xfd:EMI, 0xf9:Both */
	mt65xx_reg_sync_writel(readl(DEVAPC0_DXS_VIO_MASK) & (~0x8), DEVAPC0_DXS_VIO_MASK);
