...
static void process_dbg_opt(const char *opt)
{
	...
	if (0 == strncmp(opt, "spc_config:", 11)) {
		struct MTK_SPC_CONFIG pCfg;
		char *p = (char *)opt + 11;

		SMIMSG("%s", p);
		/* 0-no protect; 1-sec rw; 2-sec_rw nonsec_r; 3-no access */
		pCfg.domain_0_prot = (enum SPC_PROT_T)simple_strtoul(p, &p, 16);

		SMIMSG("%d,%s", pCfg.domain_0_prot, p);
		p++;
		pCfg.domain_1_prot = (enum SPC_PROT_T)simple_strtoul(p, &p, 16);
		p++;
		SMIMSG("%d", pCfg.domain_1_prot);
		pCfg.domain_2_prot = (enum SPC_PROT_T)simple_strtoul(p, &p, 16);
		p++;
		SMIMSG("%d", pCfg.domain_2_prot);
		pCfg.domain_3_prot = (enum SPC_PROT_T)simple_strtoul(p, &p, 16);
		p++;
