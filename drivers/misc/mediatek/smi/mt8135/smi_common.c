...
int smi_bwc_config(struct MTK_SMI_BWC_CONFIG *p_conf)
{
	int i=0;
	/*turn on larb clock */
...
static long smi_ioctl(struct file *pFile, unsigned int cmd, unsigned long param)
{
	...
	case MTK_IOC_SPC_CONFIG:
		{
			struct MTK_SPC_CONFIG cfg;
			ret = copy_from_user(&cfg, (void *)param, sizeof(struct MTK_SPC_CONFIG));
			if (ret) {
