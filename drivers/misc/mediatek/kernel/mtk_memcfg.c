...
static unsigned long force_inode_gfp_lowmem;
#ifdef CONFIG_SLUB_DEBUG
static unsigned long bypass_slub_debug;
#endif

static int mtk_memcfg_late_warning_flag;
...
unsigned long mtk_memcfg_set_bypass_slub_debug_flag(unsigned long flag)
{
	...
	if (bypass_slub_debug && !flag) {
		pr_err("===== slub debug is re-enabled, " "ignore this operation =====\n");
		goto out;
	} else if (!bypass_slub_debug && flag) {
		pr_alert("===== bypass slub debug =====\n");
...
static int __init mtk_memcfg_late_init(void)
{
	...
	if (!mtk_memcfg_dir) {
		...
		entry = proc_create("force_inode_gfp_lowmem",
				    ...
				    &mtk_memcfg_inode_operations);

		if (!entry) {
			pr_err("create force_inode_gfp_lowmem proc entry failed\n");
		}
		...
		entry = proc_create("bypass_slub_debug",
				    S_IRUGO | S_IWUSR, mtk_memcfg_dir,
				    &mtk_memcfg_slub_debug_operations);

		if (!entry) {
			pr_err("create bypass_slub_debug proc entry failed\n");
		}
