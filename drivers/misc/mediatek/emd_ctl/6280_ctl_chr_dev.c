...
static int emd_aseert_log_wait_timeout;
static int power_on_md(void);
static int power_off_md(void);
static int ext_md_power_state;
/*****************************************************************************************
 *   Log control section
 *****************************************************************************************/
...
static void eint_setup(void)
{
	...
	mt_eint_registration(CUST_EINT_MT6280_WD_NUM,
			     CUST_EINT_MT6280_WD_TYPE, ext_md_wdt_irq_cb, 0);
	mt_eint_unmask(CUST_EINT_MT6280_WD_NUM);

	mt_eint_registration(CUST_EINT_MT6280_USB_WAKEUP_NUM,
			     CUST_EINT_MT6280_USB_WAKEUP_TYPE, ext_md_wakeup_irq_cb, 0);
#if 0
	/* --- Ext MD wdt irq ------- */
	...
	mt65xx_eint_set_hw_debounce(CUST_EINT_DT_EXT_MD_WDT_NUM,
				    CUST_EINT_DT_EXT_MD_WDT_DEBOUNCE_CN);
	mt65xx_eint_registration(CUST_EINT_DT_EXT_MD_WDT_NUM, CUST_EINT_DT_EXT_MD_WDT_DEBOUNCE_EN,
				 CUST_EINT_DT_EXT_MD_WDT_POLARITY, ext_md_wdt_irq_cb, 0);
	...
	mt65xx_eint_set_hw_debounce(CUST_EINT_DT_EXT_MD_WK_UP_NUM,
	...
	mt65xx_eint_registration(CUST_EINT_DT_EXT_MD_WK_UP_NUM,
				 ...
				 CUST_EINT_DT_EXT_MD_WK_UP_POLARITY, ext_md_wakeup_irq_cb, 0);
#endif
}
...
{
	dev_dbg(&dev->dev, "client:%d suspend!!\n", dev->id);
	if ((dev->id < 0) || (dev->id >= EMD_CLIENT_NUM))
		dev_err(&dev->dev, "invalid id%d!!\n", dev->id);
	else {
		if (0 == dev->id) {
			dev_err(&dev->dev, "ext md enable wake up\n");
			cm_release_wakeup_md_signal();
			if (ext_md_power_state) {
...
int __init emd_chrdev_init(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(emd_chr_devs); i++) {
		...
		if (ret != 0) {
			pr_info("[emd] Regsiter emd_char_device %d failed\n", i);
			return ret;
		}
	}
#endif
