 ...
 *   Log control section
 *****************************************************************************************/
#define EMD_MSG(fmt, args...)	pr_info("[ext-md-ctl]" fmt, ##args)
...
static void eint_setup(void)
{
	...
	mt_eint_registration(CUST_EINT_DT_EXT_MD_WDT_NUM,
			     generate_eint_flag(CUST_EINT_DT_EXT_MD_WDT_POLARITY,
						CUST_EINT_DT_EXT_MD_WDT_SENSITIVE),
			     ext_md_wdt_irq_cb, 0);

	/* --- Ext MD wake up irq ------------ */
	mt_eint_set_hw_debounce(CUST_EINT_DT_EXT_MD_WK_UP_NUM,
				CUST_EINT_DT_EXT_MD_WK_UP_DEBOUNCE_CN);
	mt_eint_registration(CUST_EINT_DT_EXT_MD_WK_UP_NUM,
			     generate_eint_flag(CUST_EINT_DT_EXT_MD_WK_UP_POLARITY,
						CUST_EINT_DT_EXT_MD_WK_UP_SENSITIVE),
			     ext_md_wakeup_irq_cb, 0);
}
...
{
	int ret = 0;
	struct device *dev = &file->f_inode->i_cdev->dev;
	emd_dev_client_t *client = (emd_dev_client_t *) file->private_data;

	switch (cmd) {
	case EXT_MD_IOCTL_LET_MD_GO:
		if (1 == client->sub_dev_id) {
			ret = let_ext_md_go();
		} else {
			dev_dbg(dev, "sub dev%d call EXT_MD_IOCTL_LET_MD_GO is denied",
				client->sub_dev_id);
	...
	case EXT_MD_IOCTL_REQUEST_RESET:
		if (2 == client->sub_dev_id) {
			/* Only /dev/ext_md_ctl1 has the ablility
				* of request modem reset */
			ret = request_ext_md_reset();
		} else {
			dev_dbg(dev, "sub dev%d call EXT_MD_IOCTL_REQUEST_RESET is denied",
				client->sub_dev_id);
			ret = -EMD_ERR_ACCESS_DENY;
		}
		break;
	...
	case EXT_MD_IOCTL_POWER_ON_HOLD:
		if (0 == client->sub_dev_id) {
			/* Only /dev/ext_md_ctl0 has the ablility of power on modem */
			ret = power_on_md_and_hold_it();
		} else {
			dev_dbg(dev,
				"sub dev%d call EXT_MD_IOCTL_POWER_ON_HOLD is denied\n",
				client->sub_dev_id);
			ret = -EMD_ERR_ACCESS_DENY;
		}
		break;
...
{
	dev_dbg(&device->dev, "clint %d probe!!\n", device->id);

	if ((device->id < 0) || (device->id >= EMD_CLIENT_NUM))
		dev_err(&device->dev, "invalid id%d!!\n", device->id);
	else {
...
{
	dev_dbg(&dev->dev, "client:%d suspend!!", dev->id);
	if ((dev->id < 0) || (dev->id >= EMD_CLIENT_NUM))
		dev_err(&dev->dev, "invalid id%d!!", dev->id);
	else {
		if (0 == dev->id) {
			cm_release_wakeup_md_signal();
			/* cm_enable_ext_md_wakeup_irq(); */
...
{
	dev_dbg(&dev->dev, "client:%d resume!!\n", dev->id);
	if ((dev->id < 0) || (dev->id >= EMD_CLIENT_NUM))
		dev_err(&dev->dev, "invalid id%d!!\n", dev->id);
	else {
		if (0 == dev->id) {
			disable_ext_md_wakeup_irq();
		}
