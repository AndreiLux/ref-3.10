...
static struct class *psDevinfoClass;
...
static long dev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case READ_DEV_DATA:
		...
		if (index < data_size) {
			...
			xlog_printk(ANDROID_LOG_ERROR, DEVINFO_TAG,
				    "%s Error! Data index larger than data size. index:%d, size:%d\n",
...
static int __init devinfo_init(void)
{
	...
	psDevinfoClass = class_create(THIS_MODULE, DEV_NAME);

	if (IS_ERR(psDevinfoClass)) {
		goto class_fail;
	}

	psDevinfoDev = device_create(psDevinfoClass, NULL, devinfo_dev,
#if (LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 26))
				     NULL,
#endif				/* (LINUX_VERSION_CODE > KERNEL_VERSION(2,6,26)) */
				     DEV_NAME);

	if (IS_ERR(psDevinfoDev)) {
		goto class_fail;
	}
...
{
	device_destroy(psDevinfoClass, MKDEV(MAJOR_DEV_NUM, 0));
	class_destroy(psDevinfoClass);
