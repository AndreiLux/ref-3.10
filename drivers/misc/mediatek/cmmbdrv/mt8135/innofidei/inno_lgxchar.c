...
static struct workqueue_struct *check_block_wq;
...
/* inno_device_t* inno_dev = &g_inno_dev; */
	...
	case INNO_READ_REG:	/* no use */
		{
			...
			retval =
			    copy_from_user(reg_data, (void *)arg, sizeof(struct inno_reg_data));
			if (retval == 0) {
				/*
				   down(&inno_iic_mutex);
	...
	case INNO_SCAN_FREQUENCY:	/* if228 not support scan function  no use */
		{
			...
			retval =
			    copy_from_user(scan_area, (void *)arg,
					   sizeof(struct inno_freq_scan_area));

			if (retval == 0);	/* retval = inno_scan_frequency(scan_area->start, scan_area->end); */
			kfree(scan_area);
			break;
		}
	case INNO_SCAN_FREQUENCY_DOT:	/* if228 not support scan function no use */
		{
			...
			retval =
			    copy_from_user(scan_area, (void *)arg,
					   sizeof(struct inno_freq_scan_area));
			if (retval == 0);	/* retval = inno_scan_frequency_dot((u8)scan_area->start, (u8)scan_area->end); */
			kfree(scan_area);
			break;
		}
#endif
	...
	case INNO_GET_CHANNEL_CONFIG:
		{
			...
			retval =
			    copy_from_user(cfg, (void *)arg, sizeof(struct inno_channel_config));

			if (retval == 0)
				retval = INNO_GetChannelConfig(cfg);
			if (!retval) {
				retval =
				    copy_to_user((void *)arg, cfg,
						 sizeof(struct inno_channel_config));
				inno_msg
				    ("[lgx_ioctl] INNO_Get_CHANNEL_CONFIG:cid(%d),close(%d),start(%d),count(%d),ldpc(%d),itlv(%d),rs(%d),mod(%d) ",
				     cfg->ch_id, cfg->ch_close, cfg->ts_start, cfg->ts_count,
				     cfg->ldpc, cfg->itlv, cfg->rs, cfg->modulate);
			} else
				retval = -1;
			kfree(cfg);
			break;
	...
	case INNO_SET_CHANNEL_CONFIG:
		{
			...
			retval =
			    copy_from_user(cfg, (void *)arg, sizeof(struct inno_channel_config));
			inno_msg
			    ("[lgx_ioctl] INNO_SET_CHANNEL_CONFIG:cid(%d),close(%d),start(%d),count(%d),ldpc(%d),itlv(%d),rs(%d),mod(%d),subframeID(%d) ",
			     cfg->ch_id, cfg->ch_close, cfg->ts_start, cfg->ts_count, cfg->ldpc,
			     cfg->itlv, cfg->rs, cfg->modulate, cfg->subframe_ID);
			if (retval == 0)
	...
	case INNO_GET_SYS_STATE:
		{
			...
			retval =
			    copy_from_user(sys_state, (void *)arg, sizeof(struct inno_sys_state));

			if (retval == 0)
				retval = inno_get_system_state(sys_state);
			if (!retval)
				retval =
				    copy_to_user((void *)arg, sys_state,
						 sizeof(struct inno_sys_state));
			else
				retval = -1;
			kfree(sys_state);
			break;
	...
	case INNO_FW_DOWNLOAD:
		{
			...
			if (retval == 0) {
				for (retry = 0; retry < 1; retry++) {
					retval =
					    inno_download_fireware(fwbuf, FIRMWARE_BUF_DATA_SIZE);
					if (retval) {
						inno_err("firmware download failed retval =%d!",
							 (int)retval);
						retval = -EIO;
						continue;
					}
					...
					if (ret_tmp) {
						inno_err("INNO_CheckFWDownload fail ret =%d",
							 ret_tmp);
						retval = -1;
						continue;
					}
					if (!pass) {
						inno_err("FW Download Fail, checksum no pass =%d!",
							 pass);
					} else {
	...
	case INNO_UAM_TRANSFER:
		{
			...
			retval =
			    copy_from_user(uam_para_tmp, (void *)arg,
					   sizeof(struct inno_uam_parameter));

			if (retval == 0)
				retval = inno_uam_transfer(uam_para_tmp);
			if (!retval)
				retval =
				    copy_to_user((void *)arg, uam_para_tmp,
						 sizeof(struct inno_uam_parameter));
			else
				retval = -1;
			kfree(uam_para_tmp);
	...
	case INNO_UAM_READ_STATUS_LEN:	/* no use */
		...
		inno_dbg("INNO_UAM_READ_STATUS_LEN - return response len: 0x%x",
			 uam_para.pBufOutLen);
...
static int __init init_lgxchar(void)
{
...
#else
	ret =
	    alloc_chrdev_region(&inno_devno, innochar_minor, INNOCHAR_NR_DEVS, INNODEV_CHRDEV_NAME);
	if (ret) {
	...
	if (ret < 0) {
		inno_err
		    ("spi_register_driver fail, and then spi_register_driver unregister_chrdev\r\n");
