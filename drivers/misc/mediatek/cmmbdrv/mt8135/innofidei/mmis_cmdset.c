 ...
 *		Oct. 15, 2007
 * History:
 *		Dec. 13, 2007	----	rewrite inno_cmdset struct, and implement the functions by jimmy.li
...
static u32 mmis_get_lgx_length(u8 channelID)
{
		...
		for (j = 0; j < 500; j++) {
			...
			if (status != 0x00) {
				if (status == 0x80) {
					...
					mutex_unlock(&inno_spi_mutex);
					channel_length =
					    ((unsigned int)outBuf[2] << 8) +
					    (unsigned int)outBuf[3];
	...
	if (ret == INNO_NO_ERROR) {
		channel_length =
		    ((unsigned int)outBuf[2] << 16) | ((unsigned int)outBuf[1] << 8) | (unsigned
											int)
		    outBuf[0];
		return channel_length;
	} else {
...
static u32 mmis_get_intr_status(void)
{
	...
	/* if(intr == 0) */
	inno_msg("Warning:intr=0,int_status[0]=%d,int_status[1]=%d,int_status[2]=%d", int_status[0],
		 int_status[1], int_status[2]);
	return intr;
}
...
static u32 mmis_fetch_lgx_data(u8 channelID, u8 *pbuff, u32 len)
{
	...
	if (channelID == UAM) {
		...
		mutex_unlock(&inno_spi_mutex);
		inno_dbg("=zz= fetch_lgx_data end_0x%x,0x%x,0x%x, 0x%x", outbuf[0], outbuf[1],
			 outbuf[2], outbuf[3]);
		return 0;
	}
#if 0
	...
	case TRANS_BURST:
		...
		inno_spi_drive_cs(1);
		pr_info("mmis_fetch_lgx_data len = %d\n", len);
