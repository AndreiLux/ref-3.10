...
static void inno_intr_handler(void)
{
	...
	if (irq_wq) {
		...
		if ((irq_bug_debug >= 120) && ((irq_bug_debug % 60) == 0)) {
			inno_err("inno_intr_handler: irq worker no response, count=%d!",
				 irq_bug_debug);
			/* show_state(); */
		}
...
static void irq_work_handler(struct work_struct *_work)
{
	...
	/* else{ */
	if (!(intr_status & FW_ERR_DATA_RDY) & !(intr_status & LG2_DATA_RDY) &	/* xingyu 0922 */
	    !(intr_status & LG1_DATA_RDY) &
	    !(intr_status & LG0_DATA_RDY) & !(intr_status & UAM_DATA_RDY)) {
...
int inno_set_channel_config(struct inno_channel_config *cfg)
{
	...
	} else			/* open channel */
	{
		...
		case CFG_LGCH0:
			...
			cmd[3] = cfg->ts_count;
			cmd[4] =
			    (cfg->modulate << 6) | (cfg->rs << 4) | (cfg->itlv << 2) | cfg->ldpc;
			cmd[5] = 0;
...
int inno_get_system_state(struct inno_sys_state *state)
{
	...
	case STATTYPE_SIGNAL_STRENGTH:
...
#ifdef CMMB_DEBUG
		ret =
		    INNO_SPI_Read_WORD_Type2(FETCH_RS_TOTAL,
					     (unsigned long *)&state->statdata.rs_total_count);
		if (ret != INNO_NO_ERROR) {
		...
		ret =
		    INNO_SPI_Read_WORD_Type2(FETCH_RS_ERR,
					     (unsigned long *)&state->statdata.rs_err_count);
		if (ret != INNO_NO_ERROR) {
			inno_err("INNO_SPI_Read_WORD_Type2 fail");
			mutex_unlock(&inno_spi_mutex);
			return -1;
	...
	case STATTYPE_SIGNAL_QUALITY:
		...
		ret =
		    INNO_SPI_Read_WORD_Type2(FETCH_LDPC_TOTAL,
					     (unsigned long *)&state->statdata.ldpc_total_count);
		if (ret != INNO_NO_ERROR) {
			inno_err("INNO_SPI_Read_WORD_Type2 fail");
			mutex_unlock(&inno_spi_mutex);
			return -1;
		...
		ret =
		    INNO_SPI_Read_WORD_Type2(FETCH_LDPC_ERR,
					     (unsigned long *)&state->statdata.ldpc_err_count);
		if (ret != INNO_NO_ERROR) {
			inno_err("INNO_SPI_Read_WORD_Type2 fail");
			mutex_unlock(&inno_spi_mutex);
			return -1;
		...
		state->statdata.BER =
		    ((u32) ber_data[0]) + ((u32) (ber_data[1] << 8)) + ((u32) (ber_data[2] << 16));
...
int inno_set_frequency(u16 freq_mhz)
{
	...
	inno_msg("%s freq=%dMhz", __func__, freq_mhz);
...
int inno_get_fireware_version(struct inno_fw_ver *version)
{
	...
	if (INNO_SUCCESS != ret) {
		inno_err("%s send cmd frame failed!", __func__);
		goto out;
	}
	/* xingyu  ret = inno_i2c_read_response_frame(&resp_frame); */
	if (INNO_SUCCESS != ret) {
		inno_err("%s read response frame failed!", __func__);
		goto out;
	}
...
INNO_RET INNO_SPI_Write_Byte_Type2(unsigned long addr, unsigned char *data, int len)	/* cmd = 0x73 */
{
	...
	if (len > MT6589_CMMB_SPI_TX_MAX_PKT_LENGTH_PER_TIMES) {
		...
		for (i = 0; i < pkt_cnt; i++) {
			ptransfer[i].tx_buf =
			    data + i * MT6589_CMMB_SPI_TX_MAX_PKT_LENGTH_PER_TIMES;
	...
	if (len > CMMB_SPI_INTERFACE_MAX_PKT_LENGTH_PER_TIMES) {
		...
		if (0 != remainder) {
			transfer_1[1].tx_buf =
			    data + (CMMB_SPI_INTERFACE_MAX_PKT_LENGTH_PER_TIMES * pkt_count);
...
INNO_RET INNO_SPI_Read_Byte_Type2(unsigned long addr, unsigned char *buffer, int len)
{
	...
	if (len > CMMB_SPI_INTERFACE_MAX_PKT_LENGTH_PER_TIMES) {	/* the buffer is allocated when device open,it is 4 align */
		...
		if (0 != remainder) {
			...
			transfer_1[1].rx_buf =
			    buffer + (CMMB_SPI_INTERFACE_MAX_PKT_LENGTH_PER_TIMES * pkt_count);
			transfer_1[1].len = remainder;
			spi_message_add_tail(&transfer_1[1], &msg);
		}
...
INNO_RET INNO_SPI_Read_WORD_Type2(unsigned long addr, unsigned long *data)
{
	...
	*data =
	    (unsigned long)buffer[0] + ((unsigned long)(buffer[1] << 8)) +
...
static int UpdateCardCapabilities(void)
{
 ...
 _leave_lab:
	...
	memcpy(cardCapabilities_UAM.HistoricalChars.Buffer, atrString,
	       cardCapabilities_UAM.HistoricalChars.Length);
	...
	if ((TA2Present || (numProtocols <= 1)) && (cardCapabilities_UAM.FI == 1)
	    && (cardCapabilities_UAM.DI == 1)) {
	...
	if (OperationMode_UAM == OP_SPECIFIC) {
		if (TA2Present) {
			...
			cardCapabilities_UAM.Protocol.Selected =
			    cardCapabilities_UAM.Protocol.Supported;
		}
	} else {
...
int inno_uam_transfer(struct inno_uam_parameter *uam)
{
	...
	unsigned char cmd[270] = { 0 }, rsp[256] = {
	0};
	...
	if (bufInLen == 4) {
		...
		sw = ((unsigned short)((unsigned char)(rsp[rsplen - 1])) |
		      (((unsigned short)(unsigned char)(rsp[rsplen - 2])) << 8));
		uam->sw = sw;
...
static int INNO_Read_UAM_Rsp(unsigned char *rsp, unsigned int *rsplen)
{
	...
	while (1) {
		...
		schedule();
		/* msleep(1); */
		inno_dbg("waiting..., buffer->valid:%d, buffer->valid_size: %d", buffer->valid,
			 buffer->valid_size);
	}
