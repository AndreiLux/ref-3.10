...
int inno_irq_setup(void (*interrupthandler) (void))
{
...
#endif
	inno_msg("CMMB GPIO EINT PIN mode:num:%d, %d, dir:%d,pullen:%d,pullup%d",
	...
	mt_eint_registration(CUST_EINT_CMMB_NUM, CUST_EINT_DEBOUNCE_DISABLE, CUST_EINT_POLARITY_LOW, interrupthandler, 0);	/* 0:auto mask is no */
...
/* mt_set_gpio_pull_select(GPIO_CMMB_RST_PIN,  1); */
	inno_msg("CMMB GPIO RST PIN mode:num:%d, %d,out:%d, dir:%d,pullen:%d,pullup%d",
		 GPIO_CMMB_RST_PIN, mt_get_gpio_mode(GPIO_CMMB_RST_PIN),
		 mt_get_gpio_out(GPIO_CMMB_RST_PIN), mt_get_gpio_dir(GPIO_CMMB_RST_PIN),
		 mt_get_gpio_pull_enable(GPIO_CMMB_RST_PIN),
		 mt_get_gpio_pull_select(GPIO_CMMB_RST_PIN));
