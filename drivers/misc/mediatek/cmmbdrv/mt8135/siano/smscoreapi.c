...
static void smscore_registry_settype(char *devpath, enum sms_device_type_st type)
{
	struct smscore_registry_entry_t *entry;
...
static void list_add_locked(struct list_head *new, struct list_head *head, spinlock_t *lock)
{
	unsigned long flags;
...
int smscore_register_hotplug(hotplug_t hotplug)
{
	...
	notifyee = kmalloc(sizeof(struct smscore_device_notifyee_t), GFP_KERNEL);
	if (notifyee) {
		sms_info("notifyee");
		/* now notify callback about existing devices */
		first = &g_smscore_devices;
		for (next = first->next; next != first && !rc; next = next->next) {
			struct smscore_device_t *coredev = (struct smscore_device_t *)next;
				    ...
				    struct device *device, int arrival)
{
	...
	for (next = first->next; next != first; next = next->next) {
		rc = ((struct smscore_device_notifyee_t *)next)->hotplug(coredev, device, arrival);
		if (rc < 0)
			break;
	}
...
int smscore_register_device(struct smsdevice_params_t *params, struct smscore_device_t **coredev)
{
	...
	for (buffer = dev->common_buffer; dev->num_buffers <
	     params->num_buffers; dev->num_buffers++, buffer += params->buffer_size) {
	...
	*coredev = dev;
	...
	sms_info("core device 0x%p created, mode %d, board id %d, devpath %s", dev, dev->mode,
		 dev->board_id, dev->devpath);
					...
					void *buffer, size_t size, struct completion *completion)
{
	...
	return wait_for_completion_timeout(completion,
					   msecs_to_jiffies(SMS_PROTOCOL_MAX_RAOUNDTRIP_MS)) ? 0 :
	    -ETIME;
}
...
static int smscore_init_ir(struct smscore_device_t *coredev)
{
	...
	if (ir_io) {		/* only if IR port exist we use IR sub-module */
		...
		if (rc != 0) {
			...
			if (buffer) {
				struct SmsMsgData_ST2 *msg = (struct SmsMsgData_ST2 *)
				    SMS_ALIGN_ADDRESS(buffer);

				SMS_INIT_MSG(&msg->xMsgHeader,
					     MSG_SMS_START_IR_REQ, sizeof(struct SmsMsgData_ST2));
				msg->msgData[0] = coredev->ir.controller;
				msg->msgData[1] = coredev->ir.timeout;

				smsendian_handle_tx_message((struct SmsMsgHdr_ST2 *)msg);
				...
				sms_err("Sending IR initialization message failed");
		}
	} else
					 ...
					 void *buffer, size_t size)
{
	...
	if (coredev->mode != DEVICE_MODE_NONE) {
		...
		SMS_INIT_MSG(msg, MSG_SW_RELOAD_START_REQ, sizeof(struct SmsMsgHdr_ST));
	...
	if (coredev->mode == DEVICE_MODE_NONE) {
		struct SmsMsgData_ST *TriggerMsg = (struct SmsMsgData_ST *)msg;

		sms_debug("sending MSG_SMS_SWDOWNLOAD_TRIGGER_REQ");
		SMS_INIT_MSG(msg, MSG_SMS_SWDOWNLOAD_TRIGGER_REQ,
			     sizeof(struct SmsMsgHdr_ST) + sizeof(u32) * 5);
		...
		SMS_INIT_MSG(msg, MSG_SW_RELOAD_EXEC_REQ, sizeof(struct SmsMsgHdr_ST));
		smsendian_handle_tx_message((struct SmsMsgHdr_ST *)msg);
	 ...
	 */
	wait_for_completion_timeout(&coredev->device_ready_done, msecs_to_jiffies(400));
...
*/
	...
	fw_buf = kmalloc(ALIGN(fw->size, SMS_ALLOC_ALIGNMENT), GFP_KERNEL | GFP_DMA);
	if (!fw_buf) {
	...
	rc = (coredev->device_flags & SMS_DEVICE_FAMILY2) ?
	    ...
	    :			/* loadfirmware_handler(coredev->context, fw_buf,
				   fw_buf_size); */ pr_info("error - should not be here");
...
void smscore_unregister_device(struct smscore_device_t *coredev)
{
	...
	while (1) {
		...
		sms_info("waiting for %d buffer(s)", coredev->num_buffers - num_buffers);
		msleep(100);
	}
...
static int smscore_detect_mode(struct smscore_device_t *coredev)
{
	void *buffer = kmalloc(sizeof(struct SmsMsgHdr_ST) + SMS_DMA_ALIGNMENT,
			       GFP_KERNEL | GFP_DMA);
	struct SmsMsgHdr_ST *msg = (struct SmsMsgHdr_ST *)SMS_ALIGN_ADDRESS(buffer);
	int rc;
...
/*Stellar, NOVA A0, Nova B0, VEGA, VENICE, MING, PELE, RIO, DENVER_1530, DENVER_2160*/
	 /*DVBT*/
	    {"none", "dvb_nova_12mhz.inp", "dvb_nova_12mhz_b0.inp", "none", "none", "none", "none",
	     "dvb_rio.inp", "none", "none"},
	 /*DVBH*/ {"none", "dvb_nova_12mhz.inp", "dvb_nova_12mhz_b0.inp", "none", "none", "none",
		   "none", "dvb_rio.inp", "none", "none"},
	 /*TDMB*/ {"none", "tdmb_nova_12mhz.inp", "tdmb_nova_12mhz_b0.inp", "none", "none", "none",
		   "none", "none", "none", "tdmb_denver.inp"},
	 /*DABIP*/ {"none", "none", "none", "none", "none", "none", "none", "none", "none", "none"},
/*DVBT_BDA*/
	{"none", "dvb_nova_12mhz.inp", "dvb_nova_12mhz_b0.inp", "none", "none", "none", "none",
	 "dvb_rio.inp", "none", "none"},
	 /*ISDBT*/ {"none", "isdbt_nova_12mhz.inp", "isdbt_nova_12mhz_b0.inp", "none", "none",
		    "none", "isdbt_pele.inp", "isdbt_rio.inp", "none", "none"},
/*ISDBT_BDA*/
	{"none", "isdbt_nova_12mhz.inp", "isdbt_nova_12mhz_b0.inp", "none", "none", "none",
	 "isdbt_pele.inp", "isdbt_rio.inp", "none", "none"},
	 /*CMMB*/ {"none", "none", "none", "cmmb_vega_12mhz.inp", "cmmb_venice_12mhz.inp",
		   "cmmb_ming_app.inp", "none", "none", "none", "none"},
/*RAW - not supported*/
	{"none", "none", "none", "none", "none", "none", "none", "none", "none", "none"},
	 /*FM*/
	    {"none", "none", "fm_radio.inp", "none", "none", "none", "none", "fm_radio.inp", "none",
	     "none"},
/*FM_BDA*/
	{"none", "none", "fm_radio.inp", "none", "none", "none", "none", "fm_radio.inp", "none",
	 "none"},
	 /*ATSC*/ {"none", "none", "none", "none", "none", "none", "none", "none",
...
char *smscore_get_fw_filename(struct smscore_device_t *coredev, int mode, int lookup)
{
	...
	if (fw == NULL) {
		sms_debug
		    ("cannot find fw name in sms_boards, getting from lookup table mode %d type %d",
		     mode, type);
		return smscore_fw_lkup[mode][type];
	}
...
int smscore_init_device(struct smscore_device_t *coredev, int mode)
{
	...
	buffer = kmalloc(sizeof(struct SmsMsgData_ST) + SMS_DMA_ALIGNMENT, GFP_KERNEL | GFP_DMA);
	if (!buffer) {
	...
	SMS_INIT_MSG(&msg->xMsgHeader, MSG_SMS_INIT_DEVICE_REQ, sizeof(struct SmsMsgData_ST));
	msg->msgData[0] = mode;

	smsendian_handle_tx_message((struct SmsMsgHdr_ST *)msg);
	rc = smscore_sendrequest_and_wait(coredev, msg,
					  msg->xMsgHeader.msgLength, &coredev->init_device_done);
...
/* jackz int Len = 252 - sizeof(struct SmsMsgData_ST); */
	...
	for (i = 0; i < 10000; i++) {
		...
		for (j = 0; j < Len; j++) {
			if (pPtr[j] != (u8) (j + i)) {
				sms_err("Loopback data error at byte %u. Exp %u, Got %u", j,
					(u8) (j + i), pPtr[j]);
				/* pr_info("Loopback data error at byte %u. Exp %u, Got %u", j, (u8)(j+i), pPtr[j] ); */
...
int smscore_set_device_mode(struct smscore_device_t *coredev, int mode)
{
	...
	if (coredev->device_flags & SMS_DEVICE_FAMILY2) {
		...
		SMS_INIT_MSG(ClockMsg, MSG_SMS_NEW_CRYSTAL_REQ,
			     sizeof(struct SmsMsgHdr_ST) + sizeof(u32) * 1);
		...
		if (buffer) {
			struct SmsMsgData_ST *msg =
			    (struct SmsMsgData_ST *)SMS_ALIGN_ADDRESS(buffer);
			...
			if (rc < 0) {
				sms_err
				    ("smscore_set_device_mode, failed in set periodic stat, rc=%d\n",
				     rc);
			}
#endif
			...
			wait_for_completion_timeout(&coredev->scan_complete,
			...
			wait_for_completion_timeout(&coredev->dab_count_done,
...
int smscore_configure_board(struct smscore_device_t *coredev)
{
	...
	if (board->crystal) {
		...
		SMS_INIT_MSG(&CrysMsg.xMsgHeader, MSG_SMS_NEW_CRYSTAL_REQ, sizeof(CrysMsg));
 ...
 *
 */
static struct smscore_client_t *smscore_find_client(struct smscore_device_t *coredev, int data_type,
						    int id)
{
...
void smscore_onresponse(struct smscore_device_t *coredev, struct smscore_buffer_t *cb)
{
	...
	if (time_now - last_sample_time > 10000) {
		sms_debug("\ndata rate %d bytes/secs",
			  (int)((data_total * 1000) / (time_now - last_sample_time)));
	...
	if (rc < 0) {
		...
		case MSG_SMS_IR_SAMPLES_IND:
			sms_ir_event(coredev, (const char *)
				     ((char *)phdr
				      + sizeof(struct SmsMsgHdr_ST)),
				     (int)phdr->msgLength - sizeof(struct SmsMsgHdr_ST));
			break;
#if 1				/* JACKZ */
		...
		case MSG_SMS_GET_STATISTICS_EX_RES:
			{
				struct SmsStatisticsExRes_ST *pStatsEx =
				    (struct SmsStatisticsExRes_ST *)phdr;
				sms_debug("MSG_SMS_GET_STATISTICS_EX_RES, length: 0x%x\n",
					  (unsigned int)msgLen);
				if (pStatsEx != NULL) {
					pr_info("Reception parameters:\n");
					pr_info("Return Code: %d, ", pStatsEx->ReturnCode);
					pr_info("RF locked: %s, ",
						pStatsEx->IsRfLocked ? "Yes" : "No");
					pr_info("Demod locked: %s, ",
						pStatsEx->IsDemodLocked ? "Yes" : "No");
					pr_info("In Band Power: %d dBM, ", pStatsEx->InBandPwr);
					pr_info("SNR: %d dB, ", pStatsEx->SNR >> 16);	/* jackz, SNR needs to be right-shifted by 16 bits. */
					pr_info("Frequency: %d, ", pStatsEx->Frequency);
					pr_info("BER: %d, ", pStatsEx->BER);
		...
		case MSG_SMS_CMMB_SET_CA_CW_RES:
			{
				struct SmsMsgData_ST2 *cwRes = (struct SmsMsgData_ST2 *)phdr;
				sms_info("MSG_SMS_CMMB_SET_CA_CW_RES, RetCode: 0x%x",
					 cwRes->msgData[0]);
				break;
			}
		case MSG_SMS_CMMB_SET_CA_SALT_RES:
			{
				struct SmsMsgData_ST2 *saltRes = (struct SmsMsgData_ST2 *)phdr;
				sms_info("MSG_SMS_CMMB_SET_CA_SALT_RES, RetCode: 0x%x",
					 saltRes->msgData[0]);
				break;
			}
		...
		default:
			...
			sms_info("no client (%p) or error (%d), "
				 "type:%d dstid:%d", client, rc, phdr->msgType, phdr->msgDstId);
#endif
			break;
		}
...
int smscore_register_client(struct smscore_device_t *coredev,
			    struct smsclient_params_t *params, struct smscore_client_t **client)
{
...
void smscore_unregister_client(struct smscore_client_t *client)
{
	...
	while (!list_empty(&client->idlist)) {
		struct smscore_idlist_t *identry = (struct smscore_idlist_t *)client->idlist.next;
...
int smscore_map_common_buffer(struct smscore_device_t *coredev, struct vm_area_struct *vma)
{
	...
	if ((end - start) != size) {
		{
			sms_err("invalid size %d expected %d", (int)(end - start), (int)size);
		}
		return -EINVAL;
	}
	...
	if (remap_pfn_range
	    (vma, vma->vm_start, ((unsigned int)phy) >> PAGE_SHIFT, size,
...
static int GetGpioPinParams(u32 PinNum, u32 *pTranslatedPinNum, u32 *pGroupNum, u32 *pGroupCfg)
{
			   ...
			   struct smscore_gpio_config *pGpioConfig)
{
	...
	buffer = kmalloc(totalLen + SMS_DMA_ALIGNMENT, GFP_KERNEL | GFP_DMA);
	if (!buffer)
		return -ENOMEM;
	...
	if (!(coredev->device_flags & SMS_DEVICE_FAMILY2)) {
		...
		if (GetGpioPinParams(PinNum, &TranslatedPinNum, &GroupNum, &groupCfg) != 0)
			return -EINVAL;
...
int smscore_gpio_set_level(struct smscore_device_t *coredev, u8 PinNum, u8 NewLevel)
{
	...
	if ((NewLevel > 1) || (PinNum > MAX_GPIO_PIN_NUMBER) || (PinNum > MAX_GPIO_PIN_NUMBER))
		return -EINVAL;

	totalLen = sizeof(struct SmsMsgHdr_ST) + (3 * sizeof(u32));	/* keep it 3 ! */
	...
	/* Send message to SMS */
	smsendian_handle_tx_message((struct SmsMsgHdr_ST *)pMsg);
	rc = smscore_sendrequest_and_wait(coredev, pMsg, totalLen, &coredev->gpio_set_level_done);
...
int smscore_gpio_get_level(struct smscore_device_t *coredev, u8 PinNum, u8 *level)
{
	...
	/* Send message to SMS */
	smsendian_handle_tx_message((struct SmsMsgHdr_ST *)pMsg);
	rc = smscore_sendrequest_and_wait(coredev, pMsg, totalLen, &coredev->gpio_get_level_done);
...
static void __exit smscore_module_exit(void)
{
	...
	while (!list_empty(&g_smscore_notifyees)) {
		struct smscore_device_notifyee_t *notifyee = (struct smscore_device_notifyee_t *)
	...
	while (!list_empty(&g_smscore_registry)) {
		struct smscore_registry_entry_t *entry = (struct smscore_registry_entry_t *)
