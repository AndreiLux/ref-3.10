...
#  define PERROR(fmt, args...) \
	pr_err("spibus error: line %d- %s(): " fmt, __LINE__,\
	  __func__, ## args)
#undef PWARNING
#  define PWARNING(fmt, args...) \
	pr_warn("spibus warning: line %d- %s(): " fmt, __LINE__,  \
	__func__, ## args)

/* the debug macro - conditional compilation from the makefile */
/* #define SPIBUS_DEBUG                                                      //xingyu add */
#undef PDEBUG			/* undef it, just in case */
#ifdef SPIBUS_DEBUG
#  define PDEBUG(fmt, args...) \
	pr_debug("spibus: line %d- %s(): " fmt, __LINE__, \
	 __func__, ## args)
#else
