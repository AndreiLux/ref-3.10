...
\return	error status
*/
static unsigned long dma_map_buf(struct spiphy_dev_s *spiphy_dev, char *buf, int len, int direction)
{
	...
	if (len > CMMB_SPI_INTERFACE_MAX_PKT_LENGTH_PER_TIMES) {
		...
		if (0 != remainder) {
			transfer[1].tx_buf =
			    (txbuf ==
			     NULL) ? NULL : txbuf +
			    (CMMB_SPI_INTERFACE_MAX_PKT_LENGTH_PER_TIMES * pkt_count);
			transfer[1].rx_buf =
			    (rxbuf ==
			     NULL) ? NULL : rxbuf +
			    (CMMB_SPI_INTERFACE_MAX_PKT_LENGTH_PER_TIMES * pkt_count);
			...
			unsigned long rxbuf_phy_addr, int len)
{
	...
	if (txbuf)		/* command & firmware */
	{
		...
		if (len > 32) {
			...
			for (i = 0; i < pkt_cnt; i++) {
				smsspibus_xfer(context, txbuf_temp, txbuf_phy_addr, rxbuf_temp,
					       rxbuf_phy_addr, 32);
				txbuf_temp += 32;
				rxbuf_temp += 32;
			}
...
void smschipEintEnable(void)
{
	...
	/* register irp */
	mt_eint_set_sens(CUST_EINT_CMMB_NUM, CUST_EINT_EDGE_SENSITIVE);
	mt_eint_registration(CUST_EINT_CMMB_NUM, CUST_EINT_DEBOUNCE_DISABLE, CUST_EINT_POLARITY_HIGH, intHander, 0);	/* 1202 */
#ifdef CONFIG_ARCH_MT6575
	mt_eint_mask(CUST_EINT_CMMB_NUM);
	sms_info("[smsspiphy_init]CMMB GPIO EINT PIN mode: %d, dir:%d\n",
		 mt_get_gpio_mode(GPIO_CMMB_EINT_PIN), mt_get_gpio_dir(GPIO_CMMB_EINT_PIN));
...
void smschipEintDisable(void)
{
	...
	sms_info("CMMB GPIO EINT PIN mode:num:%d, %d, dir:%d,pullen:%d,pullup%d",
		 GPIO_CMMB_EINT_PIN, mt_get_gpio_mode(GPIO_CMMB_EINT_PIN),
		 mt_get_gpio_dir(GPIO_CMMB_EINT_PIN), mt_get_gpio_pull_enable(GPIO_CMMB_EINT_PIN),
		 mt_get_gpio_pull_select(GPIO_CMMB_EINT_PIN));
#endif
}
...
void *smsspiphy_init(void *context, void (*smsspi_interruptHandler) (void), void *intr_context)
{
	...
	pr_info("smsspi_interruptHandler =%p\n", smsspi_interruptHandler);
	...
	spiphy_dev->rx_dma_channel =
	    pxa_request_dma("spibusdrv_rx", DMA_PRIO_HIGH, spibus_dma_handler, spiphy_dev);
	...
	spiphy_dev->tx_dma_channel =
	    pxa_request_dma("spibusdrv_tx", DMA_PRIO_HIGH, spibus_dma_handler, spiphy_dev);
