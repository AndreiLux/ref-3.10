...
static void sms_ir_rc5_event(struct smscore_device_t *coredev, u32 toggle, u32 addr, u32 cmd)
{
	bool toggle_changed;
	u16 keycode;

	sms_info("IR RC5 word: address %d, command %d, toggle %d", addr, cmd, toggle);

	toggle_changed = ir_toggle != toggle;
	/* keep toggle */
	ir_toggle = toggle;

	if (addr != keyboard_layout_maps[coredev->ir.ir_kb_type].rc5_kbd_address)
		return;		/* Check for valid address */

	keycode = keyboard_layout_maps[coredev->ir.ir_kb_type].keyboard_layout_map[cmd];

	if (!toggle_changed && (keycode != KEY_VOLUMEUP && keycode != KEY_VOLUMEDOWN))
...
#define RC5_INSTR(x)    ((x)&0x3F)
	...
	sms_ir_rc5_event(coredev, RC5_TOGGLE(rc5_word), RC5_ADDR(rc5_word), RC5_INSTR(rc5_word));
}


static void sms_rc5_accumulate_bits(struct smscore_device_t *coredev, s32 ir_sample)
{
...
#define IR_DATA_RECEIVE_MAX_LEN	520	/* 128*4 + 4 + 4 */
	u32 i;
	enum ir_protocol ir_protocol = keyboard_layout_maps[coredev->ir.ir_kb_type]
	    .ir_protocol;
	s32 *samples;
	int count = len >> 2;
...
int sms_ir_init(struct smscore_device_t *coredev)
{
	...
	coredev->ir.ir_kb_type = sms_get_board(smscore_get_board_id(coredev))->ir_kb_type;
	coredev->ir.keyboard_layout_map =
	    keyboard_layout_maps[coredev->ir.ir_kb_type].keyboard_layout_map;
	sms_info("IR remote keyboard type is %d", coredev->ir.ir_kb_type);

	coredev->ir.controller = 0;	/* Todo: vega/nova SPI number */
	coredev->ir.timeout = IR_DEFAULT_TIMEOUT;
	sms_info("IR port %d, timeout %d ms", coredev->ir.controller, coredev->ir.timeout);

	snprintf(coredev->ir.name,
		 IR_DEV_NAME_MAX_LEN, "SMS IR w/kbd type %d", coredev->ir.ir_kb_type);
