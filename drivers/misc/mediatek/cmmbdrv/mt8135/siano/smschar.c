 ...
 * @return 0 on success, <0 on error.
 */
int smschar_send_fw_file(struct smschar_device_t *dev, struct smschar_send_fw_file_ioctl_t *up)
{
	int rc = 0;
...
#if 0				/* buf issue */
	coredev->fw_buf = kmalloc(ALIGN(up->fw_size, SMS_ALLOC_ALIGNMENT), GFP_KERNEL | GFP_DMA);
#else
 ...
 */
static ssize_t smschar_read(struct file *file, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	while (!list_empty(&dev->pending_data) && (copied < count)) {
		struct smscore_buffer_t *cb = (struct smscore_buffer_t *)dev->pending_data.next;
		int actual_size = min(((int)count - copied), cb->size);
		if (copy_to_user(&buf[copied], &((char *)cb->p)[cb->offset], actual_size)) {
 ...
 * @return 0 on success, <0 on error.
 */
static int smschar_wait_get_buffer(struct smschar_device_t *dev, struct smschar_buffer_t *touser)
{
	unsigned long flags;
	int rc;
 ...
 *	    <0 on error.
 */
static unsigned int smschar_poll(struct file *file, struct poll_table_struct *wait)
{
	struct smschar_device_t *dev;
	int events = 0;
	...
	case SMSCHAR_IS_DEVICE_PNP_EVENT:
		{
			...
			wait_event_interruptible(g_pnp_event, !g_pnp_status_changed);
	...
	case SMSCHAR_SEND_FW_FILE:
		{
			...
			return smschar_send_fw_file(dev, 0);
		}
 ...
 * @return 0 on success, <0 on error.
 */
static int smschar_hotplug(struct smscore_device_t *coredev, struct device *device, int arrival)
{
	int rc = 0, i;
...
{
	pr_info("SMSDev_probe start\r\n");
	SMSDev = kzalloc(sizeof(*SMSDev), GFP_KERNEL);
	if (!SMSDev) {
		pr_info("[Siano_CharDriver]SMSDev_probe   kzalloc fail \r\n");
		return -ENOMEM;
	}
