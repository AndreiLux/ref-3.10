...
static struct _rx_buffer_st *smsspi_handle_unused_bytes_buf(struct _spi_dev *dev,
							    struct _rx_buffer_st *buf, int offset,
							    int len, int unused_bytes)
{
	...
	if (!tmp_buf) {
		...
		/* pr_info("smsspi_handle_unused_bytes_buf > Failed to allocate RX buffer.\n"); */
		return NULL;
	}
	if (unused_bytes > 0) {
		...
		int align_block =
		    (((unused_bytes + SPI_PACKET_SIZE -
		       1) >> SPI_PACKET_SIZE_BITS) << SPI_PACKET_SIZE_BITS);
		memset(tmp_buf->ptr, 0, align_block - unused_bytes);
		memcpy((char *)tmp_buf->ptr +
		       (align_block - unused_bytes),
		       (char *)buf->ptr + offset + len - unused_bytes, unused_bytes);
	}
	return tmp_buf;
}
	...
	for (i = 0; i < len; i++, ptr++) {
		...
		case RxsData:
			...
			if (recieved_bytes < padded_msg_len) {
				*unused_bytes = 0;
				*missing_bytes = padded_msg_len - recieved_bytes;
				return buf;
			}
			dev->rxState = RxsWait_a5;
			if (dev->cb.msg_found_cb) {
				...
				PRN_DBG((TXT("Msg found and sent to callback func.\n")));
				/* pr_info("Msg found and sent to callback func.\n"); */
				...
				if (msg_offset & 0x3) {
					...
					memmove((unsigned char *)
						...
						+ msg_offset, (unsigned char *)
						(dev->rxPacket.msg_buf->ptr)
						+ dev->rxPacket.msg_offset,
						dev->rxPacket.msg_len - align_fix);
				}

				*unused_bytes =
				    len + offset - dev->rxPacket.msg_offset - dev->rxPacket.msg_len;
				...
				dev->cb.msg_found_cb(dev->context,
						     ...
						     msg_offset, dev->rxPacket.msg_len - align_fix);
				*missing_bytes = 0;
				return buf;
			} else {
				PRN_DBG((TXT
...
void smsspi_common_transfer_msg(struct _spi_dev *dev, struct _spi_msg *txmsg, int padding_allowed)
{
	...
	offset = 0;
	unused_bytes = 0;
	buf = dev->cb.allocate_rx_buf(dev->context, RX_PACKET_SIZE + SPI_PACKET_SIZE * 2);
	if (!buf) {
		PRN_ERR((TXT("Failed to allocate RX buffer.\n")));
		/* pr_info("smsspi_common_transfer_msg > Failed to allocate RX buffer.\n"); */
		return;
	}
	while (bytes_to_transfer || unused_bytes) {
		...
		tmp_buf =
		    smsspi_common_find_msg(dev, buf, offset, len, &unused_bytes, &missing_bytes);
		...
		if (bytes_to_transfer < missing_bytes) {
			...
			/* pr_info("a message was found, adding bytes to transfer, txmsg %d, total %d\n", tx_bytes, bytes_to_transfer); */
		}
	}
	dev->cb.free_rx_buf(dev->context, buf);
}
		      ...
		      struct _spi_dev_cb_st *cb)
{
	...
	if (cb->transfer_data_cb == 0 ||
	    cb->msg_found_cb == 0 || cb->allocate_rx_buf == 0 || cb->free_rx_buf == 0) {
