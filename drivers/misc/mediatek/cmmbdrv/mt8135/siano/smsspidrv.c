...
#ifdef CONFIG_SMP
static struct workqueue_struct *spi_thread_wq;
	...
	do {
		...
		if (!msg && !list_empty(&spi_device->txqueue))	/* ÅÐ¶ÏÁ´±íÊÇ·ñÎª¿Õ */
			msg = (struct _smsspi_txmsg *)
			    list_entry(spi_device->txqueue.next, struct _smsspi_txmsg, node);
		...
		if (msg) {
			if (msg->add_preamble) {
				...
				memcpy(&txmsg.buf[sizeof(smsspi_preamble)],
				       msg->buffer, txmsg.len - sizeof(smsspi_preamble));
				msg->add_preamble = 0;
				msg->buffer += txmsg.len - sizeof(smsspi_preamble);
...
static void msg_found(void *context, void *buf, int offset, int len)
{
	...
	struct smscore_buffer_t *cb =
	    (struct smscore_buffer_t *)(container_of(buf, struct smscore_buffer_t, p));
...
static void free_rx_buf(void *context, struct _rx_buffer_st *buf)
{
	struct _spi_device_st *spi_device = (struct _spi_device_st *)context;
	struct smscore_buffer_t *cb =
	    (struct smscore_buffer_t *)(container_of(((void *)buf), struct smscore_buffer_t, p));
...
int smsspi_register(void)
{
	...
	spi_device->txbuf =
	    dma_alloc_coherent(NULL, TX_BUFFER_SIZE,
			       &spi_device->txbuf_phy_addr, GFP_KERNEL | GFP_DMA);
#else
	...
	pr_info("**** spi_dev->txbuf = 0x %4x", (unsigned int)spi_dev->txbuf);
#endif
	if (!spi_device->txbuf) {
	...
	spi_device->phy_dev = smsspiphy_init(NULL, smsspi_int_handler, (void *)spi_device);
	if (spi_device->phy_dev == 0) {
		pr_info("%s smsspiphy_init(...) failed\n", __func__);
		/* goto phy_error; */
	}
	...
	if (ret < 0) {
		pr_info("%s smscore_register_device(...) failed\n", __func__);
		goto reg_device_error;
	}

	ret = smscore_start_device(spi_device->coredev);
	if (ret < 0) {
		pr_info("%s smscore_start_device(...) failed\n", __func__);
