...
unsigned int emmc_get_wp_size(void)
{
	...
	if (0 == sg_wp_size) {
		...
		pr_info("[%s:%d]mmc_host is : 0x%p\n", __func__, __LINE__, host_ctl->mmc);
		/* pr_info("[%s:%d]claim host done! claim_status = %d, claim_cnt = %d, claimer = 0x%x, current = 0x%x\n", __func__, __LINE__, */
...
unsigned int emmc_otp_start(void)
{
	...
	pr_info("find OTP start address is 0x%x\n", l_addr);
...
unsigned int emmc_otp_query_length(unsigned int *QLength)
{
	...
	/* 30May2013 Chaotian Jing modified */
	*QLength = g_otp_user_ccci - emmc_get_wp_size();
...
#if EMMC_OTP_DEBUG
	pr_info("EMMC_OTP: start MSDC_SINGLE_READ_WRITE!\n");
#endif
			 ...
			 unsigned int length, unsigned int *status)
{
	...
	pr_info("[%s]: %s (0x%x) length:(%d bytes) !\n", __func__, access_type ? "WRITE" : "READ",
		offset, length);
	...
	do {
		...
		if (FS_EMMC_OTP_READ == access_type) {
			...
			*status = Status;
			...
			if (OTP_SUCCESS != Status) {
				pr_info("[%s]: read before write, Read status (%d) blkno (0x%x)\n",
...
static long mt_otp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	/*
	   ...
	   {
	   pr_info("ERROR: EMMC Flash Not initialized !!\n");
	   ret = -EFAULT;
	   goto exit;
	   } */
	...
	case EMMC_OTP_GET_LENGTH:
		pr_info("OTP IOCTL: EMMC_OTP_GET_LENGTH\n");
		...
		pr_info("OTP IOCTL: The Length is %d\n", otpctl.QLength);
		break;
	case EMMC_OTP_READ:
		pr_info("OTP IOCTL: EMMC_OTP_READ Offset(0x%x), Length(0x%x)\n", otpctl.Offset,
			otpctl.Length);
		...
		if (copy_to_user(otpctl.BufferPtr, pbuf, (sizeof(char) * otpctl.Length))) {
			pr_info("EMMC_OTP IOCTL: Copy to user buffer Error !\n");
			goto error;
		}
		break;
	case EMMC_OTP_WRITE:
		pr_info("OTP IOCTL: EMMC_OTP_WRITE Offset(0x%x), Length(0x%x)\n", otpctl.Offset,
			otpctl.Length);
		if (copy_from_user(pbuf, otpctl.BufferPtr, (sizeof(char) * otpctl.Length))) {
			pr_info("EMMC_OTP IOCTL: Copy from user buffer Error !\n");
			goto error;
		}
...
static int emmc_otp_probe(struct platform_device *pdev)
{
	...
	pr_info("in emmc otp probe function\n");
...
static int __init emmc_otp_init(void)
{
	...
	pr_info("MediaTek EMMC OTP misc driver init\n");
	...
	if (entry == NULL) {
		pr_info("emmc OTP: unable to create /proc entry\n");
		return -ENOMEM;
	}
	...
	pr_info("OTP: register EMMC OTP device ...\n");
	err = misc_register(&emmc_otp_dev);
	if (unlikely(err)) {
		pr_info("OTP: failed to register EMMC OTP device!\n");
		return err;
	}
	return 0;
}
...
{
	pr_info("MediaTek EMMC OTP misc driver exit\n");
