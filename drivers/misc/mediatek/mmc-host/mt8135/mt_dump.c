...
static u32 u_msdc_base[HOST_MAX_NUM] = {
	MSDC_0_BASE, MSDC_1_BASE, MSDC_2_BASE, MSDC_3_BASE, MSDC_4_BASE };
#else
static u32 u_msdc_base[HOST_MAX_NUM] = {
	MSDC_1_BASE, MSDC_0_BASE, MSDC_2_BASE, MSDC_3_BASE, MSDC_4_BASE };
#endif
...
static void simp_msdc_dump_register(struct simp_msdc_host *host)
{
	...
	pr_emerg("Reg[00] MSDC_CFG       = 0x%.8x", sdr_read32(base + 0x00));
	pr_emerg("Reg[04] MSDC_IOCON     = 0x%.8x", sdr_read32(base + 0x04));
	pr_emerg("Reg[08] MSDC_PS        = 0x%.8x", sdr_read32(base + 0x08));
	pr_emerg("Reg[0C] MSDC_INT       = 0x%.8x", sdr_read32(base + 0x0C));
	pr_emerg("Reg[10] MSDC_INTEN     = 0x%.8x", sdr_read32(base + 0x10));
	pr_emerg("Reg[14] MSDC_FIFOCS    = 0x%.8x", sdr_read32(base + 0x14));
	pr_emerg("Reg[18] MSDC_TXDATA    = not read");
	pr_emerg("Reg[1C] MSDC_RXDATA    = not read");
	pr_emerg("Reg[30] SDC_CFG        = 0x%.8x", sdr_read32(base + 0x30));
	pr_emerg("Reg[34] SDC_CMD        = 0x%.8x", sdr_read32(base + 0x34));
	pr_emerg("Reg[38] SDC_ARG        = 0x%.8x", sdr_read32(base + 0x38));
	pr_emerg("Reg[3C] SDC_STS        = 0x%.8x", sdr_read32(base + 0x3C));
	pr_emerg("Reg[40] SDC_RESP0      = 0x%.8x", sdr_read32(base + 0x40));
	pr_emerg("Reg[44] SDC_RESP1      = 0x%.8x", sdr_read32(base + 0x44));
	pr_emerg("Reg[48] SDC_RESP2      = 0x%.8x", sdr_read32(base + 0x48));
	pr_emerg("Reg[4C] SDC_RESP3      = 0x%.8x", sdr_read32(base + 0x4C));
	pr_emerg("Reg[50] SDC_BLK_NUM    = 0x%.8x", sdr_read32(base + 0x50));
	pr_emerg("Reg[58] SDC_CSTS       = 0x%.8x", sdr_read32(base + 0x58));
	pr_emerg("Reg[5C] SDC_CSTS_EN    = 0x%.8x", sdr_read32(base + 0x5C));
	pr_emerg("Reg[60] SDC_DATCRC_STS = 0x%.8x", sdr_read32(base + 0x60));
	pr_emerg("Reg[70] EMMC_CFG0      = 0x%.8x", sdr_read32(base + 0x70));
	pr_emerg("Reg[74] EMMC_CFG1      = 0x%.8x", sdr_read32(base + 0x74));
	pr_emerg("Reg[78] EMMC_STS       = 0x%.8x", sdr_read32(base + 0x78));
	pr_emerg("Reg[7C] EMMC_IOCON     = 0x%.8x", sdr_read32(base + 0x7C));
	pr_emerg("Reg[80] SD_ACMD_RESP   = 0x%.8x", sdr_read32(base + 0x80));
	pr_emerg("Reg[84] SD_ACMD19_TRG  = 0x%.8x", sdr_read32(base + 0x84));
	pr_emerg("Reg[88] SD_ACMD19_STS  = 0x%.8x", sdr_read32(base + 0x88));
	pr_emerg("Reg[90] DMA_SA         = 0x%.8x", sdr_read32(base + 0x90));
	pr_emerg("Reg[94] DMA_CA         = 0x%.8x", sdr_read32(base + 0x94));
	pr_emerg("Reg[98] DMA_CTRL       = 0x%.8x", sdr_read32(base + 0x98));
	pr_emerg("Reg[9C] DMA_CFG        = 0x%.8x", sdr_read32(base + 0x9C));
	pr_emerg("Reg[A0] SW_DBG_SEL     = 0x%.8x", sdr_read32(base + 0xA0));
	pr_emerg("Reg[A4] SW_DBG_OUT     = 0x%.8x", sdr_read32(base + 0xA4));
	pr_emerg("Reg[B0] PATCH_BIT0     = 0x%.8x", sdr_read32(base + 0xB0));
	pr_emerg("Reg[B4] PATCH_BIT1     = 0x%.8x", sdr_read32(base + 0xB4));
	pr_emerg("Reg[E0] SD20_PAD_CTL0  = 0x%.8x", sdr_read32(base + 0xE0));
	pr_emerg("Reg[E4] SD20_PAD_CTL1  = 0x%.8x", sdr_read32(base + 0xE4));
	pr_emerg("Reg[E8] SD20_PAD_CTL2  = 0x%.8x", sdr_read32(base + 0xE8));
	pr_emerg("Reg[EC] PAD_TUNE       = 0x%.8x", sdr_read32(base + 0xEC));
	pr_emerg("Reg[F0] DAT_RD_DLY0    = 0x%.8x", sdr_read32(base + 0xF0));
	pr_emerg("Reg[F4] DAT_RD_DLY1    = 0x%.8x", sdr_read32(base + 0xF4));
	pr_emerg("Reg[F8] HW_DBG_SEL     = 0x%.8x", sdr_read32(base + 0xF8));
	pr_emerg("Rg[100] MAIN_VER       = 0x%.8x", sdr_read32(base + 0x100));
	pr_emerg("Rg[104] ECO_VER        = 0x%.8x", sdr_read32(base + 0x104));
}
...
static unsigned int simp_mmc_enable_clk(struct simp_mmc_host *host)
{
	...
	while (clock_is_on(sdr_clksrc(host->mtk_host->id)))
		disable_clock(sdr_clksrc(host->mtk_host->id), "SD");
	enable_clock(sdr_clksrc(host->mtk_host->id), "SD");
	/* if (!clock_is_on(sdr_clksrc(host->mtk_host->id))) { */
	/* pr_err("[MT_DUMP] enable msdc%d clock\n", host->mtk_host->id); */
	/* if (enable_clock(sdr_clksrc(host->mtk_host->id), "SD")) { */
	/* pr_err("[MT_DUMP] msdc%d on clock failed ===> retry once\n", host->mtk_host->id); */
	/* disable_clock(sdr_clksrc(host->mtk_host->id), "SD"); */
	/* enable_clock(sdr_clksrc(host->mtk_host->id), "SD"); */
	/* } */
	/* } */
...
static unsigned int simp_mmc_hw_reset_for_init(struct simp_mmc_host *host)
{
	...
	sdr_write32(0xf123007c, EMMC_IOCON_BOOTRST);

	/* tRSTW (RST_n pulse width) at least 1us */
	mdelay(1);

	/* set n_reset pin to high */
	sdr_write32(0xf123007c, 0);

	sdr_write32(0xf1230000, sdr_read32(0xf1230000) | MSDC_CFG_CKPDN);
			 ...
			 unsigned int arg, int rsptyp, unsigned int *resp)
{
	...
	if (intsts & MSDC_INT_CMDRDY) {
#if MTK_MMC_DUMP_DBG
		pr_emerg("msdc cmd<%d> arg<0x%x> Ready \r\n", cmd, arg);
#endif
		/* get the response */
		switch (rsptyp) {
		case RESP_NONE:
		...
		pr_emerg("msdc cmd<%d> arg<0x%x> error(0x%x)\n", cmd, arg, intsts);
		error = 1;
		goto end;
	}
...
static int simp_mmc_read_ext_csd(struct simp_mmc_host *host, struct simp_mmc_card *card)
{
	...
	if (simp_ext_csd[162] != 0x1)
		pr_err("ERROR: RST=0x%x\n", simp_ext_csd[162]);
...
static int simp_emmc_switch_bus(struct simp_mmc_host *host, struct simp_mmc_card *card)
{
	struct simp_msdc_host *phost = host->mtk_host;
	unsigned int resp;
	return simp_msdc_cmd(phost, ACMD6, ACMD6_RAW, ACMD6_ARG_EMMC, RESP_R1B, &resp);
}
...
static int simp_mmc_sd_init(struct simp_mmc_host *host)
{
	...
	if (simp_msdc_cmd(phost, CMD8, CMD8_RAW, CMD8_ARG, RESP_R7, &resp)) {
		...
		pr_info("SD v1.0, clr HCS bit\n");
		ACMD41_ARG = ACMD41_ARG_10;
	} else if (resp == CMD8_ARG) {
		pr_info("SD v2.0, set HCS bit\n");
	...
	while (retry--) {
		...
		if (resp & R3_OCR_POWER_UP_BIT) {
			...
			if (phost->card->card_cap == standard_capacity) {
				pr_info("just standard_capacity card!!\r\n");
			}
			break;
		}
...
static int simp_msdc_pio_write(struct simp_msdc_host *host, unsigned int *ptr, unsigned int size)
{
	...
	while (1) {
		...
		if (status & MSDC_INT_DATCRCERR) {
			...
			pr_info("[MSDC%d] DAT TMO error (0x%x), Left DAT: %d bytes\n",
				host->id, status, left);
			err = -110;
			break;
				...
				unsigned int dev)
{
	...
	if (PartInfo[l_dest_num].size < (len + offset)) {
		pr_info("write operation oversize!\n");
		return ret;
	}
#if MTK_MMC_DUMP_DBG
...
#if MTK_MMC_DUMP_DBG
	pr_info("l_start_offset = 0x%x len = %d buf<0x%x>\n", l_addr, len, l_buf);
#endif
...
/* unsigned int i; */
	...
	int err = 0;
#if MTK_MMC_DUMP_DBG
	pr_info("1 l_start_offset = 0x%x len =0x\n", l_addr, len);
#endif
	if (0 != len % 512) {
		pr_info("debug: parameter error!\n");
		return ret;
...
#if MTK_MMC_DUMP_DBG
	pr_info("l_start_offset = 0x%x len = %d\n", offset, len);
#endif
	if (pmsdc_extend_host->card->card_cap == standard_capacity) {
		l_addr = offset << 9;
	} else {
...
int card_dump_func_write(unsigned char *buf, unsigned int len, unsigned long long offset, int dev)
{
...
#endif
	if (offset % 512) {
		pr_info("Address isn't 512 alignment!\n");
		return SIMP_FAILED;
	}
	...
	case DUMP_INTO_EXTERN_CARD:
		...
		break;
	default:
		pr_info("unknown card type, error!\n");
		break;
	}
