...
#endif
static void msdc_check_mpu_voilation(void *data, u32 addr, int wr_vio)
{
	int index = 0;
	int status = -1;
...
static void msdc_register_emi_mpu_callback(int id)
{
	...
	case 0:
	case 4:
		emi_mpu_notifier_register(MST_ID_MMPERI_1, msdc_check_mpu_voilation, 0);
		pr_crit("msdc%d register emi_mpu_notifier\n", id);
		break;
	case 2:
		emi_mpu_notifier_register(MST_ID_MMPERI_2, msdc_check_mpu_voilation, 0);
		pr_crit("msdc%d register emi_mpu_notifier\n", id);
		break;
	default:
		break;
