...
#define ISP_REG_RANGE		(0x10000)		/* same as isp_reg.h and page-aligned */
/* seninf driver */
#define SENINF_BASE_ADDR	0x15008000		/* same as seninf_drv.cpp(chip-dependent) */
#define SENINF_REG_RANGE	(0x1000)		/* same as seninf_reg.h and page-aligned */
#define PLL_BASE_ADDR		0x10000000		/* same as seninf_drv.cpp(chip-dependent) */
#define PLL_RANGE			(0x1000)		/* same as seninf_drv.cpp and page-aligned */
#define MIPIRX_CONFIG_ADDR	0x1500C000		/* same as seninf_drv.cpp(chip-dependent) */
#define MIPIRX_CONFIG_RANGE	(0x1000)		/* same as seninf_drv.cpp and page-aligned */
#define MIPIRX_ANALOG_ADDR	0x10012000		/* same as seninf_drv.cpp(chip-dependent) */
#define MIPIRX_ANALOG_RANGE	(0x1000)
#define GPIO_BASE_ADDR		0x10005000		/* same as seninf_drv.cpp(chip-dependent) */
...
static void ISP_EnableClock(MBOOL En)
{
	...
	if (En) {		/* Enable clock. */
		...
		/*clkmux_sel(MT_CLKMUX_SMI_SEL, MT_CG_V_CLKPH_MCK, "CAMERA");*/		/* Remove, since SMI clock is set by preloader */
...
static MINT32 ISP_ReadReg(ISP_REG_IO_STRUCT *pRegIo)
{
	...
	for (i = 0; i < pRegIo->Count; i++) {
		...
		if ((ISP_ADDR_CAMINF + reg.Addr >= ISP_ADDR) &&
			(ISP_ADDR_CAMINF + reg.Addr < (ISP_ADDR_CAMINF+ISP_REG_RANGE)))
...
static MINT32 ISP_WriteRegToHw(ISP_REG_STRUCT *pReg, MUINT32 Count)
{
	...
	for (i = 0; i < Count; i++) {
		...
		if (((ISP_ADDR_CAMINF + pReg[i].Addr) >= ISP_ADDR) &&
			((ISP_ADDR_CAMINF + pReg[i].Addr) < (ISP_ADDR_CAMINF+ISP_REG_RANGE)))
...
/* spin_unlock_irqrestore(&(IspInfo.SpinLock), flags); */
	...
	if (copy_from_user(&ref_cnt_ctrl, (void *)Param, sizeof(ISP_REF_CNT_CTRL_STRUCT)) == 0) {
		...
		if (ISP_REF_CNT_ID_MAX > ref_cnt_ctrl.id) {
			...
			if ((imem_ref_cnt == 0) &&
			(ref_cnt_ctrl.ctrl == ISP_REF_CNT_DEC_AND_RESET_IF_LAST_ONE)) {
...
static MINT32 ISP_SOF_Buf_Get(unsigned long long sec, unsigned long usec)
{
	...
	if (0 == pstRTBuf->dropCnt) {
		...
		if (TempAddr == curr_pa) {
			LOG_WRN("TempAddr:Last(0x%08X) == Cur(0x%08X)", TempAddr, curr_pa);
			/* ISP_DumpReg(); */
		}
		TempAddr = curr_pa;
...
/* LOG_DBG("- E."); */
	...
	if (IrqStatus[ISP_IRQ_TYPE_INT] & ISP_IRQ_INT_STATUS_TG1_ERR_ST ||
		...
		LOG_WRN("[rtbc]Status(0x%08X), TG_ERR", IrqStatus[ISP_IRQ_TYPE_INT]);
	}
	/*  */
	if (IrqStatus_fbc_int) {
		LOG_DBG("[rtbc]dropframe st(0x%08X),co(0x%08X),c2o(0x%08X)", IrqStatus_fbc_int,
			ISP_RD32(ISP_REG_ADDR_IMGO_FBC), ISP_RD32(ISP_REG_ADDR_IMG2O_FBC));
	}
...
{
	MUINT32 i = 0;
	seq_puts(s, " MT6589 ISP Register\n");
	seq_puts(s, "====== top ====\n");
