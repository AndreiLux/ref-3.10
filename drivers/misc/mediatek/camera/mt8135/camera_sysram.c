...
static void SYSRAM_DumpLayout(void)
{
	...
	for (Index = 0; Index < SYSRAM_MEM_BANK_AMOUNT; Index++) {
		...
		LOG_DMP("[pid tgid] [Proc Name / Owner Name]\n");
		...
		while (NULL != pCurrNode) {
			...
			if (SYSRAM_IsBadOwner(User)) {
				...
				SYSRAM_USER_STRUCT * const pUserInfo = &Sysram.UserInfo[User];
			LOG_DMP("%5lu.%06lu %2d\t0x%05lX 0x%05lX  %ld    %p %p\t%p  %-4d %-4d \"%s\" / \"%s\"\n",
					pUserInfo->TimeS,
					pUserInfo->TimeUS,
					User,
					pCurrNode->Offset,
...
/* ------------------------------------------------------------------------------ */
static int SYSRAM_DumpLayoutToProc(struct seq_file *s, void *v)
{
	MUINT32 Index = 0;
	SYSRAM_MEM_NODE_STRUCT *pCurrNode = NULL;
	/*  */
	...
	seq_puts(s, "\n[SYSRAM_DumpLayoutToProc]\n");
	seq_printf(s, "AllocatedTbl = 0x%08lX\n", Sysram.AllocatedTbl);
	seq_puts(s, "=========================================\n");
	for (Index = 0; Index < SYSRAM_MEM_BANK_AMOUNT; Index++) {
		seq_printf(s, "\n [Mem Pool %ld] (IndexTbl, UserCount)=(%lX, %ld)\n",
			      Index,
...
static int SYSRAM_DumpOpen(struct inode *inode, struct file *file)
{
	return single_open(file, SYSRAM_DumpLayoutToProc, NULL);
}


/* ------------------------------------------------------------------------------ */
static int SYSRAM_ReadFlag(struct seq_file *s, void *v)
{
	/*  */
	seq_puts(s, "\r\n[SYSRAM_ReadFlag]\r\n");
	seq_puts(s, "=========================================\r\n");
	seq_printf(s, "Sysram.DebugFlag = 0x%08lX\r\n", Sysram.DebugFlag);

	return 0;
}
static int SYSRAM_ReadFlagOpen(struct inode *inode, struct file *file)
{
	return single_open(file, SYSRAM_ReadFlag, NULL);
}
