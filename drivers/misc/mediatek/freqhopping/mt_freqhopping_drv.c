...
#define FH_MEM_PLL		(g_p_fh_hal_drv->mempll)
#define FH_MAIN_PLL		(g_p_fh_hal_drv->mainpll)
#define FH_MSDC_PLL		(g_p_fh_hal_drv->msdcpll)
#define FH_MM_PLL		(g_p_fh_hal_drv->mmpll)
#define FH_VENC_PLL		(g_p_fh_hal_drv->vencpll)
#define FH_LVDS_PLL		(g_p_fh_hal_drv->lvdspll)
...
static int mt_fh_enable_usrdef(struct freqhopping_ioctl *fh_ctl)
{
	...
	memcpy(&g_fh_drv_usr_def[fh_ctl->pll_id], &(fh_ctl->ssc_setting),
...
static ssize_t freqhopping_dramc_proc_write(struct file *file, const char *buffer, size_t count,
					    loff_t *data)
{
	return (ssize_t) (g_p_fh_hal_drv->proc.dramc_write);
}
