...
/* current used uart port name, default is "ttyMT2", will be changed when wmt driver init */
...
static int _mt_combo_plt_do_deep_idle(COMBO_IF src, int enter)
{
	...
	case COMBO_IF_MSDC:
		if (enter == 0) {
			...
			CMB_STUB_LOG_DBG("MSDC interface mode, exit deep idle\n");
		} else {
			/* set_device_working_ability(MT65XX_PDN_PERI_MSDC2, DEEP_IDLE_STATE); */
			CMB_STUB_LOG_DBG("MSDC interface mode, enter deep idle\n");
		}
		ret = 0;
		break;
...
{
	CMB_STUB_LOG_DBG("set current consys chipid (0x%x)\n", chipid);
