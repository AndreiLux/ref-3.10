...
#ifndef MTK_WCN_SOC_CHIP_SUPPORT
...
{
	WMT_DETECT_DBG_FUNC("open major %d minor %d (pid %d)\n",
...
{
	WMT_DETECT_DBG_FUNC("close major %d minor %d (pid %d)\n",
			     imajor(inode), iminor(inode), current->pid);
...
static long wmt_detect_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	WMT_DETECT_DBG_FUNC("cmd (%d),arg(%ld)\n", cmd, arg);
...
int wmt_detect_ext_chip_detect(void)
{
	...
	if (0 == bgfEintStatus) {
		...
		/*combo chip exists */
		WMT_DETECT_DBG_FUNC("external combo chip detected");

		/*detect chipid by sdio_detect module */
		chipId = sdio_detect_query_chipid(1);
		if (0 <= hif_sdio_is_chipid_valid(chipId))
			WMT_DETECT_DBG_FUNC("valid external combo chip id (0x%x)\n", chipId);
		else
			WMT_DETECT_ERR_FUNC("invalid external combo chip id (0x%x)\n", chipId);
...
static int wmt_detect_probe(struct platform_device *pdev)
{
	...
	int cdevErr = -1;
	int ret = -1;
	struct mtk_wifi_power *pdata = dev_get_platdata(&pdev->dev);

	if (!pdata)
		return -EINVAL;

	if (wmt_detect_power_probe(pdata))
		return -EFAULT;
	...
	WMT_DETECT_DBG_FUNC("driver(major %d) installed success\n", gWmtDetectMajor);
...
/*deinit SDIO-DETECT module*/
	...
	WMT_DETECT_DBG_FUNC("done\n");
	return 0;
}

struct platform_driver wmt_detect_driver = {
	.probe = wmt_detect_probe,
	.remove = wmt_detect_remove,
	.driver.name = "mtk-wmt-detect"
};
