...
/* CONFIG_DRV_CDP_IN ispParam; */
	...
	case GFMT_IOCTL_CONFIG_FMT:
		{
			...
			if (!fmtParam.gfLinkGDMA && fmtParam.go_flag) {
				...
				GFMT_MSG("config GFMT driver go_flag %d!!\n", fmtParam.go_flag);
				regVal =
				    (regVal & ~REG_ISP_STR_CTL_GDMA_MASK) | (fmtOnly <<
									     REG_ISP_STR_CTL_GDMA_SHIFT);
	...
	case GFMT_IOCTL_WAIT:
		...
		if (outParams.timeout & 0x01) {
			...
			if (timeout == 0)
				GFMT_MSG("Polling GFMT Status TIMEOUT!!\n");
		} else {
			GFMT_MSG("Polling GFMT Status, (%ld), L:%d!!", timeout_jiff, __LINE__);
			wait_event_interruptible_timeout(gfmt_wait_queue, _gfmt_only_int_status,
							 timeout_jiff);
		}
#endif
	...
	case GFMT_IOCTL_START:
			...
			pr_info
			    ("Trigger ISP: %d, Link GFMT: %d,FMT %d, Enable GFMT: %d, YUV_DMA %d!!\n",
	...
	case GFMT_IOCTL_CONFIG_ISP:
			{
				pr_info("GFMT ISP Config\n");
				if (copy_from_user
				    (&ispParam, (void *)arg, sizeof(CONFIG_DRV_CDP_IN))) {
					pr_info("Copy from user failed\n");
					return -EFAULT;
				}
				...
				pr_info("ISP modify: %d\n", ispParam.numModify);
				for (idx = 0; idx < ispParam.numModify; idx++) {
					pr_info("%08x %08x\n", ispParam.modifyOffset[idx],
						ispParam.modifyValue[idx]);
					wait_pr();

					write_reg(ispParam.modifyOffset[idx],
						  ispParam.modifyValue[idx]);
				}
			}
			break;
...
#endif
	default:
		pr_info("GFMT NO THIS IOCTL COMMAND, %d!!\n", cmd);
		break;
	}
	return 0;
...
static int gfmt_ioctl_from_dec(unsigned int cmd, unsigned long arg, struct file *file)
{
	...
	case GFMT_IOCTL_RESUME:
		...
		gfmt_drv_dec_set_dst_bank0(dec_row_params.decRowBuf[0], dec_row_params.decRowBuf[1],
...
/* #if 1 */
	...
	case GFMT_IOCTL_DEINIT:
		...
		if (fmt_status == 0) {
			pr_info("GDMA_CTL status is available, HOW COULD THIS HAPPEN ??");
			*pStatus = 0;
			return -EFAULT;
		}
		gfmt_drv_deinit();
...
#endif
	default:
		pr_info("GFMT DEC IOCTL NO THIS COMMAND\n");
		break;
	}
	return 0;
}
...
static int gfmt_probe(struct platform_device *pdev)
{
	...
	class_dev =
	  (struct class_device *)device_create(gfmt_class, NULL, gfmt_devno, NULL, GFMT_DEVNAME);
#else
