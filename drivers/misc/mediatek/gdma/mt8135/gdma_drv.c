...
/* static int GDMA_Ioctl(struct inode * a_pstInode, struct file * file, unsigned int cmd, unsigned long arg) */
{
	...
	case GDMA_IOCTL_DEINIT:
		...
		if (*pStatus != GDMA_CTL_PROCESS) {
			pr_info("Permission Denied! This process can not access GDMA_CTL");
			return -EFAULT;
		}
	...
	case GDMA_IOCTL_START:{
				...
				pr_info
				    (" Link GDMA: %d, Trigger ISP: %d, FMT %d, Enable GDMA: %d, YUV_DMA %d!!\n",
	...
	default:
		pr_info("GDMA NO THIS IOCTL COMMAND, %d!!\n", cmd);
		break;
	}
	return 0;
}
...
static int gdma_probe(struct platform_device *pdev)
{
	...
	class_dev =
		(struct class_device *)device_create(gdma_class, NULL, gdma_devno, NULL, GDMA_DEVNAME);
 #else
