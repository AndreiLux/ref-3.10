...
/* dump FMT register */
	...
	pr_info("  GDMA_FMT Register Dump\n\r");
	index = 0xF5004160;
	regValue = 0x00008502;
	*(volatile unsigned int *)(index) = regValue;
	pr_info("SET addr %03x = %08x\n", index, regValue);
	wait_pr();

	index = 0xF5004164;
	regValue = *(volatile unsigned int *)(index);
	pr_info("GET addr %03x = %08x\n", index, regValue);
	wait_pr();
...
void gfmt_drv_set_io_type(unsigned int isLinkGDMA, unsigned int isInterlaced,
			  unsigned int isTopField, unsigned int fixValue)
{
	/* fixValue = 0x0f ; */
	REG_GDMA_FMT_IOTYPE =
	    (isLinkGDMA << 17) | (isInterlaced << 4) | (isTopField << 2) | ((fixValue & 0x0f) << 8);
...
void gfmt_drv_set_ultra_mode(unsigned int ultra_value)
{
	...
	REG_GDMA_FMT_ULTRA_MODE = ultra_value;
}
...
void gfmt_drv_set_dram_burst_length(unsigned int burst)
{
	...
	REG_GDMA_FMT_MIFMODE = (burst << REG_GDMA_BST_LIM_SHIFT);
}
...
void gfmt_drv_set_input_video_row_height(unsigned int height_Y, unsigned int height_C)
{
	...
	REG_GDMA_FMT_MCUMODE =
	    ((REG_GDMA_MCUMODE_MCUVFAC_MASK & height_Y) << 8) |
	    ((REG_GDMA_MCUMODE_MCUVFAC_MASK & height_C) << 4);
}
...
void gfmt_drv_set_input_webp_row_height(unsigned int height_Y, unsigned int height_C)
{
	...
	REG_GDMA_FMT_WEBPMODE =
	    (height_Y << REG_GDMA_FMT_WEBPMODE_MCUY_SHIFT) | (height_C <<
							      REG_GDMA_FMT_WEBPMODE_MCUC_SHIFT);

}



void gfmt_drv_set_pixel_range(unsigned int rangeMapY, unsigned int rangeMapC,
			      unsigned int rangeReduceEn, unsigned int rangeMapEn)
{


	REG_GDMA_FMT_RANGE =
	    (rangeMapY << 16) | (rangeMapC << 8) | (rangeReduceEn << 4) | rangeMapEn;
}

void gfmt_drv_set_src_mem_stride(unsigned int stride_Y, unsigned int stride_UV)
{

	REG_GDMA_FMT_SRCBUFL =
	    (stride_UV << REG_GDMA_SRCBUFL_C_SHIFT) | (stride_Y & REG_GDMA_SRCBUFL_MASK);
...
void gfmt_drv_set_src_buffer(unsigned int addr_Y, unsigned int addr_UV)
{
	...
	REG_GDMA_FMT_YSRCB1 = addr_Y;
	REG_GDMA_FMT_CBSRCB1 = addr_UV;
...
void gfmt_drv_set_dst_mem_stride(unsigned int stride_Y, unsigned int stride_UV)
{
	...
	REG_GDMA_FMT_TGBUFL =
	    (stride_UV << REG_GDMA_TGTBUFL_C_SHIFT) | (REG_GDMA_TGTBUFL_MASK & stride_Y);
...
void gfmt_drv_set_dst_bank0(unsigned int addr_Y, unsigned int addr_U, unsigned int addr_V)
{
	...
	REG_GDMA_FMT_YTGB1 = addr_Y;
	REG_GDMA_FMT_CBTGB1 = addr_U;
	REG_GDMA_FMT_CRTGB1 = addr_V;
...
void gfmt_drv_set_dst_bank1(unsigned int addr_Y, unsigned int addr_U, unsigned int addr_V)
{
	...
	REG_GDMA_FMT_YTGB2 = addr_Y;
	REG_GDMA_FMT_CBTGB2 = addr_U;
	REG_GDMA_FMT_CRTGB2 = addr_V;
...
void gfmt_drv_set_src_image_info(unsigned int width, unsigned int height)
{
	...
	REG_GDMA_FMT_YSRCSZ = (width << YSRCSZ_HI_SHIFT) | (height & YSRCSZ_LO_MASK);
...
void gfmt_drv_set_fmt_only(unsigned int isFmtOnly)
{
	...
	REG_GDMA_FMT_INTEN |= ((1 << 8) | (isFmtOnly << 4));	/* set write one clear */
...
int gfmt_config_fmt(GDMA_DRV_FMT_IN param)
{
	...
	gfmt_drv_set_io_type(param.gfLinkGDMA, param.fieldCompactOuput, param.isTopField,
			     param.io_fix_value /*0x0f */);
	...
	gfmt_drv_set_dst_bank0(param.gfDstBufBank0[0], param.gfDstBufBank0[1],
			       param.gfDstBufBank0[2]);

	gfmt_drv_set_dst_bank1(param.gfDstBufBank1[0], param.gfDstBufBank1[1],
			       param.gfDstBufBank1[2]);

	gfmt_drv_set_dst_mem_stride(param.gfDstBufStride_Y, param.gfDstBufStride_C);

	gfmt_drv_set_pixel_range(param.rangeMapY, param.rangeMapC, param.rangeReduceEn,
				 param.rangeMapEn);
