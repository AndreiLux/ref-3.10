...
int gdma_reset_isp(void)
{
	...
	if (u4Timeout == 0) {
		pr_info("GDMA Reset Failed\n");
		return -1;
	}


	pr_info("GDMA SW/HW Reset Done\n");
...
/* VP8     (93~96):  0x15016974 ~ 0x15016980 */
	...
	pr_info("=======================================\n\r");
	pr_info("   VDEC_TOP Register Dump\n\r");
...
void gdma_drv_set_ctl_mode(unsigned int isJPEG, unsigned int is422, unsigned int isGray,
			   unsigned int CbCrValue)
{

	REG_GDMA_CTL_IOTYPE = (CbCrValue << 8) | (isGray << 5) | (is422 << 3) | isJPEG;

}



void gdma_drv_set_src_buf_height(unsigned int y_h, unsigned int c_h, unsigned int y_h_lst,
				 unsigned int c_h_lst, unsigned int y_h_fst, unsigned int c_h_fst)
{

	REG_GDMA_CTL_MCUMODE = (y_h << 28) | (c_h << 24)
...
void gdma_drv_set_crop_offset(unsigned int initOffset_Y, unsigned int offset_Y,
			      unsigned int initOffset_C, unsigned int offset_C)
{
	REG_GDMA_CTL_SRCOFST_Y = (initOffset_Y << 16) | offset_Y;
	REG_GDMA_CTL_SRCOFST_C = (initOffset_C << 16) | offset_C;
...
void gdma_drv_set_src_bank0(unsigned int addr_Y, unsigned int addr_U, unsigned int addr_V)
{
	...
	REG_GDMA_CTL_YSRCB1 = addr_Y;
	REG_GDMA_CTL_CBSRCB1 = addr_U;
	REG_GDMA_CTL_CRSRCB1 = addr_V;
...
void gdma_drv_set_src_bank1(unsigned int addr_Y, unsigned int addr_U, unsigned int addr_V)
{
	...
	REG_GDMA_CTL_YSRCB2 = addr_Y;
	REG_GDMA_CTL_CBSRCB2 = addr_U;
	REG_GDMA_CTL_CRSRCB2 = addr_V;
...
void gdma_drv_set_src_image_info(unsigned int width, unsigned int height)
{
	...
	REG_GDMA_CTL_YSRCSZ = (width << 16) | (height);
...
void gdma_config_ctl(GDMA_DRV_CTL_IN *cfgCTL)
{
	...
	gdma_drv_set_ctl_mode(cfgCTL->isSrcJpeg, cfgCTL->gtSrcIs422, cfgCTL->isSrcGray,
			      cfgCTL->cbcrConstant);
	...
	gdma_drv_set_src_buf_height(cfgCTL->gtSrcBufHeight[0], cfgCTL->gtSrcBufHeight[1],
				    cfgCTL->gtSrcBufLastHeight[0], cfgCTL->gtSrcBufLastHeight[1],
				    cfgCTL->gtSrcBufFstHeight[0], cfgCTL->gtSrcBufFstHeight[1]);

	gdma_drv_set_crop_offset(cfgCTL->cropY_initOff, cfgCTL->cropY_Off, cfgCTL->cropC_initOff,
				 cfgCTL->cropC_Off);
...
void gdma_start(unsigned char gdmaLink, unsigned char startISP, unsigned char FMT_en,
		unsigned char colorFormat)
{
	...
	regVal =
	    (regVal & ~REG_ISP_CTL_SEL_GDMA_LNK_MASK) | (gdmaLink <<
							 REG_ISP_CTL_SEL_GDMA_LNK_SHIFT);
