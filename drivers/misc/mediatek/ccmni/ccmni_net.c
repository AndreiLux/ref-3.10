...
static int ccmni_receive(ccmni_instance_t *ccmni, int length)
{
	...
	do {
		...
		packet_info = pfp_unframe(ccmni->decode_buffer + offset_put_pkt,
					  CCCI_TTY_RX_BUFFER_SIZE - offset_put_pkt,
					  ccmni->read_buffer + offset_parse_frame, length,
					  ccmni->channel);
		...
		for (counter = 0; counter < packet_info.num_complete_packets; counter++) {
			...
			if (skb) {
				...
				memcpy(skb_put(skb, packet->pkt_size), packet->pkt_data,
				       packet->pkt_size);
				ccmni_make_etherframe(skb->data - ETH_HLEN, ccmni->dev->dev_addr,
						      packet_type);
		...
		if (packet_info.try_decode_again == 1) {
			offset_put_pkt +=
			    (last_packet.pkt_data - ccmni->decode_buffer + last_packet.pkt_size);
...
static int ccmni_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	frame_info =
	    pfp_frame(ccmni->write_buffer, skb->data, skb->len, FRAME_START, ccmni->channel);
