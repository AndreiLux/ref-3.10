...
static long tz_client_reg_sharedmem(struct file *file, unsigned long arg)
{
	...
	ret = kree_register_sharedmem(session, &mem_handle, (uint32_t) pin->start, pin->size,
				      (uint32_t) map_p, sizeof(uint32_t)*(pin->nrPages + 1));
	if (ret != TZ_RESULT_SUCCESS) {
...
TEE_update_pr_time_infile(securetime_session, mem_session);
...
ret = KREE_CreateSession(TZ_TA_MEM_UUID, &mem_session);
TEE_update_pr_time_intee(securetime_session, mem_session);
ret = KREE_CloseSession(securetime_session);
if (ret != TZ_RESULT_SUCCESS)
