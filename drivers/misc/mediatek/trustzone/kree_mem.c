...
TZ_RESULT kree_register_sharedmem(KREE_SESSION_HANDLE session, KREE_SHAREDMEM_HANDLE *mem_handle,
				  uint32_t start, uint32_t size, uint32_t map_p, uint32_t map_size)
{
	MTEEC_PARAM p[4];
	TZ_RESULT ret;
	...
	p[0].value.a = start;
	p[1].value.a = size;
	p[2].mem.buffer = (void *)map_p;
	p[2].mem.size = map_size;
				 ...
				 KREE_SHAREDMEM_HANDLE *shm_handle, KREE_SHAREDMEM_PARAM *param)
{
	...
	if (((uint32_t) param->buffer >= PAGE_OFFSET) &&
		...
		ret = kree_register_sharedmem(session, shm_handle,
			(uint32_t) param->buffer, (uint32_t) param->size,
			 0, 0);	/* set 0 for no remap... */
		if (ret != TZ_RESULT_SUCCESS) {
#ifdef DBG_KREE_MEM
