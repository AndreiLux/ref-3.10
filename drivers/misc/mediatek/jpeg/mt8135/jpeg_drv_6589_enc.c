...
#else
		reg_value = ioread32((void *)(JPEG_ENC_BASE + index));
#endif
...
{
	REG_JPEG_ENC_CTRL |= JPEG_ENC_EN_DIS_GMC;
}


void jpeg_drv_enc_set_burst_type(kal_uint32 burst_type)
{
	REG_JPEG_ENC_CTRL &= ~JPEG_ENC_CTRL_BURST_TYPE_MASK;
	REG_JPEG_ENC_CTRL |= (burst_type << JPEG_ENC_CTRL_BURST_TYPE_SHIFT_COUNT);
}
