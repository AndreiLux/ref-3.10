...
void jpeg_drv_dec_power_on(void)
{
	...
	enable_clock(MT_CG_ISP_JPGDEC_SMI_CKPDN, "JPEG");
	enable_clock(MT_CG_ISP_JPGDEC_JPG_CKPDN, "JPEG");
#if 0
	/* MCI */
...
void jpeg_drv_dec_power_off(void)
{
	disable_clock(MT_CG_ISP_JPGDEC_SMI_CKPDN, "JPEG");
	disable_clock(MT_CG_ISP_JPGDEC_JPG_CKPDN, "JPEG");
#ifdef FOR_COMPILE
	BOOL ret;
...
{
	/* REG_JPEG_MM_REG_MASK  = 0; */
	enable_clock(MT_CG_ISP_JPGENC_SMI_CKPDN, "JPEG");
	enable_clock(MT_CG_ISP_JPGENC_JPG_CKPDN, "JPEG");
#ifdef FOR_COMPILE
	BOOL ret;
...
void jpeg_drv_enc_power_off(void)
{
	disable_clock(MT_CG_ISP_JPGENC_SMI_CKPDN, "JPEG");
	disable_clock(MT_CG_ISP_JPGENC_JPG_CKPDN, "JPEG");
#ifdef FOR_COMPILE
	BOOL ret;
...
void jpeg_reg_dump(void)
{
	...
	for (index = 0; index < 0x168; index += 4) {
		reg_value = ioread32((void *)(JPG_CODEC_BASE + index));
		JPEG_WRN("+0x%x 0x%x\n", index, reg_value);
	}
}
...
static int jpeg_dec_ioctl(unsigned int cmd, unsigned long arg, struct file *file)
{
	...
	case JPEG_DEC_IOCTL_RESUME:
		/* JPEG_MSG("[JPEGDRV][IOCTL] JPEG Decoder Resume!!\n"); */
		if (*pStatus != JPEG_DEC_PROCESS) {
			JPEG_MSG
			    ("[JPEGDRV]Permission Denied! This process can not access decoder\n");
			return -EFAULT;
	...
	case JPEG_DEC_IOCTL_START:	/* OT:OK */
		...
		/* Debug: pr_info("0xF0: 0x%08x\n", *(volatile unsigned int*)(JPEG_DEC_BASE + 0xF0)); */

		jpeg_drv_dec_start();
		break;

	case JPEG_DEC_IOCTL_WAIT:
		JPEG_MSG("[JPEGDRV][IOCTL] JPEG Decoder Enter IRQ Wait!!\n");
...
/* #if 1 */
		...
		if (jpeg_isr_dec_lisr() < 0) {
			...
			wait_event_interruptible_timeout(dec_wait_queue, _jpeg_dec_int_status,
							 timeout_jiff);
			/* JPEG_MSG("[JPEGDRV]wait JPEG irq done\n"); */
		} else {
			/* JPEG_MSG("[JPEGDRV]JPEG decoder already done\n"); */
		}
#endif
	...
	case JPEG_DEC_IOCTL_DEINIT:
		...
		if (*pStatus != JPEG_DEC_PROCESS) {
			pr_info("Permission Denied! This process can not access encoder");
			return -EFAULT;
		}

		if (dec_status == 0) {
			pr_info("Encoder status is available, HOW COULD THIS HAPPEN ??");
			*pStatus = 0;
			return -EFAULT;
		}
		jpeg_drv_dec_deinit();
...
#endif
	default:
		pr_info("JPEG DEC IOCTL NO THIS COMMAND\n");
		break;
	}
	return 0;
}
...
static int jpeg_probe(struct platform_device *pdev)
{
	...
	/* mt6575_irq_set_sens(MT6575_JPEG_CODEC_IRQ_ID, MT_LEVEL_SENSITIVE); */
	/* mt6575_irq_set_polarity(MT6575_JPEG_CODEC_IRQ_ID, MT_POLARITY_LOW); */
	/* mt6575_irq_unmask(MT6575_JPEG_CODEC_IRQ_ID); */
	/*JPEG_MSG("request JPEG Encoder IRQ\n");*/
	enable_irq(MT8135_JPEG_ENC_IRQ_ID);
	if (request_irq
	    (MT8135_JPEG_ENC_IRQ_ID, jpeg_drv_enc_isr, IRQF_TRIGGER_LOW, "jpeg_enc_driver", NULL))
	{
		JPEG_ERR("JPEG ENC Driver request irq failed\n");
	}
	enable_irq(MT8135_JPEG_DEC_IRQ_ID);
	/*JPEG_MSG("request JPEG Decoder IRQ\n");*/
	/* if(request_irq(MT8135_JPEG_DEC_IRQ_ID, jpeg_drv_dec_isr, IRQF_TRIGGER_LOW, "jpeg_dec_driver" , NULL)) */
	/* if(request_irq(MT8135_JPEG_DEC_IRQ_ID, jpeg_drv_dec_isr, IRQF_TRIGGER_RISING , "jpeg_dec_driver" , NULL)) */
	if (request_irq
	    (MT8135_JPEG_DEC_IRQ_ID, jpeg_drv_dec_isr, IRQF_TRIGGER_FALLING, "jpeg_dec_driver",
	     NULL)) {
...
#ifndef FPGA_VERSION
	free_irq(MT8135_JPEG_ENC_IRQ_ID, NULL);
	free_irq(MT8135_JPEG_DEC_IRQ_ID, NULL);
#endif
	JPEG_MSG("Done\n");
	return 0;
}
