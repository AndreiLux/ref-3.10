...
int notrace aed_unwind_frame(struct stackframe *frame, unsigned long stack_address)
{
	...
	if (high != stack_address) {
		pr_info("%s: sp base(%lx) not equal to process stack base(%lx)\n", __func__, low,
			stack_address);
		return -EINVAL;
	}
	...
	if ((fp < thread_info) || (fp >= (stack_address - 4))) {
		pr_info("%s: fp(%lx) out of process stack base(%lx)\n", __func__, fp,
			stack_address);
		return -EINVAL;
	}
			       ...
			       unsigned int stack_address)
{
	...
	for (count = 0; count < MAX_EXCEPTION_FRAME; count++) {
		...
		snprintf(bt->entries[bt->nr_entries].pc_symbol, MAX_AEE_KERNEL_SYMBOL, "%pS",
			 (void *)current_stk.pc);
		snprintf(bt->entries[bt->nr_entries].lr_symbol, MAX_AEE_KERNEL_SYMBOL, "%pS",
			 (void *)current_stk.lr);
