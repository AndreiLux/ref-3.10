...
*/
static int IPANIC_OOPS_BLOCK_COUNT;
...
static int mtd_ipanic_block_read_single(struct mtd_ipanic_data *ctx, loff_t offset)
{
	...
	rc = ctx->mtd->_read(ctx->mtd, ctx->blk_offset[index], ctx->mtd->writesize, &len,
			     ctx->bounce);
	...
	if (rc == -EUCLEAN) {
		xlog_printk(ANDROID_LOG_WARN, IPANIC_LOG_TAG,
			    "ECC Check sum error corrected %lld\n", offset);
		rc = 0;
	}
...
static int mtd_ipanic_block_read(struct mtd_ipanic_data *ctx, off_t file_offset, int file_length,
				 void *buf)
{
#if 0
	...
	while (file_length > 0) {
		...
		if (rc < 0) {
			xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
				    "aee-ipanic(%s): mtd read error page_no(%d) error(%d)\n",
				    __func__, page_no, rc);
			goto error_return;
		}
		if (page_offset)
...
static void mtd_ipanic_block_erase(void)
{
	...
	for (i = 0; i < ctx->mtd->size; i += ctx->mtd->erasesize) {
		...
		if (rc < 0) {
			xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
				    "aee-ipanic: Bad block check " "failed (%d)\n", rc);
			goto out;
		}
		if (rc) {
			xlog_printk(ANDROID_LOG_WARN, IPANIC_LOG_TAG,
				...
				off_t off, int count, int *eof, void *data)
{
	...
	if (oops) {
		len +=
		    sprintf(page + len, "Module: %s\nProcess\n%s\nBacktrace\n%s", oops->module,
			    oops->process_path, oops->backtrace);
...
static void mtd_panic_notify_add(struct mtd_info *mtd)
{
	...
	if (rc < 0) {
		xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
			    "aee-ipanic: Error reading block 0 (%d)\n", rc);
		mtd_ipanic_block_erase();
		goto out_err;
	}

	xlog_printk(ANDROID_LOG_INFO, IPANIC_LOG_TAG,
		    "aee-ipanic: Bound to mtd partition '%s', write size(%d), write size shift(%d)\n",
...
static struct aee_oops *mtd_ipanic_oops_copy(void)
{
	...
	if (oops != NULL) {
		struct ipanic_oops_header *oops_header =
		    kzalloc(sizeof(struct ipanic_oops_header), GFP_KERNEL);
		if (oops_header == NULL)
			goto error_return;

		if (mtd_ipanic_block_read
		    (ctx, ctx->curr.oops_header_offset, ctx->curr.oops_header_length,
		     oops_header) != 0) {
			xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
				    "%s: mtd read header failed\n", __func__);
			kfree(oops_header);
			goto error_return;
		}
		...
		if (ctx->curr.oops_detail_length != 0) {
			...
			if (oops->detail != NULL) {
				if (mtd_ipanic_block_read
				    (ctx, ctx->curr.oops_detail_offset,
				     ctx->curr.oops_detail_length, oops->detail) != 0) {
					xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
						    "%s: mtd read detail failed\n", __func__);
					kfree(oops->detail);
					goto error_return;
				}
			}
...
#define TMPDETAILSTR  "panic detail is empty"
		...
		if (oops->console != NULL) {
			if (mtd_ipanic_block_read
			    (ctx, ctx->curr.console_offset, ctx->curr.console_length,
			     oops->console) != 0) {
				xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
					    "%s: mtd read console failed\n", __func__);
				kfree(oops->detail);
				goto error_return;
			}
		} else {
		...
		if (oops->android_main) {
			if (mtd_ipanic_block_read
			    (ctx, ctx->curr.android_main_offset, ctx->curr.android_main_length,
			     oops->android_main) != 0) {
				xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
					    "%s: mtd read android_main failed\n", __func__);
				kfree(oops->detail);
				kfree(oops->console);
				goto error_return;
			}
			...
			xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
				    "%s: kmalloc failed at android_main\n", __func__);
			aee_oops_free(oops);
			return NULL;
		}
		...
		if (oops->android_radio) {
			if (mtd_ipanic_block_read
			    (ctx, ctx->curr.android_radio_offset, ctx->curr.android_radio_length,
			     oops->android_radio) != 0) {
				xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
					    "%s: mtd read android_radio failed\n", __func__);
				kfree(oops->detail);
				kfree(oops->console);
			...
			xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
				    "%s: kmalloc failed at android_radio\n", __func__);
			aee_oops_free(oops);
			return NULL;
		}
		...
		if (oops->android_system) {
			if (mtd_ipanic_block_read
			    (ctx, ctx->curr.android_system_offset, ctx->curr.android_system_length,
			     oops->android_system) != 0) {
				xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
					    "%s: mtd read android_system failed\n", __func__);
				kfree(oops->detail);
				kfree(oops->console);
				kfree(oops->android_main);
			...
			xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
				    "%s: kmalloc failed at android_system\n", __func__);
			aee_oops_free(oops);
			return NULL;
		}
...
#if 0
		xlog_printk(ANDROID_LOG_INFO, IPANIC_LOG_TAG,
			    "android log length, 0x%x, 0x%x, 0x%x, 0x%x\n", oops->android_main_len,
			    oops->android_event_len, oops->android_radio_len,
			    oops->android_system_len);
#endif
		/* Process dump */
		...
		if (oops->userspace_info) {
			if (mtd_ipanic_block_read
			    (ctx, ctx->curr.userspace_info_offset, ctx->curr.userspace_info_length,
			     oops->userspace_info) != 0) {
				xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
					    "%s: mtd read usrespace info failed\n", __func__);
				kfree(oops->detail);
				kfree(oops->console);
				kfree(oops->android_main);
				kfree(oops->android_radio);
			...
			xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
				    "%s: kmalloc failed at userspace info failed\n", __func__);
			aee_oops_free(oops);
			return NULL;
		}
...
static int ipanic_write_all_android_buf(struct mtd_ipanic_data *ctx, int offset,
					struct ipanic_header *hdr)
{
	...
	if (rc < 0) {
		xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
			    "Error writing android log(1) to panic log (%d)\n", rc);
		rc = 0;
	}
	...
	if (rc < 0) {
		xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
			    "Error writing android log(2) to panic log (%d)\n", rc);
		rc = 0;
	}
	...
	if (rc < 0) {
		xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
			    "Error writing android log(3) to panic log (%d)\n", rc);
		rc = 0;
	}
	...
	if (rc < 0) {
		xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
			    "Error writing android log(4) to panic log! (%d)\n", rc);
		rc = 0;
	}
...
static int mtd_ipanic(struct notifier_block *this, unsigned long event, void *ptr)
{
	struct mtd_ipanic_data *ctx = &mtd_drv_ctx;
	...
	current_hdr.oops_header_length =
	    mtd_ipanic_write_oops_header(ctx->mtd, current_hdr.oops_header_offset);
	if (current_hdr.oops_header_length < 0) {
		xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
			    "Error writing oops header to panic log! (%d)\n",
			    current_hdr.oops_header_length);
		current_hdr.oops_header_length = 0;
	}

	current_hdr.oops_detail_offset =
	    ALIGN(current_hdr.oops_header_offset + current_hdr.oops_header_length,
		  ctx->mtd->writesize);
	current_hdr.oops_detail_length =
	    ipanic_write_oops_detail(ctx->mtd, current_hdr.oops_detail_offset);
	if (current_hdr.oops_detail_length < 0) {
		xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
			    "Error writing detail to panic log! (%d)\n",
			    current_hdr.oops_detail_length);
		current_hdr.oops_detail_length = 0;
	}

	/* Kernel buffer log */
	current_hdr.console_offset =
	    ALIGN(current_hdr.oops_detail_offset + current_hdr.oops_detail_length,
		  ctx->mtd->writesize);
	...
	current_hdr.userspace_info_offset =
	    ALIGN(current_hdr.console_offset + current_hdr.console_length, ctx->mtd->writesize);
	current_hdr.userspace_info_length =
	    ipanic_write_userspace(ctx, current_hdr.userspace_info_offset);
	...
	rc = ipanic_write_all_android_buf(ctx,
					  current_hdr.userspace_info_offset +
					  current_hdr.userspace_info_length, &current_hdr);
	...
	xlog_printk(ANDROID_LOG_ERROR, IPANIC_LOG_TAG,
		    "aee-ipanic: Panic dump sucessfully written to flash\n");
...
int __init aee_mtd_ipanic_init(void)
{
	...
	xlog_printk(ANDROID_LOG_INFO, IPANIC_LOG_TAG,
		    "aee-mtd-ipanic: startup, partition assgined %s\n", AEE_IPANIC_PLABEL);
	return 0;
}
