...
int ipanic_header_check(const struct ipanic_header *hdr)
{
	...
	if (hdr->magic != AEE_IPANIC_MAGIC) {
		xlog_printk(ANDROID_LOG_INFO, IPANIC_LOG_TAG,
			    "aee-ipanic: No panic data available [Magic header]\n");
		g_is_panic = 0;
		return 1;
	}
	...
	if (hdr->version != AEE_IPANIC_PHDR_VERSION) {
		xlog_printk(ANDROID_LOG_INFO, IPANIC_LOG_TAG,
			    "aee-ipanic: Version mismatch (%d != %d)\n", hdr->version,
			    AEE_IPANIC_PHDR_VERSION);
		return 2;
	}
...
void ipanic_header_dump(const struct ipanic_header *hdr)
{
	...
	xlog_printk(ANDROID_LOG_INFO, IPANIC_LOG_TAG,
		    "\theader(%u, %u) detail(%u, %u) console(%u, %u)\n", hdr->oops_header_offset,
		    hdr->oops_header_length, hdr->oops_detail_offset, hdr->oops_detail_length,
		    hdr->console_offset, hdr->console_length);
	xlog_printk(ANDROID_LOG_INFO, IPANIC_LOG_TAG, "\tprocess(%u, %u)\n",
		    hdr->userspace_info_offset, hdr->userspace_info_length);
	xlog_printk(ANDROID_LOG_INFO, IPANIC_LOG_TAG,
		    "\tandroid main(%u, %u), android system(%u, %u), android radio(%u, %u)\n",
		    hdr->android_main_offset, hdr->android_main_length, hdr->android_system_offset,
		    hdr->android_system_length, hdr->android_radio_offset,
		    hdr->android_radio_length);
}
