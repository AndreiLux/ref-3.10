...
/* AP:
 * without buffer lock or per_cpu buffer, code below may produce unwanted results.
 * It is not useful, because we have the same info in console buffer through
 * printk() calls as well. Disabling the code. */
#if defined(CONFIG_MTK_RAM_CONSOLE) && 0
static char sram_printk_buf[256];
