...
#define PK_TRC_FUNC(f)              pr_info(PFX "<%s>\n", __func__);
#define PK_TRC_VERBOSE(fmt, arg...) pr_info(PFX fmt, ##arg)
	...
	if (NULL != kdFlashlightList[flashlightIdx].flashlightInit) {
		...
		if (NULL == g_pFlashlightFunc) {
			...
			if (NULL != kdFlashlightList[KD_DEFAULT_FLASHLIGHT_INDEX].flashlightInit) {
				kdFlashlightList[KD_DEFAULT_FLASHLIGHT_INDEX].
				    flashlightInit(&g_pFlashlightFunc);
				if (NULL == g_pFlashlightFunc) {
					PK_DBG
					    ("[kdSetFlashlightDrv] KD_DEFAULT_FLASHLIGHT_INDEX init fail\n");
					return -EIO;
				}
			}
		}
