	...
	if (E_DEVAPC0 == devapc_num) {
		if (module < (MOD_NO_IN_1_DEVAPC)) {
			if (E_AP_MCU == domain_num) {
				...
				xlog_printk(ANDROID_LOG_ERROR, DEVAPC_TAG,
					    "[DEVAPC] ERROR, The setting is error, please check if domain master setting is correct or not !\n");
			}
		} else {
 ...
 * no return value.
 */
static void unmask_module_irq(unsigned int module, DEVAPC_NUM devapc_num, E_MASK_DOM domain_num)
{
 ...
 * no return value.
 */
static void clear_vio_status(unsigned int module, DEVAPC_NUM devapc_num, E_MASK_DOM domain_num)
{
	unsigned int module_index = (0x1 << module);
	...
	if (E_DEVAPC0 == devapc_num) {
		...
		xlog_printk(ANDROID_LOG_ERROR, DEVAPC_TAG,
			    "[DEVAPC] ERROR: clear_vio_status , The setting is error, please check if the setting is correct or not !\n");
	}
}
...
static irqreturn_t devapc_violation_irq(int irq, void *dev_id)
{
	...
	pr_debug("KSY Debug: [DAPC] [REE] Enter IRQ Handler...\n");

	if (((readl(DEVAPC0_DXS_VIO_STA) & 0xF) == 0) && (&g_devapc_ctrl != dev_id)) {
		xlog_printk(ANDROID_LOG_INFO, DEVAPC_TAG,
			    "[DEVAPC] ERROR AP_DEVAPC0_DXS_VIO_STA not device apc AP/MM violation!");
		return IRQ_NONE;
	}
	...
	if (r_w_violation == 1) {
		xlog_printk(ANDROID_LOG_INFO, DEVAPC_TAG,
			    "Vio Addr:0x%x , Master ID:0x%x , Dom ID:0x%x, W\n", dbg1, master_ID,
			    domain_ID);
	} else {
		xlog_printk(ANDROID_LOG_INFO, DEVAPC_TAG,
			    "Vio Addr:0x%x , Master ID:0x%x , Dom ID:0x%x, R\n", dbg1, master_ID,
			    domain_ID);
	}
	...
	if (E_DEVAPC0 == apc_index) {
		for (module_index = 0;
		     module_index < (sizeof(D_APC0_Devices) / sizeof(DEVICE_INFO));
		     module_index++) {
			if (NULL == D_APC0_Devices[module_index].device_name)
				break;

			if (TRUE == D_APC0_Devices[module_index].protect)
		...
		for (module_index = 0;
		     module_index < (sizeof(D_APC1_Devices) / sizeof(DEVICE_INFO));
		     module_index++) {
			if (NULL == D_APC1_Devices[module_index].device_name)
				break;

			if (TRUE == D_APC1_Devices[module_index].protect)
		...
		for (module_index = 0;
		     module_index < (sizeof(D_APC2_Devices) / sizeof(DEVICE_INFO));
		     module_index++) {
			if (NULL == D_APC2_Devices[module_index].device_name)
				break;

			if (TRUE == D_APC2_Devices[module_index].protect)
		...
		for (module_index = 0;
		     module_index < (sizeof(D_APC3_Devices) / sizeof(DEVICE_INFO));
		     module_index++) {
			if (NULL == D_APC3_Devices[module_index].device_name)
				break;

			if (TRUE == D_APC3_Devices[module_index].protect)
		...
		for (module_index = 0;
		     module_index < (sizeof(D_APC4_Devices) / sizeof(DEVICE_INFO));
		     module_index++) {
			if (NULL == D_APC4_Devices[module_index].device_name)
				break;

			if (TRUE == D_APC4_Devices[module_index].protect)
	...
	pr_debug("KSY Debug: [DAPC] [REE] Quit IRQ Handler...\n");
...
{
	pr_debug("KSY Debug: DAPC init_devapc() Start...\n");


	mt65xx_reg_sync_writel(readl((volatile unsigned int *)(0xF0001040)) &
			       (0xFFFFFFFF ^ (1 << 6)), 0xF0001040);
	...
	mt65xx_reg_sync_writel(readl(DEVAPC0_PD_APC_CON) & (0xFFFFFFFF ^ (1 << 2)),
			       DEVAPC0_PD_APC_CON);
	mt65xx_reg_sync_writel(readl(DEVAPC1_PD_APC_CON) & (0xFFFFFFFF ^ (1 << 2)),
			       DEVAPC1_PD_APC_CON);
	mt65xx_reg_sync_writel(readl(DEVAPC2_PD_APC_CON) & (0xFFFFFFFF ^ (1 << 2)),
			       DEVAPC2_PD_APC_CON);
	mt65xx_reg_sync_writel(readl(DEVAPC3_PD_APC_CON) & (0xFFFFFFFF ^ (1 << 2)),
			       DEVAPC3_PD_APC_CON);
	mt65xx_reg_sync_writel(readl(DEVAPC4_PD_APC_CON) & (0xFFFFFFFF ^ (1 << 2)),
			       DEVAPC4_PD_APC_CON);

	/* clean violation status & unmask device apc 0 & 1 */
	mt65xx_reg_sync_writel(0x0000007F, DEVAPC0_DXS_VIO_STA);
	mt65xx_reg_sync_writel(0x00FF00E0, DEVAPC0_DXS_VIO_MASK);
	pr_debug("KSY Debug: DAPC init_devapc() End!\n");
}

/*
 * set_default_apc: set default apc when DAPC is initializing
 */
void set_default_apc(void)
{
	int module_index = 0;
	...
	for (module_index = 0; module_index < (sizeof(D_APC1_Devices) / sizeof(DEVICE_INFO));
	     module_index++) {
		if (NULL == D_APC1_Devices[module_index].device_name)
			break;
	...
	for (module_index = 0; module_index < (sizeof(D_APC2_Devices) / sizeof(DEVICE_INFO));
	     module_index++) {
		if (NULL == D_APC2_Devices[module_index].device_name)
			break;
	...
	for (module_index = 0; module_index < (sizeof(D_APC3_Devices) / sizeof(DEVICE_INFO));
	     module_index++) {
		if (NULL == D_APC3_Devices[module_index].device_name)
			break;
	...
	for (module_index = 0; module_index < (sizeof(D_APC4_Devices) / sizeof(DEVICE_INFO));
	     module_index++) {
		if (NULL == D_APC4_Devices[module_index].device_name)
			break;
...
{
	pr_debug("KSY Debug: DAPC start_devapc() Start...\n");


	set_default_apc();
	pr_debug("KSY Debug: DAPC start_devapc() Half~\n");
	...
	mt65xx_reg_sync_writel(0x2, DEVAPC0_DXS_VIO_STA);
	pr_debug("KSY Debug: DAPC start_devapc() End!\n");
...
#ifdef CONFIG_MTK_IN_HOUSE_TEE_SUPPORT
/* Test_REE_TEE */
static void tz_show_test_msg(KREE_SESSION_HANDLE session)
{
	MTEEC_PARAM param[4];
	uint32_t paramTypes;
	TZ_RESULT ret;

	paramTypes = TZ_ParamTypes3(TZPT_VALUE_OUTPUT, TZPT_VALUE_OUTPUT, TZPT_VALUE_OUTPUT);

	ret = KREE_TeeServiceCall(session, TZCMD_DAPC_TEST, paramTypes, param);

	if (TZ_RESULT_SUCCESS != ret)
		pr_info("KSY Debug: TEE ServiceCall Error: %d\n", ret);

}

/* Test_REE_TEE */
void show_test_msg(void)
{
	TZ_RESULT ret;
	KREE_SESSION_HANDLE dapc_session;

	ret = KREE_CreateSession(TZ_TA_DAPC_UUID, &dapc_session);

	if (ret != TZ_RESULT_SUCCESS) {

		pr_info("KSY Debug: Create dapc_session(test) error %d\n", ret);
		return;
	}
	pr_debug("KSY Debug: Create dapc_session(test) OK!\n");

	tz_show_test_msg(dapc_session);
	pr_debug("KSY Debug: DAPC tz_show_test_msg is called\n");

	ret = KREE_CloseSession(dapc_session);

	if (ret != TZ_RESULT_SUCCESS)
		pr_info("KSY Debug: Close dapc_session(test) error %d\n", ret);

	pr_debug("KSY Debug: Close dapc_session(test) OK!\n");
}
#endif
...
static int devapc_probe(struct platform_device *dev)
{
	...
	pr_debug("KSY Debug: DAPC Probe, [TEE] start...\n");
	...
	/* Violation Testing */
	/* pr_debug("KSY Debug: Test SEJ start...\n"); */
	/* mt65xx_reg_sync_writel(0xFF001177, 0xF000A000); */
	/* pr_debug("KSY Debug: Test SEJ end...\n"); */

#else

	pr_debug("KSY Debug: DAPC Probe, [REE] start...\n");
	xlog_printk(ANDROID_LOG_INFO, DEVAPC_TAG, "[DAPC] module probe.\n");
	start_devapc();

#endif

	pr_debug("KSY Debug: DAPC Probe, End!\n");
	return 0;
}
...
static int devapc_resume(struct platform_device *dev)
{
	...
	pr_debug("KSY Debug: DAPC Resume, [TEE] start...\n");

#else

	pr_debug("KSY Debug: DAPC Resume, [REE] start...\n");
	...
	pr_debug("KSY Debug: DAPC Resume, End!\n");
	return 0;
}
...
static int __init devapc_init(void)
{
	...
	pr_debug("KSY Debug: DAPC Initialization, Start...\n");
	xlog_printk(ANDROID_LOG_INFO, DEVAPC_TAG, "[DAPC] module init.\n");

	ret = platform_device_register(&devapc_device);
	if (ret) {
		xlog_printk(ANDROID_LOG_ERROR, DEVAPC_TAG,
			    "[DAPC] Unable to do device register(%d)\n", ret);
		return ret;
	}
	ret = platform_driver_register(&devapc_driver);
	if (ret) {
		xlog_printk(ANDROID_LOG_ERROR, DEVAPC_TAG,
			    "[DAPC] Unable to register driver (%d)\n", ret);
		return ret;
	}
	...
	if (ret != 0) {
		xlog_printk(ANDROID_LOG_ERROR, DEVAPC_TAG,
			    "KSY Debug: [DAPC] Failed to add devapc device! (%d)\n", ret);
		return ret;
	}
#ifdef CONFIG_MTK_IN_HOUSE_TEE_SUPPORT

	pr_debug("KSY Debug: [TEE] DAPC should request IRQ in TEE...\n");
	...
	pr_debug("KSY Debug: [REE] Request IRQ in REE!\n");

	ret =
	    request_irq(MT_APARM_DOMAIN_IRQ_ID, (irq_handler_t) devapc_violation_irq,
			IRQF_TRIGGER_LOW | IRQF_SHARED, "mt6577_devapc", &g_devapc_ctrl);
	...
	if (ret != 0) {
		xlog_printk(ANDROID_LOG_ERROR, DEVAPC_TAG,
			    "KSY Debug: [DAPC] Failed to request IRQ! (%d)\n", ret);

		return ret;

	} else {

		pr_debug("KSY Debug: [DAPC] Request IRQ in REE successfully!\n");

	}

#endif

	pr_debug("KSY Debug: DAPC Initialization, End!\n");
...
static void __exit devapc_exit(void)
{
	...
	pr_debug("KSY Debug: [DAPC] module exit!\n");
