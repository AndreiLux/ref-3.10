...
#define PK_XLOG_INFO(fmt, args...) printk(PFX fmt, ##arg)
#else
...
UINT32 kdGetSensorInitFuncList(ACDK_KD_SENSOR_INIT_FUNCTION_STRUCT **ppSensorList)
{
	if (NULL == ppSensorList) {
		pr_err("[kdGetSensorInitFuncList]ERROR: NULL ppSensorList\n");
		return 1;
	}
...
/* PK_DBG("Addr : 0x%x,Val : 0x%x\n",a_u2Addr,a_u4Data); */
	...
	if (a_u4Bytes > 2) {
		pr_err("[CAMERA SENSOR] exceed 2 bytes\n");
		return -1;
	}

	if (a_u4Data >> (a_u4Bytes << 3))
		pr_warn("[CAMERA SENSOR] warning!! some data is not sent!!\n");
...
int kdSetI2CBusNum(u32 i2cBusNum)
{
	...
	if ((i2cBusNum != SUPPORT_I2C_BUS_NUM2) && (i2cBusNum != SUPPORT_I2C_BUS_NUM1)) {
		pr_err("[kdSetI2CBusNum] i2c bus number is not correct(%d)\n", i2cBusNum);
		return -1;
	}
	spin_lock(&kdsensor_drv_lock);
...
int iBurstWriteReg(u8 *pData, u32 bytes, u16 i2cId)
{
	...
	if (gI2CBusNum == SUPPORT_I2C_BUS_NUM1) {
		if (bytes > MAX_CMD_LEN) {
			pr_err("[iBurstWriteReg] exceed the max write length\n");
			return 1;
		}
...
static inline int adopt_CAMERA_HW_Open(void)
{
	...
	if ((atomic_read(&g_CamHWOpend) == 0) && (atomic_read(&g_CamHWOpend2) == 0)) {
		...
		if (g_pSensorFunc) {
			...
			pr_err(" ERROR:NULL g_pSensorFunc\n");
		}
	...
	if (g_pSensorFunc) {
		for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
			if (DUAL_CAMERA_NONE_SENSOR != g_invokeSocketIdx[i]) {
				...
				if (ERROR_NONE != err) {
					pr_warn
					    ("Warn:adopt_CAMERA_HW_CheckIsAlive(), No imgsensor alive %d\n",
					     err);
				}
			}
		}
	} else {
		pr_warn("Warn:NULL g_pSensorFunc\n");
	}
	/*  */
...
static inline int adopt_CAMERA_HW_GetInfo(void *pBuf)
{
	...
	if (g_pSensorFunc)
		...
		pr_err("[CAMERA_HW]ERROR:NULL g_pSensorFunc\n");
...
static inline int adopt_CAMERA_HW_Control(void *pBuf)
{
	...
	if (copy_from_user
		...
		pr_err("[CAMERA_HW][pFeatureData32] ioctl copy from user failed\n");
		return -EFAULT;
	}
	...
	if (copy_to_user
		...
		pr_err("[CAMERA_HW][imageWindow] ioctl copy to user failed\n");
		return -EFAULT;
	}
	/*  */
	if (copy_to_user
...
static inline int adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
	...
	case SENSOR_FEATURE_VENDOR_CMD:	/* For sensor register debug */
		...
		if (copy_from_user
			...
			kfree(pFeaturePara);
			pr_err("[CAMERA_HW][pFeaturePara] ioctl copy from user failed\n");
			return -EFAULT;
		}
		break;
	...
	if (g_pSensorFunc) {
		...
		pr_warn("[CAMERA_HW]WARN:NULL g_pSensorFunc\n");
	}
	...
	case SENSOR_FEATURE_GET_FACTORY_MODE:
		...
		if (copy_to_user
		    ((void __user *)pFeatureCtrl->pFeaturePara, (void *)pFeaturePara,
		     FeatureParaLen)) {
			kfree(pFeaturePara);
			pr_err("[CAMERA_HW][pSensorRegData] ioctl copy to user failed\n");
			return -EFAULT;
		}
		break;
	...
	case SENSOR_FEATURE_GET_GROUP_INFO:
		...
		if (copy_to_user
		    ((void __user *)pUserGroupNamePtr, (void *)kernelGroupNamePtr,
		     sizeof(char) * 32)) {
			kfree(pFeaturePara);
			pr_err("[CAMERA_HW][pFeatureReturnPara32] ioctl copy to user failed\n");
			return -EFAULT;
		}
	...
	if (copy_to_user
	    ((void __user *)pFeatureCtrl->pFeatureParaLen, (void *)&FeatureParaLen,
	     sizeof(unsigned int))) {
		pr_err("[CAMERA_HW][pFeatureParaLen] ioctl copy to user failed\n");
		return -EFAULT;
	}
	return ret;
}				/* adopt_CAMERA_HW_FeatureControl() */
	...
	if (_IOC_NONE == _IOC_DIR(a_u4Command)) {
		...
		if (_IOC_WRITE & _IOC_DIR(a_u4Command)) {
			if (copy_from_user(pBuff, (void *)a_u4Param, _IOC_SIZE(a_u4Command))) {
				kfree(pBuff);
				pr_err("[CAMERA SENSOR] ioctl copy from user failed\n");
				i4RetValue = -EFAULT;
				goto CAMERA_HW_Ioctl_EXIT;
			}
		}
...
static inline int RegisterCAMERA_HWCharDrv(void)
{
	...
	if (alloc_chrdev_region(&g_CAMERA_HWdevno, 0, 1, CAMERA_HW_DRVNAME1)) {
		pr_warn("[CAMERA SENSOR] Allocate device no failed\n");

		return -EAGAIN;
	}
#else
	if (register_chrdev_region(g_CAMERA_HWdevno, 1, CAMERA_HW_DRVNAME1)) {
		pr_warn("[CAMERA SENSOR] Register device no failed\n");
	...
	if (cdev_add(g_pCAMERA_HW_CharDrv, g_CAMERA_HWdevno, 1)) {
		pr_warn("[mt6516_IDP] Attatch file operation failed\n");
...
{
	char buffer[16] = {'\0'};
	ssize_t bytes = count < (16-(*ppos)) ? count : (16-(*ppos));

	sprintf(buffer, "{0x%02x, 0x%02x},\n", g_SubSensorReg.RegAddr, g_SubSensorReg.RegData);

	if (copy_to_user(pPage, buffer , sizeof(char)*16))
		return -EFAULT;
				    ...
				    loff_t *ppos)
{
	...
	} else if (sscanf(regBuf, "dumpreg=%d", &sensorReg.RegAddr) == 1) {
		PK_DBG("dump sensor all registers\n");
		if (g_pSensorFunc != NULL) {
			g_pSensorFunc->SensorFeatureControl(DUAL_CAMERA_SUB_SENSOR,
				    SENSOR_FEATURE_VENDOR_CMD,
				    (MUINT8 *) &sensorReg,
				    (MUINT32 *)
				    sizeof(MSDK_SENSOR_REG_INFO_STRUCT));
		}
...
static ssize_t CAMERA_Temperature_Read(struct file *file, char __user *pPage, size_t count, loff_t *ppos)
{
	...
	if (copy_to_user(pPage, buffer , sizeof(char)*min_t(size_t, 10, count)))
		return -EFAULT;
...
static int __init CAMERA_HW_i2C_init(void)
{
	...
	if (!prEntry)
		pr_err("add /proc/driver/camsensor entry fail\n");
	...
	if (!prEntry)
		pr_err("add /proc/driver/camsensor2 entry fail\n");
	...
	memset(&g_SubSensorReg, 0, sizeof(MSDK_SENSOR_REG_INFO_STRUCT));
	...
	if (!prEntry)
		pr_err("add /proc/driver/camsensor_temperature entry fail\n");
