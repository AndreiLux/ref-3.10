...
const HI704_SENSOR_INIT_INFO HI704_Initial_Setting_Info[] = {
	...
	{0x38, 0x65},
	/* CMC OFS */
	{0x40, 0x16},/* 20140731 bt Sam */
	{0x41, 0x92},/* 20140731 bt Sam */
	{0x42, 0x85},/* 20140731 bt Sam */
	{0x43, 0x0a},/* 20140731 bt Sam */
	{0x44, 0x9e},/* 20140731 bt Sam */
	{0x45, 0x15},/* 20140731 bt Sam */
	{0x46, 0x8c},/* 20140731 bt Sam */
	{0x47, 0x95},/* 20140731 bt Sam */
	{0x48, 0x20},/* 20140731 bt Sam */
	...
	{0x83, 0x01},		/* expTime:0x83,0x84,0x85 */
	{0x84, 0x86},
	{0x85, 0x00},
	...
	{0x98, 0x8C},
	{0x99, 0x23},
	{0x9c, 0x03},		/* EXP Limit 976.56 fps */
	{0x9d, 0xc0},
	{0x9e, 0x00},		/* EXP Unit */
	{0x9f, 0xc0},
	{0xb0, 0x30},		/* 20131212 (b0~bd) */
	{0xb1, 0x14},		/*Analog gain min:1x*/
	{0xb2, 0xbc},		/* f0-->bc 20150729 sam */
	{0xb3, 0x14},
	{0xb4, 0x14},
	{0xb5, 0x39},
	{0xb6, 0x26},
	...
	{0xbd, 0x18},
	{0xc0, 0x1a},
	{0xc3, 0xbb},/* 20140731 by Sam */
	{0xc4, 0xbb},/* 20140731 by Sam */
	...
	{0x82, 0x50},
	{0x83, 0x55},
	{0x84, 0x16},/* 1b-->16 sam20140729 */
	{0x85, 0x60},/* 56-->60 sam20140729 */
	{0x86, 0x20},
	{0x87, 0x31},/* 20140801  RMAX31 by Sam */
	{0x88, 0x25},
	{0x89, 0x3f},
	{0x8a, 0x36},/* 20140801  Bmin36 by Sam */
	{0x8b, 0x00},
	{0x8d, 0x44},
	{0x8e, 0x41},
	{0x8f, 0x5d},
...
const HI704_SENSOR_INIT_INFO HI704_Initial_Setting_Info_1[] = {
	/* PAGE 0 */
	/* Base on LITE-ON White-0326 version */
	{0x03, 0x00},		/* PAGEMODE(0x03) */
	{0x01, 0xf1},
	{0x01, 0xf3},		/* PWRCTL(0x01[P0])Bit[1]:Software Reset. */
	{0x01, 0xf1},
	...
	{0x9e, 0x00},		/* EXP Unit */
	{0x9f, 0xc0},
	{0xb0, 0x1a},		/* 20131212 (b0~bd) */
	{0xb1, 0x14},		/* Analog gain min:1x */
	{0xb2, 0xb0},		/* Analog gain max:3.5x */
	{0xb3, 0x16},
	{0xb4, 0x14},
	{0xb5, 0x38},
	{0xb6, 0x26},
	...
	{0xbd, 0x18},
	{0xc0, 0x1a},
	{0xc3, 0xac},		/* 48->58 20131212 */
	{0xc4, 0xac},		/* 48->58 20131212 */
...
const HI704_SENSOR_INIT_INFO HI704_Initial_Setting_Info_2[] = {
	/* PAGE 0 */
	/* Image Size/Windowing/HSYNC/VSYNC[Type1] */
	/* Base on LITE-ON Dark-0326 version  */
	{0x03, 0x00},		/* PAGEMODE(0x03) */
	{0x01, 0xf1},
	{0x01, 0xf3},		/* PWRCTL(0x01[P0])Bit[1]:Software Reset. */
	{0x01, 0xf1},
...
const HI704_SENSOR_INIT_INFO HI704_Initial_Setting_Info_3[] = {
	/* PAGE 0 */
	/* Image Size/Windowing/HSYNC/VSYNC[Type1] */
	/* Base on LITE-ON SFR -0326 version */
	{0x03, 0x00},		/* PAGEMODE(0x03) */
	{0x01, 0xf1},
	{0x01, 0xf3},		/* PWRCTL(0x01[P0])Bit[1]:Software Reset. */
	{0x01, 0xf1},
...
static void HI704_Dump_SensorReg(kal_uint8 mode)
{
	kal_uint32 iEcount;
	const HI704_SENSOR_INIT_INFO *tbl;
	switch (sub_sensor_init_setting_switch)	{
	case 1:
		tbl = HI704_Initial_Setting_Info_1;
		pr_debug("HI704_Initial_Setting_Info_1[]=\n");
		break;
	case 2:
		tbl = HI704_Initial_Setting_Info_2;
		pr_debug("HI704_Initial_Setting_Info_2[]=\n");
		break;
	case 3:
		tbl = HI704_Initial_Setting_Info_3;
		pr_debug("HI704_Initial_Setting_Info_3[]=\n");
		break;
	case 0:
	default:
		tbl = HI704_Initial_Setting_Info;
		pr_debug("HI704_Initial_Setting_Info[]=\n");
		break;
	}

	for (iEcount = 0; (!((0xff == (tbl[iEcount].address)) && (0xff == (tbl[iEcount].data))));	iEcount++) {
		if (mode == 1) {	/* dump camera register */
			if (tbl[iEcount].address == 0x03) {
				HI704_write_cmos_sensor(tbl[iEcount].address, tbl[iEcount].data);
				pr_debug("{0x%02x, 0x%02x},\n", tbl[iEcount].address, tbl[iEcount].data);
				continue;
			}
			pr_debug("{0x%02x, 0x%02x},\n",
						tbl[iEcount].address, HI704_read_cmos_sensor(tbl[iEcount].address));
		} else if (mode == 2) {	/* dump driver table */
			if (tbl[iEcount].address == 0x03) {
				pr_debug("{0x%02x, 0x%02x},\n", tbl[iEcount].address, tbl[iEcount].data);
				continue;
			}
...
static void HI704_Init_Parameter(void)
{
	...
	HI704_sensor.AELocked = KAL_FALSE;
...
static kal_uint8 HI704_power_on(void)
{
	...
	spin_lock(&hi704_yuv_drv_lock);
	HI704_sensor.pv_pclk = 12000000; /* MCLK 24MHz - PCLK 12MHz, MCLK 26MHz - PCLK 13MHz, */
	spin_unlock(&hi704_yuv_drv_lock);
	/* Software Reset */
...
static void HI704_Set_VGA_mode(void)
{
	...
	HI704_write_cmos_sensor(0x10, ADDR_0x0010);	/* VGA Size */

	HI704_write_cmos_sensor(0x20, ADDR_0x0020);
	HI704_write_cmos_sensor(0x21, ADDR_0x0021);	/* Window start X */


	HI704_write_cmos_sensor(0x40, ADDR_0x0040);	/* HBLANK: 0x70 = 112 */
	HI704_write_cmos_sensor(0x41, ADDR_0x0041);
	HI704_write_cmos_sensor(0x42, ADDR_0x0042);	/* VBLANK: 0x04 = 4 */
	HI704_write_cmos_sensor(0x43, ADDR_0x0043);

	HI704_write_cmos_sensor(0x03, 0x11);
	HI704_write_cmos_sensor(0x10, ADDR_0x1110);
	...
	HI704_write_cmos_sensor(0x83, ADDR_0x2083);
	HI704_write_cmos_sensor(0x84, ADDR_0x2084);
	HI704_write_cmos_sensor(0x85, ADDR_0x2085);
	HI704_write_cmos_sensor(0x86, ADDR_0x2086);
	HI704_write_cmos_sensor(0x87, ADDR_0x2087);

	HI704_write_cmos_sensor(0x8b, ADDR_0x208b);
	HI704_write_cmos_sensor(0x8c, ADDR_0x208c);
	HI704_write_cmos_sensor(0x8d, ADDR_0x208d);
	HI704_write_cmos_sensor(0x8e, ADDR_0x208e);

	HI704_write_cmos_sensor(0x9c, ADDR_0x209c);
	HI704_write_cmos_sensor(0x9d, ADDR_0x209d);
	HI704_write_cmos_sensor(0x9e, ADDR_0x209e);
	HI704_write_cmos_sensor(0x9f, ADDR_0x209f);
...
void HI704_night_mode(kal_bool enable)
{
	...
	if (enable)
		...
		HI704_Cal_Min_Frame_Rate(HI704_MIN_FRAMERATE_10);/* Change by Sam to 10fps 20140729 */
}
	...
	case AWB_MODE_AUTO:
		{
			...
			HI704_write_cmos_sensor(0x11, ADDR_0x2211);
			HI704_write_cmos_sensor(0x80, ADDR_0x2280);
			HI704_write_cmos_sensor(0x82, ADDR_0x2282);
			HI704_write_cmos_sensor(0x83, ADDR_0x2283);
			HI704_write_cmos_sensor(0x84, ADDR_0x2284);
			HI704_write_cmos_sensor(0x85, ADDR_0x2285);
			HI704_write_cmos_sensor(0x86, ADDR_0x2286);
			HI704_write_cmos_sensor(0x10, ADDR_0x2210);
		}
		break;
	...
	case MEFFECT_OFF:
		{
			...
			HI704_write_cmos_sensor(0x11, ADDR_0x1011);
			HI704_write_cmos_sensor(0x12, ADDR_0x1012);
			HI704_write_cmos_sensor(0x13, ADDR_0x1013);
			HI704_write_cmos_sensor(0x44, ADDR_0x1044);
			HI704_write_cmos_sensor(0x45, ADDR_0x1045);

			HI704_write_cmos_sensor(0x47, ADDR_0x1047);
			HI704_write_cmos_sensor(0x03, 0x13);
			HI704_write_cmos_sensor(0x20, ADDR_0x1320);
			HI704_write_cmos_sensor(0x21, ADDR_0x1321);
		}
		break;
	case MEFFECT_SEPIA:
		{
...
BOOL HI704_set_param_exposure(UINT16 para)
{
	...
	case AE_EV_COMP_00:	/* +0 EV */
		HI704_write_cmos_sensor(0x40, ADDR_0x1040);
		break;
...
void HI704_AE_Set_Window(UINT32 zone_addr, UINT32 prevW, UINT32 prevH)
{
	...
	if ((x0 == x1) && (y0 == y1)) {
		HI704_write_cmos_sensor(0x03, 0x20);
		HI704_write_cmos_sensor(0x60, ADDR_0x2060);	/* AEWGT */
		HI704_write_cmos_sensor(0x68, ADDR_0x2068);
		HI704_write_cmos_sensor(0x69, ADDR_0x2069);
		HI704_write_cmos_sensor(0x6A, ADDR_0x206a);
		HI704_write_cmos_sensor(0x6B, ADDR_0x206b);
		mdelay(30);
...
/* Return sensor supports AE/AWB lock or not */
void HI704_get_AEAWB_lock(UINT32 *pAElockRet32, UINT32 *pAWBlockRet32)
{
	*pAElockRet32 = 1;
	*pAWBlockRet32 = 0;
	SENSORDB("[%s]AElock=%d ,AWBlock=%d\n,", __func__, *pAElockRet32, *pAWBlockRet32);
}

static void HI704_AE_Lock(void)
{
	HI704_write_cmos_sensor(0x01, HI704_read_cmos_sensor(0x01) | 0x01);	/* Sleep: For Write Reg */
	HI704_write_cmos_sensor(0x03, 0x20);
	HI704_write_cmos_sensor(0x83, HI704_read_cmos_sensor(0x80));
	HI704_write_cmos_sensor(0x84, HI704_read_cmos_sensor(0x81));
	HI704_write_cmos_sensor(0x85, HI704_read_cmos_sensor(0x82));
...
static void HI704_AE_UnLock(void)
{
	HI704_write_cmos_sensor(0x01, HI704_read_cmos_sensor(0x01) | 0x01);	/* Sleep: For Write Reg */
	HI704_write_cmos_sensor(0x03, 0x20);
	HI704_write_cmos_sensor(0x01, HI704_read_cmos_sensor(0x01) & 0xfe);	/* Exit Sleep: For Write Reg */
...
void HI704_3ACtrl(ACDK_SENSOR_3A_LOCK_ENUM action)
{
	SENSORDB("[%s] 3A cmd : %d\n", __func__, action);
	...
	case SENSOR_3A_AE_LOCK:
		...
		spin_lock(&hi704_yuv_drv_lock);
		HI704_sensor.AELocked = KAL_TRUE;
		spin_unlock(&hi704_yuv_drv_lock);
		HI704_AE_Lock();
	break;
	...
	break;
	default:
		SENSORDB("[%s] Not support 3A ctrl: %d\n", __func__, action);
	break;
	}
	return;
}
...
/*
  get the exposure time , unit as us
  we are not afraid of overflow, UINT32_MAX can store over 400 seconds
*/
UINT32 HI704GetExposureTime(void)
{
	...
	/*
	    multi and divide one by one for avoiding overflow, also more accurate
	    actually, the real algorithm is: internal_exp_time / exp_time * (1000 / freq) * 1000     (unit as us)
	*/
	ExposureTimems = (internal_exp_time / exp_time * 1000 / freq) * 1000;	/* Unit : us */
	SENSORDB("[HI704GetExposureTime] internal_exp_time:%u, exp_time:%u, freq:%u, ExposureTimems : %u us\n",
		internal_exp_time, exp_time, freq, ExposureTimems);
...
UINT32 HI704GetISOValueAndSpeed(UINT32 *iso_speed, UINT32 *iso_value)
{
	...
	*/
	...
	SENSORDB("[HI704GetISOValueAndSpeed] Read Gain: 0x%x, Map AE_ISO_Speed : %d, temp_iso_val:%u\n",
			   ...
			   UINT8 *pFeaturePara, UINT32 *pFeatureParaLen)
{
	...
	case SENSOR_FEATURE_SET_YUV_3A_CMD:
		HI704_3ACtrl((ACDK_SENSOR_3A_LOCK_ENUM)*pFeatureData32);
		break;

	case SENSOR_FEATURE_GET_AE_AWB_LOCK_INFO:
		HI704_get_AEAWB_lock((MUINT32 *)(*pFeatureData32), (MUINT32 *)(*(pFeatureData32+1)));
		break;

	case SENSOR_FEATURE_VENDOR_CMD:	/* For sensor register debug */
		HI704_Dump_SensorReg(pSensorRegData->RegAddr);
		break;
