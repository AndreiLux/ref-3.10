...
void select_charging_curret(void)
{
	if (g_ftm_battery_flag) {
		...
		battery_xlog_printk(BAT_LOG_FULL, "[BATTERY] Default CC mode charging : %d\r\n",
				    g_temp_CC_value);
...
PMU_STATUS BAT_TopOffModeAction(void)
{
	...
	battery_xlog_printk(BAT_LOG_FULL, "[BATTERY] Top Off mode charge, timer=%ld on %ld !!\n\r",
...
PMU_STATUS BAT_BatteryCmdHoldAction(void)
{
	...
	if (g_cmd_hold_charging == KAL_FALSE) {
		BMT_status.bat_charging_state = CHR_CC;
		battery_xlog_printk(BAT_LOG_CRTI,
