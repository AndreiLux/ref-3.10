...
static kal_uint32 charging_get_charger_type(void *data)
{
	...
	*(CHARGER_TYPE *) (data) = STANDARD_HOST;
	...
	SETREG16(USB_U2PHYACR6_2, 0x80);	/* RG_USB20_BC11_SW_EN bit 7 = 1 : switch to PMIC */
	...
	/* We detect again to confirm if it's USB host */
	if (vbus_state == 0x1 && upmu_get_rgs_chrdet()) {
		count = 0;
		bq24297_set_dpdm_en(1);
		while (bq24297_get_dpdm_status() == 1) {
			count++;
			mdelay(1);
			battery_xlog_printk(BAT_LOG_CRTI, "polling again BQ24297 charger detection\r\n");
			if (count > 1000)
				break;
			if (!upmu_get_rgs_chrdet())
				break;
...
charging_hw_init, charging_dump_register, charging_enable, charging_set_cv_voltage,
	    charging_get_current, charging_set_current, charging_get_input_current, charging_set_input_current,
