...
void bq24196_dump_register(void)
{
	...
	for (i = 0; i < bq24196_REG_NUM; i++) {
		...
		/* pr_info("[bq24196_dump_register] Reg[0x%X]=0x%X\n", i, bq24196_reg[i]); */
		battery_xlog_printk(BAT_LOG_FULL, "[bq24196_dump_register] Reg[0x%X]=0x%X\n", i,
				    bq24196_reg[i]);
	}
}
				    ...
				    const char *buf, size_t size)
{
	...
	pr_info("[store_bq24196_access]\n");

	if (buf != NULL && size != 0) {
		pr_info("[store_bq24196_access] buf is %s and size is %d\n", buf, size);
...
static int __init bq24196_init(void)
{
	...
	pr_info("[bq24196_init] init start\n");
	...
	if (i2c_add_driver(&bq24196_driver) != 0)
		pr_info("[bq24196_init] failed to register bq24196 i2c driver.\n");
	else
	...
	if (ret) {
		pr_info("****[bq24196_init] Unable to device register(%d)\n", ret);
		return ret;
	}
	ret = platform_driver_register(&bq24196_user_space_driver);
	if (ret) {
		pr_info("****[bq24196_init] Unable to register driver (%d)\n", ret);
		return ret;
	}
