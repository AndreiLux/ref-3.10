...
static kal_uint32 charging_hw_init(void *data)
{
	...
	/* pull CE low */
	mt_set_gpio_mode(p_bat_charging_data->charger_enable_pin, GPIO_MODE_GPIO);
	mt_set_gpio_dir(p_bat_charging_data->charger_enable_pin, GPIO_DIR_OUT);
	mt_set_gpio_out(p_bat_charging_data->charger_enable_pin, GPIO_OUT_ZERO);
...
static kal_uint32 charging_enable_powerpath(void *data)
{
	...
	if (KAL_TRUE == enable) {
		battery_xlog_printk(BAT_LOG_CRTI, "@@@ enable power path\n");
		bq24196_set_en_hiz(0x0);
	} else {
		battery_xlog_printk(BAT_LOG_CRTI, "@@@ disable power path\n");
