...
/**********************************************************
  ...
  *
  *********************************************************/
kal_uint32 bq24158_read_interface(kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK,
				  kal_uint8 SHIFT)
{
...
kal_uint32 bq24158_config_interface(kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK,
				    kal_uint8 SHIFT)
{
	kal_uint8 bq24158_reg = 0;
	int ret = 0;
	...
	/* pr_info("[bq24158_config_interface] Check Reg[%x]=0x%x\n", RegNum, bq24158_reg); */
...
kal_uint32 bq24158_get_otg_status(void)
{
	...
	ret = bq24158_read_interface((kal_uint8) (bq24158_CON0),
				     (&val),
				     (kal_uint8) (CON0_OTG_MASK), (kal_uint8) (CON0_OTG_SHIFT)
	    );
	return val;
}
...
void bq24158_set_vsp(kal_uint32 val)
{
	...
	ret = bq24158_config_interface((kal_uint8) (bq24158_CON5),
				       (kal_uint8) (val),
				       (kal_uint8) (CON5_VSP_MASK), (kal_uint8) (CON5_VSP_SHIFT)
	    );
}
...
void bq24158_dump_register(void)
{
	...
	for (i = 0; i < bq24158_REG_NUM; i++) {
		...
		pr_info("[0x%x]=0x%x ", i, bq24158_reg[i]);
	}
	pr_info("\n");
}
				    ...
				    const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		battery_xlog_printk(BAT_LOG_FULL,
				    "[store_bq24158_access] buf is %s and size is %d\n", buf,
				    size);
		...
		if (size > 3) {
			...
			battery_xlog_printk(BAT_LOG_FULL,
					    "[store_bq24158_access] read bq24158 reg 0x%x with value 0x%x !\n",
...
#define BQ24158_BUSNUM 4
#endif

#if 0				/* move to board-proj-battery.c */
static struct i2c_board_info i2c_bq24158 __initdata = {
	I2C_BOARD_INFO("bq24158", (bq24158_SLAVE_ADDR_WRITE >> 1)) };
#endif
...
static int __init bq24158_init(void)
{
	...
	if (i2c_add_driver(&bq24158_driver) != 0) {
		battery_xlog_printk(BAT_LOG_CRTI,
				    "[bq24158_init] failed to register bq24158 i2c driver.\n");
	} else {
		battery_xlog_printk(BAT_LOG_CRTI,
				    "[bq24158_init] Success to register bq24158 i2c driver.\n");
	}
	...
	if (ret) {
		battery_xlog_printk(BAT_LOG_CRTI,
				    "****[bq24158_init] Unable to device register(%d)\n", ret);
		return ret;
	}
	ret = platform_driver_register(&bq24158_user_space_driver);
	if (ret) {
		battery_xlog_printk(BAT_LOG_CRTI,
				    "****[bq24158_init] Unable to register driver (%d)\n", ret);
		return ret;
	}
