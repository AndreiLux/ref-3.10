...
/**********************************************************
  ...
  *
  *********************************************************/
kal_uint32 fan5405_read_interface(kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK,
				  kal_uint8 SHIFT)
{
...
kal_uint32 fan5405_config_interface(kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK,
				    kal_uint8 SHIFT)
{
	kal_uint8 fan5405_reg = 0;
	int ret = 0;
	...
	/* pr_info("[fan5405_config_interface] Check Reg[%x]=0x%x\n", RegNum, fan5405_reg); */
				    ...
				    const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		battery_xlog_printk(BAT_LOG_FULL,
				    "[store_fan5405_access] buf is %s and size is %d\n", buf,
				    size);
...
static int __init fan5405_init(void)
{
	...
	if (ret) {
		battery_xlog_printk(BAT_LOG_CRTI,
				    "****[fan5405_init] Unable to device register(%d)\n", ret);
		return ret;
	}
	ret = platform_driver_register(&fan5405_user_space_driver);
	if (ret) {
		battery_xlog_printk(BAT_LOG_CRTI,
				    "****[fan5405_init] Unable to register driver (%d)\n", ret);
		return ret;
	}
