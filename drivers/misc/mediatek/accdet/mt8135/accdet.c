...
static void accdet_eint_set_polarity_pre(void)
{
	...
	mt_eint_set_polarity(cust_headset_data_hal->cust_eint_accdet_num, 1);
...
static void accdet_eint_set_polarity_next(void)
{
	...
	mt_eint_set_polarity(cust_headset_data_hal->cust_eint_accdet_num, 0);
...
{
#ifdef ACCDET_EINT
	mt_eint_unmask(cust_headset_data_hal->cust_eint_accdet_num);
#endif
}
...
{
#ifdef ACCDET_EINT
	mt_eint_mask(cust_headset_data_hal->cust_eint_accdet_num);
#endif
}
...
#ifdef ACCDET_EINT
	mt_eint_set_hw_debounce(cust_headset_data_hal->cust_eint_accdet_num,
		ACCDET_SHORT_PLUGOUT_DEBOUNCE_CN);
#endif
}
...
{
#ifdef ACCDET_EINT
	/* mt_eint_set_sens(cust_headset_data_hal->cust_eint_accdet_num, CUST_EINT_ACCDET_SENSITIVE); */
#endif
}
...
{
#ifdef ACCDET_EINT
	mt_set_gpio_mode(cust_headset_data_hal->gpio_accdet_eint_pin,
		cust_headset_data_hal->gpio_accdet_eint_pin_m_eint);
	mt_set_gpio_dir(cust_headset_data_hal->gpio_accdet_eint_pin, GPIO_DIR_IN);
	mt_set_gpio_pull_enable(cust_headset_data_hal->gpio_accdet_eint_pin,
		GPIO_PULL_DISABLE);	/* To disable GPIO PULL. */
...
{
#ifdef ACCDET_EINT
	mt_eint_registration(cust_headset_data_hal->cust_eint_accdet_num,
		cust_headset_data_hal->cust_eint_accdet_type, accdet_eint_func, 0);
	ACCDET_DEBUG
	    ("[Accdet]accdet_eint_num=%d, accdet_eint_debounce_en=%d,accdet_eint_polarity=%d\n",
	    cust_headset_data_hal->cust_eint_accdet_num,
			cust_headset_data_hal->cust_eint_accdet_debounce_en,
			cust_headset_data_hal->cust_eint_accdet_type);
#endif
}
...
void accdet_eint_polarity_reverse_plugout(void)
{
	...
	if (cust_headset_data_hal->cust_eint_accdet_type == CUST_EINTF_TRIGGER_HIGH) {
		/* mt_set_gpio_pull_select(cust_headset_data_hal->gpio_accdet_eint_pin, GPIO_PULL_DOWN); */
		accdet_eint_set_polarity_pre();
	} else {
		/* mt_set_gpio_pull_select(cust_headset_data_hal->gpio_accdet_eint_pin, GPIO_PULL_UP); */
...
void set_cust_headset_data_hal(void)
{
...
void accdet_set_pwm_delay(void)
{
	pmic_pwrap_write(ACCDET_EN_DELAY_NUM,
			 (cust_headset_settings_hal->fall_delay << 15 | cust_headset_settings_hal->
			  rise_delay));
}
...
int dump_register(void)
{
	...
	ACCDET_DEBUG(" MULTIKEY_ADC_CHANNEL(%x) is %x\n", MULTIKEY_ADC_CHANNEL,
		     accdet_PMIC_IMM_GetOneChannelValue());
