...
static long vcodec_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case VCODEC_SET_THREAD_ID:
		...
		if (rTempTID.u4VCodecThreadNum > VCODEC_THREAD_MAX_NUM) {
			MFV_LOGE("[ERROR] VCODEC_SET_THREAD_ID, u4VCodecThreadNum exceed the max number\n");
			return -EFAULT;
		}
		...
		if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
			...
			while (bLockedHW == VAL_FALSE) {
				...
				} else	/* Another one holding dec hw now */
				{
					...
					MFV_LOGW
						("LockInstance = 0x%x, CurrentInstance = 0x%x, 0x%x, CurrentTID = %d\n",
						grVcodecDecHWLock.pvHandle,
						rHWLock.pvHandle, pmem_user_v2p_video((unsigned int)rHWLock.pvHandle),
						current->pid);
	...
	case VCODEC_UNLOCKHW:
		...
		if (rHWLock.eDriverType == VAL_DRIVER_TYPE_MP4_DEC ||
			...
			{
				MFV_LOGE("[ERROR] Not owner trying to unlock dec hardware 0x%x, 0x%x, 0x%x\n",
					grVcodecDecHWLock.pvHandle, rHWLock.pvHandle,
					pmem_user_v2p_video((unsigned int)rHWLock.pvHandle));
				mutex_unlock(&VdecHWLock);
				return -EFAULT;
			}
	...
	case VCODEC_INITHWLOCK:
		{
			...
			if (hwoal_reg.u4NumOfRegister > OALMEM_STATUS_NUM) {
				MFV_LOGE("[ERROR] VCODEC_INITHWLOCK, u4NumOfRegister exceed the max number\n");
				return -EFAULT;
			}
