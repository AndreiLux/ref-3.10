...
uint32 masp_secro_md_get_data(uchar *md_info, uchar *buf, uint32 offset, uint32 len)
{
	...
	if (0 == secroimg.m_header.world_phone_support) {
		...
		if (TRUE == sec_secro_ac()) {
			masp_hal_sp_hacc_dec((uchar *) &secroimg.m_andro, cipher_len, TRUE,
					     HACC_USER1, TRUE);
		}
		...
		/* ------------------------ */
		if (TRUE == sec_secro_ac()) {
			masp_hal_sp_hacc_enc((uchar *) &secroimg.m_andro, cipher_len, TRUE,
					     HACC_USER1, TRUE);
		}
	} else {
		SMSG(TRUE, "[%s]sro v5\n", MOD);
		...
		if (SEC_OK != ret) {
			...
			if (TRUE == sec_secro_ac()) {
				masp_hal_sp_hacc_dec((uchar *) &secroimg_v5.m_md_info_v5a,
