				     ...
				     unsigned int offset, unsigned int size)
{
	...
	/* hash file                           */
	/* ----------------------------------- */
	ret = sec_hash((uchar *) file_buf, file_size, (uchar *) hash_buf, hash_len);
	if (SEC_OK != ret) {
						    ...
						    unsigned int single_hash_len)
{
	...
	for (i = 0; i < img_content_length; i++) {
		...
		/* ----------------------------------- */
		ret = sec_signfmt_get_file_hash(sec_file,
					       tmp_signal_hash_buf, single_hash_len,
					       0, 0);
		if (SEC_OK != ret) {
			/* skip unfound file */
		...
		ret = sec_hash((uchar *) double_hash_buf, single_hash_len * 2,
					...
					char *hash_buf, unsigned int hash_len, char *ext_buf)
{
	...
	/* generate final hash */
	ret = sec_hash((uchar *) verify_buf, verify_len, (uchar *) hash_buf, hash_len);
	if (SEC_OK != ret) {
		SMSG(true, "[%s] generate hash fail\n\n", MOD);
		ret = ERR_SIGN_FORMAT_GENERATE_HASH_FAIL;
		goto _gen_hash_fail;
				     ...
				     unsigned int hash_len, char *ext_buf)
{
	...
	if (SECCFG_V3 == get_seccfg_ver()
		...
		ret = sec_signfmt_calculate_image_hash_v4(part_name, &img_if->v3,
							hash_buf, hash_len, ext_buf);
		if (SEC_OK != ret)
			goto _end;
	} else if (SECCFG_V3 == get_seccfg_ver()
		   && is_signfmt_v3((SEC_IMG_HEADER *) &img_if->v3.header.va)) {
		ret = sec_signfmt_calculate_image_hash_v3(part_name, &img_if->v3,
							hash_buf, hash_len, ext_buf);
		if (SEC_OK != ret)
			goto _end;
	} else {
		ret = sec_signfmt_calculate_image_hash_v2(part_name,
							 (SEC_IMG_HEADER *) &img_if->v1.header.va,
							 (uint32) img_if->v1.image_type, hash_buf,
							 hash_len);
		if (SEC_OK != ret)
			goto _end;
	}
