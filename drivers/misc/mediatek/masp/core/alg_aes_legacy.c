 ...
 *  INTERNAL FUNCTION
 **************************************************************************/
static void a_enc(struct a_ctx_s *ctx, uchar input[16], uchar output[16]);
static void a_dec(struct a_ctx_s *ctx, uchar input[16], uchar output[16]);
...
void a_key_exp(struct a_ctx_s *ctx)
{
	ulong temp;
	ulong rcon;
	register int i;
...
struct a_ctx_s *a_alloc_ctx(uchar *key, uint32 keyLen)
{
	struct a_ctx_s *ctx;
	uint32 r;
	uint32 ks_size;
	...
	ctx = osal_kmalloc(sizeof(struct a_ctx_s) + ks_size);
...
void a_subbytes(struct a_ctx_s *ctx)
{
	int i;
...
void a_shi_row(struct a_ctx_s *ctx)
{
	uchar nstate[4][4];
	int i;
...
void a_mix_col(struct a_ctx_s *ctx)
{
	uchar nstate[4][4];
	int i;
...
void a_add_key(struct a_ctx_s *ctx, int round)
{
	int i;
...
void a_inv_shi_row(struct a_ctx_s *ctx)
{
	uchar nstate[4][4];
...
void a_sub_b(struct a_ctx_s *ctx)
{
	int i;
...
void a_inv_mix_col(struct a_ctx_s *ctx)
{
	uchar nstate[4][4];
	int i;
...
void a_free_ctx(struct a_ctx_s *ctx)
{
	osal_kfree(ctx);
}
