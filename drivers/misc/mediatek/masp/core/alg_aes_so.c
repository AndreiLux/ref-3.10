...
int a_crypt_ecb(a_ctx *ctx, int mode, const uchar input[16], uchar output[16])
{
	int i;
...
int a_crypt_cbc(a_ctx *ctx,
		int mode, size_t length, uchar iv[16], const uchar *input, uchar *output)
{
	int i;
	uchar temp[16];
...
int aes_so_enc(uchar *ip_buf, uint32 ip_len, uchar *op_buf, uint32 op_len)
{
	...
	for (i = 0; i != ip_len; i += CIPHER_BLOCK_SIZE) {
		ret =
		    a_crypt_cbc(&aes, AES_ENCRYPT, CIPHER_BLOCK_SIZE, (uchar *) g_AES_IV_TEMP,
				ip_buf + i, op_buf + i);
		if (ret != 0) {
...
int aes_so_dec(uchar *ip_buf, uint32 ip_len, uchar *op_buf, uint32 op_len)
{
	...
	for (i = 0; i != ip_len; i += CIPHER_BLOCK_SIZE) {
		ret =
		    a_crypt_cbc(&aes, AES_DECRYPT, 0x10, (uchar *) g_AES_IV_TEMP, ip_buf + i,
				op_buf + i);
		if (ret != 0) {
			SMSG(true, "hairtunes: a_cbc error -%02X\n", -ret);
			goto _err;
