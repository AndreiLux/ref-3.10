	...
	if (len == 1) {
		...
		if (err != 2) {
			...
			for (idx = 0; idx < len; idx++)
				buflen +=
				    snprintf(buf + buflen, sizeof(buf) - buflen, "%02X ",
					     data[idx]);
...
/*----------------------------------------------------------------------------*/
void hwmsen_single_rw(struct i2c_client *client, struct hwmsen_reg *regs, int reg_num)
{
			 ...
			 find_reg_t findreg, char *buf, u32 buflen)
{
	...
	if (!err) {
		...
		for (idx = startAddr; idx < regnum; idx++) {
			...
			if (NULL != (name))
				len +=
				    snprintf(buf + len, buflen - len, "%-16s = 0x%02X\n", name,
...
/*----------------------------------------------------------------------------*/
ssize_t hwmsen_show_byte(struct device *dev, struct device_attribute *attr, char *buf, u32 buflen)
{
...
/*----------------------------------------------------------------------------*/
ssize_t hwmsen_show_word(struct device *dev, struct device_attribute *attr, char *buf, u32 buflen)
{
	int err, index = to_sensor_dev_attr(attr)->index;
	struct i2c_client *client = to_i2c_client(dev);
	u8 addr = (u8) index;
