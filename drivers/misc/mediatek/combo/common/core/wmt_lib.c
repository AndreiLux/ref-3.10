...
INT32 wmt_lib_init(struct mtk_combo_data *wmt_data)
{
	INT32 iRet;
	UINT32 i;
	P_DEV_WMT pDevWmt;
	...
	/* AP:
	 * I remove code below, because cfgExist is == 0 always,
	 * unless configuration is loaded, but configuration read
	 * code is commented */
#if 0
...
MTK_WCN_BOOL wmt_lib_is_therm_ctrl_support(VOID)
{
	...
	if (!mtk_wcn_stp_is_ready()) {
		WMT_DBG_FUNC("thermal command can not be send: STP is not ready\n");
		bIsSupportTherm = MTK_WCN_BOOL_FALSE;
	}
