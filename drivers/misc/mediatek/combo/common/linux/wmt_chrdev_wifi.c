...
/*mtk80707 fix ALPS01428005*/
enum {
	WLAN_STATE_NO_READY,
	WLAN_STATE_READY,
	WLAN_STATE_RX_PWR_ON,
	WLAN_STATE_RX_FW_SWITCH,
	WLAN_STATE_RX_PWR_OFF,
	WLAN_STATE_RX_FW_SWITCH_WO_PWR_ON, /*rx fw switch, but current is not rx_pwr_on.*/
	WLAN_STATE_MAX,
};
static int wlan_state = WLAN_STATE_NO_READY;
static int wlan_pre_state = WLAN_STATE_NO_READY;
/* end */
/*
...
int wifi_reset_end(void)
{
	...
	if (powered == 1) {
		...
		if (wait_cnt >= 10) {
			WIFI_ERR_FUNC("Get wlan0 net device time out\n");
			goto done;
		}
...
ssize_t WIFI_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (0 == copy_from_user(local, buf, (count > sizeof(local)) ? sizeof(local) : count)) {
		local[11] = 0;
		WIFI_INFO_FUNC("dbgflag 1 WIFI_write %s\n", local);
		/*
		handle power off
		*/
		if (local[0] == '0') {
			wlan_pre_state = wlan_state;
			wlan_state = WLAN_STATE_RX_PWR_OFF;
			if (powered == 0) {
			...
			/*mtk80707 fix jungle ALPS01428055*/
			if (wlan_pre_state == WLAN_STATE_RX_FW_SWITCH_WO_PWR_ON) {
				WIFI_INFO_FUNC("this power off cmd is later, and new power on cmd is on the way, so ignore it.\n");
				retval = count;
				goto done;
			}
			/*end by mtk80707*/
		...
		*/
			...
			wlan_pre_state = wlan_state;
			wlan_state = WLAN_STATE_RX_PWR_ON;
			if (powered == 1) {
		...
		*/
		...
		/*
		handle AP-P2P-STA mode switch
		*/
		else if (local[0] == 'S' || local[0] == 'P' || local[0] == 'A') {
			wlan_pre_state = wlan_state;
			if (wlan_pre_state == WLAN_STATE_RX_PWR_ON) {
				wlan_state = WLAN_STATE_RX_FW_SWITCH;
			} else {
				/*
				in here, means fw switch cmd is arrive before power on cmd,
				the previous rx cmd may be "power off" or  "fw switch",
				so chip power state may be on or off
				*/
				wlan_state = WLAN_STATE_RX_FW_SWITCH_WO_PWR_ON;
			}
			if (powered == 0) {
