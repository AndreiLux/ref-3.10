...
/* [COHEC_00006052] SW work-around solution:
   using CMD52 write instead of CMD53 write for CCIR, CHLPCR, CSDIOCSR */
	...
	if (wait) {
		...
		if (time_out != 0) {
			STPSDIO_DBG_FUNC("Do FW-Own back!(Wakeup) succeed\n");
			ret = 0;
		} else {
...
/* [COHEC_00006052] SW work-around solution:
   using CMD52 write instead of CMD53 write for CCIR, CHLPCR, CSDIOCSR */
			...
			STPSDIO_INFO_FUNC
			    ("sleeping check fail. stp_is_ready(%d) irq_pending(%d) tx_packet_num(%d) rx_pkt_len(%d)\n",
...
{
	ULONG len = count;
	pr_info(DFT_TAG "write parameter len = %d\n\r", (int)len);
...
static void stp_sdio_txdbg_dump(void)
{
	...
	for (i = 0; i < STP_SDIO_TXDBG_COUNT; ++i) {
		...
		if (0 == len)
			pr_info(DFT_TAG "idx(%x) 0 == len dump skip\n", idx);
		pr_info(DFT_TAG "stp_sdio_txdbg_buffer idx(%x) bus_txlen(0x%x, %d) ts(%d)\n",
		...
		pr_info(DFT_TAG "stp_sdio_txdbg_buffer dump ok\n");
	}
	...
	for (i = 0; i < STP_SDIO_TX_PKT_LIST_SIZE; ++i) {
		...
		pr_info(DFT_TAG "tx_pkt_list idx(0x%x, %d) size(0x%x, %d), in_ts(%d), out_ts(%d)\n",
	...
	for (i = 0; i < STP_SDIO_TX_BUF_CNT; ++i) {
		...
		pr_info(DFT_TAG "pkt_buf.tx_buf dump ok\n");
	}
 ...
 * \retval
 */
static int __init stp_sdio_init(void)
{
	int ret;
	int i;
 ...
 * \retval
 */
static void __exit stp_sdio_exit(void)
{
	STPSDIO_LOUD_FUNC("start\n");
