...
do { if (gHifSdioDbgLvl >= HIF_SDIO_LOG_WARN)	\
	osal_dbg_print(DFT_TAG"[W]%s(%d):"  fmt, __func__ , __LINE__, ##arg);	\
} while (0)
#define HIF_SDIO_ERR_FUNC(fmt, arg...)	\
do { if (gHifSdioDbgLvl >= HIF_SDIO_LOG_ERR)	\
	osal_dbg_print(DFT_TAG"[E]%s(%d):"  fmt, __func__ , __LINE__, ##arg);	\
} while (0)
...
#define mtk_wcn_sdio_irq_flag_set(enable_sdio_irq)
