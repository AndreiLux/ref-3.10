 ...
 */
INT32 mtk_wcn_hif_sdio_update_cb_reg(int (*ts_update) (void))
{
	if (ts_update) {
...
INT32 mtk_wcn_hif_sdio_client_reg(const MTK_WCN_HIF_SDIO_CLTINFO *pinfo)
{
	...
	for (i = 0; i < pinfo->func_tbl_size; i++) {
		...
		if (ret) {
			...
			HIF_SDIO_WARN_FUNC(" of sdio_func are duplicated in g_hif_sdio_clt_drv_list!\n");
			goto out;
		}
	}
