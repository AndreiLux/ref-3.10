...
#define REFRESH_TIME    300	/* sec */
	...
	if (!query_cond) {
		...
		if ((now_time.tv_sec < query_time.tv_sec) ||
			...
			WMT_INFO_FUNC("temp query timeout (%d sec); retrigger\n", REFRESH_TIME);
			for (index = 0; index < HISTORY_NUM; index++)
				temp_table[index] = 99;
		}
	}
...
static int wmt_probe(struct platform_device *pdev)
{
	struct mtk_combo_data *pdata = dev_get_platdata(&pdev->dev);
	...
	if (!pdata || !pdata->pin_info) {
		WMT_ERR_FUNC("Platform data is missing or invalid\n");
		return -EINVAL;
	}
...
static int wmt_remove(struct platform_device *pdev)
{
