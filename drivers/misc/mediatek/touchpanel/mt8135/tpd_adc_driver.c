...
static int raw_x, raw_y, pre_x = 0, pre_y;
...
int tpd_driver_local_init(void)
{
	...
	if (request_irq
	    (MT65XX_TOUCH_IRQ_LINE, (irq_handler_t) tpd_handler_penirq, IRQF_TRIGGER_FALLING,
	     "mtk_tpd_penirq", NULL)) {
	...
	if (request_irq
	    (MT65XX_TOUCH_BATCH_LINE, tpd_handler_batchirq, IRQF_TRIGGER_FALLING, "mtk_tpd_favirq",
	     NULL)) {
...
int tpd_mode_select(int mode)
{
	...
	case FAV_MODE_HW:
		...
		break;
	case RAW_DATA_MODE:
		pr_info("RAW_DATA_MODE\n");
		break;
	default:
		pr_info("invalid!!! tpd_sample_mode=%d\n", tpd_sample_mode);
		return -EINVAL;
	}
...
/* else if(cy>(TPD_RES_Y-TPD_RES_Y/100)) cy=TPD_RES_Y-TPD_RES_Y/100; */
	...
	/* pr_err("tpd, raw_x=%d, raw_y=%d, cx=%d, cy=%d, cp=%d, down\n", raw_x, raw_y, cx, cy, cp); */
}
...
/* else if(cy>(TPD_RES_Y-TPD_RES_Y/100)) cy=TPD_RES_Y-TPD_RES_Y/100; */
	...
	/* pr_err("tpd, raw_x=%d, raw_y=%d, cx=%d, cy=%d, cp=%d, up\n", raw_x, raw_y, cx, cy, cp); */
}
...
int tpd_sampling(int *cx, int *cy, int *cp, int *cd)
{
	...
	if (tpd_sample_mode & TPD_SW || tpd_sample_mode & RAW_DATA_MODE) {
		for (i = 0; i < 8; i++) {
			...
			TPD_EM_PRINT(rx, ry, rz1, rz2, ((rx + 1) * (rz2 - rz1) / (rz1 + 1)),
				     TPD_TYPE_RAW_DATA);
			udelay(5);
		}
...
void tpd_tasklet(unsigned long unused)
{
	...
	if (tpd_em_pressure_threshold >= TPD_PRESSURE_NICE) {
		if (cp > tpd_em_pressure_threshold && cd != 0) {
			...
			TPD_EM_PRINT(cp, tpd_em_pressure_threshold, debt0, debt1, spl_num,
				     TPD_TYPE_REJECT2);
			if (tpd_sample_mode & FAV_MODE_SW || tpd_sample_mode & FAV_MODE_HW
			    || tpd_sample_mode & RAW_DATA_MODE) {
				tpd_fav_switch(1);
		...
		if (cp > TPD_PRESSURE_NICE && cd != 0) {
			...
			debt1 = tpd_get_debounce_time_DEBT1();
			spl_num = tpd_get_spl_number();
			TPD_EM_PRINT(cp, TPD_PRESSURE_NICE, debt0, debt1, spl_num,
				     TPD_TYPE_REJECT2);
			if (tpd_sample_mode & FAV_MODE_SW || tpd_sample_mode & FAV_MODE_HW
			    || tpd_sample_mode & RAW_DATA_MODE) {
				tpd_fav_switch(1);
	...
	if (cd == 1) {
		...
		/* pr_info("pre_x = %d, pre_y = %d, cx = %d, cy = %d\n", pre_x, pre_y, cx, cy); */

		if (pre_x != 0 || pre_y != 0) {
			if (atomic_read(&count_down) == 0) {
				do_gettimeofday(&first_report);
				latency_us =
				    ((first_report.tv_sec & 0xFFF) * 1000000 +
				     first_report.tv_usec) - ((d_time.tv_sec & 0xFFF) * 1000000 +
							      d_time.tv_usec);
			...
			if (cy == TPD_RES_Y)
				cy = TPD_RES_Y - 1;
			/* pr_info("[tpd_log]pre_x:0x%x, 0x%x\n", pre_x, pre_y); */
