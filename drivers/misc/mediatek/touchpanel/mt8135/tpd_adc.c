...
u16 tpd_read(int position, int raw_data_offset)
{
	if (tpd_sample_mode & TPD_SW) {
		...
		default:
		case TPD_X:
			return tpd_read_adc(TP_CMD_ADDR_X);
		case TPD_Y:
			return tpd_read_adc(TP_CMD_ADDR_Y);
		case TPD_Z1:
			return tpd_read_adc(TP_CMD_ADDR_Z1);
		case TPD_Z2:
			return tpd_read_adc(TP_CMD_ADDR_Z2);
		}
		return 0;
	}
	...
	if (tpd_sample_mode & FAV_MODE_HW || tpd_sample_mode & FAV_MODE_SW) {
		...
		default:
		case TPD_X:
			return __raw_readw(AUXADC_TP_DATA0) & 0xffff;
		case TPD_Y:
			return __raw_readw(AUXADC_TP_DATA1) & 0xffff;
		case TPD_Z1:
			return __raw_readw(AUXADC_TP_DATA2) & 0xffff;
		case TPD_Z2:
			return __raw_readw(AUXADC_TP_DATA3) & 0xffff;
		}
		return 0;
	}
	...
	if (tpd_sample_mode & RAW_DATA_MODE) {
		...
		default:
		case TPD_X:
			return tpd_read_rawdata(AUXADC_TP_RAW_X_DAT0, raw_data_offset);
		case TPD_Y:
			return tpd_read_rawdata(AUXADC_TP_RAW_Y_DAT0, raw_data_offset);
		case TPD_Z1:
			return tpd_read_rawdata(AUXADC_TP_RAW_Z1_DAT0, raw_data_offset);
		case TPD_Z2:
			return tpd_read_rawdata(AUXADC_TP_RAW_Z2_DAT0, raw_data_offset);
		}
		return 0;
	}
	return -1;
...
u16 tpd_read_adc(u16 pos)
{
	...
	while (TP_CON_SPL_MASK & __raw_readw(AUXADC_TP_CON0)) {;
