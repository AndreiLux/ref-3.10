...
void mt_dump_sched_traces(void)
{
	...
	for_each_possible_cpu(cpu) {
		...
		pr_info("==CPU[%d]==\n", cpu);
		b_isr->cur_event == 0 ?
		    pr_info("[ISR]last_irq:%d, dur:%llu ns (s:%llu, e:%llu)\n\n",
		...
		b_sq->cur_event == 0 ?
		    pr_info("[Softirq]last#:%d, dur:%llu ns (s:%llu, e:%llu)\n\n",
...
/****  Dump Stop Events ****/
		e_irq->cur_ts == 0 ?
		    pr_info("[IRQ disable] last duration:%llu ns (s: %llu, e: %llu)\n\n",
		...
		e_pmpt->cur_ts == 0 ?
		    pr_info("[Preempt disable] last duration:%llu ns(s: %llu, e: %llu)\n\n",
...
void mt_show_current_irq_counts(void)
{
	...
	pr_info("=========================================\nIRQ Status:\n");
	pr_info("Current irq counts record at [%llu] ns.(last at %llu, diff:+%llu ns)\n", t_cur,
	...
	for (cpu = 0; cpu < num_possible_cpus(); cpu++) {
		...
		for (irq = 0; irq < NR_IRQS; irq++) {
			...
			if (count != 0)
				pr_info(" IRQ[%3d:%14s] = %8d, (+%d times in %lld us)\n", irq,
	...
	for (cpu = 0; cpu < num_possible_cpus(); cpu++) {
		...
		for (irq = 0; irq < NR_IPI; irq++) {
			...
			if (count != 0)
				pr_info(" IRQ[%2d:  IPI] = %8d,(+%d times in %lld us)\n", irq,
					count, count - mt_local_irq_count[cpu][irq],
					usec_high(t_diff));
		}
	}
#endif
...
void mt_show_timer_info(void)
{
	int cpu;
	for_each_possible_cpu(cpu) {
		pr_info("[TimerISR#%d] last timer ISR start:%llu ns, end:%llu ns\n", cpu,
