...
void setup_mtproc_info(struct task_struct *p, unsigned long long ts)
{
	...
	if (!mtproc) {
		pr_info("mtproc unable to kmalloc\n");
		return;
	}
...
void end_mtproc_info(struct task_struct *p)
{
	...
	if (mtproc == NULL) {
		pr_info("pid:%d can't be found in mtsched proc_info.\n", p->pid);
		mutex_unlock(&mt_cputime_lock);
		return;
	}
...
void set_mtprof_comm(char *comm, int pid)
{
	...
	if (mtproc == NULL) {
		pr_info("[mtprof] no matching pid\n");
		mutex_unlock(&mt_cputime_lock);
		return;
	}
...
void mt_cputime_switch(int on)
{
	pr_info("Original mtsched enabled = %d, on = %d.\n", mtsched_enabled, on);
	mutex_lock(&mt_cputime_lock);
	...
	mutex_unlock(&mt_cputime_lock);
	pr_info("Current mtsched enabled = %d\n", mtsched_enabled);
}
