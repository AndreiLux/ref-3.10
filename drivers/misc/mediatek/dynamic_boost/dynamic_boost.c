...
#define MAX_CORES_NUMBER NR_CPUS
...
static int dboost_dvfs_hotplug_thread(void *ptr)
{
	...
	while (!kthread_should_stop()) {
		...
		/* Set cores */
		if (cores_to_set == MAX_CORES_NUMBER)
			cmd = "cpu_min_num_big=2\n cpu_min_num_little=2";
		else if (cores_to_set == 2)
			cmd = "cpu_min_num_big=0\n cpu_min_num_little=2";
		else
			cmd = "cpu_max_num_big=0\n cpu_min_num_little=0";
