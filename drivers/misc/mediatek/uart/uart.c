...
struct mtk_uart *mtk_uart_get_uart(int id)
{
	struct mtk_uart *uart = 0;
...
unsigned int mtk_uart_pdn_enable(char *port, int enable)
{
	...
	uart = &mtk_uarts[port_num];
	uart->deep_idle = enable;
	if (enable)
...
static int mtk_uart_syscore_suspend(void)
{
	...
	unsigned long flags;
	struct mtk_uart *uart;
	struct mtk_uart_mode_data *umd;
	struct platform_device *pdev;

	for (i = 0; i < UART_NR; ++i) {
		uart = &mtk_uarts[i];
		if (uart->deep_idle) {
			umd = uart->port.dev->platform_data;
			pdev = to_platform_device(uart->port.dev);
			spin_lock_irqsave(&mtk_uart_bt_lock, flags);
			ret = uart_suspend_port(&mtk_uart_drv, &uart->port);
			if (umd->suspend)
				umd->suspend(pdev, PMSG_SUSPEND);
			spin_unlock_irqrestore(&mtk_uart_bt_lock, flags);
			pr_notice("[UART%d]%s: ret=%d\n", uart->nport, __func__, ret);
		}
	}
	return ret;
}
...
static int mtk_uart_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (console_suspend_enabled == 0 && uart == console_port && uart->poweron_count > 0)
		mtk_uart_save(uart);
	if (uart && (uart->nport < UART_NR) && (!uart->deep_idle)) {
		struct mtk_uart_mode_data *umd = uart->port.dev->platform_data;
		ret = uart_suspend_port(&mtk_uart_drv, &uart->port);
		if (umd->suspend)
			umd->suspend(pdev, state);
		dev_info(&pdev->dev, "[UART%d]%s: ret=%d\n", uart->nport, __func__, ret);
	}
	return ret;
}
