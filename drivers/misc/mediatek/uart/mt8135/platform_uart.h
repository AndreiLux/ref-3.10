...
#if defined(CONFIG_MT8135_FPGA_CA7)
#define UART_NR                     2
#else
#define UART_NR                     4
#endif
...
/*------ PDN Section -----------------------*/
#define PDN_FOR_UART1	MT_CG_PERI_UART0_PDN
#define PDN_FOR_UART2	MT_CG_PERI_UART1_PDN
#define PDN_FOR_UART3	MT_CG_PERI_UART2_PDN
#define PDN_FOR_UART4	MT_CG_PERI_UART3_PDN

#define PDN_FOR_DMA     MT_CG_PERI_AP_DMA_PDN
...
do {    \
	...
	if (DBG_EVT_##evt & DBG_EVT_ERR) \
	    ...
	    pr_info("  [UART%d]:%c: " fmt , uart->nport, s[0], ##args); \
} \
} while (0)
/*---------------------------------------------------------------------------*/
...
#define MSG_TRC(fmt, args...)   pr_info("[UARTX]:T: " fmt, ##args)
#define DEV_TRC(fmt, args...)   pr_info("[UART%d]:T: " fmt, uart->nport, ##args)
#define DEV_ERR(fmt, args...)   pr_info("[UART%d]:E: " fmt, uart->nport, ##args)
/*---------------------------------------------------------------------------*/
...
/* When set "1", enable flow control
triggered by RX FIFO full when VFIFO_EN
is set. */
...
/*when UART issues flush to DMA and
all data in UART VFIFO is transferred
to VFF */
