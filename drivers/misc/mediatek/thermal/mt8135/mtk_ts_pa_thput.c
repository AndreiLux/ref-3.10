...
static ssize_t mtk_mdm_proc_timeout_write(struct file *file, const char *buf, size_t len,
					  loff_t *data)
{
...
static int __init mtk_mdm_txpwr_init(void)
{
	...
	if (!mdtxpwr_dir) {
		...
		if (!entry) {
			mtk_mdm_dprintk
			    ("[mtk_mdm_init]: create /proc/mtk_mdm_txpwr/txpwr_sw failed\n");
		}
		entry =
		...
		if (!entry) {
			mtk_mdm_dprintk
			    ("[mtk_mdm_init]: create /proc/mtk_mdm_txpwr/txpwr_value failed\n");
		}
		entry =
		...
		if (!entry) {
			mtk_mdm_dprintk
			    ("[mtk_mdm_init]: create /proc/mtk_mdm_txpwr/timeout failed\n");
		}
	}
