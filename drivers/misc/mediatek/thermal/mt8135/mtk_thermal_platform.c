...
/* Number of CPU CORE */
#if defined(CONFIG_ARCH_MT6589) || defined(CONFIG_ARCH_MT6582) || defined(CONFIG_ARCH_MT8135)
...
#elif defined(CONFIG_ARCH_MT6589) || defined(CONFIG_ARCH_MT6582) || defined(CONFIG_ARCH_MT8135)
...
{
#if defined(CONFIG_ARCH_MT6577) || defined(CONFIG_ARCH_MT6589) || defined(CONFIG_ARCH_MT6582) || defined(CONFIG_ARCH_MT8135)
	/* ****************** */
	/* CPU Usage */
	/* ****************** */
...
#if defined(CONFIG_ARCH_MT6589) || defined(CONFIG_ARCH_MT8135)
...
int mtk_thermal_get_gpu_info(int *nocores, int **gpufreq, int **gpuloading)
{
	...
	if (gpufreq) {
		gpufreqs[0] = mt_gpufreq_get_cur_freq() / 1000;	/* the return value is KHz */
		*gpufreq = gpufreqs;
	}

	if (gpuloading) {
		gpuloadings[0] = mt_gpufreq_get_cur_load();
...
int mtk_thermal_get_batt_info(int *batt_voltage, int *batt_current, int *batt_temp)
{
	...
	if (batt_current) {
		*batt_current =
		...
		if (KAL_TRUE == battery_meter_get_battery_current_sign())
			*batt_current *= -1;
#endif
	}
#elif defined(CONFIG_ARCH_MT6589) || defined(CONFIG_ARCH_MT6582) || defined(CONFIG_ARCH_MT8135)
