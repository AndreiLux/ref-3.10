...
static int mtktstdpa_debug_log;
...
static int mtktstdpa_get_hw_temp(void)
{
	...
	if (i == size) {
		...
		if ((p_info[i].value > 100) | (p_info[i].value < -30))
			pr_info("[Power/TDPA_Thermal] TDPA T=%d\n", p_info[i].value);
...
static int mtktstdpa_get_temp(struct thermal_zone_device *thermal, unsigned long *t)
{
	*t = mtktstdpa_get_hw_temp();
	return 0;
}

static int mtktstdpa_bind(struct thermal_zone_device *thermal, struct thermal_cooling_device *cdev)
{
	int table_val = 0;
...
static int mtktstdpa_get_mode(struct thermal_zone_device *thermal, enum thermal_device_mode *mode)
{
...
static int mtktstdpa_set_mode(struct thermal_zone_device *thermal, enum thermal_device_mode mode)
{
	kernelmode = mode;
	return 0;
}
...
static int mtktstdpa_get_crit_temp(struct thermal_zone_device *thermal, unsigned long *temperature)
{
...
static int sysrst_set_cur_state(struct thermal_cooling_device *cdev, unsigned long state)
{
	...
	if (cl_dev_sysrst_state == 1) {
		pr_info("Power/TDPA_Thermal: reset, reset, reset!!!");
		pr_info("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
		pr_info("*****************************************");
		pr_info("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
...
static ssize_t mtktstdpa_write(struct file *file, const char *buffer, unsigned long count,
			       void *data)
{
	...
	if (sscanf
		...
		mtktstdpa_dprintk
		    ("[mtktstdpa_write] g_THERMAL_TRIP_0=%d,g_THERMAL_TRIP_1=%d,g_THERMAL_TRIP_2=%d,g_THERMAL_TRIP_3=%d,g_THERMAL_TRIP_4=%d,\
g_THERMAL_TRIP_5=%d,g_THERMAL_TRIP_6=%d,g_THERMAL_TRIP_7=%d,g_THERMAL_TRIP_8=%d,g_THERMAL_TRIP_9=%d,\n",
