...
static unsigned int g_trip_temp[COOLER_NUM] = { 120000, 85000, 85000, 85000, 0, 0, 0, 0, 0, 0 };
...
static int wmt_wifi_tx_thro_show(struct seq_file *s, void *v)
{
	seq_printf(s, "%lu\n", tx_throughput);
...
{
	return single_open(file, wmt_wifi_tx_thro_show, NULL);
}
...
static int wmt_wifi_algo_show(struct seq_file *s, void *v)
{
	seq_printf(s, "[up]\t%3d(sec)\t%2d/%2d\n[low]\t%3d(sec)\t%2d/%2d\nrst=%2d\n",
...
{
	return single_open(file, wmt_wifi_algo_show, NULL);
}
...
static int wmt_tm_wfd_show(struct seq_file *s, void *v)
{
	seq_printf(s, "%d", tm_wfd_stat);
...
{
	return single_open(file, wmt_tm_wfd_show, NULL);
}
...
static int wmt_tm_pid_show(struct seq_file *s, void *v)
{
...
{
	return single_open(file, wmt_tm_pid_show, NULL);
}

#define check_str(x) (x[0] == '\0'?"none\t":x)

static int wmt_tm_show(struct seq_file *s, void *v)
{
	struct linux_thermal_ctrl_if *p_linux_if = 0;
...
{
	return single_open(file, wmt_tm_show, NULL);
}
...
static ssize_t wmt_tm_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf
		...
		if (p_linux_if->thz_dev) {
			p_linux_if->thz_dev->polling_delay = time_msec;
			schedule_work(&twork);
		}
...
static void wmt_tm_work(struct work_struct *work)
{
	struct linux_thermal_ctrl_if *p_linux_if = 0;
	if (pg_wmt_tm) {
		p_linux_if = &pg_wmt_tm->linux_if;
