...
static int mtktsskin_debug_log;
static int kernelmode;
static int mtktsskin_formula;
static int mtktsskin_param[6][8] = {
...
static int mtktsskin_get_hw_temp(void)
{
	...
	if (ts_skin_at_boot_time == 0) {
		...
		mtktsskin_dprintk
		    ("[mtktsskin_get_hw_temp] at boot time, return 100002 as default\n");
		return 100002;
	}
	/* get HW SKIN temp */
...
static int mtktsskin_get_temp(struct thermal_zone_device *thermal, unsigned long *t)
{
	*t = mtktsskin_get_hw_temp();
	return 0;
}

static int mtktsskin_bind(struct thermal_zone_device *thermal, struct thermal_cooling_device *cdev)
{
	int table_val = 0;
...
static int mtktsskin_get_mode(struct thermal_zone_device *thermal, enum thermal_device_mode *mode)
{
	*mode = (kernelmode) ? THERMAL_DEVICE_ENABLED : THERMAL_DEVICE_DISABLED;
	return 0;
}

static int mtktsskin_set_mode(struct thermal_zone_device *thermal, enum thermal_device_mode mode)
{
	kernelmode = mode;
	return 0;
}
...
static int mtktsskin_get_crit_temp(struct thermal_zone_device *thermal, unsigned long *temperature)
{
...
static int sysrst_set_cur_state(struct thermal_cooling_device *cdev, unsigned long state)
{
	...
	if (cl_dev_sysrst_state == 1) {
		pr_info("Power/skin_Thermal: reset, reset, reset!!!");
		pr_info("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
		pr_info("*****************************************");
		pr_info("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
...
static int mtktsskin_register_thermal(void);
static void mtktsskin_unregister_thermal(void);

static ssize_t mtktsskin_write(struct file *file, const char *buffer, unsigned long count,
			       void *data)
{
	...
	if (sscanf
		...
		mtktsskin_dprintk
		    ("[mtktsskin_write] g_THERMAL_TRIP_0=%d,g_THERMAL_TRIP_1=%d,g_THERMAL_TRIP_2=%d,g_THERMAL_TRIP_3=%d,g_THERMAL_TRIP_4=%d,\
g_THERMAL_TRIP_5=%d,g_THERMAL_TRIP_6=%d,g_THERMAL_TRIP_7=%d,g_THERMAL_TRIP_8=%d,g_THERMAL_TRIP_9=%d,\n",
...
static int mtktsskin_write_param(struct file *file, const char *buffer, unsigned long count,
				 void *data)
{
	char desc[32];
	int len = 0;
...
static int mtktsskin_read_abb(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
	...
	p += sprintf(p, "[ mtktsskin_read_abb] param = %d %d %d %d %d %d %d %d\n",
		     mtktsskin_param[0][0], mtktsskin_param[0][1], mtktsskin_param[0][2],
		     mtktsskin_param[0][3], mtktsskin_param[0][4]
...
static int mtktsskin_write_abb(struct file *file, const char *buffer, unsigned long count,
			       void *data)
{
	char desc[64];
	int len = 0;
	...
	if (sscanf(desc, "%d %d %d %d %d %d %d %d", &mtktsskin_param[0][0], &mtktsskin_param[0][1]
		   , &mtktsskin_param[0][2], &mtktsskin_param[0][3], &mtktsskin_param[0][4],
		   &mtktsskin_param[0][5]
		   , &mtktsskin_param[0][6], &mtktsskin_param[0][7]) == 8) {
		mtktsskin_dprintk("[mtktsskin_write_abb] param=%d %d %d %d %d %d %d %d\n",
				  mtktsskin_param[0][0], mtktsskin_param[0][1],
				  mtktsskin_param[0][2], mtktsskin_param[0][3],
				  mtktsskin_param[0][4], mtktsskin_param[0][5],
				  mtktsskin_param[0][6], mtktsskin_param[0][7]);
		return count;
	} else {
...
static int mtktsskin_read_battery(char *buf, char **start, off_t off, int count, int *eof,
				  void *data)
{
	int len = 0;
	char *p = buf;
	...
	p += sprintf(p, "[ mtktsskin_read_battery] param = %d %d %d %d %d %d %d %d\n",
		     mtktsskin_param[1][0], mtktsskin_param[1][1], mtktsskin_param[1][2],
		     mtktsskin_param[1][3], mtktsskin_param[1][4]
...
static int mtktsskin_write_battery(struct file *file, const char *buffer, unsigned long count,
				   void *data)
{
	char desc[64];
	int len = 0;
	...
	if (sscanf(desc, "%d %d %d %d %d %d %d %d", &mtktsskin_param[1][0], &mtktsskin_param[1][1]
		   , &mtktsskin_param[1][2], &mtktsskin_param[1][3], &mtktsskin_param[1][4],
		   &mtktsskin_param[1][5]
		   , &mtktsskin_param[1][6], &mtktsskin_param[1][7]) == 8) {
		mtktsskin_dprintk("[mtktsskin_write_battery] param=%d %d %d %d %d %d %d %d\n",
				  mtktsskin_param[1][0], mtktsskin_param[1][1],
				  mtktsskin_param[1][2], mtktsskin_param[1][3],
				  mtktsskin_param[1][4], mtktsskin_param[1][5],
				  mtktsskin_param[1][6], mtktsskin_param[1][7]);
		return count;
	} else {
...
static int mtktsskin_read_cpu(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
	...
	p += sprintf(p, "[ mtktsskin_read_cpu] param = %d %d %d %d %d %d %d %d\n",
		     mtktsskin_param[2][0], mtktsskin_param[2][1], mtktsskin_param[2][2],
		     mtktsskin_param[2][3], mtktsskin_param[2][4]
...
static int mtktsskin_write_cpu(struct file *file, const char *buffer, unsigned long count,
			       void *data)
{
	char desc[64];
	int len = 0;
	...
	if (sscanf(desc, "%d %d %d %d %d %d %d %d", &mtktsskin_param[2][0], &mtktsskin_param[2][1]
		   , &mtktsskin_param[2][2], &mtktsskin_param[2][3], &mtktsskin_param[2][4],
		   &mtktsskin_param[2][5]
		   , &mtktsskin_param[2][6], &mtktsskin_param[2][7]) == 8) {
		mtktsskin_dprintk("[mtktsskin_write_cpu] param=%d %d %d %d %d %d %d %d\n",
				  mtktsskin_param[2][0], mtktsskin_param[2][1],
				  mtktsskin_param[2][2], mtktsskin_param[2][3],
				  mtktsskin_param[2][4], mtktsskin_param[2][5],
				  mtktsskin_param[2][6], mtktsskin_param[2][7]);
		return count;
	} else {
...
static int mtktsskin_read_cpu_up(char *buf, char **start, off_t off, int count, int *eof,
				 void *data)
{
	int len = 0;
	char *p = buf;
	...
	p += sprintf(p, "[ mtktsskin_read_cpu_up] param = %d %d %d %d %d %d %d %d\n",
		     mtktsskin_param[3][0], mtktsskin_param[3][1], mtktsskin_param[3][2],
		     mtktsskin_param[3][3], mtktsskin_param[3][4]
...
static int mtktsskin_write_cpu_up(struct file *file, const char *buffer, unsigned long count,
				  void *data)
{
	char desc[64];
	int len = 0;
	...
	if (sscanf(desc, "%d %d %d %d %d %d %d %d", &mtktsskin_param[3][0], &mtktsskin_param[3][1]
		   , &mtktsskin_param[3][2], &mtktsskin_param[3][3], &mtktsskin_param[3][4],
		   &mtktsskin_param[3][5]
		   , &mtktsskin_param[3][6], &mtktsskin_param[3][7]) == 8) {
		mtktsskin_dprintk("[mtktsskin_write_cpu_up] param=%d %d %d %d %d %d %d %d\n",
				  mtktsskin_param[3][0], mtktsskin_param[3][1],
				  mtktsskin_param[3][2], mtktsskin_param[3][3],
				  mtktsskin_param[3][4], mtktsskin_param[3][5],
				  mtktsskin_param[3][6], mtktsskin_param[3][7]);
		return count;
	} else {
...
static int mtktsskin_read_pa(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
	...
	p += sprintf(p, "[ mtktsskin_read_pa] param = %d %d %d %d %d %d %d %d\n",
		     mtktsskin_param[4][0], mtktsskin_param[4][1], mtktsskin_param[4][2],
		     mtktsskin_param[4][3], mtktsskin_param[4][4]
...
static int mtktsskin_write_pa(struct file *file, const char *buffer, unsigned long count,
			      void *data)
{
	char desc[64];
	int len = 0;
	...
	if (sscanf(desc, "%d %d %d %d %d %d %d %d", &mtktsskin_param[4][0], &mtktsskin_param[4][1]
		   , &mtktsskin_param[4][2], &mtktsskin_param[4][3], &mtktsskin_param[4][4],
		   &mtktsskin_param[4][5]
		   , &mtktsskin_param[4][6], &mtktsskin_param[4][7]) == 8) {
		mtktsskin_dprintk("[mtktsskin_write_pa] param=%d %d %d %d %d %d %d %d\n",
				  mtktsskin_param[4][0], mtktsskin_param[4][1],
				  mtktsskin_param[4][2], mtktsskin_param[4][3],
				  mtktsskin_param[4][4], mtktsskin_param[4][5],
				  mtktsskin_param[4][6], mtktsskin_param[4][7]);
		return count;
	} else {
...
static int mtktsskin_read_pmic(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
	...
	p += sprintf(p, "[ mtktsskin_read_pmic] param = %d %d %d %d %d %d %d %d\n",
		     mtktsskin_param[5][0], mtktsskin_param[5][1], mtktsskin_param[5][2],
		     mtktsskin_param[5][3], mtktsskin_param[5][4]
...
static int mtktsskin_write_pmic(struct file *file, const char *buffer, unsigned long count,
				void *data)
{
	char desc[64];
	int len = 0;
	...
	if (sscanf(desc, "%d %d %d %d %d %d %d %d", &mtktsskin_param[5][0], &mtktsskin_param[5][1]
		   , &mtktsskin_param[5][2], &mtktsskin_param[5][3], &mtktsskin_param[5][4],
		   &mtktsskin_param[5][5]
		   , &mtktsskin_param[5][6], &mtktsskin_param[5][7]) == 8) {
		mtktsskin_dprintk("[mtktsskin_write_pmic] param=%d %d %d %d %d %d %d %d\n",
				  mtktsskin_param[5][0], mtktsskin_param[5][1],
				  mtktsskin_param[5][2], mtktsskin_param[5][3],
				  mtktsskin_param[5][4], mtktsskin_param[5][5],
				  mtktsskin_param[5][6], mtktsskin_param[5][7]);
		return count;
	} else {
