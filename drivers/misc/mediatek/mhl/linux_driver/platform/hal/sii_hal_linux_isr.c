...
{
	pr_info("[mhl]mhl interrupt detected!!!\n");
...
static int mhl_irq_kthread(void *data)
{
	...
	for (;;) {
		...
		pr_info("mhl_irq_kthread, mhl irq received\n");
		/* hdmi_update_impl(); */
...
halReturn_t HalInstallIrqHandler(fwIrqHandler_t irqHandler)
{
	...
	mt65xx_eint_registration(CUST_EINT_MHL_NUM, 0, MT65XX_EINT_POL_NEG, &mhl8338_irq_handler,
				 0);
