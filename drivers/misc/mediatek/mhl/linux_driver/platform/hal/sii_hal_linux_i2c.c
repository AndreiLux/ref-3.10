...
halReturn_t HalOpenI2cDevice(char const *DeviceName, char const *DriverName)
{
	...
	/* pr_info("gMhlDevice.driver.driver.name=%s\n", gMhlDevice.driver.driver.name); */
	/* pr_info("gMhlI2cIdTable.name=%s\n", gMhlI2cIdTable[0].name); */
	retVal = i2c_add_driver(&mhl_i2c_driver);
	/* pr_info("gMhlDevice.pI2cClient =%p\n", gMhlDevice.pI2cClient); */
	if (retVal != 0) {
		pr_info("I2C driver add failed, retVal=%d\n", retVal);
...
{
	/* pr_info("hdmi enter I2C_ReadByte(0x%02x, 0x%02x)\n", */
	/* deviceID, offset); */
	...
	/* pr_info("hdmi exit I2C_ReadByte 0x%02x\n", data.byte); */
...
{
	/* pr_info("hdmi enter I2C_WriteByte(0x%02x, 0x%02x, 0x%02x)\n", */
...
{
	/* pr_info("hdmi enter %s (0x%02x, 0x%02x, 0x%02x)\n", __func__, deviceID, offset, len); */
	int i;
	uint8_t accessI2cAddr;
#if USE_DEFAULT_I2C_CODE
	union i2c_smbus_data data;
