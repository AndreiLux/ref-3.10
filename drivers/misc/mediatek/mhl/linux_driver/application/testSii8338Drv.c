		...
		for (j = 0; j <= 0x10; j++) {
			if (i == 0) {
				...
				if (j == 0)
					...
					status =
					    ioctl(gMhlDrv, SII_IOCTRL_REGISTER_READ, &RegisterInfo);
					if (status < 0) {
						printf(" XX");
					} else {
	...
	case 0x1:
		{
			...
			if (status < 0) {
				...
				printf("page_0x%x[0x%x] = 0x%x\n", RegisterInfo.dev_address,
				       RegisterInfo.subaddr, RegisterInfo.value);
...
int main(int argc, char **argv)
{
	...
	usleep(1000);
	printf
	    ("Welcome to the Silicon Image Transmitter test application\nAttempting to open driver %s\n",
	     mhl_linuxdrv_name);
