	...
	while (1) {
		...
		while (*msg) {
			...
			if (!strncmp(msg, "MHLEVENT=connected", strlen("MHLEVENT=connected"))) {
			    ...
			    if (!strncmp
				(msg, "MHLEVENT=disconnected", strlen("MHLEVENT=disconnected"))) {
				msg += strlen("MHLEVENT=disconnected");
				mhlTxEventPacket.event = MHL_TX_EVENT_DISCONNECTION;
			} else
			    if (!strncmp(msg, "MHLEVENT=rcp_ready", strlen("MHLEVENT=rcp_ready"))) {
				msg += strlen("MHLEVENT=rcp_ready");
				mhlTxEventPacket.event = MHL_TX_EVENT_RCP_READY;
			} else
			    if (!strncmp
				(msg, "MHLEVENT=received_RCPK key code=0x",
				 strlen("MHLEVENT=received_RCPK key code=0x"))) {
				char str[4] = "";
			    ...
			    if (!strncmp
				(msg, "MHLEVENT=received_RCPE error code=0x",
				 strlen("MHLEVENT=received_RCPE error code=0x"))) {
				char str[4] = "";
			    ...
			    if (!strncmp
				(msg, "MHLEVENT=received_RCP key code=0x",
				 strlen("MHLEVENT=received_RCP key code=0x"))) {
				char str[4] = "";
...
int Sii_CreateRecvThread(RecvThreadParam_t *pRecvThreadParam)
{
	...
	return pthread_create(&(pRecvThreadParam->recv_thread_fd), NULL, recv_thread,
			      (void *)pRecvThreadParam);
}
