...
void SiiMhlTxDeviceIsr(void)
{
	...
	unsigned long K1;
	unsigned long K2;
	pr_info("-------------------SiiMhlTxDeviceIsr start -----------------\n");
	K1 = get_jiffies_64();
#endif
	i = 0;
	do {
...
#ifdef TRACE_INT_TIME
	K2 = get_jiffies_64();
	pr_info("-------------------SiiMhlTxDeviceIsr last %d ms----------------\n",
		(int)(K2 - K1));
#endif
}
	...
	if (intStatus) {
		...
		if (abortReason & (BIT0 | BIT1 | BIT2 | BIT3 | BIT4 | BIT7)) {
			...
			if (abortReason & CBUSABORT_BIT_PEER_ABORTED) {
				...
				mscAbortFlag = true;
				SiiOsTimerCreate("Abort Time Out", SiiMhlMscAbortTimerCB, NULL,
						 true, 2000, false, &MscAbortTimer);
#endif
