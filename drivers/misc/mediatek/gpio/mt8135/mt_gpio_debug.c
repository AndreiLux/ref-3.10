...
void mt_gpio_self_test(void)
{
	...
	for (i = 0; i < MT_GPIO_EXT_MAX; i++) {
		...
		res = mt_set_gpio_dir(i, GPIO_DIR_OUT);
		if (res != RSUCCESS) {
			GPIOERR(" set dir out fail: %d\n", res);
			break;
		}

		res = mt_get_gpio_dir(i);
		if (res != GPIO_DIR_OUT) {
			...
			for (val = 0; val < GPIO_OUT_MAX; val++) {
				res = mt_set_gpio_out(i, 0);
				if (res != RSUCCESS) {
					GPIOERR(" set out[%d] fail: %d\n", val, res);
					break;
				}

				res = mt_get_gpio_out(i);
				if (res != 0) {
		...
		res = mt_set_gpio_dir(i, GPIO_DIR_IN);
		if (res != RSUCCESS) {
		...
		res = mt_set_gpio_dir(i, old);
		if (res != RSUCCESS) {
			GPIOERR(" restore dir fail: %d\n", res);
			break;
		}
		for (val = 0; val < GPIO_PULL_EN_MAX; val++) {
			res = mt_set_gpio_pull_enable(i, val);
			if (res != RSUCCESS) {
				GPIOERR(" set pullen[%d] fail: %d\n", val, res);
				break;
			}

			res = mt_get_gpio_pull_enable(i);
			if (res != val) {
		...
		res = mt_set_gpio_pull_enable(i, old);
		if (res != RSUCCESS) {
		...
		for (val = 0; val < GPIO_PULL_MAX; val++) {
			res = mt_set_gpio_pull_select(i, val);
			if (res != RSUCCESS) {
				GPIOERR(" set pullsel[%d] fail: %d\n", val, res);
				break;
			}

			res = mt_get_gpio_pull_select(i);
			if (res != val) {
		...
		res = mt_set_gpio_pull_select(i, old);
		if (res != RSUCCESS) {
		...
		for (val = 0; val < GPIO_DATA_INV_MAX; val++) {
			res = mt_set_gpio_inversion(i, val);
			if (res != RSUCCESS) {
				GPIOERR(" set inv[%d] fail: %d\n", val, res);
				break;
			}

			res = mt_get_gpio_inversion(i);
			if (res != val) {
		...
		res = mt_set_gpio_inversion(i, old);
		if (res != RSUCCESS) {
		...
		if ((i <= GPIOEXT6) || (i >= GPIOEXT9)) {
			...
			for (val = 0; val < GPIO_MODE_MAX; val++) {
				res = mt_set_gpio_mode(i, val);
				if (res != RSUCCESS) {
					GPIOERR("set mode[%d] fail: %d\n", val, res);
					break;
				}

				res = mt_get_gpio_mode(i);
				if (res != val) {
...
void gpio_dump_regs(void)
{
	...
	/* pr_info("idx = %3d: %d %d %d %d %d %d %d %d\n", */
			  ...
			  const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "-h", 2)) {
		...
		else if (8 == sscanf(buf, "=%d:%d %d %d %d %d %d %d", &pin, &mode, &pullsel,
				&din, &dout, &pullen, &dir, &dinv)) {
			GPIOMSG("set mode(%3d, %d)=%d\n", pin, mode, mt_set_gpio_mode(pin, mode));
			GPIOMSG("set psel(%3d, %d)=%d\n", pin, pullsel,
				mt_set_gpio_pull_select(pin, pullsel));
			GPIOMSG("set dout(%3d, %d)=%d\n", pin, dout, mt_set_gpio_out(pin, dout));
...
/*		mt_get_md_gpio_debug(md_str); */
		...
		ret =
		    mt_get_clock_output(num, (long unsigned int *)&src, (long unsigned int *)&div);
...
int mt_get_clock_output(long unsigned int num, long unsigned int *src, long unsigned int *div)
{
	return RSUCCESS;
}
