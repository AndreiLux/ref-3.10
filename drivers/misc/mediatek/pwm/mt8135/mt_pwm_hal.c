...
{
	if (!(0xff & (*power_flag))) {
		pr_info("<0>" "enable_clock: main\n");	/*  */
		enable_clock(MT_CG_PERI_PWM_PDN, "PWM");
		set_bit(7, power_flag);
	}
	if (!test_bit(pwm_no, power_flag)) {
		pr_info("<0>" "enable_clock: %d\n", pwm_no);	/*  */
		if (!pmic_pad) {	/* ap pad */
			pr_info("<0>" "ap pad\n");	/*  */
			enable_clock(pwm_power_id[pwm_no], "PWM");	/* enable clock */
		} else {
			pr_info("<0>" "pmic pad\n");	/*  */
#ifndef PWM_FPGA_LDVT_TEST
...
void mt_pwm_power_off_hal(U32 pwm_no, BOOL pmic_pad, unsigned long *power_flag)
{
	...
	if (!(0x7f & (*power_flag))) {
		pr_info("<0>" "disable_clock: main\n");	/*  */
		disable_clock(MT_CG_PERI_PWM_PDN, "PWM");
...
void mt_pwm_power_on_hal(U32 pwm_no, BOOL pmic_pad, unsigned long *power_flag)
{
	pr_info("<0>" "mt_pwm_power_on_hal: %d, *power_flag=0x%lx\n", pwm_no, *power_flag);	/*  */
	if (0 == (*power_flag)) {
...
void mt_pwm_power_off_hal(U32 pwm_no, BOOL pmic_pad, unsigned long *power_flag)
{
	pr_info("<0>" "mt_pwm_power_off_hal: %d, *power_flag=0x%lx\n", pwm_no, *power_flag);	/*  */
	if (test_bit(pwm_no, power_flag)) {
...
void mt_pwm_init_power_flag(unsigned long *power_flag)
{
	...
	pr_info("mt_pwm_init_power_flag 0x%lx\n", *power_flag);
}
#endif
...
void mt_pwm_dump_regs_hal(void)
{
	...
	PWMMSG("\r\n[PWM_ENABLE is:%x]\n\r ", reg_val);
	pr_info("<0>" "peri pdn0 clock: 0x%x\n", INREG32(PERI_PDN0_STA));
...
{				/*used only locally
				   ...
				   pr_info("<0>""=============mt_pmic_pwm1_test===============\n");

				   mt_pin_set_mode(GPIO73, MT_MODE_1);
				   conf.pwm_no = PWM1;
				   ...
				   pr_info("PWM: clk_div = %x, clk_src = %x, pwm_no = %x\n", conf.clk_div, conf.clk_src, conf.pwm_no);
				   pwm_set_spec_config(&conf);
				   //
				   mt_pin_set_mode(GPIOEXT34, MT_MODE_1);
				   conf.pwm_no = PWM7;
				   conf.pmic_pad = true;
				   pwm_set_spec_config(&conf); */
}
