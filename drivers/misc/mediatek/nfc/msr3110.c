...
static int nfc_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	my_pr_debug("%s master tx 0x%p id %s", __func__, client->adapter->algo->master_xfer,
		    id->name);
...
static struct i2c_board_info nfc_board_info __initdata = { I2C_BOARD_INFO(I2C_ID_NAME, 0x59) };
...
#endif
static ssize_t msr3110_dev_write(struct file *filp, const char __user *buf, size_t count,
				 loff_t *offset)
{
	ssize_t retVal = 0;
...
static u8 SerialFlash_UpgradeA3(u8 *bufptr, u32 len)
{
	...
	if (rstatus == 0) {
		my_pr_err("%s FAIL: SerialFlash_Chg_Flash_I2C_Addrs", __func__);
		error = 1;
		goto LAST_END;
	}
	my_pr_debug("%s Success: SerialFlash_Chg_Flash_I2C_Addrs", __func__);

	SerialFlash_EntryIspMode();

	SerialFlash_ReadChipID(id);
	my_pr_err("%s ID 0x%02X 0x%02X 0x%02X", __func__, id[0], id[1], id[2]);

	if (id[0] == 0xBF && id[1] == 0x25 && id[2] == 0x02) {
		flash_type = FLASH_SST;
		my_pr_err("%s SST Flash", __func__);
	} else if (id[0] == 0xC2 && id[1] == 0x20 && id[2] == 0x11) {
		flash_type = FLASH_MX;
		my_pr_err("%s MXIC Flash", __func__);
	}

	rstatus = SerialFlash_ReadStatus();
	my_pr_err("%s Flash Status = 0x%02X", __func__, rstatus);
	...
	if (error) {
		my_pr_err("%s Erase Timeout", __func__);
		goto LAST_END;
	}
	my_pr_debug("%s Erase Done %d", __func__, waitcnt);

	/* Write */
	my_pr_debug("%s FLASH_START: %ul", __func__, FLASH_START);
	for (offset = FLASH_START; offset < FLASH_END; offset += 256) {
		if ((offset % 0x1000) == 0) {
			my_pr_debug("%s Write Address = 0x%04X from 0x%04X", __func__, offset,
				    (unsigned int)(bufptr + offset));
		}
		/* pr_debug("data 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X", rdata[0], rdata[1],rdata[2],rdata[3],rdata[4],rdata[5],rdata[6],rdata[7]); */
	...
	for (offset = FLASH_START; offset < FLASH_END; offset += 4096) {
		int i;
		if (((offset) % 0x1000) == 0) {
			my_pr_info("%s Read Address = 0x%08X", __func__, offset);
		}
		...
		if (error) {
			my_pr_err("%s Verify Error at addr:0x%X", __func__, offset + i);
			my_pr_err("%s: WData = 0x%02X", __func__, bufptr[offset + i]);
			my_pr_err("%s RData = 0x%02X", __func__, rdata[i]);

			goto LAST_END;
		}
	}

	my_pr_info("%s Verify OK", __func__);
#endif
	...
	my_pr_debug("%s Verify OK", __func__);
#endif
...
static long msr3110_dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case MSR3110_IOCTL_IRQ:
		...
		my_pr_debug("%s 3. mt_get_gpio_mode(%d): %d", __func__, GPIO_IRQ_NFC_PIN_M_EINT,
			    mt_get_gpio_mode(GPIO_IRQ_NFC_PIN));
		my_pr_debug("%s 3. mt_get_gpio_dir(%d): %d", __func__, GPIO_DIR_IN,
			    mt_get_gpio_dir(GPIO_IRQ_NFC_PIN));
		my_pr_debug("%s 3. mt_get_gpio_pull_enable(%d): %d", __func__, GPIO_PULL_ENABLE,
			    mt_get_gpio_pull_enable(GPIO_IRQ_NFC_PIN));
		my_pr_debug("%s 3. mt_get_gpio_pull_select(%d): %d", __func__, GPIO_PULL_UP,
			    mt_get_gpio_pull_select(GPIO_IRQ_NFC_PIN));
		...
		irq_interrupt =
		    wait_event_interruptible(g_msr3110_dev->irq_wait_queue,
					     (g_msr3110_dev->irq_flag != NFC_IRQ_STS_WAIT));
	...
	case MSR3110_IOCTL_IRQ_ABORT:	/* Aborting the blocking-wait of IRQ */
		...
		if (retVal != NFC_IRQ_STS_WAIT) {
			...
			my_pr_err("%s IRQ_ABORT canceled. global irq_flag: %d", __func__,
				  g_msr3110_dev->irq_flag);
			goto end;
		}
		...
		my_pr_debug("%s 4. mt_get_gpio_mode(%d): %d", __func__, GPIO_IRQ_NFC_PIN_M_EINT,
			    mt_get_gpio_mode(GPIO_IRQ_NFC_PIN));
		my_pr_debug("%s 4. mt_get_gpio_dir(%d): %d", __func__, GPIO_DIR_IN,
			    mt_get_gpio_dir(GPIO_IRQ_NFC_PIN));
		my_pr_debug("%s 4. mt_get_gpio_pull_enable(%d): %d", __func__, GPIO_PULL_ENABLE,
			    mt_get_gpio_pull_enable(GPIO_IRQ_NFC_PIN));
		my_pr_debug("%s 4. mt_get_gpio_pull_select(%d): %d", __func__, GPIO_PULL_UP,
			    mt_get_gpio_pull_select(GPIO_IRQ_NFC_PIN));
	...
	case MSR3110_IOCTL_SET_IRQ_NFC_PIN:
		...
		if (copy_from_user(&pinVal, (int __user *)arg, sizeof(pinVal))) {
			my_pr_err("%s MSR3110_IOCTL_SET_IRQ_NFC_PIN FAIL: copy_from_user",
				  __func__);
			retVal = -EFAULT;
		}
	...
	case MSR3110_IOCTL_SET_NFC_EINT_PIN:
		...
		if (copy_from_user(&pinVal, (int __user *)arg, sizeof(pinVal))) {
			my_pr_err("%s MSR3110_IOCTL_SET_NFC_EINT_PIN FAIL: copy_from_user",
				  __func__);
			retVal = -EFAULT;
		}
...
void msr3110_dev_irq_handler(void)
{
	...
	/* disable irq */
	my_pr_debug("%s mt_get_gpio_in( GPIO_IRQ_NFC_PIN): %d", __func__,
		    mt_get_gpio_in(GPIO_IRQ_NFC_PIN));
...
static void msr3110_dev_irq_pin_ctrl(u8 IrqPinMode)
{
	...
	case NFC_IRQ_PIN_CTRL_IRQ_MODE:
		...
		mt65xx_eint_set_sens(CUST_EINT_EXT_IRQ_NFC_NUM, CUST_EINT_EXT_IRQ_NFC_SENSITIVE);
		mt65xx_eint_set_hw_debounce(CUST_EINT_EXT_IRQ_NFC_NUM,
					    CUST_EINT_EXT_IRQ_NFC_DEBOUNCE_CN);
		mt65xx_eint_registration(CUST_EINT_EXT_IRQ_NFC_NUM,
					 CUST_EINT_EXT_IRQ_NFC_DEBOUNCE_EN,
					 CUST_EINT_EXT_IRQ_NFC_POLARITY, msr3110_dev_irq_handler,
					 0);
		mt65xx_eint_mask(CUST_EINT_EXT_IRQ_NFC_NUM);
...
static int __init msr3110_dev_init(void)
{
...
#if 0
	my_pr_debug("%s mt_get_gpio_mode(%d): %d", __func__, GPIO_IRQ_NFC_PIN_M_EINT,
		    mt_get_gpio_mode(GPIO_IRQ_NFC_PIN));
	my_pr_debug("%s mt_get_gpio_dir(%d): %d", __func__, GPIO_DIR_IN,
		    mt_get_gpio_dir(GPIO_IRQ_NFC_PIN));
	my_pr_debug("%s mt_get_gpio_pull_enable(%d): %d", __func__, GPIO_PULL_ENABLE,
		    mt_get_gpio_pull_enable(GPIO_IRQ_NFC_PIN));
	my_pr_debug("%s mt_get_gpio_pull_select(%d): %d", __func__, GPIO_PULL_UP,
		    mt_get_gpio_pull_select(GPIO_IRQ_NFC_PIN));
	...
	my_pr_debug("%s 1. mt_get_gpio_mode(%d): %d", __func__, GPIO_IRQ_NFC_PIN_M_EINT,
		    mt_get_gpio_mode(GPIO_IRQ_NFC_PIN));
	my_pr_debug("%s 1. mt_get_gpio_dir(%d): %d", __func__, GPIO_DIR_IN,
		    mt_get_gpio_dir(GPIO_IRQ_NFC_PIN));
	my_pr_debug("%s 1. mt_get_gpio_pull_enable(%d): %d", __func__, GPIO_PULL_ENABLE,
		    mt_get_gpio_pull_enable(GPIO_IRQ_NFC_PIN));
	my_pr_debug("%s 1. mt_get_gpio_pull_select(%d): %d", __func__, GPIO_PULL_UP,
		    mt_get_gpio_pull_select(GPIO_IRQ_NFC_PIN));

	mt65xx_eint_set_sens(CUST_EINT_EXT_IRQ_NFC_NUM, CUST_EINT_EXT_IRQ_NFC_SENSITIVE);
	mt65xx_eint_set_hw_debounce(CUST_EINT_EXT_IRQ_NFC_NUM, CUST_EINT_EXT_IRQ_NFC_DEBOUNCE_CN);
	mt65xx_eint_registration(CUST_EINT_EXT_IRQ_NFC_NUM, CUST_EINT_EXT_IRQ_NFC_DEBOUNCE_EN,
				 CUST_EINT_EXT_IRQ_NFC_POLARITY, msr3110_dev_irq_handler, 0);
	mt65xx_eint_mask(CUST_EINT_EXT_IRQ_NFC_NUM);

	my_pr_debug("%s 2. mt_get_gpio_mode(%d): %d", __func__, GPIO_IRQ_NFC_PIN_M_EINT,
		    mt_get_gpio_mode(GPIO_IRQ_NFC_PIN));
	my_pr_debug("%s 2. mt_get_gpio_dir(%d): %d", __func__, GPIO_DIR_IN,
		    mt_get_gpio_dir(GPIO_IRQ_NFC_PIN));
	my_pr_debug("%s 2. mt_get_gpio_pull_enable(%d): %d", __func__, GPIO_PULL_ENABLE,
		    mt_get_gpio_pull_enable(GPIO_IRQ_NFC_PIN));
	my_pr_debug("%s 2. mt_get_gpio_pull_select(%d): %d", __func__, GPIO_PULL_UP,
		    mt_get_gpio_pull_select(GPIO_IRQ_NFC_PIN));
#endif
