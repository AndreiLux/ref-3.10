...
TVE_STATUS TVE_Init(void)
{
	...
	if (request_irq(MT6575_TVE_IRQ_ID,
			(irq_handler_t) _TVE_InterruptHandler, 0, "mt6575-tve", NULL) < 0) {
		pr_info("[TVE][ERROR] fail to request TVE irq\n");
...
TVE_STATUS TVE_DumpRegisters(void)
{
	...
	pr_info("---------- Start dump TVE registers ----------\n" "TVE_BASE: 0x%08x\n", TVE_BASE);

	for (i = 0; i < sizeof(TVE_REGS); i += 4) {
		pr_info("TVE+%04x : 0x%08x\n", i, INREG32(TVE_BASE + i));
	}
