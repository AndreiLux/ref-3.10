...
static void _WaitForEngineNotBusy(void)
{
	...
	if (in_interrupt()) {
		...
		while (_IsEngineBusy()) {
			long ret = wait_event_interruptible_timeout(_lcd_wait_queue,
								    ...
								    WAIT_TIMEOUT);
			if (0 == ret) {
				DISP_LOG_PRINT(ANDROID_LOG_WARN, "LCD",
					       "[WARNING] Wait for LCD engine not busy timeout!!!\n");
				LCD_DumpRegisters();
				if (LCD_REG->STATUS.WAIT_SYNC) {
					DISP_LOG_PRINT(ANDROID_LOG_WARN, "LCD",
						       "reason is LCD can't wait TE signal!!!\n");
					LCD_TE_Enable(FALSE);
				}
	...
	while (_IsEngineBusy()) {
		msleep(1);	/* sleep 1ms */
		wait_time++;
		if (wait_time > 20) {	/* timeout */
			pr_info("[WARNING] Wait for LCD engine not busy timeout!!!\n");
			LCD_DumpRegisters();

			if (LCD_REG->STATUS.WAIT_SYNC) {
				pr_info("reason is LCD can't wait TE signal!!!\n");
				LCD_TE_Enable(FALSE);
			}
			OUTREG16(&LCD_REG->START, 0);
			OUTREG16(&LCD_REG->START, 0x1);
...
#if ENABLE_LCD_INTERRUPT
	if (request_irq(MT8135_DISP_DBI_IRQ_ID,
...
LCD_STATUS LCD_ConfigParallelIF(LCD_IF_ID id,
				...
				UINT32 writeWait,
				UINT32 readSetup,
				UINT32 readHold, UINT32 readLatency, UINT32 waitPeriod, UINT32 chw)
{
...
LCD_STATUS LCD_ConfigSerialIF(LCD_IF_ID id,
			      ...
			      UINT32 css,
			      UINT32 csh,
			      UINT32 rd_1st, UINT32 rd_2nd, UINT32 wr_1st, UINT32 wr_2nd)
{
	LCD_REG_SCNF config;
...
/* OUTREG32(&LCD_REG->RESET,reset); */
	LCD_OUTREG32(DISPSYS_BASE + 0x64, high);
...
LCD_STATUS LCD_ConfigDSIIfFormat(LCD_DSI_IF_FMT_COLOR_ORDER order,
				 ...
				 LCD_DSI_IF_FORMAT format, UINT32 packet_size, bool DC_DSI)
{
	/* MT6583 not support */
	return LCD_STATUS_OK;
}
...
LCD_STATUS LCD_TE_ConfigVHSyncMode(UINT32 hsDelayCnt,
				   UINT32 vsWidthCnt, LCD_TE_VS_WIDTH_CNT_DIV vsWidthCntDiv)
{
/*    LCD_REG_TECON tecon = LCD_REG->TEARING_CFG;
...
inline static void _LCD_WriteIF(DWORD baseAddr, UINT32 value, LCD_IF_MCU_WRITE_BITS bits)
{
	switch (bits) {
...
inline static UINT32 _LCD_ReadIF(DWORD baseAddr, LCD_IF_MCU_WRITE_BITS bits)
{
	switch (bits) {
	case LCD_IF_MCU_WRITE_8BIT:
...
LCD_STATUS LCD_Dump_Layer_Info(void)
{
	...
	pr_info("LCD_Dump_Layer_Info:\n");
	for (i = 0; i < 4; i++) {
		pr_info
		    ("layer=%d, en=%d, src=%d, fmt=%d, addr=0x%x, (%d, %d, %d, %d), pitch=%d, keyEn=%d, key=0x%x, aen=%d, alpha=0x%x, isTdshp=%d, curr_buff_idx=%d, next_buff_idx=%d\n",
		     cached_layer_config[i].layer, cached_layer_config[i].layer_en,
		     cached_layer_config[i].source, cached_layer_config[i].fmt,
		     cached_layer_config[i].addr, cached_layer_config[i].dst_x,
		     cached_layer_config[i].dst_y, cached_layer_config[i].dst_w,
		     cached_layer_config[i].dst_h, cached_layer_config[i].src_pitch,
		     cached_layer_config[i].keyEn, cached_layer_config[i].key,
		     cached_layer_config[i].aen, cached_layer_config[i].alpha,
		     cached_layer_config[i].isTdshp, realtime_layer_config[i].buff_idx,
...
LCD_STATUS LCD_Dynamic_Change_FB_Layer(unsigned int isAEEEnabled)
{
	...
	pr_info("[DDP] LCD_Dynamic_Change_FB_Layer, isAEEEnabled=%d\n", isAEEEnabled);
	LCD_Dump_Layer_Info();

	if (DISP_DEFAULT_UI_LAYER_ID == DISP_CHANGED_UI_LAYER_ID) {
		pr_info("[DDP] LCD_Dynamic_Change_FB_Layer, no dynamic switch\n");
		return LCD_STATUS_OK;
	}
...
#if defined(CONFIG_MTK_M4U_SUPPORT)
	_m4u_lcdc_func.m4u_dma_cache_maint(M4U_CLNTMOD_LCDC, (void *)pvbuf, w * h * bpp / 8,
					   DMA_BIDIRECTIONAL);
#endif
...
LCD_STATUS LCD_Capture_Framebuffer(unsigned int pvbuf, unsigned int bpp)
{
	...
	if (!_m4u_lcdc_func.isInit) {
		...
		unsigned int fbv =
		    (unsigned int)ioremap_cached((unsigned int)LCD_REG->LAYER[FB_LAYER].ADDRESS,
						 fbsize);
		unsigned short *fbvt = (unsigned short *)fbv;

		DBI_LOG
		    ("[LCDC init M4U] M4U not been init for LCDC, this should only happened in recovery mode\n");
		for (i = 0; i < h; i++) {
			for (j = 0; j < w_xres; j++) {
				t = fbvt[j + i * w];
				*(unsigned short *)(pvbuf + i * w_xres * pixel_bpp +
						    j * pixel_bpp) = t;
			}
		}
		iounmap((void *)fbv);
		...
		_m4u_lcdc_func.m4u_alloc_mva(M4U_CLNTMOD_LCDC, pvbuf,
	...
	LCD_SetOutputAlpha(0xff);
#if defined(CONFIG_MTK_M4U_SUPPORT)
	_m4u_lcdc_func.m4u_dma_cache_maint(M4U_CLNTMOD_LCDC, (unsigned int *)pvbuf,
	...
	_RestoreLCDRegisters();
#if defined(CONFIG_MTK_M4U_SUPPORT)
	_m4u_lcdc_func.m4u_dealloc_mva(M4U_CLNTMOD_LCDC, pvbuf,
				       DISP_GetScreenHeight() * DISP_GetScreenWidth() * bpp / 8,
				       ppbuf);
#endif
#else
	...
	pr_info("before alloc MVA: va = 0x%x, size = %d\n", pvbuf,
		DISP_GetScreenHeight() * DISP_GetScreenWidth() * bpp / 8);
	ret =
	    m4u_alloc_mva(M4U_CLNTMOD_WDMA, pvbuf,
			  DISP_GetScreenHeight() * DISP_GetScreenWidth() * bpp / 8, 0, 0, &mva);
	if (ret != 0) {
...
/* _WaitForEngineNotBusy(); */
	...
	m4u_dealloc_mva(M4U_CLNTMOD_WDMA,
			pvbuf, DISP_GetScreenHeight() * DISP_GetScreenWidth() * bpp / 8, mva);
#endif
#endif
	return LCD_STATUS_OK;
}
...
LCD_STATUS LCD_AllocUIMva(unsigned int va, unsigned int *mva, unsigned int size)
{
	...
	_m4u_lcdc_func.m4u_insert_tlb_range(M4U_CLNTMOD_LCDC_UI,
					    *mva, *mva + size - 1, RT_RANGE_HIGH_PRIORITY, 0);
#endif
	return LCD_STATUS_OK;
}
