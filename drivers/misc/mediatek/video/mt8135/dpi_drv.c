...
DPI_STATUS DPI_Init(BOOL isDpiPoweredOn)
{
	...
	MASKREG32(DISPSYS_BASE + 0x60, 0x2001, 0x2001);
#if ENABLE_DPI_INTERRUPT
	if (request_irq(MT8135_DISP_DPI0_IRQ_ID,
...
{
#if ENABLE_DPI_INTERRUPT
	free_irq(MT8135_DISP_DPI0_IRQ_ID, NULL);
#endif
	return DPI_STATUS_OK;
}
...
#endif
DPI_STATUS DPI_Init_PLL(unsigned int mipi_pll_clk_ref, unsigned int mipi_pll_clk_div1,
...
DPI_STATUS DPI_PowerOn(void)
{
	if (!s_isDpiPowerOn) {
		int ret = 0;
#if 1
		if (!clock_is_on(MT_CG_DISP_LVDS_DISP))
			ret += enable_clock(MT_CG_DISP_LVDS_DISP, "DPI0");
		if (!clock_is_on(MT_CG_DISP_LVDS_CTS))
			ret += enable_clock(MT_CG_DISP_LVDS_CTS, "DPI0");
		if (!clock_is_on(MT_CG_TOP_PDN_DPILVDS))
			ret += enable_clock(MT_CG_TOP_PDN_DPILVDS, "DPI0");
		if (ret > 0) {
...
#if 1
		ret = disable_clock(MT_CG_DISP_LVDS_DISP, "DPI0");
		ret += disable_clock(MT_CG_DISP_LVDS_CTS, "DPI0");
		ret += disable_clock(MT_CG_TOP_PDN_DPILVDS, "DPI0");
		if (1 == ret) {
			DISP_LOG_PRINT(ANDROID_LOG_ERROR, "DPI",
				       "power manager API return FALSE\n");
		}
...
DPI_STATUS DPI_ConfigLVDS(LCM_PARAMS *lcm_params)
{
	...
	if (lcm_params->dpi.lvds_tx_en == 1) {
		...
		OUTREG32(0xF0012518, 0x00203580);
		udelay(30);
		OUTREG32(0xF0012508, 0x00007fe0);
		udelay(5);
		OUTREG32(0xF0012504, 0x00c10fb7);
		udelay(5);
		OUTREG32(DISPSYS_BASE + 0xea20, 0x7);
		OUTREG32(DISPSYS_BASE + 0xea18, 0x1);
		udelay(5);
...
#if 0
		udelay(5);
		OUTREG32(DISPSYS_BASE + 0xea48, 0x032e0532);
		OUTREG32(DISPSYS_BASE + 0xea4c, 0x00060014);
		OUTREG32(DISPSYS_BASE + 0xea50, 0x000a0028);
		OUTREG32(DISPSYS_BASE + 0xea54, 0x03200500);
		OUTREG32(DISPSYS_BASE + 0xea58, 0x03998201);
		OUTREG32(DISPSYS_BASE + 0xea5c, 0x00ff8844);
		OUTREG32(DISPSYS_BASE + 0xea60, 0x00aaff55);
#endif
	}
