...
#define TV_INFO(fmt, arg...)       TV_PRINTF("[TV INFO]: %s(): "fmt, __func__, ##arg)
#define TV_WARNING(fmt, arg...)    TV_PRINTF("[TV WARNING]: %s(): "fmt, __func__, ##arg)
#define TV_ERROR(fmt, arg...)      TV_PRINTF("[TV ERROR]: %s(): %s@%d: "fmt, __func__, __FILE__, __LINE__, ##arg)
...
static BOOL _reset_tvrot(void)
{
	...
	while (TVR_REG->RESET.WARN_RESET) {
		++timeout;
		if (timeout > 100000) {
			pr_info("[TVR][ERROR] Reset timeout\n");
			return FALSE;
		}
	}
...
static BOOL _alloc_and_set_sram(const TVR_PARAM *param)
{
	...
	pr_info("[TVR] Allocate internal SRAM, main_size: %d, sub_size: %d\n", main_size, sub_size);
	...
	if (0 == main_buf_addr) {
		pr_info("[TVR][ERROR] allocate internal SRAM failed\n");
		return FALSE;
	}
...
static UINT32 _cal_dst_buffer_offset(const TVR_PARAM *param, UINT32 dstPitchInBytes)
{
	switch (param->rotation) {
	case TVR_ROT_180:
...
static BOOL _config_tvrot_reg(const TVR_PARAM *param)
{
	...
	/* descriptor mode */
	{
		...
		for (i = 0; i < ARY_SIZE(param->dstBufAddr); ++i) {
			...
			/*
			   pr_info("[TVR]58:%08x 60:%08x 68:%08x 70:%08x 78:%08x 318:%08x\n",
			   INREG32(0xf209f058),
			   INREG32(0xf209f060),
			   INREG32(0xf209f068),
			   INREG32(0xf209f070),
			...
			while (0 == TVR_REG->QUEUE_WSTA.OK) {
				++timeout;
				if (timeout > 100000) {
					pr_info("[TVR][ERROR] QUEUE_DATA timeout\n");
					return FALSE;
				}
			}
			OUTREG32(&TVR_REG->QUEUE_DATA, param->dstBufAddr[i] + dstBufOffset);
	...
	/* descriptor mode */
	{
		...
		for (i = 0; i < param->dstBufNum; ++i) {
			...
			while (0 == TVR_REG->QUEUE_WSTA.OK) {
				++timeout;
				if (timeout > 100000) {
					pr_info("[TVR][ERROR] QUEUE_DATA timeout dst addr\n");
					return FALSE;
				}
			}
			OUTREG32(&TVR_REG->QUEUE_DATA, param->dstBufAddr[i] + dstBufOffset);
			...
			while (0 == TVR_REG->QUEUE_WSTA.OK) {
				++timeout;
				if (timeout > 100000) {
					pr_info("[TVR][ERROR] QUEUE_DATA timeout src w/h\n");
					return FALSE;
				}
			}
			...
			while (0 == TVR_REG->QUEUE_WSTA.OK) {
				++timeout;
				if (timeout > 100000) {
					pr_info("[TVR][ERROR] QUEUE_DATA timeout src roi w/h\n");
					return FALSE;
				}
			}
			...
			while (0 == TVR_REG->QUEUE_WSTA.OK) {
				++timeout;
				if (timeout > 100000) {
					pr_info("[TVR][ERROR] QUEUE_DATA timeout src roi x/y\n");
					return FALSE;
				}
			}
			...
			while (0 == TVR_REG->QUEUE_WSTA.OK) {
				...
				if (timeout > 100000) {
					pr_info
					    ("[TVR][ERROR] QUEUE_DATA timeout dst with in byte\n");
					return FALSE;
				}
			}
			...
			while (0 == TVR_REG->QUEUE_WSTA.OK) {
				++timeout;
				if (timeout > 100000) {
					pr_info("[TVR][ERROR] QUEUE_DATA timeout con\n");
					return FALSE;
				}
			}
...
static __tcmfunc irqreturn_t _TVR_InterruptHandler(int irq, void *dev_id)
{
	...
	if (flag.FLAG1) {
		pr_info("[TVR][IRQ] FLAG1: SW configuration error\n");
		clr.FLAG1_CLR = 1;
	}
	if (flag.FLAG5) {
		pr_info("[TVR][IRQ] FLAG5: SW configuration error\n");
		clr.FLAG5_CLR = 1;
	}
...
TVR_STATUS TVR_Init(void)
{
	...
	if (request_irq(MT6575_TV_ROT_IRQ_ID,
			(irq_handler_t) _TVR_InterruptHandler, IRQF_TRIGGER_LOW, "mt6575-tvrot",
			NULL) < 0) {
		pr_info("[TVR][ERROR] fail to request TVR irq\n");
...
{
	if (!s_isSramAllocated) {
		pr_info("[TVR][ERROR] working SRAM is not allocated!!\n");
		return TVR_STATUS_ERROR;
	}
...
UINT32 TVR_GetWorkingAddr(void)
{
...
bool TVR_EnqueueBuffer(UINT32 addr, UINT32 *pAddrPrev)
{
	UINT32 dstPitchInBytes =
	    (_tvrContext.rot & 0x1) ? (_tvrContext.srcHeight * 2) : (_tvrContext.srcWidth * 2);
	UINT32 dstBufOffset;
	...
	/*
	   ...
	   pr_info("[TVR]40:%08x 48:%08x 50:%08x 80:%08x 318:%08x\n",
	   INREG32(0xf209f040),
	   INREG32(0xf209f048),
	   INREG32(0xf209f050),
	   INREG32(0xf209f080),
...
TVR_STATUS TVR_Wait_Done(void)
{
	...
	while (!TVR_REG->IRQ_FLAG.FLAG0) {
		++timeout;
		if (timeout > 100000) {
			pr_info("[TVR][ERROR] Wait done timeout\n");
			return TVR_STATUS_ERROR;
		}
	}
...
TVR_STATUS TVR_DeallocMva(unsigned int va, unsigned int size, unsigned int mva)
{
	...
	_m4u_tvout_func.m4u_invalid_tlb_range(M4U_CLNTMOD_TVROT,
					      (unsigned int)mva, (unsigned int)(mva + size - 1));
