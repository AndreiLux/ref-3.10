...
static UINT32 check_line_offset;

/* Optimized paramters for WVGA */
static bool is_used_tar_size_for_hqa;
...
static __tcmfunc irqreturn_t _TVC_InterruptHandler(int irq, void *dev_id)
{
	...
	/* pr_info("_TVC_InterruptHandler, status: %x\n", status); */

	if (1 == status.IRQ2) {
		status.IRQ2 = 0;
		pr_info("[WARN][TVC] Overrun !!\n");
	}
	if (1 == status.IRQ0) {
		status.IRQ0 = 0;
		/* pr_info("wake up tvc_checkline!!\n"); */
	...
	if (in_interrupt()) {
		...
		while (TVC_REG->REG_UPDATE.REG_RDY) {
			...
			if (_tvcContext.isTvcEnabled) {
				long ret =
				    wait_event_interruptible_timeout(_tvcContext.tvc_checkline,
								     !TVC_REG->REG_UPDATE.REG_RDY,
								     WAIT_TIMEOUT);
				if (0 == ret) {
					pr_info
					    ("[WARNING] Wait for TVC checkline IRQ timeout!!!\n");
				}
			} else {
				/*
...
static TVC_STATUS _AllocateInternalSRAM(void)
{
	...
	if (0 == _tvcContext.pfhDmaBufPA) {
		pr_info("[TVC][ERROR] allocate TVC internal SRAM failed\n");
...
TVC_STATUS TVC_Init(void)
{
	...
	if (request_irq(MT6575_TVC_IRQ_ID,
		...
		pr_info("[TVC][ERROR] fail to request TVC irq\n");
		return TVC_STATUS_ERROR;
	}
...
TVC_STATUS TVC_Enable(void)
{
	TV_INFO("\n");
	if (_tvcContext.isTvcEnabled) {
		pr_info("[TVC]TVC already enabled\n");
		return TVC_STATUS_OK;
	}
...
TVC_STATUS TVC_SetSrcSize(UINT32 width, UINT32 height)
{
	...
	if (_tvcContext.srcSize.width != width || _tvcContext.srcSize.height != height) {
...
TVC_STATUS TVC_SetTarSizeForHQA(BOOL enable)
{
	...
	pr_info("[TVC] Tar size for HQA is (%s)\n", (enable ? "on" : "off"));
	return TVC_STATUS_OK;
}
#endif
	...
	if (_tvcContext.srcFormatSizeDirty) {
		...
		/* Config Line Pitch */
		{
			...
			OFFSET.LINE_OFFSET = (TVC_YUV420_BLK == _tvcContext.srcFormat ||
			    ...
			    (_tvcContext.srcSize.width * 1) : (_tvcContext.srcSize.width * 2);
...
TVC_STATUS TVC_DumpRegisters(void)
{
	...
	pr_info("---------- Start dump TVC registers ----------\n" "TVC_BASE: 0x%08x\n", TVC_BASE);

	for (i = 0; i < sizeof(TVC_REGS); i += 4) {
		pr_info("TVC+%04x : 0x%08x\n", i, INREG32(TVC_BASE + i));
	}

	return TVC_STATUS_OK;
}

void TVC_ConfigSize(unsigned int src_width, unsigned int src_height, unsigned int tar_width,
		    unsigned int tar_height)
{
	...
	/* src size */
	if (_tvcContext.srcSize.width != src_width || _tvcContext.srcSize.height != src_height) {
...
TVC_STATUS TVC_CheckFormat(TVC_SRC_FORMAT format)
{
	if (format != TVC_RGB565 &&
	    format != TVC_YUV420_PLANAR && format != TVC_UYVY422 && format != TVC_YUV420_BLK)
		return TVC_STATUS_ERROR;
	else
		return TVC_STATUS_OK;
}
