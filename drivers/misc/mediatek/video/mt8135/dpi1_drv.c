...
#if (defined(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT) && defined(CONFIG_MTK_HDMI_HDCP_SUPPORT))
#if  (defined(CONFIG_MTK_INTERNAL_HDMI_SUPPORT) || defined(CONFIG_MTK_INTERNAL_MHL_SUPPORT))
#define OUTREG32(x, y) CA_DPI1_OUTREG32((unsigned int)x, (unsigned int)y)
#else
...
#if (defined(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT) && defined(CONFIG_MTK_HDMI_HDCP_SUPPORT))
#if  (defined(CONFIG_MTK_INTERNAL_HDMI_SUPPORT) || defined(CONFIG_MTK_INTERNAL_MHL_SUPPORT))
#define DPI1_MASKREG32(x, y, z)  CA_DPI1_MASKREG32(x, y, z)
#else
#define DPI1_MASKREG32(x, y, z)  MASKREG32(x, y, z)
#endif
#else
#define DPI1_MASKREG32(x, y, z)  MASKREG32(x, y, z)
#endif
...
#if (defined(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT) && defined(CONFIG_MTK_HDMI_HDCP_SUPPORT))
#if  (defined(CONFIG_MTK_INTERNAL_HDMI_SUPPORT) || defined(CONFIG_MTK_INTERNAL_MHL_SUPPORT))
void CA_DPI1_OUTREG32(unsigned int u4addr, unsigned int u4data)
{
	if ((0xF400F000 <= u4addr) && (u4addr < 0xF4010000))
		vCaDPI1WriteReg(u4addr, u4data);
	else
		mt65xx_reg_sync_writel(u4data, u4addr);
}

void CA_DPI1_MASKREG32(unsigned int u4addr, unsigned int u4msk, unsigned int u4data)
{

	CA_DPI1_OUTREG32(u4addr, (AS_UINT32(u4addr) & ~(u4msk)) | (u4data));
...
#ifndef BUILD_UBOOT
	wait_dpi_vsync = true;
	hrtimer_start(&hrtimer_vsync_dpi, ktime_set(0, VSYNC_US_TO_NS(vsync_timer_dpi1)),
		      HRTIMER_MODE_REL);
...
#if (defined(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT) && defined(CONFIG_MTK_HDMI_HDCP_SUPPORT))
#if  (defined(CONFIG_MTK_INTERNAL_HDMI_SUPPORT) || defined(CONFIG_MTK_INTERNAL_MHL_SUPPORT))
	DPI1_MASKREG32(DISPSYS_BASE + 0x60, 0xb03, 0xb03);
#else
...
#if ENABLE_DPI1_INTERRUPT
	if (request_irq(MT8135_DPI_IRQ_ID,
...
{
#if ENABLE_DPI1_INTERRUPT
	free_irq(MT8135_DPI_IRQ_ID, NULL);
#endif
	return DPI_STATUS_OK;
}
...
DPI_STATUS DPI1_Init_PLL(HDMI_VIDEO_RESOLUTION resolution)
{
	...
	default:
		{
			pr_info("[DPI1] not supported format, %s, %d, format = %d\n", __func__,
				__LINE__, resolution);
			break;
		}
	}
...
#if (defined(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT) && defined(CONFIG_MTK_HDMI_HDCP_SUPPORT))
#if  (defined(CONFIG_MTK_INTERNAL_HDMI_SUPPORT) || defined(CONFIG_MTK_INTERNAL_MHL_SUPPORT))
		DPI1_MASKREG32(0xC2080028, 0x10, 0x10);
#else
...
#if (defined(CONFIG_MTK_IN_HOUSE_TEE_SUPPORT) && defined(CONFIG_MTK_HDMI_HDCP_SUPPORT))
#if  (defined(CONFIG_MTK_INTERNAL_HDMI_SUPPORT) || defined(CONFIG_MTK_INTERNAL_MHL_SUPPORT))
		DPI1_MASKREG32(0xC2080018, 0x10, 0x10);
#else
...
DPI_STATUS DPI1_PowerOn(void)
{
	...
	if (!s_isDpiPowerOn) {
		int ret = 0;
#if 1				/* FIXME */
		ret = enable_clock(MT_CG_DISP_DPI1, "DPI1");
		ret += enable_clock(MT_CG_DISP_HDMI_DISP, "DPI1");
		ret += enable_clock(MT_CG_DISP_HDMI_PLL, "DPI1");
		ret += enable_clock(MT_CG_DISP_HDMI_AUDIO, "DPI1");
		ret += enable_clock(MT_CG_DISP_HDMI_SPDIF, "DPI1");
		if (ret > 0) {
...
DPI_STATUS DPI1_PowerOff(void)
{
	...
	if (s_isDpiPowerOn) {
		...
		_BackupDPIRegisters();
#if 1				/* FIXME */
		ret = disable_clock(MT_CG_DISP_DPI1, "DPI1");
		ret += disable_clock(MT_CG_DISP_HDMI_DISP, "DPI1");
		ret += disable_clock(MT_CG_DISP_HDMI_PLL, "DPI1");
		ret += disable_clock(MT_CG_DISP_HDMI_AUDIO, "DPI1");
		ret += disable_clock(MT_CG_DISP_HDMI_SPDIF, "DPI1");
		if (ret > 0) {
			DISP_LOG_PRINT(ANDROID_LOG_ERROR, "DPI",
				       "power manager API return FALSE\n");
		}
...
/* #ifndef BUILD_UBOOT */
	...
	while ((INREG32(DISP_MUTEX_BASE + 0x24) & 0x02) != 0) {
	}			/* polling until mutex lock complete */
#endif
	return DPI_STATUS_OK;
}
...
DPI_STATUS DPI1_ConfigVsync_LEVEN(UINT32 pulseWidth, UINT32 backPorch, UINT32 frontPorch,
				  BOOL fgInterlace)
{
	...
	vporch_leven.VBP_HALF_LEVEN = fgInterlace;
...
DPI_STATUS DPI1_ConfigVsync_REVEN(UINT32 pulseWidth, UINT32 backPorch, UINT32 frontPorch,
				  BOOL fgInterlace)
{
	...
	vporch_reven.VBP_HALF_REVEN = fgInterlace;
...
DPI_STATUS DPI1_DumpRegisters(void)
{
	...
	DISP_LOG_PRINT(ANDROID_LOG_WARN, "DPI1",
		       "---------- Start dump DPI1 registers ----------\n");
...
DPI_STATUS DPI1_Capture_Framebuffer(unsigned int pvbuf, unsigned int bpp)
{
	...
	if (pvbuf == 0 || bpp == 0) {
		DISP_LOG_PRINT(ANDROID_LOG_ERROR, "DPI",
			       "DPI1_Capture_Framebuffer, ERROR, parameters wrong: pvbuf=0x%08x, bpp=%d\n",
			       pvbuf, bpp);
		return DPI_STATUS_OK;
	}
	...
	if (DPI1_FBGetFormat() == DPI_FB_FORMAT_RGB565) {
		...
		DISP_LOG_PRINT(ANDROID_LOG_ERROR, "DPI",
			       "DPI1_Capture_Framebuffer, ERROR, dpi_fb_bpp is wrong: %d\n",
			       dpi_fb_bpp);
		return DPI_STATUS_OK;
	}
	...
	if (dpi_needPowerOff)
		...
		fbv =
		    (unsigned char *)ioremap_cached((unsigned int)DPI1_REG->FB[DPI1_GetCurrentFB()].
						    ADDR, fbsize);
	...
	if (bpp == 32 && dpi_fb_bpp == 24) {
		...
		DISP_LOG_PRINT(ANDROID_LOG_ERROR, "DPI",
			       "DPI1_Capture_Framebuffer, bpp:%d & dpi_fb_bpp:%d is not supported now\n",
			       bpp, dpi_fb_bpp);
	}
...
DPI_STATUS DPI1_MatrixCoef(UINT16 c00, UINT16 c01, UINT16 c02,
			   UINT16 c10, UINT16 c11, UINT16 c12, UINT16 c20, UINT16 c21, UINT16 c22)
{
...
DPI_STATUS DPI1_MatrixPreOffset(UINT16 preAdd0, UINT16 preAdd1, UINT16 preAdd2)
{
	...
	set0.MATRIX_PRE_ADD_0 = preAdd0;
	set0.MATRIX_PRE_ADD_1 = preAdd1;
...
DPI_STATUS DPI1_MatrixPostOffset(UINT16 postAdd0, UINT16 postAdd1, UINT16 postAdd2)
{
	...
	set0.MATRIX_POST_ADD_0 = postAdd0;
	set0.MATRIX_POST_ADD_1 = postAdd1;
...
DPI_STATUS DPI1_EmbeddedSyncSetting(BOOL embSync_R_Cr, BOOL embSync_G_Y, BOOL embSync_B_Cb,
				    BOOL esavFInv, BOOL esavVInv, BOOL esavHInv, BOOL esavCodeMan)
{
...
DPI_STATUS DPI1_OutputSetting(DPI_OUTPUT_BIT_NUM outBitNum, BOOL outBitSwap,
			      DPI_OUTPUT_CHANNEL_SWAP outChSwap, DPI_OUTPUT_YC_MAP outYCMap)
{
...
DPI_STATUS DPI1_ESAVVTimingControlLeft(UINT8 offsetOdd, UINT8 widthOdd, UINT8 offsetEven,
				       UINT8 widthEven)
{
...
DPI_STATUS DPI1_ESAVVTimingControlRight(UINT8 offsetOdd, UINT8 widthOdd, UINT8 offsetEven,
					UINT8 widthEven)
{
...
{ {{0x1f68, 0x1ed6, 0x01c2},	/* -0.148 , -0.291 ,  0.439  13bit: sign+2.10 */
   ...
   {0, 0, 0},
   {0x0100, 0x0800, 0x0800} },	/* 16 ,    128 ,    128   9bit: sign+8.0 */
...
{{0x1f53, 0x1ead, 0x0200},	/* -0.1687, -0.3313,  0.5     13bit: sign+2.10 */
 ...
 {0, 0, 0},
 {0x0000, 0x0800, 0x0800} },	/* 16 ,    128 ,    128   9bit: sign+8.0 */
  /* ----------------------------------------- */
  /* RGB to YUV HD */
  /* ----------------------------------------- */
...
DPI_STATUS DPI1_Config_ColorSpace(UINT8 ColorSpace, UINT8 HDMI_Res)
{
	...
	pr_info("[DPI1] DPI1_Config_ColorSpace: ColorSpace[%d] / HDMI Resolution[%d]\n",
		ColorSpace, HDMI_Res);

	if ((HDMI_Res == 0) || (HDMI_Res == 1))	/* SD */
	{
		if ((ColorSpace == YCBCR_422) || (ColorSpace == YCBCR_444)) {
			i = 0;
		} else if ((ColorSpace == YCBCR_422_FULL) || (ColorSpace == YCBCR_444_FULL)) {
			i = 1;
		} else {
			/* RGB */
		}
