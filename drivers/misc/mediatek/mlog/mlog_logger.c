...
#define M_FILTER_ALL        (M_MEMFREE | M_SWAPFREE | M_CACHED | M_GPUUSE | M_MLOCK | M_ACTIVE | M_INACTIVE | M_SHMEM)
...
int mlog_doread(char __user *buf, size_t len)
{
	...
	while (!error && (mlog_start != mlog_end) && len - i > MLOG_STR_LEN) {
		size_t size;
		int v;
		...
		size = snprintf(mlog_str, sizeof(mlog_str), strfmt_list[strfmt_idx++], v);

		if (strfmt_idx >= strfmt_len)
			strfmt_idx = strfmt_proc;

		spin_unlock_bh(&mlogbuf_lock);
		size = min(min(size, sizeof(mlog_str)-1), len - i);
