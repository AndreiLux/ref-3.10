...
static int kwdt_thread_test(void *arg)
{
	...
	for (;;) {
		pr_info("wd_test debug start, cpu:%d\n", cpu);
		spin_lock(&wdt_test_lock0);
		cpu = smp_processor_id();
		spin_unlock(&wdt_test_lock0);

		if (test_case == (cpu * 10 + 1)) {
			pr_info("CPU:%d, Preempt disable\n", cpu); /* cpu0 Preempt disale */
			spin_lock(&wdt_test_lock1);
		}
		if (test_case == (cpu * 10 + 2)) {
			pr_info("CPU:%d, irq & Preempt disable\n", cpu); /* cpu0 Preempt&irq disale */
			spin_lock_irqsave(&wdt_test_lock1, flags);
		}
		msleep(5 * 1000);	/* 5s */
		wdt_dump_reg();
		pr_info("wd_test debug end, cpu:%d\n", cpu);
	}
	return 0;
}
