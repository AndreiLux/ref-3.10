...
#define SPIDEV_LOG(fmt, args...) pr_info("[SPI-UT]: [%s]:[%d]" fmt, __func__, __LINE__, ##args)
			     ...
			     const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "-h", 2)) {
		...
		if (ret < 0) {
			...
			if (ret != 0) {
				ret = -ret;
				pr_err("Message transfer err:%d\n", ret);
			}
		}
	} else if (!strncmp(buf, "-stress", 7)) {
		if (sscanf(buf + 8, "%d", &len) == -1) {
			ret = -1;
			goto out;
		}
		stress_err = 0;
		if (len == 0) {
			/* xfer = (struct spi_transfer *)kzalloc(SPI_STRESS_MAX*sizeof( struct spi_transfer)
			 * , GFP_KERNEL); */
			...
			for (loop_count = 0; loop_count <= 5000; loop_count++) {
				/* msg_str = (struct spi_message *)kzalloc(SPI_STRESS_MAX * sizeof(struct spi_message)
				 *  , GFP_KERNEL); */
				/* xfer = ( struct spi_transfer * )kzalloc(SPI_STRESS_MAX * sizeof(struct spi_transfer)
				 *  , GFP_KERNEL); */
...
static DEVICE_ATTR(spi, 220, NULL, spi_store);
static DEVICE_ATTR(spi_msg, 220, NULL, spi_msg_store);
...
static int __init spi_dev_init(void)
{
	...
	pr_info("hank:dev 0xf0005d20=0x%x\n", __raw_readl((void const *)0xf0005d20));
