			 ...
			 struct page **pages, struct vm_area_struct **vmas)
{
	...
	if (!!pages != !!(gup_flags & FOLL_GET)) {
		M4UMSG
		    (" error: __m4u_get_user_pages !!pages != !!(gup_flags & FOLL_GET), pages=0x%x, gup_flags & FOLL_GET=0x%x\n",
	...
	do {
		...
		if (((~vma->vm_flags) & (VM_IO | VM_PFNMAP | VM_SHARED | VM_WRITE)) == 0) {
			M4UMSG
			    ("error: m4u_get_pages(): bypass pmem garbage pages! vma->vm_flags=0x%x, start=0x%x, module=%d\n",
		...
		do {
			...
			while (!page) {
				...
				ret =
				    handle_mm_fault(mm, vma, start,
						    (foll_flags & FOLL_WRITE) ? FAULT_FLAG_WRITE :
						    0);
				MMProfileLogEx(M4U_MMP_Events[PROFILE_FORCE_PAGING],
				...
				if (ret & VM_FAULT_ERROR) {
					if (ret & VM_FAULT_OOM) {
						M4UMSG
						    ("handle_mm_fault() error: no memory, aaddr:0x%08lx (%d pages are allocated), module=%d\n",
						     start, i, eModuleID);
						/* m4u_dump_maps(start); */
						return i ? i : -ENOMEM;
					}
					if (ret & (VM_FAULT_HWPOISON | VM_FAULT_SIGBUS)) {
						M4UMSG
						    ("handle_mm_fault() error: invalide memory address, vaddr:0x%lx (%d pages are allocated), module=%d\n",
						     start, i, eModuleID);
						/* m4u_dump_maps(start); */
						return i ? i : -EFAULT;
					}
			...
			if (IS_ERR(page)) {
				M4UMSG
				    ("handle_mm_fault() error: faulty page is returned, vaddr:0x%lx (%d pages are allocated), module=%d\n",
				     start, i, eModuleID);
				/* m4u_dump_maps(start); */
			...
			if (pages) {
				...
				/* make sure hw pte is not 0 */
				{
					...
					if (i == 3000)
						M4UMSG
						    ("error: cannot handle_mm_fault to get hw pte: va=0x%x\n",
						     start);
				}
