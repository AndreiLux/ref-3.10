			      ...
			      struct sg_table *sg_table)
{
	...
	if (pmodule_current_size[eModuleID] > gModuleMaxMVASize[eModuleID]) {
		...
		M4UMSG("hint: alloc mva but forget to free it %s 0x%x>0x%x!!\n",
			m4u_get_module_name(eModuleID),
			pmodule_current_size[eModuleID],
			gModuleMaxMVASize[eModuleID]);
	}
...
int m4u_debug_command(unsigned int command)
{
	...
	case 5:
		...
		m4u_enable_error_hang_tee(1, (bool) g_debug_enable_error_hang);
#endif
		break;
