...
#define DDPK_INFO(fmt, arg...)      if (b_ddp_show_info)       { DDPK_PRINTF("[DDPK INFO]: %s(): "fmt, __func__, ##arg); }
#define DDPK_WARNING(fmt, arg...)   if (b_ddp_show_warning)    { DDPK_PRINTF("[DDPK W]: %s(): "fmt, __func__, ##arg);   }
#define DDPK_ERROR(fmt, arg...)     if (b_ddp_show_error)      { DDPK_PRINTF("[DDPK E]: %s(): %s@%d: "fmt, __func__, __FILE__, __LINE__, ##arg); }
#define DDPK_ISRINFO(fmt, arg...)   if (b_ddp_show_isrinfo)    { DDPK_PRINTF("[DDPK ISR]: %s(): "fmt, __func__, ##arg);    }
