...
int RDMADisableInterrupt(unsigned idx)
{
	ASSERT(idx <= 2);
	DISP_REG_SET(idx * DISP_INDEX_OFFSET + DISP_REG_RDMA_INT_ENABLE, 0);
	DISP_REG_SET(idx * DISP_INDEX_OFFSET + DISP_REG_RDMA_INT_STATUS, 0);
...
int RDMA_SWReset(unsigned idx)
{
	unsigned int delay_cnt = 0;
	static unsigned int cnt;
	pr_info("[DDP] RDMA_SWReset called %d\n", cnt++);
	...
	while ((DISP_REG_GET(idx * DISP_INDEX_OFFSET + DISP_REG_RDMA_GLOBAL_CON) & 0x700) == 0x100) {
		...
		if (delay_cnt > 10000) {
			pr_info
			    ("[DDP] error, RDMA_SWReset(%d) timeout, stage 1! DISP_REG_RDMA_GLOBAL_CON=0x%x\n",
	...
	while ((DISP_REG_GET(idx * DISP_INDEX_OFFSET + DISP_REG_RDMA_GLOBAL_CON) & 0x700) != 0x100) {
		...
		if (delay_cnt > 10000) {
			pr_info
			    ("[DDP] error, RDMA_SWReset(%d) timeout, stage 2! DISP_REG_RDMA_GLOBAL_CON=0x%x\n",
			     idx, DISP_REG_GET(idx * DISP_INDEX_OFFSET + DISP_REG_RDMA_GLOBAL_CON));
			break;
		}
	}
	       ...
	       bool isRGBSwap)	/* ourput setting */
{
	...
	if ((0xFFFFFFFF != address) && (1 != idx))
		DISP_REG_SET(idx * DISP_INDEX_OFFSET + DISP_REG_RDMA_MEM_START_ADDR, address);
	else
		pr_err("Skip setting rdma address because hdmi path\n");
#else
