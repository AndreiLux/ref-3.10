...
static void process_dbg_opt(const char *opt)
{
	...
	if (0 == strncmp(opt, "regr:", 5)) {
		...
		unsigned int enable = (unsigned int)simple_strtoul(p, &p, 10);
		if (enable == 1) {
			pr_info("[DDP] debug=1, trigger AEE\n");
...
int ddp_mem_test2(void)
{
	...
	if (pSrc == 0) {
		pr_info("[DDP] error: dma_alloc_coherent error!  dma memory not available.\n");
		return 0;
	} else {
		pr_info("[ddp] pSrc=0x%x\n", (unsigned int)pSrc);
	}
	...
	if (result) {
		pr_info("[DDP] error: DDPK_Bitblt_Config fail!, ret=%d\n", result);
	}

	pr_info("DDP, DDPK_Bitblt module setting:\n");
	...
	if (result) {
		pr_info("[DDP] error: DDPK_Bitblt() fail, result=%d\n", result);
	}
	...
	/* result verify */
	{
		...
		for (t = 0; t < size; t++) {
			if (*((unsigned char *)pSrc + t) !=
			    *((unsigned char *)data_rgb888_64x64 + t)) {
				diff_cnt++;
				pr_info("t=%d, diff_cnt=%d, dst=0x%x, gold=0x%x\n",
					t,
					diff_cnt,
		...
		if (diff_cnt == 0)
			pr_info("ddp_mem_test src compare result: success\n");
		else {
			pr_info("[DDP] error: ddp_mem_test src compare result: fail\n");
			pr_info("detail, %d, %d, %%%d\n", diff_cnt, size, diff_cnt * 100 / size);
			result = -1;
		}
	}

	{
		...
		if (diff_cnt == 0)
			pr_info("ddp_mem_test result: success\n");
		else {
			pr_info("[DDP] error: ddp_mem_test result: fail\n");
			pr_info("detail, %d, %d, %%%d\n", diff_cnt, size, diff_cnt * 100 / size);
			result = -1;
		}
	}
...
int ddp_mem_test(void)
{
	...
	pSrc =
	    dma_alloc_coherent(NULL, DDP_TEST_WIDTH * DDP_TEST_HEIGHT * DDP_TEST_BPP,
			       (dma_addr_t *) &pSrcPa, GFP_KERNEL);
	if (pSrc == 0 || pSrcPa == 0) {
		pr_info("dma_alloc_coherent error!  dma memory not available.\n");
		return 0;
	} else {
		pr_info("[ddp] pSrc=0x%x, pSrcPa=0x%x\n", (unsigned int)pSrc,
			(unsigned int)pSrcPa);
	}
	memcpy((void *)pSrc, data_rgb888_64x64, DDP_TEST_WIDTH * DDP_TEST_HEIGHT * DDP_TEST_BPP);
	...
	pDst =
	    dma_alloc_coherent(NULL, DDP_TEST_WIDTH * DDP_TEST_HEIGHT * DDP_TEST_BPP,
			       (dma_addr_t *) &pDstPa, GFP_KERNEL);
	if (pDst == 0 || pDstPa == 0) {
		pr_info("dma_alloc_coherent error!  dma memory not available.\n");
		return 0;
	} else {
		pr_info("[ddp] pDst=0x%x, pDstPa=0x%x\n", (unsigned int)pDst,
			(unsigned int)pDstPa);
	}
	memset((void *)pDst, 0, DDP_TEST_WIDTH * DDP_TEST_HEIGHT * DDP_TEST_BPP);
	...
	pr_info("*after ddp test config start: -------------------\n");
	disp_dump_reg(DISP_MODULE_ROT);
	disp_dump_reg(DISP_MODULE_SCL);
	disp_dump_reg(DISP_MODULE_WDMA0);
	disp_dump_reg(DISP_MODULE_CONFIG);
	pr_info("*after ddp test config end: ---------------------\n");
	...
	pr_info("ddp_mem_test wdma wait done...\n");
	WDMAWait(0);
	pr_info("ddp_mem_test wdma done!\n");
	...
	if (0)			/* compare source */
	{
		...
		if (diff_cnt == 0)
			...
			pr_info("ddp_mem_test src compare result: fail\n");
			pr_info("detail, %d, %d, %%%d\n", diff_cnt, size, diff_cnt * 100 / size);
			result = -1;
		}
	}
	...
	if (1)			/* compare dst */
	{
		...
		if (diff_cnt == 0)
			...
			pr_info("ddp_mem_test result: fail\n");
			pr_info("detail, %d, %d, %%%d\n", diff_cnt, size, diff_cnt * 100 / size);
			result = -1;
		}
	}
	...
	if (0) {
		...
		unsigned int size = DDP_TEST_WIDTH * DDP_TEST_HEIGHT * DDP_TEST_BPP;
		for (t = 0; t < size; t++) {
			pr_info("0x%x, ", *((unsigned char *)pDst + t));
			if ((t + 1) % 12 == 0) {
				pr_info("\n%05d: ", (t + 1) / 12);
			}
		}
	 ...
	 */
	/* dealloc memory */
	dma_free_coherent(NULL, DDP_TEST_WIDTH * DDP_TEST_HEIGHT * DDP_TEST_BPP, pSrc,
			  (dma_addr_t) &pSrcPa);
	dma_free_coherent(NULL, DDP_TEST_WIDTH * DDP_TEST_HEIGHT * DDP_TEST_BPP, pDst,
			  (dma_addr_t) &pDstPa);
#endif
	return result;
