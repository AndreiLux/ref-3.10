...
#define DDP_SCL_LOG(fmt, arg...) {pr_info("[SCL]"); pr_info(fmt, ##arg); }
#else
#define DDP_SCL_LOG()
#endif
	      ...
	      int rotateDegree, int srcWidth, int srcHeight, int dstWidth, int dstHeight, int flip)
{
	...
	disp_scl_set_scale_param(srcWidth, srcHeight, dstWidth, dstHeight, isInterlace,	/* 0=progressive source, 1=interlaced source */
...
void disp_scl_set_scale_param(unsigned src_width, unsigned src_height, unsigned dst_width, unsigned dst_height, unsigned char interlaced,	/* 0=progressive source, 1=interlaced source */
	...
	if (interlaced) {
		if ((rotation == 0) || (rotation == 2)) {
			interlace_dir = 0;
			if (((rotation == 0) && (fieldID != 0))
			    || ((rotation == 2) && (fieldID == 0)))
				interlace_offset = 1;
			else
				interlace_offset = 0;
		...
		{
			interlace_dir = 1;
			if (((rotation == 1) && (flipping == fieldID))
			    || ((rotation == 3) && (flipping != fieldID)))
				interlace_offset = 1;
			else
				interlace_offset = 0;
		}
	...
	} else if (cutoff_freqz >= 13000)	/* FIR down-scale (Fc >= 0.198) XX:about 1/5 resize */
	{
		...
		if (cutoff_freqz >= 55888)
			HRZ_COEF_SEL = 6;	/* Fc >= 0.85278 */
		else if (cutoff_freqz >= 51337)
			HRZ_COEF_SEL = 7;	/* Fc >= 0.78333 */
		else if (cutoff_freqz >= 47878)
			HRZ_COEF_SEL = 8;	/* Fc >= 0.73056 */
		else if (cutoff_freqz >= 45147)
			HRZ_COEF_SEL = 9;	/* Fc >= 0.68889 */
		else if (cutoff_freqz >= 42325)
			HRZ_COEF_SEL = 10;	/* Fc >= 0.64583 */
		else if (cutoff_freqz >= 38912)
			HRZ_COEF_SEL = 11;	/* Fc >= 0.59375 */
		else if (cutoff_freqz >= 34816)
			HRZ_COEF_SEL = 12;	/* Fc >= 0.53125 */
		else if (cutoff_freqz >= 30948)
			HRZ_COEF_SEL = 13;	/* Fc >= 0.47222 */
		else if (cutoff_freqz >= 28217)
			HRZ_COEF_SEL = 14;	/* Fc >= 0.43056 */
		else if (cutoff_freqz >= 25941)
			HRZ_COEF_SEL = 15;	/* Fc >= 0.39583 */
		else if (cutoff_freqz >= 23211)
			HRZ_COEF_SEL = 16;	/* Fc >= 0.35417 */
		else if (cutoff_freqz >= 20633)
			HRZ_COEF_SEL = 17;	/* Fc >= 0.31483 */
		else if (cutoff_freqz >= 17902)
			HRZ_COEF_SEL = 18;	/* Fc >= 0.27317 */
		else if (cutoff_freqz >= 14746)
			HRZ_COEF_SEL = 19;	/* Fc >= 0.22500 */
		else if (cutoff_freqz >= 12015)
			HRZ_COEF_SEL = 20;	/* Fc >= 0.18333 */
		else
			HRZ_COEF_SEL = 21;	/* Fc >= 0.16000 */
	...
	} else if (cutoff_freqz >= 13000)	/* FIR down-scale (Fc >= 0.198) */
	{
		...
		if (cutoff_freqz >= 55888)
			VRZ_COEF_SEL = 6;	/* Fc >= 0.85278 */
		else if (cutoff_freqz >= 51337)
			VRZ_COEF_SEL = 7;	/* Fc >= 0.78333 */
		else if (cutoff_freqz >= 47878)
			VRZ_COEF_SEL = 8;	/* Fc >= 0.73056 */
		else if (cutoff_freqz >= 45147)
			VRZ_COEF_SEL = 9;	/* Fc >= 0.68889 */
		else if (cutoff_freqz >= 42325)
			VRZ_COEF_SEL = 10;	/* Fc >= 0.64583 */
		else if (cutoff_freqz >= 38912)
			VRZ_COEF_SEL = 11;	/* Fc >= 0.59375 */
		else if (cutoff_freqz >= 34816)
			VRZ_COEF_SEL = 12;	/* Fc >= 0.53125 */
		else if (cutoff_freqz >= 30948)
			VRZ_COEF_SEL = 13;	/* Fc >= 0.47222 */
		else if (cutoff_freqz >= 28217)
			VRZ_COEF_SEL = 14;	/* Fc >= 0.43056 */
		else if (cutoff_freqz >= 25941)
			VRZ_COEF_SEL = 15;	/* Fc >= 0.39583 */
		else if (cutoff_freqz >= 23211)
			VRZ_COEF_SEL = 16;	/* Fc >= 0.35417 */
		else if (cutoff_freqz >= 20633)
			VRZ_COEF_SEL = 17;	/* Fc >= 0.31483 */
		else if (cutoff_freqz >= 17902)
			VRZ_COEF_SEL = 18;	/* Fc >= 0.27317 */
		else if (cutoff_freqz >= 14746)
			VRZ_COEF_SEL = 19;	/* Fc >= 0.22500 */
		else if (cutoff_freqz >= 12015)
			VRZ_COEF_SEL = 20;	/* Fc >= 0.18333 */
		else
			VRZ_COEF_SEL = 21;	/* Fc >= 0.16000 */
	} else			/* CACC down-scale */
	{
		VRZ_POSIT = 0;	/* 0: before fifo (down-scale), 1: behind fifo (up-scale) */
		VRZ_MODE = 0;	/* 0: down-scale, 1: up-scale */
