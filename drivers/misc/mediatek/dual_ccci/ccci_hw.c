...
{
	int i, j;
	volatile unsigned int *curr_ccif_smem_addr =
	...
	CCCI_DBG_MSG(ccif->m_md_id, "cci",
		     ...
		     ccci_read32(MD_CCIF_START(ccif->m_md_reg_base)), CCIF_RCHNUM(ccif->m_reg_base),
		     ccci_read32(CCIF_RCHNUM(ccif->m_reg_base)));

	for (i = 0; i < 16; i++) {
		CCCI_DBG_MSG(ccif->m_md_id, "cci", "%08X: %08X %08X %08X %08X\n",
			     (unsigned int)curr_ccif_smem_addr, curr_ccif_smem_addr[0],
			     curr_ccif_smem_addr[1], curr_ccif_smem_addr[2],
			     curr_ccif_smem_addr[3]);
...
{
	int i;
	volatile unsigned int *ccif_tx_addr =
	    (volatile unsigned int *)CCIF_TXCHDATA(ccif->m_reg_base);
...
static int __ccif_v1_write_runtime_data(ccif_t *ccif, unsigned int buf[], int len)
{
	int i;
	volatile unsigned int *curr_ccif_smem_addr =
	    (unsigned int *)(ccif->m_reg_base + CCIF_STD_V1_RUN_TIME_DATA_OFFSET);
...
static int __ccif_v1_register_call_back(ccif_t *ccif, int (*push_func) (ccif_msg_t *, void *),
			...
			if (r_ch_val & (1 << rx_ch)) {
				...
				if ((lg_ch_rx_debug_enable[md_id] & ENABLE_ALL_RX_LOG) ||
					...
					CCCI_DBG_MSG(md_id, "cci",
						     ...
						     phy_ch_data.data[0], phy_ch_data.data[1],
						     phy_ch_data.channel, phy_ch_data.reserved,
						     rx_ch);
				}
				...
				if (unlikely(ccif->push_msg == NULL)) {
					...
					if (ccif->push_msg(&phy_ch_data, ccif->m_logic_ctl_block) !=
					    sizeof(ccif_msg_t)) {
