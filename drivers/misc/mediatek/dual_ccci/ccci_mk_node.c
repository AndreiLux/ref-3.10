...
static int register_dev_node(void *dev_class, const char *name, int major_id, int minor_start_id,
			     int index)
{
	int ret = 0;
	dev_t dev;
...
ssize_t ccci_attr_store(struct kobject *kobj, struct attribute *attr, const char *buf,
			size_t count);
...
/* global vars */
static ccci_info_t *ccci_sys_info;
			    ...
			    ssize_t(*store) (const char *, size_t))
{
	...
	while (ccci_default_attrs[i]) {
		if (!strncmp(ccci_default_attrs[i]->name, buf, strlen(ccci_default_attrs[i]->name))) {
			ccci_attr_temp =
			    container_of(ccci_default_attrs[i], ccci_attribute_t, attr);
			break;
		}
		i++;
	}
	...
	for (i = 0; i < num; i++) {
		...
		if (dev_node[i].ext_num == 0) {
			...
			for (j = 0; j < dev_node[i].ext_num; j++) {
				ret =
				    register_dev_node(dev_class, dev_node[i].name, major, minor, j);
				if (ret < 0) {
					CCCI_MSG_INF(md_id, "cci", "create %s device fail: %d\n",
						     dev_node[i].name, ret);
					return ret;
