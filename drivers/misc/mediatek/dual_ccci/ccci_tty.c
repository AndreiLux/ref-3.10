...
unsigned int tty_debug_enable[MAX_MD_NUM];
	...
	while (get_logic_ch_data(ch_info, &msg)) {
		...
		case CCCI_UART1_TX_ACK:
			...
			if (ccci_tty_writeable(&ctlb->ccci_tty_meta)) {
				...
				wake_up_interruptible_poll(&ctlb->ccci_tty_meta.poll_waitq_w,
							   POLLOUT);
			}
			break;
		...
		case CCCI_UART1_RX:
			if (ccci_tty_readable(&ctlb->ccci_tty_meta)) {
				...
				wake_up_interruptible_poll(&ctlb->ccci_tty_meta.poll_waitq_r,
							   POLLIN);
			}
			break;
		...
		case CCCI_UART2_TX_ACK:
			...
			if (ccci_tty_writeable(&ctlb->ccci_tty_modem)) {
				...
				wake_up_interruptible_poll(&ctlb->ccci_tty_modem.poll_waitq_w,
							   POLLOUT);
			}
			break;
		...
		case CCCI_UART2_RX:
			if (ccci_tty_readable(&ctlb->ccci_tty_modem)) {
				...
				wake_up_interruptible_poll(&ctlb->ccci_tty_modem.poll_waitq_r,
							   POLLIN);
			}
			break;
		...
		case CCCI_IPC_UART_TX_ACK:
			if (ccci_tty_writeable(&ctlb->ccci_tty_ipc)) {
				...
				wake_up_interruptible_poll(&ctlb->ccci_tty_ipc.poll_waitq_w,
							   POLLOUT);
			}
			break;
		...
		case CCCI_IPC_UART_RX:
			if (ccci_tty_readable(&ctlb->ccci_tty_ipc)) {
				...
				wake_up_interruptible_poll(&ctlb->ccci_tty_ipc.poll_waitq_r,
							   POLLIN);
			}
			break;
	...
	do {
		...
		if (size == 0) {
			if (file->f_flags & O_NONBLOCK) {
				...
				/* Block read */
				value =
				    wait_event_interruptible(tty_instance->read_waitq,
...
static ssize_t ccci_tty_write(struct file *file, const char __user *buf, size_t count,
			      loff_t *ppos)
{
	tty_instance_t *tty_instance = (tty_instance_t *) file->private_data;
	int ret = 0, part;
	...
	do {
		...
		if (size == 0) {
			if (file->f_flags & O_NONBLOCK) {
				...
				/* Block write */
				value =
				    wait_event_interruptible(tty_instance->write_waitq,
...
static int ccci_tty_open(struct inode *inode, struct file *file)
{
	...
	if (tty_instance->count > 1) {
		...
		tty_instance->ready = 1;
		snprintf(tty_instance->wakelock_name, sizeof(tty_instance->wakelock_name), "%s%d",
			 name, (md_id + 1));
		wake_lock_init(&tty_instance->wake_lock, WAKE_LOCK_SUSPEND,
			       tty_instance->wakelock_name);
...
int ccci_tty_init(int md_id)
{
	...
	ASSERT(ccci_uart_base_req(md_id, 0, (int *)&ctlb->uart1_shared_mem, &smem_phy, &smem_size)
	       == 0);
	...
	ASSERT(ccci_get_sub_module_cfg(md_id, "tty", (char *)&tty_buf_len, sizeof(int)) ==
	       sizeof(int));
	...
	/* MUX section */
	ASSERT(ccci_uart_base_req(md_id, 1, (int *)&ctlb->uart2_shared_mem, &smem_phy, &smem_size)
	       == 0);
	...
	/* for IPC uart */
	ASSERT(ccci_uart_base_req(md_id, 5, (int *)&ctlb->uart3_shared_mem, &smem_phy, &smem_size)
	       == 0);
