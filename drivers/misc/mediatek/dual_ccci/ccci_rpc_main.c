...
extern void ccci_rpc_work_helper(int md_id, int *p_pkt_num, RPC_PKT pkt[], RPC_BUF *p_rpc_buf,
...
static int rpc_write(int md_id, int buf_idx, RPC_PKT *pkt_src, unsigned int pkt_num)
{
	...
	rpc_buf_tmp =
	    (RPC_BUF *) ((unsigned int)(ctl_b->rpc_buf_vir) +
	...
	*((unsigned int *)pdata) = pkt_num;
	...
	msg.data0 =
	    (unsigned int)(ctl_b->rpc_buf_phy) - md_2_ap_phy_addr_offset_fixed +
	    ctl_b->rpc_smem_instance_size * buf_idx;
