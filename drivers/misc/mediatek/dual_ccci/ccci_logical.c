...
const logic_channel_static_info_t logic_ch_static_info_tab[] = {
	...
	{CCCI_FS_TX, 0, "md_nvram_tx",
	 L_CH_ATTR_TX | L_CH_ATTR_PRVLG1 | L_CH_ATTR_OPEN_CLEAR | L_CH_ATTR_PRVLG0 |
	 L_CH_ATTR_PRVLG2},
...
int get_md_wakeup_src(int md_id, char *buf, unsigned int len)
{
	...
	if (curr_str_len > 255) {
		pr_info("[ccci/ctl] wakeup source buffer not enough(req:%d>255) for MD%d\n",
			curr_str_len, md_id + 1);
	}

	pr_info("[ccci/ctl] (%d)CCIF_MD%d wakeup source: %s\n", md_id + 1, md_id + 1, log_buf);
	...
	if (kfifo_is_full(&ch_info->m_kfifo)) {
		if (ch_info->m_attrs & L_CH_DROP_TOLERATED) {
			...
			CCCI_DBG_MSG(md_id, "cci",
				     ...
				     ch_info->m_ch_name, kfifo_size(&ch_info->m_kfifo),
				     msg->data[0], msg->data[1], msg->channel, msg->reserved);
...
int ccci_logic_ctlb_init(int md_id)
{
	...
	if ((sizeof(logic_ch_static_info_tab) / sizeof(logic_channel_static_info_t)) !=
	    CCCI_MAX_CH_NUM) {
	...
	ctl_b =
	    (logic_dispatch_ctl_block_t *) kzalloc(sizeof(logic_dispatch_ctl_block_t), GFP_KERNEL);
	if (ctl_b == NULL) {
			...
			if (logic_ch_static_info_tab[i].m_kfifo_size) {
				if (0 !=
				    kfifo_alloc(&ch_info->m_kfifo,
						sizeof(ccif_msg_t) *
						logic_ch_static_info_tab[i].m_kfifo_size,
						GFP_KERNEL)) {
	...
	/* ctl_b->m_privilege = MD_BOOT_STAGE_0; */
	ctl_b->m_md_id = md_id;
	snprintf(ctl_b->m_wakelock_name, sizeof(ctl_b->m_wakelock_name), "ccci%d_logic",
		 (md_id + 1));
