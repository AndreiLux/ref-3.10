...
#define  PFP_LOG(instance_inx, flag, args...)         \
    do                                                    \
    {                                                     \
	pr_info(flag "CCMNI%d: ", instance_inx);           \
	pr_info(args);                                     \
    } while (0)
...
frame_info_t pfp_frame(unsigned char *raw_data, unsigned char *cooked_data, int cooked_size,
		       int frame_flag, int ccmni_inx)
{
	frame_info_t entry;
			  ...
			  unsigned char *raw_data, int raw_size, int ccmni_inx)
{
	...
	do {
		...
		pr_info("CCMNI%d: pfp_unframe_state=%d\n", ccmni_inx,
...
#ifdef __PFP_KERNEL_DEBUG__
				pr_info("CCMNI%d: check start flag-local_raw_data[0]=0x%02x\n",
...
#ifdef __PFP_KERNEL_DEBUG__
				pr_info("CCMNI%d: check magic num-local_raw_data[0]=0x%02x\n",
					ccmni_inx, local_raw_data[0]);
#endif
...
#ifdef __PFP_KERNEL_DEBUG__
				pr_info("CCMNI%d: local_raw_size=%d\n", ccmni_inx, local_raw_size);
#endif
				if (local_raw_size >= 2) {
					ccmni_dev[ccmni_inx].pkt_size =
					    ((local_raw_data[1] << 8) | local_raw_data[0]);

#ifdef __PFP_KERNEL_DEBUG__
					pr_info
					    ("CCMNI%d: pkt_size=%d,len[0]=0x%02x,len[1]=0x%02x\n",
					     ccmni_inx, ccmni_dev[ccmni_inx].pkt_size,
					     local_raw_data[0], local_raw_data[1]);
#endif
					 ...
					 */
#ifdef __PFP_KERNEL_DEBUG__
					pr_info("CCMNI%d: not enough len bytes\n", ccmni_inx);
#endif
					keep_parsing = 0;
				}
		...
		case PARSE_PFP_FRAME_GET_DATA_STATE:
			{
				if (local_raw_size < ccmni_dev[ccmni_inx].pkt_size) {
					 ...
					 */
#ifdef __PFP_KERNEL_DEBUG__
					pr_info("CCMNI%d: not enough pkt bytes\n", ccmni_inx);
#endif
					keep_parsing = 0;
				} else {
					/*
					...
					if (local_cooked_data_buf_free_size >=
						...
						complete_ippkt_t *previous_node =
						    ccmni_dev[ccmni_inx].last_pkt_node;
						...
						if (current_node == NULL) {
							...
							current_node =
							    (complete_ippkt_t *)
							    vmalloc(sizeof(complete_ippkt_t));
#else
...
#ifdef __PFP_KERNEL_DEBUG__
						pr_info("CCMNI%d: prepare pkt node\n", ccmni_inx);
#endif
						current_node->pkt_size =
						    ccmni_dev[ccmni_inx].pkt_size;

						/* Copy data from the input buffer to output buffer */
						memcpy(local_cooked_data, local_raw_data,
						       ccmni_dev[ccmni_inx].pkt_size);


						/* pr_info("CCMNI%d: RX_IPID:0x%02x%02x,pkt_size=%d\n",ccmni_inx,local_cooked_data[4],local_cooked_data[5],current_node->pkt_size); */
...
#ifdef __PFP_KERNEL_DEBUG__
		pr_info("Packet Node: data=0x%08x, size=%d\n", t_pkt_node->pkt_data,
			t_pkt_node->pkt_size);
#endif
