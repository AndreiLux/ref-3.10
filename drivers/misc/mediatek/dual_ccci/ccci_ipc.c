...
void find_task_to_clear(IPC_TASK task_table[], unsigned int to_id)
{
	...
	int i, tmp;
	ipc_ctl_block_t *ctl_b =
	    (ipc_ctl_block_t *) (container_of(task_table, ipc_ctl_block_t, ipc_task[0]));
		...
		if (task_table[i].to_id == to_id) {
			...
			if (time_after(task->jiffies, task_table[i].jiffies)) {
				...
				CCCI_DBG_MSG(ctl_b->m_md_id, "ipc",
					     ...
					     task->jiffies, task_table[i].jiffies, tmp,
					     task->to_id);
			}
		}
	}
...
void *read_from_ring_buffer(int md_id, ipc_ilm_t *ilm, BUFF *buff_rd, int *len)
{
	...
	*len = real_size + sizeof(ipc_ilm_t);
	...
	if (real_size > data_size)
		CCCI_MSG_INF(md_id, "ipc", "[Error]wrong real_size(%d)>data_size(%d)", real_size,
			     data_size);
...
static void recv_item(int md_id, unsigned int addr, unsigned int len, IPC_TASK *task,
		      BUFF *buff_rd)
{
	ipc_ctl_block_t *ctl_b = ipc_ctl_block[md_id];
	ipc_ilm_t *ilm =
	    (ipc_ilm_t *) ((uint32) ctl_b->ipc_mem +
			   (addr - ctl_b->ccci_ipc_smem_base_phy + md_2_ap_phy_addr_offset_fixed));
	CCCI_RECV_ITEM *item;
	unsigned long flags;
	...
	if (addr >
	    ctl_b->ccci_ipc_smem_base_phy - md_2_ap_phy_addr_offset_fixed + offset_of(CCCI_IPC_MEM,
										      ilm_md) +
	...
	if (ilm->local_para_ptr) {
		if ((uint32) ilm->local_para_ptr < (uint32) ctl_b->ccci_ipc_rd_buffer_phy ||
		    ...
		    (uint32) ctl_b->ccci_ipc_rd_buffer_phy + CCCI_IPC_BUFFER_SIZE)
			CCCI_MSG_INF(md_id, "ipc", "[Error]wrong ilm->local_para_ptr address(%p)",
				     ilm->local_para_ptr);
	}
	...
	if (ilm->peer_buff_ptr) {
		if ((uint32) ilm->peer_buff_ptr < (uint32) ctl_b->ccci_ipc_rd_buffer_phy ||
		    ...
		    (uint32) ctl_b->ccci_ipc_rd_buffer_phy + CCCI_IPC_BUFFER_SIZE)
			CCCI_MSG_INF(md_id, "ipc", "[Error]wrong ilm->peer_buff_ptr address(%p)",
				     ilm->peer_buff_ptr);
	}
...
static int write_to_ring_buffer(int md_id, uint8 *data, int count, IPC_TASK *task,
				BUFF *ipc_buffer)
{
	int ret = 0;
	int free;
	...
	ret = count;
	ilm->local_para_ptr =
	    local_para ? (local_para_struct *) (ctl_b->ccci_ipc_wr_buffer_phy + write_begin) : NULL;
	ilm->peer_buff_ptr =
	...
	if (list_empty(&task->recv_list)) {
		...
		if (signal_pending(current)) {
			CCCI_IPC_MSG(ctl_b->m_md_id,
				     ...
				     *(unsigned long long *)current->signal->shared_pending.signal.
				     sig);
			ret = -EINTR;
			goto out;
		}
			      ...
			      loff_t *ppos)
{
	...
	*task->ilm_p = *ilm;
	...
	ret =
	    ccci_ipc_write_stream(md_id, CCCI_IPC_TX, task->ilm_phy_addr, sizeof(ipc_ilm_t),
				  id_map->extq_id);
...
int __init ccci_ipc_init(int md_id)
{
	...
	ccci_ipc_base_req(md_id, (int *)(&ctl_b->ipc_mem), &ctl_b->ccci_ipc_smem_base_phy,
