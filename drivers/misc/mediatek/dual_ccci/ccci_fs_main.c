...
static int ccci_fs_send(int md_id, unsigned long arg)
{
	...
	msg.data0 =
	    ctl_b->fs_buffers_phys_addr - md_2_ap_phy_addr_offset_fixed +
	    (sizeof(fs_stream_buffer_t) * message.index);
...
int __init ccci_fs_init(int md_id)
{
	...
	snprintf(ctl_b->fs_wakelock_name, sizeof(ctl_b->fs_wakelock_name), "ccci%d_fs",
		 (md_id + 1));
