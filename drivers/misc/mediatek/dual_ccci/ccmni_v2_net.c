...
static unsigned char *ccmni_v2_phys_to_virt(int md_id, unsigned char *addr_phy)
{
	...
	p_to_v_offset =
	    (unsigned char *)(ccmni_rx_base_virt) - (unsigned char *)(ccmni_rx_base_phy);
...
static void ccmni_v2_reset_buffers(ccmni_v2_instance_t *ccmni)
{
	...
	memset(ccmni->shared_mem->q_tx_ringbuff, 0,
	       sizeof(q_ringbuf_ccmni_t) * CCMNI_CTRL_Q_TX_SIZE);

	memset(ccmni->shared_mem->q_rx_ringbuff, 0,
	       ccmni->shared_mem->rx_control.q_length * sizeof(q_ringbuf_ccmni_t));
	...
	for (count = 0; count < CCMNI_CTRL_Q_RX_SIZE_DEFAULT; count++) {
		ccmni->shared_mem->q_rx_ringbuff[count].ptr =
		    (CCMNI_CTRL_Q_RX_SIZE_DEFAULT * ccmni->channel +
		     count) * CCMNI_SINGLE_BUFF_SIZE + (unsigned char *)ccmni_rx_base_phy +
		    CCMNI_BUFF_HEADER_SIZE + CCMNI_BUFF_DBG_INFO_SIZE -
		    md_2_ap_phy_addr_offset_fixed;

		ptr_virt =
		    ccmni_v2_phys_to_virt(md_id,
					  (unsigned char *)(ccmni->shared_mem->q_rx_ringbuff[count].
							    ptr));
		...
		*((int *)(ptr_virt + CCMNI_BUFF_DATA_FIELD_SIZE)) = CCMNI_BUFF_FOOTER;
		...
		/* debug info */
		dbg_info =
		    (dbg_info_ccmni_t *) (ptr_virt - CCMNI_BUFF_HEADER_SIZE -
...
static int ccmni_v2_check_info(int md_id, int ch, const unsigned char *ccmni_ptr, int ccmni_len)
{
	...
	/* Check End Byte */
	if (*(unsigned char *)((unsigned int)(ccmni_ptr + ccmni_len + 3) & 0xfffffffc) !=
	    CCMNI_DATA_END) {
...
static int ccmni_v2_receive(ccmni_v2_instance_t *ccmni, const unsigned char *ccmni_ptr,
			    int ccmni_len)
{
...
static void ccmni_v2_read(unsigned long arg)
{
	...
	packet_cnt =
	    avai_out >= read_out ? (avai_out - read_out) : (avai_out - read_out + q_length);

	packet_cnt_save = packet_cnt;
	rx_buf_res_left_cnt =
	    avai_in >= avai_out ? (avai_in - avai_out) : (avai_in - avai_out + q_length);
	...
	for (; packet_cnt > 0; packet_cnt--) {
		...
		ccmni_ptr =
		    ccmni_v2_phys_to_virt(md_id,
					  (unsigned char *)(ccmni->shared_mem->q_rx_ringbuff[q_idx].
							    ptr));
		...
		if (-CCCI_ERR_MEM_CHECK_FAIL ==
		    ccmni_v2_check_info(md_id, ccmni->channel, ccmni_ptr, ccmni_len)) {
			...
			ccmni_ptr =
			    ccmni_v2_phys_to_virt(md_id,
						  (unsigned char *)(ccmni->shared_mem->
								    q_rx_ringbuff[avai_in].ptr));
#if CCMNI_DBG_INFO
			dbg_info =
			    (dbg_info_ccmni_t *) (ccmni_ptr - CCMNI_BUFF_HEADER_SIZE -
						  CCMNI_BUFF_DBG_INFO_SIZE);
...
static int ccmni_v2_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	ccmni_ptr =
	    ccmni_v2_phys_to_virt(md_id,
				  (unsigned char *)(ccmni->shared_mem->q_tx_ringbuff[q_idx].ptr));
	...
	/* DBG info */
	dbg_info =
	    (dbg_info_ccmni_t *) (ccmni_ptr - CCMNI_BUFF_HEADER_SIZE - CCMNI_BUFF_DBG_INFO_SIZE);
	dbg_info->avai_out_no = q_idx;
#endif
...
static void ccmni_v2_setup(struct net_device *dev)
{
	...
	CCCI_CCMNI_MSG(ccmni->m_md_id, "CCMNI%d_setup: features=0x%08x,flags=0x%08x\n",
		       ccmni->channel, dev->features, dev->flags);
