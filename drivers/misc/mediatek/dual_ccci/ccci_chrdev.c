		...
		if (ret == sizeof(ccci_msg_t)) {
			if (copy_to_user(u_buff + i, &buff, sizeof(ccci_msg_t))) {
				CCCI_MSG_INF(md_id, "chr",
					     ...
					     client->pid, current->comm, buff.data0, buff.data1,
...
{
	if (chr_ctlb[md_id]) {
		unregister_chrdev_region(MKDEV(chr_ctlb[md_id]->major, chr_ctlb[md_id]->minor),
					 CCCI_MAX_CH_NUM);
		...
		if (idx == 0) {
			...
			if (0 !=
			    kfifo_alloc(&client->private_fifo,
					sizeof(ccci_msg_t) * CCCI_VIR_CHR_KFIFO_SIZE, GFP_KERNEL)) {
...
static ssize_t ccci_vir_chr_write(struct file *file, const char __user *buf, size_t count,
				  loff_t *ppos)
{
	...
	if (ret == sizeof(buff)) {
		if (copy_to_user(u_buff, &buff, sizeof(buff))) {
			CCCI_MSG_INF(md_id, "chr",
				     ...
				     __func__, buff.data0, buff.data1, buff.channel,
				     buff.reserved);
			ret = -EFAULT;
		}
	} else {
...
void ccci_vir_chrdev_exit(int md_id)
{
	if (vir_chr_ctlb[md_id]) {
		unregister_chrdev_region(MKDEV
					 (vir_chr_ctlb[md_id]->major, vir_chr_ctlb[md_id]->minor),
					 CCCI_MAX_CH_NUM);
