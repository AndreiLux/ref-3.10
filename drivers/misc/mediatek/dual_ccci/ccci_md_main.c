	...
	if (md_ctlb[md_id] == NULL) {
		...
		if (addr_phy != NULL) {
			*(unsigned int *)addr_phy =
			    md_ctlb[md_id]->smem_table->ccci_mdlog_smem_base_phy;
		}
		if (addr_vir != NULL) {
			*(unsigned int *)addr_vir =
			    md_ctlb[md_id]->smem_table->ccci_mdlog_smem_base_virt;
		}
		if (len != NULL) {
...
int ccci_rpc_base_req(int md_id, int *addr_vir, int *addr_phy, int *len)
{
	if (md_ctlb[md_id] == NULL) {
		...
		if (addr_vir != NULL) {
			*(unsigned int *)addr_vir =
			    md_ctlb[md_id]->smem_table->ccci_rpc_smem_base_virt;
		}
		if (addr_phy != NULL) {
			*(unsigned int *)addr_phy =
...
int ccci_pmic_base_req(int md_id, void *addr_vir, void *addr_phy, int *len)
{
	if (md_ctlb[md_id] == NULL) {
		...
		if (addr_vir != NULL) {
			*(unsigned int *)addr_vir =
			    md_ctlb[md_id]->smem_table->ccci_pmic_smem_base_virt;
		}
		if (addr_phy != NULL) {
			*(unsigned int *)addr_phy =
...
int ccmni_v2_ul_base_req(int md_id, void *addr_vir, void *addr_phy)
{
	if (md_ctlb[md_id] == NULL) {
		...
		if (addr_vir != NULL) {
			*(unsigned int *)addr_vir =
			    md_ctlb[md_id]->smem_table->ccci_ccmni_smem_ul_base_virt;
		}
		if (addr_phy != NULL) {
			*(unsigned int *)addr_phy =
			    md_ctlb[md_id]->smem_table->ccci_ccmni_smem_ul_base_phy;
		}
		return 0;
	}
}
...
int ccmni_v2_dl_base_req(int md_id, void *addr_vir, void *addr_phy)
{
	if (md_ctlb[md_id] == NULL) {
		...
		if (addr_vir != NULL) {
			*(unsigned int *)addr_vir =
			    md_ctlb[md_id]->smem_table->ccci_ccmni_smem_dl_base_virt;
		}
		if (addr_phy != NULL) {
			*(unsigned int *)addr_phy =
			    md_ctlb[md_id]->smem_table->ccci_ccmni_smem_dl_base_phy;
		}
		return 0;
	}
}
...
size_t ccci_msg_filter_show(char buf[], size_t len)
{
	...
	for (i = 0; i < md_num; i++) {
		if (get_modem_is_enabled(i)) {
			curr +=
			    snprintf(&buf[curr], len - curr, "[MD%d] msg mask: %x\n", i + 1,
				     ccci_msg_mask[i]);
		}
	}
		...
		if (get_modem_is_enabled(i)) {
			curr +=
			    snprintf(&buf[curr], len - curr,
				     ...
				     fs_tx_debug_enable[i], fs_rx_debug_enable[i],
				     tty_debug_enable[i]);
		}
	}
...
static void ccci_dump_runtime_data(int md_id, modem_runtime_t *runtime, smem_alloc_t *smem)
{
	...
	*p = runtime->Postfix;
	...
	CCCI_MSG_INF(md_id, "ctl", "ccci_smem_phy,              AP=0x%x,MD=0x%x\n",
		     ...
		     (unsigned int)smem->ccci_smem_phy - md_2_ap_phy_addr_offset_fixed);
	...
	CCCI_DBG_MSG(md_id, "ctl", "md_runtime_data_smem_phy    AP=0x%x,MD=0x%x\n",
		     ...
		     smem->ccci_md_runtime_data_smem_base_phy - md_2_ap_phy_addr_offset_fixed);
...
static void ex_monitor_func(unsigned long data)
{
	...
	} else if ((ee_info_flag & ((1 << MD_EE_MSG_GET) | (1 << MD_EE_OK_MSG_GET))) ==
		   (1 << MD_EE_MSG_GET)) {
...
void md_ee_info_check(int md_id, unsigned int *p_ee_info)
{
	...
	*p_ee_info = 0;
	exp_ee_info =
	    (modem_exception_exp_t *) ctl_b->smem_table->ccci_md_ex_exp_info_smem_base_virt;
...
void ccci_check_md_no_physical_channel(int md_id, unsigned int args)
{
	...
	if (ee_info) {
		if (ctl_b->ee_info_got == 0) {
			ctl_b->ee_info_got = 1;
			exp_ee_info =
			    (modem_exception_exp_t *) ctl_b->smem_table->
			    ccci_md_ex_exp_info_smem_base_virt;
			CCCI_MSG_INF(md_id, "ctl", "receive MD_EX @S(%08X)(%d:%d)\n",
				     exp_ee_info->exception_occur, exp_ee_info->send_time,
	...
	if (trigger_ee) {
		...
		ctl_b->ee_info_flag |=
		    ((1 << MD_EE_FLOW_START) | (1 << MD_EE_FOUND_BY_TX) | ee_info);
		...
		if (ctl_b->ee_info_got == 0) {
			...
			case 1:	/* pending 50ms, dump EE buffer only */
				...
				CCCI_DBG_MSG(md_id, "cci", "Dump MD%d Exception share memory\n",
					     md_id + 1);
				ccci_mem_dump((int *)md_ctlb[md_id]->smem_table->
					      ccci_exp_smem_base_virt,
					      md_ctlb[md_id]->smem_table->ccci_exp_smem_size);
				break;
			...
			case 2:	/* pending 1500ms, trigger EE */
				...
				spin_lock_irqsave(&ctl_b->ctl_lock, flags);
				if ((ctl_b->
				     ee_info_flag & ((1 << MD_EE_PENDING_TOO_LONG) |
						     (1 << MD_STATE_UPDATE))) == 0) {
					trigger_ee_timer = 1;
				}
				ctl_b->ee_info_flag |=
				    ((1 << MD_EE_FLOW_START) | (1 << MD_EE_PENDING_TOO_LONG));
				spin_unlock_irqrestore(&ctl_b->ctl_lock, flags);
...
void ccif_isr_check(int md_id)
{
	...
	if (trigger_ee) {
		spin_lock_irqsave(&ctl_b->ctl_lock, flags);
		ctl_b->ee_info_flag |=
		    ((1 << MD_EE_FLOW_START) | (1 << MD_EE_FOUND_BY_ISR) | ee_info);
		if ((ctl_b->ee_info_flag & (1 << MD_STATE_UPDATE)) == 0) {
			trigger_ee_timer = 1;
		}
		ctl_b->md_boot_stage = MD_BOOT_STAGE_EXCEPTION;
...
static int set_md_runtime(int md_id, modem_runtime_t *runtime, modem_runtime_info_tag_t *tag)
{
	int i;
	...
	if (ctl_b->smem_table->ccci_sys_smem_size) {
		runtime->SysShareMemBase =
		    ctl_b->smem_table->ccci_sys_smem_base_phy - md_2_ap_phy_addr_offset_fixed;
	...
	if (ctl_b->smem_table->ccci_exp_smem_size) {
		runtime->ExceShareMemBase =
		    ctl_b->smem_table->ccci_exp_smem_base_phy - md_2_ap_phy_addr_offset_fixed;
	...
	if (ctl_b->smem_table->ccci_mdlog_smem_size) {
		runtime->MdlogShareMemBase =
		    ctl_b->smem_table->ccci_mdlog_smem_base_phy - md_2_ap_phy_addr_offset_fixed;
	...
	if (ctl_b->smem_table->ccci_pcm_smem_size) {
		runtime->PcmShareMemBase =
		    ctl_b->smem_table->ccci_pcm_smem_base_phy - md_2_ap_phy_addr_offset_fixed;
	...
	if (ctl_b->smem_table->ccci_pmic_smem_size) {
		runtime->PmicShareMemBase =
		    ctl_b->smem_table->ccci_pmic_smem_base_phy - md_2_ap_phy_addr_offset_fixed;
	...
	if (ctl_b->smem_table->ccci_fs_smem_size) {
		runtime->FileShareMemBase =
		    ctl_b->smem_table->ccci_fs_smem_base_phy - md_2_ap_phy_addr_offset_fixed;
	...
	if (ctl_b->smem_table->ccci_rpc_smem_size) {
		runtime->RpcShareMemBase =
		    ctl_b->smem_table->ccci_rpc_smem_base_phy - md_2_ap_phy_addr_offset_fixed;
	...
	for (i = 0; i < CCCI_UART_PORT_NUM; i++) {
		if ((ctl_b->smem_table->ccci_uart_smem_base_phy[i] != 0) &&
			...
			if (ctl_b->smem_table->ccci_uart_smem_size[i]) {
				runtime->UartShareMemBase[i] =
				    ctl_b->smem_table->ccci_uart_smem_base_phy[i] -
				    md_2_ap_phy_addr_offset_fixed;
				runtime->UartShareMemSize[i] =
	...
	for (i = 0; i < NET_PORT_NUM; i++) {
		...
		if (ccci_get_sub_module_cfg
		    (md_id, "net_dl_ctl", (char *)&dl_ctl_mem_size, sizeof(int)) != sizeof(int)) {
			CCCI_MSG_INF(md_id, "ctl", "Get net_dl_ctl fail\n");
			dl_ctl_mem_size = 0;
		}
		if (ccci_get_sub_module_cfg
		    (md_id, "net_ul_ctl", (char *)&ul_ctl_mem_size, sizeof(int)) != sizeof(int)) {
		...
		if (ul_ctl_mem_size) {
			runtime->NetULCtrlShareMemBase[i] =
			    ctl_b->smem_table->ccci_ccmni_ctl_smem_base_phy[i] + dl_ctl_mem_size -
			    md_2_ap_phy_addr_offset_fixed;
	...
	if (ctl_b->smem_table->ccci_ccmni_smem_dl_size) {
		runtime->MDDLNetShareMemBase =
		    ctl_b->smem_table->ccci_ccmni_smem_dl_base_phy - md_2_ap_phy_addr_offset_fixed;
	...
	if (ctl_b->smem_table->ccci_ccmni_smem_ul_size) {
		runtime->MDULNetShareMemBase =
		    ctl_b->smem_table->ccci_ccmni_smem_ul_base_phy - md_2_ap_phy_addr_offset_fixed;
	...
	if (ctl_b->smem_table->ccci_md_ex_exp_info_smem_size) {
		runtime->MiscInfoBase =
		    ctl_b->smem_table->ccci_misc_info_base_phy - md_2_ap_phy_addr_offset_fixed;
	...
	config_misc_info(md_id, (unsigned int *)ctl_b->smem_table->ccci_misc_info_base_virt,
	...
	tag->runtime_data_base =
	    ctl_b->smem_table->ccci_md_runtime_data_smem_base_phy - md_2_ap_phy_addr_offset_fixed;
...
int ccci_send_run_time_data(int md_id)
{
	...
	ret =
	    ccci_write_runtime_data(md_id, (unsigned char *)&runtime_tag,
				    sizeof(modem_runtime_info_tag_t));
	if (ret < 0) {
...
void ccci_md_ctrl_cb(void *private)
{
	...
	while (get_logic_ch_data(ch_info, &msg)) {
		if (msg.id == MD_INIT_START_BOOT &&
		    msg.reserved == MD_INIT_CHK_ID && ctl_b->md_boot_stage == MD_BOOT_STAGE_0) {
		...
		} else if (msg.id == NORMAL_BOOT_ID && ctl_b->md_boot_stage == MD_BOOT_STAGE_1) {
			del_timer(&ctl_b->md_boot_up_check_timer);
			...
			if (unlikely(msg.reserved != MD_EX_CHK_ID))
				...
				spin_lock_irqsave(&ctl_b->ctl_lock, flags);
				ctl_b->ee_info_flag |=
				    ((1 << MD_EE_FLOW_START) | (1 << MD_EE_MSG_GET) |
				     (1 << MD_STATE_UPDATE) | (1 << MD_EE_TIME_OUT_SET));
			...
			if (unlikely(msg.reserved != MD_EX_REC_OK_CHK_ID))
				...
				spin_lock_irqsave(&ctl_b->ctl_lock, flags);
				ctl_b->ee_info_flag |=
				    ((1 << MD_EE_FLOW_START) | (1 << MD_EE_OK_MSG_GET));
				if ((ctl_b->ee_info_flag & (1 << MD_STATE_UPDATE)) == 0) {
			...
			CCCI_MSG_INF(md_id, "ctl",
				     "receive unknow data from CCCI_CONTROL_RX = %d\n", msg.id);
		}
	}
}
...
int ccci_ipo_h_restore(int md_id, char buf[], unsigned int len)
{
	...
	if (ccci_ipo_h_platform_restore(md_id) < 0) {
		pr_info("[ccci/ctl] IPO-H p1 fail\n");
