...
int pwr_loss_open(struct inode *inode, struct file *file)
{
	...
	pr_info("Power Loss Test: Open operation !\n");
#endif
	return 0;
}
...
{
#ifdef PWR_LOSS_DEBUG
	pr_info("Power Loss Test: Release operation !\n");
#endif
	return 0;
}
...
int pwr_loss_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
...
#ifdef PWR_LOSS_DEBUG
	pr_info("Power Loss Test: IOCTL operation ! CMD=%d\n", cmd);
#endif
	...
	case PRINT_REBOOT_TIMES:
		...
		if (ret != 0) {
			pr_info
			    ("Power Loss Test: IOCTL->PRINT_REBOOT_TIMES %d Bytes can't be copied\n",
			     ret);
		}
		pr_info
		    ("Power Loss Test: -----------System Reboot Successfully Times= %d---------------!\n",
		     ((int *)l_buf)[0]);
		break;
	case PRINT_DATA_COMPARE_ERR:
		pr_info("Power Loss Test: -----------Data Compare Error---------------!\n");
		break;
	case PRINT_FILE_OPERATION_ERR:
		pr_info("Power Loss Test: -----------File Operation Error---------------!\n");
		break;
	...
	case PRINT_RAW_RW_INFO:
		ret = copy_from_user(l_buf, (int *)arg, (sizeof(l_buf[0]) * (sizeof(l_buf))));
		if (ret != 0) {
			pr_info("Power Loss Test: %d Bytes can't be copied\n", ret);
		}
		...
		break;
	default:
		pr_info("Power Loss Test: cmd code Error!\n");
		break;
	}
...
#ifdef PWR_LOSS_DEBUG
	pr_info("Power Loss Test: sleep time =%d\n", sleep_time);
#endif

	while (1) {
		pr_info("Power Loss Test: wait for reset...!\n");
		...
		if (power_loss_info.wdt_reboot_support == WDT_REBOOT_OFF) {
			...
			msleep(1000);
			pr_info("Power Loss Test: wdt reboot pause...!\n");
			continue;
		}
		up_read(&power_loss_info.rwsem);
		pr_info("Power Loss Test: ret = %d, do reset now...\n", ret);
		...
		if (!mt6573_nandchip_Reset()) {
			pr_info("NAND_MVG mt6573_nandchip_Reset Failed!\n");
		}
#endif
...
#ifdef PWR_LOSS_DEBUG
	pr_info("Power Loss Test: sleep time = 100sec\n");
#endif
...
static int power_loss_debug_write(struct file *file, const char __user *buffer,
				  unsigned long count, void *data)
{
	...
	if (1 == sscanf(cmd_buf, "%x", &wdt_reboot_support)) {
		if (wdt_reboot_support < 0) {
			pr_info("[%s] : command format is error, please help to check!!!\n",
				__func__);
			return -1;
		} else {
			down_write(&power_loss_info.rwsem);
...
static int power_loss_debug_read(char *page, char **start, off_t offset, int count, int *eof,
				 void *data)
{
	int len = 0;
...
static int power_loss_debug_init(void)
{
	...
	if (power_loss_debug) {
		...
		pr_info("[%s]: Successfully create /proc/power_loss_debug\n", __func__);
	} else {
		pr_info("[%s]: Failed to create /proc/power_loss_debug\n", __func__);
		return -1;
	}
...
static int __init power_loss_init(void)
{
	...
	if (NULL == sg_pwr_loss_dev_file) {
		pr_info("Power Loss Test: device_create Failed!\n");
		goto out;
	}
#ifdef PWR_LOSS_SW_RESET
	...
	pr_info("Power Loss Test: Init Successfully!\n");

#ifdef PWR_LOSS_SW_RESET
	kernel_thread(pwr_loss_reset_thread, NULL, CLONE_VM);	/* CLONE_KERNEL */
	pr_info("Power Loss Test: kernel thread create Successful!\n");
#endif
...
{
#ifdef PWR_LOSS_DEBUG
	pr_info("Power Loss Test: Module Exit\n");
#endif
	...
	pr_info("Power Loss Test:module exit Successfully!\n");
}
