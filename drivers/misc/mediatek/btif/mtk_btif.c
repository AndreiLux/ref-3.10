...
static int _btif_irq_reg(P_MTK_BTIF_IRQ_STR p_irq, mtk_btif_irq_handler irq_handler, void *data);
...
static int _btif_pio_write(p_mtk_btif p_btif, const unsigned char *p_buf, unsigned int buf_len);
static int _btif_dma_write(p_mtk_btif p_btif, const unsigned char *p_buf, unsigned int buf_len);

static unsigned int btif_bbs_wr_direct(p_btif_buf_str p_bbs,
				       unsigned char *p_buf, unsigned int buf_len);
static unsigned int btif_bbs_read(p_btif_buf_str p_bbs, unsigned char *p_buf, unsigned int buf_len);
...
static int _btif_send_data(p_mtk_btif p_btif, const unsigned char *p_buf, unsigned int buf_len);
...
static long btif_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
...
int _btif_controller_tx_setup(p_mtk_btif p_btif)
{
	...
	if (BTIF_MODE_DMA == p_btif->tx_mode) {
		...
		if (i_ret) {
			BTIF_ERR_FUNC("_btif_tx_dma_setup failed, \
...
*/
	...
	if (retry < max_retry) {
		if (p_btif->tx_mode == BTIF_MODE_DMA) {
			...
			i_ret +=
			    hal_btif_dma_clk_ctrl(p_btif->p_tx_dma->p_dma_info, CLK_OUT_DISABLE);
		}
		if (p_btif->rx_mode == BTIF_MODE_DMA) {
			/*disable BTIF Rx DMA's clock */
			i_ret +=
			    hal_btif_dma_clk_ctrl(p_btif->p_rx_dma->p_dma_info, CLK_OUT_DISABLE);
		}
...
/*vFIFO memory allocation*/
	p_vfifo->p_vir_addr = dma_zalloc_coherent(NULL,
						  p_vfifo->vfifo_size, &p_vfifo->phy_addr, GFP_DMA);
	if (NULL == p_vfifo->p_vir_addr) {
...
/*free DMA memory if allocated successfully before*/
	if (NULL != p_vfifo->p_vir_addr) {
		dma_free_coherent(NULL,
				  p_vfifo->vfifo_size, p_vfifo->p_vir_addr, p_vfifo->phy_addr);
...
static int btif_rx_thread(void *p_data)
{
	...
	while (1) {
		if (p_btif->enable) {
			...
			wait_for_completion_interruptible_timeout(&p_btif->rx_comp,
								  msecs_to_jiffies(360));
		} else {
	...
	do {
		while (leng_sent < how_much_get) {
			i_ret = _btif_send_data(p_btif,
						local_buf + leng_sent, how_much_get - leng_sent);
			if (0 < i_ret) {
				leng_sent += i_ret;
			} else if (0 == i_ret) {
...
unsigned int btif_bbs_read(p_btif_buf_str p_bbs, unsigned char *p_buf, unsigned int buf_len)
{
	...
	if (0 == ava_len) {
		...
		if (wr_idx >= (p_bbs)->rd_idx) {
			memcpy(p_buf, BBS_PTR(p_bbs, p_bbs->rd_idx), ava_len);
			(p_bbs)->rd_idx = wr_idx;
		} else {
			unsigned int tail_len = BBS_SIZE(p_bbs) - (p_bbs)->rd_idx;
			memcpy(p_buf, BBS_PTR(p_bbs, p_bbs->rd_idx), tail_len);
			memcpy(p_buf + tail_len, BBS_PTR(p_bbs, 0), ava_len - tail_len);
			(p_bbs)->rd_idx = wr_idx;
		}
	} else {
		rd_len = buf_len;
		if (wr_idx >= (p_bbs)->rd_idx) {
			memcpy(p_buf, BBS_PTR(p_bbs, p_bbs->rd_idx), rd_len);
	...
	while ((sent_len < buf_len)) {
		if (hal_btif_is_tx_allow(p_btif_info)) {
			i_ret = hal_btif_send_data(p_btif_info,
						   p_buf + sent_len, buf_len - sent_len);
			if (0 < i_ret) {
				sent_len += i_ret;
...
/*tx fifo in*/
		...
		if (length == buf_len) {
			...
			i_ret = 0;
			BTIF_ERR_FUNC("fifo in failed, target len(%d), \
...
/*check if log dynamic output function is enabled or not*/
	if (output_flag) {
		pr_info("BTIF-DBG, dir:%s, %d.%ds len:%d\n",
...
int btif_log_buf_dmp_out(P_BTIF_LOG_QUEUE_T p_log_que)
{
	...
	out_index = p_log_que->size >=
	    BTIF_LOG_ENTRY_NUM ? in_index : (BTIF_LOG_ENTRY_NUM -
					     p_log_que->size + in_index) % BTIF_LOG_ENTRY_NUM;
	...
	if (0 != dump_size) {
		while (dump_size--) {
			...
			pr_info("dir:%s, pkt_count:%d, %d.%ds len:%d\n",
				p_dir,
				pkt_count++, (int)p_timer->tv_sec, (int)p_timer->tv_usec, len);
/*output buffer content*/
