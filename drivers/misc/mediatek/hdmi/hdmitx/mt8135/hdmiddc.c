...
#define HDMIDDC_BASE	(0x09f0)

unsigned int hdmi_ddc_read(unsigned int u2Reg)
{
	unsigned int u4Data;
	unsigned int u2HighData, u2LowData, addr;

	u2LowData = 0;
	u2HighData = 0;
	addr = HDMIDDC_BASE + u2Reg;

	pwrap_read(addr, &u2LowData);

	addr = addr + 2;
	pwrap_read(addr, &u2HighData);

	u4Data = u2LowData | (u2HighData << 16);
	HDMI_DDC_LOG("[R]u2Reg=0x%04x, data = 0x%08x\n", u2Reg, u4Data);
	return u4Data;
}

void hdmi_ddc_write(unsigned int u2Reg, unsigned int u4Data)
{
	unsigned int u2HighData, u2LowData, addr;
	u2LowData = u4Data & 0xFFFF;
	u2HighData = (u4Data >> 16) & 0xFFFF;
	addr = HDMIDDC_BASE + u2Reg;
	pwrap_write(addr, u2LowData);
	addr = addr + 2;
	pwrap_write(addr, u2HighData);

	HDMI_DDC_LOG("[W]u2Reg=0x%04x, data = 0x%08x\n", u2Reg, u4Data);
}
