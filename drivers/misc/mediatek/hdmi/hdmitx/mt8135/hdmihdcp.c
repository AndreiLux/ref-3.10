...
void HdcpService(HDCP_CTRL_STATE_T e_hdcp_state)
{
	...
	case HDCP_WAIT_RES_CHG_OK:
		...
		if (fgIsHDCPCtrlTimeOut()) {
			if ((_bHdcpOff == 1) || (bReadByteHdmiGRL(GRL_CFG1) & CFG1_DVI))	/* disable HDCP */
			{
				vSetHDCPState(HDCP_RECEIVER_NOT_READY);
				vHDMIAVUnMute();
				vSendHdmiCmd(HDMI_HDCP_PROTOCAL_CMD);
	...
	case HDCP_INIT_AUTHENTICATION:
		...
		if (!fgDDCDataRead(RX_ID, RX_REG_BCAPS, 1, &bTemp)) {
			pr_err("mt8135 hdcp error: can't read bcaps from DDC\n");
		...
		if (fgHostKey()) {
			...
			if ((HDMI_AKSV[0] == 0) && (HDMI_AKSV[1] == 0) && (HDMI_AKSV[2] == 0)
			    && (HDMI_AKSV[3] == 0)) {
				pr_err("mt8135 hdcp error: can't read AKSV\n");
				vSetHDCPState(HDCP_RECEIVER_NOT_READY);
				vSendHdmiCmd(HDMI_HDCP_PROTOCAL_CMD);
				break;
			}
	...
	case HDCP_WAIT_R0:
		...
		if (bTemp == TRUE) {
			...
			pr_err("mt8135 hdcp error: R0 is not ready\n");
			vSetHDCPState(HDCP_RE_DO_AUTHENTICATION);
			vSendHdmiCmd(HDMI_HDCP_PROTOCAL_CMD);
			break;
		}
	...
	case HDCP_COMPARE_R0:
		...
		if (fgCompareRi() == TRUE) {
			...
			pr_err("mt8135 hdcp error: compare Ri, fail\n");
	...
	case HDCP_RE_COMPARE_R0:
		...
		if (fgIsHDCPCtrlTimeOut() && _bReCompRiCount > 3) {
			pr_err("mt8135 hdcp error: recompare R0, fail\n");
			vSetHDCPState(HDCP_RE_DO_AUTHENTICATION);
			vSendHdmiCmd(HDMI_HDCP_PROTOCAL_CMD);
			_bReCompRiCount = 0;
		} else {
	...
	case HDCP_WAIT_KSV_LIST:
		...
		if ((bTemp & RX_BIT_ADDR_READY)) {
			...
			pr_err("mt8135 hdcp error: ksv list not ready\n");
			vSetHDCPState(HDCP_RE_DO_AUTHENTICATION);
			vSendHdmiCmd(HDMI_HDCP_PROTOCAL_CMD);
			_bReCheckReadyBit = 0;
			break;
	...
	case HDCP_COMPARE_V:
		...
		if ((bTemp & HDCP_STA_V_MATCH) || (bTemp & HDCP_STA_V_RDY)) {
			if ((bTemp & HDCP_STA_V_MATCH))	/* for Simplay #7-20-5 */
			{
				...
				pr_err("mt8135 hdcp error: compare v, fail\n");
				vSetHDCPState(HDCP_RE_DO_AUTHENTICATION);
				vSendHdmiCmd(HDMI_HDCP_PROTOCAL_CMD);
			}
		}
	...
	case HDCP_CHECK_LINK_INTEGRITY:
		...
		if (fgCompareRi() == TRUE) {
			...
			pr_err("mt8135 hdcp error: check link integrity fail\n");
			bMask = bReadHdmiIntMask();
			vWriteHdmiIntMask(0xff);	/* disable INT HDCP */
			_bReCompRiCount = 0;
