...
static INT32 _stp_btm_handler(MTKSTP_BTM_T *stp_btm, P_STP_BTM_OP pStpOp)
{
	...
	case STP_OPID_BTM_PAGED_DUMP:
		...
		do {
			...
			host_state =
			    (ENUM_HOST_DUMP_STATE) wmt_plat_get_dump_info(p_emi_ctrl_state_info->
									  p_emi_ctrl_state_offset->
									  emi_apmem_ctrl_host_sync_state);
			...
			while (1) {
				chip_state =
				    (ENUM_CHIP_DUMP_STATE)
				    wmt_plat_get_dump_info(p_emi_ctrl_state_info->
							   p_emi_ctrl_state_offset->
							   emi_apmem_ctrl_chip_sync_state);
			...
			dump_phy_addr =
			    wmt_plat_get_dump_info(p_emi_ctrl_state_info->p_emi_ctrl_state_offset->
						   emi_apmem_ctrl_chip_sync_addr);
			...
			dump_vir_addr =
			    wmt_plat_get_emi_virt_add(dump_phy_addr -
						      p_emi_ctrl_state_info->emi_phy_addr);
			if (!dump_vir_addr) {
				STP_BTM_ERR_FUNC("get paged dump phy address fail\n");
				ret = -2;
				break;
			...
			if (0 == page_counter)	/* do fw assert infor paser in first paged dump */
			{
				...
				if (ret) {
					STP_BTM_ERR_FUNC
					    ("set fw issue infor fail(%d),maybe fw warm reset...\n",
					     ret);
					stp_dbg_set_fw_info("Fw Warm reset",
							    osal_strlen("Fw Warm reset"),
 ...
 paged_dump_end:
			...
			if (counter * 100 > STP_PAGED_DUMP_TIME_LIMIT) {
				isEnd =
				    wmt_plat_get_dump_info(p_emi_ctrl_state_info->
							   p_emi_ctrl_state_offset->
							   emi_apmem_ctrl_chip_paded_dump_end);
 ...
 full_dump_end:
	...
	case STP_OPID_BTM_PAGED_TRACE:
		...
		do {
			...
			buffer_start =
			    wmt_plat_get_dump_info(p_emi_ctrl_state_info->p_emi_ctrl_state_offset->
						   emi_apmem_ctrl_chip_print_buff_start);
			buffer_idx =
			    wmt_plat_get_dump_info(p_emi_ctrl_state_info->p_emi_ctrl_state_offset->
						   emi_apmem_ctrl_chip_print_buff_idx);
			...
			dump_vir_addr =
			    wmt_plat_get_emi_virt_add(buffer_start -
						      p_emi_ctrl_state_info->emi_phy_addr);
			if (!dump_vir_addr) {
				STP_BTM_ERR_FUNC("get vir dump address fail\n");
				ret = -2;
			...
			osal_memcpy(&g_paged_trace_buffer[0], dump_vir_addr,
				    buffer_idx <
				    STP_DBG_PAGED_TRACE_SIZE ? buffer_idx :
				    STP_DBG_PAGED_TRACE_SIZE);
			...
			do {
				...
				for (i = 0; i < dump_len; i++) {
					...
					pr_info("%02x ", g_paged_trace_buffer[i]);
				}
				pr_info("\n\n -- paged trace ascii output --\n\n");
				for (i = 0; i < dump_len; i++) {
					if (i % 64 == 0)
						pr_info("\n");
					pr_info("%c", g_paged_trace_buffer[i]);
				}
			} while (0);
