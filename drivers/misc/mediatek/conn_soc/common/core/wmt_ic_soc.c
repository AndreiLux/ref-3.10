...
static UCHAR WMT_PATCH_ADDRESS_CMD[] =
    { 0x01, 0x08, 0x10, 0x00, 0x01, 0x01, 0x00, 0x01, 0x3c, 0x02, 0x09, 0x02, 0x00, 0x00, 0x00,
0x00, 0xff, 0xff, 0xff, 0xff };
static UCHAR WMT_PATCH_ADDRESS_EVT[] = { 0x02, 0x08, 0x04, 0x00, 0x00, 0x00, 0x00, 0x01 };
static UCHAR WMT_PATCH_P_ADDRESS_CMD[] =
    { 0x01, 0x08, 0x10, 0x00, 0x01, 0x01, 0x00, 0x01, 0xc4, 0x04, 0x09, 0x02, 0x00, 0x3f, 0x00,
...
static UCHAR WMT_GET_SOC_ADIE_CHIPID_EVT[] =
...
static struct init_script calibration_table[] = {
	INIT_CMD(WMT_CORE_START_RF_CALIBRATION_CMD, WMT_CORE_START_RF_CALIBRATION_EVT,
		 "start RF calibration data"),
};
...
static INT32 mtk_wcn_soc_sw_init(P_WMT_HIF_CONF pWmtHifConf)
{
	...
	if (0x6592 == dDieChipid) {
		...
		iRet =
		    wmt_core_tx((PUINT8) & WMT_GET_SOC_ADIE_CHIPID_CMD[0],
				sizeof(WMT_GET_SOC_ADIE_CHIPID_CMD), &u4Res, MTK_WCN_BOOL_FALSE);
...
static INT32 mtk_wcn_soc_crystal_triming_set(VOID)
{
	...
	if (0 != iRet) {
		...
		WMT_DBG_FUNC("0x%x: nvram pBuf(0x%08x), bufLen(%d)\n", wmt_ic_ops_soc.icId, pbuf,
			     bufLen);
		if (bufLen < (uCryTimOffset + 1)) {
			WMT_ERR_FUNC
			    ("0x%x: nvram len(%d) too short, crystalTimging value offset(%d)\n",
