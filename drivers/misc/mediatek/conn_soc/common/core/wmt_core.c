	    ...
	    const UINT32 size, UINT32 * writtenSize, const MTK_WCN_BOOL bRawFlag)
{
	...
	if (0 == *writtenSize) {
		...
		WMT_WARN_FUNC
		    ("WMT-CORE: wmt_ctrl_tx_ex failed and written ret:%d, maybe no winspace in STP layer\n",
		     *writtenSize);
			...
			if (osal_memcmp(evtBuf, script[i].evt, script[i].evtSz) != 0) {
				...
				WMT_ERR_FUNC
				    ("WMT-CORE:rx(%d):[%02X,%02X,%02X,%02X,%02X] exp(%d):[%02X,%02X,%02X,%02X,%02X]\n",
				     ...
				     script[i].evtSz, script[i].evt[0], script[i].evt[1],
				     script[i].evt[2], script[i].evt[3], script[i].evt[4]);
				mtk_wcn_stp_dbg_dump_package();
				break;
			}
		}
 ...
 pwr_sv_done:
	...
	if (pWmtOp->au4OpData[0] < STP_PSM_MAX_ACTION) {
		...
		WMT_DBG_FUNC("Do STP-CB! %d %p / %p\n", pWmtOp->au4OpData[0],
			     (PVOID) pWmtOp->au4OpData[1], (PVOID) psm_cb);
		if (NULL != psm_cb) {
...
static INT32 opfunc_hw_rst(P_WMT_OP pWmtOp)
{
	...
	if (gMtkWmtCtx.eDrvStatus[WMTDRV_TYPE_WIFI] == DRV_STS_FUNC_ON) {
		...
		if (NULL != gpWmtFuncOps[WMTDRV_TYPE_WIFI]
			...
			iRet =
			    gpWmtFuncOps[WMTDRV_TYPE_WIFI]->func_off(gMtkWmtCtx.p_ic_ops,
								     wmt_conf_get_cfg());
			if (iRet) {
...
static INT32 opfunc_bgw_ds(P_WMT_OP pWmtOp)
{
	...
	UINT8 WMT_BGW_DESENSE_CMD[] =
	    { 0x01, 0x0e, 0x0f, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0x00, 0x00, 0x00 };
	...
	do {
		...
		iRet =
		    wmt_core_tx(&WMT_BGW_DESENSE_CMD[0], osal_sizeof(WMT_BGW_DESENSE_CMD),
				&u4WrittenSize, MTK_WCN_BOOL_FALSE);
		...
		if (osal_memcmp(evt_buffer, WMT_BGW_DESENSE_EVT, osal_sizeof(WMT_BGW_DESENSE_EVT))
		    != 0) {
			WMT_ERR_FUNC
...
static INT32 opfunc_set_mcu_clk(P_WMT_OP pWmtOp)
{
	...
	UINT8 WMT_SET_MCU_CLK_CMD[] =
	    { 0x01, 0x08, 0x10, 0x00, 0x01, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
0x00, 0x00, 0xff, 0xff, 0xff, 0xff };
	...
	switch (kind) {
	case 0:
		osal_memcpy(&WMT_SET_MCU_CLK_CMD[8], &WMT_EN_MCU_CLK_CMD[0],
			    osal_sizeof(WMT_EN_MCU_CLK_CMD));
		break;
	case 1:
		osal_memcpy(&WMT_SET_MCU_CLK_CMD[8], &WMT_26_MCU_CLK_CMD[0],
			    osal_sizeof(WMT_26_MCU_CLK_CMD));
		break;
	case 2:
		osal_memcpy(&WMT_SET_MCU_CLK_CMD[8], &WMT_37_MCU_CLK_CMD[0],
			    osal_sizeof(WMT_37_MCU_CLK_CMD));
		break;
	case 3:
		osal_memcpy(&WMT_SET_MCU_CLK_CMD[8], &WMT_64_MCU_CLK_CMD[0],
			    osal_sizeof(WMT_64_MCU_CLK_CMD));
		break;
	case 4:
		osal_memcpy(&WMT_SET_MCU_CLK_CMD[8], &WMT_69_MCU_CLK_CMD[0],
			    osal_sizeof(WMT_69_MCU_CLK_CMD));
		break;
	case 5:
		osal_memcpy(&WMT_SET_MCU_CLK_CMD[8], &WMT_104_MCU_CLK_CMD[0],
			    osal_sizeof(WMT_104_MCU_CLK_CMD));
		break;
	case 6:
		osal_memcpy(&WMT_SET_MCU_CLK_CMD[8], &WMT_108_MCU_CLK_CMD[0],
			    osal_sizeof(WMT_108_MCU_CLK_CMD));
		break;
	case 7:
		osal_memcpy(&WMT_SET_MCU_CLK_CMD[8], &WMT_138_MCU_CLK_CMD[0],
			    osal_sizeof(WMT_138_MCU_CLK_CMD));
		break;
	case 8:
		osal_memcpy(&WMT_SET_MCU_CLK_CMD[8], &WMT_DIS_MCU_CLK_CMD[0],
			    osal_sizeof(WMT_DIS_MCU_CLK_CMD));
		break;
	default:
	...
	do {
		...
		iRet =
		    wmt_core_tx(&WMT_SET_MCU_CLK_CMD[0], osal_sizeof(WMT_SET_MCU_CLK_CMD),
				&u4WrittenSize, MTK_WCN_BOOL_FALSE);
		...
		if (osal_memcmp(evt_buffer, WMT_SET_MCU_CLK_EVT, osal_sizeof(WMT_SET_MCU_CLK_EVT))
		    != 0) {
			WMT_ERR_FUNC
			    ("WMT_SET_MCU_CLK_EVT compare fail:0x%02x,0x%02x,0x%02x,0x%02x,0x%02x\n",
			     evt_buffer[0], evt_buffer[1], evt_buffer[2], evt_buffer[3],
...
static INT32 opfunc_adie_lpbk_test(P_WMT_OP pWmtOp)
{
	...
	do {
		...
		iRet =
		    wmt_core_tx((PUINT8) &soc_adie_chipid_cmd[0], osal_sizeof(soc_adie_chipid_cmd),
