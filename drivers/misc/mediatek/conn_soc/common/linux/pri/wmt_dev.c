...
INT32 wmt_dbg_cmd_test_api(ENUM_WMTDRV_CMD_T cmd)
{
	...
	if ((cmd != WMTDRV_CMD_ASSERT) &&
	    ...
	    (cmd != WMTDRV_CMD_WARNRST_TEST) && (cmd != WMTDRV_CMD_FWTRACE_TEST)) {
	...
	if (offset == 1) {
		do {
			...
			if (cur_idx_pagedtrace > prev_idx_pagedtrace) {
				...
				/* pr_info("\n\n ++ get emi ++ ss\n\n"); */
				wmt_lib_get_fwinfor_from_emi(1, prev_idx_pagedtrace, &gEmiBuf[0],
							     len);
			...
			if (cur_idx_pagedtrace < prev_idx_pagedtrace) {
				if (prev_idx_pagedtrace < 0x8000) {
					...
					wmt_lib_get_fwinfor_from_emi(1, prev_idx_pagedtrace,
								     &gEmiBuf[0], len);
					pr_info
					...
					pr_info("++ prev_idx_pagedtrace invalid ...++\n\\n");
				}
...
static INT32 wmt_internal_loopback(INT32 count, INT32 max)
{
	...
	for (loop = 0; loop < count; loop++) {
		...
		for (offset = 0; offset < max; offset++) {
			lpbk_buffer.out_payload[offset] =
			    (offset + 1) /*for test use: begin from 1 */ & 0xFF;
		}
		...
		/*<3> compare result */
		if (memcmp
		    (lpbk_buffer.in_payload, lpbk_buffer.out_payload, lpbk_buffer.payload_length)) {
		...
		WMT_ERR_FUNC("[%s] exec WMT_TEST_LPBK_CMD succeed(loop = %d, size = %ld)\n",
			     __func__, loop, lpbk_buffer.payload_length);
...
#define REFRESH_TIME    300	/* sec */
	...
	for (index = 0; index < HISTORY_NUM; index++) {
		if (temp_table[index] >= TEMP_THRESHOLD) {
			query_cond = 1;
			WMT_INFO_FUNC
			    ("temperature table is still intial value, we should query temp temperature..\n");
		}
	}
...
#define WMT_IOCTL_ADIE_LPBK_TEST		_IOWR(WMT_IOC_MAGIC, 26, char*)
	...
	case WMT_IOCTL_SEND_BGW_DS_CMD:
		do {
			...
			pOp->op.au4OpData[0] = effectiveLen;	/* packet length */
			pOp->op.au4OpData[1] = (UINT32) &desense_buf[0];
			pSignal->timeoutValue = MAX_EACH_WMT_CMD;
			WMT_INFO_FUNC("OPID(%d) start\n", pOp->op.opId);
			if (DISABLE_PSM_MONITOR()) {
				WMT_ERR_FUNC("wake up failed\n");
...
static int WMT_open(struct inode *inode, struct file *file)
{
	...
	ret =
	    wait_event_timeout(gWmtInitWq, gWmtInitDone != 0, msecs_to_jiffies(WMT_DEV_INIT_TO_MS));
	if (!ret) {
