...
#define WIFI_DBG_FUNC(fmt, arg...)    if (gDbgLevel >= WIFI_LOG_DBG) { pr_info(PFX "%s: "  fmt, __func__ , ##arg); }
#define WIFI_INFO_FUNC(fmt, arg...)   if (gDbgLevel >= WIFI_LOG_INFO) { pr_info(PFX "%s: "  fmt, __func__ , ##arg); }
#define WIFI_WARN_FUNC(fmt, arg...)   if (gDbgLevel >= WIFI_LOG_WARN) { pr_info(PFX "%s: "  fmt, __func__ , ##arg); }
#define WIFI_ERR_FUNC(fmt, arg...)    if (gDbgLevel >= WIFI_LOG_ERR) { pr_info(PFX "%s: "   fmt, __func__ , ##arg); }
#define WIFI_TRC_FUNC(f)              if (gDbgLevel >= WIFI_LOG_DBG) {pr_info(PFX "<%s> <%d>\n", __func__, __LINE__); }
