...
#define WMT_PLAT_LOUD_FUNC(fmt, arg...)    if (wmtPlatLogLvl >= WMT_PLAT_LOG_LOUD) { pr_debug(DFT_TAG "[L]%s:"  fmt, __func__ , ##arg); }
#define WMT_PLAT_INFO_FUNC(fmt, arg...)    if (wmtPlatLogLvl >= WMT_PLAT_LOG_INFO) { pr_err(DFT_TAG "[I]%s:"  fmt, __func__ , ##arg); }
#define WMT_PLAT_WARN_FUNC(fmt, arg...)    if (wmtPlatLogLvl >= WMT_PLAT_LOG_WARN) { pr_warn(DFT_TAG "[W]%s:"  fmt, __func__ , ##arg); }
#define WMT_PLAT_ERR_FUNC(fmt, arg...)     if (wmtPlatLogLvl >= WMT_PLAT_LOG_ERR)  { pr_err(DFT_TAG "[E]%s(%d):"  fmt, __func__ , __LINE__, ##arg); }
#define WMT_PLAT_DBG_FUNC(fmt, arg...)     if (wmtPlatLogLvl >= WMT_PLAT_LOG_DBG)  { pr_debug(DFT_TAG "[D]%s:"  fmt, __func__ , ##arg); }
...
INT32 wmt_plat_init(UINT32 co_clock_en);
