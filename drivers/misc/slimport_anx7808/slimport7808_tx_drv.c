...
void sp_tx_power_down(enum SP_TX_POWER_BLOCK sp_tx_pd_block)
{
	...
	SLIMPORT_DBG("sp_tx_power_down\n");

}

int sp_tx_power_on(enum SP_TX_POWER_BLOCK sp_tx_pd_block)
{
	unchar c;
	int ret = 0;

	ret = sp_read_reg(TX_P2, SP_POWERD_CTRL_REG, &c);
	if (ret < 0){
		SLIMPORT_ERR("sp_tx_power_on fail\n");
		return ret;
	}
	...
	ret = sp_write_reg(TX_P2, SP_POWERD_CTRL_REG, c);

	if (ret < 0) {
		SLIMPORT_ERR("sp_tx_power_on fail\n");
		return ret;
	}
	SLIMPORT_DBG("sp_tx_power_on\n");
...
static unchar sp_tx_wait_aux_finished(void)
{
	...
	while (c & AUX_BUSY) {
		...
		sp_read_reg(TX_P0, SP_TX_AUX_STATUS, &c);
		if (ccnt > 100) {
			SLIMPORT_ERR("AUX Operaton does not finished, and time out\n");
			break;
		}
	}

	if (c & 0x0F) {
		SLIMPORT_ERR("aux operation failed %.2x\n", (uint)c);
		return 0;
	} else
		return 1;
...
static void sp_tx_lvttl_bit_mapping(void)
{
	...
	if (c1 == 0x00)
		...
		SLIMPORT_DBG("HDMI input color depth is not supported\n");
	...
	if (sp_tx_test_edid) {
		...
		sp_write_reg(TX_P2, SP_TX_VID_CTRL2_REG, c);
		sp_tx_test_edid = 0;
		SLIMPORT_DBG("***color space is set to 18bit***\n");
	}
...
void sp_tx_enable_video_input(unchar enable)
{
	...
	if (enable) {
		...
		c = (c & 0xf7) | VIDEO_EN;
		sp_write_reg(TX_P2, SP_TX_VID_CTRL1_REG, c);
		SLIMPORT_DBG("Slimport Video is enabled!\n");

	} else {
		sp_read_reg(TX_P2, SP_TX_VID_CTRL1_REG, &c);
		c &= ~VIDEO_EN;
		sp_write_reg(TX_P2, SP_TX_VID_CTRL1_REG, c);
		SLIMPORT_DBG("Slimport Video is disabled!\n");
...
static void sp_tx_enhancemode_set(void)
{
	...
	if (c & ENHANCED_FRAME_CAP) {
		...
		SLIMPORT_DBG("Enhance mode enabled\n");
	} else {
		...
		SLIMPORT_DBG("Enhance mode disabled\n");
	}
}
...
static void sp_tx_hw_hdcp_enable(void)
{
	...
	SLIMPORT_DBG("Hardware HDCP is enabled\n");
...
void sp_tx_show_infomation(void)
{
	...
	SLIMPORT_DBG_INFO("*******SP Video Information*******\n");
	...
	if (sp_tx_rx_type == RX_HDMI) {
		...
		SLIMPORT_DBG_INFO("   ANX7730 BC current FW Ver : %.2x\n",
				(uint)(bytebuf[0]&0x7f));
	}
	SLIMPORT_DBG_INFO("**********************************\n");
...
unchar sp_tx_chip_located(void)
{
	...
	if ((c1 == 0x08) && (c2 == 0x78)) {
		SLIMPORT_DBG("ANX7808 BA is found\n");
		return 1;
	} else {
		sp_tx_hardware_powerdown();
		SLIMPORT_DBG("ANX7808 is not found\n");
		return 0;
	}
...
void sp_tx_vbus_poweron(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		sp_read_reg(TX_P2, SP_TX_PLL_FILTER_CTRL6, &c);
		if (!(c & 0xc0)) {
			SLIMPORT_ERR("3.3V output enabled\n");
			return;
		} else {
			SLIMPORT_ERR("VBUS power can not be supplied\n");
		}
	}
...
void sp_tx_vbus_powerdown(void)
{
	...
	sp_write_reg(TX_P2, SP_TX_PLL_FILTER_CTRL6, c);
	SLIMPORT_ERR("3.3V output disabled\n");
}
...
#ifndef SSC_1
/*	SLIMPORT_DBG_INFO("*** Config SSC 0.4% ***\n");*/
...
#else
/*	SLIMPORT_DBG_INFO("*** Config SSC 1% ***\n");*/
...
static void sp_tx_config_audio(void)
{
	...
	SLIMPORT_DBG("AUD_Freq = %ld , LS_CLK = %ld\n", aud_freq, ls_clk);
...
static unchar sp_tx_bw_lc_sel(void)
{
	...
	if (over_bw)
		SLIMPORT_ERR("over bw!\n");
	else
		SLIMPORT_DBG_INFO("The optimized BW =%.2x\n", sp_tx_bw);
...
unchar sp_tx_hw_link_training(void)
{
	...
	if (!sp_tx_hw_lt_enable) {
		...
		SLIMPORT_DBG_INFO("Hardware link training\n");
		if (!sp_tx_get_pll_lock_status()) {
			SLIMPORT_ERR("PLL not lock\n");
			return 1;
		}
...
unchar sp_tx_lt_pre_config(void)
{
	...
	if (!sp_tx_link_config_done) {
		...
		if (c & CHA_STA) {
			SLIMPORT_ERR("Stream clock not stable!\n");
			return 1;
		}
...
void sp_tx_disable_slimport_hdcp(void)
{
	...
	sp_tx_clean_hdcp();
	sp_tx_video_mute(0);
	SLIMPORT_DBG("*****slimport HDCP is disabled********\n");
}
	...
	for (i = 0; i < 5; i++) {
		...
		case 0x01:
			...
			if (((SINK_OUI[0] == 0x00) &&
					...
					(SINK_OUI[5] == 0x38) &&
					(SINK_OUI[6] == 0x33))) {
				SLIMPORT_DBG_INFO("Downstream is 9832 VGA dongle\n");
...
unchar sp_tx_get_downstream_type(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		case 0x00:
			sp_tx_rx_type = RX_DP;
			ds_port_recoginze = 1;
			/*SLIMPORT_DBG("Downstream is DP dongle\n");*/
			break;
		case 0x01:
			sp_tx_aux_dpcdread_bytes(0x00, 0x04, 0x00, 8, SINK_OUI);
			if (((SINK_OUI[0] == 0x00) && (SINK_OUI[1] == 0x22)
			    ...
			    && (SINK_OUI[6] == 0x33))) {
				sp_tx_rx_type = RX_VGA_9832;
				/*SLIMPORT_DBG("Downstream is 9832VGA dongle\n");*/
			} else {
				sp_tx_rx_type = RX_VGA_GEN;
				/*SLIMPORT_DBG("Downstream is general DP2VGA converter\n");*/
			}
			ds_port_recoginze = 1;
			break;
		case 0x02:
			if (AUX_OK == sp_tx_aux_dpcdread_bytes
				...
				if ((SINK_OUI[0] == 0xb9)
					...
					&& (SINK_OUI[6] == 0x00)) {
					sp_tx_rx_type = RX_HDMI;
					/*SLIMPORT_DBG("Downstream is 7730 HDMI dongle\n");*/

				} else {
					sp_tx_rx_type = RX_DP;
					/*SLIMPORT_DBG("sink is general DP2HDMI converter\n");*/
				}
				ds_port_recoginze = 1;
			} else
...
/* Add memset to initialize edid block
   for removing previous edid data*/
	memset(bedid_firstblock, 0x00, 128);
	memset(bedid_extblock, 0x00, 128);
...
static void sp_tx_pll_changed_int_handler(void)
{
	...
	if (sp_tx_system_state > STATE_PARSE_EDID) {
		if (!sp_tx_get_pll_lock_status()) {
			SLIMPORT_ERR("PLL :_______________PLL not lock!\n");
			sp_tx_clean_hdcp();
...
static void sp_tx_auth_done_int_handler(void)
{
	...
	if (c & SP_TX_HDCP_AUTH_PASS) {
		...
		if (bytebuf[1] & 0x08) {
			...
			sp_tx_video_mute(1);
			sp_tx_clean_hdcp();
			SLIMPORT_ERR("Re-authentication!\n");
		} else {
...
static void sp_tx_link_chk_fail_int_handler(void)
{
	if (sp_tx_system_state >= STATE_HDCP_AUTH) {
		sp_tx_set_sys_state(STATE_HDCP_AUTH);
		sp_tx_clean_hdcp();
		SLIMPORT_ERR("IRQ :____________HDCP Sync lost!\n");
	}
}
...
static void sp_tx_lt_done_int_handler(void)
{
	...
	if (c & 0x70) {
		c = (c & 0x70) >> 4;
		SLIMPORT_ERR("HW LT failed in interrupt,");
		pr_err(" ERR code = %.2x\n", (uint) c);
		...
		SLIMPORT_DBG_INFO("HW LT succeed,LANE0_SET = %.2x, ", (uint) c);
		pr_info("link_bw = %.2x\n", (uint) c1);
	}
...
static void sp_tx_link_change_int_handler(void)
{
	...
	if (((al & 0x01) == 0) || (sl_cr == 0)) {
		if ((al & 0x01) == 0)
			SLIMPORT_ERR("Lane align not done\n");

		if (sl_cr == 0)
			SLIMPORT_ERR("Lane clock recovery not done\n");
		...
		if (sp_tx_rx_type_backup !=  sp_tx_rx_type) {
			...
			if (sp_tx_get_downstream_connection(sp_tx_rx_type)) {
				...
				&& sp_tx_link_config_done) {
					sp_tx_link_config_done = 0;
					sp_tx_set_sys_state(STATE_LINK_TRAINING);
					SLIMPORT_ERR("IRQ :____________re-LT request!\n");
				}
			} /*else
				sp_tx_set_sys_state(STATE_CABLE_PLUG);*/
		}
...
static void sp_tx_polling_err_int_handler(void)
{
	...
	if (sp_tx_pd_mode == 0) {
		SLIMPORT_ERR("Cwire polling is corrupted,power down ANX7808\n");
		sp_tx_clean_hdcp();
		sp_tx_vbus_powerdown();
		sp_tx_power_down(SP_TX_PWR_TOTAL);
		sp_tx_power_down(SP_TX_PWR_REG);
...
static void sp_tx_irq_isr(void)
{
	...
	if ((irq_vector & SINK_SPECIFIC_IRQ) && (sp_tx_rx_type == RX_HDMI)) {
		...
		if ((Int_vector1 & 0x01) == 0x01) {
			...
			if (c & 0x01) {
				if (sp_tx_system_state < STATE_PARSE_EDID)
					...
					SLIMPORT_DBG("Downstream HDMI is pluged!\n");
			}
		}
		...
		if ((Int_vector1 & 0x02) == 0x02) {
			sp_tx_aux_dpcdread_bytes(0x00, 0x05, 0x18, 1, &c);
			if ((c & 0x01) != 0x01) {
				SLIMPORT_DBG("Downstream HDMI is unpluged!\n");
		...
		if (((Int_vector1 & 0x04) == 0x04)
			...
			SLIMPORT_ERR("Rx specific  IRQ : Link is down!\n");
		...
		if ((Int_vector1 & 0x08) == 0x08) {
			SLIMPORT_DBG("Downstream HDCP is done!\n");

			if ((Int_vector1 & 0x10) != 0x10)
				SLIMPORT_DBG("Downstream HDCP is passed!\n");
			else {
				if (sp_tx_system_state > STATE_CONFIG_OUTPUT) {
					sp_tx_video_mute(1);
					sp_tx_clean_hdcp();
					sp_tx_set_sys_state(STATE_HDCP_AUTH);
					SLIMPORT_ERR("Re-authentication due to downstream HDCP failure!\n");
				}
			}
		}

		if ((Int_vector1 & 0x20) == 0x20) {
			SLIMPORT_ERR("Downstream HDCP link integrity check fail!\n");
		...
		if ((Int_vector1 & 0x40) == 0x40)
			SLIMPORT_DBG("Receive CEC command from upstream done!\n");

		if ((Int_vector1 & 0x80) == 0x80)
			SLIMPORT_DBG("CEC command transfer to downstream done!\n");

		if ((Int_vector2 & 0x04) == 0x04) {
			sp_tx_aux_dpcdread_bytes(0x00, 0x05, 0x18, 1, &c);

			if ((c & 0x40) == 0x40) {
				SLIMPORT_DBG("Downstream HDMI termination is detected!\n");
	...
	if (irq_vector & TEST_IRQ) {
		...
		if (test_vector & 0x01) {
			...
			SLIMPORT_DBG("test_bw = %.2x\n", (uint)sp_tx_test_bw);

			sp_tx_aux_dpcdread_bytes(0x00, 0x02, 0x60, 1, bytebuf);
			bytebuf[0] = bytebuf[0] | TEST_ACK;
			sp_tx_aux_dpcdwrite_bytes(0x00, 0x02, 0x60, 1, bytebuf);
			SLIMPORT_DBG("Set TEST_ACK!\n");
			sp_tx_set_sys_state(STATE_LINK_TRAINING);
			SLIMPORT_DBG("IRQ:test-LT request!\n");
		...
		if (test_vector & 0x04) {
			sp_tx_set_sys_state(STATE_PARSE_EDID);
			sp_tx_test_edid = 1;
			SLIMPORT_DBG("Test EDID Requested!\n");
		}
		/*phy test pattern*/
		if (test_vector & 0x08) {
			sp_tx_phy_auto_test();
...
void sp_tx_hdcp_process(void)
{
	...
	if (sp_tx_hdcp_auth_done) {
		...
		if (sp_tx_hdcp_auth_pass) {
			...
			sp_tx_video_mute(0);
			SLIMPORT_DBG("@@@@@@@hdcp_auth_pass@@@@@@\n");
		} else {
			sp_tx_hdcp_encryption_disable();
			sp_tx_video_mute(1);
			SLIMPORT_DBG("*********hdcp_auth_failed*********\n");
			return;
		}
...
void sp_tx_set_sys_state(enum SP_TX_System_State ss)
{
	...
	SLIMPORT_DBG_INFO("SP_TX To System State: ");
	...
	case STATE_CONFIG_HDMI:
		...
		debug_puts("STATE_CONFIG_HDMI\n");
		break;
	case STATE_CONFIG_OUTPUT:
		sp_tx_system_state = STATE_CONFIG_OUTPUT;
		debug_puts("STATE_CONFIG_OUTPUT\n");
		break;
...
void hdmi_rx_set_hpd(unchar enable)
{
	...
	if (enable) {
		...
		SLIMPORT_DBG("HPD high is issued\n");
	} else {
		...
		SLIMPORT_DBG("HPD low is issued\n");
	}
...
void hdmi_rx_set_termination(unchar enable)
{
	...
	if (enable) {
		...
		SLIMPORT_DBG("Termination high is issued\n");
	} else {
		...
		c |= TERM_PD;
		sp_write_reg(RX_P0, HDMI_RX_TMDS_CTRL_REG6, c);
		SLIMPORT_DBG("Termination low is issued\n");
	}
...
static void hdmi_rx_restart_audio_chk(void)
{
	if (hdmi_system_state == HDMI_AUDIO_CONFIG) {
		SLIMPORT_DBG("WAIT_AUDIO: hdmi_rx_restart_audio_chk\n");
		g_cts_got = 0;
		g_audio_got = 0;
	}
}
...
static void hdmi_rx_set_sys_state(enum HDMI_RX_System_State ss)
{
	if (hdmi_system_state != ss) {
		...
		switch (ss) {
		case HDMI_CLOCK_DET:
			SLIMPORT_DBG("HDMI_RX:  HDMI_CLOCK_DET\n");
			break;
		case HDMI_SYNC_DET:
			SLIMPORT_DBG("HDMI_RX:  HDMI_SYNC_DET\n");
			break;
		case HDMI_VIDEO_CONFIG:
			SLIMPORT_DBG("HDMI_RX:  HDMI_VIDEO_CONFIG\n");
			break;
		case HDMI_AUDIO_CONFIG:
			SLIMPORT_DBG("HDMI_RX:  HDMI_AUDIO_CONFIG\n");
			hdmi_rx_restart_audio_chk();
			break;
		case HDMI_PLAYBACK:
			SLIMPORT_DBG("HDMI_RX:  HDMI_PLAYBACK\n");
			break;
		default:
			break;
		}
...
static unchar hdmi_rx_is_video_change(void)
{
	...
	if ((g_cur_h_res < (n - 10)) || (g_cur_h_res > (n + 10))) {
		SLIMPORT_ERR("H_Res changed\n");
		SLIMPORT_ERR("Current H_Res = %ld\n", n);
		return 1;
	}
	...
	if ((g_cur_v_res < (n - 10)) || (g_cur_v_res > (n + 10))) {
		SLIMPORT_ERR("V_Res changed\n");
		SLIMPORT_ERR("Current V_Res = %ld\n", n);
		return 1;
	}
	...
	if (g_hdmi_dvi_status != cl) {
		SLIMPORT_ERR("DVI to HDMI or HDMI to DVI Change\n");
		return 1;
	}
...
static void hdmi_rx_show_video_info(void)
{
	...
	n = (n << 8) + cl;
	v_res = n;
	SLIMPORT_DBG("*****************HDMI_RX Info******************\n");
	SLIMPORT_DBG("   HDMI_RX Is Normally Play Back\n");
	sp_read_reg(RX_P0, HDMI_RX_HDMI_STATUS_REG, &c);

	if (c & HDMI_MODE)
		SLIMPORT_DBG("   HDMI_RX Mode = HDMI Mode\n");
	else
		SLIMPORT_DBG("   HDMI_RX Mode = DVI Mode\n");
	...
	SLIMPORT_DBG("   HDMI_RX Video Resolution = %ld * %ld \n", h_res, v_res);
	sp_read_reg(RX_P0, HDMI_RX_VIDEO_STATUS_REG1, &c);

	if (c & VIDEO_TYPE)
		SLIMPORT_DBG("    Interlace Video\n");
	else
		SLIMPORT_DBG("    Progressive Video\n");

	sp_read_reg(RX_P0, HDMI_RX_SYS_CTRL1_REG, &c);

	if ((c & 0x30) == 0x00)
		SLIMPORT_DBG("   Input Pixel Clock = Not Repeated\n");
	else if ((c & 0x30) == 0x10)
		SLIMPORT_DBG("   Input Pixel Clock = 2x Video Clock. Repeated\n");
	else if ((c & 0x30) == 0x30)
		SLIMPORT_DBG("   Input Pixel Clock = 4x Vvideo Clock. Repeated\n");

	if ((c & 0xc0) == 0x00)
		SLIMPORT_DBG("   Output Video Clock = Not Divided\n");
	else if ((c & 0xc0) == 0x40)
		SLIMPORT_DBG("   Output Video Clock = Divided By 2\n");
	else if ((c & 0xc0) == 0xc0)
		SLIMPORT_DBG("   Output Video Clock = Divided By 4\n");

	if (c & 0x02)
		SLIMPORT_DBG("   Output Video Using Rising Edge To Latch Data\n");
	else
		SLIMPORT_DBG("   Output Video Using Falling Edge To Latch Data\n");

	SLIMPORT_DBG("   Input Video Color Depth = ");
	...
	SLIMPORT_DBG("   Input Video Color Space = ");
	sp_read_reg(RX_P1, HDMI_RX_AVI_DATA00_REG, &c);
	c &= 0x60;
	...
	if (c & AUTH_EN)
		SLIMPORT_DBG("   Authentication is attempted\n");
	else
		SLIMPORT_DBG("   Authentication is not attempted\n");
	...
	if (cl < 20)
		SLIMPORT_DBG("   Decryption is active\n");
	else
		SLIMPORT_DBG("   Decryption is not active\n");
...
static void hdmi_rx_clk_det_int(void)
{
	...
	SLIMPORT_DBG("*HDMI_RX Interrupt: Pixel Clock Change\n");
	if (sp_tx_system_state > STATE_CONFIG_HDMI) {
		hdmi_rx_mute_audio();
		hdmi_rx_mute_video();
		sp_tx_video_mute(1);
	...
	if (c & TMDS_CLOCK_DET) {
		SLIMPORT_ERR("Pixel clock existed\n");
...
static void hdmi_rx_sync_det_int(void)
{
	...
	SLIMPORT_DBG("*HDMI_RX Interrupt: Sync Detect\n");
...
static void hdmi_rx_hdmi_dvi_int(void)
{
	...
	SLIMPORT_DBG("*HDMI_RX Interrupt: HDMI-DVI Mode Change");
	sp_read_reg(RX_P0, HDMI_RX_HDMI_STATUS_REG, &c);
	hdmi_rx_get_video_info();

	if ((c & HDMI_MODE) == HDMI_MODE) {
		SLIMPORT_DBG("hdmi_rx_hdmi_dvi_int: HDMI MODE");
...
static void hdmi_rx_avmute_int(void)
{
	...
	if (avmute_status & MUTE_STAT) {
		SLIMPORT_DBG("HDMI_RX AV mute packet received");
...
static void hdmi_rx_hdcp_error_int(void)
{
	...
	if (g_hdcp_err_cnt >= 40) {
		g_hdcp_err_cnt = 0;
		SLIMPORT_ERR("Lots of hdcp error occured ...\n");
		hdmi_rx_mute_audio();
		hdmi_rx_mute_video();
...
static void hdmi_rx_new_avi_int(void)
{
	...
	SLIMPORT_DBG("*HDMI_RX Interrupt: New AVI Packet");
	sp_tx_avi_setup();
...
static void hdmi_rx_new_vsi_int(void)
{
		...
		SLIMPORT_ERR("*HDMI_RX Interrupt: NEW VSI packet\n");
		...
		if (!(c&INFO_FRAME_VSC_EN)) {
			...
			if ((vsi_header == 0x81) &&
				...
				SLIMPORT_DBG("3D VSI packet is detected. Config VSC packet\n");
				...
				default:
					v3d_structure = 0x00;
					SLIMPORT_ERR("3D structure is not supported\n");
					break;
				}
...
static void hdmi_rx_no_vsi_int(void)
{
		...
		sp_read_reg(TX_P0, SP_TX_3D_VSC_CTRL, &c);
		if (c&INFO_FRAME_VSC_EN) {
			SLIMPORT_ERR("No new VSI is received, disable  VSC packet\n");
			c &= ~INFO_FRAME_VSC_EN;
			sp_write_reg(TX_P0, SP_TX_3D_VSC_CTRL, c);
			sp_tx_mpeg_setup();
...
void sp_tx_config_hdmi_input(void)
{
	...
	if (hdmi_system_state == HDMI_SYNC_DET) {
		...
		if (!(c & TMDS_DE_DET)) {
			if (g_sync_expire_cntr >= SCDT_EXPIRE_TH) {
				SLIMPORT_ERR("No sync for long time\n");
				/* misc reset */
	...
	if (hdmi_rx_is_video_change()) {
		SLIMPORT_ERR("Video Changed , mute video and mute audio\n");
		g_video_stable_cntr = 0;
		...
		g_video_stable_cntr++;
		SLIMPORT_DBG("WAIT_VIDEO: Wait for video stable cntr\n");
		...
		if (!(avmute_status & MUTE_STAT)) {
			...
			if (g_hdmi_dvi_status) {
				SLIMPORT_DBG("HDMI mode: Video is stable\n");
				sp_tx_send_message(MSG_INPUT_HDMI);
				hdmi_rx_set_sys_state(HDMI_AUDIO_CONFIG);
			} else {
				SLIMPORT_DBG("DVI mode: Video is stable\n");
...
void sp_tx_phy_auto_test(void)
{
	...
	case 0x00:
		...
		SLIMPORT_DBG("lane0,emp 0db\n");
		break;
	case 0x01:
		sp_read_reg(TX_P0, 0xA3, &b_emp);
		sp_write_reg(TX_P0, 0xA3, (b_emp&~0x18)|0x08);
		SLIMPORT_DBG("lane0,emp 3.5db\n");
		break;
	case 0x02:
		sp_read_reg(TX_P0, 0xA3, &b_emp);
		sp_write_reg(TX_P0, 0xA3, (b_emp&~0x18)|0x10);
		SLIMPORT_DBG("lane0,emp 6db\n");
		break;
