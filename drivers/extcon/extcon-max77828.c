					  ...
					  const char *buf, size_t count)
{
	...
	if (!strncasecmp(buf, "PDA", 3)) {
		...
		max77828_muic_set_usb_path(info, PATH_USB_CP);
	} else
...
#define MAX77828_PATH_AUDIO_MASK \
...
static void max77828_muic_detect_dev(struct max77828_muic_info *info, int irq)
{
	...
	if (intr == INT_ATTACH) {
		...
		max77828_muic_handle_attach(info, status[0], status[1]);
...
static int max77828_resume(struct device *dev)
{
	struct max77828_muic_info *info = dev_get_drvdata(dev);
	if (info)
		max77828_muic_set_path(info, info->path);
	else
		pr_err("%s:, dev_get_drvdata fail\n", __func__);
