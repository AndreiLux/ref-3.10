...
static void	dwc3_otg_phy_tune(struct dwc3* dwc,bool id)
{
	int phyparam0 = DEFAULT_VALUE_PHYPARAM0;

	printk(KERN_DEBUG"usb: %s id:%d now PHYPARAM0 : 0x%x\n",__func__,id,	\
		usb_phy_io_read(dwc->usb3_phy,EXYNOS5_DRD_PHYPARAM0));

	if(id) {
		/* Set for same value of bootloader */
		/* To set same value as Kernel  LSI recommended value.*/
		/* txpreempamptune[16:15] 2'b01: 1X */
		phyparam0 &= ~PHYPARAM0_TXPREEMPAMPTUNE_MASK;
		phyparam0 |= PHYPARAM0_TXPREEMPAMPTUNE;
		/* txvreftune[25:22] 4'b1011: +10% */
		phyparam0 &= ~PHYPARAM0_TXVREFPTUNE_MASK;
		phyparam0 |= PHYPARAM0_TXVREFPTUNE;
	} else {
		phyparam0 = DEFAULT_VALUE_PHYPARAM0;
	}

	usb_phy_io_write(dwc->usb3_phy,phyparam0,EXYNOS5_DRD_PHYPARAM0);
	printk(KERN_DEBUG"usb: %s id:%d set PHYPARAM0 0x%x\n",__func__,id,	\
		usb_phy_io_read(dwc->usb3_phy,EXYNOS5_DRD_PHYPARAM0));
...
static int dwc3_otg_statemachine(struct otg_fsm *fsm)
{
	...
	printk(KERN_DEBUG"usb: %s fsm->reset:%d, phy->state:%d, fsm->id: %d, fsm->b_sess_vld:%d \n",
		__func__,fsm->reset, phy->state,fsm->id,fsm->b_sess_vld);
