...
int register_gpios(struct otg_notify *n)
{
	...
	pr_info("vbus detect gpio %d is registerd.\n", vbus_gpio);

redriver_en_gpio_phase:
	if (n->redriver_en_gpio < 0)
		goto err;
	...
	pr_info("redriver en gpio %d is registerd.\n", redriver_gpio);
err:
	return ret;
}
...
static void otg_notify_work(struct work_struct *data)
{
	...
	pr_info("%s event=%s, enable=%s\n", __func__,
		event_string(state_work->event),
			state_work->enable == true ? "on" : "off");

	notify = get_otg_notify();

	switch (state_work->event) {
	case NOTIFY_EVENT_NONE:
		break;
	case NOTIFY_EVENT_SMARTDOCK_USB:
	case NOTIFY_EVENT_VBUS:
		if (state_work->enable) {
	...
	case NOTIFY_EVENT_CHARGER:
		if (notify->set_charger)
			notify->set_charger(state_work->enable);
		break;
	case NOTIFY_EVENT_SMARTDOCK_TA:
	case NOTIFY_EVENT_AUDIODOCK:
		u_notify->diable_v_drive = state_work->enable;
		if (state_work->enable) {
			if (notify->is_wakelock)
				wake_lock(&u_notify->wlock);
			u_notify->ndev.mode = NOTIFY_HOST_MODE;
	...
	case NOTIFY_EVENT_DRIVE_VBUS:
		...
		u_notify->oc_noti = state_work->enable;
		if (notify->vbus_drive)
			notify->vbus_drive(state_work->enable);
		goto no_save_event;
	case NOTIFY_EVENT_OVERCURRENT:
		host_state_notify(&u_notify->ndev,
						NOTIFY_HOST_OVERCURRENT);
	...
	u_notify->c_type = state_work->event;
		...
		unsigned long event, void *param)
{
	...
	if (event < NOTIFY_EVENT_NONE ||
			event > NOTIFY_EVENT_VBUSPOWER) {
