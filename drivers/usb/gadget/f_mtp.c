...
/* USB 3.0 (G3) : 16384 */
/* USB 3.0 (CTO Odin) : 32768 (16384*2) */
/* Original : 16384 */
...
/* USB 3.0 (G3) : 262144 (MTP_BULK_BUFFER_SIZE(16384)*16) */
/* Original : 16384 (MTP_BULK_BUFFER_SIZE) */
#ifdef CONFIG_USB_G_LGE_MTP_SS
unsigned int mtp_rx_req_len = 262144;
#else
#ifdef CONFIG_USB_G_LGE_ANDROID
unsigned int mtp_rx_req_len = 32768; /* (MTP_BULK_BUFFER_SIZE*2) */
#else
...
/* USB 3.0 (G3) : 32(MTP_TX_REQ_MAX(8)*4)) */
/* Original : MTP_TX_REQ_MAX(4) */
#ifdef CONFIG_USB_G_LGE_MTP_SS
...
#ifdef CONFIG_USB_G_LGE_ANDROID
	struct delayed_work wedge_work;
#endif
...
static int mtp_open(struct inode *ip, struct file *fp)
{
	...
	if(lgeusb_get_factory_cable()) {
		pr_info("pif cable is plugged, skip mtp open\n");
		return -ENODEV;
	}
#endif
				...
				const struct usb_ctrlrequest *ctrl)
{
	...
	if (ctrl->bRequestType ==
...
#ifdef CONFIG_USB_G_LGE_ANDROID
		/* Fix Windows 7 Bug at PCSync with PID (61F1 : MTP interfacenumber is 3)
		 ...
		 */
#ifdef CONFIG_USB_G_LGE_MULTIPLE_CONFIGURATION
				&& (w_index == 0 || w_index == mtp_interface_desc.bInterfaceNumber
					|| w_index == mtp_interface_desc2.bInterfaceNumber)
#else
				&& (w_index == 0 || w_index == mtp_interface_desc.bInterfaceNumber)
#endif /*                                         */
#else /*                          */
...
#ifdef CONFIG_USB_G_LGE_ANDROID
#define WEDGE_WORK_INTERVAL           msecs_to_jiffies(50)

static int mtp_set_wedge(struct mtp_dev *dev)
{
	struct usb_composite_dev *cdev = dev->cdev;
	int	ret;

	ret = usb_ep_set_wedge(dev->ep_in);
	if (ret) {
		ERROR(cdev, "failed to set wedge ep %s, result %d\n",
					dev->ep_intr->name, ret);
		return ret;
	}

	ret = usb_ep_set_wedge(dev->ep_out);
	if (ret) {
		ERROR(cdev, "failed to set wedge ep %s, result %d\n",
					dev->ep_intr->name, ret);
		return ret;
	...
	ret = usb_ep_set_wedge(dev->ep_intr);
	if (ret) {
		ERROR(cdev, "failed to set wedge ep %s, result %d\n",
					dev->ep_intr->name, ret);
	}
...
static void set_wedge_work(struct work_struct *work)
{
	struct mtp_dev *dev =
		container_of(work, struct mtp_dev, wedge_work.work);

	if(mtp_set_wedge(dev) == -EAGAIN) {
		/* Wait for a short time and then try again */
		queue_delayed_work(dev->wq, &dev->wedge_work, WEDGE_WORK_INTERVAL);
	}
}
#endif
...
#ifdef CONFIG_USB_G_LGE_ANDROID
	if(lge_get_laf_mode() == LGE_LAF_MODE_LAF) {
		cancel_delayed_work_sync(&dev->wedge_work);
	}
#endif
...
#ifdef CONFIG_USB_G_LGE_ANDROID
	if(lge_get_laf_mode() == LGE_LAF_MODE_LAF) {
		INIT_DELAYED_WORK(&dev->wedge_work, set_wedge_work);
	}
#endif
