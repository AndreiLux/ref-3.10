...
struct touch_platform_data {
	...
	const char* inbuilt_fw_name_s3508;
...
struct touch_fw_info {
	...
	char		fw_path_s3508[256];		// used for dynamic firmware upgrade
...
struct lge_touch_data {
	...
	struct delayed_work		work_usb_status;
	unsigned long 			usb_event;
#endif
#ifdef LIGER_SUSPEND_KNOCK_ON
	struct delayed_work		proximity_check;
#endif
	struct mutex			thread_lock;
	struct mutex			suspend_resume_lock; //mutex_added_on_odin
	...
	struct wake_lock		int_wake_lock;	//wake_lock_added_on_odin
