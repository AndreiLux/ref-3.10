...
static __always_inline int max77819_2nd_read (struct max77819_io_2nd *io_2nd,
    u8 addr, u8 *val)
{
    unsigned int buf = 0;
    int rc = regmap_read(io_2nd->regmap, (unsigned int)addr, &buf);
...
static __always_inline int max77819_2nd_write (struct max77819_io_2nd *io_2nd,
    u8 addr, u8 val)
{
    unsigned int buf = (unsigned int)val;
    return regmap_write(io_2nd->regmap, (unsigned int)addr, buf);
}
