...
#elif defined(CONFIG_FB)
static void suspend_autotest_early_suspend()
#endif
{
	/* if test isn't enabled, no need to proceed below code */
	if (!test_enable)
...
#elif defined(CONFIG_FB)
static void suspend_autotest_late_resume()
#endif
{
	/* if test isn't enabled, no need to proceed below code */
	if (!test_enable)
				...
				unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK) {
		...
		if (*blank == FB_BLANK_UNBLANK) {
			printk("%s %d: resume\n",__func__,__LINE__);
			suspend_autotest_late_resume();
		} else if (*blank ==FB_BLANK_POWERDOWN) {
			printk("%s %d: suspend\n",__func__,__LINE__);
			suspend_autotest_early_suspend();
		}
	}
	return 0;
}

static struct notifier_block suspend_autotest_fb_notifier = {
	.notifier_call = fb_notifier_callback,
};
#endif

static int suspend_autotest_probe(struct platform_device *pdev)
{
	int err;
	char *pony = NULL;
...
#elif defined(CONFIG_FB)
	if ((err = fb_register_client(&suspend_autotest_fb_notifier))) {
		printk("%s %d: Unable to register fb_notifier: %d\n",__func__,__LINE__,err);
		goto err_input_dev;
	}
#endif
...
static int suspend_autotest_remove(struct platform_device *pdev)
{
	if (wake_lock_active(&test_wake_lock))
		wake_unlock(&test_wake_lock);
