...
#include <mt8135_aston/cust_kpd.h>
...
static struct i2c_board_info i2c_mpu6050 __initdata = {
	I2C_BOARD_INFO("MPU6050G", (MPU_I2C_SLAVE_ADDR >> 1)) };
...
#define IDX_PS_OFF  3
	...
	if ((DUAL_CAMERA_MAIN_SENSOR == SensorIdx) &&
	    currSensorName && (0 == strcmp(SENSOR_DRVNAME_OV2724_MIPI_RAW, currSensorName))) {
		pinSetIdx = 0;
	} else if ((DUAL_CAMERA_SUB_SENSOR == SensorIdx) &&
		   currSensorName && (0 == strcmp(SENSOR_DRVNAME_HI704_YUV, currSensorName))) {
		pinSetIdx = 1;
	} else {
	...
	if (On) {
		...
		/* Rear(Main) Camear : OV2724 */
		if (pinSetIdx == 0) {
			pr_debug("Power on main sensor...\n");
			pr_debug("RST pin: 0x%x, PDN pin: 0x%x\n", pinSet[pinSetIdx][IDX_PS_CMRST],
			       pinSet[pinSetIdx][IDX_PS_CMPDN]);

			/* Turn OFF RST & PDN pin of another camera */
			if (mt_set_gpio_mode
			...
			/* Turn OFF RST & PDN pin of current camera */
			if (mt_set_gpio_mode
			    (pinSet[pinSetIdx][IDX_PS_CMRST],
		...
		} else if (pinSetIdx == 1) {	/* Sub cam : Hi704 */
			...
			/* Turn ON MCLK (GPIO pin #182) */
			if (mt_set_gpio_mode(GPIO_CAMERA_CMMCLK_PIN, GPIO_CAMERA_CMMCLK_PIN_M_MCLK))
				PK_DBG("[CAMERA SENSOR][MCLK] set gpio mode failed!!\n");

			if (mt_set_gpio_dir(GPIO_CAMERA_CMMCLK_PIN, GPIO_DIR_OUT))
				PK_DBG("[CAMERA SENSOR][MCLK] set gpio dir failed!!\n");

			if (mt_set_gpio_out(GPIO_CAMERA_CMMCLK_PIN, GPIO_OUT_ONE))
				PK_DBG("[CAMERA SENSOR][MCLK] set gpio failed!!\n");
		...
		/* Turn Off MCLK (GPIO pin #182) to Low */
		if (mt_set_gpio_mode(GPIO_CAMERA_CMMCLK_PIN, GPIO_CAMERA_CMMCLK_PIN_M_GPIO))
			PK_DBG("[CAMERA SENSOR][MCLK] set gpio mode failed!!\n");

		if (mt_set_gpio_dir(GPIO_CAMERA_CMMCLK_PIN, GPIO_DIR_OUT))
			PK_DBG("[CAMERA SENSOR][MCLK] set gpio dir failed!!\n");

		if (mt_set_gpio_out(GPIO_CAMERA_CMMCLK_PIN, GPIO_OUT_ZERO))
			PK_DBG("[CAMERA SENSOR][MCLK] set gpio failed!!\n");
...
/* GPIO id configuration */
#define MXT_GPIO_RESET_PIN	GPIO_CTP_RST_PIN
#define MXT_GPIO_RESET_MODE	GPIO_CTP_RST_PIN_M_GPIO
#define MXT_GPIO_IRQ_PIN	GPIO_CTP_EINT_PIN
#define MXT_GPIO_IRQ_MODE	GPIO_CTP_EINT_PIN_M_EINT
#define MXT_GPIO_IRQ_NUM	CUST_EINT_TOUCH_PANEL_NUM

/* I2C device info */
#define MXT_I2C_DEV_BUS		0
#define MXT_I2C_DEV_ADDRESS	0x4c
#define MXT_I2C_BUS_CLK		400

static struct mxt_platform_data atmel_pdata = {
	.reset_gpio = MXT_GPIO_RESET_PIN,
	.irq_gpio = MXT_GPIO_IRQ_PIN,
};

static struct i2c_board_info atmel_ts_board_info __initdata = {
	I2C_BOARD_INFO("atmel_mxt_ts", MXT_I2C_DEV_ADDRESS),
	.irq = MXT_GPIO_IRQ_NUM,
...
static void __init ariel_init(void)
{
	...
	/* For ATMEL TouchScreen */
	/* Set line modes to work as an interrupt and i/o respectively */
	mt_set_gpio_mode(atmel_pdata.irq_gpio, MXT_GPIO_IRQ_MODE);
	mt_set_gpio_mode(atmel_pdata.reset_gpio, MXT_GPIO_RESET_MODE);
	i2c_register_board_info(MXT_I2C_DEV_BUS, &atmel_ts_board_info, 1);
	mt_audio_init(&audio_custom_gpio_data, 0);

	/* For Accelerometer */
	i2c_register_board_info(MPU_I2C_DEV_BUS, &i2c_mpu6050, 1);
	/* For Gyroscope */
	mpu_gpio_config();
	/* For Magnetometer */
