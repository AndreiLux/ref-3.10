...
static struct etm_driver_data etm_driver_data[] = {
	...
	{
	 .etm_regs = IOMEM(DEBUGTOP_BASE + 0x5C000),
	 .is_ptm = 1,
	 },
	{
	 .etm_regs = IOMEM(DEBUGTOP_BASE + 0x5D000),
	 .is_ptm = 1,
	 },
};
...
static int __init pftracer_init(void)
{
	...
	if (CHIP_SW_VER_01 == mt_get_chip_sw_ver()) {
		/* CA15 MCI ATCLK */
		writel(readl((volatile unsigned int *)(0xF0200248)) | 0xC,
		       (volatile unsigned int *)(0xF0200248));
	}
