...
#define CA7_CACHE_CONFIG        (CA7MCUSYS_CFGREG_BASE + 0x0000)
#define CA7_BUS_CONFIG          (CA7MCUSYS_CFGREG_BASE + 0x0014)
...
#define CPU_DORMANT_INFO(fmt, args...)                      pr_info("[Power/cpu_dormant] "fmt, ##args)
#endif
...
static void platform_save_context(void)
{
	...
	/* pr_info("[platform_save_context] cpu_id=%d", cpu_id); */
	...
	if (cpu_id == 0) {
		...
		/* !!!!FIXME!!!! Can only write CA7_CCI400_DVM_EN in secure world */
...
/* pr_info("0xF0200114: x%x", reg_read(0xF0200114)); */
...
void cpu_start_restore(void)
{
	...
	reg_write(0x10200114, reg_read(0x10200114) & ~0x10);
