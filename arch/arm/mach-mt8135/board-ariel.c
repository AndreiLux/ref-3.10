...
static void __init ariel_init(void)
{
	...
	if (system_type == BOARD_ID_ASTON)
		ariel_config_i2c(
			aston_i2c_config, ARRAY_SIZE(aston_i2c_config), system_rev);
	else
		ariel_config_i2c(
			ariel_i2c_config, ARRAY_SIZE(ariel_i2c_config), system_rev);

	mt_bls_init(NULL);

	mt_audio_init(&audio_custom_gpio_data,
		(system_type == BOARD_ID_ASTON) ? 0 : 2); /* 0=Stereo, 1=MonoLeft, 2=MonoRight */

	if (system_type == BOARD_ID_ARIEL && system_rev > BOARD_REV_ARIEL_PROTO_2_0) {
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP4
		cyttsp4_register_core_device(&cyttsp4_core_info);
		cyttsp4_register_device(&cyttsp4_mt_device_info);
#endif	/* CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP4 */
