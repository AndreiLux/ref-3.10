...
static long mpu_dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (mtk_mon == NULL) {
		pr_info("Don't Have PMU Resource\n");
		return -1;
	}
	...
	case IOCTL_WRITE_PMU:
		...
		if (ret != 0) {
			pr_info("ERROR: wrong return value [IOCTL_WRITE_PMU]\n");
			return -1;
		}
	...
	case IOCTL_READ_PMU:
		...
		if (ret != 0) {
			pr_info("ERROR: wrong return value [IOCTL_READ_PMU]\n");
			return -1;
		}
		break;
#ifdef CONFIG_CACHE_L2X0
	case IOCTL_WRITE_L2C:
		...
		if (ret != 0) {
			pr_info("ERROR: wrong return value [IOCTL_WRITE_L2C]\n");
			return -1;
		}
	...
	case IOCTL_READ_L2C:
		...
		if (ret != 0) {
			pr_info("ERROR: wrong return value [IOCTL_READ_L2C]\n");
			return -1;
		}
		break;
#endif
	...
	case IOCTL_CTRL:
		...
		case MONITOR_STOP:
			...
			break;
		default:
			pr_info("Error Number: Start(1), Stop(2)!!!\n");
			break;
		}
		break;
	...
	case IOCTL_PRINTLOG:
		...
		if (ret != 0) {
			pr_info("ERROR: wrong return value [IOCTL_PRINTLOG]\n");
			return -1;
		}
		break;
...
static int __init mon_user_init(void)
{
	...
	if (ret < 0) {
		pr_info("PMU MISC Register fail\n");
		return ret;
	}
