...
#define GPT_SYSCNT_ASSIST_ID    (GPT2)
#endif
...
struct mt_clock mt8135_gpt = {
	...
	.irq = {
		...
		.handler = gpt_handler,
		.dev_id = &mt8135_gpt.clockevent,
		.irq = MT8135_APARM_GPTTIMER_IRQ_LINE,
		},
};

struct mt_clock_init __initdata mt8135_gpt_init = {
...
{
	unsigned int cmp;
	struct clock_event_device *evt = &mt8135_gpt.clockevent;
	struct gpt_device *dev = id_to_dev(GPT_CLKEVT_ID);
	...
	pr_info("GPT1_CMP = %d, HZ = %d\n", cmp, HZ);
}

#ifndef CONFIG_CLKSRC_64_BIT
static inline void setup_clksrc(void)
{
	struct clocksource *cs = &mt8135_gpt.clocksource;
	struct gpt_device *dev = id_to_dev(GPT_CLKSRC_ID);
...
static void syscnt_assist_handler(unsigned long data)
{
	...
	do {
		...
		if ((cnt / CHECK_WARNING_TIMERS) && !(cnt % CHECK_WARNING_TIMERS)) {
			pr_info("[%s]WARNING: fail to sync GPT_CNTH!! assist(0x%08x),"
	...
	pr_info("[%s]syscnt assist IRQ!! assist(0x%08x),syscnt(0x%08x,0x%08x),"
...
static void syscnt_assist_resume(void)
{
	...
	pr_info("[%s]assist(0x%08x, 0x%08x),syscnt(0x%08x,0x%08x),loop(%u->%u)\n",
