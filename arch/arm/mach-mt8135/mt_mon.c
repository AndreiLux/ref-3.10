...
static ssize_t bm_master_evt_store(struct device_driver *driver, const char *buf, size_t count)
{
	...
	} else if (!strncmp(buf, "APMCU1", strlen("APMCU1"))) {
		bm_master_evt = BM_MASTER_AP_MCU1;
	} else if (!strncmp(buf, "GPU", strlen("GPU"))) {
		bm_master_evt = BM_MASTER_GPU;
	} else if (!strncmp(buf, "APMCU_ALL", strlen("APMCU_ALL"))) {
		bm_master_evt = BM_MASTER_AP_MCU | BM_MASTER_AP_MCU1;
...
static ssize_t cpu_pmu_evt_show(struct device_driver *driver, char *buf)
{
	...
	if (p_pmu == NULL) {
		pr_info("PMU user interface isn't ready now!\n");
		return 0;
	}
...
static ssize_t cpu_pmu_evt_store(struct device_driver *driver, const char *buf, size_t count)
{
	...
	if (i != 8) {
		pr_info("The number of parameter is wrong!!!\n");
		return -1;
	}
	...
	for (i = 0; i < 6; i++)
		pr_info("%x  ", p_pmu->perf_cfg.event_cfg[i]);
	pr_info("\n");
	p_pmu->enable();
...
static struct platform_driver mt_mon_platform_drv = {
	.driver = {
		   .name = "mt_monitor",
...
static int __init mt_mon_mod_init(void)
{
	...
	ret = driver_register(&mt_mon_platform_drv.driver);

	if (ret) {
		pr_info("fail to register mt_mon_drv\n");
		return ret;
	}

	ret = driver_create_file(&mt_mon_platform_drv.driver, &driver_attr_bm_master_evt);
	ret |= driver_create_file(&mt_mon_platform_drv.driver, &driver_attr_bm_rw_type_evt);
	ret |= driver_create_file(&mt_mon_platform_drv.driver, &driver_attr_mon_mode_evt);
	ret |= driver_create_file(&mt_mon_platform_drv.driver, &driver_attr_mon_period_evt);
	ret |= driver_create_file(&mt_mon_platform_drv.driver, &driver_attr_mon_manual_evt);
	ret |= driver_create_file(&mt_mon_platform_drv.driver, &driver_attr_cpu_pmu_cfg);

	if (ret) {
		pr_info("fail to create mt_mon sysfs files\n");
...
/* *(volatile unsigned int *)0xF702A000 |= 0x1000; */
	...
	BM_SetMonitorCounter(1, BM_MASTER_MM1,
			     BM_TRANS_TYPE_4BEAT | BM_TRANS_TYPE_8Byte | BM_TRANS_TYPE_BURST_WRAP);
	BM_SetMonitorCounter(2, BM_MASTER_AP_MCU | BM_MASTER_AP_MCU1,
			     BM_TRANS_TYPE_4BEAT | BM_TRANS_TYPE_8Byte | BM_TRANS_TYPE_BURST_WRAP);
	BM_SetMonitorCounter(3, BM_MASTER_GPU,
			     BM_TRANS_TYPE_4BEAT | BM_TRANS_TYPE_8Byte | BM_TRANS_TYPE_BURST_WRAP);
	BM_SetMonitorCounter(4, BM_MASTER_MM0_PERI,
			     BM_TRANS_TYPE_4BEAT | BM_TRANS_TYPE_8Byte | BM_TRANS_TYPE_BURST_WRAP);
...
int register_monitor(struct mtk_monitor **p_mon, MonitorMode mode)
{
	...
	if (mode == MODE_MANUAL_USER || mode == MODE_MANUAL_KERNEL) {
		if (!mt_mon_log_buff) {
			...
			if (!mt_mon_log_buff) {
				pr_warn("fail to allocate the buffer for the monitor log\n");
