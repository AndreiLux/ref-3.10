...
static struct platform_driver mt_mci_drv = {
	.driver = {
		   .name = "mci",
...
void irq_dma_handler(void *data)
{
	...
	for (i = 0; i < BUFF_LEN / sizeof(unsigned int); i++)
		if (dst_array_v[i] != src_array_v[i]) {
			pr_info("DMA ERROR at Address %x,%x != %x\n", &dst_array_v[i],
				dst_array_v[i], src_array_v[i]);
			result = -1;
		}
	if (result == 0)
		pr_info("Channel1 DMA TEST PASS\n");
	else
		pr_info("Channel1 DMA TEST failed\n");
	pr_info("in handler:MCI_EVENT0_CNT=%x,MCI_EVENT1_CNT:%x\n",
...
void irq_dma_handler2(void *data)
{
	...
	pr_info("Channel2 DMA TEST PASS\n");
	mt65xx_dma_running_status();
...
void irq_dma_handler3(void *data)
{
	...
	for (i = 0; i < BUFF_LEN / sizeof(unsigned int); i++)
		if (dst_array_v[i] != PATTERN3) {
			pr_info("DMA ERROR at Address %x,%x != %x, (%x=%x)\n", &dst_array_v[i],
	...
	pr_info("in handler:MCI_EVENT0_CNT=%x,MCI_EVENT1_CNT:%x\n",
		*((unsigned int *)MCI_EVENT0_CNT), *((unsigned int *)MCI_EVENT1_CNT));
	if (result == 0)
		pr_info("Read snoop TEST PASS\n");
	else
		pr_info("Read snoop TEST failed\n");
	mt65xx_dma_running_status();
	mt65xx_free_gdma(channel);
...
void irq_dma_handler4(void *data)
{
	...
	pr_info("in handler:MCI_EVENT0_CNT=%x,MCI_EVENT1_CNT:%x\n",
		*((unsigned int *)MCI_EVENT0_CNT), *((unsigned int *)MCI_EVENT1_CNT));
	if (result == 0)
		pr_info("Write snoop TEST PASS\n");
	else
		pr_info("Write snoop TEST failed\n");
	mt65xx_dma_running_status();
	mt65xx_free_gdma(channel);
...
static int dma_test_init(unsigned int cohen, unsigned cacheable)
{
	...
	pr_info("GDMA channel:%d\n", channel);
	if (channel < 0) {
		pr_info("ERROR Register DMA\n");
		return -1;
	}
	...
	if (cacheable) {
		...
		if (dma_mapping_error(NULL, dst_array_p) || dma_mapping_error(NULL, src_array_p)) {
			pr_info("ERROR Couldn't map DMA\n");
			return -1;
		}
		pr_info("[DMA]dst_array_v:%x,src_array_v:%x,dst_array_p:%x,src_array_p:%x\n",
...
int complex_mem_test(void)
{
	...
	pr_info("[mci][cpu%d]memory test: start:%x\n", raw_smp_processor_id(), start);
	size = len >> 2;
...
static void ts_complex_mem_test(void)
{
	...
	if (ret < 0)
		pr_info("[MCI][CPU%d]complex_mem_test failed\n", raw_smp_processor_id());
	else
		pr_info("[MCI][CPU%d]complex_mem_test passed\n", raw_smp_processor_id());
}
...
#if 1
	pr_info("in testcases:MCI_EVENT0_CNT=%x,MCI_EVENT1_CNT:%x\n",
		*((unsigned int *)MCI_EVENT0_CNT), *((unsigned int *)MCI_EVENT1_CNT));
...
static int make_cache_dirty(unsigned int dirty_cache_addr)
{
	...
	pr_info("dirty_cache_addr:%x,&src_array_v[0]=%x\n", dirty_cache_addr, &src_array_v[0]);
	pr_info("[CPU%d]in %s\n", cpu_id, __func__);
...
static int ts_ACELite_MCU_snoop_read_cache_hit(void)
{
	...
	pr_info("[DMA]dst_array_v:%x,src_array_v:%x,dst_array_p:%x,src_array_p:%x, size:%x\n",
		dst_array_v, src_array_v, dst_array_p, src_array_p,
		sizeof(unsigned int) * BUFF_LEN / sizeof(unsigned int));
...
#endif
#if 1
	pr_info("in testcases:MCI_EVENT0_CNT=%x\n", *((unsigned int *)MCI_EVENT0_CNT));
