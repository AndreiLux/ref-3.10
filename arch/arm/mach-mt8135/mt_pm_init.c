...
unsigned int mt_get_cpu_freq(void)
{
	...
	pr_info("ARMPLL_CON1 = 0x%x, TOP_CKMUXSEL = 0x%x, TOP_CKDIV1 = 0x%x\n",
		DRV_Reg32(ARMPLL_CON1), DRV_Reg32(TOP_CKMUXSEL), DRV_Reg32(TOP_CKDIV1));

	/* wait frequency meter finish */
	while (DRV_Reg32(CLK26CALI) & 0x1) {
		pr_info("wait for frequency meter finish, CLK26CALI = 0x%x\n", DRV_Reg32(CLK26CALI));
	...
	pr_info("counter = 0x%x, cpu frequency = %d Khz\n", temp, output);
