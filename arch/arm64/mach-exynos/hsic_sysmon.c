				 ...
				 enum hsic_sysmon_op op)
{
	...
	pr_debug("%s: id:%d, data len:%ld, timeout:%d", opstr, id, len, timeout);
	...
	if (!len) {
		pr_err("length(%ld) must be greater than 0", len);
		return -EINVAL;
	}
	...
	_actual_len = (int) *actual_len;
	ret = usb_bulk_msg(hs->udev, hs->pipe[op], data, len, &_actual_len,
				timeout);
	...
	if(!strncmp(in_shutdown, "system:shutdown",sizeof("system:shutdown"))) {
		...
		pm_runtime_forbid(&hs->udev->dev);		/* usb 1-2 */
		pm_runtime_forbid(hs->udev->dev.parent);	/* usb1 */
	}
	else
