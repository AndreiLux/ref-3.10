...
// 	Do not make locking state at misc_open flow
...
static void __subsystem_restart_dev(struct subsys_device *dev)
{
	...
	if (track->p_state != SUBSYS_CRASHED) {
		if (dev->track.state == SUBSYS_ONLINE &&
			...
			panic("CP Crash - Subsystem %s crashed during SSR!", name);
		}
	}
	spin_unlock_irqrestore(&track->s_lock, flags);
}
...
int subsystem_restart_dev(struct subsys_device *dev)
{
	...
	case RESET_SUBSYS_COUPLED:
		...
		break;
	case RESET_SOC:
		panic("CP Crash - please soft reset and collect cp dump - %s crashed.", name);
		break;
	default:
		panic("CP Crash - subsys-restart: Unknown restart level!\n");
		break;
	}
...
static long subsys_device_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	void *retval;
	int ret;
	...
	switch (cmd) {
		case SUBSYS_POWERUP:
			pr_info("%s CMD SUBSYS_POWERUP\n",__func__);
			ret = subsys_start(subsys_dev);
			if (ret) {
				retval = ERR_PTR(ret);
				return -1;
			}
...
static const struct file_operations subsys_device_fops = {
		.owner = THIS_MODULE,
		.unlocked_ioctl = subsys_device_ioctl,
